...
**Soft continuum high-water:** CREATE-ONLY libcgj graph parent wire **advancing toward 24800** soft only (honest `makefile_max` is a Makefile scan — verify with `./scripts/gj-continuum-makefile-snippet.sh --max`; may still report **24700** until parent wires (scan is source of truth; greppable **24800** when wired)). Wave 106 soft deepen surfaces **retquarticangle**/**retquinticangle** (CREATE-ONLY soft only). Product lamps remain **0**. Soft continuum **≠ bar3** and **≠ product complete**.
