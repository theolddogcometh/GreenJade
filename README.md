...
**Soft continuum high-water:** CREATE-ONLY libcgj graph parent wire **advancing toward 20300** soft only (honest `makefile_max` is a Makefile scan — verify with `./scripts/gj-continuum-makefile-snippet.sh --max`; may still report **20200** until parent wires (scan is source of truth; greppable **20300** when wired)). Wave 61 soft deepen surfaces **retcounterguard**/**retcoveredface** (CREATE-ONLY soft only). Product lamps remain **0**. Soft continuum **≠ bar3** and **≠ product complete**.
