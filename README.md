...
**Soft continuum high-water:** CREATE-ONLY libcgj graph parent wire **advancing toward 22400** soft only (honest `makefile_max` is a Makefile scan — verify with `./scripts/gj-continuum-makefile-snippet.sh --max`; may still report **22300** until parent wires (scan is source of truth; greppable **22400** when wired)). Wave 82 soft deepen surfaces **retmantleangle**/**retaegisangle** (CREATE-ONLY soft only). Product lamps remain **0**. Soft continuum **≠ bar3** and **≠ product complete**.
