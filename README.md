...
**Soft continuum high-water:** CREATE-ONLY libcgj graph parent wire **advancing toward 18800** soft only (honest `makefile_max` is a Makefile scan — verify with `./scripts/gj-continuum-makefile-snippet.sh --max`; may still report **18700** until parent wires (scan is source of truth; greppable **18800** when wired)). Wave 46 soft deepen surfaces **retbailey**/**retpostern** (CREATE-ONLY soft only). Product lamps remain **0**. Soft continuum **≠ bar3** and **≠ product complete**.
