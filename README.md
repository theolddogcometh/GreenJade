...
**Soft continuum high-water:** CREATE-ONLY libcgj graph parent wire **advancing toward 19200** soft only (honest `makefile_max` is a Makefile scan — verify with `./scripts/gj-continuum-makefile-snippet.sh --max`; may still report **19100** until parent wires (scan is source of truth; greppable **19200** when wired)). Wave 50 soft deepen surfaces **retsally**/**retcounterscarp** (CREATE-ONLY soft only). Product lamps remain **0**. Soft continuum **≠ bar3** and **≠ product complete**.
