...
**Soft continuum high-water:** CREATE-ONLY libcgj graph parent wire **advancing toward 25700** soft only (honest `makefile_max` is a Makefile scan — verify with `./scripts/gj-continuum-makefile-snippet.sh --max`; may still report **25600** until parent wires (scan is source of truth; greppable **25700** when wired)). Wave 115 soft deepen surfaces **retpresentangle**/**retvsyncangle** (CREATE-ONLY soft only). Product lamps remain **0**. Soft continuum **≠ bar3** and **≠ product complete**.
