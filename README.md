...
**Soft continuum high-water:** CREATE-ONLY libcgj graph parent wire **advancing toward 26100** soft only (honest `makefile_max` is a Makefile scan — verify with `./scripts/gj-continuum-makefile-snippet.sh --max`; may still report **26000** until parent wires (scan is source of truth; greppable **26100** when wired)). Wave 119 soft deepen surfaces **retqueueangle**/**reteventangle** (CREATE-ONLY soft only). Product lamps remain **0**. Soft continuum **≠ bar3** and **≠ product complete**.
