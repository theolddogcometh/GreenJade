...
**Soft continuum high-water:** CREATE-ONLY libcgj graph parent wire **advancing toward 21500** soft only (honest `makefile_max` is a Makefile scan — verify with `./scripts/gj-continuum-makefile-snippet.sh --max`; may still report **21400** until parent wires (scan is source of truth; greppable **21500** when wired)). Wave 73 soft deepen surfaces **retcurtainface**/**retbastionangle** (CREATE-ONLY soft only). Product lamps remain **0**. Soft continuum **≠ bar3** and **≠ product complete**.
