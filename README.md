...
**Soft continuum high-water:** CREATE-ONLY libcgj graph parent wire **advancing toward 17700** soft only (honest `makefile_max` is a Makefile scan — verify with `./scripts/gj-continuum-makefile-snippet.sh --max`; may still report **17600** until parent wires the next decade). Wave 35 soft deepen surfaces **retfortress**/**retpalace** (CREATE-ONLY soft only). Product lamps remain **0**. Soft continuum **≠ bar3** and **≠ product complete**.
