...
**Soft continuum high-water:** CREATE-ONLY libcgj graph parent wire **advancing toward 24200** soft only (honest `makefile_max` is a Makefile scan — verify with `./scripts/gj-continuum-makefile-snippet.sh --max`; may still report **24100** until parent wires (scan is source of truth; greppable **24100** when wired)). Wave 100 soft deepen surfaces **retspiralangle**/**rethelixangle** (CREATE-ONLY soft only). Product lamps remain **0**. Soft continuum **≠ bar3** and **≠ product complete**.
