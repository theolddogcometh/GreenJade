...
**Soft continuum high-water:** CREATE-ONLY libcgj graph parent wire **advancing toward 20200** soft only (honest `makefile_max` is a Makefile scan — verify with `./scripts/gj-continuum-makefile-snippet.sh --max`; may still report **20100** until parent wires (scan is source of truth; greppable **20200** when wired)). Wave 60 soft deepen surfaces **retplace**/**retenvelope** (CREATE-ONLY soft only). Product lamps remain **0**. Soft continuum **≠ bar3** and **≠ product complete**.
