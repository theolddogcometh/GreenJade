...
**Soft continuum high-water:** CREATE-ONLY libcgj graph parent wire **advancing toward 25000** soft only (honest `makefile_max` is a Makefile scan — verify with `./scripts/gj-continuum-makefile-snippet.sh --max`; may still report **24900** until parent wires (scan is source of truth; greppable **25000** when wired)). Wave 108 soft deepen surfaces **rethurmitangle**/**retcatmullangle** (CREATE-ONLY soft only). Product lamps remain **0**. Soft continuum **≠ bar3** and **≠ product complete**.
