...
**Soft continuum high-water:** CREATE-ONLY libcgj graph parent wire **advancing toward 18200** soft only (honest `makefile_max` is a Makefile scan — verify with `./scripts/gj-continuum-makefile-snippet.sh --max`; may still report **18100** until parent wires (scan is source of truth)). Wave 40 soft deepen surfaces **retcurtain**/**retparapet** (CREATE-ONLY soft only). Product lamps remain **0**. Soft continuum **≠ bar3** and **≠ product complete**.
