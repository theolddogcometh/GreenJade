...
**Soft continuum high-water:** CREATE-ONLY libcgj graph parent wire **advancing toward 24500** soft only (honest `makefile_max` is a Makefile scan — verify with `./scripts/gj-continuum-makefile-snippet.sh --max`; may still report **24400** until parent wires (scan is source of truth; greppable **24500** when wired)). Wave 103 soft deepen surfaces **retprojectangle**/**retaffineangle** (CREATE-ONLY soft only). Product lamps remain **0**. Soft continuum **≠ bar3** and **≠ product complete**.
