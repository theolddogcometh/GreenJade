...
**Soft continuum high-water:** CREATE-ONLY libcgj graph parent wire **advancing toward 22000** soft only (honest `makefile_max` is a Makefile scan — verify with `./scripts/gj-continuum-makefile-snippet.sh --max`; may still report **21900** until parent wires (scan is source of truth; greppable **22000** when wired)). Wave 78 soft deepen surfaces **retpalaceangle**/**retfortressangle** (CREATE-ONLY soft only). Product lamps remain **0**. Soft continuum **≠ bar3** and **≠ product complete**.
