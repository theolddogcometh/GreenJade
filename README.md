...
**Soft continuum high-water:** CREATE-ONLY libcgj graph parent wire **advancing toward 25400** soft only (honest `makefile_max` is a Makefile scan — verify with `./scripts/gj-continuum-makefile-snippet.sh --max`; may still report **25300** until parent wires (scan is source of truth; greppable **25400** when wired)). Wave 112 soft deepen surfaces **retfragmentangle**/**retvertexangle** (CREATE-ONLY soft only). Product lamps remain **0**. Soft continuum **≠ bar3** and **≠ product complete**.
