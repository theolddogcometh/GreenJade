...
**Soft continuum high-water:** CREATE-ONLY libcgj graph parent wire **advancing toward 15600** soft only (honest `makefile_max` is a Makefile scan — verify with `./scripts/gj-continuum-makefile-snippet.sh --max`; may still report prior tip until the next decade is wired). Product lamps remain **0**. Soft continuum **≠ bar3** and **≠ product complete**.
