...
**Soft continuum high-water:** CREATE-ONLY libcgj graph parent wire **advancing toward 18600** soft only (honest `makefile_max` is a Makefile scan — verify with `./scripts/gj-continuum-makefile-snippet.sh --max`; may still report **18500** until parent wires (scan is source of truth; greppable **18600** when wired)). Wave 44 soft deepen surfaces **retmerlon**/**retembrasure** (CREATE-ONLY soft only). Product lamps remain **0**. Soft continuum **≠ bar3** and **≠ product complete**.
