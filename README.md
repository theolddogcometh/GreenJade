...
**Soft continuum high-water:** CREATE-ONLY libcgj graph parent wire **advancing toward 21300** soft only (honest `makefile_max` is a Makefile scan — verify with `./scripts/gj-continuum-makefile-snippet.sh --max`; may still report **21200** until parent wires (scan is source of truth; greppable **21300** when wired)). Wave 71 soft deepen surfaces **retsallyportangle**/**retreentrantangle** (CREATE-ONLY soft only). Product lamps remain **0**. Soft continuum **≠ bar3** and **≠ product complete**.
