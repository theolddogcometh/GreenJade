...
**Soft continuum high-water:** CREATE-ONLY libcgj graph parent wire **advancing toward 21900** soft only (honest `makefile_max` is a Makefile scan — verify with `./scripts/gj-continuum-makefile-snippet.sh --max`; may still report **21800** until parent wires (scan is source of truth; greppable **21900** when wired)). Wave 77 soft deepen surfaces **retspireangle**/**retholdangle** (CREATE-ONLY soft only). Product lamps remain **0**. Soft continuum **≠ bar3** and **≠ product complete**.
