...
**Soft continuum high-water:** CREATE-ONLY libcgj graph parent wire **advancing toward 23900** soft only (honest `makefile_max` is a Makefile scan — verify with `./scripts/gj-continuum-makefile-snippet.sh --max`; may still report **23800** until parent wires (scan is source of truth; greppable **23900** when wired)). Wave 97 soft deepen surfaces **retradiusangle**/**retdiameterangle** (CREATE-ONLY soft only). Product lamps remain **0**. Soft continuum **≠ bar3** and **≠ product complete**.
