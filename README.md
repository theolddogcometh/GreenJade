...
**Soft continuum high-water:** CREATE-ONLY libcgj graph parent wire **advancing toward 18900** soft only (honest `makefile_max` is a Makefile scan — verify with `./scripts/gj-continuum-makefile-snippet.sh --max`; may still report **18800** until parent wires (scan is source of truth; greppable **18900** when wired)). Wave 47 soft deepen surfaces **retinnerward**/**retdonjon** (CREATE-ONLY soft only). Product lamps remain **0**. Soft continuum **≠ bar3** and **≠ product complete**.
