...
**Soft continuum high-water:** CREATE-ONLY libcgj graph parent wire **advancing toward 20600** soft only (honest `makefile_max` is a Makefile scan — verify with `./scripts/gj-continuum-makefile-snippet.sh --max`; may still report **20500** until parent wires (scan is source of truth; greppable **20600** when wired)). Wave 64 soft deepen surfaces **retreentrant**/**retsallyport** (CREATE-ONLY soft only). Product lamps remain **0**. Soft continuum **≠ bar3** and **≠ product complete**.
