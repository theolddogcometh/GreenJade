...
**Soft continuum high-water:** CREATE-ONLY libcgj graph parent wire **advancing toward 18100** soft only (honest `makefile_max` is a Makefile scan — verify with `./scripts/gj-continuum-makefile-snippet.sh --max`; may still report **18000** until parent wires (scan is source of truth)). Wave 39 soft deepen surfaces **retbarbican**/**retglacis** (CREATE-ONLY soft only). Product lamps remain **0**. Soft continuum **≠ bar3** and **≠ product complete**.
