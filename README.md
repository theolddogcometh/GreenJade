...
**Soft continuum high-water:** CREATE-ONLY libcgj graph parent wire **advancing toward 19400** soft only (honest `makefile_max` is a Makefile scan — verify with `./scripts/gj-continuum-makefile-snippet.sh --max`; may still report **19300** until parent wires (scan is source of truth; greppable **19400** when wired)). Wave 52 soft deepen surfaces **rettenaille**/**retdemilune** (CREATE-ONLY soft only). Product lamps remain **0**. Soft continuum **≠ bar3** and **≠ product complete**.
