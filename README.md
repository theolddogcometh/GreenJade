...
**Soft continuum high-water:** CREATE-ONLY libcgj graph parent wire **advancing toward 24900** soft only (honest `makefile_max` is a Makefile scan — verify with `./scripts/gj-continuum-makefile-snippet.sh --max`; may still report **24800** until parent wires (scan is source of truth; greppable **24900** when wired)). Wave 107 soft deepen surfaces **retsplineangle**/**retbezierangle** (CREATE-ONLY soft only). Product lamps remain **0**. Soft continuum **≠ bar3** and **≠ product complete**.
