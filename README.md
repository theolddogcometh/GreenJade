...
**Soft continuum high-water:** CREATE-ONLY libcgj graph parent wire **advancing toward 19100** soft only (honest `makefile_max` is a Makefile scan — verify with `./scripts/gj-continuum-makefile-snippet.sh --max`; may still report **19000** until parent wires (scan is source of truth; greppable **19100** when wired)). Wave 49 soft deepen surfaces **retglacisgate**/**retoutwork** (CREATE-ONLY soft only). Product lamps remain **0**. Soft continuum **≠ bar3** and **≠ product complete**.
