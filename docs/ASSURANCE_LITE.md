...
Soft≠product: `assurance-check` **PASS** ≠ Dual DoD **A/B** close (still needs stamped flash + host arping/ping/`nc`). **L1 alone never closes Dual DoD.**
...
| **Freestanding SKIP deepen** | Missing `#ifndef` guards; missing class-driver source gates (`rtl8168.c` / `xhci_msc.c` SKIP lamps); thin `net_l2` / `main.c` SKIP holds | SKIP = process law, not Dual DoD close |
| **freestanding_no_exec** | `GJ_SOFT_MODULE_RUN_INIT` default ≠ **0** or missing G-AC-1 honesty in `linux_module.c` | Soft SKIP `.ko` init in kernel; eng residual |
...
| **Soft≠product / L1 honesty** | Docs/script claim Dual DoD close from L1, or drop L3-required close rule | **PASS ≠ Dual DoD close** |
| **Stamp / dual license** | Missing `GJ_IMAGE_VERSION` or thin dual MIT/Apache greps | Identity only; not product PASS |
| **Flash bar** | Stamp shape not `YYYY.MM.DD.N` (e.g. lab bar `v2026.08.04.73`) | **Test what you fly**; L3 needs this stamp on media |
...
| **T0 product net** | **virtio-net** until UDX owns wire. |
| **Product drivers (hosts)** | Userspace **UDX/DDI** (`rtl8168_udx` / `xhci_udx` …) over hot+cold ABI. |
| **Lab dual DoD B** | Freestanding `rtl8168` + stack interim for arping/ping/:22 — **L3 host** close only. |
...
| **C1** | Lab dual DoD | Freestanding rtl wire, sshd :22 lab, freestanding MSC optional | **DUT + host** proof on a **named image stamp** | Soft inventory alone · STATUS “:22” without host `nc` |
...
| Lab DoD **B** | Host `arping` / `ping` / `nc -v … 22` against **stamped guest** | **Required independent** |
...
**Rules:**
1. Dual DoD evidence **must name the stamp** (e.g. lab flash bar `v2026.08.04.73`).
...
| Independent DoD | Host probes close **B**, not STATUS alone |
...
| **L3** | **Deliverable object code** | Does the **binary we ship/flash** behave correctly? | Stamped `greenjade.elf` on DUT; host arping/ping/`nc`; FAULT PINNED absence on that cut |

**Close Dual DoD B only with L3 evidence** (plus L1 that the probes are the right ones). L2 alone is never enough.
...
| Practice | Detail |
|----------|--------|
| **Prove the artifact** | C1 claims use **flashed** stamp + host/DUT behavior |
...
| **Not a substitute** | Coverage % never closes Dual DoD B without host L3 probes |
...
### 5.1 Dual DoD **B** (lab freestanding net) — template

| ID | Layer | Statement |
|----|-------|-----------|
| B-HLR-1 | High-level | Lab host can reach freestanding guest at **10.200.125.50** |
| B-HLR-2 | High-level | Lab host can open TCP **:22** (sshd path) on that guest |
| B-LLR-1 | Low-level | Freestanding rtl RX delivers frames under `scheduler_run` poll (R climbs) |
| B-LLR-2 | Low-level | ARP request for guest IP gets freestanding reply |
| B-LLR-3 | Low-level | ICMP echo request gets freestanding reply |
| B-LLR-4 | Low-level | TCP demux accepts dest lab IP; listen :22 after L2 ready |
| B-CODE | Code (examples) | `kernel/drv/rtl8168.c` · `kernel/drv/net_l2.c` · `kernel/net/net_eth.c` · `kernel/net/net_tcp.c` · `kernel/sched/thread.c` |
| B-V&V-1 | Verify (L3) | STATUS: no FAULT PINNED; NET UP; R > 0 under host flood on **stamped** image |
...
| B-V&V-4 | Verify (L3) | Host: `nc -v -w 3 10.200.125.50 22` connects / banner |

**Close rule:** B-HLR-1 and B-HLR-2 only after B-V&V-\* **L3** on a **named** `GJ_IMAGE_VERSION`. Soft lamps (L2 eng) alone **OPEN**.
...
| V4 | Net identity | Freestanding NET shows lab **10.200.125.50** when rtl owns wire | STATUS NET line |
| V5 | RX live under host traffic | Freestanding **R** (and L2 BR RX) climbs during arping/ping | STATUS during host flood |
| V6 | L2/L3 reachability | Host arping and/or ping succeed | Host command output |
| V7 | :22 reachability | Host `nc`/`ssh` to :22 succeeds | Host command output |
...
**Current dual DoD B:** V1–V7 and V9 required to **close**; V8 always on.  
...
| H6 | STATUS :22 without host | C1 | Close DoD B from panel alone | Host `nc` independent (L3) | Dual DoD B table |
| H7 | Tree ≠ media | C1 | Verify DoD on unflashed residual | Stamp + re-image for DoD cuts; **test what you fly** | `GJ_IMAGE_VERSION` · V9 |
| H8 | Source-only trust | C1/C3 | “Compiled clean / looks right” as DoD close | L3 host/DUT on stamped binary | §4 three layers |
...
| C1 Dual DoD B code | Author / agents on rtl·l2·eth·tcp | Host arping/ping/nc on **stamped flash** (L3) |
...
| Agents ≠ L3 | Agent “PASS” or soft inventory never closes Dual DoD; host/DUT does |
...
- STATUS :22 claimed while host `nc` times out → do not close B (H6); need R climb (H4/H8).
...
- [ ] V6–V7 (host arping/ping/nc) recorded against **that** media  
...
|----------------------|---------------|
| If it has not been tested, it does not work | C1 close needs L3 probes, not soft lamps |
...
**Image identity:** `make image-version` · `scripts/gj-image-version.sh --report` (stamp ≠ product PASS; lab flash bar e.g. `v2026.08.04.73`).
