...
| **Soft continuum / soft ship gates** | CREATE-ONLY graph high-water / greppable kernel smokes / media inventory — bring-up honesty only (companion docs). Parent-wire high-water **advancing toward 24300** soft only; honest `makefile_max` is a Makefile scan (may still report prior tip, e.g. **24200**, until next decade is wired). |
