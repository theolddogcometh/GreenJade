...
## Progress note — GreenJade Wave 24 soft continuum (2026-07-23)

**Additive only (Wave 24 exclusive for this file).** Prior progress rows and Wave 10/12/14/16/17 soft stamps above stay historical ledger (Wave 17 high-water was **toward 15900** / scan tip **15900** when wired). This section is Wave 24 honesty: soft continuum deepen toward the next parent-wire high-water. It does **not** re-litigate M0–product DoD, close any product bar, or promote Deck Top 50 rows.

### Soft stamp (Wave 24)

| Field | Value |
|-------|--------|
| **Wave** | GreenJade Wave 24 (soft continuum honesty) |
| **Continuum** | makefile target high-water **advancing toward 16600** — **soft only** (parent wires / CREATE-ONLY graph; honest scan may still show prior tip **makefile_max=16500**; **do not hardcode false 16600**) |
| **bar3** | **OPEN** |
| **Matrix** | **NOT-TRIED × 50** (no titles tried) |
| **Product lamps** | **0** (soft stubs stay zero by design) |

**Soft stamp (Wave 24):** continuum high-water **advancing toward 16600** soft only (parent wires). **Do not hardcode false `makefile_max=16600`** if `./scripts/gj-continuum-makefile-snippet.sh --max` still reports **16500** until parent wires. **Soft ≠ product complete.** Soft continuum ≠ runtime ABI product ≠ Steam **client** launch. Product lamps remain **0**. **bar3: OPEN.** Matrix **NOT-TRIED × 50**. **No bar3 claim.** multi_server=0 confine=0 lamps 0.

### Soft continuum ≠ bar3 (Wave 24 honesty)

| Soft surface | What it is | What it is **not** |
|--------------|------------|--------------------|
| Continuum **toward 16600** | CREATE-ONLY soft graph high-water advance (libcgj parent wire) | Runtime ABI product; Steam client; Deck Top 50; bar3 closed |
| Honest scan **makefile_max=16500** (when still tip) | Makefile wire truth until 16600 basenames land | Claim that 16600 is already greppable |
| Soft deepen | Honesty / bring-up surface deepen | Product complete; product lamps lit |
| Host media `STATUS=READY` / `steam-bar3-check` | Bootstrap inventory on media | Client launch; title try |
| Product lamps **0** | Soft score / bar3-ready stubs remain zero | Any product-score claim |

**Hard rule:** never promote continuum advancing toward **16600**, media READY, soft deepen, or soft ship gates to “bar3 closed,” Deck Top 50 `PASS`, product lamps > 0, or product DoD complete. Soft continuum ≠ bar3. **Soft ≠ product complete.** Never hardcode a false `makefile_max` higher than the Makefile scan.

### Explicit non-claims (Wave 24)

| Claim | Allowed? |
|-------|----------|
| “Continuum high-water advancing toward **16600** (parent wire soft)” | **Yes** — soft only |
| “Honest scan still `makefile_max=16500` until parent wires 16600” | **Yes** — scan is source of truth |
| “Soft continuum toward 16600 = Steam client / Top-50 / product score” | **No** |
| “Product lamps lit / product complete from Wave 24 soft deepen” | **No** — lamps remain **0** |
| “bar3 closed by continuum toward 16600 / media READY / kernel soft gates” | **No** |
| “Deck Top 50 titles tried / PASS from this stamp” | **No** — matrix stays **NOT-TRIED** |
| Any **bar3** closed claim from IMPLEMENTATION soft stamp alone | **No** |

### Open (unchanged by Wave 24)

- **bar3** — Steam **client** launch on DUT + Deck Top 50 leave `NOT-TRIED` — **OPEN**
- Deck Top 50 title runs — all **NOT-TRIED** (claim **targeting only**)
- Product RAM **≥ 1 TiB** full path — **open** when host allows (768G soak ≠ TiB bar)
- Soft continuum growth toward **16600** — **soft only**; does **not** close bar3; product lamps stay **0**

**Bar3 remains OPEN.** Wave 24 honesty: continuum **advancing toward 16600 soft only**; scan may still be **16500**; soft deepen ≠ product complete; product lamps **0**; soft continuum ≠ bar3; **no false PASS**.
...
*Wave 24 soft continuum (2026-07-23): high-water **advancing toward 16600 soft only**; honest scan may still be **16500**; product lamps **0**; **soft ≠ product complete**; **bar3 remains OPEN**.*
