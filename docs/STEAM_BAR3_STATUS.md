...
| **Soft continuum** | **≠ bar3** (CREATE-ONLY graph wire; `makefile_max=15500` soft only; not client run) |
...
**Media READY ≠ client run ≠ Top-50.** Soft continuum (`makefile_max=15500`, parallel waves) **≠ bar3**.
...
| Continuum soft graph | **soft** | parallel waves → **makefile_max=15500** soft only; CREATE-ONLY soft gates only; **soft continuum ≠ bar3**; **not** client run |
