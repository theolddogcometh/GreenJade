...
| **MEM_PLACE / channel striping** | [MEM_PLACE_CHANNEL.md](MEM_PLACE_CHANNEL.md) — Option A soft residual landed; product L2 **OPEN** |
...
| Rule | Meaning |
|------|---------|
| **G-AC-1** | **No Linux `.ko` binary runs in the kernel** as product (no `init_module`-style product AC, no GPL in-tree). |
...
### Dual DoD (retargeted — freestanding net out)
...
| Item | Note |
|------|------|
| **TX short-frame BM/slot** | Freestanding TX short-frame BM/slot path in cut |
| **ICMP multi-try** | Multi-try ICMP for re-verify after flash (≠ continuous product ping) |
| **soft listen :22** | Soft listen on **:22** — **not** host banner; **not** Dual DoD **B** close (DoD **B** = UDX NIC). Dual DoD **A/B** still **OPEN** |
...
- [x] **MEM_PLACE soft residual (Option A):** CREATE_PLACED 104 / MAP_REPLICA 105 + L0 soft diversify — [MEM_PLACE_CHANNEL.md](MEM_PLACE_CHANNEL.md); Soft≠product
- [ ] **MEM_PLACE product L2** — full DRAM / IMC channel-class map + real multi-channel hedge guarantee (**OPEN**; soft L0 ≠ product)
...
| 2026-08-04 | **Operator docs honesty:** dual DoD **A** Linux USB **OPEN**, **B** sshd **:22 OPEN** until DUT proof. Net: freestanding **ICMP proven earlier**; **recent boots R0/R1 regression**; hybrid **SOFT gate0**. Image stamp **STATUS (static) vYYYY.MM.DD.N** via `GJ_IMAGE_VERSION`. No test-panel photo IDs. Soft ≠ product. **G-AC-1.** Dual MIT/Apache. Bar3 only [STEAM_BAR3_STATUS.md](STEAM_BAR3_STATUS.md). |
| 2026-08-04 | **Dual DoD code land:** freestanding **ICMP proven earlier**; VT-d all-bus identity + 8168G RX; hybrid SOFT (gate0 no REAL BAR); **sshd** eth SYN-ACK rtx + accept-every-yield + eth session (DUT verify **OPEN**); **USB** stage-15 EP0 deepen + `linux_usb_soft` seed; public docs without test-panel photo IDs. Dual DoD **A/B still OPEN** until DUT flash proves stick + `:22`. Soft ≠ product. **G-AC-1.** Dual MIT/Apache. Bar3 only [STEAM_BAR3_STATUS.md](STEAM_BAR3_STATUS.md). |
...
| 2026-07-24 | **Wave 122 soft continuum (honesty only):** makefile target high-water **advancing toward 26400** (parent wires / CREATE-ONLY graph); soft deepen surfaces **retframeangle**/**retwindowangle** (CREATE-ONLY soft only); honest scan may still report **makefile_max=26300** until parent wires — **do not hardcode false max**. **Soft ≠ product complete.** Product lamps remain **0**. Top 50 **NOT-TRIED × 50**. **bar3 still open**. |
| 2026-07-24 | **Wave 121 soft continuum (honesty only):** makefile target high-water **advancing toward 26300** (parent wires / CREATE-ONLY graph); soft deepen surfaces **retstreamangle**/**retpacketangle** (CREATE-ONLY soft only); honest scan may still report **makefile_max=26200** until parent wires — **do not hardcode false max**. **Soft ≠ product complete.** Product lamps remain **0**. Top 50 **NOT-TRIED × 50**. **bar3 still open**. |
| 2026-07-24 | **Wave 120 soft continuum (honesty only):** makefile target high-water **advancing toward 26200** (parent wires / CREATE-ONLY graph); soft deepen surfaces **retchannelangle**/**retmailboxangle** (CREATE-ONLY soft only); honest scan may still report **makefile_max=26100** until parent wires — **do not hardcode false max**. **Soft ≠ product complete.** Product lamps remain **0**. Top 50 **NOT-TRIED × 50**. **bar3 still open**. |
| 2026-07-24 | **Wave 119 soft continuum (honesty only):** makefile target high-water **advancing toward 26100** (parent wires / CREATE-ONLY graph); soft deepen surfaces **retqueueangle**/**reteventangle** (CREATE-ONLY soft only); honest scan may still report **makefile_max=26000** until parent wires — **do not hardcode false max**. **Soft ≠ product complete.** Product lamps remain **0**. Top 50 **NOT-TRIED × 50**. **bar3 still open**. |
| 2026-07-24 | **Wave 118 soft continuum (honesty only):** makefile target high-water **advancing toward 26000** (parent wires / CREATE-ONLY graph); soft deepen surfaces **retbarrierangle**/**retatomicangle** (CREATE-ONLY soft only); honest scan may still report **makefile_max=25900** until parent wires — **do not hardcode false max**. **Soft ≠ product complete.** Product lamps remain **0**. Top 50 **NOT-TRIED × 50**. **bar3 still open**. |
| 2026-07-24 | **Wave 117 soft continuum (honesty only):** makefile target high-water **advancing toward 25900** (parent wires / CREATE-ONLY graph); soft deepen surfaces **retmutexangle**/**retcondangle** (CREATE-ONLY soft only); honest scan may still report **makefile_max=25800** until parent wires — **do not hardcode false max**. **Soft ≠ product complete.** Product lamps remain **0**. Top 50 **NOT-TRIED × 50**. **bar3 still open**. |
| 2026-07-24 | **Wave 116 soft continuum (honesty only):** makefile target high-water **advancing toward 25800** (parent wires / CREATE-ONLY graph); soft deepen surfaces **retfenceangle**/**retsemaphoreangle** (CREATE-ONLY soft only); honest scan may still report **makefile_max=25700** until parent wires — **do not hardcode false max**. **Soft ≠ product complete.** Product lamps remain **0**. Top 50 **NOT-TRIED × 50**. **bar3 still open**. |
| 2026-07-24 | **Wave 115 soft continuum (honesty only):** makefile target high-water **advancing toward 25700** (parent wires / CREATE-ONLY graph); soft deepen surfaces **retpresentangle**/**retvsyncangle** (CREATE-ONLY soft only); honest scan may still report **makefile_max=25600** until parent wires — **do not hardcode false max**. **Soft ≠ product complete.** Product lamps remain **0**. Top 50 **NOT-TRIED × 50**. **bar3 still open**. |
| 2026-07-24 | **Wave 114 soft continuum (honesty only):** makefile target high-water **advancing toward 25600** (parent wires / CREATE-ONLY graph); soft deepen surfaces **retframebufferangle**/**retswapchainangle** (CREATE-ONLY soft only); honest scan may still report **makefile_max=25500** until parent wires — **do not hardcode false max**. **Soft ≠ product complete.** Product lamps remain **0**. Top 50 **NOT-TRIED × 50**. **bar3 still open**. |
| 2026-07-24 | **Wave 112 soft continuum (honesty only):** makefile target high-water **advancing toward 25500** (parent wires / CREATE-ONLY graph); soft deepen surfaces **retfragmentangle**/**retvertexangle** (CREATE-ONLY soft only); honest scan may still report **makefile_max=25300** until parent wires — **do not hardcode false max**. **Soft ≠ product complete.** Product lamps remain **0**. Top 50 **NOT-TRIED × 50**. **bar3 still open**. |
| 2026-07-24 | **Wave 111 soft continuum (honesty only):** makefile target high-water **advancing toward 25300** (parent wires / CREATE-ONLY graph); soft deepen surfaces **retvoxelangle**/**rettexelangle** (CREATE-ONLY soft only); honest scan may still report **makefile_max=25200** until parent wires — **do not hardcode false max**. **Soft ≠ product complete.** Product lamps remain **0**. Top 50 **NOT-TRIED × 50**. **bar3 still open**. |
| 2026-07-24 | **Wave 86 soft continuum (honesty only):** makefile target high-water **advancing toward 22800** (parent wires / CREATE-ONLY graph); soft deepen surfaces **retforgeangle**/**retprismangle** (CREATE-ONLY soft only); honest scan may still report **makefile_max=22700** until parent wires — **do not hardcode false max**. **Soft ≠ product complete.** Product lamps remain **0**. Top 50 **NOT-TRIED × 50**. **bar3 still open**. |
| 2026-07-24 | **Wave 85 soft continuum (honesty only):** makefile target high-water **advancing toward 22700** (parent wires / CREATE-ONLY graph); soft deepen surfaces **retcrownangle**/**retshardangle** (CREATE-ONLY soft only); honest scan may still report **makefile_max=22600** until parent wires — **do not hardcode false max**. **Soft ≠ product complete.** Product lamps remain **0**. Top 50 **NOT-TRIED × 50**. **bar3 still open**. |
| 2026-07-24 | **Wave 83 soft continuum (honesty only):** makefile target high-water **advancing toward 22500** (parent wires / CREATE-ONLY graph); soft deepen surfaces **retemblemangle**/**retsigilangle** (CREATE-ONLY soft only); honest scan may still report **makefile_max=22400** until parent wires — **do not hardcode false max**. **Soft ≠ product complete.** Product lamps remain **0**. Top 50 **NOT-TRIED × 50**. **bar3 still open**. |

| 2026-07-24 | **Wave 82 soft continuum (honesty only):** makefile target high-water **advancing toward 22400** (parent wires / CREATE-ONLY graph); soft deepen surfaces **retmantleangle**/**retaegisangle** (CREATE-ONLY soft only); honest scan may still report **makefile_max=22300** until parent wires — **do not hardcode false max**. **Soft ≠ product complete.** Product lamps remain **0**. Top 50 **NOT-TRIED × 50**. **bar3 still open**. |
| 2026-07-24 | **Wave 81 soft continuum (honesty only):** makefile target high-water **advancing toward 22300** (parent wires / CREATE-ONLY graph); soft deepen surfaces **retpanoplyangle**/**retbulwarkangle** (CREATE-ONLY soft only); honest scan may still report **makefile_max=22200** until parent wires — **do not hardcode false max**. **Soft ≠ product complete.** Product lamps remain **0**. Top 50 **NOT-TRIED × 50**. **bar3 still open**. |
| 2026-07-24 | **Wave 80 soft continuum (honesty only):** makefile target high-water **advancing toward 22200** (parent wires / CREATE-ONLY graph); soft deepen surfaces **retcitadelangle**/**retbastionkeep** (CREATE-ONLY soft only); honest scan may still report **makefile_max=22100** until parent wires — **do not hardcode false max**. **Soft ≠ product complete.** Product lamps remain **0**. Top 50 **NOT-TRIED × 50**. **bar3 still open**. |
| 2026-07-24 | **Wave 79 soft continuum (honesty only):** makefile target high-water **advancing toward 22100** (parent wires / CREATE-ONLY graph); soft deepen surfaces **retkeepangle**/**retredoubtangle** (CREATE-ONLY soft only); honest scan may still report **makefile_max=22000** until parent wires — **do not hardcode false max**. **Soft ≠ product complete.** Product lamps remain **0**. Top 50 **NOT-TRIED × 50**. **bar3 still open**. |
| 2026-07-24 | **Wave 78 soft continuum (honesty only):** makefile target high-water **advancing toward 22000** (parent wires / CREATE-ONLY graph); soft deepen surfaces **retpalaceangle**/**retfortressangle** (CREATE-ONLY soft only); honest scan may still report **makefile_max=21900** until parent wires — **do not hardcode false max**. **Soft ≠ product complete.** Product lamps remain **0**. Top 50 **NOT-TRIED × 50**. **bar3 still open**. |
| 2026-07-24 | **Wave 77 soft continuum (honesty only):** makefile target high-water **advancing toward 21900** (parent wires / CREATE-ONLY graph); soft deepen surfaces **retspireangle**/**retholdangle** (CREATE-ONLY soft only); honest scan may still report **makefile_max=21800** until parent wires — **do not hardcode false max**. **Soft ≠ product complete.** Product lamps remain **0**. Top 50 **NOT-TRIED × 50**. **bar3 still open**. |
| 2026-07-24 | **Wave 76 soft continuum (honesty only):** makefile target high-water **advancing toward 21800** (parent wires / CREATE-ONLY graph); soft deepen surfaces **retgateangle**/**retwallangle** (CREATE-ONLY soft only); honest scan may still report **makefile_max=21700** until parent wires — **do not hardcode false max**. **Soft ≠ product complete.** Product lamps remain **0**. Top 50 **NOT-TRIED × 50**. **bar3 still open**. |
| 2026-07-24 | **Wave 75 soft continuum (honesty only):** makefile target high-water **advancing toward 21700** (parent wires / CREATE-ONLY graph); soft deepen surfaces **retmoatangle**/**retowerangle** (CREATE-ONLY soft only); honest scan may still report **makefile_max=21600** until parent wires — **do not hardcode false max**. **Soft ≠ product complete.** Product lamps remain **0**. Top 50 **NOT-TRIED × 50**. **bar3 still open**. |
| 2026-07-24 | **Wave 74 soft continuum (honesty only):** makefile target high-water **advancing toward 21600** (parent wires / CREATE-ONLY graph); soft deepen surfaces **retglacisangle**/**retparapetangle** (CREATE-ONLY soft only); honest scan may still report **makefile_max=21500** until parent wires — **do not hardcode false max**. **Soft ≠ product complete.** Product lamps remain **0**. Top 50 **NOT-TRIED × 50**. **bar3 still open**. |
| 2026-07-24 | **Wave 73 soft continuum (honesty only):** makefile target high-water **advancing toward 21500** (parent wires / CREATE-ONLY graph); soft deepen surfaces **retcurtainface**/**retbastionangle** (CREATE-ONLY soft only); honest scan may still report **makefile_max=21400** until parent wires — **do not hardcode false max**. **Soft ≠ product complete.** Product lamps remain **0**. Top 50 **NOT-TRIED × 50**. **bar3 still open**. |
...
| 2026-07-24 | **Wave 57 soft continuum (honesty only):** makefile target high-water **advancing toward 19900** (parent wires / CREATE-ONLY graph); soft deepen surfaces **retraverse**/**retcasemate** (CREATE-ONLY soft only); honest scan may still report **makefile_max=19800** until parent wires — **do not hardcode false max**. **Soft ≠ product complete.** Product lamps remain **0**. Top 50 **NOT-TRIED × 50**. **bar3 still open**. |

| 2026-07-24 | **Wave 56 soft continuum (honesty only):** makefile target high-water **advancing toward 19800** (parent wires / CREATE-ONLY graph); soft deepen surfaces **retgorge**/**retshoulder** (CREATE-ONLY soft only); honest scan may still report **makefile_max=19700** until parent wires — **do not hardcode false max**. **Soft ≠ product complete.** Product lamps remain **0**. Top 50 **NOT-TRIED × 50**. **bar3 still open**. |

| 2026-07-24 | **Wave 55 soft continuum (honesty only):** makefile target high-water **advancing toward 19700** (parent wires / CREATE-ONLY graph); soft deepen surfaces **retflank**/**retface** (CREATE-ONLY soft only); honest scan may still report **makefile_max=19600** until parent wires — **do not hardcode false max**. **Soft ≠ product complete.** Product lamps remain **0**. Top 50 **NOT-TRIED × 50**. **bar3 still open**. |

| 2026-07-24 | **Wave 54 soft continuum (honesty only):** makefile target high-water **advancing toward 19600** (parent wires / CREATE-ONLY graph); soft deepen surfaces **retcaponier**/**retredan** (CREATE-ONLY soft only); honest scan may still report **makefile_max=19500** until parent wires — **do not hardcode false max**. **Soft ≠ product complete.** Product lamps remain **0**. Top 50 **NOT-TRIED × 50**. **bar3 still open**. |

| 2026-07-23 | **Wave 53 soft continuum (honesty only):** makefile target high-water **advancing toward 19500** (parent wires / CREATE-ONLY graph); soft deepen surfaces **retravelin**/**retlunette** (CREATE-ONLY soft only); honest scan may still report **makefile_max=19400** until parent wires — **do not hardcode false max**. **Soft ≠ product complete.** Product lamps remain **0**. Top 50 **NOT-TRIED × 50**. **bar3 still open**. |
