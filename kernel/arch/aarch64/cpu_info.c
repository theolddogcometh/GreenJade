 ...
 *   aarch64: cpu soft deepen wave=126 areas=… stamp_storm=0
 *   aarch64: cpu soft honesty product_kernel=OPEN soft_only=1
 *   aarch64: cpu soft residual lean … Soft!=product G-AC-1 dual=MIT_OR_Apache-2.0
 *   aarch64: cpu soft exclusive multi_server=0 confine=0 product_kernel=OPEN
 *   aarch64: cpu soft open multi_server=0 confine=0 product_kernel=OPEN
 *   aarch64: cpu soft return inv_ret=… product_kernel=OPEN
 *   aarch64: cpu soft PASS | FAIL
 *
 * C0 residual (stamp-free; this unit only; Soft!=product):
 *   MRS soft probe residual lean only — not product multi-server / confine /
 *   Dual DoD close / bar3. G-AC-1: no GPL Linux cpu_info paste; no .ko AC.
 *   Dual license MIT OR Apache-2.0. product_kernel=OPEN remains OPEN.
 *   Never bumps image version. Never hard-gates aarch64: cpu PASS.
 *
...
/* Soft inventory stamp (file-local; never product gate; wave not product ver). */
#define CPU_SOFT_WAVE 126u
/*
 * Lean area count (C0 residual; stamp-free; Soft!=product):
 * id,midr,mpidr,pfr,isar,mmfr,cache,extra,regs,path,inv,inventory,surf,
 * deepen,honesty,residual,exclusive,open,return,lean = 20.
 * No ret* stamp storm. Never bumps image version.
 */
#define CPU_SOFT_AREAS 20u
...
cpu_soft_inventory(const struct cpu_soft_snap *pSnap)
{
    ...
    /* Grep: aarch64: cpu soft inv */
    aarch64_uart_puts("aarch64: cpu soft inv el1=");
     ...
     * C0 residual lean catalog — stamp-free Soft!=product.
     * residual+lean replace ret* stamp storms (never reshape primary MRS lamps).
     * product_kernel=OPEN; Dual DoD remains OPEN; never hard-gates product PASS.
     */
    ...
    aarch64_uart_puts(" catalog=id,midr,mpidr,pfr,isar,mmfr,cache,extra,"
                      "regs,path,inv,inventory,surf,deepen,honesty,residual,"
                      "exclusive,open,return,lean "
                      "logs=");
    aarch64_uart_put_hex((unsigned long)g_cCpuSoftLogs);
    aarch64_uart_puts(" soft_only=1 stamp_storm=0 version_stamp=0\n");

    /*
     * Grep: aarch64: cpu soft honesty
     * Explicit non-claim: MRS soft inventory != product kernel complete.
     * Dual DoD OPEN; multi_server/confine remain 0.
     */
    aarch64_uart_puts("aarch64: cpu soft honesty product_kernel=OPEN "
                      "soft_only=1 mrs_only=1 no_mmu_claim=1 no_gic_claim=1 "
                      "no_timer_claim=1 multi_server=0 confine=0 "
    ...
    /*
     * Grep: aarch64: cpu soft residual lean
     * C0 residual lean lamp (stamp-free; Soft!=product).
     * G-AC-1: freestanding MRS soft probe only; no GPL Linux cpu_info paste;
     * no .ko product AC. Dual MIT OR Apache-2.0. Never bumps image version.
     * Never hard-gates aarch64: cpu PASS / Dual DoD.
     */
    aarch64_uart_puts("aarch64: cpu soft residual lean ");
    if (fOk != 0) {
        aarch64_uart_puts("PASS");
    } else {
        aarch64_uart_puts("FAIL");
    }
    aarch64_uart_puts(" el1=");
    aarch64_uart_put_hex_n((unsigned long)pSnap->uEl1Ok, 1u);
    aarch64_uart_puts(" el0_a64=");
    ...
    aarch64_uart_puts(" gate_ok=");
    aarch64_uart_put_hex_n((unsigned long)pSnap->uGateOk, 1u);
    aarch64_uart_puts(" mrs=1 mmu=0 gic=0 timer=0 claim=0 "
                      "areas=");
    aarch64_uart_put_hex((unsigned long)CPU_SOFT_AREAS);
    aarch64_uart_puts(" logs=");
    aarch64_uart_put_hex((unsigned long)g_cCpuSoftLogs);
    aarch64_uart_puts(" stamp_storm=0 version_stamp=0 hard_gate=0 "
                      ...
                      "dual_dod=OPEN g_ac1=1 no_gpl=1 no_ko_product_ac=1 "
                      "soft_ne_product=1 dual=MIT_OR_Apache-2.0 unit=cpu_info.c "
                      "wave=");
    aarch64_uart_put_hex((unsigned long)CPU_SOFT_WAVE);
    aarch64_uart_puts(" (Soft!=product; dual MIT OR Apache-2.0; G-AC-1; "
                      "C0 aarch64 cpu residual lean; MRS soft probe only; "
                      "no version stamp; Dual DoD OPEN)\n");

    /* Grep: aarch64: cpu soft exclusive — exclusive residual deepen */
    aarch64_uart_puts("aarch64: cpu soft exclusive multi_server=0 "
                      "confine=0 product_kernel=OPEN soft_only=1 "
                      "residual=1 stamp_storm=0 wave=");
    aarch64_uart_put_hex((unsigned long)CPU_SOFT_WAVE);
    aarch64_uart_puts("\n");

    /* Grep: aarch64: cpu soft open — open-lamp rollup (Dual DoD OPEN) */
    aarch64_uart_puts("aarch64: cpu soft open multi_server=0 confine=0 "
                      "product_kernel=OPEN dual_dod=OPEN soft_only=1 wave=");
    aarch64_uart_put_hex((unsigned long)CPU_SOFT_WAVE);
    aarch64_uart_puts("\n");

    /*
     * Grep: aarch64: cpu soft return
     * inv_ret is soft inventory bool only; product still prints cpu PASS.
