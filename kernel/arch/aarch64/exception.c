 ...
 *   Soft ≠ product exception complete; soft.
 *
 ...
 *            aarch64: exception soft deepen wave=126 …
 *
...
#define EXC_SOFT_WAVE 126u
#define EXC_SOFT_AREAS 216u
...
aarch64_uart_puts("aarch64: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retfosseangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
aarch64_uart_puts("aarch64: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retcounterscarple stamp; Soft≠product)\n");
/*
 * ---- Wave 71 exclusive complementary surfaces (never reshape primary).
 * Return surfaces only — soft inventory; never hard-gates product paths.
 */
/* Grep: aarch64: soft retsallyportangle — Wave 71 return-sallyportangle honesty */
aarch64_uart_puts("aarch64: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retsallyportangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
aarch64_uart_puts("aarch64: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retreentrantangle stamp; Soft≠product)\n");
/*
 * ---- Wave 72 exclusive complementary surfaces (never reshape primary).
 * Return surfaces only — soft inventory; never hard-gates product paths.
 */
/* Grep: aarch64: soft retplaceofarmsangle — Wave 72 return-placeofarmsangle honesty */
aarch64_uart_puts("aarch64: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retplaceofarmsangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
aarch64_uart_puts("aarch64: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retcurtainface — Wave 73 return-curtainface honesty */
aarch64_uart_puts("aarch64: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retcurtainface honesty; Soft≠product)\n");
/* Grep: aarch64: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
aarch64_uart_puts("aarch64: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retbastionangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retglacisangle — Wave 74 return-glacisangle honesty */
aarch64_uart_puts("aarch64: soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retglacisangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
aarch64_uart_puts("aarch64: soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retparapetangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retmoatangle — Wave 75 return-moatangle honesty */
aarch64_uart_puts("aarch64: soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retmoatangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retowerangle — Wave 75 exclusive towerangle stamp */
aarch64_uart_puts("aarch64: soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retowerangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retgateangle — Wave 76 return-gateangle honesty */
aarch64_uart_puts("aarch64: soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retgateangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retwallangle — Wave 76 exclusive wallangle stamp */
aarch64_uart_puts("aarch64: soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retwallangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retspireangle — Wave 77 return-spireangle honesty */
aarch64_uart_puts("aarch64: soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retspireangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retholdangle — Wave 77 exclusive holdangle stamp */
aarch64_uart_puts("aarch64: soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retholdangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retpalaceangle — Wave 78 return-palaceangle honesty */
aarch64_uart_puts("aarch64: soft retpalaceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retpalaceangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retfortressangle — Wave 78 exclusive fortressangle stamp */
aarch64_uart_puts("aarch64: soft retfortressangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retfortressangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retkeepangle — Wave 79 return-keepangle honesty */
aarch64_uart_puts("aarch64: soft retkeepangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retkeepangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retredoubtangle — Wave 79 exclusive redoubtangle stamp */
aarch64_uart_puts("aarch64: soft retredoubtangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retredoubtangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retcitadelangle — Wave 80 return-citadelangle honesty */
aarch64_uart_puts("aarch64: soft retcitadelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retcitadelangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retbastionkeep — Wave 80 exclusive bastionkeep stamp */
aarch64_uart_puts("aarch64: soft retbastionkeep exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retbastionkeep stamp; Soft≠product)\n");
/* Grep: aarch64: soft retpanoplyangle — Wave 81 return-panoplyangle honesty */
aarch64_uart_puts("aarch64: soft retpanoplyangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retpanoplyangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retbulwarkangle — Wave 81 exclusive bulwarkangle stamp */
aarch64_uart_puts("aarch64: soft retbulwarkangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retbulwarkangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retmantleangle — Wave 82 return-mantleangle honesty */
aarch64_uart_puts("aarch64: soft retmantleangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retmantleangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retaegisangle — Wave 82 exclusive aegisangle stamp */
aarch64_uart_puts("aarch64: soft retaegisangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retaegisangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retemblemangle — Wave 83 return-emblemangle honesty */
aarch64_uart_puts("aarch64: soft retemblemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retemblemangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retsigilangle — Wave 83 exclusive sigilangle stamp */
aarch64_uart_puts("aarch64: soft retsigilangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retsigilangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retscepterangle — Wave 84 return-scepterangle honesty */
aarch64_uart_puts("aarch64: soft retscepterangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retscepterangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retglyphangle — Wave 84 exclusive glyphangle stamp */
aarch64_uart_puts("aarch64: soft retglyphangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retglyphangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retcrownangle — Wave 85 return-crownangle honesty */
aarch64_uart_puts("aarch64: soft retcrownangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retcrownangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retshardangle — Wave 85 exclusive shardangle stamp */
aarch64_uart_puts("aarch64: soft retshardangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retshardangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retforgeangle — Wave 86 return-forgeangle honesty */
aarch64_uart_puts("aarch64: soft retforgeangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retforgeangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retprismangle — Wave 86 exclusive prismangle stamp */
aarch64_uart_puts("aarch64: soft retprismangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retprismangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retflameangle — Wave 87 return-flameangle honesty */
aarch64_uart_puts("aarch64: soft retflameangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retflameangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retcipherangle — Wave 87 exclusive cipherangle stamp */
aarch64_uart_puts("aarch64: soft retcipherangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retcipherangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retbeaconangle — Wave 88 return-beaconangle honesty */
aarch64_uart_puts("aarch64: soft retbeaconangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retbeaconangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retledgerangle — Wave 88 exclusive ledgerangle stamp */
aarch64_uart_puts("aarch64: soft retledgerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retledgerangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retbannerangle — Wave 89 return-bannerangle honesty */
aarch64_uart_puts("aarch64: soft retbannerangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retbannerangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retvaultangle — Wave 89 exclusive vaultangle stamp */
aarch64_uart_puts("aarch64: soft retvaultangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retvaultangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retcrestangle — Wave 90 return-crestangle honesty */
aarch64_uart_puts("aarch64: soft retcrestangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retcrestangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft rettokenangle — Wave 90 exclusive tokenangle stamp */
aarch64_uart_puts("aarch64: soft rettokenangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (rettokenangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retbadgeangle — Wave 91 return-badgeangle honesty */
aarch64_uart_puts("aarch64: soft retbadgeangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retbadgeangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retphaseangle — Wave 91 exclusive phaseangle stamp */
aarch64_uart_puts("aarch64: soft retphaseangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retphaseangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retmarkangle — Wave 92 return-markangle honesty */
aarch64_uart_puts("aarch64: soft retmarkangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retmarkangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retpulseangle — Wave 92 exclusive pulseangle stamp */
aarch64_uart_puts("aarch64: soft retpulseangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retpulseangle stamp; Soft≠product)\n");

/* Grep: aarch64: soft retsealangle — Wave 93 return-sealangle honesty */
aarch64_uart_puts("aarch64: soft retsealangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retsealangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retboundangle — Wave 93 exclusive boundangle stamp */
aarch64_uart_puts("aarch64: soft retboundangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retboundangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retstemangle — Wave 94 return-stemangle honesty */
aarch64_uart_puts("aarch64: soft retstemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retstemangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retbladeangle — Wave 94 exclusive bladeangle stamp */
aarch64_uart_puts("aarch64: soft retbladeangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retbladeangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retchordangle — Wave 95 return-chordangle honesty */
aarch64_uart_puts("aarch64: soft retchordangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retchordangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retarcangle — Wave 95 exclusive arcangle stamp */
aarch64_uart_puts("aarch64: soft retarcangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retarcangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retsectorangle — Wave 96 return-sectorangle honesty */
aarch64_uart_puts("aarch64: soft retsectorangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retsectorangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retwedgeangle — Wave 96 exclusive wedgeangle stamp */
aarch64_uart_puts("aarch64: soft retwedgeangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retwedgeangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retradiusangle — Wave 97 return-radiusangle honesty */
aarch64_uart_puts("aarch64: soft retradiusangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retradiusangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retdiameterangle — Wave 97 exclusive diameterangle stamp */
aarch64_uart_puts("aarch64: soft retdiameterangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retdiameterangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retcircumangle — Wave 98 return-circumangle honesty */
aarch64_uart_puts("aarch64: soft retcircumangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retcircumangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retellipseangle — Wave 98 exclusive ellipseangle stamp */
aarch64_uart_puts("aarch64: soft retellipseangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retellipseangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft rethyperangle — Wave 99 return-hyperangle honesty */
aarch64_uart_puts("aarch64: soft rethyperangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (rethyperangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retparabolaangle — Wave 99 exclusive parabolaangle stamp */
aarch64_uart_puts("aarch64: soft retparabolaangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retparabolaangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retspiralangle — Wave 100 return-spiralangle honesty */
aarch64_uart_puts("aarch64: soft retspiralangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retspiralangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft rethelixangle — Wave 100 exclusive helixangle stamp */
aarch64_uart_puts("aarch64: soft rethelixangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (rethelixangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft rettorusangle — Wave 101 return-torusangle honesty */
aarch64_uart_puts("aarch64: soft rettorusangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (rettorusangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retknotangle — Wave 101 exclusive knotangle stamp */
aarch64_uart_puts("aarch64: soft retknotangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retknotangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retmoebiusangle — Wave 102 return-moebiusangle honesty */
aarch64_uart_puts("aarch64: soft retmoebiusangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retmoebiusangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retkleinangle — Wave 102 exclusive kleinangle stamp */
aarch64_uart_puts("aarch64: soft retkleinangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retkleinangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retprojectangle — Wave 103 return-projectangle honesty */
aarch64_uart_puts("aarch64: soft retprojectangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retprojectangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retaffineangle — Wave 103 exclusive affineangle stamp */
aarch64_uart_puts("aarch64: soft retaffineangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retaffineangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retlinearangle — Wave 104 return-linearangle honesty */
aarch64_uart_puts("aarch64: soft retlinearangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retlinearangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retbilinearangle — Wave 104 exclusive bilinearangle stamp */
aarch64_uart_puts("aarch64: soft retbilinearangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retbilinearangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retquadraticangle — Wave 105 return-quadraticangle honesty */
aarch64_uart_puts("aarch64: soft retquadraticangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retquadraticangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retcubicangle — Wave 105 exclusive cubicangle stamp */
aarch64_uart_puts("aarch64: soft retcubicangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retcubicangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retquarticangle — Wave 106 return-quarticangle honesty */
aarch64_uart_puts("aarch64: soft retquarticangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retquarticangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retquinticangle — Wave 106 exclusive quinticangle stamp */
aarch64_uart_puts("aarch64: soft retquinticangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retquinticangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retsplineangle — Wave 107 return-splineangle honesty */
aarch64_uart_puts("aarch64: soft retsplineangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retsplineangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retbezierangle — Wave 107 exclusive bezierangle stamp */
aarch64_uart_puts("aarch64: soft retbezierangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retbezierangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft rethurmitangle — Wave 108 return-hermitangle honesty */
aarch64_uart_puts("aarch64: soft rethurmitangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (rethurmitangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retcatmullangle — Wave 108 exclusive catmullangle stamp */
aarch64_uart_puts("aarch64: soft retcatmullangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retcatmullangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retnurbsangle — Wave 109 return-nurbsangle honesty */
aarch64_uart_puts("aarch64: soft retnurbsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retnurbsangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retbsplineangle — Wave 109 exclusive bsplineangle stamp */
aarch64_uart_puts("aarch64: soft retbsplineangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retbsplineangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retmeshangle — Wave 110 return-meshangle honesty */
aarch64_uart_puts("aarch64: soft retmeshangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retmeshangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retgridangle — Wave 110 exclusive gridangle stamp */
aarch64_uart_puts("aarch64: soft retgridangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retgridangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retvoxelangle — Wave 111 return-voxelangle honesty */
aarch64_uart_puts("aarch64: soft retvoxelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retvoxelangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft rettexelangle — Wave 111 exclusive texelangle stamp */
aarch64_uart_puts("aarch64: soft rettexelangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (rettexelangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retfragmentangle — Wave 112 return-fragmentangle honesty */
aarch64_uart_puts("aarch64: soft retfragmentangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retfragmentangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retvertexangle — Wave 112 exclusive vertexangle stamp */
aarch64_uart_puts("aarch64: soft retvertexangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retvertexangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retshaderangle — Wave 113 return-shaderangle honesty */
aarch64_uart_puts("aarch64: soft retshaderangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retshaderangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retpipelineangle — Wave 113 exclusive pipelineangle stamp */
aarch64_uart_puts("aarch64: soft retpipelineangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retpipelineangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retframebufferangle — Wave 114 return-framebufferangle honesty */
aarch64_uart_puts("aarch64: soft retframebufferangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retframebufferangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retswapchainangle — Wave 114 exclusive swapchainangle stamp */
aarch64_uart_puts("aarch64: soft retswapchainangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retswapchainangle stamp; Soft≠product)\n");
aarch64_uart_puts("aarch64: soft retpresentangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retpresentangle honesty; Soft≠product)\n");
aarch64_uart_puts("aarch64: soft retvsyncangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retvsyncangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retfenceangle — Wave 116 return-fenceangle honesty */
aarch64_uart_puts("aarch64: soft retfenceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retfenceangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retsemaphoreangle — Wave 116 exclusive semaphoreangle stamp */
aarch64_uart_puts("aarch64: soft retsemaphoreangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retsemaphoreangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retmutexangle — Wave 117 return-mutexangle honesty */
aarch64_uart_puts("aarch64: soft retmutexangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retmutexangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retcondangle — Wave 117 exclusive condangle stamp */
aarch64_uart_puts("aarch64: soft retcondangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retcondangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retbarrierangle — Wave 118 return-barrierangle honesty */
aarch64_uart_puts("aarch64: soft retbarrierangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retbarrierangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retatomicangle — Wave 118 exclusive atomicangle stamp */
aarch64_uart_puts("aarch64: soft retatomicangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retatomicangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retqueueangle — Wave 119 return-queueangle honesty */
aarch64_uart_puts("aarch64: soft retqueueangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=119 (retqueueangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft reteventangle — Wave 119 exclusive eventangle stamp */
aarch64_uart_puts("aarch64: soft reteventangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=119 (reteventangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retchannelangle — Wave 120 return-channelangle honesty */
aarch64_uart_puts("aarch64: soft retchannelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=120 (retchannelangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retmailboxangle — Wave 120 exclusive mailboxangle stamp */
aarch64_uart_puts("aarch64: soft retmailboxangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=120 (retmailboxangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retstreamangle — Wave 121 return-streamangle honesty */
aarch64_uart_puts("aarch64: soft retstreamangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=121 (retstreamangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retpacketangle — Wave 121 exclusive packetangle stamp */
aarch64_uart_puts("aarch64: soft retpacketangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=121 (retpacketangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retframeangle — Wave 122 return-frameangle honesty */
aarch64_uart_puts("aarch64: soft retframeangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=122 (retframeangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retwindowangle — Wave 122 exclusive windowangle stamp */
aarch64_uart_puts("aarch64: soft retwindowangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=122 (retwindowangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retlayerangle — Wave 123 return-layerangle honesty */
aarch64_uart_puts("aarch64: soft retlayerangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=123 (retlayerangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retcanvasangle — Wave 123 exclusive canvasangle stamp */
aarch64_uart_puts("aarch64: soft retcanvasangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=123 (retcanvasangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retbrushangle — Wave 124 return-brushangle honesty */
aarch64_uart_puts("aarch64: soft retbrushangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=124 (retbrushangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retinkangle — Wave 124 exclusive inkangle stamp */
aarch64_uart_puts("aarch64: soft retinkangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=124 (retinkangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retpaletteangle — Wave 125 return-paletteangle honesty */
aarch64_uart_puts("aarch64: soft retpaletteangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=125 (retpaletteangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retstrokeangle — Wave 125 exclusive strokeangle stamp */
aarch64_uart_puts("aarch64: soft retstrokeangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=125 (retstrokeangle stamp; Soft≠product)\n");
/* Grep: aarch64: soft retgradientangle — Wave 126 return-gradientangle honesty */
aarch64_uart_puts("aarch64: soft retgradientangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=126 (retgradientangle honesty; Soft≠product)\n");
/* Grep: aarch64: soft retblendangle — Wave 126 exclusive blendangle stamp */
aarch64_uart_puts("aarch64: soft retblendangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=126 (retblendangle stamp; Soft≠product)\n");
aarch64_uart_put_hex((unsigned long)EXC_SOFT_WAVE);
...
/* Grep: aarch64: exception soft deepen */
    ...
    aarch64_uart_puts("aarch64: exception soft honesty product_kernel=OPEN "
                      "soft_only=1 no_irq_deliver=1 wave=");
    aarch64_uart_put_hex((unsigned long)EXC_SOFT_WAVE);
    aarch64_uart_puts("\n");
