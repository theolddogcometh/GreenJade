 ...
 *   "aarch64: mmu soft residual lean ..." stamp-free honesty.
 *   Soft identity 2-block map != hierarchical VMM / user maps / product bar.
 *   G-AC-1: no Linux .ko product AC. Dual MIT OR Apache-2.0. No GPL.
 *   Dual DoD remains OPEN. Never hard-gates mmu PASS / enable path.
 *   No version stamp. No GJ_IMAGE_VERSION bump. No stamp storms.
 *
 * Greppable:
 *   aarch64: mmu PASS
 ...
 *   aarch64: mmu soft inventory wave=126 ...
 *   aarch64: mmu soft deepen wave=126 areas=... catalog=...
 *   aarch64: mmu soft surf page_ok=... ttbr_ok=... mair_ok=... bits=...
 *   aarch64: mmu soft path identity=1 product_kernel=OPEN
 *   aarch64: mmu soft honesty product_kernel=OPEN soft_only=1 Soft!=product
 *   aarch64: mmu soft exclusive multi_server=0 confine=0 residual=1
 *   aarch64: mmu soft open multi_server=0 product_kernel=OPEN
 *   aarch64: mmu soft residual lean ... Soft!=product G-AC-1
 *   aarch64: mmu soft return inv_ret=... product_kernel=OPEN
 *   aarch64: mmu soft PASS | FAIL
 *   aarch64: mmu soft SKIP (no page)
 *
 * Soft residual != product hierarchical VMM / Dual DoD close / user maps.
...
/* Soft inventory wave (greppable; residual lean stamp-free; not version stamp). */
#define MMU_SOFT_WAVE 126u

/*
 * Soft area count for lean deepen catalog (honest residual; no ret* storm):
 * page,ttbr,mair,sctlr,l1,inventory,deepen,surf,path,honesty,exclusive,
 * open,residual,return = 14.
 */
#define MMU_SOFT_AREAS 14u
...
mmu_soft_inventory(unsigned long *pL1)
{
     ...
     * Lean area catalog — identity soft scaffold only; no ret* stamp storm.
     * residual+lean replace ret*angle storms (C0 residual; Soft!=product).
     */
    aarch64_uart_puts("aarch64: mmu soft deepen wave=");
    aarch64_uart_put_hex((unsigned long)MMU_SOFT_WAVE);
    aarch64_uart_puts(" areas=");
    aarch64_uart_put_hex((unsigned long)MMU_SOFT_AREAS);
    aarch64_uart_puts(" catalog=page,ttbr,mair,sctlr,l1,inventory,deepen,surf,"
                      "path,honesty,exclusive,open,residual,return "
                      "logs=");
    aarch64_uart_put_hex((unsigned long)g_cMmuSoftLogs);
    aarch64_uart_puts(" soft_only=1 stamp_storm=0\n");

    /* Grep: aarch64: mmu soft surf — identity gate lamps */
     ...
     * product_kernel=OPEN: aarch64 product kernel remains OPEN (Dual DoD).
     */
     ...
     * Explicit non-claim: soft inventory deepen != product VMM complete.
    ...
    aarch64_uart_puts("aarch64: mmu soft honesty product_kernel=OPEN "
                      "soft_only=1 no_user_maps=1 no_hier_vmm=1 "
                      "soft_ne_product=1 dual=MIT_OR_Apache-2.0 G-AC-1=1 "
                      "wave=");
    aarch64_uart_put_hex((unsigned long)MMU_SOFT_WAVE);
    aarch64_uart_puts(" (Soft!=product; identity scaffold only)\n");

    /*
     * Grep: aarch64: mmu soft exclusive — exclusive residual deepen.
     * Soft inventory != product multi-server confine / product kernel.
     */
    aarch64_uart_puts("aarch64: mmu soft exclusive multi_server=0 "
                      "confine=0 product_kernel=OPEN soft_only=1 residual=1 "
                      "wave=");
    aarch64_uart_put_hex((unsigned long)MMU_SOFT_WAVE);
    aarch64_uart_puts("\n");

    /*
     * Grep: aarch64: mmu soft open — open-lamp rollup.
     * Dual DoD OPEN; soft != product complete.
     */
    ...
    /*
     * Grep: aarch64: mmu soft residual lean
     * C0 residual (stamp-free; Soft!=product). Never hard-gates enable path.
     * G-AC-1: not product hierarchical VMM / no .ko product AC / no GPL.
     * BAR v2026.08.04.75 stamp-free — no GJ_IMAGE_VERSION bump.
     */
    aarch64_uart_puts("aarch64: mmu soft residual lean ");
    if (fOk != 0) {
        aarch64_uart_puts("PASS");
    } else {
        aarch64_uart_puts("FAIL");
    }
    aarch64_uart_puts(" identity=1 l1_blocks=2 tg0_4k=1 t0sz=25 "
                      "page_ok=");
    aarch64_uart_put_hex(fPageOk != 0 ? 1ul : 0ul);
    aarch64_uart_puts(" ttbr_ok=");
    aarch64_uart_put_hex((fTtbrPresent != 0 && fTtbrMatch != 0 &&
    ...
    aarch64_uart_put_hex((unsigned long)(
        ...
        (fMairOk != 0 ? 1u : 0u) +
        (fSctlrOk != 0 ? 1u : 0u) +
        ((fD0Ok != 0 && fD1Ok != 0) ? 1u : 0u)));
    aarch64_uart_puts("/5 areas=");
    aarch64_uart_put_hex((unsigned long)MMU_SOFT_AREAS);
    aarch64_uart_puts(" logs=");
    aarch64_uart_put_hex((unsigned long)g_cMmuSoftLogs);
    aarch64_uart_puts(" stamp_storm=0 version_stamp=0 hard_gate=0 "
                      "user_maps=0 hier_vmm=0 product_kernel=OPEN "
                      "multi_server=0 confine=0 g_ac1=1 no_ko_product_ac=1 "
                      "soft_ne_product=1 dual=MIT_OR_Apache-2.0 unit=mmu.c "
                      "(Soft!=product; dual MIT OR Apache-2.0; G-AC-1; "
                      "C0 aarch64 identity MMU residual lean; no version stamp; "
                      "not hierarchical product VMM; Dual DoD OPEN)\n");

    /*
     * Grep: aarch64: mmu soft return
     * inv_ret is soft inventory bool; never hard-gates product mmu PASS.
