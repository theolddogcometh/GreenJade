 ...
 *
 * -------------------------------------------------------------------------
 * Soft inventory deepen + C0 residual lean (Soft!=product; this unit only)
 * -------------------------------------------------------------------------
 ...
 *   aarch64: timer soft inventory wave=126 ...
 *   aarch64: timer soft stats ...
 *   aarch64: timer soft deepen wave=126 areas=... catalog=... stamp_storm=0
 *   aarch64: timer soft path imask=1 irq_delivery=0 product_kernel=OPEN
 *   aarch64: timer soft surf freq_ok=... tick_ok=... hits=... imask=...
 *   aarch64: timer soft honesty product_kernel=OPEN soft_only=1
 *   aarch64: timer soft exclusive multi_server=0 confine=0 product_kernel=OPEN
 *   aarch64: timer soft open multi_server=0 confine=0 product_kernel=OPEN
 *   aarch64: timer soft residual lean ... Soft!=product G-AC-1 dual=MIT|Apache-2.0
 *   aarch64: timer soft return inv_ret=... freq=... tick=... product_kernel=OPEN
 *   aarch64: timer soft PASS | FAIL
 *   aarch64: timer PASS
 *
 * C0 residual (stamp-free): CNTV soft arm + ISTATUS dual-hit only; IMASK stays
 * set; irq_delivery=0; no GIC IAR/EOIR take; no product mono/jiffies path;
 * Dual DoD remains OPEN; Soft!=product; G-AC-1 (no .ko / no GPL timer tables);
 * dual MIT OR Apache-2.0. residual+lean replace ret* stamp storms.
 * Never hard-gates M0 / timer PASS. Never bumps GJ_IMAGE_VERSION.
 *
 ...
 * freq,tick,inventory,stats,path,surf,return,honesty,exclusive,open,
                     ...
                     int fFreqSoft, int fTickSoft)
{
     ...
     * Lean area catalog - CNTV soft arm only; no IRQ delivery claim.
    ...
    aarch64_uart_puts(" catalog=freq,tick,inventory,stats,path,surf,return,"
                      "honesty,exclusive,open,residual,lean,deepen "
                      "logs=");
    aarch64_uart_put_hex((unsigned long)g_cTimerSoftLogs);
    aarch64_uart_puts(" soft_only=1 stamp_storm=0\n");

    /* Grep: aarch64: timer soft surf - freq/tick lamps */
     ...
     * Explicit non-claim: soft CNTV inventory != product IRQ timer complete.
     */
    aarch64_uart_puts("aarch64: timer soft honesty product_kernel=OPEN "
                      "soft_only=1 no_irq_take=1 imask=1 cntv=1 cntp=0 "
                      "multi_server=0 confine=0 wave=");
    aarch64_uart_put_hex((unsigned long)TIMER_SOFT_WAVE);
    aarch64_uart_puts("\n");

    /* Grep: aarch64: timer soft exclusive - exclusive residual deepen */
    aarch64_uart_puts("aarch64: timer soft exclusive multi_server=0 "
                      "confine=0 product_kernel=OPEN soft_only=1 wave=");
    aarch64_uart_put_hex((unsigned long)TIMER_SOFT_WAVE);
    aarch64_uart_puts("\n");

    /* Grep: aarch64: timer soft open - open-lamp rollup */
    ...
    /*
     * Grep: aarch64: timer soft residual lean
     * C0 residual once-path lamp. Soft!=product dual MIT OR Apache-2.0.
     * Stamp-free (no ret*angle storm, no product version stamp). G-AC-1:
     * no .ko / no GPL timer tables. Dual DoD remains OPEN. Never hard-gates
     * timer PASS / M0. Soft CNTV arm != product IRQ timer / mono product.
     */
    aarch64_uart_puts("aarch64: timer soft residual lean "
                      "cntv=1 cntp=0 imask=1 irq_delivery=0 ppi=");
    aarch64_uart_put_hex((unsigned long)TIMER_PPI_VIRT);
    aarch64_uart_puts(" freq_ok=");
    aarch64_uart_put_hex((unsigned long)fFreqSoft);
    aarch64_uart_puts(" tick_ok=");
    ...
    aarch64_uart_puts(" stamp_storm=0 version_stamp=0 hard_gate=0 "
                      "product_kernel=OPEN dual_dod=OPEN multi_server=0 "
                      "confine=0 g_ac1=1 no_ko_product_ac=1 no_gpl=1 "
                      "soft_ne_product=1 dual=MIT_OR_Apache-2.0 unit=timer.c "
                      "Soft!=product G-AC-1 wave=");
    aarch64_uart_put_hex((unsigned long)TIMER_SOFT_WAVE);
    aarch64_uart_puts(" (soft residual != product IRQ timer; Dual DoD OPEN; "
                      "no version stamp; C0 aarch64 timer residual lean)\n");

    /* Grep: aarch64: timer soft return - inventory return surfaces */
