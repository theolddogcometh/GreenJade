 ...
 *   apic: soft deepen ...                     - wave stamp + area catalog
 * H1 residual lean surfaces (inventory-capped; Soft!=product dual_dod_b_irq_safe):
 *   apic: soft h1 ...                         - hazard H1 IRQ residual contract
 *   apic: soft eth ...                        - eth poll ownership (run-loop)
 *   apic: soft residual ...                   - dual_dod_b residual lean lamp
 * Soft only: wrap-OK counters + kprintf; never hard-gates product paths.
 * Soft!=product: inventory never claims multi-server / product close.
 * Soft!=product dual license: MIT OR Apache-2.0; G-AC-1 never rewritten here.
 *
 * H1 Dual DoD B residual lean (Soft!=product - exclusive with x2apic.c):
 *   apic_timer_irq / apic_ipi_resched_irq / apic_ipi_tlb_irq / apic_eoi
 *   NEVER call net_eth_poll. Eth poll = scheduler_run thr stack only.
 *   Hot IRQ lean: counters + mono (timer) + EOI only - no kprintf, no soft inv.
 *   Fault class: IRQ -> net_eth_poll -> stack smash -> #PF I=1 wild RIP.
 *   Soft inventory multi-line dumps capped (APIC_SOFT_INV_CAP); boot/smoke
 *   only - NEVER from timer/IPI IRQ. Soft PASS != product Dual DoD B close.
 *   Soft!=product · dual_dod_b_irq_safe · no_net_eth_poll · net_eth_irq=0.
 * C0 residual lean surfaces (inventory-capped only; Soft!=product; never IRQ):
 *   apic: soft residual ... - dual_dod_b_irq_safe Soft!=product C0 lean lamp
 *   apic: soft eth ...      - H1 eth poll ownership (run-loop only)
 *   apic: soft h1 ...       - hot IRQ contract (counter+EOI(+mono) only)
 *   Dual DoD A/B remain OPEN (product UDX path); soft residual != product close.
 * greppable: apic: soft
 * greppable: apic: soft exclusive
 * greppable: apic: soft residual
 * greppable: apic: soft eth
 * greppable: apic: soft h1
 * greppable: no_net_eth_poll
 * greppable: dual_dod_b_irq_safe
 * greppable: Soft!=product
 * greppable: dual_dod_b_irq_safe Soft!=product
 * greppable: h1_irq_eth_forbidden
 * greppable: C0 residual lean
 * greppable: APIC_H1_NET_ETH_POLL_FROM=0
...
/*
 * Cap full multi-line soft inventory dumps (init/cal/timer_hz/cal_log).
 * Prevents stamp-storm serial floods if inventory is re-called from activity.
 * Never invoked from apic_timer_irq / IPI IRQ. Soft!=product · Dual DoD B.
 * dual_dod_b_irq_safe: inv only boot/smoke - never hot IRQ path.
 */
#define APIC_SOFT_INV_CAP 16u

/*
 * H1 compile-time locks (C0 residual · Soft!=product · Dual DoD B).
 * This TU never calls net_eth_poll; eth poll = scheduler_run thr stack only.
 * Compile-time locked: flip requires H1 review (fault class IRQ stack smash).
 * greppable: APIC_H1_NET_ETH_POLL_FROM=0 | APIC_H1_NET_ETH_IRQ=0
 * greppable: net_eth_poll=run_loop_only | net_eth_irq=0 | no_net_eth_poll=1
 */
#define APIC_H1_NET_ETH_POLL_FROM  0  /* never call net_eth_poll from this TU */
#define APIC_H1_NET_ETH_IRQ        0  /* never eth poll from timer/IPI/EOI */
#define APIC_H1_RUN_LOOP_ONLY      1  /* net_eth_poll=run_loop_only */
#define APIC_H1_DUAL_DOD_B_OPEN    1  /* product Dual DoD B remains OPEN */

#if APIC_H1_NET_ETH_POLL_FROM != 0
#error "H1: APIC_H1_NET_ETH_POLL_FROM must be 0 (never call net_eth_poll)"
#endif
#if APIC_H1_NET_ETH_IRQ != 0
#error "H1: APIC_H1_NET_ETH_IRQ must be 0 (run-loop owns eth poll)"
#endif
#if APIC_H1_RUN_LOOP_ONLY != 1
#error "H1: APIC_H1_RUN_LOOP_ONLY must be 1 (net_eth_poll=run_loop_only)"
#endif
#if APIC_H1_DUAL_DOD_B_OPEN != 1
#error "H1: APIC_H1_DUAL_DOD_B_OPEN must be 1 (Dual DoD B product OPEN)"
#endif
 ...
 * Soft!=product: hot IRQ never soft inventory / never net_eth_poll.
 * dual_dod_b_irq_safe: counter+EOI(+mono) only on timer/IPI/EOI paths.
 * greppable: apic: soft
...
/**
 * Wave 15 greppable soft APIC inventory (smoke / soft only).
 * Prefix-stable markers under "apic: soft ...". Never hard-gates; no alloc.
 * via= names the call site (init / calibrate / timer_hz / cal_log / ...).
 * Primary Wave 10/12 field names stay stable; Wave 15+ complementary
 * sub-lines kept. Multi-line stamp storms (ret* wave spam) removed -
 * primary surfaces + deepen only, emission-capped (APIC_SOFT_INV_CAP).
 * NEVER call from apic_timer_irq / IPI IRQ handlers (Soft!=product).
 * Soft!=product · dual_dod_b_irq_safe · no_net_eth_poll on hot IRQ forever.
 * greppable: apic: soft
 * greppable: dual_dod_b_irq_safe Soft!=product
 */
static void
apic_soft_inventory(const char *szVia)
{
    ...
    /* Cap multi-line inventory storms (boot/smoke only; never IRQ). */
    if (g_u32SoftLogN >= APIC_SOFT_INV_CAP) {
        return;
    }
    ...
    /* Grep: apic: soft irq - tallies only; handlers never net_eth_poll (H1) */
    /* Soft!=product · dual_dod_b_irq_safe · IRQ never net_eth_poll */
    kprintf("apic: soft irq timer=%lu bsp=%lu ap=%lu mono=%lu "
            "resched_irq=%lu tlb_irq=%lu local_default=%lu "
            "local_prog=%lu no_net_eth_poll=1 net_eth_irq=0 "
            "hot_kprintf=0 dual_dod_b_irq_safe=1 Soft!=product "
            "h1_irq_eth_forbidden=1\n",
            (unsigned long)g_u64SoftTimerIrq,
     ...
     * H1 Dual DoD B: timer/IPI IRQ never net_eth_poll (sched run loop only).
     * Soft!=product · dual_dod_b_irq_safe denser residual tokens.
     */
    kprintf("apic: soft path claim=xapic_timer+ipi+tlb p_irq1=1 "
            "p_time4=1 p_smp5=1 mono=bsp_only x2_optional=1 "
            "no_net_eth_poll=1 net_eth_irq=0 hot_irq_kprintf=0 "
            "soft_ne_product=1 dual_dod_b_irq_safe=1 Soft!=product "
            "h1_irq_eth_forbidden=1 "
            "via=%s wave=%u (soft inventory; H1 Dual DoD B IRQ-safe; "
            "Soft!=product)\n",
     ...
     */
    /* Grep: apic: soft exclusive · Soft!=product · dual_dod_b_irq_safe */
    kprintf("apic: soft exclusive wave=%u exclusive=1 soft=1 "
            "unit=apic.c hard_gate=0 multi_server=0 "
            "hot_irq_kprintf=0 no_net_eth_poll=1 net_eth_irq=0 "
            "dual_dod_b_irq_safe=1 soft_ne_product=1 Soft!=product "
            "h1_irq_eth_forbidden=1 via=%s\n",
            (unsigned)APIC_SOFT_WAVE, szVia);

    /* Grep: apic: soft claim - product claim bounds; Soft!=product */
    kprintf("apic: soft claim xapic_timer=1 ipi=1 tlb=1 "
            ...
            "no_net_eth_poll=1 net_eth_irq=0 dual_dod_b_irq_safe=1 "
            "soft_ne_product=1 Soft!=product h1_irq_eth_forbidden=1 "
            "wave=%u via=%s\n",
            (unsigned)APIC_SOFT_WAVE, szVia);
    ...
    /* Grep: apic: soft honesty · Soft!=product · dual_dod_b_irq_safe */
    kprintf("apic: soft honesty soft_ne_product_complete=1 "
            "multi_server=0 hard_gate=0 "
            "claim=xapic_timer+ipi+tlb mono=bsp_only "
            "x2_optional=1 no_net_eth_poll=1 net_eth_irq=0 "
            "hot_irq_kprintf=0 soft_ne_product=1 "
            "dual_dod_b_irq_safe=1 h1_irq_eth_forbidden=1 "
            "Soft!=product dual_license=MIT_OR_Apache-2.0 "
            "wave=%u unit=apic.c via=%s\n",
            (unsigned)APIC_SOFT_WAVE, szVia);

    /*
     * Wave 17 deepen stamp (kept; never reshape primary).
     * Multi-line ret* stamp storms removed (Soft!=product inventory cap).
     * greppable: apic: soft deepen
     * Soft!=product · dual_dod_b_irq_safe denser residual tokens.
     */
    kprintf("apic: soft deepen wave=%u areas="
            ...
            "capacity,exclusive,claim,ratio,honesty,h1,eth,residual "
            "unit=apic.c only hot_irq_kprintf=0 no_net_eth_poll=1 "
            "net_eth_irq=0 inv_cap=%u soft_ne_product=1 "
            "dual_dod_b_irq_safe=1 h1_irq_eth_forbidden=1 "
            "Soft!=product C0_residual=1 residual=1 eth=1 via=%s\n",
            (unsigned)APIC_SOFT_WAVE, (unsigned)APIC_SOFT_INV_CAP, szVia);

    /*
     * C0 residual lean Dual DoD B / H1 (Soft!=product; inventory-capped only).
     * Three greppable lines max - no stamp storms, no version stamp.
     * IRQ never net_eth_poll; eth ownership = scheduler_run thr stack.
     * Dual DoD A/B product remain OPEN (UDX path); soft residual != close.
     * Grep: apic: soft h1 | apic: soft eth | apic: soft residual
     * Grep: dual_dod_b_irq_safe Soft!=product | no_net_eth_poll
     * Grep: C0 residual lean | APIC_H1_NET_ETH_POLL_FROM=0
     */
    /* Grep: apic: soft h1 */
    kprintf("apic: soft h1 hazard=H1 hot_path=counter+eoi+mono "
            "timer_irq=lean resched_irq=lean tlb_irq=lean eoi=lean "
            "hot_kprintf=0 hot_soft_inv=0 no_net_eth_poll=1 net_eth_irq=0 "
            "net_eth_poll=run_loop_only fault_class=H1_irq_stack_smash "
            "dual_dod_b_irq_safe=1 h1_irq_eth_forbidden=1 "
            "APIC_H1_NET_ETH_POLL_FROM=%u APIC_H1_NET_ETH_IRQ=%u "
            "soft_ne_product=1 Soft!=product "
            "(H1 residual; Soft!=product; eth poll never on hot IRQ)\n",
            (unsigned)APIC_H1_NET_ETH_POLL_FROM,
            (unsigned)APIC_H1_NET_ETH_IRQ);
    /* Grep: apic: soft eth */
    kprintf("apic: soft eth net_eth_poll=run_loop_only net_eth_irq=0 "
            "timer_irq=0 resched_irq=0 tlb_irq=0 eoi_path=0 "
            "owner=scheduler_run stack=thr fault_class=H1_irq_stack_smash "
            "no_net_eth_poll=1 dual_dod_b_irq_safe=1 "
            "h1_irq_eth_forbidden=1 soft_ne_product=1 Soft!=product "
            "APIC_H1_NET_ETH_POLL_FROM=%u C0_residual=1\n",
            (unsigned)APIC_H1_NET_ETH_POLL_FROM);
    /* Grep: apic: soft residual · C0 residual lean */
    kprintf("apic: soft residual dual_dod_a=OPEN dual_dod_b=OPEN "
            "dual_dod_b_irq_safe=1 no_net_eth_poll=1 net_eth_irq=0 "
            "owner=scheduler_run never_on_timer_irq=1 never_on_ipi_irq=1 "
            "never_on_eoi=1 product_sshd_tcp22=OPEN soft_ne_product=1 "
            "storm=0 h1_irq_eth_forbidden=1 G-AC-1=1 "
            "APIC_H1_NET_ETH_POLL_FROM=%u Soft!=product "
            "(C0 residual lean; Soft!=product; inv_cap only)\n",
            (unsigned)APIC_H1_NET_ETH_POLL_FROM);
}
...
apic_timer_irq(void)
{
    ...
    /*
     * LVT timer IRQ lean (H1 Dual DoD B residual · Soft!=product):
     * Soft counters + mono tick (BSP) + EOI only. dual_dod_b_irq_safe.
     * FORBIDDEN on this stack forever:
     *   - net_eth_poll (sched run loop only - full thread stack)
     *   - kprintf / apic_soft_inventory (stamp-storm -> stack smash)
     * Fault class: multi-line soft flood / eth residual -> IRQ stack
     * smash -> #PF I=1 wild RIP. Keep this handler minimal forever.
     * Soft!=product · dual_dod_b_irq_safe · net_eth_irq=0 · no_net_eth_poll.
     * h1_irq_eth_forbidden: never net_eth_poll from apic_timer_irq.
     */
    ...
    if (u32Cpu == 0) {
        ...
        if (timer_apic_source()) {
            /* timer_tick_apic: jiffies/futex/quantum only - never eth. */
            timer_tick_apic();
            apic_soft_inc64(&g_u64SoftTimerMono);
        }
        /* else: LAPIC armed, mono still PIT - wake only; no eth on IRQ. */
    } else {
        apic_soft_inc64(&g_u64SoftTimerIrqAp);
    }
    /* EOI: x2apic_eoi is counter+MSR only (no soft inv); MMIO EOI likewise. */
    if (x2apic_enabled()) {
        x2apic_eoi();
    ...
    /* net_eth_poll: never - run-loop only (H1 Dual DoD B residual). */
    /* Soft!=product · dual_dod_b_irq_safe · no_net_eth_poll on timer IRQ. */
}
...
{
    /*
     * Hot IRQ completion lean (device/IPI/timer call sites).
     * H1 Dual DoD B residual (Soft!=product): counter + EOI only -
     * no kprintf, no soft inventory, no net_eth_poll.
     * Soft!=product · dual_dod_b_irq_safe · h1_irq_eth_forbidden.
     */
    ...
    /* net_eth_poll: never - run-loop only (Soft!=product dual_dod_b_irq_safe). */
}
...
{
    /*
     * Resched IPI lean (H1 Dual DoD B residual · Soft!=product):
     * Wake from HLT; AP schedule loop picks work.
     * Soft counter + EOI only - never net_eth_poll, never kprintf.
     * Fault class: IRQ stack smash / #PF I=1 if eth residual lands here.
     * Soft!=product · dual_dod_b_irq_safe · no_net_eth_poll · net_eth_irq=0.
     */
...
apic_ipi_tlb_irq(void)
{
    ...
    /*
     * TLB shootdown IPI lean (H1 Dual DoD B residual · Soft!=product):
     * invlpg/CR3 + ack + EOI only - never net_eth_poll, never kprintf.
     * Fault class: IRQ stack smash / #PF I=1 if eth residual lands here.
     * Soft!=product · dual_dod_b_irq_safe · no_net_eth_poll · net_eth_irq=0.
     */
