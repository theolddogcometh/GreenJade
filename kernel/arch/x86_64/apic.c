 ...
 *   apic: soft path …                       — honesty non-claim ()
 ...
 *   apic: soft honesty …                    — soft / multi-server
 ...
 *   apic: soft deepen …                     — wave=118 areas stamp
...
apic_soft_inventory(const char *szVia)
{
    ...
    /*
     * Grep: apic: soft path
     * Honesty: soft APIC inventory ≠ product multi-server close.
     */
    ...
    kprintf("apic: soft exclusive wave=%u exclusive=1 soft=1 "
            "unit=apic.c hard_gate=0 multi_server=0 "
    ...
    kprintf("apic: soft claim xapic_timer=1 ipi=1 tlb=1 "
            ...
            "x2_optional=1 multi_server=0 hard_gate=0 "
            "wave=%u via=%s\n",
            (unsigned)APIC_SOFT_WAVE, szVia);
    ...
    kprintf("apic: soft return ready=%u cal=%u timer=1 ipi=1 tlb=1 eoi=1 "
            "init_sipi=1 bringup=1 product_kernel=OPEN "
    ...
    kprintf("apic: soft return selftest inv_ret=1 ready=%u cal=%u "
            "product_kernel=OPEN multi_server=0 wave=%u soft PASS\n",
            g_fReady ? 1u : 0u, g_fCalibrated ? 1u : 0u,
            (unsigned)APIC_SOFT_WAVE);
 ...
 * Return surfaces only — soft inventory; never hard-gates product paths.
 */
/* Grep: apic: soft retkeepgate — Wave 45 return-keepgate honesty */
kprintf("apic: soft retkeepgate soft_only=1 product_gate=0 soft_ne_product=1 "
        "never_blocks_m0=1 wave=%u "
        "(retkeepgate honesty; Soft≠product)\n",
        (unsigned)APIC_SOFT_WAVE);
/* Grep: apic: soft retouterward — Wave 45 exclusive outerward stamp */
kprintf("apic: soft retouterward exclusive=1 soft_ne_product=1 "
        "product_kernel=OPEN wave=%u "
 ...
 * Return surfaces only — soft inventory; never hard-gates product paths.
 */
/* Grep: apic: soft retbailey — Wave 46 return-bailey honesty */
kprintf("apic: soft retbailey soft_only=1 product_gate=0 soft_ne_product=1 "
        "never_blocks_m0=1 wave=%u "
        "(retbailey honesty; Soft≠product)\n",
        (unsigned)APIC_SOFT_WAVE);
/* Grep: apic: soft retpostern — Wave 46 exclusive postern stamp */
kprintf("apic: soft retpostern exclusive=1 soft_ne_product=1 "
        "product_kernel=OPEN wave=%u "
 ...
 * Return surfaces only — soft inventory; never hard-gates product paths.
 */
/* Grep: apic: soft retfosse — Wave 51 return-fosse honesty */
kprintf("apic: soft retfosse soft_only=1 product_gate=0 soft_ne_product=1 "
        "never_blocks_m0=1 wave=118 "
        "(retfosse honesty; Soft≠product)\n");
/* Grep: apic: soft retcoveredway — Wave 51 exclusive coveredway stamp */
kprintf("apic: soft retcoveredway exclusive=1 soft_ne_product=1 "
        "product_kernel=OPEN wave=118 "
 ...
 * Return surfaces only — soft inventory; never hard-gates product paths.
 */
/* Grep: apic: soft rettenaille — Wave 52 return-tenaille honesty */
kprintf("apic: soft rettenaille soft_only=1 product_gate=0 soft_ne_product=1 "
        "never_blocks_m0=1 wave=118 "
        "(rettenaille honesty; Soft≠product)\n");
/* Grep: apic: soft retdemilune — Wave 52 exclusive demilune stamp */
kprintf("apic: soft retdemilune exclusive=1 soft_ne_product=1 "
        "product_kernel=OPEN wave=118 "
 ...
 * Return surfaces only — soft inventory; never hard-gates product paths.
 */
/* Grep: apic: soft retravelin — Wave 53 return-travelin honesty */
kprintf("apic: soft retravelin soft_only=1 product_gate=0 soft_ne_product=1 "
        "never_blocks_m0=1 wave=118 "
        "(retravelin honesty; Soft≠product)\n");
/* Grep: apic: soft retlunette — Wave 53 exclusive lunette stamp */
kprintf("apic: soft retlunette exclusive=1 soft_ne_product=1 "
        "product_kernel=OPEN wave=118 "
 ...
 * Return surfaces only — soft inventory; never hard-gates product paths.
 */
/* Grep: apic: soft retcaponier — Wave 54 return-caponier honesty */
kprintf("apic: soft retcaponier soft_only=1 product_gate=0 soft_ne_product=1 "
        "never_blocks_m0=1 wave=118 "
        "(retcaponier honesty; Soft≠product)\n");
/* Grep: apic: soft retredan — Wave 54 exclusive redan stamp */
kprintf("apic: soft retredan exclusive=1 soft_ne_product=1 "
        "product_kernel=OPEN wave=118 "
 ...
 * Return surfaces only — soft inventory; never hard-gates product paths.
 */
/* Grep: apic: soft retflank — Wave 55 return-flank honesty */
kprintf("apic: soft retflank soft_only=1 product_gate=0 soft_ne_product=1 "
        "never_blocks_m0=1 wave=118 "
        "(retflank honesty; Soft≠product)\n");
/* Grep: apic: soft retface — Wave 55 exclusive face stamp */
kprintf("apic: soft retface exclusive=1 soft_ne_product=1 "
        "product_kernel=OPEN wave=118 "
 ...
 * Return surfaces only — soft inventory; never hard-gates product paths.
 */
/* Grep: apic: soft retgorge — Wave 56 return-gorge honesty */
kprintf("apic: soft retgorge soft_only=1 product_gate=0 soft_ne_product=1 "
        "never_blocks_m0=1 wave=118 "
        "(retgorge honesty; Soft≠product)\n");
/* Grep: apic: soft retshoulder — Wave 56 exclusive shoulder stamp */
kprintf("apic: soft retshoulder exclusive=1 soft_ne_product=1 "
        "product_kernel=OPEN wave=118 "
 ...
 * Return surfaces only — soft inventory; never hard-gates product paths.
 */
/* Grep: apic: soft retraverse — Wave 57 return-traverse honesty */
kprintf("apic: soft retraverse soft_only=1 product_gate=0 soft_ne_product=1 "
        "never_blocks_m0=1 wave=118 "
        "(retraverse honesty; Soft≠product)\n");
/* Grep: apic: soft retcasemate — Wave 57 exclusive casemate stamp */
kprintf("apic: soft retcasemate exclusive=1 soft_ne_product=1 "
        "product_kernel=OPEN wave=118 "
 ...
 * Return surfaces only — soft inventory; never hard-gates product paths.
 */
/* Grep: apic: soft retorillon — Wave 58 return-orillon honesty */
kprintf("apic: soft retorillon soft_only=1 product_gate=0 soft_ne_product=1 "
        "never_blocks_m0=1 wave=118 "
        "(retorillon honesty; Soft≠product)\n");
/* Grep: apic: soft retbonnette — Wave 58 exclusive bonnette stamp */
kprintf("apic: soft retbonnette exclusive=1 soft_ne_product=1 "
        "product_kernel=OPEN wave=118 "
 ...
 * Return surfaces only — soft inventory; never hard-gates product paths.
 */
/* Grep: apic: soft retcrownwork — Wave 59 return-crownwork honesty */
kprintf("apic: soft retcrownwork soft_only=1 product_gate=0 soft_ne_product=1 "
        "never_blocks_m0=1 wave=118 "
        "(retcrownwork honesty; Soft≠product)\n");
/* Grep: apic: soft rethornwork — Wave 59 exclusive hornwork stamp */
kprintf("apic: soft rethornwork exclusive=1 soft_ne_product=1 "
        "product_kernel=OPEN wave=118 "
 ...
 * Return surfaces only — soft inventory; never hard-gates product paths.
 */
/* Grep: apic: soft retplace — Wave 60 return-place honesty */
kprintf("apic: soft retplace soft_only=1 product_gate=0 soft_ne_product=1 "
        "never_blocks_m0=1 wave=118 "
        "(retplace honesty; Soft≠product)\n");
/* Grep: apic: soft retenvelope — Wave 60 exclusive envelope stamp */
kprintf("apic: soft retenvelope exclusive=1 soft_ne_product=1 "
        "product_kernel=OPEN wave=118 "
 ...
 * Return surfaces only — soft inventory; never hard-gates product paths.
 */
/* Grep: apic: soft retcounterguard — Wave 61 return-counterguard honesty */
kprintf("apic: soft retcounterguard soft_only=1 product_gate=0 soft_ne_product=1 "
        "never_blocks_m0=1 wave=118 "
        "(retcounterguard honesty; Soft≠product)\n");
/* Grep: apic: soft retcoveredface — Wave 61 exclusive coveredface stamp */
kprintf("apic: soft retcoveredface exclusive=1 soft_ne_product=1 "
        "product_kernel=OPEN wave=118 "
 ...
 * Return surfaces only — soft inventory; never hard-gates product paths.
 */
/* Grep: apic: soft retbastionface — Wave 62 return-bastionface honesty */
kprintf("apic: soft retbastionface soft_only=1 product_gate=0 soft_ne_product=1 "
        "never_blocks_m0=1 wave=118 "
        "(retbastionface honesty; Soft≠product)\n");
/* Grep: apic: soft retcurtainangle — Wave 62 exclusive curtainangle stamp */
kprintf("apic: soft retcurtainangle exclusive=1 soft_ne_product=1 "
        "product_kernel=OPEN wave=118 "
 ...
 * Return surfaces only — soft inventory; never hard-gates product paths.
 */
/* Grep: apic: soft retdoubletenaille — Wave 63 return-doubletenaille honesty */
kprintf("apic: soft retdoubletenaille soft_only=1 product_gate=0 soft_ne_product=1 "
        "never_blocks_m0=1 wave=118 "
        "(retdoubletenaille honesty; Soft≠product)\n");
/* Grep: apic: soft retplaceofarms — Wave 63 exclusive placeofarms stamp */
kprintf("apic: soft retplaceofarms exclusive=1 soft_ne_product=1 "
        "product_kernel=OPEN wave=118 "
  ...
  * Return surfaces only — soft inventory; never hard-gates product paths.
  */
 /* Grep: apic: soft retreentrant — Wave 64 return-reentrant honesty */
kprintf("apic: soft retreentrant soft_only=1 product_gate=0 soft_ne_product=1 "
        "never_blocks_m0=1 wave=118 "
        "(retreentrant honesty; Soft≠product)\n");
 /* Grep: apic: soft retsallyport — Wave 64 exclusive sallyport stamp */
kprintf("apic: soft retsallyport exclusive=1 soft_ne_product=1 "
        "product_kernel=OPEN wave=118 "
  ...
  * Return surfaces only — soft inventory; never hard-gates product paths.
  */
 /* Grep: apic: soft retgorgeangle — Wave 65 return-gorgeangle honesty */
kprintf("apic: soft retgorgeangle soft_only=1 product_gate=0 soft_ne_product=1 "
        "never_blocks_m0=1 wave=118 "
        "(retgorgeangle honesty; Soft≠product)\n");
 /* Grep: apic: soft retshoulderangle — Wave 65 exclusive shoulderangle stamp */
kprintf("apic: soft retshoulderangle exclusive=1 soft_ne_product=1 "
        "product_kernel=OPEN wave=118 "
  ...
  * Return surfaces only — soft inventory; never hard-gates product paths.
  */
 /* Grep: apic: soft retflankangle — Wave 66 return-flankangle honesty */
 kprintf("apic: soft retflankangle soft_only=1 product_gate=0 soft_ne_product=1 "
         "never_blocks_m0=1 wave=118 "
         "(retflankangle honesty; Soft≠product)\n");
 /* Grep: apic: soft retfaceangle — Wave 66 exclusive faceangle stamp */
 kprintf("apic: soft retfaceangle exclusive=1 soft_ne_product=1 "
         "product_kernel=OPEN wave=118 "
 ...
 * Return surfaces only — soft inventory; never hard-gates product paths.
 */
/* Grep: apic: soft retcaponierangle — Wave 67 return-caponierangle honesty */
kprintf("apic: soft retcaponierangle soft_only=1 product_gate=0 soft_ne_product=1 "
        "never_blocks_m0=1 wave=118 "
        "(retcaponierangle honesty; Soft≠product)\n");
/* Grep: apic: soft retredanangle — Wave 67 exclusive redanangle stamp */
kprintf("apic: soft retredanangle exclusive=1 soft_ne_product=1 "
        "product_kernel=OPEN wave=118 "
 ...
 * Return surfaces only — soft inventory; never hard-gates product paths.
 */
/* Grep: apic: soft retlunetteangle — Wave 68 return-lunetteangle honesty */
kprintf("apic: soft retlunetteangle soft_only=1 product_gate=0 soft_ne_product=1 "
        "never_blocks_m0=1 wave=118 "
        "(retlunetteangle honesty; Soft≠product)\n");
/* Grep: apic: soft rettenailleangle — Wave 68 exclusive tenailleangle stamp */
kprintf("apic: soft rettenailleangle exclusive=1 soft_ne_product=1 "
        "product_kernel=OPEN wave=118 "
 ...
 * Return surfaces only — soft inventory; never hard-gates product paths.
 */
/* Grep: apic: soft retdemiluneangle — Wave 69 return-demiluneangle honesty */
kprintf("apic: soft retdemiluneangle soft_only=1 product_gate=0 soft_ne_product=1 "
        "never_blocks_m0=1 wave=118 "
        "(retdemiluneangle honesty; Soft≠product)\n");
/* Grep: apic: soft retcoveredwayangle — Wave 69 exclusive coveredwayangle stamp */
kprintf("apic: soft retcoveredwayangle exclusive=1 soft_ne_product=1 "
        "product_kernel=OPEN wave=118 "
 ...
 * Return surfaces only — soft inventory; never hard-gates product paths.
 */
/* Grep: apic: soft retfosseangle — Wave 70 return-fosseangle honesty */
kprintf("apic: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retfosseangle honesty; Soft≠product)\n");
/* Grep: apic: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("apic: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retcounterscarple stamp; Soft≠product)\n");
/*
 * ---- Wave 71 exclusive complementary surfaces (never reshape primary).
 * Return surfaces only — soft inventory; never hard-gates product paths.
 */
/* Grep: apic: soft retsallyportangle — Wave 71 return-sallyportangle honesty */
kprintf("apic: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retsallyportangle honesty; Soft≠product)\n");
/* Grep: apic: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("apic: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retreentrantangle stamp; Soft≠product)\n");
/*
 * ---- Wave 72 exclusive complementary surfaces (never reshape primary).
 * Return surfaces only — soft inventory; never hard-gates product paths.
 */
/* Grep: apic: soft retplaceofarmsangle — Wave 72 return-placeofarmsangle honesty */
kprintf("apic: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retplaceofarmsangle honesty; Soft≠product)\n");
/* Grep: apic: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("apic: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: apic: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("apic: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retcurtainface honesty; Soft≠product)\n");
/* Grep: apic: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("apic: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retbastionangle stamp; Soft≠product)\n");
/* Grep: apic: soft retglacisangle — Wave 74 return-glacisangle honesty */
kprintf("apic: soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retglacisangle honesty; Soft≠product)\n");
/* Grep: apic: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
kprintf("apic: soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retparapetangle stamp; Soft≠product)\n");
/* Grep: apic: soft retmoatangle — Wave 75 return-moatangle honesty */
kprintf("apic: soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retmoatangle honesty; Soft≠product)\n");
/* Grep: apic: soft retowerangle — Wave 75 exclusive towerangle stamp */
kprintf("apic: soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retowerangle stamp; Soft≠product)\n");
/* Grep: apic: soft retgateangle — Wave 76 return-gateangle honesty */
kprintf("apic: soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retgateangle honesty; Soft≠product)\n");
/* Grep: apic: soft retwallangle — Wave 76 exclusive wallangle stamp */
kprintf("apic: soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retwallangle stamp; Soft≠product)\n");
/* Grep: apic: soft retspireangle — Wave 77 return-spireangle honesty */
kprintf("apic: soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retspireangle honesty; Soft≠product)\n");
/* Grep: apic: soft retholdangle — Wave 77 exclusive holdangle stamp */
kprintf("apic: soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retholdangle stamp; Soft≠product)\n");
/* Grep: apic: soft retpalaceangle — Wave 78 return-palaceangle honesty */
kprintf("apic: soft retpalaceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retpalaceangle honesty; Soft≠product)\n");
/* Grep: apic: soft retfortressangle — Wave 78 exclusive fortressangle stamp */
kprintf("apic: soft retfortressangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retfortressangle stamp; Soft≠product)\n");
/* Grep: apic: soft retkeepangle — Wave 79 return-keepangle honesty */
kprintf("apic: soft retkeepangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retkeepangle honesty; Soft≠product)\n");
/* Grep: apic: soft retredoubtangle — Wave 79 exclusive redoubtangle stamp */
kprintf("apic: soft retredoubtangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retredoubtangle stamp; Soft≠product)\n");
/* Grep: apic: soft retcitadelangle — Wave 80 return-citadelangle honesty */
kprintf("apic: soft retcitadelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retcitadelangle honesty; Soft≠product)\n");
/* Grep: apic: soft retbastionkeep — Wave 80 exclusive bastionkeep stamp */
kprintf("apic: soft retbastionkeep exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retbastionkeep stamp; Soft≠product)\n");
/* Grep: apic: soft retpanoplyangle — Wave 81 return-panoplyangle honesty */
kprintf("apic: soft retpanoplyangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retpanoplyangle honesty; Soft≠product)\n");
/* Grep: apic: soft retbulwarkangle — Wave 81 exclusive bulwarkangle stamp */
kprintf("apic: soft retbulwarkangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retbulwarkangle stamp; Soft≠product)\n");
/* Grep: apic: soft retmantleangle — Wave 82 return-mantleangle honesty */
kprintf("apic: soft retmantleangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retmantleangle honesty; Soft≠product)\n");
/* Grep: apic: soft retaegisangle — Wave 82 exclusive aegisangle stamp */
kprintf("apic: soft retaegisangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retaegisangle stamp; Soft≠product)\n");
/* Grep: apic: soft retemblemangle — Wave 83 return-emblemangle honesty */
kprintf("apic: soft retemblemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retemblemangle honesty; Soft≠product)\n");
/* Grep: apic: soft retsigilangle — Wave 83 exclusive sigilangle stamp */
kprintf("apic: soft retsigilangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retsigilangle stamp; Soft≠product)\n");
/* Grep: apic: soft retscepterangle — Wave 84 return-scepterangle honesty */
kprintf("apic: soft retscepterangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retscepterangle honesty; Soft≠product)\n");
/* Grep: apic: soft retglyphangle — Wave 84 exclusive glyphangle stamp */
kprintf("apic: soft retglyphangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retglyphangle stamp; Soft≠product)\n");
/* Grep: apic: soft retcrownangle — Wave 85 return-crownangle honesty */
kprintf("apic: soft retcrownangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retcrownangle honesty; Soft≠product)\n");
/* Grep: apic: soft retshardangle — Wave 85 exclusive shardangle stamp */
kprintf("apic: soft retshardangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retshardangle stamp; Soft≠product)\n");
/* Grep: apic: soft retforgeangle — Wave 86 return-forgeangle honesty */
kprintf("apic: soft retforgeangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retforgeangle honesty; Soft≠product)\n");
/* Grep: apic: soft retprismangle — Wave 86 exclusive prismangle stamp */
kprintf("apic: soft retprismangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retprismangle stamp; Soft≠product)\n");
/* Grep: apic: soft retflameangle — Wave 87 return-flameangle honesty */
kprintf("apic: soft retflameangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retflameangle honesty; Soft≠product)\n");
/* Grep: apic: soft retcipherangle — Wave 87 exclusive cipherangle stamp */
kprintf("apic: soft retcipherangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retcipherangle stamp; Soft≠product)\n");
/* Grep: apic: soft retbeaconangle — Wave 88 return-beaconangle honesty */
kprintf("apic: soft retbeaconangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retbeaconangle honesty; Soft≠product)\n");
/* Grep: apic: soft retledgerangle — Wave 88 exclusive ledgerangle stamp */
kprintf("apic: soft retledgerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retledgerangle stamp; Soft≠product)\n");
/* Grep: apic: soft retbannerangle — Wave 89 return-bannerangle honesty */
kprintf("apic: soft retbannerangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retbannerangle honesty; Soft≠product)\n");
/* Grep: apic: soft retvaultangle — Wave 89 exclusive vaultangle stamp */
kprintf("apic: soft retvaultangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retvaultangle stamp; Soft≠product)\n");
/* Grep: apic: soft retcrestangle — Wave 90 return-crestangle honesty */
kprintf("apic: soft retcrestangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retcrestangle honesty; Soft≠product)\n");
/* Grep: apic: soft rettokenangle — Wave 90 exclusive tokenangle stamp */
kprintf("apic: soft rettokenangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (rettokenangle stamp; Soft≠product)\n");
/* Grep: apic: soft retbadgeangle — Wave 91 return-badgeangle honesty */
kprintf("apic: soft retbadgeangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retbadgeangle honesty; Soft≠product)\n");
/* Grep: apic: soft retphaseangle — Wave 91 exclusive phaseangle stamp */
kprintf("apic: soft retphaseangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retphaseangle stamp; Soft≠product)\n");
/* Grep: apic: soft retmarkangle — Wave 92 return-markangle honesty */
kprintf("apic: soft retmarkangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retmarkangle honesty; Soft≠product)\n");
/* Grep: apic: soft retpulseangle — Wave 92 exclusive pulseangle stamp */
kprintf("apic: soft retpulseangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retpulseangle stamp; Soft≠product)\n");

/* Grep: apic: soft retsealangle — Wave 93 return-sealangle honesty */
kprintf("apic: soft retsealangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retsealangle honesty; Soft≠product)\n");
/* Grep: apic: soft retboundangle — Wave 93 exclusive boundangle stamp */
kprintf("apic: soft retboundangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retboundangle stamp; Soft≠product)\n");
/* Grep: apic: soft retstemangle — Wave 94 return-stemangle honesty */
kprintf("apic: soft retstemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retstemangle honesty; Soft≠product)\n");
/* Grep: apic: soft retbladeangle — Wave 94 exclusive bladeangle stamp */
kprintf("apic: soft retbladeangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retbladeangle stamp; Soft≠product)\n");
/* Grep: apic: soft retchordangle — Wave 95 return-chordangle honesty */
kprintf("apic: soft retchordangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retchordangle honesty; Soft≠product)\n");
/* Grep: apic: soft retarcangle — Wave 95 exclusive arcangle stamp */
kprintf("apic: soft retarcangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retarcangle stamp; Soft≠product)\n");
/* Grep: apic: soft retsectorangle — Wave 96 return-sectorangle honesty */
kprintf("apic: soft retsectorangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retsectorangle honesty; Soft≠product)\n");
/* Grep: apic: soft retwedgeangle — Wave 96 exclusive wedgeangle stamp */
kprintf("apic: soft retwedgeangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retwedgeangle stamp; Soft≠product)\n");
/* Grep: apic: soft retradiusangle — Wave 97 return-radiusangle honesty */
kprintf("apic: soft retradiusangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retradiusangle honesty; Soft≠product)\n");
/* Grep: apic: soft retdiameterangle — Wave 97 exclusive diameterangle stamp */
kprintf("apic: soft retdiameterangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retdiameterangle stamp; Soft≠product)\n");
/* Grep: apic: soft retcircumangle — Wave 98 return-circumangle honesty */
kprintf("apic: soft retcircumangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retcircumangle honesty; Soft≠product)\n");
/* Grep: apic: soft retellipseangle — Wave 98 exclusive ellipseangle stamp */
kprintf("apic: soft retellipseangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retellipseangle stamp; Soft≠product)\n");
/* Grep: apic: soft rethyperangle — Wave 99 return-hyperangle honesty */
kprintf("apic: soft rethyperangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (rethyperangle honesty; Soft≠product)\n");
/* Grep: apic: soft retparabolaangle — Wave 99 exclusive parabolaangle stamp */
kprintf("apic: soft retparabolaangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retparabolaangle stamp; Soft≠product)\n");
/* Grep: apic: soft retspiralangle — Wave 100 return-spiralangle honesty */
kprintf("apic: soft retspiralangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retspiralangle honesty; Soft≠product)\n");
/* Grep: apic: soft rethelixangle — Wave 100 exclusive helixangle stamp */
kprintf("apic: soft rethelixangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (rethelixangle stamp; Soft≠product)\n");
/* Grep: apic: soft rettorusangle — Wave 101 return-torusangle honesty */
kprintf("apic: soft rettorusangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (rettorusangle honesty; Soft≠product)\n");
/* Grep: apic: soft retknotangle — Wave 101 exclusive knotangle stamp */
kprintf("apic: soft retknotangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retknotangle stamp; Soft≠product)\n");
/* Grep: apic: soft retmoebiusangle — Wave 102 return-moebiusangle honesty */
kprintf("apic: soft retmoebiusangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retmoebiusangle honesty; Soft≠product)\n");
/* Grep: apic: soft retkleinangle — Wave 102 exclusive kleinangle stamp */
kprintf("apic: soft retkleinangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retkleinangle stamp; Soft≠product)\n");
/* Grep: apic: soft retprojectangle — Wave 103 return-projectangle honesty */
kprintf("apic: soft retprojectangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retprojectangle honesty; Soft≠product)\n");
/* Grep: apic: soft retaffineangle — Wave 103 exclusive affineangle stamp */
kprintf("apic: soft retaffineangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retaffineangle stamp; Soft≠product)\n");
/* Grep: apic: soft retlinearangle — Wave 104 return-linearangle honesty */
kprintf("apic: soft retlinearangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retlinearangle honesty; Soft≠product)\n");
/* Grep: apic: soft retbilinearangle — Wave 104 exclusive bilinearangle stamp */
kprintf("apic: soft retbilinearangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retbilinearangle stamp; Soft≠product)\n");
/* Grep: apic: soft retquadraticangle — Wave 105 return-quadraticangle honesty */
kprintf("apic: soft retquadraticangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retquadraticangle honesty; Soft≠product)\n");
/* Grep: apic: soft retcubicangle — Wave 105 exclusive cubicangle stamp */
kprintf("apic: soft retcubicangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retcubicangle stamp; Soft≠product)\n");
/* Grep: apic: soft retquarticangle — Wave 106 return-quarticangle honesty */
kprintf("apic: soft retquarticangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retquarticangle honesty; Soft≠product)\n");
/* Grep: apic: soft retquinticangle — Wave 106 exclusive quinticangle stamp */
kprintf("apic: soft retquinticangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retquinticangle stamp; Soft≠product)\n");
/* Grep: apic: soft retsplineangle — Wave 107 return-splineangle honesty */
kprintf("apic: soft retsplineangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retsplineangle honesty; Soft≠product)\n");
/* Grep: apic: soft retbezierangle — Wave 107 exclusive bezierangle stamp */
kprintf("apic: soft retbezierangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retbezierangle stamp; Soft≠product)\n");
/* Grep: apic: soft rethurmitangle — Wave 108 return-hermitangle honesty */
kprintf("apic: soft rethurmitangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (rethurmitangle honesty; Soft≠product)\n");
/* Grep: apic: soft retcatmullangle — Wave 108 exclusive catmullangle stamp */
kprintf("apic: soft retcatmullangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retcatmullangle stamp; Soft≠product)\n");
/* Grep: apic: soft retnurbsangle — Wave 109 return-nurbsangle honesty */
kprintf("apic: soft retnurbsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retnurbsangle honesty; Soft≠product)\n");
/* Grep: apic: soft retbsplineangle — Wave 109 exclusive bsplineangle stamp */
kprintf("apic: soft retbsplineangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retbsplineangle stamp; Soft≠product)\n");
/* Grep: apic: soft retmeshangle — Wave 110 return-meshangle honesty */
kprintf("apic: soft retmeshangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retmeshangle honesty; Soft≠product)\n");
/* Grep: apic: soft retgridangle — Wave 110 exclusive gridangle stamp */
kprintf("apic: soft retgridangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retgridangle stamp; Soft≠product)\n");
/* Grep: apic: soft retvoxelangle — Wave 111 return-voxelangle honesty */
kprintf("apic: soft retvoxelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retvoxelangle honesty; Soft≠product)\n");
/* Grep: apic: soft rettexelangle — Wave 111 exclusive texelangle stamp */
kprintf("apic: soft rettexelangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (rettexelangle stamp; Soft≠product)\n");
/* Grep: apic: soft retfragmentangle — Wave 112 return-fragmentangle honesty */
kprintf("apic: soft retfragmentangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retfragmentangle honesty; Soft≠product)\n");
/* Grep: apic: soft retvertexangle — Wave 112 exclusive vertexangle stamp */
kprintf("apic: soft retvertexangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retvertexangle stamp; Soft≠product)\n");
/* Grep: apic: soft retshaderangle — Wave 113 return-shaderangle honesty */
kprintf("apic: soft retshaderangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retshaderangle honesty; Soft≠product)\n");
/* Grep: apic: soft retpipelineangle — Wave 113 exclusive pipelineangle stamp */
kprintf("apic: soft retpipelineangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retpipelineangle stamp; Soft≠product)\n");
/* Grep: apic: soft retframebufferangle — Wave 114 return-framebufferangle honesty */
kprintf("apic: soft retframebufferangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retframebufferangle honesty; Soft≠product)\n");
/* Grep: apic: soft retswapchainangle — Wave 114 exclusive swapchainangle stamp */
kprintf("apic: soft retswapchainangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retswapchainangle stamp; Soft≠product)\n");
/* Grep: apic: soft retpresentangle — Wave 115 return-presentangle honesty */
kprintf("apic: soft retpresentangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retpresentangle honesty; Soft≠product)\n");
/* Grep: apic: soft retvsyncangle — Wave 115 exclusive vsyncangle stamp */
kprintf("apic: soft retvsyncangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retvsyncangle stamp; Soft≠product)\n");
/* Grep: apic: soft retfenceangle — Wave 116 return-fenceangle honesty */
kprintf("apic: soft retfenceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retfenceangle honesty; Soft≠product)\n");
/* Grep: apic: soft retsemaphoreangle — Wave 116 exclusive semaphoreangle stamp */
kprintf("apic: soft retsemaphoreangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retsemaphoreangle stamp; Soft≠product)\n");
/* Grep: apic: soft retmutexangle — Wave 117 return-mutexangle honesty */
kprintf("apic: soft retmutexangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retmutexangle honesty; Soft≠product)\n");
/* Grep: apic: soft retcondangle — Wave 117 exclusive condangle stamp */
kprintf("apic: soft retcondangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retcondangle stamp; Soft≠product)\n");
/* Grep: apic: soft retbarrierangle — Wave 118 return-barrierangle honesty */
kprintf("apic: soft retbarrierangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=118 (retbarrierangle honesty; Soft≠product)\n");
/* Grep: apic: soft retatomicangle — Wave 118 exclusive atomicangle stamp */
kprintf("apic: soft retatomicangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=118 (retatomicangle stamp; Soft≠product)\n");
/* Grep: apic: soft retqueueangle — Wave 119 return-queueangle honesty */
kprintf("apic: soft retqueueangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=119 (retqueueangle honesty; Soft≠product)\n");
/* Grep: apic: soft reteventangle — Wave 119 exclusive eventangle stamp */
kprintf("apic: soft reteventangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=119 (reteventangle stamp; Soft≠product)\n");
/* Grep: apic: soft retchannelangle — Wave 120 return-channelangle honesty */
kprintf("apic: soft retchannelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=120 (retchannelangle honesty; Soft≠product)\n");
/* Grep: apic: soft retmailboxangle — Wave 120 exclusive mailboxangle stamp */
kprintf("apic: soft retmailboxangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=120 (retmailboxangle stamp; Soft≠product)\n");
/* Grep: apic: soft retstreamangle — Wave 121 return-streamangle honesty */
kprintf("apic: soft retstreamangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=121 (retstreamangle honesty; Soft≠product)\n");
/* Grep: apic: soft retpacketangle — Wave 121 exclusive packetangle stamp */
kprintf("apic: soft retpacketangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=121 (retpacketangle stamp; Soft≠product)\n");
/* Grep: apic: soft retframeangle — Wave 122 return-frameangle honesty */
kprintf("apic: soft retframeangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=122 (retframeangle honesty; Soft≠product)\n");
/* Grep: apic: soft retwindowangle — Wave 122 exclusive windowangle stamp */
kprintf("apic: soft retwindowangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=122 (retwindowangle stamp; Soft≠product)\n");
/* Grep: apic: soft retlayerangle — Wave 123 return-layerangle honesty */
kprintf("apic: soft retlayerangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=123 (retlayerangle honesty; Soft≠product)\n");
/* Grep: apic: soft retcanvasangle — Wave 123 exclusive canvasangle stamp */
kprintf("apic: soft retcanvasangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=123 (retcanvasangle stamp; Soft≠product)\n");
/* Grep: apic: soft retbrushangle — Wave 124 return-brushangle honesty */
kprintf("apic: soft retbrushangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=124 (retbrushangle honesty; Soft≠product)\n");
/* Grep: apic: soft retinkangle — Wave 124 exclusive inkangle stamp */
kprintf("apic: soft retinkangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=124 (retinkangle stamp; Soft≠product)\n");
/* Grep: apic: soft retpaletteangle — Wave 125 return-paletteangle honesty */
kprintf("apic: soft retpaletteangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=125 (retpaletteangle honesty; Soft≠product)\n");
/* Grep: apic: soft retstrokeangle — Wave 125 exclusive strokeangle stamp */
kprintf("apic: soft retstrokeangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=125 (retstrokeangle stamp; Soft≠product)\n");
/* Grep: apic: soft retgradientangle — Wave 126 return-gradientangle honesty */
kprintf("apic: soft retgradientangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=126 (retgradientangle honesty; Soft≠product)\n");
/* Grep: apic: soft retblendangle — Wave 126 exclusive blendangle stamp */
kprintf("apic: soft retblendangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=126 (retblendangle stamp; Soft≠product)\n");
