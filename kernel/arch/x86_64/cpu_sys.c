...
#define GJ_CPU_SYSCALL_SOFT_WAVE 83u
...
kprintf("cpu: syscall: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: cpu: syscall: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("cpu: syscall: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("cpu: syscall: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: cpu: syscall: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("cpu: syscall: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("cpu: syscall: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: cpu: syscall: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("cpu: syscall: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: cpu: syscall: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("cpu: syscall: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retcurtainface honesty; Soft≠product; not bar3)\n");
/* Grep: cpu: syscall: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("cpu: syscall: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retbastionangle stamp; Soft≠product)\n");
/* Grep: cpu: syscall: soft retglacisangle — Wave 74 return-glacisangle honesty */
kprintf("cpu: syscall: soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retglacisangle honesty; Soft≠product; not bar3)\n");
/* Grep: cpu: syscall: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
kprintf("cpu: syscall: soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retparapetangle stamp; Soft≠product)\n");
/* Grep: cpu: syscall: soft retmoatangle — Wave 75 return-moatangle honesty */
kprintf("cpu: syscall: soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retmoatangle honesty; Soft≠product; not bar3)\n");
/* Grep: cpu: syscall: soft retowerangle — Wave 75 exclusive towerangle stamp */
kprintf("cpu: syscall: soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retowerangle stamp; Soft≠product)\n");
/* Grep: cpu: syscall: soft retgateangle — Wave 76 return-gateangle honesty */
kprintf("cpu: syscall: soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retgateangle honesty; Soft≠product; not bar3)\n");
/* Grep: cpu: syscall: soft retwallangle — Wave 76 exclusive wallangle stamp */
kprintf("cpu: syscall: soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retwallangle stamp; Soft≠product)\n");
/* Grep: cpu: syscall: soft retspireangle — Wave 77 return-spireangle honesty */
kprintf("cpu: syscall: soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retspireangle honesty; Soft≠product; not bar3)\n");
/* Grep: cpu: syscall: soft retholdangle — Wave 77 exclusive holdangle stamp */
kprintf("cpu: syscall: soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retholdangle stamp; Soft≠product)\n");
/* Grep: cpu: syscall: soft retpalaceangle — Wave 78 return-palaceangle honesty */
kprintf("cpu: syscall: soft retpalaceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retpalaceangle honesty; Soft≠product; not bar3)\n");
/* Grep: cpu: syscall: soft retfortressangle — Wave 78 exclusive fortressangle stamp */
kprintf("cpu: syscall: soft retfortressangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retfortressangle stamp; Soft≠product)\n");
/* Grep: cpu: syscall: soft retkeepangle — Wave 79 return-keepangle honesty */
kprintf("cpu: syscall: soft retkeepangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retkeepangle honesty; Soft≠product; not bar3)\n");
/* Grep: cpu: syscall: soft retredoubtangle — Wave 79 exclusive redoubtangle stamp */
kprintf("cpu: syscall: soft retredoubtangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retredoubtangle stamp; Soft≠product)\n");
/* Grep: cpu: syscall: soft retcitadelangle — Wave 80 return-citadelangle honesty */
kprintf("cpu: syscall: soft retcitadelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retcitadelangle honesty; Soft≠product; not bar3)\n");
/* Grep: cpu: syscall: soft retbastionkeep — Wave 80 exclusive bastionkeep stamp */
kprintf("cpu: syscall: soft retbastionkeep exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retbastionkeep stamp; Soft≠product)\n");
/* Grep: cpu: syscall: soft retpanoplyangle — Wave 81 return-panoplyangle honesty */
kprintf("cpu: syscall: soft retpanoplyangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retpanoplyangle honesty; Soft≠product; not bar3)\n");
/* Grep: cpu: syscall: soft retbulwarkangle — Wave 81 exclusive bulwarkangle stamp */
kprintf("cpu: syscall: soft retbulwarkangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retbulwarkangle stamp; Soft≠product)\n");
/* Grep: cpu: syscall: soft retmantleangle — Wave 82 return-mantleangle honesty */
kprintf("cpu: syscall: soft retmantleangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retmantleangle honesty; Soft≠product; not bar3)\n");
/* Grep: cpu: syscall: soft retaegisangle — Wave 82 exclusive aegisangle stamp */
kprintf("cpu: syscall: soft retaegisangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retaegisangle stamp; Soft≠product)\n");
/* Grep: cpu: syscall: soft retemblemangle — Wave 83 return-emblemangle honesty */
kprintf("cpu: syscall: soft retemblemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retemblemangle honesty; Soft≠product; not bar3)\n");
/* Grep: cpu: syscall: soft retsigilangle — Wave 83 exclusive sigilangle stamp */
kprintf("cpu: syscall: soft retsigilangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retsigilangle stamp; Soft≠product)\n");
