 ...
 *   "cpu: syscall soft residual lean ..."  (enter_user / enter_user32)
 *   "cpu: syscall soft residual lean enter64 ..."  (once; UDX host land)
 *   "cpu: syscall soft residual lean enter32 ..."  (once; PE32/compat)
 ...
 *   "cpu: syscall soft deepen ..."    (lean area catalog only)
 ...
 * greppable: cpu: syscall soft residual lean enter64
 * greppable: cpu: syscall soft residual lean enter32
 *
 * Lean soft residual enter_user / enter_user32 (this unit exclusive):
 *   Functional residual: soft tallies + inventory residual + once-lamps
 *   at first successful enter64 (sysretq) / enter32 (iretq CS32).
 *   Preserve enter_user / enter_user32 product paths (sysretq / iretq CS32).
 *   Userspace UDX hosts land via enter_user; residual lean is Soft!=product.
 *   hosts=rtl8168_udx|xhci_udx|ddi_host · product=UDX+ABI · freestanding=SKIP.
 *   Hard-capped inventory (GJ_CPU_SYSCALL_SOFT_INV_LOG_CAP=2): storm=0.
 *   Enter residual once-lamps: once=1 each path (no stamp storms).
 *   No version stamp, no ret*angle catalog residue.
 ...
 *   Integrity rollup on residual lean (verify+SCE+STAR+LSTAR lamps).
 *   via=inv|enter64|enter32 residual path tags (silent tallies).
 *   dual_dod_a=OPEN dual_dod_b=OPEN honesty (UDX land residual != DoD close).
 *   residual lean PASS once when integrity=1 (not product enter complete).
 *   verdict=PASS|ARMED|FAIL|IDLE on residual lean inventory line.
 *   Never hard-gates product; no GJ_IMAGE_VERSION stamp; storm=0.
 *   greppable: cpu: syscall soft residual lean
 *   greppable: cpu: syscall soft residual lean PASS
 *   greppable: cpu: syscall soft residual lean enter64|enter32
 ...
 * greppable: cpu: syscall soft residual lean
 * greppable: Soft!=product G-AC-1 udx_host product=UDX+ABI
 */
#include <gj/cpu.h>
...
/*
 * Lean soft residual serial budget (storm cap inventory).
 * Full multi-line inventory is hard-capped so soft_log re-entry cannot
 * stamp-storm serial/stack. Soft!=product. Residual lean covers
 * enter_user / enter_user32 for userspace UDX hosts only (this unit).
 * Enter residual once-lamps (g_fSoftEnter*ResLeanOnce) are separate from
 * inventory cap - fire once per path at first successful land.
 * greppable: cpu: syscall soft residual lean | GJ_CPU_SYSCALL_SOFT_INV_LOG_CAP
 * greppable: cpu: syscall soft residual lean enter64 | enter32
 */
#define GJ_CPU_SYSCALL_SOFT_INV_LOG_CAP 2u
...
static volatile u32 g_u32SoftInvSkip;     /* capped inventory skips (storm=0) */
...
/* Residual lean once-lamps (Soft!=product; first successful enter only). */
static u8 g_fSoftEnter64ResLeanOnce;     /* enter64 residual lean fired */
static u8 g_fSoftEnter32ResLeanOnce;     /* enter32 residual lean fired */
/* C0 residual lean path tallies (Soft!=product; never hard-gate). */
static volatile u32 g_u32SoftResLeanN;       /* residual lean emits total */
static volatile u32 g_u32SoftResLeanInv;     /* via=inv residual lean */
static volatile u32 g_u32SoftResLeanEnter64; /* via=enter64 residual lean */
static volatile u32 g_u32SoftResLeanEnter32; /* via=enter32 residual lean */
static volatile u32 g_u32SoftResLeanPass;    /* residual lean PASS emits */
static u8 g_fSoftResLeanPassOnce;            /* residual lean PASS once */
...
static u32 cpu_syscall_soft_res_integrity(void);
static const char *cpu_syscall_soft_res_verdict(u32 u32Integrity);
...
/**
 * C0 residual: derived integrity for SYSCALL soft residual lean.
 * Soft!=product; never hard-gates product enter / MSR program paths.
 * Requires live snapshot + ready + last verify ok + SCE + STAR layout +
 * LSTAR non-zero + per-field expect lamps from soft verify.
 * greppable: integrity= on residual lean
 */
static u32
cpu_syscall_soft_res_integrity(void)
{
    u32 u32Sce;
    u32 u32LstarNz;
    u32 u32StarMatch;
    ...
    u32StarMatch =
        (u16KernCs == (u16)GJ_CPU_SYSCALL_STAR_KERNEL_CS &&
         u16UserBase == (u16)GJ_CPU_SYSCALL_STAR_USER_BASE)
            ? 1u
            : 0u;
    if (g_SoftSnap.u8VerifyOk != 0u && u32Sce != 0u &&
        u32StarMatch != 0u && u32LstarNz != 0u &&
        g_u8SoftSceOk != 0u && g_u8SoftKernCsOk != 0u &&
        g_u8SoftUserBaseOk != 0u && g_u8SoftSysretCsOk != 0u &&
        g_u8SoftStarOk != 0u && g_u8SoftLstarOk != 0u &&
        g_u8SoftSfmaskOk != 0u) {
        return 1u;
    }
    return 0u;
...
/**
 * C0 residual: residual lean verdict string (Soft!=product honesty only).
 * PASS = integrity=1; FAIL = verify_bad seen; ARMED = live+ready no PASS;
 * IDLE = MSRs not programmed. Never product enter complete.
 */
static const char *
cpu_syscall_soft_res_verdict(u32 u32Integrity)
{
    if (!g_fSoftSnapLive) {
        return "IDLE";
    }
 ...
 * Lean residual enter_user/enter_user32: one honesty line, no stamp storms.
 * Hard-capped (GJ_CPU_SYSCALL_SOFT_INV_LOG_CAP) - soft_log re-entry skips.
 */
static void
cpu_syscall_soft_inventory(void)
{
    ...
    if (g_u32SoftInvLogs >= GJ_CPU_SYSCALL_SOFT_INV_LOG_CAP) {
        cpu_syscall_soft_inc(&g_u32SoftInvSkip);
        return;
    }
    ...
    /* Grep: cpu: syscall soft capacity (geometry + storm cap inventory) */
    kprintf("cpu: syscall soft capacity star_kern=0x%x star_user=0x%x "
            "user_cs32=0x%x user_cs64=0x%x user_ds=0x%x "
            "sfmask=0x%lx rflags_if=0x%lx inv_logs=%u inv_skip=%u "
            "cap=%u storm=0 wave=%u Soft!=product\n",
            (unsigned)GJ_CPU_SYSCALL_STAR_KERNEL_CS,
            (unsigned)GJ_CPU_SYSCALL_STAR_USER_BASE,
            ...
            g_u32SoftInvSkip, (unsigned)GJ_CPU_SYSCALL_SOFT_INV_LOG_CAP,
            (unsigned)GJ_CPU_SYSCALL_SOFT_WAVE);
    ...
    kprintf("cpu: syscall soft exclusive exclusive=1 soft=1 "
    ...
    kprintf("cpu: syscall soft ratio inits=%u enter64=%u enter32=%u "
            ...
            "info_get=%u log_call=%u inv_logs=%u inv_skip=%u "
            "ready=%u live=%u wave=%u\n",
    ...
    kprintf("cpu: syscall soft honesty soft_ne_product_complete=1 "
            "hard_gate=0 soft_only=1 storm=0 "
            "claim=star+lstar+sfmask+sce+enter64+enter32 "
            "dual_dod_a=OPEN dual_dod_b=OPEN product_complete=0 "
            "wave=%u unit=cpu_sys.c\n",
            (unsigned)GJ_CPU_SYSCALL_SOFT_WAVE);

    /*
     * Lean soft residual enter_user / enter_user32 for userspace UDX hosts
     * (Soft!=product). Functional residual - no stamp storms, no version
     * stamp. Preserves enter_user32 product path (iretq CS32 / USER_DS /
     * RSP0). G-AC-1: no Linux .ko product AC from this unit.
     * Dual DoD land residual: hosts=rtl8168_udx|xhci_udx|ddi_host via enter64;
     * Dual DoD A/B remain OPEN (UDX host land residual != DoD close).
     * C0 residual deepen: integrity + via=inv + verdict + residual lean PASS.
     * greppable: cpu: syscall soft residual lean
     * greppable: cpu: syscall soft residual lean PASS
     * greppable: dual_dod_a=OPEN dual_dod_b=OPEN
     */
    {
        ...
        u32Integrity = cpu_syscall_soft_res_integrity();
        u32StarMatchRes = u32StarMatchExpect;
        u32LstarNzRes = u32LstarNz;
        u32EnterLanded =
            (g_u32SoftEnter64 != 0u || g_u32SoftEnter32 != 0u) ? 1u : 0u;
        /* thr prefer: thr path used when any enter64 (soft ratio lamp). */
        if (g_u32SoftEnter64 == 0u) {
            u32ThrPrefer = 0u;
        } else if (g_u32SoftEnter64Thr >= g_u32SoftEnter64Irq) {
            u32ThrPrefer = 1u;
        } else {
            u32ThrPrefer = 0u;
        }
        szVerdict = cpu_syscall_soft_res_verdict(u32Integrity);

        cpu_syscall_soft_inc(&g_u32SoftResLeanN);
        cpu_syscall_soft_inc(&g_u32SoftResLeanInv);

        /* Grep: cpu: syscall soft residual lean */
        kprintf("cpu: syscall soft residual lean via=inv "
                "verdict=%s integrity=%u "
                "enter64=%u enter32=%u bad=%u "
                "enter64_thr=%u enter64_irq=%u enter32_thr=%u "
                "enter32_norsp0=%u bad_ready=%u bad_zero=%u "
                "sysretq=1 iretq32=1 user_cs64=0x%x user_cs32=0x%x "
                "user_ds=0x%x rflags_if=0x%lx ready=%u live=%u "
                "star_match=%u lstar_nz=%u sce=%u verify_ok=%u "
                "enter_landed=%u thr_prefer=%u "
                "inv=%u skip=%u cap=%u hard_gate=0 storm=0 "
                "once64=%u once32=%u "
                "lean_n=%u lean_inv=%u lean_e64=%u lean_e32=%u lean_pass=%u "
                "hosts=rtl8168_udx|xhci_udx|ddi_host "
                "product=UDX+ABI freestanding_class=SKIP "
                "dual_dod_a=OPEN dual_dod_b=OPEN "
                "udx_host=1 g_ac1=1 soft_ne_product=1 product_complete=0 "
                "dual=MIT_OR_Apache-2.0 unit=cpu_sys.c "
                "(Soft!=product; dual MIT OR Apache-2.0; G-AC-1; "
                "C0 residual deepen; Dual DoD A/B OPEN; "
                "no version stamp; preserve enter_user/enter_user32; "
                "userspace UDX host ring-3 enter residual; "
                "not product enter complete; not Dual DoD close)\n",
                szVerdict, u32Integrity,
                g_u32SoftEnter64, g_u32SoftEnter32, g_u32SoftEnterBad,
                g_u32SoftEnter64Thr, g_u32SoftEnter64Irq,
                g_u32SoftEnter32Thr, g_u32SoftEnter32NoRsp0,
                g_u32SoftEnterBadReady, g_u32SoftEnterBadZero,
                (unsigned)GJ_CPU_SOFT_USER_CS64,
                (unsigned)GJ_CPU_SOFT_USER_CS32,
                (unsigned)GJ_CPU_SOFT_USER_DS,
                (unsigned long)GJ_CPU_SOFT_RFLAGS_IF,
                (unsigned)(g_fSyscallReady ? 1u : 0u),
                (unsigned)(g_fSoftSnapLive ? 1u : 0u),
                u32StarMatchRes, u32LstarNzRes, u32Sce,
                (unsigned)g_SoftSnap.u8VerifyOk, u32EnterLanded,
                u32ThrPrefer, g_u32SoftInvLogs, g_u32SoftInvSkip,
                (unsigned)GJ_CPU_SYSCALL_SOFT_INV_LOG_CAP,
                (unsigned)g_fSoftEnter64ResLeanOnce,
                (unsigned)g_fSoftEnter32ResLeanOnce,
                g_u32SoftResLeanN, g_u32SoftResLeanInv,
                g_u32SoftResLeanEnter64, g_u32SoftResLeanEnter32,
                g_u32SoftResLeanPass);

        /*
         * Grep: cpu: syscall soft residual lean PASS
         * C0 residual: once when integrity=1 (Soft!=product honesty).
         * Not product enter complete; Dual DoD A/B remain OPEN.
         */
        if (u32Integrity != 0u && g_fSoftResLeanPassOnce == 0u) {
            g_fSoftResLeanPassOnce = 1u;
            cpu_syscall_soft_inc(&g_u32SoftResLeanPass);
            kprintf("cpu: syscall soft residual lean PASS via=inv "
                    "integrity=1 verdict=PASS "
                    "ready=1 live=1 sce=1 star_match=1 lstar_nz=1 "
                    "verify_ok=1 enter_landed=%u thr_prefer=%u "
                    "once64=%u once32=%u "
                    "hosts=rtl8168_udx|xhci_udx|ddi_host "
                    "product=UDX+ABI freestanding_class=SKIP "
                    "dual_dod_a=OPEN dual_dod_b=OPEN "
                    "udx_host=1 g_ac1=1 soft_ne_product=1 "
                    "product_complete=0 once=1 storm=0 hard_gate=0 "
                    "dual=MIT_OR_Apache-2.0 unit=cpu_sys.c "
                    "(Soft!=product; C0 residual lean PASS; "
                    "MSR soft integrity residual only; "
                    "not product enter complete; Dual DoD A/B OPEN)\n",
                    u32EnterLanded, u32ThrPrefer,
                    (unsigned)g_fSoftEnter64ResLeanOnce,
                    (unsigned)g_fSoftEnter32ResLeanOnce);
        }
    }
    ...
    /*
     * Grep: cpu: syscall soft deepen - lean area catalog only.
     * No ret*angle / retclass / retmap stamp-storm residue.
     * residual_lean + residual_enter64/32 cover UDX host land honesty.
     */
    kprintf("cpu: syscall soft deepen wave=%u ready=%u live=%u "
            "inits=%u enter64=%u enter32=%u verify_ok=%u "
            "inv_logs=%u inv_skip=%u cap=%u "
            "areas=inventory,program,star,lstar,cstar,sfmask,efer,"
            "enter,verify,expect,path,geom,msr,flags,init,capacity,"
            "exclusive,claim,ratio,honesty,residual_lean,"
            "residual_enter64,residual_enter32,residual_lean_pass,"
            "residual_integrity,residual_via,residual_dual_dod "
            "unit=cpu_sys.c only hard_gate=0 storm=0 soft_ne_product=1 "
            "udx_host=1 g_ac1=1 product=UDX+ABI freestanding_class=SKIP "
            "dual_dod_a=OPEN dual_dod_b=OPEN product_complete=0 "
            "(lean residual enter_user/user32 UDX hosts; Soft!=product; "
            "C0 residual deepen; Dual DoD A/B OPEN; G-AC-1; "
...
cpu_enter_user_prepare(u64 u64Entry, u64 u64Stack)
{
    ...
    /*
     * Product: long-mode ring-3 via SYSRETQ (userspace UDX host / user task).
     * Soft residual: enter64 tallies + once residual lean - Soft!=product;
     * never hard-gates product sysretq path.
     */
     ...
     * Soft!=product; product path is still sysretq below.
     * C0 residual: via=enter64 + Dual DoD OPEN honesty.
     */
    if (g_fSoftEnter64ResLeanOnce == 0u) {
        u32 u32IntegrityEnter;

        g_fSoftEnter64ResLeanOnce = 1u;
        cpu_syscall_soft_inc(&g_u32SoftResLeanN);
        cpu_syscall_soft_inc(&g_u32SoftResLeanEnter64);
        u32IntegrityEnter = cpu_syscall_soft_res_integrity();
        kprintf("cpu: syscall soft residual lean enter64 via=enter64 "
                "sysretq=1 swapgs=1 thr_kstack=%u "
                "user_cs64=0x%x rflags_if=0x%lx ready=1 "
                "integrity=%u verdict=%s "
                "hosts=rtl8168_udx|xhci_udx|ddi_host "
                "product=UDX+ABI freestanding_class=SKIP "
                "dual_dod_a=OPEN dual_dod_b=OPEN "
                "udx_host=1 g_ac1=1 soft_ne_product=1 product_complete=0 "
                "once=1 storm=0 hard_gate=0 dual=MIT_OR_Apache-2.0 "
                "unit=cpu_sys.c "
                "(Soft!=product; C0 residual; UDX host ring-3 land residual; "
                "not product enter complete; Dual DoD A/B OPEN)\n",
                u32ThrKstack,
                (unsigned)GJ_CPU_SOFT_USER_CS64,
                (unsigned long)GJ_CPU_SOFT_RFLAGS_IF,
...
cpu_enter_user32(u64 u64Entry, u64 u64Stack)
{
    ...
    /*
     * Product: long mode -> 32-bit compat user via iretq:
     *   SS=USER_DS, RSP=stack, RFLAGS=IF, CS=USER_CS32, RIP=entry
     * Interrupt-return path only; SYSCALL MSRs are not required.
     * Soft residual: GDT-matched constants only (0x23/0x1b/0x200).
     * Soft!=product; preserve product path for PE32/compat smoke.
     */
    if (u64Entry == 0 || u64Stack == 0) {
        cpu_syscall_soft_inc(&g_u32SoftEnterBadZero);
        cpu_syscall_soft_inc(&g_u32SoftEnterBad);
    ...
    kprintf("cpu: enter_user32 rip=0x%lx rsp=0x%lx cs32=0x%x rsp0=0x%lx\n",
            (unsigned long)u64Entry, (unsigned long)u64Stack,
            (unsigned)GJ_CPU_SOFT_USER_CS32,
            (unsigned long)u64Rsp0);
    /*
     * Grep: cpu: syscall soft residual lean enter32
     * First successful PE32/compat iretq land (once only; storm=0).
     * SYSCALL MSRs not required for this path (sce_ready independent).
     * Soft!=product; product path is still iretq below.
     * C0 residual: via=enter32 + Dual DoD OPEN honesty (PE32 path; not UDX).
     */
    if (g_fSoftEnter32ResLeanOnce == 0u) {
        u32 u32IntegrityEnter32;

        g_fSoftEnter32ResLeanOnce = 1u;
        cpu_syscall_soft_inc(&g_u32SoftResLeanN);
        cpu_syscall_soft_inc(&g_u32SoftResLeanEnter32);
        /*
         * enter32 does not require SCE; integrity still reports MSR soft
         * state for honesty (Soft!=product). pe32_compat=1 marks path.
         */
        u32IntegrityEnter32 = cpu_syscall_soft_res_integrity();
        kprintf("cpu: syscall soft residual lean enter32 via=enter32 "
                "iretq32=1 cli=1 thr_kstack=%u "
                "user_cs32=0x%x user_ds=0x%x rflags_if=0x%lx "
                "rsp0_ok=1 sce_required=0 integrity=%u verdict=%s "
                "hosts=rtl8168_udx|xhci_udx|ddi_host "
                "product=UDX+ABI freestanding_class=SKIP "
                "dual_dod_a=OPEN dual_dod_b=OPEN "
                "udx_host=0 pe32_compat=1 g_ac1=1 soft_ne_product=1 "
                "product_complete=0 "
                "once=1 storm=0 hard_gate=0 dual=MIT_OR_Apache-2.0 "
                "unit=cpu_sys.c "
                "(Soft!=product; C0 residual; PE32/compat ring-3 enter residual; "
                "not product enter complete; Dual DoD A/B OPEN)\n",
                u32ThrKstack,
                (unsigned)GJ_CPU_SOFT_USER_CS32,
                (unsigned)GJ_CPU_SOFT_USER_DS,
                (unsigned long)GJ_CPU_SOFT_RFLAGS_IF,
                u32IntegrityEnter32,
                cpu_syscall_soft_res_verdict(u32IntegrityEnter32));
    }
    __asm__ volatile(
        "cli\n\t"
        "mov %[uds], %%ax\n\t" /* USER_DS (soft residual constant) */
        "mov %%ax, %%ds\n\t"
        "mov %%ax, %%es\n\t"
        "pushq %[uss]\n\t"     /* SS */
        "pushq %[rsp]\n\t"    /* RSP */
        ...
        : [rsp] "r"(u64Stack), [rip] "r"(u64Entry),
          [uds] "i"((u16)GJ_CPU_SOFT_USER_DS),
          [uss] "i"((u64)GJ_CPU_SOFT_USER_DS),
          [rfl] "i"(GJ_CPU_SOFT_RFLAGS_IF),
          [ucs] "i"((u64)GJ_CPU_SOFT_USER_CS32)
        : "rax", "memory");
    for (;;) {
        __asm__ volatile ("hlt");
    }
...
cpu_syscall_soft_log(void)
{
     ...
     *   cpu: syscall soft inventory wave=...
     ...
     *   cpu: syscall soft residual lean ...  (enter_user / enter_user32 UDX)
     *   cpu: syscall soft residual lean PASS  (once; integrity residual)
     *   cpu: syscall soft residual lean enter64|enter32  (once-lamps at land)
     *   cpu: syscall soft capacity ...       (storm cap inventory)
     * Inventory hard-capped (GJ_CPU_SYSCALL_SOFT_INV_LOG_CAP; storm=0);
     * re-verify once per log when live.
     * Soft!=product; dual MIT OR Apache-2.0; G-AC-1; Dual DoD OPEN;
     * no version stamp; C0 residual deepen.
     */
