 ...
 *   serial: soft deepen …     — wave=85 areas stamp
...
kprintf("serial: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: serial: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("serial: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("serial: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: serial: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("serial: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("serial: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: serial: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("serial: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: serial: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("serial: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retcurtainface honesty; Soft≠product; not bar3)\n");
/* Grep: serial: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("serial: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retbastionangle stamp; Soft≠product)\n");
/* Grep: serial: soft retglacisangle — Wave 74 return-glacisangle honesty */
kprintf("serial: soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retglacisangle honesty; Soft≠product; not bar3)\n");
/* Grep: serial: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
kprintf("serial: soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retparapetangle stamp; Soft≠product)\n");
/* Grep: serial: soft retmoatangle — Wave 75 return-moatangle honesty */
kprintf("serial: soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retmoatangle honesty; Soft≠product; not bar3)\n");
/* Grep: serial: soft retowerangle — Wave 75 exclusive towerangle stamp */
kprintf("serial: soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retowerangle stamp; Soft≠product)\n");
/* Grep: serial: soft retgateangle — Wave 76 return-gateangle honesty */
kprintf("serial: soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retgateangle honesty; Soft≠product; not bar3)\n");
/* Grep: serial: soft retwallangle — Wave 76 exclusive wallangle stamp */
kprintf("serial: soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retwallangle stamp; Soft≠product)\n");
/* Grep: serial: soft retspireangle — Wave 77 return-spireangle honesty */
kprintf("serial: soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retspireangle honesty; Soft≠product; not bar3)\n");
/* Grep: serial: soft retholdangle — Wave 77 exclusive holdangle stamp */
kprintf("serial: soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retholdangle stamp; Soft≠product)\n");
/* Grep: serial: soft retpalaceangle — Wave 78 return-palaceangle honesty */
kprintf("serial: soft retpalaceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retpalaceangle honesty; Soft≠product; not bar3)\n");
/* Grep: serial: soft retfortressangle — Wave 78 exclusive fortressangle stamp */
kprintf("serial: soft retfortressangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retfortressangle stamp; Soft≠product)\n");
/* Grep: serial: soft retkeepangle — Wave 79 return-keepangle honesty */
kprintf("serial: soft retkeepangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retkeepangle honesty; Soft≠product; not bar3)\n");
/* Grep: serial: soft retredoubtangle — Wave 79 exclusive redoubtangle stamp */
kprintf("serial: soft retredoubtangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retredoubtangle stamp; Soft≠product)\n");
/* Grep: serial: soft retcitadelangle — Wave 80 return-citadelangle honesty */
kprintf("serial: soft retcitadelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retcitadelangle honesty; Soft≠product; not bar3)\n");
/* Grep: serial: soft retbastionkeep — Wave 80 exclusive bastionkeep stamp */
kprintf("serial: soft retbastionkeep exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retbastionkeep stamp; Soft≠product)\n");
/* Grep: serial: soft retpanoplyangle — Wave 81 return-panoplyangle honesty */
kprintf("serial: soft retpanoplyangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retpanoplyangle honesty; Soft≠product; not bar3)\n");
/* Grep: serial: soft retbulwarkangle — Wave 81 exclusive bulwarkangle stamp */
kprintf("serial: soft retbulwarkangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retbulwarkangle stamp; Soft≠product)\n");
/* Grep: serial: soft retmantleangle — Wave 82 return-mantleangle honesty */
kprintf("serial: soft retmantleangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retmantleangle honesty; Soft≠product; not bar3)\n");
/* Grep: serial: soft retaegisangle — Wave 82 exclusive aegisangle stamp */
kprintf("serial: soft retaegisangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retaegisangle stamp; Soft≠product)\n");
/* Grep: serial: soft retemblemangle — Wave 83 return-emblemangle honesty */
kprintf("serial: soft retemblemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retemblemangle honesty; Soft≠product; not bar3)\n");
/* Grep: serial: soft retsigilangle — Wave 83 exclusive sigilangle stamp */
kprintf("serial: soft retsigilangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retsigilangle stamp; Soft≠product)\n");
/* Grep: serial: soft retscepterangle — Wave 84 return-scepterangle honesty */
kprintf("serial: soft retscepterangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retscepterangle honesty; Soft≠product; not bar3)\n");
/* Grep: serial: soft retglyphangle — Wave 84 exclusive glyphangle stamp */
kprintf("serial: soft retglyphangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retglyphangle stamp; Soft≠product)\n");
/* Grep: serial: soft retcrownangle — Wave 85 return-crownangle honesty */
kprintf("serial: soft retcrownangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retcrownangle honesty; Soft≠product; not bar3)\n");
/* Grep: serial: soft retshardangle — Wave 85 exclusive shardangle stamp */
kprintf("serial: soft retshardangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retshardangle stamp; Soft≠product)\n");
