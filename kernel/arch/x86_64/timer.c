...
kprintf("timer: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: timer: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("timer: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("timer: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: timer: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("timer: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("timer: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: timer: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("timer: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: timer: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("timer: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retcurtainface honesty; Soft≠product; not bar3)\n");
/* Grep: timer: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("timer: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retbastionangle stamp; Soft≠product)\n");
/* Grep: timer: soft retglacisangle — Wave 74 return-glacisangle honesty */
kprintf("timer: soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retglacisangle honesty; Soft≠product; not bar3)\n");
/* Grep: timer: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
kprintf("timer: soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retparapetangle stamp; Soft≠product)\n");
/* Grep: timer: soft retmoatangle — Wave 75 return-moatangle honesty */
kprintf("timer: soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retmoatangle honesty; Soft≠product; not bar3)\n");
/* Grep: timer: soft retowerangle — Wave 75 exclusive towerangle stamp */
kprintf("timer: soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retowerangle stamp; Soft≠product)\n");
/* Grep: timer: soft retgateangle — Wave 76 return-gateangle honesty */
kprintf("timer: soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retgateangle honesty; Soft≠product; not bar3)\n");
/* Grep: timer: soft retwallangle — Wave 76 exclusive wallangle stamp */
kprintf("timer: soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retwallangle stamp; Soft≠product)\n");
/* Grep: timer: soft retspireangle — Wave 77 return-spireangle honesty */
kprintf("timer: soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retspireangle honesty; Soft≠product; not bar3)\n");
/* Grep: timer: soft retholdangle — Wave 77 exclusive holdangle stamp */
kprintf("timer: soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retholdangle stamp; Soft≠product)\n");
/* Grep: timer: soft retpalaceangle — Wave 78 return-palaceangle honesty */
kprintf("timer: soft retpalaceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retpalaceangle honesty; Soft≠product; not bar3)\n");
/* Grep: timer: soft retfortressangle — Wave 78 exclusive fortressangle stamp */
kprintf("timer: soft retfortressangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retfortressangle stamp; Soft≠product)\n");
/* Grep: timer: soft retkeepangle — Wave 79 return-keepangle honesty */
kprintf("timer: soft retkeepangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retkeepangle honesty; Soft≠product; not bar3)\n");
/* Grep: timer: soft retredoubtangle — Wave 79 exclusive redoubtangle stamp */
kprintf("timer: soft retredoubtangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retredoubtangle stamp; Soft≠product)\n");
/* Grep: timer: soft retcitadelangle — Wave 80 return-citadelangle honesty */
kprintf("timer: soft retcitadelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retcitadelangle honesty; Soft≠product; not bar3)\n");
/* Grep: timer: soft retbastionkeep — Wave 80 exclusive bastionkeep stamp */
kprintf("timer: soft retbastionkeep exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retbastionkeep stamp; Soft≠product)\n");
/* Grep: timer: soft retpanoplyangle — Wave 81 return-panoplyangle honesty */
kprintf("timer: soft retpanoplyangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retpanoplyangle honesty; Soft≠product; not bar3)\n");
/* Grep: timer: soft retbulwarkangle — Wave 81 exclusive bulwarkangle stamp */
kprintf("timer: soft retbulwarkangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retbulwarkangle stamp; Soft≠product)\n");
/* Grep: timer: soft retmantleangle — Wave 82 return-mantleangle honesty */
kprintf("timer: soft retmantleangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retmantleangle honesty; Soft≠product; not bar3)\n");
/* Grep: timer: soft retaegisangle — Wave 82 exclusive aegisangle stamp */
kprintf("timer: soft retaegisangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retaegisangle stamp; Soft≠product)\n");
/* Grep: timer: soft retemblemangle — Wave 83 return-emblemangle honesty */
kprintf("timer: soft retemblemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retemblemangle honesty; Soft≠product; not bar3)\n");
/* Grep: timer: soft retsigilangle — Wave 83 exclusive sigilangle stamp */
kprintf("timer: soft retsigilangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retsigilangle stamp; Soft≠product)\n");
