...
kprintf("timer: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: timer: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("timer: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("timer: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: timer: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("timer: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("timer: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: timer: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("timer: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: timer: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("timer: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retcurtainface honesty; Soft≠product; not bar3)\n");
/* Grep: timer: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("timer: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retbastionangle stamp; Soft≠product)\n");
/* Grep: timer: soft retglacisangle — Wave 74 return-glacisangle honesty */
kprintf("timer: soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retglacisangle honesty; Soft≠product; not bar3)\n");
/* Grep: timer: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
kprintf("timer: soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retparapetangle stamp; Soft≠product)\n");
/* Grep: timer: soft retmoatangle — Wave 75 return-moatangle honesty */
kprintf("timer: soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retmoatangle honesty; Soft≠product; not bar3)\n");
/* Grep: timer: soft retowerangle — Wave 75 exclusive towerangle stamp */
kprintf("timer: soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retowerangle stamp; Soft≠product)\n");
/* Grep: timer: soft retgateangle — Wave 76 return-gateangle honesty */
kprintf("timer: soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retgateangle honesty; Soft≠product; not bar3)\n");
/* Grep: timer: soft retwallangle — Wave 76 exclusive wallangle stamp */
kprintf("timer: soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retwallangle stamp; Soft≠product)\n");
/* Grep: timer: soft retspireangle — Wave 77 return-spireangle honesty */
kprintf("timer: soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retspireangle honesty; Soft≠product; not bar3)\n");
/* Grep: timer: soft retholdangle — Wave 77 exclusive holdangle stamp */
kprintf("timer: soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retholdangle stamp; Soft≠product)\n");
/* Grep: timer: soft retpalaceangle — Wave 78 return-palaceangle honesty */
kprintf("timer: soft retpalaceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retpalaceangle honesty; Soft≠product; not bar3)\n");
/* Grep: timer: soft retfortressangle — Wave 78 exclusive fortressangle stamp */
kprintf("timer: soft retfortressangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retfortressangle stamp; Soft≠product)\n");
/* Grep: timer: soft retkeepangle — Wave 79 return-keepangle honesty */
kprintf("timer: soft retkeepangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retkeepangle honesty; Soft≠product; not bar3)\n");
/* Grep: timer: soft retredoubtangle — Wave 79 exclusive redoubtangle stamp */
kprintf("timer: soft retredoubtangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retredoubtangle stamp; Soft≠product)\n");
/* Grep: timer: soft retcitadelangle — Wave 80 return-citadelangle honesty */
kprintf("timer: soft retcitadelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retcitadelangle honesty; Soft≠product; not bar3)\n");
/* Grep: timer: soft retbastionkeep — Wave 80 exclusive bastionkeep stamp */
kprintf("timer: soft retbastionkeep exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retbastionkeep stamp; Soft≠product)\n");
/* Grep: timer: soft retpanoplyangle — Wave 81 return-panoplyangle honesty */
kprintf("timer: soft retpanoplyangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retpanoplyangle honesty; Soft≠product; not bar3)\n");
/* Grep: timer: soft retbulwarkangle — Wave 81 exclusive bulwarkangle stamp */
kprintf("timer: soft retbulwarkangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retbulwarkangle stamp; Soft≠product)\n");
/* Grep: timer: soft retmantleangle — Wave 82 return-mantleangle honesty */
kprintf("timer: soft retmantleangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retmantleangle honesty; Soft≠product; not bar3)\n");
/* Grep: timer: soft retaegisangle — Wave 82 exclusive aegisangle stamp */
kprintf("timer: soft retaegisangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retaegisangle stamp; Soft≠product)\n");
/* Grep: timer: soft retemblemangle — Wave 83 return-emblemangle honesty */
kprintf("timer: soft retemblemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retemblemangle honesty; Soft≠product; not bar3)\n");
/* Grep: timer: soft retsigilangle — Wave 83 exclusive sigilangle stamp */
kprintf("timer: soft retsigilangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retsigilangle stamp; Soft≠product)\n");
/* Grep: timer: soft retscepterangle — Wave 84 return-scepterangle honesty */
kprintf("timer: soft retscepterangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retscepterangle honesty; Soft≠product; not bar3)\n");
/* Grep: timer: soft retglyphangle — Wave 84 exclusive glyphangle stamp */
kprintf("timer: soft retglyphangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retglyphangle stamp; Soft≠product)\n");
/* Grep: timer: soft retcrownangle — Wave 85 return-crownangle honesty */
kprintf("timer: soft retcrownangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retcrownangle honesty; Soft≠product; not bar3)\n");
/* Grep: timer: soft retshardangle — Wave 85 exclusive shardangle stamp */
kprintf("timer: soft retshardangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retshardangle stamp; Soft≠product)\n");
/* Grep: timer: soft retforgeangle — Wave 86 return-forgeangle honesty */
kprintf("timer: soft retforgeangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retforgeangle honesty; Soft≠product; not bar3)\n");
/* Grep: timer: soft retprismangle — Wave 86 exclusive prismangle stamp */
kprintf("timer: soft retprismangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retprismangle stamp; Soft≠product)\n");
/* Grep: timer: soft retflameangle — Wave 87 return-flameangle honesty */
kprintf("timer: soft retflameangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retflameangle honesty; Soft≠product; not bar3)\n");
/* Grep: timer: soft retcipherangle — Wave 87 exclusive cipherangle stamp */
kprintf("timer: soft retcipherangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retcipherangle stamp; Soft≠product)\n");
/* Grep: timer: soft retbeaconangle — Wave 88 return-beaconangle honesty */
kprintf("timer: soft retbeaconangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retbeaconangle honesty; Soft≠product; not bar3)\n");
/* Grep: timer: soft retledgerangle — Wave 88 exclusive ledgerangle stamp */
kprintf("timer: soft retledgerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retledgerangle stamp; Soft≠product)\n");
/* Grep: timer: soft retbannerangle — Wave 89 return-bannerangle honesty */
kprintf("timer: soft retbannerangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retbannerangle honesty; Soft≠product; not bar3)\n");
/* Grep: timer: soft retvaultangle — Wave 89 exclusive vaultangle stamp */
kprintf("timer: soft retvaultangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retvaultangle stamp; Soft≠product)\n");
/* Grep: timer: soft retcrestangle — Wave 90 return-crestangle honesty */
kprintf("timer: soft retcrestangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retcrestangle honesty; Soft≠product; not bar3)\n");
/* Grep: timer: soft rettokenangle — Wave 90 exclusive tokenangle stamp */
kprintf("timer: soft rettokenangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (rettokenangle stamp; Soft≠product)\n");
/* Grep: timer: soft retbadgeangle — Wave 91 return-badgeangle honesty */
kprintf("timer: soft retbadgeangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retbadgeangle honesty; Soft≠product; not bar3)\n");
/* Grep: timer: soft retphaseangle — Wave 91 exclusive phaseangle stamp */
kprintf("timer: soft retphaseangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retphaseangle stamp; Soft≠product)\n");
/* Grep: timer: soft retmarkangle — Wave 92 return-markangle honesty */
kprintf("timer: soft retmarkangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retmarkangle honesty; Soft≠product; not bar3)\n");
/* Grep: timer: soft retpulseangle — Wave 92 exclusive pulseangle stamp */
kprintf("timer: soft retpulseangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retpulseangle stamp; Soft≠product)\n");

/* Grep: timer: soft retsealangle — Wave 93 return-sealangle honesty */
kprintf("timer: soft retsealangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retsealangle honesty; Soft≠product; not bar3)\n");
/* Grep: timer: soft retboundangle — Wave 93 exclusive boundangle stamp */
kprintf("timer: soft retboundangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retboundangle stamp; Soft≠product)\n");
/* Grep: timer: soft retstemangle — Wave 94 return-stemangle honesty */
kprintf("timer: soft retstemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retstemangle honesty; Soft≠product; not bar3)\n");
/* Grep: timer: soft retbladeangle — Wave 94 exclusive bladeangle stamp */
kprintf("timer: soft retbladeangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retbladeangle stamp; Soft≠product)\n");
/* Grep: timer: soft retchordangle — Wave 95 return-chordangle honesty */
kprintf("timer: soft retchordangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retchordangle honesty; Soft≠product; not bar3)\n");
/* Grep: timer: soft retarcangle — Wave 95 exclusive arcangle stamp */
kprintf("timer: soft retarcangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retarcangle stamp; Soft≠product)\n");
/* Grep: timer: soft retsectorangle — Wave 96 return-sectorangle honesty */
kprintf("timer: soft retsectorangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retsectorangle honesty; Soft≠product; not bar3)\n");
/* Grep: timer: soft retwedgeangle — Wave 96 exclusive wedgeangle stamp */
kprintf("timer: soft retwedgeangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retwedgeangle stamp; Soft≠product)\n");
