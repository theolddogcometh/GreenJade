 ...
 *   boot: soft deepen        — wave=92 stamp + area count
                        ...
                        const char *szHandoff)
{
    ...
    kprintf("boot: soft note milestone=wave92 exclusive=1 "
    ...
    kprintf("boot: soft catalog honesty,inventory,path,handoff,memmap,gop,"
            ...
            "catalog,return,retmap,deepen wave=%u areas_expect=74 soft PASS\n",
            (unsigned)BI_SOFT_WAVE);
    cAreas++;
    ...
    kprintf("boot: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=92\n");
...
kprintf("boot: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=92 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: boot: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("boot: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=92 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("boot: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=92 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: boot: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("boot: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=92 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("boot: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=92 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: boot: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("boot: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=92 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: boot: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("boot: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=92 (retcurtainface honesty; Soft≠product; not bar3)\n");
/* Grep: boot: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("boot: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=92 (retbastionangle stamp; Soft≠product)\n");
/* Grep: boot: soft retglacisangle — Wave 74 return-glacisangle honesty */
kprintf("boot: soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=92 (retglacisangle honesty; Soft≠product; not bar3)\n");
/* Grep: boot: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
kprintf("boot: soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=92 (retparapetangle stamp; Soft≠product)\n");
/* Grep: boot: soft retmoatangle — Wave 75 return-moatangle honesty */
kprintf("boot: soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=92 (retmoatangle honesty; Soft≠product; not bar3)\n");
/* Grep: boot: soft retowerangle — Wave 75 exclusive towerangle stamp */
kprintf("boot: soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=92 (retowerangle stamp; Soft≠product)\n");
/* Grep: boot: soft retgateangle — Wave 76 return-gateangle honesty */
kprintf("boot: soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=92 (retgateangle honesty; Soft≠product; not bar3)\n");
/* Grep: boot: soft retwallangle — Wave 76 exclusive wallangle stamp */
kprintf("boot: soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=92 (retwallangle stamp; Soft≠product)\n");
/* Grep: boot: soft retspireangle — Wave 77 return-spireangle honesty */
kprintf("boot: soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=92 (retspireangle honesty; Soft≠product; not bar3)\n");
/* Grep: boot: soft retholdangle — Wave 77 exclusive holdangle stamp */
kprintf("boot: soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=92 (retholdangle stamp; Soft≠product)\n");
/* Grep: boot: soft retpalaceangle — Wave 78 return-palaceangle honesty */
kprintf("boot: soft retpalaceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=92 (retpalaceangle honesty; Soft≠product; not bar3)\n");
/* Grep: boot: soft retfortressangle — Wave 78 exclusive fortressangle stamp */
kprintf("boot: soft retfortressangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=92 (retfortressangle stamp; Soft≠product)\n");
/* Grep: boot: soft retkeepangle — Wave 79 return-keepangle honesty */
kprintf("boot: soft retkeepangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=92 (retkeepangle honesty; Soft≠product; not bar3)\n");
/* Grep: boot: soft retredoubtangle — Wave 79 exclusive redoubtangle stamp */
kprintf("boot: soft retredoubtangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=92 (retredoubtangle stamp; Soft≠product)\n");
/* Grep: boot: soft retcitadelangle — Wave 80 return-citadelangle honesty */
kprintf("boot: soft retcitadelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=92 (retcitadelangle honesty; Soft≠product; not bar3)\n");
/* Grep: boot: soft retbastionkeep — Wave 80 exclusive bastionkeep stamp */
kprintf("boot: soft retbastionkeep exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=92 (retbastionkeep stamp; Soft≠product)\n");
/* Grep: boot: soft retpanoplyangle — Wave 81 return-panoplyangle honesty */
kprintf("boot: soft retpanoplyangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=92 (retpanoplyangle honesty; Soft≠product; not bar3)\n");
/* Grep: boot: soft retbulwarkangle — Wave 81 exclusive bulwarkangle stamp */
kprintf("boot: soft retbulwarkangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=92 (retbulwarkangle stamp; Soft≠product)\n");
/* Grep: boot: soft retmantleangle — Wave 82 return-mantleangle honesty */
kprintf("boot: soft retmantleangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=92 (retmantleangle honesty; Soft≠product; not bar3)\n");
/* Grep: boot: soft retaegisangle — Wave 82 exclusive aegisangle stamp */
kprintf("boot: soft retaegisangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=92 (retaegisangle stamp; Soft≠product)\n");
/* Grep: boot: soft retemblemangle — Wave 83 return-emblemangle honesty */
kprintf("boot: soft retemblemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=92 (retemblemangle honesty; Soft≠product; not bar3)\n");
/* Grep: boot: soft retsigilangle — Wave 83 exclusive sigilangle stamp */
kprintf("boot: soft retsigilangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=92 (retsigilangle stamp; Soft≠product)\n");
/* Grep: boot: soft retscepterangle — Wave 84 return-scepterangle honesty */
kprintf("boot: soft retscepterangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=92 (retscepterangle honesty; Soft≠product; not bar3)\n");
/* Grep: boot: soft retglyphangle — Wave 84 exclusive glyphangle stamp */
kprintf("boot: soft retglyphangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=92 (retglyphangle stamp; Soft≠product)\n");
/* Grep: boot: soft retcrownangle — Wave 85 return-crownangle honesty */
kprintf("boot: soft retcrownangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=92 (retcrownangle honesty; Soft≠product; not bar3)\n");
/* Grep: boot: soft retshardangle — Wave 85 exclusive shardangle stamp */
kprintf("boot: soft retshardangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=92 (retshardangle stamp; Soft≠product)\n");
/* Grep: boot: soft retforgeangle — Wave 86 return-forgeangle honesty */
kprintf("boot: soft retforgeangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=92 (retforgeangle honesty; Soft≠product; not bar3)\n");
/* Grep: boot: soft retprismangle — Wave 86 exclusive prismangle stamp */
kprintf("boot: soft retprismangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=92 (retprismangle stamp; Soft≠product)\n");
/* Grep: boot: soft retflameangle — Wave 87 return-flameangle honesty */
kprintf("boot: soft retflameangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=92 (retflameangle honesty; Soft≠product; not bar3)\n");
/* Grep: boot: soft retcipherangle — Wave 87 exclusive cipherangle stamp */
kprintf("boot: soft retcipherangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=92 (retcipherangle stamp; Soft≠product)\n");
/* Grep: boot: soft retbeaconangle — Wave 88 return-beaconangle honesty */
kprintf("boot: soft retbeaconangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=92 (retbeaconangle honesty; Soft≠product; not bar3)\n");
/* Grep: boot: soft retledgerangle — Wave 88 exclusive ledgerangle stamp */
kprintf("boot: soft retledgerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=92 (retledgerangle stamp; Soft≠product)\n");
/* Grep: boot: soft retbannerangle — Wave 89 return-bannerangle honesty */
kprintf("boot: soft retbannerangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=92 (retbannerangle honesty; Soft≠product; not bar3)\n");
/* Grep: boot: soft retvaultangle — Wave 89 exclusive vaultangle stamp */
kprintf("boot: soft retvaultangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=92 (retvaultangle stamp; Soft≠product)\n");
/* Grep: boot: soft retcrestangle — Wave 90 return-crestangle honesty */
kprintf("boot: soft retcrestangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=92 (retcrestangle honesty; Soft≠product; not bar3)\n");
/* Grep: boot: soft rettokenangle — Wave 90 exclusive tokenangle stamp */
kprintf("boot: soft rettokenangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=92 (rettokenangle stamp; Soft≠product)\n");
/* Grep: boot: soft retbadgeangle — Wave 91 return-badgeangle honesty */
kprintf("boot: soft retbadgeangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=92 (retbadgeangle honesty; Soft≠product; not bar3)\n");
/* Grep: boot: soft retphaseangle — Wave 91 exclusive phaseangle stamp */
kprintf("boot: soft retphaseangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=92 (retphaseangle stamp; Soft≠product)\n");
/* Grep: boot: soft retmarkangle — Wave 92 return-markangle honesty */
kprintf("boot: soft retmarkangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=92 (retmarkangle honesty; Soft≠product; not bar3)\n");
/* Grep: boot: soft retpulseangle — Wave 92 exclusive pulseangle stamp */
kprintf("boot: soft retpulseangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=92 (retpulseangle stamp; Soft≠product)\n");
