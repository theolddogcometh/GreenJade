 ...
 *   boot: soft deepen        — wave=54 stamp + area count
                        ...
                        const char *szHandoff)
{
    ...
    kprintf("boot: soft note milestone=wave54 exclusive=1 "
    ...
    kprintf("boot: soft catalog honesty,inventory,path,handoff,memmap,gop,"
            ...
            "catalog,return,retmap,deepen wave=%u areas_expect=30 soft PASS\n",
            (unsigned)BI_SOFT_WAVE);
    cAreas++;
    ...
    kprintf("boot: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=54\n");
