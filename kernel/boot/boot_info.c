 ...
 *   boot: soft deepen        — wave=72 stamp + area count
                        ...
                        const char *szHandoff)
{
    ...
    kprintf("boot: soft note milestone=wave72 exclusive=1 "
    ...
    kprintf("boot: soft catalog honesty,inventory,path,handoff,memmap,gop,"
            ...
            "catalog,return,retmap,deepen wave=%u areas_expect=54 soft PASS\n",
            (unsigned)BI_SOFT_WAVE);
    cAreas++;
    ...
    kprintf("boot: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=72\n");
...
kprintf("boot: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=72 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: boot: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("boot: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=72 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("boot: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=72 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: boot: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("boot: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=72 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("boot: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=72 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: boot: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("boot: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=72 (retdoubletenailleangle stamp; Soft≠product)\n");
