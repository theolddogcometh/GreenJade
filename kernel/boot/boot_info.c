 ...
 *
 * Wave 30 exclusive soft deepen — greppable prefix-stable "boot: soft …"
 ...
 *   boot: soft deepen        — wave=30 stamp + area count
...
/* Wave 30 exclusive soft deepen stamp (this unit only). */
#define BI_SOFT_WAVE 30u
                        ...
                        const char *szHandoff)
{
    ...
    kprintf("boot: soft note milestone=wave30 exclusive=1 "
    ...
    kprintf("boot: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=30\n");
            ...
            */
            ...
            kprintf("boot: soft retflame exclusive=1 soft_ne_product=1 "
                    ...
                    kprintf("boot: soft retforge exclusive=1 soft_ne_product=1 "
                             ...
                             * Soft≠product; not bar3.
                             */
                            /* Grep: boot: soft retsigil — Wave 30 return-sigil honesty */
                            kprintf("boot: soft retsigil soft_only=1 product_gate=0 soft_ne_product=1 "
                                    "never_blocks_m0=1 wave=%u "
                                    "(retsigil honesty; Soft≠product; not bar3)\n",
                                    (unsigned)BI_SOFT_WAVE);
                            /* Grep: boot: soft retemblem — Wave 30 exclusive emblem stamp */
