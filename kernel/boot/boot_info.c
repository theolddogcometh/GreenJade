 ...
 *
 * Wave 32 exclusive soft deepen — greppable prefix-stable "boot: soft …"
 ...
 *   boot: soft deepen        — wave=32 stamp + area count
...
/* Wave 32 exclusive soft deepen stamp (this unit only). */
#define BI_SOFT_WAVE 32u
                        ...
                        const char *szHandoff)
{
    ...
    kprintf("boot: soft note milestone=wave32 exclusive=1 "
    ...
    kprintf("boot: soft catalog honesty,inventory,path,handoff,memmap,gop,"
            ...
            "catalog,return,retmap,deepen wave=%u areas_expect=28 soft PASS\n",
            (unsigned)BI_SOFT_WAVE);
    cAreas++;
    ...
    kprintf("boot: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=32\n");
 ...
 * Soft≠product; not bar3.
 */
/* Grep: boot: soft retbulwark — Wave 32 return-bulwark honesty */
kprintf("boot: soft retbulwark soft_only=1 product_gate=0 soft_ne_product=1 "
        "never_blocks_m0=1 wave=%u "
        "(retbulwark honesty; Soft≠product; not bar3)\n",
        (unsigned)BI_SOFT_WAVE);
/* Grep: boot: soft retpanoply — Wave 32 exclusive panoply stamp */
