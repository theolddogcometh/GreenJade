 ...
 *     includes lean soft residual + Multiboot2 tag inventory (P-BOOT-2)
 *
 * Lean soft residual (this unit only; Soft!=product; G-AC-1; C0):
 *   Diagnostics only - never hard-gate product / ExitBootServices.
 *   No version stamp, no stamp storms (prior wave deepen collapsed).
 *   Soft residual != product dual-license driver path (userspace UDX+ABI).
 *   Dual license of this unit: MIT OR Apache-2.0 only. No GPL.
 *   C0 residual deepen: compact real surfaces only (stamp-free).
 *   Dual DoD A/B remain OPEN (this unit never closes DoD / bar3).
 *   G-AC-1: soft media .ko handoff is never product AC.
 *
 *   boot: soft residual      - one-line residual surface (lean)
 *   boot: soft residual lean - cover lamp PASS (class=C0)
 *   boot: soft honesty       - explicit non-claims (soft never gates EBS)
 *   boot: soft inventory     - compact master surface
 ...
 *   boot: soft caps          - compact per-bit presence rollup
 *   boot: soft source        - source class + publish tallies
 ...
 *   boot: soft stats         - soft counter rollup
 *   boot: soft validate      - handoff validity classify
 *   boot: soft check         - flags-vs-fields consistency (soft)
 *   boot: soft identity      - identity-bridge note (separate unit)
 *   boot: soft media         - ESP .ko soft handoff (D4; G-AC-1)
 *   boot: soft mb2 ...       - Multiboot2 tag walk (P-BOOT-2 dev)
 ...
 *
 * greppable: boot: soft residual
 * greppable: boot: soft residual lean
 ...
 * greppable: boot: soft kernel
 * greppable: boot: soft rsdp
 ...
 * greppable: boot: soft validate
 * greppable: boot: soft check
 * greppable: boot: soft identity
 * greppable: boot: soft media
 * greppable: boot: soft mb2
 * greppable: boot_info: soft mb2 inventory
 * greppable: boot: soft PASS
...
static u32 g_cSoftDeriveMedia; /* GJ_BOOT_F_SOFT_MEDIA derive (D4 soft only) */
...
/*
 * Soft EFI memmap type-class page tallies (lean residual companion).
 * greppable: boot: soft memmap
 */
...
boot_info_derive_flags(const struct gj_boot_info *pInfo)
{
    ...
    if (pInfo->u64SoftMediaPhys != 0 && pInfo->u64SoftMediaBytes != 0) {
        ...
        boot_soft_inc(&g_cSoftDeriveMedia);
    }
    return u32Flags;
}
...
/*
 * Soft EFI memmap walk (type-class pages + base soft memmap).
 * greppable: boot: soft memmap
 */
static void
...
/*
 * Lean Multiboot2 soft inventory (P-BOOT-2 dev; SKIP on UEFI product).
 * Hard-capped lines; no per-absent-tag SKIP storm. Soft!=product.
 * greppable: boot: soft mb2 | boot_info: soft mb2 inventory
 */
static void
boot_info_soft_mb2_log(const struct gj_boot_info *pInfo)
{
    ...
    if (!softInv.fPresent) {
        /* Grep: boot: soft mb2 SKIP | boot: soft mb2 path | inventory */
        kprintf("boot: soft mb2 SKIP pa=0 source=%u flags_mb2=%d "
                "inv_calls=%u path=p_boot_2_dev_only product=uefi "
                "reason=no_mb2_phys Soft!=product\n",
                (unsigned)u32Source,
    ...
    if (softInv.fReject) {
        ...
        kprintf("boot: soft mb2 REJECT pa=0x%x total_size=%u source=%u "
                "floor=%u ceil=%u Soft!=product\n",
    ...
    kprintf("boot: soft mb2 %s pa=0x%x total=%u tags=%u end=%u "
            ...
            "other=%u bad=%u overrun=%u source=%u Soft!=product\n",
    ...
    /* Compact tag + payload rollup (present fields only; no SKIP storm). */
    kprintf("boot: soft mb2 tag meminfo=%u bootdev=%u vbe=%u "
            "efi32=%u efi64=%u efi_mmap=%u efi_bs=%u efi_ih=%u efi_surf=%u "
            "smbios=%u apm=%u network=%u load_base=%u boot_loader=%u "
            "lower_kib=%u upper_kib=%u load_base_addr=0x%x\n",
            ...
            (unsigned)cEfiSurf, (unsigned)softInv.cSmbios,
            (unsigned)softInv.cApm, (unsigned)softInv.cNetwork,
            (unsigned)softInv.cLoadBase, (unsigned)softInv.cBootLoader,
    ...
    if (cAcpi > 0) {
        ...
        kprintf("boot: soft mb2 acpi old=%u new=%u\n",
                (unsigned)softInv.cAcpiOld, (unsigned)softInv.cAcpiNew);
    }
...
/*
 * Lean soft residual inventory - greppable "boot: soft ...".
 * Compact handoff snapshot; never allocates; never hard-gates EBS/product.
 * No version stamp, no stamp storms (was ~30+ kprintf + wave= deepens).
 * C0 residual deepen: restore real classify/counter surfaces only.
 * Soft != product. Dual MIT OR Apache-2.0 of this unit only. No GPL.
 * Soft residual != product dual-license userspace driver path (G-AC-1).
 * Higher-half product move remains OPEN (identity bridge is separate unit).
 * Dual DoD A/B remain OPEN; bar3 remains OPEN; class=C0 only.
 * Soft media .ko handoff never product AC (G-AC-1). product=UDX+ABI.
 *
 * greppable: boot: soft residual | boot: soft residual lean
 * greppable: boot: soft honesty | boot: soft inventory
 * greppable: boot: soft path | boot: soft handoff | boot: soft memmap
 * greppable: boot: soft gop | boot: soft flags | boot: soft caps
 * greppable: boot: soft source | boot: soft kernel | boot: soft rsdp
 * greppable: boot: soft contract | boot: soft fields | boot: soft geometry
 * greppable: boot: soft publish | boot: soft stats | boot: soft validate
 * greppable: boot: soft check | boot: soft identity | boot: soft PASS
 * greppable: Soft!=product | G-AC-1 | Dual DoD | class=C0
 */
static void
                        ...
                        const char *szHandoff)
{
    ...
    fMediaBit = (u32Flags & GJ_BOOT_F_SOFT_MEDIA) != 0;
    ...
    fMediaSpan = (pInfo != NULL && pInfo->u64SoftMediaPhys != 0 &&
                  pInfo->u64SoftMediaBytes != 0)
                     ? 1u
                     : 0u;
    ...
    cCapBits = (u32)fMemmapBit + (u32)fRsdpBit + (u32)fFbBit +
               (u32)fKernelBit + (u32)fMb2Bit + (u32)fMediaBit;
    ...
    /*
     * Soft flags-vs-fields consistency (observability only).
     * mismatch counts flag bits that disagree with field presence.
     * greppable: boot: soft check
     */
    fMapFields = (fMapPtr != 0 && fMapBytes != 0) ? 1u : 0u;
    cMismatch = 0;
     ...
     * Dual DoD A/B OPEN; bar3 OPEN; class=C0; product path = UDX+ABI.
     * greppable: boot: soft honesty | Dual DoD | Soft!=product | G-AC-1
     */
    kprintf("boot: soft honesty never_gates_ebs=1 never_gates_product=1 "
            "higher_half_product=OPEN identity_bridge=separate "
            "p_boot_1=uefi p_boot_2=mb2_dev soft_only=1 class=C0 "
            "dual_dod_a=OPEN dual_dod_b=OPEN Dual DoD A/B OPEN "
            "bar3=OPEN product=UDX+ABI G-AC-1 no_ko_product_ac=1 "
            "no_version_stamp=1 Soft!=product\n");

    /*
     * Grep: boot: soft residual
     * One-line residual surface - lean, no per-area stamp storm.
     */
    kprintf("boot: soft residual handoff=%s source=%u src=%s path=%s "
            "magic_ok=%u version_ok=%u valid=%d memmap=%s gop=%s "
            "flags=0x%x cap_bits=%u check=%s mismatch=%u set=%u log=%u "
            "uefi_pub=%u mb2_pub=%u media=%u "
            "soft=1 product=0 class=C0 dual_dod_a=OPEN dual_dod_b=OPEN "
            "Dual DoD A/B OPEN bar3=OPEN product_path=UDX+ABI "
            "Soft!=product dual=MIT_OR_Apache-2.0 G-AC-1 residual!=product "
            "no_version_stamp=1 residual_deepen=1 storm=0\n",
            szHandoff, (unsigned)u32Source, szSrcName, szPathClaim,
            (unsigned)fMagicOk, (unsigned)fVersionOk, fValid, szMemmap,
            szGop, (unsigned)u32Flags, (unsigned)cCapBits, szCheck,
            (unsigned)cMismatch, (unsigned)g_cSoftSetGlobal,
            (unsigned)g_cSoftLogCalls, (unsigned)g_cSoftUefiPublish,
            (unsigned)g_cSoftMb2Publish, (unsigned)fMediaBit);

    /*
     * Grep: boot: soft residual lean PASS
     * Cover lamp only - never claims Dual DoD / bar3 / product AC.
     */
    kprintf("boot: soft residual lean PASS handoff=%s memmap=%s gop=%s "
            "check=%s cap_bits=%u media=%u class=C0 soft=1 product=0 "
            "dual_dod_a=OPEN dual_dod_b=OPEN Dual DoD A/B OPEN bar3=OPEN "
            "product_path=UDX+ABI Soft!=product dual=MIT_OR_Apache-2.0 "
            "G-AC-1 no_version_stamp=1 storm=0 residual_deepen=1\n",
            szHandoff, szMemmap, szGop, szCheck, (unsigned)cCapBits,
            (unsigned)fMediaBit);

    /* Grep: boot: soft inventory */
    kprintf("boot: soft inventory handoff=%s source=%u src=%s path=%s "
            "magic_ok=%d memmap=%s gop=%s set=%u log=%u mb2_inv=%u "
            "memmap_calls=%u gop_calls=%u cap_bits=%u check=%s "
            "class=C0 dual_dod_a=OPEN dual_dod_b=OPEN Dual DoD A/B OPEN "
            "Soft!=product\n",
    ...
    kprintf("boot: soft path claim=%s p_boot_1=uefi_product "
            ...
            "class=C0 product_path=UDX+ABI Soft!=product\n",
    ...
    if (pInfo == NULL) {
        kprintf("boot: soft handoff STUB pInfo=NULL magic=0 version=0 "
                "source=0 flags=0x0 Soft!=product\n");
    } else {
        kprintf("boot: soft handoff %s magic=0x%x version=%u source=%u "
                ...
                "fb=0x%lx media=0x%lx+%lu Soft!=product\n",
    ...
    if (pDeep == NULL || !pDeep->base.fPresent) {
        kprintf("boot: soft memmap SKIP present=0 descs=0 usable=0 "
                "flags_memmap=%u stride_fb=%u Soft!=product\n",
                (unsigned)fMemmapBit,
    ...
    kprintf("boot: soft flags raw=0x%x memmap=%u rsdp=%u fb=%u "
            "kernel_img=%u mb2_info=%u soft_media=%u "
            "derive_memmap=%u derive_rsdp=%u derive_fb=%u "
            "derive_kernel=%u derive_mb2=%u derive_media=%u Soft!=product\n",
            (unsigned)u32Flags, (unsigned)fMemmapBit, (unsigned)fRsdpBit,
            (unsigned)fFbBit, (unsigned)fKernelBit, (unsigned)fMb2Bit,
            (unsigned)fMediaBit, (unsigned)g_cSoftDeriveMemmap,
            (unsigned)g_cSoftDeriveRsdp, (unsigned)g_cSoftDeriveFb,
            (unsigned)g_cSoftDeriveKernel, (unsigned)g_cSoftDeriveMb2,
            (unsigned)g_cSoftDeriveMedia);

    /* Grep: boot: soft caps - compact presence rollup (one line). */
    kprintf("boot: soft caps pop=%u memmap=%u rsdp=%u fb=%u "
            "kernel_img=%u mb2_info=%u soft_media=%u Soft!=product\n",
            (unsigned)cCapBits, (unsigned)fMemmapBit, (unsigned)fRsdpBit,
            (unsigned)fFbBit, (unsigned)fKernelBit, (unsigned)fMb2Bit,
            (unsigned)fMediaBit);
    ...
    /* Grep: boot: soft publish */
    kprintf("boot: soft publish set=%u uefi=%u mb2=%u unknown=%u "
            "clear=%u valid_ok=%u valid_fail=%u log=%u Soft!=product\n",
    ...
    /*
     * Grep: boot: soft check - flags vs field presence consistency.
     * Soft only; never gates product / ExitBootServices.
     */
    kprintf("boot: soft check %s mismatch=%u "
            "memmap_flag=%u map_fields=%u rsdp_flag=%u rsdp_field=%u "
            "fb_flag=%u fb_field=%u kernel_flag=%u kernel_span=%u "
            "mb2_flag=%u mb2_phys=%u media_flag=%u media_span=%u "
            "Soft!=product\n",
            szCheck, (unsigned)cMismatch, (unsigned)fMemmapBit,
            (unsigned)fMapFields, (unsigned)fRsdpBit, (unsigned)fRsdp,
            (unsigned)fFbBit, (unsigned)fFbBase, (unsigned)fKernelBit,
            (unsigned)fKernelSpan, (unsigned)fMb2Bit, (unsigned)fMb2Phys,
            (unsigned)fMediaBit, (unsigned)fMediaSpan);

    /*
     * Grep: boot: soft identity - bridge note only; install is identity_map.c.
     * Never claims higher-half product / Dual DoD / bar3 from this unit.
     */
    kprintf("boot: soft identity bridge=separate unit=identity_map.c "
            "cover_gib=4 higher_half_product=OPEN soft_note=1 "
            "bar3=OPEN dual_dod_a=OPEN dual_dod_b=OPEN "
            "Dual DoD A/B OPEN class=C0 Soft!=product\n");

    /* Grep: boot: soft PASS */
    kprintf("boot: soft PASS handoff=%s memmap=%s gop=%s check=%s "
            "residual=lean storm=0 class=C0 dual_dod_a=OPEN "
            "dual_dod_b=OPEN Dual DoD A/B OPEN bar3=OPEN "
            "Soft!=product G-AC-1\n",
            szHandoff, szMemmap, szGop, szCheck);
}
...
boot_info_soft_log(const struct gj_boot_info *pInfo)
{
    ...
    if (pInfo == NULL) {
        ...
        kprintf("boot: soft mb2 SKIP pa=0 reason=null_info log=%u\n",
                (unsigned)g_cSoftLogCalls);
        kprintf("boot_info: soft mb2 inventory SKIP present=0\n");
        /* Lean residual null path under boot: soft ... */
     ...
     * Never product AC; product drivers = userspace UDX+ABI.
     * greppable: boot: soft media PASS|SKIP | Soft!=product | G-AC-1
     */
    if (pInfo->u64SoftMediaPhys != 0 && pInfo->u64SoftMediaBytes != 0) {
        kprintf("boot: soft media PASS phys=0x%lx bytes=%lu flags_media=%d "
                "product_ac=0 G-AC-1 no_ko_product_ac=1 product=UDX+ABI "
                "Soft!=product (ESP UEFI handoff; D4 soft only)\n",
                (unsigned long)pInfo->u64SoftMediaPhys,
                (unsigned long)pInfo->u64SoftMediaBytes,
                (u32Flags & GJ_BOOT_F_SOFT_MEDIA) != 0);
    } else {
        kprintf("boot: soft media SKIP phys=0 bytes=0 product_ac=0 "
                "G-AC-1 product=UDX+ABI Soft!=product "
                "(no UEFI ESP .ko handoff)\n");
    }

    /* Lean soft residual (no version stamp; no stamp storms; class=C0). */
