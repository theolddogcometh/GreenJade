 ...
 *   boot: soft deepen        — wave=80 stamp + area count
                        ...
                        const char *szHandoff)
{
    ...
    kprintf("boot: soft note milestone=wave80 exclusive=1 "
    ...
    kprintf("boot: soft catalog honesty,inventory,path,handoff,memmap,gop,"
            ...
            "catalog,return,retmap,deepen wave=%u areas_expect=60 soft PASS\n",
            (unsigned)BI_SOFT_WAVE);
    cAreas++;
    ...
    kprintf("boot: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=80\n");
...
kprintf("boot: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=80 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: boot: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("boot: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=80 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("boot: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=80 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: boot: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("boot: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=80 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("boot: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=80 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: boot: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("boot: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=80 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: boot: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("boot: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=80 (retcurtainface honesty; Soft≠product; not bar3)\n");
/* Grep: boot: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("boot: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=80 (retbastionangle stamp; Soft≠product)\n");
/* Grep: boot: soft retglacisangle — Wave 74 return-glacisangle honesty */
kprintf("boot: soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=80 (retglacisangle honesty; Soft≠product; not bar3)\n");
/* Grep: boot: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
kprintf("boot: soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=80 (retparapetangle stamp; Soft≠product)\n");
/* Grep: boot: soft retmoatangle — Wave 75 return-moatangle honesty */
kprintf("boot: soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=80 (retmoatangle honesty; Soft≠product; not bar3)\n");
/* Grep: boot: soft retowerangle — Wave 75 exclusive towerangle stamp */
kprintf("boot: soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=80 (retowerangle stamp; Soft≠product)\n");
/* Grep: boot: soft retgateangle — Wave 76 return-gateangle honesty */
kprintf("boot: soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=80 (retgateangle honesty; Soft≠product; not bar3)\n");
/* Grep: boot: soft retwallangle — Wave 76 exclusive wallangle stamp */
kprintf("boot: soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=80 (retwallangle stamp; Soft≠product)\n");
/* Grep: boot: soft retspireangle — Wave 77 return-spireangle honesty */
kprintf("boot: soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=80 (retspireangle honesty; Soft≠product; not bar3)\n");
/* Grep: boot: soft retholdangle — Wave 77 exclusive holdangle stamp */
kprintf("boot: soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=80 (retholdangle stamp; Soft≠product)\n");
/* Grep: boot: soft retpalaceangle — Wave 78 return-palaceangle honesty */
kprintf("boot: soft retpalaceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=80 (retpalaceangle honesty; Soft≠product; not bar3)\n");
/* Grep: boot: soft retfortressangle — Wave 78 exclusive fortressangle stamp */
kprintf("boot: soft retfortressangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=80 (retfortressangle stamp; Soft≠product)\n");
/* Grep: boot: soft retkeepangle — Wave 79 return-keepangle honesty */
kprintf("boot: soft retkeepangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=80 (retkeepangle honesty; Soft≠product; not bar3)\n");
/* Grep: boot: soft retredoubtangle — Wave 79 exclusive redoubtangle stamp */
kprintf("boot: soft retredoubtangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=80 (retredoubtangle stamp; Soft≠product)\n");
/* Grep: boot: soft retcitadelangle — Wave 80 return-citadelangle honesty */
kprintf("boot: soft retcitadelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=80 (retcitadelangle honesty; Soft≠product; not bar3)\n");
/* Grep: boot: soft retbastionkeep — Wave 80 exclusive bastionkeep stamp */
kprintf("boot: soft retbastionkeep exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=80 (retbastionkeep stamp; Soft≠product)\n");
