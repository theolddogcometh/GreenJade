 ...
 *
 * Wave 23 exclusive soft deepen — greppable prefix-stable "boot: soft …"
 ...
 *   boot: soft deepen        — wave=23 stamp + area count
...
/* Wave 23 exclusive soft deepen stamp (this unit only). */
#define BI_SOFT_WAVE 23u
                        ...
                        const char *szHandoff)
{
    ...
    kprintf("boot: soft note milestone=wave23 exclusive=1 "
    ...
    kprintf("boot: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=23\n");
 ...
 * Soft≠product; not bar3.
            */
            /* Grep: boot: soft rettoken — Wave 23 return-token honesty */
            kprintf("boot: soft rettoken soft_only=1 product_gate=0 soft_ne_product=1 "
                    "never_blocks_m0=1 wave=%u "
                    "(rettoken honesty; Soft≠product; not bar3)\n",
                    (unsigned)BI_SOFT_WAVE);
            /* Grep: boot: soft retcrest — Wave 23 exclusive crest stamp */
