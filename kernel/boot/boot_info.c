 ...
 *   boot: soft deepen        — wave=64 stamp + area count
                        ...
                        const char *szHandoff)
{
    ...
    kprintf("boot: soft note milestone=wave64 exclusive=1 "
    ...
    kprintf("boot: soft catalog honesty,inventory,path,handoff,memmap,gop,"
            ...
            "catalog,return,retmap,deepen wave=%u areas_expect=44 soft PASS\n",
            (unsigned)BI_SOFT_WAVE);
    cAreas++;
    ...
    kprintf("boot: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=64\n");
