 ...
 *   identity: soft deepen     — wave=89 stamp + area count
...
#define IDMAP_SOFT_WAVE 89u /* Wave 51 exclusive soft deepen stamp */
...
identity_soft_inventory(u64 u64Cr3Expect, u64 u64Cr3Read, u32 cLeavesBuilt)
{
    ...
    kprintf("identity: soft note milestone=wave89 exclusive=1 "
    ...
    kprintf("identity: soft catalog honesty,verdict,inventory,layout,pd,"
            ...
            "twin,install,catalog,return,retmap,deepen wave=%u areas_expect=62 soft PASS\n",
            (unsigned)IDMAP_SOFT_WAVE);
    cAreas++;
    ...
    kprintf("identity: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=89\n");
...
kprintf("identity: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=89 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("identity: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=89 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("identity: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=89 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("identity: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=89 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("identity: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=89 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("identity: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=89 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: identity: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("identity: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=89 (retcurtainface honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("identity: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=89 (retbastionangle stamp; Soft≠product)\n");
/* Grep: identity: soft retglacisangle — Wave 74 return-glacisangle honesty */
kprintf("identity: soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=89 (retglacisangle honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
kprintf("identity: soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=89 (retparapetangle stamp; Soft≠product)\n");
/* Grep: identity: soft retmoatangle — Wave 75 return-moatangle honesty */
kprintf("identity: soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=89 (retmoatangle honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retowerangle — Wave 75 exclusive towerangle stamp */
kprintf("identity: soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=89 (retowerangle stamp; Soft≠product)\n");
/* Grep: identity: soft retgateangle — Wave 76 return-gateangle honesty */
kprintf("identity: soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=89 (retgateangle honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retwallangle — Wave 76 exclusive wallangle stamp */
kprintf("identity: soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=89 (retwallangle stamp; Soft≠product)\n");
/* Grep: identity: soft retspireangle — Wave 77 return-spireangle honesty */
kprintf("identity: soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=89 (retspireangle honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retholdangle — Wave 77 exclusive holdangle stamp */
kprintf("identity: soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=89 (retholdangle stamp; Soft≠product)\n");
/* Grep: identity: soft retpalaceangle — Wave 78 return-palaceangle honesty */
kprintf("identity: soft retpalaceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=89 (retpalaceangle honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retfortressangle — Wave 78 exclusive fortressangle stamp */
kprintf("identity: soft retfortressangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=89 (retfortressangle stamp; Soft≠product)\n");
/* Grep: identity: soft retkeepangle — Wave 79 return-keepangle honesty */
kprintf("identity: soft retkeepangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=89 (retkeepangle honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retredoubtangle — Wave 79 exclusive redoubtangle stamp */
kprintf("identity: soft retredoubtangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=89 (retredoubtangle stamp; Soft≠product)\n");
/* Grep: identity: soft retcitadelangle — Wave 80 return-citadelangle honesty */
kprintf("identity: soft retcitadelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=89 (retcitadelangle honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retbastionkeep — Wave 80 exclusive bastionkeep stamp */
kprintf("identity: soft retbastionkeep exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=89 (retbastionkeep stamp; Soft≠product)\n");
/* Grep: identity: soft retpanoplyangle — Wave 81 return-panoplyangle honesty */
kprintf("identity: soft retpanoplyangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=89 (retpanoplyangle honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retbulwarkangle — Wave 81 exclusive bulwarkangle stamp */
kprintf("identity: soft retbulwarkangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=89 (retbulwarkangle stamp; Soft≠product)\n");
/* Grep: identity: soft retmantleangle — Wave 82 return-mantleangle honesty */
kprintf("identity: soft retmantleangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=89 (retmantleangle honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retaegisangle — Wave 82 exclusive aegisangle stamp */
kprintf("identity: soft retaegisangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=89 (retaegisangle stamp; Soft≠product)\n");
/* Grep: identity: soft retemblemangle — Wave 83 return-emblemangle honesty */
kprintf("identity: soft retemblemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=89 (retemblemangle honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retsigilangle — Wave 83 exclusive sigilangle stamp */
kprintf("identity: soft retsigilangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=89 (retsigilangle stamp; Soft≠product)\n");
/* Grep: identity: soft retscepterangle — Wave 84 return-scepterangle honesty */
kprintf("identity: soft retscepterangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=89 (retscepterangle honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retglyphangle — Wave 84 exclusive glyphangle stamp */
kprintf("identity: soft retglyphangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=89 (retglyphangle stamp; Soft≠product)\n");
/* Grep: identity: soft retcrownangle — Wave 85 return-crownangle honesty */
kprintf("identity: soft retcrownangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=89 (retcrownangle honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retshardangle — Wave 85 exclusive shardangle stamp */
kprintf("identity: soft retshardangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=89 (retshardangle stamp; Soft≠product)\n");
/* Grep: identity: soft retforgeangle — Wave 86 return-forgeangle honesty */
kprintf("identity: soft retforgeangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=89 (retforgeangle honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retprismangle — Wave 86 exclusive prismangle stamp */
kprintf("identity: soft retprismangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=89 (retprismangle stamp; Soft≠product)\n");
/* Grep: identity: soft retflameangle — Wave 87 return-flameangle honesty */
kprintf("identity: soft retflameangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=89 (retflameangle honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retcipherangle — Wave 87 exclusive cipherangle stamp */
kprintf("identity: soft retcipherangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=89 (retcipherangle stamp; Soft≠product)\n");
/* Grep: identity: soft retbeaconangle — Wave 88 return-beaconangle honesty */
kprintf("identity: soft retbeaconangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=89 (retbeaconangle honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retledgerangle — Wave 88 exclusive ledgerangle stamp */
kprintf("identity: soft retledgerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=89 (retledgerangle stamp; Soft≠product)\n");
/* Grep: identity: soft retbannerangle — Wave 89 return-bannerangle honesty */
kprintf("identity: soft retbannerangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=89 (retbannerangle honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retvaultangle — Wave 89 exclusive vaultangle stamp */
kprintf("identity: soft retvaultangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=89 (retvaultangle stamp; Soft≠product)\n");
