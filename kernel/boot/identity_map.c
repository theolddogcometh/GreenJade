 ...
 *
 * Wave 16 exclusive soft deepen (this unit only — greppable "identity: soft …"):
 ...
 *   identity: soft catalog    — Wave 16 area name rollup
 *   identity: soft deepen     — wave=16 stamp + area count
...
#define IDMAP_SOFT_WAVE     16u /* Wave 16 exclusive soft deepen stamp */
 ...
 *
 * Wave 16 exclusive soft deepen — prefix-stable greppable markers:
...
identity_soft_inventory(u64 u64Cr3Expect, u64 u64Cr3Read, u32 cLeavesBuilt)
{
    ...
    /* Grep: identity: soft surfaces (Wave 16 deepen) */
    kprintf("identity: soft surfaces count=%u wave=%u "
            "names=honesty,verdict,inventory,layout,pd,geometry,flags,"
            "leaf,zero,twin,install,catalog,surfaces,note,deepen\n",
            (unsigned)cAreas + 3u, (unsigned)IDMAP_SOFT_WAVE);
    cAreas++;

    /* Grep: identity: soft note (Wave 16 deepen) */
    kprintf("identity: soft note milestone=wave16 exclusive=1 "
            "bridge_only=1 soft_only=1 not_bar3=1 wave=%u\n",
            (unsigned)IDMAP_SOFT_WAVE);
    cAreas++;

    /* Grep: identity: soft catalog — Wave 16 area name rollup. */
    ...
    /* Grep: identity: soft deepen — Wave 16 stamp + area count. */
...
boot_install_identity_4gib(void)
{
    ...
    /* Grep: identity: soft … (Wave 16 exclusive soft inventory deepen) */
