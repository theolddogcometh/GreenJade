 ...
 *   identity: soft deepen     — wave=81 stamp + area count
...
#define IDMAP_SOFT_WAVE 81u /* Wave 51 exclusive soft deepen stamp */
...
identity_soft_inventory(u64 u64Cr3Expect, u64 u64Cr3Read, u32 cLeavesBuilt)
{
    ...
    kprintf("identity: soft note milestone=wave81 exclusive=1 "
    ...
    kprintf("identity: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=81\n");
...
kprintf("identity: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=81 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("identity: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=81 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("identity: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=81 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("identity: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=81 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("identity: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=81 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("identity: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=81 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: identity: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("identity: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=81 (retcurtainface honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("identity: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=81 (retbastionangle stamp; Soft≠product)\n");
/* Grep: identity: soft retglacisangle — Wave 74 return-glacisangle honesty */
kprintf("identity: soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=81 (retglacisangle honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
kprintf("identity: soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=81 (retparapetangle stamp; Soft≠product)\n");
/* Grep: identity: soft retmoatangle — Wave 75 return-moatangle honesty */
kprintf("identity: soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=81 (retmoatangle honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retowerangle — Wave 75 exclusive towerangle stamp */
kprintf("identity: soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=81 (retowerangle stamp; Soft≠product)\n");
/* Grep: identity: soft retgateangle — Wave 76 return-gateangle honesty */
kprintf("identity: soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=81 (retgateangle honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retwallangle — Wave 76 exclusive wallangle stamp */
kprintf("identity: soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=81 (retwallangle stamp; Soft≠product)\n");
/* Grep: identity: soft retspireangle — Wave 77 return-spireangle honesty */
kprintf("identity: soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=81 (retspireangle honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retholdangle — Wave 77 exclusive holdangle stamp */
kprintf("identity: soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=81 (retholdangle stamp; Soft≠product)\n");
/* Grep: identity: soft retpalaceangle — Wave 78 return-palaceangle honesty */
kprintf("identity: soft retpalaceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=81 (retpalaceangle honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retfortressangle — Wave 78 exclusive fortressangle stamp */
kprintf("identity: soft retfortressangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=81 (retfortressangle stamp; Soft≠product)\n");
/* Grep: identity: soft retkeepangle — Wave 79 return-keepangle honesty */
kprintf("identity: soft retkeepangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=81 (retkeepangle honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retredoubtangle — Wave 79 exclusive redoubtangle stamp */
kprintf("identity: soft retredoubtangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=81 (retredoubtangle stamp; Soft≠product)\n");
/* Grep: identity: soft retcitadelangle — Wave 80 return-citadelangle honesty */
kprintf("identity: soft retcitadelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=81 (retcitadelangle honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retbastionkeep — Wave 80 exclusive bastionkeep stamp */
kprintf("identity: soft retbastionkeep exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=81 (retbastionkeep stamp; Soft≠product)\n");
/* Grep: identity: soft retpanoplyangle — Wave 81 return-panoplyangle honesty */
kprintf("identity: soft retpanoplyangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=81 (retpanoplyangle honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retbulwarkangle — Wave 81 exclusive bulwarkangle stamp */
kprintf("identity: soft retbulwarkangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=81 (retbulwarkangle stamp; Soft≠product)\n");
