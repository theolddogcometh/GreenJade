 ...
 *   identity: soft deepen     — wave=85 stamp + area count
...
#define IDMAP_SOFT_WAVE 85u /* Wave 51 exclusive soft deepen stamp */
...
identity_soft_inventory(u64 u64Cr3Expect, u64 u64Cr3Read, u32 cLeavesBuilt)
{
    ...
    kprintf("identity: soft note milestone=wave85 exclusive=1 "
    ...
    kprintf("identity: soft catalog honesty,verdict,inventory,layout,pd,"
            ...
            "twin,install,catalog,return,retmap,deepen wave=%u areas_expect=58 soft PASS\n",
            (unsigned)IDMAP_SOFT_WAVE);
    cAreas++;
    ...
    kprintf("identity: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=85\n");
...
kprintf("identity: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("identity: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("identity: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("identity: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("identity: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("identity: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: identity: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("identity: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retcurtainface honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("identity: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retbastionangle stamp; Soft≠product)\n");
/* Grep: identity: soft retglacisangle — Wave 74 return-glacisangle honesty */
kprintf("identity: soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retglacisangle honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
kprintf("identity: soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retparapetangle stamp; Soft≠product)\n");
/* Grep: identity: soft retmoatangle — Wave 75 return-moatangle honesty */
kprintf("identity: soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retmoatangle honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retowerangle — Wave 75 exclusive towerangle stamp */
kprintf("identity: soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retowerangle stamp; Soft≠product)\n");
/* Grep: identity: soft retgateangle — Wave 76 return-gateangle honesty */
kprintf("identity: soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retgateangle honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retwallangle — Wave 76 exclusive wallangle stamp */
kprintf("identity: soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retwallangle stamp; Soft≠product)\n");
/* Grep: identity: soft retspireangle — Wave 77 return-spireangle honesty */
kprintf("identity: soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retspireangle honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retholdangle — Wave 77 exclusive holdangle stamp */
kprintf("identity: soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retholdangle stamp; Soft≠product)\n");
/* Grep: identity: soft retpalaceangle — Wave 78 return-palaceangle honesty */
kprintf("identity: soft retpalaceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retpalaceangle honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retfortressangle — Wave 78 exclusive fortressangle stamp */
kprintf("identity: soft retfortressangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retfortressangle stamp; Soft≠product)\n");
/* Grep: identity: soft retkeepangle — Wave 79 return-keepangle honesty */
kprintf("identity: soft retkeepangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retkeepangle honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retredoubtangle — Wave 79 exclusive redoubtangle stamp */
kprintf("identity: soft retredoubtangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retredoubtangle stamp; Soft≠product)\n");
/* Grep: identity: soft retcitadelangle — Wave 80 return-citadelangle honesty */
kprintf("identity: soft retcitadelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retcitadelangle honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retbastionkeep — Wave 80 exclusive bastionkeep stamp */
kprintf("identity: soft retbastionkeep exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retbastionkeep stamp; Soft≠product)\n");
/* Grep: identity: soft retpanoplyangle — Wave 81 return-panoplyangle honesty */
kprintf("identity: soft retpanoplyangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retpanoplyangle honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retbulwarkangle — Wave 81 exclusive bulwarkangle stamp */
kprintf("identity: soft retbulwarkangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retbulwarkangle stamp; Soft≠product)\n");
/* Grep: identity: soft retmantleangle — Wave 82 return-mantleangle honesty */
kprintf("identity: soft retmantleangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retmantleangle honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retaegisangle — Wave 82 exclusive aegisangle stamp */
kprintf("identity: soft retaegisangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retaegisangle stamp; Soft≠product)\n");
/* Grep: identity: soft retemblemangle — Wave 83 return-emblemangle honesty */
kprintf("identity: soft retemblemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retemblemangle honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retsigilangle — Wave 83 exclusive sigilangle stamp */
kprintf("identity: soft retsigilangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retsigilangle stamp; Soft≠product)\n");
/* Grep: identity: soft retscepterangle — Wave 84 return-scepterangle honesty */
kprintf("identity: soft retscepterangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retscepterangle honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retglyphangle — Wave 84 exclusive glyphangle stamp */
kprintf("identity: soft retglyphangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retglyphangle stamp; Soft≠product)\n");
/* Grep: identity: soft retcrownangle — Wave 85 return-crownangle honesty */
kprintf("identity: soft retcrownangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retcrownangle honesty; Soft≠product; not bar3)\n");
/* Grep: identity: soft retshardangle — Wave 85 exclusive shardangle stamp */
kprintf("identity: soft retshardangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retshardangle stamp; Soft≠product)\n");
