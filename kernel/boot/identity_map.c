 ...
 *
 * Wave 17 exclusive soft deepen (this unit only — greppable "identity: soft …"):
 ...
 *   identity: soft catalog    — Wave 17 area name rollup
 *   identity: soft deepen     — wave=17 stamp + area count
...
#define IDMAP_SOFT_WAVE     17u /* Wave 17 exclusive soft deepen stamp */
 ...
 *
 * Wave 17 exclusive soft deepen — prefix-stable greppable markers:
...
identity_soft_inventory(u64 u64Cr3Expect, u64 u64Cr3Read, u32 cLeavesBuilt)
{
    ...
    /* Grep: identity: soft surfaces (Wave 17 deepen) */
    kprintf("identity: soft surfaces count=%u wave=%u "
            "names=honesty,verdict,inventory,layout,pd,geometry,flags,"
            "leaf,zero,twin,install,catalog,surfaces,note,return,deepen\n",
            (unsigned)cAreas + 4u, (unsigned)IDMAP_SOFT_WAVE);
    cAreas++;

    /* Grep: identity: soft note (Wave 17 deepen) */
    kprintf("identity: soft note milestone=wave17 exclusive=1 "
            "bridge_only=1 soft_only=1 not_bar3=1 wave=%u\n",
            (unsigned)IDMAP_SOFT_WAVE);
    cAreas++;

    /* Grep: identity: soft catalog — Wave 17 area name rollup. */
    kprintf("identity: soft catalog honesty,verdict,inventory,layout,pd,"
            "geometry,flags,cover,cr3,link,pml4,stats,path,leaf,zero,"
            "twin,install,catalog,return,deepen wave=%u areas_expect=20 soft PASS\n",
            (unsigned)IDMAP_SOFT_WAVE);
    cAreas++;

    /* Grep: identity: soft return (Wave 17 deepen) */
    ...
    /* Grep: identity: soft deepen — Wave 17 stamp + area count. */
...
boot_install_identity_4gib(void)
{
    ...
    /* Grep: identity: soft … (Wave 17 exclusive soft inventory deepen) */
