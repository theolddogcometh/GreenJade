 ...
 *   GJ-EFI: soft deepen     — wave=85 stamp + area catalog
...
soft_wave14_inventory(u64 u64Entry)
{
    ...
    com1_puts("GJ-EFI: soft note milestone=wave85 exclusive=1 "
    ...
    com1_put_u64_dec((u64)GJ_EFI_SOFT_WAVE);
    com1_puts(" areas_expect=55 soft PASS\n");
    cAreas++;
...
kprintf("soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retcurtainface honesty; Soft≠product; not bar3)\n");
/* Grep: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retbastionangle stamp; Soft≠product)\n");
/* Grep: soft retglacisangle — Wave 74 return-glacisangle honesty */
kprintf("soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retglacisangle honesty; Soft≠product; not bar3)\n");
/* Grep: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
kprintf("soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retparapetangle stamp; Soft≠product)\n");
/* Grep: soft retmoatangle — Wave 75 return-moatangle honesty */
kprintf("soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retmoatangle honesty; Soft≠product; not bar3)\n");
/* Grep: soft retowerangle — Wave 75 exclusive towerangle stamp */
kprintf("soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retowerangle stamp; Soft≠product)\n");
/* Grep: soft retgateangle — Wave 76 return-gateangle honesty */
kprintf("soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retgateangle honesty; Soft≠product; not bar3)\n");
/* Grep: soft retwallangle — Wave 76 exclusive wallangle stamp */
kprintf("soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retwallangle stamp; Soft≠product)\n");
/* Grep: soft retspireangle — Wave 77 return-spireangle honesty */
kprintf("soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retspireangle honesty; Soft≠product; not bar3)\n");
/* Grep: soft retholdangle — Wave 77 exclusive holdangle stamp */
kprintf("soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retholdangle stamp; Soft≠product)\n");
/* Grep: soft retpalaceangle — Wave 78 return-palaceangle honesty */
kprintf("soft retpalaceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retpalaceangle honesty; Soft≠product; not bar3)\n");
/* Grep: soft retfortressangle — Wave 78 exclusive fortressangle stamp */
kprintf("soft retfortressangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retfortressangle stamp; Soft≠product)\n");
/* Grep: soft retkeepangle — Wave 79 return-keepangle honesty */
kprintf("soft retkeepangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retkeepangle honesty; Soft≠product; not bar3)\n");
/* Grep: soft retredoubtangle — Wave 79 exclusive redoubtangle stamp */
kprintf("soft retredoubtangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retredoubtangle stamp; Soft≠product)\n");
/* Grep: soft retcitadelangle — Wave 80 return-citadelangle honesty */
kprintf("soft retcitadelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retcitadelangle honesty; Soft≠product; not bar3)\n");
/* Grep: soft retbastionkeep — Wave 80 exclusive bastionkeep stamp */
kprintf("soft retbastionkeep exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retbastionkeep stamp; Soft≠product)\n");
/* Grep: soft retpanoplyangle — Wave 81 return-panoplyangle honesty */
kprintf("soft retpanoplyangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retpanoplyangle honesty; Soft≠product; not bar3)\n");
/* Grep: soft retbulwarkangle — Wave 81 exclusive bulwarkangle stamp */
kprintf("soft retbulwarkangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retbulwarkangle stamp; Soft≠product)\n");
/* Grep: soft retmantleangle — Wave 82 return-mantleangle honesty */
kprintf("soft retmantleangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retmantleangle honesty; Soft≠product; not bar3)\n");
/* Grep: soft retaegisangle — Wave 82 exclusive aegisangle stamp */
kprintf("soft retaegisangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retaegisangle stamp; Soft≠product)\n");
/* Grep: soft retemblemangle — Wave 83 return-emblemangle honesty */
kprintf("soft retemblemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retemblemangle honesty; Soft≠product; not bar3)\n");
/* Grep: soft retsigilangle — Wave 83 exclusive sigilangle stamp */
kprintf("soft retsigilangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retsigilangle stamp; Soft≠product)\n");
/* Grep: soft retscepterangle — Wave 84 return-scepterangle honesty */
kprintf("soft retscepterangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retscepterangle honesty; Soft≠product; not bar3)\n");
/* Grep: soft retglyphangle — Wave 84 exclusive glyphangle stamp */
kprintf("soft retglyphangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retglyphangle stamp; Soft≠product)\n");
/* Grep: soft retcrownangle — Wave 85 return-crownangle honesty */
kprintf("soft retcrownangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retcrownangle honesty; Soft≠product; not bar3)\n");
/* Grep: soft retshardangle — Wave 85 exclusive shardangle stamp */
kprintf("soft retshardangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retshardangle stamp; Soft≠product)\n");
