...
smp_bringup_soft_log(void)
{
    ...
    kprintf("smp: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=88\n");
...
kprintf("smp: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: smp: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("smp: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("smp: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: smp: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("smp: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("smp: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: smp: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("smp: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: smp: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("smp: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retcurtainface honesty; Soft≠product; not bar3)\n");
/* Grep: smp: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("smp: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retbastionangle stamp; Soft≠product)\n");
/* Grep: smp: soft retglacisangle — Wave 74 return-glacisangle honesty */
kprintf("smp: soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retglacisangle honesty; Soft≠product; not bar3)\n");
/* Grep: smp: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
kprintf("smp: soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retparapetangle stamp; Soft≠product)\n");
/* Grep: smp: soft retmoatangle — Wave 75 return-moatangle honesty */
kprintf("smp: soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retmoatangle honesty; Soft≠product; not bar3)\n");
/* Grep: smp: soft retowerangle — Wave 75 exclusive towerangle stamp */
kprintf("smp: soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retowerangle stamp; Soft≠product)\n");
/* Grep: smp: soft retgateangle — Wave 76 return-gateangle honesty */
kprintf("smp: soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retgateangle honesty; Soft≠product; not bar3)\n");
/* Grep: smp: soft retwallangle — Wave 76 exclusive wallangle stamp */
kprintf("smp: soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retwallangle stamp; Soft≠product)\n");
/* Grep: smp: soft retspireangle — Wave 77 return-spireangle honesty */
kprintf("smp: soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retspireangle honesty; Soft≠product; not bar3)\n");
/* Grep: smp: soft retholdangle — Wave 77 exclusive holdangle stamp */
kprintf("smp: soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retholdangle stamp; Soft≠product)\n");
/* Grep: smp: soft retpalaceangle — Wave 78 return-palaceangle honesty */
kprintf("smp: soft retpalaceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retpalaceangle honesty; Soft≠product; not bar3)\n");
/* Grep: smp: soft retfortressangle — Wave 78 exclusive fortressangle stamp */
kprintf("smp: soft retfortressangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retfortressangle stamp; Soft≠product)\n");
/* Grep: smp: soft retkeepangle — Wave 79 return-keepangle honesty */
kprintf("smp: soft retkeepangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retkeepangle honesty; Soft≠product; not bar3)\n");
/* Grep: smp: soft retredoubtangle — Wave 79 exclusive redoubtangle stamp */
kprintf("smp: soft retredoubtangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retredoubtangle stamp; Soft≠product)\n");
/* Grep: smp: soft retcitadelangle — Wave 80 return-citadelangle honesty */
kprintf("smp: soft retcitadelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retcitadelangle honesty; Soft≠product; not bar3)\n");
/* Grep: smp: soft retbastionkeep — Wave 80 exclusive bastionkeep stamp */
kprintf("smp: soft retbastionkeep exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retbastionkeep stamp; Soft≠product)\n");
/* Grep: smp: soft retpanoplyangle — Wave 81 return-panoplyangle honesty */
kprintf("smp: soft retpanoplyangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retpanoplyangle honesty; Soft≠product; not bar3)\n");
/* Grep: smp: soft retbulwarkangle — Wave 81 exclusive bulwarkangle stamp */
kprintf("smp: soft retbulwarkangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retbulwarkangle stamp; Soft≠product)\n");
/* Grep: smp: soft retmantleangle — Wave 82 return-mantleangle honesty */
kprintf("smp: soft retmantleangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retmantleangle honesty; Soft≠product; not bar3)\n");
/* Grep: smp: soft retaegisangle — Wave 82 exclusive aegisangle stamp */
kprintf("smp: soft retaegisangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retaegisangle stamp; Soft≠product)\n");
/* Grep: smp: soft retemblemangle — Wave 83 return-emblemangle honesty */
kprintf("smp: soft retemblemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retemblemangle honesty; Soft≠product; not bar3)\n");
/* Grep: smp: soft retsigilangle — Wave 83 exclusive sigilangle stamp */
kprintf("smp: soft retsigilangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retsigilangle stamp; Soft≠product)\n");
/* Grep: smp: soft retscepterangle — Wave 84 return-scepterangle honesty */
kprintf("smp: soft retscepterangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retscepterangle honesty; Soft≠product; not bar3)\n");
/* Grep: smp: soft retglyphangle — Wave 84 exclusive glyphangle stamp */
kprintf("smp: soft retglyphangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retglyphangle stamp; Soft≠product)\n");
/* Grep: smp: soft retcrownangle — Wave 85 return-crownangle honesty */
kprintf("smp: soft retcrownangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retcrownangle honesty; Soft≠product; not bar3)\n");
/* Grep: smp: soft retshardangle — Wave 85 exclusive shardangle stamp */
kprintf("smp: soft retshardangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retshardangle stamp; Soft≠product)\n");
/* Grep: smp: soft retforgeangle — Wave 86 return-forgeangle honesty */
kprintf("smp: soft retforgeangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retforgeangle honesty; Soft≠product; not bar3)\n");
/* Grep: smp: soft retprismangle — Wave 86 exclusive prismangle stamp */
kprintf("smp: soft retprismangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retprismangle stamp; Soft≠product)\n");
/* Grep: smp: soft retflameangle — Wave 87 return-flameangle honesty */
kprintf("smp: soft retflameangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retflameangle honesty; Soft≠product; not bar3)\n");
/* Grep: smp: soft retcipherangle — Wave 87 exclusive cipherangle stamp */
kprintf("smp: soft retcipherangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retcipherangle stamp; Soft≠product)\n");
/* Grep: smp: soft retbeaconangle — Wave 88 return-beaconangle honesty */
kprintf("smp: soft retbeaconangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retbeaconangle honesty; Soft≠product; not bar3)\n");
/* Grep: smp: soft retledgerangle — Wave 88 exclusive ledgerangle stamp */
kprintf("smp: soft retledgerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retledgerangle stamp; Soft≠product)\n");

                                kprintf("smp: soft deepen wave=88 areas=119 verdict=%s tried=%u "
        ...
        kprintf("smp: soft ret_surface bringup=ok|timeout|skip|status_fail "
                ...
                "skip=dis|bsp|cap product_kernel=OPEN areas=71 wave=88\n");
        ...
        kprintf("smp: soft surface deepen,verdict,handshake,online,sipi,"
                ...
                "PASS,inventory,ap_run areas=71 wave=88\n");
