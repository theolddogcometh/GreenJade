 ...
 *
 * Wave 33 exclusive soft deepen (this unit only — greppable "smp: soft …"):
...
smp_bringup_soft_log(void)
{
    ...
    kprintf("smp: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=33\n");
 ...
 * Soft≠product; not bar3.
 */
/* Grep: smp: soft retbastion — Wave 33 return-bastion honesty */
kprintf("smp: soft retbastion soft_only=1 product_gate=0 soft_ne_product=1 "
        "never_blocks_m0=1 wave=33 "
        "(retbastion honesty; Soft≠product; not bar3)\n");
/* Grep: smp: soft retcitadel — Wave 33 exclusive citadel stamp */
kprintf("smp: soft retcitadel exclusive=1 soft_ne_product=1 "
        "product_kernel=OPEN bar3=0 wave=33 "
        "(retcitadel stamp; Soft≠product)\n");
                                kprintf("smp: soft deepen wave=33 areas=29 verdict=%s tried=%u "
        ...
        kprintf("smp: soft ret_surface bringup=ok|timeout|skip|status_fail "
                ...
                "skip=dis|bsp|cap product_kernel=OPEN areas=25 wave=33\n");
        ...
        kprintf("smp: soft surface deepen,verdict,handshake,online,sipi,"
                ...
                "PASS,inventory,ap_run areas=25 wave=33\n");
