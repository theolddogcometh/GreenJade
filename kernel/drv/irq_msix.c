 ...
 *   irq: soft deepen    / irq_msix: soft deepen     — wave=83 areas stamp
...
#define IRQ_MSIX_SOFT_DEEPEN_AREAS 110u
...
kprintf("irq_msix: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: irq_msix: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("irq_msix: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("irq_msix: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: irq_msix: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("irq_msix: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("irq_msix: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: irq_msix: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("irq_msix: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: irq_msix: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("irq_msix: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retcurtainface honesty; Soft≠product; not bar3)\n");
/* Grep: irq_msix: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("irq_msix: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retbastionangle stamp; Soft≠product)\n");
/* Grep: irq_msix: soft retglacisangle — Wave 74 return-glacisangle honesty */
kprintf("irq_msix: soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retglacisangle honesty; Soft≠product; not bar3)\n");
/* Grep: irq_msix: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
kprintf("irq_msix: soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retparapetangle stamp; Soft≠product)\n");
/* Grep: irq_msix: soft retmoatangle — Wave 75 return-moatangle honesty */
kprintf("irq_msix: soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retmoatangle honesty; Soft≠product; not bar3)\n");
/* Grep: irq_msix: soft retowerangle — Wave 75 exclusive towerangle stamp */
kprintf("irq_msix: soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retowerangle stamp; Soft≠product)\n");
/* Grep: irq_msix: soft retgateangle — Wave 76 return-gateangle honesty */
kprintf("irq_msix: soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retgateangle honesty; Soft≠product; not bar3)\n");
/* Grep: irq_msix: soft retwallangle — Wave 76 exclusive wallangle stamp */
kprintf("irq_msix: soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retwallangle stamp; Soft≠product)\n");
/* Grep: irq_msix: soft retspireangle — Wave 77 return-spireangle honesty */
kprintf("irq_msix: soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retspireangle honesty; Soft≠product; not bar3)\n");
/* Grep: irq_msix: soft retholdangle — Wave 77 exclusive holdangle stamp */
kprintf("irq_msix: soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retholdangle stamp; Soft≠product)\n");
/* Grep: irq_msix: soft retpalaceangle — Wave 78 return-palaceangle honesty */
kprintf("irq_msix: soft retpalaceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retpalaceangle honesty; Soft≠product; not bar3)\n");
/* Grep: irq_msix: soft retfortressangle — Wave 78 exclusive fortressangle stamp */
kprintf("irq_msix: soft retfortressangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retfortressangle stamp; Soft≠product)\n");
/* Grep: irq_msix: soft retkeepangle — Wave 79 return-keepangle honesty */
kprintf("irq_msix: soft retkeepangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retkeepangle honesty; Soft≠product; not bar3)\n");
/* Grep: irq_msix: soft retredoubtangle — Wave 79 exclusive redoubtangle stamp */
kprintf("irq_msix: soft retredoubtangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retredoubtangle stamp; Soft≠product)\n");
/* Grep: irq_msix: soft retcitadelangle — Wave 80 return-citadelangle honesty */
kprintf("irq_msix: soft retcitadelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retcitadelangle honesty; Soft≠product; not bar3)\n");
/* Grep: irq_msix: soft retbastionkeep — Wave 80 exclusive bastionkeep stamp */
kprintf("irq_msix: soft retbastionkeep exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retbastionkeep stamp; Soft≠product)\n");
/* Grep: irq_msix: soft retpanoplyangle — Wave 81 return-panoplyangle honesty */
kprintf("irq_msix: soft retpanoplyangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retpanoplyangle honesty; Soft≠product; not bar3)\n");
/* Grep: irq_msix: soft retbulwarkangle — Wave 81 exclusive bulwarkangle stamp */
kprintf("irq_msix: soft retbulwarkangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retbulwarkangle stamp; Soft≠product)\n");
/* Grep: irq_msix: soft retmantleangle — Wave 82 return-mantleangle honesty */
kprintf("irq_msix: soft retmantleangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retmantleangle honesty; Soft≠product; not bar3)\n");
/* Grep: irq_msix: soft retaegisangle — Wave 82 exclusive aegisangle stamp */
kprintf("irq_msix: soft retaegisangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retaegisangle stamp; Soft≠product)\n");
/* Grep: irq_msix: soft retemblemangle — Wave 83 return-emblemangle honesty */
kprintf("irq_msix: soft retemblemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retemblemangle honesty; Soft≠product; not bar3)\n");
/* Grep: irq_msix: soft retsigilangle — Wave 83 exclusive sigilangle stamp */
kprintf("irq_msix: soft retsigilangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retsigilangle stamp; Soft≠product)\n");
