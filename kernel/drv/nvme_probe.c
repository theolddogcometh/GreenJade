 ...
 *   nvme: soft deepen     — wave=83 areas stamp
...
#define NVME_SOFT_DEEPEN_AREAS 112u
...
kprintf("nvme: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: nvme: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("nvme: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("nvme: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: nvme: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("nvme: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("nvme: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: nvme: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("nvme: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: nvme: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("nvme: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retcurtainface honesty; Soft≠product; not bar3)\n");
/* Grep: nvme: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("nvme: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retbastionangle stamp; Soft≠product)\n");
/* Grep: nvme: soft retglacisangle — Wave 74 return-glacisangle honesty */
kprintf("nvme: soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retglacisangle honesty; Soft≠product; not bar3)\n");
/* Grep: nvme: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
kprintf("nvme: soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retparapetangle stamp; Soft≠product)\n");
/* Grep: nvme: soft retmoatangle — Wave 75 return-moatangle honesty */
kprintf("nvme: soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retmoatangle honesty; Soft≠product; not bar3)\n");
/* Grep: nvme: soft retowerangle — Wave 75 exclusive towerangle stamp */
kprintf("nvme: soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retowerangle stamp; Soft≠product)\n");
/* Grep: nvme: soft retgateangle — Wave 76 return-gateangle honesty */
kprintf("nvme: soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retgateangle honesty; Soft≠product; not bar3)\n");
/* Grep: nvme: soft retwallangle — Wave 76 exclusive wallangle stamp */
kprintf("nvme: soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retwallangle stamp; Soft≠product)\n");
/* Grep: nvme: soft retspireangle — Wave 77 return-spireangle honesty */
kprintf("nvme: soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retspireangle honesty; Soft≠product; not bar3)\n");
/* Grep: nvme: soft retholdangle — Wave 77 exclusive holdangle stamp */
kprintf("nvme: soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retholdangle stamp; Soft≠product)\n");
/* Grep: nvme: soft retpalaceangle — Wave 78 return-palaceangle honesty */
kprintf("nvme: soft retpalaceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retpalaceangle honesty; Soft≠product; not bar3)\n");
/* Grep: nvme: soft retfortressangle — Wave 78 exclusive fortressangle stamp */
kprintf("nvme: soft retfortressangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retfortressangle stamp; Soft≠product)\n");
/* Grep: nvme: soft retkeepangle — Wave 79 return-keepangle honesty */
kprintf("nvme: soft retkeepangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retkeepangle honesty; Soft≠product; not bar3)\n");
/* Grep: nvme: soft retredoubtangle — Wave 79 exclusive redoubtangle stamp */
kprintf("nvme: soft retredoubtangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retredoubtangle stamp; Soft≠product)\n");
/* Grep: nvme: soft retcitadelangle — Wave 80 return-citadelangle honesty */
kprintf("nvme: soft retcitadelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retcitadelangle honesty; Soft≠product; not bar3)\n");
/* Grep: nvme: soft retbastionkeep — Wave 80 exclusive bastionkeep stamp */
kprintf("nvme: soft retbastionkeep exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retbastionkeep stamp; Soft≠product)\n");
/* Grep: nvme: soft retpanoplyangle — Wave 81 return-panoplyangle honesty */
kprintf("nvme: soft retpanoplyangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retpanoplyangle honesty; Soft≠product; not bar3)\n");
/* Grep: nvme: soft retbulwarkangle — Wave 81 exclusive bulwarkangle stamp */
kprintf("nvme: soft retbulwarkangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retbulwarkangle stamp; Soft≠product)\n");
/* Grep: nvme: soft retmantleangle — Wave 82 return-mantleangle honesty */
kprintf("nvme: soft retmantleangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retmantleangle honesty; Soft≠product; not bar3)\n");
/* Grep: nvme: soft retaegisangle — Wave 82 exclusive aegisangle stamp */
kprintf("nvme: soft retaegisangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retaegisangle stamp; Soft≠product)\n");
/* Grep: nvme: soft retemblemangle — Wave 83 return-emblemangle honesty */
kprintf("nvme: soft retemblemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retemblemangle honesty; Soft≠product; not bar3)\n");
/* Grep: nvme: soft retsigilangle — Wave 83 exclusive sigilangle stamp */
kprintf("nvme: soft retsigilangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retsigilangle stamp; Soft≠product)\n");
