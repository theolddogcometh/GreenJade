 ...
 * greppable "virtio-input: soft …"; never hard-gates):
 ...
 * Soft only — / not product multi-device input close.
 *
 * greppable: virtio-input: soft
 */
static void
soft_inventory(const char *szVia)
{
     ...
     * Soft inventory / multi-device product close.
     */
    ...
    kprintf("virtio-input: soft honesty multi_slot=1 soft_ring=1 "
            "soft_axes=1 status_q=0 multi_dev=0 steam=0 "
     ...
     */
    {
        ...
        kprintf("virtio-input: soft contract soft_only=1 game_io=0 "
                "product_input=0 steam=0 multi_dev=0 wave=%u "
                "soft PASS\n",
                (unsigned)VI_SOFT_WAVE);
    }
...
kprintf("virtio-input: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retfosseangle honesty; Soft≠product)\n");
/* Grep: virtio-input: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("virtio-input: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retcounterscarple stamp; Soft≠product)\n");
/*
 * ---- Wave 71 exclusive complementary surfaces (never reshape primary).
 * Return surfaces only — soft inventory; never hard-gates product paths.
 */
/* Grep: virtio-input: soft retsallyportangle — Wave 71 return-sallyportangle honesty */
kprintf("virtio-input: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retsallyportangle honesty; Soft≠product)\n");
/* Grep: virtio-input: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("virtio-input: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retreentrantangle stamp; Soft≠product)\n");
/*
 * ---- Wave 72 exclusive complementary surfaces (never reshape primary).
 * Return surfaces only — soft inventory; never hard-gates product paths.
 */
/* Grep: virtio-input: soft retplaceofarmsangle — Wave 72 return-placeofarmsangle honesty */
kprintf("virtio-input: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retplaceofarmsangle honesty; Soft≠product)\n");
/* Grep: virtio-input: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("virtio-input: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("virtio-input: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retcurtainface honesty; Soft≠product)\n");
/* Grep: virtio-input: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("virtio-input: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retbastionangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retglacisangle — Wave 74 return-glacisangle honesty */
kprintf("virtio-input: soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retglacisangle honesty; Soft≠product)\n");
/* Grep: virtio-input: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
kprintf("virtio-input: soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retparapetangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retmoatangle — Wave 75 return-moatangle honesty */
kprintf("virtio-input: soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retmoatangle honesty; Soft≠product)\n");
/* Grep: virtio-input: soft retowerangle — Wave 75 exclusive towerangle stamp */
kprintf("virtio-input: soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retowerangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retgateangle — Wave 76 return-gateangle honesty */
kprintf("virtio-input: soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retgateangle honesty; Soft≠product)\n");
/* Grep: virtio-input: soft retwallangle — Wave 76 exclusive wallangle stamp */
kprintf("virtio-input: soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retwallangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retspireangle — Wave 77 return-spireangle honesty */
kprintf("virtio-input: soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retspireangle honesty; Soft≠product)\n");
/* Grep: virtio-input: soft retholdangle — Wave 77 exclusive holdangle stamp */
kprintf("virtio-input: soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retholdangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retpalaceangle — Wave 78 return-palaceangle honesty */
kprintf("virtio-input: soft retpalaceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retpalaceangle honesty; Soft≠product)\n");
/* Grep: virtio-input: soft retfortressangle — Wave 78 exclusive fortressangle stamp */
kprintf("virtio-input: soft retfortressangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retfortressangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retkeepangle — Wave 79 return-keepangle honesty */
kprintf("virtio-input: soft retkeepangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retkeepangle honesty; Soft≠product)\n");
/* Grep: virtio-input: soft retredoubtangle — Wave 79 exclusive redoubtangle stamp */
kprintf("virtio-input: soft retredoubtangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retredoubtangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retcitadelangle — Wave 80 return-citadelangle honesty */
kprintf("virtio-input: soft retcitadelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retcitadelangle honesty; Soft≠product)\n");
/* Grep: virtio-input: soft retbastionkeep — Wave 80 exclusive bastionkeep stamp */
kprintf("virtio-input: soft retbastionkeep exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retbastionkeep stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retpanoplyangle — Wave 81 return-panoplyangle honesty */
kprintf("virtio-input: soft retpanoplyangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retpanoplyangle honesty; Soft≠product)\n");
/* Grep: virtio-input: soft retbulwarkangle — Wave 81 exclusive bulwarkangle stamp */
kprintf("virtio-input: soft retbulwarkangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retbulwarkangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retmantleangle — Wave 82 return-mantleangle honesty */
kprintf("virtio-input: soft retmantleangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retmantleangle honesty; Soft≠product)\n");
/* Grep: virtio-input: soft retaegisangle — Wave 82 exclusive aegisangle stamp */
kprintf("virtio-input: soft retaegisangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retaegisangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retemblemangle — Wave 83 return-emblemangle honesty */
kprintf("virtio-input: soft retemblemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retemblemangle honesty; Soft≠product)\n");
/* Grep: virtio-input: soft retsigilangle — Wave 83 exclusive sigilangle stamp */
kprintf("virtio-input: soft retsigilangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retsigilangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retscepterangle — Wave 84 return-scepterangle honesty */
kprintf("virtio-input: soft retscepterangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retscepterangle honesty; Soft≠product)\n");
/* Grep: virtio-input: soft retglyphangle — Wave 84 exclusive glyphangle stamp */
kprintf("virtio-input: soft retglyphangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retglyphangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retcrownangle — Wave 85 return-crownangle honesty */
kprintf("virtio-input: soft retcrownangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retcrownangle honesty; Soft≠product)\n");
/* Grep: virtio-input: soft retshardangle — Wave 85 exclusive shardangle stamp */
kprintf("virtio-input: soft retshardangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retshardangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retforgeangle — Wave 86 return-forgeangle honesty */
kprintf("virtio-input: soft retforgeangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retforgeangle honesty; Soft≠product)\n");
/* Grep: virtio-input: soft retprismangle — Wave 86 exclusive prismangle stamp */
kprintf("virtio-input: soft retprismangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retprismangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retflameangle — Wave 87 return-flameangle honesty */
kprintf("virtio-input: soft retflameangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retflameangle honesty; Soft≠product)\n");
/* Grep: virtio-input: soft retcipherangle — Wave 87 exclusive cipherangle stamp */
kprintf("virtio-input: soft retcipherangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retcipherangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retbeaconangle — Wave 88 return-beaconangle honesty */
kprintf("virtio-input: soft retbeaconangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retbeaconangle honesty; Soft≠product)\n");
/* Grep: virtio-input: soft retledgerangle — Wave 88 exclusive ledgerangle stamp */
kprintf("virtio-input: soft retledgerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retledgerangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retbannerangle — Wave 89 return-bannerangle honesty */
kprintf("virtio-input: soft retbannerangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retbannerangle honesty; Soft≠product)\n");
/* Grep: virtio-input: soft retvaultangle — Wave 89 exclusive vaultangle stamp */
kprintf("virtio-input: soft retvaultangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retvaultangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retcrestangle — Wave 90 return-crestangle honesty */
kprintf("virtio-input: soft retcrestangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retcrestangle honesty; Soft≠product)\n");
/* Grep: virtio-input: soft rettokenangle — Wave 90 exclusive tokenangle stamp */
kprintf("virtio-input: soft rettokenangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (rettokenangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retbadgeangle — Wave 91 return-badgeangle honesty */
kprintf("virtio-input: soft retbadgeangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retbadgeangle honesty; Soft≠product)\n");
/* Grep: virtio-input: soft retphaseangle — Wave 91 exclusive phaseangle stamp */
kprintf("virtio-input: soft retphaseangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retphaseangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retmarkangle — Wave 92 return-markangle honesty */
kprintf("virtio-input: soft retmarkangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retmarkangle honesty; Soft≠product)\n");
/* Grep: virtio-input: soft retpulseangle — Wave 92 exclusive pulseangle stamp */
kprintf("virtio-input: soft retpulseangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retpulseangle stamp; Soft≠product)\n");

/* Grep: virtio-input: soft retsealangle — Wave 93 return-sealangle honesty */
kprintf("virtio-input: soft retsealangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retsealangle honesty; Soft≠product)\n");
/* Grep: virtio-input: soft retboundangle — Wave 93 exclusive boundangle stamp */
kprintf("virtio-input: soft retboundangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retboundangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retstemangle — Wave 94 return-stemangle honesty */
kprintf("virtio-input: soft retstemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retstemangle honesty; Soft≠product)\n");
/* Grep: virtio-input: soft retbladeangle — Wave 94 exclusive bladeangle stamp */
kprintf("virtio-input: soft retbladeangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retbladeangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retchordangle — Wave 95 return-chordangle honesty */
kprintf("virtio-input: soft retchordangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retchordangle honesty; Soft≠product)\n");
/* Grep: virtio-input: soft retarcangle — Wave 95 exclusive arcangle stamp */
kprintf("virtio-input: soft retarcangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retarcangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retsectorangle — Wave 96 return-sectorangle honesty */
kprintf("virtio-input: soft retsectorangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retsectorangle honesty; Soft≠product)\n");
/* Grep: virtio-input: soft retwedgeangle — Wave 96 exclusive wedgeangle stamp */
kprintf("virtio-input: soft retwedgeangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retwedgeangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retradiusangle — Wave 97 return-radiusangle honesty */
kprintf("virtio-input: soft retradiusangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retradiusangle honesty; Soft≠product)\n");
/* Grep: virtio-input: soft retdiameterangle — Wave 97 exclusive diameterangle stamp */
kprintf("virtio-input: soft retdiameterangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retdiameterangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retcircumangle — Wave 98 return-circumangle honesty */
kprintf("virtio-input: soft retcircumangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retcircumangle honesty; Soft≠product)\n");
/* Grep: virtio-input: soft retellipseangle — Wave 98 exclusive ellipseangle stamp */
kprintf("virtio-input: soft retellipseangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retellipseangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft rethyperangle — Wave 99 return-hyperangle honesty */
kprintf("virtio-input: soft rethyperangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (rethyperangle honesty; Soft≠product)\n");
/* Grep: virtio-input: soft retparabolaangle — Wave 99 exclusive parabolaangle stamp */
kprintf("virtio-input: soft retparabolaangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retparabolaangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retspiralangle — Wave 100 return-spiralangle honesty */
kprintf("virtio-input: soft retspiralangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retspiralangle honesty; Soft≠product)\n");
/* Grep: virtio-input: soft rethelixangle — Wave 100 exclusive helixangle stamp */
kprintf("virtio-input: soft rethelixangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (rethelixangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft rettorusangle — Wave 101 return-torusangle honesty */
kprintf("virtio-input: soft rettorusangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (rettorusangle honesty; Soft≠product)\n");
/* Grep: virtio-input: soft retknotangle — Wave 101 exclusive knotangle stamp */
kprintf("virtio-input: soft retknotangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retknotangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retmoebiusangle — Wave 102 return-moebiusangle honesty */
kprintf("virtio-input: soft retmoebiusangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retmoebiusangle honesty; Soft≠product)\n");
/* Grep: virtio-input: soft retkleinangle — Wave 102 exclusive kleinangle stamp */
kprintf("virtio-input: soft retkleinangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retkleinangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retprojectangle — Wave 103 return-projectangle honesty */
kprintf("virtio-input: soft retprojectangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retprojectangle honesty; Soft≠product)\n");
/* Grep: virtio-input: soft retaffineangle — Wave 103 exclusive affineangle stamp */
kprintf("virtio-input: soft retaffineangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retaffineangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retlinearangle — Wave 104 return-linearangle honesty */
kprintf("virtio-input: soft retlinearangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retlinearangle honesty; Soft≠product)\n");
/* Grep: virtio-input: soft retbilinearangle — Wave 104 exclusive bilinearangle stamp */
kprintf("virtio-input: soft retbilinearangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retbilinearangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retquadraticangle — Wave 105 return-quadraticangle honesty */
kprintf("virtio-input: soft retquadraticangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retquadraticangle honesty; Soft≠product)\n");
/* Grep: virtio-input: soft retcubicangle — Wave 105 exclusive cubicangle stamp */
kprintf("virtio-input: soft retcubicangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retcubicangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retquarticangle — Wave 106 return-quarticangle honesty */
kprintf("virtio-input: soft retquarticangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retquarticangle honesty; Soft≠product)\n");
/* Grep: virtio-input: soft retquinticangle — Wave 106 exclusive quinticangle stamp */
kprintf("virtio-input: soft retquinticangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retquinticangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retsplineangle — Wave 107 return-splineangle honesty */
kprintf("virtio-input: soft retsplineangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retsplineangle honesty; Soft≠product)\n");
/* Grep: virtio-input: soft retbezierangle — Wave 107 exclusive bezierangle stamp */
kprintf("virtio-input: soft retbezierangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retbezierangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft rethurmitangle — Wave 108 return-hermitangle honesty */
kprintf("virtio-input: soft rethurmitangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (rethurmitangle honesty; Soft≠product)\n");
/* Grep: virtio-input: soft retcatmullangle — Wave 108 exclusive catmullangle stamp */
kprintf("virtio-input: soft retcatmullangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retcatmullangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retnurbsangle — Wave 109 return-nurbsangle honesty */
kprintf("virtio-input: soft retnurbsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retnurbsangle honesty; Soft≠product)\n");
/* Grep: virtio-input: soft retbsplineangle — Wave 109 exclusive bsplineangle stamp */
kprintf("virtio-input: soft retbsplineangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retbsplineangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retmeshangle — Wave 110 return-meshangle honesty */
kprintf("virtio-input: soft retmeshangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retmeshangle honesty; Soft≠product)\n");
/* Grep: virtio-input: soft retgridangle — Wave 110 exclusive gridangle stamp */
kprintf("virtio-input: soft retgridangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retgridangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retvoxelangle — Wave 111 return-voxelangle honesty */
kprintf("virtio-input: soft retvoxelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retvoxelangle honesty; Soft≠product)\n");
/* Grep: virtio-input: soft rettexelangle — Wave 111 exclusive texelangle stamp */
kprintf("virtio-input: soft rettexelangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (rettexelangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retfragmentangle — Wave 112 return-fragmentangle honesty */
kprintf("virtio-input: soft retfragmentangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retfragmentangle honesty; Soft≠product)\n");
/* Grep: virtio-input: soft retvertexangle — Wave 112 exclusive vertexangle stamp */
kprintf("virtio-input: soft retvertexangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retvertexangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retshaderangle — Wave 113 return-shaderangle honesty */
kprintf("virtio-input: soft retshaderangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retshaderangle honesty; Soft≠product)\n");
/* Grep: virtio-input: soft retpipelineangle — Wave 113 exclusive pipelineangle stamp */
kprintf("virtio-input: soft retpipelineangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retpipelineangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retframebufferangle — Wave 114 return-framebufferangle honesty */
kprintf("virtio-input: soft retframebufferangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retframebufferangle honesty; Soft≠product)\n");
/* Grep: virtio-input: soft retswapchainangle — Wave 114 exclusive swapchainangle stamp */
kprintf("virtio-input: soft retswapchainangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retswapchainangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retpresentangle — Wave 115 return-presentangle honesty */
kprintf("virtio-input: soft retpresentangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retpresentangle honesty; Soft≠product)\n");
/* Grep: virtio-input: soft retvsyncangle — Wave 115 exclusive vsyncangle stamp */
kprintf("virtio-input: soft retvsyncangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retvsyncangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retfenceangle — Wave 116 return-fenceangle honesty */
kprintf("virtio-input: soft retfenceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retfenceangle honesty; Soft≠product)\n");
/* Grep: virtio-input: soft retsemaphoreangle — Wave 116 exclusive semaphoreangle stamp */
kprintf("virtio-input: soft retsemaphoreangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retsemaphoreangle stamp; Soft≠product)\n");
