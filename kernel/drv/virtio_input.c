 ...
 *   virtio-input: soft deepen wave=107 …
...
#define VI_SOFT_WAVE 107u
#define VI_SOFT_AREAS 207u
...
kprintf("virtio-input: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio-input: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("virtio-input: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("virtio-input: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio-input: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("virtio-input: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("virtio-input: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio-input: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("virtio-input: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("virtio-input: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retcurtainface honesty; Soft≠product; not bar3)\n");
/* Grep: virtio-input: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("virtio-input: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retbastionangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retglacisangle — Wave 74 return-glacisangle honesty */
kprintf("virtio-input: soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retglacisangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio-input: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
kprintf("virtio-input: soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retparapetangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retmoatangle — Wave 75 return-moatangle honesty */
kprintf("virtio-input: soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retmoatangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio-input: soft retowerangle — Wave 75 exclusive towerangle stamp */
kprintf("virtio-input: soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retowerangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retgateangle — Wave 76 return-gateangle honesty */
kprintf("virtio-input: soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retgateangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio-input: soft retwallangle — Wave 76 exclusive wallangle stamp */
kprintf("virtio-input: soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retwallangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retspireangle — Wave 77 return-spireangle honesty */
kprintf("virtio-input: soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retspireangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio-input: soft retholdangle — Wave 77 exclusive holdangle stamp */
kprintf("virtio-input: soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retholdangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retpalaceangle — Wave 78 return-palaceangle honesty */
kprintf("virtio-input: soft retpalaceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retpalaceangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio-input: soft retfortressangle — Wave 78 exclusive fortressangle stamp */
kprintf("virtio-input: soft retfortressangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retfortressangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retkeepangle — Wave 79 return-keepangle honesty */
kprintf("virtio-input: soft retkeepangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retkeepangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio-input: soft retredoubtangle — Wave 79 exclusive redoubtangle stamp */
kprintf("virtio-input: soft retredoubtangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retredoubtangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retcitadelangle — Wave 80 return-citadelangle honesty */
kprintf("virtio-input: soft retcitadelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retcitadelangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio-input: soft retbastionkeep — Wave 80 exclusive bastionkeep stamp */
kprintf("virtio-input: soft retbastionkeep exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retbastionkeep stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retpanoplyangle — Wave 81 return-panoplyangle honesty */
kprintf("virtio-input: soft retpanoplyangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retpanoplyangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio-input: soft retbulwarkangle — Wave 81 exclusive bulwarkangle stamp */
kprintf("virtio-input: soft retbulwarkangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retbulwarkangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retmantleangle — Wave 82 return-mantleangle honesty */
kprintf("virtio-input: soft retmantleangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retmantleangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio-input: soft retaegisangle — Wave 82 exclusive aegisangle stamp */
kprintf("virtio-input: soft retaegisangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retaegisangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retemblemangle — Wave 83 return-emblemangle honesty */
kprintf("virtio-input: soft retemblemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retemblemangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio-input: soft retsigilangle — Wave 83 exclusive sigilangle stamp */
kprintf("virtio-input: soft retsigilangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retsigilangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retscepterangle — Wave 84 return-scepterangle honesty */
kprintf("virtio-input: soft retscepterangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retscepterangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio-input: soft retglyphangle — Wave 84 exclusive glyphangle stamp */
kprintf("virtio-input: soft retglyphangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retglyphangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retcrownangle — Wave 85 return-crownangle honesty */
kprintf("virtio-input: soft retcrownangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retcrownangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio-input: soft retshardangle — Wave 85 exclusive shardangle stamp */
kprintf("virtio-input: soft retshardangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retshardangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retforgeangle — Wave 86 return-forgeangle honesty */
kprintf("virtio-input: soft retforgeangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retforgeangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio-input: soft retprismangle — Wave 86 exclusive prismangle stamp */
kprintf("virtio-input: soft retprismangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retprismangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retflameangle — Wave 87 return-flameangle honesty */
kprintf("virtio-input: soft retflameangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retflameangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio-input: soft retcipherangle — Wave 87 exclusive cipherangle stamp */
kprintf("virtio-input: soft retcipherangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retcipherangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retbeaconangle — Wave 88 return-beaconangle honesty */
kprintf("virtio-input: soft retbeaconangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retbeaconangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio-input: soft retledgerangle — Wave 88 exclusive ledgerangle stamp */
kprintf("virtio-input: soft retledgerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retledgerangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retbannerangle — Wave 89 return-bannerangle honesty */
kprintf("virtio-input: soft retbannerangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retbannerangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio-input: soft retvaultangle — Wave 89 exclusive vaultangle stamp */
kprintf("virtio-input: soft retvaultangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retvaultangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retcrestangle — Wave 90 return-crestangle honesty */
kprintf("virtio-input: soft retcrestangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retcrestangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio-input: soft rettokenangle — Wave 90 exclusive tokenangle stamp */
kprintf("virtio-input: soft rettokenangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (rettokenangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retbadgeangle — Wave 91 return-badgeangle honesty */
kprintf("virtio-input: soft retbadgeangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retbadgeangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio-input: soft retphaseangle — Wave 91 exclusive phaseangle stamp */
kprintf("virtio-input: soft retphaseangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retphaseangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retmarkangle — Wave 92 return-markangle honesty */
kprintf("virtio-input: soft retmarkangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retmarkangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio-input: soft retpulseangle — Wave 92 exclusive pulseangle stamp */
kprintf("virtio-input: soft retpulseangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retpulseangle stamp; Soft≠product)\n");

/* Grep: virtio-input: soft retsealangle — Wave 93 return-sealangle honesty */
kprintf("virtio-input: soft retsealangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retsealangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio-input: soft retboundangle — Wave 93 exclusive boundangle stamp */
kprintf("virtio-input: soft retboundangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retboundangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retstemangle — Wave 94 return-stemangle honesty */
kprintf("virtio-input: soft retstemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retstemangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio-input: soft retbladeangle — Wave 94 exclusive bladeangle stamp */
kprintf("virtio-input: soft retbladeangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retbladeangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retchordangle — Wave 95 return-chordangle honesty */
kprintf("virtio-input: soft retchordangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retchordangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio-input: soft retarcangle — Wave 95 exclusive arcangle stamp */
kprintf("virtio-input: soft retarcangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retarcangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retsectorangle — Wave 96 return-sectorangle honesty */
kprintf("virtio-input: soft retsectorangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retsectorangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio-input: soft retwedgeangle — Wave 96 exclusive wedgeangle stamp */
kprintf("virtio-input: soft retwedgeangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retwedgeangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retradiusangle — Wave 97 return-radiusangle honesty */
kprintf("virtio-input: soft retradiusangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retradiusangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio-input: soft retdiameterangle — Wave 97 exclusive diameterangle stamp */
kprintf("virtio-input: soft retdiameterangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retdiameterangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retcircumangle — Wave 98 return-circumangle honesty */
kprintf("virtio-input: soft retcircumangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retcircumangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio-input: soft retellipseangle — Wave 98 exclusive ellipseangle stamp */
kprintf("virtio-input: soft retellipseangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retellipseangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft rethyperangle — Wave 99 return-hyperangle honesty */
kprintf("virtio-input: soft rethyperangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (rethyperangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio-input: soft retparabolaangle — Wave 99 exclusive parabolaangle stamp */
kprintf("virtio-input: soft retparabolaangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retparabolaangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retspiralangle — Wave 100 return-spiralangle honesty */
kprintf("virtio-input: soft retspiralangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retspiralangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio-input: soft rethelixangle — Wave 100 exclusive helixangle stamp */
kprintf("virtio-input: soft rethelixangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (rethelixangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft rettorusangle — Wave 101 return-torusangle honesty */
kprintf("virtio-input: soft rettorusangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (rettorusangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio-input: soft retknotangle — Wave 101 exclusive knotangle stamp */
kprintf("virtio-input: soft retknotangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retknotangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retmoebiusangle — Wave 102 return-moebiusangle honesty */
kprintf("virtio-input: soft retmoebiusangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retmoebiusangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio-input: soft retkleinangle — Wave 102 exclusive kleinangle stamp */
kprintf("virtio-input: soft retkleinangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retkleinangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retprojectangle — Wave 103 return-projectangle honesty */
kprintf("virtio-input: soft retprojectangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retprojectangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio-input: soft retaffineangle — Wave 103 exclusive affineangle stamp */
kprintf("virtio-input: soft retaffineangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retaffineangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retlinearangle — Wave 104 return-linearangle honesty */
kprintf("virtio-input: soft retlinearangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retlinearangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio-input: soft retbilinearangle — Wave 104 exclusive bilinearangle stamp */
kprintf("virtio-input: soft retbilinearangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retbilinearangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retquadraticangle — Wave 105 return-quadraticangle honesty */
kprintf("virtio-input: soft retquadraticangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retquadraticangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio-input: soft retcubicangle — Wave 105 exclusive cubicangle stamp */
kprintf("virtio-input: soft retcubicangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retcubicangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retquarticangle — Wave 106 return-quarticangle honesty */
kprintf("virtio-input: soft retquarticangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retquarticangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio-input: soft retquinticangle — Wave 106 exclusive quinticangle stamp */
kprintf("virtio-input: soft retquinticangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retquinticangle stamp; Soft≠product)\n");
/* Grep: virtio-input: soft retsplineangle — Wave 107 return-splineangle honesty */
kprintf("virtio-input: soft retsplineangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retsplineangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio-input: soft retbezierangle — Wave 107 exclusive bezierangle stamp */
kprintf("virtio-input: soft retbezierangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retbezierangle stamp; Soft≠product)\n");
