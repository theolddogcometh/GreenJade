...
virtio_net_soft_inventory(const char *szVia)
{
     ...
     */
    {
        ...
        kprintf("virtio-net: soft contract soft_only=1 game_io=0 "
                "product_net=0 gso=0 csum=0 mq=0 wave=%u "
                "soft PASS\n",
                (unsigned)VIRTIO_NET_SOFT_DEEPEN_WAVE);
    }
...
kprintf("virtio-net: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retfosseangle honesty; Soft≠product)\n");
/* Grep: virtio-net: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("virtio-net: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retcounterscarple stamp; Soft≠product)\n");
/*
 * ---- Wave 71 exclusive complementary surfaces (never reshape primary).
 * Return surfaces only — soft inventory; never hard-gates product paths.
 */
/* Grep: virtio-net: soft retsallyportangle — Wave 71 return-sallyportangle honesty */
kprintf("virtio-net: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retsallyportangle honesty; Soft≠product)\n");
/* Grep: virtio-net: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("virtio-net: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retreentrantangle stamp; Soft≠product)\n");
/*
 * ---- Wave 72 exclusive complementary surfaces (never reshape primary).
 * Return surfaces only — soft inventory; never hard-gates product paths.
 */
/* Grep: virtio-net: soft retplaceofarmsangle — Wave 72 return-placeofarmsangle honesty */
kprintf("virtio-net: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retplaceofarmsangle honesty; Soft≠product)\n");
/* Grep: virtio-net: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("virtio-net: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: virtio-net: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("virtio-net: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retcurtainface honesty; Soft≠product)\n");
/* Grep: virtio-net: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("virtio-net: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retbastionangle stamp; Soft≠product)\n");
/* Grep: virtio-net: soft retglacisangle — Wave 74 return-glacisangle honesty */
kprintf("virtio-net: soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retglacisangle honesty; Soft≠product)\n");
/* Grep: virtio-net: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
kprintf("virtio-net: soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retparapetangle stamp; Soft≠product)\n");
/* Grep: virtio-net: soft retmoatangle — Wave 75 return-moatangle honesty */
kprintf("virtio-net: soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retmoatangle honesty; Soft≠product)\n");
/* Grep: virtio-net: soft retowerangle — Wave 75 exclusive towerangle stamp */
kprintf("virtio-net: soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retowerangle stamp; Soft≠product)\n");
/* Grep: virtio-net: soft retgateangle — Wave 76 return-gateangle honesty */
kprintf("virtio-net: soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retgateangle honesty; Soft≠product)\n");
/* Grep: virtio-net: soft retwallangle — Wave 76 exclusive wallangle stamp */
kprintf("virtio-net: soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retwallangle stamp; Soft≠product)\n");
/* Grep: virtio-net: soft retspireangle — Wave 77 return-spireangle honesty */
kprintf("virtio-net: soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retspireangle honesty; Soft≠product)\n");
/* Grep: virtio-net: soft retholdangle — Wave 77 exclusive holdangle stamp */
kprintf("virtio-net: soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retholdangle stamp; Soft≠product)\n");
/* Grep: virtio-net: soft retpalaceangle — Wave 78 return-palaceangle honesty */
kprintf("virtio-net: soft retpalaceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retpalaceangle honesty; Soft≠product)\n");
/* Grep: virtio-net: soft retfortressangle — Wave 78 exclusive fortressangle stamp */
kprintf("virtio-net: soft retfortressangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retfortressangle stamp; Soft≠product)\n");
/* Grep: virtio-net: soft retkeepangle — Wave 79 return-keepangle honesty */
kprintf("virtio-net: soft retkeepangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retkeepangle honesty; Soft≠product)\n");
/* Grep: virtio-net: soft retredoubtangle — Wave 79 exclusive redoubtangle stamp */
kprintf("virtio-net: soft retredoubtangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retredoubtangle stamp; Soft≠product)\n");
/* Grep: virtio-net: soft retcitadelangle — Wave 80 return-citadelangle honesty */
kprintf("virtio-net: soft retcitadelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retcitadelangle honesty; Soft≠product)\n");
/* Grep: virtio-net: soft retbastionkeep — Wave 80 exclusive bastionkeep stamp */
kprintf("virtio-net: soft retbastionkeep exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retbastionkeep stamp; Soft≠product)\n");
/* Grep: virtio-net: soft retpanoplyangle — Wave 81 return-panoplyangle honesty */
kprintf("virtio-net: soft retpanoplyangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retpanoplyangle honesty; Soft≠product)\n");
/* Grep: virtio-net: soft retbulwarkangle — Wave 81 exclusive bulwarkangle stamp */
kprintf("virtio-net: soft retbulwarkangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retbulwarkangle stamp; Soft≠product)\n");
/* Grep: virtio-net: soft retmantleangle — Wave 82 return-mantleangle honesty */
kprintf("virtio-net: soft retmantleangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retmantleangle honesty; Soft≠product)\n");
/* Grep: virtio-net: soft retaegisangle — Wave 82 exclusive aegisangle stamp */
kprintf("virtio-net: soft retaegisangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retaegisangle stamp; Soft≠product)\n");
/* Grep: virtio-net: soft retemblemangle — Wave 83 return-emblemangle honesty */
kprintf("virtio-net: soft retemblemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retemblemangle honesty; Soft≠product)\n");
/* Grep: virtio-net: soft retsigilangle — Wave 83 exclusive sigilangle stamp */
kprintf("virtio-net: soft retsigilangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retsigilangle stamp; Soft≠product)\n");
/* Grep: virtio-net: soft retscepterangle — Wave 84 return-scepterangle honesty */
kprintf("virtio-net: soft retscepterangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retscepterangle honesty; Soft≠product)\n");
/* Grep: virtio-net: soft retglyphangle — Wave 84 exclusive glyphangle stamp */
kprintf("virtio-net: soft retglyphangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retglyphangle stamp; Soft≠product)\n");
/* Grep: virtio-net: soft retcrownangle — Wave 85 return-crownangle honesty */
kprintf("virtio-net: soft retcrownangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retcrownangle honesty; Soft≠product)\n");
/* Grep: virtio-net: soft retshardangle — Wave 85 exclusive shardangle stamp */
kprintf("virtio-net: soft retshardangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retshardangle stamp; Soft≠product)\n");
/* Grep: virtio-net: soft retforgeangle — Wave 86 return-forgeangle honesty */
kprintf("virtio-net: soft retforgeangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retforgeangle honesty; Soft≠product)\n");
/* Grep: virtio-net: soft retprismangle — Wave 86 exclusive prismangle stamp */
kprintf("virtio-net: soft retprismangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retprismangle stamp; Soft≠product)\n");
/* Grep: virtio-net: soft retflameangle — Wave 87 return-flameangle honesty */
kprintf("virtio-net: soft retflameangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retflameangle honesty; Soft≠product)\n");
/* Grep: virtio-net: soft retcipherangle — Wave 87 exclusive cipherangle stamp */
kprintf("virtio-net: soft retcipherangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retcipherangle stamp; Soft≠product)\n");
/* Grep: virtio-net: soft retbeaconangle — Wave 88 return-beaconangle honesty */
kprintf("virtio-net: soft retbeaconangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retbeaconangle honesty; Soft≠product)\n");
/* Grep: virtio-net: soft retledgerangle — Wave 88 exclusive ledgerangle stamp */
kprintf("virtio-net: soft retledgerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retledgerangle stamp; Soft≠product)\n");
/* Grep: virtio-net: soft retbannerangle — Wave 89 return-bannerangle honesty */
kprintf("virtio-net: soft retbannerangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retbannerangle honesty; Soft≠product)\n");
/* Grep: virtio-net: soft retvaultangle — Wave 89 exclusive vaultangle stamp */
kprintf("virtio-net: soft retvaultangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retvaultangle stamp; Soft≠product)\n");
/* Grep: virtio-net: soft retcrestangle — Wave 90 return-crestangle honesty */
kprintf("virtio-net: soft retcrestangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retcrestangle honesty; Soft≠product)\n");
/* Grep: virtio-net: soft rettokenangle — Wave 90 exclusive tokenangle stamp */
kprintf("virtio-net: soft rettokenangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (rettokenangle stamp; Soft≠product)\n");
/* Grep: virtio-net: soft retbadgeangle — Wave 91 return-badgeangle honesty */
kprintf("virtio-net: soft retbadgeangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retbadgeangle honesty; Soft≠product)\n");
/* Grep: virtio-net: soft retphaseangle — Wave 91 exclusive phaseangle stamp */
kprintf("virtio-net: soft retphaseangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retphaseangle stamp; Soft≠product)\n");
/* Grep: virtio-net: soft retmarkangle — Wave 92 return-markangle honesty */
kprintf("virtio-net: soft retmarkangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retmarkangle honesty; Soft≠product)\n");
/* Grep: virtio-net: soft retpulseangle — Wave 92 exclusive pulseangle stamp */
kprintf("virtio-net: soft retpulseangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retpulseangle stamp; Soft≠product)\n");

/* Grep: virtio-net: soft retsealangle — Wave 93 return-sealangle honesty */
kprintf("virtio-net: soft retsealangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retsealangle honesty; Soft≠product)\n");
/* Grep: virtio-net: soft retboundangle — Wave 93 exclusive boundangle stamp */
kprintf("virtio-net: soft retboundangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retboundangle stamp; Soft≠product)\n");
/* Grep: virtio-net: soft retstemangle — Wave 94 return-stemangle honesty */
kprintf("virtio-net: soft retstemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retstemangle honesty; Soft≠product)\n");
/* Grep: virtio-net: soft retbladeangle — Wave 94 exclusive bladeangle stamp */
kprintf("virtio-net: soft retbladeangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retbladeangle stamp; Soft≠product)\n");
/* Grep: virtio-net: soft retchordangle — Wave 95 return-chordangle honesty */
kprintf("virtio-net: soft retchordangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retchordangle honesty; Soft≠product)\n");
/* Grep: virtio-net: soft retarcangle — Wave 95 exclusive arcangle stamp */
kprintf("virtio-net: soft retarcangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retarcangle stamp; Soft≠product)\n");
/* Grep: virtio-net: soft retsectorangle — Wave 96 return-sectorangle honesty */
kprintf("virtio-net: soft retsectorangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retsectorangle honesty; Soft≠product)\n");
/* Grep: virtio-net: soft retwedgeangle — Wave 96 exclusive wedgeangle stamp */
kprintf("virtio-net: soft retwedgeangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retwedgeangle stamp; Soft≠product)\n");
/* Grep: virtio-net: soft retradiusangle — Wave 97 return-radiusangle honesty */
kprintf("virtio-net: soft retradiusangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retradiusangle honesty; Soft≠product)\n");
/* Grep: virtio-net: soft retdiameterangle — Wave 97 exclusive diameterangle stamp */
kprintf("virtio-net: soft retdiameterangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retdiameterangle stamp; Soft≠product)\n");
/* Grep: virtio-net: soft retcircumangle — Wave 98 return-circumangle honesty */
kprintf("virtio-net: soft retcircumangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retcircumangle honesty; Soft≠product)\n");
/* Grep: virtio-net: soft retellipseangle — Wave 98 exclusive ellipseangle stamp */
kprintf("virtio-net: soft retellipseangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retellipseangle stamp; Soft≠product)\n");
/* Grep: virtio-net: soft rethyperangle — Wave 99 return-hyperangle honesty */
kprintf("virtio-net: soft rethyperangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (rethyperangle honesty; Soft≠product)\n");
/* Grep: virtio-net: soft retparabolaangle — Wave 99 exclusive parabolaangle stamp */
kprintf("virtio-net: soft retparabolaangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retparabolaangle stamp; Soft≠product)\n");
/* Grep: virtio-net: soft retspiralangle — Wave 100 return-spiralangle honesty */
kprintf("virtio-net: soft retspiralangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retspiralangle honesty; Soft≠product)\n");
/* Grep: virtio-net: soft rethelixangle — Wave 100 exclusive helixangle stamp */
kprintf("virtio-net: soft rethelixangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (rethelixangle stamp; Soft≠product)\n");
/* Grep: virtio-net: soft rettorusangle — Wave 101 return-torusangle honesty */
kprintf("virtio-net: soft rettorusangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (rettorusangle honesty; Soft≠product)\n");
/* Grep: virtio-net: soft retknotangle — Wave 101 exclusive knotangle stamp */
kprintf("virtio-net: soft retknotangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retknotangle stamp; Soft≠product)\n");
/* Grep: virtio-net: soft retmoebiusangle — Wave 102 return-moebiusangle honesty */
kprintf("virtio-net: soft retmoebiusangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retmoebiusangle honesty; Soft≠product)\n");
/* Grep: virtio-net: soft retkleinangle — Wave 102 exclusive kleinangle stamp */
kprintf("virtio-net: soft retkleinangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retkleinangle stamp; Soft≠product)\n");
/* Grep: virtio-net: soft retprojectangle — Wave 103 return-projectangle honesty */
kprintf("virtio-net: soft retprojectangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retprojectangle honesty; Soft≠product)\n");
/* Grep: virtio-net: soft retaffineangle — Wave 103 exclusive affineangle stamp */
kprintf("virtio-net: soft retaffineangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retaffineangle stamp; Soft≠product)\n");
/* Grep: virtio-net: soft retlinearangle — Wave 104 return-linearangle honesty */
kprintf("virtio-net: soft retlinearangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retlinearangle honesty; Soft≠product)\n");
/* Grep: virtio-net: soft retbilinearangle — Wave 104 exclusive bilinearangle stamp */
kprintf("virtio-net: soft retbilinearangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retbilinearangle stamp; Soft≠product)\n");
/* Grep: virtio-net: soft retquadraticangle — Wave 105 return-quadraticangle honesty */
kprintf("virtio-net: soft retquadraticangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retquadraticangle honesty; Soft≠product)\n");
/* Grep: virtio-net: soft retcubicangle — Wave 105 exclusive cubicangle stamp */
kprintf("virtio-net: soft retcubicangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retcubicangle stamp; Soft≠product)\n");
/* Grep: virtio-net: soft retquarticangle — Wave 106 return-quarticangle honesty */
kprintf("virtio-net: soft retquarticangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retquarticangle honesty; Soft≠product)\n");
/* Grep: virtio-net: soft retquinticangle — Wave 106 exclusive quinticangle stamp */
kprintf("virtio-net: soft retquinticangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retquinticangle stamp; Soft≠product)\n");
/* Grep: virtio-net: soft retsplineangle — Wave 107 return-splineangle honesty */
kprintf("virtio-net: soft retsplineangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retsplineangle honesty; Soft≠product)\n");
/* Grep: virtio-net: soft retbezierangle — Wave 107 exclusive bezierangle stamp */
kprintf("virtio-net: soft retbezierangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retbezierangle stamp; Soft≠product)\n");
/* Grep: virtio-net: soft rethurmitangle — Wave 108 return-hermitangle honesty */
kprintf("virtio-net: soft rethurmitangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (rethurmitangle honesty; Soft≠product)\n");
/* Grep: virtio-net: soft retcatmullangle — Wave 108 exclusive catmullangle stamp */
kprintf("virtio-net: soft retcatmullangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retcatmullangle stamp; Soft≠product)\n");
/* Grep: virtio-net: soft retnurbsangle — Wave 109 return-nurbsangle honesty */
kprintf("virtio-net: soft retnurbsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retnurbsangle honesty; Soft≠product)\n");
/* Grep: virtio-net: soft retbsplineangle — Wave 109 exclusive bsplineangle stamp */
kprintf("virtio-net: soft retbsplineangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retbsplineangle stamp; Soft≠product)\n");
/* Grep: virtio-net: soft retmeshangle — Wave 110 return-meshangle honesty */
kprintf("virtio-net: soft retmeshangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retmeshangle honesty; Soft≠product)\n");
/* Grep: virtio-net: soft retgridangle — Wave 110 exclusive gridangle stamp */
kprintf("virtio-net: soft retgridangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retgridangle stamp; Soft≠product)\n");
/* Grep: virtio-net: soft retvoxelangle — Wave 111 return-voxelangle honesty */
kprintf("virtio-net: soft retvoxelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retvoxelangle honesty; Soft≠product)\n");
/* Grep: virtio-net: soft rettexelangle — Wave 111 exclusive texelangle stamp */
kprintf("virtio-net: soft rettexelangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (rettexelangle stamp; Soft≠product)\n");
/* Grep: virtio-net: soft retfragmentangle — Wave 112 return-fragmentangle honesty */
kprintf("virtio-net: soft retfragmentangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retfragmentangle honesty; Soft≠product)\n");
/* Grep: virtio-net: soft retvertexangle — Wave 112 exclusive vertexangle stamp */
kprintf("virtio-net: soft retvertexangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retvertexangle stamp; Soft≠product)\n");
/* Grep: virtio-net: soft retshaderangle — Wave 113 return-shaderangle honesty */
kprintf("virtio-net: soft retshaderangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retshaderangle honesty; Soft≠product)\n");
/* Grep: virtio-net: soft retpipelineangle — Wave 113 exclusive pipelineangle stamp */
kprintf("virtio-net: soft retpipelineangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retpipelineangle stamp; Soft≠product)\n");
/* Grep: virtio-net: soft retframebufferangle — Wave 114 return-framebufferangle honesty */
kprintf("virtio-net: soft retframebufferangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retframebufferangle honesty; Soft≠product)\n");
/* Grep: virtio-net: soft retswapchainangle — Wave 114 exclusive swapchainangle stamp */
kprintf("virtio-net: soft retswapchainangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retswapchainangle stamp; Soft≠product)\n");
/* Grep: virtio-net: soft retpresentangle — Wave 115 return-presentangle honesty */
kprintf("virtio-net: soft retpresentangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retpresentangle honesty; Soft≠product)\n");
/* Grep: virtio-net: soft retvsyncangle — Wave 115 exclusive vsyncangle stamp */
kprintf("virtio-net: soft retvsyncangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retvsyncangle stamp; Soft≠product)\n");
/* Grep: virtio-net: soft retfenceangle — Wave 116 return-fenceangle honesty */
kprintf("virtio-net: soft retfenceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=116 (retfenceangle honesty; Soft≠product)\n");
/* Grep: virtio-net: soft retsemaphoreangle — Wave 116 exclusive semaphoreangle stamp */
kprintf("virtio-net: soft retsemaphoreangle exclusive=1 soft_ne_product=1 product_kernel=OPEN wave=116 (retsemaphoreangle stamp; Soft≠product)\n");
