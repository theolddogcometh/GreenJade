...
#define VIRTIO_PCI_SOFT_DEEPEN_AREAS 122u
...
kprintf("virtio: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("virtio: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("virtio: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("virtio: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("virtio: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("virtio: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: virtio: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("virtio: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retcurtainface honesty; Soft≠product; not bar3)\n");
/* Grep: virtio: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("virtio: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retbastionangle stamp; Soft≠product)\n");
/* Grep: virtio: soft retglacisangle — Wave 74 return-glacisangle honesty */
kprintf("virtio: soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retglacisangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
kprintf("virtio: soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retparapetangle stamp; Soft≠product)\n");
/* Grep: virtio: soft retmoatangle — Wave 75 return-moatangle honesty */
kprintf("virtio: soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retmoatangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio: soft retowerangle — Wave 75 exclusive towerangle stamp */
kprintf("virtio: soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retowerangle stamp; Soft≠product)\n");
/* Grep: virtio: soft retgateangle — Wave 76 return-gateangle honesty */
kprintf("virtio: soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retgateangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio: soft retwallangle — Wave 76 exclusive wallangle stamp */
kprintf("virtio: soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retwallangle stamp; Soft≠product)\n");
/* Grep: virtio: soft retspireangle — Wave 77 return-spireangle honesty */
kprintf("virtio: soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retspireangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio: soft retholdangle — Wave 77 exclusive holdangle stamp */
kprintf("virtio: soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retholdangle stamp; Soft≠product)\n");
/* Grep: virtio: soft retpalaceangle — Wave 78 return-palaceangle honesty */
kprintf("virtio: soft retpalaceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retpalaceangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio: soft retfortressangle — Wave 78 exclusive fortressangle stamp */
kprintf("virtio: soft retfortressangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retfortressangle stamp; Soft≠product)\n");
/* Grep: virtio: soft retkeepangle — Wave 79 return-keepangle honesty */
kprintf("virtio: soft retkeepangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retkeepangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio: soft retredoubtangle — Wave 79 exclusive redoubtangle stamp */
kprintf("virtio: soft retredoubtangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retredoubtangle stamp; Soft≠product)\n");
/* Grep: virtio: soft retcitadelangle — Wave 80 return-citadelangle honesty */
kprintf("virtio: soft retcitadelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retcitadelangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio: soft retbastionkeep — Wave 80 exclusive bastionkeep stamp */
kprintf("virtio: soft retbastionkeep exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retbastionkeep stamp; Soft≠product)\n");
/* Grep: virtio: soft retpanoplyangle — Wave 81 return-panoplyangle honesty */
kprintf("virtio: soft retpanoplyangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retpanoplyangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio: soft retbulwarkangle — Wave 81 exclusive bulwarkangle stamp */
kprintf("virtio: soft retbulwarkangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retbulwarkangle stamp; Soft≠product)\n");
/* Grep: virtio: soft retmantleangle — Wave 82 return-mantleangle honesty */
kprintf("virtio: soft retmantleangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retmantleangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio: soft retaegisangle — Wave 82 exclusive aegisangle stamp */
kprintf("virtio: soft retaegisangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retaegisangle stamp; Soft≠product)\n");
/* Grep: virtio: soft retemblemangle — Wave 83 return-emblemangle honesty */
kprintf("virtio: soft retemblemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retemblemangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio: soft retsigilangle — Wave 83 exclusive sigilangle stamp */
kprintf("virtio: soft retsigilangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retsigilangle stamp; Soft≠product)\n");
/* Grep: virtio: soft retscepterangle — Wave 84 return-scepterangle honesty */
kprintf("virtio: soft retscepterangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retscepterangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio: soft retglyphangle — Wave 84 exclusive glyphangle stamp */
kprintf("virtio: soft retglyphangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retglyphangle stamp; Soft≠product)\n");
/* Grep: virtio: soft retcrownangle — Wave 85 return-crownangle honesty */
kprintf("virtio: soft retcrownangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retcrownangle honesty; Soft≠product; not bar3)\n");
/* Grep: virtio: soft retshardangle — Wave 85 exclusive shardangle stamp */
kprintf("virtio: soft retshardangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retshardangle stamp; Soft≠product)\n");
