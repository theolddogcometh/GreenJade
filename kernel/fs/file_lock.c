...
soft_inventory_log(void)
{
    ...
    kprintf("file_lock: soft inventory wave=114 slots=%u waiters_max=%u "
    ...
    kprintf("file_lock: soft total set_ok=%u unlk_ok=%u deny=%u get=%u "
            "block=%u wake=%u logs=%u wave=114\n",
    ...
    kprintf("file_lock: soft catalog slots=%u waiters_max=%u wake_budget=%u "
            "deadlk_depth=%u tag=%u eof_sent=-1 wave=114\n",
            GJ_FLOCK_MAX, GJ_FLOCK_MAX_WAITERS, GJ_FLOCK_SOFT_WAKE_MAX,
    ...
    kprintf("file_lock: soft capacity slots=%u waiters_max=%u "
            "wake_budget=%u deadlk_depth=%u multi_waiter=1 wave=114\n",
            GJ_FLOCK_MAX, GJ_FLOCK_MAX_WAITERS, GJ_FLOCK_SOFT_WAKE_MAX,
    ...
    kprintf("file_lock: soft headroom free=%u held=%u peak=%u "
            "waiters=%u wait_free=%u wait_peak=%u occ_pct=%u wave=114\n",
    ...
    kprintf("file_lock: soft surface held=%u waiters=%u deny=%u set_ok=%u "
            "block=%u wake=%u surf=0x%x wave=114\n",
    ...
    kprintf("file_lock: soft terminal set_ok=%u unlk_ok=%u deny=%u get=%u "
            "block=%u wake=%u soft PASS wave=114\n",
            u32SetOk, u32UnlkOk, u32Deny, u32Get, u32Block, u32Wake);
    ...
    kprintf("file_lock: soft return set_ok=%u unlk_ok=%u get=%u get_hit=%u "
            ...
            "release_fd=%u release_pid=%u product_flock=OPEN wave=114\n",
    ...
    kprintf("file_lock: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=114\n");
...
kprintf("file_lock: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=114 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: file_lock: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("file_lock: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=114 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("file_lock: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=114 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: file_lock: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("file_lock: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=114 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("file_lock: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=114 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: file_lock: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("file_lock: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=114 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: file_lock: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("file_lock: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=114 (retcurtainface honesty; Soft≠product; not bar3)\n");
/* Grep: file_lock: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("file_lock: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=114 (retbastionangle stamp; Soft≠product)\n");
/* Grep: file_lock: soft retglacisangle — Wave 74 return-glacisangle honesty */
kprintf("file_lock: soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=114 (retglacisangle honesty; Soft≠product; not bar3)\n");
/* Grep: file_lock: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
kprintf("file_lock: soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=114 (retparapetangle stamp; Soft≠product)\n");
/* Grep: file_lock: soft retmoatangle — Wave 75 return-moatangle honesty */
kprintf("file_lock: soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=114 (retmoatangle honesty; Soft≠product; not bar3)\n");
/* Grep: file_lock: soft retowerangle — Wave 75 exclusive towerangle stamp */
kprintf("file_lock: soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=114 (retowerangle stamp; Soft≠product)\n");
/* Grep: file_lock: soft retgateangle — Wave 76 return-gateangle honesty */
kprintf("file_lock: soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=114 (retgateangle honesty; Soft≠product; not bar3)\n");
/* Grep: file_lock: soft retwallangle — Wave 76 exclusive wallangle stamp */
kprintf("file_lock: soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=114 (retwallangle stamp; Soft≠product)\n");
/* Grep: file_lock: soft retspireangle — Wave 77 return-spireangle honesty */
kprintf("file_lock: soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=114 (retspireangle honesty; Soft≠product; not bar3)\n");
/* Grep: file_lock: soft retholdangle — Wave 77 exclusive holdangle stamp */
kprintf("file_lock: soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=114 (retholdangle stamp; Soft≠product)\n");
/* Grep: file_lock: soft retpalaceangle — Wave 78 return-palaceangle honesty */
kprintf("file_lock: soft retpalaceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=114 (retpalaceangle honesty; Soft≠product; not bar3)\n");
/* Grep: file_lock: soft retfortressangle — Wave 78 exclusive fortressangle stamp */
kprintf("file_lock: soft retfortressangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=114 (retfortressangle stamp; Soft≠product)\n");
/* Grep: file_lock: soft retkeepangle — Wave 79 return-keepangle honesty */
kprintf("file_lock: soft retkeepangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=114 (retkeepangle honesty; Soft≠product; not bar3)\n");
/* Grep: file_lock: soft retredoubtangle — Wave 79 exclusive redoubtangle stamp */
kprintf("file_lock: soft retredoubtangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=114 (retredoubtangle stamp; Soft≠product)\n");
/* Grep: file_lock: soft retcitadelangle — Wave 80 return-citadelangle honesty */
kprintf("file_lock: soft retcitadelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=114 (retcitadelangle honesty; Soft≠product; not bar3)\n");
/* Grep: file_lock: soft retbastionkeep — Wave 80 exclusive bastionkeep stamp */
kprintf("file_lock: soft retbastionkeep exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=114 (retbastionkeep stamp; Soft≠product)\n");
/* Grep: file_lock: soft retpanoplyangle — Wave 81 return-panoplyangle honesty */
kprintf("file_lock: soft retpanoplyangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=114 (retpanoplyangle honesty; Soft≠product; not bar3)\n");
/* Grep: file_lock: soft retbulwarkangle — Wave 81 exclusive bulwarkangle stamp */
kprintf("file_lock: soft retbulwarkangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=114 (retbulwarkangle stamp; Soft≠product)\n");
/* Grep: file_lock: soft retmantleangle — Wave 82 return-mantleangle honesty */
kprintf("file_lock: soft retmantleangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=114 (retmantleangle honesty; Soft≠product; not bar3)\n");
/* Grep: file_lock: soft retaegisangle — Wave 82 exclusive aegisangle stamp */
kprintf("file_lock: soft retaegisangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=114 (retaegisangle stamp; Soft≠product)\n");
/* Grep: file_lock: soft retemblemangle — Wave 83 return-emblemangle honesty */
kprintf("file_lock: soft retemblemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=114 (retemblemangle honesty; Soft≠product; not bar3)\n");
/* Grep: file_lock: soft retsigilangle — Wave 83 exclusive sigilangle stamp */
kprintf("file_lock: soft retsigilangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=114 (retsigilangle stamp; Soft≠product)\n");
/* Grep: file_lock: soft retscepterangle — Wave 84 return-scepterangle honesty */
kprintf("file_lock: soft retscepterangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=114 (retscepterangle honesty; Soft≠product; not bar3)\n");
/* Grep: file_lock: soft retglyphangle — Wave 84 exclusive glyphangle stamp */
kprintf("file_lock: soft retglyphangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=114 (retglyphangle stamp; Soft≠product)\n");
/* Grep: file_lock: soft retcrownangle — Wave 85 return-crownangle honesty */
kprintf("file_lock: soft retcrownangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=114 (retcrownangle honesty; Soft≠product; not bar3)\n");
/* Grep: file_lock: soft retshardangle — Wave 85 exclusive shardangle stamp */
kprintf("file_lock: soft retshardangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=114 (retshardangle stamp; Soft≠product)\n");
/* Grep: file_lock: soft retforgeangle — Wave 86 return-forgeangle honesty */
kprintf("file_lock: soft retforgeangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=114 (retforgeangle honesty; Soft≠product; not bar3)\n");
/* Grep: file_lock: soft retprismangle — Wave 86 exclusive prismangle stamp */
kprintf("file_lock: soft retprismangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=114 (retprismangle stamp; Soft≠product)\n");
/* Grep: file_lock: soft retflameangle — Wave 87 return-flameangle honesty */
kprintf("file_lock: soft retflameangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=114 (retflameangle honesty; Soft≠product; not bar3)\n");
/* Grep: file_lock: soft retcipherangle — Wave 87 exclusive cipherangle stamp */
kprintf("file_lock: soft retcipherangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=114 (retcipherangle stamp; Soft≠product)\n");
/* Grep: file_lock: soft retbeaconangle — Wave 88 return-beaconangle honesty */
kprintf("file_lock: soft retbeaconangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=114 (retbeaconangle honesty; Soft≠product; not bar3)\n");
/* Grep: file_lock: soft retledgerangle — Wave 88 exclusive ledgerangle stamp */
kprintf("file_lock: soft retledgerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=114 (retledgerangle stamp; Soft≠product)\n");
/* Grep: file_lock: soft retbannerangle — Wave 89 return-bannerangle honesty */
kprintf("file_lock: soft retbannerangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=114 (retbannerangle honesty; Soft≠product; not bar3)\n");
/* Grep: file_lock: soft retvaultangle — Wave 89 exclusive vaultangle stamp */
kprintf("file_lock: soft retvaultangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=114 (retvaultangle stamp; Soft≠product)\n");
/* Grep: file_lock: soft retcrestangle — Wave 90 return-crestangle honesty */
kprintf("file_lock: soft retcrestangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=114 (retcrestangle honesty; Soft≠product; not bar3)\n");
/* Grep: file_lock: soft rettokenangle — Wave 90 exclusive tokenangle stamp */
kprintf("file_lock: soft rettokenangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=114 (rettokenangle stamp; Soft≠product)\n");
/* Grep: file_lock: soft retbadgeangle — Wave 91 return-badgeangle honesty */
kprintf("file_lock: soft retbadgeangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=114 (retbadgeangle honesty; Soft≠product; not bar3)\n");
/* Grep: file_lock: soft retphaseangle — Wave 91 exclusive phaseangle stamp */
kprintf("file_lock: soft retphaseangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=114 (retphaseangle stamp; Soft≠product)\n");
/* Grep: file_lock: soft retmarkangle — Wave 92 return-markangle honesty */
kprintf("file_lock: soft retmarkangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=114 (retmarkangle honesty; Soft≠product; not bar3)\n");
/* Grep: file_lock: soft retpulseangle — Wave 92 exclusive pulseangle stamp */
kprintf("file_lock: soft retpulseangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=114 (retpulseangle stamp; Soft≠product)\n");

/* Grep: file_lock: soft retsealangle — Wave 93 return-sealangle honesty */
kprintf("file_lock: soft retsealangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=114 (retsealangle honesty; Soft≠product; not bar3)\n");
/* Grep: file_lock: soft retboundangle — Wave 93 exclusive boundangle stamp */
kprintf("file_lock: soft retboundangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=114 (retboundangle stamp; Soft≠product)\n");
/* Grep: file_lock: soft retstemangle — Wave 94 return-stemangle honesty */
kprintf("file_lock: soft retstemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=114 (retstemangle honesty; Soft≠product; not bar3)\n");
/* Grep: file_lock: soft retbladeangle — Wave 94 exclusive bladeangle stamp */
kprintf("file_lock: soft retbladeangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=114 (retbladeangle stamp; Soft≠product)\n");
/* Grep: file_lock: soft retchordangle — Wave 95 return-chordangle honesty */
kprintf("file_lock: soft retchordangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=114 (retchordangle honesty; Soft≠product; not bar3)\n");
/* Grep: file_lock: soft retarcangle — Wave 95 exclusive arcangle stamp */
kprintf("file_lock: soft retarcangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=114 (retarcangle stamp; Soft≠product)\n");
/* Grep: file_lock: soft retsectorangle — Wave 96 return-sectorangle honesty */
kprintf("file_lock: soft retsectorangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=114 (retsectorangle honesty; Soft≠product; not bar3)\n");
/* Grep: file_lock: soft retwedgeangle — Wave 96 exclusive wedgeangle stamp */
kprintf("file_lock: soft retwedgeangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=114 (retwedgeangle stamp; Soft≠product)\n");
/* Grep: file_lock: soft retradiusangle — Wave 97 return-radiusangle honesty */
kprintf("file_lock: soft retradiusangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=114 (retradiusangle honesty; Soft≠product; not bar3)\n");
/* Grep: file_lock: soft retdiameterangle — Wave 97 exclusive diameterangle stamp */
kprintf("file_lock: soft retdiameterangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=114 (retdiameterangle stamp; Soft≠product)\n");
/* Grep: file_lock: soft retcircumangle — Wave 98 return-circumangle honesty */
kprintf("file_lock: soft retcircumangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=114 (retcircumangle honesty; Soft≠product; not bar3)\n");
/* Grep: file_lock: soft retellipseangle — Wave 98 exclusive ellipseangle stamp */
kprintf("file_lock: soft retellipseangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=114 (retellipseangle stamp; Soft≠product)\n");
/* Grep: file_lock: soft rethyperangle — Wave 99 return-hyperangle honesty */
kprintf("file_lock: soft rethyperangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=114 (rethyperangle honesty; Soft≠product; not bar3)\n");
/* Grep: file_lock: soft retparabolaangle — Wave 99 exclusive parabolaangle stamp */
kprintf("file_lock: soft retparabolaangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=114 (retparabolaangle stamp; Soft≠product)\n");
/* Grep: file_lock: soft retspiralangle — Wave 100 return-spiralangle honesty */
kprintf("file_lock: soft retspiralangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=114 (retspiralangle honesty; Soft≠product; not bar3)\n");
/* Grep: file_lock: soft rethelixangle — Wave 100 exclusive helixangle stamp */
kprintf("file_lock: soft rethelixangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=114 (rethelixangle stamp; Soft≠product)\n");
/* Grep: file_lock: soft rettorusangle — Wave 101 return-torusangle honesty */
kprintf("file_lock: soft rettorusangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=114 (rettorusangle honesty; Soft≠product; not bar3)\n");
/* Grep: file_lock: soft retknotangle — Wave 101 exclusive knotangle stamp */
kprintf("file_lock: soft retknotangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=114 (retknotangle stamp; Soft≠product)\n");
/* Grep: file_lock: soft retmoebiusangle — Wave 102 return-moebiusangle honesty */
kprintf("file_lock: soft retmoebiusangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=114 (retmoebiusangle honesty; Soft≠product; not bar3)\n");
/* Grep: file_lock: soft retkleinangle — Wave 102 exclusive kleinangle stamp */
kprintf("file_lock: soft retkleinangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=114 (retkleinangle stamp; Soft≠product)\n");
/* Grep: file_lock: soft retprojectangle — Wave 103 return-projectangle honesty */
kprintf("file_lock: soft retprojectangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=114 (retprojectangle honesty; Soft≠product; not bar3)\n");
/* Grep: file_lock: soft retaffineangle — Wave 103 exclusive affineangle stamp */
kprintf("file_lock: soft retaffineangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=114 (retaffineangle stamp; Soft≠product)\n");
/* Grep: file_lock: soft retlinearangle — Wave 104 return-linearangle honesty */
kprintf("file_lock: soft retlinearangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=114 (retlinearangle honesty; Soft≠product; not bar3)\n");
/* Grep: file_lock: soft retbilinearangle — Wave 104 exclusive bilinearangle stamp */
kprintf("file_lock: soft retbilinearangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=114 (retbilinearangle stamp; Soft≠product)\n");
/* Grep: file_lock: soft retquadraticangle — Wave 105 return-quadraticangle honesty */
kprintf("file_lock: soft retquadraticangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=114 (retquadraticangle honesty; Soft≠product; not bar3)\n");
/* Grep: file_lock: soft retcubicangle — Wave 105 exclusive cubicangle stamp */
kprintf("file_lock: soft retcubicangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=114 (retcubicangle stamp; Soft≠product)\n");
/* Grep: file_lock: soft retquarticangle — Wave 106 return-quarticangle honesty */
kprintf("file_lock: soft retquarticangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=114 (retquarticangle honesty; Soft≠product; not bar3)\n");
/* Grep: file_lock: soft retquinticangle — Wave 106 exclusive quinticangle stamp */
kprintf("file_lock: soft retquinticangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=114 (retquinticangle stamp; Soft≠product)\n");
/* Grep: file_lock: soft retsplineangle — Wave 107 return-splineangle honesty */
kprintf("file_lock: soft retsplineangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=114 (retsplineangle honesty; Soft≠product; not bar3)\n");
/* Grep: file_lock: soft retbezierangle — Wave 107 exclusive bezierangle stamp */
kprintf("file_lock: soft retbezierangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=114 (retbezierangle stamp; Soft≠product)\n");
/* Grep: file_lock: soft rethurmitangle — Wave 108 return-hermitangle honesty */
kprintf("file_lock: soft rethurmitangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=114 (rethurmitangle honesty; Soft≠product; not bar3)\n");
/* Grep: file_lock: soft retcatmullangle — Wave 108 exclusive catmullangle stamp */
kprintf("file_lock: soft retcatmullangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=114 (retcatmullangle stamp; Soft≠product)\n");
/* Grep: file_lock: soft retnurbsangle — Wave 109 return-nurbsangle honesty */
kprintf("file_lock: soft retnurbsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=114 (retnurbsangle honesty; Soft≠product; not bar3)\n");
/* Grep: file_lock: soft retbsplineangle — Wave 109 exclusive bsplineangle stamp */
kprintf("file_lock: soft retbsplineangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=114 (retbsplineangle stamp; Soft≠product)\n");
/* Grep: file_lock: soft retmeshangle — Wave 110 return-meshangle honesty */
kprintf("file_lock: soft retmeshangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=114 (retmeshangle honesty; Soft≠product; not bar3)\n");
/* Grep: file_lock: soft retgridangle — Wave 110 exclusive gridangle stamp */
kprintf("file_lock: soft retgridangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=114 (retgridangle stamp; Soft≠product)\n");
/* Grep: file_lock: soft retvoxelangle — Wave 111 return-voxelangle honesty */
kprintf("file_lock: soft retvoxelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=114 (retvoxelangle honesty; Soft≠product; not bar3)\n");
/* Grep: file_lock: soft rettexelangle — Wave 111 exclusive texelangle stamp */
kprintf("file_lock: soft rettexelangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=114 (rettexelangle stamp; Soft≠product)\n");
/* Grep: file_lock: soft retfragmentangle — Wave 112 return-fragmentangle honesty */
kprintf("file_lock: soft retfragmentangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=114 (retfragmentangle honesty; Soft≠product; not bar3)\n");
/* Grep: file_lock: soft retvertexangle — Wave 112 exclusive vertexangle stamp */
kprintf("file_lock: soft retvertexangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=114 (retvertexangle stamp; Soft≠product)\n");
/* Grep: file_lock: soft retshaderangle — Wave 113 return-shaderangle honesty */
kprintf("file_lock: soft retshaderangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=114 (retshaderangle honesty; Soft≠product; not bar3)\n");
/* Grep: file_lock: soft retpipelineangle — Wave 113 exclusive pipelineangle stamp */
kprintf("file_lock: soft retpipelineangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=114 (retpipelineangle stamp; Soft≠product)\n");
/* Grep: file_lock: soft retframebufferangle — Wave 114 return-framebufferangle honesty */
kprintf("file_lock: soft retframebufferangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=114 (retframebufferangle honesty; Soft≠product; not bar3)\n");
/* Grep: file_lock: soft retswapchainangle — Wave 114 exclusive swapchainangle stamp */
kprintf("file_lock: soft retswapchainangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=114 (retswapchainangle stamp; Soft≠product)\n");
                            kprintf("file_lock: soft deepen wave=114 areas=total,rate,deadlk,grant,,retclass,retlane"
    ...
    kprintf("file_lock: soft path claim=advisory_flock set=carve+insert "
            ...
            "deadlk=FLOCK_SOFT_DEADLOCK soft_honesty=soft wave=114 "
            "(advisory soft inventory; not bar3)\n");
}
