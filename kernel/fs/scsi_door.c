...
soft_inventory_log(const char *szVia)
{
    ...
    kprintf("scsi_door: soft inventory via=%s enter=%u ok=%u fail=%u "
            ...
            "shim=kernel_mid product=userspace_scsi_mid wave=113\n",
    ...
    kprintf("scsi_door: soft catalog "
            ...
            "timeout_ms=5000 lun=0 wave=113\n",
    ...
    kprintf("scsi_door: soft total ok=%u fail=%u enter=%u ok_bp=%u "
            "soft_mid_ok=%u virt_mid_ok=%u logs=%u wave=113\n",
    ...
    kprintf("scsi_door: soft shape zero_cb=%u short_inq=%u sync_whole=%u "
            "lun0=%u timeout_fix=%u peak_lba=0x%x wave=113\n",
    ...
    kprintf("scsi_door: soft surface ready=%u soft_lun=%u raw_ok=%u "
            "enter=%u ok=%u fail=%u surf=0x%x wave=113\n",
    ...
    kprintf("scsi_door: soft return enter=%u ok=%u fail=%u deny_null=%u "
            ...
            "soft_lun=%u product_scsi_mid=OPEN wave=113\n",
    ...
    kprintf("scsi_door: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=113\n");
...
kprintf("scsi_door: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=113 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("scsi_door: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=113 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("scsi_door: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=113 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("scsi_door: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=113 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("scsi_door: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=113 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("scsi_door: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=113 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: scsi_door: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("scsi_door: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=113 (retcurtainface honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("scsi_door: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=113 (retbastionangle stamp; Soft≠product)\n");
/* Grep: scsi_door: soft retglacisangle — Wave 74 return-glacisangle honesty */
kprintf("scsi_door: soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=113 (retglacisangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
kprintf("scsi_door: soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=113 (retparapetangle stamp; Soft≠product)\n");
/* Grep: scsi_door: soft retmoatangle — Wave 75 return-moatangle honesty */
kprintf("scsi_door: soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=113 (retmoatangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retowerangle — Wave 75 exclusive towerangle stamp */
kprintf("scsi_door: soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=113 (retowerangle stamp; Soft≠product)\n");
/* Grep: scsi_door: soft retgateangle — Wave 76 return-gateangle honesty */
kprintf("scsi_door: soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=113 (retgateangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retwallangle — Wave 76 exclusive wallangle stamp */
kprintf("scsi_door: soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=113 (retwallangle stamp; Soft≠product)\n");
/* Grep: scsi_door: soft retspireangle — Wave 77 return-spireangle honesty */
kprintf("scsi_door: soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=113 (retspireangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retholdangle — Wave 77 exclusive holdangle stamp */
kprintf("scsi_door: soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=113 (retholdangle stamp; Soft≠product)\n");
/* Grep: scsi_door: soft retpalaceangle — Wave 78 return-palaceangle honesty */
kprintf("scsi_door: soft retpalaceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=113 (retpalaceangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retfortressangle — Wave 78 exclusive fortressangle stamp */
kprintf("scsi_door: soft retfortressangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=113 (retfortressangle stamp; Soft≠product)\n");
/* Grep: scsi_door: soft retkeepangle — Wave 79 return-keepangle honesty */
kprintf("scsi_door: soft retkeepangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=113 (retkeepangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retredoubtangle — Wave 79 exclusive redoubtangle stamp */
kprintf("scsi_door: soft retredoubtangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=113 (retredoubtangle stamp; Soft≠product)\n");
/* Grep: scsi_door: soft retcitadelangle — Wave 80 return-citadelangle honesty */
kprintf("scsi_door: soft retcitadelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=113 (retcitadelangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retbastionkeep — Wave 80 exclusive bastionkeep stamp */
kprintf("scsi_door: soft retbastionkeep exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=113 (retbastionkeep stamp; Soft≠product)\n");
/* Grep: scsi_door: soft retpanoplyangle — Wave 81 return-panoplyangle honesty */
kprintf("scsi_door: soft retpanoplyangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=113 (retpanoplyangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retbulwarkangle — Wave 81 exclusive bulwarkangle stamp */
kprintf("scsi_door: soft retbulwarkangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=113 (retbulwarkangle stamp; Soft≠product)\n");
/* Grep: scsi_door: soft retmantleangle — Wave 82 return-mantleangle honesty */
kprintf("scsi_door: soft retmantleangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=113 (retmantleangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retaegisangle — Wave 82 exclusive aegisangle stamp */
kprintf("scsi_door: soft retaegisangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=113 (retaegisangle stamp; Soft≠product)\n");
/* Grep: scsi_door: soft retemblemangle — Wave 83 return-emblemangle honesty */
kprintf("scsi_door: soft retemblemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=113 (retemblemangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retsigilangle — Wave 83 exclusive sigilangle stamp */
kprintf("scsi_door: soft retsigilangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=113 (retsigilangle stamp; Soft≠product)\n");
/* Grep: scsi_door: soft retscepterangle — Wave 84 return-scepterangle honesty */
kprintf("scsi_door: soft retscepterangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=113 (retscepterangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retglyphangle — Wave 84 exclusive glyphangle stamp */
kprintf("scsi_door: soft retglyphangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=113 (retglyphangle stamp; Soft≠product)\n");
/* Grep: scsi_door: soft retcrownangle — Wave 85 return-crownangle honesty */
kprintf("scsi_door: soft retcrownangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=113 (retcrownangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retshardangle — Wave 85 exclusive shardangle stamp */
kprintf("scsi_door: soft retshardangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=113 (retshardangle stamp; Soft≠product)\n");
/* Grep: scsi_door: soft retforgeangle — Wave 86 return-forgeangle honesty */
kprintf("scsi_door: soft retforgeangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=113 (retforgeangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retprismangle — Wave 86 exclusive prismangle stamp */
kprintf("scsi_door: soft retprismangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=113 (retprismangle stamp; Soft≠product)\n");
/* Grep: scsi_door: soft retflameangle — Wave 87 return-flameangle honesty */
kprintf("scsi_door: soft retflameangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=113 (retflameangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retcipherangle — Wave 87 exclusive cipherangle stamp */
kprintf("scsi_door: soft retcipherangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=113 (retcipherangle stamp; Soft≠product)\n");
/* Grep: scsi_door: soft retbeaconangle — Wave 88 return-beaconangle honesty */
kprintf("scsi_door: soft retbeaconangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=113 (retbeaconangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retledgerangle — Wave 88 exclusive ledgerangle stamp */
kprintf("scsi_door: soft retledgerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=113 (retledgerangle stamp; Soft≠product)\n");
/* Grep: scsi_door: soft retbannerangle — Wave 89 return-bannerangle honesty */
kprintf("scsi_door: soft retbannerangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=113 (retbannerangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retvaultangle — Wave 89 exclusive vaultangle stamp */
kprintf("scsi_door: soft retvaultangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=113 (retvaultangle stamp; Soft≠product)\n");
/* Grep: scsi_door: soft retcrestangle — Wave 90 return-crestangle honesty */
kprintf("scsi_door: soft retcrestangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=113 (retcrestangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft rettokenangle — Wave 90 exclusive tokenangle stamp */
kprintf("scsi_door: soft rettokenangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=113 (rettokenangle stamp; Soft≠product)\n");
/* Grep: scsi_door: soft retbadgeangle — Wave 91 return-badgeangle honesty */
kprintf("scsi_door: soft retbadgeangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=113 (retbadgeangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retphaseangle — Wave 91 exclusive phaseangle stamp */
kprintf("scsi_door: soft retphaseangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=113 (retphaseangle stamp; Soft≠product)\n");
/* Grep: scsi_door: soft retmarkangle — Wave 92 return-markangle honesty */
kprintf("scsi_door: soft retmarkangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=113 (retmarkangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retpulseangle — Wave 92 exclusive pulseangle stamp */
kprintf("scsi_door: soft retpulseangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=113 (retpulseangle stamp; Soft≠product)\n");

/* Grep: scsi_door: soft retsealangle — Wave 93 return-sealangle honesty */
kprintf("scsi_door: soft retsealangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=113 (retsealangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retboundangle — Wave 93 exclusive boundangle stamp */
kprintf("scsi_door: soft retboundangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=113 (retboundangle stamp; Soft≠product)\n");
/* Grep: scsi_door: soft retstemangle — Wave 94 return-stemangle honesty */
kprintf("scsi_door: soft retstemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=113 (retstemangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retbladeangle — Wave 94 exclusive bladeangle stamp */
kprintf("scsi_door: soft retbladeangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=113 (retbladeangle stamp; Soft≠product)\n");
/* Grep: scsi_door: soft retchordangle — Wave 95 return-chordangle honesty */
kprintf("scsi_door: soft retchordangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=113 (retchordangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retarcangle — Wave 95 exclusive arcangle stamp */
kprintf("scsi_door: soft retarcangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=113 (retarcangle stamp; Soft≠product)\n");
/* Grep: scsi_door: soft retsectorangle — Wave 96 return-sectorangle honesty */
kprintf("scsi_door: soft retsectorangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=113 (retsectorangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retwedgeangle — Wave 96 exclusive wedgeangle stamp */
kprintf("scsi_door: soft retwedgeangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=113 (retwedgeangle stamp; Soft≠product)\n");
/* Grep: scsi_door: soft retradiusangle — Wave 97 return-radiusangle honesty */
kprintf("scsi_door: soft retradiusangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=113 (retradiusangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retdiameterangle — Wave 97 exclusive diameterangle stamp */
kprintf("scsi_door: soft retdiameterangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=113 (retdiameterangle stamp; Soft≠product)\n");
/* Grep: scsi_door: soft retcircumangle — Wave 98 return-circumangle honesty */
kprintf("scsi_door: soft retcircumangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=113 (retcircumangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retellipseangle — Wave 98 exclusive ellipseangle stamp */
kprintf("scsi_door: soft retellipseangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=113 (retellipseangle stamp; Soft≠product)\n");
/* Grep: scsi_door: soft rethyperangle — Wave 99 return-hyperangle honesty */
kprintf("scsi_door: soft rethyperangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=113 (rethyperangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retparabolaangle — Wave 99 exclusive parabolaangle stamp */
kprintf("scsi_door: soft retparabolaangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=113 (retparabolaangle stamp; Soft≠product)\n");
/* Grep: scsi_door: soft retspiralangle — Wave 100 return-spiralangle honesty */
kprintf("scsi_door: soft retspiralangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=113 (retspiralangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft rethelixangle — Wave 100 exclusive helixangle stamp */
kprintf("scsi_door: soft rethelixangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=113 (rethelixangle stamp; Soft≠product)\n");
/* Grep: scsi_door: soft rettorusangle — Wave 101 return-torusangle honesty */
kprintf("scsi_door: soft rettorusangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=113 (rettorusangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retknotangle — Wave 101 exclusive knotangle stamp */
kprintf("scsi_door: soft retknotangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=113 (retknotangle stamp; Soft≠product)\n");
/* Grep: scsi_door: soft retmoebiusangle — Wave 102 return-moebiusangle honesty */
kprintf("scsi_door: soft retmoebiusangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=113 (retmoebiusangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retkleinangle — Wave 102 exclusive kleinangle stamp */
kprintf("scsi_door: soft retkleinangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=113 (retkleinangle stamp; Soft≠product)\n");
/* Grep: scsi_door: soft retprojectangle — Wave 103 return-projectangle honesty */
kprintf("scsi_door: soft retprojectangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=113 (retprojectangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retaffineangle — Wave 103 exclusive affineangle stamp */
kprintf("scsi_door: soft retaffineangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=113 (retaffineangle stamp; Soft≠product)\n");
/* Grep: scsi_door: soft retlinearangle — Wave 104 return-linearangle honesty */
kprintf("scsi_door: soft retlinearangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=113 (retlinearangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retbilinearangle — Wave 104 exclusive bilinearangle stamp */
kprintf("scsi_door: soft retbilinearangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=113 (retbilinearangle stamp; Soft≠product)\n");
/* Grep: scsi_door: soft retquadraticangle — Wave 105 return-quadraticangle honesty */
kprintf("scsi_door: soft retquadraticangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=113 (retquadraticangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retcubicangle — Wave 105 exclusive cubicangle stamp */
kprintf("scsi_door: soft retcubicangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=113 (retcubicangle stamp; Soft≠product)\n");
/* Grep: scsi_door: soft retquarticangle — Wave 106 return-quarticangle honesty */
kprintf("scsi_door: soft retquarticangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=113 (retquarticangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retquinticangle — Wave 106 exclusive quinticangle stamp */
kprintf("scsi_door: soft retquinticangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=113 (retquinticangle stamp; Soft≠product)\n");
/* Grep: scsi_door: soft retsplineangle — Wave 107 return-splineangle honesty */
kprintf("scsi_door: soft retsplineangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=113 (retsplineangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retbezierangle — Wave 107 exclusive bezierangle stamp */
kprintf("scsi_door: soft retbezierangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=113 (retbezierangle stamp; Soft≠product)\n");
/* Grep: scsi_door: soft rethurmitangle — Wave 108 return-hermitangle honesty */
kprintf("scsi_door: soft rethurmitangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=113 (rethurmitangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retcatmullangle — Wave 108 exclusive catmullangle stamp */
kprintf("scsi_door: soft retcatmullangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=113 (retcatmullangle stamp; Soft≠product)\n");
/* Grep: scsi_door: soft retnurbsangle — Wave 109 return-nurbsangle honesty */
kprintf("scsi_door: soft retnurbsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=113 (retnurbsangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retbsplineangle — Wave 109 exclusive bsplineangle stamp */
kprintf("scsi_door: soft retbsplineangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=113 (retbsplineangle stamp; Soft≠product)\n");
/* Grep: scsi_door: soft retmeshangle — Wave 110 return-meshangle honesty */
kprintf("scsi_door: soft retmeshangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=113 (retmeshangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retgridangle — Wave 110 exclusive gridangle stamp */
kprintf("scsi_door: soft retgridangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=113 (retgridangle stamp; Soft≠product)\n");
/* Grep: scsi_door: soft retvoxelangle — Wave 111 return-voxelangle honesty */
kprintf("scsi_door: soft retvoxelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=113 (retvoxelangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft rettexelangle — Wave 111 exclusive texelangle stamp */
kprintf("scsi_door: soft rettexelangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=113 (rettexelangle stamp; Soft≠product)\n");
/* Grep: scsi_door: soft retfragmentangle — Wave 112 return-fragmentangle honesty */
kprintf("scsi_door: soft retfragmentangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=113 (retfragmentangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retvertexangle — Wave 112 exclusive vertexangle stamp */
kprintf("scsi_door: soft retvertexangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=113 (retvertexangle stamp; Soft≠product)\n");
/* Grep: scsi_door: soft retshaderangle — Wave 113 return-shaderangle honesty */
kprintf("scsi_door: soft retshaderangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=113 (retshaderangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retpipelineangle — Wave 113 exclusive pipelineangle stamp */
kprintf("scsi_door: soft retpipelineangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=113 (retpipelineangle stamp; Soft≠product)\n");
                            kprintf("scsi_door: soft deepen wave=113 areas=total,rate,raw,shape,,retclass,retlane"
    ...
    if (fSoftPass != 0) {
        kprintf("scsi_door: soft inventory PASS via=%s logs=%u "
                "mid_ready=%u soft_lun=%u wave=113\n",
                szViaSafe, g_u32SoftInvSamples, u32Ready, u32Soft);
        kprintf("scsi_door: soft PASS via=%s wave=113\n", szViaSafe);
    } else {
        kprintf("scsi_door: soft FAIL via=%s mid_ready=0 wave=113 "
