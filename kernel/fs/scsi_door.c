...
soft_inventory_log(const char *szVia)
{
    ...
    kprintf("scsi_door: soft inventory via=%s enter=%u ok=%u fail=%u "
            ...
            "shim=kernel_mid product=userspace_scsi_mid wave=78\n",
    ...
    kprintf("scsi_door: soft total ok=%u fail=%u enter=%u ok_bp=%u "
            "soft_mid_ok=%u virt_mid_ok=%u logs=%u wave=78\n",
    ...
    kprintf("scsi_door: soft shape zero_cb=%u short_inq=%u sync_whole=%u "
            "lun0=%u timeout_fix=%u peak_lba=0x%x wave=78\n",
    ...
    kprintf("scsi_door: soft surface ready=%u soft_lun=%u raw_ok=%u "
            "enter=%u ok=%u fail=%u surf=0x%x wave=78\n",
    ...
    kprintf("scsi_door: soft return enter=%u ok=%u fail=%u deny_null=%u "
            ...
            "soft_lun=%u product_scsi_mid=OPEN wave=78\n",
    ...
    kprintf("scsi_door: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=78\n");
...
kprintf("scsi_door: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=78 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("scsi_door: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=78 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("scsi_door: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=78 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("scsi_door: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=78 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("scsi_door: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=78 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("scsi_door: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=78 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: scsi_door: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("scsi_door: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=78 (retcurtainface honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("scsi_door: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=78 (retbastionangle stamp; Soft≠product)\n");
/* Grep: scsi_door: soft retglacisangle — Wave 74 return-glacisangle honesty */
kprintf("scsi_door: soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=78 (retglacisangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
kprintf("scsi_door: soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=78 (retparapetangle stamp; Soft≠product)\n");
/* Grep: scsi_door: soft retmoatangle — Wave 75 return-moatangle honesty */
kprintf("scsi_door: soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=78 (retmoatangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retowerangle — Wave 75 exclusive towerangle stamp */
kprintf("scsi_door: soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=78 (retowerangle stamp; Soft≠product)\n");
/* Grep: scsi_door: soft retgateangle — Wave 76 return-gateangle honesty */
kprintf("scsi_door: soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=78 (retgateangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retwallangle — Wave 76 exclusive wallangle stamp */
kprintf("scsi_door: soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=78 (retwallangle stamp; Soft≠product)\n");
/* Grep: scsi_door: soft retspireangle — Wave 77 return-spireangle honesty */
kprintf("scsi_door: soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=78 (retspireangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retholdangle — Wave 77 exclusive holdangle stamp */
kprintf("scsi_door: soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=78 (retholdangle stamp; Soft≠product)\n");
/* Grep: scsi_door: soft retpalaceangle — Wave 78 return-palaceangle honesty */
kprintf("scsi_door: soft retpalaceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=78 (retpalaceangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_door: soft retfortressangle — Wave 78 exclusive fortressangle stamp */
kprintf("scsi_door: soft retfortressangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=78 (retfortressangle stamp; Soft≠product)\n");

                            kprintf("scsi_door: soft deepen wave=78 areas=total,rate,raw,shape,,retclass,retlane"
    ...
    if (fSoftPass != 0) {
        kprintf("scsi_door: soft inventory PASS via=%s logs=%u "
                "mid_ready=%u soft_lun=%u wave=78\n",
                szViaSafe, g_u32SoftInvSamples, u32Ready, u32Soft);
        kprintf("scsi_door: soft PASS via=%s wave=78\n", szViaSafe);
    } else {
        kprintf("scsi_door: soft FAIL via=%s mid_ready=0 wave=78 "
