...
soft_inventory_log(const char *szVia)
{
    ...
    kprintf("scsi_mid: soft inventory via=%s wave=111 enter=%u ok=%u fail=%u "
    ...
    kprintf("scsi_mid: soft total ok=%u fail=%u enter=%u ok_bp=%u "
            "soft_ok=%u soft_fail=%u logs=%u wave=111\n",
    ...
    kprintf("scsi_mid: soft surface ready=%u soft=%u virtio=%u prefer=%u "
            "enter=%u ok=%u surf=0x%x wave=111\n",
    ...
    kprintf("scsi_mid: soft return enter=%u ok=%u fail=%u via_virtio=%u "
            ...
            "no_path=%u mid_ready=%u product_scsi_mid=OPEN wave=111\n",
    ...
    kprintf("scsi_mid: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=111\n");
...
kprintf("scsi_mid: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=111 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("scsi_mid: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=111 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("scsi_mid: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=111 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("scsi_mid: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=111 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("scsi_mid: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=111 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("scsi_mid: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=111 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("scsi_mid: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=111 (retcurtainface honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("scsi_mid: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=111 (retbastionangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retglacisangle — Wave 74 return-glacisangle honesty */
kprintf("scsi_mid: soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=111 (retglacisangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
kprintf("scsi_mid: soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=111 (retparapetangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retmoatangle — Wave 75 return-moatangle honesty */
kprintf("scsi_mid: soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=111 (retmoatangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retowerangle — Wave 75 exclusive towerangle stamp */
kprintf("scsi_mid: soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=111 (retowerangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retgateangle — Wave 76 return-gateangle honesty */
kprintf("scsi_mid: soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=111 (retgateangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retwallangle — Wave 76 exclusive wallangle stamp */
kprintf("scsi_mid: soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=111 (retwallangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retspireangle — Wave 77 return-spireangle honesty */
kprintf("scsi_mid: soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=111 (retspireangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retholdangle — Wave 77 exclusive holdangle stamp */
kprintf("scsi_mid: soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=111 (retholdangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retpalaceangle — Wave 78 return-palaceangle honesty */
kprintf("scsi_mid: soft retpalaceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=111 (retpalaceangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retfortressangle — Wave 78 exclusive fortressangle stamp */
kprintf("scsi_mid: soft retfortressangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=111 (retfortressangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retkeepangle — Wave 79 return-keepangle honesty */
kprintf("scsi_mid: soft retkeepangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=111 (retkeepangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retredoubtangle — Wave 79 exclusive redoubtangle stamp */
kprintf("scsi_mid: soft retredoubtangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=111 (retredoubtangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retcitadelangle — Wave 80 return-citadelangle honesty */
kprintf("scsi_mid: soft retcitadelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=111 (retcitadelangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retbastionkeep — Wave 80 exclusive bastionkeep stamp */
kprintf("scsi_mid: soft retbastionkeep exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=111 (retbastionkeep stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retpanoplyangle — Wave 81 return-panoplyangle honesty */
kprintf("scsi_mid: soft retpanoplyangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=111 (retpanoplyangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retbulwarkangle — Wave 81 exclusive bulwarkangle stamp */
kprintf("scsi_mid: soft retbulwarkangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=111 (retbulwarkangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retmantleangle — Wave 82 return-mantleangle honesty */
kprintf("scsi_mid: soft retmantleangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=111 (retmantleangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retaegisangle — Wave 82 exclusive aegisangle stamp */
kprintf("scsi_mid: soft retaegisangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=111 (retaegisangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retemblemangle — Wave 83 return-emblemangle honesty */
kprintf("scsi_mid: soft retemblemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=111 (retemblemangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retsigilangle — Wave 83 exclusive sigilangle stamp */
kprintf("scsi_mid: soft retsigilangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=111 (retsigilangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retscepterangle — Wave 84 return-scepterangle honesty */
kprintf("scsi_mid: soft retscepterangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=111 (retscepterangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retglyphangle — Wave 84 exclusive glyphangle stamp */
kprintf("scsi_mid: soft retglyphangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=111 (retglyphangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retcrownangle — Wave 85 return-crownangle honesty */
kprintf("scsi_mid: soft retcrownangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=111 (retcrownangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retshardangle — Wave 85 exclusive shardangle stamp */
kprintf("scsi_mid: soft retshardangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=111 (retshardangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retforgeangle — Wave 86 return-forgeangle honesty */
kprintf("scsi_mid: soft retforgeangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=111 (retforgeangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retprismangle — Wave 86 exclusive prismangle stamp */
kprintf("scsi_mid: soft retprismangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=111 (retprismangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retflameangle — Wave 87 return-flameangle honesty */
kprintf("scsi_mid: soft retflameangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=111 (retflameangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retcipherangle — Wave 87 exclusive cipherangle stamp */
kprintf("scsi_mid: soft retcipherangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=111 (retcipherangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retbeaconangle — Wave 88 return-beaconangle honesty */
kprintf("scsi_mid: soft retbeaconangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=111 (retbeaconangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retledgerangle — Wave 88 exclusive ledgerangle stamp */
kprintf("scsi_mid: soft retledgerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=111 (retledgerangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retbannerangle — Wave 89 return-bannerangle honesty */
kprintf("scsi_mid: soft retbannerangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=111 (retbannerangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retvaultangle — Wave 89 exclusive vaultangle stamp */
kprintf("scsi_mid: soft retvaultangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=111 (retvaultangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retcrestangle — Wave 90 return-crestangle honesty */
kprintf("scsi_mid: soft retcrestangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=111 (retcrestangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft rettokenangle — Wave 90 exclusive tokenangle stamp */
kprintf("scsi_mid: soft rettokenangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=111 (rettokenangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retbadgeangle — Wave 91 return-badgeangle honesty */
kprintf("scsi_mid: soft retbadgeangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=111 (retbadgeangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retphaseangle — Wave 91 exclusive phaseangle stamp */
kprintf("scsi_mid: soft retphaseangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=111 (retphaseangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retmarkangle — Wave 92 return-markangle honesty */
kprintf("scsi_mid: soft retmarkangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=111 (retmarkangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retpulseangle — Wave 92 exclusive pulseangle stamp */
kprintf("scsi_mid: soft retpulseangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=111 (retpulseangle stamp; Soft≠product)\n");

/* Grep: scsi_mid: soft retsealangle — Wave 93 return-sealangle honesty */
kprintf("scsi_mid: soft retsealangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=111 (retsealangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retboundangle — Wave 93 exclusive boundangle stamp */
kprintf("scsi_mid: soft retboundangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=111 (retboundangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retstemangle — Wave 94 return-stemangle honesty */
kprintf("scsi_mid: soft retstemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=111 (retstemangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retbladeangle — Wave 94 exclusive bladeangle stamp */
kprintf("scsi_mid: soft retbladeangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=111 (retbladeangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retchordangle — Wave 95 return-chordangle honesty */
kprintf("scsi_mid: soft retchordangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=111 (retchordangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retarcangle — Wave 95 exclusive arcangle stamp */
kprintf("scsi_mid: soft retarcangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=111 (retarcangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retsectorangle — Wave 96 return-sectorangle honesty */
kprintf("scsi_mid: soft retsectorangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=111 (retsectorangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retwedgeangle — Wave 96 exclusive wedgeangle stamp */
kprintf("scsi_mid: soft retwedgeangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=111 (retwedgeangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retradiusangle — Wave 97 return-radiusangle honesty */
kprintf("scsi_mid: soft retradiusangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=111 (retradiusangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retdiameterangle — Wave 97 exclusive diameterangle stamp */
kprintf("scsi_mid: soft retdiameterangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=111 (retdiameterangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retcircumangle — Wave 98 return-circumangle honesty */
kprintf("scsi_mid: soft retcircumangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=111 (retcircumangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retellipseangle — Wave 98 exclusive ellipseangle stamp */
kprintf("scsi_mid: soft retellipseangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=111 (retellipseangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft rethyperangle — Wave 99 return-hyperangle honesty */
kprintf("scsi_mid: soft rethyperangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=111 (rethyperangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retparabolaangle — Wave 99 exclusive parabolaangle stamp */
kprintf("scsi_mid: soft retparabolaangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=111 (retparabolaangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retspiralangle — Wave 100 return-spiralangle honesty */
kprintf("scsi_mid: soft retspiralangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=111 (retspiralangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft rethelixangle — Wave 100 exclusive helixangle stamp */
kprintf("scsi_mid: soft rethelixangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=111 (rethelixangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft rettorusangle — Wave 101 return-torusangle honesty */
kprintf("scsi_mid: soft rettorusangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=111 (rettorusangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retknotangle — Wave 101 exclusive knotangle stamp */
kprintf("scsi_mid: soft retknotangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=111 (retknotangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retmoebiusangle — Wave 102 return-moebiusangle honesty */
kprintf("scsi_mid: soft retmoebiusangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=111 (retmoebiusangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retkleinangle — Wave 102 exclusive kleinangle stamp */
kprintf("scsi_mid: soft retkleinangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=111 (retkleinangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retprojectangle — Wave 103 return-projectangle honesty */
kprintf("scsi_mid: soft retprojectangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=111 (retprojectangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retaffineangle — Wave 103 exclusive affineangle stamp */
kprintf("scsi_mid: soft retaffineangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=111 (retaffineangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retlinearangle — Wave 104 return-linearangle honesty */
kprintf("scsi_mid: soft retlinearangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=111 (retlinearangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retbilinearangle — Wave 104 exclusive bilinearangle stamp */
kprintf("scsi_mid: soft retbilinearangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=111 (retbilinearangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retquadraticangle — Wave 105 return-quadraticangle honesty */
kprintf("scsi_mid: soft retquadraticangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=111 (retquadraticangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retcubicangle — Wave 105 exclusive cubicangle stamp */
kprintf("scsi_mid: soft retcubicangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=111 (retcubicangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retquarticangle — Wave 106 return-quarticangle honesty */
kprintf("scsi_mid: soft retquarticangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=111 (retquarticangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retquinticangle — Wave 106 exclusive quinticangle stamp */
kprintf("scsi_mid: soft retquinticangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=111 (retquinticangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retsplineangle — Wave 107 return-splineangle honesty */
kprintf("scsi_mid: soft retsplineangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=111 (retsplineangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retbezierangle — Wave 107 exclusive bezierangle stamp */
kprintf("scsi_mid: soft retbezierangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=111 (retbezierangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft rethurmitangle — Wave 108 return-hermitangle honesty */
kprintf("scsi_mid: soft rethurmitangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=111 (rethurmitangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retcatmullangle — Wave 108 exclusive catmullangle stamp */
kprintf("scsi_mid: soft retcatmullangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=111 (retcatmullangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retnurbsangle — Wave 109 return-nurbsangle honesty */
kprintf("scsi_mid: soft retnurbsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=111 (retnurbsangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retbsplineangle — Wave 109 exclusive bsplineangle stamp */
kprintf("scsi_mid: soft retbsplineangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=111 (retbsplineangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retmeshangle — Wave 110 return-meshangle honesty */
kprintf("scsi_mid: soft retmeshangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=111 (retmeshangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retgridangle — Wave 110 exclusive gridangle stamp */
kprintf("scsi_mid: soft retgridangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=111 (retgridangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retvoxelangle — Wave 111 return-voxelangle honesty */
kprintf("scsi_mid: soft retvoxelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=111 (retvoxelangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft rettexelangle — Wave 111 exclusive texelangle stamp */
kprintf("scsi_mid: soft rettexelangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=111 (rettexelangle stamp; Soft≠product)\n");
                            kprintf("scsi_mid: soft deepen wave=111 areas=total,soft,sense,xfer,,retclass,retlane"
    ...
    if (fSoftPass != 0) {
        kprintf("scsi_mid: soft inventory PASS via=%s logs=%u "
                ...
                u32Virtio, u32Prefer);
        kprintf("scsi_mid: soft PASS via=%s wave=111\n", szViaSafe);
    } else {
        kprintf("scsi_mid: soft FAIL via=%s mid_ready=0 wave=111 "
