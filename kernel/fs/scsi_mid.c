 ...
 *
 * Soft inventory (Wave 13 base + Wave 18 exclusive soft deepen — this unit):
 ...
 * Wave 18 exclusive (this unit only — greppable "scsi_mid: soft …"):
...
/*
 * Soft product inventory (Wave 13 base + Wave 18 exclusive deepen).
...
/* Wave 18 exclusive soft deepen — complementary path tallies. */
...
/* ---- Soft inventory (Wave 13 base + Wave 18 exclusive deepen) ----------- */
...
/**
 * Greppable soft scsi_mid inventory (Wave 13 base; Wave 18 exclusive deepen).
...
soft_inventory_log(const char *szVia)
{
    ...
    kprintf("scsi_mid: soft inventory via=%s wave=18 enter=%u ok=%u fail=%u "
    ...
    /*
     * Wave 18 exclusive deepen (complementary; never reshapes primary lines).
     */
    /* Grep: scsi_mid: soft total */
    kprintf("scsi_mid: soft total ok=%u fail=%u enter=%u ok_bp=%u "
            "soft_ok=%u soft_fail=%u logs=%u wave=18\n",
    ...
    kprintf("scsi_mid: soft honesty soft_lun=soft soft_ne_product=1 "
            ...
            "inquiry_soft_fallback=1 wave=18\n");

    /* Grep: scsi_mid: soft capacity — Wave 18 design-constant lamps. */
    kprintf("scsi_mid: soft capacity soft_lun=1 virtio_prefer=1 "
            "inquiry_fallback=1 sticky_prefer=1 sec=512 wave=18\n");

    /* Grep: scsi_mid: soft headroom — Wave 18 live path lamps. */
    kprintf("scsi_mid: soft headroom mid_ready=%u soft_lun=%u virtio=%u "
            "enter=%u ok=%u fail=%u logs=%u wave=18\n",
            u32Ready, u32SoftAct, u32Virtio, g_u32SoftEnter, g_u32SoftOk,
            g_u32SoftFail, g_u32SoftInvSamples);

    /* Grep: scsi_mid: soft surface — Wave 18 surface bit lamps. */
    kprintf("scsi_mid: soft surface ready=%u soft=%u virtio=%u prefer=%u "
            "enter=%u ok=%u surf=0x%x wave=18\n",
    ...
    /* Grep: scsi_mid: soft ratio — Wave 18 ok/fail basis points. */
    {
    ...
    /* Grep: scsi_mid: soft return — Wave 18 API return surfaces */
    kprintf("scsi_mid: soft return enter=%u ok=%u fail=%u via_virtio=%u "
            "via_soft=%u via_fallback=%u deny_null=%u deny_cdb=%u "
            "no_path=%u mid_ready=%u product_scsi_mid=OPEN wave=18\n",
            g_u32SoftEnter, g_u32SoftOk, g_u32SoftFail,
            g_u32SoftViaVirtio, g_u32SoftViaSoft, g_u32SoftViaFallback,
            g_u32SoftDenyNull, g_u32SoftDenyCdbLen, g_u32SoftDenyNoPath,
            u32Ready);

    /* Grep: scsi_mid: soft retmap — Wave 18 return-surface map */
    kprintf("scsi_mid: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=18\n");

    /* Grep: scsi_mid: soft deepen */
    kprintf("scsi_mid: soft deepen wave=18 areas=total,soft,sense,xfer,"
            "peak,last,honesty,capacity,headroom,surface,ratio,return logs=%u "
            "(Wave 18 exclusive; soft LUN honesty remains soft; not bar3)\n",
            g_u32SoftInvSamples);
    ...
    if (fSoftPass != 0) {
        kprintf("scsi_mid: soft inventory PASS via=%s logs=%u "
                ...
                u32Virtio, u32Prefer);
        kprintf("scsi_mid: soft PASS via=%s wave=18\n", szViaSafe);
    } else {
        kprintf("scsi_mid: soft FAIL via=%s mid_ready=0 wave=18 "
...
scsi_mid_init(void)
{
    ...
    g_fSoftOnce = 0;
    /* Wave 18 exclusive soft deepen tallies (reset on re-init). */
