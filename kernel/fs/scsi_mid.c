...
soft_inventory_log(const char *szVia)
{
    ...
    kprintf("scsi_mid: soft inventory via=%s wave=72 enter=%u ok=%u fail=%u "
    ...
    kprintf("scsi_mid: soft total ok=%u fail=%u enter=%u ok_bp=%u "
            "soft_ok=%u soft_fail=%u logs=%u wave=72\n",
    ...
    kprintf("scsi_mid: soft surface ready=%u soft=%u virtio=%u prefer=%u "
            "enter=%u ok=%u surf=0x%x wave=72\n",
    ...
    kprintf("scsi_mid: soft return enter=%u ok=%u fail=%u via_virtio=%u "
            ...
            "no_path=%u mid_ready=%u product_scsi_mid=OPEN wave=72\n",
    ...
    kprintf("scsi_mid: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=72\n");
...
kprintf("scsi_mid: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=72 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("scsi_mid: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=72 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("scsi_mid: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=72 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("scsi_mid: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=72 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("scsi_mid: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=72 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("scsi_mid: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=72 (retdoubletenailleangle stamp; Soft≠product)\n");

                            kprintf("scsi_mid: soft deepen wave=72 areas=total,soft,sense,xfer,,retclass,retlane"
    ...
    if (fSoftPass != 0) {
        kprintf("scsi_mid: soft inventory PASS via=%s logs=%u "
                ...
                u32Virtio, u32Prefer);
        kprintf("scsi_mid: soft PASS via=%s wave=72\n", szViaSafe);
    } else {
        kprintf("scsi_mid: soft FAIL via=%s mid_ready=0 wave=72 "
