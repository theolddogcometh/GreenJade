 ...
 *
 * Soft inventory (Wave 13 base + Wave 19 exclusive soft deepen — this unit):
...
/*
 * Soft product inventory (Wave 13 base + Wave 19 exclusive deepen).
...
/* Wave 19 exclusive soft deepen — complementary path tallies. */
...
/* ---- Soft inventory (Wave 13 base + Wave 19 exclusive deepen) ----------- */
...
/**
 * Greppable soft scsi_mid inventory (Wave 13 base; Wave 19 exclusive deepen).
...
soft_inventory_log(const char *szVia)
{
    ...
    kprintf("scsi_mid: soft inventory via=%s wave=19 enter=%u ok=%u fail=%u "
    ...
    /*
     * Wave 19 exclusive deepen (complementary; never reshapes primary lines).
     */
    /* Grep: scsi_mid: soft total */
    kprintf("scsi_mid: soft total ok=%u fail=%u enter=%u ok_bp=%u "
            "soft_ok=%u soft_fail=%u logs=%u wave=19\n",
    ...
    kprintf("scsi_mid: soft surface ready=%u soft=%u virtio=%u prefer=%u "
            "enter=%u ok=%u surf=0x%x wave=19\n",
    ...
    kprintf("scsi_mid: soft return enter=%u ok=%u fail=%u via_virtio=%u "
            ...
            "no_path=%u mid_ready=%u product_scsi_mid=OPEN wave=19\n",
    ...
    kprintf("scsi_mid: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=19\n");
     ...
     */
    /* Grep: scsi_mid: soft retclass — Wave 19 return-class taxonomy */
    kprintf("scsi_mid: soft retclass ok|fail|inval|nodev|busy|nomem "
            "soft_only=1 product_gate=0 wave=%u "
            "(retclass taxonomy; Soft≠product; not bar3)\n",
            (unsigned)GJ_SCSI_SOFT_SECTORS);
    /* Grep: scsi_mid: soft retlane — Wave 19 return-lane catalog */
    kprintf("scsi_mid: soft retlane inv|selftest|rate|retcode|retmap|class "
            "product_kernel=OPEN soft_ne_product=1 wave=%u "
            "(retlane catalog; Soft≠product)\n",
            (unsigned)GJ_SCSI_SOFT_SECTORS);
    kprintf("scsi_mid: soft deepen wave=19 areas=total,soft,sense,xfer,,retclass,retlane"
            "peak,last,honesty,capacity,headroom,surface,ratio,return logs=%u "
            "(Wave 19 exclusive; soft LUN honesty remains soft; not bar3)\n",
            g_u32SoftInvSamples);
    ...
    if (fSoftPass != 0) {
        kprintf("scsi_mid: soft inventory PASS via=%s logs=%u "
                ...
                u32Virtio, u32Prefer);
        kprintf("scsi_mid: soft PASS via=%s wave=19\n", szViaSafe);
    } else {
        kprintf("scsi_mid: soft FAIL via=%s mid_ready=0 wave=19 "
...
scsi_mid_init(void)
{
    ...
    g_fSoftOnce = 0;
    /* Wave 19 exclusive soft deepen tallies (reset on re-init). */
