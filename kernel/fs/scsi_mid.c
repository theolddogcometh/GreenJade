...
soft_inventory_log(const char *szVia)
{
    ...
    kprintf("scsi_mid: soft inventory via=%s wave=101 enter=%u ok=%u fail=%u "
    ...
    kprintf("scsi_mid: soft total ok=%u fail=%u enter=%u ok_bp=%u "
            "soft_ok=%u soft_fail=%u logs=%u wave=101\n",
    ...
    kprintf("scsi_mid: soft surface ready=%u soft=%u virtio=%u prefer=%u "
            "enter=%u ok=%u surf=0x%x wave=101\n",
    ...
    kprintf("scsi_mid: soft return enter=%u ok=%u fail=%u via_virtio=%u "
            ...
            "no_path=%u mid_ready=%u product_scsi_mid=OPEN wave=101\n",
    ...
    kprintf("scsi_mid: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=101\n");
...
kprintf("scsi_mid: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=101 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("scsi_mid: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=101 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("scsi_mid: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=101 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("scsi_mid: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=101 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("scsi_mid: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=101 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("scsi_mid: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=101 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("scsi_mid: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=101 (retcurtainface honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("scsi_mid: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=101 (retbastionangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retglacisangle — Wave 74 return-glacisangle honesty */
kprintf("scsi_mid: soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=101 (retglacisangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
kprintf("scsi_mid: soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=101 (retparapetangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retmoatangle — Wave 75 return-moatangle honesty */
kprintf("scsi_mid: soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=101 (retmoatangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retowerangle — Wave 75 exclusive towerangle stamp */
kprintf("scsi_mid: soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=101 (retowerangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retgateangle — Wave 76 return-gateangle honesty */
kprintf("scsi_mid: soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=101 (retgateangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retwallangle — Wave 76 exclusive wallangle stamp */
kprintf("scsi_mid: soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=101 (retwallangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retspireangle — Wave 77 return-spireangle honesty */
kprintf("scsi_mid: soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=101 (retspireangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retholdangle — Wave 77 exclusive holdangle stamp */
kprintf("scsi_mid: soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=101 (retholdangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retpalaceangle — Wave 78 return-palaceangle honesty */
kprintf("scsi_mid: soft retpalaceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=101 (retpalaceangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retfortressangle — Wave 78 exclusive fortressangle stamp */
kprintf("scsi_mid: soft retfortressangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=101 (retfortressangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retkeepangle — Wave 79 return-keepangle honesty */
kprintf("scsi_mid: soft retkeepangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=101 (retkeepangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retredoubtangle — Wave 79 exclusive redoubtangle stamp */
kprintf("scsi_mid: soft retredoubtangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=101 (retredoubtangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retcitadelangle — Wave 80 return-citadelangle honesty */
kprintf("scsi_mid: soft retcitadelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=101 (retcitadelangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retbastionkeep — Wave 80 exclusive bastionkeep stamp */
kprintf("scsi_mid: soft retbastionkeep exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=101 (retbastionkeep stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retpanoplyangle — Wave 81 return-panoplyangle honesty */
kprintf("scsi_mid: soft retpanoplyangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=101 (retpanoplyangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retbulwarkangle — Wave 81 exclusive bulwarkangle stamp */
kprintf("scsi_mid: soft retbulwarkangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=101 (retbulwarkangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retmantleangle — Wave 82 return-mantleangle honesty */
kprintf("scsi_mid: soft retmantleangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=101 (retmantleangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retaegisangle — Wave 82 exclusive aegisangle stamp */
kprintf("scsi_mid: soft retaegisangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=101 (retaegisangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retemblemangle — Wave 83 return-emblemangle honesty */
kprintf("scsi_mid: soft retemblemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=101 (retemblemangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retsigilangle — Wave 83 exclusive sigilangle stamp */
kprintf("scsi_mid: soft retsigilangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=101 (retsigilangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retscepterangle — Wave 84 return-scepterangle honesty */
kprintf("scsi_mid: soft retscepterangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=101 (retscepterangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retglyphangle — Wave 84 exclusive glyphangle stamp */
kprintf("scsi_mid: soft retglyphangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=101 (retglyphangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retcrownangle — Wave 85 return-crownangle honesty */
kprintf("scsi_mid: soft retcrownangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=101 (retcrownangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retshardangle — Wave 85 exclusive shardangle stamp */
kprintf("scsi_mid: soft retshardangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=101 (retshardangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retforgeangle — Wave 86 return-forgeangle honesty */
kprintf("scsi_mid: soft retforgeangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=101 (retforgeangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retprismangle — Wave 86 exclusive prismangle stamp */
kprintf("scsi_mid: soft retprismangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=101 (retprismangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retflameangle — Wave 87 return-flameangle honesty */
kprintf("scsi_mid: soft retflameangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=101 (retflameangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retcipherangle — Wave 87 exclusive cipherangle stamp */
kprintf("scsi_mid: soft retcipherangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=101 (retcipherangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retbeaconangle — Wave 88 return-beaconangle honesty */
kprintf("scsi_mid: soft retbeaconangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=101 (retbeaconangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retledgerangle — Wave 88 exclusive ledgerangle stamp */
kprintf("scsi_mid: soft retledgerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=101 (retledgerangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retbannerangle — Wave 89 return-bannerangle honesty */
kprintf("scsi_mid: soft retbannerangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=101 (retbannerangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retvaultangle — Wave 89 exclusive vaultangle stamp */
kprintf("scsi_mid: soft retvaultangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=101 (retvaultangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retcrestangle — Wave 90 return-crestangle honesty */
kprintf("scsi_mid: soft retcrestangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=101 (retcrestangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft rettokenangle — Wave 90 exclusive tokenangle stamp */
kprintf("scsi_mid: soft rettokenangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=101 (rettokenangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retbadgeangle — Wave 91 return-badgeangle honesty */
kprintf("scsi_mid: soft retbadgeangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=101 (retbadgeangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retphaseangle — Wave 91 exclusive phaseangle stamp */
kprintf("scsi_mid: soft retphaseangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=101 (retphaseangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retmarkangle — Wave 92 return-markangle honesty */
kprintf("scsi_mid: soft retmarkangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=101 (retmarkangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retpulseangle — Wave 92 exclusive pulseangle stamp */
kprintf("scsi_mid: soft retpulseangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=101 (retpulseangle stamp; Soft≠product)\n");

/* Grep: scsi_mid: soft retsealangle — Wave 93 return-sealangle honesty */
kprintf("scsi_mid: soft retsealangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=101 (retsealangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retboundangle — Wave 93 exclusive boundangle stamp */
kprintf("scsi_mid: soft retboundangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=101 (retboundangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retstemangle — Wave 94 return-stemangle honesty */
kprintf("scsi_mid: soft retstemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=101 (retstemangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retbladeangle — Wave 94 exclusive bladeangle stamp */
kprintf("scsi_mid: soft retbladeangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=101 (retbladeangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retchordangle — Wave 95 return-chordangle honesty */
kprintf("scsi_mid: soft retchordangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=101 (retchordangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retarcangle — Wave 95 exclusive arcangle stamp */
kprintf("scsi_mid: soft retarcangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=101 (retarcangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retsectorangle — Wave 96 return-sectorangle honesty */
kprintf("scsi_mid: soft retsectorangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=101 (retsectorangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retwedgeangle — Wave 96 exclusive wedgeangle stamp */
kprintf("scsi_mid: soft retwedgeangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=101 (retwedgeangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retradiusangle — Wave 97 return-radiusangle honesty */
kprintf("scsi_mid: soft retradiusangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=101 (retradiusangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retdiameterangle — Wave 97 exclusive diameterangle stamp */
kprintf("scsi_mid: soft retdiameterangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=101 (retdiameterangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retcircumangle — Wave 98 return-circumangle honesty */
kprintf("scsi_mid: soft retcircumangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=101 (retcircumangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retellipseangle — Wave 98 exclusive ellipseangle stamp */
kprintf("scsi_mid: soft retellipseangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=101 (retellipseangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft rethyperangle — Wave 99 return-hyperangle honesty */
kprintf("scsi_mid: soft rethyperangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=101 (rethyperangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retparabolaangle — Wave 99 exclusive parabolaangle stamp */
kprintf("scsi_mid: soft retparabolaangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=101 (retparabolaangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retspiralangle — Wave 100 return-spiralangle honesty */
kprintf("scsi_mid: soft retspiralangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=101 (retspiralangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft rethelixangle — Wave 100 exclusive helixangle stamp */
kprintf("scsi_mid: soft rethelixangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=101 (rethelixangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft rettorusangle — Wave 101 return-torusangle honesty */
kprintf("scsi_mid: soft rettorusangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=101 (rettorusangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retknotangle — Wave 101 exclusive knotangle stamp */
kprintf("scsi_mid: soft retknotangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=101 (retknotangle stamp; Soft≠product)\n");
                            kprintf("scsi_mid: soft deepen wave=101 areas=total,soft,sense,xfer,,retclass,retlane"
    ...
    if (fSoftPass != 0) {
        kprintf("scsi_mid: soft inventory PASS via=%s logs=%u "
                ...
                u32Virtio, u32Prefer);
        kprintf("scsi_mid: soft PASS via=%s wave=101\n", szViaSafe);
    } else {
        kprintf("scsi_mid: soft FAIL via=%s mid_ready=0 wave=101 "
