 ...
 *
 * Soft inventory (Wave 13 base + Wave 23 exclusive soft deepen — this unit):
...
/*
 * Soft product inventory (Wave 13 base + Wave 23 exclusive deepen).
...
/* Wave 23 exclusive soft deepen — complementary path tallies. */
...
/* ---- Soft inventory (Wave 13 base + Wave 23 exclusive deepen) ----------- */
...
/**
 * Greppable soft scsi_mid inventory (Wave 13 base; Wave 23 exclusive deepen).
...
soft_inventory_log(const char *szVia)
{
    ...
    kprintf("scsi_mid: soft inventory via=%s wave=23 enter=%u ok=%u fail=%u "
    ...
    /*
     * Wave 23 exclusive deepen (complementary; never reshapes primary lines).
     */
    /* Grep: scsi_mid: soft total */
    kprintf("scsi_mid: soft total ok=%u fail=%u enter=%u ok_bp=%u "
            "soft_ok=%u soft_fail=%u logs=%u wave=23\n",
    ...
    kprintf("scsi_mid: soft surface ready=%u soft=%u virtio=%u prefer=%u "
            "enter=%u ok=%u surf=0x%x wave=23\n",
    ...
    kprintf("scsi_mid: soft return enter=%u ok=%u fail=%u via_virtio=%u "
            ...
            "no_path=%u mid_ready=%u product_scsi_mid=OPEN wave=23\n",
    ...
    kprintf("scsi_mid: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=23\n");
 ...
 * Soft≠product; not bar3.
            */
            /* Grep: scsi_mid: soft rettoken — Wave 23 return-token honesty */
            kprintf("scsi_mid: soft rettoken soft_only=1 product_gate=0 soft_ne_product=1 "
                    "never_blocks_m0=1 wave=%u "
                    "(rettoken honesty; Soft≠product; not bar3)\n",
                    (unsigned)GJ_SCSI_SOFT_SECTORS);
            /* Grep: scsi_mid: soft retcrest — Wave 23 exclusive crest stamp */
            kprintf("scsi_mid: soft retcrest exclusive=1 soft_ne_product=1 "
                    "product_kernel=OPEN bar3=0 wave=%u "
                    "(retcrest stamp; Soft≠product)\n",
                    (unsigned)GJ_SCSI_SOFT_SECTORS);
    kprintf("scsi_mid: soft deepen wave=23 areas=total,soft,sense,xfer,,retclass,retlane"
            "peak,last,honesty,capacity,headroom,surface,ratio,return logs=%u "
            "(Wave 23 exclusive; soft LUN honesty remains soft; not bar3)\n",
            g_u32SoftInvSamples);
    ...
    if (fSoftPass != 0) {
        kprintf("scsi_mid: soft inventory PASS via=%s logs=%u "
                ...
                u32Virtio, u32Prefer);
        kprintf("scsi_mid: soft PASS via=%s wave=23\n", szViaSafe);
    } else {
        kprintf("scsi_mid: soft FAIL via=%s mid_ready=0 wave=23 "
...
scsi_mid_init(void)
{
    ...
    g_fSoftOnce = 0;
    /* Wave 23 exclusive soft deepen tallies (reset on re-init). */
