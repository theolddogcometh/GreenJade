...
soft_inventory_log(const char *szVia)
{
    ...
    kprintf("scsi_mid: soft inventory via=%s wave=92 enter=%u ok=%u fail=%u "
    ...
    kprintf("scsi_mid: soft total ok=%u fail=%u enter=%u ok_bp=%u "
            "soft_ok=%u soft_fail=%u logs=%u wave=92\n",
    ...
    kprintf("scsi_mid: soft surface ready=%u soft=%u virtio=%u prefer=%u "
            "enter=%u ok=%u surf=0x%x wave=92\n",
    ...
    kprintf("scsi_mid: soft return enter=%u ok=%u fail=%u via_virtio=%u "
            ...
            "no_path=%u mid_ready=%u product_scsi_mid=OPEN wave=92\n",
    ...
    kprintf("scsi_mid: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=92\n");
...
kprintf("scsi_mid: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=92 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("scsi_mid: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=92 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("scsi_mid: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=92 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("scsi_mid: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=92 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("scsi_mid: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=92 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("scsi_mid: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=92 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("scsi_mid: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=92 (retcurtainface honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("scsi_mid: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=92 (retbastionangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retglacisangle — Wave 74 return-glacisangle honesty */
kprintf("scsi_mid: soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=92 (retglacisangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
kprintf("scsi_mid: soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=92 (retparapetangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retmoatangle — Wave 75 return-moatangle honesty */
kprintf("scsi_mid: soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=92 (retmoatangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retowerangle — Wave 75 exclusive towerangle stamp */
kprintf("scsi_mid: soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=92 (retowerangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retgateangle — Wave 76 return-gateangle honesty */
kprintf("scsi_mid: soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=92 (retgateangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retwallangle — Wave 76 exclusive wallangle stamp */
kprintf("scsi_mid: soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=92 (retwallangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retspireangle — Wave 77 return-spireangle honesty */
kprintf("scsi_mid: soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=92 (retspireangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retholdangle — Wave 77 exclusive holdangle stamp */
kprintf("scsi_mid: soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=92 (retholdangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retpalaceangle — Wave 78 return-palaceangle honesty */
kprintf("scsi_mid: soft retpalaceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=92 (retpalaceangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retfortressangle — Wave 78 exclusive fortressangle stamp */
kprintf("scsi_mid: soft retfortressangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=92 (retfortressangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retkeepangle — Wave 79 return-keepangle honesty */
kprintf("scsi_mid: soft retkeepangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=92 (retkeepangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retredoubtangle — Wave 79 exclusive redoubtangle stamp */
kprintf("scsi_mid: soft retredoubtangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=92 (retredoubtangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retcitadelangle — Wave 80 return-citadelangle honesty */
kprintf("scsi_mid: soft retcitadelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=92 (retcitadelangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retbastionkeep — Wave 80 exclusive bastionkeep stamp */
kprintf("scsi_mid: soft retbastionkeep exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=92 (retbastionkeep stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retpanoplyangle — Wave 81 return-panoplyangle honesty */
kprintf("scsi_mid: soft retpanoplyangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=92 (retpanoplyangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retbulwarkangle — Wave 81 exclusive bulwarkangle stamp */
kprintf("scsi_mid: soft retbulwarkangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=92 (retbulwarkangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retmantleangle — Wave 82 return-mantleangle honesty */
kprintf("scsi_mid: soft retmantleangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=92 (retmantleangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retaegisangle — Wave 82 exclusive aegisangle stamp */
kprintf("scsi_mid: soft retaegisangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=92 (retaegisangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retemblemangle — Wave 83 return-emblemangle honesty */
kprintf("scsi_mid: soft retemblemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=92 (retemblemangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retsigilangle — Wave 83 exclusive sigilangle stamp */
kprintf("scsi_mid: soft retsigilangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=92 (retsigilangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retscepterangle — Wave 84 return-scepterangle honesty */
kprintf("scsi_mid: soft retscepterangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=92 (retscepterangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retglyphangle — Wave 84 exclusive glyphangle stamp */
kprintf("scsi_mid: soft retglyphangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=92 (retglyphangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retcrownangle — Wave 85 return-crownangle honesty */
kprintf("scsi_mid: soft retcrownangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=92 (retcrownangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retshardangle — Wave 85 exclusive shardangle stamp */
kprintf("scsi_mid: soft retshardangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=92 (retshardangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retforgeangle — Wave 86 return-forgeangle honesty */
kprintf("scsi_mid: soft retforgeangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=92 (retforgeangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retprismangle — Wave 86 exclusive prismangle stamp */
kprintf("scsi_mid: soft retprismangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=92 (retprismangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retflameangle — Wave 87 return-flameangle honesty */
kprintf("scsi_mid: soft retflameangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=92 (retflameangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retcipherangle — Wave 87 exclusive cipherangle stamp */
kprintf("scsi_mid: soft retcipherangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=92 (retcipherangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retbeaconangle — Wave 88 return-beaconangle honesty */
kprintf("scsi_mid: soft retbeaconangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=92 (retbeaconangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retledgerangle — Wave 88 exclusive ledgerangle stamp */
kprintf("scsi_mid: soft retledgerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=92 (retledgerangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retbannerangle — Wave 89 return-bannerangle honesty */
kprintf("scsi_mid: soft retbannerangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=92 (retbannerangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retvaultangle — Wave 89 exclusive vaultangle stamp */
kprintf("scsi_mid: soft retvaultangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=92 (retvaultangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retcrestangle — Wave 90 return-crestangle honesty */
kprintf("scsi_mid: soft retcrestangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=92 (retcrestangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft rettokenangle — Wave 90 exclusive tokenangle stamp */
kprintf("scsi_mid: soft rettokenangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=92 (rettokenangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retbadgeangle — Wave 91 return-badgeangle honesty */
kprintf("scsi_mid: soft retbadgeangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=92 (retbadgeangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retphaseangle — Wave 91 exclusive phaseangle stamp */
kprintf("scsi_mid: soft retphaseangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=92 (retphaseangle stamp; Soft≠product)\n");
/* Grep: scsi_mid: soft retmarkangle — Wave 92 return-markangle honesty */
kprintf("scsi_mid: soft retmarkangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=92 (retmarkangle honesty; Soft≠product; not bar3)\n");
/* Grep: scsi_mid: soft retpulseangle — Wave 92 exclusive pulseangle stamp */
kprintf("scsi_mid: soft retpulseangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=92 (retpulseangle stamp; Soft≠product)\n");

                            kprintf("scsi_mid: soft deepen wave=92 areas=total,soft,sense,xfer,,retclass,retlane"
    ...
    if (fSoftPass != 0) {
        kprintf("scsi_mid: soft inventory PASS via=%s logs=%u "
                ...
                u32Virtio, u32Prefer);
        kprintf("scsi_mid: soft PASS via=%s wave=92\n", szViaSafe);
    } else {
        kprintf("scsi_mid: soft FAIL via=%s mid_ready=0 wave=92 "
