 ...
 *   - Wave 19 exclusive: capacity|geom|terminal|return|retmap (wave=73 areas=51)
 ...
 *        ring_calls=… owned=… map_va=0x… token=… wave=73
...
store_soft_inventory_log(void)
{
    ...
    kprintf("store_door: soft catalog xfer_max=%u sector=%u "
            ...
            "export,map,kick,ring_state wave=73\n",
    ...
    kprintf("store_door: soft path claim=storaged rw=blk|scsi "
            ...
            "wave=73 (soft inventory; not bar3)\n");
     ...
     */
    {
        ...
        kprintf("store_door: soft terminal ok=%u fail_like=%u rw=%u "
                "claims=%u blk=%u scsi=%u soft PASS wave=73\n",
        ...
        kprintf("store_door: soft return ok=%u inval=%u nodev=%u busy=%u "
                ...
                "rw=%u product_storaged=OPEN wave=73\n",
    ...
    kprintf("store_door: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=73\n");
...
kprintf("store_door: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=73 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: store_door: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("store_door: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=73 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("store_door: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=73 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: store_door: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("store_door: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=73 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("store_door: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=73 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: store_door: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("store_door: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=73 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: store_door: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("store_door: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=73 (retcurtainface honesty; Soft≠product; not bar3)\n");
/* Grep: store_door: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("store_door: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=73 (retbastionangle stamp; Soft≠product)\n");

                                kprintf("store_door: soft deepen wave=73 areas=101 calls=%u rw=%u "
    ...
    kprintf("store_door: soft PASS calls=%u wave=73\n", g_u32Calls);
}
...
store_door_init(void)
{
    ...
    /* Grep: store_door: soft (baseline inventory after init; wave=73) */
    store_soft_inventory_log();
}
