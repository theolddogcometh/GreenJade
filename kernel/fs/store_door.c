 ...
 *   - Wave 19 exclusive: capacity|geom|terminal|return|retmap (wave=26 areas=19)
 ...
 *        ring_calls=… owned=… map_va=0x… token=… wave=26
...
store_soft_inventory_log(void)
{
    ...
    kprintf("store_door: soft catalog xfer_max=%u sector=%u "
            ...
            "export,map,kick,ring_state wave=26\n",
     ...
     */
    {
        ...
        kprintf("store_door: soft terminal ok=%u fail_like=%u rw=%u "
                "claims=%u blk=%u scsi=%u soft PASS wave=26\n",
        ...
        kprintf("store_door: soft return ok=%u inval=%u nodev=%u busy=%u "
                ...
                "rw=%u product_storaged=OPEN wave=26\n",
    ...
    kprintf("store_door: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=26\n");
 ...
 * Soft≠product; not bar3.
                */
                 ...
                 * Soft≠product; not bar3.
                 */
                /* Grep: store_door: soft retcipher — Wave 26 return-cipher honesty */
                kprintf("store_door: soft retcipher soft_only=1 product_gate=0 soft_ne_product=1 "
                        "never_blocks_m0=1 wave=26 "
                        "(retcipher honesty; Soft≠product; not bar3)\n");
                /* Grep: store_door: soft retflame — Wave 26 exclusive flame stamp */
                kprintf("store_door: soft retflame exclusive=1 soft_ne_product=1 "
                        "product_kernel=OPEN bar3=0 wave=26 "
                        "(retflame stamp; Soft≠product)\n");
        kprintf("store_door: soft deepen wave=26 areas=21 calls=%u rw=%u "
    ...
    kprintf("store_door: soft PASS calls=%u wave=26\n", g_u32Calls);
}
...
store_door_init(void)
{
    ...
    /* Grep: store_door: soft (baseline inventory after init; wave=26) */
    store_soft_inventory_log();
}
