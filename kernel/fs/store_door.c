 ...
 *   - Wave 19 exclusive: capacity|geom|terminal|return|retmap (wave=30 areas=23)
 ...
 *        ring_calls=… owned=… map_va=0x… token=… wave=30
...
store_soft_inventory_log(void)
{
    ...
    kprintf("store_door: soft catalog xfer_max=%u sector=%u "
            ...
            "export,map,kick,ring_state wave=30\n",
     ...
     */
    {
        ...
        kprintf("store_door: soft terminal ok=%u fail_like=%u rw=%u "
                "claims=%u blk=%u scsi=%u soft PASS wave=30\n",
        ...
        kprintf("store_door: soft return ok=%u inval=%u nodev=%u busy=%u "
                ...
                "rw=%u product_storaged=OPEN wave=30\n",
    ...
    kprintf("store_door: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=30\n");
 ...
 * Soft≠product; not bar3.
                */
                ...
                kprintf("store_door: soft retflame exclusive=1 soft_ne_product=1 "
                        ...
                        kprintf("store_door: soft retforge exclusive=1 soft_ne_product=1 "
                                 ...
                                 * Soft≠product; not bar3.
                                 */
                                /* Grep: store_door: soft retsigil — Wave 30 return-sigil honesty */
                                kprintf("store_door: soft retsigil soft_only=1 product_gate=0 soft_ne_product=1 "
                                        "never_blocks_m0=1 wave=30 "
                                        "(retsigil honesty; Soft≠product; not bar3)\n");
                                /* Grep: store_door: soft retemblem — Wave 30 exclusive emblem stamp */
                                kprintf("store_door: soft retemblem exclusive=1 soft_ne_product=1 "
                                        "product_kernel=OPEN bar3=0 wave=30 "
                                        "(retemblem stamp; Soft≠product)\n");
                                kprintf("store_door: soft deepen wave=30 areas=25 calls=%u rw=%u "
    ...
    kprintf("store_door: soft PASS calls=%u wave=30\n", g_u32Calls);
}
...
store_door_init(void)
{
    ...
    /* Grep: store_door: soft (baseline inventory after init; wave=30) */
    store_soft_inventory_log();
}
