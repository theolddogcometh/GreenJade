 ...
 *   - Wave 19 exclusive: capacity|geom|terminal|return|retmap (wave=33 areas=25)
 ...
 *        ring_calls=… owned=… map_va=0x… token=… wave=33
...
store_soft_inventory_log(void)
{
    ...
    kprintf("store_door: soft catalog xfer_max=%u sector=%u "
            ...
            "export,map,kick,ring_state wave=33\n",
     ...
     */
    {
        ...
        kprintf("store_door: soft terminal ok=%u fail_like=%u rw=%u "
                "claims=%u blk=%u scsi=%u soft PASS wave=33\n",
        ...
        kprintf("store_door: soft return ok=%u inval=%u nodev=%u busy=%u "
                ...
                "rw=%u product_storaged=OPEN wave=33\n",
    ...
    kprintf("store_door: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=33\n");
 ...
 * Soft≠product; not bar3.
 */
/* Grep: store_door: soft retbastion — Wave 33 return-bastion honesty */
kprintf("store_door: soft retbastion soft_only=1 product_gate=0 soft_ne_product=1 "
        "never_blocks_m0=1 wave=33 "
        "(retbastion honesty; Soft≠product; not bar3)\n");
/* Grep: store_door: soft retcitadel — Wave 33 exclusive citadel stamp */
kprintf("store_door: soft retcitadel exclusive=1 soft_ne_product=1 "
        "product_kernel=OPEN bar3=0 wave=33 "
        "(retcitadel stamp; Soft≠product)\n");
                                kprintf("store_door: soft deepen wave=33 areas=31 calls=%u rw=%u "
    ...
    kprintf("store_door: soft PASS calls=%u wave=33\n", g_u32Calls);
}
...
store_door_init(void)
{
    ...
    /* Grep: store_door: soft (baseline inventory after init; wave=33) */
    store_soft_inventory_log();
}
