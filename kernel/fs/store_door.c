 ...
 *   - Wave 19 exclusive: capacity|geom|terminal|return|retmap (wave=39 areas=25)
 ...
 *        ring_calls=… owned=… map_va=0x… token=… wave=39
...
store_soft_inventory_log(void)
{
    ...
    kprintf("store_door: soft catalog xfer_max=%u sector=%u "
            ...
            "export,map,kick,ring_state wave=39\n",
     ...
     */
    {
        ...
        kprintf("store_door: soft terminal ok=%u fail_like=%u rw=%u "
                "claims=%u blk=%u scsi=%u soft PASS wave=39\n",
        ...
        kprintf("store_door: soft return ok=%u inval=%u nodev=%u busy=%u "
                ...
                "rw=%u product_storaged=OPEN wave=39\n",
    ...
    kprintf("store_door: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=39\n");
...
kprintf("store_door: soft retglacis exclusive=1 soft_ne_product=1 "
        ...
        "(retglacis stamp; Soft≠product)\n");
                                kprintf("store_door: soft deepen wave=39 areas=37 calls=%u rw=%u "
    ...
    kprintf("store_door: soft PASS calls=%u wave=39\n", g_u32Calls);
}
...
store_door_init(void)
{
    ...
    /* Grep: store_door: soft (baseline inventory after init; wave=39) */
    store_soft_inventory_log();
}
