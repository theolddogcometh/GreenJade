 ...
 *   - Wave 19 exclusive: capacity|geom|terminal|return|retmap (wave=77 areas=53)
 ...
 *        ring_calls=… owned=… map_va=0x… token=… wave=77
...
store_soft_inventory_log(void)
{
    ...
    kprintf("store_door: soft catalog xfer_max=%u sector=%u "
            ...
            "export,map,kick,ring_state wave=77\n",
    ...
    kprintf("store_door: soft path claim=storaged rw=blk|scsi "
            ...
            "wave=77 (soft inventory; not bar3)\n");
     ...
     */
    {
        ...
        kprintf("store_door: soft terminal ok=%u fail_like=%u rw=%u "
                "claims=%u blk=%u scsi=%u soft PASS wave=77\n",
        ...
        kprintf("store_door: soft return ok=%u inval=%u nodev=%u busy=%u "
                ...
                "rw=%u product_storaged=OPEN wave=77\n",
    ...
    kprintf("store_door: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=77\n");
...
kprintf("store_door: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=77 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: store_door: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("store_door: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=77 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("store_door: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=77 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: store_door: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("store_door: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=77 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("store_door: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=77 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: store_door: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("store_door: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=77 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: store_door: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("store_door: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=77 (retcurtainface honesty; Soft≠product; not bar3)\n");
/* Grep: store_door: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("store_door: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=77 (retbastionangle stamp; Soft≠product)\n");
/* Grep: store_door: soft retglacisangle — Wave 74 return-glacisangle honesty */
kprintf("store_door: soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=77 (retglacisangle honesty; Soft≠product; not bar3)\n");
/* Grep: store_door: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
kprintf("store_door: soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=77 (retparapetangle stamp; Soft≠product)\n");
/* Grep: store_door: soft retmoatangle — Wave 75 return-moatangle honesty */
kprintf("store_door: soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=77 (retmoatangle honesty; Soft≠product; not bar3)\n");
/* Grep: store_door: soft retowerangle — Wave 75 exclusive towerangle stamp */
kprintf("store_door: soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=77 (retowerangle stamp; Soft≠product)\n");
/* Grep: store_door: soft retgateangle — Wave 76 return-gateangle honesty */
kprintf("store_door: soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=77 (retgateangle honesty; Soft≠product; not bar3)\n");
/* Grep: store_door: soft retwallangle — Wave 76 exclusive wallangle stamp */
kprintf("store_door: soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=77 (retwallangle stamp; Soft≠product)\n");
/* Grep: store_door: soft retspireangle — Wave 77 return-spireangle honesty */
kprintf("store_door: soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=77 (retspireangle honesty; Soft≠product; not bar3)\n");
/* Grep: store_door: soft retholdangle — Wave 77 exclusive holdangle stamp */
kprintf("store_door: soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=77 (retholdangle stamp; Soft≠product)\n");

                                kprintf("store_door: soft deepen wave=77 areas=103 calls=%u rw=%u "
    ...
    kprintf("store_door: soft PASS calls=%u wave=77\n", g_u32Calls);
}
...
store_door_init(void)
{
    ...
    /* Grep: store_door: soft (baseline inventory after init; wave=77) */
    store_soft_inventory_log();
}
