 ...
 *   - Wave 19 exclusive: capacity|geom|terminal|return|retmap (wave=43 areas=29)
...
soft_inventory_log(void)
{
    ...
    kprintf("vfs_door: soft total ok=%u fail=%u calls=%u logs=%u "
            "unknown_op=%u not_init=%u wave=43\n",
    ...
    kprintf("vfs_door: soft catalog ver=%u inodes=%u fd_max=%u file_max=%u "
            "secs=%u data0=%u magic=0x%x sec=%u wave=43\n",
    ...
    kprintf("vfs_door: soft path claim=vfsd format_mount=free_or_owned "
            ...
            "product_vfsd=1 wave=43 (soft inventory; not bar3)\n",
     ...
     */
    {
        ...
        kprintf("vfs_door: soft capacity inodes=%u fd_max=%u file_max=%u "
                "secs=%u sec=%u multi_client=1 ramdisk_soft=1 wave=43\n",
        ...
        kprintf("vfs_door: soft geom ver=%u magic=0x%x lba_super=%u "
                "lba_freemap=%u lba_inodes=%u data0=%u wave=43\n",
        ...
        kprintf("vfs_door: soft return ok=%u fail=%u owned=%u mounted=%u "
                ...
                "ram=%u blk=%u product_vfsd=OPEN wave=43\n",
    ...
    kprintf("vfs_door: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=43\n");
...
kprintf("vfs_door: soft retarrowslit exclusive=1 soft_ne_product=1 "
        ...
        "(retarrowslit stamp; Soft≠product)\n");
                                ...
                                kprintf("vfs_door: soft deepen wave=43 areas=43 ver=%u calls=%u "
                "ok=%u fail=%u logs=%u\n",
    ...
    if (fSoftPass != 0) {
        kprintf("vfs_door: soft inventory PASS ver=%u logs=%u "
                "mounted=%u ram=%u blk=%u wave=43\n",
                VFS_VERSION, u32Samples, u32Mounted, u32Ram, u32Blk);
        kprintf("vfs_door: soft PASS ver=%u wave=43\n", VFS_VERSION);
    } else {
