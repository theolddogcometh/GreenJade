 ...
 *   - Wave 19 exclusive: capacity|geom|terminal|return|retmap (wave=73 areas=53)
...
soft_inventory_log(void)
{
    ...
    kprintf("vfs_door: soft total ok=%u fail=%u calls=%u logs=%u "
            "unknown_op=%u not_init=%u wave=73\n",
    ...
    kprintf("vfs_door: soft catalog ver=%u inodes=%u fd_max=%u file_max=%u "
            "secs=%u data0=%u magic=0x%x sec=%u wave=73\n",
    ...
    kprintf("vfs_door: soft path claim=vfsd format_mount=free_or_owned "
            ...
            "product_vfsd=1 wave=73 (soft inventory; not bar3)\n",
     ...
     */
    {
        ...
        kprintf("vfs_door: soft capacity inodes=%u fd_max=%u file_max=%u "
                "secs=%u sec=%u multi_client=1 ramdisk_soft=1 wave=73\n",
        ...
        kprintf("vfs_door: soft geom ver=%u magic=0x%x lba_super=%u "
                "lba_freemap=%u lba_inodes=%u data0=%u wave=73\n",
        ...
        kprintf("vfs_door: soft return ok=%u fail=%u owned=%u mounted=%u "
                ...
                "ram=%u blk=%u product_vfsd=OPEN wave=73\n",
    ...
    kprintf("vfs_door: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=73\n");
...
kprintf("vfs_door: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=73 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: vfs_door: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("vfs_door: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=73 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("vfs_door: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=73 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: vfs_door: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("vfs_door: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=73 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("vfs_door: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=73 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: vfs_door: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("vfs_door: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=73 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: vfs_door: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("vfs_door: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=73 (retcurtainface honesty; Soft≠product; not bar3)\n");
/* Grep: vfs_door: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("vfs_door: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=73 (retbastionangle stamp; Soft≠product)\n");

                                kprintf("vfs_door: soft deepen wave=73 areas=103 ver=%u calls=%u "
                "ok=%u fail=%u logs=%u\n",
    ...
    if (fSoftPass != 0) {
        kprintf("vfs_door: soft inventory PASS ver=%u logs=%u "
                "mounted=%u ram=%u blk=%u wave=73\n",
                VFS_VERSION, u32Samples, u32Mounted, u32Ram, u32Blk);
        kprintf("vfs_door: soft PASS ver=%u wave=73\n", VFS_VERSION);
    } else {
