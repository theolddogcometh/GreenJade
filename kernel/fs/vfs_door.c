 ...
 *   - Wave 19 exclusive: capacity|geom|terminal|return|retmap (wave=30 areas=25)
...
soft_inventory_log(void)
{
    ...
    kprintf("vfs_door: soft total ok=%u fail=%u calls=%u logs=%u "
            "unknown_op=%u not_init=%u wave=30\n",
    ...
    kprintf("vfs_door: soft catalog ver=%u inodes=%u fd_max=%u file_max=%u "
            "secs=%u data0=%u magic=0x%x sec=%u wave=30\n",
    ...
    kprintf("vfs_door: soft path claim=vfsd format_mount=free_or_owned "
            ...
            "product_vfsd=1 wave=30 (soft inventory; not bar3)\n",
     ...
     */
    {
        ...
        kprintf("vfs_door: soft capacity inodes=%u fd_max=%u file_max=%u "
                "secs=%u sec=%u multi_client=1 ramdisk_soft=1 wave=30\n",
        ...
        kprintf("vfs_door: soft geom ver=%u magic=0x%x lba_super=%u "
                "lba_freemap=%u lba_inodes=%u data0=%u wave=30\n",
        ...
        kprintf("vfs_door: soft return ok=%u fail=%u owned=%u mounted=%u "
                ...
                "ram=%u blk=%u product_vfsd=OPEN wave=30\n",
    ...
    kprintf("vfs_door: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=30\n");
 ...
 * Soft≠product; not bar3.
                */
                ...
                kprintf("vfs_door: soft retflame exclusive=1 soft_ne_product=1 "
                        ...
                        kprintf("vfs_door: soft retforge exclusive=1 soft_ne_product=1 "
                                 ...
                                 * Soft≠product; not bar3.
                                 */
                                /* Grep: vfs_door: soft retsigil — Wave 30 return-sigil honesty */
                                kprintf("vfs_door: soft retsigil soft_only=1 product_gate=0 soft_ne_product=1 "
                                        "never_blocks_m0=1 wave=30 "
                                        "(retsigil honesty; Soft≠product; not bar3)\n");
                                /* Grep: vfs_door: soft retemblem — Wave 30 exclusive emblem stamp */
                                kprintf("vfs_door: soft retemblem exclusive=1 soft_ne_product=1 "
                                        "product_kernel=OPEN bar3=0 wave=30 "
                                        "(retemblem stamp; Soft≠product)\n");
                                kprintf("vfs_door: soft deepen wave=30 areas=27 ver=%u calls=%u "
                "ok=%u fail=%u logs=%u\n",
    ...
    if (fSoftPass != 0) {
        kprintf("vfs_door: soft inventory PASS ver=%u logs=%u "
                "mounted=%u ram=%u blk=%u wave=30\n",
                VFS_VERSION, u32Samples, u32Mounted, u32Ram, u32Blk);
        kprintf("vfs_door: soft PASS ver=%u wave=30\n", VFS_VERSION);
    } else {
