 ...
 *
 * Soft door inventory (Wave 12 base; Wave 14 deepen; Wave 17 exclusive):
 ...
 *   - Wave 17 exclusive: capacity|geom|terminal|return (wave=17 areas=20)
...
soft_inventory_log(void)
{
    ...
    kprintf("vfs_door: soft total ok=%u fail=%u calls=%u logs=%u "
            "unknown_op=%u not_init=%u wave=17\n",
    ...
    kprintf("vfs_door: soft catalog ver=%u inodes=%u fd_max=%u file_max=%u "
            "secs=%u data0=%u magic=0x%x sec=%u wave=17\n",
    ...
    kprintf("vfs_door: soft path claim=vfsd format_mount=free_or_owned "
            ...
            "product_vfsd=1 wave=17 (soft inventory; not bar3)\n",
     ...
     */
    {
        ...
        kprintf("vfs_door: soft surface inventory,claim,layout,name,fd,fmt,"
                ...
                "ratio,headroom,capacity,geom,terminal,return,deepen "
                "areas=20 wave=17\n");
        /* Grep: vfs_door: soft capacity — Wave 17 design-constant lamps. */
        kprintf("vfs_door: soft capacity inodes=%u fd_max=%u file_max=%u "
                "secs=%u sec=%u multi_client=1 ramdisk_soft=1 wave=17\n",
    ...
    if (fSoftPass != 0) {
        kprintf("vfs_door: soft inventory PASS ver=%u logs=%u "
                "mounted=%u ram=%u blk=%u wave=17\n",
                VFS_VERSION, u32Samples, u32Mounted, u32Ram, u32Blk);
        kprintf("vfs_door: soft PASS ver=%u wave=17\n", VFS_VERSION);
    } else {
