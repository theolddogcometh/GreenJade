 ...
 *   - Wave 19 exclusive: capacity|geom|terminal|return|retmap (wave=78 areas=57)
...
soft_inventory_log(void)
{
    ...
    kprintf("vfs_ram: soft inventory seeded=%u files=%u fds=%u pipes=%u "
            ...
            "sym=%u blk=%u scsi=%u samples=%u wave=78\n",
    ...
    kprintf("vfs_ram: soft catalog eventfd_max=%u epoll_max=%u "
            ...
            "inotify_watch=%u pipe_buf=%u wave=78\n",
    ...
    kprintf("vfs_ram: soft peak_w16 files=%u fds=%u pipes=%u eventfd=%u "
            "epoll=%u timerfd=%u signalfd=%u inotify=%u sym=%u wave=78\n",
            g_u32SoftFilesPeak, g_u32SoftFdPeak, g_u32SoftPipePeak,
            g_u32SoftEventfdPeak, g_u32SoftEpollPeak, g_u32SoftTimerfdPeak,
            g_u32SoftSignalfdPeak, g_u32SoftInotifyPeak, g_u32SoftSymPeak);
    ...
    kprintf("vfs_ram: soft path cold_linux=1 ramfs+specials=1 "
            ...
            "notify=epoll|inotify|poll wave=78 "
     ...
     */
    {
        ...
        kprintf("vfs_ram: soft ratio occ_pct=%u fd_occ=%u ok_bp=%u "
                "fail_bp=%u files=%u fds=%u wave=78\n",
        ...
        kprintf("vfs_ram: soft headroom file_head=%u fd_head=%u "
                "max_files=%u max_fds=%u pipes=%u wave=78\n",
        ...
        kprintf("vfs_ram: soft capacity max_files=%u max_fds=%u max_data=%u "
                "max_pipes=%u pipe_buf=%u path=%u wave=78\n",
        ...
        kprintf("vfs_ram: soft return ok=%u fail=%u files=%u fds=%u "
                ...
                "product_vfs_ram=OPEN wave=78\n",
                g_u32SoftOk, g_u32SoftFail, u32Files, u32Fds, g_u32SoftSeeded,
                g_u32SoftOpenOk, g_u32SoftCloseOk, g_u32SoftReadOk,
                g_u32SoftWriteOk);

        /* Grep: vfs_ram: soft retmap — Wave 19 return-surface map */
    kprintf("vfs_ram: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=78\n");
...
kprintf("vfs_ram: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=78 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: vfs_ram: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("vfs_ram: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=78 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("vfs_ram: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=78 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: vfs_ram: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("vfs_ram: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=78 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("vfs_ram: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=78 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: vfs_ram: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("vfs_ram: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=78 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: vfs_ram: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("vfs_ram: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=78 (retcurtainface honesty; Soft≠product; not bar3)\n");
/* Grep: vfs_ram: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("vfs_ram: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=78 (retbastionangle stamp; Soft≠product)\n");
/* Grep: vfs_ram: soft retglacisangle — Wave 74 return-glacisangle honesty */
kprintf("vfs_ram: soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=78 (retglacisangle honesty; Soft≠product; not bar3)\n");
/* Grep: vfs_ram: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
kprintf("vfs_ram: soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=78 (retparapetangle stamp; Soft≠product)\n");
/* Grep: vfs_ram: soft retmoatangle — Wave 75 return-moatangle honesty */
kprintf("vfs_ram: soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=78 (retmoatangle honesty; Soft≠product; not bar3)\n");
/* Grep: vfs_ram: soft retowerangle — Wave 75 exclusive towerangle stamp */
kprintf("vfs_ram: soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=78 (retowerangle stamp; Soft≠product)\n");
/* Grep: vfs_ram: soft retgateangle — Wave 76 return-gateangle honesty */
kprintf("vfs_ram: soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=78 (retgateangle honesty; Soft≠product; not bar3)\n");
/* Grep: vfs_ram: soft retwallangle — Wave 76 exclusive wallangle stamp */
kprintf("vfs_ram: soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=78 (retwallangle stamp; Soft≠product)\n");
/* Grep: vfs_ram: soft retspireangle — Wave 77 return-spireangle honesty */
kprintf("vfs_ram: soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=78 (retspireangle honesty; Soft≠product; not bar3)\n");
/* Grep: vfs_ram: soft retholdangle — Wave 77 exclusive holdangle stamp */
kprintf("vfs_ram: soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=78 (retholdangle stamp; Soft≠product)\n");
/* Grep: vfs_ram: soft retpalaceangle — Wave 78 return-palaceangle honesty */
kprintf("vfs_ram: soft retpalaceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=78 (retpalaceangle honesty; Soft≠product; not bar3)\n");
/* Grep: vfs_ram: soft retfortressangle — Wave 78 exclusive fortressangle stamp */
kprintf("vfs_ram: soft retfortressangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=78 (retfortressangle stamp; Soft≠product)\n");

                                kprintf("vfs_ram: soft deepen wave=78 areas=107 seeded=%u files=%u "
    ...
    kprintf("vfs_ram: soft PASS seeded=%u wave=78\n", g_u32SoftSeeded);
    (void)fSoftPass;
}
