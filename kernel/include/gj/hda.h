 ...
 *
 * Dual path model:
 *   Soft path always available for smoke (stream ring, CORB/RIRB software
 *   rings, BDL kick, multi-stream mixdown, soft codec NID map).
 *   Real DMA/CORB when PCI HDA BAR0 present (QEMU intel-hda); reg shadow
 *   used when no BAR.
 *
 * Stream 0 compatibility:
 *   hda_stream_* is the default path (stream id 0). Multi-stream APIs
 *   (hda_stream_n_*) address SD0 + SD1 (GJ_HDA_STREAMS_MAX=2).
 *
 * Soft deepen (v1):
 *   - Multi-stream mixer + per-stream soft BDL/LPIB + soft codec
 *   - Master/stream gain 0..256 (256 = unity) applied on write/mix
...
/** Default PCM ring capacity (bytes) for stream 0 path. */
...
/**
 * PCI probe for HDA class/subclass; map BAR0 when present.
 * Soft path remains ready even if no device (smoke without hardware).
 */
void hda_stub_probe(void);

/** Non-zero when probe completed and soft (or HW) path is usable. */
int  hda_stub_ready(void);

/* ---- Software stream (works with or without PCI HDA present) -------- */

/**
 * Open default stream (id 0) with format. Returns 0 or -1 on bad args /
 * already open soft conflict.
 */
int  hda_stream_open(u32 u32Channels, u32 u32RateHz, u32 u32Bits);
void hda_stream_close(void);
int  hda_stream_ready(void);

/** Queue interleaved PCM samples; returns bytes accepted (may short). */
u32  hda_stream_write(const void *pData, u32 u32Bytes);

/** Start consuming the ring (software “DMA running”). Returns 0 or -1. */
...
/**
 * Soft BDL entry: addr is software cookie / host VA stand-in until real DMA.
 * u32Ioc: interrupt-on-completion soft flag for segment.
 */
...
/** Pop RIRB response; returns 1 if filled, 0 if empty. */
int  hda_rirb_pop(u32 *pResp);

u32  hda_corb_sent(void);
u32  hda_rirb_count(void);

/** Install BDL for stream 0; software copies into PCM ring on start/kick. */
...
/**
 * Kick BDL stream 0: feed BDL segments into PCM ring and start
 * (software DMA). Returns 0 or -1.
 */
...
/** Master + per-stream gain in 0..256 (256 = unity). Returns 0 or -1. */
...
/**
 * Multi-stream + soft codec + mixdown + soft BDL/LPIB smoke.
 * greppable PASS. Returns 0 on success.
 */
