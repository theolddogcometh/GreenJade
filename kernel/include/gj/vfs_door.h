 ...
 *
 * VFS door - multi-client file namespace over store/LBA mini-FS.
 * Pure C11 freestanding dual MIT OR Apache-2.0 surface for userspace hosts:
 *   vfsd / shell / native GJ_SYS_VFS clients (not multi-server confine).
 *
 * Dispatch contract:
 ...
 *
 * Lean soft residual (this unit exclusive; Soft!=product dual license):
 *   Userspace host handoff residual (not product multi-server DoD):
 *     CLAIM -> FORMAT/MOUNT -> name R/W or OPEN/READFD/WRITEFD/SEEKFD
 *   Product path host = vfsd (claim owner); shell + native GJ_SYS_VFS
 *     clients share the same door surface (multi_client door fds).
 *   Once-lamp functional residual: path_normalize + multi-sec geom +
 *     opcode/policy constants - never mutates door product state.
...
/**
 * open: arg1=name path (accepts /mnt/foo or bare), arg2=flags
 ...
 * Call before GJ_SYS_VFS / vfsd spawn. Emits lean residual baseline once.
 */
...
/**
 * Soft path: first-claim count + reclaims (diagnostics).
 * greppable: vfs_door claim soft
 * Diagnostic readers emit lean residual once only (no inventory storm).
 */
