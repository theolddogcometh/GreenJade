 ...
 *
 * Soft door call inventory (Wave 19 exclusive deepen — this unit only):
 ...
 *   - return rate / retcode (Wave 19 deepen): call|recv|install rate lamps
 ...
 *
 * Soft ephemeral single-use REPLY (Wave 19 deepen — not full MIG product):
...
#define DOOR_SOFT_DEEPEN_WAVE  19u
/* +return selftest|retmap over Wave 17 return rate|retcode. */
#define DOOR_SOFT_DEEPEN_AREAS 30u
...
/*
 * Soft product inventory (Wave 19 exclusive). Cumulative path tallies across
...
/**
 * Greppable soft door call inventory (Wave 19 exclusive; product / smoke).
 ...
 *   door: soft return selftest — Wave 19 terminal return surface
 *   door: soft retmap     — Wave 19 return-surface map
 *   door: soft deepen     — wave=19 areas stamp
...
door_soft_inventory_log(const struct gj_door *pDoor)
{
    ...
    /* Grep: door: soft deepen wave (Wave 19 stamp) */
    /*
     * ---- Wave 19 exclusive complementary surfaces (never reshape primary).
     * Return surfaces only — soft inventory; never hard-gates product paths.
     * Soft≠product; not bar3.
     */
    /* Grep: door: soft retclass — Wave 19 return-class taxonomy */
    kprintf("door: soft retclass ok|fail|inval|nodev|busy|nomem "
            "soft_only=1 product_gate=0 wave=%u "
            "(retclass taxonomy; Soft≠product; not bar3)\n",
            (unsigned)DOOR_SOFT_DEEPEN_WAVE);
    /* Grep: door: soft retlane — Wave 19 return-lane catalog */
...
door_cold_init(void)
{
    ...
    /* Grep: door: soft (baseline inventory after cold init; wave=19) */
