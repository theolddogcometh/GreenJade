 ...
 *   notify: soft deepen            — wave=80 areas stamp
...
#define NOTIFY_SOFT_DEEPEN_AREAS 106u
...
kprintf("notify: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=80 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("notify: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=80 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("notify: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=80 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("notify: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=80 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("notify: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=80 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("notify: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=80 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: notify: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("notify: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=80 (retcurtainface honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("notify: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=80 (retbastionangle stamp; Soft≠product)\n");
/* Grep: notify: soft retglacisangle — Wave 74 return-glacisangle honesty */
kprintf("notify: soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=80 (retglacisangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
kprintf("notify: soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=80 (retparapetangle stamp; Soft≠product)\n");
/* Grep: notify: soft retmoatangle — Wave 75 return-moatangle honesty */
kprintf("notify: soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=80 (retmoatangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retowerangle — Wave 75 exclusive towerangle stamp */
kprintf("notify: soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=80 (retowerangle stamp; Soft≠product)\n");
/* Grep: notify: soft retgateangle — Wave 76 return-gateangle honesty */
kprintf("notify: soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=80 (retgateangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retwallangle — Wave 76 exclusive wallangle stamp */
kprintf("notify: soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=80 (retwallangle stamp; Soft≠product)\n");
/* Grep: notify: soft retspireangle — Wave 77 return-spireangle honesty */
kprintf("notify: soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=80 (retspireangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retholdangle — Wave 77 exclusive holdangle stamp */
kprintf("notify: soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=80 (retholdangle stamp; Soft≠product)\n");
/* Grep: notify: soft retpalaceangle — Wave 78 return-palaceangle honesty */
kprintf("notify: soft retpalaceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=80 (retpalaceangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retfortressangle — Wave 78 exclusive fortressangle stamp */
kprintf("notify: soft retfortressangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=80 (retfortressangle stamp; Soft≠product)\n");
/* Grep: notify: soft retkeepangle — Wave 79 return-keepangle honesty */
kprintf("notify: soft retkeepangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=80 (retkeepangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retredoubtangle — Wave 79 exclusive redoubtangle stamp */
kprintf("notify: soft retredoubtangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=80 (retredoubtangle stamp; Soft≠product)\n");
/* Grep: notify: soft retcitadelangle — Wave 80 return-citadelangle honesty */
kprintf("notify: soft retcitadelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=80 (retcitadelangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retbastionkeep — Wave 80 exclusive bastionkeep stamp */
kprintf("notify: soft retbastionkeep exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=80 (retbastionkeep stamp; Soft≠product)\n");
