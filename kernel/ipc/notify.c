 ...
 *   notify: soft deepen            — wave=107 areas stamp
...
#define NOTIFY_SOFT_DEEPEN_WAVE 107u
/* +return selftest|retmap over Wave 17 return rate|retcode. */
#define NOTIFY_SOFT_DEEPEN_AREAS 152u
...
kprintf("notify: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("notify: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("notify: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("notify: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("notify: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("notify: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: notify: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("notify: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retcurtainface honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("notify: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retbastionangle stamp; Soft≠product)\n");
/* Grep: notify: soft retglacisangle — Wave 74 return-glacisangle honesty */
kprintf("notify: soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retglacisangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
kprintf("notify: soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retparapetangle stamp; Soft≠product)\n");
/* Grep: notify: soft retmoatangle — Wave 75 return-moatangle honesty */
kprintf("notify: soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retmoatangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retowerangle — Wave 75 exclusive towerangle stamp */
kprintf("notify: soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retowerangle stamp; Soft≠product)\n");
/* Grep: notify: soft retgateangle — Wave 76 return-gateangle honesty */
kprintf("notify: soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retgateangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retwallangle — Wave 76 exclusive wallangle stamp */
kprintf("notify: soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retwallangle stamp; Soft≠product)\n");
/* Grep: notify: soft retspireangle — Wave 77 return-spireangle honesty */
kprintf("notify: soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retspireangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retholdangle — Wave 77 exclusive holdangle stamp */
kprintf("notify: soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retholdangle stamp; Soft≠product)\n");
/* Grep: notify: soft retpalaceangle — Wave 78 return-palaceangle honesty */
kprintf("notify: soft retpalaceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retpalaceangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retfortressangle — Wave 78 exclusive fortressangle stamp */
kprintf("notify: soft retfortressangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retfortressangle stamp; Soft≠product)\n");
/* Grep: notify: soft retkeepangle — Wave 79 return-keepangle honesty */
kprintf("notify: soft retkeepangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retkeepangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retredoubtangle — Wave 79 exclusive redoubtangle stamp */
kprintf("notify: soft retredoubtangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retredoubtangle stamp; Soft≠product)\n");
/* Grep: notify: soft retcitadelangle — Wave 80 return-citadelangle honesty */
kprintf("notify: soft retcitadelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retcitadelangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retbastionkeep — Wave 80 exclusive bastionkeep stamp */
kprintf("notify: soft retbastionkeep exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retbastionkeep stamp; Soft≠product)\n");
/* Grep: notify: soft retpanoplyangle — Wave 81 return-panoplyangle honesty */
kprintf("notify: soft retpanoplyangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retpanoplyangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retbulwarkangle — Wave 81 exclusive bulwarkangle stamp */
kprintf("notify: soft retbulwarkangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retbulwarkangle stamp; Soft≠product)\n");
/* Grep: notify: soft retmantleangle — Wave 82 return-mantleangle honesty */
kprintf("notify: soft retmantleangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retmantleangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retaegisangle — Wave 82 exclusive aegisangle stamp */
kprintf("notify: soft retaegisangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retaegisangle stamp; Soft≠product)\n");
/* Grep: notify: soft retemblemangle — Wave 83 return-emblemangle honesty */
kprintf("notify: soft retemblemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retemblemangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retsigilangle — Wave 83 exclusive sigilangle stamp */
kprintf("notify: soft retsigilangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retsigilangle stamp; Soft≠product)\n");
/* Grep: notify: soft retscepterangle — Wave 84 return-scepterangle honesty */
kprintf("notify: soft retscepterangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retscepterangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retglyphangle — Wave 84 exclusive glyphangle stamp */
kprintf("notify: soft retglyphangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retglyphangle stamp; Soft≠product)\n");
/* Grep: notify: soft retcrownangle — Wave 85 return-crownangle honesty */
kprintf("notify: soft retcrownangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retcrownangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retshardangle — Wave 85 exclusive shardangle stamp */
kprintf("notify: soft retshardangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retshardangle stamp; Soft≠product)\n");
/* Grep: notify: soft retforgeangle — Wave 86 return-forgeangle honesty */
kprintf("notify: soft retforgeangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retforgeangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retprismangle — Wave 86 exclusive prismangle stamp */
kprintf("notify: soft retprismangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retprismangle stamp; Soft≠product)\n");
/* Grep: notify: soft retflameangle — Wave 87 return-flameangle honesty */
kprintf("notify: soft retflameangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retflameangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retcipherangle — Wave 87 exclusive cipherangle stamp */
kprintf("notify: soft retcipherangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retcipherangle stamp; Soft≠product)\n");
/* Grep: notify: soft retbeaconangle — Wave 88 return-beaconangle honesty */
kprintf("notify: soft retbeaconangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retbeaconangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retledgerangle — Wave 88 exclusive ledgerangle stamp */
kprintf("notify: soft retledgerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retledgerangle stamp; Soft≠product)\n");
/* Grep: notify: soft retbannerangle — Wave 89 return-bannerangle honesty */
kprintf("notify: soft retbannerangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retbannerangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retvaultangle — Wave 89 exclusive vaultangle stamp */
kprintf("notify: soft retvaultangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retvaultangle stamp; Soft≠product)\n");
/* Grep: notify: soft retcrestangle — Wave 90 return-crestangle honesty */
kprintf("notify: soft retcrestangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retcrestangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft rettokenangle — Wave 90 exclusive tokenangle stamp */
kprintf("notify: soft rettokenangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (rettokenangle stamp; Soft≠product)\n");
/* Grep: notify: soft retbadgeangle — Wave 91 return-badgeangle honesty */
kprintf("notify: soft retbadgeangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retbadgeangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retphaseangle — Wave 91 exclusive phaseangle stamp */
kprintf("notify: soft retphaseangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retphaseangle stamp; Soft≠product)\n");
/* Grep: notify: soft retmarkangle — Wave 92 return-markangle honesty */
kprintf("notify: soft retmarkangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retmarkangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retpulseangle — Wave 92 exclusive pulseangle stamp */
kprintf("notify: soft retpulseangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retpulseangle stamp; Soft≠product)\n");

/* Grep: notify: soft retsealangle — Wave 93 return-sealangle honesty */
kprintf("notify: soft retsealangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retsealangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retboundangle — Wave 93 exclusive boundangle stamp */
kprintf("notify: soft retboundangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retboundangle stamp; Soft≠product)\n");
/* Grep: notify: soft retstemangle — Wave 94 return-stemangle honesty */
kprintf("notify: soft retstemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retstemangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retbladeangle — Wave 94 exclusive bladeangle stamp */
kprintf("notify: soft retbladeangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retbladeangle stamp; Soft≠product)\n");
/* Grep: notify: soft retchordangle — Wave 95 return-chordangle honesty */
kprintf("notify: soft retchordangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retchordangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retarcangle — Wave 95 exclusive arcangle stamp */
kprintf("notify: soft retarcangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retarcangle stamp; Soft≠product)\n");
/* Grep: notify: soft retsectorangle — Wave 96 return-sectorangle honesty */
kprintf("notify: soft retsectorangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retsectorangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retwedgeangle — Wave 96 exclusive wedgeangle stamp */
kprintf("notify: soft retwedgeangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retwedgeangle stamp; Soft≠product)\n");
/* Grep: notify: soft retradiusangle — Wave 97 return-radiusangle honesty */
kprintf("notify: soft retradiusangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retradiusangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retdiameterangle — Wave 97 exclusive diameterangle stamp */
kprintf("notify: soft retdiameterangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retdiameterangle stamp; Soft≠product)\n");
/* Grep: notify: soft retcircumangle — Wave 98 return-circumangle honesty */
kprintf("notify: soft retcircumangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retcircumangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retellipseangle — Wave 98 exclusive ellipseangle stamp */
kprintf("notify: soft retellipseangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retellipseangle stamp; Soft≠product)\n");
/* Grep: notify: soft rethyperangle — Wave 99 return-hyperangle honesty */
kprintf("notify: soft rethyperangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (rethyperangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retparabolaangle — Wave 99 exclusive parabolaangle stamp */
kprintf("notify: soft retparabolaangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retparabolaangle stamp; Soft≠product)\n");
/* Grep: notify: soft retspiralangle — Wave 100 return-spiralangle honesty */
kprintf("notify: soft retspiralangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retspiralangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft rethelixangle — Wave 100 exclusive helixangle stamp */
kprintf("notify: soft rethelixangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (rethelixangle stamp; Soft≠product)\n");
/* Grep: notify: soft rettorusangle — Wave 101 return-torusangle honesty */
kprintf("notify: soft rettorusangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (rettorusangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retknotangle — Wave 101 exclusive knotangle stamp */
kprintf("notify: soft retknotangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retknotangle stamp; Soft≠product)\n");
/* Grep: notify: soft retmoebiusangle — Wave 102 return-moebiusangle honesty */
kprintf("notify: soft retmoebiusangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retmoebiusangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retkleinangle — Wave 102 exclusive kleinangle stamp */
kprintf("notify: soft retkleinangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retkleinangle stamp; Soft≠product)\n");
/* Grep: notify: soft retprojectangle — Wave 103 return-projectangle honesty */
kprintf("notify: soft retprojectangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retprojectangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retaffineangle — Wave 103 exclusive affineangle stamp */
kprintf("notify: soft retaffineangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retaffineangle stamp; Soft≠product)\n");
/* Grep: notify: soft retlinearangle — Wave 104 return-linearangle honesty */
kprintf("notify: soft retlinearangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retlinearangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retbilinearangle — Wave 104 exclusive bilinearangle stamp */
kprintf("notify: soft retbilinearangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retbilinearangle stamp; Soft≠product)\n");
/* Grep: notify: soft retquadraticangle — Wave 105 return-quadraticangle honesty */
kprintf("notify: soft retquadraticangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retquadraticangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retcubicangle — Wave 105 exclusive cubicangle stamp */
kprintf("notify: soft retcubicangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retcubicangle stamp; Soft≠product)\n");
/* Grep: notify: soft retquarticangle — Wave 106 return-quarticangle honesty */
kprintf("notify: soft retquarticangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retquarticangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retquinticangle — Wave 106 exclusive quinticangle stamp */
kprintf("notify: soft retquinticangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retquinticangle stamp; Soft≠product)\n");
/* Grep: notify: soft retsplineangle — Wave 107 return-splineangle honesty */
kprintf("notify: soft retsplineangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=107 (retsplineangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retbezierangle — Wave 107 exclusive bezierangle stamp */
kprintf("notify: soft retbezierangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=107 (retbezierangle stamp; Soft≠product)\n");
