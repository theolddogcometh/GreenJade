 ...
 *   notify: soft deepen            — wave=96 areas stamp
...
#define NOTIFY_SOFT_DEEPEN_WAVE 95u
/* +return selftest|retmap over Wave 17 return rate|retcode. */
#define NOTIFY_SOFT_DEEPEN_AREAS 134u
...
kprintf("notify: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("notify: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("notify: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("notify: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("notify: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("notify: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: notify: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("notify: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retcurtainface honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("notify: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retbastionangle stamp; Soft≠product)\n");
/* Grep: notify: soft retglacisangle — Wave 74 return-glacisangle honesty */
kprintf("notify: soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retglacisangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
kprintf("notify: soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retparapetangle stamp; Soft≠product)\n");
/* Grep: notify: soft retmoatangle — Wave 75 return-moatangle honesty */
kprintf("notify: soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retmoatangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retowerangle — Wave 75 exclusive towerangle stamp */
kprintf("notify: soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retowerangle stamp; Soft≠product)\n");
/* Grep: notify: soft retgateangle — Wave 76 return-gateangle honesty */
kprintf("notify: soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retgateangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retwallangle — Wave 76 exclusive wallangle stamp */
kprintf("notify: soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retwallangle stamp; Soft≠product)\n");
/* Grep: notify: soft retspireangle — Wave 77 return-spireangle honesty */
kprintf("notify: soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retspireangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retholdangle — Wave 77 exclusive holdangle stamp */
kprintf("notify: soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retholdangle stamp; Soft≠product)\n");
/* Grep: notify: soft retpalaceangle — Wave 78 return-palaceangle honesty */
kprintf("notify: soft retpalaceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retpalaceangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retfortressangle — Wave 78 exclusive fortressangle stamp */
kprintf("notify: soft retfortressangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retfortressangle stamp; Soft≠product)\n");
/* Grep: notify: soft retkeepangle — Wave 79 return-keepangle honesty */
kprintf("notify: soft retkeepangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retkeepangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retredoubtangle — Wave 79 exclusive redoubtangle stamp */
kprintf("notify: soft retredoubtangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retredoubtangle stamp; Soft≠product)\n");
/* Grep: notify: soft retcitadelangle — Wave 80 return-citadelangle honesty */
kprintf("notify: soft retcitadelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retcitadelangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retbastionkeep — Wave 80 exclusive bastionkeep stamp */
kprintf("notify: soft retbastionkeep exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retbastionkeep stamp; Soft≠product)\n");
/* Grep: notify: soft retpanoplyangle — Wave 81 return-panoplyangle honesty */
kprintf("notify: soft retpanoplyangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retpanoplyangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retbulwarkangle — Wave 81 exclusive bulwarkangle stamp */
kprintf("notify: soft retbulwarkangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retbulwarkangle stamp; Soft≠product)\n");
/* Grep: notify: soft retmantleangle — Wave 82 return-mantleangle honesty */
kprintf("notify: soft retmantleangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retmantleangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retaegisangle — Wave 82 exclusive aegisangle stamp */
kprintf("notify: soft retaegisangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retaegisangle stamp; Soft≠product)\n");
/* Grep: notify: soft retemblemangle — Wave 83 return-emblemangle honesty */
kprintf("notify: soft retemblemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retemblemangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retsigilangle — Wave 83 exclusive sigilangle stamp */
kprintf("notify: soft retsigilangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retsigilangle stamp; Soft≠product)\n");
/* Grep: notify: soft retscepterangle — Wave 84 return-scepterangle honesty */
kprintf("notify: soft retscepterangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retscepterangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retglyphangle — Wave 84 exclusive glyphangle stamp */
kprintf("notify: soft retglyphangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retglyphangle stamp; Soft≠product)\n");
/* Grep: notify: soft retcrownangle — Wave 85 return-crownangle honesty */
kprintf("notify: soft retcrownangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retcrownangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retshardangle — Wave 85 exclusive shardangle stamp */
kprintf("notify: soft retshardangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retshardangle stamp; Soft≠product)\n");
/* Grep: notify: soft retforgeangle — Wave 86 return-forgeangle honesty */
kprintf("notify: soft retforgeangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retforgeangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retprismangle — Wave 86 exclusive prismangle stamp */
kprintf("notify: soft retprismangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retprismangle stamp; Soft≠product)\n");
/* Grep: notify: soft retflameangle — Wave 87 return-flameangle honesty */
kprintf("notify: soft retflameangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retflameangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retcipherangle — Wave 87 exclusive cipherangle stamp */
kprintf("notify: soft retcipherangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retcipherangle stamp; Soft≠product)\n");
/* Grep: notify: soft retbeaconangle — Wave 88 return-beaconangle honesty */
kprintf("notify: soft retbeaconangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retbeaconangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retledgerangle — Wave 88 exclusive ledgerangle stamp */
kprintf("notify: soft retledgerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retledgerangle stamp; Soft≠product)\n");
/* Grep: notify: soft retbannerangle — Wave 89 return-bannerangle honesty */
kprintf("notify: soft retbannerangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retbannerangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retvaultangle — Wave 89 exclusive vaultangle stamp */
kprintf("notify: soft retvaultangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retvaultangle stamp; Soft≠product)\n");
/* Grep: notify: soft retcrestangle — Wave 90 return-crestangle honesty */
kprintf("notify: soft retcrestangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retcrestangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft rettokenangle — Wave 90 exclusive tokenangle stamp */
kprintf("notify: soft rettokenangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (rettokenangle stamp; Soft≠product)\n");
/* Grep: notify: soft retbadgeangle — Wave 91 return-badgeangle honesty */
kprintf("notify: soft retbadgeangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retbadgeangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retphaseangle — Wave 91 exclusive phaseangle stamp */
kprintf("notify: soft retphaseangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retphaseangle stamp; Soft≠product)\n");
/* Grep: notify: soft retmarkangle — Wave 92 return-markangle honesty */
kprintf("notify: soft retmarkangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retmarkangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retpulseangle — Wave 92 exclusive pulseangle stamp */
kprintf("notify: soft retpulseangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retpulseangle stamp; Soft≠product)\n");

/* Grep: notify: soft retsealangle — Wave 93 return-sealangle honesty */
kprintf("notify: soft retsealangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retsealangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retboundangle — Wave 93 exclusive boundangle stamp */
kprintf("notify: soft retboundangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retboundangle stamp; Soft≠product)\n");
/* Grep: notify: soft retstemangle — Wave 94 return-stemangle honesty */
kprintf("notify: soft retstemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retstemangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retbladeangle — Wave 94 exclusive bladeangle stamp */
kprintf("notify: soft retbladeangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retbladeangle stamp; Soft≠product)\n");
/* Grep: notify: soft retchordangle — Wave 95 return-chordangle honesty */
kprintf("notify: soft retchordangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retchordangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retarcangle — Wave 95 exclusive arcangle stamp */
kprintf("notify: soft retarcangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retarcangle stamp; Soft≠product)\n");
/* Grep: notify: soft retsectorangle — Wave 96 return-sectorangle honesty */
kprintf("notify: soft retsectorangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retsectorangle honesty; Soft≠product; not bar3)\n");
/* Grep: notify: soft retwedgeangle — Wave 96 exclusive wedgeangle stamp */
kprintf("notify: soft retwedgeangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retwedgeangle stamp; Soft≠product)\n");
