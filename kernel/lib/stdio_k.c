...
/* Grep: kprintf: soft deepen wave */
    ...
    kpf_soft_kv(" skip=", g_u64SoftSkip);
    console_write(" (Wave 38 exclusive; soft only; not libc)\n");
    ...
    console_write("kprintf: soft note milestone=wave38 exclusive=1 "
...
/* Grep: stdio_k: soft deepen */
    ...
    kpf_soft_kv(" calls=", g_u64SoftCalls);
    kpf_soft_kv(" logs=", (u64)g_u32SoftLogged);
    console_write(" (Wave 38 exclusive; soft only)\n");
    ...
    console_write("stdio_k: soft note milestone=wave38 exclusive=1 "
                  "soft_only=1 wave=");
    print_u64((u64)KPF_SOFT_WAVE, 10u, 0, ' ');
    console_write("\n");
