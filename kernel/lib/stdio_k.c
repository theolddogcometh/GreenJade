 ...
 * Soft!=product; not bar3.
 */
/* Grep: kprintf: soft retbulwark — Wave 32 return-bulwark honesty */
console_write("kprintf: soft retbulwark soft_only=1 product_gate=0 soft_ne_product=1 "
        "never_blocks_m0=1 wave=32 "
        "(retbulwark honesty; Soft!=product; not bar3)\n");
/* Grep: kprintf: soft retpanoply — Wave 32 exclusive panoply stamp */
...
/* Grep: kprintf: soft deepen wave */
    ...
    kpf_soft_kv(" skip=", g_u64SoftSkip);
    console_write(" (Wave 32 exclusive; soft only; not libc)\n");
    ...
    console_write("kprintf: soft note milestone=wave32 exclusive=1 "
...
/* Grep: stdio_k: soft deepen */
    ...
    kpf_soft_kv(" calls=", g_u64SoftCalls);
    kpf_soft_kv(" logs=", (u64)g_u32SoftLogged);
    console_write(" (Wave 32 exclusive; soft only)\n");
    ...
    console_write("stdio_k: soft note milestone=wave32 exclusive=1 "
                  "soft_only=1 wave=");
    print_u64((u64)KPF_SOFT_WAVE, 10u, 0, ' ');
    console_write("\n");
