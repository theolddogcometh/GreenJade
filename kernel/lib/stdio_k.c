...
kprintf("kprintf: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=70 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: kprintf: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("kprintf: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=70 (retcounterscarple stamp; Soft≠product)\n");
...
/* Grep: kprintf: soft deepen wave */
    ...
    console_write("kprintf: soft note milestone=wave70 exclusive=1 "
...
/* Grep: stdio_k: soft deepen */
    ...
    kpf_soft_kv(" calls=", g_u64SoftCalls);
    kpf_soft_kv(" logs=", (u64)g_u32SoftLogged);
    console_write(" (Wave 70 exclusive; soft only)\n");
    ...
    console_write("stdio_k: soft note milestone=wave70 exclusive=1 "
                  "soft_only=1 wave=");
    print_u64((u64)KPF_SOFT_WAVE, 10u, 0, ' ');
    console_write("\n");
