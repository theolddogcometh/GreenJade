...
kernel_after_mmap(struct gj_mem_region *aRegions, size_t cRegions)
{
     ...
     * Wave 21 exclusive deepen: soft inventory + path + capacity +
     ...
     */
    {
        ...
        enum { MAIN_SOFT_SMOKE_WAVE = 21u, MAIN_SOFT_SMOKE_AREAS = 22u };
        ...
        kprintf("soft: surfaces count=%u "
                ...
                "return,retmap,retclass,retlane,retbound,retseal,retpulse,retmark,deepen,PASS wave=%u\n",
                (unsigned)MAIN_SOFT_SMOKE_AREAS,
                (unsigned)MAIN_SOFT_SMOKE_WAVE);
        ...
        kprintf("soft: catalog wave=%u areas=%u "
                ...
                "inventory,path,capacity,surfaces,catalog,note,honesty,"
                "return,retmap,retclass,retlane,retbound,retseal,retpulse,retmark,deepen,PASS\n",
                (unsigned)MAIN_SOFT_SMOKE_WAVE,
                (unsigned)MAIN_SOFT_SMOKE_AREAS);

        /* Grep: soft: note (Wave 20 deepen) */
        kprintf("soft: note milestone=wave21 exclusive=1 soft_only=1 "
        ...
        kprintf("soft: retseal exclusive=1 soft_ne_product=1 "
                ...
                */
                /* Grep: soft: retpulse — Wave 21 return-pulse honesty */
                kprintf("soft: retpulse soft_only=1 product_gate=0 soft_ne_product=1 "
                        "never_blocks_m0=1 wave=%u "
                        "(retpulse honesty; Soft≠product; not bar3)\n",
                        (unsigned)MAIN_SOFT_SMOKE_WAVE);
                /* Grep: soft: retmark — Wave 21 exclusive mark stamp */
