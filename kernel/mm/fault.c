 ...
 *     deepen wave=85 stamp + surf= bitmask
...
kprintf("fault: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: fault: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("fault: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("fault: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: fault: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("fault: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("fault: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: fault: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("fault: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: fault: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("fault: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retcurtainface honesty; Soft≠product; not bar3)\n");
/* Grep: fault: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("fault: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retbastionangle stamp; Soft≠product)\n");
/* Grep: fault: soft retglacisangle — Wave 74 return-glacisangle honesty */
kprintf("fault: soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retglacisangle honesty; Soft≠product; not bar3)\n");
/* Grep: fault: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
kprintf("fault: soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retparapetangle stamp; Soft≠product)\n");
/* Grep: fault: soft retmoatangle — Wave 75 return-moatangle honesty */
kprintf("fault: soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retmoatangle honesty; Soft≠product; not bar3)\n");
/* Grep: fault: soft retowerangle — Wave 75 exclusive towerangle stamp */
kprintf("fault: soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retowerangle stamp; Soft≠product)\n");
/* Grep: fault: soft retgateangle — Wave 76 return-gateangle honesty */
kprintf("fault: soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retgateangle honesty; Soft≠product; not bar3)\n");
/* Grep: fault: soft retwallangle — Wave 76 exclusive wallangle stamp */
kprintf("fault: soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retwallangle stamp; Soft≠product)\n");
/* Grep: fault: soft retspireangle — Wave 77 return-spireangle honesty */
kprintf("fault: soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retspireangle honesty; Soft≠product; not bar3)\n");
/* Grep: fault: soft retholdangle — Wave 77 exclusive holdangle stamp */
kprintf("fault: soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retholdangle stamp; Soft≠product)\n");
/* Grep: fault: soft retpalaceangle — Wave 78 return-palaceangle honesty */
kprintf("fault: soft retpalaceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retpalaceangle honesty; Soft≠product; not bar3)\n");
/* Grep: fault: soft retfortressangle — Wave 78 exclusive fortressangle stamp */
kprintf("fault: soft retfortressangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retfortressangle stamp; Soft≠product)\n");
/* Grep: fault: soft retkeepangle — Wave 79 return-keepangle honesty */
kprintf("fault: soft retkeepangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retkeepangle honesty; Soft≠product; not bar3)\n");
/* Grep: fault: soft retredoubtangle — Wave 79 exclusive redoubtangle stamp */
kprintf("fault: soft retredoubtangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retredoubtangle stamp; Soft≠product)\n");
/* Grep: fault: soft retcitadelangle — Wave 80 return-citadelangle honesty */
kprintf("fault: soft retcitadelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retcitadelangle honesty; Soft≠product; not bar3)\n");
/* Grep: fault: soft retbastionkeep — Wave 80 exclusive bastionkeep stamp */
kprintf("fault: soft retbastionkeep exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retbastionkeep stamp; Soft≠product)\n");
/* Grep: fault: soft retpanoplyangle — Wave 81 return-panoplyangle honesty */
kprintf("fault: soft retpanoplyangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retpanoplyangle honesty; Soft≠product; not bar3)\n");
/* Grep: fault: soft retbulwarkangle — Wave 81 exclusive bulwarkangle stamp */
kprintf("fault: soft retbulwarkangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retbulwarkangle stamp; Soft≠product)\n");
/* Grep: fault: soft retmantleangle — Wave 82 return-mantleangle honesty */
kprintf("fault: soft retmantleangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retmantleangle honesty; Soft≠product; not bar3)\n");
/* Grep: fault: soft retaegisangle — Wave 82 exclusive aegisangle stamp */
kprintf("fault: soft retaegisangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retaegisangle stamp; Soft≠product)\n");
/* Grep: fault: soft retemblemangle — Wave 83 return-emblemangle honesty */
kprintf("fault: soft retemblemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retemblemangle honesty; Soft≠product; not bar3)\n");
/* Grep: fault: soft retsigilangle — Wave 83 exclusive sigilangle stamp */
kprintf("fault: soft retsigilangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retsigilangle stamp; Soft≠product)\n");
/* Grep: fault: soft retscepterangle — Wave 84 return-scepterangle honesty */
kprintf("fault: soft retscepterangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retscepterangle honesty; Soft≠product; not bar3)\n");
/* Grep: fault: soft retglyphangle — Wave 84 exclusive glyphangle stamp */
kprintf("fault: soft retglyphangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retglyphangle stamp; Soft≠product)\n");
/* Grep: fault: soft retcrownangle — Wave 85 return-crownangle honesty */
kprintf("fault: soft retcrownangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retcrownangle honesty; Soft≠product; not bar3)\n");
/* Grep: fault: soft retshardangle — Wave 85 exclusive shardangle stamp */
kprintf("fault: soft retshardangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retshardangle stamp; Soft≠product)\n");
