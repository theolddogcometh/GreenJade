 ...
 *   - Stats rollup + deepen wave=90 stamp + PASS/PARTIAL/INIT/NONE
...
#define MEMOBJ_SOFT_WAVE 90u
/* Catalog areas prior to deepen (honesty..OPEN). Soft ≠ product. */
#define MEMOBJ_SOFT_AREAS 168u
 ...
 *   memobj: soft deepen     — wave=90 stamp + area count
 *   memobj: soft PASS|PARTIAL|INIT|NONE | inventory PASS
 * greppable: memobj: soft
...
kprintf("memobj: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=90 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: memobj: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("memobj: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=90 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("memobj: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=90 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: memobj: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("memobj: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=90 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("memobj: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=90 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: memobj: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("memobj: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=90 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: memobj: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("memobj: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=90 (retcurtainface honesty; Soft≠product; not bar3)\n");
/* Grep: memobj: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("memobj: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=90 (retbastionangle stamp; Soft≠product)\n");
/* Grep: memobj: soft retglacisangle — Wave 74 return-glacisangle honesty */
kprintf("memobj: soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=90 (retglacisangle honesty; Soft≠product; not bar3)\n");
/* Grep: memobj: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
kprintf("memobj: soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=90 (retparapetangle stamp; Soft≠product)\n");
/* Grep: memobj: soft retmoatangle — Wave 75 return-moatangle honesty */
kprintf("memobj: soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=90 (retmoatangle honesty; Soft≠product; not bar3)\n");
/* Grep: memobj: soft retowerangle — Wave 75 exclusive towerangle stamp */
kprintf("memobj: soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=90 (retowerangle stamp; Soft≠product)\n");
/* Grep: memobj: soft retgateangle — Wave 76 return-gateangle honesty */
kprintf("memobj: soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=90 (retgateangle honesty; Soft≠product; not bar3)\n");
/* Grep: memobj: soft retwallangle — Wave 76 exclusive wallangle stamp */
kprintf("memobj: soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=90 (retwallangle stamp; Soft≠product)\n");
/* Grep: memobj: soft retspireangle — Wave 77 return-spireangle honesty */
kprintf("memobj: soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=90 (retspireangle honesty; Soft≠product; not bar3)\n");
/* Grep: memobj: soft retholdangle — Wave 77 exclusive holdangle stamp */
kprintf("memobj: soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=90 (retholdangle stamp; Soft≠product)\n");
/* Grep: memobj: soft retpalaceangle — Wave 78 return-palaceangle honesty */
kprintf("memobj: soft retpalaceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=90 (retpalaceangle honesty; Soft≠product; not bar3)\n");
/* Grep: memobj: soft retfortressangle — Wave 78 exclusive fortressangle stamp */
kprintf("memobj: soft retfortressangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=90 (retfortressangle stamp; Soft≠product)\n");
/* Grep: memobj: soft retkeepangle — Wave 79 return-keepangle honesty */
kprintf("memobj: soft retkeepangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=90 (retkeepangle honesty; Soft≠product; not bar3)\n");
/* Grep: memobj: soft retredoubtangle — Wave 79 exclusive redoubtangle stamp */
kprintf("memobj: soft retredoubtangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=90 (retredoubtangle stamp; Soft≠product)\n");
/* Grep: memobj: soft retcitadelangle — Wave 80 return-citadelangle honesty */
kprintf("memobj: soft retcitadelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=90 (retcitadelangle honesty; Soft≠product; not bar3)\n");
/* Grep: memobj: soft retbastionkeep — Wave 80 exclusive bastionkeep stamp */
kprintf("memobj: soft retbastionkeep exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=90 (retbastionkeep stamp; Soft≠product)\n");
/* Grep: memobj: soft retpanoplyangle — Wave 81 return-panoplyangle honesty */
kprintf("memobj: soft retpanoplyangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=90 (retpanoplyangle honesty; Soft≠product; not bar3)\n");
/* Grep: memobj: soft retbulwarkangle — Wave 81 exclusive bulwarkangle stamp */
kprintf("memobj: soft retbulwarkangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=90 (retbulwarkangle stamp; Soft≠product)\n");
/* Grep: memobj: soft retmantleangle — Wave 82 return-mantleangle honesty */
kprintf("memobj: soft retmantleangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=90 (retmantleangle honesty; Soft≠product; not bar3)\n");
/* Grep: memobj: soft retaegisangle — Wave 82 exclusive aegisangle stamp */
kprintf("memobj: soft retaegisangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=90 (retaegisangle stamp; Soft≠product)\n");
/* Grep: memobj: soft retemblemangle — Wave 83 return-emblemangle honesty */
kprintf("memobj: soft retemblemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=90 (retemblemangle honesty; Soft≠product; not bar3)\n");
/* Grep: memobj: soft retsigilangle — Wave 83 exclusive sigilangle stamp */
kprintf("memobj: soft retsigilangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=90 (retsigilangle stamp; Soft≠product)\n");
/* Grep: memobj: soft retscepterangle — Wave 84 return-scepterangle honesty */
kprintf("memobj: soft retscepterangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=90 (retscepterangle honesty; Soft≠product; not bar3)\n");
/* Grep: memobj: soft retglyphangle — Wave 84 exclusive glyphangle stamp */
kprintf("memobj: soft retglyphangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=90 (retglyphangle stamp; Soft≠product)\n");
/* Grep: memobj: soft retcrownangle — Wave 85 return-crownangle honesty */
kprintf("memobj: soft retcrownangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=90 (retcrownangle honesty; Soft≠product; not bar3)\n");
/* Grep: memobj: soft retshardangle — Wave 85 exclusive shardangle stamp */
kprintf("memobj: soft retshardangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=90 (retshardangle stamp; Soft≠product)\n");
/* Grep: memobj: soft retforgeangle — Wave 86 return-forgeangle honesty */
kprintf("memobj: soft retforgeangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=90 (retforgeangle honesty; Soft≠product; not bar3)\n");
/* Grep: memobj: soft retprismangle — Wave 86 exclusive prismangle stamp */
kprintf("memobj: soft retprismangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=90 (retprismangle stamp; Soft≠product)\n");
/* Grep: memobj: soft retflameangle — Wave 87 return-flameangle honesty */
kprintf("memobj: soft retflameangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=90 (retflameangle honesty; Soft≠product; not bar3)\n");
/* Grep: memobj: soft retcipherangle — Wave 87 exclusive cipherangle stamp */
kprintf("memobj: soft retcipherangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=90 (retcipherangle stamp; Soft≠product)\n");
/* Grep: memobj: soft retbeaconangle — Wave 88 return-beaconangle honesty */
kprintf("memobj: soft retbeaconangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=90 (retbeaconangle honesty; Soft≠product; not bar3)\n");
/* Grep: memobj: soft retledgerangle — Wave 88 exclusive ledgerangle stamp */
kprintf("memobj: soft retledgerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=90 (retledgerangle stamp; Soft≠product)\n");
/* Grep: memobj: soft retbannerangle — Wave 89 return-bannerangle honesty */
kprintf("memobj: soft retbannerangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=90 (retbannerangle honesty; Soft≠product; not bar3)\n");
/* Grep: memobj: soft retvaultangle — Wave 89 exclusive vaultangle stamp */
kprintf("memobj: soft retvaultangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=90 (retvaultangle stamp; Soft≠product)\n");
/* Grep: memobj: soft retcrestangle — Wave 90 return-crestangle honesty */
kprintf("memobj: soft retcrestangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=90 (retcrestangle honesty; Soft≠product; not bar3)\n");
/* Grep: memobj: soft rettokenangle — Wave 90 exclusive tokenangle stamp */
kprintf("memobj: soft rettokenangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=90 (rettokenangle stamp; Soft≠product)\n");
