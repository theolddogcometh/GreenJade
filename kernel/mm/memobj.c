 ...
 *   - Stats rollup + deepen wave=81 stamp + PASS/PARTIAL/INIT/NONE
...
#define MEMOBJ_SOFT_WAVE 81u
/* Catalog areas prior to deepen (honesty..OPEN). Soft ≠ product. */
#define MEMOBJ_SOFT_AREAS 150u
 ...
 *   memobj: soft deepen     — wave=81 stamp + area count
 *   memobj: soft PASS|PARTIAL|INIT|NONE | inventory PASS
 * greppable: memobj: soft
...
kprintf("memobj: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=81 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: memobj: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("memobj: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=81 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("memobj: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=81 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: memobj: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("memobj: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=81 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("memobj: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=81 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: memobj: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("memobj: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=81 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: memobj: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("memobj: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=81 (retcurtainface honesty; Soft≠product; not bar3)\n");
/* Grep: memobj: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("memobj: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=81 (retbastionangle stamp; Soft≠product)\n");
/* Grep: memobj: soft retglacisangle — Wave 74 return-glacisangle honesty */
kprintf("memobj: soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=81 (retglacisangle honesty; Soft≠product; not bar3)\n");
/* Grep: memobj: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
kprintf("memobj: soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=81 (retparapetangle stamp; Soft≠product)\n");
/* Grep: memobj: soft retmoatangle — Wave 75 return-moatangle honesty */
kprintf("memobj: soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=81 (retmoatangle honesty; Soft≠product; not bar3)\n");
/* Grep: memobj: soft retowerangle — Wave 75 exclusive towerangle stamp */
kprintf("memobj: soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=81 (retowerangle stamp; Soft≠product)\n");
/* Grep: memobj: soft retgateangle — Wave 76 return-gateangle honesty */
kprintf("memobj: soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=81 (retgateangle honesty; Soft≠product; not bar3)\n");
/* Grep: memobj: soft retwallangle — Wave 76 exclusive wallangle stamp */
kprintf("memobj: soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=81 (retwallangle stamp; Soft≠product)\n");
/* Grep: memobj: soft retspireangle — Wave 77 return-spireangle honesty */
kprintf("memobj: soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=81 (retspireangle honesty; Soft≠product; not bar3)\n");
/* Grep: memobj: soft retholdangle — Wave 77 exclusive holdangle stamp */
kprintf("memobj: soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=81 (retholdangle stamp; Soft≠product)\n");
/* Grep: memobj: soft retpalaceangle — Wave 78 return-palaceangle honesty */
kprintf("memobj: soft retpalaceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=81 (retpalaceangle honesty; Soft≠product; not bar3)\n");
/* Grep: memobj: soft retfortressangle — Wave 78 exclusive fortressangle stamp */
kprintf("memobj: soft retfortressangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=81 (retfortressangle stamp; Soft≠product)\n");
/* Grep: memobj: soft retkeepangle — Wave 79 return-keepangle honesty */
kprintf("memobj: soft retkeepangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=81 (retkeepangle honesty; Soft≠product; not bar3)\n");
/* Grep: memobj: soft retredoubtangle — Wave 79 exclusive redoubtangle stamp */
kprintf("memobj: soft retredoubtangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=81 (retredoubtangle stamp; Soft≠product)\n");
/* Grep: memobj: soft retcitadelangle — Wave 80 return-citadelangle honesty */
kprintf("memobj: soft retcitadelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=81 (retcitadelangle honesty; Soft≠product; not bar3)\n");
/* Grep: memobj: soft retbastionkeep — Wave 80 exclusive bastionkeep stamp */
kprintf("memobj: soft retbastionkeep exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=81 (retbastionkeep stamp; Soft≠product)\n");
/* Grep: memobj: soft retpanoplyangle — Wave 81 return-panoplyangle honesty */
kprintf("memobj: soft retpanoplyangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=81 (retpanoplyangle honesty; Soft≠product; not bar3)\n");
/* Grep: memobj: soft retbulwarkangle — Wave 81 exclusive bulwarkangle stamp */
kprintf("memobj: soft retbulwarkangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=81 (retbulwarkangle stamp; Soft≠product)\n");
