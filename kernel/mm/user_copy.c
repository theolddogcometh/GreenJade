 ...
 *
 * Soft copy_from/to_user inventory (Wave 9 base; Wave 33 exclusive deepen):
 ...
 *   "user_copy: soft deepen …"    wave=33 stamp + area count
 ...
 *   user_copy: soft return selftest — Wave 19 terminal return surface
 *   user_copy: soft retmap     — Wave 19 return-surface map
 *   user_copy: soft deepen     — wave=33 stamp + areas
 ...
 * Soft≠product; not bar3.
 */
/* Grep: user_copy: soft retbastion — Wave 33 return-bastion honesty */
kprintf("user_copy: soft retbastion soft_only=1 product_gate=0 soft_ne_product=1 "
        "never_blocks_m0=1 wave=%u "
        "(retbastion honesty; Soft≠product; not bar3)\n",
        (unsigned)USER_COPY_SOFT_WAVE);
/* Grep: user_copy: soft retcitadel — Wave 33 exclusive citadel stamp */
kprintf("user_copy: soft retcitadel exclusive=1 soft_ne_product=1 "
        ...
        (unsigned)USER_COPY_SOFT_WAVE);
            ...
            "(Wave 33 exclusive; not product SEH; not bar3; soft≠product)\n",
            (unsigned)USER_COPY_SOFT_WAVE,
            (unsigned)u32Areas,
            (unsigned long long)u64Logs,
