 ...
 *   "user_copy: soft deepen …"    wave=85 stamp + area count
 ...
 *   user_copy: soft return selftest — Wave 19 terminal return surface
 *   user_copy: soft retmap     — Wave 19 return-surface map
 *   user_copy: soft deepen     — wave=85 stamp + areas
...
kprintf("user_copy: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: user_copy: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("user_copy: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("user_copy: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: user_copy: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("user_copy: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("user_copy: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: user_copy: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("user_copy: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: user_copy: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("user_copy: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retcurtainface honesty; Soft≠product; not bar3)\n");
/* Grep: user_copy: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("user_copy: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retbastionangle stamp; Soft≠product)\n");
/* Grep: user_copy: soft retglacisangle — Wave 74 return-glacisangle honesty */
kprintf("user_copy: soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retglacisangle honesty; Soft≠product; not bar3)\n");
/* Grep: user_copy: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
kprintf("user_copy: soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retparapetangle stamp; Soft≠product)\n");
/* Grep: user_copy: soft retmoatangle — Wave 75 return-moatangle honesty */
kprintf("user_copy: soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retmoatangle honesty; Soft≠product; not bar3)\n");
/* Grep: user_copy: soft retowerangle — Wave 75 exclusive towerangle stamp */
kprintf("user_copy: soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retowerangle stamp; Soft≠product)\n");
/* Grep: user_copy: soft retgateangle — Wave 76 return-gateangle honesty */
kprintf("user_copy: soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retgateangle honesty; Soft≠product; not bar3)\n");
/* Grep: user_copy: soft retwallangle — Wave 76 exclusive wallangle stamp */
kprintf("user_copy: soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retwallangle stamp; Soft≠product)\n");
/* Grep: user_copy: soft retspireangle — Wave 77 return-spireangle honesty */
kprintf("user_copy: soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retspireangle honesty; Soft≠product; not bar3)\n");
/* Grep: user_copy: soft retholdangle — Wave 77 exclusive holdangle stamp */
kprintf("user_copy: soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retholdangle stamp; Soft≠product)\n");
/* Grep: user_copy: soft retpalaceangle — Wave 78 return-palaceangle honesty */
kprintf("user_copy: soft retpalaceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retpalaceangle honesty; Soft≠product; not bar3)\n");
/* Grep: user_copy: soft retfortressangle — Wave 78 exclusive fortressangle stamp */
kprintf("user_copy: soft retfortressangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retfortressangle stamp; Soft≠product)\n");
/* Grep: user_copy: soft retkeepangle — Wave 79 return-keepangle honesty */
kprintf("user_copy: soft retkeepangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retkeepangle honesty; Soft≠product; not bar3)\n");
/* Grep: user_copy: soft retredoubtangle — Wave 79 exclusive redoubtangle stamp */
kprintf("user_copy: soft retredoubtangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retredoubtangle stamp; Soft≠product)\n");
/* Grep: user_copy: soft retcitadelangle — Wave 80 return-citadelangle honesty */
kprintf("user_copy: soft retcitadelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retcitadelangle honesty; Soft≠product; not bar3)\n");
/* Grep: user_copy: soft retbastionkeep — Wave 80 exclusive bastionkeep stamp */
kprintf("user_copy: soft retbastionkeep exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retbastionkeep stamp; Soft≠product)\n");
/* Grep: user_copy: soft retpanoplyangle — Wave 81 return-panoplyangle honesty */
kprintf("user_copy: soft retpanoplyangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retpanoplyangle honesty; Soft≠product; not bar3)\n");
/* Grep: user_copy: soft retbulwarkangle — Wave 81 exclusive bulwarkangle stamp */
kprintf("user_copy: soft retbulwarkangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retbulwarkangle stamp; Soft≠product)\n");
/* Grep: user_copy: soft retmantleangle — Wave 82 return-mantleangle honesty */
kprintf("user_copy: soft retmantleangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retmantleangle honesty; Soft≠product; not bar3)\n");
/* Grep: user_copy: soft retaegisangle — Wave 82 exclusive aegisangle stamp */
kprintf("user_copy: soft retaegisangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retaegisangle stamp; Soft≠product)\n");
/* Grep: user_copy: soft retemblemangle — Wave 83 return-emblemangle honesty */
kprintf("user_copy: soft retemblemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retemblemangle honesty; Soft≠product; not bar3)\n");
/* Grep: user_copy: soft retsigilangle — Wave 83 exclusive sigilangle stamp */
kprintf("user_copy: soft retsigilangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retsigilangle stamp; Soft≠product)\n");
/* Grep: user_copy: soft retscepterangle — Wave 84 return-scepterangle honesty */
kprintf("user_copy: soft retscepterangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retscepterangle honesty; Soft≠product; not bar3)\n");
/* Grep: user_copy: soft retglyphangle — Wave 84 exclusive glyphangle stamp */
kprintf("user_copy: soft retglyphangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retglyphangle stamp; Soft≠product)\n");
/* Grep: user_copy: soft retcrownangle — Wave 85 return-crownangle honesty */
kprintf("user_copy: soft retcrownangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retcrownangle honesty; Soft≠product; not bar3)\n");
/* Grep: user_copy: soft retshardangle — Wave 85 exclusive shardangle stamp */
kprintf("user_copy: soft retshardangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retshardangle stamp; Soft≠product)\n");
