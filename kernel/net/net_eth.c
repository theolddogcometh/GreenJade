 ...
 *   net: eth soft deepen         — wave=87 areas stamp
...
net_eth_soft_log(void)
{
    ...
    kprintf("net_eth: soft %s frames_rx=%u drop=%u ok=%u vlan=%u tcp=%u "
            "arp=%u udp=%u icmp=%u poll_max=%u wave=87\n",
            szVerdict, g_u32FramesRx, g_u32FramesDrop, g_u32FramesOk,
            g_u32VlanSkip, g_u32TcpDemux, g_u32ArpReplies, g_u32UdpEchoes,
            g_u32IcmpEchoes, (u32)NET_ETH_POLL_MAX);
    ...
    kprintf("net_eth: soft inventory verdict=%s ready=%u frames_rx=%u "
            "drop=%u ok=%u vlan=%u proto=%u tcp=%u arp=%u udp=%u icmp=%u "
            "tx_ok=%u tx_fail=%u bytes_rx=%u bytes_tx=%u poll_max=%u "
            "log_n=%u cap=%u cadence=%u wave=87\n",
            szVerdict, u32Ready, g_u32FramesRx, g_u32FramesDrop,
            g_u32FramesOk, g_u32VlanSkip, u32Proto, g_u32TcpDemux,
            g_u32ArpReplies, g_u32UdpEchoes, g_u32IcmpEchoes, g_u32TxOk,
            g_u32TxFail, g_u32BytesRx, g_u32BytesTx, (u32)NET_ETH_POLL_MAX,
    ...
    kprintf("net_eth: soft frames rx=%u drop=%u ok=%u vlan=%u short=%u "
            "etype=%u ipv4=%u proto=%u bytes_rx=%u wave=87\n",
            g_u32FramesRx, g_u32FramesDrop, g_u32FramesOk, g_u32VlanSkip,
    ...
    kprintf("net_eth: soft demux arp_ok=%u icmp_ok=%u udp_ok=%u tcp_ok=%u "
            "vlan_skip=%u drop_short=%u drop_etype=%u drop_ipv4=%u "
            "drop_proto=%u frames_ok=%u wave=87\n",
            g_u32ArpReplies, g_u32IcmpEchoes, g_u32UdpEchoes, g_u32TcpDemux,
            g_u32VlanSkip, g_u32DropShort, g_u32DropEtype, u32DropIpv4,
            g_u32DropProto, g_u32FramesOk);
    ...
    kprintf("net_eth: soft tcp seen=%u demux=%u miss=%u "
            "(net_tcp_input soft; multi-seg via net_tcp_poll) wave=87\n",
            g_u32TcpSeen, g_u32TcpDemux, g_u32TcpMiss);
    ...
    kprintf("net_eth: soft link ready=%u polls=%u nodev=%u drain=%u "
            "last_batch=%u batch_max=%u link_ch=%u log_n=%u "
            "ip=%u.%u.%u.%u mac=%02x:%02x:%02x:%02x:%02x:%02x wave=87\n",
            u32Ready, g_u32Polls, g_u32PollsNoDev, g_u32PollsDrain,
            g_u32LastBatch, g_u32BatchMax, g_u32LinkChanges, g_u32SoftLogN,
            g_aOurIp[0], g_aOurIp[1], g_aOurIp[2], g_aOurIp[3],
            g_aOurMac[0], g_aOurMac[1], g_aOurMac[2], g_aOurMac[3],
    ...
    kprintf("net_eth: soft tx ok=%u fail=%u bytes=%u arp=%u icmp=%u udp=%u "
            "(reply path only; not full stack TX) wave=87\n",
            g_u32TxOk, g_u32TxFail, g_u32BytesTx, g_u32ArpReplies,
            g_u32IcmpEchoes, g_u32UdpEchoes);
    ...
    kprintf("net_eth: soft path virtio_rx=batch demux=arp|icmp|udp7|tcp "
            "vlan=skip_count ihl=icmp_soft rtx=net_tcp_poll "
            "guest=10.0.2.15/52:54:00:12:34:56 "
            "wave=87 (soft inventory; not netstackd; not bar3)\n");
    ...
    kprintf("net_eth: soft stats verdict=%s ready=%u rx=%u drop=%u ok=%u "
            ...
            "bytes_tx=%u polls=%u log_n=%u wave=87\n",
            szVerdict, u32Ready, g_u32FramesRx, g_u32FramesDrop,
     ...
     */
    {
        ...
        kprintf("net: eth soft ratio drop_bp=%u ok_bp=%u proto_bp=%u "
                "tx_fail_bp=%u rx=%u ok=%u drop=%u wave=87\n",
        ...
        kprintf("net_eth: soft headroom poll_max=%u last_batch=%u "
                "batch_head=%u log_cap=%u log_head=%u wave=87\n",
                (u32)NET_ETH_POLL_MAX, g_u32LastBatch, u32BatchHead,
                (u32)NET_ETH_SOFT_LOG_CAP, u32PollHead);
        ...
        kprintf("net: eth soft surface inventory,frames,demux,arp,icmp,udp,"
                ...
                "geom,terminal,return,retmap,deepen areas=78 wave=87\n");
        ...
        kprintf("net: eth soft capacity poll_max=%u log_cap=%u cadence=%u "
                "guest=10.0.2.15 arp=1 icmp=1 udp7=1 tcp=1 wave=87\n",
        ...
        kprintf("net: eth soft geom eth_max=1514 poll_max=%u "
                "batch_max=%u last_batch=%u wave=87\n",
        ...
        kprintf("net: eth soft terminal ready=%u rx=%u ok=%u drop=%u "
                "tx_ok=%u tx_fail=%u soft %s wave=87\n",
        ...
        kprintf("net: eth soft return ready=%u rx=%u ok=%u drop=%u "
                ...
                "product_netstackd=OPEN wave=87\n",
                u32Ready, g_u32FramesRx, g_u32FramesOk, g_u32FramesDrop,
        ...
        kprintf("net_eth: soft return ready=%u rx=%u ok=%u drop=%u "
                "tx_ok=%u tx_fail=%u product_netstackd=OPEN wave=87\n",
                u32Ready, g_u32FramesRx, g_u32FramesOk, g_u32FramesDrop,
...
kprintf("net: eth: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=87 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: net: eth: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("net: eth: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=87 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("net: eth: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=87 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: net: eth: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("net: eth: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=87 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("net: eth: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=87 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: net: eth: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("net: eth: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=87 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: net: eth: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("net: eth: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=87 (retcurtainface honesty; Soft≠product; not bar3)\n");
/* Grep: net: eth: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("net: eth: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=87 (retbastionangle stamp; Soft≠product)\n");
/* Grep: net: eth: soft retglacisangle — Wave 74 return-glacisangle honesty */
kprintf("net: eth: soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=87 (retglacisangle honesty; Soft≠product; not bar3)\n");
/* Grep: net: eth: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
kprintf("net: eth: soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=87 (retparapetangle stamp; Soft≠product)\n");
/* Grep: net: eth: soft retmoatangle — Wave 75 return-moatangle honesty */
kprintf("net: eth: soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=87 (retmoatangle honesty; Soft≠product; not bar3)\n");
/* Grep: net: eth: soft retowerangle — Wave 75 exclusive towerangle stamp */
kprintf("net: eth: soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=87 (retowerangle stamp; Soft≠product)\n");
/* Grep: net: eth: soft retgateangle — Wave 76 return-gateangle honesty */
kprintf("net: eth: soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=87 (retgateangle honesty; Soft≠product; not bar3)\n");
/* Grep: net: eth: soft retwallangle — Wave 76 exclusive wallangle stamp */
kprintf("net: eth: soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=87 (retwallangle stamp; Soft≠product)\n");
/* Grep: net: eth: soft retspireangle — Wave 77 return-spireangle honesty */
kprintf("net: eth: soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=87 (retspireangle honesty; Soft≠product; not bar3)\n");
/* Grep: net: eth: soft retholdangle — Wave 77 exclusive holdangle stamp */
kprintf("net: eth: soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=87 (retholdangle stamp; Soft≠product)\n");
/* Grep: net: eth: soft retpalaceangle — Wave 78 return-palaceangle honesty */
kprintf("net: eth: soft retpalaceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=87 (retpalaceangle honesty; Soft≠product; not bar3)\n");
/* Grep: net: eth: soft retfortressangle — Wave 78 exclusive fortressangle stamp */
kprintf("net: eth: soft retfortressangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=87 (retfortressangle stamp; Soft≠product)\n");
/* Grep: net: eth: soft retkeepangle — Wave 79 return-keepangle honesty */
kprintf("net: eth: soft retkeepangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=87 (retkeepangle honesty; Soft≠product; not bar3)\n");
/* Grep: net: eth: soft retredoubtangle — Wave 79 exclusive redoubtangle stamp */
kprintf("net: eth: soft retredoubtangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=87 (retredoubtangle stamp; Soft≠product)\n");
/* Grep: net: eth: soft retcitadelangle — Wave 80 return-citadelangle honesty */
kprintf("net: eth: soft retcitadelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=87 (retcitadelangle honesty; Soft≠product; not bar3)\n");
/* Grep: net: eth: soft retbastionkeep — Wave 80 exclusive bastionkeep stamp */
kprintf("net: eth: soft retbastionkeep exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=87 (retbastionkeep stamp; Soft≠product)\n");
/* Grep: net: eth: soft retpanoplyangle — Wave 81 return-panoplyangle honesty */
kprintf("net: eth: soft retpanoplyangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=87 (retpanoplyangle honesty; Soft≠product; not bar3)\n");
/* Grep: net: eth: soft retbulwarkangle — Wave 81 exclusive bulwarkangle stamp */
kprintf("net: eth: soft retbulwarkangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=87 (retbulwarkangle stamp; Soft≠product)\n");
/* Grep: net: eth: soft retmantleangle — Wave 82 return-mantleangle honesty */
kprintf("net: eth: soft retmantleangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=87 (retmantleangle honesty; Soft≠product; not bar3)\n");
/* Grep: net: eth: soft retaegisangle — Wave 82 exclusive aegisangle stamp */
kprintf("net: eth: soft retaegisangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=87 (retaegisangle stamp; Soft≠product)\n");
/* Grep: net: eth: soft retemblemangle — Wave 83 return-emblemangle honesty */
kprintf("net: eth: soft retemblemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=87 (retemblemangle honesty; Soft≠product; not bar3)\n");
/* Grep: net: eth: soft retsigilangle — Wave 83 exclusive sigilangle stamp */
kprintf("net: eth: soft retsigilangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=87 (retsigilangle stamp; Soft≠product)\n");
/* Grep: net: eth: soft retscepterangle — Wave 84 return-scepterangle honesty */
kprintf("net: eth: soft retscepterangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=87 (retscepterangle honesty; Soft≠product; not bar3)\n");
/* Grep: net: eth: soft retglyphangle — Wave 84 exclusive glyphangle stamp */
kprintf("net: eth: soft retglyphangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=87 (retglyphangle stamp; Soft≠product)\n");
/* Grep: net: eth: soft retcrownangle — Wave 85 return-crownangle honesty */
kprintf("net: eth: soft retcrownangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=87 (retcrownangle honesty; Soft≠product; not bar3)\n");
/* Grep: net: eth: soft retshardangle — Wave 85 exclusive shardangle stamp */
kprintf("net: eth: soft retshardangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=87 (retshardangle stamp; Soft≠product)\n");
/* Grep: net: eth: soft retforgeangle — Wave 86 return-forgeangle honesty */
kprintf("net: eth: soft retforgeangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=87 (retforgeangle honesty; Soft≠product; not bar3)\n");
/* Grep: net: eth: soft retprismangle — Wave 86 exclusive prismangle stamp */
kprintf("net: eth: soft retprismangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=87 (retprismangle stamp; Soft≠product)\n");
/* Grep: net: eth: soft retflameangle — Wave 87 return-flameangle honesty */
kprintf("net: eth: soft retflameangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=87 (retflameangle honesty; Soft≠product; not bar3)\n");
/* Grep: net: eth: soft retcipherangle — Wave 87 exclusive cipherangle stamp */
kprintf("net: eth: soft retcipherangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=87 (retcipherangle stamp; Soft≠product)\n");

                                kprintf("net: eth soft deepen wave=87 areas=120 verdict=%s ready=%u "
    ...
    kprintf("net_eth: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=87\n");

    /* Grep: net_eth: soft deepen (twin) */
        kprintf("net_eth: soft deepen wave=87 areas=118 verdict=%s ready=%u "
                "rx=%u ok=%u proto=%u logs=%u\n",
                szVerdict, u32Ready, g_u32FramesRx, g_u32FramesOk, u32Proto,
                g_u32SoftLogN);
    }
    ...
    kprintf("net: eth soft PASS wave=87 logs=%u verdict=%s ready=%u "
    ...
    kprintf("net_eth: soft PASS wave=87 logs=%u verdict=%s ready=%u "
            "frames_ok=%u proto=%u "
            "(soft inventory only; not product gate)\n",
            g_u32SoftLogN, szVerdict, u32Ready, g_u32FramesOk, u32Proto);
}
