...
elf_soft_inventory(const char *szVia)
{
    ...
    kprintf("elf: soft surface inventory,probe,load,reloc,reloc_kind,so,"
            ...
            "surface,deepen,catalog,bias,capacity areas=51 wave=%u\n",
            GJ_ELF_SOFT_WAVE);
 ...
 * Soft≠product; not bar3.
    ...
    kprintf("elf_load: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=71\n");
...
kprintf("elf_load: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=71 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: elf_load: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("elf_load: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=71 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("elf_load: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=71 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: elf_load: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("elf_load: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=71 (retreentrantangle stamp; Soft≠product)\n");
