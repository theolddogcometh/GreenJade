...
elf_soft_inventory(const char *szVia)
{
    ...
    kprintf("elf: soft surface inventory,probe,load,reloc,reloc_kind,so,"
            ...
            "surface,deepen,catalog,bias,capacity areas=55 wave=%u\n",
            GJ_ELF_SOFT_WAVE);
 ...
 * Soft≠product; not bar3.
    ...
    kprintf("elf_load: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=73\n");
...
kprintf("elf_load: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=73 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: elf_load: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("elf_load: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=73 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("elf_load: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=73 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: elf_load: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("elf_load: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=73 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("elf_load: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=73 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: elf_load: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("elf_load: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=73 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: elf_load: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("elf_load: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=73 (retcurtainface honesty; Soft≠product; not bar3)\n");
/* Grep: elf_load: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("elf_load: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=73 (retbastionangle stamp; Soft≠product)\n");
