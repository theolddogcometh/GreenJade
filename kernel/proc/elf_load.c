...
elf_soft_inventory(const char *szVia)
{
    ...
    kprintf("elf: soft surface inventory,probe,load,reloc,reloc_kind,so,"
            ...
            "surface,deepen,catalog,bias,capacity areas=47 wave=%u\n",
            GJ_ELF_SOFT_WAVE);
 ...
 * Soft≠product; not bar3.
    ...
    kprintf("elf_load: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=65\n");
