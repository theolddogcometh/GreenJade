 ...
 */
#define GJ_EXCEPT_SOFT_WAVE 96u
...
soft_inventory_log(void)
{
    ...
    kprintf("except: soft ret_surface reg=bind|thr0|dead|null "
            ...
            "product_kernel=OPEN areas=81 wave=%u\n",
            GJ_EXCEPT_SOFT_WAVE);
    ...
    kprintf("except: soft surface inventory,register,deliver,take,drop,"
            ...
            "surface,deepen areas=85 wave=%u\n",
            GJ_EXCEPT_SOFT_WAVE);

    /* Grep: except: soft retmap — Wave 19 return-surface map */
    kprintf("except: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=96\n");
...
kprintf("except: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: except: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("except: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("except: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: except: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("except: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("except: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: except: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("except: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: except: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("except: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retcurtainface honesty; Soft≠product; not bar3)\n");
/* Grep: except: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("except: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retbastionangle stamp; Soft≠product)\n");
/* Grep: except: soft retglacisangle — Wave 74 return-glacisangle honesty */
kprintf("except: soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retglacisangle honesty; Soft≠product; not bar3)\n");
/* Grep: except: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
kprintf("except: soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retparapetangle stamp; Soft≠product)\n");
/* Grep: except: soft retmoatangle — Wave 75 return-moatangle honesty */
kprintf("except: soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retmoatangle honesty; Soft≠product; not bar3)\n");
/* Grep: except: soft retowerangle — Wave 75 exclusive towerangle stamp */
kprintf("except: soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retowerangle stamp; Soft≠product)\n");
/* Grep: except: soft retgateangle — Wave 76 return-gateangle honesty */
kprintf("except: soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retgateangle honesty; Soft≠product; not bar3)\n");
/* Grep: except: soft retwallangle — Wave 76 exclusive wallangle stamp */
kprintf("except: soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retwallangle stamp; Soft≠product)\n");
/* Grep: except: soft retspireangle — Wave 77 return-spireangle honesty */
kprintf("except: soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retspireangle honesty; Soft≠product; not bar3)\n");
/* Grep: except: soft retholdangle — Wave 77 exclusive holdangle stamp */
kprintf("except: soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retholdangle stamp; Soft≠product)\n");
/* Grep: except: soft retpalaceangle — Wave 78 return-palaceangle honesty */
kprintf("except: soft retpalaceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retpalaceangle honesty; Soft≠product; not bar3)\n");
/* Grep: except: soft retfortressangle — Wave 78 exclusive fortressangle stamp */
kprintf("except: soft retfortressangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retfortressangle stamp; Soft≠product)\n");
/* Grep: except: soft retkeepangle — Wave 79 return-keepangle honesty */
kprintf("except: soft retkeepangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retkeepangle honesty; Soft≠product; not bar3)\n");
/* Grep: except: soft retredoubtangle — Wave 79 exclusive redoubtangle stamp */
kprintf("except: soft retredoubtangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retredoubtangle stamp; Soft≠product)\n");
/* Grep: except: soft retcitadelangle — Wave 80 return-citadelangle honesty */
kprintf("except: soft retcitadelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retcitadelangle honesty; Soft≠product; not bar3)\n");
/* Grep: except: soft retbastionkeep — Wave 80 exclusive bastionkeep stamp */
kprintf("except: soft retbastionkeep exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retbastionkeep stamp; Soft≠product)\n");
/* Grep: except: soft retpanoplyangle — Wave 81 return-panoplyangle honesty */
kprintf("except: soft retpanoplyangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retpanoplyangle honesty; Soft≠product; not bar3)\n");
/* Grep: except: soft retbulwarkangle — Wave 81 exclusive bulwarkangle stamp */
kprintf("except: soft retbulwarkangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retbulwarkangle stamp; Soft≠product)\n");
/* Grep: except: soft retmantleangle — Wave 82 return-mantleangle honesty */
kprintf("except: soft retmantleangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retmantleangle honesty; Soft≠product; not bar3)\n");
/* Grep: except: soft retaegisangle — Wave 82 exclusive aegisangle stamp */
kprintf("except: soft retaegisangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retaegisangle stamp; Soft≠product)\n");
/* Grep: except: soft retemblemangle — Wave 83 return-emblemangle honesty */
kprintf("except: soft retemblemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retemblemangle honesty; Soft≠product; not bar3)\n");
/* Grep: except: soft retsigilangle — Wave 83 exclusive sigilangle stamp */
kprintf("except: soft retsigilangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retsigilangle stamp; Soft≠product)\n");
/* Grep: except: soft retscepterangle — Wave 84 return-scepterangle honesty */
kprintf("except: soft retscepterangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retscepterangle honesty; Soft≠product; not bar3)\n");
/* Grep: except: soft retglyphangle — Wave 84 exclusive glyphangle stamp */
kprintf("except: soft retglyphangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retglyphangle stamp; Soft≠product)\n");
/* Grep: except: soft retcrownangle — Wave 85 return-crownangle honesty */
kprintf("except: soft retcrownangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retcrownangle honesty; Soft≠product; not bar3)\n");
/* Grep: except: soft retshardangle — Wave 85 exclusive shardangle stamp */
kprintf("except: soft retshardangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retshardangle stamp; Soft≠product)\n");
/* Grep: except: soft retforgeangle — Wave 86 return-forgeangle honesty */
kprintf("except: soft retforgeangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retforgeangle honesty; Soft≠product; not bar3)\n");
/* Grep: except: soft retprismangle — Wave 86 exclusive prismangle stamp */
kprintf("except: soft retprismangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retprismangle stamp; Soft≠product)\n");
/* Grep: except: soft retflameangle — Wave 87 return-flameangle honesty */
kprintf("except: soft retflameangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retflameangle honesty; Soft≠product; not bar3)\n");
/* Grep: except: soft retcipherangle — Wave 87 exclusive cipherangle stamp */
kprintf("except: soft retcipherangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retcipherangle stamp; Soft≠product)\n");
/* Grep: except: soft retbeaconangle — Wave 88 return-beaconangle honesty */
kprintf("except: soft retbeaconangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retbeaconangle honesty; Soft≠product; not bar3)\n");
/* Grep: except: soft retledgerangle — Wave 88 exclusive ledgerangle stamp */
kprintf("except: soft retledgerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retledgerangle stamp; Soft≠product)\n");
/* Grep: except: soft retbannerangle — Wave 89 return-bannerangle honesty */
kprintf("except: soft retbannerangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retbannerangle honesty; Soft≠product; not bar3)\n");
/* Grep: except: soft retvaultangle — Wave 89 exclusive vaultangle stamp */
kprintf("except: soft retvaultangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retvaultangle stamp; Soft≠product)\n");
/* Grep: except: soft retcrestangle — Wave 90 return-crestangle honesty */
kprintf("except: soft retcrestangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retcrestangle honesty; Soft≠product; not bar3)\n");
/* Grep: except: soft rettokenangle — Wave 90 exclusive tokenangle stamp */
kprintf("except: soft rettokenangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (rettokenangle stamp; Soft≠product)\n");
/* Grep: except: soft retbadgeangle — Wave 91 return-badgeangle honesty */
kprintf("except: soft retbadgeangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retbadgeangle honesty; Soft≠product; not bar3)\n");
/* Grep: except: soft retphaseangle — Wave 91 exclusive phaseangle stamp */
kprintf("except: soft retphaseangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retphaseangle stamp; Soft≠product)\n");
/* Grep: except: soft retmarkangle — Wave 92 return-markangle honesty */
kprintf("except: soft retmarkangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retmarkangle honesty; Soft≠product; not bar3)\n");
/* Grep: except: soft retpulseangle — Wave 92 exclusive pulseangle stamp */
kprintf("except: soft retpulseangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retpulseangle stamp; Soft≠product)\n");

/* Grep: except: soft retsealangle — Wave 93 return-sealangle honesty */
kprintf("except: soft retsealangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retsealangle honesty; Soft≠product; not bar3)\n");
/* Grep: except: soft retboundangle — Wave 93 exclusive boundangle stamp */
kprintf("except: soft retboundangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retboundangle stamp; Soft≠product)\n");
/* Grep: except: soft retstemangle — Wave 94 return-stemangle honesty */
kprintf("except: soft retstemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retstemangle honesty; Soft≠product; not bar3)\n");
/* Grep: except: soft retbladeangle — Wave 94 exclusive bladeangle stamp */
kprintf("except: soft retbladeangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retbladeangle stamp; Soft≠product)\n");
/* Grep: except: soft retchordangle — Wave 95 return-chordangle honesty */
kprintf("except: soft retchordangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retchordangle honesty; Soft≠product; not bar3)\n");
/* Grep: except: soft retarcangle — Wave 95 exclusive arcangle stamp */
kprintf("except: soft retarcangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retarcangle stamp; Soft≠product)\n");
/* Grep: except: soft retsectorangle — Wave 96 return-sectorangle honesty */
kprintf("except: soft retsectorangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=96 (retsectorangle honesty; Soft≠product; not bar3)\n");
/* Grep: except: soft retwedgeangle — Wave 96 exclusive wedgeangle stamp */
kprintf("except: soft retwedgeangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=96 (retwedgeangle stamp; Soft≠product)\n");
