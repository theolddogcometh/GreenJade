...
user_soft_inventory(const char *szVia)
{
    ...
    kprintf("user: soft ret_surface map=ring3_ok|ring3_fail|pers_ok|pers_fail "
            ...
            "recheck=pass|fail product_kernel=OPEN areas=27 wave=%u\n",
            GJ_USER_SOFT_WAVE);
 ...
 * Soft≠product; not bar3.
    ...
    kprintf("user_task: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=58\n");
