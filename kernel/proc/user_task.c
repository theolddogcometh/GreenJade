...
user_soft_inventory(const char *szVia)
{
    ...
    kprintf("user: soft ret_surface map=ring3_ok|ring3_fail|pers_ok|pers_fail "
            ...
            "recheck=pass|fail product_kernel=OPEN areas=61 wave=%u\n",
            GJ_USER_SOFT_WAVE);
 ...
 * Soft≠product; not bar3.
    ...
    kprintf("user_task: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=86\n");
...
kprintf("user_task: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=86 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: user_task: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("user_task: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=86 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("user_task: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=86 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: user_task: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("user_task: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=86 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("user_task: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=86 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: user_task: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("user_task: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=86 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: user_task: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("user_task: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=86 (retcurtainface honesty; Soft≠product; not bar3)\n");
/* Grep: user_task: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("user_task: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=86 (retbastionangle stamp; Soft≠product)\n");
/* Grep: user_task: soft retglacisangle — Wave 74 return-glacisangle honesty */
kprintf("user_task: soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=86 (retglacisangle honesty; Soft≠product; not bar3)\n");
/* Grep: user_task: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
kprintf("user_task: soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=86 (retparapetangle stamp; Soft≠product)\n");
/* Grep: user_task: soft retmoatangle — Wave 75 return-moatangle honesty */
kprintf("user_task: soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=86 (retmoatangle honesty; Soft≠product; not bar3)\n");
/* Grep: user_task: soft retowerangle — Wave 75 exclusive towerangle stamp */
kprintf("user_task: soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=86 (retowerangle stamp; Soft≠product)\n");
/* Grep: user_task: soft retgateangle — Wave 76 return-gateangle honesty */
kprintf("user_task: soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=86 (retgateangle honesty; Soft≠product; not bar3)\n");
/* Grep: user_task: soft retwallangle — Wave 76 exclusive wallangle stamp */
kprintf("user_task: soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=86 (retwallangle stamp; Soft≠product)\n");
/* Grep: user_task: soft retspireangle — Wave 77 return-spireangle honesty */
kprintf("user_task: soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=86 (retspireangle honesty; Soft≠product; not bar3)\n");
/* Grep: user_task: soft retholdangle — Wave 77 exclusive holdangle stamp */
kprintf("user_task: soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=86 (retholdangle stamp; Soft≠product)\n");
/* Grep: user_task: soft retpalaceangle — Wave 78 return-palaceangle honesty */
kprintf("user_task: soft retpalaceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=86 (retpalaceangle honesty; Soft≠product; not bar3)\n");
/* Grep: user_task: soft retfortressangle — Wave 78 exclusive fortressangle stamp */
kprintf("user_task: soft retfortressangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=86 (retfortressangle stamp; Soft≠product)\n");
/* Grep: user_task: soft retkeepangle — Wave 79 return-keepangle honesty */
kprintf("user_task: soft retkeepangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=86 (retkeepangle honesty; Soft≠product; not bar3)\n");
/* Grep: user_task: soft retredoubtangle — Wave 79 exclusive redoubtangle stamp */
kprintf("user_task: soft retredoubtangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=86 (retredoubtangle stamp; Soft≠product)\n");
/* Grep: user_task: soft retcitadelangle — Wave 80 return-citadelangle honesty */
kprintf("user_task: soft retcitadelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=86 (retcitadelangle honesty; Soft≠product; not bar3)\n");
/* Grep: user_task: soft retbastionkeep — Wave 80 exclusive bastionkeep stamp */
kprintf("user_task: soft retbastionkeep exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=86 (retbastionkeep stamp; Soft≠product)\n");
/* Grep: user_task: soft retpanoplyangle — Wave 81 return-panoplyangle honesty */
kprintf("user_task: soft retpanoplyangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=86 (retpanoplyangle honesty; Soft≠product; not bar3)\n");
/* Grep: user_task: soft retbulwarkangle — Wave 81 exclusive bulwarkangle stamp */
kprintf("user_task: soft retbulwarkangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=86 (retbulwarkangle stamp; Soft≠product)\n");
/* Grep: user_task: soft retmantleangle — Wave 82 return-mantleangle honesty */
kprintf("user_task: soft retmantleangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=86 (retmantleangle honesty; Soft≠product; not bar3)\n");
/* Grep: user_task: soft retaegisangle — Wave 82 exclusive aegisangle stamp */
kprintf("user_task: soft retaegisangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=86 (retaegisangle stamp; Soft≠product)\n");
/* Grep: user_task: soft retemblemangle — Wave 83 return-emblemangle honesty */
kprintf("user_task: soft retemblemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=86 (retemblemangle honesty; Soft≠product; not bar3)\n");
/* Grep: user_task: soft retsigilangle — Wave 83 exclusive sigilangle stamp */
kprintf("user_task: soft retsigilangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=86 (retsigilangle stamp; Soft≠product)\n");
/* Grep: user_task: soft retscepterangle — Wave 84 return-scepterangle honesty */
kprintf("user_task: soft retscepterangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=86 (retscepterangle honesty; Soft≠product; not bar3)\n");
/* Grep: user_task: soft retglyphangle — Wave 84 exclusive glyphangle stamp */
kprintf("user_task: soft retglyphangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=86 (retglyphangle stamp; Soft≠product)\n");
/* Grep: user_task: soft retcrownangle — Wave 85 return-crownangle honesty */
kprintf("user_task: soft retcrownangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=86 (retcrownangle honesty; Soft≠product; not bar3)\n");
/* Grep: user_task: soft retshardangle — Wave 85 exclusive shardangle stamp */
kprintf("user_task: soft retshardangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=86 (retshardangle stamp; Soft≠product)\n");
/* Grep: user_task: soft retforgeangle — Wave 86 return-forgeangle honesty */
kprintf("user_task: soft retforgeangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=86 (retforgeangle honesty; Soft≠product; not bar3)\n");
/* Grep: user_task: soft retprismangle — Wave 86 exclusive prismangle stamp */
kprintf("user_task: soft retprismangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=86 (retprismangle stamp; Soft≠product)\n");
