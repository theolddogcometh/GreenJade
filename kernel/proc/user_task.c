...
user_soft_inventory(const char *szVia)
{
    ...
    kprintf("user: soft ret_surface map=ring3_ok|ring3_fail|pers_ok|pers_fail "
            ...
            "recheck=pass|fail product_kernel=OPEN areas=55 wave=%u\n",
            GJ_USER_SOFT_WAVE);
 ...
 * Soft≠product; not bar3.
    ...
    kprintf("user_task: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=83\n");
...
kprintf("user_task: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: user_task: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("user_task: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("user_task: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: user_task: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("user_task: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("user_task: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: user_task: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("user_task: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: user_task: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("user_task: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retcurtainface honesty; Soft≠product; not bar3)\n");
/* Grep: user_task: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("user_task: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retbastionangle stamp; Soft≠product)\n");
/* Grep: user_task: soft retglacisangle — Wave 74 return-glacisangle honesty */
kprintf("user_task: soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retglacisangle honesty; Soft≠product; not bar3)\n");
/* Grep: user_task: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
kprintf("user_task: soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retparapetangle stamp; Soft≠product)\n");
/* Grep: user_task: soft retmoatangle — Wave 75 return-moatangle honesty */
kprintf("user_task: soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retmoatangle honesty; Soft≠product; not bar3)\n");
/* Grep: user_task: soft retowerangle — Wave 75 exclusive towerangle stamp */
kprintf("user_task: soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retowerangle stamp; Soft≠product)\n");
/* Grep: user_task: soft retgateangle — Wave 76 return-gateangle honesty */
kprintf("user_task: soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retgateangle honesty; Soft≠product; not bar3)\n");
/* Grep: user_task: soft retwallangle — Wave 76 exclusive wallangle stamp */
kprintf("user_task: soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retwallangle stamp; Soft≠product)\n");
/* Grep: user_task: soft retspireangle — Wave 77 return-spireangle honesty */
kprintf("user_task: soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retspireangle honesty; Soft≠product; not bar3)\n");
/* Grep: user_task: soft retholdangle — Wave 77 exclusive holdangle stamp */
kprintf("user_task: soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retholdangle stamp; Soft≠product)\n");
/* Grep: user_task: soft retpalaceangle — Wave 78 return-palaceangle honesty */
kprintf("user_task: soft retpalaceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retpalaceangle honesty; Soft≠product; not bar3)\n");
/* Grep: user_task: soft retfortressangle — Wave 78 exclusive fortressangle stamp */
kprintf("user_task: soft retfortressangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retfortressangle stamp; Soft≠product)\n");
/* Grep: user_task: soft retkeepangle — Wave 79 return-keepangle honesty */
kprintf("user_task: soft retkeepangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retkeepangle honesty; Soft≠product; not bar3)\n");
/* Grep: user_task: soft retredoubtangle — Wave 79 exclusive redoubtangle stamp */
kprintf("user_task: soft retredoubtangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retredoubtangle stamp; Soft≠product)\n");
/* Grep: user_task: soft retcitadelangle — Wave 80 return-citadelangle honesty */
kprintf("user_task: soft retcitadelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retcitadelangle honesty; Soft≠product; not bar3)\n");
/* Grep: user_task: soft retbastionkeep — Wave 80 exclusive bastionkeep stamp */
kprintf("user_task: soft retbastionkeep exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retbastionkeep stamp; Soft≠product)\n");
/* Grep: user_task: soft retpanoplyangle — Wave 81 return-panoplyangle honesty */
kprintf("user_task: soft retpanoplyangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retpanoplyangle honesty; Soft≠product; not bar3)\n");
/* Grep: user_task: soft retbulwarkangle — Wave 81 exclusive bulwarkangle stamp */
kprintf("user_task: soft retbulwarkangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retbulwarkangle stamp; Soft≠product)\n");
/* Grep: user_task: soft retmantleangle — Wave 82 return-mantleangle honesty */
kprintf("user_task: soft retmantleangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retmantleangle honesty; Soft≠product; not bar3)\n");
/* Grep: user_task: soft retaegisangle — Wave 82 exclusive aegisangle stamp */
kprintf("user_task: soft retaegisangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retaegisangle stamp; Soft≠product)\n");
/* Grep: user_task: soft retemblemangle — Wave 83 return-emblemangle honesty */
kprintf("user_task: soft retemblemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=83 (retemblemangle honesty; Soft≠product; not bar3)\n");
/* Grep: user_task: soft retsigilangle — Wave 83 exclusive sigilangle stamp */
kprintf("user_task: soft retsigilangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=83 (retsigilangle stamp; Soft≠product)\n");
