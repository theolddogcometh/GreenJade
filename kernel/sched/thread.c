 ...
 *
 * Soft product deepen (Wave 20 exclusive; this unit only):
 ...
 *   - Wave 19 greppable "thread: soft …" deepen (wave=20 stamp):
...
#define THREAD_SOFT_DEEPEN_WAVE  20u
/* Fixed greppable categories emitted under "thread: soft …". */
#define THREAD_SOFT_DEEPEN_AREAS 37u
...
sched_soft_inventory_print(void)
{
    ...
    kprintf("thread: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=20\n");

    /* Grep: thread: soft deepen wave (Wave 20 stamp) */
    /*
     ...
     * Soft≠product; not bar3.
     */
    /* Grep: thread: soft retbound — Wave 20 return-bound honesty */
    kprintf("thread: soft retbound soft_only=1 product_gate=0 hard_gate=0 "
            "never_blocks_m0=1 wave=%u "
            "(retbound honesty; Soft≠product; not bar3)\n",
            (unsigned)THREAD_SOFT_DEEPEN_WAVE);
    /* Grep: thread: soft retseal — Wave 20 exclusive seal stamp */
