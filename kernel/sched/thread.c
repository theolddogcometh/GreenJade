 ...
 *
 * Soft product deepen (Wave 21 exclusive; this unit only):
 ...
 *   - Wave 19 greppable "thread: soft …" deepen (wave=21 stamp):
...
#define THREAD_SOFT_DEEPEN_WAVE  21u
/* Fixed greppable categories emitted under "thread: soft …". */
#define THREAD_SOFT_DEEPEN_AREAS 39u
...
sched_soft_inventory_print(void)
{
    ...
    kprintf("thread: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=21\n");

    /* Grep: thread: soft deepen wave (Wave 21 stamp) */
    /*
    ...
    kprintf("thread: soft retseal exclusive=1 soft_ne_product=1 "
             ...
             * Soft≠product; not bar3.
            */
            /* Grep: thread: soft retpulse — Wave 21 return-pulse honesty */
            kprintf("thread: soft retpulse soft_only=1 product_gate=0 soft_ne_product=1 "
                    "never_blocks_m0=1 wave=%u "
                    "(retpulse honesty; Soft≠product; not bar3)\n",
                    (unsigned)THREAD_SOFT_DEEPEN_WAVE);
            /* Grep: thread: soft retmark — Wave 21 exclusive mark stamp */
