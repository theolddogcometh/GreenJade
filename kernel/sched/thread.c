 ...
 *   - Wave 19 greppable "thread: soft …" deepen (wave=85 stamp):
...
#define THREAD_SOFT_DEEPEN_AREAS 123u
...
sched_soft_inventory_print(void)
{
    ...
    kprintf("thread: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=85\n");
...
kprintf("thread: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: thread: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("thread: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("thread: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: thread: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("thread: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("thread: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: thread: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("thread: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: thread: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("thread: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retcurtainface honesty; Soft≠product; not bar3)\n");
/* Grep: thread: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("thread: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retbastionangle stamp; Soft≠product)\n");
/* Grep: thread: soft retglacisangle — Wave 74 return-glacisangle honesty */
kprintf("thread: soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retglacisangle honesty; Soft≠product; not bar3)\n");
/* Grep: thread: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
kprintf("thread: soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retparapetangle stamp; Soft≠product)\n");
/* Grep: thread: soft retmoatangle — Wave 75 return-moatangle honesty */
kprintf("thread: soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retmoatangle honesty; Soft≠product; not bar3)\n");
/* Grep: thread: soft retowerangle — Wave 75 exclusive towerangle stamp */
kprintf("thread: soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retowerangle stamp; Soft≠product)\n");
/* Grep: thread: soft retgateangle — Wave 76 return-gateangle honesty */
kprintf("thread: soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retgateangle honesty; Soft≠product; not bar3)\n");
/* Grep: thread: soft retwallangle — Wave 76 exclusive wallangle stamp */
kprintf("thread: soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retwallangle stamp; Soft≠product)\n");
/* Grep: thread: soft retspireangle — Wave 77 return-spireangle honesty */
kprintf("thread: soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retspireangle honesty; Soft≠product; not bar3)\n");
/* Grep: thread: soft retholdangle — Wave 77 exclusive holdangle stamp */
kprintf("thread: soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retholdangle stamp; Soft≠product)\n");
/* Grep: thread: soft retpalaceangle — Wave 78 return-palaceangle honesty */
kprintf("thread: soft retpalaceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retpalaceangle honesty; Soft≠product; not bar3)\n");
/* Grep: thread: soft retfortressangle — Wave 78 exclusive fortressangle stamp */
kprintf("thread: soft retfortressangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retfortressangle stamp; Soft≠product)\n");
/* Grep: thread: soft retkeepangle — Wave 79 return-keepangle honesty */
kprintf("thread: soft retkeepangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retkeepangle honesty; Soft≠product; not bar3)\n");
/* Grep: thread: soft retredoubtangle — Wave 79 exclusive redoubtangle stamp */
kprintf("thread: soft retredoubtangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retredoubtangle stamp; Soft≠product)\n");
/* Grep: thread: soft retcitadelangle — Wave 80 return-citadelangle honesty */
kprintf("thread: soft retcitadelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retcitadelangle honesty; Soft≠product; not bar3)\n");
/* Grep: thread: soft retbastionkeep — Wave 80 exclusive bastionkeep stamp */
kprintf("thread: soft retbastionkeep exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retbastionkeep stamp; Soft≠product)\n");
/* Grep: thread: soft retpanoplyangle — Wave 81 return-panoplyangle honesty */
kprintf("thread: soft retpanoplyangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retpanoplyangle honesty; Soft≠product; not bar3)\n");
/* Grep: thread: soft retbulwarkangle — Wave 81 exclusive bulwarkangle stamp */
kprintf("thread: soft retbulwarkangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retbulwarkangle stamp; Soft≠product)\n");
/* Grep: thread: soft retmantleangle — Wave 82 return-mantleangle honesty */
kprintf("thread: soft retmantleangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retmantleangle honesty; Soft≠product; not bar3)\n");
/* Grep: thread: soft retaegisangle — Wave 82 exclusive aegisangle stamp */
kprintf("thread: soft retaegisangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retaegisangle stamp; Soft≠product)\n");
/* Grep: thread: soft retemblemangle — Wave 83 return-emblemangle honesty */
kprintf("thread: soft retemblemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retemblemangle honesty; Soft≠product; not bar3)\n");
/* Grep: thread: soft retsigilangle — Wave 83 exclusive sigilangle stamp */
kprintf("thread: soft retsigilangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retsigilangle stamp; Soft≠product)\n");
/* Grep: thread: soft retscepterangle — Wave 84 return-scepterangle honesty */
kprintf("thread: soft retscepterangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retscepterangle honesty; Soft≠product; not bar3)\n");
/* Grep: thread: soft retglyphangle — Wave 84 exclusive glyphangle stamp */
kprintf("thread: soft retglyphangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retglyphangle stamp; Soft≠product)\n");
/* Grep: thread: soft retcrownangle — Wave 85 return-crownangle honesty */
kprintf("thread: soft retcrownangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retcrownangle honesty; Soft≠product; not bar3)\n");
/* Grep: thread: soft retshardangle — Wave 85 exclusive shardangle stamp */
kprintf("thread: soft retshardangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retshardangle stamp; Soft≠product)\n");
