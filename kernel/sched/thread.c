 ...
 *
 * Soft product deepen (Wave 19 exclusive; this unit only):
 ...
 *   - Wave 19 greppable "thread: soft …" deepen (wave=19 stamp):
...
#define THREAD_SOFT_DEEPEN_WAVE  19u
/* Fixed greppable categories emitted under "thread: soft …". */
#define THREAD_SOFT_DEEPEN_AREAS 35u
...
sched_soft_inventory_print(void)
{
    ...
    kprintf("thread: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=19\n");

    /* Grep: thread: soft deepen wave (Wave 19 stamp) */
    /*
     * ---- Wave 19 exclusive complementary surfaces (never reshape primary).
     * Return surfaces only — soft inventory; never hard-gates product paths.
     * Soft≠product; not bar3.
     */
    /* Grep: thread: soft retclass — Wave 19 return-class taxonomy */
    kprintf("thread: soft retclass ok|fail|inval|nodev|busy|nomem "
            "soft_only=1 product_gate=0 wave=%u "
            "(retclass taxonomy; Soft≠product; not bar3)\n",
            (unsigned)THREAD_SOFT_DEEPEN_WAVE);
    /* Grep: thread: soft retlane — Wave 19 return-lane catalog */
