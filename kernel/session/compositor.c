...
soft_inventory_log(void)
{
    ...
    kprintf("compositor: soft catalog honesty,inventory,init,present,batch,"
            ...
            "wave=%u areas_expect=59 soft PASS\n",
            GJ_COMP_SOFT_WAVE);
    cAreas++;
    ...
    kprintf("compositor: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=85\n");
...
kprintf("compositor: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("compositor: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("compositor: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("compositor: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("compositor: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("compositor: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: compositor: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("compositor: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retcurtainface honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("compositor: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retbastionangle stamp; Soft≠product)\n");
/* Grep: compositor: soft retglacisangle — Wave 74 return-glacisangle honesty */
kprintf("compositor: soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retglacisangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
kprintf("compositor: soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retparapetangle stamp; Soft≠product)\n");
/* Grep: compositor: soft retmoatangle — Wave 75 return-moatangle honesty */
kprintf("compositor: soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retmoatangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retowerangle — Wave 75 exclusive towerangle stamp */
kprintf("compositor: soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retowerangle stamp; Soft≠product)\n");
/* Grep: compositor: soft retgateangle — Wave 76 return-gateangle honesty */
kprintf("compositor: soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retgateangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retwallangle — Wave 76 exclusive wallangle stamp */
kprintf("compositor: soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retwallangle stamp; Soft≠product)\n");
/* Grep: compositor: soft retspireangle — Wave 77 return-spireangle honesty */
kprintf("compositor: soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retspireangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retholdangle — Wave 77 exclusive holdangle stamp */
kprintf("compositor: soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retholdangle stamp; Soft≠product)\n");
/* Grep: compositor: soft retpalaceangle — Wave 78 return-palaceangle honesty */
kprintf("compositor: soft retpalaceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retpalaceangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retfortressangle — Wave 78 exclusive fortressangle stamp */
kprintf("compositor: soft retfortressangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retfortressangle stamp; Soft≠product)\n");
/* Grep: compositor: soft retkeepangle — Wave 79 return-keepangle honesty */
kprintf("compositor: soft retkeepangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retkeepangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retredoubtangle — Wave 79 exclusive redoubtangle stamp */
kprintf("compositor: soft retredoubtangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retredoubtangle stamp; Soft≠product)\n");
/* Grep: compositor: soft retcitadelangle — Wave 80 return-citadelangle honesty */
kprintf("compositor: soft retcitadelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retcitadelangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retbastionkeep — Wave 80 exclusive bastionkeep stamp */
kprintf("compositor: soft retbastionkeep exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retbastionkeep stamp; Soft≠product)\n");
/* Grep: compositor: soft retpanoplyangle — Wave 81 return-panoplyangle honesty */
kprintf("compositor: soft retpanoplyangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retpanoplyangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retbulwarkangle — Wave 81 exclusive bulwarkangle stamp */
kprintf("compositor: soft retbulwarkangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retbulwarkangle stamp; Soft≠product)\n");
/* Grep: compositor: soft retmantleangle — Wave 82 return-mantleangle honesty */
kprintf("compositor: soft retmantleangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retmantleangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retaegisangle — Wave 82 exclusive aegisangle stamp */
kprintf("compositor: soft retaegisangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retaegisangle stamp; Soft≠product)\n");
/* Grep: compositor: soft retemblemangle — Wave 83 return-emblemangle honesty */
kprintf("compositor: soft retemblemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retemblemangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retsigilangle — Wave 83 exclusive sigilangle stamp */
kprintf("compositor: soft retsigilangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retsigilangle stamp; Soft≠product)\n");
/* Grep: compositor: soft retscepterangle — Wave 84 return-scepterangle honesty */
kprintf("compositor: soft retscepterangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retscepterangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retglyphangle — Wave 84 exclusive glyphangle stamp */
kprintf("compositor: soft retglyphangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retglyphangle stamp; Soft≠product)\n");
/* Grep: compositor: soft retcrownangle — Wave 85 return-crownangle honesty */
kprintf("compositor: soft retcrownangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retcrownangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retshardangle — Wave 85 exclusive shardangle stamp */
kprintf("compositor: soft retshardangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retshardangle stamp; Soft≠product)\n");
