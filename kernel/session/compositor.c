...
soft_inventory_log(void)
{
    ...
    kprintf("compositor: soft catalog honesty,inventory,init,present,batch,"
            ...
            "wave=%u areas_expect=79 soft PASS\n",
            GJ_COMP_SOFT_WAVE);
    cAreas++;
    ...
    kprintf("compositor: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=100\n");
...
kprintf("compositor: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=100 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("compositor: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=100 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("compositor: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=100 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("compositor: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=100 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("compositor: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=100 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("compositor: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=100 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: compositor: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("compositor: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=100 (retcurtainface honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("compositor: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=100 (retbastionangle stamp; Soft≠product)\n");
/* Grep: compositor: soft retglacisangle — Wave 74 return-glacisangle honesty */
kprintf("compositor: soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=100 (retglacisangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
kprintf("compositor: soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=100 (retparapetangle stamp; Soft≠product)\n");
/* Grep: compositor: soft retmoatangle — Wave 75 return-moatangle honesty */
kprintf("compositor: soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=100 (retmoatangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retowerangle — Wave 75 exclusive towerangle stamp */
kprintf("compositor: soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=100 (retowerangle stamp; Soft≠product)\n");
/* Grep: compositor: soft retgateangle — Wave 76 return-gateangle honesty */
kprintf("compositor: soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=100 (retgateangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retwallangle — Wave 76 exclusive wallangle stamp */
kprintf("compositor: soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=100 (retwallangle stamp; Soft≠product)\n");
/* Grep: compositor: soft retspireangle — Wave 77 return-spireangle honesty */
kprintf("compositor: soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=100 (retspireangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retholdangle — Wave 77 exclusive holdangle stamp */
kprintf("compositor: soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=100 (retholdangle stamp; Soft≠product)\n");
/* Grep: compositor: soft retpalaceangle — Wave 78 return-palaceangle honesty */
kprintf("compositor: soft retpalaceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=100 (retpalaceangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retfortressangle — Wave 78 exclusive fortressangle stamp */
kprintf("compositor: soft retfortressangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=100 (retfortressangle stamp; Soft≠product)\n");
/* Grep: compositor: soft retkeepangle — Wave 79 return-keepangle honesty */
kprintf("compositor: soft retkeepangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=100 (retkeepangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retredoubtangle — Wave 79 exclusive redoubtangle stamp */
kprintf("compositor: soft retredoubtangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=100 (retredoubtangle stamp; Soft≠product)\n");
/* Grep: compositor: soft retcitadelangle — Wave 80 return-citadelangle honesty */
kprintf("compositor: soft retcitadelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=100 (retcitadelangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retbastionkeep — Wave 80 exclusive bastionkeep stamp */
kprintf("compositor: soft retbastionkeep exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=100 (retbastionkeep stamp; Soft≠product)\n");
/* Grep: compositor: soft retpanoplyangle — Wave 81 return-panoplyangle honesty */
kprintf("compositor: soft retpanoplyangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=100 (retpanoplyangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retbulwarkangle — Wave 81 exclusive bulwarkangle stamp */
kprintf("compositor: soft retbulwarkangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=100 (retbulwarkangle stamp; Soft≠product)\n");
/* Grep: compositor: soft retmantleangle — Wave 82 return-mantleangle honesty */
kprintf("compositor: soft retmantleangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=100 (retmantleangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retaegisangle — Wave 82 exclusive aegisangle stamp */
kprintf("compositor: soft retaegisangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=100 (retaegisangle stamp; Soft≠product)\n");
/* Grep: compositor: soft retemblemangle — Wave 83 return-emblemangle honesty */
kprintf("compositor: soft retemblemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=100 (retemblemangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retsigilangle — Wave 83 exclusive sigilangle stamp */
kprintf("compositor: soft retsigilangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=100 (retsigilangle stamp; Soft≠product)\n");
/* Grep: compositor: soft retscepterangle — Wave 84 return-scepterangle honesty */
kprintf("compositor: soft retscepterangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=100 (retscepterangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retglyphangle — Wave 84 exclusive glyphangle stamp */
kprintf("compositor: soft retglyphangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=100 (retglyphangle stamp; Soft≠product)\n");
/* Grep: compositor: soft retcrownangle — Wave 85 return-crownangle honesty */
kprintf("compositor: soft retcrownangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=100 (retcrownangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retshardangle — Wave 85 exclusive shardangle stamp */
kprintf("compositor: soft retshardangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=100 (retshardangle stamp; Soft≠product)\n");
/* Grep: compositor: soft retforgeangle — Wave 86 return-forgeangle honesty */
kprintf("compositor: soft retforgeangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=100 (retforgeangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retprismangle — Wave 86 exclusive prismangle stamp */
kprintf("compositor: soft retprismangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=100 (retprismangle stamp; Soft≠product)\n");
/* Grep: compositor: soft retflameangle — Wave 87 return-flameangle honesty */
kprintf("compositor: soft retflameangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=100 (retflameangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retcipherangle — Wave 87 exclusive cipherangle stamp */
kprintf("compositor: soft retcipherangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=100 (retcipherangle stamp; Soft≠product)\n");
/* Grep: compositor: soft retbeaconangle — Wave 88 return-beaconangle honesty */
kprintf("compositor: soft retbeaconangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=100 (retbeaconangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retledgerangle — Wave 88 exclusive ledgerangle stamp */
kprintf("compositor: soft retledgerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=100 (retledgerangle stamp; Soft≠product)\n");
/* Grep: compositor: soft retbannerangle — Wave 89 return-bannerangle honesty */
kprintf("compositor: soft retbannerangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=100 (retbannerangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retvaultangle — Wave 89 exclusive vaultangle stamp */
kprintf("compositor: soft retvaultangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=100 (retvaultangle stamp; Soft≠product)\n");
/* Grep: compositor: soft retcrestangle — Wave 90 return-crestangle honesty */
kprintf("compositor: soft retcrestangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=100 (retcrestangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft rettokenangle — Wave 90 exclusive tokenangle stamp */
kprintf("compositor: soft rettokenangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=100 (rettokenangle stamp; Soft≠product)\n");
/* Grep: compositor: soft retbadgeangle — Wave 91 return-badgeangle honesty */
kprintf("compositor: soft retbadgeangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=100 (retbadgeangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retphaseangle — Wave 91 exclusive phaseangle stamp */
kprintf("compositor: soft retphaseangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=100 (retphaseangle stamp; Soft≠product)\n");
/* Grep: compositor: soft retmarkangle — Wave 92 return-markangle honesty */
kprintf("compositor: soft retmarkangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=100 (retmarkangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retpulseangle — Wave 92 exclusive pulseangle stamp */
kprintf("compositor: soft retpulseangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=100 (retpulseangle stamp; Soft≠product)\n");

/* Grep: compositor: soft retsealangle — Wave 93 return-sealangle honesty */
kprintf("compositor: soft retsealangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=100 (retsealangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retboundangle — Wave 93 exclusive boundangle stamp */
kprintf("compositor: soft retboundangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=100 (retboundangle stamp; Soft≠product)\n");
/* Grep: compositor: soft retstemangle — Wave 94 return-stemangle honesty */
kprintf("compositor: soft retstemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=100 (retstemangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retbladeangle — Wave 94 exclusive bladeangle stamp */
kprintf("compositor: soft retbladeangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=100 (retbladeangle stamp; Soft≠product)\n");
/* Grep: compositor: soft retchordangle — Wave 95 return-chordangle honesty */
kprintf("compositor: soft retchordangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=100 (retchordangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retarcangle — Wave 95 exclusive arcangle stamp */
kprintf("compositor: soft retarcangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=100 (retarcangle stamp; Soft≠product)\n");
/* Grep: compositor: soft retsectorangle — Wave 96 return-sectorangle honesty */
kprintf("compositor: soft retsectorangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=100 (retsectorangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retwedgeangle — Wave 96 exclusive wedgeangle stamp */
kprintf("compositor: soft retwedgeangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=100 (retwedgeangle stamp; Soft≠product)\n");
/* Grep: compositor: soft retradiusangle — Wave 97 return-radiusangle honesty */
kprintf("compositor: soft retradiusangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=100 (retradiusangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retdiameterangle — Wave 97 exclusive diameterangle stamp */
kprintf("compositor: soft retdiameterangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=100 (retdiameterangle stamp; Soft≠product)\n");
/* Grep: compositor: soft retcircumangle — Wave 98 return-circumangle honesty */
kprintf("compositor: soft retcircumangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=100 (retcircumangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retellipseangle — Wave 98 exclusive ellipseangle stamp */
kprintf("compositor: soft retellipseangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=100 (retellipseangle stamp; Soft≠product)\n");
/* Grep: compositor: soft rethyperangle — Wave 99 return-hyperangle honesty */
kprintf("compositor: soft rethyperangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=100 (rethyperangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retparabolaangle — Wave 99 exclusive parabolaangle stamp */
kprintf("compositor: soft retparabolaangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=100 (retparabolaangle stamp; Soft≠product)\n");
/* Grep: compositor: soft retspiralangle — Wave 100 return-spiralangle honesty */
kprintf("compositor: soft retspiralangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=100 (retspiralangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft rethelixangle — Wave 100 exclusive helixangle stamp */
kprintf("compositor: soft rethelixangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=100 (rethelixangle stamp; Soft≠product)\n");
