...
soft_inventory_log(void)
{
    ...
    kprintf("compositor: soft catalog honesty,inventory,init,present,batch,"
            ...
            "wave=%u areas_expect=49 soft PASS\n",
            GJ_COMP_SOFT_WAVE);
    cAreas++;
    ...
    kprintf("compositor: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=72\n");
...
kprintf("compositor: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=72 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("compositor: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=72 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("compositor: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=72 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("compositor: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=72 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("compositor: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=72 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: compositor: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("compositor: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=72 (retdoubletenailleangle stamp; Soft≠product)\n");
