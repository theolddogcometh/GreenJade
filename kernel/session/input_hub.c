 ...
 *
 * Soft input hub inventory (Wave 33 exclusive deepen; this unit only):
...
soft_inventory_log(void)
{
    ...
    kprintf("input_hub: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=33\n");
 ...
 * Soft≠product; not bar3.
 */
/* Grep: input: soft retbastion — Wave 33 return-bastion honesty */
kprintf("input: soft retbastion soft_only=1 product_gate=0 soft_ne_product=1 "
        "never_blocks_m0=1 wave=%u "
        "(retbastion honesty; Soft≠product; not bar3)\n",
        (unsigned)GJ_INPUT_SOFT_WAVE);
/* Grep: input: soft retcitadel — Wave 33 exclusive citadel stamp */
