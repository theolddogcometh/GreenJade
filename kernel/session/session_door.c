...
sess_soft_inventory_log(void)
{
    ...
    kprintf("session_door: soft catalog honesty,inventory,claim,present,"
            ...
            "wave=%u areas_expect=52 soft PASS\n",
            GJ_SESS_SOFT_WAVE);
    cAreas++;
    ...
    kprintf("session_door: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=72\n");
...
kprintf("session_door: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=72 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: session_door: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("session_door: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=72 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("session_door: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=72 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: session_door: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("session_door: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=72 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("session_door: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=72 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: session_door: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("session_door: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=72 (retdoubletenailleangle stamp; Soft≠product)\n");
