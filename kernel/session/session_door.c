 ...
 *
 * Soft door inventory (Wave 23 exclusive deepen; this unit only):
...
/*
 * Soft product inventory (Wave 23 exclusive deepen). Cumulative path tallies.
 * greppable: session_door: soft …
 */
...
sess_soft_inventory_log(void)
{
    ...
    kprintf("session_door: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=23\n");
 ...
 * Soft≠product; not bar3.
            */
            /* Grep: session_door: soft rettoken — Wave 23 return-token honesty */
            kprintf("session_door: soft rettoken soft_only=1 product_gate=0 soft_ne_product=1 "
                    "never_blocks_m0=1 wave=%u "
                    "(rettoken honesty; Soft≠product; not bar3)\n",
                    (unsigned)GJ_SESS_SOFT_WAVE);
            /* Grep: session_door: soft retcrest — Wave 23 exclusive crest stamp */
