 ...
 *
 * Soft door inventory (Wave 32 exclusive deepen; this unit only):
...
/*
 * Soft product inventory (Wave 32 exclusive deepen). Cumulative path tallies.
 * greppable: session_door: soft …
 */
...
sess_soft_inventory_log(void)
{
    ...
    kprintf("session_door: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=32\n");
 ...
 * Soft≠product; not bar3.
 */
/* Grep: session_door: soft retbulwark — Wave 32 return-bulwark honesty */
kprintf("session_door: soft retbulwark soft_only=1 product_gate=0 soft_ne_product=1 "
        "never_blocks_m0=1 wave=%u "
        "(retbulwark honesty; Soft≠product; not bar3)\n",
        (unsigned)GJ_SESS_SOFT_WAVE);
/* Grep: session_door: soft retpanoply — Wave 32 exclusive panoply stamp */
