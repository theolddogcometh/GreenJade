 ...
 *
 * Soft product inventory (Wave 14 base + Wave 21 exclusive deepen; this unit only):
...
#define GJ_COLD_SOFT_WAVE  21u
/* Soft inventory area count (fixed greppable categories for deepen stamp). */
#define GJ_COLD_SOFT_AREAS 27u

/*
 * Soft product inventory (Wave 21 exclusive deepen). Cumulative unless noted live.
...
soft_inventory_log(void)
{
    ...
    kprintf("cold_ipc: soft note milestone=wave21 exclusive=1 "
    ...
    kprintf("cold_ipc: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=21\n");
    ...
    kprintf("cold_ipc: soft retseal exclusive=1 soft_ne_product=1 "
             ...
             * Soft≠product; not bar3.
            */
            /* Grep: cold_ipc: soft retpulse — Wave 21 return-pulse honesty */
            kprintf("cold_ipc: soft retpulse soft_only=1 product_gate=0 soft_ne_product=1 "
                    "never_blocks_m0=1 wave=%u "
                    "(retpulse honesty; Soft≠product; not bar3)\n",
                    (unsigned)GJ_COLD_SOFT_WAVE);
            /* Grep: cold_ipc: soft retmark — Wave 21 exclusive mark stamp */
    ...
    kprintf("cold_ipc: soft deepen wave=%u areas=%u logs=%u "
            ...
            "(Wave 21 exclusive; soft inventory; not bar3)\n",
            (unsigned)GJ_COLD_SOFT_WAVE, (unsigned)GJ_COLD_SOFT_AREAS,
    ...
    kprintf("cold: soft note milestone=wave21 exclusive=1 "
            "soft_only=1 not_bar3=1 submits=%llu logs=%u wave=%u\n",
            (unsigned long long)g_u64Submits, g_u32SoftLogN,
            (unsigned)GJ_COLD_SOFT_WAVE);
    ...
    kprintf("cold: soft retseal exclusive=1 soft_ne_product=1 "
             ...
             * Soft≠product; not bar3.
            */
            /* Grep: cold: soft retpulse — Wave 21 return-pulse honesty */
            kprintf("cold: soft retpulse soft_only=1 product_gate=0 soft_ne_product=1 "
                    "never_blocks_m0=1 wave=%u "
                    "(retpulse honesty; Soft≠product; not bar3)\n",
                    (unsigned)GJ_COLD_SOFT_WAVE);
            /* Grep: cold: soft retmark — Wave 21 exclusive mark stamp */
