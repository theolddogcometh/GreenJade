...
soft_inventory_log(void)
{
    ...
    kprintf("cold_ipc: soft note milestone=wave53 exclusive=1 "
    ...
    kprintf("cold_ipc: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=53\n");
 ...
 * Soft≠product; not bar3.
    ...
    kprintf("cold: soft note milestone=wave53 exclusive=1 "
            "soft_only=1 not_bar3=1 submits=%llu logs=%u wave=%u\n",
            (unsigned long long)g_u64Submits, g_u32SoftLogN,
            (unsigned)GJ_COLD_SOFT_WAVE);
