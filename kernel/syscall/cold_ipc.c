 ...
 *
 * Soft product inventory (Wave 14 base + Wave 27 exclusive deepen; this unit only):
...
#define GJ_COLD_SOFT_WAVE  27u
/* Soft inventory area count (fixed greppable categories for deepen stamp). */
#define GJ_COLD_SOFT_AREAS 39u

/*
 * Soft product inventory (Wave 27 exclusive deepen). Cumulative unless noted live.
...
soft_inventory_log(void)
{
    ...
    kprintf("cold_ipc: soft note milestone=wave27 exclusive=1 "
    ...
    kprintf("cold_ipc: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=27\n");
 ...
 * Soft≠product; not bar3.
            */
            ...
            kprintf("cold_ipc: soft retflame exclusive=1 soft_ne_product=1 "
                     ...
                     * Soft≠product; not bar3.
                     */
                    /* Grep: cold_ipc: soft retprism — Wave 27 return-prism honesty */
                    kprintf("cold_ipc: soft retprism soft_only=1 product_gate=0 soft_ne_product=1 "
                            "never_blocks_m0=1 wave=%u "
                            "(retprism honesty; Soft≠product; not bar3)\n",
                            (unsigned)GJ_COLD_SOFT_WAVE);
                    /* Grep: cold_ipc: soft retforge — Wave 27 exclusive forge stamp */
    ...
    kprintf("cold_ipc: soft deepen wave=%u areas=%u logs=%u "
            ...
            "(Wave 27 exclusive; soft inventory; not bar3)\n",
            (unsigned)GJ_COLD_SOFT_WAVE, (unsigned)GJ_COLD_SOFT_AREAS,
    ...
    kprintf("cold: soft note milestone=wave27 exclusive=1 "
            "soft_only=1 not_bar3=1 submits=%llu logs=%u wave=%u\n",
            (unsigned long long)g_u64Submits, g_u32SoftLogN,
            (unsigned)GJ_COLD_SOFT_WAVE);
 ...
 * Soft≠product; not bar3.
            */
            ...
            kprintf("cold: soft retflame exclusive=1 soft_ne_product=1 "
                     ...
                     * Soft≠product; not bar3.
                     */
                    /* Grep: cold: soft retprism — Wave 27 return-prism honesty */
                    kprintf("cold: soft retprism soft_only=1 product_gate=0 soft_ne_product=1 "
                            "never_blocks_m0=1 wave=%u "
                            "(retprism honesty; Soft≠product; not bar3)\n",
                            (unsigned)GJ_COLD_SOFT_WAVE);
                    /* Grep: cold: soft retforge — Wave 27 exclusive forge stamp */
