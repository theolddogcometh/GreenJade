 ...
 *
 * Soft inventory (Wave 15 base + Wave 29 exclusive deepen; this unit only):
 ...
 *   "syscall: soft deepen …"      — wave=29 area stamp
...
/* Wave 29 soft inventory stamp + area count (greppable deepen). */
#define SYSCALL_SOFT_WAVE  29u
#define SYSCALL_SOFT_AREAS 42u
...
/*
 * Wave 29 exclusive deepen (file-local; never hard-gates; wrap OK).
...
entry_soft_inventory_log(void)
{
    ...
    kprintf("syscall: soft note milestone=wave29 exclusive=1 "
    ...
    kprintf("syscall: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=29\n");
            ...
            */
            ...
            kprintf("syscall: soft retflame exclusive=1 soft_ne_product=1 "
                    ...
                    kprintf("syscall: soft retforge exclusive=1 soft_ne_product=1 "
                             ...
                             * Soft≠product; not bar3.
                             */
                            /* Grep: syscall: soft retglyph — Wave 29 return-glyph honesty */
                            kprintf("syscall: soft retglyph soft_only=1 product_gate=0 soft_ne_product=1 "
                                    "never_blocks_m0=1 wave=%u "
                                    "(retglyph honesty; Soft≠product; not bar3)\n",
                                    (unsigned)SYSCALL_SOFT_WAVE);
                            /* Grep: syscall: soft retscepter — Wave 29 exclusive scepter stamp */
