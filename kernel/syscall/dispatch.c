 ...
 *   "syscall: soft deepen …"      — wave=35 area stamp
...
/* Wave 35 soft inventory stamp + area count (greppable deepen). */
#define SYSCALL_SOFT_WAVE 35u
#define SYSCALL_SOFT_AREAS 54u
...
entry_soft_inventory_log(void)
{
    ...
    kprintf("syscall: soft note milestone=wave35 exclusive=1 "
    ...
    kprintf("syscall: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=35\n");
