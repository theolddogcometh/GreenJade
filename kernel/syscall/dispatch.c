 ...
 *
 * Soft inventory (Wave 15 base + Wave 28 exclusive deepen; this unit only):
 ...
 *   "syscall: soft deepen …"      — wave=28 area stamp
...
/* Wave 28 soft inventory stamp + area count (greppable deepen). */
#define SYSCALL_SOFT_WAVE  28u
...
/*
 * Wave 28 exclusive deepen (file-local; never hard-gates; wrap OK).
...
entry_soft_inventory_log(void)
{
    ...
    kprintf("syscall: soft note milestone=wave28 exclusive=1 "
    ...
    kprintf("syscall: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=28\n");
            ...
            */
            ...
            kprintf("syscall: soft retflame exclusive=1 soft_ne_product=1 "
                    ...
                    kprintf("syscall: soft retforge exclusive=1 soft_ne_product=1 "
                             ...
                             * Soft≠product; not bar3.
                             */
                            /* Grep: syscall: soft retshard — Wave 28 return-shard honesty */
                            kprintf("syscall: soft retshard soft_only=1 product_gate=0 soft_ne_product=1 "
                                "never_blocks_m0=1 wave=%u "
                                "(retshard honesty; Soft≠product; not bar3)\n",
                                (unsigned)SYSCALL_SOFT_WAVE);
                            /* Grep: syscall: soft retcrown — Wave 28 exclusive crown stamp */
