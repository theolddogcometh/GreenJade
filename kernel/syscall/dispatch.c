 ...
 *   "syscall: soft deepen …"      — wave=65 area stamp
...
#define SYSCALL_SOFT_WAVE 65u
#define SYSCALL_SOFT_AREAS 110u
...
entry_soft_inventory_log(void)
{
    ...
    kprintf("syscall: soft note milestone=wave65 exclusive=1 "
    ...
    kprintf("syscall: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=65\n");
