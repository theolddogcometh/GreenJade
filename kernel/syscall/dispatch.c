 ...
 *   "syscall: soft deepen …"      — wave=63 area stamp
...
#define SYSCALL_SOFT_WAVE 63u
#define SYSCALL_SOFT_AREAS 106u
...
entry_soft_inventory_log(void)
{
    ...
    kprintf("syscall: soft note milestone=wave63 exclusive=1 "
    ...
    kprintf("syscall: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=63\n");
