 ...
 *   "syscall: soft deepen …"      — wave=52 area stamp
...
#define SYSCALL_SOFT_WAVE 52u
#define SYSCALL_SOFT_AREAS 84u
...
entry_soft_inventory_log(void)
{
    ...
    kprintf("syscall: soft note milestone=wave52 exclusive=1 "
    ...
    kprintf("syscall: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=52\n");
