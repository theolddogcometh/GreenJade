 ...
 *
 * Soft inventory (Wave 15 base + Wave 17 exclusive deepen; this unit only):
 ...
 *   "syscall: soft surfaces …"    — Wave 17 surface count lamp
 *   "syscall: soft note …"        — Wave 16 milestone note
 *   "syscall: soft deepen …"      — wave=17 area stamp
...
/* Wave 17 soft inventory stamp + area count (greppable deepen). */
#define SYSCALL_SOFT_WAVE  17u
#define SYSCALL_SOFT_AREAS 21u
...
/*
 * Wave 17 exclusive deepen (file-local; never hard-gates; wrap OK).
...
entry_soft_inventory_log(void)
{
    ...
    /* Wave 17: complete share of route + combined edge enter. */
    if (u64Route != 0) {
    ...
    /* Grep: syscall: soft surfaces (Wave 17 deepen) */
    kprintf("syscall: soft surfaces count=%u "
            ...
            "catalog,surfaces,note,return,deepen,path,PASS wave=%u\n",
            SYSCALL_SOFT_AREAS, (unsigned)SYSCALL_SOFT_WAVE);

    /* Grep: syscall: soft note (Wave 17 deepen) */
    kprintf("syscall: soft note milestone=wave17 exclusive=1 "
    ...
    kprintf("syscall: soft catalog wave=%u areas=%u "
            ...
            "lifecycle,outcome,last,api,rates,honesty,edge,share,"
            "catalog,surfaces,note,return,deepen,path,PASS\n",
            (unsigned)SYSCALL_SOFT_WAVE, SYSCALL_SOFT_AREAS);

    /* Grep: syscall: soft return (Wave 17 deepen) */
