 ...
 *   "syscall: soft deepen …"      — wave=69 area stamp
...
#define SYSCALL_SOFT_WAVE 69u
#define SYSCALL_SOFT_AREAS 118u
...
entry_soft_inventory_log(void)
{
    ...
    kprintf("syscall: soft note milestone=wave69 exclusive=1 "
    ...
    kprintf("syscall: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=69\n");
