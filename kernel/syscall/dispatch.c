 ...
 *   "syscall: soft deepen …"      — wave=88 area stamp
...
#define SYSCALL_SOFT_WAVE 88u
#define SYSCALL_SOFT_AREAS 156u
...
entry_soft_inventory_log(void)
{
    ...
    kprintf("syscall: soft note milestone=wave88 exclusive=1 "
    ...
    kprintf("syscall: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=88\n");
...
kprintf("syscall: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("syscall: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("syscall: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("syscall: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("syscall: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("syscall: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: syscall: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("syscall: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retcurtainface honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("syscall: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retbastionangle stamp; Soft≠product)\n");
/* Grep: syscall: soft retglacisangle — Wave 74 return-glacisangle honesty */
kprintf("syscall: soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retglacisangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
kprintf("syscall: soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retparapetangle stamp; Soft≠product)\n");
/* Grep: syscall: soft retmoatangle — Wave 75 return-moatangle honesty */
kprintf("syscall: soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retmoatangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retowerangle — Wave 75 exclusive towerangle stamp */
kprintf("syscall: soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retowerangle stamp; Soft≠product)\n");
/* Grep: syscall: soft retgateangle — Wave 76 return-gateangle honesty */
kprintf("syscall: soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retgateangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retwallangle — Wave 76 exclusive wallangle stamp */
kprintf("syscall: soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retwallangle stamp; Soft≠product)\n");
/* Grep: syscall: soft retspireangle — Wave 77 return-spireangle honesty */
kprintf("syscall: soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retspireangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retholdangle — Wave 77 exclusive holdangle stamp */
kprintf("syscall: soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retholdangle stamp; Soft≠product)\n");
/* Grep: syscall: soft retpalaceangle — Wave 78 return-palaceangle honesty */
kprintf("syscall: soft retpalaceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retpalaceangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retfortressangle — Wave 78 exclusive fortressangle stamp */
kprintf("syscall: soft retfortressangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retfortressangle stamp; Soft≠product)\n");
/* Grep: syscall: soft retkeepangle — Wave 79 return-keepangle honesty */
kprintf("syscall: soft retkeepangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retkeepangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retredoubtangle — Wave 79 exclusive redoubtangle stamp */
kprintf("syscall: soft retredoubtangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retredoubtangle stamp; Soft≠product)\n");
/* Grep: syscall: soft retcitadelangle — Wave 80 return-citadelangle honesty */
kprintf("syscall: soft retcitadelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retcitadelangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retbastionkeep — Wave 80 exclusive bastionkeep stamp */
kprintf("syscall: soft retbastionkeep exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retbastionkeep stamp; Soft≠product)\n");
/* Grep: syscall: soft retpanoplyangle — Wave 81 return-panoplyangle honesty */
kprintf("syscall: soft retpanoplyangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retpanoplyangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retbulwarkangle — Wave 81 exclusive bulwarkangle stamp */
kprintf("syscall: soft retbulwarkangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retbulwarkangle stamp; Soft≠product)\n");
/* Grep: syscall: soft retmantleangle — Wave 82 return-mantleangle honesty */
kprintf("syscall: soft retmantleangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retmantleangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retaegisangle — Wave 82 exclusive aegisangle stamp */
kprintf("syscall: soft retaegisangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retaegisangle stamp; Soft≠product)\n");
/* Grep: syscall: soft retemblemangle — Wave 83 return-emblemangle honesty */
kprintf("syscall: soft retemblemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retemblemangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retsigilangle — Wave 83 exclusive sigilangle stamp */
kprintf("syscall: soft retsigilangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retsigilangle stamp; Soft≠product)\n");
/* Grep: syscall: soft retscepterangle — Wave 84 return-scepterangle honesty */
kprintf("syscall: soft retscepterangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retscepterangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retglyphangle — Wave 84 exclusive glyphangle stamp */
kprintf("syscall: soft retglyphangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retglyphangle stamp; Soft≠product)\n");
/* Grep: syscall: soft retcrownangle — Wave 85 return-crownangle honesty */
kprintf("syscall: soft retcrownangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retcrownangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retshardangle — Wave 85 exclusive shardangle stamp */
kprintf("syscall: soft retshardangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retshardangle stamp; Soft≠product)\n");
/* Grep: syscall: soft retforgeangle — Wave 86 return-forgeangle honesty */
kprintf("syscall: soft retforgeangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retforgeangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retprismangle — Wave 86 exclusive prismangle stamp */
kprintf("syscall: soft retprismangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retprismangle stamp; Soft≠product)\n");
/* Grep: syscall: soft retflameangle — Wave 87 return-flameangle honesty */
kprintf("syscall: soft retflameangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retflameangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retcipherangle — Wave 87 exclusive cipherangle stamp */
kprintf("syscall: soft retcipherangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retcipherangle stamp; Soft≠product)\n");
/* Grep: syscall: soft retbeaconangle — Wave 88 return-beaconangle honesty */
kprintf("syscall: soft retbeaconangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retbeaconangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retledgerangle — Wave 88 exclusive ledgerangle stamp */
kprintf("syscall: soft retledgerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retledgerangle stamp; Soft≠product)\n");
