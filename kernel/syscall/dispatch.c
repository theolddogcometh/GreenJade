 ...
 *   "syscall: soft deepen …"      — wave=68 area stamp
...
#define SYSCALL_SOFT_WAVE 68u
#define SYSCALL_SOFT_AREAS 116u
...
entry_soft_inventory_log(void)
{
    ...
    kprintf("syscall: soft note milestone=wave68 exclusive=1 "
    ...
    kprintf("syscall: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=68\n");
