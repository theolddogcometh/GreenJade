 ...
 *
 * Soft inventory (Wave 15 base + Wave 21 exclusive deepen; this unit only):
 ...
 *   "syscall: soft deepen …"      — wave=21 area stamp
...
/* Wave 21 soft inventory stamp + area count (greppable deepen). */
#define SYSCALL_SOFT_WAVE  21u
#define SYSCALL_SOFT_AREAS 28u
...
/*
 * Wave 21 exclusive deepen (file-local; never hard-gates; wrap OK).
...
entry_soft_inventory_log(void)
{
    ...
    kprintf("syscall: soft note milestone=wave21 exclusive=1 "
    ...
    kprintf("syscall: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=21\n");
    ...
    kprintf("syscall: soft retseal exclusive=1 soft_ne_product=1 "
             ...
             * Soft≠product; not bar3.
            */
            /* Grep: syscall: soft retpulse — Wave 21 return-pulse honesty */
            kprintf("syscall: soft retpulse soft_only=1 product_gate=0 soft_ne_product=1 "
                    "never_blocks_m0=1 wave=%u "
                    "(retpulse honesty; Soft≠product; not bar3)\n",
                    (unsigned)SYSCALL_SOFT_WAVE);
            /* Grep: syscall: soft retmark — Wave 21 exclusive mark stamp */
