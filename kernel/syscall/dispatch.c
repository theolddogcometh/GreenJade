 ...
 *
 * Soft inventory (Wave 15 base + Wave 23 exclusive deepen; this unit only):
 ...
 *   "syscall: soft deepen …"      — wave=23 area stamp
...
/* Wave 23 soft inventory stamp + area count (greppable deepen). */
#define SYSCALL_SOFT_WAVE  23u
#define SYSCALL_SOFT_AREAS 32u
...
/*
 * Wave 23 exclusive deepen (file-local; never hard-gates; wrap OK).
...
entry_soft_inventory_log(void)
{
    ...
    kprintf("syscall: soft note milestone=wave23 exclusive=1 "
    ...
    kprintf("syscall: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=23\n");
 ...
 * Soft≠product; not bar3.
            */
            /* Grep: syscall: soft rettoken — Wave 23 return-token honesty */
            kprintf("syscall: soft rettoken soft_only=1 product_gate=0 soft_ne_product=1 "
                    "never_blocks_m0=1 wave=%u "
                    "(rettoken honesty; Soft≠product; not bar3)\n",
                    (unsigned)SYSCALL_SOFT_WAVE);
            /* Grep: syscall: soft retcrest — Wave 23 exclusive crest stamp */
