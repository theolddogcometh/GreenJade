 ...
 *   "syscall: soft deepen …"      — wave=51 area stamp
...
#define SYSCALL_SOFT_WAVE 51u
#define SYSCALL_SOFT_AREAS 82u
...
entry_soft_inventory_log(void)
{
    ...
    kprintf("syscall: soft note milestone=wave51 exclusive=1 "
    ...
    kprintf("syscall: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=51\n");
