 ...
 *   "syscall: soft deepen …"      — wave=50 area stamp
...
#define SYSCALL_SOFT_WAVE 50u
#define SYSCALL_SOFT_AREAS 80u
...
entry_soft_inventory_log(void)
{
    ...
    kprintf("syscall: soft note milestone=wave50 exclusive=1 "
    ...
    kprintf("syscall: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=50\n");
