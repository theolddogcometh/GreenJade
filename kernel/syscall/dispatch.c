 ...
 *   "syscall: soft deepen …"      — wave=62 area stamp
...
#define SYSCALL_SOFT_WAVE 62u
#define SYSCALL_SOFT_AREAS 104u
...
entry_soft_inventory_log(void)
{
    ...
    kprintf("syscall: soft note milestone=wave62 exclusive=1 "
    ...
    kprintf("syscall: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=62\n");
