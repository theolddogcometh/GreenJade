 ...
 *   "syscall: soft deepen …"      — wave=56 area stamp
...
#define SYSCALL_SOFT_WAVE 56u
#define SYSCALL_SOFT_AREAS 92u
...
entry_soft_inventory_log(void)
{
    ...
    kprintf("syscall: soft note milestone=wave56 exclusive=1 "
    ...
    kprintf("syscall: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=56\n");
