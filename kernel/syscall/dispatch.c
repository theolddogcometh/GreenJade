 ...
 *   "syscall: soft deepen …"      — wave=66 area stamp
...
#define SYSCALL_SOFT_WAVE 66u
#define SYSCALL_SOFT_AREAS 112u
...
entry_soft_inventory_log(void)
{
    ...
    kprintf("syscall: soft note milestone=wave66 exclusive=1 "
    ...
    kprintf("syscall: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=66\n");
