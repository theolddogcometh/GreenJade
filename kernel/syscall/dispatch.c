 ...
 *
 * Soft inventory (Wave 15 base + Wave 33 exclusive deepen; this unit only):
 ...
 *   "syscall: soft deepen …"      — wave=33 area stamp
...
/* Wave 33 soft inventory stamp + area count (greppable deepen). */
#define SYSCALL_SOFT_WAVE 33u
#define SYSCALL_SOFT_AREAS 50u
...
/*
 * Wave 33 exclusive deepen (file-local; never hard-gates; wrap OK).
...
entry_soft_inventory_log(void)
{
    ...
    kprintf("syscall: soft note milestone=wave33 exclusive=1 "
    ...
    kprintf("syscall: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=33\n");
 ...
 * Soft≠product; not bar3.
 */
/* Grep: syscall: soft retbastion — Wave 33 return-bastion honesty */
kprintf("syscall: soft retbastion soft_only=1 product_gate=0 soft_ne_product=1 "
        "never_blocks_m0=1 wave=%u "
        "(retbastion honesty; Soft≠product; not bar3)\n",
        (unsigned)SYSCALL_SOFT_WAVE);
/* Grep: syscall: soft retcitadel — Wave 33 exclusive citadel stamp */
