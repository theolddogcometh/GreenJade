 ...
 *
 * Soft inventory (Wave 15 base + Wave 20 exclusive deepen; this unit only):
 ...
 *   "syscall: soft deepen …"      — wave=20 area stamp
...
/* Wave 20 soft inventory stamp + area count (greppable deepen). */
#define SYSCALL_SOFT_WAVE  20u
#define SYSCALL_SOFT_AREAS 26u
...
/*
 * Wave 20 exclusive deepen (file-local; never hard-gates; wrap OK).
...
entry_soft_inventory_log(void)
{
    ...
    kprintf("syscall: soft note milestone=wave20 exclusive=1 "
    ...
    kprintf("syscall: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=20\n");
     ...
     * Soft≠product; not bar3.
     */
    /* Grep: syscall: soft retbound — Wave 20 return-bound honesty */
    kprintf("syscall: soft retbound soft_only=1 product_gate=0 hard_gate=0 "
            "never_blocks_m0=1 wave=%u "
            "(retbound honesty; Soft≠product; not bar3)\n",
            (unsigned)SYSCALL_SOFT_WAVE);
    /* Grep: syscall: soft retseal — Wave 20 exclusive seal stamp */
