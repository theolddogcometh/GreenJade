 ...
 *
 * Soft inventory (Wave 15 base + Wave 25 exclusive deepen; this unit only):
 ...
 *   "syscall: soft deepen …"      — wave=25 area stamp
...
/* Wave 25 soft inventory stamp + area count (greppable deepen). */
#define SYSCALL_SOFT_WAVE  25u
#define SYSCALL_SOFT_AREAS 36u
...
/*
 * Wave 25 exclusive deepen (file-local; never hard-gates; wrap OK).
...
entry_soft_inventory_log(void)
{
    ...
    kprintf("syscall: soft note milestone=wave25 exclusive=1 "
    ...
    kprintf("syscall: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=25\n");
 ...
 * Soft≠product; not bar3.
            */
             ...
             * Soft≠product; not bar3.
             */
            /* Grep: syscall: soft retledger — Wave 25 return-ledger honesty */
            kprintf("syscall: soft retledger soft_only=1 product_gate=0 soft_ne_product=1 "
                    "never_blocks_m0=1 wave=%u "
                    "(retledger honesty; Soft≠product; not bar3)\n",
                    (unsigned)SYSCALL_SOFT_WAVE);
            /* Grep: syscall: soft retbeacon — Wave 25 exclusive beacon stamp */
