 ...
 *   "syscall: soft deepen …"      — wave=61 area stamp
...
#define SYSCALL_SOFT_WAVE 61u
#define SYSCALL_SOFT_AREAS 102u
...
entry_soft_inventory_log(void)
{
    ...
    kprintf("syscall: soft note milestone=wave61 exclusive=1 "
    ...
    kprintf("syscall: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=61\n");
