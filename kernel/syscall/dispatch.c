 ...
 *
 * Soft inventory (Wave 15 base + Wave 24 exclusive deepen; this unit only):
 ...
 *   "syscall: soft deepen …"      — wave=24 area stamp
...
/* Wave 24 soft inventory stamp + area count (greppable deepen). */
#define SYSCALL_SOFT_WAVE  24u
#define SYSCALL_SOFT_AREAS 34u
...
/*
 * Wave 24 exclusive deepen (file-local; never hard-gates; wrap OK).
...
entry_soft_inventory_log(void)
{
    ...
    kprintf("syscall: soft note milestone=wave24 exclusive=1 "
    ...
    kprintf("syscall: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=24\n");
 ...
 * Soft≠product; not bar3.
            */
             ...
             * Soft≠product; not bar3.
             */
            /* Grep: syscall: soft retvault — Wave 24 return-vault honesty */
            kprintf("syscall: soft retvault soft_only=1 product_gate=0 soft_ne_product=1 "
                    "never_blocks_m0=1 wave=%u "
                    "(retvault honesty; Soft≠product; not bar3)\n",
                    (unsigned)SYSCALL_SOFT_WAVE);
            /* Grep: syscall: soft retbanner — Wave 24 exclusive banner stamp */
