 ...
 *   "syscall: soft deepen …"      — wave=102 area stamp
...
#define SYSCALL_SOFT_WAVE 102u
#define SYSCALL_SOFT_AREAS 184u
...
entry_soft_inventory_log(void)
{
    ...
    kprintf("syscall: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=102\n");
...
kprintf("syscall: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=102 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("syscall: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=102 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("syscall: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=102 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("syscall: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=102 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("syscall: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=102 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("syscall: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=102 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: syscall: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("syscall: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=102 (retcurtainface honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("syscall: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=102 (retbastionangle stamp; Soft≠product)\n");
/* Grep: syscall: soft retglacisangle — Wave 74 return-glacisangle honesty */
kprintf("syscall: soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=102 (retglacisangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
kprintf("syscall: soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=102 (retparapetangle stamp; Soft≠product)\n");
/* Grep: syscall: soft retmoatangle — Wave 75 return-moatangle honesty */
kprintf("syscall: soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=102 (retmoatangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retowerangle — Wave 75 exclusive towerangle stamp */
kprintf("syscall: soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=102 (retowerangle stamp; Soft≠product)\n");
/* Grep: syscall: soft retgateangle — Wave 76 return-gateangle honesty */
kprintf("syscall: soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=102 (retgateangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retwallangle — Wave 76 exclusive wallangle stamp */
kprintf("syscall: soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=102 (retwallangle stamp; Soft≠product)\n");
/* Grep: syscall: soft retspireangle — Wave 77 return-spireangle honesty */
kprintf("syscall: soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=102 (retspireangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retholdangle — Wave 77 exclusive holdangle stamp */
kprintf("syscall: soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=102 (retholdangle stamp; Soft≠product)\n");
/* Grep: syscall: soft retpalaceangle — Wave 78 return-palaceangle honesty */
kprintf("syscall: soft retpalaceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=102 (retpalaceangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retfortressangle — Wave 78 exclusive fortressangle stamp */
kprintf("syscall: soft retfortressangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=102 (retfortressangle stamp; Soft≠product)\n");
/* Grep: syscall: soft retkeepangle — Wave 79 return-keepangle honesty */
kprintf("syscall: soft retkeepangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=102 (retkeepangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retredoubtangle — Wave 79 exclusive redoubtangle stamp */
kprintf("syscall: soft retredoubtangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=102 (retredoubtangle stamp; Soft≠product)\n");
/* Grep: syscall: soft retcitadelangle — Wave 80 return-citadelangle honesty */
kprintf("syscall: soft retcitadelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=102 (retcitadelangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retbastionkeep — Wave 80 exclusive bastionkeep stamp */
kprintf("syscall: soft retbastionkeep exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=102 (retbastionkeep stamp; Soft≠product)\n");
/* Grep: syscall: soft retpanoplyangle — Wave 81 return-panoplyangle honesty */
kprintf("syscall: soft retpanoplyangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=102 (retpanoplyangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retbulwarkangle — Wave 81 exclusive bulwarkangle stamp */
kprintf("syscall: soft retbulwarkangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=102 (retbulwarkangle stamp; Soft≠product)\n");
/* Grep: syscall: soft retmantleangle — Wave 82 return-mantleangle honesty */
kprintf("syscall: soft retmantleangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=102 (retmantleangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retaegisangle — Wave 82 exclusive aegisangle stamp */
kprintf("syscall: soft retaegisangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=102 (retaegisangle stamp; Soft≠product)\n");
/* Grep: syscall: soft retemblemangle — Wave 83 return-emblemangle honesty */
kprintf("syscall: soft retemblemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=102 (retemblemangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retsigilangle — Wave 83 exclusive sigilangle stamp */
kprintf("syscall: soft retsigilangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=102 (retsigilangle stamp; Soft≠product)\n");
/* Grep: syscall: soft retscepterangle — Wave 84 return-scepterangle honesty */
kprintf("syscall: soft retscepterangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=102 (retscepterangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retglyphangle — Wave 84 exclusive glyphangle stamp */
kprintf("syscall: soft retglyphangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=102 (retglyphangle stamp; Soft≠product)\n");
/* Grep: syscall: soft retcrownangle — Wave 85 return-crownangle honesty */
kprintf("syscall: soft retcrownangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=102 (retcrownangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retshardangle — Wave 85 exclusive shardangle stamp */
kprintf("syscall: soft retshardangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=102 (retshardangle stamp; Soft≠product)\n");
/* Grep: syscall: soft retforgeangle — Wave 86 return-forgeangle honesty */
kprintf("syscall: soft retforgeangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=102 (retforgeangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retprismangle — Wave 86 exclusive prismangle stamp */
kprintf("syscall: soft retprismangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=102 (retprismangle stamp; Soft≠product)\n");
/* Grep: syscall: soft retflameangle — Wave 87 return-flameangle honesty */
kprintf("syscall: soft retflameangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=102 (retflameangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retcipherangle — Wave 87 exclusive cipherangle stamp */
kprintf("syscall: soft retcipherangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=102 (retcipherangle stamp; Soft≠product)\n");
/* Grep: syscall: soft retbeaconangle — Wave 88 return-beaconangle honesty */
kprintf("syscall: soft retbeaconangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=102 (retbeaconangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retledgerangle — Wave 88 exclusive ledgerangle stamp */
kprintf("syscall: soft retledgerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=102 (retledgerangle stamp; Soft≠product)\n");
/* Grep: syscall: soft retbannerangle — Wave 89 return-bannerangle honesty */
kprintf("syscall: soft retbannerangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=102 (retbannerangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retvaultangle — Wave 89 exclusive vaultangle stamp */
kprintf("syscall: soft retvaultangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=102 (retvaultangle stamp; Soft≠product)\n");
/* Grep: syscall: soft retcrestangle — Wave 90 return-crestangle honesty */
kprintf("syscall: soft retcrestangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=102 (retcrestangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft rettokenangle — Wave 90 exclusive tokenangle stamp */
kprintf("syscall: soft rettokenangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=102 (rettokenangle stamp; Soft≠product)\n");
/* Grep: syscall: soft retbadgeangle — Wave 91 return-badgeangle honesty */
kprintf("syscall: soft retbadgeangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=102 (retbadgeangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retphaseangle — Wave 91 exclusive phaseangle stamp */
kprintf("syscall: soft retphaseangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=102 (retphaseangle stamp; Soft≠product)\n");
/* Grep: syscall: soft retmarkangle — Wave 92 return-markangle honesty */
kprintf("syscall: soft retmarkangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=102 (retmarkangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retpulseangle — Wave 92 exclusive pulseangle stamp */
kprintf("syscall: soft retpulseangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=102 (retpulseangle stamp; Soft≠product)\n");

/* Grep: syscall: soft retsealangle — Wave 93 return-sealangle honesty */
kprintf("syscall: soft retsealangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=102 (retsealangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retboundangle — Wave 93 exclusive boundangle stamp */
kprintf("syscall: soft retboundangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=102 (retboundangle stamp; Soft≠product)\n");
/* Grep: syscall: soft retstemangle — Wave 94 return-stemangle honesty */
kprintf("syscall: soft retstemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=102 (retstemangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retbladeangle — Wave 94 exclusive bladeangle stamp */
kprintf("syscall: soft retbladeangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=102 (retbladeangle stamp; Soft≠product)\n");
/* Grep: syscall: soft retchordangle — Wave 95 return-chordangle honesty */
kprintf("syscall: soft retchordangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=102 (retchordangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retarcangle — Wave 95 exclusive arcangle stamp */
kprintf("syscall: soft retarcangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=102 (retarcangle stamp; Soft≠product)\n");
/* Grep: syscall: soft retsectorangle — Wave 96 return-sectorangle honesty */
kprintf("syscall: soft retsectorangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=102 (retsectorangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retwedgeangle — Wave 96 exclusive wedgeangle stamp */
kprintf("syscall: soft retwedgeangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=102 (retwedgeangle stamp; Soft≠product)\n");
/* Grep: syscall: soft retradiusangle — Wave 97 return-radiusangle honesty */
kprintf("syscall: soft retradiusangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=102 (retradiusangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retdiameterangle — Wave 97 exclusive diameterangle stamp */
kprintf("syscall: soft retdiameterangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=102 (retdiameterangle stamp; Soft≠product)\n");
/* Grep: syscall: soft retcircumangle — Wave 98 return-circumangle honesty */
kprintf("syscall: soft retcircumangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=102 (retcircumangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retellipseangle — Wave 98 exclusive ellipseangle stamp */
kprintf("syscall: soft retellipseangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=102 (retellipseangle stamp; Soft≠product)\n");
/* Grep: syscall: soft rethyperangle — Wave 99 return-hyperangle honesty */
kprintf("syscall: soft rethyperangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=102 (rethyperangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retparabolaangle — Wave 99 exclusive parabolaangle stamp */
kprintf("syscall: soft retparabolaangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=102 (retparabolaangle stamp; Soft≠product)\n");
/* Grep: syscall: soft retspiralangle — Wave 100 return-spiralangle honesty */
kprintf("syscall: soft retspiralangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=102 (retspiralangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft rethelixangle — Wave 100 exclusive helixangle stamp */
kprintf("syscall: soft rethelixangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=102 (rethelixangle stamp; Soft≠product)\n");
/* Grep: syscall: soft rettorusangle — Wave 101 return-torusangle honesty */
kprintf("syscall: soft rettorusangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=102 (rettorusangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retknotangle — Wave 101 exclusive knotangle stamp */
kprintf("syscall: soft retknotangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=102 (retknotangle stamp; Soft≠product)\n");
/* Grep: syscall: soft retmoebiusangle — Wave 102 return-moebiusangle honesty */
kprintf("syscall: soft retmoebiusangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=102 (retmoebiusangle honesty; Soft≠product; not bar3)\n");
/* Grep: syscall: soft retkleinangle — Wave 102 exclusive kleinangle stamp */
kprintf("syscall: soft retkleinangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=102 (retkleinangle stamp; Soft≠product)\n");
