 ...
 *
 * Soft inventory (Wave 15 base + Wave 34 exclusive deepen; this unit only):
 ...
 *   "syscall: soft deepen …"      — wave=34 area stamp
...
/* Wave 34 soft inventory stamp + area count (greppable deepen). */
#define SYSCALL_SOFT_WAVE 34u
#define SYSCALL_SOFT_AREAS 52u
...
/*
 * Wave 34 exclusive deepen (file-local; never hard-gates; wrap OK).
...
entry_soft_inventory_log(void)
{
    ...
    kprintf("syscall: soft note milestone=wave34 exclusive=1 "
    ...
    kprintf("syscall: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=34\n");
