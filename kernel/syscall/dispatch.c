 ...
 *   "syscall: soft deepen …"      — wave=64 area stamp
...
#define SYSCALL_SOFT_WAVE 64u
#define SYSCALL_SOFT_AREAS 108u
...
entry_soft_inventory_log(void)
{
    ...
    kprintf("syscall: soft note milestone=wave64 exclusive=1 "
    ...
    kprintf("syscall: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=64\n");
