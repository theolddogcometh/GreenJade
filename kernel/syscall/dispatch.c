 ...
 *   "syscall: soft deepen …"      — wave=40 area stamp
...
/* Wave 40 soft inventory stamp + area count (greppable deepen). */
#define SYSCALL_SOFT_WAVE 40u
...
entry_soft_inventory_log(void)
{
    ...
    kprintf("syscall: soft note milestone=wave40 exclusive=1 "
    ...
    kprintf("syscall: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=40\n");
