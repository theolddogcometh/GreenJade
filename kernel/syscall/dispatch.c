 ...
 *   "syscall: soft deepen …"      — wave=59 area stamp
...
#define SYSCALL_SOFT_WAVE 59u
#define SYSCALL_SOFT_AREAS 98u
...
entry_soft_inventory_log(void)
{
    ...
    kprintf("syscall: soft note milestone=wave59 exclusive=1 "
    ...
    kprintf("syscall: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=59\n");
