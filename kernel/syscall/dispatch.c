 ...
 *
 * Soft inventory (Wave 15 base + Wave 22 exclusive deepen; this unit only):
 ...
 *   "syscall: soft deepen …"      — wave=22 area stamp
...
/* Wave 22 soft inventory stamp + area count (greppable deepen). */
#define SYSCALL_SOFT_WAVE  22u
#define SYSCALL_SOFT_AREAS 30u
...
/*
 * Wave 22 exclusive deepen (file-local; never hard-gates; wrap OK).
...
entry_soft_inventory_log(void)
{
    ...
    kprintf("syscall: soft note milestone=wave22 exclusive=1 "
    ...
    kprintf("syscall: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=22\n");
    ...
    kprintf("syscall: soft retseal exclusive=1 soft_ne_product=1 "
            ...
            */
             ...
             * Soft≠product; not bar3.
            */
            /* Grep: syscall: soft retphase — Wave 22 return-phase honesty */
            kprintf("syscall: soft retphase soft_only=1 product_gate=0 soft_ne_product=1 "
                    "never_blocks_m0=1 wave=%u "
                    "(retphase honesty; Soft≠product; not bar3)\n",
                    (unsigned)SYSCALL_SOFT_WAVE);
            /* Grep: syscall: soft retbadge — Wave 22 exclusive badge stamp */
