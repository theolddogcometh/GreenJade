 ...
 *   "syscall: soft deepen …"      — wave=60 area stamp
...
#define SYSCALL_SOFT_WAVE 60u
#define SYSCALL_SOFT_AREAS 100u
...
entry_soft_inventory_log(void)
{
    ...
    kprintf("syscall: soft note milestone=wave60 exclusive=1 "
    ...
    kprintf("syscall: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=60\n");
