 ...
 *
 * Soft inventory (Wave 15 base + Wave 30 exclusive deepen; this unit only):
 ...
 *   "syscall: soft deepen …"      — wave=30 area stamp
...
/* Wave 30 soft inventory stamp + area count (greppable deepen). */
#define SYSCALL_SOFT_WAVE  30u
#define SYSCALL_SOFT_AREAS 44u
...
/*
 * Wave 30 exclusive deepen (file-local; never hard-gates; wrap OK).
...
entry_soft_inventory_log(void)
{
    ...
    kprintf("syscall: soft note milestone=wave30 exclusive=1 "
    ...
    kprintf("syscall: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=30\n");
            ...
            */
            ...
            kprintf("syscall: soft retflame exclusive=1 soft_ne_product=1 "
                    ...
                    kprintf("syscall: soft retforge exclusive=1 soft_ne_product=1 "
                             ...
                             * Soft≠product; not bar3.
                             */
                            /* Grep: syscall: soft retsigil — Wave 30 return-sigil honesty */
                            kprintf("syscall: soft retsigil soft_only=1 product_gate=0 soft_ne_product=1 "
                                    "never_blocks_m0=1 wave=%u "
                                    "(retsigil honesty; Soft≠product; not bar3)\n",
                                    (unsigned)SYSCALL_SOFT_WAVE);
                            /* Grep: syscall: soft retemblem — Wave 30 exclusive emblem stamp */
