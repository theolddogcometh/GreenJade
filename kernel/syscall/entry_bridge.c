...
#define ENTRY_BRIDGE_SOFT_WAVE 71u
/* inventory|path|rates|honesty|last|surfaces|note|catalog|deepen|PASS */
#define ENTRY_BRIDGE_SOFT_AREAS 112u
...
entry_bridge_soft_inventory_log(void)
{
    ...
    kprintf("entry_bridge: soft note milestone=wave71 exclusive=1 "
    ...
    kprintf("entry_bridge: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=71\n");
...
kprintf("entry_bridge: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=71 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: entry_bridge: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("entry_bridge: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=71 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("entry_bridge: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=71 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: entry_bridge: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("entry_bridge: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=71 (retreentrantangle stamp; Soft≠product)\n");

                            kprintf("entry_bridge: soft deepen wave=%u areas=%u enter=%lu "
            "route=%lu logs=%lu "
            "(Wave 71 exclusive; not bar3)\n",
            (unsigned)ENTRY_BRIDGE_SOFT_WAVE,
            (unsigned)ENTRY_BRIDGE_SOFT_AREAS,
            (unsigned long)u64Enter,
            (unsigned long)u64Route,
