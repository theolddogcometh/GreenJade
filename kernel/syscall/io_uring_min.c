 ...
 *
 * Soft inventory (Wave 14 base + Wave 34 exclusive deepen) — greppable "io_uring: soft …":
 ...
 *   io_uring: soft deepen      — wave=34 areas stamp
...
#define GJ_IORING_SOFT_WAVE 34u
/* Soft inventory area count (fixed greppable categories for deepen stamp). */
#define GJ_IORING_SOFT_AREAS 50u

/*
 * Soft product inventory (Wave 34 exclusive deepen). File-local sticky
...
/**
 * Greppable soft inventory (Wave 34 exclusive deepen). Prefix "io_uring: soft …".
...
ioring_soft_log(void)
{
    ...
    kprintf("io_uring: soft note milestone=wave34 exclusive=1 "
...
/* Grep: io_uring: soft deepen wave (Wave 15 stamp) */
    ...
    kprintf("io_uring: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=34\n");
...
kprintf("io_uring: soft retkeep exclusive=1 soft_ne_product=1 "
        ...
        (unsigned)GJ_IORING_SOFT_WAVE);
            ...
            "(Wave 34 exclusive; min rings soft scaffold; not product game I/O; "
            "not bar3)\n",
            (unsigned)GJ_IORING_SOFT_WAVE,
            (unsigned)GJ_IORING_SOFT_AREAS,
            (unsigned long)g_soft.u64SoftLog,
