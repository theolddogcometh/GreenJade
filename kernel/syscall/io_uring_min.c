 ...
 *   io_uring: soft deepen      — wave=88 areas stamp
...
ioring_soft_log(void)
{
    ...
    kprintf("io_uring: soft note milestone=wave88 exclusive=1 "
...
/* Grep: io_uring: soft deepen wave (Wave 15 stamp) */
    ...
    kprintf("io_uring: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=88\n");
...
kprintf("io_uring: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: io_uring: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("io_uring: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("io_uring: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: io_uring: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("io_uring: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("io_uring: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: io_uring: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("io_uring: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: io_uring: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("io_uring: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retcurtainface honesty; Soft≠product; not bar3)\n");
/* Grep: io_uring: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("io_uring: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retbastionangle stamp; Soft≠product)\n");
/* Grep: io_uring: soft retglacisangle — Wave 74 return-glacisangle honesty */
kprintf("io_uring: soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retglacisangle honesty; Soft≠product; not bar3)\n");
/* Grep: io_uring: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
kprintf("io_uring: soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retparapetangle stamp; Soft≠product)\n");
/* Grep: io_uring: soft retmoatangle — Wave 75 return-moatangle honesty */
kprintf("io_uring: soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retmoatangle honesty; Soft≠product; not bar3)\n");
/* Grep: io_uring: soft retowerangle — Wave 75 exclusive towerangle stamp */
kprintf("io_uring: soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retowerangle stamp; Soft≠product)\n");
/* Grep: io_uring: soft retgateangle — Wave 76 return-gateangle honesty */
kprintf("io_uring: soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retgateangle honesty; Soft≠product; not bar3)\n");
/* Grep: io_uring: soft retwallangle — Wave 76 exclusive wallangle stamp */
kprintf("io_uring: soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retwallangle stamp; Soft≠product)\n");
/* Grep: io_uring: soft retspireangle — Wave 77 return-spireangle honesty */
kprintf("io_uring: soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retspireangle honesty; Soft≠product; not bar3)\n");
/* Grep: io_uring: soft retholdangle — Wave 77 exclusive holdangle stamp */
kprintf("io_uring: soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retholdangle stamp; Soft≠product)\n");
/* Grep: io_uring: soft retpalaceangle — Wave 78 return-palaceangle honesty */
kprintf("io_uring: soft retpalaceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retpalaceangle honesty; Soft≠product; not bar3)\n");
/* Grep: io_uring: soft retfortressangle — Wave 78 exclusive fortressangle stamp */
kprintf("io_uring: soft retfortressangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retfortressangle stamp; Soft≠product)\n");
/* Grep: io_uring: soft retkeepangle — Wave 79 return-keepangle honesty */
kprintf("io_uring: soft retkeepangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retkeepangle honesty; Soft≠product; not bar3)\n");
/* Grep: io_uring: soft retredoubtangle — Wave 79 exclusive redoubtangle stamp */
kprintf("io_uring: soft retredoubtangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retredoubtangle stamp; Soft≠product)\n");
/* Grep: io_uring: soft retcitadelangle — Wave 80 return-citadelangle honesty */
kprintf("io_uring: soft retcitadelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retcitadelangle honesty; Soft≠product; not bar3)\n");
/* Grep: io_uring: soft retbastionkeep — Wave 80 exclusive bastionkeep stamp */
kprintf("io_uring: soft retbastionkeep exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retbastionkeep stamp; Soft≠product)\n");
/* Grep: io_uring: soft retpanoplyangle — Wave 81 return-panoplyangle honesty */
kprintf("io_uring: soft retpanoplyangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retpanoplyangle honesty; Soft≠product; not bar3)\n");
/* Grep: io_uring: soft retbulwarkangle — Wave 81 exclusive bulwarkangle stamp */
kprintf("io_uring: soft retbulwarkangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retbulwarkangle stamp; Soft≠product)\n");
/* Grep: io_uring: soft retmantleangle — Wave 82 return-mantleangle honesty */
kprintf("io_uring: soft retmantleangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retmantleangle honesty; Soft≠product; not bar3)\n");
/* Grep: io_uring: soft retaegisangle — Wave 82 exclusive aegisangle stamp */
kprintf("io_uring: soft retaegisangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retaegisangle stamp; Soft≠product)\n");
/* Grep: io_uring: soft retemblemangle — Wave 83 return-emblemangle honesty */
kprintf("io_uring: soft retemblemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retemblemangle honesty; Soft≠product; not bar3)\n");
/* Grep: io_uring: soft retsigilangle — Wave 83 exclusive sigilangle stamp */
kprintf("io_uring: soft retsigilangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retsigilangle stamp; Soft≠product)\n");
/* Grep: io_uring: soft retscepterangle — Wave 84 return-scepterangle honesty */
kprintf("io_uring: soft retscepterangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retscepterangle honesty; Soft≠product; not bar3)\n");
/* Grep: io_uring: soft retglyphangle — Wave 84 exclusive glyphangle stamp */
kprintf("io_uring: soft retglyphangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retglyphangle stamp; Soft≠product)\n");
/* Grep: io_uring: soft retcrownangle — Wave 85 return-crownangle honesty */
kprintf("io_uring: soft retcrownangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retcrownangle honesty; Soft≠product; not bar3)\n");
/* Grep: io_uring: soft retshardangle — Wave 85 exclusive shardangle stamp */
kprintf("io_uring: soft retshardangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retshardangle stamp; Soft≠product)\n");
/* Grep: io_uring: soft retforgeangle — Wave 86 return-forgeangle honesty */
kprintf("io_uring: soft retforgeangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retforgeangle honesty; Soft≠product; not bar3)\n");
/* Grep: io_uring: soft retprismangle — Wave 86 exclusive prismangle stamp */
kprintf("io_uring: soft retprismangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retprismangle stamp; Soft≠product)\n");
/* Grep: io_uring: soft retflameangle — Wave 87 return-flameangle honesty */
kprintf("io_uring: soft retflameangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retflameangle honesty; Soft≠product; not bar3)\n");
/* Grep: io_uring: soft retcipherangle — Wave 87 exclusive cipherangle stamp */
kprintf("io_uring: soft retcipherangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retcipherangle stamp; Soft≠product)\n");
/* Grep: io_uring: soft retbeaconangle — Wave 88 return-beaconangle honesty */
kprintf("io_uring: soft retbeaconangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=88 (retbeaconangle honesty; Soft≠product; not bar3)\n");
/* Grep: io_uring: soft retledgerangle — Wave 88 exclusive ledgerangle stamp */
kprintf("io_uring: soft retledgerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=88 (retledgerangle stamp; Soft≠product)\n");
