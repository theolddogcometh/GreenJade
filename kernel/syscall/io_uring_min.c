 ...
 *   io_uring: soft deepen      — wave=40 areas stamp
...
#define GJ_IORING_SOFT_WAVE 40u
...
ioring_soft_log(void)
{
    ...
    kprintf("io_uring: soft note milestone=wave40 exclusive=1 "
...
/* Grep: io_uring: soft deepen wave (Wave 15 stamp) */
    ...
    kprintf("io_uring: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=40\n");
