 ...
 *   io_uring: soft deepen      — wave=80 areas stamp
...
ioring_soft_log(void)
{
    ...
    kprintf("io_uring: soft note milestone=wave80 exclusive=1 "
...
/* Grep: io_uring: soft deepen wave (Wave 15 stamp) */
    ...
    kprintf("io_uring: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=80\n");
...
kprintf("io_uring: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=80 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: io_uring: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("io_uring: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=80 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("io_uring: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=80 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: io_uring: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("io_uring: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=80 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("io_uring: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=80 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: io_uring: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("io_uring: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=80 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: io_uring: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("io_uring: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=80 (retcurtainface honesty; Soft≠product; not bar3)\n");
/* Grep: io_uring: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("io_uring: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=80 (retbastionangle stamp; Soft≠product)\n");
/* Grep: io_uring: soft retglacisangle — Wave 74 return-glacisangle honesty */
kprintf("io_uring: soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=80 (retglacisangle honesty; Soft≠product; not bar3)\n");
/* Grep: io_uring: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
kprintf("io_uring: soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=80 (retparapetangle stamp; Soft≠product)\n");
/* Grep: io_uring: soft retmoatangle — Wave 75 return-moatangle honesty */
kprintf("io_uring: soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=80 (retmoatangle honesty; Soft≠product; not bar3)\n");
/* Grep: io_uring: soft retowerangle — Wave 75 exclusive towerangle stamp */
kprintf("io_uring: soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=80 (retowerangle stamp; Soft≠product)\n");
/* Grep: io_uring: soft retgateangle — Wave 76 return-gateangle honesty */
kprintf("io_uring: soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=80 (retgateangle honesty; Soft≠product; not bar3)\n");
/* Grep: io_uring: soft retwallangle — Wave 76 exclusive wallangle stamp */
kprintf("io_uring: soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=80 (retwallangle stamp; Soft≠product)\n");
/* Grep: io_uring: soft retspireangle — Wave 77 return-spireangle honesty */
kprintf("io_uring: soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=80 (retspireangle honesty; Soft≠product; not bar3)\n");
/* Grep: io_uring: soft retholdangle — Wave 77 exclusive holdangle stamp */
kprintf("io_uring: soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=80 (retholdangle stamp; Soft≠product)\n");
/* Grep: io_uring: soft retpalaceangle — Wave 78 return-palaceangle honesty */
kprintf("io_uring: soft retpalaceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=80 (retpalaceangle honesty; Soft≠product; not bar3)\n");
/* Grep: io_uring: soft retfortressangle — Wave 78 exclusive fortressangle stamp */
kprintf("io_uring: soft retfortressangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=80 (retfortressangle stamp; Soft≠product)\n");
/* Grep: io_uring: soft retkeepangle — Wave 79 return-keepangle honesty */
kprintf("io_uring: soft retkeepangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=80 (retkeepangle honesty; Soft≠product; not bar3)\n");
/* Grep: io_uring: soft retredoubtangle — Wave 79 exclusive redoubtangle stamp */
kprintf("io_uring: soft retredoubtangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=80 (retredoubtangle stamp; Soft≠product)\n");
/* Grep: io_uring: soft retcitadelangle — Wave 80 return-citadelangle honesty */
kprintf("io_uring: soft retcitadelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=80 (retcitadelangle honesty; Soft≠product; not bar3)\n");
/* Grep: io_uring: soft retbastionkeep — Wave 80 exclusive bastionkeep stamp */
kprintf("io_uring: soft retbastionkeep exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=80 (retbastionkeep stamp; Soft≠product)\n");
