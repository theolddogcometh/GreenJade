 ...
 *
 * Soft inventory (Wave 14 base + Wave 33 exclusive deepen) — greppable "io_uring: soft …":
 ...
 *   io_uring: soft deepen      — wave=33 areas stamp
...
#define GJ_IORING_SOFT_WAVE 33u
/* Soft inventory area count (fixed greppable categories for deepen stamp). */
#define GJ_IORING_SOFT_AREAS 48u

/*
 * Soft product inventory (Wave 33 exclusive deepen). File-local sticky
...
/**
 * Greppable soft inventory (Wave 33 exclusive deepen). Prefix "io_uring: soft …".
...
ioring_soft_log(void)
{
    ...
    kprintf("io_uring: soft note milestone=wave33 exclusive=1 "
...
/* Grep: io_uring: soft deepen wave (Wave 15 stamp) */
    ...
    kprintf("io_uring: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=33\n");
 ...
 * Soft≠product; not bar3.
 */
/* Grep: io_uring: soft retbastion — Wave 33 return-bastion honesty */
kprintf("io_uring: soft retbastion soft_only=1 product_gate=0 soft_ne_product=1 "
        "never_blocks_m0=1 wave=%u "
        "(retbastion honesty; Soft≠product; not bar3)\n",
        (unsigned)GJ_IORING_SOFT_WAVE);
/* Grep: io_uring: soft retcitadel — Wave 33 exclusive citadel stamp */
kprintf("io_uring: soft retcitadel exclusive=1 soft_ne_product=1 "
        ...
        (unsigned)GJ_IORING_SOFT_WAVE);
            ...
            "(Wave 33 exclusive; min rings soft scaffold; not product game I/O; "
            "not bar3)\n",
            (unsigned)GJ_IORING_SOFT_WAVE,
            (unsigned)GJ_IORING_SOFT_AREAS,
            (unsigned long)g_soft.u64SoftLog,
