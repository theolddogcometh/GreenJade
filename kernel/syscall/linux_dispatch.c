 ...
 *
 * Soft inventory (Wave 12/14 base + Wave 32 exclusive deepen; this unit):
...
/*
 * Soft product inventory (Wave 12/14 base + Wave 32 exclusive deepen).
...
#define LINUX_DISPATCH_SOFT_WAVE 32u
#define LINUX_DISPATCH_SOFT_AREAS 58u
...
soft_dispatch_deepen_log(void)
{
    ...
    kprintf("linux: nr class soft note milestone=wave32 exclusive=1 "
 ...
 * Soft≠product; not bar3.
    ...
    kprintf("linux: dispatch soft note milestone=wave32 exclusive=1 "
            "soft_only=1 not_bar3=1 wave=%u\n",
            (unsigned)LINUX_DISPATCH_SOFT_WAVE);
 ...
 * Soft≠product; not bar3.
 */
/* Grep: linux: dispatch: soft retbulwark — Wave 32 return-bulwark honesty */
kprintf("linux: dispatch: soft retbulwark soft_only=1 product_gate=0 soft_ne_product=1 "
        "never_blocks_m0=1 wave=%u "
        "(retbulwark honesty; Soft≠product; not bar3)\n",
        (unsigned)LINUX_DISPATCH_SOFT_WAVE);
/* Grep: linux: dispatch: soft retpanoply — Wave 32 exclusive panoply stamp */
...
gj_linux_nr_class_soft_log(void)
{
    ...
    /* Wave 12 + Wave 32 exclusive deepen (additive; never hard-gates). */
