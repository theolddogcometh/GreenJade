...
#define LINUX_DISPATCH_SOFT_WAVE 82u
#define LINUX_DISPATCH_SOFT_AREAS 154u
...
soft_dispatch_deepen_log(void)
{
    ...
    kprintf("linux: nr class soft note milestone=wave82 exclusive=1 "
 ...
 * Soft≠product; not bar3.
    ...
    kprintf("linux: dispatch soft note milestone=wave82 exclusive=1 "
            "soft_only=1 not_bar3=1 wave=%u\n",
            (unsigned)LINUX_DISPATCH_SOFT_WAVE);
...
kprintf("linux: dispatch: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=82 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: linux: dispatch: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("linux: dispatch: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=82 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("linux: dispatch: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=82 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: linux: dispatch: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("linux: dispatch: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=82 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("linux: dispatch: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=82 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: linux: dispatch: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("linux: dispatch: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=82 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: linux: dispatch: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("linux: dispatch: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=82 (retcurtainface honesty; Soft≠product; not bar3)\n");
/* Grep: linux: dispatch: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("linux: dispatch: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=82 (retbastionangle stamp; Soft≠product)\n");
/* Grep: linux: dispatch: soft retglacisangle — Wave 74 return-glacisangle honesty */
kprintf("linux: dispatch: soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=82 (retglacisangle honesty; Soft≠product; not bar3)\n");
/* Grep: linux: dispatch: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
kprintf("linux: dispatch: soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=82 (retparapetangle stamp; Soft≠product)\n");
/* Grep: linux: dispatch: soft retmoatangle — Wave 75 return-moatangle honesty */
kprintf("linux: dispatch: soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=82 (retmoatangle honesty; Soft≠product; not bar3)\n");
/* Grep: linux: dispatch: soft retowerangle — Wave 75 exclusive towerangle stamp */
kprintf("linux: dispatch: soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=82 (retowerangle stamp; Soft≠product)\n");
/* Grep: linux: dispatch: soft retgateangle — Wave 76 return-gateangle honesty */
kprintf("linux: dispatch: soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=82 (retgateangle honesty; Soft≠product; not bar3)\n");
/* Grep: linux: dispatch: soft retwallangle — Wave 76 exclusive wallangle stamp */
kprintf("linux: dispatch: soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=82 (retwallangle stamp; Soft≠product)\n");
/* Grep: linux: dispatch: soft retspireangle — Wave 77 return-spireangle honesty */
kprintf("linux: dispatch: soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=82 (retspireangle honesty; Soft≠product; not bar3)\n");
/* Grep: linux: dispatch: soft retholdangle — Wave 77 exclusive holdangle stamp */
kprintf("linux: dispatch: soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=82 (retholdangle stamp; Soft≠product)\n");
/* Grep: linux: dispatch: soft retpalaceangle — Wave 78 return-palaceangle honesty */
kprintf("linux: dispatch: soft retpalaceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=82 (retpalaceangle honesty; Soft≠product; not bar3)\n");
/* Grep: linux: dispatch: soft retfortressangle — Wave 78 exclusive fortressangle stamp */
kprintf("linux: dispatch: soft retfortressangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=82 (retfortressangle stamp; Soft≠product)\n");
/* Grep: linux: dispatch: soft retkeepangle — Wave 79 return-keepangle honesty */
kprintf("linux: dispatch: soft retkeepangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=82 (retkeepangle honesty; Soft≠product; not bar3)\n");
/* Grep: linux: dispatch: soft retredoubtangle — Wave 79 exclusive redoubtangle stamp */
kprintf("linux: dispatch: soft retredoubtangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=82 (retredoubtangle stamp; Soft≠product)\n");
/* Grep: linux: dispatch: soft retcitadelangle — Wave 80 return-citadelangle honesty */
kprintf("linux: dispatch: soft retcitadelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=82 (retcitadelangle honesty; Soft≠product; not bar3)\n");
/* Grep: linux: dispatch: soft retbastionkeep — Wave 80 exclusive bastionkeep stamp */
kprintf("linux: dispatch: soft retbastionkeep exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=82 (retbastionkeep stamp; Soft≠product)\n");
/* Grep: linux: dispatch: soft retpanoplyangle — Wave 81 return-panoplyangle honesty */
kprintf("linux: dispatch: soft retpanoplyangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=82 (retpanoplyangle honesty; Soft≠product; not bar3)\n");
/* Grep: linux: dispatch: soft retbulwarkangle — Wave 81 exclusive bulwarkangle stamp */
kprintf("linux: dispatch: soft retbulwarkangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=82 (retbulwarkangle stamp; Soft≠product)\n");
/* Grep: linux: dispatch: soft retmantleangle — Wave 82 return-mantleangle honesty */
kprintf("linux: dispatch: soft retmantleangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=82 (retmantleangle honesty; Soft≠product; not bar3)\n");
/* Grep: linux: dispatch: soft retaegisangle — Wave 82 exclusive aegisangle stamp */
kprintf("linux: dispatch: soft retaegisangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=82 (retaegisangle stamp; Soft≠product)\n");
