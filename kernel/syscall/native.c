 ...
 *   native: soft deepen      — wave=44 area stamp
...
#define NATIVE_SOFT_WAVE 44u
#define NATIVE_SOFT_AREAS 78u
...
native_soft_inventory_log(void)
{
    ...
    kprintf("native: soft note milestone=wave44 exclusive=1 "
            "soft_only=1 not_bar3=1 wave=%u\n",
            (unsigned)NATIVE_SOFT_WAVE);

    /* Grep: native: soft retmap — Wave 19 return-surface map */
    kprintf("native: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=44\n");
