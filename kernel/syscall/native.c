 ...
 *
 * Soft inventory (Wave 11/14 base + Wave 32 exclusive deepen; this unit only):
 ...
 *   native: soft deepen      — wave=32 area stamp
...
/*
 * Wave 11/14 base + Wave 32 exclusive deepen (file-local; never hard-gates).
...
#define NATIVE_SOFT_WAVE 32u
#define NATIVE_SOFT_AREAS 58u
...
native_soft_inventory_log(void)
{
    ...
    kprintf("native: soft note milestone=wave32 exclusive=1 "
            "soft_only=1 not_bar3=1 wave=%u\n",
            (unsigned)NATIVE_SOFT_WAVE);

    /* Grep: native: soft retmap — Wave 19 return-surface map */
    kprintf("native: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=32\n");
 ...
 * Soft≠product; not bar3.
 */
/* Grep: native: soft retbulwark — Wave 32 return-bulwark honesty */
kprintf("native: soft retbulwark soft_only=1 product_gate=0 soft_ne_product=1 "
        "never_blocks_m0=1 wave=%u "
        "(retbulwark honesty; Soft≠product; not bar3)\n",
        (unsigned)NATIVE_SOFT_WAVE);
/* Grep: native: soft retpanoply — Wave 32 exclusive panoply stamp */
