 ...
 *   native: soft deepen      — wave=85 area stamp
...
#define NATIVE_SOFT_WAVE 85u
#define NATIVE_SOFT_AREAS 160u
...
native_soft_inventory_log(void)
{
    ...
    kprintf("native: soft note milestone=wave85 exclusive=1 "
            "soft_only=1 not_bar3=1 wave=%u\n",
            (unsigned)NATIVE_SOFT_WAVE);

    /* Grep: native: soft retmap — Wave 19 return-surface map */
    kprintf("native: soft retmap ok|fail|inval|nodev|busy|nomem product_gate=0 soft_only=1 wave=85\n");
...
kprintf("native: soft retfosseangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retfosseangle honesty; Soft≠product; not bar3)\n");
/* Grep: native: soft retcounterscarple — Wave 70 exclusive counterscarple stamp */
kprintf("native: soft retcounterscarple exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retcounterscarple stamp; Soft≠product)\n");
/*
...
kprintf("native: soft retsallyportangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retsallyportangle honesty; Soft≠product; not bar3)\n");
/* Grep: native: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp */
kprintf("native: soft retreentrantangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retreentrantangle stamp; Soft≠product)\n");
/*
...
kprintf("native: soft retplaceofarmsangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retplaceofarmsangle honesty; Soft≠product; not bar3)\n");
/* Grep: native: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp */
kprintf("native: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retdoubletenailleangle stamp; Soft≠product)\n");
/* Grep: native: soft retcurtainface — Wave 73 return-curtainface honesty */
kprintf("native: soft retcurtainface soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retcurtainface honesty; Soft≠product; not bar3)\n");
/* Grep: native: soft retbastionangle — Wave 73 exclusive bastionangle stamp */
kprintf("native: soft retbastionangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retbastionangle stamp; Soft≠product)\n");
/* Grep: native: soft retglacisangle — Wave 74 return-glacisangle honesty */
kprintf("native: soft retglacisangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retglacisangle honesty; Soft≠product; not bar3)\n");
/* Grep: native: soft retparapetangle — Wave 74 exclusive parapetangle stamp */
kprintf("native: soft retparapetangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retparapetangle stamp; Soft≠product)\n");
/* Grep: native: soft retmoatangle — Wave 75 return-moatangle honesty */
kprintf("native: soft retmoatangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retmoatangle honesty; Soft≠product; not bar3)\n");
/* Grep: native: soft retowerangle — Wave 75 exclusive towerangle stamp */
kprintf("native: soft retowerangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retowerangle stamp; Soft≠product)\n");
/* Grep: native: soft retgateangle — Wave 76 return-gateangle honesty */
kprintf("native: soft retgateangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retgateangle honesty; Soft≠product; not bar3)\n");
/* Grep: native: soft retwallangle — Wave 76 exclusive wallangle stamp */
kprintf("native: soft retwallangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retwallangle stamp; Soft≠product)\n");
/* Grep: native: soft retspireangle — Wave 77 return-spireangle honesty */
kprintf("native: soft retspireangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retspireangle honesty; Soft≠product; not bar3)\n");
/* Grep: native: soft retholdangle — Wave 77 exclusive holdangle stamp */
kprintf("native: soft retholdangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retholdangle stamp; Soft≠product)\n");
/* Grep: native: soft retpalaceangle — Wave 78 return-palaceangle honesty */
kprintf("native: soft retpalaceangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retpalaceangle honesty; Soft≠product; not bar3)\n");
/* Grep: native: soft retfortressangle — Wave 78 exclusive fortressangle stamp */
kprintf("native: soft retfortressangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retfortressangle stamp; Soft≠product)\n");
/* Grep: native: soft retkeepangle — Wave 79 return-keepangle honesty */
kprintf("native: soft retkeepangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retkeepangle honesty; Soft≠product; not bar3)\n");
/* Grep: native: soft retredoubtangle — Wave 79 exclusive redoubtangle stamp */
kprintf("native: soft retredoubtangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retredoubtangle stamp; Soft≠product)\n");
/* Grep: native: soft retcitadelangle — Wave 80 return-citadelangle honesty */
kprintf("native: soft retcitadelangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retcitadelangle honesty; Soft≠product; not bar3)\n");
/* Grep: native: soft retbastionkeep — Wave 80 exclusive bastionkeep stamp */
kprintf("native: soft retbastionkeep exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retbastionkeep stamp; Soft≠product)\n");
/* Grep: native: soft retpanoplyangle — Wave 81 return-panoplyangle honesty */
kprintf("native: soft retpanoplyangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retpanoplyangle honesty; Soft≠product; not bar3)\n");
/* Grep: native: soft retbulwarkangle — Wave 81 exclusive bulwarkangle stamp */
kprintf("native: soft retbulwarkangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retbulwarkangle stamp; Soft≠product)\n");
/* Grep: native: soft retmantleangle — Wave 82 return-mantleangle honesty */
kprintf("native: soft retmantleangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retmantleangle honesty; Soft≠product; not bar3)\n");
/* Grep: native: soft retaegisangle — Wave 82 exclusive aegisangle stamp */
kprintf("native: soft retaegisangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retaegisangle stamp; Soft≠product)\n");
/* Grep: native: soft retemblemangle — Wave 83 return-emblemangle honesty */
kprintf("native: soft retemblemangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retemblemangle honesty; Soft≠product; not bar3)\n");
/* Grep: native: soft retsigilangle — Wave 83 exclusive sigilangle stamp */
kprintf("native: soft retsigilangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retsigilangle stamp; Soft≠product)\n");
/* Grep: native: soft retscepterangle — Wave 84 return-scepterangle honesty */
kprintf("native: soft retscepterangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retscepterangle honesty; Soft≠product; not bar3)\n");
/* Grep: native: soft retglyphangle — Wave 84 exclusive glyphangle stamp */
kprintf("native: soft retglyphangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retglyphangle stamp; Soft≠product)\n");
/* Grep: native: soft retcrownangle — Wave 85 return-crownangle honesty */
kprintf("native: soft retcrownangle soft_only=1 product_gate=0 soft_ne_product=1 never_blocks_m0=1 wave=85 (retcrownangle honesty; Soft≠product; not bar3)\n");
/* Grep: native: soft retshardangle — Wave 85 exclusive shardangle stamp */
kprintf("native: soft retshardangle exclusive=1 soft_ne_product=1 product_kernel=OPEN bar3=0 wave=85 (retshardangle stamp; Soft≠product)\n");
