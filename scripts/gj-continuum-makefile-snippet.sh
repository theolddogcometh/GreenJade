...
# *decade* is a closed batch range ending at a milestone M (e.g. M=19900
# → exclusive wave 19801–19900). Soft host probes
...
#      Example generator for the 19900 wave (Wave 57 exclusive continuum track):
#        user/libcgj/src/_gen_milestone_19900.py
#        python3 user/libcgj/src/_gen_milestone_19900.py --dry-run
#      (prior Wave 56: _gen_milestone_19800.py / M=19800)
...
# graph_batch19900.c (and beyond) once those basenames are wired.
#
# Wave 57 soft note (19900-era): the exclusive target decade is M=19900
# (CREATE-ONLY 19801–19900; extension identity 19801–19850 ahead of the
# decade identity/fill/marker bands). Soft deepen surfaces this wave
# (docs/userland soft stamps): retraverse / retcasemate (CREATE-ONLY soft
# names only — not product). Until parent pastes those basenames into
# CGJ_SRCS, ``--max`` may still report the prior high-water
# (e.g. makefile_max=19800). **Scan is the source of truth** — do not
# hardcode a false makefile_max here or in side panels. Soft high-water
# may be described as **advancing toward 19900** while the honest scan
...
# reports. Soft graph growth through graph_batch19900.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_19900
# and gj_product_score_19900 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (19900-era / Wave 57 example)
# ------------------------------------------------
# After CREATE-ONLY wave 19801–19900 exists on disk:
#
...
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 19851 19900
#
...
#
# For M=19900 that is extension identity 19801–19850, decade identity
# 19851–19875, fill 19876–19890, markers 19891–19900. Prior decade
# M=19800: identity 19751–19775, fill 19776–19790, markers 19791–19800.
# See _gen_milestone_19900.py module docstring (when present) and
# user/libcgj/tests/NOTES_milestone_19900.txt (when present).
#
# Usage:
...
#   user/libcgj/src/graph_batch19800.c \   ← prior high-water (if still tip)
#   user/libcgj/src/graph_batch19801.c \   ← Wave 57 helper output starts here
#   … through graph_batch19900.c once CREATE-ONLY + parent wire land
#
...
#
# See also:
#   user/libcgj/src/_gen_milestone_19800.py   (prior Wave 56 CREATE-ONLY emitter)
#   user/libcgj/src/_gen_milestone_19900.py   (Wave 57 CREATE-ONLY emitter, when present)
#   user/libcgj/tests/NOTES_milestone_19800.txt
#   user/libcgj/tests/NOTES_milestone_19900.txt
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch19900.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 57 exclusive target decade: M=19900 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Soft deepen surfaces: retraverse /
  retcasemate (CREATE-ONLY soft names only). Until parent wires 19801–19900,
  --max may still report prior tip (e.g. makefile_max=19800). Soft high-water
  may advance toward 19900 while scan remains at prior tip.

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_19900.py)
...
Decade bands for milestone M=19900 (docs only; helper emits paths only):
  extension 19801-19850 | identity 19851-19875 | fill 19876-19890 | markers 19891-19900
  (prior M=19800: identity 19751-19775 | fill 19776-19790 | markers 19791-19800)
