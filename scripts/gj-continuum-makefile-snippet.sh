...
# *decade* is a closed batch range ending at a milestone M (e.g. M=20400
# → exclusive wave 20301–20400). Soft host probes
...
#      Example generator for the 20400 wave (Wave 62 exclusive continuum track):
#        user/libcgj/src/_gen_milestone_20400.py
#        python3 user/libcgj/src/_gen_milestone_20400.py --dry-run
#      (prior Wave 61: _gen_milestone_20300.py / M=20300)
...
# graph_batch20400.c (and beyond) once those basenames are wired.
#
# Wave 62 soft note (20400-era): the exclusive target decade is M=20400
# (CREATE-ONLY 20301–20400; extension identity 20301–20350 ahead of the
# decade identity/fill/marker bands). Soft deepen surfaces this wave
# (docs/userland soft stamps): retbastionface / retcurtainangle (CREATE-ONLY soft
# names only — not product). Until parent pastes those basenames into
# CGJ_SRCS, ``--max`` may still report the prior high-water
# (e.g. makefile_max=20300). **Scan is the source of truth** — do not
# hardcode a false makefile_max here or in side panels. Soft high-water
# may be described as **advancing toward 20400** while the honest scan
...
# reports. Soft graph growth through graph_batch20400.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_20400
# and gj_product_score_20400 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (20400-era / Wave 62 example)
# ------------------------------------------------
# After CREATE-ONLY wave 20301–20400 exists on disk:
#
...
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 20351 20400
#
...
#
# For M=20400 that is extension identity 20301–20350, decade identity
# 20351–20375, fill 20376–20390, markers 20391–20400. Prior decade
# M=20300: identity 20251–20275, fill 20276–20290, markers 20291–20300.
# See _gen_milestone_20400.py module docstring (when present) and
# user/libcgj/tests/NOTES_milestone_20400.txt (when present).
#
# Usage:
...
#   user/libcgj/src/graph_batch20300.c \   ← prior high-water (if still tip)
#   user/libcgj/src/graph_batch20301.c \   ← Wave 62 helper output starts here
#   … through graph_batch20400.c once CREATE-ONLY + parent wire land
#
...
#
# See also:
#   user/libcgj/src/_gen_milestone_20300.py   (prior Wave 61 CREATE-ONLY emitter)
#   user/libcgj/src/_gen_milestone_20400.py   (Wave 62 CREATE-ONLY emitter, when present)
#   user/libcgj/tests/NOTES_milestone_20300.txt
#   user/libcgj/tests/NOTES_milestone_20400.txt
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch20400.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 62 exclusive target decade: M=20400 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Soft deepen surfaces: retbastionface /
  retcurtainangle (CREATE-ONLY soft names only). Until parent wires 20301–20400,
  --max may still report prior tip (e.g. makefile_max=20300). Soft high-water
  may advance toward 20400 while scan remains at prior tip.

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_20400.py)
...
Decade bands for milestone M=20400 (docs only; helper emits paths only):
  extension 20301-20350 | identity 20351-20375 | fill 20376-20390 | markers 20391-20400
  (prior M=20300: identity 20251-20275 | fill 20276-20290 | markers 20291-20300)
