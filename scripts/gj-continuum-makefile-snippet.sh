...
# *decade* is a closed batch range ending at a milestone M (e.g. M=25300
# → exclusive wave 25201–25300). Soft host probes
...
# graph_batch25300.c (and beyond) once those basenames are wired.
#
# Wave 111 soft note (25300-era): the exclusive target decade is M=25300
# (CREATE-ONLY 25201–25300; extension identity 25201–25250 ahead of the
# decade identity/fill/marker bands). Soft deepen surfaces this wave
# (docs/userland soft stamps): retvoxelangle / rettexelangle (CREATE-ONLY soft
# names only — not product). Until parent pastes those basenames into
# CGJ_SRCS, ``--max`` may still report the prior high-water
# (e.g. makefile_max=25200). **Scan is the source of truth** — do not
# hardcode a false makefile_max here or in side panels. Soft high-water
# may be described as **advancing toward 25300** while the honest scan
# remains at the prior tip (often makefile_max=25200).
#
...
# reports. Soft graph growth through graph_batch25300.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_25300
# and gj_product_score_25300 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (25300-era / Wave 111 example)
# ------------------------------------------------
# After CREATE-ONLY wave 25201–25300 exists on disk:
#
...
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 25251 25300
#
...
#   user/libcgj/src/graph_batch25201.c \   ← Wave 111 helper output starts here
#   … through graph_batch25300.c once CREATE-ONLY + parent wire land
#
...
#   user/libcgj/src/_gen_milestone_25300.py   (Wave 111 CREATE-ONLY emitter, when present)
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch25300.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 111 exclusive target decade: M=25300 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Soft deepen surfaces: retvoxelangle /
  rettexelangle (CREATE-ONLY soft names only). Until parent wires 25201–25300,
  --max may still report prior tip (e.g. makefile_max=25200). Soft high-water
  may advance toward 25300 while scan remains at prior tip (often 25200).

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_25300.py)
...
Decade bands for milestone M=25300 (docs only; helper emits paths only):
  extension 25201-25250 | identity 25251-25275 | fill 25276-25290 | markers 25291-25300
