...
# *decade* is a closed batch range ending at a milestone M (e.g. M=16800
# → exclusive wave 16751–16800). Soft host probes
...
#      Example generator for the 16800 wave (Wave 26 exclusive continuum track):
#        user/libcgj/src/_gen_milestone_16800.py
#        python3 user/libcgj/src/_gen_milestone_16800.py --dry-run
#      (prior Wave 25: _gen_milestone_16700.py / M=16700)
...
# graph_batch16800.c (and beyond) once those basenames are wired.
#
# Wave 26 soft note (16800-era): the exclusive target decade is M=16800
# (CREATE-ONLY 16751–16800; optional extension 16701–16750). Until parent
# pastes those basenames into CGJ_SRCS, ``--max`` may still report the
# prior high-water (e.g. makefile_max=16700). **Scan is the source of
# truth** — do not hardcode a false makefile_max here or in side panels.
# Soft high-water may be described as **advancing toward 16800** while the
...
# reports. Soft graph growth through graph_batch16800.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_16800
# and gj_product_score_16800 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (16800-era / Wave 26 example)
# ------------------------------------------------
# After CREATE-ONLY wave 16751–16800 exists on disk (optional extension
# 16701–16750 may land ahead of the decade bands):
#
#   # Prefer exist-only so missing TUs never produce dead Makefile lines:
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 16751 16800
...
#
# For M=16800 that is identity 16751–16775, fill 16776–16790,
# markers 16791–16800. Continuum extension 16701–16750 sits ahead of
# the decade bands. Prior decade M=16700: identity 16651–16675,
# fill 16676–16690, markers 16691–16700.
# See _gen_milestone_16700.py module docstring (when present) and
# user/libcgj/tests/NOTES_milestone_16700.txt (when present).
#
# Usage:
#   ./scripts/gj-continuum-makefile-snippet.sh 16651 16700
#   ./scripts/gj-continuum-makefile-snippet.sh 16751 16800
#   ./scripts/gj-continuum-makefile-snippet.sh --next 100
#   ./scripts/gj-continuum-makefile-snippet.sh --next          # default decade 100
...
#   user/libcgj/src/graph_batch16700.c \   ← prior high-water (if still tip)
#   user/libcgj/src/graph_batch16701.c \   ← Wave 26 helper output starts here
#   … through graph_batch16800.c once CREATE-ONLY + parent wire land
#
...
#
# See also:
#   user/libcgj/src/_gen_milestone_16700.py   (prior Wave 25 CREATE-ONLY emitter)
#   user/libcgj/src/_gen_milestone_16800.py   (Wave 26 CREATE-ONLY emitter, when present)
#   user/libcgj/tests/NOTES_milestone_16700.txt
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch16800.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 26 exclusive target decade: M=16800 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Until parent wires 16701–16800, --max may
  still report prior tip (e.g. makefile_max=16700). Soft high-water may
  advance toward 16800 while scan remains at prior tip.

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_16800.py)
...
Decade bands for milestone M=16800 (docs only; helper emits paths only):
  identity 16751-16775 | fill 16776-16790 | markers 16791-16800
  extension 16701-16750 (pre-decade; may land ahead of identity/fill/markers)
  (prior M=16700: identity 16651-16675 | fill 16676-16690 | markers 16691-16700)
