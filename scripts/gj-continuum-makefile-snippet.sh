...
# *decade* is a closed batch range ending at a milestone M (e.g. M=16700
# → exclusive wave 16651–16700). Soft host probes
...
#      Example generator for the 16700 wave (Wave 25 exclusive continuum track):
#        user/libcgj/src/_gen_milestone_16700.py
#        python3 user/libcgj/src/_gen_milestone_16700.py --dry-run
#      (prior Wave 24: _gen_milestone_16600.py / M=16600)
...
# graph_batch16700.c (and beyond) once those basenames are wired.
#
# Wave 25 soft note (16700-era): the exclusive target decade is M=16700
# (CREATE-ONLY 16651–16700; optional extension 16601–16650). Until parent
# pastes those basenames into CGJ_SRCS, ``--max`` may still report the
# prior high-water (e.g. makefile_max=16600). **Scan is the source of
# truth** — do not hardcode a false makefile_max here or in side panels.
# Soft high-water may be described as **advancing toward 16700** while the
...
# reports. Soft graph growth through graph_batch16700.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_16700
# and gj_product_score_16700 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (16700-era / Wave 25 example)
# ------------------------------------------------
# After CREATE-ONLY wave 16651–16700 exists on disk (optional extension
# 16601–16650 may land ahead of the decade bands):
#
#   # Prefer exist-only so missing TUs never produce dead Makefile lines:
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 16651 16700
...
#
# For M=16700 that is identity 16651–16675, fill 16676–16690,
# markers 16691–16700. Continuum extension 16601–16650 sits ahead of
# the decade bands. Prior decade M=16600: identity 16551–16575,
# fill 16576–16590, markers 16591–16600.
# See _gen_milestone_16600.py module docstring (when present) and
# user/libcgj/tests/NOTES_milestone_16600.txt (when present).
#
# Usage:
...
#   user/libcgj/src/graph_batch16600.c \   ← prior high-water (if still tip)
#   user/libcgj/src/graph_batch16601.c \   ← Wave 25 helper output starts here
#   … through graph_batch16700.c once CREATE-ONLY + parent wire land
#
...
#
# See also:
#   user/libcgj/src/_gen_milestone_16600.py   (prior Wave 24 CREATE-ONLY emitter)
#   user/libcgj/src/_gen_milestone_16700.py   (Wave 25 CREATE-ONLY emitter, when present)
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch16700.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 25 exclusive target decade: M=16700 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Until parent wires 16601–16700, --max may
  still report prior tip (e.g. makefile_max=16600). Soft high-water may
  advance toward 16700 while scan remains at prior tip.

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_16700.py)
...
Decade bands for milestone M=16700 (docs only; helper emits paths only):
  identity 16651-16675 | fill 16676-16690 | markers 16691-16700
  extension 16601-16650 (pre-decade; may land ahead of identity/fill/markers)
  (prior M=16600: identity 16551-16575 | fill 16576-16590 | markers 16591-16600)
