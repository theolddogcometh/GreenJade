...
# *decade* is a closed batch range ending at a milestone M (e.g. M=19600
# → exclusive wave 19501–19600). Soft host probes
...
#      Example generator for the 19600 wave (Wave 54 exclusive continuum track):
#        user/libcgj/src/_gen_milestone_19600.py
#        python3 user/libcgj/src/_gen_milestone_19600.py --dry-run
#      (prior Wave 53: _gen_milestone_19500.py / M=19500)
...
# graph_batch19600.c (and beyond) once those basenames are wired.
#
# Wave 54 soft note (19600-era): the exclusive target decade is M=19600
# (CREATE-ONLY 19501–19600; extension identity 19501–19550 ahead of the
# decade identity/fill/marker bands). Soft deepen surfaces this wave
# (docs/userland soft stamps): retcaponier / retredan (CREATE-ONLY soft
# names only — not product). Until parent pastes those basenames into
# CGJ_SRCS, ``--max`` may still report the prior high-water
# (e.g. makefile_max=19500). **Scan is the source of truth** — do not
# hardcode a false makefile_max here or in side panels. Soft high-water
# may be described as **advancing toward 19600** while the honest scan
...
# reports. Soft graph growth through graph_batch19600.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_19600
# and gj_product_score_19600 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (19600-era / Wave 54 example)
# ------------------------------------------------
# After CREATE-ONLY wave 19501–19600 exists on disk:
#
...
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 19551 19600
#
...
#
# For M=19600 that is extension identity 19501–19550, decade identity
# 19551–19575, fill 19576–19590, markers 19591–19600. Prior decade
# M=19500: identity 19451–19475, fill 19476–19490, markers 19491–19500.
# See _gen_milestone_19600.py module docstring (when present) and
# user/libcgj/tests/NOTES_milestone_19600.txt (when present).
#
# Usage:
...
#   user/libcgj/src/graph_batch19500.c \   ← prior high-water (if still tip)
#   user/libcgj/src/graph_batch19501.c \   ← Wave 54 helper output starts here
#   … through graph_batch19600.c once CREATE-ONLY + parent wire land
#
...
#
# See also:
#   user/libcgj/src/_gen_milestone_19500.py   (prior Wave 53 CREATE-ONLY emitter)
#   user/libcgj/src/_gen_milestone_19600.py   (Wave 54 CREATE-ONLY emitter, when present)
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch19600.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 54 exclusive target decade: M=19600 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Soft deepen surfaces: retcaponier /
  retredan (CREATE-ONLY soft names only). Until parent wires 19501–19600,
  --max may still report prior tip (e.g. makefile_max=19500). Soft high-water
  may advance toward 19600 while scan remains at prior tip.

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_19600.py)
...
Decade bands for milestone M=19600 (docs only; helper emits paths only):
  extension 19501-19550 | identity 19551-19575 | fill 19576-19590 | markers 19591-19600
  (prior M=19500: identity 19451-19475 | fill 19476-19490 | markers 19491-19500)
