...
# *decade* is a closed batch range ending at a milestone M (e.g. M=25500
# → exclusive wave 25401–25500). Soft host probes
...
#      Example generator for the 25500 wave (Wave 113 exclusive continuum track):
#        user/libcgj/src/_gen_milestone_25500.py
...
# graph_batch25500.c (and beyond) once those basenames are wired.
#
# Wave 113 soft note (25500-era): the exclusive target decade is M=25500
# (CREATE-ONLY 25401–25500; extension identity 25401–25450 ahead of the
# decade identity/fill/marker bands). Soft deepen surfaces this wave
# (docs/userland soft stamps): retshaderangle / retpipelineangle (CREATE-ONLY soft
# names only — not product). Until parent pastes those basenames into
# CGJ_SRCS, ``--max`` may still report the prior high-water
# (e.g. makefile_max=25400). **Scan is the source of truth** — do not
# hardcode a false makefile_max here or in side panels. Soft high-water
# may be described as **advancing toward 25500** while the honest scan
# remains at the prior tip (often makefile_max=25400).
#
...
# reports. Soft graph growth through graph_batch25500.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_25500
# and gj_product_score_25500 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (25500-era / Wave 113 example)
# ------------------------------------------------
# After CREATE-ONLY wave 25401–25500 exists on disk:
#
...
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 25451 25500
#
...
#   user/libcgj/src/graph_batch25400.c \   ← prior high-water (if still tip)
#   user/libcgj/src/graph_batch25401.c \   ← Wave 113 helper output starts here
#   … through graph_batch25500.c once CREATE-ONLY + parent wire land
#
...
#   user/libcgj/src/_gen_milestone_25500.py   (Wave 113 CREATE-ONLY emitter, when present)
#   user/libcgj/src/_gen_milestone_25400.py   (prior Wave 112 CREATE-ONLY emitter, when present)
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch25500.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 113 exclusive target decade: M=25500 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Soft deepen surfaces: retshaderangle /
  retpipelineangle (CREATE-ONLY soft names only). Until parent wires 25401–25500,
  --max may still report prior tip (e.g. makefile_max=25400). Soft high-water
  may advance toward 25500 while scan remains at prior tip (often 25400).

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_25500.py)
...
Decade bands for milestone M=25500 (docs only; helper emits paths only):
  extension 25401-25450 | identity 25451-25475 | fill 25476-25490 | markers 25491-25500
