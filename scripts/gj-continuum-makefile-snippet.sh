...
# *decade* is a closed batch range ending at a milestone M (e.g. M=18300
# → exclusive wave 18251–18300). Soft host probes
...
#      Example generator for the 18300 wave (Wave 41 exclusive continuum track):
#        user/libcgj/src/_gen_milestone_18300.py
#        python3 user/libcgj/src/_gen_milestone_18300.py --dry-run
#      (prior Wave 40: _gen_milestone_18200.py / M=18200)
...
# graph_batch18300.c (and beyond) once those basenames are wired.
#
# Wave 41 soft note (18300-era): the exclusive target decade is M=18300
# (CREATE-ONLY 18251–18300; optional extension 18201–18250). Soft deepen
# surfaces this wave (docs/userland soft stamps): retravelin / retditch
...
# (e.g. makefile_max=18200). **Scan is the source of truth** — do not
# hardcode a false makefile_max here or in side panels. Soft high-water
# may be described as **advancing toward 18300** while the honest scan
...
# reports. Soft graph growth through graph_batch18300.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_18300
# and gj_product_score_18300 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (18300-era / Wave 41 example)
# ------------------------------------------------
# After CREATE-ONLY wave 18251–18300 exists on disk (optional extension
# 18201–18250 may land ahead of the decade bands):
#
#   # Prefer exist-only so missing TUs never produce dead Makefile lines:
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 18251 18300
...
#   ./scripts/gj-continuum-makefile-snippet.sh 18251 18300
#
...
#
# For M=18300 that is identity 18251–18275, fill 18276–18290,
# markers 18291–18300. Continuum extension 18201–18250 sits ahead of
# the decade bands. Prior decade M=18200: identity 18151–18175,
# fill 18176–18190, markers 18191–18200.
# See _gen_milestone_18300.py module docstring (when present) and
# user/libcgj/tests/NOTES_milestone_18300.txt (when present).
#
# Usage:
...
#   user/libcgj/src/graph_batch18200.c \   ← prior high-water (if still tip)
#   user/libcgj/src/graph_batch18201.c \   ← Wave 41 helper output starts here
#   … through graph_batch18300.c once CREATE-ONLY + parent wire land
#
...
#
# See also:
#   user/libcgj/src/_gen_milestone_18200.py   (prior Wave 40 CREATE-ONLY emitter)
#   user/libcgj/src/_gen_milestone_18300.py   (Wave 41 CREATE-ONLY emitter, when present)
#   user/libcgj/tests/NOTES_milestone_18200.txt
#   user/libcgj/tests/NOTES_milestone_18300.txt
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch18300.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 41 exclusive target decade: M=18300 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Soft deepen surfaces: retravelin /
  retditch (CREATE-ONLY soft names only). Until parent wires 18201–18300,
  --max may still report prior tip (e.g. makefile_max=18200). Soft high-water
  may advance toward 18300 while scan remains at prior tip.

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_18300.py)
...
Decade bands for milestone M=18300 (docs only; helper emits paths only):
  identity 18251-18275 | fill 18276-18290 | markers 18291-18300
  extension 18201-18250 (pre-decade; may land ahead of identity/fill/markers)
  (prior M=18200: identity 18151-18175 | fill 18176-18190 | markers 18191-18200)
