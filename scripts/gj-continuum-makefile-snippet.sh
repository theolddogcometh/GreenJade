...
# *decade* is a closed batch range ending at a milestone M (e.g. M=20000
# → exclusive wave 19901–20000). Soft host probes
...
#      Example generator for the 20000 wave (Wave 58 exclusive continuum track):
#        user/libcgj/src/_gen_milestone_20000.py
#        python3 user/libcgj/src/_gen_milestone_20000.py --dry-run
#      (prior Wave 57: _gen_milestone_19900.py / M=19900)
...
# graph_batch20000.c (and beyond) once those basenames are wired.
#
# Wave 58 soft note (20000-era): the exclusive target decade is M=20000
# (CREATE-ONLY 19901–20000; extension identity 19901–19950 ahead of the
# decade identity/fill/marker bands). Soft deepen surfaces this wave
# (docs/userland soft stamps): retorillon / retbonnette (CREATE-ONLY soft
# names only — not product). Until parent pastes those basenames into
# CGJ_SRCS, ``--max`` may still report the prior high-water
# (e.g. makefile_max=19900). **Scan is the source of truth** — do not
# hardcode a false makefile_max here or in side panels. Soft high-water
# may be described as **advancing toward 20000** while the honest scan
...
# reports. Soft graph growth through graph_batch20000.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_20000
# and gj_product_score_20000 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (20000-era / Wave 58 example)
# ------------------------------------------------
# After CREATE-ONLY wave 19901–20000 exists on disk:
#
...
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 19951 20000
#
...
#
# For M=20000 that is extension identity 19901–19950, decade identity
# 19951–19975, fill 19976–19990, markers 19991–20000. Prior decade
# M=19900: identity 19851–19875, fill 19876–19890, markers 19891–19900.
# See _gen_milestone_20000.py module docstring (when present) and
# user/libcgj/tests/NOTES_milestone_20000.txt (when present).
#
# Usage:
...
#   user/libcgj/src/graph_batch19900.c \   ← prior high-water (if still tip)
#   user/libcgj/src/graph_batch19901.c \   ← Wave 58 helper output starts here
#   … through graph_batch20000.c once CREATE-ONLY + parent wire land
#
...
#
# See also:
#   user/libcgj/src/_gen_milestone_19900.py   (prior Wave 57 CREATE-ONLY emitter)
#   user/libcgj/src/_gen_milestone_20000.py   (Wave 58 CREATE-ONLY emitter, when present)
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch20000.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 58 exclusive target decade: M=20000 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Soft deepen surfaces: retorillon /
  retbonnette (CREATE-ONLY soft names only). Until parent wires 19901–20000,
  --max may still report prior tip (e.g. makefile_max=19900). Soft high-water
  may advance toward 20000 while scan remains at prior tip.

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_20000.py)
...
Decade bands for milestone M=20000 (docs only; helper emits paths only):
  extension 19901-19950 | identity 19951-19975 | fill 19976-19990 | markers 19991-20000
  (prior M=19900: identity 19851-19875 | fill 19876-19890 | markers 19891-19900)
