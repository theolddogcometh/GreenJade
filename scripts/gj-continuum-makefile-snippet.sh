...
# *decade* is a closed batch range ending at a milestone M (e.g. M=24800
# → exclusive wave 24701–24800). Soft host probes
...
# graph_batch24800.c (and beyond) once those basenames are wired.
#
# Wave 106 soft note (24800-era): the exclusive target decade is M=24800
# (CREATE-ONLY 24701–24800; extension identity 24701–24750 ahead of the
# decade identity/fill/marker bands). Soft deepen surfaces this wave
# (docs/userland soft stamps): retquarticangle / retquinticangle (CREATE-ONLY soft
# names only — not product). Until parent pastes those basenames into
# CGJ_SRCS, ``--max`` may still report the prior high-water
# (e.g. makefile_max=24700). **Scan is the source of truth** — do not
# hardcode a false makefile_max here or in side panels. Soft high-water
# may be described as **advancing toward 24800** while the honest scan
# remains at the prior tip (often makefile_max=24700).
#
...
# reports. Soft graph growth through graph_batch24800.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_24800
# and gj_product_score_24800 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (24800-era / Wave 106 example)
# ------------------------------------------------
# After CREATE-ONLY wave 24701–24800 exists on disk:
#
...
#   user/libcgj/src/graph_batch24701.c \   ← Wave 106 helper output starts here
#   … through graph_batch24800.c once CREATE-ONLY + parent wire land
#
...
#   user/libcgj/src/_gen_milestone_24800.py   (Wave 106 CREATE-ONLY emitter, when present)
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch24800.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 106 exclusive target decade: M=24800 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Soft deepen surfaces: retquarticangle /
  retquinticangle (CREATE-ONLY soft names only). Until parent wires 24701–24800,
  --max may still report prior tip (e.g. makefile_max=24700). Soft high-water
  may advance toward 24800 while scan remains at prior tip (often 24700).

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_24800.py)
...
Decade bands for milestone M=24800 (docs only; helper emits paths only):
  extension 24701-24750 | identity 24751-24775 | fill 24776-24790 | markers 24791-24800
