...
# *decade* is a closed batch range ending at a milestone M (e.g. M=21100
# → exclusive wave 21001–21100). Soft host probes
...
#      Example generator for the 21100 wave (Wave 69 exclusive continuum track):
#        user/libcgj/src/_gen_milestone_21100.py
#        python3 user/libcgj/src/_gen_milestone_21100.py --dry-run
#      (prior Wave 68: _gen_milestone_21000.py / M=21000)
...
# graph_batch21100.c (and beyond) once those basenames are wired.
#
# Wave 69 soft note (21100-era): the exclusive target decade is M=21100
# (CREATE-ONLY 21001–21100; extension identity 21001–21050 ahead of the
# decade identity/fill/marker bands). Soft deepen surfaces this wave
# (docs/userland soft stamps): retdemiluneangle / retcoveredwayangle (CREATE-ONLY soft
# names only — not product). Until parent pastes those basenames into
# CGJ_SRCS, ``--max`` may still report the prior high-water
# (e.g. makefile_max=21000). **Scan is the source of truth** — do not
# hardcode a false makefile_max here or in side panels. Soft high-water
# may be described as **advancing toward 21100** while the honest scan
...
# reports. Soft graph growth through graph_batch21100.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_21100
# and gj_product_score_21100 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (21100-era / Wave 69 example)
# ------------------------------------------------
# After CREATE-ONLY wave 21001–21100 exists on disk:
#
...
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 21051 21100
#
...
#
# For M=21100 that is extension identity 21001–21050, decade identity
# 21051–21075, fill 21076–21090, markers 21091–21100. Prior decade
# M=21000: identity 20951–20975, fill 20976–20990, markers 20991–21000.
# See _gen_milestone_21100.py module docstring (when present) and
# user/libcgj/tests/NOTES_milestone_21100.txt (when present).
#
# Usage:
...
#   user/libcgj/src/graph_batch21000.c \   ← prior high-water (if still tip)
#   user/libcgj/src/graph_batch21001.c \   ← Wave 69 helper output starts here
#   … through graph_batch21100.c once CREATE-ONLY + parent wire land
#
...
#
# See also:
#   user/libcgj/src/_gen_milestone_21000.py   (prior Wave 68 CREATE-ONLY emitter)
#   user/libcgj/src/_gen_milestone_21100.py   (Wave 69 CREATE-ONLY emitter, when present)
#   user/libcgj/tests/NOTES_milestone_21000.txt
#   user/libcgj/tests/NOTES_milestone_21100.txt
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch21100.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 69 exclusive target decade: M=21100 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Soft deepen surfaces: retdemiluneangle /
  retcoveredwayangle (CREATE-ONLY soft names only). Until parent wires 21001–21100,
  --max may still report prior tip (e.g. makefile_max=21000). Soft high-water
  may advance toward 21100 while scan remains at prior tip.

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_21100.py)
...
Decade bands for milestone M=21100 (docs only; helper emits paths only):
  extension 21001-21050 | identity 21051-21075 | fill 21076-21090 | markers 21091-21100
  (prior M=21000: identity 20951-20975 | fill 20976-20990 | markers 20991-21000)
