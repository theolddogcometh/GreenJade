...
# *decade* is a closed batch range ending at a milestone M (e.g. M=22400
# → exclusive wave 22301–22400). Soft host probes
...
#      Example generator for the 22400 wave (Wave 82 exclusive continuum track):
#        user/libcgj/src/_gen_milestone_22400.py
#        python3 user/libcgj/src/_gen_milestone_22400.py --dry-run
#      (prior Wave 81: _gen_milestone_22300.py / M=22300; Wave 80: _gen_milestone_22200.py / M=22200; Wave 79: _gen_milestone_22100.py / M=22100)
...
# graph_batch22400.c (and beyond) once those basenames are wired.
#
# Wave 82 soft note (22400-era): the exclusive target decade is M=22400
# (CREATE-ONLY 22301–22400; extension identity 22301–22350 ahead of the
# decade identity/fill/marker bands). Soft deepen surfaces this wave
# (docs/userland soft stamps): retmantleangle / retaegisangle (CREATE-ONLY soft
# names only — not product). Until parent pastes those basenames into
# CGJ_SRCS, ``--max`` may still report the prior high-water
# (e.g. makefile_max=22300). **Scan is the source of truth** — do not
# hardcode a false makefile_max here or in side panels. Soft high-water
# may be described as **advancing toward 22400** while the honest scan
...
# reports. Soft graph growth through graph_batch22400.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_22400
# and gj_product_score_22400 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (22400-era / Wave 82 example)
# ------------------------------------------------
# After CREATE-ONLY wave 22301–22400 exists on disk:
#
...
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 22351 22400
#
...
#
# For M=22400 that is extension identity 22301–22350, decade identity
# 22351–22375, fill 22376–22390, markers 22391–22400. Prior decade M=22300: identity 22251–22275, fill 22276–22290, markers 22291–22300. Prior decade
# M=22200: identity 22151–22175, fill 22176–22190, markers 22191–22200.
# See _gen_milestone_22400.py module docstring (when present) and
# user/libcgj/tests/NOTES_milestone_22300.txt
#   user/libcgj/tests/NOTES_milestone_22400.txt (when present).
#
# Usage:
...
#   user/libcgj/src/graph_batch22300.c \   ← prior high-water (if still tip)
#   user/libcgj/src/graph_batch22301.c \   ← Wave 82 helper output starts here
#   … through graph_batch22400.c once CREATE-ONLY + parent wire land
#
...
#   user/libcgj/src/_gen_milestone_22400.py   (Wave 82 CREATE-ONLY emitter, when present)
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch22400.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 82 exclusive target decade: M=22400 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Soft deepen surfaces: retmantleangle /
  retaegisangle (CREATE-ONLY soft names only). Until parent wires 22301–22400,
  --max may still report prior tip (e.g. makefile_max=22300). Soft high-water
  may advance toward 22400 while scan remains at prior tip.

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_22400.py)
...
Decade bands for milestone M=22400 (docs only; helper emits paths only):
  extension 22301-22350 | identity 22351-22375 | fill 22376-22390 | markers 22391-22400
