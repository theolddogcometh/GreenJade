...
# *decade* is a closed batch range ending at a milestone M (e.g. M=23200
# → exclusive wave 23101–23200). Soft host probes
...
# graph_batch23200.c (and beyond) once those basenames are wired.
#
# Wave 90 soft note (23200-era): the exclusive target decade is M=23200
# (CREATE-ONLY 23101–23200; extension identity 23101–23150 ahead of the
# decade identity/fill/marker bands). Soft deepen surfaces this wave
# (docs/userland soft stamps): retcrestangle / rettokenangle (CREATE-ONLY soft
# names only — not product). Until parent pastes those basenames into
# CGJ_SRCS, ``--max`` may still report the prior high-water
# (e.g. makefile_max=23100). **Scan is the source of truth** — do not
# hardcode a false makefile_max here or in side panels. Soft high-water
# may be described as **advancing toward 23200** while the honest scan
...
# reports. Soft graph growth through graph_batch23200.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_23200
# and gj_product_score_23200 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (23200-era / Wave 90 example)
# ------------------------------------------------
# After CREATE-ONLY wave 23101–23200 exists on disk:
#
...
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 23151 23200
#
...
#   user/libcgj/tests/NOTES_milestone_23200.txt (when present).
#
# Usage:
...
#   user/libcgj/src/graph_batch23100.c \   ← prior high-water (if still tip)
#   user/libcgj/src/graph_batch23101.c \   ← Wave 90 helper output starts here
#   … through graph_batch23200.c once CREATE-ONLY + parent wire land
#
...
#   user/libcgj/src/_gen_milestone_23200.py   (Wave 90 CREATE-ONLY emitter, when present)
...
#   user/libcgj/tests/NOTES_milestone_23000.txt
#   user/libcgj/tests/NOTES_milestone_23100.txt
#   user/libcgj/tests/NOTES_milestone_23200.txt
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch23200.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 90 exclusive target decade: M=23200 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Soft deepen surfaces: retcrestangle /
  rettokenangle (CREATE-ONLY soft names only). Until parent wires 23101–23200,
  --max may still report prior tip (e.g. makefile_max=23100). Soft high-water
  may advance toward 23200 while scan remains at prior tip.

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_23200.py)
...
Decade bands for milestone M=23200 (docs only; helper emits paths only):
  extension 23101-23150 | identity 23151-23175 | fill 23176-23190 | markers 23191-23200
  (prior M=23100: identity 23051-23075 | fill 23076-23090 | markers 23091-23100)
