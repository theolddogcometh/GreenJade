...
# *decade* is a closed batch range ending at a milestone M (e.g. M=17600
# → exclusive wave 17551–17600). Soft host probes
...
#      Example generator for the 17600 wave (Wave 34 exclusive continuum track):
#        user/libcgj/src/_gen_milestone_17600.py
#        python3 user/libcgj/src/_gen_milestone_17600.py --dry-run
#      (prior Wave 33: _gen_milestone_17500.py / M=17500)
...
# graph_batch17600.c (and beyond) once those basenames are wired.
#
# Wave 34 soft note (17600-era): the exclusive target decade is M=17600
# (CREATE-ONLY 17551–17600; optional extension 17501–17550). Until parent
# pastes those basenames into CGJ_SRCS, ``--max`` may still report the
# prior high-water (e.g. makefile_max=17500). **Scan is the source of
# truth** — do not hardcode a false makefile_max here or in side panels.
# Soft high-water may be described as **advancing toward 17600** while the
...
# reports. Soft graph growth through graph_batch17600.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_17600
# and gj_product_score_17600 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (17600-era / Wave 34 example)
# ------------------------------------------------
# After CREATE-ONLY wave 17551–17600 exists on disk (optional extension
# 17501–17550 may land ahead of the decade bands):
#
#   # Prefer exist-only so missing TUs never produce dead Makefile lines:
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 17551 17600
...
#   ./scripts/gj-continuum-makefile-snippet.sh 17551 17600
#
...
#
# For M=17600 that is identity 17551–17575, fill 17576–17590,
# markers 17591–17600. Continuum extension 17501–17550 sits ahead of
# the decade bands. Prior decade M=17500: identity 17451–17475,
# fill 17476–17490, markers 17491–17500.
# See _gen_milestone_17600.py module docstring (when present) and
# user/libcgj/tests/NOTES_milestone_17600.txt (when present).
#
# Usage:
...
#   user/libcgj/src/graph_batch17500.c \   ← prior high-water (if still tip)
#   user/libcgj/src/graph_batch17501.c \   ← Wave 34 helper output starts here
#   … through graph_batch17600.c once CREATE-ONLY + parent wire land
#
...
#
# See also:
#   user/libcgj/src/_gen_milestone_17500.py   (prior Wave 33 CREATE-ONLY emitter)
#   user/libcgj/src/_gen_milestone_17600.py   (Wave 34 CREATE-ONLY emitter, when present)
#   user/libcgj/tests/NOTES_milestone_17500.txt
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch17600.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 34 exclusive target decade: M=17600 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Until parent wires 17501–17600, --max may
  still report prior tip (e.g. makefile_max=17500). Soft high-water may
  advance toward 17600 while scan remains at prior tip.

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_17600.py)
...
Decade bands for milestone M=17600 (docs only; helper emits paths only):
  identity 17551-17575 | fill 17576-17590 | markers 17591-17600
  extension 17501-17550 (pre-decade; may land ahead of identity/fill/markers)
  (prior M=17500: identity 17451-17475 | fill 17476-17490 | markers 17491-17500)
