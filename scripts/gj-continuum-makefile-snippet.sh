...
# *decade* is a closed batch range ending at a milestone M (e.g. M=23700
# → exclusive wave 23601–23700). Soft host probes
...
# graph_batch23700.c (and beyond) once those basenames are wired.
#
# Wave 95 soft note (23700-era): the exclusive target decade is M=23700
# (CREATE-ONLY 23601–23700; extension identity 23601–23650 ahead of the
# decade identity/fill/marker bands). Soft deepen surfaces this wave
# (docs/userland soft stamps): retchordangle / retarcangle (CREATE-ONLY soft
# names only — not product). Until parent pastes those basenames into
# CGJ_SRCS, ``--max`` may still report the prior high-water
# (e.g. makefile_max=23600). **Scan is the source of truth** — do not
# hardcode a false makefile_max here or in side panels. Soft high-water
# may be described as **advancing toward 23700** while the honest scan
...
# reports. Soft graph growth through graph_batch23700.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_23700
# and gj_product_score_23700 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (23700-era / Wave 95 example)
# ------------------------------------------------
# After CREATE-ONLY wave 23601–23700 exists on disk:
#
...
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 23651 23700
#
...
#   user/libcgj/src/graph_batch23600.c \   ← prior high-water (if still tip)
#   user/libcgj/src/graph_batch23601.c \   ← Wave 95 helper output starts here
#   … through graph_batch23700.c once CREATE-ONLY + parent wire land
#
...
#   user/libcgj/src/_gen_milestone_23700.py   (Wave 95 CREATE-ONLY emitter, when present)
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch23700.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 95 exclusive target decade: M=23700 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Soft deepen surfaces: retchordangle /
  retarcangle (CREATE-ONLY soft names only). Until parent wires 23601–23700,
  --max may still report prior tip (e.g. makefile_max=23600). Soft high-water
  may advance toward 23700 while scan remains at prior tip.

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_23700.py)
...
Decade bands for milestone M=23700 (docs only; helper emits paths only):
  extension 23601-23650 | identity 23651-23675 | fill 23676-23690 | markers 23691-23700
