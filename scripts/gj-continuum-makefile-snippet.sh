...
# *decade* is a closed batch range ending at a milestone M (e.g. M=18800
# → exclusive wave 18701–18800). Soft host probes
...
#      Example generator for the 18800 wave (Wave 46 exclusive continuum track):
#        user/libcgj/src/_gen_milestone_18800.py
#        python3 user/libcgj/src/_gen_milestone_18800.py --dry-run
#      (prior Wave 45: _gen_milestone_18700.py / M=18700)
...
# graph_batch18800.c (and beyond) once those basenames are wired.
#
# Wave 46 soft note (18800-era): the exclusive target decade is M=18800
# (CREATE-ONLY 18701–18800; extension identity 18701–18750 ahead of the
# decade identity/fill/marker bands). Soft deepen surfaces this wave
# (docs/userland soft stamps): retbailey / retpostern (CREATE-ONLY soft
# names only — not product). Until parent pastes those basenames into
# CGJ_SRCS, ``--max`` may still report the prior high-water
# (e.g. makefile_max=18700). **Scan is the source of truth** — do not
# hardcode a false makefile_max here or in side panels. Soft high-water
# may be described as **advancing toward 18800** while the honest scan
...
# reports. Soft graph growth through graph_batch18800.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_18800
# and gj_product_score_18800 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (18800-era / Wave 46 example)
# ------------------------------------------------
# After CREATE-ONLY wave 18701–18800 exists on disk:
#
...
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 18751 18800
#
...
#
# For M=18800 that is extension identity 18701–18750, decade identity
# 18751–18775, fill 18776–18790, markers 18791–18800. Prior decade
# M=18700: identity 18651–18675, fill 18676–18690, markers 18691–18700.
# See _gen_milestone_18800.py module docstring (when present) and
# user/libcgj/tests/NOTES_milestone_18800.txt (when present).
#
# Usage:
...
#   user/libcgj/src/graph_batch18700.c \   ← prior high-water (if still tip)
#   user/libcgj/src/graph_batch18701.c \   ← Wave 46 helper output starts here
#   … through graph_batch18800.c once CREATE-ONLY + parent wire land
#
...
#
# See also:
#   user/libcgj/src/_gen_milestone_18700.py   (prior Wave 45 CREATE-ONLY emitter)
#   user/libcgj/src/_gen_milestone_18800.py   (Wave 46 CREATE-ONLY emitter, when present)
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch18800.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 46 exclusive target decade: M=18800 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Soft deepen surfaces: retbailey /
  retpostern (CREATE-ONLY soft names only). Until parent wires 18701–18800,
  --max may still report prior tip (e.g. makefile_max=18700). Soft high-water
  may advance toward 18800 while scan remains at prior tip.

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_18800.py)
...
Decade bands for milestone M=18800 (docs only; helper emits paths only):
  extension 18701-18750 | identity 18751-18775 | fill 18776-18790 | markers 18791-18800
  (prior M=18700: identity 18651-18675 | fill 18676-18690 | markers 18691-18700)
