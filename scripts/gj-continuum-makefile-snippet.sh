...
# *decade* is a closed batch range ending at a milestone M (e.g. M=18200
# → exclusive wave 18151–18200). Soft host probes
...
#      Example generator for the 18200 wave (Wave 40 exclusive continuum track):
#        user/libcgj/src/_gen_milestone_18200.py
#        python3 user/libcgj/src/_gen_milestone_18200.py --dry-run
#      (prior Wave 39: _gen_milestone_18100.py / M=18100)
...
# graph_batch18200.c (and beyond) once those basenames are wired.
#
# Wave 40 soft note (18200-era): the exclusive target decade is M=18200
# (CREATE-ONLY 18151–18200; optional extension 18101–18150). Soft deepen
# surfaces this wave (docs/userland soft stamps): retcurtain / retparapet
...
# (e.g. makefile_max=18100). **Scan is the source of truth** — do not
# hardcode a false makefile_max here or in side panels. Soft high-water
# may be described as **advancing toward 18200** while the honest scan
...
# reports. Soft graph growth through graph_batch18200.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_18200
# and gj_product_score_18200 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (18200-era / Wave 40 example)
# ------------------------------------------------
# After CREATE-ONLY wave 18151–18200 exists on disk (optional extension
# 18101–18150 may land ahead of the decade bands):
#
#   # Prefer exist-only so missing TUs never produce dead Makefile lines:
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 18151 18200
...
#   ./scripts/gj-continuum-makefile-snippet.sh 18151 18200
#
...
#
# For M=18200 that is identity 18151–18175, fill 18176–18190,
# markers 18191–18200. Continuum extension 18101–18150 sits ahead of
# the decade bands. Prior decade M=18100: identity 18051–18075,
# fill 18076–18090, markers 18091–18100.
# See _gen_milestone_18200.py module docstring (when present) and
# user/libcgj/tests/NOTES_milestone_18200.txt (when present).
#
# Usage:
...
#   user/libcgj/src/graph_batch18100.c \   ← prior high-water (if still tip)
#   user/libcgj/src/graph_batch18101.c \   ← Wave 40 helper output starts here
#   … through graph_batch18200.c once CREATE-ONLY + parent wire land
#
...
#
# See also:
#   user/libcgj/src/_gen_milestone_18100.py   (prior Wave 39 CREATE-ONLY emitter)
#   user/libcgj/src/_gen_milestone_18200.py   (Wave 40 CREATE-ONLY emitter, when present)
#   user/libcgj/tests/NOTES_milestone_18100.txt
#   user/libcgj/tests/NOTES_milestone_18200.txt
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch18200.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 40 exclusive target decade: M=18200 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Soft deepen surfaces: retcurtain /
  retparapet (CREATE-ONLY soft names only). Until parent wires 18101–18200,
  --max may still report prior tip (e.g. makefile_max=18100). Soft high-water
  may advance toward 18200 while scan remains at prior tip.

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_18200.py)
...
Decade bands for milestone M=18200 (docs only; helper emits paths only):
  identity 18151-18175 | fill 18176-18190 | markers 18191-18200
  extension 18101-18150 (pre-decade; may land ahead of identity/fill/markers)
  (prior M=18100: identity 18051-18075 | fill 18076-18090 | markers 18091-18100)
