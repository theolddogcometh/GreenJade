...
# *decade* is a closed batch range ending at a milestone M (e.g. M=19500
# → exclusive wave 19401–19500). Soft host probes
...
#      Example generator for the 19500 wave (Wave 53 exclusive continuum track):
#        user/libcgj/src/_gen_milestone_19500.py
#        python3 user/libcgj/src/_gen_milestone_19500.py --dry-run
#      (prior Wave 52: _gen_milestone_19400.py / M=19400)
...
# graph_batch19500.c (and beyond) once those basenames are wired.
#
# Wave 53 soft note (19500-era): the exclusive target decade is M=19500
# (CREATE-ONLY 19401–19500; extension identity 19401–19450 ahead of the
# decade identity/fill/marker bands). Soft deepen surfaces this wave
# (docs/userland soft stamps): retravelin / retlunette (CREATE-ONLY soft
# names only — not product). Until parent pastes those basenames into
# CGJ_SRCS, ``--max`` may still report the prior high-water
# (e.g. makefile_max=19400). **Scan is the source of truth** — do not
# hardcode a false makefile_max here or in side panels. Soft high-water
# may be described as **advancing toward 19500** while the honest scan
...
# reports. Soft graph growth through graph_batch19500.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_19500
# and gj_product_score_19500 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (19500-era / Wave 53 example)
# ------------------------------------------------
# After CREATE-ONLY wave 19401–19500 exists on disk:
#
...
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 19451 19500
#
...
#
# For M=19500 that is extension identity 19401–19450, decade identity
# 19451–19475, fill 19476–19490, markers 19491–19500. Prior decade
# M=19400: identity 19351–19375, fill 19376–19390, markers 19391–19400.
# See _gen_milestone_19500.py module docstring (when present) and
# user/libcgj/tests/NOTES_milestone_19500.txt (when present).
#
# Usage:
...
#   user/libcgj/src/graph_batch19400.c \   ← prior high-water (if still tip)
#   user/libcgj/src/graph_batch19401.c \   ← Wave 53 helper output starts here
#   … through graph_batch19500.c once CREATE-ONLY + parent wire land
#
...
#
# See also:
#   user/libcgj/src/_gen_milestone_19400.py   (prior Wave 52 CREATE-ONLY emitter)
#   user/libcgj/src/_gen_milestone_19500.py   (Wave 53 CREATE-ONLY emitter, when present)
#   user/libcgj/tests/NOTES_milestone_19400.txt
#   user/libcgj/tests/NOTES_milestone_19500.txt
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch19500.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 53 exclusive target decade: M=19500 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Soft deepen surfaces: retravelin /
  retlunette (CREATE-ONLY soft names only). Until parent wires 19401–19500,
  --max may still report prior tip (e.g. makefile_max=19400). Soft high-water
  may advance toward 19500 while scan remains at prior tip.

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_19500.py)
...
Decade bands for milestone M=19500 (docs only; helper emits paths only):
  extension 19401-19450 | identity 19451-19475 | fill 19476-19490 | markers 19491-19500
  (prior M=19400: identity 19351-19375 | fill 19376-19390 | markers 19391-19400)
