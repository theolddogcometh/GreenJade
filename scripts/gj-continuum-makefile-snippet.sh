...
# *decade* is a closed batch range ending at a milestone M (e.g. M=25200
# → exclusive wave 25101–25200). Soft host probes
...
# graph_batch25200.c (and beyond) once those basenames are wired.
#
# Wave 110 soft note (25200-era): the exclusive target decade is M=25200
# (CREATE-ONLY 25101–25200; extension identity 25101–25150 ahead of the
# decade identity/fill/marker bands). Soft deepen surfaces this wave
# (docs/userland soft stamps): retmeshangle / retgridangle (CREATE-ONLY soft
# names only — not product). Until parent pastes those basenames into
# CGJ_SRCS, ``--max`` may still report the prior high-water
# (e.g. makefile_max=25100). **Scan is the source of truth** — do not
# hardcode a false makefile_max here or in side panels. Soft high-water
# may be described as **advancing toward 25200** while the honest scan
# remains at the prior tip (often makefile_max=25100).
#
...
# reports. Soft graph growth through graph_batch25200.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_25200
# and gj_product_score_25200 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (25200-era / Wave 110 example)
# ------------------------------------------------
# After CREATE-ONLY wave 25101–25200 exists on disk:
#
...
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 25151 25200
#
...
#   user/libcgj/src/graph_batch25101.c \   ← Wave 110 helper output starts here
#   … through graph_batch25200.c once CREATE-ONLY + parent wire land
#
...
#   user/libcgj/src/_gen_milestone_25200.py   (Wave 110 CREATE-ONLY emitter, when present)
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch25200.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 110 exclusive target decade: M=25200 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Soft deepen surfaces: retmeshangle /
  retgridangle (CREATE-ONLY soft names only). Until parent wires 25101–25200,
  --max may still report prior tip (e.g. makefile_max=25100). Soft high-water
  may advance toward 25200 while scan remains at prior tip (often 25100).

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_25200.py)
...
Decade bands for milestone M=25200 (docs only; helper emits paths only):
  extension 25101-25150 | identity 25151-25175 | fill 25176-25190 | markers 25191-25200
