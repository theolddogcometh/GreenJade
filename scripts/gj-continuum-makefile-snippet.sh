...
# *decade* is a closed batch range ending at a milestone M (e.g. M=25000
# → exclusive wave 24901–25000). Soft host probes
...
# graph_batch25000.c (and beyond) once those basenames are wired.
#
# Wave 108 soft note (25000-era): the exclusive target decade is M=25000
# (CREATE-ONLY 24901–25000; extension identity 24901–24950 ahead of the
# decade identity/fill/marker bands). Soft deepen surfaces this wave
# (docs/userland soft stamps): rethurmitangle / retcatmullangle (CREATE-ONLY soft
# names only — not product). Until parent pastes those basenames into
# CGJ_SRCS, ``--max`` may still report the prior high-water
# (e.g. makefile_max=24900). **Scan is the source of truth** — do not
# hardcode a false makefile_max here or in side panels. Soft high-water
# may be described as **advancing toward 25000** while the honest scan
# remains at the prior tip (often makefile_max=24900).
#
...
# reports. Soft graph growth through graph_batch25000.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_25000
# and gj_product_score_25000 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (25000-era / Wave 108 example)
# ------------------------------------------------
# After CREATE-ONLY wave 24901–25000 exists on disk:
#
...
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 24951 25000
#
...
#   user/libcgj/src/graph_batch24700.c \   ← prior high-water (if still tip)
#   user/libcgj/src/graph_batch24901.c \   ← Wave 108 helper output starts here
#   … through graph_batch24900.c once CREATE-ONLY + parent wire land
#
...
#   user/libcgj/src/_gen_milestone_25000.py   (Wave 108 CREATE-ONLY emitter, when present)
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch25000.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 108 exclusive target decade: M=25000 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Soft deepen surfaces: rethurmitangle /
  retcatmullangle (CREATE-ONLY soft names only). Until parent wires 24901–25000,
  --max may still report prior tip (e.g. makefile_max=24900). Soft high-water
  may advance toward 25000 while scan remains at prior tip (often 24900).

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_25000.py)
...
Decade bands for milestone M=25000 (docs only; helper emits paths only):
  extension 24901-24950 | identity 24951-24975 | fill 24976-24990 | markers 24991-25000
  (prior M=24900: identity 24851-24875 | fill 24876-24890 | markers 24891-24900)
