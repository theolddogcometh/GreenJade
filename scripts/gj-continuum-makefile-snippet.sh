...
# *decade* is a closed batch range ending at a milestone M (e.g. M=15900
# → exclusive wave 15851–15900). Soft host probes
...
#      Example generator for the 15900 wave (Wave 17 exclusive):
#        user/libcgj/src/_gen_milestone_15900.py
#        python3 user/libcgj/src/_gen_milestone_15900.py --dry-run
#      (prior Wave 16: _gen_milestone_15800.py / M=15800)
...
# graph_batch15900.c (and beyond) once those basenames are wired.
#
# Wave 17 soft note (15900-era): the exclusive target decade is M=15900
# (CREATE-ONLY 15851–15900; optional extension 15801–15850). Until parent
# pastes those basenames into CGJ_SRCS, ``--max`` may still report the
# prior high-water (e.g. makefile_max=15800). **Scan is the source of
# truth** — do not hardcode a false makefile_max here or in side panels.
# Soft high-water may be described as **advancing toward 15900** while the
...
# reports. Soft graph growth through graph_batch15900.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_15900
# and gj_product_score_15900 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (15900-era / Wave 17 example)
# ------------------------------------------------
# After CREATE-ONLY wave 15851–15900 exists on disk (optional extension
# 15801–15850 may land ahead of the decade bands):
#
#   # Prefer exist-only so missing TUs never produce dead Makefile lines:
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 15851 15900
...
#   ./scripts/gj-continuum-makefile-snippet.sh 15851 15900
#
...
#
# For M=15900 that is identity 15851–15875, fill 15876–15890,
# markers 15891–15900. Continuum extension 15801–15850 sits ahead of
# the decade bands. Prior decade M=15800: identity 15751–15775,
# fill 15776–15790, markers 15791–15800.
# See _gen_milestone_15900.py module docstring (when present) and
# user/libcgj/tests/NOTES_milestone_15900.txt (when present).
#
# Usage:
...
#   user/libcgj/src/graph_batch15800.c \   ← prior high-water (if still tip)
#   user/libcgj/src/graph_batch15801.c \   ← Wave 17 helper output starts here
#   … through graph_batch15900.c once CREATE-ONLY + parent wire land
#
...
#
# See also:
#   user/libcgj/src/_gen_milestone_15800.py   (prior Wave 16 CREATE-ONLY emitter)
#   user/libcgj/src/_gen_milestone_15900.py   (Wave 17 CREATE-ONLY emitter, when present)
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch15900.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 17 exclusive target decade: M=15900 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Until parent wires 15801–15900, --max may
  still report prior tip (e.g. makefile_max=15800). Soft high-water may
  advance toward 15900 while scan remains at prior tip.

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_15900.py)
...
Decade bands for milestone M=15900 (docs only; helper emits paths only):
  identity 15851-15875 | fill 15876-15890 | markers 15891-15900
  extension 15801-15850 (pre-decade; may land ahead of identity/fill/markers)
  (prior M=15800: identity 15751-15775 | fill 15776-15790 | markers 15791-15800)
...
# No artificial upper bound: correctly reports through 15900+ when wired.
makefile_max() {
	if [[ ! -f "$MAKEFILE" ]]; then
