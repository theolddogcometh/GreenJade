...
# *decade* is a closed batch range ending at a milestone M (e.g. M=17900
# → exclusive wave 17851–17900). Soft host probes
...
#      Example generator for the 17900 wave (Wave 37 exclusive continuum track):
#        user/libcgj/src/_gen_milestone_17900.py
#        python3 user/libcgj/src/_gen_milestone_17900.py --dry-run
#      (prior Wave 36: _gen_milestone_17800.py / M=17800)
...
# graph_batch17900.c (and beyond) once those basenames are wired.
#
# Wave 37 soft note (17900-era): the exclusive target decade is M=17900
# (CREATE-ONLY 17851–17900; optional extension 17801–17850). Soft deepen
# surfaces this wave (docs/userland soft stamps): retwall / retgate
...
# (e.g. makefile_max=17800). **Scan is the source of truth** — do not
# hardcode a false makefile_max here or in side panels. Soft high-water
# may be described as **advancing toward 17900** while the honest scan
...
# reports. Soft graph growth through graph_batch17900.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_17900
# and gj_product_score_17900 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (17900-era / Wave 37 example)
# ------------------------------------------------
# After CREATE-ONLY wave 17851–17900 exists on disk (optional extension
# 17801–17850 may land ahead of the decade bands):
#
#   # Prefer exist-only so missing TUs never produce dead Makefile lines:
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 17851 17900
...
#   ./scripts/gj-continuum-makefile-snippet.sh 17851 17900
#
...
#
# For M=17900 that is identity 17851–17875, fill 17876–17890,
# markers 17891–17900. Continuum extension 17801–17850 sits ahead of
# the decade bands. Prior decade M=17800: identity 17751–17775,
# fill 17776–17790, markers 17791–17800.
# See _gen_milestone_17900.py module docstring (when present) and
# user/libcgj/tests/NOTES_milestone_17900.txt (when present).
#
# Usage:
...
#   user/libcgj/src/graph_batch17800.c \   ← prior high-water (if still tip)
#   user/libcgj/src/graph_batch17801.c \   ← Wave 37 helper output starts here
#   … through graph_batch17900.c once CREATE-ONLY + parent wire land
#
...
#
# See also:
#   user/libcgj/src/_gen_milestone_17800.py   (prior Wave 36 CREATE-ONLY emitter)
#   user/libcgj/src/_gen_milestone_17900.py   (Wave 37 CREATE-ONLY emitter, when present)
#   user/libcgj/tests/NOTES_milestone_17800.txt
#   user/libcgj/tests/NOTES_milestone_17900.txt
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch17900.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 37 exclusive target decade: M=17900 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Soft deepen surfaces: retwall /
  retgate (CREATE-ONLY soft names only). Until parent wires 17801–17900,
  --max may still report prior tip (e.g. makefile_max=17800). Soft high-water
  may advance toward 17900 while scan remains at prior tip.

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_17900.py)
...
Decade bands for milestone M=17900 (docs only; helper emits paths only):
  identity 17851-17875 | fill 17876-17890 | markers 17891-17900
  extension 17801-17850 (pre-decade; may land ahead of identity/fill/markers)
  (prior M=17800: identity 17751-17775 | fill 17776-17790 | markers 17791-17800)
