...
# *decade* is a closed batch range ending at a milestone M (e.g. M=19300
# → exclusive wave 19201–19300). Soft host probes
...
#      Example generator for the 19300 wave (Wave 51 exclusive continuum track):
#        user/libcgj/src/_gen_milestone_19300.py
#        python3 user/libcgj/src/_gen_milestone_19300.py --dry-run
#      (prior Wave 50: _gen_milestone_19200.py / M=19200)
...
# graph_batch19300.c (and beyond) once those basenames are wired.
#
# Wave 51 soft note (19300-era): the exclusive target decade is M=19300
# (CREATE-ONLY 19201–19300; extension identity 19201–19250 ahead of the
# decade identity/fill/marker bands). Soft deepen surfaces this wave
# (docs/userland soft stamps): retfosse / retcoveredway (CREATE-ONLY soft
# names only — not product). Until parent pastes those basenames into
# CGJ_SRCS, ``--max`` may still report the prior high-water
# (e.g. makefile_max=19200). **Scan is the source of truth** — do not
# hardcode a false makefile_max here or in side panels. Soft high-water
# may be described as **advancing toward 19300** while the honest scan
...
# reports. Soft graph growth through graph_batch19300.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_19300
# and gj_product_score_19300 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (19300-era / Wave 51 example)
# ------------------------------------------------
# After CREATE-ONLY wave 19201–19300 exists on disk:
#
...
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 19251 19300
#
...
#
# For M=19300 that is extension identity 19201–19250, decade identity
# 19251–19275, fill 19276–19290, markers 19291–19300. Prior decade
# M=19200: identity 19151–19175, fill 19176–19190, markers 19191–19200.
# See _gen_milestone_19300.py module docstring (when present) and
# user/libcgj/tests/NOTES_milestone_19300.txt (when present).
#
# Usage:
...
#   user/libcgj/src/graph_batch19200.c \   ← prior high-water (if still tip)
#   user/libcgj/src/graph_batch19201.c \   ← Wave 51 helper output starts here
#   … through graph_batch19300.c once CREATE-ONLY + parent wire land
#
...
#
# See also:
#   user/libcgj/src/_gen_milestone_19200.py   (prior Wave 50 CREATE-ONLY emitter)
#   user/libcgj/src/_gen_milestone_19300.py   (Wave 51 CREATE-ONLY emitter, when present)
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch19300.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 51 exclusive target decade: M=19300 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Soft deepen surfaces: retfosse /
  retcoveredway (CREATE-ONLY soft names only). Until parent wires 19201–19300,
  --max may still report prior tip (e.g. makefile_max=19200). Soft high-water
  may advance toward 19300 while scan remains at prior tip.

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_19300.py)
...
Decade bands for milestone M=19300 (docs only; helper emits paths only):
  extension 19201-19250 | identity 19251-19275 | fill 19276-19290 | markers 19291-19300
  (prior M=19200: identity 19151-19175 | fill 19176-19190 | markers 19191-19200)
