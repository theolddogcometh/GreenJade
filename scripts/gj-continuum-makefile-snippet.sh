...
# *decade* is a closed batch range ending at a milestone M (e.g. M=18000
# → exclusive wave 17951–18000). Soft host probes
...
#      Example generator for the 18000 wave (Wave 38 exclusive continuum track):
#        user/libcgj/src/_gen_milestone_18000.py
#        python3 user/libcgj/src/_gen_milestone_18000.py --dry-run
#      (prior Wave 37: _gen_milestone_17900.py / M=17900)
...
# graph_batch18000.c (and beyond) once those basenames are wired.
#
# Wave 38 soft note (18000-era): the exclusive target decade is M=18000
# (CREATE-ONLY 17951–18000; optional extension 17901–17950). Soft deepen
# surfaces this wave (docs/userland soft stamps): retmoat / retower
...
# (e.g. makefile_max=17900). **Scan is the source of truth** — do not
# hardcode a false makefile_max here or in side panels. Soft high-water
# may be described as **advancing toward 18000** while the honest scan
...
# reports. Soft graph growth through graph_batch18000.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_18000
# and gj_product_score_18000 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (18000-era / Wave 38 example)
# ------------------------------------------------
# After CREATE-ONLY wave 17951–18000 exists on disk (optional extension
# 17901–17950 may land ahead of the decade bands):
#
#   # Prefer exist-only so missing TUs never produce dead Makefile lines:
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 17951 18000
...
#   ./scripts/gj-continuum-makefile-snippet.sh 17951 18000
#
...
#
# For M=18000 that is identity 17951–17975, fill 17976–17990,
# markers 17991–18000. Continuum extension 17901–17950 sits ahead of
# the decade bands. Prior decade M=17900: identity 17851–17875,
# fill 17876–17890, markers 17891–17900.
# See _gen_milestone_18000.py module docstring (when present) and
# user/libcgj/tests/NOTES_milestone_18000.txt (when present).
#
# Usage:
...
#   user/libcgj/src/graph_batch17900.c \   ← prior high-water (if still tip)
#   user/libcgj/src/graph_batch17901.c \   ← Wave 38 helper output starts here
#   … through graph_batch18000.c once CREATE-ONLY + parent wire land
#
...
#
# See also:
#   user/libcgj/src/_gen_milestone_17900.py   (prior Wave 37 CREATE-ONLY emitter)
#   user/libcgj/src/_gen_milestone_18000.py   (Wave 38 CREATE-ONLY emitter, when present)
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch18000.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 38 exclusive target decade: M=18000 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Soft deepen surfaces: retmoat /
  retower (CREATE-ONLY soft names only). Until parent wires 17901–18000,
  --max may still report prior tip (e.g. makefile_max=17900). Soft high-water
  may advance toward 18000 while scan remains at prior tip.

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_18000.py)
...
Decade bands for milestone M=18000 (docs only; helper emits paths only):
  identity 17951-17975 | fill 17976-17990 | markers 17991-18000
  extension 17901-17950 (pre-decade; may land ahead of identity/fill/markers)
  (prior M=17900: identity 17851-17875 | fill 17876-17890 | markers 17891-17900)
