...
# *decade* is a closed batch range ending at a milestone M (e.g. M=17800
# → exclusive wave 17751–17800). Soft host probes
...
#      Example generator for the 17800 wave (Wave 36 exclusive continuum track):
#        user/libcgj/src/_gen_milestone_17800.py
#        python3 user/libcgj/src/_gen_milestone_17800.py --dry-run
#      (prior Wave 35: _gen_milestone_17700.py / M=17700)
...
# graph_batch17800.c (and beyond) once those basenames are wired.
#
# Wave 36 soft note (17800-era): the exclusive target decade is M=17800
# (CREATE-ONLY 17751–17800; optional extension 17701–17750). Soft deepen
# surfaces this wave (docs/userland soft stamps): rethold / retspire
...
# (e.g. makefile_max=17700). **Scan is the source of truth** — do not
# hardcode a false makefile_max here or in side panels. Soft high-water
# may be described as **advancing toward 17800** while the honest scan
...
# reports. Soft graph growth through graph_batch17800.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_17800
# and gj_product_score_17800 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (17800-era / Wave 36 example)
# ------------------------------------------------
# After CREATE-ONLY wave 17751–17800 exists on disk (optional extension
# 17701–17750 may land ahead of the decade bands):
#
#   # Prefer exist-only so missing TUs never produce dead Makefile lines:
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 17751 17800
...
#   ./scripts/gj-continuum-makefile-snippet.sh 17751 17800
#
...
#
# For M=17800 that is identity 17751–17775, fill 17776–17790,
# markers 17791–17800. Continuum extension 17701–17750 sits ahead of
# the decade bands. Prior decade M=17700: identity 17651–17675,
# fill 17676–17690, markers 17691–17700.
# See _gen_milestone_17800.py module docstring (when present) and
# user/libcgj/tests/NOTES_milestone_17800.txt (when present).
#
# Usage:
...
#   user/libcgj/src/graph_batch17700.c \   ← prior high-water (if still tip)
#   user/libcgj/src/graph_batch17701.c \   ← Wave 36 helper output starts here
#   … through graph_batch17800.c once CREATE-ONLY + parent wire land
#
...
#
# See also:
#   user/libcgj/src/_gen_milestone_17700.py   (prior Wave 35 CREATE-ONLY emitter)
#   user/libcgj/src/_gen_milestone_17800.py   (Wave 36 CREATE-ONLY emitter, when present)
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch17800.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 36 exclusive target decade: M=17800 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Soft deepen surfaces: rethold /
  retspire (CREATE-ONLY soft names only). Until parent wires 17701–17800,
  --max may still report prior tip (e.g. makefile_max=17700). Soft high-water
  may advance toward 17800 while scan remains at prior tip.

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_17800.py)
...
Decade bands for milestone M=17800 (docs only; helper emits paths only):
  identity 17751-17775 | fill 17776-17790 | markers 17791-17800
  extension 17701-17750 (pre-decade; may land ahead of identity/fill/markers)
  (prior M=17700: identity 17651-17675 | fill 17676-17690 | markers 17691-17700)
