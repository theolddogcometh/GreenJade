...
# *decade* is a closed batch range ending at a milestone M (e.g. M=25800
# → exclusive wave 25701–25800). Soft host probes
...
# graph_batch25800.c (and beyond) once those basenames are wired.
#
# Wave 116 soft note (25800-era): the exclusive target decade is M=25800
# (CREATE-ONLY 25701–25800; extension identity 25701-25750 ahead of the
# decade identity/fill/marker bands). Soft deepen surfaces this wave
# (docs/userland soft stamps): retfenceangle / retsemaphoreangle (CREATE-ONLY soft
# names only — not product). Until parent pastes those basenames into
# CGJ_SRCS, ``--max`` may still report the prior high-water
# (e.g. makefile_max=25700). **Scan is the source of truth** — do not
# hardcode a false makefile_max here or in side panels. Soft high-water
# may be described as **advancing toward 25800** while the honest scan
# remains at the prior tip (often makefile_max=25700).
#
...
# reports. Soft graph growth through graph_batch25800.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_25700
# and gj_product_score_25700 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (25800-era / Wave 116 example)
# ------------------------------------------------
# After CREATE-ONLY wave 25701–25800 exists on disk:
#
...
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 25751 25800
#
...
#   user/libcgj/src/graph_batch25700.c \   ← prior high-water (if still tip)
#   user/libcgj/src/graph_batch25701.c \   ← Wave 116 helper output starts here
#   … through graph_batch25800.c once CREATE-ONLY + parent wire land
#
...
#   user/libcgj/src/_gen_milestone_25800.py   (Wave 116 CREATE-ONLY emitter, when present)
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch25800.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 116 exclusive target decade: M=25800 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Soft deepen surfaces: retfenceangle /
  retsemaphoreangle (CREATE-ONLY soft names only). Until parent wires 25701–25800,
  --max may still report prior tip (e.g. makefile_max=25700). Soft high-water
  may advance toward 25800 while scan remains at prior tip (often 25700).

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_25800.py)
...
Decade bands for milestone M=25800 (docs only; helper emits paths only):
  extension 25701-25750 | identity 25751-25775 | fill 25776-25790 | markers 25791-25800
    (prior M=25700: identity 25651-25675 | fill 25676-25690 | markers 25691-25700; prior M=25600: identity 25551-25575 | fill 25576-25590 | markers 25591-25600; prior M=25500: identity 25451-25475 | fill 25476-25490 | markers 25491-25500)
