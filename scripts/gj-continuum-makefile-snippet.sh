...
# *decade* is a closed batch range ending at a milestone M (e.g. M=26000
# → exclusive wave 25901–26000). Soft host probes
...
# graph_batch26000.c (and beyond) once those basenames are wired.
#
# Wave 118 soft note (26000-era): the exclusive target decade is M=26000
# (CREATE-ONLY 25901–26000; extension identity 25901-25950 ahead of the
# decade identity/fill/marker bands). Soft deepen surfaces this wave
# (docs/userland soft stamps): retbarrierangle / retatomicangle (CREATE-ONLY soft
# names only — not product). Until parent pastes those basenames into
# CGJ_SRCS, ``--max`` may still report the prior high-water
# (e.g. makefile_max=25900). **Scan is the source of truth** — do not
# hardcode a false makefile_max here or in side panels. Soft high-water
# may be described as **advancing toward 26000** while the honest scan
# remains at the prior tip (often makefile_max=25900 after W117 ship).
#
...
# reports. Soft graph growth through graph_batch26000.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_25900
# and gj_product_score_25900 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (26000-era / Wave 118 example)
# ------------------------------------------------
# After CREATE-ONLY wave 25901–26000 exists on disk:
#
...
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 25951 26000
#
...
#   user/libcgj/src/graph_batch25900.c \   ← prior high-water (if still tip after W117)
#   user/libcgj/src/graph_batch25901.c \   ← Wave 118 helper output starts here
#   … through graph_batch26000.c once CREATE-ONLY + parent wire land
#
...
#   user/libcgj/src/_gen_milestone_26000.py   (Wave 118 CREATE-ONLY emitter, when present)
#   user/libcgj/src/_gen_milestone_25900.py   (prior Wave 117 CREATE-ONLY emitter, when present)
#   user/libcgj/src/_gen_milestone_25800.py   (prior Wave 116 CREATE-ONLY emitter, when present)
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch26000.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 118 exclusive target decade: M=26000 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Soft deepen surfaces: retbarrierangle /
  retatomicangle (CREATE-ONLY soft names only). Until parent wires 25901–26000,
  --max may still report prior tip (e.g. makefile_max=25900). Soft high-water
  may advance toward 26000 while scan remains at prior tip (often 25900).

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_26000.py)
...
Decade bands for milestone M=26000 (docs only; helper emits paths only):
  extension 25901-25950 | identity 25951-25975 | fill 25976-25990 | markers 25991-26000
    (prior M=25900: identity 25851-25875 | fill 25876-25890 | markers 25891-25900; prior M=25800: identity 25751-25775 | fill 25776-25790 | markers 25791-25800; prior M=25700: identity 25651-25675 | fill 25676-25690 | markers 25691-25700; prior M=25600: identity 25551-25575 | fill 25576-25590 | markers 25591-25600; prior M=25500: identity 25451-25475 | fill 25476-25490 | markers 25491-25500)
