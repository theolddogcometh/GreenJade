...
# *decade* is a closed batch range ending at a milestone M (e.g. M=15600
# → exclusive wave 15551–15600). Soft host probes
...
#      Example generator for the 15600 wave (Wave 14 exclusive):
#        user/libcgj/src/_gen_milestone_15600.py
#        python3 user/libcgj/src/_gen_milestone_15600.py --dry-run
#      (prior Wave 13: _gen_milestone_15500.py / M=15500)
...
# graph_batch15600.c (and beyond) once those basenames are wired.
#
# Wave 14 soft note (15600-era): the exclusive target decade is M=15600
# (CREATE-ONLY 15551–15600; optional extension 15501–15550). Until parent
# pastes those basenames into CGJ_SRCS, ``--max`` may still report the
# prior high-water (e.g. makefile_max=15500). **Scan is the source of
...
# reports. Soft graph growth through graph_batch15600.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_15600
# and gj_product_score_15600 remain 0 by design).
#
# Decade paste hygiene (15600-era / Wave 14 example)
# ------------------------------------------------
# After CREATE-ONLY wave 15551–15600 exists on disk (optional extension
# 15501–15550 may land ahead of the decade bands):
#
#   # Prefer exist-only so missing TUs never produce dead Makefile lines:
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 15551 15600
...
#
# For M=15600 that is identity 15551–15575, fill 15576–15590,
# markers 15591–15600. Continuum extension 15501–15550 sits ahead of
# the decade bands. Prior decade M=15500: identity 15451–15475,
# fill 15476–15490, markers 15491–15500.
# See _gen_milestone_15600.py module docstring (when present) and
# user/libcgj/tests/NOTES_milestone_15600.txt (when present).
#
# Usage:
...
#   user/libcgj/src/graph_batch15500.c \   ← prior high-water (if still tip)
#   user/libcgj/src/graph_batch15501.c \   ← Wave 14 helper output starts here
#   … through graph_batch15600.c once CREATE-ONLY + parent wire land
#
...
#
# See also:
#   user/libcgj/src/_gen_milestone_15500.py   (prior Wave 13 CREATE-ONLY emitter)
#   user/libcgj/src/_gen_milestone_15600.py   (Wave 14 CREATE-ONLY emitter)
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch15600.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 14 exclusive target decade: M=15600 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Until parent wires 15501–15600, --max may
  still report prior tip (e.g. makefile_max=15500).

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_15600.py)
...
Decade bands for milestone M=15600 (docs only; helper emits paths only):
  identity 15551-15575 | fill 15576-15590 | markers 15591-15600
  extension 15501-15550 (pre-decade; may land ahead of identity/fill/markers)
  (prior M=15500: identity 15451-15475 | fill 15476-15490 | markers 15491-15500)
...
# No artificial upper bound: correctly reports through 15600+ when wired.
makefile_max() {
	if [[ ! -f "$MAKEFILE" ]]; then
