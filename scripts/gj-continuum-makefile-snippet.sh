...
# *decade* is a closed batch range ending at a milestone M (e.g. M=23900
# → exclusive wave 23801–23900). Soft host probes
...
# graph_batch23900.c (and beyond) once those basenames are wired.
#
# Wave 97 soft note (23900-era): the exclusive target decade is M=23900
# (CREATE-ONLY 23801–23900; extension identity 23801–23850 ahead of the
# decade identity/fill/marker bands). Soft deepen surfaces this wave
# (docs/userland soft stamps): retradiusangle / retdiameterangle (CREATE-ONLY soft
# names only — not product). Until parent pastes those basenames into
# CGJ_SRCS, ``--max`` may still report the prior high-water
# (e.g. makefile_max=23800). **Scan is the source of truth** — do not
# hardcode a false makefile_max here or in side panels. Soft high-water
# may be described as **advancing toward 23900** while the honest scan
...
# reports. Soft graph growth through graph_batch23900.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_23900
# and gj_product_score_23900 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (23900-era / Wave 97 example)
# ------------------------------------------------
# After CREATE-ONLY wave 23801–23900 exists on disk:
#
...
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 23851 23900
#
...
#   user/libcgj/tests/NOTES_milestone_23900.txt
...
#   user/libcgj/src/graph_batch23701.c \   ← Wave 97 helper output starts here
#   … through graph_batch23900.c once CREATE-ONLY + parent wire land
#
...
#   user/libcgj/src/_gen_milestone_23900.py   (Wave 97 CREATE-ONLY emitter, when present)
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch23900.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 97 exclusive target decade: M=23900 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Soft deepen surfaces: retradiusangle /
  retdiameterangle (CREATE-ONLY soft names only). Until parent wires 23801–23900,
  --max may still report prior tip (e.g. makefile_max=23800). Soft high-water
  may advance toward 23900 while scan remains at prior tip.

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_23900.py)
...
Decade bands for milestone M=23900 (docs only; helper emits paths only):
  extension 23701-23750 | identity 23751-23775 | fill 23776-23790 | markers 23791-23800
