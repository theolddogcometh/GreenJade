...
# *decade* is a closed batch range ending at a milestone M (e.g. M=18100
# → exclusive wave 18051–18100). Soft host probes
...
#      Example generator for the 18100 wave (Wave 39 exclusive continuum track):
#        user/libcgj/src/_gen_milestone_18100.py
#        python3 user/libcgj/src/_gen_milestone_18100.py --dry-run
#      (prior Wave 38: _gen_milestone_18000.py / M=18000)
...
# graph_batch18100.c (and beyond) once those basenames are wired.
#
# Wave 39 soft note (18100-era): the exclusive target decade is M=18100
# (CREATE-ONLY 18051–18100; optional extension 18001–18050). Soft deepen
# surfaces this wave (docs/userland soft stamps): retbarbican / retglacis
...
# (e.g. makefile_max=18000). **Scan is the source of truth** — do not
# hardcode a false makefile_max here or in side panels. Soft high-water
# may be described as **advancing toward 18100** while the honest scan
...
# reports. Soft graph growth through graph_batch18100.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_18100
# and gj_product_score_18100 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (18100-era / Wave 39 example)
# ------------------------------------------------
# After CREATE-ONLY wave 18051–18100 exists on disk (optional extension
# 18001–18050 may land ahead of the decade bands):
#
#   # Prefer exist-only so missing TUs never produce dead Makefile lines:
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 18051 18100
...
#   ./scripts/gj-continuum-makefile-snippet.sh 18051 18100
#
...
#
# For M=18100 that is identity 18051–18075, fill 18076–18090,
# markers 18091–18100. Continuum extension 18001–18050 sits ahead of
# the decade bands. Prior decade M=18000: identity 17951–17975,
# fill 17976–17990, markers 17991–18000.
# See _gen_milestone_18100.py module docstring (when present) and
# user/libcgj/tests/NOTES_milestone_18100.txt (when present).
#
# Usage:
...
#   user/libcgj/src/graph_batch18000.c \   ← prior high-water (if still tip)
#   user/libcgj/src/graph_batch18001.c \   ← Wave 39 helper output starts here
#   … through graph_batch18100.c once CREATE-ONLY + parent wire land
#
...
#
# See also:
#   user/libcgj/src/_gen_milestone_18000.py   (prior Wave 38 CREATE-ONLY emitter)
#   user/libcgj/src/_gen_milestone_18100.py   (Wave 39 CREATE-ONLY emitter, when present)
#   user/libcgj/tests/NOTES_milestone_18000.txt
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch18100.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 39 exclusive target decade: M=18100 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Soft deepen surfaces: retbarbican /
  retglacis (CREATE-ONLY soft names only). Until parent wires 18001–18100,
  --max may still report prior tip (e.g. makefile_max=18000). Soft high-water
  may advance toward 18100 while scan remains at prior tip.

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_18100.py)
...
Decade bands for milestone M=18100 (docs only; helper emits paths only):
  identity 18051-18075 | fill 18076-18090 | markers 18091-18100
  extension 18001-18050 (pre-decade; may land ahead of identity/fill/markers)
  (prior M=18000: identity 17951-17975 | fill 17976-17990 | markers 17991-18000)
