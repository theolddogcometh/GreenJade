...
# *decade* is a closed batch range ending at a milestone M (e.g. M=17700
# → exclusive wave 17651–17700). Soft host probes
...
#      Example generator for the 17700 wave (Wave 35 exclusive continuum track):
#        user/libcgj/src/_gen_milestone_17700.py
#        python3 user/libcgj/src/_gen_milestone_17700.py --dry-run
#      (prior Wave 34: _gen_milestone_17600.py / M=17600)
...
# graph_batch17700.c (and beyond) once those basenames are wired.
#
# Wave 35 soft note (17700-era): the exclusive target decade is M=17700
# (CREATE-ONLY 17651–17700; optional extension 17601–17650). Soft deepen
# surfaces this wave (docs/userland soft stamps): retfortress / retpalace
...
# (e.g. makefile_max=17600). **Scan is the source of truth** — do not
# hardcode a false makefile_max here or in side panels. Soft high-water
# may be described as **advancing toward 17700** while the honest scan
...
# reports. Soft graph growth through graph_batch17700.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_17700
# and gj_product_score_17700 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (17700-era / Wave 35 example)
# ------------------------------------------------
# After CREATE-ONLY wave 17651–17700 exists on disk (optional extension
# 17601–17650 may land ahead of the decade bands):
#
#   # Prefer exist-only so missing TUs never produce dead Makefile lines:
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 17651 17700
...
#   ./scripts/gj-continuum-makefile-snippet.sh 17651 17700
#
...
#
# For M=17700 that is identity 17651–17675, fill 17676–17690,
# markers 17691–17700. Continuum extension 17601–17650 sits ahead of
# the decade bands. Prior decade M=17600: identity 17551–17575,
# fill 17576–17590, markers 17591–17600.
# See _gen_milestone_17700.py module docstring (when present) and
# user/libcgj/tests/NOTES_milestone_17700.txt (when present).
#
# Usage:
...
#   user/libcgj/src/graph_batch17600.c \   ← prior high-water (if still tip)
#   user/libcgj/src/graph_batch17601.c \   ← Wave 35 helper output starts here
#   … through graph_batch17700.c once CREATE-ONLY + parent wire land
#
...
#
# See also:
#   user/libcgj/src/_gen_milestone_17600.py   (prior Wave 34 CREATE-ONLY emitter)
#   user/libcgj/src/_gen_milestone_17700.py   (Wave 35 CREATE-ONLY emitter, when present)
#   user/libcgj/tests/NOTES_milestone_17600.txt
#   user/libcgj/tests/NOTES_milestone_17700.txt
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch17700.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 35 exclusive target decade: M=17700 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Soft deepen surfaces: retfortress /
  retpalace (CREATE-ONLY soft names only). Until parent wires 17601–17700,
  --max may still report prior tip (e.g. makefile_max=17600). Soft high-water
  may advance toward 17700 while scan remains at prior tip.

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_17700.py)
...
Decade bands for milestone M=17700 (docs only; helper emits paths only):
  identity 17651-17675 | fill 17676-17690 | markers 17691-17700
  extension 17601-17650 (pre-decade; may land ahead of identity/fill/markers)
  (prior M=17600: identity 17551-17575 | fill 17576-17590 | markers 17591-17600)
