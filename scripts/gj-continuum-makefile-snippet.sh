...
# *decade* is a closed batch range ending at a milestone M (e.g. M=18900
# → exclusive wave 18801–18900). Soft host probes
...
#      Example generator for the 18900 wave (Wave 47 exclusive continuum track):
#        user/libcgj/src/_gen_milestone_18900.py
#        python3 user/libcgj/src/_gen_milestone_18900.py --dry-run
#      (prior Wave 46: _gen_milestone_18800.py / M=18800)
...
# graph_batch18900.c (and beyond) once those basenames are wired.
#
# Wave 47 soft note (18900-era): the exclusive target decade is M=18900
# (CREATE-ONLY 18801–18900; extension identity 18801–18850 ahead of the
# decade identity/fill/marker bands). Soft deepen surfaces this wave
# (docs/userland soft stamps): retinnerward / retdonjon (CREATE-ONLY soft
# names only — not product). Until parent pastes those basenames into
# CGJ_SRCS, ``--max`` may still report the prior high-water
# (e.g. makefile_max=18800). **Scan is the source of truth** — do not
# hardcode a false makefile_max here or in side panels. Soft high-water
# may be described as **advancing toward 18900** while the honest scan
...
# reports. Soft graph growth through graph_batch18900.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_18900
# and gj_product_score_18900 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (18900-era / Wave 47 example)
# ------------------------------------------------
# After CREATE-ONLY wave 18801–18900 exists on disk:
#
...
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 18851 18900
#
...
#
# For M=18900 that is extension identity 18801–18850, decade identity
# 18851–18875, fill 18876–18890, markers 18891–18900. Prior decade
# M=18800: identity 18751–18775, fill 18776–18790, markers 18791–18800.
# See _gen_milestone_18900.py module docstring (when present) and
# user/libcgj/tests/NOTES_milestone_18900.txt (when present).
#
# Usage:
...
#   user/libcgj/src/graph_batch18800.c \   ← prior high-water (if still tip)
#   user/libcgj/src/graph_batch18801.c \   ← Wave 47 helper output starts here
#   … through graph_batch18900.c once CREATE-ONLY + parent wire land
#
...
#
# See also:
#   user/libcgj/src/_gen_milestone_18800.py   (prior Wave 46 CREATE-ONLY emitter)
#   user/libcgj/src/_gen_milestone_18900.py   (Wave 47 CREATE-ONLY emitter, when present)
#   user/libcgj/tests/NOTES_milestone_18800.txt
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch18900.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 47 exclusive target decade: M=18900 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Soft deepen surfaces: retinnerward /
  retdonjon (CREATE-ONLY soft names only). Until parent wires 18801–18900,
  --max may still report prior tip (e.g. makefile_max=18800). Soft high-water
  may advance toward 18900 while scan remains at prior tip.

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_18900.py)
...
Decade bands for milestone M=18900 (docs only; helper emits paths only):
  extension 18801-18850 | identity 18851-18875 | fill 18876-18890 | markers 18891-18900
  (prior M=18800: identity 18751-18775 | fill 18776-18790 | markers 18791-18800)
