...
# *decade* is a closed batch range ending at a milestone M (e.g. M=16600
# → exclusive wave 16551–16600). Soft host probes
...
#      Example generator for the 16600 wave (Wave 24 exclusive continuum track):
#        user/libcgj/src/_gen_milestone_16600.py
#        python3 user/libcgj/src/_gen_milestone_16600.py --dry-run
#      (prior Wave 23: _gen_milestone_16500.py / M=16500)
...
# graph_batch16600.c (and beyond) once those basenames are wired.
#
# Wave 24 soft note (16600-era): the exclusive target decade is M=16600
# (CREATE-ONLY 16551–16600; optional extension 16501–16550). Until parent
# pastes those basenames into CGJ_SRCS, ``--max`` may still report the
# prior high-water (e.g. makefile_max=16500). **Scan is the source of
# truth** — do not hardcode a false makefile_max here or in side panels.
# Soft high-water may be described as **advancing toward 16600** while the
...
# reports. Soft graph growth through graph_batch16600.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_16600
# and gj_product_score_16600 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (16600-era / Wave 24 example)
# ------------------------------------------------
# After CREATE-ONLY wave 16551–16600 exists on disk (optional extension
# 16501–16550 may land ahead of the decade bands):
#
#   # Prefer exist-only so missing TUs never produce dead Makefile lines:
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 16551 16600
...
#   ./scripts/gj-continuum-makefile-snippet.sh 16551 16600
#
...
#
# For M=16600 that is identity 16551–16575, fill 16576–16590,
# markers 16591–16600. Continuum extension 16501–16550 sits ahead of
# the decade bands. Prior decade M=16500: identity 16451–16475,
# fill 16476–16490, markers 16491–16500.
# See _gen_milestone_16500.py module docstring (when present) and
# user/libcgj/tests/NOTES_milestone_16500.txt (when present).
#
# Usage:
#   ./scripts/gj-continuum-makefile-snippet.sh 16451 16500
#   ./scripts/gj-continuum-makefile-snippet.sh 16551 16600
#   ./scripts/gj-continuum-makefile-snippet.sh --next 100
#   ./scripts/gj-continuum-makefile-snippet.sh --next          # default decade 100
...
#   user/libcgj/src/graph_batch16500.c \   ← prior high-water (if still tip)
#   user/libcgj/src/graph_batch16501.c \   ← Wave 24 helper output starts here
#   … through graph_batch16600.c once CREATE-ONLY + parent wire land
#
...
#
# See also:
#   user/libcgj/src/_gen_milestone_16500.py   (prior Wave 23 CREATE-ONLY emitter)
#   user/libcgj/src/_gen_milestone_16600.py   (Wave 24 CREATE-ONLY emitter, when present)
#   user/libcgj/tests/NOTES_milestone_16500.txt
#   user/libcgj/tests/NOTES_milestone_16600.txt
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch16600.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 24 exclusive target decade: M=16600 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Until parent wires 16501–16600, --max may
  still report prior tip (e.g. makefile_max=16500). Soft high-water may
  advance toward 16600 while scan remains at prior tip.

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_16600.py)
...
Decade bands for milestone M=16600 (docs only; helper emits paths only):
  identity 16551-16575 | fill 16576-16590 | markers 16591-16600
  extension 16501-16550 (pre-decade; may land ahead of identity/fill/markers)
  (prior M=16500: identity 16451-16475 | fill 16476-16490 | markers 16491-16500)
