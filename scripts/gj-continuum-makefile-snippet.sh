...
# *decade* is a closed batch range ending at a milestone M (e.g. M=23300
# → exclusive wave 23201–23300). Soft host probes
...
# graph_batch23300.c (and beyond) once those basenames are wired.
#
# Wave 91 soft note (23300-era): the exclusive target decade is M=23300
# (CREATE-ONLY 23201–23300; extension identity 23201–23250 ahead of the
# decade identity/fill/marker bands). Soft deepen surfaces this wave
# (docs/userland soft stamps): retbadgeangle / retphaseangle (CREATE-ONLY soft
# names only — not product). Until parent pastes those basenames into
# CGJ_SRCS, ``--max`` may still report the prior high-water
# (e.g. makefile_max=23200). **Scan is the source of truth** — do not
# hardcode a false makefile_max here or in side panels. Soft high-water
# may be described as **advancing toward 23300** while the honest scan
...
# reports. Soft graph growth through graph_batch23300.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_23300
# and gj_product_score_23300 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (23300-era / Wave 91 example)
# ------------------------------------------------
# After CREATE-ONLY wave 23201–23300 exists on disk:
#
...
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 23251 23300
#
...
#   user/libcgj/tests/NOTES_milestone_23300.txt (when present).
#
# Usage:
...
#   user/libcgj/src/graph_batch23200.c \   ← prior high-water (if still tip)
#   user/libcgj/src/graph_batch23201.c \   ← Wave 91 helper output starts here
#   … through graph_batch23300.c once CREATE-ONLY + parent wire land
#
...
#   user/libcgj/src/_gen_milestone_23300.py   (Wave 91 CREATE-ONLY emitter, when present)
#   user/libcgj/src/_gen_milestone_23200.py   (prior Wave 90 CREATE-ONLY emitter, when present)
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch23300.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 91 exclusive target decade: M=23300 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Soft deepen surfaces: retbadgeangle /
  retphaseangle (CREATE-ONLY soft names only). Until parent wires 23201–23300,
  --max may still report prior tip (e.g. makefile_max=23200). Soft high-water
  may advance toward 23300 while scan remains at prior tip.

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_23300.py)
...
Decade bands for milestone M=23300 (docs only; helper emits paths only):
  extension 23201-23250 | identity 23251-23275 | fill 23276-23290 | markers 23291-23300
  (prior M=23200: identity 23151-23175 | fill 23176-23190 | markers 23191-23200)
