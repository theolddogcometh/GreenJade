...
# *decade* is a closed batch range ending at a milestone M (e.g. M=23800
# → exclusive wave 23701–23800). Soft host probes
...
# graph_batch23800.c (and beyond) once those basenames are wired.
#
# Wave 96 soft note (23800-era): the exclusive target decade is M=23800
# (CREATE-ONLY 23701–23800; extension identity 23701–23750 ahead of the
# decade identity/fill/marker bands). Soft deepen surfaces this wave
# (docs/userland soft stamps): retsectorangle / retwedgeangle (CREATE-ONLY soft
# names only — not product). Until parent pastes those basenames into
# CGJ_SRCS, ``--max`` may still report the prior high-water
# (e.g. makefile_max=23700). **Scan is the source of truth** — do not
# hardcode a false makefile_max here or in side panels. Soft high-water
# may be described as **advancing toward 23800** while the honest scan
...
# reports. Soft graph growth through graph_batch23800.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_23800
# and gj_product_score_23800 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (23800-era / Wave 96 example)
# ------------------------------------------------
# After CREATE-ONLY wave 23701–23800 exists on disk:
#
...
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 23751 23800
#
...
#   user/libcgj/src/graph_batch23701.c \   ← Wave 96 helper output starts here
#   … through graph_batch23800.c once CREATE-ONLY + parent wire land
#
...
#   user/libcgj/src/_gen_milestone_23800.py   (Wave 96 CREATE-ONLY emitter, when present)
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch23800.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 96 exclusive target decade: M=23800 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Soft deepen surfaces: retsectorangle /
  retwedgeangle (CREATE-ONLY soft names only). Until parent wires 23701–23800,
  --max may still report prior tip (e.g. makefile_max=23700). Soft high-water
  may advance toward 23800 while scan remains at prior tip.

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_23800.py)
...
Decade bands for milestone M=23800 (docs only; helper emits paths only):
  extension 23701-23750 | identity 23751-23775 | fill 23776-23790 | markers 23791-23800
  (prior M=23700: identity 23651-23675 | fill 23676-23690 | markers 23691-23700)
