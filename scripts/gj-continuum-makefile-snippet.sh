...
# *decade* is a closed batch range ending at a milestone M (e.g. M=16000
# → exclusive wave 15951–16000). Soft host probes
...
#      Example generator for the 16000 wave (Wave 18 exclusive):
#        user/libcgj/src/_gen_milestone_16000.py
...
# graph_batch16000.c (and beyond) once those basenames are wired.
#
# Wave 18 soft note (16000-era): the exclusive target decade is M=16000
# (CREATE-ONLY 15951–16000; optional extension 15901–15950). Until parent
# pastes those basenames into CGJ_SRCS, ``--max`` may still report the
# prior high-water (e.g. makefile_max=15900). **Scan is the source of
# truth** — do not hardcode a false makefile_max here or in side panels.
# Soft high-water may be described as **advancing toward 16000** while the
...
# reports. Soft graph growth through graph_batch16000.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_16000
# and gj_product_score_16000 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (16000-era / Wave 18 example)
# ------------------------------------------------
# After CREATE-ONLY wave 15951–16000 exists on disk (optional extension
# 15901–15950 may land ahead of the decade bands):
#
#   # Prefer exist-only so missing TUs never produce dead Makefile lines:
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 15951 16000
...
#
# For M=16000 that is identity 15951–15975, fill 15976–15990,
# markers 15991–16000. Continuum extension 15901–15950 sits ahead of
# the decade bands. Prior decade M=15900: identity 15851–15875,
# fill 15876–15890, markers 15891–15900.
# See _gen_milestone_16000.py module docstring (when present) and
# user/libcgj/tests/NOTES_milestone_16000.txt (when present).
#
# Usage:
...
#   user/libcgj/src/graph_batch15900.c \   ← prior high-water (if still tip)
#   user/libcgj/src/graph_batch15901.c \   ← Wave 18 helper output starts here
#   … through graph_batch16000.c once CREATE-ONLY + parent wire land
#
...
#   user/libcgj/src/_gen_milestone_16000.py   (Wave 18 CREATE-ONLY emitter, when present)
#   user/libcgj/tests/NOTES_milestone_15900.txt
#   user/libcgj/tests/NOTES_milestone_16000.txt
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch16000.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 18 exclusive target decade: M=16000 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Until parent wires 15901–16000, --max may
  still report prior tip (e.g. makefile_max=15900). Soft high-water may
  advance toward 16000 while scan remains at prior tip.

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_16000.py)
...
Decade bands for milestone M=16000 (docs only; helper emits paths only):
  identity 15951-15975 | fill 15976-15990 | markers 15991-16000
  extension 15901-15950 (pre-decade; may land ahead of identity/fill/markers)
  (prior M=15900: identity 15851-15875 | fill 15876-15890 | markers 15891-15900)
...
# No artificial upper bound: correctly reports through 16000+ when wired.
makefile_max() {
	if [[ ! -f "$MAKEFILE" ]]; then
