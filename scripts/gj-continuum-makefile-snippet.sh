...
# *decade* is a closed batch range ending at a milestone M (e.g. M=25100
# → exclusive wave 25001–25100). Soft host probes
...
# graph_batch25100.c (and beyond) once those basenames are wired.
#
# Wave 109 soft note (25100-era): the exclusive target decade is M=25100
# (CREATE-ONLY 25001–25100; extension identity 25001–25050 ahead of the
# decade identity/fill/marker bands). Soft deepen surfaces this wave
# (docs/userland soft stamps): retnurbsangle / retbsplineangle (CREATE-ONLY soft
# names only — not product). Until parent pastes those basenames into
# CGJ_SRCS, ``--max`` may still report the prior high-water
# (e.g. makefile_max=25000). **Scan is the source of truth** — do not
# hardcode a false makefile_max here or in side panels. Soft high-water
# may be described as **advancing toward 25100** while the honest scan
# remains at the prior tip (often makefile_max=25000).
#
...
# reports. Soft graph growth through graph_batch25100.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_25100
# and gj_product_score_25100 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (25100-era / Wave 109 example)
# ------------------------------------------------
# After CREATE-ONLY wave 25001–25100 exists on disk:
#
...
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 25051 25100
#
...
#
# For M=25100 that is extension identity 25001–25050, decade identity
...
# See _gen_milestone_25100.py module docstring (when present) and
...
#   user/libcgj/src/graph_batch25001.c \   ← Wave 109 helper output starts here
#   … through graph_batch25100.c once CREATE-ONLY + parent wire land
#
...
#   user/libcgj/src/_gen_milestone_25100.py   (Wave 109 CREATE-ONLY emitter, when present)
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch25100.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 109 exclusive target decade: M=25100 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Soft deepen surfaces: retnurbsangle /
  retbsplineangle (CREATE-ONLY soft names only). Until parent wires 25001–25100,
  --max may still report prior tip (e.g. makefile_max=25000). Soft high-water
  may advance toward 25100 while scan remains at prior tip (often 25000).

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_25100.py)
...
Decade bands for milestone M=25100 (docs only; helper emits paths only):
  extension 25001-25050 | identity 25051-25075 | fill 25076-25090 | markers 25091-25100
