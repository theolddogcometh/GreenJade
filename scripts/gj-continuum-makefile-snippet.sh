...
# *decade* is a closed batch range ending at a milestone M (e.g. M=22100
# → exclusive wave 22001–22100). Soft host probes
...
#      Example generator for the 22100 wave (Wave 79 exclusive continuum track):
#        user/libcgj/src/_gen_milestone_22100.py
#        python3 user/libcgj/src/_gen_milestone_22100.py --dry-run
#      (prior Wave 78: _gen_milestone_22000.py / M=22000; Wave 77: _gen_milestone_21900.py / M=21900)
...
# graph_batch22100.c (and beyond) once those basenames are wired.
#
# Wave 79 soft note (22100-era): the exclusive target decade is M=22100
# (CREATE-ONLY 22001–22100; extension identity 22001–22050 ahead of the
# decade identity/fill/marker bands). Soft deepen surfaces this wave
# (docs/userland soft stamps): retkeepangle / retredoubtangle (CREATE-ONLY soft
# names only — not product). Until parent pastes those basenames into
# CGJ_SRCS, ``--max`` may still report the prior high-water
# (e.g. makefile_max=22000). **Scan is the source of truth** — do not
# hardcode a false makefile_max here or in side panels. Soft high-water
# may be described as **advancing toward 22100** while the honest scan
...
# reports. Soft graph growth through graph_batch22100.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_22100
# and gj_product_score_22100 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (22100-era / Wave 79 example)
# ------------------------------------------------
# After CREATE-ONLY wave 22001–22100 exists on disk:
#
...
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 22051 22100
#
...
#
# For M=22100 that is extension identity 22001–22050, decade identity
# 22051–22075, fill 22076–22090, markers 22091–22100. Prior decade
# M=22000: identity 21951–21975, fill 21976–21990, markers 21991–22000.
# See _gen_milestone_22100.py module docstring (when present) and
# user/libcgj/tests/NOTES_milestone_22100.txt (when present).
#
# Usage:
...
#   user/libcgj/src/graph_batch22000.c \   ← prior high-water (if still tip)
#   user/libcgj/src/graph_batch22001.c \   ← Wave 79 helper output starts here
#   … through graph_batch22100.c once CREATE-ONLY + parent wire land
#
...
#   user/libcgj/src/_gen_milestone_22100.py   (Wave 79 CREATE-ONLY emitter, when present)
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch22100.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 79 exclusive target decade: M=22100 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Soft deepen surfaces: retkeepangle /
  retredoubtangle (CREATE-ONLY soft names only). Until parent wires 22001–22100,
  --max may still report prior tip (e.g. makefile_max=22000). Soft high-water
  may advance toward 22100 while scan remains at prior tip.

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_22100.py)
...
Decade bands for milestone M=22100 (docs only; helper emits paths only):
  extension 22001-22050 | identity 22051-22075 | fill 22076-22090 | markers 22091-22100
  (prior M=22000: identity 21951-21975 | fill 21976-21990 | markers 21991-22000)
