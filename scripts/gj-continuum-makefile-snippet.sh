...
# *decade* is a closed batch range ending at a milestone M (e.g. M=24900
# → exclusive wave 24801–24900). Soft host probes
...
# graph_batch24900.c (and beyond) once those basenames are wired.
#
# Wave 107 soft note (24900-era): the exclusive target decade is M=24900
# (CREATE-ONLY 24801–24900; extension identity 24801–24850 ahead of the
# decade identity/fill/marker bands). Soft deepen surfaces this wave
# (docs/userland soft stamps): retsplineangle / retbezierangle (CREATE-ONLY soft
# names only — not product). Until parent pastes those basenames into
# CGJ_SRCS, ``--max`` may still report the prior high-water
# (e.g. makefile_max=24800). **Scan is the source of truth** — do not
# hardcode a false makefile_max here or in side panels. Soft high-water
# may be described as **advancing toward 24900** while the honest scan
# remains at the prior tip (often makefile_max=24800).
#
...
# reports. Soft graph growth through graph_batch24900.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_24900
# and gj_product_score_24900 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (24900-era / Wave 107 example)
# ------------------------------------------------
# After CREATE-ONLY wave 24801–24900 exists on disk:
#
...
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 24751 24800
#
...
#   user/libcgj/src/graph_batch24801.c \   ← Wave 107 helper output starts here
...
#   user/libcgj/src/_gen_milestone_24900.py   (Wave 107 CREATE-ONLY emitter, when present)
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch24900.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 107 exclusive target decade: M=24900 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Soft deepen surfaces: retsplineangle /
  retbezierangle (CREATE-ONLY soft names only). Until parent wires 24801–24900,
  --max may still report prior tip (e.g. makefile_max=24800). Soft high-water
  may advance toward 24900 while scan remains at prior tip (often 24800).

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_24900.py)
...
Decade bands for milestone M=24900 (docs only; helper emits paths only):
  extension 24801-24850 | identity 24851-24875 | fill 24876-24890 | markers 24891-24900
