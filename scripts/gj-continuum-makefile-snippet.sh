...
# *decade* is a closed batch range ending at a milestone M (e.g. M=22300
# → exclusive wave 22201–22300). Soft host probes
...
#      Example generator for the 22300 wave (Wave 81 exclusive continuum track):
#        user/libcgj/src/_gen_milestone_22300.py
#        python3 user/libcgj/src/_gen_milestone_22300.py --dry-run
#      (prior Wave 80: _gen_milestone_22200.py / M=22200; Wave 79: _gen_milestone_22100.py / M=22100; Wave 78: _gen_milestone_22000.py / M=22000)
...
# graph_batch22300.c (and beyond) once those basenames are wired.
#
# Wave 81 soft note (22300-era): the exclusive target decade is M=22300
# (CREATE-ONLY 22201–22300; extension identity 22201–22250 ahead of the
# decade identity/fill/marker bands). Soft deepen surfaces this wave
# (docs/userland soft stamps): retpanoplyangle / retbulwarkangle (CREATE-ONLY soft
# names only — not product). Until parent pastes those basenames into
# CGJ_SRCS, ``--max`` may still report the prior high-water
# (e.g. makefile_max=22200). **Scan is the source of truth** — do not
# hardcode a false makefile_max here or in side panels. Soft high-water
# may be described as **advancing toward 22300** while the honest scan
...
# reports. Soft graph growth through graph_batch22300.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_22300
# and gj_product_score_22300 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (22300-era / Wave 81 example)
# ------------------------------------------------
# After CREATE-ONLY wave 22201–22300 exists on disk:
#
...
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 22251 22300
#
...
#
# For M=22300 that is extension identity 22201–22250, decade identity
# 22251–22275, fill 22276–22290, markers 22291–22300. Prior decade M=22200: identity 22151–22175, fill 22176–22190, markers 22191–22200. Prior decade
# M=22100: identity 22051–22075, fill 22076–22090, markers 22091–22100.
# See _gen_milestone_22300.py module docstring (when present) and
# user/libcgj/tests/NOTES_milestone_22200.txt
#   user/libcgj/tests/NOTES_milestone_22300.txt (when present).
#
# Usage:
...
#   user/libcgj/src/graph_batch22200.c \   ← prior high-water (if still tip)
#   user/libcgj/src/graph_batch22201.c \   ← Wave 81 helper output starts here
#   … through graph_batch22300.c once CREATE-ONLY + parent wire land
#
...
#   user/libcgj/src/_gen_milestone_22000.py   (prior Wave 78 CREATE-ONLY emitter)
#   user/libcgj/src/_gen_milestone_22200.py   (prior Wave 80 CREATE-ONLY emitter, when present)
#   user/libcgj/src/_gen_milestone_22300.py   (Wave 81 CREATE-ONLY emitter, when present)
#   user/libcgj/tests/NOTES_milestone_22100.txt
#   user/libcgj/tests/NOTES_milestone_22000.txt
#   user/libcgj/tests/NOTES_milestone_22200.txt
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch22300.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 81 exclusive target decade: M=22300 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Soft deepen surfaces: retpanoplyangle /
  retbulwarkangle (CREATE-ONLY soft names only). Until parent wires 22201–22300,
  --max may still report prior tip (e.g. makefile_max=22200). Soft high-water
  may advance toward 22300 while scan remains at prior tip.

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_22300.py)
...
Decade bands for milestone M=22300 (docs only; helper emits paths only):
  extension 22201-22250 | identity 22251-22275 | fill 22276-22290 | markers 22291-22300
  (prior M=22200: identity 22151-22175 | fill 22176-22190 | markers 22191-22200)
  (prior M=22100: identity 22051-22075 | fill 22076-22090 | markers 22091-22100)
