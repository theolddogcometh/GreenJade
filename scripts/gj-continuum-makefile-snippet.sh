...
# *decade* is a closed batch range ending at a milestone M (e.g. M=22200
# → exclusive wave 22101–22200). Soft host probes
...
#      Example generator for the 22200 wave (Wave 80 exclusive continuum track):
#        user/libcgj/src/_gen_milestone_22200.py
#        python3 user/libcgj/src/_gen_milestone_22200.py --dry-run
#      (prior Wave 79: _gen_milestone_22100.py / M=22100; Wave 78: _gen_milestone_22000.py / M=22000)
...
# graph_batch22200.c (and beyond) once those basenames are wired.
#
# Wave 80 soft note (22200-era): the exclusive target decade is M=22200
# (CREATE-ONLY 22101–22200; extension identity 22101–22150 ahead of the
# decade identity/fill/marker bands). Soft deepen surfaces this wave
# (docs/userland soft stamps): retcitadelangle / retbastionkeep (CREATE-ONLY soft
# names only — not product). Until parent pastes those basenames into
# CGJ_SRCS, ``--max`` may still report the prior high-water
# (e.g. makefile_max=22100). **Scan is the source of truth** — do not
# hardcode a false makefile_max here or in side panels. Soft high-water
# may be described as **advancing toward 22200** while the honest scan
...
# reports. Soft graph growth through graph_batch22200.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_22200
# and gj_product_score_22200 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (22200-era / Wave 80 example)
# ------------------------------------------------
# After CREATE-ONLY wave 22101–22200 exists on disk:
#
...
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 22151 22200
#
...
#
# For M=22200 that is extension identity 22101–22150, decade identity
# 22151–22175, fill 22176–22190, markers 22191–22200. Prior decade
# M=22100: identity 22051–22075, fill 22076–22090, markers 22091–22100.
# See _gen_milestone_22200.py module docstring (when present) and
# user/libcgj/tests/NOTES_milestone_22200.txt (when present).
#
# Usage:
...
#   user/libcgj/src/graph_batch22100.c \   ← prior high-water (if still tip)
#   user/libcgj/src/graph_batch22101.c \   ← Wave 80 helper output starts here
#   … through graph_batch22200.c once CREATE-ONLY + parent wire land
#
...
#   user/libcgj/src/_gen_milestone_22200.py   (Wave 80 CREATE-ONLY emitter, when present)
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch22200.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 80 exclusive target decade: M=22200 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Soft deepen surfaces: retcitadelangle /
  retbastionkeep (CREATE-ONLY soft names only). Until parent wires 22101–22200,
  --max may still report prior tip (e.g. makefile_max=22100). Soft high-water
  may advance toward 22200 while scan remains at prior tip.

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_22200.py)
...
Decade bands for milestone M=22200 (docs only; helper emits paths only):
  extension 22101-22150 | identity 22151-22175 | fill 22176-22190 | markers 22191-22200
