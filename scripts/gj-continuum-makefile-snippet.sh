...
# *decade* is a closed batch range ending at a milestone M (e.g. M=17300
# → exclusive wave 17251–17300). Soft host probes
...
#      Example generator for the 17300 wave (Wave 31 exclusive continuum track):
#        user/libcgj/src/_gen_milestone_17300.py
#        python3 user/libcgj/src/_gen_milestone_17300.py --dry-run
#      (prior Wave 30: _gen_milestone_17200.py / M=17200)
...
# graph_batch17300.c (and beyond) once those basenames are wired.
#
# Wave 31 soft note (17300-era): the exclusive target decade is M=17300
# (CREATE-ONLY 17251–17300; optional extension 17201–17250). Until parent
# pastes those basenames into CGJ_SRCS, ``--max`` may still report the
# prior high-water (e.g. makefile_max=17200). **Scan is the source of
# truth** — do not hardcode a false makefile_max here or in side panels.
# Soft high-water may be described as **advancing toward 17300** while the
...
# reports. Soft graph growth through graph_batch17300.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_17300
# and gj_product_score_17300 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (17300-era / Wave 31 example)
# ------------------------------------------------
# After CREATE-ONLY wave 17251–17300 exists on disk (optional extension
# 17201–17250 may land ahead of the decade bands):
#
#   # Prefer exist-only so missing TUs never produce dead Makefile lines:
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 17251 17300
...
#   ./scripts/gj-continuum-makefile-snippet.sh 17251 17300
#
...
#
# For M=17300 that is identity 17251–17275, fill 17276–17290,
# markers 17291–17300. Continuum extension 17201–17250 sits ahead of
# the decade bands. Prior decade M=17200: identity 17151–17175,
# fill 17176–17190, markers 17191–17200.
# See _gen_milestone_17300.py module docstring (when present) and
# user/libcgj/tests/NOTES_milestone_17300.txt (when present).
#
# Usage:
...
#   user/libcgj/src/graph_batch17200.c \   ← prior high-water (if still tip)
#   user/libcgj/src/graph_batch17201.c \   ← Wave 31 helper output starts here
#   … through graph_batch17300.c once CREATE-ONLY + parent wire land
#
...
#
# See also:
#   user/libcgj/src/_gen_milestone_17200.py   (prior Wave 30 CREATE-ONLY emitter)
#   user/libcgj/src/_gen_milestone_17300.py   (Wave 31 CREATE-ONLY emitter, when present)
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch17300.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 31 exclusive target decade: M=17300 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Until parent wires 17201–17300, --max may
  still report prior tip (e.g. makefile_max=17200). Soft high-water may
  advance toward 17300 while scan remains at prior tip.

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_17300.py)
...
Decade bands for milestone M=17300 (docs only; helper emits paths only):
  identity 17251-17275 | fill 17276-17290 | markers 17291-17300
  extension 17201-17250 (pre-decade; may land ahead of identity/fill/markers)
  (prior M=17200: identity 17151-17175 | fill 17176-17190 | markers 17191-17200)
