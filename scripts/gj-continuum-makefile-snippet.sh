...
# *decade* is a closed batch range ending at a milestone M (e.g. M=22900
# → exclusive wave 22801–22900). Soft host probes
...
#      Example generator for the 22900 wave (Wave 87 exclusive continuum track):
#        user/libcgj/src/_gen_milestone_22900.py
#        python3 user/libcgj/src/_gen_milestone_22900.py --dry-run
#      (prior Wave 86: _gen_milestone_22800.py / M=22800; Wave 85: _gen_milestone_22700.py / M=22700; Wave 84: _gen_milestone_22600.py / M=22600; Wave 83: _gen_milestone_22500.py / M=22500; Wave 82: _gen_milestone_22400.py / M=22400; Wave 81: _gen_milestone_22300.py / M=22300)
...
# graph_batch22900.c (and beyond) once those basenames are wired.
#
# Wave 87 soft note (22900-era): the exclusive target decade is M=22900
# (CREATE-ONLY 22801–22900; extension identity 22801–22850 ahead of the
# decade identity/fill/marker bands). Soft deepen surfaces this wave
# (docs/userland soft stamps): retflameangle / retcipherangle (CREATE-ONLY soft
# names only — not product). Until parent pastes those basenames into
# CGJ_SRCS, ``--max`` may still report the prior high-water
# (e.g. makefile_max=22800). **Scan is the source of truth** — do not
# hardcode a false makefile_max here or in side panels. Soft high-water
# may be described as **advancing toward 22900** while the honest scan
...
# reports. Soft graph growth through graph_batch22900.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_22900
# and gj_product_score_22900 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (22900-era / Wave 87 example)
# ------------------------------------------------
# After CREATE-ONLY wave 22801–22900 exists on disk:
#
...
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 22851 22900
#
...
#
# For M=22900 that is extension identity 22801–22850, decade identity
# 22851–22875, fill 22876–22890, markers 22891–22900. Prior decade M=22800: identity 22751–22775, fill 22776–22790, markers 22791–22800. Prior decade M=22700: identity 22651–22675, fill 22676–22690, markers 22691–22700. Prior decade M=22500: identity 22451–22475, fill 22476–22490, markers 22491–22500. Prior decade M=22400: identity 22351–22375, fill 22376–22390, markers 22391–22400. Prior decade
# M=22300: identity 22251–22275, fill 22276–22290, markers 22291–22300.
# See _gen_milestone_22900.py module docstring (when present) and
...
#   user/libcgj/tests/NOTES_milestone_22900.txt (when present).
#
# Usage:
...
#   user/libcgj/src/graph_batch22800.c \   ← prior high-water (if still tip)
#   user/libcgj/src/graph_batch22801.c \   ← Wave 87 helper output starts here
#   … through graph_batch22900.c once CREATE-ONLY + parent wire land
#
...
#   user/libcgj/src/_gen_milestone_22900.py   (Wave 87 CREATE-ONLY emitter, when present)
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch22900.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 87 exclusive target decade: M=22900 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Soft deepen surfaces: retflameangle /
  retcipherangle (CREATE-ONLY soft names only). Until parent wires 22801–22900,
  --max may still report prior tip (e.g. makefile_max=22800). Soft high-water
  may advance toward 22900 while scan remains at prior tip.

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_22900.py)
...
Decade bands for milestone M=22900 (docs only; helper emits paths only):
  extension 22801-22850 | identity 22851-22875 | fill 22876-22890 | markers 22891-22900
