...
# *decade* is a closed batch range ending at a milestone M (e.g. M=19200
# → exclusive wave 19101–19200). Soft host probes
...
#      Example generator for the 19200 wave (Wave 50 exclusive continuum track):
#        user/libcgj/src/_gen_milestone_19200.py
#        python3 user/libcgj/src/_gen_milestone_19200.py --dry-run
#      (prior Wave 49: _gen_milestone_19100.py / M=19100)
...
# graph_batch19200.c (and beyond) once those basenames are wired.
#
# Wave 50 soft note (19200-era): the exclusive target decade is M=19200
# (CREATE-ONLY 19101–19200; extension identity 19101–19150 ahead of the
# decade identity/fill/marker bands). Soft deepen surfaces this wave
# (docs/userland soft stamps): retsally / retcounterscarp (CREATE-ONLY soft
# names only — not product). Until parent pastes those basenames into
# CGJ_SRCS, ``--max`` may still report the prior high-water
# (e.g. makefile_max=19100). **Scan is the source of truth** — do not
# hardcode a false makefile_max here or in side panels. Soft high-water
# may be described as **advancing toward 19200** while the honest scan
...
# reports. Soft graph growth through graph_batch19200.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_19200
# and gj_product_score_19200 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (19200-era / Wave 50 example)
# ------------------------------------------------
# After CREATE-ONLY wave 19101–19200 exists on disk:
#
...
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 19151 19200
#
...
#
# For M=19200 that is extension identity 19101–19150, decade identity
# 19151–19175, fill 19176–19190, markers 19191–19200. Prior decade
# M=19100: identity 19051–19075, fill 19076–19090, markers 19091–19100.
# See _gen_milestone_19200.py module docstring (when present) and
# user/libcgj/tests/NOTES_milestone_19200.txt (when present).
#
# Usage:
...
#   user/libcgj/src/graph_batch19100.c \   ← prior high-water (if still tip)
#   user/libcgj/src/graph_batch19101.c \   ← Wave 50 helper output starts here
#   … through graph_batch19200.c once CREATE-ONLY + parent wire land
#
...
#
# See also:
#   user/libcgj/src/_gen_milestone_19100.py   (prior Wave 49 CREATE-ONLY emitter)
#   user/libcgj/src/_gen_milestone_19200.py   (Wave 50 CREATE-ONLY emitter, when present)
#   user/libcgj/tests/NOTES_milestone_19100.txt
#   user/libcgj/tests/NOTES_milestone_19200.txt
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch19200.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 50 exclusive target decade: M=19200 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Soft deepen surfaces: retsally /
  retcounterscarp (CREATE-ONLY soft names only). Until parent wires 19101–19200,
  --max may still report prior tip (e.g. makefile_max=19100). Soft high-water
  may advance toward 19200 while scan remains at prior tip.

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_19200.py)
...
Decade bands for milestone M=19200 (docs only; helper emits paths only):
  extension 19101-19150 | identity 19151-19175 | fill 19176-19190 | markers 19191-19200
  (prior M=19100: identity 19051-19075 | fill 19076-19090 | markers 19091-19100)
