...
# *decade* is a closed batch range ending at a milestone M (e.g. M=22000
# → exclusive wave 21901–22000). Soft host probes
...
#      Example generator for the 22000 wave (Wave 78 exclusive continuum track):
#        user/libcgj/src/_gen_milestone_22000.py
#        python3 user/libcgj/src/_gen_milestone_22000.py --dry-run
#      (prior Wave 77: _gen_milestone_21900.py / M=21900; Wave 76: _gen_milestone_21800.py / M=21800)
...
# graph_batch22000.c (and beyond) once those basenames are wired.
#
# Wave 78 soft note (22000-era): the exclusive target decade is M=22000
# (CREATE-ONLY 21901–22000; extension identity 21901–21950 ahead of the
# decade identity/fill/marker bands). Soft deepen surfaces this wave
# (docs/userland soft stamps): retpalaceangle / retfortressangle (CREATE-ONLY soft
# names only — not product). Until parent pastes those basenames into
# CGJ_SRCS, ``--max`` may still report the prior high-water
# (e.g. makefile_max=21900). **Scan is the source of truth** — do not
# hardcode a false makefile_max here or in side panels. Soft high-water
# may be described as **advancing toward 22000** while the honest scan
...
# reports. Soft graph growth through graph_batch22000.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_22000
# and gj_product_score_22000 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (22000-era / Wave 78 example)
# ------------------------------------------------
# After CREATE-ONLY wave 21901–22000 exists on disk:
#
...
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 21951 22000
#
...
#
# For M=22000 that is extension identity 21901–21950, decade identity
# 21951–21975, fill 21976–21990, markers 21991–22000. Prior decade
# M=21900: identity 21851–21875, fill 21876–21890, markers 21891–21900.
# See _gen_milestone_22000.py module docstring (when present) and
# user/libcgj/tests/NOTES_milestone_22000.txt (when present).
#
# Usage:
...
#   user/libcgj/src/graph_batch21900.c \   ← prior high-water (if still tip)
#   user/libcgj/src/graph_batch21901.c \   ← Wave 78 helper output starts here
#   … through graph_batch22000.c once CREATE-ONLY + parent wire land
#
...
#
# See also:
#   user/libcgj/src/_gen_milestone_21900.py   (prior Wave 77 CREATE-ONLY emitter)
#   user/libcgj/src/_gen_milestone_22000.py   (Wave 78 CREATE-ONLY emitter, when present)
#   user/libcgj/tests/NOTES_milestone_21900.txt
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch22000.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 78 exclusive target decade: M=22000 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Soft deepen surfaces: retpalaceangle /
  retfortressangle (CREATE-ONLY soft names only). Until parent wires 21901–22000,
  --max may still report prior tip (e.g. makefile_max=21900). Soft high-water
  may advance toward 22000 while scan remains at prior tip.

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_22000.py)
...
Decade bands for milestone M=22000 (docs only; helper emits paths only):
  extension 21901-21950 | identity 21951-21975 | fill 21976-21990 | markers 21991-22000
  (prior M=21900: identity 21851-21875 | fill 21876-21890 | markers 21891-21900)
