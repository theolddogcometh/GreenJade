...
# *decade* is a closed batch range ending at a milestone M (e.g. M=21500
# → exclusive wave 21401–21500). Soft host probes
...
#      Example generator for the 21500 wave (Wave 73 exclusive continuum track):
#        user/libcgj/src/_gen_milestone_21500.py
#        python3 user/libcgj/src/_gen_milestone_21500.py --dry-run
#      (prior Wave 72: _gen_milestone_21400.py / M=21400; Wave 71: _gen_milestone_21300.py / M=21300)
...
# graph_batch21500.c (and beyond) once those basenames are wired.
#
# Wave 73 soft note (21500-era): the exclusive target decade is M=21500
# (CREATE-ONLY 21401–21500; extension identity 21401–21450 ahead of the
# decade identity/fill/marker bands). Soft deepen surfaces this wave
# (docs/userland soft stamps): retcurtainface / retbastionangle (CREATE-ONLY soft
# names only — not product). Until parent pastes those basenames into
# CGJ_SRCS, ``--max`` may still report the prior high-water
# (e.g. makefile_max=21400). **Scan is the source of truth** — do not
# hardcode a false makefile_max here or in side panels. Soft high-water
# may be described as **advancing toward 21500** while the honest scan
...
# reports. Soft graph growth through graph_batch21500.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_21500
# and gj_product_score_21500 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (21500-era / Wave 73 example)
# ------------------------------------------------
# After CREATE-ONLY wave 21401–21500 exists on disk:
#
...
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 21451 21500
#
...
#
# For M=21500 that is extension identity 21401–21450, decade identity
# 21451–21475, fill 21476–21490, markers 21491–21500. Prior decade
# M=21400: identity 21351–21375, fill 21376–21390, markers 21391–21400.
# See _gen_milestone_21500.py module docstring (when present) and
# user/libcgj/tests/NOTES_milestone_21500.txt (when present).
#
# Usage:
...
#   user/libcgj/src/graph_batch21400.c \   ← prior high-water (if still tip)
#   user/libcgj/src/graph_batch21401.c \   ← Wave 73 helper output starts here
#   … through graph_batch21500.c once CREATE-ONLY + parent wire land
#
...
#
# See also:
#   user/libcgj/src/_gen_milestone_21400.py   (prior Wave 72 CREATE-ONLY emitter)
#   user/libcgj/src/_gen_milestone_21500.py   (Wave 73 CREATE-ONLY emitter, when present)
#   user/libcgj/tests/NOTES_milestone_21400.txt
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch21500.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 73 exclusive target decade: M=21500 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Soft deepen surfaces: retcurtainface /
  retbastionangle (CREATE-ONLY soft names only). Until parent wires 21401–21500,
  --max may still report prior tip (e.g. makefile_max=21400). Soft high-water
  may advance toward 21500 while scan remains at prior tip.

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_21500.py)
...
Decade bands for milestone M=21500 (docs only; helper emits paths only):
  extension 21401-21450 | identity 21451-21475 | fill 21476-21490 | markers 21491-21500
  (prior M=21400: identity 21351-21375 | fill 21376-21390 | markers 21391-21400)
