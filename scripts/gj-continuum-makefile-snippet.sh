...
# *decade* is a closed batch range ending at a milestone M (e.g. M=20100
# → exclusive wave 20001–20100). Soft host probes
...
#      Example generator for the 20100 wave (Wave 59 exclusive continuum track):
#        user/libcgj/src/_gen_milestone_20100.py
#        python3 user/libcgj/src/_gen_milestone_20100.py --dry-run
#      (prior Wave 58: _gen_milestone_20000.py / M=20000)
...
# graph_batch20100.c (and beyond) once those basenames are wired.
#
# Wave 59 soft note (20100-era): the exclusive target decade is M=20100
# (CREATE-ONLY 20001–20100; extension identity 20001–20050 ahead of the
# decade identity/fill/marker bands). Soft deepen surfaces this wave
# (docs/userland soft stamps): retcrownwork / rethornwork (CREATE-ONLY soft
# names only — not product). Until parent pastes those basenames into
# CGJ_SRCS, ``--max`` may still report the prior high-water
# (e.g. makefile_max=20000). **Scan is the source of truth** — do not
# hardcode a false makefile_max here or in side panels. Soft high-water
# may be described as **advancing toward 20100** while the honest scan
...
# reports. Soft graph growth through graph_batch20100.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_20100
# and gj_product_score_20100 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (20100-era / Wave 59 example)
# ------------------------------------------------
# After CREATE-ONLY wave 20001–20100 exists on disk:
#
...
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 20051 20100
#
...
#
# For M=20100 that is extension identity 20001–20050, decade identity
# 20051–20075, fill 20076–20090, markers 20091–20100. Prior decade
# M=20000: identity 19951–19975, fill 19976–19990, markers 19991–20000.
# See _gen_milestone_20100.py module docstring (when present) and
# user/libcgj/tests/NOTES_milestone_20100.txt (when present).
#
# Usage:
...
#   user/libcgj/src/graph_batch20000.c \   ← prior high-water (if still tip)
#   user/libcgj/src/graph_batch20001.c \   ← Wave 59 helper output starts here
#   … through graph_batch20100.c once CREATE-ONLY + parent wire land
#
...
#
# See also:
#   user/libcgj/src/_gen_milestone_20000.py   (prior Wave 58 CREATE-ONLY emitter)
#   user/libcgj/src/_gen_milestone_20100.py   (Wave 59 CREATE-ONLY emitter, when present)
#   user/libcgj/tests/NOTES_milestone_20000.txt
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch20100.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 59 exclusive target decade: M=20100 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Soft deepen surfaces: retcrownwork /
  rethornwork (CREATE-ONLY soft names only). Until parent wires 20001–20100,
  --max may still report prior tip (e.g. makefile_max=20000). Soft high-water
  may advance toward 20100 while scan remains at prior tip.

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_20100.py)
...
Decade bands for milestone M=20100 (docs only; helper emits paths only):
  extension 20001-20050 | identity 20051-20075 | fill 20076-20090 | markers 20091-20100
  (prior M=20000: identity 19951-19975 | fill 19976-19990 | markers 19991-20000)
