...
# *decade* is a closed batch range ending at a milestone M (e.g. M=23400
# → exclusive wave 23301–23400). Soft host probes
...
# graph_batch23400.c (and beyond) once those basenames are wired.
#
# Wave 92 soft note (23400-era): the exclusive target decade is M=23400
# (CREATE-ONLY 23301–23400; extension identity 23301–23350 ahead of the
# decade identity/fill/marker bands). Soft deepen surfaces this wave
# (docs/userland soft stamps): retmarkangle / retpulseangle (CREATE-ONLY soft
# names only — not product). Until parent pastes those basenames into
# CGJ_SRCS, ``--max`` may still report the prior high-water
# (e.g. makefile_max=23300). **Scan is the source of truth** — do not
# hardcode a false makefile_max here or in side panels. Soft high-water
# may be described as **advancing toward 23400** while the honest scan
...
# reports. Soft graph growth through graph_batch23400.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_23400
# and gj_product_score_23400 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (23400-era / Wave 92 example)
# ------------------------------------------------
# After CREATE-ONLY wave 23301–23400 exists on disk:
#
...
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 23351 23400
#
...
#   user/libcgj/tests/NOTES_milestone_23400.txt (when present).
#
# Usage:
...
#   user/libcgj/src/graph_batch23300.c \   ← prior high-water (if still tip)
#   user/libcgj/src/graph_batch23301.c \   ← Wave 92 helper output starts here
#   … through graph_batch23400.c once CREATE-ONLY + parent wire land
#
...
#   user/libcgj/src/_gen_milestone_23400.py   (Wave 92 CREATE-ONLY emitter, when present)
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch23400.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 92 exclusive target decade: M=23400 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Soft deepen surfaces: retmarkangle /
  retpulseangle (CREATE-ONLY soft names only). Until parent wires 23301–23400,
  --max may still report prior tip (e.g. makefile_max=23300). Soft high-water
  may advance toward 23400 while scan remains at prior tip.

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_23400.py)
...
Decade bands for milestone M=23400 (docs only; helper emits paths only):
  extension 23301-23350 | identity 23351-23375 | fill 23376-23390 | markers 23391-23400
  (prior M=23300: identity 23151-23175 | fill 23176-23190 | markers 23191-23300)
