...
# *decade* is a closed batch range ending at a milestone M (e.g. M=19400
# → exclusive wave 19301–19400). Soft host probes
...
#      Example generator for the 19400 wave (Wave 52 exclusive continuum track):
#        user/libcgj/src/_gen_milestone_19400.py
#        python3 user/libcgj/src/_gen_milestone_19400.py --dry-run
#      (prior Wave 51: _gen_milestone_19300.py / M=19300)
...
# graph_batch19400.c (and beyond) once those basenames are wired.
#
# Wave 52 soft note (19400-era): the exclusive target decade is M=19400
# (CREATE-ONLY 19301–19400; extension identity 19301–19350 ahead of the
# decade identity/fill/marker bands). Soft deepen surfaces this wave
# (docs/userland soft stamps): rettenaille / retdemilune (CREATE-ONLY soft
# names only — not product). Until parent pastes those basenames into
# CGJ_SRCS, ``--max`` may still report the prior high-water
# (e.g. makefile_max=19300). **Scan is the source of truth** — do not
# hardcode a false makefile_max here or in side panels. Soft high-water
# may be described as **advancing toward 19400** while the honest scan
...
# reports. Soft graph growth through graph_batch19400.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_19400
# and gj_product_score_19400 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (19400-era / Wave 52 example)
# ------------------------------------------------
# After CREATE-ONLY wave 19301–19400 exists on disk:
#
...
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 19351 19400
#
...
#
# For M=19400 that is extension identity 19301–19350, decade identity
# 19351–19375, fill 19376–19390, markers 19391–19400. Prior decade
# M=19300: identity 19251–19275, fill 19276–19290, markers 19291–19300.
# See _gen_milestone_19400.py module docstring (when present) and
# user/libcgj/tests/NOTES_milestone_19400.txt (when present).
#
# Usage:
...
#   user/libcgj/src/graph_batch19300.c \   ← prior high-water (if still tip)
#   user/libcgj/src/graph_batch19301.c \   ← Wave 52 helper output starts here
#   … through graph_batch19400.c once CREATE-ONLY + parent wire land
#
...
#
# See also:
#   user/libcgj/src/_gen_milestone_19300.py   (prior Wave 51 CREATE-ONLY emitter)
#   user/libcgj/src/_gen_milestone_19400.py   (Wave 52 CREATE-ONLY emitter, when present)
#   user/libcgj/tests/NOTES_milestone_19300.txt
#   user/libcgj/tests/NOTES_milestone_19400.txt
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch19400.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 52 exclusive target decade: M=19400 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Soft deepen surfaces: rettenaille /
  retdemilune (CREATE-ONLY soft names only). Until parent wires 19301–19400,
  --max may still report prior tip (e.g. makefile_max=19300). Soft high-water
  may advance toward 19400 while scan remains at prior tip.

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_19400.py)
...
Decade bands for milestone M=19400 (docs only; helper emits paths only):
  extension 19301-19350 | identity 19351-19375 | fill 19376-19390 | markers 19391-19400
  (prior M=19300: identity 19251-19275 | fill 19276-19290 | markers 19291-19300)
