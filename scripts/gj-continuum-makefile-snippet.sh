...
# *decade* is a closed batch range ending at a milestone M (e.g. M=21000
# → exclusive wave 20901–21000). Soft host probes
...
#      Example generator for the 21000 wave (Wave 68 exclusive continuum track):
#        user/libcgj/src/_gen_milestone_21000.py
#        python3 user/libcgj/src/_gen_milestone_21000.py --dry-run
#      (prior Wave 67: _gen_milestone_20900.py / M=20900)
...
# graph_batch21000.c (and beyond) once those basenames are wired.
#
# Wave 68 soft note (21000-era): the exclusive target decade is M=21000
# (CREATE-ONLY 20901–21000; extension identity 20901–20950 ahead of the
# decade identity/fill/marker bands). Soft deepen surfaces this wave
# (docs/userland soft stamps): retlunetteangle / rettenailleangle (CREATE-ONLY soft
# names only — not product). Until parent pastes those basenames into
# CGJ_SRCS, ``--max`` may still report the prior high-water
# (e.g. makefile_max=20900). **Scan is the source of truth** — do not
# hardcode a false makefile_max here or in side panels. Soft high-water
# may be described as **advancing toward 21000** while the honest scan
...
# reports. Soft graph growth through graph_batch21000.c is **not** bar3,
# not Deck Top-50 title runs, and not product score (gj_bar3_ready_21000
# and gj_product_score_21000 remain 0 by design). Soft ≠ product complete.
#
# Decade paste hygiene (21000-era / Wave 68 example)
# ------------------------------------------------
# After CREATE-ONLY wave 20901–21000 exists on disk:
#
...
#   ./scripts/gj-continuum-makefile-snippet.sh --exist-only 20951 21000
#
...
#
# For M=21000 that is extension identity 20901–20950, decade identity
# 20951–20975, fill 20976–20990, markers 20991–21000. Prior decade
# M=20900: identity 20851–20875, fill 20876–20890, markers 20891–20900.
# See _gen_milestone_21000.py module docstring (when present) and
# user/libcgj/tests/NOTES_milestone_21000.txt (when present).
#
# Usage:
...
#   user/libcgj/src/graph_batch20900.c \   ← prior high-water (if still tip)
#   user/libcgj/src/graph_batch20901.c \   ← Wave 68 helper output starts here
#   … through graph_batch21000.c once CREATE-ONLY + parent wire land
#
...
#
# See also:
#   user/libcgj/src/_gen_milestone_20900.py   (prior Wave 67 CREATE-ONLY emitter)
#   user/libcgj/src/_gen_milestone_21000.py   (Wave 68 CREATE-ONLY emitter, when present)
#   user/libcgj/tests/NOTES_milestone_20900.txt
...
makefile_max honesty:
  ...
  Scan has no artificial cap (supports through graph_batch21000.c and beyond).
  High-water is whatever the scan prints (source of truth) — do not hardcode.
  Wave 68 exclusive target decade: M=21000 (soft graph only — not bar3 /
  Steam client / Top-50 titles). Soft deepen surfaces: retlunetteangle /
  rettenailleangle (CREATE-ONLY soft names only). Until parent wires 20901–21000,
  --max may still report prior tip (e.g. makefile_max=20900). Soft high-water
  may advance toward 21000 while scan remains at prior tip.

Workflow:
  1. Land CREATE-ONLY TUs (e.g. user/libcgj/src/_gen_milestone_21000.py)
...
Decade bands for milestone M=21000 (docs only; helper emits paths only):
  extension 20901-20950 | identity 20951-20975 | fill 20976-20990 | markers 20991-21000
  (prior M=20900: identity 20851-20875 | fill 20876-20890 | markers 20891-20900)
