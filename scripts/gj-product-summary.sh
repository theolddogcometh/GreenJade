...
#   high-water toward 24500              Wave 103 soft target (note 24400 greppable only when N>=24500; 24300 prior tip when N>=24400)
#   prior tip makefile_max=24400         may still be scan truth until parent wires 24500
#   (do not hardcode false makefile_max if scan still 24400 until parent wires)
#   soft deepen retprojectangle/retaffineangle      Wave 103 CREATE-ONLY soft names only (≠ product)
...
# Wave 103: soft target M=24500; note makefile_max=24500 greppable only when N>=24500.
# Scan is source of truth — never hardcode false 24500 if scan still 24400.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 103 high-water stamp: note 24400 greppable only when N>=24500; 24300 prior tip when N>=24400
