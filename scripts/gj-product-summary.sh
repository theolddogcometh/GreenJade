...
#   high-water toward 23700              Wave 95 soft target (note 23700 greppable only when N>=23700; 23500 prior tip when N>=23600)
#   prior tip makefile_max=23600         may still be scan truth until parent wires 23700
#   (do not hardcode false makefile_max if scan still 23600 until parent wires)
#   soft deepen retchordangle/retarcangle      Wave 95 CREATE-ONLY soft names only (≠ product)
...
# Wave 95: soft target M=23700; note makefile_max=23700 greppable only when N>=23700.
# Scan is source of truth — never hardcode false 23700 if scan still 23600.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 95 high-water stamp: note 23700 greppable only when N>=23700; 23600 prior tip when N>=23600
