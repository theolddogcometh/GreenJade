...
#   high-water toward 21600              Wave 74 soft target (note 21600 greppable only when N>=21600; 21500 prior tip when N>=21500)
#   prior tip makefile_max=21500         may still be scan truth until parent wires 21600
#   (do not hardcode false makefile_max if scan still 21500 until parent wires)
#   soft deepen retglacisangle/retparapetangle      Wave 74 CREATE-ONLY soft names only (≠ product)
...
# Wave 74: soft target M=21600; note makefile_max=21600 greppable only when N>=21600.
# Scan is source of truth — never hardcode false 21600 if scan still 21500.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 74 high-water stamp: note 21600 greppable only when N>=21600; 21500 prior tip
