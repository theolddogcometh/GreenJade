...
#   high-water toward 15800              Wave 16 soft target (note 15800 greppable only when N>=15800)
#   prior tip makefile_max=15700         may still be scan truth until parent wires 15800
#   (do not hardcode false makefile_max if scan still 15700 until parent wires)
...
# Wave 16: soft target M=15800; note makefile_max=15800 greppable only when N>=15800.
# Scan is source of truth — never hardcode false 15800 if scan still 15700.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 16 high-water stamp: note 15800 greppable only when N>=15800
