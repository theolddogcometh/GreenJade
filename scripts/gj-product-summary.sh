...
#   high-water toward 26800              Wave 126 soft target (note 26700 greppable only when N>=26700; 26600 prior tip when N>=26700)
...
#   soft deepen retgradientangle/retblendangle  Wave 126 CREATE-ONLY soft names only (≠ product)
...
# Wave 126: soft target M=26800; note makefile_max=26800 greppable only when N>=26700.
...
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 126 high-water stamp: note 26700 greppable only when N>=26700; 26600 prior tip when N>=26700
