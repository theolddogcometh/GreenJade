...
#   high-water toward 25500              Wave 113 soft target (note 25500 greppable only when N>=25500; 25400 prior tip when N>=25400)
#   prior tip makefile_max=25400         may still be scan truth until parent wires 25500
#   (do not hardcode false makefile_max if scan still prior tip until parent wires)
#   soft deepen retshaderangle/retpipelineangle  Wave 113 CREATE-ONLY soft names only (≠ product)
...
# Wave 113: soft target M=25500; note makefile_max=25500 greppable only when N>=25500.
# Scan is source of truth — never hardcode false 25500 if scan still prior tip (25400).
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 113 high-water stamp: note 25500 greppable only when N>=25500; 25400 prior tip when N>=25400
