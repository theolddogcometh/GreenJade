...
#   high-water toward 19500              Wave 53 soft target (note 19500 greppable only when N>=19500; 19400 prior tip when N>=19400)
#   prior tip makefile_max=19400         may still be scan truth until parent wires 19500
#   (do not hardcode false makefile_max if scan still 19400 until parent wires)
#   soft deepen retravelin/retlunette      Wave 53 CREATE-ONLY soft names only (≠ product)
...
# Wave 53: soft target M=19500; note makefile_max=19500 greppable only when N>=19500.
# Scan is source of truth — never hardcode false 19500 if scan still 19400.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 53 high-water stamp: note 19500 greppable only when N>=19500; 19400 prior tip
