...
#   high-water toward 25600              Wave 114 soft target (note 25600 greppable only when N>=25600; 25500 prior tip when N>=25500)
#   prior tip makefile_max=25500         may still be scan truth until parent wires 25600
#   (do not hardcode false makefile_max if scan still prior tip until parent wires)
#   soft deepen retframebufferangle/retswapchainangle  Wave 114 CREATE-ONLY soft names only (≠ product)
...
# Wave 114: soft target M=25600; note makefile_max=25600 greppable only when N>=25600.
# Scan is source of truth — never hardcode false 25600 if scan still prior tip (25500).
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 114 high-water stamp: note 25600 greppable only when N>=25600; 25500 prior tip when N>=25500
