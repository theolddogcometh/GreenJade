...
#   high-water toward 17500              Wave 33 soft target (note 17500 greppable only when N>=17500; 17400 prior tip when N>=17400)
#   prior tip makefile_max=17400         may still be scan truth until parent wires 17500
#   (do not hardcode false makefile_max if scan still 17400 until parent wires)
...
# Wave 33: soft target M=17500; note makefile_max=17500 greppable only when N>=17500.
# Scan is source of truth — never hardcode false 17500 if scan still 17400.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 33 high-water stamp: note 17500 greppable only when N>=17500; 17400 prior tip
