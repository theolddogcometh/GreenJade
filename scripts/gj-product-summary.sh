...
#   high-water toward 25000              Wave 108 soft target (note 25000 greppable only when N>=25000; 24900 prior tip when N>=24900)
#   prior tip makefile_max=24900         may still be scan truth until parent wires 25000
#   (do not hardcode false makefile_max if scan still prior tip until parent wires)
#   soft deepen rethurmitangle/retcatmullangle      Wave 108 CREATE-ONLY soft names only (≠ product)
...
# Wave 108: soft target M=25000; note makefile_max=25000 greppable only when N>=25000.
# Scan is source of truth — never hardcode false 25000 if scan still prior tip (24900).
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 108 high-water stamp: note 25000 greppable only when N>=25000; 24900 prior tip when N>=24900
