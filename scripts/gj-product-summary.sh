...
#   high-water toward 21200              Wave 70 soft target (note 21200 greppable only when N>=21200; 21100 prior tip when N>=21100)
#   prior tip makefile_max=21100         may still be scan truth until parent wires 21200
#   (do not hardcode false makefile_max if scan still 21100 until parent wires)
#   soft deepen retfosseangle/retcounterscarple      Wave 70 CREATE-ONLY soft names only (≠ product)
...
# Wave 70: soft target M=21200; note makefile_max=21200 greppable only when N>=21200.
# Scan is source of truth — never hardcode false 21200 if scan still 21100.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 70 high-water stamp: note 21200 greppable only when N>=21200; 21100 prior tip
