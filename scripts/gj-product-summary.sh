...
#   high-water toward 23800              Wave 96 soft target (note 23800 greppable only when N>=23800; 23700 prior tip when N>=23700)
#   prior tip makefile_max=23700         may still be scan truth until parent wires 23800
#   (do not hardcode false makefile_max if scan still 23700 until parent wires)
#   soft deepen retsectorangle/retwedgeangle      Wave 96 CREATE-ONLY soft names only (≠ product)
...
# Wave 96: soft target M=23800; note makefile_max=23800 greppable only when N>=23800.
# Scan is source of truth — never hardcode false 23800 if scan still 23700.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 96 high-water stamp: note 23800 greppable only when N>=23800; 23700 prior tip when N>=23700
