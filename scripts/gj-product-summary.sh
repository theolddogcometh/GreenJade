...
#   high-water toward 15700              Wave 15 soft target (note 15700 greppable only when N>=15700)
#   prior tip makefile_max=15600         may still be scan truth until parent wires 15700
#   (do not hardcode false makefile_max if scan still 15600 until parent wires)
...
# Wave 15: soft target M=15700; note makefile_max=15700 greppable only when N>=15700.
# Scan is source of truth — never hardcode false 15700 if scan still 15600.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 15 high-water stamp: note 15700 greppable only when N>=15700
