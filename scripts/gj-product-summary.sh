...
#   high-water toward 15900              Wave 17 soft target (note 15900 greppable only when N>=15900)
#   prior tip makefile_max=15800         may still be scan truth until parent wires 15900
#   (do not hardcode false makefile_max if scan still 15800 until parent wires)
...
# Wave 17: soft target M=15900; note makefile_max=15900 greppable only when N>=15900.
# Scan is source of truth — never hardcode false 15900 if scan still 15800.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 17 high-water stamp: note 15900 greppable only when N>=15900
