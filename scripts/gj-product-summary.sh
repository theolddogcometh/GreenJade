...
#   high-water toward 23300              Wave 91 soft target (note 23300 greppable only when N>=23300; 23200 prior tip when N>=23200)
#   prior tip makefile_max=23200         may still be scan truth until parent wires 23300
#   (do not hardcode false makefile_max if scan still 23200 until parent wires)
#   soft deepen retbadgeangle/retphaseangle      Wave 91 CREATE-ONLY soft names only (≠ product)
...
# Wave 91: soft target M=23300; note makefile_max=23300 greppable only when N>=23300.
# Scan is source of truth — never hardcode false 23300 if scan still 23200.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 91 high-water stamp: note 23300 greppable only when N>=23300; 23200 prior tip when N>=23200
