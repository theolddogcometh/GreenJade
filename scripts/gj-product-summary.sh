...
#   high-water makefile_max=15300        noted when greppable (soft graph ≠ bar3)
...
# Note makefile_max=15300 when greppable from helper stdout (N>=15300).
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave high-water stamp: note makefile_max=15300 only when greppable
