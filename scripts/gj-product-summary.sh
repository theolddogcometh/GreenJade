...
#   high-water toward 23400              Wave 92 soft target (note 23400 greppable only when N>=23400; 23300 prior tip when N>=23300)
#   prior tip makefile_max=23300         may still be scan truth until parent wires 23400
#   (do not hardcode false makefile_max if scan still 23300 until parent wires)
#   soft deepen retmarkangle/retpulseangle      Wave 92 CREATE-ONLY soft names only (≠ product)
...
# Wave 92: soft target M=23400; note makefile_max=23400 greppable only when N>=23400.
# Scan is source of truth — never hardcode false 23400 if scan still 23300.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 92 high-water stamp: note 23400 greppable only when N>=23400; 23300 prior tip when N>=23300
