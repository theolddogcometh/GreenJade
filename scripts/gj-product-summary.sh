...
#   high-water toward 19800              Wave 56 soft target (note 19800 greppable only when N>=19800; 19700 prior tip when N>=19700)
#   prior tip makefile_max=19700         may still be scan truth until parent wires 19800
#   (do not hardcode false makefile_max if scan still 19700 until parent wires)
#   soft deepen retgorge/retshoulder      Wave 56 CREATE-ONLY soft names only (≠ product)
...
# Wave 56: soft target M=19800; note makefile_max=19800 greppable only when N>=19800.
# Scan is source of truth — never hardcode false 19800 if scan still 19700.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 56 high-water stamp: note 19800 greppable only when N>=19800; 19700 prior tip
