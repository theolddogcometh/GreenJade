...
#   high-water toward 22000              Wave 78 soft target (note 22000 greppable only when N>=22000; 21900 prior tip when N>=21900)
#   prior tip makefile_max=21900         may still be scan truth until parent wires 22000
#   (do not hardcode false makefile_max if scan still 21900 until parent wires)
#   soft deepen retpalaceangle/retfortressangle      Wave 78 CREATE-ONLY soft names only (≠ product)
...
# Wave 78: soft target M=22000; note makefile_max=22000 greppable only when N>=22000.
# Scan is source of truth — never hardcode false 22000 if scan still 21900.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 78 high-water stamp: note 22000 greppable only when N>=22000; 21900 prior tip
