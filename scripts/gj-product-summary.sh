...
#   high-water toward 19200              Wave 50 soft target (note 19200 greppable only when N>=19200; 19100 prior tip when N>=19100)
#   prior tip makefile_max=19100         may still be scan truth until parent wires 19200
#   (do not hardcode false makefile_max if scan still 19100 until parent wires)
#   soft deepen retsally/retcounterscarp      Wave 50 CREATE-ONLY soft names only (≠ product)
...
# Wave 50: soft target M=19200; note makefile_max=19200 greppable only when N>=19200.
# Scan is source of truth — never hardcode false 19200 if scan still 19100.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 50 high-water stamp: note 19200 greppable only when N>=19200; 19100 prior tip
