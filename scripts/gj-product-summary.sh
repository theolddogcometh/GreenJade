...
#   high-water toward 19900              Wave 57 soft target (note 19900 greppable only when N>=19900; 19800 prior tip when N>=19800)
#   prior tip makefile_max=19800         may still be scan truth until parent wires 19900
#   (do not hardcode false makefile_max if scan still 19800 until parent wires)
#   soft deepen retraverse/retcasemate      Wave 57 CREATE-ONLY soft names only (≠ product)
...
# Wave 57: soft target M=19900; note makefile_max=19900 greppable only when N>=19900.
# Scan is source of truth — never hardcode false 19900 if scan still 19800.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 57 high-water stamp: note 19900 greppable only when N>=19900; 19800 prior tip
