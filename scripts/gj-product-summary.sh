...
#   high-water toward 17800              Wave 36 soft target (note 17800 greppable only when N>=17800; 17700 prior tip when N>=17700)
#   prior tip makefile_max=17700         may still be scan truth until parent wires 17800
#   (do not hardcode false makefile_max if scan still 17700 until parent wires)
#   soft deepen rethold/retspire         Wave 36 CREATE-ONLY soft names only (≠ product)
...
# Wave 36: soft target M=17800; note makefile_max=17800 greppable only when N>=17800.
# Scan is source of truth — never hardcode false 17800 if scan still 17700.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 36 high-water stamp: note 17800 greppable only when N>=17800; 17700 prior tip
