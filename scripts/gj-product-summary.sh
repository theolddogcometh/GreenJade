...
#   high-water toward 16700              Wave 25 soft target (note 16600 greppable only when N>=16600; 16700 only when N>=16700)
#   prior tip makefile_max=16600         may still be scan truth until parent wires 16700
#   (do not hardcode false makefile_max if scan still 16600 until parent wires)
...
# Wave 25: soft target M=16700; note makefile_max=16700 greppable only when N>=16700.
# Scan is source of truth — never hardcode false 16700 if scan still 16600.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 25 high-water stamp: note 16700 greppable only when N>=16700
