...
#   high-water toward 19100              Wave 49 soft target (note 19100 greppable only when N>=19100; 19000 prior tip when N>=19000)
#   prior tip makefile_max=19000         may still be scan truth until parent wires 19100
#   (do not hardcode false makefile_max if scan still 19000 until parent wires)
#   soft deepen retglacisgate/retoutwork      Wave 49 CREATE-ONLY soft names only (≠ product)
...
# Wave 49: soft target M=19100; note makefile_max=19100 greppable only when N>=19100.
# Scan is source of truth — never hardcode false 19100 if scan still 19000.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 49 high-water stamp: note 19100 greppable only when N>=19100; 19000 prior tip
