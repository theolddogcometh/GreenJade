...
#   high-water toward 24300              Wave 101 soft target (note 24300 greppable only when N>=24300; 24200 prior tip when N>=24200)
#   prior tip makefile_max=24200         may still be scan truth until parent wires 24300
#   (do not hardcode false makefile_max if scan still 24200 until parent wires)
#   soft deepen rettorusangle/retknotangle      Wave 101 CREATE-ONLY soft names only (≠ product)
...
# Wave 101: soft target M=24300; note makefile_max=24300 greppable only when N>=24300.
# Scan is source of truth — never hardcode false 24300 if scan still 24200.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 101 high-water stamp: note 24300 greppable only when N>=24300; 24200 prior tip when N>=24200
