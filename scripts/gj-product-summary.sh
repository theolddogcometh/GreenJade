...
#   high-water toward 17900              Wave 37 soft target (note 17900 greppable only when N>=17900; 17800 prior tip when N>=17800)
#   prior tip makefile_max=17800         may still be scan truth until parent wires 17900
#   (do not hardcode false makefile_max if scan still 17800 until parent wires)
#   soft deepen retwall/retgate         Wave 37 CREATE-ONLY soft names only (≠ product)
...
# Wave 37: soft target M=17900; note makefile_max=17900 greppable only when N>=17900.
# Scan is source of truth — never hardcode false 17900 if scan still 17800.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 37 high-water stamp: note 17900 greppable only when N>=17900; 17800 prior tip
