...
#   high-water toward 23000              Wave 88 soft target (note 23000 greppable only when N>=23000; 22900 prior tip when N>=22900)
#   prior tip makefile_max=22900         may still be scan truth until parent wires 23000
#   (do not hardcode false makefile_max if scan still 22900 until parent wires)
#   soft deepen retbeaconangle/retledgerangle      Wave 88 CREATE-ONLY soft names only (≠ product)
...
# Wave 88: soft target M=23000; note makefile_max=23000 greppable only when N>=23000.
# Scan is source of truth — never hardcode false 23000 if scan still 22900.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 88 high-water stamp: note 23000 greppable only when N>=23000; 22900 prior tip when N>=22900
