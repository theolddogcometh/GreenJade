...
#   high-water toward 23100              Wave 89 soft target (note 23100 greppable only when N>=23100; 23000 prior tip when N>=23000)
#   prior tip makefile_max=23000         may still be scan truth until parent wires 23100
#   (do not hardcode false makefile_max if scan still 23000 until parent wires)
#   soft deepen retbannerangle/retvaultangle      Wave 89 CREATE-ONLY soft names only (≠ product)
...
# Wave 89: soft target M=23100; note makefile_max=23100 greppable only when N>=23100.
# Scan is source of truth — never hardcode false 23100 if scan still 23000.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 89 high-water stamp: note 23100 greppable only when N>=23100; 23000 prior tip when N>=23000
