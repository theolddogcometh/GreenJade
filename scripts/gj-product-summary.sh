...
#   high-water toward 18500              Wave 43 soft target (note 18500 greppable only when N>=18500; 18400 prior tip when N>=18400)
#   prior tip makefile_max=18400         may still be scan truth until parent wires 18500
#   (do not hardcode false makefile_max if scan still 18400 until parent wires)
#   soft deepen retmachicolation/retarrowslit      Wave 43 CREATE-ONLY soft names only (≠ product)
...
# Wave 43: soft target M=18500; note makefile_max=18500 greppable only when N>=18500.
# Scan is source of truth — never hardcode false 18500 if scan still 18400.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 43 high-water stamp: note 18500 greppable only when N>=18500; 18400 prior tip
