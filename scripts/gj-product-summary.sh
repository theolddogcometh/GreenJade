...
#   high-water toward 20700              Wave 65 soft target (note 20700 greppable only when N>=20700; 20600 prior tip when N>=20600)
#   prior tip makefile_max=20600         may still be scan truth until parent wires 20700
#   (do not hardcode false makefile_max if scan still 20600 until parent wires)
#   soft deepen retgorgeangle/retshoulderangle      Wave 65 CREATE-ONLY soft names only (≠ product)
...
# Wave 65: soft target M=20700; note makefile_max=20700 greppable only when N>=20700.
# Scan is source of truth — never hardcode false 20700 if scan still 20600.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 65 high-water stamp: note 20700 greppable only when N>=20700; 20600 prior tip
