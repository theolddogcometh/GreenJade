...
#   high-water toward 22600              Wave 84 soft target (note 22600 greppable only when N>=22600; 22500 prior tip when N>=22500)
#   prior tip makefile_max=22500         may still be scan truth until parent wires 22600
#   (do not hardcode false makefile_max if scan still 22500 until parent wires)
#   soft deepen retscepterangle/retglyphangle      Wave 84 CREATE-ONLY soft names only (≠ product)
...
# Wave 84: soft target M=22600; note makefile_max=22600 greppable only when N>=22600.
# Scan is source of truth — never hardcode false 22600 if scan still 22500.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 84 high-water stamp: note 22600 greppable only when N>=22600; 22500 prior tip
