...
#   high-water toward 24800              Wave 106 soft target (note 24800 greppable only when N>=24800; 24700 prior tip when N>=24700)
#   prior tip makefile_max=24700         may still be scan truth until parent wires 24800
#   (do not hardcode false makefile_max if scan still prior tip until parent wires)
#   soft deepen retquarticangle/retquinticangle      Wave 106 CREATE-ONLY soft names only (≠ product)
...
# Wave 106: soft target M=24800; note makefile_max=24800 greppable only when N>=24800.
# Scan is source of truth — never hardcode false 24800 if scan still prior tip (24700).
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 106 high-water stamp: note 24800 greppable only when N>=24800; 24700 prior tip when N>=24700
