...
#   high-water toward 22900              Wave 87 soft target (note 22900 greppable only when N>=22900; 22800 prior tip when N>=22800)
#   prior tip makefile_max=22800         may still be scan truth until parent wires 22900
#   (do not hardcode false makefile_max if scan still 22800 until parent wires)
#   soft deepen retflameangle/retcipherangle      Wave 87 CREATE-ONLY soft names only (≠ product)
...
# Wave 87: soft target M=22900; note makefile_max=22900 greppable only when N>=22900.
# Scan is source of truth — never hardcode false 22900 if scan still 22800.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 87 high-water stamp: note 22900 greppable only when N>=22900; 22800 prior tip when N>=22800
