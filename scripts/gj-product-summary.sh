...
#   high-water toward 19700              Wave 55 soft target (note 19700 greppable only when N>=19700; 19600 prior tip when N>=19600)
#   prior tip makefile_max=19600         may still be scan truth until parent wires 19700
#   (do not hardcode false makefile_max if scan still 19600 until parent wires)
#   soft deepen retflank/retface      Wave 55 CREATE-ONLY soft names only (≠ product)
...
# Wave 55: soft target M=19700; note makefile_max=19700 greppable only when N>=19700.
# Scan is source of truth — never hardcode false 19700 if scan still 19600.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 55 high-water stamp: note 19700 greppable only when N>=19700; 19600 prior tip
