...
#   high-water toward 16500              Wave 23 soft target (note 16400 greppable only when N>=16400; 16500 only when N>=16500)
#   prior tip makefile_max=16400         may still be scan truth until parent wires 16500
#   (do not hardcode false makefile_max if scan still 16400 until parent wires)
...
# Wave 23: soft target M=16500; note makefile_max=16500 greppable only when N>=16500.
# Scan is source of truth — never hardcode false 16500 if scan still 16400.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 23 high-water stamp: note 16500 greppable only when N>=16500
