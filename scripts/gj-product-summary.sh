...
#   high-water toward 20300              Wave 61 soft target (note 20300 greppable only when N>=20300; 20200 prior tip when N>=20200)
#   prior tip makefile_max=20200         may still be scan truth until parent wires 20300
#   (do not hardcode false makefile_max if scan still 20200 until parent wires)
#   soft deepen retcounterguard/retcoveredface      Wave 61 CREATE-ONLY soft names only (≠ product)
...
# Wave 61: soft target M=20300; note makefile_max=20300 greppable only when N>=20300.
# Scan is source of truth — never hardcode false 20300 if scan still 20200.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 61 high-water stamp: note 20300 greppable only when N>=20300; 20200 prior tip
