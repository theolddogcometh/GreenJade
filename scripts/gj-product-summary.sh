...
#   high-water toward 22400              Wave 82 soft target (note 22400 greppable only when N>=22400; 22300 prior tip when N>=22300)
#   prior tip makefile_max=22300         may still be scan truth until parent wires 22400
#   (do not hardcode false makefile_max if scan still 22300 until parent wires)
#   soft deepen retmantleangle/retaegisangle      Wave 82 CREATE-ONLY soft names only (≠ product)
...
# Wave 82: soft target M=22400; note makefile_max=22400 greppable only when N>=22400.
# Scan is source of truth — never hardcode false 22400 if scan still 22300.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 82 high-water stamp: note 22400 greppable only when N>=22400; 22300 prior tip
