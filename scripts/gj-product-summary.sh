...
#   high-water toward 19300              Wave 51 soft target (note 19300 greppable only when N>=19300; 19200 prior tip when N>=19200)
#   prior tip makefile_max=19200         may still be scan truth until parent wires 19300
#   (do not hardcode false makefile_max if scan still 19200 until parent wires)
#   soft deepen retfosse/retcoveredway      Wave 51 CREATE-ONLY soft names only (≠ product)
...
# Wave 51: soft target M=19300; note makefile_max=19300 greppable only when N>=19300.
# Scan is source of truth — never hardcode false 19300 if scan still 19200.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 51 high-water stamp: note 19300 greppable only when N>=19300; 19200 prior tip
