...
#   high-water toward 20200              Wave 60 soft target (note 20200 greppable only when N>=20200; 20100 prior tip when N>=20100)
#   prior tip makefile_max=20100         may still be scan truth until parent wires 20200
#   (do not hardcode false makefile_max if scan still 20100 until parent wires)
#   soft deepen retplace/retenvelope      Wave 60 CREATE-ONLY soft names only (≠ product)
...
# Wave 60: soft target M=20200; note makefile_max=20200 greppable only when N>=20200.
# Scan is source of truth — never hardcode false 20200 if scan still 20100.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 60 high-water stamp: note 20200 greppable only when N>=20200; 20100 prior tip
