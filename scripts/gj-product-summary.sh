...
#   high-water toward 17600              Wave 34 soft target (note 17600 greppable only when N>=17600; 17500 prior tip when N>=17500)
#   prior tip makefile_max=17500         may still be scan truth until parent wires 17600
#   (do not hardcode false makefile_max if scan still 17500 until parent wires)
...
# Wave 34: soft target M=17600; note makefile_max=17600 greppable only when N>=17600.
# Scan is source of truth — never hardcode false 17600 if scan still 17500.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 34 high-water stamp: note 17600 greppable only when N>=17600; 17500 prior tip
