...
#   high-water toward 21700              Wave 75 soft target (note 21700 greppable only when N>=21700; 21600 prior tip when N>=21600)
#   prior tip makefile_max=21600         may still be scan truth until parent wires 21700
#   (do not hardcode false makefile_max if scan still 21600 until parent wires)
#   soft deepen retmoatangle/retowerangle      Wave 75 CREATE-ONLY soft names only (≠ product)
...
# Wave 75: soft target M=21700; note makefile_max=21700 greppable only when N>=21700.
# Scan is source of truth — never hardcode false 21700 if scan still 21600.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 75 high-water stamp: note 21700 greppable only when N>=21700; 21600 prior tip
