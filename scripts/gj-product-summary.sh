...
#   high-water toward 17700              Wave 35 soft target (note 17700 greppable only when N>=17700; 17600 prior tip when N>=17600)
#   prior tip makefile_max=17600         may still be scan truth until parent wires 17700
#   (do not hardcode false makefile_max if scan still 17600 until parent wires)
#   soft deepen retfortress/retpalace    Wave 35 CREATE-ONLY soft names only (≠ product)
...
# Wave 35: soft target M=17700; note makefile_max=17700 greppable only when N>=17700.
# Scan is source of truth — never hardcode false 17700 if scan still 17600.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 35 high-water stamp: note 17700 greppable only when N>=17700; 17600 prior tip
