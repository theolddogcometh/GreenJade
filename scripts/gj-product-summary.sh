...
#   high-water toward 20000              Wave 58 soft target (note 20000 greppable only when N>=20000; 19900 prior tip when N>=19900)
#   prior tip makefile_max=19900         may still be scan truth until parent wires 20000
#   (do not hardcode false makefile_max if scan still 19900 until parent wires)
#   soft deepen retorillon/retbonnette      Wave 58 CREATE-ONLY soft names only (≠ product)
...
# Wave 58: soft target M=20000; note makefile_max=20000 greppable only when N>=20000.
# Scan is source of truth — never hardcode false 20000 if scan still 19900.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 58 high-water stamp: note 20000 greppable only when N>=20000; 19900 prior tip
