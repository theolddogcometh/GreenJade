...
#   high-water toward 25700              Wave 115 soft target (note 25700 greppable only when N>=25700; 25600 prior tip when N>=25600)
#   prior tip makefile_max=25600         may still be scan truth until parent wires 25700
#   (do not hardcode false makefile_max if scan still prior tip until parent wires)
#   soft deepen retpresentangle/retvsyncangle  Wave 115 CREATE-ONLY soft names only (≠ product)
...
# Wave 115: soft target M=25700; note makefile_max=25700 greppable only when N>=25700.
# Scan is source of truth — never hardcode false 25700 if scan still prior tip (25600).
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 115 high-water stamp: note 25700 greppable only when N>=25700; 25600 prior tip when N>=25600
