...
#   high-water toward 22700              Wave 85 soft target (note 22700 greppable only when N>=22700; 22600 prior tip when N>=22600)
#   prior tip makefile_max=22600         may still be scan truth until parent wires 22700
#   (do not hardcode false makefile_max if scan still 22600 until parent wires)
#   soft deepen retcrownangle/retshardangle      Wave 85 CREATE-ONLY soft names only (≠ product)
...
# Wave 85: soft target M=22700; note makefile_max=22700 greppable only when N>=22700.
# Scan is source of truth — never hardcode false 22700 if scan still 22600.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 85 high-water stamp: note 22700 greppable only when N>=22700; 22600 prior tip
