...
#   high-water toward 22800              Wave 86 soft target (note 22800 greppable only when N>=22800; 22700 prior tip when N>=22700)
#   prior tip makefile_max=22700         may still be scan truth until parent wires 22800
#   (do not hardcode false makefile_max if scan still 22700 until parent wires)
#   soft deepen retforgeangle/retprismangle      Wave 86 CREATE-ONLY soft names only (≠ product)
...
# Wave 86: soft target M=22800; note makefile_max=22800 greppable only when N>=22800.
# Scan is source of truth — never hardcode false 22800 if scan still 22700.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 86 high-water stamp: note 22800 greppable only when N>=22800; 22700 prior tip
