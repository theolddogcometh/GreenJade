...
#   high-water toward 18200              Wave 40 soft target (note 18200 greppable only when N>=18200; 18100 prior tip when N>=18100)
#   prior tip makefile_max=18100         may still be scan truth until parent wires 18200
#   (do not hardcode false makefile_max if scan still 18100 until parent wires)
#   soft deepen retcurtain/retparapet    Wave 40 CREATE-ONLY soft names only (≠ product)
...
# Wave 40: soft target M=18200; note makefile_max=18200 greppable only when N>=18200.
# Scan is source of truth — never hardcode false 18200 if scan still 18100.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 40 high-water stamp: note 18200 greppable only when N>=18200; 18100 prior tip
