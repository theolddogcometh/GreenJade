...
#   high-water toward 18900              Wave 47 soft target (note 18900 greppable only when N>=18900; 18800 prior tip when N>=18800)
#   prior tip makefile_max=18800         may still be scan truth until parent wires 18900
#   (do not hardcode false makefile_max if scan still 18800 until parent wires)
#   soft deepen retinnerward/retdonjon      Wave 47 CREATE-ONLY soft names only (≠ product)
...
# Wave 47: soft target M=18900; note makefile_max=18900 greppable only when N>=18900.
# Scan is source of truth — never hardcode false 18900 if scan still 18800.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 47 high-water stamp: note 18900 greppable only when N>=18900; 18800 prior tip
