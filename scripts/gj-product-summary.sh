...
#   high-water toward 22500              Wave 83 soft target (note 22500 greppable only when N>=22500; 22400 prior tip when N>=22400)
#   prior tip makefile_max=22400         may still be scan truth until parent wires 22500
#   (do not hardcode false makefile_max if scan still 22400 until parent wires)
#   soft deepen retemblemangle/retsigilangle      Wave 83 CREATE-ONLY soft names only (≠ product)
...
# Wave 83: soft target M=22500; note makefile_max=22500 greppable only when N>=22500.
# Scan is source of truth — never hardcode false 22500 if scan still 22400.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 83 high-water stamp: note 22500 greppable only when N>=22500; 22400 prior tip
