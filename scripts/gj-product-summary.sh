...
#   high-water toward 23200              Wave 90 soft target (note 23200 greppable only when N>=23200; 23100 prior tip when N>=23100)
#   prior tip makefile_max=23100         may still be scan truth until parent wires 23200
#   (do not hardcode false makefile_max if scan still 23100 until parent wires)
#   soft deepen retcrestangle/rettokenangle      Wave 90 CREATE-ONLY soft names only (≠ product)
...
# Wave 90: soft target M=23200; note makefile_max=23200 greppable only when N>=23200.
# Scan is source of truth — never hardcode false 23200 if scan still 23100.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 90 high-water stamp: note 23200 greppable only when N>=23200; 23100 prior tip when N>=23100
