...
#   high-water toward 26700              Wave 125 soft target (note 26700 greppable only when N>=26700; 26600 prior tip when N>=26600)
#   prior tip makefile_max=26600         may still be scan truth until parent wires 26700
#   (do not hardcode false makefile_max if scan still prior tip until parent wires)
#   soft deepen retpaletteangle/retstrokeangle  Wave 125 CREATE-ONLY soft names only (≠ product)
...
# Wave 125: soft target M=26700; note makefile_max=26700 greppable only when N>=26700.
# Scan is source of truth — never hardcode false 26700 if scan still prior tip (26600).
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 125 high-water stamp: note 26700 greppable only when N>=26700; 26600 prior tip when N>=26600
