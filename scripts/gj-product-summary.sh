...
#   high-water toward 26600              Wave 124 soft target (note 26600 greppable only when N>=26600; 26500 prior tip when N>=26500)
#   prior tip makefile_max=26500         may still be scan truth until parent wires 26600
#   (do not hardcode false makefile_max if scan still prior tip until parent wires)
#   soft deepen retbrushangle/retinkangle  Wave 124 CREATE-ONLY soft names only (≠ product)
...
# Wave 124: soft target M=26600; note makefile_max=26600 greppable only when N>=26600.
# Scan is source of truth — never hardcode false 26600 if scan still prior tip (26500).
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 124 high-water stamp: note 26600 greppable only when N>=26600; 26500 prior tip when N>=26500
