...
#   high-water toward 20500              Wave 63 soft target (note 20500 greppable only when N>=20500; 20400 prior tip when N>=20400)
#   prior tip makefile_max=20400         may still be scan truth until parent wires 20500
#   (do not hardcode false makefile_max if scan still 20400 until parent wires)
#   soft deepen retdoubletenaille/retplaceofarms      Wave 63 CREATE-ONLY soft names only (≠ product)
...
# Wave 63: soft target M=20500; note makefile_max=20500 greppable only when N>=20500.
# Scan is source of truth — never hardcode false 20500 if scan still 20400.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 63 high-water stamp: note 20500 greppable only when N>=20500; 20400 prior tip
