...
#   high-water toward 22300              Wave 81 soft target (note 22300 greppable only when N>=22300; 22200 prior tip when N>=22200)
#   prior tip makefile_max=22200         may still be scan truth until parent wires 22300
#   (do not hardcode false makefile_max if scan still 22200 until parent wires)
#   soft deepen retpanoplyangle/retbulwarkangle      Wave 81 CREATE-ONLY soft names only (≠ product)
...
# Wave 81: soft target M=22300; note makefile_max=22300 greppable only when N>=22300.
# Scan is source of truth — never hardcode false 22300 if scan still 22200.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 81 high-water stamp: note 22300 greppable only when N>=22300; 22200 prior tip
