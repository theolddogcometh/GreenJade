...
#   high-water toward 18400              Wave 42 soft target (note 18400 greppable only when N>=18400; 18300 prior tip when N>=18300)
#   prior tip makefile_max=18300         may still be scan truth until parent wires 18400
#   (do not hardcode false makefile_max if scan still 18300 until parent wires)
#   soft deepen retportcullis/retbattlement      Wave 42 CREATE-ONLY soft names only (≠ product)
...
# Wave 42: soft target M=18400; note makefile_max=18400 greppable only when N>=18400.
# Scan is source of truth — never hardcode false 18400 if scan still 18300.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 42 high-water stamp: note 18400 greppable only when N>=18400; 18300 prior tip
