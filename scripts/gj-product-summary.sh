...
#   high-water toward 19000              Wave 48 soft target (note 19000 greppable only when N>=19000; 18900 prior tip when N>=18900)
#   prior tip makefile_max=18900         may still be scan truth until parent wires 19000
#   (do not hardcode false makefile_max if scan still 18900 until parent wires)
#   soft deepen retchevaux/retpalisade      Wave 48 CREATE-ONLY soft names only (≠ product)
...
# Wave 48: soft target M=19000; note makefile_max=19000 greppable only when N>=19000.
# Scan is source of truth — never hardcode false 19000 if scan still 18900.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 48 high-water stamp: note 19000 greppable only when N>=19000; 18900 prior tip
