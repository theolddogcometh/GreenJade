...
#   high-water toward 24900              Wave 107 soft target (note 24900 greppable only when N>=24900; 24800 prior tip when N>=24800)
#   prior tip makefile_max=24800         may still be scan truth until parent wires 24900
#   (do not hardcode false makefile_max if scan still prior tip until parent wires)
#   soft deepen retsplineangle/retbezierangle      Wave 107 CREATE-ONLY soft names only (≠ product)
...
# Wave 107: soft target M=24900; note makefile_max=24900 greppable only when N>=24900.
# Scan is source of truth — never hardcode false 24900 if scan still prior tip (24800).
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 107 high-water stamp: note 24900 greppable only when N>=24900; 24800 prior tip when N>=24800
