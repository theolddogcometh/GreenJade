...
#   high-water toward 20600              Wave 64 soft target (note 20600 greppable only when N>=20600; 20500 prior tip when N>=20500)
#   prior tip makefile_max=20500         may still be scan truth until parent wires 20600
#   (do not hardcode false makefile_max if scan still 20500 until parent wires)
#   soft deepen retreentrant/retsallyport      Wave 64 CREATE-ONLY soft names only (≠ product)
...
# Wave 64: soft target M=20600; note makefile_max=20600 greppable only when N>=20600.
# Scan is source of truth — never hardcode false 20600 if scan still 20500.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 64 high-water stamp: note 20600 greppable only when N>=20600; 20500 prior tip
