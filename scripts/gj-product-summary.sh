...
#   high-water toward 19600              Wave 54 soft target (note 19600 greppable only when N>=19600; 19500 prior tip when N>=19500)
#   prior tip makefile_max=19500         may still be scan truth until parent wires 19600
#   (do not hardcode false makefile_max if scan still 19500 until parent wires)
#   soft deepen retcaponier/retredan      Wave 54 CREATE-ONLY soft names only (≠ product)
...
# Wave 54: soft target M=19600; note makefile_max=19600 greppable only when N>=19600.
# Scan is source of truth — never hardcode false 19600 if scan still 19500.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 54 high-water stamp: note 19600 greppable only when N>=19600; 19500 prior tip
