...
#   high-water toward 21300              Wave 71 soft target (note 21300 greppable only when N>=21300; 21200 prior tip when N>=21200)
#   prior tip makefile_max=21200         may still be scan truth until parent wires 21300
#   (do not hardcode false makefile_max if scan still 21200 until parent wires)
#   soft deepen retsallyportangle/retreentrantangle      Wave 71 CREATE-ONLY soft names only (≠ product)
...
# Wave 71: soft target M=21300; note makefile_max=21300 greppable only when N>=21300.
# Scan is source of truth — never hardcode false 21300 if scan still 21200.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 71 high-water stamp: note 21300 greppable only when N>=21300; 21200 prior tip
