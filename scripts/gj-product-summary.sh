...
#   high-water toward 16800              Wave 26 soft target (note 16700 greppable only when N>=16700; 16800 only when N>=16800)
#   prior tip makefile_max=16700         may still be scan truth until parent wires 16800
#   (do not hardcode false makefile_max if scan still 16700 until parent wires)
...
# Wave 26: soft target M=16800; note makefile_max=16800 greppable only when N>=16800.
# Scan is source of truth — never hardcode false 16800 if scan still 16700.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 26 high-water stamp: note 16800 greppable only when N>=16800
