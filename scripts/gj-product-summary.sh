...
#   high-water toward 20900              Wave 67 soft target (note 20900 greppable only when N>=20900; 20800 prior tip when N>=20800)
#   prior tip makefile_max=20800         may still be scan truth until parent wires 20900
#   (do not hardcode false makefile_max if scan still 20800 until parent wires)
#   soft deepen retcaponierangle/retredanangle      Wave 67 CREATE-ONLY soft names only (≠ product)
...
# Wave 67: soft target M=20900; note makefile_max=20900 greppable only when N>=20900.
# Scan is source of truth — never hardcode false 20900 if scan still 20800.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 67 high-water stamp: note 20900 greppable only when N>=20900; 20800 prior tip
