...
#   high-water toward 20400              Wave 62 soft target (note 20400 greppable only when N>=20400; 20300 prior tip when N>=20300)
#   prior tip makefile_max=20300         may still be scan truth until parent wires 20400
#   (do not hardcode false makefile_max if scan still 20300 until parent wires)
#   soft deepen retbastionface/retcurtainangle      Wave 62 CREATE-ONLY soft names only (≠ product)
...
# Wave 62: soft target M=20400; note makefile_max=20400 greppable only when N>=20400.
# Scan is source of truth — never hardcode false 20400 if scan still 20300.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 62 high-water stamp: note 20400 greppable only when N>=20400; 20300 prior tip
