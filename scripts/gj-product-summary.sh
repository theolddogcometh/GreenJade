...
#   high-water toward 20800              Wave 66 soft target (note 20800 greppable only when N>=20800; 20700 prior tip when N>=20700)
#   prior tip makefile_max=20700         may still be scan truth until parent wires 20800
#   (do not hardcode false makefile_max if scan still 20700 until parent wires)
#   soft deepen retflankangle/retfaceangle      Wave 66 CREATE-ONLY soft names only (≠ product)
...
# Wave 66: soft target M=20800; note makefile_max=20800 greppable only when N>=20800.
# Scan is source of truth — never hardcode false 20800 if scan still 20700.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 66 high-water stamp: note 20800 greppable only when N>=20800; 20700 prior tip
