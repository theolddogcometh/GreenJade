...
#   high-water toward 16600              Wave 24 soft target (note 16500 greppable only when N>=16500; 16600 only when N>=16600)
#   prior tip makefile_max=16500         may still be scan truth until parent wires 16600
...
# Wave 24: soft target M=16600; note makefile_max=16600 greppable only when N>=16600.
# Scan is source of truth — never hardcode false 16600 if scan still 16500.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 24 high-water stamp: note 16600 greppable only when N>=16600
