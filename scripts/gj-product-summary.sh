...
#   high-water toward 18300              Wave 41 soft target (note 18300 greppable only when N>=18300; 18200 prior tip when N>=18200)
#   prior tip makefile_max=18200         may still be scan truth until parent wires 18300
#   (do not hardcode false makefile_max if scan still 18200 until parent wires)
#   soft deepen retravelin/retditch      Wave 41 CREATE-ONLY soft names only (≠ product)
...
# Wave 41: soft target M=18300; note makefile_max=18300 greppable only when N>=18300.
# Scan is source of truth — never hardcode false 18300 if scan still 18200.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 41 high-water stamp: note 18300 greppable only when N>=18300; 18200 prior tip
