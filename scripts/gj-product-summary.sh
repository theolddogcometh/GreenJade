...
#   high-water toward 24200              Wave 100 soft target (note 24100 greppable only when N>=24200; 24000 prior tip when N>=24100)
#   prior tip makefile_max=24100         may still be scan truth until parent wires 24200
#   (do not hardcode false makefile_max if scan still 24100 until parent wires)
#   soft deepen retspiralangle/rethelixangle      Wave 100 CREATE-ONLY soft names only (≠ product)
...
# Wave 100: soft target M=24200; note makefile_max=24200 greppable only when N>=24200.
# Scan is source of truth — never hardcode false 24200 if scan still 24100.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 100 high-water stamp: note 24100 greppable only when N>=24200; 24000 prior tip when N>=24100
