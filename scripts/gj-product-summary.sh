...
#   high-water toward 19400              Wave 52 soft target (note 19400 greppable only when N>=19400; 19300 prior tip when N>=19300)
#   prior tip makefile_max=19300         may still be scan truth until parent wires 19400
#   (do not hardcode false makefile_max if scan still 19300 until parent wires)
#   soft deepen rettenaille/retdemilune      Wave 52 CREATE-ONLY soft names only (≠ product)
...
# Wave 52: soft target M=19400; note makefile_max=19400 greppable only when N>=19400.
# Scan is source of truth — never hardcode false 19400 if scan still 19300.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 52 high-water stamp: note 19400 greppable only when N>=19400; 19300 prior tip
