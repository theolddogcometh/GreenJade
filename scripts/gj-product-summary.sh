...
#   high-water toward 20100              Wave 59 soft target (note 20100 greppable only when N>=20100; 20000 prior tip when N>=20000)
#   prior tip makefile_max=20000         may still be scan truth until parent wires 20100
#   (do not hardcode false makefile_max if scan still 20000 until parent wires)
#   soft deepen retcrownwork/rethornwork      Wave 59 CREATE-ONLY soft names only (≠ product)
...
# Wave 59: soft target M=20100; note makefile_max=20100 greppable only when N>=20100.
# Scan is source of truth — never hardcode false 20100 if scan still 20000.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
	...
	if [[ -n "$_mx_n" ]]; then
		...
		# Wave 59 high-water stamp: note 20100 greppable only when N>=20100; 20000 prior tip
