...
# Soft companions (Wave 61 exclusive — info only, never hard-fail):
#   continuum high-water toward 20300; makefile_max=20300 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 20300 if scan still 20200; 20200 prior tip when N>=20200)
#   soft deepen retcounterguard/retcoveredface (CREATE-ONLY soft names only)
...
# --- Wave 61 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 20300; greppable 20300 only when N>=20300; 20200 prior tip when N>=20200.
# Scan is source of truth — do not hardcode false makefile_max if still 20200.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 61) ---"
info_check "makefile_max=20300 (log)" 'makefile_max=20300'
info_check "makefile_max=20200 (log)" 'makefile_max=20200'
info_check "ubar3open / bar3=0 (log)" 'ubar3open|bar3=0|bar3:[[:space:]]*OPEN|bar3 OPEN'
# Host continuum scan: stamp makefile_max=20300 greppable only when N>=20300.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 20300 ]]; then
      echo "  info: continuum high-water  makefile_max=20300 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 20200 ]]; then
      echo "  info: continuum high-water  makefile_max=20200 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=20300  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=20300  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 61  target decade M=20300 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retcounterguard/retcoveredface (CREATE-ONLY soft ≠ product)"
else
