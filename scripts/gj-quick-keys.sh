...
# Soft companions (Wave 95 exclusive — info only, never hard-fail):
#   continuum high-water toward 23700; makefile_max=23700 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode false 23700 if scan still 23600; 23500 prior tip when N>=23600)
#   soft deepen retchordangle/retarcangle (CREATE-ONLY soft names only)
...
# --- Wave 95 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 23700; greppable 23700 only when N>=23700; 23500 prior tip when N>=23600.
# Scan is source of truth — do not hardcode false makefile_max if still 23600.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 95) ---"
info_check "makefile_max=23700 (log)" 'makefile_max=23700'
info_check "makefile_max=23600 (log)" 'makefile_max=23600'
info_check "ubar3open / bar3=0 (log)" 'ubar3open|bar3=0|bar3:[[:space:]]*OPEN|bar3 OPEN'
# Host continuum scan: stamp makefile_max=23700 greppable only when N>=23700.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 23700 ]]; then
      echo "  info: continuum high-water  makefile_max=23700 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 23600 ]]; then
      echo "  info: continuum high-water  makefile_max=23600 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=23700  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=23700  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 95  target decade M=23700 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retchordangle/retarcangle (CREATE-ONLY soft ≠ product)"
else
