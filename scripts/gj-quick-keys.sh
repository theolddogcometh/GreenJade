...
# Soft companions (Wave 55 exclusive — info only, never hard-fail):
#   continuum high-water toward 19700; makefile_max=19700 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 19700 if scan still 19600; 19600 prior tip when N>=19600)
#   soft deepen retflank/retface (CREATE-ONLY soft names only)
...
# --- Wave 55 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 19700; greppable 19700 only when N>=19700; 19600 prior tip when N>=19600.
# Scan is source of truth — do not hardcode false makefile_max if still 19600.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 55) ---"
...
# Host continuum scan: stamp makefile_max=19700 greppable only when N>=19700.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 19700 ]]; then
      echo "  info: continuum high-water  makefile_max=19700 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 19600 ]]; then
      echo "  info: continuum high-water  makefile_max=19600 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=19700  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=19700  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 55  target decade M=19700 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retflank/retface (CREATE-ONLY soft ≠ product)"
else
