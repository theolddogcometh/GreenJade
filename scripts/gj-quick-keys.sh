...
# Soft companions (Wave 56 exclusive — info only, never hard-fail):
#   continuum high-water toward 19800; makefile_max=19800 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 19800 if scan still 19700; 19700 prior tip when N>=19700)
#   soft deepen retgorge/retshoulder (CREATE-ONLY soft names only)
...
# --- Wave 56 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 19800; greppable 19800 only when N>=19800; 19700 prior tip when N>=19700.
# Scan is source of truth — do not hardcode false makefile_max if still 19700.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 56) ---"
info_check "makefile_max=19800 (log)" 'makefile_max=19800'
info_check "makefile_max=19700 (log)" 'makefile_max=19700'
info_check "ubar3open / bar3=0 (log)" 'ubar3open|bar3=0|bar3:[[:space:]]*OPEN|bar3 OPEN'
# Host continuum scan: stamp makefile_max=19800 greppable only when N>=19800.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 19800 ]]; then
      echo "  info: continuum high-water  makefile_max=19800 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 19700 ]]; then
      echo "  info: continuum high-water  makefile_max=19700 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=19800  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=19800  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 56  target decade M=19800 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retgorge/retshoulder (CREATE-ONLY soft ≠ product)"
else
