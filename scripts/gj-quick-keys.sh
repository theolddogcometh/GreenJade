...
# Soft companions (Wave 37 exclusive — info only, never hard-fail):
#   continuum high-water toward 17900; makefile_max=17900 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 17900 if scan still 17800; 17900 only when N>=17900)
#   soft deepen retwall/retgate (CREATE-ONLY soft names only)
...
# --- Wave 37 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 17900; greppable 17900 only when N>=17900; 17800 prior tip when N>=17800.
# Scan is source of truth — do not hardcode false makefile_max if still 17800.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 37) ---"
info_check "makefile_max=17900 (log)" 'makefile_max=17900'
...
# Host continuum scan: stamp makefile_max=17900 greppable only when N>=17900.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 17900 ]]; then
      echo "  info: continuum high-water  makefile_max=17900 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 17800 ]]; then
      echo "  info: continuum high-water  makefile_max=17800 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=17900  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=17900  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 37  target decade M=17900 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retwall/retgate (CREATE-ONLY soft ≠ product)"
else
