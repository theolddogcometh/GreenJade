...
# Soft companions (Wave 48 exclusive — info only, never hard-fail):
#   continuum high-water toward 19000; makefile_max=19000 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 19000 if scan still 18900; 18900 prior tip when N>=18900)
#   soft deepen retchevaux/retpalisade (CREATE-ONLY soft names only)
...
# --- Wave 48 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 19000; greppable 19000 only when N>=19000; 18900 prior tip when N>=18900.
# Scan is source of truth — do not hardcode false makefile_max if still 18900.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 48) ---"
...
# Host continuum scan: stamp makefile_max=19000 greppable only when N>=19000.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 19000 ]]; then
      echo "  info: continuum high-water  makefile_max=19000 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 18900 ]]; then
      echo "  info: continuum high-water  makefile_max=18900 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=19000  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=19000  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 48  target decade M=19000 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retchevaux/retpalisade (CREATE-ONLY soft ≠ product)"
else
