...
# Soft companions (Wave 89 exclusive — info only, never hard-fail):
#   continuum high-water toward 23100; makefile_max=23100 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 23100 if scan still 23000; 23000 prior tip when N>=23000)
#   soft deepen retbannerangle/retvaultangle (CREATE-ONLY soft names only)
...
# --- Wave 89 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 23100; greppable 23100 only when N>=23100; 23000 prior tip when N>=23000.
# Scan is source of truth — do not hardcode false makefile_max if still 23000.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 89) ---"
info_check "makefile_max=23100 (log)" 'makefile_max=23100'
...
# Host continuum scan: stamp makefile_max=23100 greppable only when N>=23100.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 23100 ]]; then
      echo "  info: continuum high-water  makefile_max=23100 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 23000 ]]; then
      echo "  info: continuum high-water  makefile_max=23000 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=23100  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=23100  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 89  target decade M=23100 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retbannerangle/retvaultangle (CREATE-ONLY soft ≠ product)"
else
