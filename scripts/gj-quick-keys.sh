...
# Soft companions (Wave 18 exclusive — info only, never hard-fail):
#   continuum high-water toward 16000; makefile_max=15900 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 15900 if scan still 15900)
...
# --- Wave 18 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 16000; greppable 16000 only when N>=16000.
# Scan is source of truth — do not hardcode false makefile_max if still 15900.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 18) ---"
info_check "makefile_max=16000 (log)" 'makefile_max=16000'
info_check "makefile_max=15900 (log)" 'makefile_max=15900'
info_check "ubar3open / bar3=0 (log)" 'ubar3open|bar3=0|bar3:[[:space:]]*OPEN|bar3 OPEN'
# Host continuum scan: stamp makefile_max=16000 greppable only when N>=16000.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 16000 ]]; then
      echo "  info: continuum high-water  makefile_max=16000 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 15900 ]]; then
      echo "  info: continuum high-water  makefile_max=15900 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=16000  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=16000  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 18  target decade M=16000 (CREATE-ONLY soft; parent paste wires)"
else
