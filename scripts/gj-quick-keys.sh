...
# Soft companions (Wave 31 exclusive — info only, never hard-fail):
#   continuum high-water toward 17300; makefile_max=17200 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 17300 if scan still 17200)
...
# --- Wave 31 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 17300; greppable 17200 only when N>=17200; 17300 only when N>=17300.
# Scan is source of truth — do not hardcode false makefile_max if still 17200.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 31) ---"
info_check "makefile_max=17200 (log)" 'makefile_max=17200'
...
# Host continuum scan: stamp makefile_max=17300 greppable only when N>=17300.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 17300 ]]; then
      echo "  info: continuum high-water  makefile_max=17300 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 17200 ]]; then
      echo "  info: continuum high-water  makefile_max=17200 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=17300  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=17300  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 31  target decade M=17300 (CREATE-ONLY soft; parent paste wires)"
else
