...
# Soft companions (Wave 38 exclusive — info only, never hard-fail):
#   continuum high-water toward 18000; makefile_max=18000 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 17900 if scan still 17900; 17900 only when N>=18000)
#   soft deepen retmoat/retower (CREATE-ONLY soft names only)
...
# --- Wave 38 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 18000; greppable 18000 only when N>=18000; 17900 prior tip when N>=17900.
# Scan is source of truth — do not hardcode false makefile_max if still 17900.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 38) ---"
...
# Host continuum scan: stamp makefile_max=18000 greppable only when N>=18000.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 18000 ]]; then
      echo "  info: continuum high-water  makefile_max=18000 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 17900 ]]; then
      echo "  info: continuum high-water  makefile_max=17900 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=18000  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=18000  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 38  target decade M=18000 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retmoat/retower (CREATE-ONLY soft ≠ product)"
else
