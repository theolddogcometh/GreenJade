...
# Soft companions (Wave 76 exclusive — info only, never hard-fail):
#   continuum high-water toward 21800; makefile_max=21800 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 21800 if scan still 21700; 21700 prior tip when N>=21700)
#   soft deepen retgateangle/retwallangle (CREATE-ONLY soft names only)
...
# --- Wave 76 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 21800; greppable 21800 only when N>=21800; 21700 prior tip when N>=21700.
# Scan is source of truth — do not hardcode false makefile_max if still 21700.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 76) ---"
...
# Host continuum scan: stamp makefile_max=21800 greppable only when N>=21800.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 21800 ]]; then
      echo "  info: continuum high-water  makefile_max=21800 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 21700 ]]; then
      echo "  info: continuum high-water  makefile_max=21700 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=21800  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=21800  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 76  target decade M=21800 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retgateangle/retwallangle (CREATE-ONLY soft ≠ product)"
else
