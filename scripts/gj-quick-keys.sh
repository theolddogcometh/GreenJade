...
# Soft companions (Wave 24 exclusive — info only, never hard-fail):
#   continuum high-water toward 16600; makefile_max=16500 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 16600 if scan still 16500)
...
# --- Wave 24 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 16600; greppable 16500 only when N>=16500; 16600 only when N>=16600.
# Scan is source of truth — do not hardcode false makefile_max if still 16500.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 24) ---"
info_check "makefile_max=16600 (log)" 'makefile_max=16600'
info_check "makefile_max=16500 (log)" 'makefile_max=16500'
info_check "ubar3open / bar3=0 (log)" 'ubar3open|bar3=0|bar3:[[:space:]]*OPEN|bar3 OPEN'
# Host continuum scan: stamp makefile_max=16600 greppable only when N>=16600.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 16600 ]]; then
      echo "  info: continuum high-water  makefile_max=16600 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 16500 ]]; then
      echo "  info: continuum high-water  makefile_max=16500 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=16600  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=16600  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 24  target decade M=16600 (CREATE-ONLY soft; parent paste wires)"
else
