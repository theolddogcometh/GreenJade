...
# Soft companions (Wave 33 exclusive — info only, never hard-fail):
#   continuum high-water toward 17500; makefile_max=17400 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 17500 if scan still 17400; 17500 only when N>=17500)
...
# --- Wave 33 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 17500; greppable 17500 only when N>=17500; 17400 prior tip when N>=17400.
# Scan is source of truth — do not hardcode false makefile_max if still 17400.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 33) ---"
info_check "makefile_max=17500 (log)" 'makefile_max=17500'
info_check "makefile_max=17400 (log)" 'makefile_max=17400'
info_check "ubar3open / bar3=0 (log)" 'ubar3open|bar3=0|bar3:[[:space:]]*OPEN|bar3 OPEN'
# Host continuum scan: stamp makefile_max=17500 greppable only when N>=17500.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 17500 ]]; then
      echo "  info: continuum high-water  makefile_max=17500 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 17400 ]]; then
      echo "  info: continuum high-water  makefile_max=17400 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=17500  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=17500  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 33  target decade M=17500 (CREATE-ONLY soft; parent paste wires)"
else
