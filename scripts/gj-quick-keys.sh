...
# Soft companions (Wave 30 exclusive — info only, never hard-fail):
#   continuum high-water toward 17200; makefile_max=17100 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 17200 if scan still 17100)
...
# --- Wave 30 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 17200; greppable 17100 only when N>=17100; 17200 only when N>=17200.
# Scan is source of truth — do not hardcode false makefile_max if still 17100.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 30) ---"
info_check "makefile_max=17100 (log)" 'makefile_max=17100'
...
# Host continuum scan: stamp makefile_max=17200 greppable only when N>=17200.
