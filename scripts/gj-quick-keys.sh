...
# Soft companions (Wave 103 exclusive — info only, never hard-fail):
#   continuum high-water toward 24500; makefile_max=24400 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode false 24500 if scan still 24400; 24000 prior tip when N>=24400)
#   soft deepen retprojectangle/retaffineangle (CREATE-ONLY soft names only)
...
# --- Wave 103 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 24500; greppable 24500 only when N>=24500; 24300 prior tip when N>=24400.
...
echo "  --- continuum / bar3 soft (wave 103) ---"
info_check "makefile_max=24500 (log)" 'makefile_max=24400'
info_check "makefile_max=24500 (log)" 'makefile_max=24400'
info_check "ubar3open / bar3=0 (log)" 'ubar3open|bar3=0|bar3:[[:space:]]*OPEN|bar3 OPEN'
# Host continuum scan: stamp makefile_max=24500 greppable only when N>=24500.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 24500 ]]; then
      echo "  info: continuum high-water  makefile_max=24500 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 24400 ]]; then
      echo "  info: continuum high-water  makefile_max=24400 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=24500  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=24500  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 103  target decade M=24500 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retprojectangle/retaffineangle (CREATE-ONLY soft ≠ product)"
else
