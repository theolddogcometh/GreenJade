...
# Soft companions (Wave 49 exclusive — info only, never hard-fail):
#   continuum high-water toward 19100; makefile_max=19100 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 19100 if scan still 19000; 19000 prior tip when N>=19000)
#   soft deepen retglacisgate/retoutwork (CREATE-ONLY soft names only)
...
# --- Wave 49 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 19100; greppable 19100 only when N>=19100; 19000 prior tip when N>=19000.
# Scan is source of truth — do not hardcode false makefile_max if still 19000.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 49) ---"
info_check "makefile_max=19100 (log)" 'makefile_max=19100'
info_check "makefile_max=19000 (log)" 'makefile_max=19000'
info_check "ubar3open / bar3=0 (log)" 'ubar3open|bar3=0|bar3:[[:space:]]*OPEN|bar3 OPEN'
# Host continuum scan: stamp makefile_max=19100 greppable only when N>=19100.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 19100 ]]; then
      echo "  info: continuum high-water  makefile_max=19100 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 19000 ]]; then
      echo "  info: continuum high-water  makefile_max=19000 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=19100  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=19100  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 49  target decade M=19100 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retglacisgate/retoutwork (CREATE-ONLY soft ≠ product)"
else
