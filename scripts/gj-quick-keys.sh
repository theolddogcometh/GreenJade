...
# Soft companions (Wave 43 exclusive — info only, never hard-fail):
#   continuum high-water toward 18500; makefile_max=18500 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 18500 if scan still 18400; 18400 prior tip when N>=18400)
#   soft deepen retmachicolation/retarrowslit (CREATE-ONLY soft names only)
...
# --- Wave 43 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 18500; greppable 18500 only when N>=18500; 18400 prior tip when N>=18400.
# Scan is source of truth — do not hardcode false makefile_max if still 18400.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 43) ---"
info_check "makefile_max=18500 (log)" 'makefile_max=18500'
info_check "makefile_max=18400 (log)" 'makefile_max=18400'
info_check "ubar3open / bar3=0 (log)" 'ubar3open|bar3=0|bar3:[[:space:]]*OPEN|bar3 OPEN'
# Host continuum scan: stamp makefile_max=18500 greppable only when N>=18500.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 18500 ]]; then
      echo "  info: continuum high-water  makefile_max=18500 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 18400 ]]; then
      echo "  info: continuum high-water  makefile_max=18400 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=18500  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=18500  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 43  target decade M=18500 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retmachicolation/retarrowslit (CREATE-ONLY soft ≠ product)"
else
