...
# Soft companions (Wave 25 exclusive — info only, never hard-fail):
#   continuum high-water toward 16700; makefile_max=16600 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 16700 if scan still 16600)
...
# --- Wave 25 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 16700; greppable 16600 only when N>=16600; 16700 only when N>=16700.
# Scan is source of truth — do not hardcode false makefile_max if still 16600.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 25) ---"
...
# Host continuum scan: stamp makefile_max=16700 greppable only when N>=16700.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 16700 ]]; then
      echo "  info: continuum high-water  makefile_max=16700 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 16600 ]]; then
      echo "  info: continuum high-water  makefile_max=16600 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=16700  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=16700  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 25  target decade M=16700 (CREATE-ONLY soft; parent paste wires)"
else
