...
# Soft companions (Wave 42 exclusive — info only, never hard-fail):
#   continuum high-water toward 18400; makefile_max=18400 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 18400 if scan still 18300; 18300 prior tip when N>=18300)
#   soft deepen retportcullis/retbattlement (CREATE-ONLY soft names only)
...
# --- Wave 42 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 18400; greppable 18400 only when N>=18400; 18300 prior tip when N>=18300.
# Scan is source of truth — do not hardcode false makefile_max if still 18300.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 42) ---"
info_check "makefile_max=18400 (log)" 'makefile_max=18400'
info_check "makefile_max=18300 (log)" 'makefile_max=18300'
info_check "ubar3open / bar3=0 (log)" 'ubar3open|bar3=0|bar3:[[:space:]]*OPEN|bar3 OPEN'
# Host continuum scan: stamp makefile_max=18400 greppable only when N>=18400.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 18400 ]]; then
      echo "  info: continuum high-water  makefile_max=18400 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 18300 ]]; then
      echo "  info: continuum high-water  makefile_max=18300 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=18400  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=18400  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 42  target decade M=18400 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retportcullis/retbattlement (CREATE-ONLY soft ≠ product)"
else
