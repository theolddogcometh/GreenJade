...
# Soft companions (Wave 105 exclusive — info only, never hard-fail):
#   continuum high-water toward 24700; makefile_max=24700 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode false 24700 if scan still prior tip; 24600 prior tip when N>=24600)
#   soft deepen retquadraticangle/retcubicangle (CREATE-ONLY soft names only)
...
# --- Wave 105 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 24700; greppable 24700 only when N>=24700; 24600 prior tip when N>=24600.
...
echo "  --- continuum / bar3 soft (wave 105) ---"
...
# Host continuum scan: stamp makefile_max=24700 greppable only when N>=24700.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 24700 ]]; then
      echo "  info: continuum high-water  makefile_max=24700 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 24600 ]]; then
      echo "  info: continuum high-water  makefile_max=24600 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=24700  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=24700  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 105  target decade M=24700 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retquadraticangle/retcubicangle (CREATE-ONLY soft ≠ product)"
else
