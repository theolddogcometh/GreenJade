...
# Soft companions (Wave 16 exclusive — info only, never hard-fail):
#   continuum high-water toward 15800; makefile_max=15800 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 15800 if scan still 15700)
...
# --- Wave 16 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 15800; greppable 15800 only when N>=15800.
# Scan is source of truth — do not hardcode false makefile_max if still 15700.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 16) ---"
...
# Host continuum scan: stamp makefile_max=15800 greppable only when N>=15800.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 15800 ]]; then
      echo "  info: continuum high-water  makefile_max=15800 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 15700 ]]; then
      echo "  info: continuum high-water  makefile_max=15700 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=15800  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=15800  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 16  target decade M=15800 (CREATE-ONLY soft; parent paste wires)"
else
