...
# Soft companions (Wave 19 exclusive — info only, never hard-fail):
#   continuum high-water toward 16100; makefile_max=16000 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 16100 if scan still 16000)
...
# --- Wave 19 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 16100; greppable 16100 only when N>=16100.
# Scan is source of truth — do not hardcode false makefile_max if still 16000.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 19) ---"
info_check "makefile_max=16100 (log)" 'makefile_max=16100'
...
# Host continuum scan: stamp makefile_max=16100 greppable only when N>=16100.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 16100 ]]; then
      echo "  info: continuum high-water  makefile_max=16100 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 16000 ]]; then
      echo "  info: continuum high-water  makefile_max=16000 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=16100  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=16100  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 19  target decade M=16100 (CREATE-ONLY soft; parent paste wires)"
else
