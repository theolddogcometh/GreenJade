...
# Soft companions (Wave 36 exclusive — info only, never hard-fail):
#   continuum high-water toward 17800; makefile_max=17700 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 17800 if scan still 17700; 17800 only when N>=17800)
#   soft deepen rethold/retspire (CREATE-ONLY soft names only)
...
# --- Wave 36 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 17800; greppable 17800 only when N>=17800; 17700 prior tip when N>=17700.
# Scan is source of truth — do not hardcode false makefile_max if still 17700.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 36) ---"
info_check "makefile_max=17800 (log)" 'makefile_max=17800'
...
# Host continuum scan: stamp makefile_max=17800 greppable only when N>=17800.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 17800 ]]; then
      echo "  info: continuum high-water  makefile_max=17800 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 17700 ]]; then
      echo "  info: continuum high-water  makefile_max=17700 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=17800  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=17800  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 36  target decade M=17800 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  rethold/retspire (CREATE-ONLY soft ≠ product)"
else
