...
# Soft companions (Wave 74 exclusive — info only, never hard-fail):
#   continuum high-water toward 21600; makefile_max=21600 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 21600 if scan still 21500; 21500 prior tip when N>=21500)
#   soft deepen retglacisangle/retparapetangle (CREATE-ONLY soft names only)
...
# --- Wave 74 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 21600; greppable 21600 only when N>=21600; 21500 prior tip when N>=21500.
# Scan is source of truth — do not hardcode false makefile_max if still 21500.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 74) ---"
info_check "makefile_max=21600 (log)" 'makefile_max=21600'
info_check "makefile_max=21500 (log)" 'makefile_max=21500'
info_check "ubar3open / bar3=0 (log)" 'ubar3open|bar3=0|bar3:[[:space:]]*OPEN|bar3 OPEN'
# Host continuum scan: stamp makefile_max=21600 greppable only when N>=21600.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 21600 ]]; then
      echo "  info: continuum high-water  makefile_max=21600 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 21500 ]]; then
      echo "  info: continuum high-water  makefile_max=21500 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=21600  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=21600  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 74  target decade M=21600 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retglacisangle/retparapetangle (CREATE-ONLY soft ≠ product)"
else
