...
# Soft companions (Wave 75 exclusive — info only, never hard-fail):
#   continuum high-water toward 21700; makefile_max=21700 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 21700 if scan still 21600; 21600 prior tip when N>=21600)
#   soft deepen retmoatangle/retowerangle (CREATE-ONLY soft names only)
...
# --- Wave 75 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 21700; greppable 21700 only when N>=21700; 21600 prior tip when N>=21600.
# Scan is source of truth — do not hardcode false makefile_max if still 21600.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 75) ---"
info_check "makefile_max=21700 (log)" 'makefile_max=21700'
...
# Host continuum scan: stamp makefile_max=21700 greppable only when N>=21700.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 21700 ]]; then
      echo "  info: continuum high-water  makefile_max=21700 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 21600 ]]; then
      echo "  info: continuum high-water  makefile_max=21600 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=21700  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=21700  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 75  target decade M=21700 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retmoatangle/retowerangle (CREATE-ONLY soft ≠ product)"
else
