...
# Soft companions (Wave 39 exclusive — info only, never hard-fail):
#   continuum high-water toward 18100; makefile_max=18100 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 18100 if scan still 18000; 18000 prior tip when N>=18000)
#   soft deepen retbarbican/retglacis (CREATE-ONLY soft names only)
...
# --- Wave 39 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 18100; greppable 18100 only when N>=18100; 18000 prior tip when N>=18000.
# Scan is source of truth — do not hardcode false makefile_max if still 18000.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 39) ---"
info_check "makefile_max=18100 (log)" 'makefile_max=18100'
info_check "makefile_max=18000 (log)" 'makefile_max=18000'
info_check "ubar3open / bar3=0 (log)" 'ubar3open|bar3=0|bar3:[[:space:]]*OPEN|bar3 OPEN'
# Host continuum scan: stamp makefile_max=18100 greppable only when N>=18100.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 18100 ]]; then
      echo "  info: continuum high-water  makefile_max=18100 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 18000 ]]; then
      echo "  info: continuum high-water  makefile_max=18000 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=18100  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=18100  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 39  target decade M=18100 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retbarbican/retglacis (CREATE-ONLY soft ≠ product)"
else
