...
# Soft companions (Wave 59 exclusive — info only, never hard-fail):
#   continuum high-water toward 20100; makefile_max=20100 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 20100 if scan still 20000; 20000 prior tip when N>=20000)
#   soft deepen retcrownwork/rethornwork (CREATE-ONLY soft names only)
...
# --- Wave 59 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 20100; greppable 20100 only when N>=20100; 20000 prior tip when N>=20000.
# Scan is source of truth — do not hardcode false makefile_max if still 20000.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 59) ---"
info_check "makefile_max=20100 (log)" 'makefile_max=20100'
info_check "makefile_max=20000 (log)" 'makefile_max=20000'
info_check "ubar3open / bar3=0 (log)" 'ubar3open|bar3=0|bar3:[[:space:]]*OPEN|bar3 OPEN'
# Host continuum scan: stamp makefile_max=20100 greppable only when N>=20100.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 20100 ]]; then
      echo "  info: continuum high-water  makefile_max=20100 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 20000 ]]; then
      echo "  info: continuum high-water  makefile_max=20000 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=20100  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=20100  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 59  target decade M=20100 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retcrownwork/rethornwork (CREATE-ONLY soft ≠ product)"
else
