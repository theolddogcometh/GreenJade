...
# Soft companions (Wave 68 exclusive — info only, never hard-fail):
#   continuum high-water toward 21000; makefile_max=21000 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 21000 if scan still 20900; 20900 prior tip when N>=20900)
#   soft deepen retlunetteangle/rettenailleangle (CREATE-ONLY soft names only)
...
# --- Wave 68 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 21000; greppable 21000 only when N>=21000; 20900 prior tip when N>=20900.
# Scan is source of truth — do not hardcode false makefile_max if still 20900.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 68) ---"
...
# Host continuum scan: stamp makefile_max=21000 greppable only when N>=21000.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 21000 ]]; then
      echo "  info: continuum high-water  makefile_max=21000 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 20900 ]]; then
      echo "  info: continuum high-water  makefile_max=20900 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=21000  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=21000  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 68  target decade M=21000 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retlunetteangle/rettenailleangle (CREATE-ONLY soft ≠ product)"
else
