...
# Soft companions (Wave 17 exclusive — info only, never hard-fail):
#   continuum high-water toward 15900; makefile_max=15900 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 15900 if scan still 15800)
...
# --- Wave 17 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 15900; greppable 15900 only when N>=15900.
# Scan is source of truth — do not hardcode false makefile_max if still 15800.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 17) ---"
info_check "makefile_max=15900 (log)" 'makefile_max=15900'
info_check "makefile_max=15800 (log)" 'makefile_max=15800'
info_check "ubar3open / bar3=0 (log)" 'ubar3open|bar3=0|bar3:[[:space:]]*OPEN|bar3 OPEN'
# Host continuum scan: stamp makefile_max=15900 greppable only when N>=15900.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 15900 ]]; then
      echo "  info: continuum high-water  makefile_max=15900 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 15800 ]]; then
      echo "  info: continuum high-water  makefile_max=15800 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=15900  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=15900  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 17  target decade M=15900 (CREATE-ONLY soft; parent paste wires)"
else
