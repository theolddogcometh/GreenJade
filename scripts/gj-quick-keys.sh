...
# Soft companions (Wave 64 exclusive — info only, never hard-fail):
#   continuum high-water toward 20600; makefile_max=20600 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 20600 if scan still 20500; 20500 prior tip when N>=20500)
#   soft deepen retreentrant/retsallyport (CREATE-ONLY soft names only)
...
# --- Wave 64 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 20600; greppable 20600 only when N>=20600; 20500 prior tip when N>=20500.
# Scan is source of truth — do not hardcode false makefile_max if still 20500.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 64) ---"
...
# Host continuum scan: stamp makefile_max=20600 greppable only when N>=20600.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 20600 ]]; then
      echo "  info: continuum high-water  makefile_max=20600 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 20500 ]]; then
      echo "  info: continuum high-water  makefile_max=20500 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=20600  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=20600  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 64  target decade M=20600 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retreentrant/retsallyport (CREATE-ONLY soft ≠ product)"
else
