...
# Soft companions (Wave 52 exclusive — info only, never hard-fail):
#   continuum high-water toward 19400; makefile_max=19400 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 19400 if scan still 19300; 19300 prior tip when N>=19300)
#   soft deepen rettenaille/retdemilune (CREATE-ONLY soft names only)
...
# --- Wave 52 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 19400; greppable 19400 only when N>=19400; 19300 prior tip when N>=19300.
# Scan is source of truth — do not hardcode false makefile_max if still 19300.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 52) ---"
info_check "makefile_max=19400 (log)" 'makefile_max=19400'
info_check "makefile_max=19300 (log)" 'makefile_max=19300'
info_check "ubar3open / bar3=0 (log)" 'ubar3open|bar3=0|bar3:[[:space:]]*OPEN|bar3 OPEN'
# Host continuum scan: stamp makefile_max=19400 greppable only when N>=19400.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 19400 ]]; then
      echo "  info: continuum high-water  makefile_max=19400 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 19300 ]]; then
      echo "  info: continuum high-water  makefile_max=19300 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=19400  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=19400  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 52  target decade M=19400 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  rettenaille/retdemilune (CREATE-ONLY soft ≠ product)"
else
