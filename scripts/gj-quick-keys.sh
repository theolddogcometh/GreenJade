...
# Soft companions (Wave 96 exclusive — info only, never hard-fail):
#   continuum high-water toward 23800; makefile_max=23800 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode false 23800 if scan still 23700; 23700 prior tip when N>=23700)
#   soft deepen retsectorangle/retwedgeangle (CREATE-ONLY soft names only)
...
# --- Wave 96 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 23800; greppable 23800 only when N>=23800; 23700 prior tip when N>=23700.
# Scan is source of truth — do not hardcode false makefile_max if still 23700.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 96) ---"
info_check "makefile_max=23800 (log)" 'makefile_max=23800'
info_check "makefile_max=23700 (log)" 'makefile_max=23700'
info_check "ubar3open / bar3=0 (log)" 'ubar3open|bar3=0|bar3:[[:space:]]*OPEN|bar3 OPEN'
# Host continuum scan: stamp makefile_max=23800 greppable only when N>=23800.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 23800 ]]; then
      echo "  info: continuum high-water  makefile_max=23800 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 23700 ]]; then
      echo "  info: continuum high-water  makefile_max=23700 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=23800  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=23800  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 96  target decade M=23800 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retsectorangle/retwedgeangle (CREATE-ONLY soft ≠ product)"
else
