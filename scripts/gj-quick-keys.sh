...
# Soft companions (Wave 90 exclusive — info only, never hard-fail):
#   continuum high-water toward 23200; makefile_max=23200 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 23200 if scan still 23100; 23100 prior tip when N>=23100)
#   soft deepen retcrestangle/rettokenangle (CREATE-ONLY soft names only)
...
# --- Wave 90 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 23200; greppable 23200 only when N>=23200; 23100 prior tip when N>=23100.
# Scan is source of truth — do not hardcode false makefile_max if still 23100.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 90) ---"
info_check "makefile_max=23200 (log)" 'makefile_max=23200'
...
# Host continuum scan: stamp makefile_max=23200 greppable only when N>=23200.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 23200 ]]; then
      echo "  info: continuum high-water  makefile_max=23200 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 23100 ]]; then
      echo "  info: continuum high-water  makefile_max=23100 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=23200  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=23200  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 90  target decade M=23200 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retcrestangle/rettokenangle (CREATE-ONLY soft ≠ product)"
else
