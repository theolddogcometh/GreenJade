...
# Soft companions (Wave 87 exclusive — info only, never hard-fail):
#   continuum high-water toward 22900; makefile_max=22900 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 22900 if scan still 22800; 22800 prior tip when N>=22800)
#   soft deepen retflameangle/retcipherangle (CREATE-ONLY soft names only)
...
# --- Wave 87 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 22900; greppable 22900 only when N>=22900; 22800 prior tip when N>=22800.
# Scan is source of truth — do not hardcode false makefile_max if still 22800.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 87) ---"
info_check "makefile_max=22900 (log)" 'makefile_max=22900'
info_check "makefile_max=22800 (log)" 'makefile_max=22800'
info_check "ubar3open / bar3=0 (log)" 'ubar3open|bar3=0|bar3:[[:space:]]*OPEN|bar3 OPEN'
# Host continuum scan: stamp makefile_max=22900 greppable only when N>=22900.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 22900 ]]; then
      echo "  info: continuum high-water  makefile_max=22900 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 22800 ]]; then
      echo "  info: continuum high-water  makefile_max=22800 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=22900  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=22900  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 87  target decade M=22900 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retflameangle/retcipherangle (CREATE-ONLY soft ≠ product)"
else
