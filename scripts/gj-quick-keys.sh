...
# Soft companions (Wave 73 exclusive — info only, never hard-fail):
#   continuum high-water toward 21500; makefile_max=21500 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 21500 if scan still 21400; 21400 prior tip when N>=21400)
#   soft deepen retcurtainface/retbastionangle (CREATE-ONLY soft names only)
...
# --- Wave 73 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 21500; greppable 21500 only when N>=21500; 21400 prior tip when N>=21400.
# Scan is source of truth — do not hardcode false makefile_max if still 21400.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 73) ---"
...
# Host continuum scan: stamp makefile_max=21500 greppable only when N>=21500.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 21500 ]]; then
      echo "  info: continuum high-water  makefile_max=21500 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 21400 ]]; then
      echo "  info: continuum high-water  makefile_max=21400 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=21500  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=21500  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 73  target decade M=21500 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retcurtainface/retbastionangle (CREATE-ONLY soft ≠ product)"
else
