...
# Soft companions (Wave 110 exclusive — info only, never hard-fail):
#   continuum high-water toward 25200; makefile_max=25200 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode false 25200 if scan still prior tip; 25100 prior tip when N>=25100)
#   soft deepen retmeshangle/retgridangle (CREATE-ONLY soft names only)
...
# --- Wave 110 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 25200; greppable 25200 only when N>=25200; 25100 prior tip when N>=25100.
...
echo "  --- continuum / bar3 soft (wave 110) ---"
...
# Host continuum scan: stamp makefile_max=25200 greppable only when N>=25200.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 25200 ]]; then
      echo "  info: continuum high-water  makefile_max=25200 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 25100 ]]; then
      echo "  info: continuum high-water  makefile_max=25100 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=25200  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=25200  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 110  target decade M=25200 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retmeshangle/retgridangle (CREATE-ONLY soft ≠ product)"
else
