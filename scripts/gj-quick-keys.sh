...
# Soft companions (Wave 111 exclusive — info only, never hard-fail):
#   continuum high-water toward 25300; makefile_max=25300 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode false 25300 if scan still prior tip; 25200 prior tip when N>=25200)
#   soft deepen retvoxelangle/rettexelangle (CREATE-ONLY soft names only)
...
# --- Wave 111 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 25300; greppable 25300 only when N>=25300; 25200 prior tip when N>=25200.
...
echo "  --- continuum / bar3 soft (wave 111) ---"
info_check "makefile_max=25300 (log)" 'makefile_max=25300'
info_check "makefile_max=25200 (log)" 'makefile_max=25200'
info_check "ubar3open / bar3=0 (log)" 'ubar3open|bar3=0|bar3:[[:space:]]*OPEN|bar3 OPEN'
# Host continuum scan: stamp makefile_max=25300 greppable only when N>=25300.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 25300 ]]; then
      echo "  info: continuum high-water  makefile_max=25300 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 25200 ]]; then
      echo "  info: continuum high-water  makefile_max=25200 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=25300  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=25300  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 111  target decade M=25300 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retvoxelangle/rettexelangle (CREATE-ONLY soft ≠ product)"
else
