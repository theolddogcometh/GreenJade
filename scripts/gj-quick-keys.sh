...
# Soft companions (Wave 60 exclusive — info only, never hard-fail):
#   continuum high-water toward 20200; makefile_max=20200 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 20200 if scan still 20100; 20100 prior tip when N>=20100)
#   soft deepen retplace/retenvelope (CREATE-ONLY soft names only)
...
# --- Wave 60 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 20200; greppable 20200 only when N>=20200; 20100 prior tip when N>=20100.
# Scan is source of truth — do not hardcode false makefile_max if still 20100.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 60) ---"
...
# Host continuum scan: stamp makefile_max=20200 greppable only when N>=20200.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 20200 ]]; then
      echo "  info: continuum high-water  makefile_max=20200 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 20100 ]]; then
      echo "  info: continuum high-water  makefile_max=20100 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=20200  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=20200  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 60  target decade M=20200 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retplace/retenvelope (CREATE-ONLY soft ≠ product)"
else
