...
# Soft companions (Wave 23 exclusive — info only, never hard-fail):
#   continuum high-water toward 16500; makefile_max=16400 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 16500 if scan still 16400)
...
# --- Wave 23 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 16500; greppable 16400 only when N>=16400; 16500 only when N>=16500.
# Scan is source of truth — do not hardcode false makefile_max if still 16400.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 23) ---"
...
# Host continuum scan: stamp makefile_max=16500 greppable only when N>=16500.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 16500 ]]; then
      echo "  info: continuum high-water  makefile_max=16500 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 16400 ]]; then
      echo "  info: continuum high-water  makefile_max=16400 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=16500  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=16500  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 23  target decade M=16500 (CREATE-ONLY soft; parent paste wires)"
else
