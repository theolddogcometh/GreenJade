...
# Soft companions (Wave 47 exclusive — info only, never hard-fail):
#   continuum high-water toward 18900; makefile_max=18900 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 18900 if scan still 18800; 18800 prior tip when N>=18800)
#   soft deepen retinnerward/retdonjon (CREATE-ONLY soft names only)
...
# --- Wave 47 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 18900; greppable 18900 only when N>=18900; 18800 prior tip when N>=18800.
# Scan is source of truth — do not hardcode false makefile_max if still 18800.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 47) ---"
info_check "makefile_max=18900 (log)" 'makefile_max=18900'
info_check "makefile_max=18800 (log)" 'makefile_max=18800'
info_check "ubar3open / bar3=0 (log)" 'ubar3open|bar3=0|bar3:[[:space:]]*OPEN|bar3 OPEN'
# Host continuum scan: stamp makefile_max=18900 greppable only when N>=18900.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 18900 ]]; then
      echo "  info: continuum high-water  makefile_max=18900 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 18800 ]]; then
      echo "  info: continuum high-water  makefile_max=18800 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=18900  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=18900  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 47  target decade M=18900 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retinnerward/retdonjon (CREATE-ONLY soft ≠ product)"
else
