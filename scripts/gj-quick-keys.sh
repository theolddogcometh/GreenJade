...
# Soft companions (Wave 109 exclusive — info only, never hard-fail):
#   continuum high-water toward 25100; makefile_max=25100 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode false 25100 if scan still prior tip; 25000 prior tip when N>=25000)
#   soft deepen retnurbsangle/retbsplineangle (CREATE-ONLY soft names only)
...
# --- Wave 109 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 25100; greppable 25100 only when N>=25100; 25000 prior tip when N>=25000.
...
echo "  --- continuum / bar3 soft (wave 109) ---"
info_check "makefile_max=25100 (log)" 'makefile_max=25100'
...
# Host continuum scan: stamp makefile_max=25100 greppable only when N>=25100.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 25100 ]]; then
      echo "  info: continuum high-water  makefile_max=25100 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 25000 ]]; then
      echo "  info: continuum high-water  makefile_max=25000 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=25100  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=25100  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 109  target decade M=25100 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retnurbsangle/retbsplineangle (CREATE-ONLY soft ≠ product)"
else
