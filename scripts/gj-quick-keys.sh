...
# Soft companions (Wave 118 exclusive — info only, never hard-fail):
#   continuum high-water toward 26000; makefile_max=25900 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode false 26000 if scan still prior tip; 25800 prior tip when N>=25900)
#   soft deepen retbarrierangle/retatomicangle (CREATE-ONLY soft names only)
...
# --- Wave 118 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 26000; greppable 26000 only when N>=26000; 25900 prior tip when N>=25900.
...
echo "  --- continuum / bar3 soft (wave 118) ---"
info_check "makefile_max=26000 (log)" 'makefile_max=26000'
info_check "makefile_max=25900 (log)" 'makefile_max=25900'
info_check "ubar3open / bar3=0 (log)" 'ubar3open|bar3=0|bar3:[[:space:]]*OPEN|bar3 OPEN'
# Host continuum scan: stamp makefile_max=26000 greppable only when N>=26000.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 26000 ]]; then
      echo "  info: continuum high-water  makefile_max=26000 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 25900 ]]; then
      echo "  info: continuum high-water  makefile_max=25900 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=26000  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=26000  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 118  target decade M=26000 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retbarrierangle/retatomicangle (CREATE-ONLY soft ≠ product)"
else
