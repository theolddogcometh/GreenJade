...
# Soft companions (Wave 112 exclusive — info only, never hard-fail):
#   continuum high-water toward 25400; makefile_max=25400 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode false 25400 if scan still prior tip; 25300 prior tip when N>=25300)
#   soft deepen retfragmentangle/retvertexangle (CREATE-ONLY soft names only)
...
# --- Wave 112 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 25400; greppable 25400 only when N>=25400; 25300 prior tip when N>=25300.
...
echo "  --- continuum / bar3 soft (wave 112) ---"
info_check "makefile_max=25400 (log)" 'makefile_max=25400'
info_check "makefile_max=25300 (log)" 'makefile_max=25300'
info_check "ubar3open / bar3=0 (log)" 'ubar3open|bar3=0|bar3:[[:space:]]*OPEN|bar3 OPEN'
# Host continuum scan: stamp makefile_max=25400 greppable only when N>=25400.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 25400 ]]; then
      echo "  info: continuum high-water  makefile_max=25400 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 25300 ]]; then
      echo "  info: continuum high-water  makefile_max=25300 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=25400  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=25400  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 112  target decade M=25400 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retfragmentangle/retvertexangle (CREATE-ONLY soft ≠ product)"
else
