...
# Soft companions (Wave 62 exclusive — info only, never hard-fail):
#   continuum high-water toward 20400; makefile_max=20400 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 20400 if scan still 20300; 20300 prior tip when N>=20300)
#   soft deepen retbastionface/retcurtainangle (CREATE-ONLY soft names only)
...
# --- Wave 62 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 20400; greppable 20400 only when N>=20400; 20300 prior tip when N>=20300.
# Scan is source of truth — do not hardcode false makefile_max if still 20300.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 62) ---"
info_check "makefile_max=20400 (log)" 'makefile_max=20400'
info_check "makefile_max=20300 (log)" 'makefile_max=20300'
info_check "ubar3open / bar3=0 (log)" 'ubar3open|bar3=0|bar3:[[:space:]]*OPEN|bar3 OPEN'
# Host continuum scan: stamp makefile_max=20400 greppable only when N>=20400.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 20400 ]]; then
      echo "  info: continuum high-water  makefile_max=20400 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 20300 ]]; then
      echo "  info: continuum high-water  makefile_max=20300 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=20400  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=20400  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 62  target decade M=20400 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retbastionface/retcurtainangle (CREATE-ONLY soft ≠ product)"
else
