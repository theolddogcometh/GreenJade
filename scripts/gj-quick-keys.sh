...
# Soft companions (Wave 83 exclusive — info only, never hard-fail):
#   continuum high-water toward 22500; makefile_max=22500 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 22500 if scan still 22400; 22400 prior tip when N>=22400)
#   soft deepen retemblemangle/retsigilangle (CREATE-ONLY soft names only)
...
# --- Wave 83 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 22500; greppable 22500 only when N>=22500; 22400 prior tip when N>=22400.
# Scan is source of truth — do not hardcode false makefile_max if still 22400.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 83) ---"
info_check "makefile_max=22500 (log)" 'makefile_max=22500'
info_check "makefile_max=22400 (log)" 'makefile_max=22400'
info_check "ubar3open / bar3=0 (log)" 'ubar3open|bar3=0|bar3:[[:space:]]*OPEN|bar3 OPEN'
# Host continuum scan: stamp makefile_max=22500 greppable only when N>=22500.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 22500 ]]; then
      echo "  info: continuum high-water  makefile_max=22500 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 22400 ]]; then
      echo "  info: continuum high-water  makefile_max=22400 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=22500  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=22500  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 83  target decade M=22500 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retemblemangle/retsigilangle (CREATE-ONLY soft ≠ product)"
else
