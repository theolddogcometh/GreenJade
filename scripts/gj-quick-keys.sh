...
# Soft companions (Wave 77 exclusive — info only, never hard-fail):
#   continuum high-water toward 21900; makefile_max=21900 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 21900 if scan still 21800; 21800 prior tip when N>=21800)
#   soft deepen retspireangle/retholdangle (CREATE-ONLY soft names only)
...
# --- Wave 77 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 21900; greppable 21900 only when N>=21900; 21800 prior tip when N>=21800.
# Scan is source of truth — do not hardcode false makefile_max if still 21800.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 77) ---"
info_check "makefile_max=21900 (log)" 'makefile_max=21900'
info_check "makefile_max=21800 (log)" 'makefile_max=21800'
info_check "ubar3open / bar3=0 (log)" 'ubar3open|bar3=0|bar3:[[:space:]]*OPEN|bar3 OPEN'
# Host continuum scan: stamp makefile_max=21900 greppable only when N>=21900.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 21900 ]]; then
      echo "  info: continuum high-water  makefile_max=21900 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 21800 ]]; then
      echo "  info: continuum high-water  makefile_max=21800 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=21900  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=21900  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 77  target decade M=21900 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retspireangle/retholdangle (CREATE-ONLY soft ≠ product)"
else
