...
# Soft companions (Wave 29 exclusive — info only, never hard-fail):
#   continuum high-water toward 17100; makefile_max=17000 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 17000 if scan still 17000)
...
# --- Wave 29 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 17100; greppable 17000 only when N>=17000; 17100 only when N>=17100.
# Scan is source of truth — do not hardcode false makefile_max if still 17000.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 29) ---"
...
# Host continuum scan: stamp makefile_max=17100 greppable only when N>=17100.
