...
# Soft companions (Wave 116 exclusive — info only, never hard-fail):
#   continuum high-water toward 25800; makefile_max=25700 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode false 25800 if scan still prior tip; 25600 prior tip when N>=25700)
#   soft deepen retfenceangle/retsemaphoreangle (CREATE-ONLY soft names only)
...
# --- Wave 116 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 25800; greppable 25800 only when N>=25800; 25600 prior tip when N>=25700.
...
echo "  --- continuum / bar3 soft (wave 116) ---"
info_check "makefile_max=25800 (log)" 'makefile_max=25800'
info_check "makefile_max=25700 (log)" 'makefile_max=25700'
info_check "ubar3open / bar3=0 (log)" 'ubar3open|bar3=0|bar3:[[:space:]]*OPEN|bar3 OPEN'
# Host continuum scan: stamp makefile_max=25800 greppable only when N>=25800.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 25800 ]]; then
      echo "  info: continuum high-water  makefile_max=25800 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 25700 ]]; then
      echo "  info: continuum high-water  makefile_max=25700 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=25800  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=25800  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 116  target decade M=25800 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retfenceangle/retsemaphoreangle (CREATE-ONLY soft ≠ product)"
else
