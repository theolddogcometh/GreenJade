...
# Soft companions (Wave 88 exclusive — info only, never hard-fail):
#   continuum high-water toward 23000; makefile_max=23000 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 23000 if scan still 22900; 22900 prior tip when N>=22900)
#   soft deepen retbeaconangle/retledgerangle (CREATE-ONLY soft names only)
...
# --- Wave 88 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 23000; greppable 23000 only when N>=23000; 22900 prior tip when N>=22900.
# Scan is source of truth — do not hardcode false makefile_max if still 22900.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 88) ---"
info_check "makefile_max=23000 (log)" 'makefile_max=23000'
info_check "makefile_max=22900 (log)" 'makefile_max=22900'
info_check "ubar3open / bar3=0 (log)" 'ubar3open|bar3=0|bar3:[[:space:]]*OPEN|bar3 OPEN'
# Host continuum scan: stamp makefile_max=23000 greppable only when N>=23000.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 23000 ]]; then
      echo "  info: continuum high-water  makefile_max=23000 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 22900 ]]; then
      echo "  info: continuum high-water  makefile_max=22900 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=23000  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=23000  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 88  target decade M=23000 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retbeaconangle/retledgerangle (CREATE-ONLY soft ≠ product)"
else
