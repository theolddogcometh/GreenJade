...
# Soft companions (Wave 82 exclusive — info only, never hard-fail):
#   continuum high-water toward 22400; makefile_max=22400 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 22400 if scan still 22300; 22300 prior tip when N>=22300)
#   soft deepen retmantleangle/retaegisangle (CREATE-ONLY soft names only)
...
# --- Wave 82 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 22400; greppable 22400 only when N>=22400; 22300 prior tip when N>=22300.
# Scan is source of truth — do not hardcode false makefile_max if still 22300.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 82) ---"
...
# Host continuum scan: stamp makefile_max=22400 greppable only when N>=22400.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 22400 ]]; then
      echo "  info: continuum high-water  makefile_max=22400 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 22300 ]]; then
      echo "  info: continuum high-water  makefile_max=22300 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=22400  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=22400  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 82  target decade M=22400 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retmantleangle/retaegisangle (CREATE-ONLY soft ≠ product)"
else
