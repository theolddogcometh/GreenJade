...
# Soft companions (Wave 91 exclusive — info only, never hard-fail):
#   continuum high-water toward 23300; makefile_max=23300 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 23300 if scan still 23200; 23200 prior tip when N>=23200)
#   soft deepen retbadgeangle/retphaseangle (CREATE-ONLY soft names only)
...
# --- Wave 91 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 23300; greppable 23300 only when N>=23300; 23200 prior tip when N>=23200.
# Scan is source of truth — do not hardcode false makefile_max if still 23200.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 91) ---"
info_check "makefile_max=23300 (log)" 'makefile_max=23300'
...
# Host continuum scan: stamp makefile_max=23300 greppable only when N>=23300.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 23300 ]]; then
      echo "  info: continuum high-water  makefile_max=23300 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 23200 ]]; then
      echo "  info: continuum high-water  makefile_max=23200 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=23300  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=23300  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 91  target decade M=23300 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retbadgeangle/retphaseangle (CREATE-ONLY soft ≠ product)"
else
