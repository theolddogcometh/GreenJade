...
# Soft companions (Wave 84 exclusive — info only, never hard-fail):
#   continuum high-water toward 22600; makefile_max=22600 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 22600 if scan still 22500; 22500 prior tip when N>=22500)
#   soft deepen retscepterangle/retglyphangle (CREATE-ONLY soft names only)
...
# --- Wave 84 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 22600; greppable 22600 only when N>=22600; 22500 prior tip when N>=22500.
# Scan is source of truth — do not hardcode false makefile_max if still 22500.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 84) ---"
info_check "makefile_max=22600 (log)" 'makefile_max=22600'
info_check "makefile_max=22500 (log)" 'makefile_max=22500'
info_check "ubar3open / bar3=0 (log)" 'ubar3open|bar3=0|bar3:[[:space:]]*OPEN|bar3 OPEN'
# Host continuum scan: stamp makefile_max=22600 greppable only when N>=22600.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 22600 ]]; then
      echo "  info: continuum high-water  makefile_max=22600 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 22500 ]]; then
      echo "  info: continuum high-water  makefile_max=22500 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=22600  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=22600  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 84  target decade M=22600 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retscepterangle/retglyphangle (CREATE-ONLY soft ≠ product)"
else
