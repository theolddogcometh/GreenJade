...
# Soft companions (Wave 51 exclusive — info only, never hard-fail):
#   continuum high-water toward 19300; makefile_max=19300 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 19300 if scan still 19200; 19200 prior tip when N>=19200)
#   soft deepen retfosse/retcoveredway (CREATE-ONLY soft names only)
...
# --- Wave 51 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 19300; greppable 19300 only when N>=19300; 19200 prior tip when N>=19200.
# Scan is source of truth — do not hardcode false makefile_max if still 19200.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 51) ---"
...
# Host continuum scan: stamp makefile_max=19300 greppable only when N>=19300.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 19300 ]]; then
      echo "  info: continuum high-water  makefile_max=19300 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 19200 ]]; then
      echo "  info: continuum high-water  makefile_max=19200 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=19300  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=19300  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 51  target decade M=19300 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retfosse/retcoveredway (CREATE-ONLY soft ≠ product)"
else
