...
# Soft companions (Wave 78 exclusive — info only, never hard-fail):
#   continuum high-water toward 22000; makefile_max=22000 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 22000 if scan still 21900; 21900 prior tip when N>=21900)
#   soft deepen retpalaceangle/retfortressangle (CREATE-ONLY soft names only)
...
# --- Wave 78 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 22000; greppable 22000 only when N>=22000; 21900 prior tip when N>=21900.
# Scan is source of truth — do not hardcode false makefile_max if still 21900.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 78) ---"
info_check "makefile_max=22000 (log)" 'makefile_max=22000'
info_check "makefile_max=21900 (log)" 'makefile_max=21900'
info_check "ubar3open / bar3=0 (log)" 'ubar3open|bar3=0|bar3:[[:space:]]*OPEN|bar3 OPEN'
# Host continuum scan: stamp makefile_max=22000 greppable only when N>=22000.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 22000 ]]; then
      echo "  info: continuum high-water  makefile_max=22000 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 21900 ]]; then
      echo "  info: continuum high-water  makefile_max=21900 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=22000  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=22000  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 78  target decade M=22000 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retpalaceangle/retfortressangle (CREATE-ONLY soft ≠ product)"
else
