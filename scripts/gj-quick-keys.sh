...
# Soft companions (Wave 115 exclusive — info only, never hard-fail):
#   continuum high-water toward 25700; makefile_max=25600 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode false 25700 if scan still prior tip; 25600 prior tip when N>=25600)
#   soft deepen retpresentangle/retvsyncangle (CREATE-ONLY soft names only)
...
# --- Wave 115 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 25700; greppable 25700 only when N>=25700; 25600 prior tip when N>=25600.
...
echo "  --- continuum / bar3 soft (wave 115) ---"
...
# Host continuum scan: stamp makefile_max=25700 greppable only when N>=25700.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 25700 ]]; then
      echo "  info: continuum high-water  makefile_max=25700 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 25600 ]]; then
      echo "  info: continuum high-water  makefile_max=25600 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=25700  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=25700  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 115  target decade M=25700 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retpresentangle/retvsyncangle (CREATE-ONLY soft ≠ product)"
else
