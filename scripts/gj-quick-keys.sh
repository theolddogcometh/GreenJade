...
# Soft companions (Wave 108 exclusive — info only, never hard-fail):
#   continuum high-water toward 25000; makefile_max=25000 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode false 25000 if scan still prior tip; 24900 prior tip when N>=24900)
#   soft deepen rethurmitangle/retcatmullangle (CREATE-ONLY soft names only)
...
# --- Wave 108 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 25000; greppable 25000 only when N>=25000; 24900 prior tip when N>=24900.
...
echo "  --- continuum / bar3 soft (wave 108) ---"
info_check "makefile_max=25000 (log)" 'makefile_max=25000'
info_check "makefile_max=24900 (log)" 'makefile_max=24900'
info_check "ubar3open / bar3=0 (log)" 'ubar3open|bar3=0|bar3:[[:space:]]*OPEN|bar3 OPEN'
# Host continuum scan: stamp makefile_max=25000 greppable only when N>=25000.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 25000 ]]; then
      echo "  info: continuum high-water  makefile_max=25000 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 24900 ]]; then
      echo "  info: continuum high-water  makefile_max=24900 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=25000  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=25000  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 108  target decade M=25000 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  rethurmitangle/retcatmullangle (CREATE-ONLY soft ≠ product)"
else
