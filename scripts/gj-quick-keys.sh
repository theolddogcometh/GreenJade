...
# Soft companions (Wave 58 exclusive — info only, never hard-fail):
#   continuum high-water toward 20000; makefile_max=20000 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 20000 if scan still 19900; 19900 prior tip when N>=19900)
#   soft deepen retorillon/retbonnette (CREATE-ONLY soft names only)
...
# --- Wave 58 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 20000; greppable 20000 only when N>=20000; 19900 prior tip when N>=19900.
# Scan is source of truth — do not hardcode false makefile_max if still 19900.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 58) ---"
...
# Host continuum scan: stamp makefile_max=20000 greppable only when N>=20000.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 20000 ]]; then
      echo "  info: continuum high-water  makefile_max=20000 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 19900 ]]; then
      echo "  info: continuum high-water  makefile_max=19900 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=20000  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=20000  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 58  target decade M=20000 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retorillon/retbonnette (CREATE-ONLY soft ≠ product)"
else
