...
# Soft companions (Wave 81 exclusive — info only, never hard-fail):
#   continuum high-water toward 22300; makefile_max=22300 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 22300 if scan still 22200; 22200 prior tip when N>=22200)
#   soft deepen retpanoplyangle/retbulwarkangle (CREATE-ONLY soft names only)
...
# --- Wave 81 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 22300; greppable 22300 only when N>=22300; 22200 prior tip when N>=22200.
# Scan is source of truth — do not hardcode false makefile_max if still 22200.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 81) ---"
info_check "makefile_max=22300 (log)" 'makefile_max=22300'
info_check "makefile_max=22200 (log)" 'makefile_max=22200'
info_check "ubar3open / bar3=0 (log)" 'ubar3open|bar3=0|bar3:[[:space:]]*OPEN|bar3 OPEN'
# Host continuum scan: stamp makefile_max=22300 greppable only when N>=22300.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 22300 ]]; then
      echo "  info: continuum high-water  makefile_max=22300 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 22200 ]]; then
      echo "  info: continuum high-water  makefile_max=22200 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=22300  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=22300  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 81  target decade M=22300 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retpanoplyangle/retbulwarkangle (CREATE-ONLY soft ≠ product)"
else
