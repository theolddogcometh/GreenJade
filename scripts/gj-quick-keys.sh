...
# Soft companions (Wave 28 exclusive — info only, never hard-fail):
#   continuum high-water toward 17000; makefile_max=16900 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 17000 if scan still 16900)
...
# --- Wave 28 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 17000; greppable 16900 only when N>=16900; 17000 only when N>=17000.
# Scan is source of truth — do not hardcode false makefile_max if still 16900.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 28) ---"
info_check "makefile_max=16900 (log)" 'makefile_max=16900'
info_check "makefile_max=17000 (log)" 'makefile_max=17000'
info_check "ubar3open / bar3=0 (log)" 'ubar3open|bar3=0|bar3:[[:space:]]*OPEN|bar3 OPEN'
# Host continuum scan: stamp makefile_max=17000 greppable only when N>=17000.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 17000 ]]; then
      echo "  info: continuum high-water  makefile_max=17000 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 16900 ]]; then
      echo "  info: continuum high-water  makefile_max=16900 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=17000  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=17000  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 28  target decade M=17000 (CREATE-ONLY soft; parent paste wires)"
else
