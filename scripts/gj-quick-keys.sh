...
# Soft companions (Wave 44 exclusive — info only, never hard-fail):
#   continuum high-water toward 18600; makefile_max=18600 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 18500 if scan still 18500; 18500 prior tip when N>=18500)
#   soft deepen retmerlon/retembrasure (CREATE-ONLY soft names only)
...
# --- Wave 44 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 18600; greppable 18600 only when N>=18600; 18500 prior tip when N>=18500.
# Scan is source of truth — do not hardcode false makefile_max if still 18500.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 44) ---"
...
# Host continuum scan: stamp makefile_max=18600 greppable only when N>=18600.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 18600 ]]; then
      echo "  info: continuum high-water  makefile_max=18600 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 18500 ]]; then
      echo "  info: continuum high-water  makefile_max=18500 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=18600  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=18600  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 44  target decade M=18600 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retmerlon/retembrasure (CREATE-ONLY soft ≠ product)"
else
