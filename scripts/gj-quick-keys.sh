...
# Soft companions (Wave 92 exclusive — info only, never hard-fail):
#   continuum high-water toward 23400; makefile_max=23400 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 23400 if scan still 23300; 23300 prior tip when N>=23300)
#   soft deepen retmarkangle/retpulseangle (CREATE-ONLY soft names only)
...
# --- Wave 92 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 23400; greppable 23400 only when N>=23400; 23300 prior tip when N>=23300.
# Scan is source of truth — do not hardcode false makefile_max if still 23300.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 92) ---"
info_check "makefile_max=23400 (log)" 'makefile_max=23400'
...
# Host continuum scan: stamp makefile_max=23400 greppable only when N>=23400.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 23400 ]]; then
      echo "  info: continuum high-water  makefile_max=23400 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 23300 ]]; then
      echo "  info: continuum high-water  makefile_max=23300 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=23400  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=23400  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 92  target decade M=23400 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retmarkangle/retpulseangle (CREATE-ONLY soft ≠ product)"
else
