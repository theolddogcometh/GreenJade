...
# Soft companions (Wave 22 exclusive — info only, never hard-fail):
#   continuum high-water toward 16400; makefile_max=16300 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 16400 if scan still 16300)
...
# --- Wave 22 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 16400; greppable 16300 only when N>=16300; 16400 only when N>=16400.
# Scan is source of truth — do not hardcode false makefile_max if still 16300.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 22) ---"
info_check "makefile_max=16400 (log)" 'makefile_max=16400'
info_check "makefile_max=16300 (log)" 'makefile_max=16300'
info_check "ubar3open / bar3=0 (log)" 'ubar3open|bar3=0|bar3:[[:space:]]*OPEN|bar3 OPEN'
# Host continuum scan: stamp makefile_max=16400 greppable only when N>=16400.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 16400 ]]; then
      echo "  info: continuum high-water  makefile_max=16400 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 16300 ]]; then
      echo "  info: continuum high-water  makefile_max=16300 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=16400  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=16400  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 22  target decade M=16400 (CREATE-ONLY soft; parent paste wires)"
else
