...
# Soft companions (Wave 67 exclusive — info only, never hard-fail):
#   continuum high-water toward 20900; makefile_max=20900 when greppable (soft ≠ bar3)
#   (scan is source of truth; do not hardcode 20900 if scan still 20800; 20800 prior tip when N>=20800)
#   soft deepen retcaponierangle/retredanangle (CREATE-ONLY soft names only)
...
# --- Wave 67 soft companions (never increments miss; hard keys stay hard) ---
# Continuum high-water toward 20900; greppable 20900 only when N>=20900; 20800 prior tip when N>=20800.
# Scan is source of truth — do not hardcode false makefile_max if still 20800.
# Bar3 open stamp: media READY ≠ client run ≠ Top-50; always OPEN honesty.
echo "  --- continuum / bar3 soft (wave 67) ---"
info_check "makefile_max=20900 (log)" 'makefile_max=20900'
info_check "makefile_max=20800 (log)" 'makefile_max=20800'
info_check "ubar3open / bar3=0 (log)" 'ubar3open|bar3=0|bar3:[[:space:]]*OPEN|bar3 OPEN'
# Host continuum scan: stamp makefile_max=20900 greppable only when N>=20900.
if [[ -f "$ROOT/scripts/gj-continuum-makefile-snippet.sh" ]]; then
  ...
  if [[ -n "${_mx_n:-}" ]]; then
    ...
    if [[ "$_mx_n" -ge 20900 ]]; then
      echo "  info: continuum high-water  makefile_max=20900 greppable  (soft graph ≠ bar3)"
    elif [[ "$_mx_n" -ge 20800 ]]; then
      echo "  info: continuum high-water  makefile_max=20800 greppable  (prior tip; soft ≠ product)"
      echo "  info: continuum high-water  makefile_max=20900  (absent; tree max=$_mx_n; scan=truth)"
    else
      echo "  info: continuum high-water  makefile_max=20900  (absent; tree max=$_mx_n; scan=truth)"
    fi
  else
  ...
  echo "  info: continuum wave 67  target decade M=20900 (CREATE-ONLY soft; parent paste wires)"
  echo "  info: continuum soft deepen  retcaponierangle/retredanangle (CREATE-ONLY soft ≠ product)"
else
