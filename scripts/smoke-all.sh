...
#   continuum high-water via product-summary (Wave 40: toward M=18200;
#     greppable 18200 only when scan says so — not a hard gate; ≠ bar3/Top50;
#     soft deepen retcurtain/retparapet CREATE-ONLY only; soft ≠ product complete;
...
# (Wave 40: continuum high-water side panel notes toward M=18200; greppable
#  18200 only when scan ≥18200 — hard gates stay hard; soft graph ≠ product bar)
if [ -x scripts/gj-product-summary.sh ] || [ -f scripts/gj-product-summary.sh ]; then
    echo "  --- gj-product-summary (soft; wave 40 continuum side panel) ---"
