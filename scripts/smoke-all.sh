...
#   continuum high-water via product-summary (Wave 39: toward M=18100;
#     greppable 18000 only when scan says so — not a hard gate; ≠ bar3/Top50;
#     soft deepen retbarbican/retglacis CREATE-ONLY only; soft ≠ product complete;
...
# (Wave 39: continuum high-water side panel notes toward M=18100; greppable
#  18000 only when scan ≥18000 — hard gates stay hard; soft graph ≠ product bar)
if [ -x scripts/gj-product-summary.sh ] || [ -f scripts/gj-product-summary.sh ]; then
    echo "  --- gj-product-summary (soft; wave 39 continuum side panel) ---"
