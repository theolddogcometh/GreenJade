...
#   continuum high-water via product-summary (Wave 42: toward M=18400;
#     greppable 18300 only when scan says so — not a hard gate; ≠ bar3/Top50;
#     soft deepen retportcullis/retbattlement CREATE-ONLY only; soft ≠ product complete;
...
# (Wave 42: continuum high-water side panel notes toward M=18400; greppable
#  18300 only when scan ≥18300 — hard gates stay hard; soft graph ≠ product bar)
if [ -x scripts/gj-product-summary.sh ] || [ -f scripts/gj-product-summary.sh ]; then
    echo "  --- gj-product-summary (soft; wave 42 continuum side panel) ---"
