...
#
# Wave 124 soft note (exclusive scripts high-water era):
#   Continuum soft high-water advancing toward M=26600 (CREATE-ONLY soft graph).
#   Soft deepen surfaces: retbrushangle / retinkangle (CREATE-ONLY soft names only).
#   Honest makefile_max is a Makefile scan — may still report 26400 until parent wires 26500.
#   Do not hardcode false makefile_max=26500 if scan still prior tip. Product lamps remain 0.
...
echo "  note:   Wave 124 soft  continuum toward M=26600 ≠ bar3; makefile_max=scan truth; retbrushangle/retinkangle soft only"
