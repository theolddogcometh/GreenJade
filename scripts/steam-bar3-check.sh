...
#
# Wave 125 soft note (exclusive scripts high-water era):
#   Continuum soft high-water advancing toward M=26700 (CREATE-ONLY soft graph).
#   Soft deepen surfaces: retpaletteangle / retstrokeangle (CREATE-ONLY soft names only).
#   Honest makefile_max is a Makefile scan — may still report 26600 until parent wires 26700.
#   Do not hardcode false makefile_max=26700 if scan still prior tip. Product lamps remain 0.
...
echo "  note:   Wave 125 soft  continuum toward M=26700 ≠ bar3; makefile_max=scan truth; retpaletteangle/retstrokeangle soft only"
