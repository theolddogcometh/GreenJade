...
#
# Wave 114 soft note (exclusive scripts high-water era):
#   Continuum soft high-water advancing toward M=25600 (CREATE-ONLY soft graph).
#   Soft deepen surfaces: retframebufferangle / retswapchainangle (CREATE-ONLY soft names only).
#   Honest makefile_max is a Makefile scan — may still report 25500 until parent wires 25600.
#   Do not hardcode false makefile_max=25600 if scan still prior tip. Product lamps remain 0.
...
echo "  note:   Wave 114 soft  continuum toward M=25600 ≠ bar3; makefile_max=scan truth; retframebufferangle/retswapchainangle soft only"
