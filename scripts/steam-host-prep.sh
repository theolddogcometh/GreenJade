...
chmod +x scripts/fetch-steam-bootstrap.sh scripts/stage-steam-tree.sh
...
EOF
	...
	echo "steam-host-prep: copy PASS → $dest (STATUS=$(cat "$dest/steam/STATUS" 2>/dev/null || echo ?))"
}
...
if [ "$mode" = "fetch-only" ]; then
	echo "steam-host-prep: PASS (fetch-only)"
	exit 0
fi
...
case "$mode" in
stage)
	echo "steam-host-prep: PASS (staged only)"
...
rootfs)
	...
	echo "steam-host-prep: PASS rootfs/opt/steam"
	exit 0
	;;
all-img)
	echo "steam-host-prep: [3/n] rebuild hwtest-img with steam (option 2 pack)"
	make -j"$(nproc)" hwtest-img
	echo "steam-host-prep: PASS img=build/greenjade-hwtest.img"
...
label)
	...
	umount "$mnt"
	rmdir "$mnt" 2>/dev/null || true
	echo "steam-host-prep: PASS (label GJ-PERSIST)"
	exit 0
	;;
dev)
	need_root "--to-dev"
	...
	umount "$mnt"
	rmdir "$mnt" 2>/dev/null || true
	echo "steam-host-prep: PASS (dev $target)"
	exit 0
	;;
img)
	...
	rmdir "$mnt" 2>/dev/null || true
	echo "steam-host-prep: PASS (img $img p2)"
	exit 0
	;;
*)
