 ...
 *
 * Soft inventory (Wave 63 exclusive deepen):
 ...
 *   hda_client: soft deepen wave=63 areas=… ok=… skip=… played=…
...
/*
 * Wave stamp + inventory area count (Wave 63 exclusive deepen).
 * Areas: suite door played inventory steps miss last deepen path
 *        ops first peak format ratio honesty
 */
#define HDA_SOFT_WAVE 63u
#define HDA_SOFT_AREAS  15u

/* Soft suite sub-step bits (Wave 63 greppable steps line). */
...
soft_stats(const char *tag, unsigned *aSt)
{
    ...
    /* Wave 63: first successful STATS snapshot. */
    if (g_fSoftFirst == 0u) {
    ...
    /* Wave 63: peak q/p/u across soft STATS. */
...
soft_inventory_log(void)
{
    ...
    /* Grep: hda_client: soft inventory (Wave 63 rollup) */
    o = 0;
    ...
    /* Grep: hda_client: soft steps (Wave 63 per-sub-step lamps) */
    o = 0;
    ...
    /* Grep: hda_client: soft first (Wave 63 first STATS) */
    o = 0;
    ...
    /* Grep: hda_client: soft peak (Wave 63 peak q/p/u) */
    o = 0;
    ...
    /* Grep: hda_client: soft ops (Wave 63 total door op attempts) */
    o = 0;
    ...
    /* Grep: hda_client: soft format (Wave 63 format lamps) */
    o = 0;
    ...
    /* Grep: hda_client: soft ratio (Wave 63 ok/skip + door rollup) */
    o = 0;
    ...
    /* Grep: hda_client: soft deepen (Wave 63 stamp) */
    o = 0;
    ...
    /* Grep: hda_client: soft honesty (Wave 63; never bar3) */
    msg("hda_client: soft honesty not-bar3 not-pipewire not-product-audio "
        "soft-inventory-only multi_server=0 confine=0 bar3=0 product_kernel=OPEN wave=63\n");
...
soft_suite(void)
{
    ...
    /* Wave 63 exclusive soft door deepen. */
    ...
    /* Wave 63 soft inventory — greppable hda_client: soft / hda-gj: soft. */
    soft_inventory_log();
