 ...
 *
 * Soft inventory (Wave 44 exclusive deepen):
 ...
 *   hda_client: soft deepen wave=44 areas=… ok=… skip=… played=…
...
/*
 * Wave stamp + inventory area count (Wave 44 exclusive deepen).
 * Areas: suite door played inventory steps miss last deepen path
 *        ops first peak format ratio honesty
 */
#define HDA_SOFT_WAVE   43u
#define HDA_SOFT_AREAS  15u

/* Soft suite sub-step bits (Wave 44 greppable steps line). */
...
/*
 * Soft product inventory (Wave 44 exclusive deepen). Cumulative for this
...
soft_stats(const char *tag, unsigned *aSt)
{
    ...
    /* Wave 44: first successful STATS snapshot. */
    if (g_fSoftFirst == 0u) {
    ...
    /* Wave 44: peak q/p/u across soft STATS. */
...
/*
 * Greppable soft inventory (Wave 44 exclusive deepen).
...
soft_inventory_log(void)
{
    ...
    /* Grep: hda_client: soft inventory (Wave 44 rollup) */
    o = 0;
    ...
    /* Grep: hda_client: soft steps (Wave 44 per-sub-step lamps) */
    o = 0;
    ...
    /* Grep: hda_client: soft first (Wave 44 first STATS) */
    o = 0;
    ...
    /* Grep: hda_client: soft peak (Wave 44 peak q/p/u) */
    o = 0;
    ...
    /* Grep: hda_client: soft ops (Wave 44 total door op attempts) */
    o = 0;
    ...
    /* Grep: hda_client: soft format (Wave 44 format lamps) */
    o = 0;
    ...
    /* Grep: hda_client: soft ratio (Wave 44 ok/skip + door rollup) */
    o = 0;
    ...
    /* Grep: hda_client: soft deepen (Wave 44 stamp) */
    o = 0;
    ...
    /* Grep: hda_client: soft honesty (Wave 44; never bar3) */
    msg("hda_client: soft honesty not-bar3 not-pipewire not-product-audio "
        "soft-inventory-only multi_server=0 confine=0 bar3=0 product_kernel=OPEN wave=44\n");
...
soft_suite(void)
{
    ...
    /* Wave 44 exclusive soft door deepen. */
    ...
    /* Wave 44 soft inventory — greppable hda_client: soft / hda-gj: soft. */
    soft_inventory_log();
