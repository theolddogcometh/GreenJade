 ...
 *   init: soft path soft=1 bar3=0 multi_server=0 confine=0 wave=19
 *   init: soft honesty multi_server=0 confine=0 bar3=0 exclusive=1 soft=1 product_kernel=OPEN wave=19
 *   init: soft deepen product_kernel=OPEN wave=19 areas=… multi_server=0 confine=0 bar3=0
 *   init: soft exclusive product_kernel=OPEN wave=19 multi_server=0 confine=0 bar3=0 userland=1
 *
 * Soft deepen Wave 19 (never hard-fails boot; exclusive soft inventory):
 ...
 * Wave 19: ids/clocks/fds/rlimit + path honesty + deepen wave stamp.
...
soft_inventory_log(void)
{
    ...
    /*
     * Grep: init: soft path (Wave 19 honesty).
    ...
    /*
     * Grep: init: soft honesty (Wave 19 exclusive deepen).
     * Soft inventory ≠ product multi-server confine.
     */
    gj_puts("init: soft honesty multi_server=0 confine=0 bar3=0 "
            "exclusive=1 soft=1 product_kernel=OPEN wave=19\n");

    /* Grep: init: soft deepen wave (Wave 19 stamp) */
    (void)gj_snprintf(aLine, sizeof(aLine),
                      "init: soft deepen wave=19 areas=%u "
                      "multi_server=0 confine=0 bar3=0 product_kernel=OPEN\n",
                      (unsigned long)g_cSoftAreas);
    gj_puts(aLine);

    /*
     * Grep: init: soft exclusive (Wave 19 exclusive deepen).
     * Soft inventory ≠ product multi-server confine / continuum.
     */
    gj_puts("init: soft exclusive product_kernel=OPEN wave=19 multi_server=0 confine=0 "
            "bar3=0 userland=1 kernel=0 continuum=0 product_kernel=OPEN\n");

    /*
     * Grep: init: soft open (Wave 19 open-lamp rollup).
