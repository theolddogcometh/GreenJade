 ...
 *   init: soft path soft=1 bar3=0 multi_server=0 confine=0 wave=56
 *   init: soft honesty multi_server=0 confine=0 bar3=0 exclusive=1 soft=1 product_kernel=OPEN wave=56
 *   init: soft deepen product_kernel=OPEN wave=56 areas=… multi_server=0 confine=0 bar3=0
 *   init: soft exclusive product_kernel=OPEN wave=56 multi_server=0 confine=0 bar3=0 userland=1
 *
 * Soft deepen Wave 56 (never hard-fails boot; exclusive soft inventory):
 ...
 * Wave 56: ids/clocks/fds/rlimit + path honesty + deepen wave stamp.
...
soft_inventory_log(void)
{
    ...
    /*
     * Grep: init: soft path (Wave 56 honesty).
    ...
    /*
     * Grep: init: soft honesty (Wave 56 exclusive deepen).
     * Soft inventory ≠ product multi-server confine.
     */
    gj_puts("init: soft honesty multi_server=0 confine=0 bar3=0 "
            "exclusive=1 soft=1 product_kernel=OPEN wave=56\n");

    /* Grep: init: soft deepen wave (Wave 56 stamp) */
    (void)gj_snprintf(aLine, sizeof(aLine),
                      "init: soft deepen wave=56 areas=%u "
                      "multi_server=0 confine=0 bar3=0 product_kernel=OPEN\n",
                      (unsigned long)g_cSoftAreas);
    gj_puts(aLine);

    /*
     * Grep: init: soft exclusive (Wave 56 exclusive deepen).
     * Soft inventory ≠ product multi-server confine / continuum.
     */
    gj_puts("init: soft exclusive product_kernel=OPEN wave=56 multi_server=0 confine=0 "
            "bar3=0 userland=1 kernel=0 continuum=0 product_kernel=OPEN\n");

    /*
     * Grep: init: soft open (Wave 56 open-lamp rollup).
    ...
    /*
     * Grep: init: soft retgorge — Wave 56 return-gorge honesty
     * Soft inventory only; continuum toward 19800; Soft≠product; not bar3.
     */
    gj_puts("init: soft retgorge soft_only=1 product_gate=0 "
            "soft_ne_product=1 continuum_toward=19800 "
            "(retgorge honesty; Soft!=product; not bar3)\n");

    /*
     * Grep: init: soft retshoulder — Wave 56 exclusive shoulder stamp
     * Soft inventory only; Soft≠product.
     */
    gj_puts("init: soft retshoulder exclusive=1 soft_ne_product=1 "
