 ...
 *   init: soft path soft=1 bar3=0 multi_server=0 confine=0 wave=51
 *   init: soft honesty multi_server=0 confine=0 bar3=0 exclusive=1 soft=1 product_kernel=OPEN wave=51
 *   init: soft deepen product_kernel=OPEN wave=51 areas=… multi_server=0 confine=0 bar3=0
 *   init: soft exclusive product_kernel=OPEN wave=51 multi_server=0 confine=0 bar3=0 userland=1
 *
 * Soft deepen Wave 51 (never hard-fails boot; exclusive soft inventory):
 ...
 * Wave 51: ids/clocks/fds/rlimit + path honesty + deepen wave stamp.
...
soft_inventory_log(void)
{
    ...
    /*
     * Grep: init: soft path (Wave 51 honesty).
    ...
    /*
     * Grep: init: soft honesty (Wave 51 exclusive deepen).
     * Soft inventory ≠ product multi-server confine.
     */
    gj_puts("init: soft honesty multi_server=0 confine=0 bar3=0 "
            "exclusive=1 soft=1 product_kernel=OPEN wave=51\n");

    /* Grep: init: soft deepen wave (Wave 51 stamp) */
    (void)gj_snprintf(aLine, sizeof(aLine),
                      "init: soft deepen wave=51 areas=%u "
                      "multi_server=0 confine=0 bar3=0 product_kernel=OPEN\n",
                      (unsigned long)g_cSoftAreas);
    gj_puts(aLine);

    /*
     * Grep: init: soft exclusive (Wave 51 exclusive deepen).
     * Soft inventory ≠ product multi-server confine / continuum.
     */
    gj_puts("init: soft exclusive product_kernel=OPEN wave=51 multi_server=0 confine=0 "
            "bar3=0 userland=1 kernel=0 continuum=0 product_kernel=OPEN\n");

    /*
     * Grep: init: soft open (Wave 51 open-lamp rollup).
    ...
    /*
     * Grep: init: soft retfosse — Wave 51 return-glacisgate honesty
     * Soft inventory only; continuum toward 19300; Soft≠product; not bar3.
     */
    gj_puts("init: soft retfosse soft_only=1 product_gate=0 "
            "soft_ne_product=1 continuum_toward=19300 "
            "(retfosse honesty; Soft!=product; not bar3)\n");

    /*
     * Grep: init: soft retcoveredway — Wave 51 exclusive outwork stamp
     * Soft inventory only; Soft≠product.
     */
    gj_puts("init: soft retcoveredway exclusive=1 soft_ne_product=1 "
