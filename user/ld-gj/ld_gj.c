 ...
 *
 * Soft inventory (Wave 15 exclusive deepen — greppable "ld-gj: soft …"):
 ...
 *   ld-gj: soft deepen wave=15 areas=… so=… hit=… meth=…
...
/* Wave 15 soft inventory stamp + fixed greppable area count. */
#define GJ_LD_SOFT_WAVE   15u
...
/*
 * Wave 15 soft inventory tallies (diagnostics only; never hard-gate).
...
load_so_from_handoff(const struct gj_ld_handoff *pHo)
{
    ...
    g_cSoImg = cOk;
    /* Wave 15 soft inventory: remember hash table lamps across load. */
    g_cSoftSoHash = cHash;
    g_cSoftSoGnu = cGnu;
    if (cOk > 0) {
...
soft_inventory_log(void)
{
    ...
    /* Grep: ld-gj: soft deepen wave (Wave 15 stamp) */
    o = 0u;
    ...
    /* Grep: ld-gj: soft path (Wave 15 honesty; not bar3) */
    o = 0u;
    ...
    soft_append_s(aLine, sizeof(aLine), &o,
                  " resolve=sysv+gnu+scan bar3=0 (soft; not bar3)\n");
    aLine[o] = '\0';
    gj_puts(aLine);

    /* Grep: ld-gj: soft resolve (Wave 15 companion probe rollup) */
    o = 0u;
 ...
 * function exports + method-path markers + Wave 15 soft companions.
...
soft_resolve_probe(void)
{
    ...
    if (g_cSoImg == 0) {
        /* Wave 15 soft inventory still greppable with empty SO registry. */
        soft_inventory_log();
        return;
    }
    ...
    /* Wave 15 exclusive soft inventory rollup (greppable "ld-gj: soft …"). */
    soft_inventory_log();
}
...
_start(void)
{
    ...
    if (pHo->u64Magic == GJ_LD_HANDOFF_MAGIC) {
        fHandoff = 1;
        g_fSoftHandoff = 1u; /* Wave 15 soft inventory lamp */
