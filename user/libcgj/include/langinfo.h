 ...
 *
 * Soft residual (C2 libcgj langinfo.h; Soft!=product; G-AC-1; Dual DoD A/B OPEN):
 *   soft     = nl_langinfo + nl_item catalog for C locale only (soft fill OK)
 *   product  = UDX/DDI + hot/cold ABI hosts; not freestanding class; not .ko AC
 *   honesty  = C-locale langinfo != full glibc locale product; agent PASS != Dual
 *              DoD close; stamp-free bar honesty (no version stamp)
 *   law      = dual MIT OR Apache-2.0; no GPL; H2 no stamp storms
 * greppable: libcgj: langinfo soft residual Soft!=product
 */
#pragma once
