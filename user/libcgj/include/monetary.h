 ...
 *
 * Soft residual (C2 libcgj monetary.h; Soft!=product; G-AC-1; Dual DoD A/B OPEN):
 *   soft     = strfmon/strfmon_l integer/literal bring-up (no SSE doubles)
 *   product  = UDX/DDI + hot/cold ABI hosts; not freestanding class; not .ko AC
 *   honesty  = monetary soft != full locale money product; agent PASS != Dual DoD
 *              close; stamp-free bar honesty (no version stamp)
 *   law      = dual MIT OR Apache-2.0; no GPL; H2 no stamp storms
 * greppable: libcgj: monetary soft residual Soft!=product
 */
#pragma once
