 ...
 *
 * Soft residual (C2 libcgj mqueue.h; Soft!=product; G-AC-1; Dual DoD A/B OPEN):
 *   soft     = mqd_t + mq_open/close/send/receive/getattr/setattr/unlink;
 *              bring-up returns ENOSYS (kernel also ENOSYS until wired)
 *   product  = UDX/DDI + hot/cold ABI hosts; not freestanding class; not .ko AC
 *   honesty  = mq_* soft ENOSYS != product IPC completeness; agent PASS != Dual
 *              DoD close; stamp-free bar honesty (no version stamp)
 *   law      = dual MIT OR Apache-2.0; no GPL; H2 no stamp storms
 * greppable: libcgj: mqueue soft residual Soft!=product
 */
#pragma once
