...
"""One-shot CREATE-ONLY: graph_batch68001.c .. graph_batch70000.c via O_EXCL.
Pattern: graph_batch25850 simple + decade lamps (xx91-xx00) at each *00.
Matches high-range rewire style used for 66001..68000 (template from 25850/25891-25900).
"""
...
def main() -> int:
    ...
    for n in range(LO, HI + 1):
        ...
        if status == "created":
            ...
            exists += 1
    planned = HI - LO + 1
    line = f"created={created} already_existed={exists} planned={planned} range={LO}..{HI}"
    print(line)
    (OUT / "_emit_wave_70000_result.txt").write_text(
        line
