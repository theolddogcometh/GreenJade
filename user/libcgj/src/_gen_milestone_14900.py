...
"""
Soft continuum generator — MILESTONE 14900 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 14851–14900 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=14900   when parent has wired through graph_batch14900.c
...
graph batches ending at M. For M = **14900** the exclusive range is
**14851–14900** (50 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Identity          │ 14851–14875  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 14876–14890  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 14891–14900  │ decade soft gates probed by host     │
  │                   │ (10 TUs)     │ cgj_soft_milestone_14900.*           │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 14900 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_14900        1          graph_batch14891.c
  gj_libcgj_green_14900       1          graph_batch14892.c
  gj_bar3_ready_14900         0          graph_batch14893.c  (bar3 open)
  gj_product_score_14900      0          graph_batch14894.c  (score open)
  gj_continuum_ready_14900    1          graph_batch14895.c
  gj_smoke_soft_14900         1          graph_batch14896.c
  gj_dyn_soft_14900           1          graph_batch14897.c
  gj_milestone_tag_14900      14900      graph_batch14898.c
  gj_continuum_wave_14900     14900      graph_batch14899.c
  gj_batch_id_14900           14900      graph_batch14900.c
  gj_graph_milestone_14900    14900      graph_batch14900.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=14900`` means parent wired CGJ_SRCS through
    ``graph_batch14900.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_14900`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_14900`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M14900_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_14800`` / ``gj_*_14700`` / bare ``gj_batch_id`` surfaces.
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_14900.py

  # Restrict to marker band only (soft-gate TUs):
  python3 user/libcgj/src/_gen_milestone_14900.py --band markers

  # Bands: identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_14900.py --force
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 14900 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 14900
# Exclusive CREATE-ONLY range for this decade wave (50 TUs).
WAVE_LO = 14851
WAVE_HI = 14900  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
IDENTITY_LO, IDENTITY_HI = 14851, 14875
FILL_LO, FILL_HI = 14876, 14890
MARKERS_LO, MARKERS_HI = 14891, 14900

# Fill soft-unit stem per batch (unique symbol fragment before _u_N).
# Order is fixed to match the landed CREATE-ONLY continuum.
...
# Stems become gj_<stem>_14900. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (14891, "shell_green", 1, "shell product green extract (wave 14900)"),
    (14892, "libcgj_green", 1, "libcgj product green extract (wave 14900)"),
    (14893, "bar3_ready", 0, "product bar3 readiness gate (wave 14900)"),
    (14894, "product_score", 0, "product score lamp (wave 14900)"),
    (14895, "continuum_ready", 1, "continuum soft-ready lamp (wave 14900)"),
    (14896, "smoke_soft", 1, "static smoke soft lamp (wave 14900)"),
    (14897, "dyn_soft", 1, "dyn/dlopen soft lamp (wave 14900)"),
    (14898, "milestone_tag", MILESTONE, "milestone 14900 product tag"),
    (14899, "continuum_wave", MILESTONE, "continuum wave id tag (wave 14900)"),
    # 14900 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    # Special-case titles for a few stems to match landed comments.
    title_map = {
        14876: "continuum wave fill soft unit + batch id",
        14880: "continuum bridge soft unit + batch id",
        14890: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (14891–14899)."""
    assert MARKERS_LO <= n <= 14899
    mk = _marker_name(n)
    ms = _marker_str(n)
...
def render_milestone_capstone() -> str:
    """graph_batch14900.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    return f"""{HEADER_SPDX}\
 ...
 * (batches 14891-14899: shell_green_{n}, libcgj_green_{n},
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 14891–14899; 14900 is capstone
