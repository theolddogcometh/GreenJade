...
"""
Soft continuum generator — MILESTONE 15100 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 15051–15100 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=15100   when parent has wired through graph_batch15100.c
...
graph batches ending at M. For M = **15100** the exclusive range is
**15051–15100** (50 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Identity          │ 15051–15075  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 15076–15090  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 15091–15100  │ decade soft gates (Wave 9 exclusive) │
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 15100 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_15100        1          graph_batch15091.c
  gj_libcgj_green_15100       1          graph_batch15092.c
  gj_bar3_ready_15100         0          graph_batch15093.c  (bar3 open)
  gj_product_score_15100      0          graph_batch15094.c  (score open)
  gj_continuum_ready_15100    1          graph_batch15095.c
  gj_smoke_soft_15100         1          graph_batch15096.c
  gj_dyn_soft_15100           1          graph_batch15097.c
  gj_milestone_tag_15100      15100      graph_batch15098.c
  gj_continuum_wave_15100     15100      graph_batch15099.c
  gj_batch_id_15100           15100      graph_batch15100.c
  gj_graph_milestone_15100    15100      graph_batch15100.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=15100`` means parent wired CGJ_SRCS through
    ``graph_batch15100.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_15100`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_15100`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M15100_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_15000`` / ``gj_*_14900`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 9 exclusive CREATE-ONLY lands the **markers** band
  (15091–15100) first; identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_15100.py

  # Restrict to marker band only (soft-gate TUs; Wave 9 exclusive):
  python3 user/libcgj/src/_gen_milestone_15100.py --band markers

  # Bands: identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_15100.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_15000.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 15100 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 15100
# Exclusive CREATE-ONLY range for this decade wave (50 TUs).
WAVE_LO = 15051
WAVE_HI = 15100  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
IDENTITY_LO, IDENTITY_HI = 15051, 15075
FILL_LO, FILL_HI = 15076, 15090
MARKERS_LO, MARKERS_HI = 15091, 15100
...
# Stems become gj_<stem>_15100. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (15091, "shell_green", 1, "shell product green extract (wave 15100)"),
    (15092, "libcgj_green", 1, "libcgj product green extract (wave 15100)"),
    (15093, "bar3_ready", 0, "product bar3 readiness gate (wave 15100)"),
    (15094, "product_score", 0, "product readiness score tag (wave 15100)"),
    (15095, "continuum_ready", 1, "continuum readiness gate (wave 15100)"),
    (15096, "smoke_soft", 1, "smoke soft lamp (wave 15100)"),
    (15097, "dyn_soft", 1, "dyn soft lamp (wave 15100)"),
    (15098, "milestone_tag", MILESTONE, "milestone 15100 product tag"),
    (15099, "continuum_wave", MILESTONE, "milestone 15100 continuum wave identity"),
    # 15100 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        15076: "continuum wave fill soft unit + batch id",
        15080: "continuum bridge soft unit + batch id",
        15090: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (15091–15099), rich Wave-9 style."""
    assert MARKERS_LO <= n <= 15099
    mk = _marker_name(n)
    ms = _marker_str(n)
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_15000 / gj_shell_green_14900 / gj_shell_green_14800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_15000 / gj_libcgj_green_14900 / gj_libcgj_green_14800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_15000 / gj_bar3_ready_14900 / gj_bar3_ready_14800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_15000 / gj_product_score_14900 / gj_product_score_14800\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_15000 / gj_continuum_ready_14900 /\n"
            f" * gj_continuum_ready_14800 and sibling {MILESTONE} milestone symbols\n"
        ...
        band_note = (
            ...
            f" * (cgj_soft_milestone_{MILESTONE}.*) may dlsym / direct-link this lamp.\n"
            f" * Soft rule: missing symbol → skip; present but wrong value → hard fail.\n"
            f" * Soft continuum ready ≠ bar3 ready (bar3 stays 0 at batch15093).\n"
        )
        fn_doc = (
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_15000 / gj_smoke_soft_14900 / gj_smoke_soft_14800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_15000 / gj_dyn_soft_14900 / gj_dyn_soft_14800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_15000 / gj_milestone_tag_14900 / gj_milestone_tag_14800,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_15000 / gj_continuum_wave_14900 /\n"
            f" * gj_continuum_wave_14800, gj_milestone_tag_{MILESTONE} (batch15098), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch15100.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    return f"""{HEADER_SPDX}\
 ...
 * (batches 15091-15099: shell_green_{n}, libcgj_green_{n},
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 15091–15099; 15100 is capstone
