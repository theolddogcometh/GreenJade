...
"""
Soft continuum generator — MILESTONE 15200 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 15151–15200 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=15200   when parent has wired through graph_batch15200.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 15151 15200
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 15151 15200
...
graph batches ending at M. For M = **15200** the exclusive range is
**15151–15200** (50 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Identity          │ 15151–15175  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 15176–15190  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 15191–15200  │ decade soft gates (Wave 10 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 15200 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_15200        1          graph_batch15191.c
  gj_libcgj_green_15200       1          graph_batch15192.c
  gj_bar3_ready_15200         0          graph_batch15193.c  (bar3 open)
  gj_product_score_15200      0          graph_batch15194.c  (score open)
  gj_continuum_ready_15200    1          graph_batch15195.c
  gj_smoke_soft_15200         1          graph_batch15196.c
  gj_dyn_soft_15200           1          graph_batch15197.c
  gj_milestone_tag_15200      15200      graph_batch15198.c
  gj_continuum_wave_15200     15200      graph_batch15199.c
  gj_batch_id_15200           15200      graph_batch15200.c
  gj_graph_milestone_15200    15200      graph_batch15200.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=15200`` means parent wired CGJ_SRCS through
    ``graph_batch15200.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_15200`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_15200`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M15200_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_15100`` / ``gj_*_15000`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 10 exclusive CREATE-ONLY lands the **markers** band
  (15191–15200) first; identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_15200.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_15200.py

  # Restrict to marker band only (soft-gate TUs; Wave 10 exclusive):
  python3 user/libcgj/src/_gen_milestone_15200.py --band markers

  # Bands: identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_15200.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_15100.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 15200 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 15200
# Exclusive CREATE-ONLY range for this decade wave (50 TUs).
WAVE_LO = 15151
WAVE_HI = 15200  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
IDENTITY_LO, IDENTITY_HI = 15151, 15175
FILL_LO, FILL_HI = 15176, 15190
MARKERS_LO, MARKERS_HI = 15191, 15200
...
# Stems become gj_<stem>_15200. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (15191, "shell_green", 1, "shell product green extract (wave 15200)"),
    (15192, "libcgj_green", 1, "libcgj product green extract (wave 15200)"),
    (15193, "bar3_ready", 0, "product bar3 readiness gate (wave 15200)"),
    (15194, "product_score", 0, "product readiness score tag (wave 15200)"),
    (15195, "continuum_ready", 1, "continuum readiness gate (wave 15200)"),
    (15196, "smoke_soft", 1, "smoke soft lamp (wave 15200)"),
    (15197, "dyn_soft", 1, "dyn soft lamp (wave 15200)"),
    (15198, "milestone_tag", MILESTONE, "milestone 15200 product tag"),
    (15199, "continuum_wave", MILESTONE, "milestone 15200 continuum wave identity"),
    # 15200 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        15176: "continuum wave fill soft unit + batch id",
        15180: "continuum bridge soft unit + batch id",
        15190: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (15191–15199), rich Wave-10 style."""
    assert MARKERS_LO <= n <= 15199
    mk = _marker_name(n)
    ms = _marker_str(n)
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_15100 / gj_shell_green_15000 / gj_shell_green_14900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_15100 / gj_libcgj_green_15000 / gj_libcgj_green_14900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_15100 / gj_bar3_ready_15000 / gj_bar3_ready_14900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_15100 / gj_product_score_15000 / gj_product_score_14900\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_15100 / gj_continuum_ready_15000 /\n"
            f" * gj_continuum_ready_14900 and sibling {MILESTONE} milestone symbols\n"
        ...
        band_note = (
            ...
            f" * (cgj_soft_milestone_{MILESTONE}.*) may dlsym / direct-link this lamp.\n"
            f" * Soft rule: missing symbol → skip; present but wrong value → hard fail.\n"
            f" * Soft continuum ready ≠ bar3 ready (bar3 stays 0 at batch15193).\n"
        )
        fn_doc = (
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_15100 / gj_smoke_soft_15000 / gj_smoke_soft_14900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_15100 / gj_dyn_soft_15000 / gj_dyn_soft_14900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_15100 / gj_milestone_tag_15000 / gj_milestone_tag_14900,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_15100 / gj_continuum_wave_15000 /\n"
            f" * gj_continuum_wave_14900, gj_milestone_tag_{MILESTONE} (batch15198), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch15200.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    return f"""{HEADER_SPDX}\
 ...
 * (batches 15191-15199: shell_green_{n}, libcgj_green_{n},
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 15191–15199; 15200 is capstone
