...
"""
Soft continuum generator — MILESTONE 15400 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 15351–15400 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=15400   when parent has wired through graph_batch15400.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 15351 15400
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 15351 15400
...
graph batches ending at M. For M = **15400** the exclusive range is
**15351–15400** (50 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Identity          │ 15351–15375  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 15376–15390  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 15391–15400  │ decade soft gates (Wave 12 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 15400 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_15400        1          graph_batch15391.c
  gj_libcgj_green_15400       1          graph_batch15392.c
  gj_bar3_ready_15400         0          graph_batch15393.c  (bar3 open)
  gj_product_score_15400      0          graph_batch15394.c  (score open)
  gj_continuum_ready_15400    1          graph_batch15395.c
  gj_smoke_soft_15400         1          graph_batch15396.c
  gj_dyn_soft_15400           1          graph_batch15397.c
  gj_milestone_tag_15400      15400      graph_batch15398.c
  gj_continuum_wave_15400     15400      graph_batch15399.c
  gj_batch_id_15400           15400      graph_batch15400.c
  gj_graph_milestone_15400    15400      graph_batch15400.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=15400`` means parent wired CGJ_SRCS through
    ``graph_batch15400.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_15400`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_15400`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M15400_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_15300`` / ``gj_*_15200`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 12 exclusive CREATE-ONLY lands the **markers** band
  (15391–15400) first; identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_15400.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_15400.py

  # Restrict to marker band only (soft-gate TUs; Wave 12 exclusive):
  python3 user/libcgj/src/_gen_milestone_15400.py --band markers

  # Bands: identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_15400.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_15300.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 15400 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 15400
# Exclusive CREATE-ONLY range for this decade wave (50 TUs).
WAVE_LO = 15351
WAVE_HI = 15400  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
IDENTITY_LO, IDENTITY_HI = 15351, 15375
FILL_LO, FILL_HI = 15376, 15390
MARKERS_LO, MARKERS_HI = 15391, 15400
...
# Stems become gj_<stem>_15400. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (15391, "shell_green", 1, "shell product green extract (wave 15400)"),
    (15392, "libcgj_green", 1, "libcgj product green extract (wave 15400)"),
    (15393, "bar3_ready", 0, "product bar3 readiness gate (wave 15400)"),
    (15394, "product_score", 0, "product readiness score tag (wave 15400)"),
    (15395, "continuum_ready", 1, "continuum readiness gate (wave 15400)"),
    (15396, "smoke_soft", 1, "smoke soft lamp (wave 15400)"),
    (15397, "dyn_soft", 1, "dyn soft lamp (wave 15400)"),
    (15398, "milestone_tag", MILESTONE, "milestone 15400 product tag"),
    (15399, "continuum_wave", MILESTONE, "milestone 15400 continuum wave identity"),
    # 15400 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        15376: "continuum wave fill soft unit + batch id",
        15380: "continuum bridge soft unit + batch id",
        15390: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (15391–15399), rich Wave-12 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 15393
    tag_batch = MARKERS_LO + 7  # 15398
    wave_batch = MARKERS_LO + 8  # 15399
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_15300 / gj_shell_green_15200 / gj_shell_green_15100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_15300 / gj_libcgj_green_15200 / gj_libcgj_green_15100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_15300 / gj_bar3_ready_15200 / gj_bar3_ready_15100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_15300 / gj_product_score_15200 / gj_product_score_15100\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_15300 / gj_continuum_ready_15200 /\n"
            f" * gj_continuum_ready_15100 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_15300 / gj_smoke_soft_15200 / gj_smoke_soft_15100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_15300 / gj_dyn_soft_15200 / gj_dyn_soft_15100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_15300 / gj_milestone_tag_15200 / gj_milestone_tag_15100,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_15300 / gj_continuum_wave_15200 /\n"
            f" * gj_continuum_wave_15100, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch15400.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 15399
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 15391–15399; 15400 is capstone
