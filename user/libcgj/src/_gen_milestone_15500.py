...
"""
Soft continuum generator — MILESTONE 15500 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 15451–15500 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=15500   when parent has wired through graph_batch15500.c
...
graph batches ending at M. For M = **15500** the exclusive range is
**15451–15500** (50 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Identity          │ 15451–15475  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 15476–15490  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 15491–15500  │ decade soft gates (Wave 13 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 15500 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_15500        1          graph_batch15491.c
  gj_libcgj_green_15500       1          graph_batch15492.c
  gj_bar3_ready_15500         0          graph_batch15493.c  (bar3 open)
  gj_product_score_15500      0          graph_batch15494.c  (score open)
  gj_continuum_ready_15500    1          graph_batch15495.c
  gj_smoke_soft_15500         1          graph_batch15496.c
  gj_dyn_soft_15500           1          graph_batch15497.c
  gj_milestone_tag_15500      15500      graph_batch15498.c
  gj_continuum_wave_15500     15500      graph_batch15499.c
  gj_batch_id_15500           15500      graph_batch15500.c
  gj_graph_milestone_15500    15500      graph_batch15500.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=15500`` means parent wired CGJ_SRCS through
    ``graph_batch15500.c``. It is **soft graph growth only**.
  ...
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M15500_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_15400`` / ``gj_*_15300`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 13 exclusive CREATE-ONLY lands the **markers** band
  (15491–15500) first; identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_15500.py

  # Restrict to marker band only (soft-gate TUs; Wave 13 exclusive):
  python3 user/libcgj/src/_gen_milestone_15500.py --band markers

  # Bands: identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_15500.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_15400.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 15500 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 15500
# Exclusive CREATE-ONLY range for this decade wave (50 TUs).
WAVE_LO = 15451
WAVE_HI = 15500  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
IDENTITY_LO, IDENTITY_HI = 15451, 15475
FILL_LO, FILL_HI = 15476, 15490
MARKERS_LO, MARKERS_HI = 15491, 15500
...
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (15491, "shell_green", 1, "shell product green extract (wave 15500)"),
    (15492, "libcgj_green", 1, "libcgj product green extract (wave 15500)"),
    (15493, "bar3_ready", 0, "product bar3 readiness gate (wave 15500)"),
    (15494, "product_score", 0, "product readiness score tag (wave 15500)"),
    (15495, "continuum_ready", 1, "continuum readiness gate (wave 15500)"),
    (15496, "smoke_soft", 1, "smoke soft lamp (wave 15500)"),
    (15497, "dyn_soft", 1, "dyn soft lamp (wave 15500)"),
    (15498, "milestone_tag", MILESTONE, "milestone 15500 product tag"),
    (15499, "continuum_wave", MILESTONE, "milestone 15500 continuum wave identity"),
    # 15500 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        15476: "continuum wave fill soft unit + batch id",
        15480: "continuum bridge soft unit + batch id",
        15490: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (15491–15499), rich Wave-13 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 15493
    tag_batch = MARKERS_LO + 7  # 15498
    wave_batch = MARKERS_LO + 8  # 15499
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_15400 / gj_shell_green_15300 / gj_shell_green_15200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_15400 / gj_libcgj_green_15300 / gj_libcgj_green_15200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_15400 / gj_bar3_ready_15300 / gj_bar3_ready_15200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_15400 / gj_product_score_15300 / gj_product_score_15200\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_15400 / gj_smoke_soft_15300 / gj_smoke_soft_15200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_15400 / gj_dyn_soft_15300 / gj_dyn_soft_15200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_15400 / gj_milestone_tag_15300 / gj_milestone_tag_15200,\n"
...
def render_milestone_capstone() -> str:
    """graph_batch15500.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 15499
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 15491–15499; 15500 is capstone
