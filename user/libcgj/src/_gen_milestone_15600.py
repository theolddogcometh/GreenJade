...
"""
Soft continuum generator — MILESTONE 15600 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 15501–15600 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=15600   when parent has wired through graph_batch15600.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 15501 15600
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 15501 15600
...
graph batches ending at M. For M = **15600** the exclusive range is
**15501–15600** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 15501–15550  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 15551–15575  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 15576–15590  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 15591–15600  │ decade soft gates (Wave 14 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 15600 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_15600        1          graph_batch15591.c
  gj_libcgj_green_15600       1          graph_batch15592.c
  gj_bar3_ready_15600         0          graph_batch15593.c  (bar3 open)
  gj_product_score_15600      0          graph_batch15594.c  (score open)
  gj_continuum_ready_15600    1          graph_batch15595.c
  gj_smoke_soft_15600         1          graph_batch15596.c
  gj_dyn_soft_15600           1          graph_batch15597.c
  gj_milestone_tag_15600      15600      graph_batch15598.c
  gj_continuum_wave_15600     15600      graph_batch15599.c
  gj_batch_id_15600           15600      graph_batch15600.c
  gj_graph_milestone_15600    15600      graph_batch15600.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=15600`` means parent wired CGJ_SRCS through
    ``graph_batch15600.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_15500`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_15500`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M15600_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_15500`` / ``gj_*_15400`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 14 exclusive CREATE-ONLY lands the **markers** band
  (15591–15600) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_15600.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_15600.py

  # Restrict to marker band only (soft-gate TUs; Wave 14 exclusive):
  python3 user/libcgj/src/_gen_milestone_15600.py --band markers

  # Extension identity only (15501–15550):
  python3 user/libcgj/src/_gen_milestone_15600.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_15600.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_15500.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 15600 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 15600
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 15501
WAVE_HI = 15600  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 15501, 15550
IDENTITY_LO, IDENTITY_HI = 15551, 15575
FILL_LO, FILL_HI = 15576, 15590
MARKERS_LO, MARKERS_HI = 15591, 15600
...
# Stems become gj_<stem>_15500. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (15591, "shell_green", 1, "shell product green extract (wave 15600)"),
    (15592, "libcgj_green", 1, "libcgj product green extract (wave 15600)"),
    (15593, "bar3_ready", 0, "product bar3 readiness gate (wave 15600)"),
    (15594, "product_score", 0, "product readiness score tag (wave 15600)"),
    (15595, "continuum_ready", 1, "continuum readiness gate (wave 15600)"),
    (15596, "smoke_soft", 1, "smoke soft lamp (wave 15600)"),
    (15597, "dyn_soft", 1, "dyn soft lamp (wave 15600)"),
    (15598, "milestone_tag", MILESTONE, "milestone 15600 product tag"),
    (15599, "continuum_wave", MILESTONE, "milestone 15600 continuum wave identity"),
    # 15600 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        15576: "continuum wave fill soft unit + batch id",
        15580: "continuum bridge soft unit + batch id",
        15590: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (15591–15599), rich Wave-14 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 15593
    tag_batch = MARKERS_LO + 7  # 15598
    wave_batch = MARKERS_LO + 8  # 15599
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_15500 / gj_shell_green_15400 / gj_shell_green_15300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_15500 / gj_libcgj_green_15400 / gj_libcgj_green_15300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_15500 / gj_bar3_ready_15400 / gj_bar3_ready_15300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_15500 / gj_product_score_15400 / gj_product_score_15300\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_15400 / gj_continuum_ready_15300 /\n"
            f" * gj_continuum_ready_15200 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_15500 / gj_smoke_soft_15400 / gj_smoke_soft_15300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_15500 / gj_dyn_soft_15400 / gj_dyn_soft_15300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_15500 / gj_milestone_tag_15400 / gj_milestone_tag_15300,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_15400 / gj_continuum_wave_15300 /\n"
            f" * gj_continuum_wave_15200, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch15600.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 15599
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 15591–15599; 15600 is capstone
