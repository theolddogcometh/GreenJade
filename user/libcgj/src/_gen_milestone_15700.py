...
"""
Soft continuum generator — MILESTONE 15700 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 15601–15700 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=15700   when parent has wired through graph_batch15700.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 15601 15700
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 15601 15700
...
graph batches ending at M. For M = **15700** the exclusive range is
**15601–15700** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 15601–15650  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 15651–15675  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 15676–15690  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 15691–15700  │ decade soft gates (Wave 15 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 15700 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_15700        1          graph_batch15691.c
  gj_libcgj_green_15700       1          graph_batch15692.c
  gj_bar3_ready_15700         0          graph_batch15693.c  (bar3 open)
  gj_product_score_15700      0          graph_batch15694.c  (score open)
  gj_continuum_ready_15700    1          graph_batch15695.c
  gj_smoke_soft_15700         1          graph_batch15696.c
  gj_dyn_soft_15700           1          graph_batch15697.c
  gj_milestone_tag_15700      15700      graph_batch15698.c
  gj_continuum_wave_15700     15700      graph_batch15699.c
  gj_batch_id_15700           15700      graph_batch15700.c
  gj_graph_milestone_15700    15700      graph_batch15700.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=15700`` means parent wired CGJ_SRCS through
    ``graph_batch15700.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_15700`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_15700`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M15700_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_15600`` / ``gj_*_15500`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 15 exclusive CREATE-ONLY lands the **markers** band
  (15691–15700) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_15700.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_15700.py

  # Restrict to marker band only (soft-gate TUs; Wave 15 exclusive):
  python3 user/libcgj/src/_gen_milestone_15700.py --band markers

  # Extension identity only (15601–15650):
  python3 user/libcgj/src/_gen_milestone_15700.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_15700.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_15600.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 15700 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 15700
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 15601
WAVE_HI = 15700  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 15601, 15650
IDENTITY_LO, IDENTITY_HI = 15651, 15675
FILL_LO, FILL_HI = 15676, 15690
MARKERS_LO, MARKERS_HI = 15691, 15700
...
# Stems become gj_<stem>_15700. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (15691, "shell_green", 1, "shell product green extract (wave 15700)"),
    (15692, "libcgj_green", 1, "libcgj product green extract (wave 15700)"),
    (15693, "bar3_ready", 0, "product bar3 readiness gate (wave 15700)"),
    (15694, "product_score", 0, "product readiness score tag (wave 15700)"),
    (15695, "continuum_ready", 1, "continuum readiness gate (wave 15700)"),
    (15696, "smoke_soft", 1, "smoke soft lamp (wave 15700)"),
    (15697, "dyn_soft", 1, "dyn soft lamp (wave 15700)"),
    (15698, "milestone_tag", MILESTONE, "milestone 15700 product tag"),
    (15699, "continuum_wave", MILESTONE, "milestone 15700 continuum wave identity"),
    # 15700 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        15676: "continuum wave fill soft unit + batch id",
        15680: "continuum bridge soft unit + batch id",
        15690: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (15691–15699), rich Wave-15 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 15693
    tag_batch = MARKERS_LO + 7  # 15698
    wave_batch = MARKERS_LO + 8  # 15699
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_15600 / gj_shell_green_15500 / gj_shell_green_15400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_15600 / gj_libcgj_green_15500 / gj_libcgj_green_15400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_15600 / gj_bar3_ready_15500 / gj_bar3_ready_15400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_15600 / gj_product_score_15500 / gj_product_score_15400\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_15600 / gj_continuum_ready_15500 /\n"
            f" * gj_continuum_ready_15400 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_15600 / gj_smoke_soft_15500 / gj_smoke_soft_15400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_15600 / gj_dyn_soft_15500 / gj_dyn_soft_15400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_15600 / gj_milestone_tag_15500 / gj_milestone_tag_15400,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_15600 / gj_continuum_wave_15500 /\n"
            f" * gj_continuum_wave_15400, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch15700.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 15699
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 15691–15699; 15700 is capstone
