...
"""
Soft continuum generator — MILESTONE 15800 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 15701–15800 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=15800   when parent has wired through graph_batch15800.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 15701 15800
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 15701 15800
...
graph batches ending at M. For M = **15800** the exclusive range is
**15701–15800** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 15701–15750  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 15751–15775  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 15776–15790  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 15791–15800  │ decade soft gates (Wave 16 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 15800 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_15800        1          graph_batch15791.c
  gj_libcgj_green_15800       1          graph_batch15792.c
  gj_bar3_ready_15800         0          graph_batch15793.c  (bar3 open)
  gj_product_score_15800      0          graph_batch15794.c  (score open)
  gj_continuum_ready_15800    1          graph_batch15795.c
  gj_smoke_soft_15800         1          graph_batch15796.c
  gj_dyn_soft_15800           1          graph_batch15797.c
  gj_milestone_tag_15800      15800      graph_batch15798.c
  gj_continuum_wave_15800     15800      graph_batch15799.c
  gj_batch_id_15800           15800      graph_batch15800.c
  gj_graph_milestone_15800    15800      graph_batch15800.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=15800`` means parent wired CGJ_SRCS through
    ``graph_batch15800.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_15800`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_15800`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M15800_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_15700`` / ``gj_*_15600`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 16 exclusive CREATE-ONLY lands the **markers** band
  (15791–15800) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_15800.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_15800.py

  # Restrict to marker band only (soft-gate TUs; Wave 16 exclusive):
  python3 user/libcgj/src/_gen_milestone_15800.py --band markers

  # Extension identity only (15701–15750):
  python3 user/libcgj/src/_gen_milestone_15800.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_15800.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_15700.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 15800 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 15800
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 15701
WAVE_HI = 15800  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 15701, 15750
IDENTITY_LO, IDENTITY_HI = 15751, 15775
FILL_LO, FILL_HI = 15776, 15790
MARKERS_LO, MARKERS_HI = 15791, 15800
...
# Stems become gj_<stem>_15800. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (15791, "shell_green", 1, "shell product green extract (wave 15800)"),
    (15792, "libcgj_green", 1, "libcgj product green extract (wave 15800)"),
    (15793, "bar3_ready", 0, "product bar3 readiness gate (wave 15800)"),
    (15794, "product_score", 0, "product readiness score tag (wave 15800)"),
    (15795, "continuum_ready", 1, "continuum readiness gate (wave 15800)"),
    (15796, "smoke_soft", 1, "smoke soft lamp (wave 15800)"),
    (15797, "dyn_soft", 1, "dyn soft lamp (wave 15800)"),
    (15798, "milestone_tag", MILESTONE, "milestone 15800 product tag"),
    (15799, "continuum_wave", MILESTONE, "milestone 15800 continuum wave identity"),
    # 15800 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        15776: "continuum wave fill soft unit + batch id",
        15780: "continuum bridge soft unit + batch id",
        15790: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (15791–15799), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 15793
    tag_batch = MARKERS_LO + 7  # 15798
    wave_batch = MARKERS_LO + 8  # 15799
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_15700 / gj_shell_green_15600 / gj_shell_green_15500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_15700 / gj_libcgj_green_15600 / gj_libcgj_green_15500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_15700 / gj_bar3_ready_15600 / gj_bar3_ready_15500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_15700 / gj_product_score_15600 / gj_product_score_15500\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_15700 / gj_continuum_ready_15600 /\n"
            f" * gj_continuum_ready_15500 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_15700 / gj_smoke_soft_15600 / gj_smoke_soft_15500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_15700 / gj_dyn_soft_15600 / gj_dyn_soft_15500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_15700 / gj_milestone_tag_15600 / gj_milestone_tag_15500,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_15700 / gj_continuum_wave_15600 /\n"
            f" * gj_continuum_wave_15500, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch15800.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 15799
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 15791–15799; 15800 is capstone
