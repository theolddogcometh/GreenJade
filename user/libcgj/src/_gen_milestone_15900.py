...
"""
Soft continuum generator — MILESTONE 15900 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 15801–15900 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=15900   when parent has wired through graph_batch15900.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 15801 15900
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 15801 15900
...
**15801–15900** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 15801–15850  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 15851–15875  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 15876–15890  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 15891–15900  │ decade soft gates (Wave 17 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 15900 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_15900        1          graph_batch15891.c
  gj_libcgj_green_15900       1          graph_batch15892.c
  gj_bar3_ready_15900         0          graph_batch15893.c  (bar3 open)
  gj_product_score_15900      0          graph_batch15894.c  (score open)
  gj_continuum_ready_15900    1          graph_batch15895.c
  gj_smoke_soft_15900         1          graph_batch15896.c
  gj_dyn_soft_15900           1          graph_batch15897.c
  gj_milestone_tag_15900      15900      graph_batch15898.c
  gj_continuum_wave_15900     15900      graph_batch15899.c
  gj_batch_id_15900           15900      graph_batch15900.c
  gj_graph_milestone_15900    15900      graph_batch15900.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=15900`` means parent wired CGJ_SRCS through
    ``graph_batch15900.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_15900`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_15900`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M15900_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_15800`` / ``gj_*_15700`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 17 exclusive CREATE-ONLY lands the **markers** band
  (15891–15900) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_15900.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_15900.py

  # Restrict to marker band only (soft-gate TUs; Wave 17 exclusive):
  python3 user/libcgj/src/_gen_milestone_15900.py --band markers

  # Extension identity only (15801–15850):
  python3 user/libcgj/src/_gen_milestone_15900.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_15900.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_15800.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 15900 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 15900
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 15801
WAVE_HI = 15900  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 15801, 15850
IDENTITY_LO, IDENTITY_HI = 15851, 15875
FILL_LO, FILL_HI = 15876, 15890
MARKERS_LO, MARKERS_HI = 15891, 15900
...
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (15891, "shell_green", 1, "shell product green extract (wave 15900)"),
    (15892, "libcgj_green", 1, "libcgj product green extract (wave 15900)"),
    (15893, "bar3_ready", 0, "product bar3 readiness gate (wave 15900)"),
    (15894, "product_score", 0, "product readiness score tag (wave 15900)"),
    (15895, "continuum_ready", 1, "continuum readiness gate (wave 15900)"),
    (15896, "smoke_soft", 1, "smoke soft lamp (wave 15900)"),
    (15897, "dyn_soft", 1, "dyn soft lamp (wave 15900)"),
    (15898, "milestone_tag", MILESTONE, "milestone 15900 product tag"),
    (15899, "continuum_wave", MILESTONE, "milestone 15900 continuum wave identity"),
    # 15900 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        15876: "continuum wave fill soft unit + batch id",
        15880: "continuum bridge soft unit + batch id",
        15890: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (15891–15899), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 15893
    tag_batch = MARKERS_LO + 7  # 15898
    wave_batch = MARKERS_LO + 8  # 15899
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_15800 / gj_shell_green_15700 / gj_shell_green_15600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_15800 / gj_libcgj_green_15700 / gj_libcgj_green_15600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_15800 / gj_bar3_ready_15700 / gj_bar3_ready_15600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_15800 / gj_product_score_15700 / gj_product_score_15600\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_15800 / gj_continuum_ready_15700 /\n"
            f" * gj_continuum_ready_15600 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_15800 / gj_smoke_soft_15700 / gj_smoke_soft_15600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_15800 / gj_dyn_soft_15700 / gj_dyn_soft_15600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_15800 / gj_milestone_tag_15700 / gj_milestone_tag_15600,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_15800 / gj_continuum_wave_15700 /\n"
            f" * gj_continuum_wave_15600, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch15900.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 15899
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 15891–15899; 15800 is capstone
