...
"""
Soft continuum generator — MILESTONE 16000 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 15901–16000 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=16000   when parent has wired through graph_batch16000.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 15901 16000
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 15901 16000
...
graph batches ending at M. For M = **16000** the exclusive range is
**15901–16000** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 15901–15950  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 15951–15975  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 15976–15990  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 15991–16000  │ decade soft gates (Wave 18 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 16000 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_16000        1          graph_batch15991.c
  gj_libcgj_green_16000       1          graph_batch15992.c
  gj_bar3_ready_16000         0          graph_batch15993.c  (bar3 open)
  gj_product_score_16000      0          graph_batch15994.c  (score open)
  gj_continuum_ready_16000    1          graph_batch15995.c
  gj_smoke_soft_16000         1          graph_batch15996.c
  gj_dyn_soft_16000           1          graph_batch15997.c
  gj_milestone_tag_16000      16000      graph_batch15998.c
  gj_continuum_wave_16000     16000      graph_batch15999.c
  gj_batch_id_16000           16000      graph_batch16000.c
  gj_graph_milestone_16000    16000      graph_batch16000.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=16000`` means parent wired CGJ_SRCS through
    ``graph_batch16000.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_16000`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_16000`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M16000_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_15900`` / ``gj_*_15800`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 18 exclusive CREATE-ONLY lands the **markers** band
  (15991–16000) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_16000.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_16000.py

  # Restrict to marker band only (soft-gate TUs; Wave 18 exclusive):
  python3 user/libcgj/src/_gen_milestone_16000.py --band markers

  # Extension identity only (15901–15950):
  python3 user/libcgj/src/_gen_milestone_16000.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_16000.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_15900.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 16000 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 16000
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 15901
WAVE_HI = 16000  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 15901, 15950
IDENTITY_LO, IDENTITY_HI = 15951, 15975
FILL_LO, FILL_HI = 15976, 15990
MARKERS_LO, MARKERS_HI = 15991, 16000
...
# Stems become gj_<stem>_16000. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (15991, "shell_green", 1, "shell product green extract (wave 16000)"),
    (15992, "libcgj_green", 1, "libcgj product green extract (wave 16000)"),
    (15993, "bar3_ready", 0, "product bar3 readiness gate (wave 16000)"),
    (15994, "product_score", 0, "product readiness score tag (wave 16000)"),
    (15995, "continuum_ready", 1, "continuum readiness gate (wave 16000)"),
    (15996, "smoke_soft", 1, "smoke soft lamp (wave 16000)"),
    (15997, "dyn_soft", 1, "dyn soft lamp (wave 16000)"),
    (15998, "milestone_tag", MILESTONE, "milestone 16000 product tag"),
    (15999, "continuum_wave", MILESTONE, "milestone 16000 continuum wave identity"),
    # 16000 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        15976: "continuum wave fill soft unit + batch id",
        15980: "continuum bridge soft unit + batch id",
        15990: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (15991–15999), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 15993
    tag_batch = MARKERS_LO + 7  # 15998
    wave_batch = MARKERS_LO + 8  # 15999
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_15900 / gj_shell_green_15800 / gj_shell_green_15700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_15900 / gj_libcgj_green_15800 / gj_libcgj_green_15700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_15900 / gj_bar3_ready_15800 / gj_bar3_ready_15700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_15900 / gj_product_score_15800 / gj_product_score_15700\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_15900 / gj_continuum_ready_15800 /\n"
            f" * gj_continuum_ready_15700 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_15900 / gj_smoke_soft_15800 / gj_smoke_soft_15700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_15900 / gj_dyn_soft_15800 / gj_dyn_soft_15700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_15900 / gj_milestone_tag_15800 / gj_milestone_tag_15700,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_15900 / gj_continuum_wave_15800 /\n"
            f" * gj_continuum_wave_15700, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch16000.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 15999
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 15991–15999; 16000 is capstone
