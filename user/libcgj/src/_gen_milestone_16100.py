...
"""
Soft continuum generator — MILESTONE 16100 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 16001–16100 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=16000   when parent has wired through graph_batch16100.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 16001 16100
...
graph batches ending at M. For M = **16100** the exclusive range is
**16001–16100** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 16001–16050  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 16051–16075  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 16076–16090  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 16091–16100  │ decade soft gates (Wave 19 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 16100 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_16100        1          graph_batch16091.c
  gj_libcgj_green_16100       1          graph_batch16092.c
  gj_bar3_ready_16100         0          graph_batch16093.c  (bar3 open)
  gj_product_score_16100      0          graph_batch16094.c  (score open)
  gj_continuum_ready_16100    1          graph_batch16095.c
  gj_smoke_soft_16100         1          graph_batch16096.c
  gj_dyn_soft_16100           1          graph_batch16097.c
  gj_milestone_tag_16100      16000      graph_batch16098.c
  gj_continuum_wave_16100     16000      graph_batch16099.c
  gj_batch_id_16100           16000      graph_batch16100.c
  gj_graph_milestone_16100    16000      graph_batch16100.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=16000`` means parent wired CGJ_SRCS through
    ``graph_batch16100.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_16100`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_16100`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M16100_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_16000`` / ``gj_*_15900`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 19 exclusive CREATE-ONLY lands the **markers** band
  (16091–16100) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_16100.py

  # Restrict to marker band only (soft-gate TUs; Wave 19 exclusive):
  python3 user/libcgj/src/_gen_milestone_16100.py --band markers

  # Extension identity only (16001–16050):
  python3 user/libcgj/src/_gen_milestone_16100.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_16100.py --force
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 16100 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 16100
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 16001
WAVE_HI = 16100  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 16001, 16050
IDENTITY_LO, IDENTITY_HI = 16051, 16075
FILL_LO, FILL_HI = 16076, 16090
MARKERS_LO, MARKERS_HI = 16091, 16100
...
# Stems become gj_<stem>_16100. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (16091, "shell_green", 1, "shell product green extract (wave 16100)"),
    (16092, "libcgj_green", 1, "libcgj product green extract (wave 16100)"),
    (16093, "bar3_ready", 0, "product bar3 readiness gate (wave 16100)"),
    (16094, "product_score", 0, "product readiness score tag (wave 16100)"),
    (16095, "continuum_ready", 1, "continuum readiness gate (wave 16100)"),
    (16096, "smoke_soft", 1, "smoke soft lamp (wave 16100)"),
    (16097, "dyn_soft", 1, "dyn soft lamp (wave 16100)"),
    (16098, "milestone_tag", MILESTONE, "milestone 16100 product tag"),
    (16099, "continuum_wave", MILESTONE, "milestone 16100 continuum wave identity"),
    # 16100 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        16076: "continuum wave fill soft unit + batch id",
        16080: "continuum bridge soft unit + batch id",
        16090: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (16091–16099), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 16093
    tag_batch = MARKERS_LO + 7  # 16098
    wave_batch = MARKERS_LO + 8  # 16099
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_16000 / gj_shell_green_15900 / gj_shell_green_15800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_16000 / gj_libcgj_green_15900 / gj_libcgj_green_15800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_16000 / gj_bar3_ready_15900 / gj_bar3_ready_15800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_16000 / gj_product_score_15900 / gj_product_score_15800\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_16000 / gj_smoke_soft_15900 / gj_smoke_soft_15800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_16000 / gj_dyn_soft_15900 / gj_dyn_soft_15800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_16000 / gj_milestone_tag_15900 / gj_milestone_tag_15800,\n"
...
def render_milestone_capstone() -> str:
    """graph_batch16100.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 16099
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 16091–16099; 16000 is capstone
