...
"""
Soft continuum generator — MILESTONE 16200 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 16101–16200 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=16200   when parent has wired through graph_batch16200.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 16101 16200
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 16101 16200
...
graph batches ending at M. For M = **16200** the exclusive range is
**16101–16200** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 16101–16150  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 16151–16175  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 16176–16190  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 16191–16200  │ decade soft gates (Wave 20 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 16200 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_16200        1          graph_batch16191.c
  gj_libcgj_green_16200       1          graph_batch16192.c
  gj_bar3_ready_16200         0          graph_batch16193.c  (bar3 open)
  gj_product_score_16200      0          graph_batch16194.c  (score open)
  gj_continuum_ready_16200    1          graph_batch16195.c
  gj_smoke_soft_16200         1          graph_batch16196.c
  gj_dyn_soft_16200           1          graph_batch16197.c
  gj_milestone_tag_16200      16200      graph_batch16198.c
  gj_continuum_wave_16200     16200      graph_batch16199.c
  gj_batch_id_16200           16200      graph_batch16200.c
  gj_graph_milestone_16200    16200      graph_batch16200.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=16200`` means parent wired CGJ_SRCS through
    ``graph_batch16200.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_16200`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_16200`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M16200_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_16100`` / ``gj_*_16000`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 20 exclusive CREATE-ONLY lands the **markers** band
  (16191–16200) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_16200.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_16200.py

  # Restrict to marker band only (soft-gate TUs; Wave 20 exclusive):
  python3 user/libcgj/src/_gen_milestone_16200.py --band markers

  # Extension identity only (16101–16150):
  python3 user/libcgj/src/_gen_milestone_16200.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_16200.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_16100.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 16200 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 16200
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 16101
WAVE_HI = 16200  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 16101, 16150
IDENTITY_LO, IDENTITY_HI = 16151, 16175
FILL_LO, FILL_HI = 16176, 16190
MARKERS_LO, MARKERS_HI = 16191, 16200
...
# Stems become gj_<stem>_16200. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (16191, "shell_green", 1, "shell product green extract (wave 16200)"),
    (16192, "libcgj_green", 1, "libcgj product green extract (wave 16200)"),
    (16193, "bar3_ready", 0, "product bar3 readiness gate (wave 16200)"),
    (16194, "product_score", 0, "product readiness score tag (wave 16200)"),
    (16195, "continuum_ready", 1, "continuum readiness gate (wave 16200)"),
    (16196, "smoke_soft", 1, "smoke soft lamp (wave 16200)"),
    (16197, "dyn_soft", 1, "dyn soft lamp (wave 16200)"),
    (16198, "milestone_tag", MILESTONE, "milestone 16200 product tag"),
    (16199, "continuum_wave", MILESTONE, "milestone 16200 continuum wave identity"),
    # 16200 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        16176: "continuum wave fill soft unit + batch id",
        16180: "continuum bridge soft unit + batch id",
        16190: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (16191–16199), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 16193
    tag_batch = MARKERS_LO + 7  # 16198
    wave_batch = MARKERS_LO + 8  # 16199
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_16100 / gj_shell_green_16000 / gj_shell_green_15900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_16100 / gj_libcgj_green_16000 / gj_libcgj_green_15900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_16100 / gj_bar3_ready_16000 / gj_bar3_ready_15900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_16100 / gj_product_score_16000 / gj_product_score_15900\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_16000 / gj_continuum_ready_15900 /\n"
            f" * gj_continuum_ready_15800 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_16100 / gj_smoke_soft_16000 / gj_smoke_soft_15900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_16100 / gj_dyn_soft_16000 / gj_dyn_soft_15900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_16100 / gj_milestone_tag_16000 / gj_milestone_tag_15900,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_16000 / gj_continuum_wave_15900 /\n"
            f" * gj_continuum_wave_15800, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch16200.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 16199
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 16191–16199; 16100 is capstone
