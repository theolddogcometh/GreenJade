...
"""
Soft continuum generator — MILESTONE 16300 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 16201–16300 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=16300   when parent has wired through graph_batch16300.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 16201 16300
...
graph batches ending at M. For M = **16300** the exclusive range is
**16201–16300** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 16201–16250  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 16251–16275  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 16276–16290  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 16291–16300  │ decade soft gates (Wave 21 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 16300 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_16300        1          graph_batch16291.c
  gj_libcgj_green_16300       1          graph_batch16292.c
  gj_bar3_ready_16300         0          graph_batch16293.c  (bar3 open)
  gj_product_score_16300      0          graph_batch16294.c  (score open)
  gj_continuum_ready_16300    1          graph_batch16295.c
  gj_smoke_soft_16300         1          graph_batch16296.c
  gj_dyn_soft_16300           1          graph_batch16297.c
  gj_milestone_tag_16300      16300      graph_batch16298.c
  gj_continuum_wave_16300     16300      graph_batch16299.c
  gj_batch_id_16300           16300      graph_batch16300.c
  gj_graph_milestone_16300    16300      graph_batch16300.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=16300`` means parent wired CGJ_SRCS through
    ``graph_batch16300.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_16300`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_16300`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M16300_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_16200`` / ``gj_*_16100`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 21 exclusive CREATE-ONLY lands the **markers** band
  (16291–16300) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_16300.py

  # Restrict to marker band only (soft-gate TUs; Wave 21 exclusive):
  python3 user/libcgj/src/_gen_milestone_16300.py --band markers

  # Extension identity only (16201–16250):
  python3 user/libcgj/src/_gen_milestone_16300.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_16300.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_16200.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 16300 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 16300
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 16201
WAVE_HI = 16300  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 16201, 16250
IDENTITY_LO, IDENTITY_HI = 16251, 16275
FILL_LO, FILL_HI = 16276, 16290
MARKERS_LO, MARKERS_HI = 16291, 16300
...
# Stems become gj_<stem>_16300. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (16291, "shell_green", 1, "shell product green extract (wave 16300)"),
    (16292, "libcgj_green", 1, "libcgj product green extract (wave 16300)"),
    (16293, "bar3_ready", 0, "product bar3 readiness gate (wave 16300)"),
    (16294, "product_score", 0, "product readiness score tag (wave 16300)"),
    (16295, "continuum_ready", 1, "continuum readiness gate (wave 16300)"),
    (16296, "smoke_soft", 1, "smoke soft lamp (wave 16300)"),
    (16297, "dyn_soft", 1, "dyn soft lamp (wave 16300)"),
    (16298, "milestone_tag", MILESTONE, "milestone 16300 product tag"),
    (16299, "continuum_wave", MILESTONE, "milestone 16300 continuum wave identity"),
    # 16300 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        16276: "continuum wave fill soft unit + batch id",
        16280: "continuum bridge soft unit + batch id",
        16290: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (16291–16299), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 16293
    tag_batch = MARKERS_LO + 7  # 16298
    wave_batch = MARKERS_LO + 8  # 16299
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_16200 / gj_shell_green_16100 / gj_shell_green_16000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_16200 / gj_libcgj_green_16100 / gj_libcgj_green_16000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_16200 / gj_bar3_ready_16100 / gj_bar3_ready_16000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_16200 / gj_product_score_16100 / gj_product_score_16000\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_16200 / gj_continuum_ready_16100 /\n"
            f" * gj_continuum_ready_16000 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_16200 / gj_smoke_soft_16100 / gj_smoke_soft_16000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_16200 / gj_dyn_soft_16100 / gj_dyn_soft_16000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_16200 / gj_milestone_tag_16100 / gj_milestone_tag_16000,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_16200 / gj_continuum_wave_16100 /\n"
            f" * gj_continuum_wave_16000, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch16300.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 16299
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 16291–16299; 16300 is capstone
