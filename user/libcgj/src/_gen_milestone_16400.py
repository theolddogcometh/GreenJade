...
"""
Soft continuum generator — MILESTONE 16400 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 16301–16400 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=16400   when parent has wired through graph_batch16400.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 16301 16400
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 16301 16400
...
graph batches ending at M. For M = **16400** the exclusive range is
**16301–16400** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 16301–16350  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 16351–16375  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 16376–16390  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 16391–16400  │ decade soft gates (Wave 22 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 16400 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_16400        1          graph_batch16391.c
  gj_libcgj_green_16400       1          graph_batch16392.c
  gj_bar3_ready_16400         0          graph_batch16393.c  (bar3 open)
  gj_product_score_16400      0          graph_batch16394.c  (score open)
  gj_continuum_ready_16400    1          graph_batch16395.c
  gj_smoke_soft_16400         1          graph_batch16396.c
  gj_dyn_soft_16400           1          graph_batch16397.c
  gj_milestone_tag_16400      16400      graph_batch16398.c
  gj_continuum_wave_16400     16400      graph_batch16399.c
  gj_batch_id_16400           16400      graph_batch16400.c
  gj_graph_milestone_16400    16400      graph_batch16400.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=16400`` means parent wired CGJ_SRCS through
    ``graph_batch16400.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_16400`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_16400`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M16400_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_16300`` / ``gj_*_16200`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 22 exclusive CREATE-ONLY lands the **markers** band
  (16391–16400) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_16400.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_16400.py

  # Restrict to marker band only (soft-gate TUs; Wave 22 exclusive):
  python3 user/libcgj/src/_gen_milestone_16400.py --band markers

  # Extension identity only (16301–16350):
  python3 user/libcgj/src/_gen_milestone_16400.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_16400.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_16300.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 16400 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 16400
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 16301
WAVE_HI = 16400  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 16301, 16350
IDENTITY_LO, IDENTITY_HI = 16351, 16375
FILL_LO, FILL_HI = 16376, 16390
MARKERS_LO, MARKERS_HI = 16391, 16400
...
# Stems become gj_<stem>_16400. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (16391, "shell_green", 1, "shell product green extract (wave 16400)"),
    (16392, "libcgj_green", 1, "libcgj product green extract (wave 16400)"),
    (16393, "bar3_ready", 0, "product bar3 readiness gate (wave 16400)"),
    (16394, "product_score", 0, "product readiness score tag (wave 16400)"),
    (16395, "continuum_ready", 1, "continuum readiness gate (wave 16400)"),
    (16396, "smoke_soft", 1, "smoke soft lamp (wave 16400)"),
    (16397, "dyn_soft", 1, "dyn soft lamp (wave 16400)"),
    (16398, "milestone_tag", MILESTONE, "milestone 16400 product tag"),
    (16399, "continuum_wave", MILESTONE, "milestone 16400 continuum wave identity"),
    # 16400 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        16376: "continuum wave fill soft unit + batch id",
        16380: "continuum bridge soft unit + batch id",
        16390: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (16391–16399), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 16393
    tag_batch = MARKERS_LO + 7  # 16398
    wave_batch = MARKERS_LO + 8  # 16399
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_16300 / gj_shell_green_16200 / gj_shell_green_16100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_16300 / gj_libcgj_green_16200 / gj_libcgj_green_16100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_16300 / gj_bar3_ready_16200 / gj_bar3_ready_16100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_16300 / gj_product_score_16200 / gj_product_score_16100\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_16300 / gj_continuum_ready_16200 /\n"
            f" * gj_continuum_ready_16100 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_16300 / gj_smoke_soft_16200 / gj_smoke_soft_16100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_16300 / gj_dyn_soft_16200 / gj_dyn_soft_16100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_16300 / gj_milestone_tag_16200 / gj_milestone_tag_16100,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_16300 / gj_continuum_wave_16200 /\n"
            f" * gj_continuum_wave_16100, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch16400.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 16399
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 16391–16399; 16400 is capstone
