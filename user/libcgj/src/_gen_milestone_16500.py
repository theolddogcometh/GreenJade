...
"""
Soft continuum generator — MILESTONE 16500 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 16401–16500 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=16500   when parent has wired through graph_batch16500.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 16401 16500
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 16401 16500
...
**16401–16500** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 16401–16450  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 16451–16475  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 16476–16490  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 16491–16500  │ decade soft gates (Wave 23 exclusive)│
...
----------------------------------------------------------
  ...
  --------------------------  ---------  --------------------
  gj_shell_green_16500        1          graph_batch16491.c
  gj_libcgj_green_16500       1          graph_batch16492.c
  gj_bar3_ready_16500         0          graph_batch16493.c  (bar3 open)
  gj_product_score_16500      0          graph_batch16494.c  (score open)
  gj_continuum_ready_16500    1          graph_batch16495.c
  gj_smoke_soft_16500         1          graph_batch16496.c
  gj_dyn_soft_16500           1          graph_batch16497.c
  gj_milestone_tag_16500      16400      graph_batch16498.c
  gj_continuum_wave_16500     16400      graph_batch16499.c
  gj_batch_id_16500           16400      graph_batch16500.c
  gj_graph_milestone_16500    16400      graph_batch16500.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=16500`` means parent wired CGJ_SRCS through
    ``graph_batch16500.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_16500`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_16500`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M16500_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_16400`` / ``gj_*_16300`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 23 exclusive CREATE-ONLY lands the **markers** band
  (16491–16500) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_16500.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_16500.py

  # Restrict to marker band only (soft-gate TUs; Wave 23 exclusive):
  python3 user/libcgj/src/_gen_milestone_16500.py --band markers

  # Extension identity only (16401–16450):
  python3 user/libcgj/src/_gen_milestone_16500.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_16500.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_16400.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 16500 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 16500
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 16401
WAVE_HI = 16500  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 16401, 16450
IDENTITY_LO, IDENTITY_HI = 16451, 16475
FILL_LO, FILL_HI = 16476, 16490
MARKERS_LO, MARKERS_HI = 16491, 16500
...
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (16491, "shell_green", 1, "shell product green extract (wave 16500)"),
    (16492, "libcgj_green", 1, "libcgj product green extract (wave 16500)"),
    (16493, "bar3_ready", 0, "product bar3 readiness gate (wave 16500)"),
    (16494, "product_score", 0, "product readiness score tag (wave 16500)"),
    (16495, "continuum_ready", 1, "continuum readiness gate (wave 16500)"),
    (16496, "smoke_soft", 1, "smoke soft lamp (wave 16500)"),
    (16497, "dyn_soft", 1, "dyn soft lamp (wave 16500)"),
    (16498, "milestone_tag", MILESTONE, "milestone 16500 product tag"),
    (16499, "continuum_wave", MILESTONE, "milestone 16500 continuum wave identity"),
    # 16500 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        16476: "continuum wave fill soft unit + batch id",
        16480: "continuum bridge soft unit + batch id",
        16490: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (16491–16499), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 16493
    tag_batch = MARKERS_LO + 7  # 16498
    wave_batch = MARKERS_LO + 8  # 16499
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_16500 / gj_shell_green_16300 / gj_shell_green_16200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_16500 / gj_libcgj_green_16300 / gj_libcgj_green_16200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_16500 / gj_bar3_ready_16300 / gj_bar3_ready_16200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_16500 / gj_product_score_16300 / gj_product_score_16200\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_16500 / gj_smoke_soft_16300 / gj_smoke_soft_16200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_16500 / gj_dyn_soft_16300 / gj_dyn_soft_16200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_16500 / gj_milestone_tag_16300 / gj_milestone_tag_16200,\n"
...
def render_milestone_capstone() -> str:
    """graph_batch16500.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 16499
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 16491–16499; 16400 is capstone
