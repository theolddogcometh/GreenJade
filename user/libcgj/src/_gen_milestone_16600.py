...
"""
Soft continuum generator — MILESTONE 16600 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 16501–16600 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=16600   when parent has wired through graph_batch16600.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 16501 16600
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 16501 16600
...
graph batches ending at M. For M = **16600** the exclusive range is
**16501–16600** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 16501–16550  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 16551–16575  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 16576–16590  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 16591–16600  │ decade soft gates (Wave 24 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 16600 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_16600        1          graph_batch16591.c
  gj_libcgj_green_16600       1          graph_batch16592.c
  gj_bar3_ready_16600         0          graph_batch16593.c  (bar3 open)
  gj_product_score_16600      0          graph_batch16594.c  (score open)
  gj_continuum_ready_16600    1          graph_batch16595.c
  gj_smoke_soft_16600         1          graph_batch16596.c
  gj_dyn_soft_16600           1          graph_batch16597.c
  gj_milestone_tag_16600      16600      graph_batch16598.c
  gj_continuum_wave_16600     16600      graph_batch16599.c
  gj_batch_id_16600           16600      graph_batch16600.c
  gj_graph_milestone_16600    16600      graph_batch16600.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=16600`` means parent wired CGJ_SRCS through
    ``graph_batch16600.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_16600`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_16600`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M16600_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_16500`` / ``gj_*_16400`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 24 exclusive CREATE-ONLY lands the **markers** band
  (16591–16600) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_16600.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_16600.py

  # Restrict to marker band only (soft-gate TUs; Wave 24 exclusive):
  python3 user/libcgj/src/_gen_milestone_16600.py --band markers

  # Extension identity only (16501–16550):
  python3 user/libcgj/src/_gen_milestone_16600.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_16600.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_16500.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 16600 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 16600
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 16501
WAVE_HI = 16600  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 16501, 16550
IDENTITY_LO, IDENTITY_HI = 16551, 16575
FILL_LO, FILL_HI = 16576, 16590
MARKERS_LO, MARKERS_HI = 16591, 16600
...
# Stems become gj_<stem>_16600. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (16591, "shell_green", 1, "shell product green extract (wave 16600)"),
    (16592, "libcgj_green", 1, "libcgj product green extract (wave 16600)"),
    (16593, "bar3_ready", 0, "product bar3 readiness gate (wave 16600)"),
    (16594, "product_score", 0, "product readiness score tag (wave 16600)"),
    (16595, "continuum_ready", 1, "continuum readiness gate (wave 16600)"),
    (16596, "smoke_soft", 1, "smoke soft lamp (wave 16600)"),
    (16597, "dyn_soft", 1, "dyn soft lamp (wave 16600)"),
    (16598, "milestone_tag", MILESTONE, "milestone 16600 product tag"),
    (16599, "continuum_wave", MILESTONE, "milestone 16600 continuum wave identity"),
    # 16600 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        16576: "continuum wave fill soft unit + batch id",
        16580: "continuum bridge soft unit + batch id",
        16590: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (16591–16599), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 16593
    tag_batch = MARKERS_LO + 7  # 16598
    wave_batch = MARKERS_LO + 8  # 16599
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_16500 / gj_shell_green_16400 / gj_shell_green_16300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_16500 / gj_libcgj_green_16400 / gj_libcgj_green_16300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_16500 / gj_bar3_ready_16400 / gj_bar3_ready_16300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_16500 / gj_product_score_16400 / gj_product_score_16300\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_16500 / gj_continuum_ready_16400 /\n"
            f" * gj_continuum_ready_16300 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_16500 / gj_smoke_soft_16400 / gj_smoke_soft_16300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_16500 / gj_dyn_soft_16400 / gj_dyn_soft_16300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_16500 / gj_milestone_tag_16400 / gj_milestone_tag_16300,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_16500 / gj_continuum_wave_16400 /\n"
            f" * gj_continuum_wave_16300, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch16600.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 16599
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 16591–16599; 16600 is capstone
