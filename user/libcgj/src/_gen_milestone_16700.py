...
"""
Soft continuum generator — MILESTONE 16700 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 16601–16700 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=16700   when parent has wired through graph_batch16700.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 16601 16700
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 16601 16700
...
graph batches ending at M. For M = **16700** the exclusive range is
**16601–16700** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 16601–16650  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 16651–16675  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 16676–16690  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 16691–16700  │ decade soft gates (Wave 25 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 16700 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_16700        1          graph_batch16691.c
  gj_libcgj_green_16700       1          graph_batch16692.c
  gj_bar3_ready_16700         0          graph_batch16693.c  (bar3 open)
  gj_product_score_16700      0          graph_batch16694.c  (score open)
  gj_continuum_ready_16700    1          graph_batch16695.c
  gj_smoke_soft_16700         1          graph_batch16696.c
  gj_dyn_soft_16700           1          graph_batch16697.c
  gj_milestone_tag_16700      16700      graph_batch16698.c
  gj_continuum_wave_16700     16700      graph_batch16699.c
  gj_batch_id_16700           16700      graph_batch16700.c
  gj_graph_milestone_16700    16700      graph_batch16700.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=16700`` means parent wired CGJ_SRCS through
    ``graph_batch16700.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_16700`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_16700`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M16700_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_16600`` / ``gj_*_16500`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 25 exclusive CREATE-ONLY lands the **markers** band
  (16691–16700) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_16700.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_16700.py

  # Restrict to marker band only (soft-gate TUs; Wave 25 exclusive):
  python3 user/libcgj/src/_gen_milestone_16700.py --band markers

  # Extension identity only (16601–16650):
  python3 user/libcgj/src/_gen_milestone_16700.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_16700.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_16600.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 16700 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 16700
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 16601
WAVE_HI = 16700  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 16601, 16650
IDENTITY_LO, IDENTITY_HI = 16651, 16675
FILL_LO, FILL_HI = 16676, 16690
MARKERS_LO, MARKERS_HI = 16691, 16700
...
# Stems become gj_<stem>_16700. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (16691, "shell_green", 1, "shell product green extract (wave 16700)"),
    (16692, "libcgj_green", 1, "libcgj product green extract (wave 16700)"),
    (16693, "bar3_ready", 0, "product bar3 readiness gate (wave 16700)"),
    (16694, "product_score", 0, "product readiness score tag (wave 16700)"),
    (16695, "continuum_ready", 1, "continuum readiness gate (wave 16700)"),
    (16696, "smoke_soft", 1, "smoke soft lamp (wave 16700)"),
    (16697, "dyn_soft", 1, "dyn soft lamp (wave 16700)"),
    (16698, "milestone_tag", MILESTONE, "milestone 16700 product tag"),
    (16699, "continuum_wave", MILESTONE, "milestone 16700 continuum wave identity"),
    # 16700 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        16676: "continuum wave fill soft unit + batch id",
        16680: "continuum bridge soft unit + batch id",
        16690: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (16691–16699), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 16693
    tag_batch = MARKERS_LO + 7  # 16698
    wave_batch = MARKERS_LO + 8  # 16699
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_16600 / gj_shell_green_16500 / gj_shell_green_16400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_16600 / gj_libcgj_green_16500 / gj_libcgj_green_16400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_16600 / gj_bar3_ready_16500 / gj_bar3_ready_16400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_16600 / gj_product_score_16500 / gj_product_score_16400\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_16600 / gj_continuum_ready_16500 /\n"
            f" * gj_continuum_ready_16400 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_16600 / gj_smoke_soft_16500 / gj_smoke_soft_16400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_16600 / gj_dyn_soft_16500 / gj_dyn_soft_16400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_16600 / gj_milestone_tag_16500 / gj_milestone_tag_16400,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_16600 / gj_continuum_wave_16500 /\n"
            f" * gj_continuum_wave_16400, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch16700.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 16699
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 16691–16699; 16700 is capstone
