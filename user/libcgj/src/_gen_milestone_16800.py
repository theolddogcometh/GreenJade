...
"""
Soft continuum generator — MILESTONE 16800 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 16701–16800 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=16800   when parent has wired through graph_batch16800.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 16701 16800
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 16701 16800
...
graph batches ending at M. For M = **16800** the exclusive range is
**16701–16800** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 16701–16750  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 16751–16775  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 16776–16790  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 16791–16800  │ decade soft gates (Wave 26 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 16800 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_16800        1          graph_batch16791.c
  gj_libcgj_green_16800       1          graph_batch16792.c
  gj_bar3_ready_16800         0          graph_batch16793.c  (bar3 open)
  gj_product_score_16800      0          graph_batch16794.c  (score open)
  gj_continuum_ready_16800    1          graph_batch16795.c
  gj_smoke_soft_16800         1          graph_batch16796.c
  gj_dyn_soft_16800           1          graph_batch16797.c
  gj_milestone_tag_16800      16800      graph_batch16798.c
  gj_continuum_wave_16800     16800      graph_batch16799.c
  gj_batch_id_16800           16800      graph_batch16800.c
  gj_graph_milestone_16800    16800      graph_batch16800.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=16800`` means parent wired CGJ_SRCS through
    ``graph_batch16800.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_16800`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_16800`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M16800_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_16700`` / ``gj_*_16600`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 26 exclusive CREATE-ONLY lands the **markers** band
  (16791–16800) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_16800.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_16800.py

  # Restrict to marker band only (soft-gate TUs; Wave 26 exclusive):
  python3 user/libcgj/src/_gen_milestone_16800.py --band markers

  # Extension identity only (16701–16750):
  python3 user/libcgj/src/_gen_milestone_16800.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_16800.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_16700.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 16800 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 16800
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 16701
WAVE_HI = 16800  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 16701, 16750
IDENTITY_LO, IDENTITY_HI = 16751, 16775
FILL_LO, FILL_HI = 16776, 16790
MARKERS_LO, MARKERS_HI = 16791, 16800
...
# Stems become gj_<stem>_16800. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (16791, "shell_green", 1, "shell product green extract (wave 16800)"),
    (16792, "libcgj_green", 1, "libcgj product green extract (wave 16800)"),
    (16793, "bar3_ready", 0, "product bar3 readiness gate (wave 16800)"),
    (16794, "product_score", 0, "product readiness score tag (wave 16800)"),
    (16795, "continuum_ready", 1, "continuum readiness gate (wave 16800)"),
    (16796, "smoke_soft", 1, "smoke soft lamp (wave 16800)"),
    (16797, "dyn_soft", 1, "dyn soft lamp (wave 16800)"),
    (16798, "milestone_tag", MILESTONE, "milestone 16800 product tag"),
    (16799, "continuum_wave", MILESTONE, "milestone 16800 continuum wave identity"),
    # 16800 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        16776: "continuum wave fill soft unit + batch id",
        16780: "continuum bridge soft unit + batch id",
        16790: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (16791–16799), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 16793
    tag_batch = MARKERS_LO + 7  # 16798
    wave_batch = MARKERS_LO + 8  # 16799
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_16700 / gj_shell_green_16600 / gj_shell_green_16500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_16700 / gj_libcgj_green_16600 / gj_libcgj_green_16500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_16700 / gj_bar3_ready_16600 / gj_bar3_ready_16500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_16700 / gj_product_score_16600 / gj_product_score_16500\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_16700 / gj_continuum_ready_16600 /\n"
            f" * gj_continuum_ready_16500 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_16700 / gj_smoke_soft_16600 / gj_smoke_soft_16500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_16700 / gj_dyn_soft_16600 / gj_dyn_soft_16500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_16700 / gj_milestone_tag_16600 / gj_milestone_tag_16500,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_16700 / gj_continuum_wave_16600 /\n"
            f" * gj_continuum_wave_16500, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch16800.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 16799
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 16791–16799; 16800 is capstone
