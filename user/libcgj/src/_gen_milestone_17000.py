...
"""
Soft continuum generator — MILESTONE 17000 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 16901–17000 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=17000   when parent has wired through graph_batch17000.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 16901 17000
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 16901 17000
...
graph batches ending at M. For M = **17000** the exclusive range is
**16901–17000** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 16901–16950  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 16951–16975  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 16976–16990  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 16991–17000  │ decade soft gates (Wave 28 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 17000 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_17000        1          graph_batch16991.c
  gj_libcgj_green_17000       1          graph_batch16992.c
  gj_bar3_ready_17000         0          graph_batch16993.c  (bar3 open)
  gj_product_score_17000      0          graph_batch16994.c  (score open)
  gj_continuum_ready_17000    1          graph_batch16995.c
  gj_smoke_soft_17000         1          graph_batch16996.c
  gj_dyn_soft_17000           1          graph_batch16997.c
  gj_milestone_tag_17000      17000      graph_batch16998.c
  gj_continuum_wave_17000     17000      graph_batch16999.c
  gj_batch_id_17000           17000      graph_batch17000.c
  gj_graph_milestone_17000    17000      graph_batch17000.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=17000`` means parent wired CGJ_SRCS through
    ``graph_batch17000.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_17000`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_17000`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M17000_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_16900`` / ``gj_*_16800`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 28 exclusive CREATE-ONLY lands the **markers** band
  (16991–17000) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_17000.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_17000.py

  # Restrict to marker band only (soft-gate TUs; Wave 28 exclusive):
  python3 user/libcgj/src/_gen_milestone_17000.py --band markers

  # Extension identity only (16901–16950):
  python3 user/libcgj/src/_gen_milestone_17000.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_17000.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_16900.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 17000 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 17000
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 16901
WAVE_HI = 17000  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 16901, 16950
IDENTITY_LO, IDENTITY_HI = 16951, 16975
FILL_LO, FILL_HI = 16976, 16990
MARKERS_LO, MARKERS_HI = 16991, 17000
...
# Stems become gj_<stem>_17000. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (16991, "shell_green", 1, "shell product green extract (wave 17000)"),
    (16992, "libcgj_green", 1, "libcgj product green extract (wave 17000)"),
    (16993, "bar3_ready", 0, "product bar3 readiness gate (wave 17000)"),
    (16994, "product_score", 0, "product readiness score tag (wave 17000)"),
    (16995, "continuum_ready", 1, "continuum readiness gate (wave 17000)"),
    (16996, "smoke_soft", 1, "smoke soft lamp (wave 17000)"),
    (16997, "dyn_soft", 1, "dyn soft lamp (wave 17000)"),
    (16998, "milestone_tag", MILESTONE, "milestone 17000 product tag"),
    (16999, "continuum_wave", MILESTONE, "milestone 17000 continuum wave identity"),
    # 17000 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        16976: "continuum wave fill soft unit + batch id",
        16980: "continuum bridge soft unit + batch id",
        16990: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (16991–16999), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 16993
    tag_batch = MARKERS_LO + 7  # 16998
    wave_batch = MARKERS_LO + 8  # 16999
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_16900 / gj_shell_green_16800 / gj_shell_green_16700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_16900 / gj_libcgj_green_16800 / gj_libcgj_green_16700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_16900 / gj_bar3_ready_16800 / gj_bar3_ready_16700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_16900 / gj_product_score_16800 / gj_product_score_16700\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_16900 / gj_continuum_ready_16800 /\n"
            f" * gj_continuum_ready_16700 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_16900 / gj_smoke_soft_16800 / gj_smoke_soft_16700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_16900 / gj_dyn_soft_16800 / gj_dyn_soft_16700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_16900 / gj_milestone_tag_16800 / gj_milestone_tag_16700,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_16900 / gj_continuum_wave_16800 /\n"
            f" * gj_continuum_wave_16700, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch17000.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 16999
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 16991–16999; 17000 is capstone
