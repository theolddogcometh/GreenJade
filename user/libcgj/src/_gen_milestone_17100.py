...
"""
Soft continuum generator — MILESTONE 17100 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 17001–17100 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=17100   when parent has wired through graph_batch17100.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 17001 17100
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 17001 17100
...
**17001–17100** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 17001–17050  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 17051–17075  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 17076–17090  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 17091–17100  │ decade soft gates (Wave 29 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 17100 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_17100        1          graph_batch17091.c
  gj_libcgj_green_17100       1          graph_batch17092.c
  gj_bar3_ready_17100         0          graph_batch17093.c  (bar3 open)
  gj_product_score_17100      0          graph_batch17094.c  (score open)
  gj_continuum_ready_17100    1          graph_batch17095.c
  gj_smoke_soft_17100         1          graph_batch17096.c
  gj_dyn_soft_17100           1          graph_batch17097.c
  gj_milestone_tag_17100      17000      graph_batch17098.c
  gj_continuum_wave_17100     17000      graph_batch17099.c
  gj_batch_id_17100           17000      graph_batch17100.c
  gj_graph_milestone_17100    17000      graph_batch17100.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=17100`` means parent wired CGJ_SRCS through
    ``graph_batch17100.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_17100`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_17100`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M17100_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_17000`` / ``gj_*_16900`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 29 exclusive CREATE-ONLY lands the **markers** band
  (17091–17100) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_17100.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_17100.py

  # Restrict to marker band only (soft-gate TUs; Wave 29 exclusive):
  python3 user/libcgj/src/_gen_milestone_17100.py --band markers

  # Extension identity only (17001–17050):
  python3 user/libcgj/src/_gen_milestone_17100.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_17100.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_17000.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 17100 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 17100
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 17001
WAVE_HI = 17100  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 17001, 17050
IDENTITY_LO, IDENTITY_HI = 17051, 17075
FILL_LO, FILL_HI = 17076, 17090
MARKERS_LO, MARKERS_HI = 17091, 17100
...
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (17091, "shell_green", 1, "shell product green extract (wave 17100)"),
    (17092, "libcgj_green", 1, "libcgj product green extract (wave 17100)"),
    (17093, "bar3_ready", 0, "product bar3 readiness gate (wave 17100)"),
    (17094, "product_score", 0, "product readiness score tag (wave 17100)"),
    (17095, "continuum_ready", 1, "continuum readiness gate (wave 17100)"),
    (17096, "smoke_soft", 1, "smoke soft lamp (wave 17100)"),
    (17097, "dyn_soft", 1, "dyn soft lamp (wave 17100)"),
    (17098, "milestone_tag", MILESTONE, "milestone 17100 product tag"),
    (17099, "continuum_wave", MILESTONE, "milestone 17100 continuum wave identity"),
    # 17100 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        17076: "continuum wave fill soft unit + batch id",
        17080: "continuum bridge soft unit + batch id",
        17090: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (17091–17099), rich Wave-16 style."""
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_17000 / gj_shell_green_16900 / gj_shell_green_16800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_17000 / gj_libcgj_green_16900 / gj_libcgj_green_16800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_17000 / gj_bar3_ready_16900 / gj_bar3_ready_16800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_17000 / gj_product_score_16900 / gj_product_score_16800\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_17000 / gj_continuum_ready_16900 /\n"
            f" * gj_continuum_ready_16800 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_17000 / gj_smoke_soft_16900 / gj_smoke_soft_16800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_17000 / gj_dyn_soft_16900 / gj_dyn_soft_16800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_17000 / gj_milestone_tag_16900 / gj_milestone_tag_16800,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_17000 / gj_continuum_wave_16900 /\n"
            f" * gj_continuum_wave_16800, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch17100.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 17099
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 17091–17099; 17000 is capstone
