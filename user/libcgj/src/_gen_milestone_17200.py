...
"""
Soft continuum generator — MILESTONE 17200 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 17101–17200 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=17200   when parent has wired through graph_batch17200.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 17101 17200
...
graph batches ending at M. For M = **17200** the exclusive range is
**17101–17200** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 17101–17150  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 17151–17175  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 17176–17190  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 17191–17200  │ decade soft gates (Wave 30 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 17200 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_17200        1          graph_batch17191.c
  gj_libcgj_green_17200       1          graph_batch17192.c
  gj_bar3_ready_17200         0          graph_batch17193.c  (bar3 open)
  gj_product_score_17200      0          graph_batch17194.c  (score open)
  gj_continuum_ready_17200    1          graph_batch17195.c
  gj_smoke_soft_17200         1          graph_batch17196.c
  gj_dyn_soft_17200           1          graph_batch17197.c
  gj_milestone_tag_17200      17200      graph_batch17198.c
  gj_continuum_wave_17200     17200      graph_batch17199.c
  gj_batch_id_17200           17200      graph_batch17200.c
  gj_graph_milestone_17200    17200      graph_batch17200.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=17200`` means parent wired CGJ_SRCS through
    ``graph_batch17200.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_17200`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_17200`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M17200_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_17100`` / ``gj_*_17000`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 30 exclusive CREATE-ONLY lands the **markers** band
  (17191–17200) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_17200.py

  # Restrict to marker band only (soft-gate TUs; Wave 30 exclusive):
  python3 user/libcgj/src/_gen_milestone_17200.py --band markers

  # Extension identity only (17101–17150):
  python3 user/libcgj/src/_gen_milestone_17200.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_17200.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_17100.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 17200 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 17200
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 17101
WAVE_HI = 17200  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 17101, 17150
IDENTITY_LO, IDENTITY_HI = 17151, 17175
FILL_LO, FILL_HI = 17176, 17190
MARKERS_LO, MARKERS_HI = 17191, 17200
...
# Stems become gj_<stem>_17200. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (17191, "shell_green", 1, "shell product green extract (wave 17200)"),
    (17192, "libcgj_green", 1, "libcgj product green extract (wave 17200)"),
    (17193, "bar3_ready", 0, "product bar3 readiness gate (wave 17200)"),
    (17194, "product_score", 0, "product readiness score tag (wave 17200)"),
    (17195, "continuum_ready", 1, "continuum readiness gate (wave 17200)"),
    (17196, "smoke_soft", 1, "smoke soft lamp (wave 17200)"),
    (17197, "dyn_soft", 1, "dyn soft lamp (wave 17200)"),
    (17198, "milestone_tag", MILESTONE, "milestone 17200 product tag"),
    (17199, "continuum_wave", MILESTONE, "milestone 17200 continuum wave identity"),
    # 17200 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        17176: "continuum wave fill soft unit + batch id",
        17180: "continuum bridge soft unit + batch id",
        17190: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (17191–17199), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 17193
    tag_batch = MARKERS_LO + 7  # 17198
    wave_batch = MARKERS_LO + 8  # 17199
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_17100 / gj_shell_green_17000 / gj_shell_green_16900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_17100 / gj_libcgj_green_17000 / gj_libcgj_green_16900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_17100 / gj_bar3_ready_17000 / gj_bar3_ready_16900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_17100 / gj_product_score_17000 / gj_product_score_16900\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_17100 / gj_continuum_ready_17000 /\n"
            f" * gj_continuum_ready_16900 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_17100 / gj_smoke_soft_17000 / gj_smoke_soft_16900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_17100 / gj_dyn_soft_17000 / gj_dyn_soft_16900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_17100 / gj_milestone_tag_17000 / gj_milestone_tag_16900,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_17100 / gj_continuum_wave_17000 /\n"
            f" * gj_continuum_wave_16900, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch17200.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 17199
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 17191–17199; 17200 is capstone
