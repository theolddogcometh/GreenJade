...
"""
Soft continuum generator — MILESTONE 17300 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 17201–17300 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=17300   when parent has wired through graph_batch17300.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 17201 17300
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 17201 17300
...
graph batches ending at M. For M = **17300** the exclusive range is
**17201–17300** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 17201–17250  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 17251–17275  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 17276–17290  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 17291–17300  │ decade soft gates (Wave 31 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 17300 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_17300        1          graph_batch17291.c
  gj_libcgj_green_17300       1          graph_batch17292.c
  gj_bar3_ready_17300         0          graph_batch17293.c  (bar3 open)
  gj_product_score_17300      0          graph_batch17294.c  (score open)
  gj_continuum_ready_17300    1          graph_batch17295.c
  gj_smoke_soft_17300         1          graph_batch17296.c
  gj_dyn_soft_17300           1          graph_batch17297.c
  gj_milestone_tag_17300      17300      graph_batch17298.c
  gj_continuum_wave_17300     17300      graph_batch17299.c
  gj_batch_id_17300           17300      graph_batch17300.c
  gj_graph_milestone_17300    17300      graph_batch17300.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=17300`` means parent wired CGJ_SRCS through
    ``graph_batch17300.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_17300`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_17300`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M17300_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_17200`` / ``gj_*_17100`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 31 exclusive CREATE-ONLY lands the **markers** band
  (17291–17300) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_17300.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_17300.py

  # Restrict to marker band only (soft-gate TUs; Wave 31 exclusive):
  python3 user/libcgj/src/_gen_milestone_17300.py --band markers

  # Extension identity only (17201–17250):
  python3 user/libcgj/src/_gen_milestone_17300.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_17300.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_17200.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 17300 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 17300
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 17201
WAVE_HI = 17300  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 17201, 17250
IDENTITY_LO, IDENTITY_HI = 17251, 17275
FILL_LO, FILL_HI = 17276, 17290
MARKERS_LO, MARKERS_HI = 17291, 17300
...
# Stems become gj_<stem>_17300. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (17291, "shell_green", 1, "shell product green extract (wave 17300)"),
    (17292, "libcgj_green", 1, "libcgj product green extract (wave 17300)"),
    (17293, "bar3_ready", 0, "product bar3 readiness gate (wave 17300)"),
    (17294, "product_score", 0, "product readiness score tag (wave 17300)"),
    (17295, "continuum_ready", 1, "continuum readiness gate (wave 17300)"),
    (17296, "smoke_soft", 1, "smoke soft lamp (wave 17300)"),
    (17297, "dyn_soft", 1, "dyn soft lamp (wave 17300)"),
    (17298, "milestone_tag", MILESTONE, "milestone 17300 product tag"),
    (17299, "continuum_wave", MILESTONE, "milestone 17300 continuum wave identity"),
    # 17300 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        17276: "continuum wave fill soft unit + batch id",
        17280: "continuum bridge soft unit + batch id",
        17290: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (17291–17299), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 17293
    tag_batch = MARKERS_LO + 7  # 17298
    wave_batch = MARKERS_LO + 8  # 17299
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_17200 / gj_shell_green_17100 / gj_shell_green_17000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_17200 / gj_libcgj_green_17100 / gj_libcgj_green_17000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_17200 / gj_bar3_ready_17100 / gj_bar3_ready_17000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_17200 / gj_product_score_17100 / gj_product_score_17000\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_17200 / gj_continuum_ready_17100 /\n"
            f" * gj_continuum_ready_17000 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_17200 / gj_smoke_soft_17100 / gj_smoke_soft_17000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_17200 / gj_dyn_soft_17100 / gj_dyn_soft_17000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_17200 / gj_milestone_tag_17100 / gj_milestone_tag_17000,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_17200 / gj_continuum_wave_17100 /\n"
            f" * gj_continuum_wave_17000, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch17300.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 17299
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 17291–17299; 17300 is capstone
