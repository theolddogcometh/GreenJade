...
"""
Soft continuum generator — MILESTONE 17400 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 17301–17400 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=17400   when parent has wired through graph_batch17400.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 17301 17400
...
graph batches ending at M. For M = **17400** the exclusive range is
**17301–17400** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 17301–17350  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 17351–17375  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 17376–17390  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 17391–17400  │ decade soft gates (Wave 32 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 17400 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_17400        1          graph_batch17391.c
  gj_libcgj_green_17400       1          graph_batch17392.c
  gj_bar3_ready_17400         0          graph_batch17393.c  (bar3 open)
  gj_product_score_17400      0          graph_batch17394.c  (score open)
  gj_continuum_ready_17400    1          graph_batch17395.c
  gj_smoke_soft_17400         1          graph_batch17396.c
  gj_dyn_soft_17400           1          graph_batch17397.c
  gj_milestone_tag_17400      17400      graph_batch17398.c
  gj_continuum_wave_17400     17400      graph_batch17399.c
  gj_batch_id_17400           17400      graph_batch17400.c
  gj_graph_milestone_17400    17400      graph_batch17400.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=17400`` means parent wired CGJ_SRCS through
    ``graph_batch17400.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_17400`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_17400`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M17400_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_17300`` / ``gj_*_17200`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 32 exclusive CREATE-ONLY lands the **markers** band
  (17391–17400) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_17400.py

  # Restrict to marker band only (soft-gate TUs; Wave 32 exclusive):
  python3 user/libcgj/src/_gen_milestone_17400.py --band markers

  # Extension identity only (17301–17350):
  python3 user/libcgj/src/_gen_milestone_17400.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_17400.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_17300.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 17400 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 17400
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 17301
WAVE_HI = 17400  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 17301, 17350
IDENTITY_LO, IDENTITY_HI = 17351, 17375
FILL_LO, FILL_HI = 17376, 17390
MARKERS_LO, MARKERS_HI = 17391, 17400
...
# Stems become gj_<stem>_17400. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (17391, "shell_green", 1, "shell product green extract (wave 17400)"),
    (17392, "libcgj_green", 1, "libcgj product green extract (wave 17400)"),
    (17393, "bar3_ready", 0, "product bar3 readiness gate (wave 17400)"),
    (17394, "product_score", 0, "product readiness score tag (wave 17400)"),
    (17395, "continuum_ready", 1, "continuum readiness gate (wave 17400)"),
    (17396, "smoke_soft", 1, "smoke soft lamp (wave 17400)"),
    (17397, "dyn_soft", 1, "dyn soft lamp (wave 17400)"),
    (17398, "milestone_tag", MILESTONE, "milestone 17400 product tag"),
    (17399, "continuum_wave", MILESTONE, "milestone 17400 continuum wave identity"),
    # 17400 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        17376: "continuum wave fill soft unit + batch id",
        17380: "continuum bridge soft unit + batch id",
        17390: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (17391–17399), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 17393
    tag_batch = MARKERS_LO + 7  # 17398
    wave_batch = MARKERS_LO + 8  # 17399
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_17300 / gj_shell_green_17200 / gj_shell_green_17100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_17300 / gj_libcgj_green_17200 / gj_libcgj_green_17100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_17300 / gj_bar3_ready_17200 / gj_bar3_ready_17100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_17300 / gj_product_score_17200 / gj_product_score_17100\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_17300 / gj_continuum_ready_17200 /\n"
            f" * gj_continuum_ready_17100 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_17300 / gj_smoke_soft_17200 / gj_smoke_soft_17100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_17300 / gj_dyn_soft_17200 / gj_dyn_soft_17100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_17300 / gj_milestone_tag_17200 / gj_milestone_tag_17100,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_17300 / gj_continuum_wave_17200 /\n"
            f" * gj_continuum_wave_17100, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch17400.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 17399
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 17391–17399; 17400 is capstone
