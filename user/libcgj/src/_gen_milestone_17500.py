...
"""
Soft continuum generator — MILESTONE 17500 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 17401–17500 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=17500   when parent has wired through graph_batch17500.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 17401 17500
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 17401 17500
...
graph batches ending at M. For M = **17500** the exclusive range is
**17401–17500** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 17401–17450  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 17451–17475  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 17476–17490  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 17491–17500  │ decade soft gates (Wave 33 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 17500 marker KATs (soft host probe expectations)
----------------------------------------------------------
  ...
  gj_milestone_tag_17500      17500      graph_batch17498.c
  gj_continuum_wave_17500     17500      graph_batch17499.c
  gj_batch_id_17500           17500      graph_batch17500.c
  gj_graph_milestone_17500    17500      graph_batch17500.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=17500`` means parent wired CGJ_SRCS through
...
--------------------
  ...
  Wave 33 exclusive CREATE-ONLY lands the **markers** band
  (17491–17500) first; extension/identity/fill may be emitted later via
...
-----
  ...
  # Restrict to marker band only (soft-gate TUs; Wave 33 exclusive):
  python3 user/libcgj/src/_gen_milestone_17500.py --band markers

  # Extension identity only (17401–17450):
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 17500 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 17500
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 17401
WAVE_HI = 17500  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 17401, 17450
IDENTITY_LO, IDENTITY_HI = 17451, 17475
FILL_LO, FILL_HI = 17476, 17490
MARKERS_LO, MARKERS_HI = 17491, 17500
...
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (17491, "shell_green", 1, "shell product green extract (wave 17500)"),
    (17492, "libcgj_green", 1, "libcgj product green extract (wave 17500)"),
    (17493, "bar3_ready", 0, "product bar3 readiness gate (wave 17500)"),
    (17494, "product_score", 0, "product readiness score tag (wave 17500)"),
    (17495, "continuum_ready", 1, "continuum readiness gate (wave 17500)"),
    (17496, "smoke_soft", 1, "smoke soft lamp (wave 17500)"),
    (17497, "dyn_soft", 1, "dyn soft lamp (wave 17500)"),
    (17498, "milestone_tag", MILESTONE, "milestone 17500 product tag"),
    (17499, "continuum_wave", MILESTONE, "milestone 17500 continuum wave identity"),
    # 17500 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        17476: "continuum wave fill soft unit + batch id",
        17480: "continuum bridge soft unit + batch id",
        17490: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (17491–17499), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 17493
    tag_batch = MARKERS_LO + 7  # 17498
    wave_batch = MARKERS_LO + 8  # 17499
...
def render_milestone_capstone() -> str:
    ...
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 17499
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 17491–17499; 17500 is capstone
