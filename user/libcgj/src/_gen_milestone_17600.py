...
"""
Soft continuum generator — MILESTONE 17600 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 17501–17600 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=17600   when parent has wired through graph_batch17500.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 17501 17600
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 17501 17600
...
graph batches ending at M. For M = **17600** the exclusive range is
**17501–17600** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 17501–17550  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 17551–17575  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 17576–17590  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 17591–17600  │ decade soft gates (Wave 34 exclusive)│
...
----------------------------------------------------------
  ...
  --------------------------  ---------  --------------------
  gj_shell_green_17500        1          graph_batch17491.c
  gj_libcgj_green_17500       1          graph_batch17492.c
  gj_bar3_ready_17500         0          graph_batch17493.c  (bar3 open)
  gj_product_score_17500      0          graph_batch17494.c  (score open)
  gj_continuum_ready_17500    1          graph_batch17495.c
  gj_smoke_soft_17500         1          graph_batch17496.c
  gj_dyn_soft_17500           1          graph_batch17497.c
  gj_milestone_tag_17500      17600      graph_batch17498.c
  gj_continuum_wave_17500     17600      graph_batch17499.c
  gj_batch_id_17500           17600      graph_batch17500.c
  gj_graph_milestone_17500    17600      graph_batch17500.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=17600`` means parent wired CGJ_SRCS through
    ``graph_batch17500.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_17500`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_17500`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M17500_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_17400`` / ``gj_*_17300`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 34 exclusive CREATE-ONLY lands the **markers** band
  (17591–17600) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_17500.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_17500.py

  # Restrict to marker band only (soft-gate TUs; Wave 34 exclusive):
  python3 user/libcgj/src/_gen_milestone_17500.py --band markers

  # Extension identity only (17501–17550):
  python3 user/libcgj/src/_gen_milestone_17500.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_17500.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_17400.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 17600 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 17600
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 17501
WAVE_HI = 17600  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 17501, 17550
IDENTITY_LO, IDENTITY_HI = 17551, 17575
FILL_LO, FILL_HI = 17576, 17590
MARKERS_LO, MARKERS_HI = 17591, 17600
...
# Stems become gj_<stem>_17500. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (17591, "shell_green", 1, "shell product green extract (wave 17600)"),
    (17592, "libcgj_green", 1, "libcgj product green extract (wave 17600)"),
    (17593, "bar3_ready", 0, "product bar3 readiness gate (wave 17600)"),
    (17594, "product_score", 0, "product readiness score tag (wave 17600)"),
    (17595, "continuum_ready", 1, "continuum readiness gate (wave 17600)"),
    (17596, "smoke_soft", 1, "smoke soft lamp (wave 17600)"),
    (17597, "dyn_soft", 1, "dyn soft lamp (wave 17600)"),
    (17598, "milestone_tag", MILESTONE, "milestone 17600 product tag"),
    (17599, "continuum_wave", MILESTONE, "milestone 17600 continuum wave identity"),
    # 17600 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        17576: "continuum wave fill soft unit + batch id",
        17580: "continuum bridge soft unit + batch id",
        17590: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (17591–17599), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 17593
    tag_batch = MARKERS_LO + 7  # 17598
    wave_batch = MARKERS_LO + 8  # 17599
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_17400 / gj_shell_green_17300 / gj_shell_green_17200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_17400 / gj_libcgj_green_17300 / gj_libcgj_green_17200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_17400 / gj_bar3_ready_17300 / gj_bar3_ready_17200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_17400 / gj_product_score_17300 / gj_product_score_17200\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_17400 / gj_smoke_soft_17300 / gj_smoke_soft_17200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_17400 / gj_dyn_soft_17300 / gj_dyn_soft_17200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_17400 / gj_milestone_tag_17300 / gj_milestone_tag_17200,\n"
...
def render_milestone_capstone() -> str:
    """graph_batch17500.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 17599
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 17591–17599; 17600 is capstone
