...
"""
Soft continuum generator — MILESTONE 17700 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 17601–17700 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=17700   when parent has wired through graph_batch17600.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 17601 17700
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 17601 17700
...
graph batches ending at M. For M = **17700** the exclusive range is
**17601–17700** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 17601–17650  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 17651–17675  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 17676–17690  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 17691–17700  │ decade soft gates (Wave 35 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 17700 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_17700        1          graph_batch17691.c
  gj_libcgj_green_17700       1          graph_batch17692.c
  gj_bar3_ready_17700         0          graph_batch17693.c  (bar3 open)
  gj_product_score_17700      0          graph_batch17694.c  (score open)
  gj_continuum_ready_17700    1          graph_batch17695.c
  gj_smoke_soft_17700         1          graph_batch17696.c
  gj_dyn_soft_17700           1          graph_batch17697.c
  gj_milestone_tag_17700      17700      graph_batch17698.c
  gj_continuum_wave_17700     17700      graph_batch17699.c
  gj_batch_id_17700           17700      graph_batch17700.c
  gj_graph_milestone_17700    17700      graph_batch17700.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=17700`` means parent wired CGJ_SRCS through
    ``graph_batch17600.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_17700`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_17700`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M17700_DIRECT``.
...
--------------------
  ...
  Wave 35 exclusive CREATE-ONLY lands the **markers** band
  (17691–17700) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_17700.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_17700.py

  # Restrict to marker band only (soft-gate TUs; Wave 35 exclusive):
  python3 user/libcgj/src/_gen_milestone_17700.py --band markers

  # Extension identity only (17601–17650):
  python3 user/libcgj/src/_gen_milestone_17700.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_17700.py --force
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 17700 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 17700
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 17601
WAVE_HI = 17700  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 17601, 17650
IDENTITY_LO, IDENTITY_HI = 17651, 17675
FILL_LO, FILL_HI = 17676, 17690
MARKERS_LO, MARKERS_HI = 17691, 17700
...
# Stems become gj_<stem>_17700. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (17691, "shell_green", 1, "shell product green extract (wave 17700)"),
    (17692, "libcgj_green", 1, "libcgj product green extract (wave 17700)"),
    (17693, "bar3_ready", 0, "product bar3 readiness gate (wave 17700)"),
    (17694, "product_score", 0, "product readiness score tag (wave 17700)"),
    (17695, "continuum_ready", 1, "continuum readiness gate (wave 17700)"),
    (17696, "smoke_soft", 1, "smoke soft lamp (wave 17700)"),
    (17697, "dyn_soft", 1, "dyn soft lamp (wave 17700)"),
    (17698, "milestone_tag", MILESTONE, "milestone 17700 product tag"),
    (17699, "continuum_wave", MILESTONE, "milestone 17700 continuum wave identity"),
    # 17700 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        17676: "continuum wave fill soft unit + batch id",
        17680: "continuum bridge soft unit + batch id",
        17690: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (17691–17699), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 17693
    tag_batch = MARKERS_LO + 7  # 17698
    wave_batch = MARKERS_LO + 8  # 17699
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_17600 / gj_continuum_ready_17500 /\n"
            f" * gj_continuum_ready_17400 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_17600 / gj_continuum_wave_17500 /\n"
            f" * gj_continuum_wave_17400, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch17700.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 17699
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 17691–17699; 17700 is capstone
