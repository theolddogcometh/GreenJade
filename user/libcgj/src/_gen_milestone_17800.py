...
"""
Soft continuum generator — MILESTONE 17800 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 17701–17800 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=17800   when parent has wired through graph_batch17800.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 17701 17800
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 17701 17800
...
graph batches ending at M. For M = **17800** the exclusive range is
**17701–17800** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 17701–17750  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 17751–17775  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 17776–17790  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 17791–17800  │ decade soft gates (Wave 36 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 17600 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_17800        1          graph_batch17791.c
  gj_libcgj_green_17800       1          graph_batch17792.c
  gj_bar3_ready_17800         0          graph_batch17793.c  (bar3 open)
  gj_product_score_17800      0          graph_batch17794.c  (score open)
  gj_continuum_ready_17800    1          graph_batch17795.c
  gj_smoke_soft_17800         1          graph_batch17796.c
  gj_dyn_soft_17800           1          graph_batch17797.c
  gj_milestone_tag_17800      17600      graph_batch17798.c
  gj_continuum_wave_17800     17600      graph_batch17799.c
  gj_batch_id_17800           17600      graph_batch17800.c
  gj_graph_milestone_17800    17600      graph_batch17800.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=17800`` means parent wired CGJ_SRCS through
    ``graph_batch17800.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_17800`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_17800`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M17800_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_17600`` / ``gj_*_17500`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 36 exclusive CREATE-ONLY lands the **markers** band
  (17791–17800) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_17800.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_17800.py

  # Restrict to marker band only (soft-gate TUs; Wave 36 exclusive):
  python3 user/libcgj/src/_gen_milestone_17800.py --band markers

  # Extension identity only (17701–17750):
  python3 user/libcgj/src/_gen_milestone_17800.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_17800.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_17600.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 17800 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 17800
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 17701
WAVE_HI = 17800  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 17701, 17750
IDENTITY_LO, IDENTITY_HI = 17751, 17775
FILL_LO, FILL_HI = 17776, 17790
MARKERS_LO, MARKERS_HI = 17791, 17800
...
# Stems become gj_<stem>_17800. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (17791, "shell_green", 1, "shell product green extract (wave 17800)"),
    (17792, "libcgj_green", 1, "libcgj product green extract (wave 17800)"),
    (17793, "bar3_ready", 0, "product bar3 readiness gate (wave 17800)"),
    (17794, "product_score", 0, "product readiness score tag (wave 17800)"),
    (17795, "continuum_ready", 1, "continuum readiness gate (wave 17800)"),
    (17796, "smoke_soft", 1, "smoke soft lamp (wave 17800)"),
    (17797, "dyn_soft", 1, "dyn soft lamp (wave 17800)"),
    (17798, "milestone_tag", MILESTONE, "milestone 17800 product tag"),
    (17799, "continuum_wave", MILESTONE, "milestone 17800 continuum wave identity"),
    # 17800 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        17776: "continuum wave fill soft unit + batch id",
        17780: "continuum bridge soft unit + batch id",
        17790: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (17791–17799), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 17793
    tag_batch = MARKERS_LO + 7  # 17798
    wave_batch = MARKERS_LO + 8  # 17799
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_17600 / gj_shell_green_17500 / gj_shell_green_17400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_17600 / gj_libcgj_green_17500 / gj_libcgj_green_17400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_17600 / gj_bar3_ready_17500 / gj_bar3_ready_17400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_17600 / gj_product_score_17500 / gj_product_score_17400\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_17400 / gj_continuum_ready_17300 /\n"
            f" * gj_continuum_ready_17200 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_17600 / gj_smoke_soft_17500 / gj_smoke_soft_17400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_17600 / gj_dyn_soft_17500 / gj_dyn_soft_17400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_17600 / gj_milestone_tag_17500 / gj_milestone_tag_17400,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_17400 / gj_continuum_wave_17300 /\n"
            f" * gj_continuum_wave_17200, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch17800.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 17799
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 17791–17799; 17600 is capstone
