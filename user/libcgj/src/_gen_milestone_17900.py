...
"""
Soft continuum generator — MILESTONE 17900 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 17801–17900 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=17900   when parent has wired through graph_batch17800.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 17801 17900
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 17801 17900
...
graph batches ending at M. For M = **17900** the exclusive range is
**17801–17900** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 17801–17850  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 17851–17875  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 17876–17890  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 17891–17900  │ decade soft gates (Wave 37 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 17900 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_17900        1          graph_batch17891.c
  gj_libcgj_green_17900       1          graph_batch17892.c
  gj_bar3_ready_17900         0          graph_batch17893.c  (bar3 open)
  gj_product_score_17900      0          graph_batch17894.c  (score open)
  gj_continuum_ready_17900    1          graph_batch17895.c
  gj_smoke_soft_17900         1          graph_batch17896.c
  gj_dyn_soft_17900           1          graph_batch17897.c
  gj_milestone_tag_17900      17900      graph_batch17898.c
  gj_continuum_wave_17900     17900      graph_batch17899.c
  gj_batch_id_17900           17900      graph_batch17900.c
  gj_graph_milestone_17900    17900      graph_batch17900.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=17900`` means parent wired CGJ_SRCS through
    ``graph_batch17800.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_17900`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_17900`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M17900_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_17700`` / ``gj_*_17600`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 37 exclusive CREATE-ONLY lands the **markers** band
  (17891–17900) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_17900.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_17900.py

  # Restrict to marker band only (soft-gate TUs; Wave 37 exclusive):
  python3 user/libcgj/src/_gen_milestone_17900.py --band markers

  # Extension identity only (17801–17850):
  python3 user/libcgj/src/_gen_milestone_17900.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_17900.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_17700.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 17900 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 17900
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 17801
WAVE_HI = 17900  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 17801, 17850
IDENTITY_LO, IDENTITY_HI = 17851, 17875
FILL_LO, FILL_HI = 17876, 17890
MARKERS_LO, MARKERS_HI = 17891, 17900
...
# Stems become gj_<stem>_17900. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (17891, "shell_green", 1, "shell product green extract (wave 17900)"),
    (17892, "libcgj_green", 1, "libcgj product green extract (wave 17900)"),
    (17893, "bar3_ready", 0, "product bar3 readiness gate (wave 17900)"),
    (17894, "product_score", 0, "product readiness score tag (wave 17900)"),
    (17895, "continuum_ready", 1, "continuum readiness gate (wave 17900)"),
    (17896, "smoke_soft", 1, "smoke soft lamp (wave 17900)"),
    (17897, "dyn_soft", 1, "dyn soft lamp (wave 17900)"),
    (17898, "milestone_tag", MILESTONE, "milestone 17900 product tag"),
    (17899, "continuum_wave", MILESTONE, "milestone 17900 continuum wave identity"),
    # 17900 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        17876: "continuum wave fill soft unit + batch id",
        17880: "continuum bridge soft unit + batch id",
        17890: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (17891–17899), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 17893
    tag_batch = MARKERS_LO + 7  # 17898
    wave_batch = MARKERS_LO + 8  # 17899
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_17700 / gj_shell_green_17600 / gj_shell_green_17500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_17700 / gj_libcgj_green_17600 / gj_libcgj_green_17500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_17700 / gj_bar3_ready_17600 / gj_bar3_ready_17500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_17700 / gj_product_score_17600 / gj_product_score_17500\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_17700 / gj_continuum_ready_17600 /\n"
            f" * gj_continuum_ready_17500 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_17700 / gj_smoke_soft_17600 / gj_smoke_soft_17500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_17700 / gj_dyn_soft_17600 / gj_dyn_soft_17500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_17700 / gj_milestone_tag_17600 / gj_milestone_tag_17500,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_17700 / gj_continuum_wave_17600 /\n"
            f" * gj_continuum_wave_17500, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    ...
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 17899
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 17891–17899; 17900 is capstone
