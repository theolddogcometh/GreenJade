...
"""
Soft continuum generator — MILESTONE 18000 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 17901–18000 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=18000   when parent has wired through graph_batch18000.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 17901 18000
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 17901 18000
...
graph batches ending at M. For M = **18000** the exclusive range is
**17901–18000** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 17901–17950  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 17951–17975  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 17976–17990  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 17991–18000  │ decade soft gates (Wave 38 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 18000 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_18000        1          graph_batch17991.c
  gj_libcgj_green_18000       1          graph_batch17992.c
  gj_bar3_ready_18000         0          graph_batch17993.c  (bar3 open)
  gj_product_score_18000      0          graph_batch17994.c  (score open)
  gj_continuum_ready_18000    1          graph_batch17995.c
  gj_smoke_soft_18000         1          graph_batch17996.c
  gj_dyn_soft_18000           1          graph_batch17997.c
  gj_milestone_tag_18000      18000      graph_batch17998.c
  gj_continuum_wave_18000     18000      graph_batch17999.c
  gj_batch_id_18000           18000      graph_batch18000.c
  gj_graph_milestone_18000    18000      graph_batch18000.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=18000`` means parent wired CGJ_SRCS through
    ``graph_batch18000.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_18000`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_18000`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M18000_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_17900`` / ``gj_*_17800`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 38 exclusive CREATE-ONLY lands the **markers** band
  (17991–18000) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_18000.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_18000.py

  # Restrict to marker band only (soft-gate TUs; Wave 38 exclusive):
  python3 user/libcgj/src/_gen_milestone_18000.py --band markers

  # Extension identity only (17901–17950):
  python3 user/libcgj/src/_gen_milestone_18000.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_18000.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_17900.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 18000 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 18000
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 17901
WAVE_HI = 18000  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 17901, 17950
IDENTITY_LO, IDENTITY_HI = 17951, 17975
FILL_LO, FILL_HI = 17976, 17990
MARKERS_LO, MARKERS_HI = 17991, 18000
...
# Stems become gj_<stem>_18000. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (17991, "shell_green", 1, "shell product green extract (wave 18000)"),
    (17992, "libcgj_green", 1, "libcgj product green extract (wave 18000)"),
    (17993, "bar3_ready", 0, "product bar3 readiness gate (wave 18000)"),
    (17994, "product_score", 0, "product readiness score tag (wave 18000)"),
    (17995, "continuum_ready", 1, "continuum readiness gate (wave 18000)"),
    (17996, "smoke_soft", 1, "smoke soft lamp (wave 18000)"),
    (17997, "dyn_soft", 1, "dyn soft lamp (wave 18000)"),
    (17998, "milestone_tag", MILESTONE, "milestone 18000 product tag"),
    (17999, "continuum_wave", MILESTONE, "milestone 18000 continuum wave identity"),
    # 18000 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        17976: "continuum wave fill soft unit + batch id",
        17980: "continuum bridge soft unit + batch id",
        17990: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (17991–17999), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 17993
    tag_batch = MARKERS_LO + 7  # 17998
    wave_batch = MARKERS_LO + 8  # 17999
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_17800 / gj_shell_green_17700 / gj_shell_green_17600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_17800 / gj_libcgj_green_17700 / gj_libcgj_green_17600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_17800 / gj_bar3_ready_17700 / gj_bar3_ready_17600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_17800 / gj_product_score_17700 / gj_product_score_17600\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_17800 / gj_continuum_ready_17700 /\n"
            f" * gj_continuum_ready_17600 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_17800 / gj_smoke_soft_17700 / gj_smoke_soft_17600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_17800 / gj_dyn_soft_17700 / gj_dyn_soft_17600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_17800 / gj_milestone_tag_17700 / gj_milestone_tag_17600,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_17800 / gj_continuum_wave_17700 /\n"
            f" * gj_continuum_wave_17600, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch18000.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 17999
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 17991–17999; 18000 is capstone
