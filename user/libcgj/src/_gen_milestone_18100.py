...
"""
Soft continuum generator — MILESTONE 18100 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 18001–18100 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=18100   when parent has wired through graph_batch18100.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 18001 18100
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 18001 18100
...
graph batches ending at M. For M = **18100** the exclusive range is
**18001–18100** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 18001–18050  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 18051–18075  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 18076–18090  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 18091–18100  │ decade soft gates (Wave 39 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 18100 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_18100        1          graph_batch18091.c
  gj_libcgj_green_18100       1          graph_batch18092.c
  gj_bar3_ready_18100         0          graph_batch18093.c  (bar3 open)
  gj_product_score_18100      0          graph_batch18094.c  (score open)
  gj_continuum_ready_18100    1          graph_batch18095.c
  gj_smoke_soft_18100         1          graph_batch18096.c
  gj_dyn_soft_18100           1          graph_batch18097.c
  gj_milestone_tag_18100      18100      graph_batch18098.c
  gj_continuum_wave_18100     18100      graph_batch18099.c
  gj_batch_id_18100           18100      graph_batch18100.c
  gj_graph_milestone_18100    18100      graph_batch18100.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=18100`` means parent wired CGJ_SRCS through
    ``graph_batch18100.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_18100`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_18100`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M18100_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_18000`` / ``gj_*_17900`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 39 exclusive CREATE-ONLY lands the **markers** band
  (18091–18100) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_18100.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_18100.py

  # Restrict to marker band only (soft-gate TUs; Wave 39 exclusive):
  python3 user/libcgj/src/_gen_milestone_18100.py --band markers

  # Extension identity only (18001–18050):
  python3 user/libcgj/src/_gen_milestone_18100.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_18100.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_18000.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 18100 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 18100
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 18001
WAVE_HI = 18100  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 18001, 18050
IDENTITY_LO, IDENTITY_HI = 18051, 18075
FILL_LO, FILL_HI = 18076, 18090
MARKERS_LO, MARKERS_HI = 18091, 18100
...
# Stems become gj_<stem>_18100. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (18091, "shell_green", 1, "shell product green extract (wave 18100)"),
    (18092, "libcgj_green", 1, "libcgj product green extract (wave 18100)"),
    (18093, "bar3_ready", 0, "product bar3 readiness gate (wave 18100)"),
    (18094, "product_score", 0, "product readiness score tag (wave 18100)"),
    (18095, "continuum_ready", 1, "continuum readiness gate (wave 18100)"),
    (18096, "smoke_soft", 1, "smoke soft lamp (wave 18100)"),
    (18097, "dyn_soft", 1, "dyn soft lamp (wave 18100)"),
    (18098, "milestone_tag", MILESTONE, "milestone 18100 product tag"),
    (18099, "continuum_wave", MILESTONE, "milestone 18100 continuum wave identity"),
    # 18100 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        18076: "continuum wave fill soft unit + batch id",
        18080: "continuum bridge soft unit + batch id",
        18090: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (18091–18099), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 18093
    tag_batch = MARKERS_LO + 7  # 18098
    wave_batch = MARKERS_LO + 8  # 18099
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_18000 / gj_shell_green_17900 / gj_shell_green_17800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_18000 / gj_libcgj_green_17900 / gj_libcgj_green_17800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_18000 / gj_bar3_ready_17900 / gj_bar3_ready_17800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_18000 / gj_product_score_17900 / gj_product_score_17800\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_18000 / gj_continuum_ready_17900 /\n"
            f" * gj_continuum_ready_17800 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_18000 / gj_smoke_soft_17900 / gj_smoke_soft_17800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_18000 / gj_dyn_soft_17900 / gj_dyn_soft_17800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_18000 / gj_milestone_tag_17900 / gj_milestone_tag_17800,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_18000 / gj_continuum_wave_17900 /\n"
            f" * gj_continuum_wave_17800, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch18100.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 18099
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 18091–18099; 18100 is capstone
