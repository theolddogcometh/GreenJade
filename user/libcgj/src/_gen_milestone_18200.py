...
"""
Soft continuum generator — MILESTONE 18200 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 18101–18200 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=18200   when parent has wired through graph_batch18200.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 18101 18200
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 18101 18200
...
graph batches ending at M. For M = **18200** the exclusive range is
**18101–18200** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 18101–18150  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 18151–18175  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 18176–18190  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 18191–18200  │ decade soft gates (Wave 40 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 18200 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_18200        1          graph_batch18191.c
  gj_libcgj_green_18200       1          graph_batch18192.c
  gj_bar3_ready_18200         0          graph_batch18193.c  (bar3 open)
  gj_product_score_18200      0          graph_batch18194.c  (score open)
  gj_continuum_ready_18200    1          graph_batch18195.c
  gj_smoke_soft_18200         1          graph_batch18196.c
  gj_dyn_soft_18200           1          graph_batch18197.c
  gj_milestone_tag_18200      18200      graph_batch18198.c
  gj_continuum_wave_18200     18200      graph_batch18199.c
  gj_batch_id_18200           18200      graph_batch18200.c
  gj_graph_milestone_18200    18200      graph_batch18200.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=18200`` means parent wired CGJ_SRCS through
    ``graph_batch18200.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_18200`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_18200`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M18200_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_18100`` / ``gj_*_18000`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 40 exclusive CREATE-ONLY lands the **markers** band
  (18191–18200) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_18200.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_18200.py

  # Restrict to marker band only (soft-gate TUs; Wave 40 exclusive):
  python3 user/libcgj/src/_gen_milestone_18200.py --band markers

  # Extension identity only (18101–18150):
  python3 user/libcgj/src/_gen_milestone_18200.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_18200.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_18100.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 18200 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 18200
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 18101
WAVE_HI = 18200  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 18101, 18150
IDENTITY_LO, IDENTITY_HI = 18151, 18175
FILL_LO, FILL_HI = 18176, 18190
MARKERS_LO, MARKERS_HI = 18191, 18200
...
# Stems become gj_<stem>_18200. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (18191, "shell_green", 1, "shell product green extract (wave 18200)"),
    (18192, "libcgj_green", 1, "libcgj product green extract (wave 18200)"),
    (18193, "bar3_ready", 0, "product bar3 readiness gate (wave 18200)"),
    (18194, "product_score", 0, "product readiness score tag (wave 18200)"),
    (18195, "continuum_ready", 1, "continuum readiness gate (wave 18200)"),
    (18196, "smoke_soft", 1, "smoke soft lamp (wave 18200)"),
    (18197, "dyn_soft", 1, "dyn soft lamp (wave 18200)"),
    (18198, "milestone_tag", MILESTONE, "milestone 18200 product tag"),
    (18199, "continuum_wave", MILESTONE, "milestone 18200 continuum wave identity"),
    # 18200 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        18176: "continuum wave fill soft unit + batch id",
        18180: "continuum bridge soft unit + batch id",
        18190: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (18191–18199), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 18193
    tag_batch = MARKERS_LO + 7  # 18198
    wave_batch = MARKERS_LO + 8  # 18199
...
def render_milestone_capstone() -> str:
    """graph_batch18200.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 18199
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 18191–18199; 18200 is capstone
