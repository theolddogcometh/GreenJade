...
"""
Soft continuum generator — MILESTONE 18400 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 18301–18400 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=18400   when parent has wired through graph_batch18400.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 18301 18400
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 18301 18400
...
graph batches ending at M. For M = **18400** the exclusive range is
**18301–18400** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 18301–18350  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 18351–18375  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 18376–18390  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 18391–18400  │ decade soft gates (Wave 42 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 18400 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_18400        1          graph_batch18391.c
  gj_libcgj_green_18400       1          graph_batch18392.c
  gj_bar3_ready_18400         0          graph_batch18393.c  (bar3 open)
  gj_product_score_18400      0          graph_batch18394.c  (score open)
  gj_continuum_ready_18400    1          graph_batch18395.c
  gj_smoke_soft_18400         1          graph_batch18396.c
  gj_dyn_soft_18400           1          graph_batch18397.c
  gj_milestone_tag_18400      18400      graph_batch18398.c
  gj_continuum_wave_18400     18400      graph_batch18399.c
  gj_batch_id_18400           18400      graph_batch18400.c
  gj_graph_milestone_18400    18400      graph_batch18400.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=18400`` means parent wired CGJ_SRCS through
    ``graph_batch18400.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_18400`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_18400`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M18400_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_18300`` / ``gj_*_18200`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 42 exclusive CREATE-ONLY lands the **markers** band
  (18391–18400) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_18400.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_18400.py

  # Restrict to marker band only (soft-gate TUs; Wave 42 exclusive):
  python3 user/libcgj/src/_gen_milestone_18400.py --band markers

  # Extension identity only (18301–18350):
  python3 user/libcgj/src/_gen_milestone_18400.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_18400.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_18300.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 18400 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 18400
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 18301
WAVE_HI = 18400  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 18301, 18350
IDENTITY_LO, IDENTITY_HI = 18351, 18375
FILL_LO, FILL_HI = 18376, 18390
MARKERS_LO, MARKERS_HI = 18391, 18400
...
# Stems become gj_<stem>_18400. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (18391, "shell_green", 1, "shell product green extract (wave 18400)"),
    (18392, "libcgj_green", 1, "libcgj product green extract (wave 18400)"),
    (18393, "bar3_ready", 0, "product bar3 readiness gate (wave 18400)"),
    (18394, "product_score", 0, "product readiness score tag (wave 18400)"),
    (18395, "continuum_ready", 1, "continuum readiness gate (wave 18400)"),
    (18396, "smoke_soft", 1, "smoke soft lamp (wave 18400)"),
    (18397, "dyn_soft", 1, "dyn soft lamp (wave 18400)"),
    (18398, "milestone_tag", MILESTONE, "milestone 18400 product tag"),
    (18399, "continuum_wave", MILESTONE, "milestone 18400 continuum wave identity"),
    # 18400 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        18376: "continuum wave fill soft unit + batch id",
        18380: "continuum bridge soft unit + batch id",
        18390: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (18391–18399), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 18393
    tag_batch = MARKERS_LO + 7  # 18398
    wave_batch = MARKERS_LO + 8  # 18399
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_18200 / gj_shell_green_18100 / gj_shell_green_18000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_18200 / gj_libcgj_green_18100 / gj_libcgj_green_18000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_18200 / gj_bar3_ready_18100 / gj_bar3_ready_18000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_18200 / gj_product_score_18100 / gj_product_score_18000\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_18200 / gj_continuum_ready_18100 /\n"
            f" * gj_continuum_ready_18000 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_18200 / gj_smoke_soft_18100 / gj_smoke_soft_18000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_18200 / gj_dyn_soft_18100 / gj_dyn_soft_18000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_18200 / gj_milestone_tag_18100 / gj_milestone_tag_18000,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_18200 / gj_continuum_wave_18100 /\n"
            f" * gj_continuum_wave_18000, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch18400.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 18399
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 18391–18399; 18400 is capstone
