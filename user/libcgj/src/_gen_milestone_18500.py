...
"""
Soft continuum generator — MILESTONE 18500 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 18401–18500 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=18500   when parent has wired through graph_batch18500.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 18401 18500
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 18401 18500
...
graph batches ending at M. For M = **18500** the exclusive range is
**18401–18500** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 18401–18450  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 18451–18475  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 18476–18490  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 18491–18500  │ decade soft gates (Wave 43 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 18500 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_18500        1          graph_batch18491.c
  gj_libcgj_green_18500       1          graph_batch18492.c
  gj_bar3_ready_18500         0          graph_batch18493.c  (bar3 open)
  gj_product_score_18500      0          graph_batch18494.c  (score open)
  gj_continuum_ready_18500    1          graph_batch18495.c
  gj_smoke_soft_18500         1          graph_batch18496.c
  gj_dyn_soft_18500           1          graph_batch18497.c
  gj_milestone_tag_18500      18500      graph_batch18498.c
  gj_continuum_wave_18500     18500      graph_batch18499.c
  gj_batch_id_18500           18500      graph_batch18500.c
  gj_graph_milestone_18500    18500      graph_batch18500.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=18500`` means parent wired CGJ_SRCS through
    ``graph_batch18500.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_18500`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_18500`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M18500_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_18400`` / ``gj_*_18300`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 43 exclusive CREATE-ONLY lands the **markers** band
  (18491–18500) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_18500.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_18500.py

  # Restrict to marker band only (soft-gate TUs; Wave 43 exclusive):
  python3 user/libcgj/src/_gen_milestone_18500.py --band markers

  # Extension identity only (18401–18450):
  python3 user/libcgj/src/_gen_milestone_18500.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_18500.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_18400.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 18500 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 18500
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 18401
WAVE_HI = 18500  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 18401, 18450
IDENTITY_LO, IDENTITY_HI = 18451, 18475
FILL_LO, FILL_HI = 18476, 18490
MARKERS_LO, MARKERS_HI = 18491, 18500
...
# Stems become gj_<stem>_18500. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (18491, "shell_green", 1, "shell product green extract (wave 18500)"),
    (18492, "libcgj_green", 1, "libcgj product green extract (wave 18500)"),
    (18493, "bar3_ready", 0, "product bar3 readiness gate (wave 18500)"),
    (18494, "product_score", 0, "product readiness score tag (wave 18500)"),
    (18495, "continuum_ready", 1, "continuum readiness gate (wave 18500)"),
    (18496, "smoke_soft", 1, "smoke soft lamp (wave 18500)"),
    (18497, "dyn_soft", 1, "dyn soft lamp (wave 18500)"),
    (18498, "milestone_tag", MILESTONE, "milestone 18500 product tag"),
    (18499, "continuum_wave", MILESTONE, "milestone 18500 continuum wave identity"),
    # 18500 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        18476: "continuum wave fill soft unit + batch id",
        18480: "continuum bridge soft unit + batch id",
        18490: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (18491–18499), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 18493
    tag_batch = MARKERS_LO + 7  # 18498
    wave_batch = MARKERS_LO + 8  # 18499
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_18300 / gj_shell_green_18200 / gj_shell_green_18100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_18300 / gj_libcgj_green_18200 / gj_libcgj_green_18100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_18300 / gj_bar3_ready_18200 / gj_bar3_ready_18100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_18300 / gj_product_score_18200 / gj_product_score_18100\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_18300 / gj_continuum_ready_18200 /\n"
            f" * gj_continuum_ready_18100 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_18300 / gj_smoke_soft_18200 / gj_smoke_soft_18100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_18300 / gj_dyn_soft_18200 / gj_dyn_soft_18100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_18300 / gj_milestone_tag_18200 / gj_milestone_tag_18100,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_18300 / gj_continuum_wave_18200 /\n"
            f" * gj_continuum_wave_18100, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch18500.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 18499
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 18491–18499; 18500 is capstone
