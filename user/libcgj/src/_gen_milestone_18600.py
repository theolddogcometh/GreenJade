...
"""
Soft continuum generator — MILESTONE 18600 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 18501–18600 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=18600   when parent has wired through graph_batch18600.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 18501 18600
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 18501 18600
...
graph batches ending at M. For M = **18600** the exclusive range is
**18501–18600** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 18501–18550  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 18551–18575  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 18576–18590  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 18591–18600  │ decade soft gates (Wave 44 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 18600 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_18600        1          graph_batch18591.c
  gj_libcgj_green_18600       1          graph_batch18592.c
  gj_bar3_ready_18600         0          graph_batch18593.c  (bar3 open)
  gj_product_score_18600      0          graph_batch18594.c  (score open)
  gj_continuum_ready_18600    1          graph_batch18595.c
  gj_smoke_soft_18600         1          graph_batch18596.c
  gj_dyn_soft_18600           1          graph_batch18597.c
  gj_milestone_tag_18600      18600      graph_batch18598.c
  gj_continuum_wave_18600     18600      graph_batch18599.c
  gj_batch_id_18600           18600      graph_batch18600.c
  gj_graph_milestone_18600    18600      graph_batch18600.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=18600`` means parent wired CGJ_SRCS through
    ``graph_batch18600.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_18600`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_18600`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M18600_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_18500`` / ``gj_*_18400`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 44 exclusive CREATE-ONLY lands the **markers** band
  (18591–18600) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_18600.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_18600.py

  # Restrict to marker band only (soft-gate TUs; Wave 44 exclusive):
  python3 user/libcgj/src/_gen_milestone_18600.py --band markers

  # Extension identity only (18501–18550):
  python3 user/libcgj/src/_gen_milestone_18600.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_18600.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_18500.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 18600 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 18600
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 18501
WAVE_HI = 18600  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 18501, 18550
IDENTITY_LO, IDENTITY_HI = 18551, 18575
FILL_LO, FILL_HI = 18576, 18590
MARKERS_LO, MARKERS_HI = 18591, 18600
...
# Stems become gj_<stem>_18600. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (18591, "shell_green", 1, "shell product green extract (wave 18600)"),
    (18592, "libcgj_green", 1, "libcgj product green extract (wave 18600)"),
    (18593, "bar3_ready", 0, "product bar3 readiness gate (wave 18600)"),
    (18594, "product_score", 0, "product readiness score tag (wave 18600)"),
    (18595, "continuum_ready", 1, "continuum readiness gate (wave 18600)"),
    (18596, "smoke_soft", 1, "smoke soft lamp (wave 18600)"),
    (18597, "dyn_soft", 1, "dyn soft lamp (wave 18600)"),
    (18598, "milestone_tag", MILESTONE, "milestone 18600 product tag"),
    (18599, "continuum_wave", MILESTONE, "milestone 18600 continuum wave identity"),
    # 18600 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        18576: "continuum wave fill soft unit + batch id",
        18580: "continuum bridge soft unit + batch id",
        18590: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (18591–18599), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 18593
    tag_batch = MARKERS_LO + 7  # 18598
    wave_batch = MARKERS_LO + 8  # 18599
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_18400 / gj_shell_green_18300 / gj_shell_green_18200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_18400 / gj_libcgj_green_18300 / gj_libcgj_green_18200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_18400 / gj_bar3_ready_18300 / gj_bar3_ready_18200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_18400 / gj_product_score_18300 / gj_product_score_18200\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_18400 / gj_continuum_ready_18300 /\n"
            f" * gj_continuum_ready_18200 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_18400 / gj_smoke_soft_18300 / gj_smoke_soft_18200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_18400 / gj_dyn_soft_18300 / gj_dyn_soft_18200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_18400 / gj_milestone_tag_18300 / gj_milestone_tag_18200,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_18400 / gj_continuum_wave_18300 /\n"
            f" * gj_continuum_wave_18200, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch18600.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 18599
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 18591–18599; 18600 is capstone
