...
"""
Soft continuum generator — MILESTONE 18700 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 18601–18700 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=18700   when parent has wired through graph_batch18700.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 18601 18700
...
graph batches ending at M. For M = **18700** the exclusive range is
**18601–18700** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 18601–18650  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 18651–18675  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 18676–18690  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 18691–18700  │ decade soft gates (Wave 45 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 18700 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_18700        1          graph_batch18691.c
  gj_libcgj_green_18700       1          graph_batch18692.c
  gj_bar3_ready_18700         0          graph_batch18693.c  (bar3 open)
  gj_product_score_18700      0          graph_batch18694.c  (score open)
  gj_continuum_ready_18700    1          graph_batch18695.c
  gj_smoke_soft_18700         1          graph_batch18696.c
  gj_dyn_soft_18700           1          graph_batch18697.c
  gj_milestone_tag_18700      18700      graph_batch18698.c
  gj_continuum_wave_18700     18700      graph_batch18699.c
  gj_batch_id_18700           18700      graph_batch18700.c
  gj_graph_milestone_18700    18700      graph_batch18700.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=18700`` means parent wired CGJ_SRCS through
    ``graph_batch18700.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_18700`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_18700`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M18700_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_18600`` / ``gj_*_18500`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 45 exclusive CREATE-ONLY lands the **markers** band
  (18691–18700) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_18700.py

  # Restrict to marker band only (soft-gate TUs; Wave 45 exclusive):
  python3 user/libcgj/src/_gen_milestone_18700.py --band markers

  # Extension identity only (18601–18650):
  python3 user/libcgj/src/_gen_milestone_18700.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_18700.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_18600.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 18700 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 18700
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 18601
WAVE_HI = 18700  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 18601, 18650
IDENTITY_LO, IDENTITY_HI = 18651, 18675
FILL_LO, FILL_HI = 18676, 18690
MARKERS_LO, MARKERS_HI = 18691, 18700
...
# Stems become gj_<stem>_18700. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (18691, "shell_green", 1, "shell product green extract (wave 18700)"),
    (18692, "libcgj_green", 1, "libcgj product green extract (wave 18700)"),
    (18693, "bar3_ready", 0, "product bar3 readiness gate (wave 18700)"),
    (18694, "product_score", 0, "product readiness score tag (wave 18700)"),
    (18695, "continuum_ready", 1, "continuum readiness gate (wave 18700)"),
    (18696, "smoke_soft", 1, "smoke soft lamp (wave 18700)"),
    (18697, "dyn_soft", 1, "dyn soft lamp (wave 18700)"),
    (18698, "milestone_tag", MILESTONE, "milestone 18700 product tag"),
    (18699, "continuum_wave", MILESTONE, "milestone 18700 continuum wave identity"),
    # 18700 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        18676: "continuum wave fill soft unit + batch id",
        18680: "continuum bridge soft unit + batch id",
        18690: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (18691–18699), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 18693
    tag_batch = MARKERS_LO + 7  # 18698
    wave_batch = MARKERS_LO + 8  # 18699
...
def render_milestone_capstone() -> str:
    """graph_batch18700.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 18699
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 18691–18699; 18700 is capstone
