...
"""
Soft continuum generator — MILESTONE 18800 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 18701–18800 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=18800   when parent has wired through graph_batch18800.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 18701 18800
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 18701 18800
...
graph batches ending at M. For M = **18800** the exclusive range is
**18701–18800** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 18701–18750  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 18751–18775  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 18776–18790  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 18791–18800  │ decade soft gates (Wave 46 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 18800 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_18800        1          graph_batch18791.c
  gj_libcgj_green_18800       1          graph_batch18792.c
  gj_bar3_ready_18800         0          graph_batch18793.c  (bar3 open)
  gj_product_score_18800      0          graph_batch18794.c  (score open)
  gj_continuum_ready_18800    1          graph_batch18795.c
  gj_smoke_soft_18800         1          graph_batch18796.c
  gj_dyn_soft_18800           1          graph_batch18797.c
  gj_milestone_tag_18800      18800      graph_batch18798.c
  gj_continuum_wave_18800     18800      graph_batch18799.c
  gj_batch_id_18800           18800      graph_batch18800.c
  gj_graph_milestone_18800    18800      graph_batch18800.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=18800`` means parent wired CGJ_SRCS through
    ``graph_batch18800.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_18800`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_18800`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M18800_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_18700`` / ``gj_*_18600`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 46 exclusive CREATE-ONLY lands the **markers** band
  (18791–18800) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_18800.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_18800.py

  # Restrict to marker band only (soft-gate TUs; Wave 46 exclusive):
  python3 user/libcgj/src/_gen_milestone_18800.py --band markers

  # Extension identity only (18701–18750):
  python3 user/libcgj/src/_gen_milestone_18800.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_18800.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_18700.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 18800 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 18800
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 18701
WAVE_HI = 18800  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 18701, 18750
IDENTITY_LO, IDENTITY_HI = 18751, 18775
FILL_LO, FILL_HI = 18776, 18790
MARKERS_LO, MARKERS_HI = 18791, 18800
...
# Stems become gj_<stem>_18800. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (18791, "shell_green", 1, "shell product green extract (wave 18800)"),
    (18792, "libcgj_green", 1, "libcgj product green extract (wave 18800)"),
    (18793, "bar3_ready", 0, "product bar3 readiness gate (wave 18800)"),
    (18794, "product_score", 0, "product readiness score tag (wave 18800)"),
    (18795, "continuum_ready", 1, "continuum readiness gate (wave 18800)"),
    (18796, "smoke_soft", 1, "smoke soft lamp (wave 18800)"),
    (18797, "dyn_soft", 1, "dyn soft lamp (wave 18800)"),
    (18798, "milestone_tag", MILESTONE, "milestone 18800 product tag"),
    (18799, "continuum_wave", MILESTONE, "milestone 18800 continuum wave identity"),
    # 18800 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        18776: "continuum wave fill soft unit + batch id",
        18780: "continuum bridge soft unit + batch id",
        18790: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (18791–18799), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 18793
    tag_batch = MARKERS_LO + 7  # 18798
    wave_batch = MARKERS_LO + 8  # 18799
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_18600 / gj_shell_green_18500 / gj_shell_green_18400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_18600 / gj_libcgj_green_18500 / gj_libcgj_green_18400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_18600 / gj_bar3_ready_18500 / gj_bar3_ready_18400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_18600 / gj_product_score_18500 / gj_product_score_18400\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_18600 / gj_smoke_soft_18500 / gj_smoke_soft_18400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_18600 / gj_dyn_soft_18500 / gj_dyn_soft_18400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_18600 / gj_milestone_tag_18500 / gj_milestone_tag_18400,\n"
...
def render_milestone_capstone() -> str:
    """graph_batch18800.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 18799
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 18791–18799; 18800 is capstone
