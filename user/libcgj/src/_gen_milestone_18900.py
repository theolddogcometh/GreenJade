...
"""
Soft continuum generator — MILESTONE 18900 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 18801–18900 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=18900   when parent has wired through graph_batch18900.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 18801 18900
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 18801 18900
...
graph batches ending at M. For M = **18900** the exclusive range is
**18801–18900** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 18801–18850  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 18851–18875  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 18876–18890  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 18891–18900  │ decade soft gates (Wave 47 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 18900 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_18900        1          graph_batch18891.c
  gj_libcgj_green_18900       1          graph_batch18892.c
  gj_bar3_ready_18900         0          graph_batch18893.c  (bar3 open)
  gj_product_score_18900      0          graph_batch18894.c  (score open)
  gj_continuum_ready_18900    1          graph_batch18895.c
  gj_smoke_soft_18900         1          graph_batch18896.c
  gj_dyn_soft_18900           1          graph_batch18897.c
  gj_milestone_tag_18900      18900      graph_batch18898.c
  gj_continuum_wave_18900     18900      graph_batch18899.c
  gj_batch_id_18900           18900      graph_batch18900.c
  gj_graph_milestone_18900    18900      graph_batch18900.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=18900`` means parent wired CGJ_SRCS through
    ``graph_batch18900.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_18900`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_18900`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M18900_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_18800`` / ``gj_*_18700`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 47 exclusive CREATE-ONLY lands the **markers** band
  (18891–18900) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_18900.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_18900.py

  # Restrict to marker band only (soft-gate TUs; Wave 47 exclusive):
  python3 user/libcgj/src/_gen_milestone_18900.py --band markers

  # Extension identity only (18801–18850):
  python3 user/libcgj/src/_gen_milestone_18900.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_18900.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_18800.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 18900 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 18900
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 18801
WAVE_HI = 18900  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 18801, 18850
IDENTITY_LO, IDENTITY_HI = 18851, 18875
FILL_LO, FILL_HI = 18876, 18890
MARKERS_LO, MARKERS_HI = 18891, 18900
...
# Stems become gj_<stem>_18900. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (18891, "shell_green", 1, "shell product green extract (wave 18900)"),
    (18892, "libcgj_green", 1, "libcgj product green extract (wave 18900)"),
    (18893, "bar3_ready", 0, "product bar3 readiness gate (wave 18900)"),
    (18894, "product_score", 0, "product readiness score tag (wave 18900)"),
    (18895, "continuum_ready", 1, "continuum readiness gate (wave 18900)"),
    (18896, "smoke_soft", 1, "smoke soft lamp (wave 18900)"),
    (18897, "dyn_soft", 1, "dyn soft lamp (wave 18900)"),
    (18898, "milestone_tag", MILESTONE, "milestone 18900 product tag"),
    (18899, "continuum_wave", MILESTONE, "milestone 18900 continuum wave identity"),
    # 18900 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        18876: "continuum wave fill soft unit + batch id",
        18880: "continuum bridge soft unit + batch id",
        18890: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (18891–18899), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 18893
    tag_batch = MARKERS_LO + 7  # 18898
    wave_batch = MARKERS_LO + 8  # 18899
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_18700 / gj_shell_green_18600 / gj_shell_green_18500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_18700 / gj_libcgj_green_18600 / gj_libcgj_green_18500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_18700 / gj_bar3_ready_18600 / gj_bar3_ready_18500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_18700 / gj_product_score_18600 / gj_product_score_18500\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_18700 / gj_continuum_ready_18600 /\n"
            f" * gj_continuum_ready_18500 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_18700 / gj_smoke_soft_18600 / gj_smoke_soft_18500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_18700 / gj_dyn_soft_18600 / gj_dyn_soft_18500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_18700 / gj_milestone_tag_18600 / gj_milestone_tag_18500,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_18700 / gj_continuum_wave_18600 /\n"
            f" * gj_continuum_wave_18500, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch18900.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 18899
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 18891–18899; 18900 is capstone
