...
"""
Soft continuum generator — MILESTONE 19000 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 18901–19000 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=19000   when parent has wired through graph_batch19000.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 18901 19000
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 18901 19000
...
graph batches ending at M. For M = **19000** the exclusive range is
**18901–19000** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 18901–18950  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 18951–18975  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 18976–18990  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 18991–19000  │ decade soft gates (Wave 48 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 19000 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_19000        1          graph_batch18991.c
  gj_libcgj_green_19000       1          graph_batch18992.c
  gj_bar3_ready_19000         0          graph_batch18993.c  (bar3 open)
  gj_product_score_19000      0          graph_batch18994.c  (score open)
  gj_continuum_ready_19000    1          graph_batch18995.c
  gj_smoke_soft_19000         1          graph_batch18996.c
  gj_dyn_soft_19000           1          graph_batch18997.c
  gj_milestone_tag_19000      19000      graph_batch18998.c
  gj_continuum_wave_19000     19000      graph_batch18999.c
  gj_batch_id_19000           19000      graph_batch19000.c
  gj_graph_milestone_19000    19000      graph_batch19000.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=19000`` means parent wired CGJ_SRCS through
    ``graph_batch19000.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_19000`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_19000`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M19000_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_18900`` / ``gj_*_18800`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 48 exclusive CREATE-ONLY lands the **markers** band
  (18991–19000) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_19000.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_19000.py

  # Restrict to marker band only (soft-gate TUs; Wave 48 exclusive):
  python3 user/libcgj/src/_gen_milestone_19000.py --band markers

  # Extension identity only (18901–18950):
  python3 user/libcgj/src/_gen_milestone_19000.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_19000.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_18900.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 19000 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 19000
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 18901
WAVE_HI = 19000  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 18901, 18950
IDENTITY_LO, IDENTITY_HI = 18951, 18975
FILL_LO, FILL_HI = 18976, 18990
MARKERS_LO, MARKERS_HI = 18991, 19000
...
# Stems become gj_<stem>_19000. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (18991, "shell_green", 1, "shell product green extract (wave 19000)"),
    (18992, "libcgj_green", 1, "libcgj product green extract (wave 19000)"),
    (18993, "bar3_ready", 0, "product bar3 readiness gate (wave 19000)"),
    (18994, "product_score", 0, "product readiness score tag (wave 19000)"),
    (18995, "continuum_ready", 1, "continuum readiness gate (wave 19000)"),
    (18996, "smoke_soft", 1, "smoke soft lamp (wave 19000)"),
    (18997, "dyn_soft", 1, "dyn soft lamp (wave 19000)"),
    (18998, "milestone_tag", MILESTONE, "milestone 19000 product tag"),
    (18999, "continuum_wave", MILESTONE, "milestone 19000 continuum wave identity"),
    # 19000 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        18976: "continuum wave fill soft unit + batch id",
        18980: "continuum bridge soft unit + batch id",
        18990: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (18991–18999), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 18993
    tag_batch = MARKERS_LO + 7  # 18998
    wave_batch = MARKERS_LO + 8  # 18999
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_18800 / gj_shell_green_18700 / gj_shell_green_18600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_18800 / gj_libcgj_green_18700 / gj_libcgj_green_18600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_18800 / gj_bar3_ready_18700 / gj_bar3_ready_18600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_18800 / gj_product_score_18700 / gj_product_score_18600\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_18800 / gj_continuum_ready_18700 /\n"
            f" * gj_continuum_ready_18600 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_18800 / gj_smoke_soft_18700 / gj_smoke_soft_18600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_18800 / gj_dyn_soft_18700 / gj_dyn_soft_18600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_18800 / gj_milestone_tag_18700 / gj_milestone_tag_18600,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_18800 / gj_continuum_wave_18700 /\n"
            f" * gj_continuum_wave_18600, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch19000.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 18999
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 18991–18999; 19000 is capstone
