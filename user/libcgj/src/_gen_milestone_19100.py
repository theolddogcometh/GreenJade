...
"""
Soft continuum generator — MILESTONE 19100 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 19001–19100 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=19100   when parent has wired through graph_batch19100.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 19001 19100
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 19001 19100
...
graph batches ending at M. For M = **19100** the exclusive range is
**19001–19100** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 19001–19050  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 19051–19075  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 19076–19090  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 19091–19100  │ decade soft gates (Wave 49 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 19100 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_19100        1          graph_batch19091.c
  gj_libcgj_green_19100       1          graph_batch19092.c
  gj_bar3_ready_19100         0          graph_batch19093.c  (bar3 open)
  gj_product_score_19100      0          graph_batch19094.c  (score open)
  gj_continuum_ready_19100    1          graph_batch19095.c
  gj_smoke_soft_19100         1          graph_batch19096.c
  gj_dyn_soft_19100           1          graph_batch19097.c
  gj_milestone_tag_19100      19100      graph_batch19098.c
  gj_continuum_wave_19100     19100      graph_batch19099.c
  gj_batch_id_19100           19100      graph_batch19100.c
  gj_graph_milestone_19100    19100      graph_batch19100.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=19100`` means parent wired CGJ_SRCS through
    ``graph_batch19100.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_19100`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_19100`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M19100_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_19000`` / ``gj_*_18900`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 49 exclusive CREATE-ONLY lands the **markers** band
  (19091–19100) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_19100.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_19100.py

  # Restrict to marker band only (soft-gate TUs; Wave 49 exclusive):
  python3 user/libcgj/src/_gen_milestone_19100.py --band markers

  # Extension identity only (19001–19050):
  python3 user/libcgj/src/_gen_milestone_19100.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_19100.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_19000.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 19100 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 19100
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 19001
WAVE_HI = 19100  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 19001, 19050
IDENTITY_LO, IDENTITY_HI = 19051, 19075
FILL_LO, FILL_HI = 19076, 19090
MARKERS_LO, MARKERS_HI = 19091, 19100
...
# Stems become gj_<stem>_19100. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (19091, "shell_green", 1, "shell product green extract (wave 19100)"),
    (19092, "libcgj_green", 1, "libcgj product green extract (wave 19100)"),
    (19093, "bar3_ready", 0, "product bar3 readiness gate (wave 19100)"),
    (19094, "product_score", 0, "product readiness score tag (wave 19100)"),
    (19095, "continuum_ready", 1, "continuum readiness gate (wave 19100)"),
    (19096, "smoke_soft", 1, "smoke soft lamp (wave 19100)"),
    (19097, "dyn_soft", 1, "dyn soft lamp (wave 19100)"),
    (19098, "milestone_tag", MILESTONE, "milestone 19100 product tag"),
    (19099, "continuum_wave", MILESTONE, "milestone 19100 continuum wave identity"),
    # 19100 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        19076: "continuum wave fill soft unit + batch id",
        19080: "continuum bridge soft unit + batch id",
        19090: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (19091–19099), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 19093
    tag_batch = MARKERS_LO + 7  # 19098
    wave_batch = MARKERS_LO + 8  # 19099
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_18900 / gj_shell_green_18800 / gj_shell_green_18700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_18900 / gj_libcgj_green_18800 / gj_libcgj_green_18700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_18900 / gj_bar3_ready_18800 / gj_bar3_ready_18700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_18900 / gj_product_score_18800 / gj_product_score_18700\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_18900 / gj_continuum_ready_18800 /\n"
            f" * gj_continuum_ready_18700 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_18900 / gj_smoke_soft_18800 / gj_smoke_soft_18700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_18900 / gj_dyn_soft_18800 / gj_dyn_soft_18700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_18900 / gj_milestone_tag_18800 / gj_milestone_tag_18700,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_18900 / gj_continuum_wave_18800 /\n"
            f" * gj_continuum_wave_18700, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch19100.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 19099
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 19091–19099; 19100 is capstone
