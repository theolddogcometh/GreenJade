...
"""
Soft continuum generator — MILESTONE 19200 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 19101–19200 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=19200   when parent has wired through graph_batch19200.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 19101 19200
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 19101 19200
...
graph batches ending at M. For M = **19200** the exclusive range is
**19101–19200** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 19101–19150  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 19151–19175  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 19176–19190  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 19191–19200  │ decade soft gates (Wave 50 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 19200 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_19200        1          graph_batch19191.c
  gj_libcgj_green_19200       1          graph_batch19192.c
  gj_bar3_ready_19200         0          graph_batch19193.c  (bar3 open)
  gj_product_score_19200      0          graph_batch19194.c  (score open)
  gj_continuum_ready_19200    1          graph_batch19195.c
  gj_smoke_soft_19200         1          graph_batch19196.c
  gj_dyn_soft_19200           1          graph_batch19197.c
  gj_milestone_tag_19200      19200      graph_batch19198.c
  gj_continuum_wave_19200     19200      graph_batch19199.c
  gj_batch_id_19200           19200      graph_batch19200.c
  gj_graph_milestone_19200    19200      graph_batch19200.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=19200`` means parent wired CGJ_SRCS through
    ``graph_batch19200.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_19200`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_19200`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M19200_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_19100`` / ``gj_*_19000`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 50 exclusive CREATE-ONLY lands the **markers** band
  (19191–19200) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_19200.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_19200.py

  # Restrict to marker band only (soft-gate TUs; Wave 50 exclusive):
  python3 user/libcgj/src/_gen_milestone_19200.py --band markers

  # Extension identity only (19101–19150):
  python3 user/libcgj/src/_gen_milestone_19200.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_19200.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_19100.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 19200 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 19200
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 19101
WAVE_HI = 19200  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 19101, 19150
IDENTITY_LO, IDENTITY_HI = 19151, 19175
FILL_LO, FILL_HI = 19176, 19190
MARKERS_LO, MARKERS_HI = 19191, 19200
...
# Stems become gj_<stem>_19200. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (19191, "shell_green", 1, "shell product green extract (wave 19200)"),
    (19192, "libcgj_green", 1, "libcgj product green extract (wave 19200)"),
    (19193, "bar3_ready", 0, "product bar3 readiness gate (wave 19200)"),
    (19194, "product_score", 0, "product readiness score tag (wave 19200)"),
    (19195, "continuum_ready", 1, "continuum readiness gate (wave 19200)"),
    (19196, "smoke_soft", 1, "smoke soft lamp (wave 19200)"),
    (19197, "dyn_soft", 1, "dyn soft lamp (wave 19200)"),
    (19198, "milestone_tag", MILESTONE, "milestone 19200 product tag"),
    (19199, "continuum_wave", MILESTONE, "milestone 19200 continuum wave identity"),
    # 19200 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        19176: "continuum wave fill soft unit + batch id",
        19180: "continuum bridge soft unit + batch id",
        19190: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (19191–19199), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 19193
    tag_batch = MARKERS_LO + 7  # 19198
    wave_batch = MARKERS_LO + 8  # 19199
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_19100 / gj_shell_green_19000 / gj_shell_green_18900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_19100 / gj_libcgj_green_19000 / gj_libcgj_green_18900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_19100 / gj_bar3_ready_19000 / gj_bar3_ready_18900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_19100 / gj_product_score_19000 / gj_product_score_18900\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_18600 / gj_continuum_ready_18500 /\n"
            f" * gj_continuum_ready_18400 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_19100 / gj_smoke_soft_19000 / gj_smoke_soft_18900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_19100 / gj_dyn_soft_19000 / gj_dyn_soft_18900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_19100 / gj_milestone_tag_19000 / gj_milestone_tag_18900,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_18600 / gj_continuum_wave_18500 /\n"
            f" * gj_continuum_wave_18400, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch19200.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 19199
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 19191–19199; 19200 is capstone
