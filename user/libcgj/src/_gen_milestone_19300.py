...
"""
Soft continuum generator — MILESTONE 19300 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 19201–19300 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=19300   when parent has wired through graph_batch19300.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 19201 19300
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 19201 19300
...
graph batches ending at M. For M = **19300** the exclusive range is
**19201–19300** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 19201–19250  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 19251–19275  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 19276–19290  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 19291–19300  │ decade soft gates (Wave 51 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 19300 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_19300        1          graph_batch19291.c
  gj_libcgj_green_19300       1          graph_batch19292.c
  gj_bar3_ready_19300         0          graph_batch19293.c  (bar3 open)
  gj_product_score_19300      0          graph_batch19294.c  (score open)
  gj_continuum_ready_19300    1          graph_batch19295.c
  gj_smoke_soft_19300         1          graph_batch19296.c
  gj_dyn_soft_19300           1          graph_batch19297.c
  gj_milestone_tag_19300      19300      graph_batch19298.c
  gj_continuum_wave_19300     19300      graph_batch19299.c
  gj_batch_id_19300           19300      graph_batch19300.c
  gj_graph_milestone_19300    19300      graph_batch19300.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=19300`` means parent wired CGJ_SRCS through
    ``graph_batch19300.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_19300`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_19300`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M19300_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_19200`` / ``gj_*_19100`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 51 exclusive CREATE-ONLY lands the **markers** band
  (19291–19300) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_19300.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_19300.py

  # Restrict to marker band only (soft-gate TUs; Wave 51 exclusive):
  python3 user/libcgj/src/_gen_milestone_19300.py --band markers

  # Extension identity only (19201–19250):
  python3 user/libcgj/src/_gen_milestone_19300.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_19300.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_19200.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 19300 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 19300
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 19201
WAVE_HI = 19300  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 19201, 19250
IDENTITY_LO, IDENTITY_HI = 19251, 19275
FILL_LO, FILL_HI = 19276, 19290
MARKERS_LO, MARKERS_HI = 19291, 19300
...
# Stems become gj_<stem>_19300. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (19291, "shell_green", 1, "shell product green extract (wave 19300)"),
    (19292, "libcgj_green", 1, "libcgj product green extract (wave 19300)"),
    (19293, "bar3_ready", 0, "product bar3 readiness gate (wave 19300)"),
    (19294, "product_score", 0, "product readiness score tag (wave 19300)"),
    (19295, "continuum_ready", 1, "continuum readiness gate (wave 19300)"),
    (19296, "smoke_soft", 1, "smoke soft lamp (wave 19300)"),
    (19297, "dyn_soft", 1, "dyn soft lamp (wave 19300)"),
    (19298, "milestone_tag", MILESTONE, "milestone 19300 product tag"),
    (19299, "continuum_wave", MILESTONE, "milestone 19300 continuum wave identity"),
    # 19300 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        19276: "continuum wave fill soft unit + batch id",
        19280: "continuum bridge soft unit + batch id",
        19290: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (19291–19299), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 19293
    tag_batch = MARKERS_LO + 7  # 19298
    wave_batch = MARKERS_LO + 8  # 19299
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_19200 / gj_shell_green_19100 / gj_shell_green_19000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_19200 / gj_libcgj_green_19100 / gj_libcgj_green_19000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_19200 / gj_bar3_ready_19100 / gj_bar3_ready_19000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_19200 / gj_product_score_19100 / gj_product_score_19000\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_19200 / gj_continuum_ready_19100 /\n"
            f" * gj_continuum_ready_19000 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_19200 / gj_smoke_soft_19100 / gj_smoke_soft_19000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_19200 / gj_dyn_soft_19100 / gj_dyn_soft_19000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_19200 / gj_milestone_tag_19100 / gj_milestone_tag_19000,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_19200 / gj_continuum_wave_19100 /\n"
            f" * gj_continuum_wave_19000, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch19300.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 19299
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 19291–19299; 19300 is capstone
