...
"""
Soft continuum generator — MILESTONE 19400 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 19301–19400 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=19400   when parent has wired through graph_batch19400.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 19301 19400
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 19301 19400
...
graph batches ending at M. For M = **19400** the exclusive range is
**19301–19400** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 19301–19350  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 19351–19375  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 19376–19390  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 19391–19400  │ decade soft gates (Wave 52 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 19400 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_19400        1          graph_batch19391.c
  gj_libcgj_green_19400       1          graph_batch19392.c
  gj_bar3_ready_19400         0          graph_batch19393.c  (bar3 open)
  gj_product_score_19400      0          graph_batch19394.c  (score open)
  gj_continuum_ready_19400    1          graph_batch19395.c
  gj_smoke_soft_19400         1          graph_batch19396.c
  gj_dyn_soft_19400           1          graph_batch19397.c
  gj_milestone_tag_19400      19400      graph_batch19398.c
  gj_continuum_wave_19400     19400      graph_batch19399.c
  gj_batch_id_19400           19400      graph_batch19400.c
  gj_graph_milestone_19400    19400      graph_batch19400.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=19400`` means parent wired CGJ_SRCS through
    ``graph_batch19400.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_19400`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_19400`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M19400_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_19300`` / ``gj_*_19200`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 52 exclusive CREATE-ONLY lands the **markers** band
  (19391–19400) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_19400.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_19400.py

  # Restrict to marker band only (soft-gate TUs; Wave 52 exclusive):
  python3 user/libcgj/src/_gen_milestone_19400.py --band markers

  # Extension identity only (19301–19350):
  python3 user/libcgj/src/_gen_milestone_19400.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_19400.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_19300.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 19400 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 19400
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 19301
WAVE_HI = 19400  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 19301, 19350
IDENTITY_LO, IDENTITY_HI = 19351, 19375
FILL_LO, FILL_HI = 19376, 19390
MARKERS_LO, MARKERS_HI = 19391, 19400
...
# Stems become gj_<stem>_19400. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (19391, "shell_green", 1, "shell product green extract (wave 19400)"),
    (19392, "libcgj_green", 1, "libcgj product green extract (wave 19400)"),
    (19393, "bar3_ready", 0, "product bar3 readiness gate (wave 19400)"),
    (19394, "product_score", 0, "product readiness score tag (wave 19400)"),
    (19395, "continuum_ready", 1, "continuum readiness gate (wave 19400)"),
    (19396, "smoke_soft", 1, "smoke soft lamp (wave 19400)"),
    (19397, "dyn_soft", 1, "dyn soft lamp (wave 19400)"),
    (19398, "milestone_tag", MILESTONE, "milestone 19400 product tag"),
    (19399, "continuum_wave", MILESTONE, "milestone 19400 continuum wave identity"),
    # 19400 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        19376: "continuum wave fill soft unit + batch id",
        19380: "continuum bridge soft unit + batch id",
        19390: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (19391–19399), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 19393
    tag_batch = MARKERS_LO + 7  # 19398
    wave_batch = MARKERS_LO + 8  # 19399
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_19300 / gj_shell_green_19200 / gj_shell_green_19100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_19300 / gj_libcgj_green_19200 / gj_libcgj_green_19100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_19300 / gj_bar3_ready_19200 / gj_bar3_ready_19100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_19300 / gj_product_score_19200 / gj_product_score_19100\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_19300 / gj_continuum_ready_19200 /\n"
            f" * gj_continuum_ready_19100 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_19300 / gj_smoke_soft_19200 / gj_smoke_soft_19100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_19300 / gj_dyn_soft_19200 / gj_dyn_soft_19100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_19300 / gj_milestone_tag_19200 / gj_milestone_tag_19100,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_19300 / gj_continuum_wave_19200 /\n"
            f" * gj_continuum_wave_19100, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch19400.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 19399
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 19391–19399; 19400 is capstone
