...
"""
Soft continuum generator — MILESTONE 19500 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 19401–19500 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=19500   when parent has wired through graph_batch19500.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 19401 19500
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 19401 19500
...
graph batches ending at M. For M = **19500** the exclusive range is
**19401–19500** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 19401–19450  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 19451–19475  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 19476–19490  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 19491–19500  │ decade soft gates (Wave 53 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 19500 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_19500        1          graph_batch19491.c
  gj_libcgj_green_19500       1          graph_batch19492.c
  gj_bar3_ready_19500         0          graph_batch19493.c  (bar3 open)
  gj_product_score_19500      0          graph_batch19494.c  (score open)
  gj_continuum_ready_19500    1          graph_batch19495.c
  gj_smoke_soft_19500         1          graph_batch19496.c
  gj_dyn_soft_19500           1          graph_batch19497.c
  gj_milestone_tag_19500      19500      graph_batch19498.c
  gj_continuum_wave_19500     19500      graph_batch19499.c
  gj_batch_id_19500           19500      graph_batch19500.c
  gj_graph_milestone_19500    19500      graph_batch19500.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=19500`` means parent wired CGJ_SRCS through
    ``graph_batch19500.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_19500`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_19500`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M19500_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_19400`` / ``gj_*_19300`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 53 exclusive CREATE-ONLY lands the **markers** band
  (19491–19500) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_19500.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_19500.py

  # Restrict to marker band only (soft-gate TUs; Wave 53 exclusive):
  python3 user/libcgj/src/_gen_milestone_19500.py --band markers

  # Extension identity only (19401–19450):
  python3 user/libcgj/src/_gen_milestone_19500.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_19500.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_19400.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 19500 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 19500
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 19401
WAVE_HI = 19500  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 19401, 19450
IDENTITY_LO, IDENTITY_HI = 19451, 19475
FILL_LO, FILL_HI = 19476, 19490
MARKERS_LO, MARKERS_HI = 19491, 19500
...
# Stems become gj_<stem>_19500. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (19491, "shell_green", 1, "shell product green extract (wave 19500)"),
    (19492, "libcgj_green", 1, "libcgj product green extract (wave 19500)"),
    (19493, "bar3_ready", 0, "product bar3 readiness gate (wave 19500)"),
    (19494, "product_score", 0, "product readiness score tag (wave 19500)"),
    (19495, "continuum_ready", 1, "continuum readiness gate (wave 19500)"),
    (19496, "smoke_soft", 1, "smoke soft lamp (wave 19500)"),
    (19497, "dyn_soft", 1, "dyn soft lamp (wave 19500)"),
    (19498, "milestone_tag", MILESTONE, "milestone 19500 product tag"),
    (19499, "continuum_wave", MILESTONE, "milestone 19500 continuum wave identity"),
    # 19500 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        19476: "continuum wave fill soft unit + batch id",
        19480: "continuum bridge soft unit + batch id",
        19490: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (19491–19499), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 19493
    tag_batch = MARKERS_LO + 7  # 19498
    wave_batch = MARKERS_LO + 8  # 19499
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_19400 / gj_shell_green_19300 / gj_shell_green_19200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_19400 / gj_libcgj_green_19300 / gj_libcgj_green_19200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_19400 / gj_bar3_ready_19300 / gj_bar3_ready_19200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_19400 / gj_product_score_19300 / gj_product_score_19200\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_19400 / gj_smoke_soft_19300 / gj_smoke_soft_19200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_19400 / gj_dyn_soft_19300 / gj_dyn_soft_19200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_19400 / gj_milestone_tag_19300 / gj_milestone_tag_19200,\n"
...
def render_milestone_capstone() -> str:
    """graph_batch19500.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 19499
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 19491–19499; 19500 is capstone
