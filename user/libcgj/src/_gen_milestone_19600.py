...
"""
Soft continuum generator — MILESTONE 19600 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 19501–19600 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=19600   when parent has wired through graph_batch19600.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 19501 19600
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 19501 19600
...
graph batches ending at M. For M = **19600** the exclusive range is
**19501–19600** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 19501–19550  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 19551–19575  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 19576–19590  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 19591–19600  │ decade soft gates (Wave 54 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 19600 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_19600        1          graph_batch19591.c
  gj_libcgj_green_19600       1          graph_batch19592.c
  gj_bar3_ready_19600         0          graph_batch19593.c  (bar3 open)
  gj_product_score_19600      0          graph_batch19594.c  (score open)
  gj_continuum_ready_19600    1          graph_batch19595.c
  gj_smoke_soft_19600         1          graph_batch19596.c
  gj_dyn_soft_19600           1          graph_batch19597.c
  gj_milestone_tag_19600      19600      graph_batch19598.c
  gj_continuum_wave_19600     19600      graph_batch19599.c
  gj_batch_id_19600           19600      graph_batch19600.c
  gj_graph_milestone_19600    19600      graph_batch19600.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=19600`` means parent wired CGJ_SRCS through
    ``graph_batch19600.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_19600`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_19600`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M19600_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_19500`` / ``gj_*_19400`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 54 exclusive CREATE-ONLY lands the **markers** band
  (19591–19600) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_19600.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_19600.py

  # Restrict to marker band only (soft-gate TUs; Wave 54 exclusive):
  python3 user/libcgj/src/_gen_milestone_19600.py --band markers

  # Extension identity only (19501–19550):
  python3 user/libcgj/src/_gen_milestone_19600.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_19600.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_19500.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 19600 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 19600
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 19501
WAVE_HI = 19600  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 19501, 19550
IDENTITY_LO, IDENTITY_HI = 19551, 19575
FILL_LO, FILL_HI = 19576, 19590
MARKERS_LO, MARKERS_HI = 19591, 19600
...
# Stems become gj_<stem>_19600. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (19591, "shell_green", 1, "shell product green extract (wave 19600)"),
    (19592, "libcgj_green", 1, "libcgj product green extract (wave 19600)"),
    (19593, "bar3_ready", 0, "product bar3 readiness gate (wave 19600)"),
    (19594, "product_score", 0, "product readiness score tag (wave 19600)"),
    (19595, "continuum_ready", 1, "continuum readiness gate (wave 19600)"),
    (19596, "smoke_soft", 1, "smoke soft lamp (wave 19600)"),
    (19597, "dyn_soft", 1, "dyn soft lamp (wave 19600)"),
    (19598, "milestone_tag", MILESTONE, "milestone 19600 product tag"),
    (19599, "continuum_wave", MILESTONE, "milestone 19600 continuum wave identity"),
    # 19600 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        19576: "continuum wave fill soft unit + batch id",
        19580: "continuum bridge soft unit + batch id",
        19590: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (19591–19599), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 19593
    tag_batch = MARKERS_LO + 7  # 19598
    wave_batch = MARKERS_LO + 8  # 19599
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_19500 / gj_shell_green_19400 / gj_shell_green_19300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_19500 / gj_libcgj_green_19400 / gj_libcgj_green_19300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_19500 / gj_bar3_ready_19400 / gj_bar3_ready_19300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_19500 / gj_product_score_19400 / gj_product_score_19300\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_19500 / gj_continuum_ready_19400 /\n"
            f" * gj_continuum_ready_19300 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_19500 / gj_smoke_soft_19400 / gj_smoke_soft_19300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_19500 / gj_dyn_soft_19400 / gj_dyn_soft_19300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_19500 / gj_milestone_tag_19400 / gj_milestone_tag_19300,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_19500 / gj_continuum_wave_19400 /\n"
            f" * gj_continuum_wave_19300, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch19600.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 19599
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 19591–19599; 19600 is capstone
