...
"""
Soft continuum generator — MILESTONE 19700 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 19601–19700 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=19700   when parent has wired through graph_batch19700.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 19601 19700
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 19601 19700
...
graph batches ending at M. For M = **19700** the exclusive range is
**19601–19700** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 19601–19650  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 19651–19675  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 19676–19690  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 19691–19700  │ decade soft gates (Wave 55 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 19700 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_19700        1          graph_batch19691.c
  gj_libcgj_green_19700       1          graph_batch19692.c
  gj_bar3_ready_19700         0          graph_batch19693.c  (bar3 open)
  gj_product_score_19700      0          graph_batch19694.c  (score open)
  gj_continuum_ready_19700    1          graph_batch19695.c
  gj_smoke_soft_19700         1          graph_batch19696.c
  gj_dyn_soft_19700           1          graph_batch19697.c
  gj_milestone_tag_19700      19700      graph_batch19698.c
  gj_continuum_wave_19700     19700      graph_batch19699.c
  gj_batch_id_19700           19700      graph_batch19700.c
  gj_graph_milestone_19700    19700      graph_batch19700.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=19700`` means parent wired CGJ_SRCS through
    ``graph_batch19700.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_19700`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_19700`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M19700_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_19600`` / ``gj_*_19500`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 55 exclusive CREATE-ONLY lands the **markers** band
  (19691–19700) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_19700.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_19700.py

  # Restrict to marker band only (soft-gate TUs; Wave 55 exclusive):
  python3 user/libcgj/src/_gen_milestone_19700.py --band markers

  # Extension identity only (19601–19650):
  python3 user/libcgj/src/_gen_milestone_19700.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_19700.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_19600.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 19700 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 19700
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 19601
WAVE_HI = 19700  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 19601, 19650
IDENTITY_LO, IDENTITY_HI = 19651, 19675
FILL_LO, FILL_HI = 19676, 19690
MARKERS_LO, MARKERS_HI = 19691, 19700
...
# Stems become gj_<stem>_19700. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (19691, "shell_green", 1, "shell product green extract (wave 19700)"),
    (19692, "libcgj_green", 1, "libcgj product green extract (wave 19700)"),
    (19693, "bar3_ready", 0, "product bar3 readiness gate (wave 19700)"),
    (19694, "product_score", 0, "product readiness score tag (wave 19700)"),
    (19695, "continuum_ready", 1, "continuum readiness gate (wave 19700)"),
    (19696, "smoke_soft", 1, "smoke soft lamp (wave 19700)"),
    (19697, "dyn_soft", 1, "dyn soft lamp (wave 19700)"),
    (19698, "milestone_tag", MILESTONE, "milestone 19700 product tag"),
    (19699, "continuum_wave", MILESTONE, "milestone 19700 continuum wave identity"),
    # 19700 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        19676: "continuum wave fill soft unit + batch id",
        19680: "continuum bridge soft unit + batch id",
        19690: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (19691–19699), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 19693
    tag_batch = MARKERS_LO + 7  # 19698
    wave_batch = MARKERS_LO + 8  # 19699
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_19600 / gj_shell_green_19500 / gj_shell_green_19400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_19600 / gj_libcgj_green_19500 / gj_libcgj_green_19400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_19600 / gj_bar3_ready_19500 / gj_bar3_ready_19400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_19600 / gj_product_score_19500 / gj_product_score_19400\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_19600 / gj_continuum_ready_19500 /\n"
            f" * gj_continuum_ready_19400 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_19600 / gj_smoke_soft_19500 / gj_smoke_soft_19400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_19600 / gj_dyn_soft_19500 / gj_dyn_soft_19400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_19600 / gj_milestone_tag_19500 / gj_milestone_tag_19400,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_19600 / gj_continuum_wave_19500 /\n"
            f" * gj_continuum_wave_19400, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch19700.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 19699
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 19691–19699; 19700 is capstone
