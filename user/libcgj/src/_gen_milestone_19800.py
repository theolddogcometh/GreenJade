...
"""
Soft continuum generator — MILESTONE 19800 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 19701–19800 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=19800   when parent has wired through graph_batch19800.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 19701 19800
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 19701 19800
...
graph batches ending at M. For M = **19800** the exclusive range is
**19701–19800** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 19701–19750  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 19751–19775  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 19776–19790  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 19791–19800  │ decade soft gates (Wave 56 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 19800 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_19800        1          graph_batch19791.c
  gj_libcgj_green_19800       1          graph_batch19792.c
  gj_bar3_ready_19800         0          graph_batch19793.c  (bar3 open)
  gj_product_score_19800      0          graph_batch19794.c  (score open)
  gj_continuum_ready_19800    1          graph_batch19795.c
  gj_smoke_soft_19800         1          graph_batch19796.c
  gj_dyn_soft_19800           1          graph_batch19797.c
  gj_milestone_tag_19800      19800      graph_batch19798.c
  gj_continuum_wave_19800     19800      graph_batch19799.c
  gj_batch_id_19800           19800      graph_batch19800.c
  gj_graph_milestone_19800    19800      graph_batch19800.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=19800`` means parent wired CGJ_SRCS through
    ``graph_batch19800.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_19800`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_19800`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M19800_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_19700`` / ``gj_*_19600`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 56 exclusive CREATE-ONLY lands the **markers** band
  (19791–19800) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_19800.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_19800.py

  # Restrict to marker band only (soft-gate TUs; Wave 56 exclusive):
  python3 user/libcgj/src/_gen_milestone_19800.py --band markers

  # Extension identity only (19701–19750):
  python3 user/libcgj/src/_gen_milestone_19800.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_19800.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_19700.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 19800 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 19800
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 19701
WAVE_HI = 19800  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 19701, 19750
IDENTITY_LO, IDENTITY_HI = 19751, 19775
FILL_LO, FILL_HI = 19776, 19790
MARKERS_LO, MARKERS_HI = 19791, 19800
...
# Stems become gj_<stem>_19800. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (19791, "shell_green", 1, "shell product green extract (wave 19800)"),
    (19792, "libcgj_green", 1, "libcgj product green extract (wave 19800)"),
    (19793, "bar3_ready", 0, "product bar3 readiness gate (wave 19800)"),
    (19794, "product_score", 0, "product readiness score tag (wave 19800)"),
    (19795, "continuum_ready", 1, "continuum readiness gate (wave 19800)"),
    (19796, "smoke_soft", 1, "smoke soft lamp (wave 19800)"),
    (19797, "dyn_soft", 1, "dyn soft lamp (wave 19800)"),
    (19798, "milestone_tag", MILESTONE, "milestone 19800 product tag"),
    (19799, "continuum_wave", MILESTONE, "milestone 19800 continuum wave identity"),
    # 19800 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        19776: "continuum wave fill soft unit + batch id",
        19780: "continuum bridge soft unit + batch id",
        19790: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (19791–19799), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 19793
    tag_batch = MARKERS_LO + 7  # 19798
    wave_batch = MARKERS_LO + 8  # 19799
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_19700 / gj_shell_green_19600 / gj_shell_green_19500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_19700 / gj_libcgj_green_19600 / gj_libcgj_green_19500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_19700 / gj_bar3_ready_19600 / gj_bar3_ready_19500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_19700 / gj_product_score_19600 / gj_product_score_19500\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_19700 / gj_continuum_ready_19600 /\n"
            f" * gj_continuum_ready_19500 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_19700 / gj_smoke_soft_19600 / gj_smoke_soft_19500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_19700 / gj_dyn_soft_19600 / gj_dyn_soft_19500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_19700 / gj_milestone_tag_19600 / gj_milestone_tag_19500,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_19700 / gj_continuum_wave_19600 /\n"
            f" * gj_continuum_wave_19500, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch19800.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 19799
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 19791–19799; 19800 is capstone
