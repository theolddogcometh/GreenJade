...
"""
Soft continuum generator — MILESTONE 19900 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 19801–19900 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=19900   when parent has wired through graph_batch19900.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 19801 19900
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 19801 19900
...
graph batches ending at M. For M = **19900** the exclusive range is
**19801–19900** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 19801–19850  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 19851–19875  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 19876–19890  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 19891–19900  │ decade soft gates (Wave 57 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 19900 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_19900        1          graph_batch19891.c
  gj_libcgj_green_19900       1          graph_batch19892.c
  gj_bar3_ready_19900         0          graph_batch19893.c  (bar3 open)
  gj_product_score_19900      0          graph_batch19894.c  (score open)
  gj_continuum_ready_19900    1          graph_batch19895.c
  gj_smoke_soft_19900         1          graph_batch19896.c
  gj_dyn_soft_19900           1          graph_batch19897.c
  gj_milestone_tag_19900      19900      graph_batch19898.c
  gj_continuum_wave_19900     19900      graph_batch19899.c
  gj_batch_id_19900           19900      graph_batch19900.c
  gj_graph_milestone_19900    19900      graph_batch19900.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=19900`` means parent wired CGJ_SRCS through
    ``graph_batch19900.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_19900`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_19900`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M19900_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_19800`` / ``gj_*_19700`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 57 exclusive CREATE-ONLY lands the **markers** band
  (19891–19900) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_19900.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_19900.py

  # Restrict to marker band only (soft-gate TUs; Wave 57 exclusive):
  python3 user/libcgj/src/_gen_milestone_19900.py --band markers

  # Extension identity only (19801–19850):
  python3 user/libcgj/src/_gen_milestone_19900.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_19900.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_19800.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 19900 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 19900
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 19801
WAVE_HI = 19900  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 19801, 19850
IDENTITY_LO, IDENTITY_HI = 19851, 19875
FILL_LO, FILL_HI = 19876, 19890
MARKERS_LO, MARKERS_HI = 19891, 19900
...
# Stems become gj_<stem>_19900. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (19891, "shell_green", 1, "shell product green extract (wave 19900)"),
    (19892, "libcgj_green", 1, "libcgj product green extract (wave 19900)"),
    (19893, "bar3_ready", 0, "product bar3 readiness gate (wave 19900)"),
    (19894, "product_score", 0, "product readiness score tag (wave 19900)"),
    (19895, "continuum_ready", 1, "continuum readiness gate (wave 19900)"),
    (19896, "smoke_soft", 1, "smoke soft lamp (wave 19900)"),
    (19897, "dyn_soft", 1, "dyn soft lamp (wave 19900)"),
    (19898, "milestone_tag", MILESTONE, "milestone 19900 product tag"),
    (19899, "continuum_wave", MILESTONE, "milestone 19900 continuum wave identity"),
    # 19900 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        19876: "continuum wave fill soft unit + batch id",
        19880: "continuum bridge soft unit + batch id",
        19890: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (19891–19899), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 19893
    tag_batch = MARKERS_LO + 7  # 19898
    wave_batch = MARKERS_LO + 8  # 19899
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_19800 / gj_shell_green_19700 / gj_shell_green_19600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_19800 / gj_libcgj_green_19700 / gj_libcgj_green_19600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_19800 / gj_bar3_ready_19700 / gj_bar3_ready_19600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_19800 / gj_product_score_19700 / gj_product_score_19600\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_19800 / gj_continuum_ready_19700 /\n"
            f" * gj_continuum_ready_19600 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_19800 / gj_smoke_soft_19700 / gj_smoke_soft_19600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_19800 / gj_dyn_soft_19700 / gj_dyn_soft_19600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_19800 / gj_milestone_tag_19700 / gj_milestone_tag_19600,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_19800 / gj_continuum_wave_19700 /\n"
            f" * gj_continuum_wave_19600, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch19900.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 19899
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 19891–19899; 19900 is capstone
