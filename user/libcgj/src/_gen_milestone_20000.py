...
"""
Soft continuum generator — MILESTONE 20000 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 19901–20000 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=20000   when parent has wired through graph_batch20000.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 19901 20000
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 19901 20000
...
graph batches ending at M. For M = **20000** the exclusive range is
**19901–20000** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 19901–19950  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 19951–19975  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 19976–19990  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 19991–20000  │ decade soft gates (Wave 58 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 20000 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_20000        1          graph_batch19991.c
  gj_libcgj_green_20000       1          graph_batch19992.c
  gj_bar3_ready_20000         0          graph_batch19993.c  (bar3 open)
  gj_product_score_20000      0          graph_batch19994.c  (score open)
  gj_continuum_ready_20000    1          graph_batch19995.c
  gj_smoke_soft_20000         1          graph_batch19996.c
  gj_dyn_soft_20000           1          graph_batch19997.c
  gj_milestone_tag_20000      20000      graph_batch19998.c
  gj_continuum_wave_20000     20000      graph_batch19999.c
  gj_batch_id_20000           20000      graph_batch20000.c
  gj_graph_milestone_20000    20000      graph_batch20000.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=20000`` means parent wired CGJ_SRCS through
    ``graph_batch20000.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_20000`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_20000`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M20000_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_19900`` / ``gj_*_19800`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 58 exclusive CREATE-ONLY lands the **markers** band
  (19991–20000) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_20000.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_20000.py

  # Restrict to marker band only (soft-gate TUs; Wave 58 exclusive):
  python3 user/libcgj/src/_gen_milestone_20000.py --band markers

  # Extension identity only (19901–19950):
  python3 user/libcgj/src/_gen_milestone_20000.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_20000.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_19900.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 20000 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 20000
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 19901
WAVE_HI = 20000  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 19901, 19950
IDENTITY_LO, IDENTITY_HI = 19951, 19975
FILL_LO, FILL_HI = 19976, 19990
MARKERS_LO, MARKERS_HI = 19991, 20000
...
# Stems become gj_<stem>_20000. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (19991, "shell_green", 1, "shell product green extract (wave 20000)"),
    (19992, "libcgj_green", 1, "libcgj product green extract (wave 20000)"),
    (19993, "bar3_ready", 0, "product bar3 readiness gate (wave 20000)"),
    (19994, "product_score", 0, "product readiness score tag (wave 20000)"),
    (19995, "continuum_ready", 1, "continuum readiness gate (wave 20000)"),
    (19996, "smoke_soft", 1, "smoke soft lamp (wave 20000)"),
    (19997, "dyn_soft", 1, "dyn soft lamp (wave 20000)"),
    (19998, "milestone_tag", MILESTONE, "milestone 20000 product tag"),
    (19999, "continuum_wave", MILESTONE, "milestone 20000 continuum wave identity"),
    # 20000 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        19976: "continuum wave fill soft unit + batch id",
        19980: "continuum bridge soft unit + batch id",
        19990: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (19991–19999), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 19993
    tag_batch = MARKERS_LO + 7  # 19998
    wave_batch = MARKERS_LO + 8  # 19999
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_19900 / gj_shell_green_19800 / gj_shell_green_19700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_19900 / gj_libcgj_green_19800 / gj_libcgj_green_19700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_19900 / gj_bar3_ready_19800 / gj_bar3_ready_19700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_19900 / gj_product_score_19800 / gj_product_score_19700\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_19900 / gj_continuum_ready_19800 /\n"
            f" * gj_continuum_ready_19700 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_19900 / gj_smoke_soft_19800 / gj_smoke_soft_19700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_19900 / gj_dyn_soft_19800 / gj_dyn_soft_19700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_19900 / gj_milestone_tag_19800 / gj_milestone_tag_19700,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_19900 / gj_continuum_wave_19800 /\n"
            f" * gj_continuum_wave_19700, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch20000.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 19999
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 19991–19999; 20000 is capstone
