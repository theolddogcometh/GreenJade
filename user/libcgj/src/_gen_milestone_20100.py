...
"""
Soft continuum generator — MILESTONE 20100 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 20001–20100 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=20100   when parent has wired through graph_batch20100.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 20001 20100
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 20001 20100
...
graph batches ending at M. For M = **20100** the exclusive range is
**20001–20100** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 20001–20050  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 20051–20075  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 20076–20090  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 20091–20100  │ decade soft gates (Wave 59 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 20100 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_20100        1          graph_batch20091.c
  gj_libcgj_green_20100       1          graph_batch20092.c
  gj_bar3_ready_20100         0          graph_batch20093.c  (bar3 open)
  gj_product_score_20100      0          graph_batch20094.c  (score open)
  gj_continuum_ready_20100    1          graph_batch20095.c
  gj_smoke_soft_20100         1          graph_batch20096.c
  gj_dyn_soft_20100           1          graph_batch20097.c
  gj_milestone_tag_20100      20100      graph_batch20098.c
  gj_continuum_wave_20100     20100      graph_batch20099.c
  gj_batch_id_20100           20100      graph_batch20100.c
  gj_graph_milestone_20100    20100      graph_batch20100.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=20100`` means parent wired CGJ_SRCS through
    ``graph_batch20100.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_20100`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_20100`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M20100_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_20000`` / ``gj_*_19900`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 59 exclusive CREATE-ONLY lands the **markers** band
  (20091–20100) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_20100.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_20100.py

  # Restrict to marker band only (soft-gate TUs; Wave 59 exclusive):
  python3 user/libcgj/src/_gen_milestone_20100.py --band markers

  # Extension identity only (20001–20050):
  python3 user/libcgj/src/_gen_milestone_20100.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_20100.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_20000.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 20100 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 20100
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 20001
WAVE_HI = 20100  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 20001, 20050
IDENTITY_LO, IDENTITY_HI = 20051, 20075
FILL_LO, FILL_HI = 20076, 20090
MARKERS_LO, MARKERS_HI = 20091, 20100
...
# Stems become gj_<stem>_20100. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (20091, "shell_green", 1, "shell product green extract (wave 20100)"),
    (20092, "libcgj_green", 1, "libcgj product green extract (wave 20100)"),
    (20093, "bar3_ready", 0, "product bar3 readiness gate (wave 20100)"),
    (20094, "product_score", 0, "product readiness score tag (wave 20100)"),
    (20095, "continuum_ready", 1, "continuum readiness gate (wave 20100)"),
    (20096, "smoke_soft", 1, "smoke soft lamp (wave 20100)"),
    (20097, "dyn_soft", 1, "dyn soft lamp (wave 20100)"),
    (20098, "milestone_tag", MILESTONE, "milestone 20100 product tag"),
    (20099, "continuum_wave", MILESTONE, "milestone 20100 continuum wave identity"),
    # 20100 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        20076: "continuum wave fill soft unit + batch id",
        20080: "continuum bridge soft unit + batch id",
        20090: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (20091–20099), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 20093
    tag_batch = MARKERS_LO + 7  # 20098
    wave_batch = MARKERS_LO + 8  # 20099
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_20000 / gj_shell_green_19900 / gj_shell_green_19800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_20000 / gj_libcgj_green_19900 / gj_libcgj_green_19800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_20000 / gj_bar3_ready_19900 / gj_bar3_ready_19800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_20000 / gj_product_score_19900 / gj_product_score_19800\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_20000 / gj_continuum_ready_19900 /\n"
            f" * gj_continuum_ready_19800 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_20000 / gj_smoke_soft_19900 / gj_smoke_soft_19800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_20000 / gj_dyn_soft_19900 / gj_dyn_soft_19800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_20000 / gj_milestone_tag_19900 / gj_milestone_tag_19800,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_20000 / gj_continuum_wave_19900 /\n"
            f" * gj_continuum_wave_19800, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    ...
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 20099
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 20091–20099; 20100 is capstone
