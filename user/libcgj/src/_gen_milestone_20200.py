...
"""
Soft continuum generator — MILESTONE 20200 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 20101–20200 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=20200   when parent has wired through graph_batch20200.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 20101 20200
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 20101 20200
...
graph batches ending at M. For M = **20200** the exclusive range is
**20101–20200** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 20101–20150  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 20151–20175  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 20176–20190  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 20191–20200  │ decade soft gates (Wave 60 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 20200 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_20200        1          graph_batch20191.c
  gj_libcgj_green_20200       1          graph_batch20192.c
  gj_bar3_ready_20200         0          graph_batch20193.c  (bar3 open)
  gj_product_score_20200      0          graph_batch20194.c  (score open)
  gj_continuum_ready_20200    1          graph_batch20195.c
  gj_smoke_soft_20200         1          graph_batch20196.c
  gj_dyn_soft_20200           1          graph_batch20197.c
  gj_milestone_tag_20200      20200      graph_batch20198.c
  gj_continuum_wave_20200     20200      graph_batch20199.c
  gj_batch_id_20200           20200      graph_batch20200.c
  gj_graph_milestone_20200    20200      graph_batch20200.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=20200`` means parent wired CGJ_SRCS through
    ``graph_batch20200.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_20200`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_20200`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M20200_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_20100`` / ``gj_*_20000`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 60 exclusive CREATE-ONLY lands the **markers** band
  (20191–20200) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_20200.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_20200.py

  # Restrict to marker band only (soft-gate TUs; Wave 60 exclusive):
  python3 user/libcgj/src/_gen_milestone_20200.py --band markers

  # Extension identity only (20101–20150):
  python3 user/libcgj/src/_gen_milestone_20200.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_20200.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_20100.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 20200 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 20200
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 20101
WAVE_HI = 20200  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 20101, 20150
IDENTITY_LO, IDENTITY_HI = 20151, 20175
FILL_LO, FILL_HI = 20176, 20190
MARKERS_LO, MARKERS_HI = 20191, 20200
...
# Stems become gj_<stem>_20200. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (20191, "shell_green", 1, "shell product green extract (wave 20200)"),
    (20192, "libcgj_green", 1, "libcgj product green extract (wave 20200)"),
    (20193, "bar3_ready", 0, "product bar3 readiness gate (wave 20200)"),
    (20194, "product_score", 0, "product readiness score tag (wave 20200)"),
    (20195, "continuum_ready", 1, "continuum readiness gate (wave 20200)"),
    (20196, "smoke_soft", 1, "smoke soft lamp (wave 20200)"),
    (20197, "dyn_soft", 1, "dyn soft lamp (wave 20200)"),
    (20198, "milestone_tag", MILESTONE, "milestone 20200 product tag"),
    (20199, "continuum_wave", MILESTONE, "milestone 20200 continuum wave identity"),
    # 20200 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        20176: "continuum wave fill soft unit + batch id",
        20180: "continuum bridge soft unit + batch id",
        20190: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (20191–20199), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 20193
    tag_batch = MARKERS_LO + 7  # 20198
    wave_batch = MARKERS_LO + 8  # 20199
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_20100 / gj_shell_green_20000 / gj_shell_green_19900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_20100 / gj_libcgj_green_20000 / gj_libcgj_green_19900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_20100 / gj_bar3_ready_20000 / gj_bar3_ready_19900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_20100 / gj_product_score_20000 / gj_product_score_19900\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_20100 / gj_continuum_ready_20000 /\n"
            f" * gj_continuum_ready_19900 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_20100 / gj_smoke_soft_20000 / gj_smoke_soft_19900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_20100 / gj_dyn_soft_20000 / gj_dyn_soft_19900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_20100 / gj_milestone_tag_20000 / gj_milestone_tag_19900,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_20100 / gj_continuum_wave_20000 /\n"
            f" * gj_continuum_wave_19900, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch20200.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 20199
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 20191–20199; 20200 is capstone
