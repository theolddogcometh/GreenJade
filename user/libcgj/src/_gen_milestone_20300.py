...
"""
Soft continuum generator — MILESTONE 20300 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 20201–20300 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=20300   when parent has wired through graph_batch20300.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 20201 20300
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 20201 20300
...
graph batches ending at M. For M = **20300** the exclusive range is
**20201–20300** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 20201–20250  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 20251–20275  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 20276–20290  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 20291–20300  │ decade soft gates (Wave 61 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 20300 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_20300        1          graph_batch20291.c
  gj_libcgj_green_20300       1          graph_batch20292.c
  gj_bar3_ready_20300         0          graph_batch20293.c  (bar3 open)
  gj_product_score_20300      0          graph_batch20294.c  (score open)
  gj_continuum_ready_20300    1          graph_batch20295.c
  gj_smoke_soft_20300         1          graph_batch20296.c
  gj_dyn_soft_20300           1          graph_batch20297.c
  gj_milestone_tag_20300      20300      graph_batch20298.c
  gj_continuum_wave_20300     20300      graph_batch20299.c
  gj_batch_id_20300           20300      graph_batch20300.c
  gj_graph_milestone_20300    20300      graph_batch20300.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=20300`` means parent wired CGJ_SRCS through
    ``graph_batch20300.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_20300`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_20300`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M20300_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_20200`` / ``gj_*_20100`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 61 exclusive CREATE-ONLY lands the **markers** band
  (20291–20300) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_20300.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_20300.py

  # Restrict to marker band only (soft-gate TUs; Wave 61 exclusive):
  python3 user/libcgj/src/_gen_milestone_20300.py --band markers

  # Extension identity only (20201–20250):
  python3 user/libcgj/src/_gen_milestone_20300.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_20300.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_20200.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 20300 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 20300
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 20201
WAVE_HI = 20300  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 20201, 20250
IDENTITY_LO, IDENTITY_HI = 20251, 20275
FILL_LO, FILL_HI = 20276, 20290
MARKERS_LO, MARKERS_HI = 20291, 20300
...
# Stems become gj_<stem>_20300. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (20291, "shell_green", 1, "shell product green extract (wave 20300)"),
    (20292, "libcgj_green", 1, "libcgj product green extract (wave 20300)"),
    (20293, "bar3_ready", 0, "product bar3 readiness gate (wave 20300)"),
    (20294, "product_score", 0, "product readiness score tag (wave 20300)"),
    (20295, "continuum_ready", 1, "continuum readiness gate (wave 20300)"),
    (20296, "smoke_soft", 1, "smoke soft lamp (wave 20300)"),
    (20297, "dyn_soft", 1, "dyn soft lamp (wave 20300)"),
    (20298, "milestone_tag", MILESTONE, "milestone 20300 product tag"),
    (20299, "continuum_wave", MILESTONE, "milestone 20300 continuum wave identity"),
    # 20300 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        20276: "continuum wave fill soft unit + batch id",
        20280: "continuum bridge soft unit + batch id",
        20290: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (20291–20299), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 20293
    tag_batch = MARKERS_LO + 7  # 20298
    wave_batch = MARKERS_LO + 8  # 20299
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_20200 / gj_shell_green_20100 / gj_shell_green_20300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_20200 / gj_libcgj_green_20100 / gj_libcgj_green_20300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_20200 / gj_bar3_ready_20100 / gj_bar3_ready_20300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_20200 / gj_product_score_20100 / gj_product_score_20300\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_20200 / gj_continuum_ready_20100 /\n"
            f" * gj_continuum_ready_20000 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_20200 / gj_smoke_soft_20100 / gj_smoke_soft_20300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_20200 / gj_dyn_soft_20100 / gj_dyn_soft_20300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_20200 / gj_milestone_tag_20100 / gj_milestone_tag_20300,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_20200 / gj_continuum_wave_20100 /\n"
            f" * gj_continuum_wave_20000, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch20300.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 20299
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 20291–20299; 20300 is capstone
