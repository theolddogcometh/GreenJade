...
"""
Soft continuum generator — MILESTONE 20400 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 20301–20400 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=20400   when parent has wired through graph_batch20400.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 20301 20400
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 20301 20400
...
graph batches ending at M. For M = **20400** the exclusive range is
**20301–20400** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 20301–20350  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 20351–20375  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 20376–20390  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 20391–20400  │ decade soft gates (Wave 62 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 20400 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_20400        1          graph_batch20391.c
  gj_libcgj_green_20400       1          graph_batch20392.c
  gj_bar3_ready_20400         0          graph_batch20393.c  (bar3 open)
  gj_product_score_20400      0          graph_batch20394.c  (score open)
  gj_continuum_ready_20400    1          graph_batch20395.c
  gj_smoke_soft_20400         1          graph_batch20396.c
  gj_dyn_soft_20400           1          graph_batch20397.c
  gj_milestone_tag_20400      20400      graph_batch20398.c
  gj_continuum_wave_20400     20400      graph_batch20399.c
  gj_batch_id_20400           20400      graph_batch20400.c
  gj_graph_milestone_20400    20400      graph_batch20400.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=20400`` means parent wired CGJ_SRCS through
    ``graph_batch20400.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_20400`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_20400`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M20400_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_20300`` / ``gj_*_20200`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 62 exclusive CREATE-ONLY lands the **markers** band
  (20391–20400) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_20400.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_20400.py

  # Restrict to marker band only (soft-gate TUs; Wave 62 exclusive):
  python3 user/libcgj/src/_gen_milestone_20400.py --band markers

  # Extension identity only (20301–20350):
  python3 user/libcgj/src/_gen_milestone_20400.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_20400.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_20300.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 20400 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 20400
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 20301
WAVE_HI = 20400  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 20301, 20350
IDENTITY_LO, IDENTITY_HI = 20351, 20375
FILL_LO, FILL_HI = 20376, 20390
MARKERS_LO, MARKERS_HI = 20391, 20400
...
# Stems become gj_<stem>_20400. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (20391, "shell_green", 1, "shell product green extract (wave 20400)"),
    (20392, "libcgj_green", 1, "libcgj product green extract (wave 20400)"),
    (20393, "bar3_ready", 0, "product bar3 readiness gate (wave 20400)"),
    (20394, "product_score", 0, "product readiness score tag (wave 20400)"),
    (20395, "continuum_ready", 1, "continuum readiness gate (wave 20400)"),
    (20396, "smoke_soft", 1, "smoke soft lamp (wave 20400)"),
    (20397, "dyn_soft", 1, "dyn soft lamp (wave 20400)"),
    (20398, "milestone_tag", MILESTONE, "milestone 20400 product tag"),
    (20399, "continuum_wave", MILESTONE, "milestone 20400 continuum wave identity"),
    # 20400 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        20376: "continuum wave fill soft unit + batch id",
        20380: "continuum bridge soft unit + batch id",
        20390: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (20391–20399), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 20393
    tag_batch = MARKERS_LO + 7  # 20398
    wave_batch = MARKERS_LO + 8  # 20399
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_20300 / gj_shell_green_20200 / gj_shell_green_20100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_20300 / gj_libcgj_green_20200 / gj_libcgj_green_20100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_20300 / gj_bar3_ready_20200 / gj_bar3_ready_20100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_20300 / gj_product_score_20200 / gj_product_score_20100\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_20300 / gj_continuum_ready_20200 /\n"
            f" * gj_continuum_ready_20100 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_20300 / gj_smoke_soft_20200 / gj_smoke_soft_20100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_20300 / gj_dyn_soft_20200 / gj_dyn_soft_20100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_20300 / gj_milestone_tag_20200 / gj_milestone_tag_20100,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_20300 / gj_continuum_wave_20200 /\n"
            f" * gj_continuum_wave_20100, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch20400.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 20399
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 20391–20399; 20400 is capstone
