...
"""
Soft continuum generator — MILESTONE 20500 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 20401–20500 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=20500   when parent has wired through graph_batch20500.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 20401 20500
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 20401 20500
...
graph batches ending at M. For M = **20500** the exclusive range is
**20401–20500** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 20401–20450  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 20451–20475  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 20476–20490  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 20491–20500  │ decade soft gates (Wave 63 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 20500 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_20500        1          graph_batch20491.c
  gj_libcgj_green_20500       1          graph_batch20492.c
  gj_bar3_ready_20500         0          graph_batch20493.c  (bar3 open)
  gj_product_score_20500      0          graph_batch20494.c  (score open)
  gj_continuum_ready_20500    1          graph_batch20495.c
  gj_smoke_soft_20500         1          graph_batch20496.c
  gj_dyn_soft_20500           1          graph_batch20497.c
  gj_milestone_tag_20500      20500      graph_batch20498.c
  gj_continuum_wave_20500     20500      graph_batch20499.c
  gj_batch_id_20500           20500      graph_batch20500.c
  gj_graph_milestone_20500    20500      graph_batch20500.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=20500`` means parent wired CGJ_SRCS through
    ``graph_batch20500.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_20500`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_20500`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M20500_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_20400`` / ``gj_*_20300`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 63 exclusive CREATE-ONLY lands the **markers** band
  (20491–20500) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_20500.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_20500.py

  # Restrict to marker band only (soft-gate TUs; Wave 63 exclusive):
  python3 user/libcgj/src/_gen_milestone_20500.py --band markers

  # Extension identity only (20401–20450):
  python3 user/libcgj/src/_gen_milestone_20500.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_20500.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_20400.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 20500 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 20500
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 20401
WAVE_HI = 20500  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 20401, 20450
IDENTITY_LO, IDENTITY_HI = 20451, 20475
FILL_LO, FILL_HI = 20476, 20490
MARKERS_LO, MARKERS_HI = 20491, 20500
...
# Stems become gj_<stem>_20500. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (20491, "shell_green", 1, "shell product green extract (wave 20500)"),
    (20492, "libcgj_green", 1, "libcgj product green extract (wave 20500)"),
    (20493, "bar3_ready", 0, "product bar3 readiness gate (wave 20500)"),
    (20494, "product_score", 0, "product readiness score tag (wave 20500)"),
    (20495, "continuum_ready", 1, "continuum readiness gate (wave 20500)"),
    (20496, "smoke_soft", 1, "smoke soft lamp (wave 20500)"),
    (20497, "dyn_soft", 1, "dyn soft lamp (wave 20500)"),
    (20498, "milestone_tag", MILESTONE, "milestone 20500 product tag"),
    (20499, "continuum_wave", MILESTONE, "milestone 20500 continuum wave identity"),
    # 20500 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        20476: "continuum wave fill soft unit + batch id",
        20480: "continuum bridge soft unit + batch id",
        20490: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (20491–20499), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 20493
    tag_batch = MARKERS_LO + 7  # 20498
    wave_batch = MARKERS_LO + 8  # 20499
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_20400 / gj_shell_green_20300 / gj_shell_green_20200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_20400 / gj_libcgj_green_20300 / gj_libcgj_green_20200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_20400 / gj_bar3_ready_20300 / gj_bar3_ready_20200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_20400 / gj_product_score_20300 / gj_product_score_20200\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_20400 / gj_continuum_ready_20300 /\n"
            f" * gj_continuum_ready_20200 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_20400 / gj_smoke_soft_20300 / gj_smoke_soft_20200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_20400 / gj_dyn_soft_20300 / gj_dyn_soft_20200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_20400 / gj_milestone_tag_20300 / gj_milestone_tag_20200,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_20400 / gj_continuum_wave_20300 /\n"
            f" * gj_continuum_wave_20200, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch20500.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 20499
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 20491–20499; 20500 is capstone
