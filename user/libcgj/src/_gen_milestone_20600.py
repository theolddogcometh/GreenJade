...
"""
Soft continuum generator — MILESTONE 20600 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 20501–20600 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=20600   when parent has wired through graph_batch20600.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 20501 20600
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 20501 20600
...
graph batches ending at M. For M = **20600** the exclusive range is
**20501–20600** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 20501–20550  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 20551–20575  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 20576–20590  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 20591–20600  │ decade soft gates (Wave 64 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 20600 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_20600        1          graph_batch20591.c
  gj_libcgj_green_20600       1          graph_batch20592.c
  gj_bar3_ready_20600         0          graph_batch20593.c  (bar3 open)
  gj_product_score_20600      0          graph_batch20594.c  (score open)
  gj_continuum_ready_20600    1          graph_batch20595.c
  gj_smoke_soft_20600         1          graph_batch20596.c
  gj_dyn_soft_20600           1          graph_batch20597.c
  gj_milestone_tag_20600      20600      graph_batch20598.c
  gj_continuum_wave_20600     20600      graph_batch20599.c
  gj_batch_id_20600           20600      graph_batch20600.c
  gj_graph_milestone_20600    20600      graph_batch20600.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=20600`` means parent wired CGJ_SRCS through
    ``graph_batch20600.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_20600`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_20600`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M20600_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_20500`` / ``gj_*_20400`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 64 exclusive CREATE-ONLY lands the **markers** band
  (20591–20600) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_20600.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_20600.py

  # Restrict to marker band only (soft-gate TUs; Wave 64 exclusive):
  python3 user/libcgj/src/_gen_milestone_20600.py --band markers

  # Extension identity only (20501–20550):
  python3 user/libcgj/src/_gen_milestone_20600.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_20600.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_20500.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 20600 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 20600
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 20501
WAVE_HI = 20600  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 20501, 20550
IDENTITY_LO, IDENTITY_HI = 20551, 20575
FILL_LO, FILL_HI = 20576, 20590
MARKERS_LO, MARKERS_HI = 20591, 20600
...
# Stems become gj_<stem>_20600. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (20591, "shell_green", 1, "shell product green extract (wave 20600)"),
    (20592, "libcgj_green", 1, "libcgj product green extract (wave 20600)"),
    (20593, "bar3_ready", 0, "product bar3 readiness gate (wave 20600)"),
    (20594, "product_score", 0, "product readiness score tag (wave 20600)"),
    (20595, "continuum_ready", 1, "continuum readiness gate (wave 20600)"),
    (20596, "smoke_soft", 1, "smoke soft lamp (wave 20600)"),
    (20597, "dyn_soft", 1, "dyn soft lamp (wave 20600)"),
    (20598, "milestone_tag", MILESTONE, "milestone 20600 product tag"),
    (20599, "continuum_wave", MILESTONE, "milestone 20600 continuum wave identity"),
    # 20600 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        20576: "continuum wave fill soft unit + batch id",
        20580: "continuum bridge soft unit + batch id",
        20590: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (20591–20599), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 20593
    tag_batch = MARKERS_LO + 7  # 20598
    wave_batch = MARKERS_LO + 8  # 20599
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_20500 / gj_shell_green_20400 / gj_shell_green_20300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_20500 / gj_libcgj_green_20400 / gj_libcgj_green_20300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_20500 / gj_bar3_ready_20400 / gj_bar3_ready_20300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_20500 / gj_product_score_20400 / gj_product_score_20300\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_20500 / gj_continuum_ready_20400 /\n"
            f" * gj_continuum_ready_20300 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_20500 / gj_smoke_soft_20400 / gj_smoke_soft_20300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_20500 / gj_dyn_soft_20400 / gj_dyn_soft_20300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_20500 / gj_milestone_tag_20400 / gj_milestone_tag_20300,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_20500 / gj_continuum_wave_20400 /\n"
            f" * gj_continuum_wave_20300, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch20600.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 20599
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 20591–20599; 20600 is capstone
