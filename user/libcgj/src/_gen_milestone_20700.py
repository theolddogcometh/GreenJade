...
"""
Soft continuum generator — MILESTONE 20700 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 20601–20700 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=20700   when parent has wired through graph_batch20700.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 20601 20700
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 20601 20700
...
graph batches ending at M. For M = **20700** the exclusive range is
**20601–20700** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 20601–20650  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 20651–20675  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 20676–20690  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 20691–20700  │ decade soft gates (Wave 65 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 20700 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_20700        1          graph_batch20691.c
  gj_libcgj_green_20700       1          graph_batch20692.c
  gj_bar3_ready_20700         0          graph_batch20693.c  (bar3 open)
  gj_product_score_20700      0          graph_batch20694.c  (score open)
  gj_continuum_ready_20700    1          graph_batch20695.c
  gj_smoke_soft_20700         1          graph_batch20696.c
  gj_dyn_soft_20700           1          graph_batch20697.c
  gj_milestone_tag_20700      20700      graph_batch20698.c
  gj_continuum_wave_20700     20700      graph_batch20699.c
  gj_batch_id_20700           20700      graph_batch20700.c
  gj_graph_milestone_20700    20700      graph_batch20700.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=20700`` means parent wired CGJ_SRCS through
    ``graph_batch20700.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_20700`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_20700`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M20700_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_20600`` / ``gj_*_20500`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 65 exclusive CREATE-ONLY lands the **markers** band
  (20691–20700) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_20700.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_20700.py

  # Restrict to marker band only (soft-gate TUs; Wave 65 exclusive):
  python3 user/libcgj/src/_gen_milestone_20700.py --band markers

  # Extension identity only (20601–20650):
  python3 user/libcgj/src/_gen_milestone_20700.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_20700.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_20600.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 20700 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 20700
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 20601
WAVE_HI = 20700  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 20601, 20650
IDENTITY_LO, IDENTITY_HI = 20651, 20675
FILL_LO, FILL_HI = 20676, 20690
MARKERS_LO, MARKERS_HI = 20691, 20700
...
# Stems become gj_<stem>_20700. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (20691, "shell_green", 1, "shell product green extract (wave 20700)"),
    (20692, "libcgj_green", 1, "libcgj product green extract (wave 20700)"),
    (20693, "bar3_ready", 0, "product bar3 readiness gate (wave 20700)"),
    (20694, "product_score", 0, "product readiness score tag (wave 20700)"),
    (20695, "continuum_ready", 1, "continuum readiness gate (wave 20700)"),
    (20696, "smoke_soft", 1, "smoke soft lamp (wave 20700)"),
    (20697, "dyn_soft", 1, "dyn soft lamp (wave 20700)"),
    (20698, "milestone_tag", MILESTONE, "milestone 20700 product tag"),
    (20699, "continuum_wave", MILESTONE, "milestone 20700 continuum wave identity"),
    # 20700 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        20676: "continuum wave fill soft unit + batch id",
        20680: "continuum bridge soft unit + batch id",
        20690: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (20691–20699), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 20693
    tag_batch = MARKERS_LO + 7  # 20698
    wave_batch = MARKERS_LO + 8  # 20699
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_20600 / gj_shell_green_20500 / gj_shell_green_20400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_20600 / gj_libcgj_green_20500 / gj_libcgj_green_20400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_20600 / gj_bar3_ready_20500 / gj_bar3_ready_20400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_20600 / gj_product_score_20500 / gj_product_score_20400\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_20600 / gj_continuum_ready_20500 /\n"
            f" * gj_continuum_ready_20400 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_20600 / gj_smoke_soft_20500 / gj_smoke_soft_20400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_20600 / gj_dyn_soft_20500 / gj_dyn_soft_20400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_20600 / gj_milestone_tag_20500 / gj_milestone_tag_20400,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_20600 / gj_continuum_wave_20500 /\n"
            f" * gj_continuum_wave_20400, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch20700.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 20699
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 20691–20699; 20700 is capstone
