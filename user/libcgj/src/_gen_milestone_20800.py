...
"""
Soft continuum generator — MILESTONE 20800 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 20701–20800 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=20800   when parent has wired through graph_batch20800.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 20701 20800
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 20701 20800
...
graph batches ending at M. For M = **20800** the exclusive range is
**20701–20800** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 20701–20750  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 20751–20775  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 20776–20790  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 20791–20800  │ decade soft gates (Wave 66 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 20800 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_20800        1          graph_batch20791.c
  gj_libcgj_green_20800       1          graph_batch20792.c
  gj_bar3_ready_20800         0          graph_batch20793.c  (bar3 open)
  gj_product_score_20800      0          graph_batch20794.c  (score open)
  gj_continuum_ready_20800    1          graph_batch20795.c
  gj_smoke_soft_20800         1          graph_batch20796.c
  gj_dyn_soft_20800           1          graph_batch20797.c
  gj_milestone_tag_20800      20800      graph_batch20798.c
  gj_continuum_wave_20800     20800      graph_batch20799.c
  gj_batch_id_20800           20800      graph_batch20800.c
  gj_graph_milestone_20800    20800      graph_batch20800.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=20800`` means parent wired CGJ_SRCS through
    ``graph_batch20800.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_20800`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_20800`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M20800_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_20700`` / ``gj_*_20600`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 66 exclusive CREATE-ONLY lands the **markers** band
  (20791–20800) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_20800.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_20800.py

  # Restrict to marker band only (soft-gate TUs; Wave 66 exclusive):
  python3 user/libcgj/src/_gen_milestone_20800.py --band markers

  # Extension identity only (20701–20750):
  python3 user/libcgj/src/_gen_milestone_20800.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_20800.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_20700.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 20800 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 20800
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 20701
WAVE_HI = 20800  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 20701, 20750
IDENTITY_LO, IDENTITY_HI = 20751, 20775
FILL_LO, FILL_HI = 20776, 20790
MARKERS_LO, MARKERS_HI = 20791, 20800
...
# Stems become gj_<stem>_20800. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (20791, "shell_green", 1, "shell product green extract (wave 20800)"),
    (20792, "libcgj_green", 1, "libcgj product green extract (wave 20800)"),
    (20793, "bar3_ready", 0, "product bar3 readiness gate (wave 20800)"),
    (20794, "product_score", 0, "product readiness score tag (wave 20800)"),
    (20795, "continuum_ready", 1, "continuum readiness gate (wave 20800)"),
    (20796, "smoke_soft", 1, "smoke soft lamp (wave 20800)"),
    (20797, "dyn_soft", 1, "dyn soft lamp (wave 20800)"),
    (20798, "milestone_tag", MILESTONE, "milestone 20800 product tag"),
    (20799, "continuum_wave", MILESTONE, "milestone 20800 continuum wave identity"),
    # 20800 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        20776: "continuum wave fill soft unit + batch id",
        20780: "continuum bridge soft unit + batch id",
        20790: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (20791–20799), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 20793
    tag_batch = MARKERS_LO + 7  # 20798
    wave_batch = MARKERS_LO + 8  # 20799
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_20700 / gj_shell_green_20600 / gj_shell_green_20500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_20700 / gj_libcgj_green_20600 / gj_libcgj_green_20500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_20700 / gj_bar3_ready_20600 / gj_bar3_ready_20500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_20700 / gj_product_score_20600 / gj_product_score_20500\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_20700 / gj_continuum_ready_20600 /\n"
            f" * gj_continuum_ready_20500 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_20700 / gj_smoke_soft_20600 / gj_smoke_soft_20500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_20700 / gj_dyn_soft_20600 / gj_dyn_soft_20500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_20700 / gj_milestone_tag_20600 / gj_milestone_tag_20500,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_20700 / gj_continuum_wave_20600 /\n"
            f" * gj_continuum_wave_20500, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch20800.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 20799
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 20791–20799; 20800 is capstone
