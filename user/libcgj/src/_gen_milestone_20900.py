...
"""
Soft continuum generator — MILESTONE 20900 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 20801–20900 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=20900   when parent has wired through graph_batch20900.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 20801 20900
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 20801 20900
...
graph batches ending at M. For M = **20900** the exclusive range is
**20801–20900** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 20801–20850  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 20851–20875  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 20876–20890  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 20891–20900  │ decade soft gates (Wave 67 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 20900 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_20900        1          graph_batch20891.c
  gj_libcgj_green_20900       1          graph_batch20892.c
  gj_bar3_ready_20900         0          graph_batch20893.c  (bar3 open)
  gj_product_score_20900      0          graph_batch20894.c  (score open)
  gj_continuum_ready_20900    1          graph_batch20895.c
  gj_smoke_soft_20900         1          graph_batch20896.c
  gj_dyn_soft_20900           1          graph_batch20897.c
  gj_milestone_tag_20900      20900      graph_batch20898.c
  gj_continuum_wave_20900     20900      graph_batch20899.c
  gj_batch_id_20900           20900      graph_batch20900.c
  gj_graph_milestone_20900    20900      graph_batch20900.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=20900`` means parent wired CGJ_SRCS through
    ``graph_batch20900.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_20900`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_20900`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M20900_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_20800`` / ``gj_*_20700`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 67 exclusive CREATE-ONLY lands the **markers** band
  (20891–20900) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_20900.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_20900.py

  # Restrict to marker band only (soft-gate TUs; Wave 67 exclusive):
  python3 user/libcgj/src/_gen_milestone_20900.py --band markers

  # Extension identity only (20801–20850):
  python3 user/libcgj/src/_gen_milestone_20900.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_20900.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_20800.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 20900 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 20900
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 20801
WAVE_HI = 20900  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 20801, 20850
IDENTITY_LO, IDENTITY_HI = 20851, 20875
FILL_LO, FILL_HI = 20876, 20890
MARKERS_LO, MARKERS_HI = 20891, 20900
...
# Stems become gj_<stem>_20900. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (20891, "shell_green", 1, "shell product green extract (wave 20900)"),
    (20892, "libcgj_green", 1, "libcgj product green extract (wave 20900)"),
    (20893, "bar3_ready", 0, "product bar3 readiness gate (wave 20900)"),
    (20894, "product_score", 0, "product readiness score tag (wave 20900)"),
    (20895, "continuum_ready", 1, "continuum readiness gate (wave 20900)"),
    (20896, "smoke_soft", 1, "smoke soft lamp (wave 20900)"),
    (20897, "dyn_soft", 1, "dyn soft lamp (wave 20900)"),
    (20898, "milestone_tag", MILESTONE, "milestone 20900 product tag"),
    (20899, "continuum_wave", MILESTONE, "milestone 20900 continuum wave identity"),
    # 20900 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        20876: "continuum wave fill soft unit + batch id",
        20880: "continuum bridge soft unit + batch id",
        20890: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (20891–20899), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 20893
    tag_batch = MARKERS_LO + 7  # 20898
    wave_batch = MARKERS_LO + 8  # 20899
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_20800 / gj_shell_green_20700 / gj_shell_green_20600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_20800 / gj_libcgj_green_20700 / gj_libcgj_green_20600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_20800 / gj_bar3_ready_20700 / gj_bar3_ready_20600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_20800 / gj_product_score_20700 / gj_product_score_20600\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_20800 / gj_continuum_ready_20700 /\n"
            f" * gj_continuum_ready_20600 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_20800 / gj_smoke_soft_20700 / gj_smoke_soft_20600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_20800 / gj_dyn_soft_20700 / gj_dyn_soft_20600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_20800 / gj_milestone_tag_20700 / gj_milestone_tag_20600,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_20800 / gj_continuum_wave_20700 /\n"
            f" * gj_continuum_wave_20600, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch20900.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 20899
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 20891–20899; 20900 is capstone
