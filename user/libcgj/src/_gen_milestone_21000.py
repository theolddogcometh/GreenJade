...
"""
Soft continuum generator — MILESTONE 21000 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 20901–21000 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=21000   when parent has wired through graph_batch21000.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 20901 21000
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 20901 21000
...
graph batches ending at M. For M = **21000** the exclusive range is
**20901–21000** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 20901–20950  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 20951–20975  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 20976–20990  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 20991–21000  │ decade soft gates (Wave 68 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 21000 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_21000        1          graph_batch20991.c
  gj_libcgj_green_21000       1          graph_batch20992.c
  gj_bar3_ready_21000         0          graph_batch20993.c  (bar3 open)
  gj_product_score_21000      0          graph_batch20994.c  (score open)
  gj_continuum_ready_21000    1          graph_batch20995.c
  gj_smoke_soft_21000         1          graph_batch20996.c
  gj_dyn_soft_21000           1          graph_batch20997.c
  gj_milestone_tag_21000      21000      graph_batch20998.c
  gj_continuum_wave_21000     21000      graph_batch20999.c
  gj_batch_id_21000           21000      graph_batch21000.c
  gj_graph_milestone_21000    21000      graph_batch21000.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=21000`` means parent wired CGJ_SRCS through
    ``graph_batch21000.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_21000`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_21000`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M21000_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_20900`` / ``gj_*_20800`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 68 exclusive CREATE-ONLY lands the **markers** band
  (20991–21000) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_21000.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_21000.py

  # Restrict to marker band only (soft-gate TUs; Wave 68 exclusive):
  python3 user/libcgj/src/_gen_milestone_21000.py --band markers

  # Extension identity only (20901–20950):
  python3 user/libcgj/src/_gen_milestone_21000.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_21000.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_20900.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 21000 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 21000
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 20901
WAVE_HI = 21000  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 20901, 20950
IDENTITY_LO, IDENTITY_HI = 20951, 20975
FILL_LO, FILL_HI = 20976, 20990
MARKERS_LO, MARKERS_HI = 20991, 21000
...
# Stems become gj_<stem>_21000. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (20991, "shell_green", 1, "shell product green extract (wave 21000)"),
    (20992, "libcgj_green", 1, "libcgj product green extract (wave 21000)"),
    (20993, "bar3_ready", 0, "product bar3 readiness gate (wave 21000)"),
    (20994, "product_score", 0, "product readiness score tag (wave 21000)"),
    (20995, "continuum_ready", 1, "continuum readiness gate (wave 21000)"),
    (20996, "smoke_soft", 1, "smoke soft lamp (wave 21000)"),
    (20997, "dyn_soft", 1, "dyn soft lamp (wave 21000)"),
    (20998, "milestone_tag", MILESTONE, "milestone 21000 product tag"),
    (20999, "continuum_wave", MILESTONE, "milestone 21000 continuum wave identity"),
    # 21000 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        20976: "continuum wave fill soft unit + batch id",
        20980: "continuum bridge soft unit + batch id",
        20990: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (20991–20999), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 20993
    tag_batch = MARKERS_LO + 7  # 20998
    wave_batch = MARKERS_LO + 8  # 20999
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_20900 / gj_shell_green_20800 / gj_shell_green_20700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_20900 / gj_libcgj_green_20800 / gj_libcgj_green_20700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_20900 / gj_bar3_ready_20800 / gj_bar3_ready_20700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_20900 / gj_product_score_20800 / gj_product_score_20700\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_20900 / gj_continuum_ready_20800 /\n"
            f" * gj_continuum_ready_20700 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_20900 / gj_smoke_soft_20800 / gj_smoke_soft_20700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_20900 / gj_dyn_soft_20800 / gj_dyn_soft_20700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_20900 / gj_milestone_tag_20800 / gj_milestone_tag_20700,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_20900 / gj_continuum_wave_20800 /\n"
            f" * gj_continuum_wave_20700, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch21000.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 20999
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 20991–20999; 21000 is capstone
