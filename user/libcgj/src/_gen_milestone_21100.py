...
"""
Soft continuum generator — MILESTONE 21100 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 21001–21100 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=21100   when parent has wired through graph_batch21100.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 21001 21100
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 21001 21100
...
graph batches ending at M. For M = **21100** the exclusive range is
**21001–21100** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 21001–21050  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 21051–21075  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 21076–21090  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 21091–21100  │ decade soft gates (Wave 69 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 21100 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_21100        1          graph_batch21091.c
  gj_libcgj_green_21100       1          graph_batch21092.c
  gj_bar3_ready_21100         0          graph_batch21093.c  (bar3 open)
  gj_product_score_21100      0          graph_batch21094.c  (score open)
  gj_continuum_ready_21100    1          graph_batch21095.c
  gj_smoke_soft_21100         1          graph_batch21096.c
  gj_dyn_soft_21100           1          graph_batch21097.c
  gj_milestone_tag_21100      21100      graph_batch21098.c
  gj_continuum_wave_21100     21100      graph_batch21099.c
  gj_batch_id_21100           21100      graph_batch21100.c
  gj_graph_milestone_21100    21100      graph_batch21100.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=21100`` means parent wired CGJ_SRCS through
    ``graph_batch21100.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_21100`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_21100`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M21100_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_21000`` / ``gj_*_20900`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 69 exclusive CREATE-ONLY lands the **markers** band
  (21091–21100) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_21100.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_21100.py

  # Restrict to marker band only (soft-gate TUs; Wave 69 exclusive):
  python3 user/libcgj/src/_gen_milestone_21100.py --band markers

  # Extension identity only (21001–21050):
  python3 user/libcgj/src/_gen_milestone_21100.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_21100.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_21000.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 21100 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 21100
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 21001
WAVE_HI = 21100  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 21001, 21050
IDENTITY_LO, IDENTITY_HI = 21051, 21075
FILL_LO, FILL_HI = 21076, 21090
MARKERS_LO, MARKERS_HI = 21091, 21100
...
# Stems become gj_<stem>_21100. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (21091, "shell_green", 1, "shell product green extract (wave 21100)"),
    (21092, "libcgj_green", 1, "libcgj product green extract (wave 21100)"),
    (21093, "bar3_ready", 0, "product bar3 readiness gate (wave 21100)"),
    (21094, "product_score", 0, "product readiness score tag (wave 21100)"),
    (21095, "continuum_ready", 1, "continuum readiness gate (wave 21100)"),
    (21096, "smoke_soft", 1, "smoke soft lamp (wave 21100)"),
    (21097, "dyn_soft", 1, "dyn soft lamp (wave 21100)"),
    (21098, "milestone_tag", MILESTONE, "milestone 21100 product tag"),
    (21099, "continuum_wave", MILESTONE, "milestone 21100 continuum wave identity"),
    # 21100 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        21076: "continuum wave fill soft unit + batch id",
        21080: "continuum bridge soft unit + batch id",
        21090: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (21091–21099), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 21093
    tag_batch = MARKERS_LO + 7  # 21098
    wave_batch = MARKERS_LO + 8  # 21099
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_21000 / gj_shell_green_20900 / gj_shell_green_20800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_21000 / gj_libcgj_green_20900 / gj_libcgj_green_20800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_21000 / gj_bar3_ready_20900 / gj_bar3_ready_20800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_21000 / gj_product_score_20900 / gj_product_score_20800\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_21000 / gj_continuum_ready_20900 /\n"
            f" * gj_continuum_ready_20800 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_21000 / gj_smoke_soft_20900 / gj_smoke_soft_20800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_21000 / gj_dyn_soft_20900 / gj_dyn_soft_20800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_21000 / gj_milestone_tag_20900 / gj_milestone_tag_20800,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_21000 / gj_continuum_wave_20900 /\n"
            f" * gj_continuum_wave_20800, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch21100.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 21099
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 21091–21099; 21100 is capstone
