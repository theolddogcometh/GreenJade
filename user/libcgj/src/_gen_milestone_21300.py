...
"""
Soft continuum generator — MILESTONE 21300 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 21201–21300 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=21300   when parent has wired through graph_batch21300.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 21201 21300
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 21201 21300
...
graph batches ending at M. For M = **21300** the exclusive range is
**21201–21300** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 21201–21250  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 21251–21275  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 21276–21290  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 21291–21300  │ decade soft gates (Wave 71 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 21300 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_21300        1          graph_batch21291.c
  gj_libcgj_green_21300       1          graph_batch21292.c
  gj_bar3_ready_21300         0          graph_batch21293.c  (bar3 open)
  gj_product_score_21300      0          graph_batch21294.c  (score open)
  gj_continuum_ready_21300    1          graph_batch21295.c
  gj_smoke_soft_21300         1          graph_batch21296.c
  gj_dyn_soft_21300           1          graph_batch21297.c
  gj_milestone_tag_21300      21300      graph_batch21298.c
  gj_continuum_wave_21300     21300      graph_batch21299.c
  gj_batch_id_21300           21300      graph_batch21300.c
  gj_graph_milestone_21300    21300      graph_batch21300.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=21300`` means parent wired CGJ_SRCS through
    ``graph_batch21300.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_21300`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_21300`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M21300_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_21200`` / ``gj_*_21100`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 71 exclusive CREATE-ONLY lands the **markers** band
  (21291–21300) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_21300.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_21300.py

  # Restrict to marker band only (soft-gate TUs; Wave 71 exclusive):
  python3 user/libcgj/src/_gen_milestone_21300.py --band markers

  # Extension identity only (21201–21250):
  python3 user/libcgj/src/_gen_milestone_21300.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_21300.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_21200.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 21300 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 21300
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 21201
WAVE_HI = 21300  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 21201, 21250
IDENTITY_LO, IDENTITY_HI = 21251, 21275
FILL_LO, FILL_HI = 21276, 21290
MARKERS_LO, MARKERS_HI = 21291, 21300
...
# Stems become gj_<stem>_21300. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (21291, "shell_green", 1, "shell product green extract (wave 21300)"),
    (21292, "libcgj_green", 1, "libcgj product green extract (wave 21300)"),
    (21293, "bar3_ready", 0, "product bar3 readiness gate (wave 21300)"),
    (21294, "product_score", 0, "product readiness score tag (wave 21300)"),
    (21295, "continuum_ready", 1, "continuum readiness gate (wave 21300)"),
    (21296, "smoke_soft", 1, "smoke soft lamp (wave 21300)"),
    (21297, "dyn_soft", 1, "dyn soft lamp (wave 21300)"),
    (21298, "milestone_tag", MILESTONE, "milestone 21300 product tag"),
    (21299, "continuum_wave", MILESTONE, "milestone 21300 continuum wave identity"),
    # 21300 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        21276: "continuum wave fill soft unit + batch id",
        21280: "continuum bridge soft unit + batch id",
        21290: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (21291–21299), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 21293
    tag_batch = MARKERS_LO + 7  # 21298
    wave_batch = MARKERS_LO + 8  # 21299
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_21200 / gj_shell_green_21100 / gj_shell_green_21000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_21200 / gj_libcgj_green_21100 / gj_libcgj_green_21000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_21200 / gj_bar3_ready_21100 / gj_bar3_ready_21000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_21200 / gj_product_score_21100 / gj_product_score_21000\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_21200 / gj_continuum_ready_21100 /\n"
            f" * gj_continuum_ready_21000 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_21200 / gj_smoke_soft_21100 / gj_smoke_soft_21000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_21200 / gj_dyn_soft_21100 / gj_dyn_soft_21000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_21200 / gj_milestone_tag_21100 / gj_milestone_tag_21000,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_21200 / gj_continuum_wave_21100 /\n"
            f" * gj_continuum_wave_21000, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch21300.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 21299
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 21291–21299; 21300 is capstone
