...
"""
Soft continuum generator — MILESTONE 21400 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 21301–21400 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=21400   when parent has wired through graph_batch21400.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 21301 21400
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 21301 21400
...
graph batches ending at M. For M = **21400** the exclusive range is
**21301–21400** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 21301–21350  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 21351–21375  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 21376–21390  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 21391–21400  │ decade soft gates (Wave 72 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 21400 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_21400        1          graph_batch21391.c
  gj_libcgj_green_21400       1          graph_batch21392.c
  gj_bar3_ready_21400         0          graph_batch21393.c  (bar3 open)
  gj_product_score_21400      0          graph_batch21394.c  (score open)
  gj_continuum_ready_21400    1          graph_batch21395.c
  gj_smoke_soft_21400         1          graph_batch21396.c
  gj_dyn_soft_21400           1          graph_batch21397.c
  gj_milestone_tag_21400      21400      graph_batch21398.c
  gj_continuum_wave_21400     21400      graph_batch21399.c
  gj_batch_id_21400           21400      graph_batch21400.c
  gj_graph_milestone_21400    21400      graph_batch21400.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=21400`` means parent wired CGJ_SRCS through
    ``graph_batch21400.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_21400`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_21400`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M21400_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_21300`` / ``gj_*_21200`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 72 exclusive CREATE-ONLY lands the **markers** band
  (21391–21400) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_21400.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_21400.py

  # Restrict to marker band only (soft-gate TUs; Wave 72 exclusive):
  python3 user/libcgj/src/_gen_milestone_21400.py --band markers

  # Extension identity only (21301–21350):
  python3 user/libcgj/src/_gen_milestone_21400.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_21400.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_21300.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 21400 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 21400
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 21301
WAVE_HI = 21400  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 21301, 21350
IDENTITY_LO, IDENTITY_HI = 21351, 21375
FILL_LO, FILL_HI = 21376, 21390
MARKERS_LO, MARKERS_HI = 21391, 21400
...
# Stems become gj_<stem>_21400. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (21391, "shell_green", 1, "shell product green extract (wave 21400)"),
    (21392, "libcgj_green", 1, "libcgj product green extract (wave 21400)"),
    (21393, "bar3_ready", 0, "product bar3 readiness gate (wave 21400)"),
    (21394, "product_score", 0, "product readiness score tag (wave 21400)"),
    (21395, "continuum_ready", 1, "continuum readiness gate (wave 21400)"),
    (21396, "smoke_soft", 1, "smoke soft lamp (wave 21400)"),
    (21397, "dyn_soft", 1, "dyn soft lamp (wave 21400)"),
    (21398, "milestone_tag", MILESTONE, "milestone 21400 product tag"),
    (21399, "continuum_wave", MILESTONE, "milestone 21400 continuum wave identity"),
    # 21400 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        21376: "continuum wave fill soft unit + batch id",
        21380: "continuum bridge soft unit + batch id",
        21390: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (21391–21399), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 21393
    tag_batch = MARKERS_LO + 7  # 21398
    wave_batch = MARKERS_LO + 8  # 21399
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_21300 / gj_shell_green_21200 / gj_shell_green_21100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_21300 / gj_libcgj_green_21200 / gj_libcgj_green_21100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_21300 / gj_bar3_ready_21200 / gj_bar3_ready_21100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_21300 / gj_product_score_21200 / gj_product_score_21100\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_21300 / gj_continuum_ready_21200 /\n"
            f" * gj_continuum_ready_21100 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_21300 / gj_smoke_soft_21200 / gj_smoke_soft_21100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_21300 / gj_dyn_soft_21200 / gj_dyn_soft_21100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_21300 / gj_milestone_tag_21200 / gj_milestone_tag_21100,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_21300 / gj_continuum_wave_21200 /\n"
            f" * gj_continuum_wave_21100, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch21400.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 21399
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 21391–21399; 21400 is capstone
