...
"""
Soft continuum generator — MILESTONE 21500 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 21401–21500 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=21500   when parent has wired through graph_batch21500.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 21401 21500
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 21401 21500
...
graph batches ending at M. For M = **21500** the exclusive range is
**21401–21500** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 21401–21450  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 21451–21475  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 21476–21490  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 21491–21500  │ decade soft gates (Wave 73 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 21500 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_21500        1          graph_batch21491.c
  gj_libcgj_green_21500       1          graph_batch21492.c
  gj_bar3_ready_21500         0          graph_batch21493.c  (bar3 open)
  gj_product_score_21500      0          graph_batch21494.c  (score open)
  gj_continuum_ready_21500    1          graph_batch21495.c
  gj_smoke_soft_21500         1          graph_batch21496.c
  gj_dyn_soft_21500           1          graph_batch21497.c
  gj_milestone_tag_21500      21500      graph_batch21498.c
  gj_continuum_wave_21500     21500      graph_batch21499.c
  gj_batch_id_21500           21500      graph_batch21500.c
  gj_graph_milestone_21500    21500      graph_batch21500.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=21500`` means parent wired CGJ_SRCS through
    ``graph_batch21500.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_21500`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_21500`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M21500_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_21400`` / ``gj_*_21300`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 73 exclusive CREATE-ONLY lands the **markers** band
  (21491–21500) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_21500.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_21500.py

  # Restrict to marker band only (soft-gate TUs; Wave 73 exclusive):
  python3 user/libcgj/src/_gen_milestone_21500.py --band markers

  # Extension identity only (21401–21450):
  python3 user/libcgj/src/_gen_milestone_21500.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_21500.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_21400.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 21500 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 21500
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 21401
WAVE_HI = 21500  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 21401, 21450
IDENTITY_LO, IDENTITY_HI = 21451, 21475
FILL_LO, FILL_HI = 21476, 21490
MARKERS_LO, MARKERS_HI = 21491, 21500
...
# Stems become gj_<stem>_21500. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (21491, "shell_green", 1, "shell product green extract (wave 21500)"),
    (21492, "libcgj_green", 1, "libcgj product green extract (wave 21500)"),
    (21493, "bar3_ready", 0, "product bar3 readiness gate (wave 21500)"),
    (21494, "product_score", 0, "product readiness score tag (wave 21500)"),
    (21495, "continuum_ready", 1, "continuum readiness gate (wave 21500)"),
    (21496, "smoke_soft", 1, "smoke soft lamp (wave 21500)"),
    (21497, "dyn_soft", 1, "dyn soft lamp (wave 21500)"),
    (21498, "milestone_tag", MILESTONE, "milestone 21500 product tag"),
    (21499, "continuum_wave", MILESTONE, "milestone 21500 continuum wave identity"),
    # 21500 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        21476: "continuum wave fill soft unit + batch id",
        21480: "continuum bridge soft unit + batch id",
        21490: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (21491–21499), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 21493
    tag_batch = MARKERS_LO + 7  # 21498
    wave_batch = MARKERS_LO + 8  # 21499
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_21400 / gj_shell_green_21300 / gj_shell_green_21200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_21400 / gj_libcgj_green_21300 / gj_libcgj_green_21200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_21400 / gj_bar3_ready_21300 / gj_bar3_ready_21200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_21400 / gj_product_score_21300 / gj_product_score_21200\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_21400 / gj_continuum_ready_21300 /\n"
            f" * gj_continuum_ready_21200 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_21400 / gj_smoke_soft_21300 / gj_smoke_soft_21200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_21400 / gj_dyn_soft_21300 / gj_dyn_soft_21200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_21400 / gj_milestone_tag_21300 / gj_milestone_tag_21200,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_21400 / gj_continuum_wave_21300 /\n"
            f" * gj_continuum_wave_21200, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch21500.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 21499
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 21491–21499; 21500 is capstone
