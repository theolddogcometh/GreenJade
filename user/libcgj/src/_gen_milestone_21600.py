...
"""
Soft continuum generator — MILESTONE 21600 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 21501–21600 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=21600   when parent has wired through graph_batch21600.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 21501 21600
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 21501 21600
...
graph batches ending at M. For M = **21600** the exclusive range is
**21501–21600** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 21501–21550  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 21551–21575  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 21576–21590  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 21591–21600  │ decade soft gates (Wave 74 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 21600 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_21600        1          graph_batch21591.c
  gj_libcgj_green_21600       1          graph_batch21592.c
  gj_bar3_ready_21600         0          graph_batch21593.c  (bar3 open)
  gj_product_score_21600      0          graph_batch21594.c  (score open)
  gj_continuum_ready_21600    1          graph_batch21595.c
  gj_smoke_soft_21600         1          graph_batch21596.c
  gj_dyn_soft_21600           1          graph_batch21597.c
  gj_milestone_tag_21600      21600      graph_batch21598.c
  gj_continuum_wave_21600     21600      graph_batch21599.c
  gj_batch_id_21600           21600      graph_batch21600.c
  gj_graph_milestone_21600    21600      graph_batch21600.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=21600`` means parent wired CGJ_SRCS through
    ``graph_batch21600.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_21600`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_21600`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M21600_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_21500`` / ``gj_*_21400`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 74 exclusive CREATE-ONLY lands the **markers** band
  (21591–21600) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_21600.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_21600.py

  # Restrict to marker band only (soft-gate TUs; Wave 74 exclusive):
  python3 user/libcgj/src/_gen_milestone_21600.py --band markers

  # Extension identity only (21501–21550):
  python3 user/libcgj/src/_gen_milestone_21600.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_21600.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_21500.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 21600 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 21600
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 21501
WAVE_HI = 21600  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 21501, 21550
IDENTITY_LO, IDENTITY_HI = 21551, 21575
FILL_LO, FILL_HI = 21576, 21590
MARKERS_LO, MARKERS_HI = 21591, 21600
...
# Stems become gj_<stem>_21600. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (21591, "shell_green", 1, "shell product green extract (wave 21600)"),
    (21592, "libcgj_green", 1, "libcgj product green extract (wave 21600)"),
    (21593, "bar3_ready", 0, "product bar3 readiness gate (wave 21600)"),
    (21594, "product_score", 0, "product readiness score tag (wave 21600)"),
    (21595, "continuum_ready", 1, "continuum readiness gate (wave 21600)"),
    (21596, "smoke_soft", 1, "smoke soft lamp (wave 21600)"),
    (21597, "dyn_soft", 1, "dyn soft lamp (wave 21600)"),
    (21598, "milestone_tag", MILESTONE, "milestone 21600 product tag"),
    (21599, "continuum_wave", MILESTONE, "milestone 21600 continuum wave identity"),
    # 21600 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        21576: "continuum wave fill soft unit + batch id",
        21580: "continuum bridge soft unit + batch id",
        21590: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (21591–21599), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 21593
    tag_batch = MARKERS_LO + 7  # 21598
    wave_batch = MARKERS_LO + 8  # 21599
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_21500 / gj_shell_green_21400 / gj_shell_green_21300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_21500 / gj_libcgj_green_21400 / gj_libcgj_green_21300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_21500 / gj_bar3_ready_21400 / gj_bar3_ready_21300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_21500 / gj_product_score_21400 / gj_product_score_21300\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_21500 / gj_continuum_ready_21400 /\n"
            f" * gj_continuum_ready_21300 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_21500 / gj_smoke_soft_21400 / gj_smoke_soft_21300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_21500 / gj_dyn_soft_21400 / gj_dyn_soft_21300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_21500 / gj_milestone_tag_21400 / gj_milestone_tag_21300,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_21500 / gj_continuum_wave_21400 /\n"
            f" * gj_continuum_wave_21300, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch21600.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 21599
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 21591–21599; 21600 is capstone
