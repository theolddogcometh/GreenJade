...
"""
Soft continuum generator — MILESTONE 21700 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 21601–21700 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=21700   when parent has wired through graph_batch21700.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 21601 21700
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 21601 21700
...
graph batches ending at M. For M = **21700** the exclusive range is
**21601–21700** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 21601–21650  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 21651–21675  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 21676–21690  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 21691–21700  │ decade soft gates (Wave 75 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 21700 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_21700        1          graph_batch21691.c
  gj_libcgj_green_21700       1          graph_batch21692.c
  gj_bar3_ready_21700         0          graph_batch21693.c  (bar3 open)
  gj_product_score_21700      0          graph_batch21694.c  (score open)
  gj_continuum_ready_21700    1          graph_batch21695.c
  gj_smoke_soft_21700         1          graph_batch21696.c
  gj_dyn_soft_21700           1          graph_batch21697.c
  gj_milestone_tag_21700      21700      graph_batch21698.c
  gj_continuum_wave_21700     21700      graph_batch21699.c
  gj_batch_id_21700           21700      graph_batch21700.c
  gj_graph_milestone_21700    21700      graph_batch21700.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=21700`` means parent wired CGJ_SRCS through
    ``graph_batch21700.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_21700`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_21700`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M21700_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_21600`` / ``gj_*_21500`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 75 exclusive CREATE-ONLY lands the **markers** band
  (21691–21700) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_21700.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_21700.py

  # Restrict to marker band only (soft-gate TUs; Wave 75 exclusive):
  python3 user/libcgj/src/_gen_milestone_21700.py --band markers

  # Extension identity only (21601–21650):
  python3 user/libcgj/src/_gen_milestone_21700.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_21700.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_21600.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 21700 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 21700
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 21601
WAVE_HI = 21700  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 21601, 21650
IDENTITY_LO, IDENTITY_HI = 21651, 21675
FILL_LO, FILL_HI = 21676, 21690
MARKERS_LO, MARKERS_HI = 21691, 21700
...
# Stems become gj_<stem>_21700. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (21691, "shell_green", 1, "shell product green extract (wave 21700)"),
    (21692, "libcgj_green", 1, "libcgj product green extract (wave 21700)"),
    (21693, "bar3_ready", 0, "product bar3 readiness gate (wave 21700)"),
    (21694, "product_score", 0, "product readiness score tag (wave 21700)"),
    (21695, "continuum_ready", 1, "continuum readiness gate (wave 21700)"),
    (21696, "smoke_soft", 1, "smoke soft lamp (wave 21700)"),
    (21697, "dyn_soft", 1, "dyn soft lamp (wave 21700)"),
    (21698, "milestone_tag", MILESTONE, "milestone 21700 product tag"),
    (21699, "continuum_wave", MILESTONE, "milestone 21700 continuum wave identity"),
    # 21700 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        21676: "continuum wave fill soft unit + batch id",
        21680: "continuum bridge soft unit + batch id",
        21690: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (21691–21699), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 21693
    tag_batch = MARKERS_LO + 7  # 21698
    wave_batch = MARKERS_LO + 8  # 21699
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_21600 / gj_shell_green_21500 / gj_shell_green_21400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_21600 / gj_libcgj_green_21500 / gj_libcgj_green_21400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_21600 / gj_bar3_ready_21500 / gj_bar3_ready_21400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_21600 / gj_product_score_21500 / gj_product_score_21400\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_21600 / gj_continuum_ready_21500 /\n"
            f" * gj_continuum_ready_21400 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_21600 / gj_smoke_soft_21500 / gj_smoke_soft_21400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_21600 / gj_dyn_soft_21500 / gj_dyn_soft_21400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_21600 / gj_milestone_tag_21500 / gj_milestone_tag_21400,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_21600 / gj_continuum_wave_21500 /\n"
            f" * gj_continuum_wave_21400, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch21700.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 21699
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 21691–21699; 21700 is capstone
