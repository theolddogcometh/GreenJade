...
"""
Soft continuum generator — MILESTONE 21800 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 21701–21800 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=21800   when parent has wired through graph_batch21800.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 21701 21800
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 21701 21800
...
graph batches ending at M. For M = **21800** the exclusive range is
**21701–21800** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 21701–21750  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 21751–21775  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 21776–21790  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 21791–21800  │ decade soft gates (Wave 76 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 21800 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_21800        1          graph_batch21791.c
  gj_libcgj_green_21800       1          graph_batch21792.c
  gj_bar3_ready_21800         0          graph_batch21793.c  (bar3 open)
  gj_product_score_21800      0          graph_batch21794.c  (score open)
  gj_continuum_ready_21800    1          graph_batch21795.c
  gj_smoke_soft_21800         1          graph_batch21796.c
  gj_dyn_soft_21800           1          graph_batch21797.c
  gj_milestone_tag_21800      21800      graph_batch21798.c
  gj_continuum_wave_21800     21800      graph_batch21799.c
  gj_batch_id_21800           21800      graph_batch21800.c
  gj_graph_milestone_21800    21800      graph_batch21800.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=21800`` means parent wired CGJ_SRCS through
    ``graph_batch21800.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_21800`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_21800`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M21800_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_21700`` / ``gj_*_21600`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 76 exclusive CREATE-ONLY lands the **markers** band
  (21791–21800) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_21800.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_21800.py

  # Restrict to marker band only (soft-gate TUs; Wave 76 exclusive):
  python3 user/libcgj/src/_gen_milestone_21800.py --band markers

  # Extension identity only (21701–21750):
  python3 user/libcgj/src/_gen_milestone_21800.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_21800.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_21700.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 21800 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 21800
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 21701
WAVE_HI = 21800  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 21701, 21750
IDENTITY_LO, IDENTITY_HI = 21751, 21775
FILL_LO, FILL_HI = 21776, 21790
MARKERS_LO, MARKERS_HI = 21791, 21800
...
# Stems become gj_<stem>_21800. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (21791, "shell_green", 1, "shell product green extract (wave 21800)"),
    (21792, "libcgj_green", 1, "libcgj product green extract (wave 21800)"),
    (21793, "bar3_ready", 0, "product bar3 readiness gate (wave 21800)"),
    (21794, "product_score", 0, "product readiness score tag (wave 21800)"),
    (21795, "continuum_ready", 1, "continuum readiness gate (wave 21800)"),
    (21796, "smoke_soft", 1, "smoke soft lamp (wave 21800)"),
    (21797, "dyn_soft", 1, "dyn soft lamp (wave 21800)"),
    (21798, "milestone_tag", MILESTONE, "milestone 21800 product tag"),
    (21799, "continuum_wave", MILESTONE, "milestone 21800 continuum wave identity"),
    # 21800 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        21776: "continuum wave fill soft unit + batch id",
        21780: "continuum bridge soft unit + batch id",
        21790: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (21791–21799), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 21793
    tag_batch = MARKERS_LO + 7  # 21798
    wave_batch = MARKERS_LO + 8  # 21799
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_21700 / gj_shell_green_21600 / gj_shell_green_21500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_21700 / gj_libcgj_green_21600 / gj_libcgj_green_21500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_21700 / gj_bar3_ready_21600 / gj_bar3_ready_21500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_21700 / gj_product_score_21600 / gj_product_score_21500\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_21700 / gj_continuum_ready_21600 /\n"
            f" * gj_continuum_ready_21500 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_21700 / gj_smoke_soft_21600 / gj_smoke_soft_21500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_21700 / gj_dyn_soft_21600 / gj_dyn_soft_21500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_21700 / gj_milestone_tag_21600 / gj_milestone_tag_21500,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_21700 / gj_continuum_wave_21600 /\n"
            f" * gj_continuum_wave_21500, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch21800.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 21799
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 21791–21799; 21800 is capstone
