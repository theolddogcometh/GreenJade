...
"""
Soft continuum generator — MILESTONE 21900 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 21801–21900 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=21900   when parent has wired through graph_batch21900.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 21801 21900
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 21801 21900
...
graph batches ending at M. For M = **21900** the exclusive range is
**21801–21900** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 21801–21850  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 21851–21875  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 21876–21890  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 21891–21900  │ decade soft gates (Wave 77 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 21900 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_21900        1          graph_batch21891.c
  gj_libcgj_green_21900       1          graph_batch21892.c
  gj_bar3_ready_21900         0          graph_batch21893.c  (bar3 open)
  gj_product_score_21900      0          graph_batch21894.c  (score open)
  gj_continuum_ready_21900    1          graph_batch21895.c
  gj_smoke_soft_21900         1          graph_batch21896.c
  gj_dyn_soft_21900           1          graph_batch21897.c
  gj_milestone_tag_21900      21900      graph_batch21898.c
  gj_continuum_wave_21900     21900      graph_batch21899.c
  gj_batch_id_21900           21900      graph_batch21900.c
  gj_graph_milestone_21900    21900      graph_batch21900.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=21900`` means parent wired CGJ_SRCS through
    ``graph_batch21900.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_21900`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_21900`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M21900_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_21800`` / ``gj_*_21700`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 77 exclusive CREATE-ONLY lands the **markers** band
  (21891–21900) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_21900.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_21900.py

  # Restrict to marker band only (soft-gate TUs; Wave 77 exclusive):
  python3 user/libcgj/src/_gen_milestone_21900.py --band markers

  # Extension identity only (21801–21850):
  python3 user/libcgj/src/_gen_milestone_21900.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_21900.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_21800.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 21900 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 21900
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 21801
WAVE_HI = 21900  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 21801, 21850
IDENTITY_LO, IDENTITY_HI = 21851, 21875
FILL_LO, FILL_HI = 21876, 21890
MARKERS_LO, MARKERS_HI = 21891, 21900
...
# Stems become gj_<stem>_21900. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (21891, "shell_green", 1, "shell product green extract (wave 21900)"),
    (21892, "libcgj_green", 1, "libcgj product green extract (wave 21900)"),
    (21893, "bar3_ready", 0, "product bar3 readiness gate (wave 21900)"),
    (21894, "product_score", 0, "product readiness score tag (wave 21900)"),
    (21895, "continuum_ready", 1, "continuum readiness gate (wave 21900)"),
    (21896, "smoke_soft", 1, "smoke soft lamp (wave 21900)"),
    (21897, "dyn_soft", 1, "dyn soft lamp (wave 21900)"),
    (21898, "milestone_tag", MILESTONE, "milestone 21900 product tag"),
    (21899, "continuum_wave", MILESTONE, "milestone 21900 continuum wave identity"),
    # 21900 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        21876: "continuum wave fill soft unit + batch id",
        21880: "continuum bridge soft unit + batch id",
        21890: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (21891–21899), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 21893
    tag_batch = MARKERS_LO + 7  # 21898
    wave_batch = MARKERS_LO + 8  # 21899
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_21800 / gj_shell_green_21700 / gj_shell_green_21600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_21800 / gj_libcgj_green_21700 / gj_libcgj_green_21600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_21800 / gj_bar3_ready_21700 / gj_bar3_ready_21600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_21800 / gj_product_score_21700 / gj_product_score_21600\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_21800 / gj_continuum_ready_21700 /\n"
            f" * gj_continuum_ready_21600 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_21800 / gj_smoke_soft_21700 / gj_smoke_soft_21600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_21800 / gj_dyn_soft_21700 / gj_dyn_soft_21600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_21800 / gj_milestone_tag_21700 / gj_milestone_tag_21600,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_21800 / gj_continuum_wave_21700 /\n"
            f" * gj_continuum_wave_21600, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch21900.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 21899
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 21891–21899; 21900 is capstone
