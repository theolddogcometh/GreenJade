...
"""
Soft continuum generator — MILESTONE 22000 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 21901–22000 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=22000   when parent has wired through graph_batch22000.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 21901 22000
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 21901 22000
...
graph batches ending at M. For M = **22000** the exclusive range is
**21901–22000** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 21901–21950  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 21951–21975  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 21976–21990  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 21991–22000  │ decade soft gates (Wave 78 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 22000 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_22000        1          graph_batch21991.c
  gj_libcgj_green_22000       1          graph_batch21992.c
  gj_bar3_ready_22000         0          graph_batch21993.c  (bar3 open)
  gj_product_score_22000      0          graph_batch21994.c  (score open)
  gj_continuum_ready_22000    1          graph_batch21995.c
  gj_smoke_soft_22000         1          graph_batch21996.c
  gj_dyn_soft_22000           1          graph_batch21997.c
  gj_milestone_tag_22000      22000      graph_batch21998.c
  gj_continuum_wave_22000     22000      graph_batch21999.c
  gj_batch_id_22000           22000      graph_batch22000.c
  gj_graph_milestone_22000    22000      graph_batch22000.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=22000`` means parent wired CGJ_SRCS through
    ``graph_batch22000.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_22000`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_22000`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M22000_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_21900`` / ``gj_*_21800`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 78 exclusive CREATE-ONLY lands the **markers** band
  (21991–22000) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_22000.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_22000.py

  # Restrict to marker band only (soft-gate TUs; Wave 78 exclusive):
  python3 user/libcgj/src/_gen_milestone_22000.py --band markers

  # Extension identity only (21901–21950):
  python3 user/libcgj/src/_gen_milestone_22000.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_22000.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_21900.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 22000 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 22000
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 21901
WAVE_HI = 22000  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 21901, 21950
IDENTITY_LO, IDENTITY_HI = 21951, 21975
FILL_LO, FILL_HI = 21976, 21990
MARKERS_LO, MARKERS_HI = 21991, 22000
...
# Stems become gj_<stem>_22000. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (21991, "shell_green", 1, "shell product green extract (wave 22000)"),
    (21992, "libcgj_green", 1, "libcgj product green extract (wave 22000)"),
    (21993, "bar3_ready", 0, "product bar3 readiness gate (wave 22000)"),
    (21994, "product_score", 0, "product readiness score tag (wave 22000)"),
    (21995, "continuum_ready", 1, "continuum readiness gate (wave 22000)"),
    (21996, "smoke_soft", 1, "smoke soft lamp (wave 22000)"),
    (21997, "dyn_soft", 1, "dyn soft lamp (wave 22000)"),
    (21998, "milestone_tag", MILESTONE, "milestone 22000 product tag"),
    (21999, "continuum_wave", MILESTONE, "milestone 22000 continuum wave identity"),
    # 22000 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        21976: "continuum wave fill soft unit + batch id",
        21980: "continuum bridge soft unit + batch id",
        21990: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (21991–21999), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 21993
    tag_batch = MARKERS_LO + 7  # 21998
    wave_batch = MARKERS_LO + 8  # 21999
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_21900 / gj_shell_green_21800 / gj_shell_green_21700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_21900 / gj_libcgj_green_21800 / gj_libcgj_green_21700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_21900 / gj_bar3_ready_21800 / gj_bar3_ready_21700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_21900 / gj_product_score_21800 / gj_product_score_21700\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_21900 / gj_continuum_ready_21800 /\n"
            f" * gj_continuum_ready_21700 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_21900 / gj_smoke_soft_21800 / gj_smoke_soft_21700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_21900 / gj_dyn_soft_21800 / gj_dyn_soft_21700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_21900 / gj_milestone_tag_21800 / gj_milestone_tag_21700,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_21900 / gj_continuum_wave_21800 /\n"
            f" * gj_continuum_wave_21700, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch22000.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 21999
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 21991–21999; 22000 is capstone
