...
"""
Soft continuum generator — MILESTONE 22100 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 22001–22100 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=22100   when parent has wired through graph_batch22100.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 22001 22100
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 22001 22100
...
graph batches ending at M. For M = **22100** the exclusive range is
**22001–22100** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 22001–22050  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 22051–22075  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 22076–22090  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 22091–22100  │ decade soft gates (Wave 79 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 22100 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_22100        1          graph_batch22091.c
  gj_libcgj_green_22100       1          graph_batch22092.c
  gj_bar3_ready_22100         0          graph_batch22093.c  (bar3 open)
  gj_product_score_22100      0          graph_batch22094.c  (score open)
  gj_continuum_ready_22100    1          graph_batch22095.c
  gj_smoke_soft_22100         1          graph_batch22096.c
  gj_dyn_soft_22100           1          graph_batch22097.c
  gj_milestone_tag_22100      22100      graph_batch22098.c
  gj_continuum_wave_22100     22100      graph_batch22099.c
  gj_batch_id_22100           22100      graph_batch22100.c
  gj_graph_milestone_22100    22100      graph_batch22100.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=22100`` means parent wired CGJ_SRCS through
    ``graph_batch22100.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_22100`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_22100`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M22100_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_22000`` / ``gj_*_21900`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 79 exclusive CREATE-ONLY lands the **markers** band
  (22091–22100) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_22100.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_22100.py

  # Restrict to marker band only (soft-gate TUs; Wave 79 exclusive):
  python3 user/libcgj/src/_gen_milestone_22100.py --band markers

  # Extension identity only (22001–22050):
  python3 user/libcgj/src/_gen_milestone_22100.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_22100.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_22000.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 22100 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 22100
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 22001
WAVE_HI = 22100  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 22001, 22050
IDENTITY_LO, IDENTITY_HI = 22051, 22075
FILL_LO, FILL_HI = 22076, 22090
MARKERS_LO, MARKERS_HI = 22091, 22100
...
# Stems become gj_<stem>_22100. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (22091, "shell_green", 1, "shell product green extract (wave 22100)"),
    (22092, "libcgj_green", 1, "libcgj product green extract (wave 22100)"),
    (22093, "bar3_ready", 0, "product bar3 readiness gate (wave 22100)"),
    (22094, "product_score", 0, "product readiness score tag (wave 22100)"),
    (22095, "continuum_ready", 1, "continuum readiness gate (wave 22100)"),
    (22096, "smoke_soft", 1, "smoke soft lamp (wave 22100)"),
    (22097, "dyn_soft", 1, "dyn soft lamp (wave 22100)"),
    (22098, "milestone_tag", MILESTONE, "milestone 22100 product tag"),
    (22099, "continuum_wave", MILESTONE, "milestone 22100 continuum wave identity"),
    # 22100 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        22076: "continuum wave fill soft unit + batch id",
        22080: "continuum bridge soft unit + batch id",
        22090: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (22091–22099), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 22093
    tag_batch = MARKERS_LO + 7  # 22098
    wave_batch = MARKERS_LO + 8  # 22099
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_22100 / gj_shell_green_21900 / gj_shell_green_21800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_22100 / gj_libcgj_green_21900 / gj_libcgj_green_21800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_22100 / gj_bar3_ready_21900 / gj_bar3_ready_21800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_22100 / gj_product_score_21900 / gj_product_score_21800\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_22000 / gj_continuum_ready_21900 /\n"
            f" * gj_continuum_ready_21800 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_22100 / gj_smoke_soft_21900 / gj_smoke_soft_21800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_22100 / gj_dyn_soft_21900 / gj_dyn_soft_21800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_22100 / gj_milestone_tag_21900 / gj_milestone_tag_21800,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_22000 / gj_continuum_wave_21900 /\n"
            f" * gj_continuum_wave_21800, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch22100.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 22099
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 22091–22099; 22100 is capstone
