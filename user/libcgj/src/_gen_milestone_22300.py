...
"""
Soft continuum generator — MILESTONE 22300 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 22201–22300 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=22300   when parent has wired through graph_batch22300.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 22201 22300
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 22201 22300
...
graph batches ending at M. For M = **22300** the exclusive range is
**22201–22300** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 22201–22250  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 22251–22275  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 22276–22290  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 22291–22300  │ decade soft gates (Wave 81 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 22300 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_22300        1          graph_batch22291.c
  gj_libcgj_green_22300       1          graph_batch22292.c
  gj_bar3_ready_22300         0          graph_batch22293.c  (bar3 open)
  gj_product_score_22300      0          graph_batch22294.c  (score open)
  gj_continuum_ready_22300    1          graph_batch22295.c
  gj_smoke_soft_22300         1          graph_batch22296.c
  gj_dyn_soft_22300           1          graph_batch22297.c
  gj_milestone_tag_22300      22300      graph_batch22298.c
  gj_continuum_wave_22300     22300      graph_batch22299.c
  gj_batch_id_22300           22300      graph_batch22300.c
  gj_graph_milestone_22300    22300      graph_batch22300.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=22300`` means parent wired CGJ_SRCS through
    ``graph_batch22300.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_22300`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_22300`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M22300_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_22200`` / ``gj_*_22100`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 81 exclusive CREATE-ONLY lands the **markers** band
  (22291–22300) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_22300.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_22300.py

  # Restrict to marker band only (soft-gate TUs; Wave 81 exclusive):
  python3 user/libcgj/src/_gen_milestone_22300.py --band markers

  # Extension identity only (22201–22250):
  python3 user/libcgj/src/_gen_milestone_22300.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_22300.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_22200.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 22300 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 22300
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 22201
WAVE_HI = 22300  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 22201, 22250
IDENTITY_LO, IDENTITY_HI = 22251, 22275
FILL_LO, FILL_HI = 22276, 22290
MARKERS_LO, MARKERS_HI = 22291, 22300
...
# Stems become gj_<stem>_22300. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (22291, "shell_green", 1, "shell product green extract (wave 22300)"),
    (22292, "libcgj_green", 1, "libcgj product green extract (wave 22300)"),
    (22293, "bar3_ready", 0, "product bar3 readiness gate (wave 22300)"),
    (22294, "product_score", 0, "product readiness score tag (wave 22300)"),
    (22295, "continuum_ready", 1, "continuum readiness gate (wave 22300)"),
    (22296, "smoke_soft", 1, "smoke soft lamp (wave 22300)"),
    (22297, "dyn_soft", 1, "dyn soft lamp (wave 22300)"),
    (22298, "milestone_tag", MILESTONE, "milestone 22300 product tag"),
    (22299, "continuum_wave", MILESTONE, "milestone 22300 continuum wave identity"),
    # 22300 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        22276: "continuum wave fill soft unit + batch id",
        22280: "continuum bridge soft unit + batch id",
        22290: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (22291–22299), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 22293
    tag_batch = MARKERS_LO + 7  # 22298
    wave_batch = MARKERS_LO + 8  # 22299
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_22200 / gj_shell_green_22100 / gj_shell_green_22000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_22200 / gj_libcgj_green_22100 / gj_libcgj_green_22000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_22200 / gj_bar3_ready_22100 / gj_bar3_ready_22000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_22200 / gj_product_score_22100 / gj_product_score_22000\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_22200 / gj_continuum_ready_22100 /\n"
            f" * gj_continuum_ready_22000 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_22200 / gj_smoke_soft_22100 / gj_smoke_soft_22000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_22200 / gj_dyn_soft_22100 / gj_dyn_soft_22000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_22200 / gj_milestone_tag_22100 / gj_milestone_tag_22000,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_22200 / gj_continuum_wave_22100 /\n"
            f" * gj_continuum_wave_22000, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch22300.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 22299
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 22291–22299; 22300 is capstone
