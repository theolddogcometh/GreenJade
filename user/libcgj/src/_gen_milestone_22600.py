...
"""
Soft continuum generator — MILESTONE 22600 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 22501–22600 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=22600   when parent has wired through graph_batch22600.c
...
graph batches ending at M. For M = **22600** the exclusive range is
**22501–22600** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 22501–22550  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 22551–22575  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 22576–22590  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 22591–22600  │ decade soft gates (Wave 84 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 22600 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_22600        1          graph_batch22591.c
  gj_libcgj_green_22600       1          graph_batch22592.c
  gj_bar3_ready_22600         0          graph_batch22593.c  (bar3 open)
  gj_product_score_22600      0          graph_batch22594.c  (score open)
  gj_continuum_ready_22600    1          graph_batch22595.c
  gj_smoke_soft_22600         1          graph_batch22596.c
  gj_dyn_soft_22600           1          graph_batch22597.c
  gj_milestone_tag_22600      22600      graph_batch22598.c
  gj_continuum_wave_22600     22600      graph_batch22599.c
  gj_batch_id_22600           22600      graph_batch22600.c
  gj_graph_milestone_22600    22600      graph_batch22600.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=22600`` means parent wired CGJ_SRCS through
    ``graph_batch22600.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_22600`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_22600`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M22600_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_22500`` / ``gj_*_22400`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 84 exclusive CREATE-ONLY lands the **markers** band
  (22591–22600) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_22600.py

  # Restrict to marker band only (soft-gate TUs; Wave 84 exclusive):
  python3 user/libcgj/src/_gen_milestone_22600.py --band markers

  # Extension identity only (22501–22550):
  python3 user/libcgj/src/_gen_milestone_22600.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_22600.py --force
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 22600 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 22600
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 22501
WAVE_HI = 22600  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 22501, 22550
IDENTITY_LO, IDENTITY_HI = 22551, 22575
FILL_LO, FILL_HI = 22576, 22590
MARKERS_LO, MARKERS_HI = 22591, 22600
...
# Stems become gj_<stem>_22600. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (22591, "shell_green", 1, "shell product green extract (wave 22600)"),
    (22592, "libcgj_green", 1, "libcgj product green extract (wave 22600)"),
    (22593, "bar3_ready", 0, "product bar3 readiness gate (wave 22600)"),
    (22594, "product_score", 0, "product readiness score tag (wave 22600)"),
    (22595, "continuum_ready", 1, "continuum readiness gate (wave 22600)"),
    (22596, "smoke_soft", 1, "smoke soft lamp (wave 22600)"),
    (22597, "dyn_soft", 1, "dyn soft lamp (wave 22600)"),
    (22598, "milestone_tag", MILESTONE, "milestone 22600 product tag"),
    (22599, "continuum_wave", MILESTONE, "milestone 22600 continuum wave identity"),
    # 22600 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        22576: "continuum wave fill soft unit + batch id",
        22580: "continuum bridge soft unit + batch id",
        22590: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (22591–22599), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 22593
    tag_batch = MARKERS_LO + 7  # 22598
    wave_batch = MARKERS_LO + 8  # 22599
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_22500 / gj_shell_green_22400 / gj_shell_green_22300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_22500 / gj_libcgj_green_22400 / gj_libcgj_green_22300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_22500 / gj_bar3_ready_22400 / gj_bar3_ready_22300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_22500 / gj_product_score_22400 / gj_product_score_22300\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_22500 / gj_continuum_ready_22400 /\n"
            f" * gj_continuum_ready_22300 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_22500 / gj_smoke_soft_22400 / gj_smoke_soft_22300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_22500 / gj_dyn_soft_22400 / gj_dyn_soft_22300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_22500 / gj_milestone_tag_22400 / gj_milestone_tag_22300,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_22500 / gj_continuum_wave_22400 /\n"
            f" * gj_continuum_wave_22300, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch22600.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 22599
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 22591–22599; 22600 is capstone
