...
"""
Soft continuum generator — MILESTONE 22700 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 22601–22700 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=22700   when parent has wired through graph_batch22700.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 22601 22700
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 22601 22700
...
graph batches ending at M. For M = **22700** the exclusive range is
**22601–22700** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 22601–22650  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 22651–22675  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 22676–22690  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 22691–22700  │ decade soft gates (Wave 85 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 22700 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_22700        1          graph_batch22691.c
  gj_libcgj_green_22700       1          graph_batch22692.c
  gj_bar3_ready_22700         0          graph_batch22693.c  (bar3 open)
  gj_product_score_22700      0          graph_batch22694.c  (score open)
  gj_continuum_ready_22700    1          graph_batch22695.c
  gj_smoke_soft_22700         1          graph_batch22696.c
  gj_dyn_soft_22700           1          graph_batch22697.c
  gj_milestone_tag_22700      22700      graph_batch22698.c
  gj_continuum_wave_22700     22700      graph_batch22699.c
  gj_batch_id_22700           22700      graph_batch22700.c
  gj_graph_milestone_22700    22700      graph_batch22700.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=22700`` means parent wired CGJ_SRCS through
    ``graph_batch22700.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_22700`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_22700`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M22700_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_22600`` / ``gj_*_22500`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 85 exclusive CREATE-ONLY lands the **markers** band
  (22691–22700) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_22700.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_22700.py

  # Restrict to marker band only (soft-gate TUs; Wave 85 exclusive):
  python3 user/libcgj/src/_gen_milestone_22700.py --band markers

  # Extension identity only (22601–22650):
  python3 user/libcgj/src/_gen_milestone_22700.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_22700.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_22600.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 22700 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 22700
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 22601
WAVE_HI = 22700  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 22601, 22650
IDENTITY_LO, IDENTITY_HI = 22651, 22675
FILL_LO, FILL_HI = 22676, 22690
MARKERS_LO, MARKERS_HI = 22691, 22700
...
# Stems become gj_<stem>_22700. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (22691, "shell_green", 1, "shell product green extract (wave 22700)"),
    (22692, "libcgj_green", 1, "libcgj product green extract (wave 22700)"),
    (22693, "bar3_ready", 0, "product bar3 readiness gate (wave 22700)"),
    (22694, "product_score", 0, "product readiness score tag (wave 22700)"),
    (22695, "continuum_ready", 1, "continuum readiness gate (wave 22700)"),
    (22696, "smoke_soft", 1, "smoke soft lamp (wave 22700)"),
    (22697, "dyn_soft", 1, "dyn soft lamp (wave 22700)"),
    (22698, "milestone_tag", MILESTONE, "milestone 22700 product tag"),
    (22699, "continuum_wave", MILESTONE, "milestone 22700 continuum wave identity"),
    # 22700 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        22676: "continuum wave fill soft unit + batch id",
        22680: "continuum bridge soft unit + batch id",
        22690: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (22691–22699), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 22693
    tag_batch = MARKERS_LO + 7  # 22698
    wave_batch = MARKERS_LO + 8  # 22699
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_22600 / gj_shell_green_22500 / gj_shell_green_22400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_22600 / gj_libcgj_green_22500 / gj_libcgj_green_22400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_22600 / gj_bar3_ready_22500 / gj_bar3_ready_22400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_22600 / gj_product_score_22500 / gj_product_score_22400\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_22600 / gj_continuum_ready_22500 /\n"
            f" * gj_continuum_ready_22400 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_22600 / gj_smoke_soft_22500 / gj_smoke_soft_22400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_22600 / gj_dyn_soft_22500 / gj_dyn_soft_22400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_22600 / gj_milestone_tag_22500 / gj_milestone_tag_22400,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_22600 / gj_continuum_wave_22500 /\n"
            f" * gj_continuum_wave_22400, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch22700.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 22699
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 22691–22699; 22700 is capstone
