...
"""
Soft continuum generator — MILESTONE 22800 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 22701–22800 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=22800   when parent has wired through graph_batch22800.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 22701 22800
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 22701 22800
...
graph batches ending at M. For M = **22800** the exclusive range is
**22701–22800** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 22701–22750  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 22751–22775  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 22776–22790  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 22791–22800  │ decade soft gates (Wave 86 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 22800 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_22800        1          graph_batch22791.c
  gj_libcgj_green_22800       1          graph_batch22792.c
  gj_bar3_ready_22800         0          graph_batch22793.c  (bar3 open)
  gj_product_score_22800      0          graph_batch22794.c  (score open)
  gj_continuum_ready_22800    1          graph_batch22795.c
  gj_smoke_soft_22800         1          graph_batch22796.c
  gj_dyn_soft_22800           1          graph_batch22797.c
  gj_milestone_tag_22800      22800      graph_batch22798.c
  gj_continuum_wave_22800     22800      graph_batch22799.c
  gj_batch_id_22800           22800      graph_batch22800.c
  gj_graph_milestone_22800    22800      graph_batch22800.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=22800`` means parent wired CGJ_SRCS through
    ``graph_batch22800.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_22800`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_22800`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M22800_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_22700`` / ``gj_*_22600`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 86 exclusive CREATE-ONLY lands the **markers** band
  (22791–22800) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_22800.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_22800.py

  # Restrict to marker band only (soft-gate TUs; Wave 86 exclusive):
  python3 user/libcgj/src/_gen_milestone_22800.py --band markers

  # Extension identity only (22701–22750):
  python3 user/libcgj/src/_gen_milestone_22800.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_22800.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_22700.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 22800 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 22800
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 22701
WAVE_HI = 22800  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 22701, 22750
IDENTITY_LO, IDENTITY_HI = 22751, 22775
FILL_LO, FILL_HI = 22776, 22790
MARKERS_LO, MARKERS_HI = 22791, 22800
...
# Stems become gj_<stem>_22800. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (22791, "shell_green", 1, "shell product green extract (wave 22800)"),
    (22792, "libcgj_green", 1, "libcgj product green extract (wave 22800)"),
    (22793, "bar3_ready", 0, "product bar3 readiness gate (wave 22800)"),
    (22794, "product_score", 0, "product readiness score tag (wave 22800)"),
    (22795, "continuum_ready", 1, "continuum readiness gate (wave 22800)"),
    (22796, "smoke_soft", 1, "smoke soft lamp (wave 22800)"),
    (22797, "dyn_soft", 1, "dyn soft lamp (wave 22800)"),
    (22798, "milestone_tag", MILESTONE, "milestone 22800 product tag"),
    (22799, "continuum_wave", MILESTONE, "milestone 22800 continuum wave identity"),
    # 22800 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        22776: "continuum wave fill soft unit + batch id",
        22780: "continuum bridge soft unit + batch id",
        22790: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (22791–22799), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 22793
    tag_batch = MARKERS_LO + 7  # 22798
    wave_batch = MARKERS_LO + 8  # 22799
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_22700 / gj_shell_green_22600 / gj_shell_green_22500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_22700 / gj_libcgj_green_22600 / gj_libcgj_green_22500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_22700 / gj_bar3_ready_22600 / gj_bar3_ready_22500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_22700 / gj_product_score_22600 / gj_product_score_22500\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_22700 / gj_continuum_ready_22600 /\n"
            f" * gj_continuum_ready_22500 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_22700 / gj_smoke_soft_22600 / gj_smoke_soft_22500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_22700 / gj_dyn_soft_22600 / gj_dyn_soft_22500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_22700 / gj_milestone_tag_22600 / gj_milestone_tag_22500,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_22700 / gj_continuum_wave_22600 /\n"
            f" * gj_continuum_wave_22500, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch22800.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 22799
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 22791–22799; 22800 is capstone
