...
"""
Soft continuum generator — MILESTONE 22900 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 22801–22900 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=22900   when parent has wired through graph_batch22900.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 22801 22900
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 22801 22900
...
graph batches ending at M. For M = **22900** the exclusive range is
**22801–22900** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 22801–22850  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 22851–22875  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 22876–22890  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 22891–22900  │ decade soft gates (Wave 87 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 22900 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_22900        1          graph_batch22891.c
  gj_libcgj_green_22900       1          graph_batch22892.c
  gj_bar3_ready_22900         0          graph_batch22893.c  (bar3 open)
  gj_product_score_22900      0          graph_batch22894.c  (score open)
  gj_continuum_ready_22900    1          graph_batch22895.c
  gj_smoke_soft_22900         1          graph_batch22896.c
  gj_dyn_soft_22900           1          graph_batch22897.c
  gj_milestone_tag_22900      22900      graph_batch22898.c
  gj_continuum_wave_22900     22900      graph_batch22899.c
  gj_batch_id_22900           22900      graph_batch22900.c
  gj_graph_milestone_22900    22900      graph_batch22900.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=22900`` means parent wired CGJ_SRCS through
    ``graph_batch22900.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_22900`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_22900`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M22900_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_22800`` / ``gj_*_22700`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 87 exclusive CREATE-ONLY lands the **markers** band
  (22891–22900) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_22900.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_22900.py

  # Restrict to marker band only (soft-gate TUs; Wave 87 exclusive):
  python3 user/libcgj/src/_gen_milestone_22900.py --band markers

  # Extension identity only (22801–22850):
  python3 user/libcgj/src/_gen_milestone_22900.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_22900.py --force
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 22900 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 22900
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 22801
WAVE_HI = 22900  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 22801, 22850
IDENTITY_LO, IDENTITY_HI = 22851, 22875
FILL_LO, FILL_HI = 22876, 22890
MARKERS_LO, MARKERS_HI = 22891, 22900
...
# Stems become gj_<stem>_22900. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (22891, "shell_green", 1, "shell product green extract (wave 22900)"),
    (22892, "libcgj_green", 1, "libcgj product green extract (wave 22900)"),
    (22893, "bar3_ready", 0, "product bar3 readiness gate (wave 22900)"),
    (22894, "product_score", 0, "product readiness score tag (wave 22900)"),
    (22895, "continuum_ready", 1, "continuum readiness gate (wave 22900)"),
    (22896, "smoke_soft", 1, "smoke soft lamp (wave 22900)"),
    (22897, "dyn_soft", 1, "dyn soft lamp (wave 22900)"),
    (22898, "milestone_tag", MILESTONE, "milestone 22900 product tag"),
    (22899, "continuum_wave", MILESTONE, "milestone 22900 continuum wave identity"),
    # 22900 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        22876: "continuum wave fill soft unit + batch id",
        22880: "continuum bridge soft unit + batch id",
        22890: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (22891–22899), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 22893
    tag_batch = MARKERS_LO + 7  # 22898
    wave_batch = MARKERS_LO + 8  # 22899
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_22800 / gj_shell_green_22700 / gj_shell_green_22600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_22800 / gj_libcgj_green_22700 / gj_libcgj_green_22600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_22800 / gj_bar3_ready_22700 / gj_bar3_ready_22600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_22800 / gj_product_score_22700 / gj_product_score_22600\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_22800 / gj_continuum_ready_22700 /\n"
            f" * gj_continuum_ready_22600 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_22800 / gj_smoke_soft_22700 / gj_smoke_soft_22600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_22800 / gj_dyn_soft_22700 / gj_dyn_soft_22600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_22800 / gj_milestone_tag_22700 / gj_milestone_tag_22600,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_22800 / gj_continuum_wave_22700 /\n"
            f" * gj_continuum_wave_22600, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch22900.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 22899
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 22891–22899; 22900 is capstone
