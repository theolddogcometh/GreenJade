...
"""
Soft continuum generator — MILESTONE 23000 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 22901–23000 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=23000   when parent has wired through graph_batch23000.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 22901 23000
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 22901 23000
...
graph batches ending at M. For M = **23000** the exclusive range is
**22901–23000** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 22901–22950  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 22951–22975  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 22976–22990  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 22991–23000  │ decade soft gates (Wave 88 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 23000 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_23000        1          graph_batch22991.c
  gj_libcgj_green_23000       1          graph_batch22992.c
  gj_bar3_ready_23000         0          graph_batch22993.c  (bar3 open)
  gj_product_score_23000      0          graph_batch22994.c  (score open)
  gj_continuum_ready_23000    1          graph_batch22995.c
  gj_smoke_soft_23000         1          graph_batch22996.c
  gj_dyn_soft_23000           1          graph_batch22997.c
  gj_milestone_tag_23000      23000      graph_batch22998.c
  gj_continuum_wave_23000     23000      graph_batch22999.c
  gj_batch_id_23000           23000      graph_batch23000.c
  gj_graph_milestone_23000    23000      graph_batch23000.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=23000`` means parent wired CGJ_SRCS through
    ``graph_batch23000.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_23000`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_23000`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M23000_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_22900`` / ``gj_*_22800`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 88 exclusive CREATE-ONLY lands the **markers** band
  (22991–23000) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_23000.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_23000.py

  # Restrict to marker band only (soft-gate TUs; Wave 88 exclusive):
  python3 user/libcgj/src/_gen_milestone_23000.py --band markers

  # Extension identity only (22901–22950):
  python3 user/libcgj/src/_gen_milestone_23000.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_23000.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_22800.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 23000 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 23000
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 22901
WAVE_HI = 23000  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 22901, 22950
IDENTITY_LO, IDENTITY_HI = 22951, 22975
FILL_LO, FILL_HI = 22976, 22990
MARKERS_LO, MARKERS_HI = 22991, 23000
...
# Stems become gj_<stem>_23000. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (22991, "shell_green", 1, "shell product green extract (wave 23000)"),
    (22992, "libcgj_green", 1, "libcgj product green extract (wave 23000)"),
    (22993, "bar3_ready", 0, "product bar3 readiness gate (wave 23000)"),
    (22994, "product_score", 0, "product readiness score tag (wave 23000)"),
    (22995, "continuum_ready", 1, "continuum readiness gate (wave 23000)"),
    (22996, "smoke_soft", 1, "smoke soft lamp (wave 23000)"),
    (22997, "dyn_soft", 1, "dyn soft lamp (wave 23000)"),
    (22998, "milestone_tag", MILESTONE, "milestone 23000 product tag"),
    (22999, "continuum_wave", MILESTONE, "milestone 23000 continuum wave identity"),
    # 23000 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        22976: "continuum wave fill soft unit + batch id",
        22980: "continuum bridge soft unit + batch id",
        22990: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (22991–22999), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 22993
    tag_batch = MARKERS_LO + 7  # 22998
    wave_batch = MARKERS_LO + 8  # 22999
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_22900 / gj_shell_green_22800 / gj_shell_green_22700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_22900 / gj_libcgj_green_22800 / gj_libcgj_green_22700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_22900 / gj_bar3_ready_22800 / gj_bar3_ready_22700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_22900 / gj_product_score_22800 / gj_product_score_22700\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_22900 / gj_continuum_ready_22800 /\n"
            f" * gj_continuum_ready_22700 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_22900 / gj_smoke_soft_22800 / gj_smoke_soft_22700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_22900 / gj_dyn_soft_22800 / gj_dyn_soft_22700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_22900 / gj_milestone_tag_22800 / gj_milestone_tag_22700,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_22900 / gj_continuum_wave_22800 /\n"
            f" * gj_continuum_wave_22700, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch23000.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 22999
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 22991–22999; 23000 is capstone
