...
"""
Soft continuum generator — MILESTONE 23100 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 23001–23100 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=23100   when parent has wired through graph_batch23100.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 23001 23100
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 23001 23100
...
graph batches ending at M. For M = **23100** the exclusive range is
**23001–23100** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 23001–23050  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 23051–23075  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 23076–23090  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 23091–23100  │ decade soft gates (Wave 89 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 23100 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_23100        1          graph_batch23091.c
  gj_libcgj_green_23100       1          graph_batch23092.c
  gj_bar3_ready_23100         0          graph_batch23093.c  (bar3 open)
  gj_product_score_23100      0          graph_batch23094.c  (score open)
  gj_continuum_ready_23100    1          graph_batch23095.c
  gj_smoke_soft_23100         1          graph_batch23096.c
  gj_dyn_soft_23100           1          graph_batch23097.c
  gj_milestone_tag_23100      23100      graph_batch23098.c
  gj_continuum_wave_23100     23100      graph_batch23099.c
  gj_batch_id_23100           23100      graph_batch23100.c
  gj_graph_milestone_23100    23100      graph_batch23100.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=23100`` means parent wired CGJ_SRCS through
    ``graph_batch23100.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_23100`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_23100`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M23100_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_23000`` / ``gj_*_22900`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 89 exclusive CREATE-ONLY lands the **markers** band
  (23091–23100) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_23100.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_23100.py

  # Restrict to marker band only (soft-gate TUs; Wave 89 exclusive):
  python3 user/libcgj/src/_gen_milestone_23100.py --band markers

  # Extension identity only (23001–23050):
  python3 user/libcgj/src/_gen_milestone_23100.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_23100.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_23000.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 23100 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 23100
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 23001
WAVE_HI = 23100  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 23001, 23050
IDENTITY_LO, IDENTITY_HI = 23051, 23075
FILL_LO, FILL_HI = 23076, 23090
MARKERS_LO, MARKERS_HI = 23091, 23100
...
# Stems become gj_<stem>_23100. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (23091, "shell_green", 1, "shell product green extract (wave 23100)"),
    (23092, "libcgj_green", 1, "libcgj product green extract (wave 23100)"),
    (23093, "bar3_ready", 0, "product bar3 readiness gate (wave 23100)"),
    (23094, "product_score", 0, "product readiness score tag (wave 23100)"),
    (23095, "continuum_ready", 1, "continuum readiness gate (wave 23100)"),
    (23096, "smoke_soft", 1, "smoke soft lamp (wave 23100)"),
    (23097, "dyn_soft", 1, "dyn soft lamp (wave 23100)"),
    (23098, "milestone_tag", MILESTONE, "milestone 23100 product tag"),
    (23099, "continuum_wave", MILESTONE, "milestone 23100 continuum wave identity"),
    # 23100 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        23076: "continuum wave fill soft unit + batch id",
        23080: "continuum bridge soft unit + batch id",
        23090: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (23091–23099), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 23093
    tag_batch = MARKERS_LO + 7  # 23098
    wave_batch = MARKERS_LO + 8  # 23099
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_23000 / gj_shell_green_22900 / gj_shell_green_22800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_23000 / gj_libcgj_green_22900 / gj_libcgj_green_22800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_23000 / gj_bar3_ready_22900 / gj_bar3_ready_22800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_23000 / gj_product_score_22900 / gj_product_score_22800\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_23000 / gj_continuum_ready_22900 /\n"
            f" * gj_continuum_ready_22800 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_23000 / gj_smoke_soft_22900 / gj_smoke_soft_22800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_23000 / gj_dyn_soft_22900 / gj_dyn_soft_22800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_23000 / gj_milestone_tag_22900 / gj_milestone_tag_22800,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_23000 / gj_continuum_wave_22900 /\n"
            f" * gj_continuum_wave_22800, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch23100.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 23099
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 23091–23099; 23100 is capstone
