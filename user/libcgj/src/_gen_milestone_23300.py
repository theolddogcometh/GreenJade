...
"""
Soft continuum generator — MILESTONE 23300 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 23201–23300 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=23300   when parent has wired through graph_batch23300.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 23201 23300
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 23201 23300
...
graph batches ending at M. For M = **23300** the exclusive range is
**23201–23300** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 23201–23250  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 23251–23275  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 23276–23290  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 23291–23300  │ decade soft gates (Wave 91 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 23300 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_23300        1          graph_batch23291.c
  gj_libcgj_green_23300       1          graph_batch23292.c
  gj_bar3_ready_23300         0          graph_batch23293.c  (bar3 open)
  gj_product_score_23300      0          graph_batch23294.c  (score open)
  gj_continuum_ready_23300    1          graph_batch23295.c
  gj_smoke_soft_23300         1          graph_batch23296.c
  gj_dyn_soft_23300           1          graph_batch23297.c
  gj_milestone_tag_23300      23300      graph_batch23298.c
  gj_continuum_wave_23300     23300      graph_batch23299.c
  gj_batch_id_23300           23300      graph_batch23300.c
  gj_graph_milestone_23300    23300      graph_batch23300.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=23300`` means parent wired CGJ_SRCS through
    ``graph_batch23300.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_23300`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_23300`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M23300_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_23200`` / ``gj_*_23100`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 91 exclusive CREATE-ONLY lands the **markers** band
  (23291–23300) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_23300.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_23300.py

  # Restrict to marker band only (soft-gate TUs; Wave 91 exclusive):
  python3 user/libcgj/src/_gen_milestone_23300.py --band markers

  # Extension identity only (23201–23250):
  python3 user/libcgj/src/_gen_milestone_23300.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_23300.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_23200.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 23300 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 23300
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 23201
WAVE_HI = 23300  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 23201, 23250
IDENTITY_LO, IDENTITY_HI = 23251, 23275
FILL_LO, FILL_HI = 23276, 23290
MARKERS_LO, MARKERS_HI = 23291, 23300
...
# Stems become gj_<stem>_23300. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (23291, "shell_green", 1, "shell product green extract (wave 23300)"),
    (23292, "libcgj_green", 1, "libcgj product green extract (wave 23300)"),
    (23293, "bar3_ready", 0, "product bar3 readiness gate (wave 23300)"),
    (23294, "product_score", 0, "product readiness score tag (wave 23300)"),
    (23295, "continuum_ready", 1, "continuum readiness gate (wave 23300)"),
    (23296, "smoke_soft", 1, "smoke soft lamp (wave 23300)"),
    (23297, "dyn_soft", 1, "dyn soft lamp (wave 23300)"),
    (23298, "milestone_tag", MILESTONE, "milestone 23300 product tag"),
    (23299, "continuum_wave", MILESTONE, "milestone 23300 continuum wave identity"),
    # 23300 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        23276: "continuum wave fill soft unit + batch id",
        23280: "continuum bridge soft unit + batch id",
        23290: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (23291–23299), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 23293
    tag_batch = MARKERS_LO + 7  # 23298
    wave_batch = MARKERS_LO + 8  # 23299
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_23200 / gj_shell_green_23100 / gj_shell_green_23000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_23200 / gj_libcgj_green_23100 / gj_libcgj_green_23000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_23200 / gj_bar3_ready_23100 / gj_bar3_ready_23000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_23200 / gj_product_score_23100 / gj_product_score_23000\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_23200 / gj_continuum_ready_23100 /\n"
            f" * gj_continuum_ready_23000 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_23200 / gj_smoke_soft_23100 / gj_smoke_soft_23000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_23200 / gj_dyn_soft_23100 / gj_dyn_soft_23000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_23200 / gj_milestone_tag_23100 / gj_milestone_tag_23000,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_23200 / gj_continuum_wave_23100 /\n"
            f" * gj_continuum_wave_23000, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch23300.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 23299
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 23291–23299; 23300 is capstone
