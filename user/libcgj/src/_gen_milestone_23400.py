...
"""
Soft continuum generator — MILESTONE 23400 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 23301–23400 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=23400   when parent has wired through graph_batch23400.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 23301 23400
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 23301 23400
...
graph batches ending at M. For M = **23400** the exclusive range is
**23301–23400** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 23301–23350  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 23351–23375  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 23376–23390  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 23391–23400  │ decade soft gates (Wave 90 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 23400 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_23400        1          graph_batch23391.c
  gj_libcgj_green_23400       1          graph_batch23392.c
  gj_bar3_ready_23400         0          graph_batch23393.c  (bar3 open)
  gj_product_score_23400      0          graph_batch23394.c  (score open)
  gj_continuum_ready_23400    1          graph_batch23395.c
  gj_smoke_soft_23400         1          graph_batch23396.c
  gj_dyn_soft_23400           1          graph_batch23397.c
  gj_milestone_tag_23400      23400      graph_batch23398.c
  gj_continuum_wave_23400     23400      graph_batch23399.c
  gj_batch_id_23400           23400      graph_batch23400.c
  gj_graph_milestone_23400    23400      graph_batch23400.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=23400`` means parent wired CGJ_SRCS through
    ``graph_batch23400.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_23400`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_23400`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M23400_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_23300`` / ``gj_*_23200`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 90 exclusive CREATE-ONLY lands the **markers** band
  (23391–23400) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_23400.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_23400.py

  # Restrict to marker band only (soft-gate TUs; Wave 90 exclusive):
  python3 user/libcgj/src/_gen_milestone_23400.py --band markers

  # Extension identity only (23301–23350):
  python3 user/libcgj/src/_gen_milestone_23400.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_23400.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_23300.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 23400 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 23400
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 23301
WAVE_HI = 23400  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 23301, 23350
IDENTITY_LO, IDENTITY_HI = 23351, 23375
FILL_LO, FILL_HI = 23376, 23390
MARKERS_LO, MARKERS_HI = 23391, 23400
...
# Stems become gj_<stem>_23400. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (23391, "shell_green", 1, "shell product green extract (wave 23400)"),
    (23392, "libcgj_green", 1, "libcgj product green extract (wave 23400)"),
    (23393, "bar3_ready", 0, "product bar3 readiness gate (wave 23400)"),
    (23394, "product_score", 0, "product readiness score tag (wave 23400)"),
    (23395, "continuum_ready", 1, "continuum readiness gate (wave 23400)"),
    (23396, "smoke_soft", 1, "smoke soft lamp (wave 23400)"),
    (23397, "dyn_soft", 1, "dyn soft lamp (wave 23400)"),
    (23398, "milestone_tag", MILESTONE, "milestone 23400 product tag"),
    (23399, "continuum_wave", MILESTONE, "milestone 23400 continuum wave identity"),
    # 23400 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        23376: "continuum wave fill soft unit + batch id",
        23380: "continuum bridge soft unit + batch id",
        23390: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (23391–23399), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 23393
    tag_batch = MARKERS_LO + 7  # 23398
    wave_batch = MARKERS_LO + 8  # 23399
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_23300 / gj_shell_green_23200 / gj_shell_green_23100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_23300 / gj_libcgj_green_23200 / gj_libcgj_green_23100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_23300 / gj_bar3_ready_23200 / gj_bar3_ready_23100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_23300 / gj_product_score_23200 / gj_product_score_23100\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_23300 / gj_continuum_ready_23200 /\n"
            f" * gj_continuum_ready_23100 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_23300 / gj_smoke_soft_23200 / gj_smoke_soft_23100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_23300 / gj_dyn_soft_23200 / gj_dyn_soft_23100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_23300 / gj_milestone_tag_23200 / gj_milestone_tag_23100,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_23300 / gj_continuum_wave_23200 /\n"
            f" * gj_continuum_wave_23100, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch23400.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 23399
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 23391–23399; 23400 is capstone
