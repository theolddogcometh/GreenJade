...
"""
Soft continuum generator — MILESTONE 23500 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 23401–23500 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=23500   when parent has wired through graph_batch23500.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 23401 23500
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 23401 23500
...
graph batches ending at M. For M = **23500** the exclusive range is
**23401–23500** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 23401–23450  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 23451–23475  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 23476–23490  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 23491–23500  │ decade soft gates (Wave 90 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 23500 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_23500        1          graph_batch23491.c
  gj_libcgj_green_23500       1          graph_batch23492.c
  gj_bar3_ready_23500         0          graph_batch23493.c  (bar3 open)
  gj_product_score_23500      0          graph_batch23494.c  (score open)
  gj_continuum_ready_23500    1          graph_batch23495.c
  gj_smoke_soft_23500         1          graph_batch23496.c
  gj_dyn_soft_23500           1          graph_batch23497.c
  gj_milestone_tag_23500      23500      graph_batch23498.c
  gj_continuum_wave_23500     23500      graph_batch23499.c
  gj_batch_id_23500           23500      graph_batch23500.c
  gj_graph_milestone_23500    23500      graph_batch23500.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=23500`` means parent wired CGJ_SRCS through
    ``graph_batch23500.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_23500`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_23500`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M23500_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_23400`` / ``gj_*_23300`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  (23491–23500) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_23500.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_23500.py

  # Restrict to marker band only (soft-gate TUs; Wave 90 exclusive):
  python3 user/libcgj/src/_gen_milestone_23500.py --band markers

  # Extension identity only (23401–23450):
  python3 user/libcgj/src/_gen_milestone_23500.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_23500.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_23400.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 23500 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 23500
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 23401
WAVE_HI = 23500  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 23401, 23450
IDENTITY_LO, IDENTITY_HI = 23451, 23475
FILL_LO, FILL_HI = 23476, 23490
MARKERS_LO, MARKERS_HI = 23491, 23500
...
# Stems become gj_<stem>_23500. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (23491, "shell_green", 1, "shell product green extract (wave 23500)"),
    (23492, "libcgj_green", 1, "libcgj product green extract (wave 23500)"),
    (23493, "bar3_ready", 0, "product bar3 readiness gate (wave 23500)"),
    (23494, "product_score", 0, "product readiness score tag (wave 23500)"),
    (23495, "continuum_ready", 1, "continuum readiness gate (wave 23500)"),
    (23496, "smoke_soft", 1, "smoke soft lamp (wave 23500)"),
    (23497, "dyn_soft", 1, "dyn soft lamp (wave 23500)"),
    (23498, "milestone_tag", MILESTONE, "milestone 23500 product tag"),
    (23499, "continuum_wave", MILESTONE, "milestone 23500 continuum wave identity"),
    # 23500 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        23476: "continuum wave fill soft unit + batch id",
        23480: "continuum bridge soft unit + batch id",
        23490: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (23491–23499), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 23493
    tag_batch = MARKERS_LO + 7  # 23498
    wave_batch = MARKERS_LO + 8  # 23499
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_23400 / gj_shell_green_23300 / gj_shell_green_23200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_23400 / gj_libcgj_green_23300 / gj_libcgj_green_23200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_23400 / gj_bar3_ready_23300 / gj_bar3_ready_23200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_23400 / gj_product_score_23300 / gj_product_score_23200\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_23400 / gj_continuum_ready_23300 /\n"
            f" * gj_continuum_ready_23200 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_23400 / gj_smoke_soft_23300 / gj_smoke_soft_23200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_23400 / gj_dyn_soft_23300 / gj_dyn_soft_23200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_23400 / gj_milestone_tag_23300 / gj_milestone_tag_23200,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_23400 / gj_continuum_wave_23300 /\n"
            f" * gj_continuum_wave_23200, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch23500.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 23499
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 23491–23499; 23500 is capstone
