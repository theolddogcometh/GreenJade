...
"""
Soft continuum generator — MILESTONE 23600 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 23501–23600 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=23600   when parent has wired through graph_batch23600.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 23501 23600
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 23501 23600
...
graph batches ending at M. For M = **23600** the exclusive range is
**23501–23600** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 23501–23550  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 23551–23575  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 23576–23590  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 23591–23600  │ decade soft gates (Wave 94 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 23600 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_23600        1          graph_batch23591.c
  gj_libcgj_green_23600       1          graph_batch23592.c
  gj_bar3_ready_23600         0          graph_batch23593.c  (bar3 open)
  gj_product_score_23600      0          graph_batch23594.c  (score open)
  gj_continuum_ready_23600    1          graph_batch23595.c
  gj_smoke_soft_23600         1          graph_batch23596.c
  gj_dyn_soft_23600           1          graph_batch23597.c
  gj_milestone_tag_23600      23600      graph_batch23598.c
  gj_continuum_wave_23600     23600      graph_batch23599.c
  gj_batch_id_23600           23600      graph_batch23600.c
  gj_graph_milestone_23600    23600      graph_batch23600.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=23600`` means parent wired CGJ_SRCS through
    ``graph_batch23600.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_23600`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_23600`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M23600_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_23500`` / ``gj_*_23400`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 94 exclusive CREATE-ONLY lands the **markers** band
  (23591–23600) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_23600.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_23600.py

  # Restrict to marker band only (soft-gate TUs; Wave 94 exclusive):
  python3 user/libcgj/src/_gen_milestone_23600.py --band markers

  # Extension identity only (23501–23550):
  python3 user/libcgj/src/_gen_milestone_23600.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_23600.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_23500.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 23600 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 23600
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 23501
WAVE_HI = 23600  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 23501, 23550
IDENTITY_LO, IDENTITY_HI = 23551, 23575
FILL_LO, FILL_HI = 23576, 23590
MARKERS_LO, MARKERS_HI = 23591, 23600
...
# Stems become gj_<stem>_23600. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (23591, "shell_green", 1, "shell product green extract (wave 23600)"),
    (23592, "libcgj_green", 1, "libcgj product green extract (wave 23600)"),
    (23593, "bar3_ready", 0, "product bar3 readiness gate (wave 23600)"),
    (23594, "product_score", 0, "product readiness score tag (wave 23600)"),
    (23595, "continuum_ready", 1, "continuum readiness gate (wave 23600)"),
    (23596, "smoke_soft", 1, "smoke soft lamp (wave 23600)"),
    (23597, "dyn_soft", 1, "dyn soft lamp (wave 23600)"),
    (23598, "milestone_tag", MILESTONE, "milestone 23600 product tag"),
    (23599, "continuum_wave", MILESTONE, "milestone 23600 continuum wave identity"),
    # 23600 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        23576: "continuum wave fill soft unit + batch id",
        23580: "continuum bridge soft unit + batch id",
        23590: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (23591–23599), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 23593
    tag_batch = MARKERS_LO + 7  # 23598
    wave_batch = MARKERS_LO + 8  # 23599
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_23500 / gj_shell_green_23400 / gj_shell_green_23300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_23500 / gj_libcgj_green_23400 / gj_libcgj_green_23300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_23500 / gj_bar3_ready_23400 / gj_bar3_ready_23300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_23500 / gj_product_score_23400 / gj_product_score_23300\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_23500 / gj_continuum_ready_23400 /\n"
            f" * gj_continuum_ready_23300 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_23500 / gj_smoke_soft_23400 / gj_smoke_soft_23300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_23500 / gj_dyn_soft_23400 / gj_dyn_soft_23300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_23500 / gj_milestone_tag_23400 / gj_milestone_tag_23300,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_23500 / gj_continuum_wave_23400 /\n"
            f" * gj_continuum_wave_23300, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch23600.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 23599
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 23591–23599; 23600 is capstone
