...
"""
Soft continuum generator — MILESTONE 23700 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 23601–23700 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=23700   when parent has wired through graph_batch23700.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 23601 23700
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 23601 23700
...
graph batches ending at M. For M = **23700** the exclusive range is
**23601–23700** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 23601–23650  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 23651–23675  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 23676–23690  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 23691–23700  │ decade soft gates (Wave 95 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 23700 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_23700        1          graph_batch23691.c
  gj_libcgj_green_23700       1          graph_batch23692.c
  gj_bar3_ready_23700         0          graph_batch23693.c  (bar3 open)
  gj_product_score_23700      0          graph_batch23694.c  (score open)
  gj_continuum_ready_23700    1          graph_batch23695.c
  gj_smoke_soft_23700         1          graph_batch23696.c
  gj_dyn_soft_23700           1          graph_batch23697.c
  gj_milestone_tag_23700      23700      graph_batch23698.c
  gj_continuum_wave_23700     23700      graph_batch23699.c
  gj_batch_id_23700           23700      graph_batch23700.c
  gj_graph_milestone_23700    23700      graph_batch23700.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=23700`` means parent wired CGJ_SRCS through
    ``graph_batch23700.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_23700`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_23700`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M23700_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_23600`` / ``gj_*_23500`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 95 exclusive CREATE-ONLY lands the **markers** band
  (23691–23700) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_23700.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_23700.py

  # Restrict to marker band only (soft-gate TUs; Wave 95 exclusive):
  python3 user/libcgj/src/_gen_milestone_23700.py --band markers

  # Extension identity only (23601–23650):
  python3 user/libcgj/src/_gen_milestone_23700.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_23700.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_23600.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 23700 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 23700
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 23601
WAVE_HI = 23700  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 23601, 23650
IDENTITY_LO, IDENTITY_HI = 23651, 23675
FILL_LO, FILL_HI = 23676, 23690
MARKERS_LO, MARKERS_HI = 23691, 23700
...
# Stems become gj_<stem>_23700. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (23691, "shell_green", 1, "shell product green extract (wave 23700)"),
    (23692, "libcgj_green", 1, "libcgj product green extract (wave 23700)"),
    (23693, "bar3_ready", 0, "product bar3 readiness gate (wave 23700)"),
    (23694, "product_score", 0, "product readiness score tag (wave 23700)"),
    (23695, "continuum_ready", 1, "continuum readiness gate (wave 23700)"),
    (23696, "smoke_soft", 1, "smoke soft lamp (wave 23700)"),
    (23697, "dyn_soft", 1, "dyn soft lamp (wave 23700)"),
    (23698, "milestone_tag", MILESTONE, "milestone 23700 product tag"),
    (23699, "continuum_wave", MILESTONE, "milestone 23700 continuum wave identity"),
    # 23700 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        23676: "continuum wave fill soft unit + batch id",
        23680: "continuum bridge soft unit + batch id",
        23690: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (23691–23699), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 23693
    tag_batch = MARKERS_LO + 7  # 23698
    wave_batch = MARKERS_LO + 8  # 23699
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_23600 / gj_shell_green_23500 / gj_shell_green_23400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_23600 / gj_libcgj_green_23500 / gj_libcgj_green_23400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_23600 / gj_bar3_ready_23500 / gj_bar3_ready_23400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_23600 / gj_product_score_23500 / gj_product_score_23400\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_23600 / gj_continuum_ready_23500 /\n"
            f" * gj_continuum_ready_23400 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_23600 / gj_smoke_soft_23500 / gj_smoke_soft_23400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_23600 / gj_dyn_soft_23500 / gj_dyn_soft_23400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_23600 / gj_milestone_tag_23500 / gj_milestone_tag_23400,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_23600 / gj_continuum_wave_23500 /\n"
            f" * gj_continuum_wave_23400, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch23700.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 23699
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 23691–23699; 23700 is capstone
