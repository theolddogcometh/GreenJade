...
"""
Soft continuum generator — MILESTONE 23900 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 23801–23900 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=23900   when parent has wired through graph_batch23900.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 23801 23900
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 23801 23900
...
graph batches ending at M. For M = **23900** the exclusive range is
**23801–23900** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 23801–23850  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 23851–23875  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 23876–23890  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 23891–23900  │ decade soft gates (Wave 97 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 23900 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_23900        1          graph_batch23891.c
  gj_libcgj_green_23900       1          graph_batch23892.c
  gj_bar3_ready_23900         0          graph_batch23893.c  (bar3 open)
  gj_product_score_23900      0          graph_batch23894.c  (score open)
  gj_continuum_ready_23900    1          graph_batch23895.c
  gj_smoke_soft_23900         1          graph_batch23896.c
  gj_dyn_soft_23900           1          graph_batch23897.c
  gj_milestone_tag_23900      23900      graph_batch23898.c
  gj_continuum_wave_23900     23900      graph_batch23899.c
  gj_batch_id_23900           23900      graph_batch23900.c
  gj_graph_milestone_23900    23900      graph_batch23900.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=23900`` means parent wired CGJ_SRCS through
    ``graph_batch23900.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_23900`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_23900`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M23900_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_23800`` / ``gj_*_23700`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 97 exclusive CREATE-ONLY lands the **markers** band
  (23891–23900) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_23900.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_23900.py

  # Restrict to marker band only (soft-gate TUs; Wave 97 exclusive):
  python3 user/libcgj/src/_gen_milestone_23900.py --band markers

  # Extension identity only (23801–23850):
  python3 user/libcgj/src/_gen_milestone_23900.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_23900.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_23800.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 23900 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 23900
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 23801
WAVE_HI = 23900  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 23801, 23850
IDENTITY_LO, IDENTITY_HI = 23851, 23875
FILL_LO, FILL_HI = 23876, 23890
MARKERS_LO, MARKERS_HI = 23891, 23900
...
# Stems become gj_<stem>_23900. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (23891, "shell_green", 1, "shell product green extract (wave 23900)"),
    (23892, "libcgj_green", 1, "libcgj product green extract (wave 23900)"),
    (23893, "bar3_ready", 0, "product bar3 readiness gate (wave 23900)"),
    (23894, "product_score", 0, "product readiness score tag (wave 23900)"),
    (23895, "continuum_ready", 1, "continuum readiness gate (wave 23900)"),
    (23896, "smoke_soft", 1, "smoke soft lamp (wave 23900)"),
    (23897, "dyn_soft", 1, "dyn soft lamp (wave 23900)"),
    (23898, "milestone_tag", MILESTONE, "milestone 23900 product tag"),
    (23899, "continuum_wave", MILESTONE, "milestone 23900 continuum wave identity"),
    # 23900 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        23876: "continuum wave fill soft unit + batch id",
        23880: "continuum bridge soft unit + batch id",
        23890: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (23891–23899), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 23893
    tag_batch = MARKERS_LO + 7  # 23898
    wave_batch = MARKERS_LO + 8  # 23899
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_23800 / gj_shell_green_23700 / gj_shell_green_23600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_23800 / gj_libcgj_green_23700 / gj_libcgj_green_23600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_23800 / gj_bar3_ready_23700 / gj_bar3_ready_23600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_23800 / gj_product_score_23700 / gj_product_score_23600\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_23800 / gj_continuum_ready_23700 /\n"
            f" * gj_continuum_ready_23600 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_23800 / gj_smoke_soft_23700 / gj_smoke_soft_23600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_23800 / gj_dyn_soft_23700 / gj_dyn_soft_23600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_23800 / gj_milestone_tag_23700 / gj_milestone_tag_23600,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_23800 / gj_continuum_wave_23700 /\n"
            f" * gj_continuum_wave_23600, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch23900.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 23899
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 23891–23899; 23900 is capstone
