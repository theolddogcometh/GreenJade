...
"""
Soft continuum generator — MILESTONE 24000 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 23901–24000 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=24000   when parent has wired through graph_batch24000.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 23901 24000
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 23901 24000
...
graph batches ending at M. For M = **24000** the exclusive range is
**23901–24000** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 23901–23950  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 23951–23975  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 23976–23990  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 23991–24000  │ decade soft gates (Wave 98 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 24000 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_24000        1          graph_batch23991.c
  gj_libcgj_green_24000       1          graph_batch23992.c
  gj_bar3_ready_24000         0          graph_batch23993.c  (bar3 open)
  gj_product_score_24000      0          graph_batch23994.c  (score open)
  gj_continuum_ready_24000    1          graph_batch23995.c
  gj_smoke_soft_24000         1          graph_batch23996.c
  gj_dyn_soft_24000           1          graph_batch23997.c
  gj_milestone_tag_24000      24000      graph_batch23998.c
  gj_continuum_wave_24000     24000      graph_batch23999.c
  gj_batch_id_24000           24000      graph_batch24000.c
  gj_graph_milestone_24000    24000      graph_batch24000.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=24000`` means parent wired CGJ_SRCS through
    ``graph_batch24000.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_24000`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_24000`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M24000_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_23900`` / ``gj_*_23800`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 98 exclusive CREATE-ONLY lands the **markers** band
  (23991–24000) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_24000.py

  # Restrict to marker band only (soft-gate TUs; Wave 98 exclusive):
  python3 user/libcgj/src/_gen_milestone_24000.py --band markers

  # Extension identity only (23901–23950):
  python3 user/libcgj/src/_gen_milestone_24000.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_24000.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_23900.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 24000 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 24000
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 23901
WAVE_HI = 24000  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 23901, 23950
IDENTITY_LO, IDENTITY_HI = 23951, 23975
FILL_LO, FILL_HI = 23976, 23990
MARKERS_LO, MARKERS_HI = 23991, 24000
...
# Stems become gj_<stem>_24000. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (23991, "shell_green", 1, "shell product green extract (wave 24000)"),
    (23992, "libcgj_green", 1, "libcgj product green extract (wave 24000)"),
    (23993, "bar3_ready", 0, "product bar3 readiness gate (wave 24000)"),
    (23994, "product_score", 0, "product readiness score tag (wave 24000)"),
    (23995, "continuum_ready", 1, "continuum readiness gate (wave 24000)"),
    (23996, "smoke_soft", 1, "smoke soft lamp (wave 24000)"),
    (23997, "dyn_soft", 1, "dyn soft lamp (wave 24000)"),
    (23998, "milestone_tag", MILESTONE, "milestone 24000 product tag"),
    (23999, "continuum_wave", MILESTONE, "milestone 24000 continuum wave identity"),
    # 24000 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        23976: "continuum wave fill soft unit + batch id",
        23980: "continuum bridge soft unit + batch id",
        23990: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (23991–23999), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 23993
    tag_batch = MARKERS_LO + 7  # 23998
    wave_batch = MARKERS_LO + 8  # 23999
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_23900 / gj_shell_green_23800 / gj_shell_green_23700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_23900 / gj_libcgj_green_23800 / gj_libcgj_green_23700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_23900 / gj_bar3_ready_23800 / gj_bar3_ready_23700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_23900 / gj_product_score_23800 / gj_product_score_23700\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_23900 / gj_continuum_ready_23800 /\n"
            f" * gj_continuum_ready_23700 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_23900 / gj_smoke_soft_23800 / gj_smoke_soft_23700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_23900 / gj_dyn_soft_23800 / gj_dyn_soft_23700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_23900 / gj_milestone_tag_23800 / gj_milestone_tag_23700,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_23900 / gj_continuum_wave_23800 /\n"
            f" * gj_continuum_wave_23700, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch24000.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 23999
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 23991–23999; 24000 is capstone
