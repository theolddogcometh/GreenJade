...
"""
Soft continuum generator — MILESTONE 24100 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 24001–24100 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=24100   when parent has wired through graph_batch24100.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 24001 24100
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 24001 24100
...
graph batches ending at M. For M = **24100** the exclusive range is
**24001–24100** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 24001–24050  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 24051–24075  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 24076–24090  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 24091–24100  │ decade soft gates (Wave 99 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 24100 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_24100        1          graph_batch24091.c
  gj_libcgj_green_24100       1          graph_batch24092.c
  gj_bar3_ready_24100         0          graph_batch24093.c  (bar3 open)
  gj_product_score_24100      0          graph_batch24094.c  (score open)
  gj_continuum_ready_24100    1          graph_batch24095.c
  gj_smoke_soft_24100         1          graph_batch24096.c
  gj_dyn_soft_24100           1          graph_batch24097.c
  gj_milestone_tag_24100      24100      graph_batch24098.c
  gj_continuum_wave_24100     24100      graph_batch24099.c
  gj_batch_id_24100           24100      graph_batch24100.c
  gj_graph_milestone_24100    24100      graph_batch24100.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=24100`` means parent wired CGJ_SRCS through
    ``graph_batch24100.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_24100`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_24100`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M24100_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_24000`` / ``gj_*_23900`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 99 exclusive CREATE-ONLY lands the **markers** band
  (24091–24100) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_24100.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_24100.py

  # Restrict to marker band only (soft-gate TUs; Wave 99 exclusive):
  python3 user/libcgj/src/_gen_milestone_24100.py --band markers

  # Extension identity only (24001–24050):
  python3 user/libcgj/src/_gen_milestone_24100.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_24100.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_24000.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 24100 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 24100
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 24001
WAVE_HI = 24100  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 24001, 24050
IDENTITY_LO, IDENTITY_HI = 24051, 24075
FILL_LO, FILL_HI = 24076, 24090
MARKERS_LO, MARKERS_HI = 24091, 24100
...
# Stems become gj_<stem>_24100. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (24091, "shell_green", 1, "shell product green extract (wave 24100)"),
    (24092, "libcgj_green", 1, "libcgj product green extract (wave 24100)"),
    (24093, "bar3_ready", 0, "product bar3 readiness gate (wave 24100)"),
    (24094, "product_score", 0, "product readiness score tag (wave 24100)"),
    (24095, "continuum_ready", 1, "continuum readiness gate (wave 24100)"),
    (24096, "smoke_soft", 1, "smoke soft lamp (wave 24100)"),
    (24097, "dyn_soft", 1, "dyn soft lamp (wave 24100)"),
    (24098, "milestone_tag", MILESTONE, "milestone 24100 product tag"),
    (24099, "continuum_wave", MILESTONE, "milestone 24100 continuum wave identity"),
    # 24100 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        24076: "continuum wave fill soft unit + batch id",
        24080: "continuum bridge soft unit + batch id",
        24090: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (24091–24099), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 24093
    tag_batch = MARKERS_LO + 7  # 24098
    wave_batch = MARKERS_LO + 8  # 24099
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_24000 / gj_shell_green_23900 / gj_shell_green_23800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_24000 / gj_libcgj_green_23900 / gj_libcgj_green_23800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_24000 / gj_bar3_ready_23900 / gj_bar3_ready_23800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_24000 / gj_product_score_23900 / gj_product_score_23800\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_24000 / gj_continuum_ready_23900 /\n"
            f" * gj_continuum_ready_23800 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_24000 / gj_smoke_soft_23900 / gj_smoke_soft_23800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_24000 / gj_dyn_soft_23900 / gj_dyn_soft_23800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_24000 / gj_milestone_tag_23900 / gj_milestone_tag_23800,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_24000 / gj_continuum_wave_23900 /\n"
            f" * gj_continuum_wave_23800, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch24100.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 24099
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 24091–24099; 24100 is capstone
