...
"""
Soft continuum generator — MILESTONE 24300 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 24201–24300 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=24300   when parent has wired through graph_batch24300.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 24201 24300
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 24201 24300
...
graph batches ending at M. For M = **24300** the exclusive range is
**24201–24300** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 24201–24250  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 24251–24275  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 24276–24290  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 24291–24300  │ decade soft gates (Wave 101 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 24300 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_24300        1          graph_batch24291.c
  gj_libcgj_green_24300       1          graph_batch24292.c
  gj_bar3_ready_24300         0          graph_batch24293.c  (bar3 open)
  gj_product_score_24300      0          graph_batch24294.c  (score open)
  gj_continuum_ready_24300    1          graph_batch24295.c
  gj_smoke_soft_24300         1          graph_batch24296.c
  gj_dyn_soft_24300           1          graph_batch24297.c
  gj_milestone_tag_24300      24300      graph_batch24298.c
  gj_continuum_wave_24300     24300      graph_batch24299.c
  gj_batch_id_24300           24300      graph_batch24300.c
  gj_graph_milestone_24300    24300      graph_batch24300.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=24300`` means parent wired CGJ_SRCS through
    ``graph_batch24300.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_24300`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_24300`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M24300_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_24200`` / ``gj_*_24100`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 101 exclusive CREATE-ONLY lands the **markers** band
  (24291–24300) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_24300.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_24300.py

  # Restrict to marker band only (soft-gate TUs; Wave 101 exclusive):
  python3 user/libcgj/src/_gen_milestone_24300.py --band markers

  # Extension identity only (24201–24250):
  python3 user/libcgj/src/_gen_milestone_24300.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_24300.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_24200.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 24300 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 24300
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 24201
WAVE_HI = 24300  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 24201, 24250
IDENTITY_LO, IDENTITY_HI = 24251, 24275
FILL_LO, FILL_HI = 24276, 24290
MARKERS_LO, MARKERS_HI = 24291, 24300
...
# Stems become gj_<stem>_24300. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (24291, "shell_green", 1, "shell product green extract (wave 24300)"),
    (24292, "libcgj_green", 1, "libcgj product green extract (wave 24300)"),
    (24293, "bar3_ready", 0, "product bar3 readiness gate (wave 24300)"),
    (24294, "product_score", 0, "product readiness score tag (wave 24300)"),
    (24295, "continuum_ready", 1, "continuum readiness gate (wave 24300)"),
    (24296, "smoke_soft", 1, "smoke soft lamp (wave 24300)"),
    (24297, "dyn_soft", 1, "dyn soft lamp (wave 24300)"),
    (24298, "milestone_tag", MILESTONE, "milestone 24300 product tag"),
    (24299, "continuum_wave", MILESTONE, "milestone 24300 continuum wave identity"),
    # 24300 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        24276: "continuum wave fill soft unit + batch id",
        24280: "continuum bridge soft unit + batch id",
        24290: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (24291–24299), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 24293
    tag_batch = MARKERS_LO + 7  # 24298
    wave_batch = MARKERS_LO + 8  # 24299
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_24200 / gj_shell_green_24100 / gj_shell_green_24000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_24200 / gj_libcgj_green_24100 / gj_libcgj_green_24000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_24200 / gj_bar3_ready_24100 / gj_bar3_ready_24000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_24200 / gj_product_score_24100 / gj_product_score_24000\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_24200 / gj_continuum_ready_24100 /\n"
            f" * gj_continuum_ready_24000 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_24200 / gj_smoke_soft_24100 / gj_smoke_soft_24000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_24200 / gj_dyn_soft_24100 / gj_dyn_soft_24000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_24200 / gj_milestone_tag_24100 / gj_milestone_tag_24000,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_24200 / gj_continuum_wave_24100 /\n"
            f" * gj_continuum_wave_24000, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch24300.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 24299
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 24291–24299; 24300 is capstone
