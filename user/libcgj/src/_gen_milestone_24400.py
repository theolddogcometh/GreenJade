...
"""
Soft continuum generator — MILESTONE 24400 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 24301–24400 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=24400   when parent has wired through graph_batch24400.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 24301 24400
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 24301 24400
...
graph batches ending at M. For M = **24400** the exclusive range is
**24301–24400** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 24301–24350  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 24351–24375  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 24376–24390  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 24391–24400  │ decade soft gates (Wave 102 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 24400 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_24400        1          graph_batch24391.c
  gj_libcgj_green_24400       1          graph_batch24392.c
  gj_bar3_ready_24400         0          graph_batch24393.c  (bar3 open)
  gj_product_score_24400      0          graph_batch24394.c  (score open)
  gj_continuum_ready_24400    1          graph_batch24395.c
  gj_smoke_soft_24400         1          graph_batch24396.c
  gj_dyn_soft_24400           1          graph_batch24397.c
  gj_milestone_tag_24400      24400      graph_batch24398.c
  gj_continuum_wave_24400     24400      graph_batch24399.c
  gj_batch_id_24400           24400      graph_batch24400.c
  gj_graph_milestone_24400    24400      graph_batch24400.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=24400`` means parent wired CGJ_SRCS through
    ``graph_batch24400.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_24400`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_24400`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M24400_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_24300`` / ``gj_*_24200`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 102 exclusive CREATE-ONLY lands the **markers** band
  (24391–24400) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_24400.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_24400.py

  # Restrict to marker band only (soft-gate TUs; Wave 102 exclusive):
  python3 user/libcgj/src/_gen_milestone_24400.py --band markers

  # Extension identity only (24301–24350):
  python3 user/libcgj/src/_gen_milestone_24400.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_24400.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_24300.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 24400 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 24400
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 24301
WAVE_HI = 24400  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 24301, 24350
IDENTITY_LO, IDENTITY_HI = 24351, 24375
FILL_LO, FILL_HI = 24376, 24390
MARKERS_LO, MARKERS_HI = 24391, 24400
...
# Stems become gj_<stem>_24400. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (24391, "shell_green", 1, "shell product green extract (wave 24400)"),
    (24392, "libcgj_green", 1, "libcgj product green extract (wave 24400)"),
    (24393, "bar3_ready", 0, "product bar3 readiness gate (wave 24400)"),
    (24394, "product_score", 0, "product readiness score tag (wave 24400)"),
    (24395, "continuum_ready", 1, "continuum readiness gate (wave 24400)"),
    (24396, "smoke_soft", 1, "smoke soft lamp (wave 24400)"),
    (24397, "dyn_soft", 1, "dyn soft lamp (wave 24400)"),
    (24398, "milestone_tag", MILESTONE, "milestone 24400 product tag"),
    (24399, "continuum_wave", MILESTONE, "milestone 24400 continuum wave identity"),
    # 24400 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        24376: "continuum wave fill soft unit + batch id",
        24380: "continuum bridge soft unit + batch id",
        24390: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (24391–24399), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 24393
    tag_batch = MARKERS_LO + 7  # 24398
    wave_batch = MARKERS_LO + 8  # 24399
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_24300 / gj_shell_green_24200 / gj_shell_green_24100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_24300 / gj_libcgj_green_24200 / gj_libcgj_green_24100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_24300 / gj_bar3_ready_24200 / gj_bar3_ready_24100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_24300 / gj_product_score_24200 / gj_product_score_24100\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_24300 / gj_continuum_ready_24200 /\n"
            f" * gj_continuum_ready_24100 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_24300 / gj_smoke_soft_24200 / gj_smoke_soft_24100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_24300 / gj_dyn_soft_24200 / gj_dyn_soft_24100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_24300 / gj_milestone_tag_24200 / gj_milestone_tag_24100,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_24300 / gj_continuum_wave_24200 /\n"
            f" * gj_continuum_wave_24100, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch24400.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 24399
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 24391–24399; 24400 is capstone
