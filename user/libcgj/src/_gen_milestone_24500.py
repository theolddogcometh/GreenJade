...
"""
Soft continuum generator — MILESTONE 24500 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 24401–24500 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=24500   when parent has wired through __GB24500__

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 24401 24500
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 24401 24500
...
graph batches ending at M. For M = **24500** the exclusive range is
**24401–24500** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 24401–24450  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 24451–24475  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 24476–24490  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 24491–24500  │ decade soft gates (Wave 103 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 24500 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_24500        1          graph_batch24491.c
  gj_libcgj_green_24500       1          graph_batch24492.c
  gj_bar3_ready_24500         0          graph_batch24493.c  (bar3 open)
  gj_product_score_24500      0          graph_batch24494.c  (score open)
  gj_continuum_ready_24500    1          graph_batch24495.c
  gj_smoke_soft_24500         1          graph_batch24496.c
  gj_dyn_soft_24500           1          graph_batch24497.c
  gj_milestone_tag_24500      24500      graph_batch24498.c
  gj_continuum_wave_24500     24500      graph_batch24499.c
  gj_batch_id_24500           24500      __GB24500__
  gj_graph_milestone_24500    24500      __GB24500__
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=24500`` means parent wired CGJ_SRCS through
    ``__GB24500__``. It is **soft graph growth only**.
  * ``gj_bar3_ready_24500`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_24500`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M24500_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_24400`` / ``gj_*_24300`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 103 exclusive CREATE-ONLY lands the **markers** band
  (24491–24500) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_24500.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_24500.py

  # Restrict to marker band only (soft-gate TUs; Wave 103 exclusive):
  python3 user/libcgj/src/_gen_milestone_24500.py --band markers

  # Extension identity only (24401–24450):
  python3 user/libcgj/src/_gen_milestone_24500.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_24500.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_24400.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 24500 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 24500
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 24401
WAVE_HI = 24500  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 24401, 24450
IDENTITY_LO, IDENTITY_HI = 24451, 24475
FILL_LO, FILL_HI = 24476, 24490
MARKERS_LO, MARKERS_HI = 24491, 24500
...
# Stems become gj_<stem>_24500. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (24491, "shell_green", 1, "shell product green extract (wave 24500)"),
    (24492, "libcgj_green", 1, "libcgj product green extract (wave 24500)"),
    (24493, "bar3_ready", 0, "product bar3 readiness gate (wave 24500)"),
    (24494, "product_score", 0, "product readiness score tag (wave 24500)"),
    (24495, "continuum_ready", 1, "continuum readiness gate (wave 24500)"),
    (24496, "smoke_soft", 1, "smoke soft lamp (wave 24500)"),
    (24497, "dyn_soft", 1, "dyn soft lamp (wave 24500)"),
    (24498, "milestone_tag", MILESTONE, "milestone 24500 product tag"),
    (24499, "continuum_wave", MILESTONE, "milestone 24500 continuum wave identity"),
    # 24500 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        24476: "continuum wave fill soft unit + batch id",
        24480: "continuum bridge soft unit + batch id",
        24490: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (24491–24499), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 24493
    tag_batch = MARKERS_LO + 7  # 24498
    wave_batch = MARKERS_LO + 8  # 24499
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_24400 / gj_shell_green_24300 / gj_shell_green_24200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_24400 / gj_libcgj_green_24300 / gj_libcgj_green_24200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_24400 / gj_bar3_ready_24300 / gj_bar3_ready_24200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_24400 / gj_product_score_24300 / gj_product_score_24200\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_24400 / gj_continuum_ready_24300 /\n"
            f" * gj_continuum_ready_24200 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_24400 / gj_smoke_soft_24300 / gj_smoke_soft_24200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_24400 / gj_dyn_soft_24300 / gj_dyn_soft_24200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_24400 / gj_milestone_tag_24300 / gj_milestone_tag_24200,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_24400 / gj_continuum_wave_24300 /\n"
            f" * gj_continuum_wave_24200, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """__GB24500__ — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 24499
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 24491–24499; 24500 is capstone
