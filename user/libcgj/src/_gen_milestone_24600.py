...
"""
Soft continuum generator — MILESTONE 24600 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 24501–24600 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=24600   when parent has wired through graph_batch24600.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 24501 24600
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 24501 24600
...
graph batches ending at M. For M = **24600** the exclusive range is
**24501–24600** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 24501–24550  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 24551–24575  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 24576–24590  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 24591–24600  │ decade soft gates (Wave 104 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 24600 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_24600        1          graph_batch24591.c
  gj_libcgj_green_24600       1          graph_batch24592.c
  gj_bar3_ready_24600         0          graph_batch24593.c  (bar3 open)
  gj_product_score_24600      0          graph_batch24594.c  (score open)
  gj_continuum_ready_24600    1          graph_batch24595.c
  gj_smoke_soft_24600         1          graph_batch24596.c
  gj_dyn_soft_24600           1          graph_batch24597.c
  gj_milestone_tag_24600      24600      graph_batch24598.c
  gj_continuum_wave_24600     24600      graph_batch24599.c
  gj_batch_id_24600           24600      graph_batch24600.c
  gj_graph_milestone_24600    24600      graph_batch24600.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=24600`` means parent wired CGJ_SRCS through
    ``graph_batch24600.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_24600`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_24600`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M24600_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_24500`` / ``gj_*_24400`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 104 exclusive CREATE-ONLY lands the **markers** band
  (24591–24600) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_24600.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_24600.py

  # Restrict to marker band only (soft-gate TUs; Wave 104 exclusive):
  python3 user/libcgj/src/_gen_milestone_24600.py --band markers

  # Extension identity only (24501–24550):
  python3 user/libcgj/src/_gen_milestone_24600.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_24600.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_24500.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 24600 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 24600
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 24501
WAVE_HI = 24600  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 24501, 24550
IDENTITY_LO, IDENTITY_HI = 24551, 24575
FILL_LO, FILL_HI = 24576, 24590
MARKERS_LO, MARKERS_HI = 24591, 24600
...
# Stems become gj_<stem>_24600. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (24591, "shell_green", 1, "shell product green extract (wave 24600)"),
    (24592, "libcgj_green", 1, "libcgj product green extract (wave 24600)"),
    (24593, "bar3_ready", 0, "product bar3 readiness gate (wave 24600)"),
    (24594, "product_score", 0, "product readiness score tag (wave 24600)"),
    (24595, "continuum_ready", 1, "continuum readiness gate (wave 24600)"),
    (24596, "smoke_soft", 1, "smoke soft lamp (wave 24600)"),
    (24597, "dyn_soft", 1, "dyn soft lamp (wave 24600)"),
    (24598, "milestone_tag", MILESTONE, "milestone 24600 product tag"),
    (24599, "continuum_wave", MILESTONE, "milestone 24600 continuum wave identity"),
    # 24600 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        24576: "continuum wave fill soft unit + batch id",
        24580: "continuum bridge soft unit + batch id",
        24590: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (24591–24599), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 24593
    tag_batch = MARKERS_LO + 7  # 24598
    wave_batch = MARKERS_LO + 8  # 24599
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_24500 / gj_shell_green_24400 / gj_shell_green_24300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_24500 / gj_libcgj_green_24400 / gj_libcgj_green_24300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_24500 / gj_bar3_ready_24400 / gj_bar3_ready_24300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_24500 / gj_product_score_24400 / gj_product_score_24300\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_24500 / gj_continuum_ready_24400 /\n"
            f" * gj_continuum_ready_24300 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_24500 / gj_smoke_soft_24400 / gj_smoke_soft_24300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_24500 / gj_dyn_soft_24400 / gj_dyn_soft_24300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_24500 / gj_milestone_tag_24400 / gj_milestone_tag_24300,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_24500 / gj_continuum_wave_24400 /\n"
            f" * gj_continuum_wave_24300, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch24600.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 24599
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 24591–24599; 24600 is capstone
