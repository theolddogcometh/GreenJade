...
"""
Soft continuum generator — MILESTONE 24700 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 24601–24700 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=24700   when parent has wired through graph_batch24700.c
...
graph batches ending at M. For M = **24700** the exclusive range is
**24601–24700** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 24601–24650  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 24651–24675  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 24676–24690  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 24691–24700  │ decade soft gates (Wave 105 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 24700 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_24700        1          graph_batch24691.c
  gj_libcgj_green_24700       1          graph_batch24692.c
  gj_bar3_ready_24700         0          graph_batch24693.c  (bar3 open)
  gj_product_score_24700      0          graph_batch24694.c  (score open)
  gj_continuum_ready_24700    1          graph_batch24695.c
  gj_smoke_soft_24700         1          graph_batch24696.c
  gj_dyn_soft_24700           1          graph_batch24697.c
  gj_milestone_tag_24700      24700      graph_batch24698.c
  gj_continuum_wave_24700     24700      graph_batch24699.c
  gj_batch_id_24700           24700      graph_batch24700.c
  gj_graph_milestone_24700    24700      graph_batch24700.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=24700`` means parent wired CGJ_SRCS through
    ``graph_batch24700.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_24700`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_24700`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M24700_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_24600`` / ``gj_*_24500`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 105 exclusive CREATE-ONLY lands the **markers** band
  (24691–24700) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_24700.py

  # Restrict to marker band only (soft-gate TUs; Wave 105 exclusive):
  python3 user/libcgj/src/_gen_milestone_24700.py --band markers

  # Extension identity only (24601–24650):
  python3 user/libcgj/src/_gen_milestone_24700.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_24700.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_24600.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 24700 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 24700
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 24601
WAVE_HI = 24700  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 24601, 24650
IDENTITY_LO, IDENTITY_HI = 24651, 24675
FILL_LO, FILL_HI = 24676, 24690
MARKERS_LO, MARKERS_HI = 24691, 24700
...
# Stems become gj_<stem>_24700. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (24691, "shell_green", 1, "shell product green extract (wave 24700)"),
    (24692, "libcgj_green", 1, "libcgj product green extract (wave 24700)"),
    (24693, "bar3_ready", 0, "product bar3 readiness gate (wave 24700)"),
    (24694, "product_score", 0, "product readiness score tag (wave 24700)"),
    (24695, "continuum_ready", 1, "continuum readiness gate (wave 24700)"),
    (24696, "smoke_soft", 1, "smoke soft lamp (wave 24700)"),
    (24697, "dyn_soft", 1, "dyn soft lamp (wave 24700)"),
    (24698, "milestone_tag", MILESTONE, "milestone 24700 product tag"),
    (24699, "continuum_wave", MILESTONE, "milestone 24700 continuum wave identity"),
    # 24700 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        24676: "continuum wave fill soft unit + batch id",
        24680: "continuum bridge soft unit + batch id",
        24690: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (24691–24699), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 24693
    tag_batch = MARKERS_LO + 7  # 24698
    wave_batch = MARKERS_LO + 8  # 24699
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_24600 / gj_shell_green_24500 / gj_shell_green_24400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_24600 / gj_libcgj_green_24500 / gj_libcgj_green_24400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_24600 / gj_bar3_ready_24500 / gj_bar3_ready_24400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_24600 / gj_product_score_24500 / gj_product_score_24400\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_24600 / gj_continuum_ready_24500 /\n"
            f" * gj_continuum_ready_24400 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_24600 / gj_smoke_soft_24500 / gj_smoke_soft_24400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_24600 / gj_dyn_soft_24500 / gj_dyn_soft_24400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_24600 / gj_milestone_tag_24500 / gj_milestone_tag_24400,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_24600 / gj_continuum_wave_24500 /\n"
            f" * gj_continuum_wave_24400, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch24700.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 24699
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 24691–24699; 24700 is capstone
