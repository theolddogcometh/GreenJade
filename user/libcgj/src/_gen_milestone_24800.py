...
"""
Soft continuum generator — MILESTONE 24800 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 24701–24800 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=24800   when parent has wired through graph_batch24800.c
...
graph batches ending at M. For M = **24800** the exclusive range is
**24701–24800** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 24701–24750  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 24751–24775  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 24776–24790  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 24791–24800  │ decade soft gates (Wave 106 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 24800 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_24800        1          graph_batch24791.c
  gj_libcgj_green_24800       1          graph_batch24792.c
  gj_bar3_ready_24800         0          graph_batch24793.c  (bar3 open)
  gj_product_score_24800      0          graph_batch24794.c  (score open)
  gj_continuum_ready_24800    1          graph_batch24795.c
  gj_smoke_soft_24800         1          graph_batch24796.c
  gj_dyn_soft_24800           1          graph_batch24797.c
  gj_milestone_tag_24800      24800      graph_batch24798.c
  gj_continuum_wave_24800     24800      graph_batch24799.c
  gj_batch_id_24800           24800      graph_batch24800.c
  gj_graph_milestone_24800    24800      graph_batch24800.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=24800`` means parent wired CGJ_SRCS through
    ``graph_batch24800.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_24800`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_24800`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M24800_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_24700`` / ``gj_*_24600`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 106 exclusive CREATE-ONLY lands the **markers** band
  (24791–24800) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_24800.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_24800.py

  # Restrict to marker band only (soft-gate TUs; Wave 106 exclusive):
  python3 user/libcgj/src/_gen_milestone_24800.py --band markers

  # Extension identity only (24701–24750):
  python3 user/libcgj/src/_gen_milestone_24800.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_24800.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_24700.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 24800 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 24800
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 24701
WAVE_HI = 24800  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 24701, 24750
IDENTITY_LO, IDENTITY_HI = 24751, 24775
FILL_LO, FILL_HI = 24776, 24790
MARKERS_LO, MARKERS_HI = 24791, 24800
...
# Stems become gj_<stem>_24800. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (24791, "shell_green", 1, "shell product green extract (wave 24800)"),
    (24792, "libcgj_green", 1, "libcgj product green extract (wave 24800)"),
    (24793, "bar3_ready", 0, "product bar3 readiness gate (wave 24800)"),
    (24794, "product_score", 0, "product readiness score tag (wave 24800)"),
    (24795, "continuum_ready", 1, "continuum readiness gate (wave 24800)"),
    (24796, "smoke_soft", 1, "smoke soft lamp (wave 24800)"),
    (24797, "dyn_soft", 1, "dyn soft lamp (wave 24800)"),
    (24798, "milestone_tag", MILESTONE, "milestone 24800 product tag"),
    (24799, "continuum_wave", MILESTONE, "milestone 24800 continuum wave identity"),
    # 24800 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        24776: "continuum wave fill soft unit + batch id",
        24780: "continuum bridge soft unit + batch id",
        24790: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (24791–24799), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 24793
    tag_batch = MARKERS_LO + 7  # 24798
    wave_batch = MARKERS_LO + 8  # 24799
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_24700 / gj_shell_green_24600 / gj_shell_green_24500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_24700 / gj_libcgj_green_24600 / gj_libcgj_green_24500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_24700 / gj_bar3_ready_24600 / gj_bar3_ready_24500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_24700 / gj_product_score_24600 / gj_product_score_24500\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_24700 / gj_continuum_ready_24600 /\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_24700 / gj_smoke_soft_24600 / gj_smoke_soft_24500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_24700 / gj_dyn_soft_24600 / gj_dyn_soft_24500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_24700 / gj_milestone_tag_24600 / gj_milestone_tag_24500,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_24700 / gj_continuum_wave_24600 /\n"
...
def render_milestone_capstone() -> str:
    """graph_batch24800.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 24799
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 24791–24799; 24800 is capstone
