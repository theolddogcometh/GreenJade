...
"""
Soft continuum generator — MILESTONE 24900 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 24801–24900 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=24900   when parent has wired through graph_batch24900.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 24801 24900
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 24801 24900
...
graph batches ending at M. For M = **24900** the exclusive range is
**24801–24900** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 24801–24850  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 24851–24875  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 24876–24890  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 24891–24900  │ decade soft gates (Wave 107 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 24900 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_24900        1          graph_batch24891.c
  gj_libcgj_green_24900       1          graph_batch24892.c
  gj_bar3_ready_24900         0          graph_batch24893.c  (bar3 open)
  gj_product_score_24900      0          graph_batch24894.c  (score open)
  gj_continuum_ready_24900    1          graph_batch24895.c
  gj_smoke_soft_24900         1          graph_batch24896.c
  gj_dyn_soft_24900           1          graph_batch24897.c
  gj_milestone_tag_24900      24900      graph_batch24898.c
  gj_continuum_wave_24900     24900      graph_batch24899.c
  gj_batch_id_24900           24900      graph_batch24900.c
  gj_graph_milestone_24900    24900      graph_batch24900.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=24900`` means parent wired CGJ_SRCS through
    ``graph_batch24900.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_24900`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_24900`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M24900_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_24800`` / ``gj_*_24700`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 107 exclusive CREATE-ONLY lands the **markers** band
  (24891–24900) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_24900.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_24900.py

  # Restrict to marker band only (soft-gate TUs; Wave 107 exclusive):
  python3 user/libcgj/src/_gen_milestone_24900.py --band markers

  # Extension identity only (24801–24850):
  python3 user/libcgj/src/_gen_milestone_24900.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_24900.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_24800.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 24900 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 24900
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 24801
WAVE_HI = 24900  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 24801, 24850
IDENTITY_LO, IDENTITY_HI = 24851, 24875
FILL_LO, FILL_HI = 24876, 24890
MARKERS_LO, MARKERS_HI = 24891, 24900
...
# Stems become gj_<stem>_24900. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (24891, "shell_green", 1, "shell product green extract (wave 24900)"),
    (24892, "libcgj_green", 1, "libcgj product green extract (wave 24900)"),
    (24893, "bar3_ready", 0, "product bar3 readiness gate (wave 24900)"),
    (24894, "product_score", 0, "product readiness score tag (wave 24900)"),
    (24895, "continuum_ready", 1, "continuum readiness gate (wave 24900)"),
    (24896, "smoke_soft", 1, "smoke soft lamp (wave 24900)"),
    (24897, "dyn_soft", 1, "dyn soft lamp (wave 24900)"),
    (24898, "milestone_tag", MILESTONE, "milestone 24900 product tag"),
    (24899, "continuum_wave", MILESTONE, "milestone 24900 continuum wave identity"),
    # 24900 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        24876: "continuum wave fill soft unit + batch id",
        24880: "continuum bridge soft unit + batch id",
        24890: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (24891–24899), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 24893
    tag_batch = MARKERS_LO + 7  # 24898
    wave_batch = MARKERS_LO + 8  # 24899
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_24800 / gj_shell_green_24700 / gj_shell_green_24500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_24800 / gj_libcgj_green_24700 / gj_libcgj_green_24500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_24800 / gj_bar3_ready_24700 / gj_bar3_ready_24500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_24800 / gj_product_score_24700 / gj_product_score_24500\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_24800 / gj_continuum_ready_24700 /\n"
            f" * gj_continuum_ready_24500 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_24800 / gj_smoke_soft_24700 / gj_smoke_soft_24500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_24800 / gj_dyn_soft_24700 / gj_dyn_soft_24500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_24800 / gj_milestone_tag_24700 / gj_milestone_tag_24500,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_24800 / gj_continuum_wave_24700 /\n"
            f" * gj_continuum_wave_24500, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch24900.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 24899
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 24891–24899; 24900 is capstone
