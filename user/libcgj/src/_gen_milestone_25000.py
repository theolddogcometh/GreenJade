...
"""
Soft continuum generator — MILESTONE 25000 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 24901–25000 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=25000   when parent has wired through graph_batch25000.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 24901 25000
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 24901 25000
...
graph batches ending at M. For M = **25000** the exclusive range is
**24901–25000** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 24901–24950  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 24951–24975  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 24976–24990  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 24991–25000  │ decade soft gates (Wave 108 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 25000 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_25000        1          graph_batch24991.c
  gj_libcgj_green_25000       1          graph_batch24992.c
  gj_bar3_ready_25000         0          graph_batch24993.c  (bar3 open)
  gj_product_score_25000      0          graph_batch24994.c  (score open)
  gj_continuum_ready_25000    1          graph_batch24995.c
  gj_smoke_soft_25000         1          graph_batch24996.c
  gj_dyn_soft_25000           1          graph_batch24997.c
  gj_milestone_tag_25000      25000      graph_batch24998.c
  gj_continuum_wave_25000     25000      graph_batch24999.c
  gj_batch_id_25000           25000      graph_batch25000.c
  gj_graph_milestone_25000    25000      graph_batch25000.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=25000`` means parent wired CGJ_SRCS through
    ``graph_batch25000.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_25000`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_25000`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M25000_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_24900`` / ``gj_*_24800`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 108 exclusive CREATE-ONLY lands the **markers** band
  (24991–25000) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_25000.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_25000.py

  # Restrict to marker band only (soft-gate TUs; Wave 108 exclusive):
  python3 user/libcgj/src/_gen_milestone_25000.py --band markers

  # Extension identity only (24901–24950):
  python3 user/libcgj/src/_gen_milestone_25000.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_25000.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_24900.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 25000 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 25000
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 24901
WAVE_HI = 25000  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 24901, 24950
IDENTITY_LO, IDENTITY_HI = 24951, 24975
FILL_LO, FILL_HI = 24976, 24990
MARKERS_LO, MARKERS_HI = 24991, 25000
...
# Stems become gj_<stem>_25000. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (24991, "shell_green", 1, "shell product green extract (wave 25000)"),
    (24992, "libcgj_green", 1, "libcgj product green extract (wave 25000)"),
    (24993, "bar3_ready", 0, "product bar3 readiness gate (wave 25000)"),
    (24994, "product_score", 0, "product readiness score tag (wave 25000)"),
    (24995, "continuum_ready", 1, "continuum readiness gate (wave 25000)"),
    (24996, "smoke_soft", 1, "smoke soft lamp (wave 25000)"),
    (24997, "dyn_soft", 1, "dyn soft lamp (wave 25000)"),
    (24998, "milestone_tag", MILESTONE, "milestone 25000 product tag"),
    (24999, "continuum_wave", MILESTONE, "milestone 25000 continuum wave identity"),
    # 25000 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        24976: "continuum wave fill soft unit + batch id",
        24980: "continuum bridge soft unit + batch id",
        24990: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (24991–24999), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 24993
    tag_batch = MARKERS_LO + 7  # 24998
    wave_batch = MARKERS_LO + 8  # 24999
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_24900 / gj_shell_green_24800 / gj_shell_green_24700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_24900 / gj_libcgj_green_24800 / gj_libcgj_green_24700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_24900 / gj_bar3_ready_24800 / gj_bar3_ready_24700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_24900 / gj_product_score_24800 / gj_product_score_24700\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_24900 / gj_continuum_ready_24800 /\n"
            f" * gj_continuum_ready_24700 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_24900 / gj_smoke_soft_24800 / gj_smoke_soft_24700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_24900 / gj_dyn_soft_24800 / gj_dyn_soft_24700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_24900 / gj_milestone_tag_24800 / gj_milestone_tag_24700,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_24900 / gj_continuum_wave_24800 /\n"
            f" * gj_continuum_wave_24700, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch25000.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 24999
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 24991–24999; 25000 is capstone
