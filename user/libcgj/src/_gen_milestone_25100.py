...
"""
Soft continuum generator — MILESTONE 25100 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 25001–25100 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=25100   when parent has wired through graph_batch25100.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 25001 25100
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 25001 25100
...
graph batches ending at M. For M = **25100** the exclusive range is
**25001–25100** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 25001–25050  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 25051–25075  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 25076–25090  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 25091–25100  │ decade soft gates (Wave 109 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 25100 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_25100        1          graph_batch25091.c
  gj_libcgj_green_25100       1          graph_batch25092.c
  gj_bar3_ready_25100         0          graph_batch25093.c  (bar3 open)
  gj_product_score_25100      0          graph_batch25094.c  (score open)
  gj_continuum_ready_25100    1          graph_batch25095.c
  gj_smoke_soft_25100         1          graph_batch25096.c
  gj_dyn_soft_25100           1          graph_batch25097.c
  gj_milestone_tag_25100      25100      graph_batch25098.c
  gj_continuum_wave_25100     25100      graph_batch25099.c
  gj_batch_id_25100           25100      graph_batch25100.c
  gj_graph_milestone_25100    25100      graph_batch25100.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=25100`` means parent wired CGJ_SRCS through
    ``graph_batch25100.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_25100`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_25100`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M25100_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_25000`` / ``gj_*_24900`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 109 exclusive CREATE-ONLY lands the **markers** band
  (25091–25100) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_25100.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_25100.py

  # Restrict to marker band only (soft-gate TUs; Wave 109 exclusive):
  python3 user/libcgj/src/_gen_milestone_25100.py --band markers

  # Extension identity only (25001–25050):
  python3 user/libcgj/src/_gen_milestone_25100.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_25100.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_25000.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 25100 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 25100
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 25001
WAVE_HI = 25100  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 25001, 25050
IDENTITY_LO, IDENTITY_HI = 25051, 25075
FILL_LO, FILL_HI = 25076, 25090
MARKERS_LO, MARKERS_HI = 25091, 25100
...
# Stems become gj_<stem>_25100. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (25091, "shell_green", 1, "shell product green extract (wave 25100)"),
    (25092, "libcgj_green", 1, "libcgj product green extract (wave 25100)"),
    (25093, "bar3_ready", 0, "product bar3 readiness gate (wave 25100)"),
    (25094, "product_score", 0, "product readiness score tag (wave 25100)"),
    (25095, "continuum_ready", 1, "continuum readiness gate (wave 25100)"),
    (25096, "smoke_soft", 1, "smoke soft lamp (wave 25100)"),
    (25097, "dyn_soft", 1, "dyn soft lamp (wave 25100)"),
    (25098, "milestone_tag", MILESTONE, "milestone 25100 product tag"),
    (25099, "continuum_wave", MILESTONE, "milestone 25100 continuum wave identity"),
    # 25100 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        25076: "continuum wave fill soft unit + batch id",
        25080: "continuum bridge soft unit + batch id",
        25090: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (25091–25099), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 25093
    tag_batch = MARKERS_LO + 7  # 25098
    wave_batch = MARKERS_LO + 8  # 25099
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_25000 / gj_shell_green_24900 / gj_shell_green_24800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_25000 / gj_libcgj_green_24900 / gj_libcgj_green_24800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_25000 / gj_bar3_ready_24900 / gj_bar3_ready_24800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_25000 / gj_product_score_24900 / gj_product_score_24800\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_25000 / gj_continuum_ready_24900 /\n"
            f" * gj_continuum_ready_24800 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_25000 / gj_smoke_soft_24900 / gj_smoke_soft_24800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_25000 / gj_dyn_soft_24900 / gj_dyn_soft_24800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_25000 / gj_milestone_tag_24900 / gj_milestone_tag_24800,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_25000 / gj_continuum_wave_24900 /\n"
            f" * gj_continuum_wave_24800, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch25100.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 25099
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 25091–25099; 25100 is capstone
