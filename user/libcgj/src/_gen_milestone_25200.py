...
"""
Soft continuum generator — MILESTONE 25200 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 25101–25200 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=25200   when parent has wired through graph_batch25200.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 25101 25200
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 25101 25200
...
graph batches ending at M. For M = **25200** the exclusive range is
**25101–25200** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 25101–25150  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 25151–25175  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 25176–25190  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 25191–25200  │ decade soft gates (Wave 110 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 25200 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_25200        1          graph_batch25191.c
  gj_libcgj_green_25200       1          graph_batch25192.c
  gj_bar3_ready_25200         0          graph_batch25193.c  (bar3 open)
  gj_product_score_25200      0          graph_batch25194.c  (score open)
  gj_continuum_ready_25200    1          graph_batch25195.c
  gj_smoke_soft_25200         1          graph_batch25196.c
  gj_dyn_soft_25200           1          graph_batch25197.c
  gj_milestone_tag_25200      25200      graph_batch25198.c
  gj_continuum_wave_25200     25200      graph_batch25199.c
  gj_batch_id_25200           25200      graph_batch25200.c
  gj_graph_milestone_25200    25200      graph_batch25200.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=25200`` means parent wired CGJ_SRCS through
    ``graph_batch25200.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_25200`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_25200`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M25200_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_25100`` / ``gj_*_25000`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 110 exclusive CREATE-ONLY lands the **markers** band
  (25191–25200) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_25200.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_25200.py

  # Restrict to marker band only (soft-gate TUs; Wave 110 exclusive):
  python3 user/libcgj/src/_gen_milestone_25200.py --band markers

  # Extension identity only (25101–25150):
  python3 user/libcgj/src/_gen_milestone_25200.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_25200.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_25100.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 25200 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 25200
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 25101
WAVE_HI = 25200  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 25101, 25150
IDENTITY_LO, IDENTITY_HI = 25151, 25175
FILL_LO, FILL_HI = 25176, 25190
MARKERS_LO, MARKERS_HI = 25191, 25200
...
# Stems become gj_<stem>_25200. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (25191, "shell_green", 1, "shell product green extract (wave 25200)"),
    (25192, "libcgj_green", 1, "libcgj product green extract (wave 25200)"),
    (25193, "bar3_ready", 0, "product bar3 readiness gate (wave 25200)"),
    (25194, "product_score", 0, "product readiness score tag (wave 25200)"),
    (25195, "continuum_ready", 1, "continuum readiness gate (wave 25200)"),
    (25196, "smoke_soft", 1, "smoke soft lamp (wave 25200)"),
    (25197, "dyn_soft", 1, "dyn soft lamp (wave 25200)"),
    (25198, "milestone_tag", MILESTONE, "milestone 25200 product tag"),
    (25199, "continuum_wave", MILESTONE, "milestone 25200 continuum wave identity"),
    # 25200 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        25176: "continuum wave fill soft unit + batch id",
        25180: "continuum bridge soft unit + batch id",
        25190: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (25191–25199), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 25193
    tag_batch = MARKERS_LO + 7  # 25198
    wave_batch = MARKERS_LO + 8  # 25199
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_25100 / gj_shell_green_25000 / gj_shell_green_24900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_25100 / gj_libcgj_green_25000 / gj_libcgj_green_24900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_25100 / gj_bar3_ready_25000 / gj_bar3_ready_24900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_25100 / gj_product_score_25000 / gj_product_score_24900\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_25100 / gj_continuum_ready_25000 /\n"
            f" * gj_continuum_ready_24900 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_25100 / gj_smoke_soft_25000 / gj_smoke_soft_24900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_25100 / gj_dyn_soft_25000 / gj_dyn_soft_24900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_25100 / gj_milestone_tag_25000 / gj_milestone_tag_24900,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_25100 / gj_continuum_wave_25000 /\n"
            f" * gj_continuum_wave_24900, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch25200.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 25199
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 25191–25199; 25200 is capstone
