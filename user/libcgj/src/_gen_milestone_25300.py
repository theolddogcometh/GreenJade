...
"""
Soft continuum generator — MILESTONE 25300 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 25201–25300 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=25300   when parent has wired through graph_batch25300.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 25201 25300
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 25201 25300
...
graph batches ending at M. For M = **25300** the exclusive range is
**25201–25300** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 25201–25250  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 25251–25275  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 25276–25290  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 25291–25300  │ decade soft gates (Wave 111 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 25300 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_25300        1          graph_batch25291.c
  gj_libcgj_green_25300       1          graph_batch25292.c
  gj_bar3_ready_25300         0          graph_batch25293.c  (bar3 open)
  gj_product_score_25300      0          graph_batch25294.c  (score open)
  gj_continuum_ready_25300    1          graph_batch25295.c
  gj_smoke_soft_25300         1          graph_batch25296.c
  gj_dyn_soft_25300           1          graph_batch25297.c
  gj_milestone_tag_25300      25300      graph_batch25298.c
  gj_continuum_wave_25300     25300      graph_batch25299.c
  gj_batch_id_25300           25300      graph_batch25300.c
  gj_graph_milestone_25300    25300      graph_batch25300.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=25300`` means parent wired CGJ_SRCS through
    ``graph_batch25300.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_25300`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_25300`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M25300_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_25200`` / ``gj_*_25100`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 111 exclusive CREATE-ONLY lands the **markers** band
  (25291–25300) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_25300.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_25300.py

  # Restrict to marker band only (soft-gate TUs; Wave 111 exclusive):
  python3 user/libcgj/src/_gen_milestone_25300.py --band markers

  # Extension identity only (25201–25250):
  python3 user/libcgj/src/_gen_milestone_25300.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_25300.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_25200.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 25300 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 25300
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 25201
WAVE_HI = 25300  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 25201, 25250
IDENTITY_LO, IDENTITY_HI = 25251, 25275
FILL_LO, FILL_HI = 25276, 25290
MARKERS_LO, MARKERS_HI = 25291, 25300
...
# Stems become gj_<stem>_25300. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (25291, "shell_green", 1, "shell product green extract (wave 25300)"),
    (25292, "libcgj_green", 1, "libcgj product green extract (wave 25300)"),
    (25293, "bar3_ready", 0, "product bar3 readiness gate (wave 25300)"),
    (25294, "product_score", 0, "product readiness score tag (wave 25300)"),
    (25295, "continuum_ready", 1, "continuum readiness gate (wave 25300)"),
    (25296, "smoke_soft", 1, "smoke soft lamp (wave 25300)"),
    (25297, "dyn_soft", 1, "dyn soft lamp (wave 25300)"),
    (25298, "milestone_tag", MILESTONE, "milestone 25300 product tag"),
    (25299, "continuum_wave", MILESTONE, "milestone 25300 continuum wave identity"),
    # 25300 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        25276: "continuum wave fill soft unit + batch id",
        25280: "continuum bridge soft unit + batch id",
        25290: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (25291–25299), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 25293
    tag_batch = MARKERS_LO + 7  # 25298
    wave_batch = MARKERS_LO + 8  # 25299
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_25200 / gj_shell_green_25100 / gj_shell_green_25000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_25200 / gj_libcgj_green_25100 / gj_libcgj_green_25000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_25200 / gj_bar3_ready_25100 / gj_bar3_ready_25000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_25200 / gj_product_score_25100 / gj_product_score_25000\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_25200 / gj_continuum_ready_25100 /\n"
            f" * gj_continuum_ready_25000 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_25200 / gj_smoke_soft_25100 / gj_smoke_soft_25000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_25200 / gj_dyn_soft_25100 / gj_dyn_soft_25000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_25200 / gj_milestone_tag_25100 / gj_milestone_tag_25000,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_25200 / gj_continuum_wave_25100 /\n"
            f" * gj_continuum_wave_25000, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch25300.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 25299
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 25291–25299; 25300 is capstone
