...
"""
Soft continuum generator — MILESTONE 25400 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 25301–25400 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=25400   when parent has wired through graph_batch25400.c
...
graph batches ending at M. For M = **25400** the exclusive range is
**25301–25400** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 25301–25350  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 25351–25375  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 25376–25390  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 25391–25400  │ decade soft gates (Wave 112 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 25400 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_25400        1          graph_batch25391.c
  gj_libcgj_green_25400       1          graph_batch25392.c
  gj_bar3_ready_25400         0          graph_batch25393.c  (bar3 open)
  gj_product_score_25400      0          graph_batch25394.c  (score open)
  gj_continuum_ready_25400    1          graph_batch25395.c
  gj_smoke_soft_25400         1          graph_batch25396.c
  gj_dyn_soft_25400           1          graph_batch25397.c
  gj_milestone_tag_25400      25400      graph_batch25398.c
  gj_continuum_wave_25400     25400      graph_batch25399.c
  gj_batch_id_25400           25400      graph_batch25400.c
  gj_graph_milestone_25400    25400      graph_batch25400.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=25400`` means parent wired CGJ_SRCS through
    ``graph_batch25400.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_25400`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_25400`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M25400_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_25300`` / ``gj_*_25200`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 112 exclusive CREATE-ONLY lands the **markers** band
  (25391–25400) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_25400.py

  # Restrict to marker band only (soft-gate TUs; Wave 112 exclusive):
  python3 user/libcgj/src/_gen_milestone_25400.py --band markers

  # Extension identity only (25301–25350):
  python3 user/libcgj/src/_gen_milestone_25400.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_25400.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_25300.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 25400 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 25400
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 25301
WAVE_HI = 25400  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 25301, 25350
IDENTITY_LO, IDENTITY_HI = 25351, 25375
FILL_LO, FILL_HI = 25376, 25390
MARKERS_LO, MARKERS_HI = 25391, 25400
...
# Stems become gj_<stem>_25400. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (25391, "shell_green", 1, "shell product green extract (wave 25400)"),
    (25392, "libcgj_green", 1, "libcgj product green extract (wave 25400)"),
    (25393, "bar3_ready", 0, "product bar3 readiness gate (wave 25400)"),
    (25394, "product_score", 0, "product readiness score tag (wave 25400)"),
    (25395, "continuum_ready", 1, "continuum readiness gate (wave 25400)"),
    (25396, "smoke_soft", 1, "smoke soft lamp (wave 25400)"),
    (25397, "dyn_soft", 1, "dyn soft lamp (wave 25400)"),
    (25398, "milestone_tag", MILESTONE, "milestone 25400 product tag"),
    (25399, "continuum_wave", MILESTONE, "milestone 25400 continuum wave identity"),
    # 25400 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        25376: "continuum wave fill soft unit + batch id",
        25380: "continuum bridge soft unit + batch id",
        25390: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (25391–25399), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 25393
    tag_batch = MARKERS_LO + 7  # 25398
    wave_batch = MARKERS_LO + 8  # 25399
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_25300 / gj_shell_green_25200 / gj_shell_green_25100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_25300 / gj_libcgj_green_25200 / gj_libcgj_green_25100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_25300 / gj_bar3_ready_25200 / gj_bar3_ready_25100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_25300 / gj_product_score_25200 / gj_product_score_25100\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_25300 / gj_continuum_ready_25200 /\n"
            f" * gj_continuum_ready_25100 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_25300 / gj_smoke_soft_25200 / gj_smoke_soft_25100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_25300 / gj_dyn_soft_25200 / gj_dyn_soft_25100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_25300 / gj_milestone_tag_25200 / gj_milestone_tag_25100,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_25300 / gj_continuum_wave_25200 /\n"
            f" * gj_continuum_wave_25100, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch25400.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 25399
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 25391–25399; 25400 is capstone
