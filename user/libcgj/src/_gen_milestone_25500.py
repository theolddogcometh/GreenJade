...
"""
Soft continuum generator — MILESTONE 25500 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 25401–25500 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=25500   when parent has wired through graph_batch25500.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 25401 25500
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 25401 25500
...
graph batches ending at M. For M = **25500** the exclusive range is
**25401–25500** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 25401–25450  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 25451–25475  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 25476–25490  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 25491–25500  │ decade soft gates (Wave 113 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 25500 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_25500        1          graph_batch25491.c
  gj_libcgj_green_25500       1          graph_batch25492.c
  gj_bar3_ready_25500         0          graph_batch25493.c  (bar3 open)
  gj_product_score_25500      0          graph_batch25494.c  (score open)
  gj_continuum_ready_25500    1          graph_batch25495.c
  gj_smoke_soft_25500         1          graph_batch25496.c
  gj_dyn_soft_25500           1          graph_batch25497.c
  gj_milestone_tag_25500      25500      graph_batch25498.c
  gj_continuum_wave_25500     25500      graph_batch25499.c
  gj_batch_id_25500           25500      graph_batch25500.c
  gj_graph_milestone_25500    25500      graph_batch25500.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=25500`` means parent wired CGJ_SRCS through
    ``graph_batch25500.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_25500`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_25500`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M25500_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_25400`` / ``gj_*_25300`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 113 exclusive CREATE-ONLY lands the **markers** band
  (25491–25500) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_25500.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_25500.py

  # Restrict to marker band only (soft-gate TUs; Wave 113 exclusive):
  python3 user/libcgj/src/_gen_milestone_25500.py --band markers

  # Extension identity only (25401–25450):
  python3 user/libcgj/src/_gen_milestone_25500.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_25500.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_25400.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 25500 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 25500
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 25401
WAVE_HI = 25500  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 25401, 25450
IDENTITY_LO, IDENTITY_HI = 25451, 25475
FILL_LO, FILL_HI = 25476, 25490
MARKERS_LO, MARKERS_HI = 25491, 25500
...
# Stems become gj_<stem>_25500. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (25491, "shell_green", 1, "shell product green extract (wave 25500)"),
    (25492, "libcgj_green", 1, "libcgj product green extract (wave 25500)"),
    (25493, "bar3_ready", 0, "product bar3 readiness gate (wave 25500)"),
    (25494, "product_score", 0, "product readiness score tag (wave 25500)"),
    (25495, "continuum_ready", 1, "continuum readiness gate (wave 25500)"),
    (25496, "smoke_soft", 1, "smoke soft lamp (wave 25500)"),
    (25497, "dyn_soft", 1, "dyn soft lamp (wave 25500)"),
    (25498, "milestone_tag", MILESTONE, "milestone 25500 product tag"),
    (25499, "continuum_wave", MILESTONE, "milestone 25500 continuum wave identity"),
    # 25500 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        25476: "continuum wave fill soft unit + batch id",
        25480: "continuum bridge soft unit + batch id",
        25490: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (25491–25499), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 25493
    tag_batch = MARKERS_LO + 7  # 25498
    wave_batch = MARKERS_LO + 8  # 25499
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_25400 / gj_shell_green_25300 / gj_shell_green_25200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_25400 / gj_libcgj_green_25300 / gj_libcgj_green_25200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_25400 / gj_bar3_ready_25300 / gj_bar3_ready_25200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_25400 / gj_product_score_25300 / gj_product_score_25200\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_25400 / gj_continuum_ready_25300 /\n"
            f" * gj_continuum_ready_25200 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_25400 / gj_smoke_soft_25300 / gj_smoke_soft_25200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_25400 / gj_dyn_soft_25300 / gj_dyn_soft_25200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_25400 / gj_milestone_tag_25300 / gj_milestone_tag_25200,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_25400 / gj_continuum_wave_25300 /\n"
            f" * gj_continuum_wave_25200, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch25500.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 25499
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 25491–25499; 25500 is capstone
