...
"""
Soft continuum generator — MILESTONE 25600 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 25501–25600 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=25600   when parent has wired through graph_batch25600.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 25501 25600
...
graph batches ending at M. For M = **25600** the exclusive range is
**25501–25600** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 25501–25550  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 25551–25575  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 25576–25590  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 25591–25600  │ decade soft gates (Wave 114 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 25600 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_25600        1          graph_batch25591.c
  gj_libcgj_green_25600       1          graph_batch25592.c
  gj_bar3_ready_25600         0          graph_batch25593.c  (bar3 open)
  gj_product_score_25600      0          graph_batch25594.c  (score open)
  gj_continuum_ready_25600    1          graph_batch25595.c
  gj_smoke_soft_25600         1          graph_batch25596.c
  gj_dyn_soft_25600           1          graph_batch25597.c
  gj_milestone_tag_25600      25600      graph_batch25598.c
  gj_continuum_wave_25600     25600      graph_batch25599.c
  gj_batch_id_25600           25600      graph_batch25600.c
  gj_graph_milestone_25600    25600      graph_batch25600.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=25600`` means parent wired CGJ_SRCS through
    ``graph_batch25600.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_25600`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_25600`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M25600_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_25500`` / ``gj_*_25400`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 114 exclusive CREATE-ONLY lands the **markers** band
  (25591–25600) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_25600.py

  # Restrict to marker band only (soft-gate TUs; Wave 114 exclusive):
  python3 user/libcgj/src/_gen_milestone_25600.py --band markers

  # Extension identity only (25501–25550):
  python3 user/libcgj/src/_gen_milestone_25600.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_25600.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_25500.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 25600 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 25600
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 25501
WAVE_HI = 25600  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 25501, 25550
IDENTITY_LO, IDENTITY_HI = 25551, 25575
FILL_LO, FILL_HI = 25576, 25590
MARKERS_LO, MARKERS_HI = 25591, 25600
...
# Stems become gj_<stem>_25600. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (25591, "shell_green", 1, "shell product green extract (wave 25600)"),
    (25592, "libcgj_green", 1, "libcgj product green extract (wave 25600)"),
    (25593, "bar3_ready", 0, "product bar3 readiness gate (wave 25600)"),
    (25594, "product_score", 0, "product readiness score tag (wave 25600)"),
    (25595, "continuum_ready", 1, "continuum readiness gate (wave 25600)"),
    (25596, "smoke_soft", 1, "smoke soft lamp (wave 25600)"),
    (25597, "dyn_soft", 1, "dyn soft lamp (wave 25600)"),
    (25598, "milestone_tag", MILESTONE, "milestone 25600 product tag"),
    (25599, "continuum_wave", MILESTONE, "milestone 25600 continuum wave identity"),
    # 25600 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        25576: "continuum wave fill soft unit + batch id",
        25580: "continuum bridge soft unit + batch id",
        25590: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (25591–25599), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 25593
    tag_batch = MARKERS_LO + 7  # 25598
    wave_batch = MARKERS_LO + 8  # 25599
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_25500 / gj_shell_green_25400 / gj_shell_green_25300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_25500 / gj_libcgj_green_25400 / gj_libcgj_green_25300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_25500 / gj_bar3_ready_25400 / gj_bar3_ready_25300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_25500 / gj_product_score_25400 / gj_product_score_25300\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_25500 / gj_continuum_ready_25400 /\n"
            f" * gj_continuum_ready_25300 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_25500 / gj_smoke_soft_25400 / gj_smoke_soft_25300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_25500 / gj_dyn_soft_25400 / gj_dyn_soft_25300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_25500 / gj_milestone_tag_25400 / gj_milestone_tag_25300,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_25500 / gj_continuum_wave_25400 /\n"
            f" * gj_continuum_wave_25300, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch25600.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 25599
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 25591–25599; 25600 is capstone
