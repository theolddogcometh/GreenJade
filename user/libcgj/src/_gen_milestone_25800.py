...
"""
Soft continuum generator — MILESTONE 25800 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 25701–25800 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=25800   when parent has wired through graph_batch25800.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 25701 25800
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 25701 25800
...
graph batches ending at M. For M = **25800** the exclusive range is
**25701–25800** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 25701–25750  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 25751–25775  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 25776–25790  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 25791–25800  │ decade soft gates (Wave 116 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 25800 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_25800        1          graph_batch25791.c
  gj_libcgj_green_25800       1          graph_batch25792.c
  gj_bar3_ready_25800         0          graph_batch25793.c  (bar3 open)
  gj_product_score_25800      0          graph_batch25794.c  (score open)
  gj_continuum_ready_25800    1          graph_batch25795.c
  gj_smoke_soft_25800         1          graph_batch25796.c
  gj_dyn_soft_25800           1          graph_batch25797.c
  gj_milestone_tag_25800      25800      graph_batch25798.c
  gj_continuum_wave_25800     25800      graph_batch25799.c
  gj_batch_id_25800           25800      graph_batch25800.c
  gj_graph_milestone_25800    25800      graph_batch25800.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=25800`` means parent wired CGJ_SRCS through
    ``graph_batch25800.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_25800`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_25800`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M25800_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_25700`` / ``gj_*_25600`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 116 exclusive CREATE-ONLY lands the **markers** band
  (25791–25800) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_25800.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_25800.py

  # Restrict to marker band only (soft-gate TUs; Wave 116 exclusive):
  python3 user/libcgj/src/_gen_milestone_25800.py --band markers

  # Extension identity only (25701–25750):
  python3 user/libcgj/src/_gen_milestone_25800.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_25800.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_25700.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 25800 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 25800
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 25701
WAVE_HI = 25800  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 25701, 25750
IDENTITY_LO, IDENTITY_HI = 25751, 25775
FILL_LO, FILL_HI = 25776, 25790
MARKERS_LO, MARKERS_HI = 25791, 25800
...
# Stems become gj_<stem>_25800. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (25791, "shell_green", 1, "shell product green extract (wave 25800)"),
    (25792, "libcgj_green", 1, "libcgj product green extract (wave 25800)"),
    (25793, "bar3_ready", 0, "product bar3 readiness gate (wave 25800)"),
    (25794, "product_score", 0, "product readiness score tag (wave 25800)"),
    (25795, "continuum_ready", 1, "continuum readiness gate (wave 25800)"),
    (25796, "smoke_soft", 1, "smoke soft lamp (wave 25800)"),
    (25797, "dyn_soft", 1, "dyn soft lamp (wave 25800)"),
    (25798, "milestone_tag", MILESTONE, "milestone 25800 product tag"),
    (25799, "continuum_wave", MILESTONE, "milestone 25800 continuum wave identity"),
    # 25800 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        25776: "continuum wave fill soft unit + batch id",
        25780: "continuum bridge soft unit + batch id",
        25790: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (25791–25799), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 25793
    tag_batch = MARKERS_LO + 7  # 25798
    wave_batch = MARKERS_LO + 8  # 25799
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_25700 / gj_shell_green_25600 / gj_shell_green_25500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_25700 / gj_libcgj_green_25600 / gj_libcgj_green_25500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_25700 / gj_bar3_ready_25600 / gj_bar3_ready_25500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_25700 / gj_product_score_25600 / gj_product_score_25500\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_25700 / gj_continuum_ready_25600 /\n"
            f" * gj_continuum_ready_25500 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_25700 / gj_smoke_soft_25600 / gj_smoke_soft_25500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_25700 / gj_dyn_soft_25600 / gj_dyn_soft_25500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_25700 / gj_milestone_tag_25600 / gj_milestone_tag_25500,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_25700 / gj_continuum_wave_25600 /\n"
            f" * gj_continuum_wave_25500, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch25800.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 25799
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 25791–25799; 25800 is capstone
