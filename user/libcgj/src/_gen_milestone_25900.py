...
"""
Soft continuum generator — MILESTONE 25900 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 25801–25900 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=25900   when parent has wired through graph_batch25900.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 25801 25900
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 25801 25900
...
graph batches ending at M. For M = **25900** the exclusive range is
**25801–25900** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 25801–25850  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 25851–25875  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 25876–25890  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 25891–25900  │ decade soft gates (Wave 117 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 25900 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_25900        1          graph_batch25891.c
  gj_libcgj_green_25900       1          graph_batch25892.c
  gj_bar3_ready_25900         0          graph_batch25893.c  (bar3 open)
  gj_product_score_25900      0          graph_batch25894.c  (score open)
  gj_continuum_ready_25900    1          graph_batch25895.c
  gj_smoke_soft_25900         1          graph_batch25896.c
  gj_dyn_soft_25900           1          graph_batch25897.c
  gj_milestone_tag_25900      25900      graph_batch25898.c
  gj_continuum_wave_25900     25900      graph_batch25899.c
  gj_batch_id_25900           25900      graph_batch25900.c
  gj_graph_milestone_25900    25900      graph_batch25900.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=25900`` means parent wired CGJ_SRCS through
    ``graph_batch25900.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_25900`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_25900`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M25900_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_25800`` / ``gj_*_25700`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 117 exclusive CREATE-ONLY lands the **markers** band
  (25891–25900) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_25900.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_25900.py

  # Restrict to marker band only (soft-gate TUs; Wave 117 exclusive):
  python3 user/libcgj/src/_gen_milestone_25900.py --band markers

  # Extension identity only (25801–25850):
  python3 user/libcgj/src/_gen_milestone_25900.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_25900.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_25800.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 25900 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 25900
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 25801
WAVE_HI = 25900  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 25801, 25850
IDENTITY_LO, IDENTITY_HI = 25851, 25875
FILL_LO, FILL_HI = 25876, 25890
MARKERS_LO, MARKERS_HI = 25891, 25900
...
# Stems become gj_<stem>_25900. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (25891, "shell_green", 1, "shell product green extract (wave 25900)"),
    (25892, "libcgj_green", 1, "libcgj product green extract (wave 25900)"),
    (25893, "bar3_ready", 0, "product bar3 readiness gate (wave 25900)"),
    (25894, "product_score", 0, "product readiness score tag (wave 25900)"),
    (25895, "continuum_ready", 1, "continuum readiness gate (wave 25900)"),
    (25896, "smoke_soft", 1, "smoke soft lamp (wave 25900)"),
    (25897, "dyn_soft", 1, "dyn soft lamp (wave 25900)"),
    (25898, "milestone_tag", MILESTONE, "milestone 25900 product tag"),
    (25899, "continuum_wave", MILESTONE, "milestone 25900 continuum wave identity"),
    # 25900 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        25876: "continuum wave fill soft unit + batch id",
        25880: "continuum bridge soft unit + batch id",
        25890: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (25891–25899), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 25893
    tag_batch = MARKERS_LO + 7  # 25898
    wave_batch = MARKERS_LO + 8  # 25899
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_25800 / gj_shell_green_25700 / gj_shell_green_25600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_25800 / gj_libcgj_green_25700 / gj_libcgj_green_25600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_25800 / gj_bar3_ready_25700 / gj_bar3_ready_25600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_25800 / gj_product_score_25700 / gj_product_score_25600\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_25800 / gj_continuum_ready_25700 /\n"
            f" * gj_continuum_ready_25600 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_25800 / gj_smoke_soft_25700 / gj_smoke_soft_25600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_25800 / gj_dyn_soft_25700 / gj_dyn_soft_25600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_25800 / gj_milestone_tag_25700 / gj_milestone_tag_25600,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_25800 / gj_continuum_wave_25700 /\n"
            f" * gj_continuum_wave_25600, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch25900.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 25899
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 25891–25899; 25900 is capstone
