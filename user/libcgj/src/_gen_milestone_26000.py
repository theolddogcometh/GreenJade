...
"""
Soft continuum generator — MILESTONE 26000 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 25901–26000 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=26000   when parent has wired through graph_batch26000.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 25901 26000
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 25901 26000
...
graph batches ending at M. For M = **26000** the exclusive range is
**25901–26000** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 25901–25950  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 25951–25975  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 25976–25990  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 25991–26000  │ decade soft gates (Wave 118 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 26000 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_26000        1          graph_batch25991.c
  gj_libcgj_green_26000       1          graph_batch25992.c
  gj_bar3_ready_26000         0          graph_batch25993.c  (bar3 open)
  gj_product_score_26000      0          graph_batch25994.c  (score open)
  gj_continuum_ready_26000    1          graph_batch25995.c
  gj_smoke_soft_26000         1          graph_batch25996.c
  gj_dyn_soft_26000           1          graph_batch25997.c
  gj_milestone_tag_26000      26000      graph_batch25998.c
  gj_continuum_wave_26000     26000      graph_batch25999.c
  gj_batch_id_26000           26000      graph_batch26000.c
  gj_graph_milestone_26000    26000      graph_batch26000.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=26000`` means parent wired CGJ_SRCS through
    ``graph_batch26000.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_26000`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_26000`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M26000_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_25900`` / ``gj_*_25800`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 118 exclusive CREATE-ONLY lands the **markers** band
  (25991–26000) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_26000.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_26000.py

  # Restrict to marker band only (soft-gate TUs; Wave 118 exclusive):
  python3 user/libcgj/src/_gen_milestone_26000.py --band markers

  # Extension identity only (25901–25950):
  python3 user/libcgj/src/_gen_milestone_26000.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_26000.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_25900.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 26000 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 26000
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 25901
WAVE_HI = 26000  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 25901, 25950
IDENTITY_LO, IDENTITY_HI = 25951, 25975
FILL_LO, FILL_HI = 25976, 25990
MARKERS_LO, MARKERS_HI = 25991, 26000
...
# Stems become gj_<stem>_26000. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (25991, "shell_green", 1, "shell product green extract (wave 26000)"),
    (25992, "libcgj_green", 1, "libcgj product green extract (wave 26000)"),
    (25993, "bar3_ready", 0, "product bar3 readiness gate (wave 26000)"),
    (25994, "product_score", 0, "product readiness score tag (wave 26000)"),
    (25995, "continuum_ready", 1, "continuum readiness gate (wave 26000)"),
    (25996, "smoke_soft", 1, "smoke soft lamp (wave 26000)"),
    (25997, "dyn_soft", 1, "dyn soft lamp (wave 26000)"),
    (25998, "milestone_tag", MILESTONE, "milestone 26000 product tag"),
    (25999, "continuum_wave", MILESTONE, "milestone 26000 continuum wave identity"),
    # 26000 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        25976: "continuum wave fill soft unit + batch id",
        25980: "continuum bridge soft unit + batch id",
        25990: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (25991–25999), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 25993
    tag_batch = MARKERS_LO + 7  # 25998
    wave_batch = MARKERS_LO + 8  # 25999
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_25900 / gj_shell_green_25800 / gj_shell_green_25700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_25900 / gj_libcgj_green_25800 / gj_libcgj_green_25700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_25900 / gj_bar3_ready_25800 / gj_bar3_ready_25700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_25900 / gj_product_score_25800 / gj_product_score_25700\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_25900 / gj_continuum_ready_25800 /\n"
            f" * gj_continuum_ready_25700 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_25900 / gj_smoke_soft_25800 / gj_smoke_soft_25700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_25900 / gj_dyn_soft_25800 / gj_dyn_soft_25700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_25900 / gj_milestone_tag_25800 / gj_milestone_tag_25700,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_25900 / gj_continuum_wave_25800 /\n"
            f" * gj_continuum_wave_25700, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch26000.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 25999
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 25991–25999; 26000 is capstone
