...
"""
Soft continuum generator — MILESTONE 26100 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 26001–26100 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=26100   when parent has wired through graph_batch26100.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 26001 26100
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 26001 26100
...
graph batches ending at M. For M = **26100** the exclusive range is
**26001–26100** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 26001–26050  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 26051–26075  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 26076–26090  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 26091–26100  │ decade soft gates (Wave 119 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 26100 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_26100        1          graph_batch26091.c
  gj_libcgj_green_26100       1          graph_batch26092.c
  gj_bar3_ready_26100         0          graph_batch26093.c  (bar3 open)
  gj_product_score_26100      0          graph_batch26094.c  (score open)
  gj_continuum_ready_26100    1          graph_batch26095.c
  gj_smoke_soft_26100         1          graph_batch26096.c
  gj_dyn_soft_26100           1          graph_batch26097.c
  gj_milestone_tag_26100      26100      graph_batch26098.c
  gj_continuum_wave_26100     26100      graph_batch26099.c
  gj_batch_id_26100           26100      graph_batch26100.c
  gj_graph_milestone_26100    26100      graph_batch26100.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=26100`` means parent wired CGJ_SRCS through
    ``graph_batch26100.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_26100`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_26100`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M26100_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_26000`` / ``gj_*_25900`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 119 exclusive CREATE-ONLY lands the **markers** band
  (26091–26100) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_26100.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_26100.py

  # Restrict to marker band only (soft-gate TUs; Wave 119 exclusive):
  python3 user/libcgj/src/_gen_milestone_26100.py --band markers

  # Extension identity only (26001–26050):
  python3 user/libcgj/src/_gen_milestone_26100.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_26100.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_26000.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 26100 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 26100
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 26001
WAVE_HI = 26100  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 26001, 26050
IDENTITY_LO, IDENTITY_HI = 26051, 26075
FILL_LO, FILL_HI = 26076, 26090
MARKERS_LO, MARKERS_HI = 26091, 26100
...
# Stems become gj_<stem>_26100. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (26091, "shell_green", 1, "shell product green extract (wave 26100)"),
    (26092, "libcgj_green", 1, "libcgj product green extract (wave 26100)"),
    (26093, "bar3_ready", 0, "product bar3 readiness gate (wave 26100)"),
    (26094, "product_score", 0, "product readiness score tag (wave 26100)"),
    (26095, "continuum_ready", 1, "continuum readiness gate (wave 26100)"),
    (26096, "smoke_soft", 1, "smoke soft lamp (wave 26100)"),
    (26097, "dyn_soft", 1, "dyn soft lamp (wave 26100)"),
    (26098, "milestone_tag", MILESTONE, "milestone 26100 product tag"),
    (26099, "continuum_wave", MILESTONE, "milestone 26100 continuum wave identity"),
    # 26100 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        26076: "continuum wave fill soft unit + batch id",
        26080: "continuum bridge soft unit + batch id",
        26090: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (26091–26099), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 26093
    tag_batch = MARKERS_LO + 7  # 26098
    wave_batch = MARKERS_LO + 8  # 26099
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_26000 / gj_shell_green_25900 / gj_shell_green_25800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_26000 / gj_libcgj_green_25900 / gj_libcgj_green_25800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_26000 / gj_bar3_ready_25900 / gj_bar3_ready_25800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_26000 / gj_product_score_25900 / gj_product_score_25800\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_26000 / gj_continuum_ready_25900 /\n"
            f" * gj_continuum_ready_25800 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_26000 / gj_smoke_soft_25900 / gj_smoke_soft_25800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_26000 / gj_dyn_soft_25900 / gj_dyn_soft_25800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_26000 / gj_milestone_tag_25900 / gj_milestone_tag_25800,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_26000 / gj_continuum_wave_25900 /\n"
            f" * gj_continuum_wave_25800, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch26100.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 26099
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 26091–26099; 26100 is capstone
