...
"""
Soft continuum generator — MILESTONE 26200 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 26101–26200 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=26200   when parent has wired through graph_batch26200.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 26101 26200
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 26101 26200
...
graph batches ending at M. For M = **26200** the exclusive range is
**26101–26200** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 26101–26150  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 26151–26175  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 26176–26190  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 26191–26200  │ decade soft gates (Wave 120 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 26200 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_26200        1          graph_batch26191.c
  gj_libcgj_green_26200       1          graph_batch26192.c
  gj_bar3_ready_26200         0          graph_batch26193.c  (bar3 open)
  gj_product_score_26200      0          graph_batch26194.c  (score open)
  gj_continuum_ready_26200    1          graph_batch26195.c
  gj_smoke_soft_26200         1          graph_batch26196.c
  gj_dyn_soft_26200           1          graph_batch26197.c
  gj_milestone_tag_26200      26200      graph_batch26198.c
  gj_continuum_wave_26200     26200      graph_batch26199.c
  gj_batch_id_26200           26200      graph_batch26200.c
  gj_graph_milestone_26200    26200      graph_batch26200.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=26200`` means parent wired CGJ_SRCS through
    ``graph_batch26200.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_26200`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_26200`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M26200_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_26100`` / ``gj_*_26000`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 120 exclusive CREATE-ONLY lands the **markers** band
  (26191–26200) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_26200.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_26200.py

  # Restrict to marker band only (soft-gate TUs; Wave 120 exclusive):
  python3 user/libcgj/src/_gen_milestone_26200.py --band markers

  # Extension identity only (26101–26150):
  python3 user/libcgj/src/_gen_milestone_26200.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_26200.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_26100.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 26200 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 26200
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 26101
WAVE_HI = 26200  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 26101, 26150
IDENTITY_LO, IDENTITY_HI = 26151, 26175
FILL_LO, FILL_HI = 26176, 26190
MARKERS_LO, MARKERS_HI = 26191, 26200
...
# Stems become gj_<stem>_26200. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (26191, "shell_green", 1, "shell product green extract (wave 26200)"),
    (26192, "libcgj_green", 1, "libcgj product green extract (wave 26200)"),
    (26193, "bar3_ready", 0, "product bar3 readiness gate (wave 26200)"),
    (26194, "product_score", 0, "product readiness score tag (wave 26200)"),
    (26195, "continuum_ready", 1, "continuum readiness gate (wave 26200)"),
    (26196, "smoke_soft", 1, "smoke soft lamp (wave 26200)"),
    (26197, "dyn_soft", 1, "dyn soft lamp (wave 26200)"),
    (26198, "milestone_tag", MILESTONE, "milestone 26200 product tag"),
    (26199, "continuum_wave", MILESTONE, "milestone 26200 continuum wave identity"),
    # 26200 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        26176: "continuum wave fill soft unit + batch id",
        26180: "continuum bridge soft unit + batch id",
        26190: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (26191–26199), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 26193
    tag_batch = MARKERS_LO + 7  # 26198
    wave_batch = MARKERS_LO + 8  # 26199
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_26100 / gj_shell_green_26000 / gj_shell_green_25900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_26100 / gj_libcgj_green_26000 / gj_libcgj_green_25900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_26100 / gj_bar3_ready_26000 / gj_bar3_ready_25900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_26100 / gj_product_score_26000 / gj_product_score_25900\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_26100 / gj_continuum_ready_26000 /\n"
            f" * gj_continuum_ready_25900 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_26100 / gj_smoke_soft_26000 / gj_smoke_soft_25900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_26100 / gj_dyn_soft_26000 / gj_dyn_soft_25900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_26100 / gj_milestone_tag_26000 / gj_milestone_tag_25900,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_26100 / gj_continuum_wave_26000 /\n"
            f" * gj_continuum_wave_25900, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch26200.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 26199
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 26191–26199; 26200 is capstone
