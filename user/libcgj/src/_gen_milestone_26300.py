...
"""
Soft continuum generator — MILESTONE 26300 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 26201–26300 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=26300   when parent has wired through graph_batch26300.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 26201 26300
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 26201 26300
...
graph batches ending at M. For M = **26300** the exclusive range is
**26201–26300** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 26201–26250  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 26251–26275  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 26276–26290  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 26291–26300  │ decade soft gates (Wave 121 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 26300 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_26300        1          graph_batch26291.c
  gj_libcgj_green_26300       1          graph_batch26292.c
  gj_bar3_ready_26300         0          graph_batch26293.c  (bar3 open)
  gj_product_score_26300      0          graph_batch26294.c  (score open)
  gj_continuum_ready_26300    1          graph_batch26295.c
  gj_smoke_soft_26300         1          graph_batch26296.c
  gj_dyn_soft_26300           1          graph_batch26297.c
  gj_milestone_tag_26300      26300      graph_batch26298.c
  gj_continuum_wave_26300     26300      graph_batch26299.c
  gj_batch_id_26300           26300      graph_batch26300.c
  gj_graph_milestone_26300    26300      graph_batch26300.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=26300`` means parent wired CGJ_SRCS through
    ``graph_batch26300.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_26300`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_26300`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M26300_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_26200`` / ``gj_*_26100`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 121 exclusive CREATE-ONLY lands the **markers** band
  (26291–26300) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_26300.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_26300.py

  # Restrict to marker band only (soft-gate TUs; Wave 121 exclusive):
  python3 user/libcgj/src/_gen_milestone_26300.py --band markers

  # Extension identity only (26201–26250):
  python3 user/libcgj/src/_gen_milestone_26300.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_26300.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_26200.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 26300 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 26300
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 26201
WAVE_HI = 26300  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 26201, 26250
IDENTITY_LO, IDENTITY_HI = 26251, 26275
FILL_LO, FILL_HI = 26276, 26290
MARKERS_LO, MARKERS_HI = 26291, 26300
...
# Stems become gj_<stem>_26300. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (26291, "shell_green", 1, "shell product green extract (wave 26300)"),
    (26292, "libcgj_green", 1, "libcgj product green extract (wave 26300)"),
    (26293, "bar3_ready", 0, "product bar3 readiness gate (wave 26300)"),
    (26294, "product_score", 0, "product readiness score tag (wave 26300)"),
    (26295, "continuum_ready", 1, "continuum readiness gate (wave 26300)"),
    (26296, "smoke_soft", 1, "smoke soft lamp (wave 26300)"),
    (26297, "dyn_soft", 1, "dyn soft lamp (wave 26300)"),
    (26298, "milestone_tag", MILESTONE, "milestone 26300 product tag"),
    (26299, "continuum_wave", MILESTONE, "milestone 26300 continuum wave identity"),
    # 26300 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        26276: "continuum wave fill soft unit + batch id",
        26280: "continuum bridge soft unit + batch id",
        26290: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (26291–26299), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 26293
    tag_batch = MARKERS_LO + 7  # 26298
    wave_batch = MARKERS_LO + 8  # 26299
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_26200 / gj_shell_green_26100 / gj_shell_green_26000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_26200 / gj_libcgj_green_26100 / gj_libcgj_green_26000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_26200 / gj_bar3_ready_26100 / gj_bar3_ready_26000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_26200 / gj_product_score_26100 / gj_product_score_26000\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_26200 / gj_continuum_ready_26100 /\n"
            f" * gj_continuum_ready_26000 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_26200 / gj_smoke_soft_26100 / gj_smoke_soft_26000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_26200 / gj_dyn_soft_26100 / gj_dyn_soft_26000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_26200 / gj_milestone_tag_26100 / gj_milestone_tag_26000,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_26200 / gj_continuum_wave_26100 /\n"
            f" * gj_continuum_wave_26000, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch26300.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 26299
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 26291–26299; 26300 is capstone
