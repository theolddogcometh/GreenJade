...
"""
Soft continuum generator — MILESTONE 26400 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 26301–26400 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=26400   when parent has wired through graph_batch26400.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 26301 26400
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 26301 26400
...
graph batches ending at M. For M = **26400** the exclusive range is
**26301–26400** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 26301–26350  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 26351–26375  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 26376–26390  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 26391–26400  │ decade soft gates (Wave 122 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 26400 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_26400        1          graph_batch26391.c
  gj_libcgj_green_26400       1          graph_batch26392.c
  gj_bar3_ready_26400         0          graph_batch26393.c  (bar3 open)
  gj_product_score_26400      0          graph_batch26394.c  (score open)
  gj_continuum_ready_26400    1          graph_batch26395.c
  gj_smoke_soft_26400         1          graph_batch26396.c
  gj_dyn_soft_26400           1          graph_batch26397.c
  gj_milestone_tag_26400      26400      graph_batch26398.c
  gj_continuum_wave_26400     26400      graph_batch26399.c
  gj_batch_id_26400           26400      graph_batch26400.c
  gj_graph_milestone_26400    26400      graph_batch26400.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=26400`` means parent wired CGJ_SRCS through
    ``graph_batch26400.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_26400`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_26400`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M26400_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_26300`` / ``gj_*_26200`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 122 exclusive CREATE-ONLY lands the **markers** band
  (26391–26400) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_26400.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_26400.py

  # Restrict to marker band only (soft-gate TUs; Wave 122 exclusive):
  python3 user/libcgj/src/_gen_milestone_26400.py --band markers

  # Extension identity only (26301–26350):
  python3 user/libcgj/src/_gen_milestone_26400.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_26400.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_26300.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 26400 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 26400
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 26301
WAVE_HI = 26400  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 26301, 26350
IDENTITY_LO, IDENTITY_HI = 26351, 26375
FILL_LO, FILL_HI = 26376, 26390
MARKERS_LO, MARKERS_HI = 26391, 26400
...
# Stems become gj_<stem>_26400. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (26391, "shell_green", 1, "shell product green extract (wave 26400)"),
    (26392, "libcgj_green", 1, "libcgj product green extract (wave 26400)"),
    (26393, "bar3_ready", 0, "product bar3 readiness gate (wave 26400)"),
    (26394, "product_score", 0, "product readiness score tag (wave 26400)"),
    (26395, "continuum_ready", 1, "continuum readiness gate (wave 26400)"),
    (26396, "smoke_soft", 1, "smoke soft lamp (wave 26400)"),
    (26397, "dyn_soft", 1, "dyn soft lamp (wave 26400)"),
    (26398, "milestone_tag", MILESTONE, "milestone 26400 product tag"),
    (26399, "continuum_wave", MILESTONE, "milestone 26400 continuum wave identity"),
    # 26400 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        26376: "continuum wave fill soft unit + batch id",
        26380: "continuum bridge soft unit + batch id",
        26390: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (26391–26399), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 26393
    tag_batch = MARKERS_LO + 7  # 26398
    wave_batch = MARKERS_LO + 8  # 26399
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_26300 / gj_shell_green_26200 / gj_shell_green_26100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_26300 / gj_libcgj_green_26200 / gj_libcgj_green_26100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_26300 / gj_bar3_ready_26200 / gj_bar3_ready_26100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_26300 / gj_product_score_26200 / gj_product_score_26100\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_26300 / gj_continuum_ready_26200 /\n"
            f" * gj_continuum_ready_26100 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_26300 / gj_smoke_soft_26200 / gj_smoke_soft_26100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_26300 / gj_dyn_soft_26200 / gj_dyn_soft_26100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_26300 / gj_milestone_tag_26200 / gj_milestone_tag_26100,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_26300 / gj_continuum_wave_26200 /\n"
            f" * gj_continuum_wave_26100, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch26400.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 26399
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 26391–26399; 26400 is capstone
