...
"""
Soft continuum generator — MILESTONE 26600 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 26501–26600 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=26600   when parent has wired through graph_batch26600.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 26501 26600
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 26501 26600
...
graph batches ending at M. For M = **26600** the exclusive range is
**26501–26600** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 26501–26550  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 26551–26575  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 26576–26590  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 26591–26600  │ decade soft gates (Wave 124 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 26600 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_26600        1          graph_batch26591.c
  gj_libcgj_green_26600       1          graph_batch26592.c
  gj_bar3_ready_26600         0          graph_batch26593.c  (bar3 open)
  gj_product_score_26600      0          graph_batch26594.c  (score open)
  gj_continuum_ready_26600    1          graph_batch26595.c
  gj_smoke_soft_26600         1          graph_batch26596.c
  gj_dyn_soft_26600           1          graph_batch26597.c
  gj_milestone_tag_26600      26600      graph_batch26598.c
  gj_continuum_wave_26600     26600      graph_batch26599.c
  gj_batch_id_26600           26600      graph_batch26600.c
  gj_graph_milestone_26600    26600      graph_batch26600.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=26600`` means parent wired CGJ_SRCS through
    ``graph_batch26600.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_26600`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_26600`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M26600_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_26500`` / ``gj_*_26400`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 124 exclusive CREATE-ONLY lands the **markers** band
  (26591–26600) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_26600.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_26600.py

  # Restrict to marker band only (soft-gate TUs; Wave 124 exclusive):
  python3 user/libcgj/src/_gen_milestone_26600.py --band markers

  # Extension identity only (26501–26550):
  python3 user/libcgj/src/_gen_milestone_26600.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_26600.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_26500.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 26600 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 26600
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 26501
WAVE_HI = 26600  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 26501, 26550
IDENTITY_LO, IDENTITY_HI = 26551, 26575
FILL_LO, FILL_HI = 26576, 26590
MARKERS_LO, MARKERS_HI = 26591, 26600
...
# Stems become gj_<stem>_26600. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (26591, "shell_green", 1, "shell product green extract (wave 26600)"),
    (26592, "libcgj_green", 1, "libcgj product green extract (wave 26600)"),
    (26593, "bar3_ready", 0, "product bar3 readiness gate (wave 26600)"),
    (26594, "product_score", 0, "product readiness score tag (wave 26600)"),
    (26595, "continuum_ready", 1, "continuum readiness gate (wave 26600)"),
    (26596, "smoke_soft", 1, "smoke soft lamp (wave 26600)"),
    (26597, "dyn_soft", 1, "dyn soft lamp (wave 26600)"),
    (26598, "milestone_tag", MILESTONE, "milestone 26600 product tag"),
    (26599, "continuum_wave", MILESTONE, "milestone 26600 continuum wave identity"),
    # 26600 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        26576: "continuum wave fill soft unit + batch id",
        26580: "continuum bridge soft unit + batch id",
        26590: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (26591–26599), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 26593
    tag_batch = MARKERS_LO + 7  # 26598
    wave_batch = MARKERS_LO + 8  # 26599
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_26500 / gj_shell_green_26400 / gj_shell_green_26300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_26500 / gj_libcgj_green_26400 / gj_libcgj_green_26300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_26500 / gj_bar3_ready_26400 / gj_bar3_ready_26300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_26500 / gj_product_score_26400 / gj_product_score_26300\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_26500 / gj_continuum_ready_26400 /\n"
            f" * gj_continuum_ready_26300 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_26500 / gj_smoke_soft_26400 / gj_smoke_soft_26300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_26500 / gj_dyn_soft_26400 / gj_dyn_soft_26300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_26500 / gj_milestone_tag_26400 / gj_milestone_tag_26300,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_26500 / gj_continuum_wave_26400 /\n"
            f" * gj_continuum_wave_26300, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch26600.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 26599
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 26591–26599; 26600 is capstone
