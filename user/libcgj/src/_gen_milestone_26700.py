...
"""
Soft continuum generator — MILESTONE 26700 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 26601–26700 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=26700   when parent has wired through graph_batch26700.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 26601 26700
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 26601 26700
...
graph batches ending at M. For M = **26700** the exclusive range is
**26601–26700** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 26601–26650  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 26651–26675  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 26676–26690  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 26691–26700  │ decade soft gates (Wave 125 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 26700 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_26700        1          graph_batch26691.c
  gj_libcgj_green_26700       1          graph_batch26692.c
  gj_bar3_ready_26700         0          graph_batch26693.c  (bar3 open)
  gj_product_score_26700      0          graph_batch26694.c  (score open)
  gj_continuum_ready_26700    1          graph_batch26695.c
  gj_smoke_soft_26700         1          graph_batch26696.c
  gj_dyn_soft_26700           1          graph_batch26697.c
  gj_milestone_tag_26700      26700      graph_batch26698.c
  gj_continuum_wave_26700     26700      graph_batch26699.c
  gj_batch_id_26700           26700      graph_batch26700.c
  gj_graph_milestone_26700    26700      graph_batch26700.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=26700`` means parent wired CGJ_SRCS through
    ``graph_batch26700.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_26700`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_26700`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M26700_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_26600`` / ``gj_*_26500`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 125 exclusive CREATE-ONLY lands the **markers** band
  (26691–26700) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_26700.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_26700.py

  # Restrict to marker band only (soft-gate TUs; Wave 125 exclusive):
  python3 user/libcgj/src/_gen_milestone_26700.py --band markers

  # Extension identity only (26601–26650):
  python3 user/libcgj/src/_gen_milestone_26700.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_26700.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_26600.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 26700 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 26700
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 26601
WAVE_HI = 26700  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 26601, 26650
IDENTITY_LO, IDENTITY_HI = 26651, 26675
FILL_LO, FILL_HI = 26676, 26690
MARKERS_LO, MARKERS_HI = 26691, 26700
...
# Stems become gj_<stem>_26700. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (26691, "shell_green", 1, "shell product green extract (wave 26700)"),
    (26692, "libcgj_green", 1, "libcgj product green extract (wave 26700)"),
    (26693, "bar3_ready", 0, "product bar3 readiness gate (wave 26700)"),
    (26694, "product_score", 0, "product readiness score tag (wave 26700)"),
    (26695, "continuum_ready", 1, "continuum readiness gate (wave 26700)"),
    (26696, "smoke_soft", 1, "smoke soft lamp (wave 26700)"),
    (26697, "dyn_soft", 1, "dyn soft lamp (wave 26700)"),
    (26698, "milestone_tag", MILESTONE, "milestone 26700 product tag"),
    (26699, "continuum_wave", MILESTONE, "milestone 26700 continuum wave identity"),
    # 26700 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        26676: "continuum wave fill soft unit + batch id",
        26680: "continuum bridge soft unit + batch id",
        26690: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (26691–26699), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 26693
    tag_batch = MARKERS_LO + 7  # 26698
    wave_batch = MARKERS_LO + 8  # 26699
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_26600 / gj_shell_green_26500 / gj_shell_green_26400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_26600 / gj_libcgj_green_26500 / gj_libcgj_green_26400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_26600 / gj_bar3_ready_26500 / gj_bar3_ready_26400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_26600 / gj_product_score_26500 / gj_product_score_26400\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_26600 / gj_continuum_ready_26500 /\n"
            f" * gj_continuum_ready_26400 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_26600 / gj_smoke_soft_26500 / gj_smoke_soft_26400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_26600 / gj_dyn_soft_26500 / gj_dyn_soft_26400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_26600 / gj_milestone_tag_26500 / gj_milestone_tag_26400,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_26600 / gj_continuum_wave_26500 /\n"
            f" * gj_continuum_wave_26400, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch26700.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 26699
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 26691–26699; 26700 is capstone
