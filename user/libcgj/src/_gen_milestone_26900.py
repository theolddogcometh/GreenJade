...
"""
Soft continuum generator — MILESTONE 26900 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 26801–26900 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=26900   when parent has wired through graph_batch26900.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 26801 26900
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 26801 26900
...
graph batches ending at M. For M = **26900** the exclusive range is
**26801–26900** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 26801–26850  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 26851–26875  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 26876–26890  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 26891–26900  │ decade soft gates (Wave 127 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 26900 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_26900        1          graph_batch26891.c
  gj_libcgj_green_26900       1          graph_batch26892.c
  gj_bar3_ready_26900         0          graph_batch26893.c  (bar3 open)
  gj_product_score_26900      0          graph_batch26894.c  (score open)
  gj_continuum_ready_26900    1          graph_batch26895.c
  gj_smoke_soft_26900         1          graph_batch26896.c
  gj_dyn_soft_26900           1          graph_batch26897.c
  gj_milestone_tag_26900      26900      graph_batch26898.c
  gj_continuum_wave_26900     26900      graph_batch26899.c
  gj_batch_id_26900           26900      graph_batch26900.c
  gj_graph_milestone_26900    26900      graph_batch26900.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=26900`` means parent wired CGJ_SRCS through
    ``graph_batch26900.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_26900`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_26900`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M26900_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_26800`` / ``gj_*_26700`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 127 exclusive CREATE-ONLY lands the **markers** band
  (26891–26900) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_26900.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_26900.py

  # Restrict to marker band only (soft-gate TUs; Wave 127 exclusive):
  python3 user/libcgj/src/_gen_milestone_26900.py --band markers

  # Extension identity only (26801–26850):
  python3 user/libcgj/src/_gen_milestone_26900.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_26900.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_26800.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 26900 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 26900
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 26801
WAVE_HI = 26900  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 26801, 26850
IDENTITY_LO, IDENTITY_HI = 26851, 26875
FILL_LO, FILL_HI = 26876, 26890
MARKERS_LO, MARKERS_HI = 26891, 26900
...
# Stems become gj_<stem>_26900. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (26891, "shell_green", 1, "shell product green extract (wave 26900)"),
    (26892, "libcgj_green", 1, "libcgj product green extract (wave 26900)"),
    (26893, "bar3_ready", 0, "product bar3 readiness gate (wave 26900)"),
    (26894, "product_score", 0, "product readiness score tag (wave 26900)"),
    (26895, "continuum_ready", 1, "continuum readiness gate (wave 26900)"),
    (26896, "smoke_soft", 1, "smoke soft lamp (wave 26900)"),
    (26897, "dyn_soft", 1, "dyn soft lamp (wave 26900)"),
    (26898, "milestone_tag", MILESTONE, "milestone 26900 product tag"),
    (26899, "continuum_wave", MILESTONE, "milestone 26900 continuum wave identity"),
    # 26900 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        26876: "continuum wave fill soft unit + batch id",
        26880: "continuum bridge soft unit + batch id",
        26890: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (26891–26899), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 26893
    tag_batch = MARKERS_LO + 7  # 26898
    wave_batch = MARKERS_LO + 8  # 26899
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_26800 / gj_shell_green_26700 / gj_shell_green_26600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_26800 / gj_libcgj_green_26700 / gj_libcgj_green_26600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_26800 / gj_bar3_ready_26700 / gj_bar3_ready_26600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_26800 / gj_product_score_26700 / gj_product_score_26600\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_26800 / gj_continuum_ready_26700 /\n"
            f" * gj_continuum_ready_26600 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_26800 / gj_smoke_soft_26700 / gj_smoke_soft_26600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_26800 / gj_dyn_soft_26700 / gj_dyn_soft_26600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_26800 / gj_milestone_tag_26700 / gj_milestone_tag_26600,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_26800 / gj_continuum_wave_26700 /\n"
            f" * gj_continuum_wave_26600, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch26900.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 26899
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 26891–26899; 26900 is capstone
