...
"""
Soft continuum generator — MILESTONE 27000 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 26901–27000 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=27000   when parent has wired through graph_batch27000.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 26901 27000
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 26901 27000
...
graph batches ending at M. For M = **27000** the exclusive range is
**26901–27000** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 26901–26950  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 26951–26975  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 26976–26990  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 26991–27000  │ decade soft gates (Wave 128 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 27000 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_27000        1          graph_batch26991.c
  gj_libcgj_green_27000       1          graph_batch26992.c
  gj_bar3_ready_27000         0          graph_batch26993.c  (bar3 open)
  gj_product_score_27000      0          graph_batch26994.c  (score open)
  gj_continuum_ready_27000    1          graph_batch26995.c
  gj_smoke_soft_27000         1          graph_batch26996.c
  gj_dyn_soft_27000           1          graph_batch26997.c
  gj_milestone_tag_27000      27000      graph_batch26998.c
  gj_continuum_wave_27000     27000      graph_batch26999.c
  gj_batch_id_27000           27000      graph_batch27000.c
  gj_graph_milestone_27000    27000      graph_batch27000.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=27000`` means parent wired CGJ_SRCS through
    ``graph_batch27000.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_27000`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_27000`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M27000_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_26900`` / ``gj_*_26800`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 128 exclusive CREATE-ONLY lands the **markers** band
  (26991–27000) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_27000.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_27000.py

  # Restrict to marker band only (soft-gate TUs; Wave 128 exclusive):
  python3 user/libcgj/src/_gen_milestone_27000.py --band markers

  # Extension identity only (26901–26950):
  python3 user/libcgj/src/_gen_milestone_27000.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_27000.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_26900.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 27000 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 27000
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 26901
WAVE_HI = 27000  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 26901, 26950
IDENTITY_LO, IDENTITY_HI = 26951, 26975
FILL_LO, FILL_HI = 26976, 26990
MARKERS_LO, MARKERS_HI = 26991, 27000
...
# Stems become gj_<stem>_27000. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (26991, "shell_green", 1, "shell product green extract (wave 27000)"),
    (26992, "libcgj_green", 1, "libcgj product green extract (wave 27000)"),
    (26993, "bar3_ready", 0, "product bar3 readiness gate (wave 27000)"),
    (26994, "product_score", 0, "product readiness score tag (wave 27000)"),
    (26995, "continuum_ready", 1, "continuum readiness gate (wave 27000)"),
    (26996, "smoke_soft", 1, "smoke soft lamp (wave 27000)"),
    (26997, "dyn_soft", 1, "dyn soft lamp (wave 27000)"),
    (26998, "milestone_tag", MILESTONE, "milestone 27000 product tag"),
    (26999, "continuum_wave", MILESTONE, "milestone 27000 continuum wave identity"),
    # 27000 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        26976: "continuum wave fill soft unit + batch id",
        26980: "continuum bridge soft unit + batch id",
        26990: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (26991–26999), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 26993
    tag_batch = MARKERS_LO + 7  # 26998
    wave_batch = MARKERS_LO + 8  # 26999
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_26900 / gj_shell_green_26800 / gj_shell_green_26700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_26900 / gj_libcgj_green_26800 / gj_libcgj_green_26700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_26900 / gj_bar3_ready_26800 / gj_bar3_ready_26700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_26900 / gj_product_score_26800 / gj_product_score_26700\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_26900 / gj_continuum_ready_26800 /\n"
            f" * gj_continuum_ready_26700 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_26900 / gj_smoke_soft_26800 / gj_smoke_soft_26700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_26900 / gj_dyn_soft_26800 / gj_dyn_soft_26700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_26900 / gj_milestone_tag_26800 / gj_milestone_tag_26700,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_26900 / gj_continuum_wave_26800 /\n"
            f" * gj_continuum_wave_26700, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch27000.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 26999
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 26991–26999; 27000 is capstone
