...
"""
Soft continuum generator — MILESTONE 27200 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 27101–27200 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=27200   when parent has wired through graph_batch27200.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 27101 27200
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 27101 27200
...
graph batches ending at M. For M = **27200** the exclusive range is
**27101–27200** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 27101–27150  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 27151–27175  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 27176–27190  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 27191–27200  │ decade soft gates (Wave 130 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 27200 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_27200        1          graph_batch27191.c
  gj_libcgj_green_27200       1          graph_batch27192.c
  gj_bar3_ready_27200         0          graph_batch27193.c  (bar3 open)
  gj_product_score_27200      0          graph_batch27194.c  (score open)
  gj_continuum_ready_27200    1          graph_batch27195.c
  gj_smoke_soft_27200         1          graph_batch27196.c
  gj_dyn_soft_27200           1          graph_batch27197.c
  gj_milestone_tag_27200      27200      graph_batch27198.c
  gj_continuum_wave_27200     27200      graph_batch27199.c
  gj_batch_id_27200           27200      graph_batch27200.c
  gj_graph_milestone_27200    27200      graph_batch27200.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=27200`` means parent wired CGJ_SRCS through
    ``graph_batch27200.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_27200`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_27200`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M27200_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_27100`` / ``gj_*_27000`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 130 exclusive CREATE-ONLY lands the **markers** band
  (27191–27200) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_27200.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_27200.py

  # Restrict to marker band only (soft-gate TUs; Wave 130 exclusive):
  python3 user/libcgj/src/_gen_milestone_27200.py --band markers

  # Extension identity only (27101–27150):
  python3 user/libcgj/src/_gen_milestone_27200.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_27200.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_27100.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 27200 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 27200
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 27101
WAVE_HI = 27200  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 27101, 27150
IDENTITY_LO, IDENTITY_HI = 27151, 27175
FILL_LO, FILL_HI = 27176, 27190
MARKERS_LO, MARKERS_HI = 27191, 27200
...
# Stems become gj_<stem>_27200. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (27191, "shell_green", 1, "shell product green extract (wave 27200)"),
    (27192, "libcgj_green", 1, "libcgj product green extract (wave 27200)"),
    (27193, "bar3_ready", 0, "product bar3 readiness gate (wave 27200)"),
    (27194, "product_score", 0, "product readiness score tag (wave 27200)"),
    (27195, "continuum_ready", 1, "continuum readiness gate (wave 27200)"),
    (27196, "smoke_soft", 1, "smoke soft lamp (wave 27200)"),
    (27197, "dyn_soft", 1, "dyn soft lamp (wave 27200)"),
    (27198, "milestone_tag", MILESTONE, "milestone 27200 product tag"),
    (27199, "continuum_wave", MILESTONE, "milestone 27200 continuum wave identity"),
    # 27200 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        27176: "continuum wave fill soft unit + batch id",
        27180: "continuum bridge soft unit + batch id",
        27190: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (27191–27199), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 27193
    tag_batch = MARKERS_LO + 7  # 27198
    wave_batch = MARKERS_LO + 8  # 27199
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_27100 / gj_shell_green_27000 / gj_shell_green_26900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_27100 / gj_libcgj_green_27000 / gj_libcgj_green_26900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_27100 / gj_bar3_ready_27000 / gj_bar3_ready_26900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_27100 / gj_product_score_27000 / gj_product_score_26900\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_27100 / gj_continuum_ready_27000 /\n"
            f" * gj_continuum_ready_26900 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_27100 / gj_smoke_soft_27000 / gj_smoke_soft_26900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_27100 / gj_dyn_soft_27000 / gj_dyn_soft_26900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_27100 / gj_milestone_tag_27000 / gj_milestone_tag_26900,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_27100 / gj_continuum_wave_27000 /\n"
            f" * gj_continuum_wave_26900, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch27200.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 27199
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 27191–27199; 27200 is capstone
