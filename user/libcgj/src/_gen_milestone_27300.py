...
"""
Soft continuum generator — MILESTONE 27300 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 27201–27300 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=27300   when parent has wired through graph_batch27300.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 27201 27300
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 27201 27300
...
graph batches ending at M. For M = **27300** the exclusive range is
**27201–27300** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 27201–27250  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 27251–27275  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 27276–27290  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 27291–27300  │ decade soft gates (Wave 131 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 27300 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_27300        1          graph_batch27291.c
  gj_libcgj_green_27300       1          graph_batch27292.c
  gj_bar3_ready_27300         0          graph_batch27293.c  (bar3 open)
  gj_product_score_27300      0          graph_batch27294.c  (score open)
  gj_continuum_ready_27300    1          graph_batch27295.c
  gj_smoke_soft_27300         1          graph_batch27296.c
  gj_dyn_soft_27300           1          graph_batch27297.c
  gj_milestone_tag_27300      27300      graph_batch27298.c
  gj_continuum_wave_27300     27300      graph_batch27299.c
  gj_batch_id_27300           27300      graph_batch27300.c
  gj_graph_milestone_27300    27300      graph_batch27300.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=27300`` means parent wired CGJ_SRCS through
    ``graph_batch27300.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_27300`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_27300`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M27300_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_27200`` / ``gj_*_27100`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 131 exclusive CREATE-ONLY lands the **markers** band
  (27291–27300) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_27300.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_27300.py

  # Restrict to marker band only (soft-gate TUs; Wave 131 exclusive):
  python3 user/libcgj/src/_gen_milestone_27300.py --band markers

  # Extension identity only (27201–27250):
  python3 user/libcgj/src/_gen_milestone_27300.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_27300.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_27200.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 27300 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 27300
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 27201
WAVE_HI = 27300  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 27201, 27250
IDENTITY_LO, IDENTITY_HI = 27251, 27275
FILL_LO, FILL_HI = 27276, 27290
MARKERS_LO, MARKERS_HI = 27291, 27300
...
# Stems become gj_<stem>_27300. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (27291, "shell_green", 1, "shell product green extract (wave 27300)"),
    (27292, "libcgj_green", 1, "libcgj product green extract (wave 27300)"),
    (27293, "bar3_ready", 0, "product bar3 readiness gate (wave 27300)"),
    (27294, "product_score", 0, "product readiness score tag (wave 27300)"),
    (27295, "continuum_ready", 1, "continuum readiness gate (wave 27300)"),
    (27296, "smoke_soft", 1, "smoke soft lamp (wave 27300)"),
    (27297, "dyn_soft", 1, "dyn soft lamp (wave 27300)"),
    (27298, "milestone_tag", MILESTONE, "milestone 27300 product tag"),
    (27299, "continuum_wave", MILESTONE, "milestone 27300 continuum wave identity"),
    # 27300 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        27276: "continuum wave fill soft unit + batch id",
        27280: "continuum bridge soft unit + batch id",
        27290: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (27291–27299), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 27293
    tag_batch = MARKERS_LO + 7  # 27298
    wave_batch = MARKERS_LO + 8  # 27299
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_27200 / gj_shell_green_27100 / gj_shell_green_27000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_27200 / gj_libcgj_green_27100 / gj_libcgj_green_27000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_27200 / gj_bar3_ready_27100 / gj_bar3_ready_27000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_27200 / gj_product_score_27100 / gj_product_score_27000\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_27200 / gj_continuum_ready_27100 /\n"
            f" * gj_continuum_ready_27000 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_27200 / gj_smoke_soft_27100 / gj_smoke_soft_27000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_27200 / gj_dyn_soft_27100 / gj_dyn_soft_27000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_27200 / gj_milestone_tag_27100 / gj_milestone_tag_27000,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_27200 / gj_continuum_wave_27100 /\n"
            f" * gj_continuum_wave_27000, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch27300.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 27299
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 27291–27299; 27300 is capstone
