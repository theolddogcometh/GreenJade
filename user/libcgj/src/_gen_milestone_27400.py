...
"""
Soft continuum generator — MILESTONE 27400 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 27301–27400 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=27400   when parent has wired through graph_batch27400.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 27301 27400
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 27301 27400
...
graph batches ending at M. For M = **27400** the exclusive range is
**27301–27400** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 27301–27350  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 27351–27375  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 27376–27390  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 27391–27400  │ decade soft gates (Wave 132 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 27400 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_27400        1          graph_batch27391.c
  gj_libcgj_green_27400       1          graph_batch27392.c
  gj_bar3_ready_27400         0          graph_batch27393.c  (bar3 open)
  gj_product_score_27400      0          graph_batch27394.c  (score open)
  gj_continuum_ready_27400    1          graph_batch27395.c
  gj_smoke_soft_27400         1          graph_batch27396.c
  gj_dyn_soft_27400           1          graph_batch27397.c
  gj_milestone_tag_27400      27400      graph_batch27398.c
  gj_continuum_wave_27400     27400      graph_batch27399.c
  gj_batch_id_27400           27400      graph_batch27400.c
  gj_graph_milestone_27400    27400      graph_batch27400.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=27400`` means parent wired CGJ_SRCS through
    ``graph_batch27400.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_27400`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_27400`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M27400_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_27300`` / ``gj_*_27200`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 132 exclusive CREATE-ONLY lands the **markers** band
  (27391–27400) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_27400.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_27400.py

  # Restrict to marker band only (soft-gate TUs; Wave 132 exclusive):
  python3 user/libcgj/src/_gen_milestone_27400.py --band markers

  # Extension identity only (27301–27350):
  python3 user/libcgj/src/_gen_milestone_27400.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_27400.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_27300.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 27400 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 27400
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 27301
WAVE_HI = 27400  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 27301, 27350
IDENTITY_LO, IDENTITY_HI = 27351, 27375
FILL_LO, FILL_HI = 27376, 27390
MARKERS_LO, MARKERS_HI = 27391, 27400
...
# Stems become gj_<stem>_27400. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (27391, "shell_green", 1, "shell product green extract (wave 27400)"),
    (27392, "libcgj_green", 1, "libcgj product green extract (wave 27400)"),
    (27393, "bar3_ready", 0, "product bar3 readiness gate (wave 27400)"),
    (27394, "product_score", 0, "product readiness score tag (wave 27400)"),
    (27395, "continuum_ready", 1, "continuum readiness gate (wave 27400)"),
    (27396, "smoke_soft", 1, "smoke soft lamp (wave 27400)"),
    (27397, "dyn_soft", 1, "dyn soft lamp (wave 27400)"),
    (27398, "milestone_tag", MILESTONE, "milestone 27400 product tag"),
    (27399, "continuum_wave", MILESTONE, "milestone 27400 continuum wave identity"),
    # 27400 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        27376: "continuum wave fill soft unit + batch id",
        27380: "continuum bridge soft unit + batch id",
        27390: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (27391–27399), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 27393
    tag_batch = MARKERS_LO + 7  # 27398
    wave_batch = MARKERS_LO + 8  # 27399
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_27300 / gj_shell_green_27200 / gj_shell_green_27100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_27300 / gj_libcgj_green_27200 / gj_libcgj_green_27100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_27300 / gj_bar3_ready_27200 / gj_bar3_ready_27100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_27300 / gj_product_score_27200 / gj_product_score_27100\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_27300 / gj_continuum_ready_27200 /\n"
            f" * gj_continuum_ready_27100 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_27300 / gj_smoke_soft_27200 / gj_smoke_soft_27100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_27300 / gj_dyn_soft_27200 / gj_dyn_soft_27100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_27300 / gj_milestone_tag_27200 / gj_milestone_tag_27100,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_27300 / gj_continuum_wave_27200 /\n"
            f" * gj_continuum_wave_27100, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch27400.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 27399
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 27391–27399; 27400 is capstone
