...
"""
Soft continuum generator — MILESTONE 27500 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 27401–27500 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=27500   when parent has wired through graph_batch27500.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 27401 27500
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 27401 27500
...
graph batches ending at M. For M = **27500** the exclusive range is
**27401–27500** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 27401–27450  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 27451–27475  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 27476–27490  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 27491–27500  │ decade soft gates (Wave 133 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 27500 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_27500        1          graph_batch27491.c
  gj_libcgj_green_27500       1          graph_batch27492.c
  gj_bar3_ready_27500         0          graph_batch27493.c  (bar3 open)
  gj_product_score_27500      0          graph_batch27494.c  (score open)
  gj_continuum_ready_27500    1          graph_batch27495.c
  gj_smoke_soft_27500         1          graph_batch27496.c
  gj_dyn_soft_27500           1          graph_batch27497.c
  gj_milestone_tag_27500      27500      graph_batch27498.c
  gj_continuum_wave_27500     27500      graph_batch27499.c
  gj_batch_id_27500           27500      graph_batch27500.c
  gj_graph_milestone_27500    27500      graph_batch27500.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=27500`` means parent wired CGJ_SRCS through
    ``graph_batch27500.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_27500`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_27500`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M27500_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_27400`` / ``gj_*_27300`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 133 exclusive CREATE-ONLY lands the **markers** band
  (27491–27500) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_27500.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_27500.py

  # Restrict to marker band only (soft-gate TUs; Wave 133 exclusive):
  python3 user/libcgj/src/_gen_milestone_27500.py --band markers

  # Extension identity only (27401–27450):
  python3 user/libcgj/src/_gen_milestone_27500.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_27500.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_27400.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 27500 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 27500
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 27401
WAVE_HI = 27500  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 27401, 27450
IDENTITY_LO, IDENTITY_HI = 27451, 27475
FILL_LO, FILL_HI = 27476, 27490
MARKERS_LO, MARKERS_HI = 27491, 27500
...
# Stems become gj_<stem>_27500. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (27491, "shell_green", 1, "shell product green extract (wave 27500)"),
    (27492, "libcgj_green", 1, "libcgj product green extract (wave 27500)"),
    (27493, "bar3_ready", 0, "product bar3 readiness gate (wave 27500)"),
    (27494, "product_score", 0, "product readiness score tag (wave 27500)"),
    (27495, "continuum_ready", 1, "continuum readiness gate (wave 27500)"),
    (27496, "smoke_soft", 1, "smoke soft lamp (wave 27500)"),
    (27497, "dyn_soft", 1, "dyn soft lamp (wave 27500)"),
    (27498, "milestone_tag", MILESTONE, "milestone 27500 product tag"),
    (27499, "continuum_wave", MILESTONE, "milestone 27500 continuum wave identity"),
    # 27500 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        27476: "continuum wave fill soft unit + batch id",
        27480: "continuum bridge soft unit + batch id",
        27490: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (27491–27499), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 27493
    tag_batch = MARKERS_LO + 7  # 27498
    wave_batch = MARKERS_LO + 8  # 27499
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_27400 / gj_shell_green_27300 / gj_shell_green_26900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_27400 / gj_libcgj_green_27300 / gj_libcgj_green_26900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_27400 / gj_bar3_ready_27300 / gj_bar3_ready_26900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_27400 / gj_product_score_27300 / gj_product_score_26900\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_27400 / gj_continuum_ready_27300 /\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_27400 / gj_smoke_soft_27300 / gj_smoke_soft_26900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_27400 / gj_dyn_soft_27300 / gj_dyn_soft_26900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_27400 / gj_milestone_tag_27300 / gj_milestone_tag_26900,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_27400 / gj_continuum_wave_27300 /\n"
...
def render_milestone_capstone() -> str:
    """graph_batch27500.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 27499
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 27491–27499; 27500 is capstone
