...
"""
Soft continuum generator — MILESTONE 27600 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 27501–27600 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=27600   when parent has wired through graph_batch27600.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 27501 27600
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 27501 27600
...
graph batches ending at M. For M = **27600** the exclusive range is
**27501–27600** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 27501–27550  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 27551–27575  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 27576–27590  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 27591–27600  │ decade soft gates (Wave 134 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 27600 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_27600        1          graph_batch27591.c
  gj_libcgj_green_27600       1          graph_batch27592.c
  gj_bar3_ready_27600         0          graph_batch27593.c  (bar3 open)
  gj_product_score_27600      0          graph_batch27594.c  (score open)
  gj_continuum_ready_27600    1          graph_batch27595.c
  gj_smoke_soft_27600         1          graph_batch27596.c
  gj_dyn_soft_27600           1          graph_batch27597.c
  gj_milestone_tag_27600      27600      graph_batch27598.c
  gj_continuum_wave_27600     27600      graph_batch27599.c
  gj_batch_id_27600           27600      graph_batch27600.c
  gj_graph_milestone_27600    27600      graph_batch27600.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=27600`` means parent wired CGJ_SRCS through
    ``graph_batch27600.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_27600`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_27600`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M27600_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_27500`` / ``gj_*_27400`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 134 exclusive CREATE-ONLY lands the **markers** band
  (27591–27600) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_27600.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_27600.py

  # Restrict to marker band only (soft-gate TUs; Wave 134 exclusive):
  python3 user/libcgj/src/_gen_milestone_27600.py --band markers

  # Extension identity only (27501–27550):
  python3 user/libcgj/src/_gen_milestone_27600.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_27600.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_27500.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 27600 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 27600
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 27501
WAVE_HI = 27600  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 27501, 27550
IDENTITY_LO, IDENTITY_HI = 27551, 27575
FILL_LO, FILL_HI = 27576, 27590
MARKERS_LO, MARKERS_HI = 27591, 27600
...
# Stems become gj_<stem>_27600. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (27591, "shell_green", 1, "shell product green extract (wave 27600)"),
    (27592, "libcgj_green", 1, "libcgj product green extract (wave 27600)"),
    (27593, "bar3_ready", 0, "product bar3 readiness gate (wave 27600)"),
    (27594, "product_score", 0, "product readiness score tag (wave 27600)"),
    (27595, "continuum_ready", 1, "continuum readiness gate (wave 27600)"),
    (27596, "smoke_soft", 1, "smoke soft lamp (wave 27600)"),
    (27597, "dyn_soft", 1, "dyn soft lamp (wave 27600)"),
    (27598, "milestone_tag", MILESTONE, "milestone 27600 product tag"),
    (27599, "continuum_wave", MILESTONE, "milestone 27600 continuum wave identity"),
    # 27600 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        27576: "continuum wave fill soft unit + batch id",
        27580: "continuum bridge soft unit + batch id",
        27590: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (27591–27599), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 27593
    tag_batch = MARKERS_LO + 7  # 27598
    wave_batch = MARKERS_LO + 8  # 27599
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_27500 / gj_shell_green_27400 / gj_shell_green_27300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_27500 / gj_libcgj_green_27400 / gj_libcgj_green_27300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_27500 / gj_bar3_ready_27400 / gj_bar3_ready_27300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_27500 / gj_product_score_27400 / gj_product_score_27300\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_27500 / gj_continuum_ready_27400 /\n"
            f" * gj_continuum_ready_27300 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_27500 / gj_smoke_soft_27400 / gj_smoke_soft_27300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_27500 / gj_dyn_soft_27400 / gj_dyn_soft_27300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_27500 / gj_milestone_tag_27400 / gj_milestone_tag_27300,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_27500 / gj_continuum_wave_27400 /\n"
            f" * gj_continuum_wave_27300, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch27600.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 27599
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 27591–27599; 27600 is capstone
