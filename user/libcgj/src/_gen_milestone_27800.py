...
"""
Soft continuum generator — MILESTONE 27800 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 27701–27800 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=27800   when parent has wired through graph_batch27800.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 27701 27800
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 27701 27800
...
graph batches ending at M. For M = **27800** the exclusive range is
**27701–27800** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 27701–27750  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 27751–27775  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 27776–27790  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 27791–27800  │ decade soft gates (Wave 136 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 27800 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_27800        1          graph_batch27791.c
  gj_libcgj_green_27800       1          graph_batch27792.c
  gj_bar3_ready_27800         0          graph_batch27793.c  (bar3 open)
  gj_product_score_27800      0          graph_batch27794.c  (score open)
  gj_continuum_ready_27800    1          graph_batch27795.c
  gj_smoke_soft_27800         1          graph_batch27796.c
  gj_dyn_soft_27800           1          graph_batch27797.c
  gj_milestone_tag_27800      27800      graph_batch27798.c
  gj_continuum_wave_27800     27800      graph_batch27799.c
  gj_batch_id_27800           27800      graph_batch27800.c
  gj_graph_milestone_27800    27800      graph_batch27800.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=27800`` means parent wired CGJ_SRCS through
    ``graph_batch27800.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_27800`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_27800`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M27800_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_27700`` / ``gj_*_27600`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 136 exclusive CREATE-ONLY lands the **markers** band
  (27791–27800) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_27800.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_27800.py

  # Restrict to marker band only (soft-gate TUs; Wave 136 exclusive):
  python3 user/libcgj/src/_gen_milestone_27800.py --band markers

  # Extension identity only (27701–27750):
  python3 user/libcgj/src/_gen_milestone_27800.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_27800.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_27700.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 27800 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 27800
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 27701
WAVE_HI = 27800  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 27701, 27750
IDENTITY_LO, IDENTITY_HI = 27751, 27775
FILL_LO, FILL_HI = 27776, 27790
MARKERS_LO, MARKERS_HI = 27791, 27800
...
# Stems become gj_<stem>_27800. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (27791, "shell_green", 1, "shell product green extract (wave 27800)"),
    (27792, "libcgj_green", 1, "libcgj product green extract (wave 27800)"),
    (27793, "bar3_ready", 0, "product bar3 readiness gate (wave 27800)"),
    (27794, "product_score", 0, "product readiness score tag (wave 27800)"),
    (27795, "continuum_ready", 1, "continuum readiness gate (wave 27800)"),
    (27796, "smoke_soft", 1, "smoke soft lamp (wave 27800)"),
    (27797, "dyn_soft", 1, "dyn soft lamp (wave 27800)"),
    (27798, "milestone_tag", MILESTONE, "milestone 27800 product tag"),
    (27799, "continuum_wave", MILESTONE, "milestone 27800 continuum wave identity"),
    # 27800 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        27776: "continuum wave fill soft unit + batch id",
        27780: "continuum bridge soft unit + batch id",
        27790: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (27791–27799), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 27793
    tag_batch = MARKERS_LO + 7  # 27798
    wave_batch = MARKERS_LO + 8  # 27799
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_27700 / gj_shell_green_27600 / gj_shell_green_27500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_27700 / gj_libcgj_green_27600 / gj_libcgj_green_27500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_27700 / gj_bar3_ready_27600 / gj_bar3_ready_27500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_27700 / gj_product_score_27600 / gj_product_score_27500\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_27700 / gj_continuum_ready_27600 /\n"
            f" * gj_continuum_ready_27500 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_27700 / gj_smoke_soft_27600 / gj_smoke_soft_27500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_27700 / gj_dyn_soft_27600 / gj_dyn_soft_27500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_27700 / gj_milestone_tag_27600 / gj_milestone_tag_27500,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_27700 / gj_continuum_wave_27600 /\n"
            f" * gj_continuum_wave_27500, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch27800.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 27799
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 27791–27799; 27800 is capstone
