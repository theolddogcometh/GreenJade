...
"""
Soft continuum generator — MILESTONE 27900 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 27801–27900 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=27900   when parent has wired through graph_batch27900.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 27801 27900
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 27801 27900
...
graph batches ending at M. For M = **27900** the exclusive range is
**27801–27900** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 27801–27850  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 27851–27875  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 27876–27890  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 27891–27900  │ decade soft gates (Wave 137 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 27900 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_27900        1          graph_batch27891.c
  gj_libcgj_green_27900       1          graph_batch27892.c
  gj_bar3_ready_27900         0          graph_batch27893.c  (bar3 open)
  gj_product_score_27900      0          graph_batch27894.c  (score open)
  gj_continuum_ready_27900    1          graph_batch27895.c
  gj_smoke_soft_27900         1          graph_batch27896.c
  gj_dyn_soft_27900           1          graph_batch27897.c
  gj_milestone_tag_27900      27900      graph_batch27898.c
  gj_continuum_wave_27900     27900      graph_batch27899.c
  gj_batch_id_27900           27900      graph_batch27900.c
  gj_graph_milestone_27900    27900      graph_batch27900.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=27900`` means parent wired CGJ_SRCS through
    ``graph_batch27900.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_27900`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_27900`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M27900_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_27800`` / ``gj_*_27700`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 137 exclusive CREATE-ONLY lands the **markers** band
  (27891–27900) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_27900.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_27900.py

  # Restrict to marker band only (soft-gate TUs; Wave 137 exclusive):
  python3 user/libcgj/src/_gen_milestone_27900.py --band markers

  # Extension identity only (27801–27850):
  python3 user/libcgj/src/_gen_milestone_27900.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_27900.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_27800.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 27900 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 27900
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 27801
WAVE_HI = 27900  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 27801, 27850
IDENTITY_LO, IDENTITY_HI = 27851, 27875
FILL_LO, FILL_HI = 27876, 27890
MARKERS_LO, MARKERS_HI = 27891, 27900
...
# Stems become gj_<stem>_27900. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (27891, "shell_green", 1, "shell product green extract (wave 27900)"),
    (27892, "libcgj_green", 1, "libcgj product green extract (wave 27900)"),
    (27893, "bar3_ready", 0, "product bar3 readiness gate (wave 27900)"),
    (27894, "product_score", 0, "product readiness score tag (wave 27900)"),
    (27895, "continuum_ready", 1, "continuum readiness gate (wave 27900)"),
    (27896, "smoke_soft", 1, "smoke soft lamp (wave 27900)"),
    (27897, "dyn_soft", 1, "dyn soft lamp (wave 27900)"),
    (27898, "milestone_tag", MILESTONE, "milestone 27900 product tag"),
    (27899, "continuum_wave", MILESTONE, "milestone 27900 continuum wave identity"),
    # 27900 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        27876: "continuum wave fill soft unit + batch id",
        27880: "continuum bridge soft unit + batch id",
        27890: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (27891–27899), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 27893
    tag_batch = MARKERS_LO + 7  # 27898
    wave_batch = MARKERS_LO + 8  # 27899
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_27800 / gj_shell_green_27700 / gj_shell_green_27600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_27800 / gj_libcgj_green_27700 / gj_libcgj_green_27600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_27800 / gj_bar3_ready_27700 / gj_bar3_ready_27600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_27800 / gj_product_score_27700 / gj_product_score_27600\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_27800 / gj_continuum_ready_27700 /\n"
            f" * gj_continuum_ready_27600 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_27800 / gj_smoke_soft_27700 / gj_smoke_soft_27600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_27800 / gj_dyn_soft_27700 / gj_dyn_soft_27600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_27800 / gj_milestone_tag_27700 / gj_milestone_tag_27600,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_27800 / gj_continuum_wave_27700 /\n"
            f" * gj_continuum_wave_27600, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch27900.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 27899
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 27891–27899; 27900 is capstone
