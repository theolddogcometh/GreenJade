...
"""
Soft continuum generator — MILESTONE 28000 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 27901–28000 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=28000   when parent has wired through graph_batch28000.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 27901 28000
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 27901 28000
...
graph batches ending at M. For M = **28000** the exclusive range is
**27901–28000** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 27901–27950  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 27951–27975  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 27976–27990  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 27991–28000  │ decade soft gates (Wave 138 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 28000 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_28000        1          graph_batch27991.c
  gj_libcgj_green_28000       1          graph_batch27992.c
  gj_bar3_ready_28000         0          graph_batch27993.c  (bar3 open)
  gj_product_score_28000      0          graph_batch27994.c  (score open)
  gj_continuum_ready_28000    1          graph_batch27995.c
  gj_smoke_soft_28000         1          graph_batch27996.c
  gj_dyn_soft_28000           1          graph_batch27997.c
  gj_milestone_tag_28000      28000      graph_batch27998.c
  gj_continuum_wave_28000     28000      graph_batch27999.c
  gj_batch_id_28000           28000      graph_batch28000.c
  gj_graph_milestone_28000    28000      graph_batch28000.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=28000`` means parent wired CGJ_SRCS through
    ``graph_batch28000.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_28000`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_28000`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M28000_DIRECT``.
...
--------------------
  ...
  Wave 138 exclusive CREATE-ONLY lands the **markers** band
  (27991–28000) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_28000.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_28000.py

  # Restrict to marker band only (soft-gate TUs; Wave 138 exclusive):
  python3 user/libcgj/src/_gen_milestone_28000.py --band markers

  # Extension identity only (27901–27950):
  python3 user/libcgj/src/_gen_milestone_28000.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_28000.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_27900.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 28000 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 28000
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 27901
WAVE_HI = 28000  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 27901, 27950
IDENTITY_LO, IDENTITY_HI = 27951, 27975
FILL_LO, FILL_HI = 27976, 27990
MARKERS_LO, MARKERS_HI = 27991, 28000
...
# Stems become gj_<stem>_28000. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (27991, "shell_green", 1, "shell product green extract (wave 28000)"),
    (27992, "libcgj_green", 1, "libcgj product green extract (wave 28000)"),
    (27993, "bar3_ready", 0, "product bar3 readiness gate (wave 28000)"),
    (27994, "product_score", 0, "product readiness score tag (wave 28000)"),
    (27995, "continuum_ready", 1, "continuum readiness gate (wave 28000)"),
    (27996, "smoke_soft", 1, "smoke soft lamp (wave 28000)"),
    (27997, "dyn_soft", 1, "dyn soft lamp (wave 28000)"),
    (27998, "milestone_tag", MILESTONE, "milestone 28000 product tag"),
    (27999, "continuum_wave", MILESTONE, "milestone 28000 continuum wave identity"),
    # 28000 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        27976: "continuum wave fill soft unit + batch id",
        27980: "continuum bridge soft unit + batch id",
        27990: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (27991–27999), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 27993
    tag_batch = MARKERS_LO + 7  # 27998
    wave_batch = MARKERS_LO + 8  # 27999
...
def render_milestone_capstone() -> str:
    """graph_batch28000.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 27999
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 27991–27999; 28000 is capstone
