...
"""
Soft continuum generator — MILESTONE 28100 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 28001–28100 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=28100   when parent has wired through graph_batch28100.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 28001 28100
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 28001 28100
...
graph batches ending at M. For M = **28100** the exclusive range is
**28001–28100** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 28001–28050  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 28051–28075  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 28076–28090  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 28091–28100  │ decade soft gates (Wave 139 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 28100 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_28100        1          graph_batch28091.c
  gj_libcgj_green_28100       1          graph_batch28092.c
  gj_bar3_ready_28100         0          graph_batch28093.c  (bar3 open)
  gj_product_score_28100      0          graph_batch28094.c  (score open)
  gj_continuum_ready_28100    1          graph_batch28095.c
  gj_smoke_soft_28100         1          graph_batch28096.c
  gj_dyn_soft_28100           1          graph_batch28097.c
  gj_milestone_tag_28100      28100      graph_batch28098.c
  gj_continuum_wave_28100     28100      graph_batch28099.c
  gj_batch_id_28100           28100      graph_batch28100.c
  gj_graph_milestone_28100    28100      graph_batch28100.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=28100`` means parent wired CGJ_SRCS through
    ``graph_batch28100.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_28100`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_28100`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M28100_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_27900`` / ``gj_*_27800`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 139 exclusive CREATE-ONLY lands the **markers** band
  (28091–28100) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_28100.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_28100.py

  # Restrict to marker band only (soft-gate TUs; Wave 139 exclusive):
  python3 user/libcgj/src/_gen_milestone_28100.py --band markers

  # Extension identity only (28001–28050):
  python3 user/libcgj/src/_gen_milestone_28100.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_28100.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_28000.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 28100 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 28100
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 28001
WAVE_HI = 28100  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 28001, 28050
IDENTITY_LO, IDENTITY_HI = 28051, 28075
FILL_LO, FILL_HI = 28076, 28090
MARKERS_LO, MARKERS_HI = 28091, 28100
...
# Stems become gj_<stem>_28100. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (28091, "shell_green", 1, "shell product green extract (wave 28100)"),
    (28092, "libcgj_green", 1, "libcgj product green extract (wave 28100)"),
    (28093, "bar3_ready", 0, "product bar3 readiness gate (wave 28100)"),
    (28094, "product_score", 0, "product readiness score tag (wave 28100)"),
    (28095, "continuum_ready", 1, "continuum readiness gate (wave 28100)"),
    (28096, "smoke_soft", 1, "smoke soft lamp (wave 28100)"),
    (28097, "dyn_soft", 1, "dyn soft lamp (wave 28100)"),
    (28098, "milestone_tag", MILESTONE, "milestone 28100 product tag"),
    (28099, "continuum_wave", MILESTONE, "milestone 28100 continuum wave identity"),
    # 28100 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        28076: "continuum wave fill soft unit + batch id",
        28080: "continuum bridge soft unit + batch id",
        28090: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (28091–28099), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 28093
    tag_batch = MARKERS_LO + 7  # 28098
    wave_batch = MARKERS_LO + 8  # 28099
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_27900 / gj_shell_green_27800 / gj_shell_green_27700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_27900 / gj_libcgj_green_27800 / gj_libcgj_green_27700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_27900 / gj_bar3_ready_27800 / gj_bar3_ready_27700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_27900 / gj_product_score_27800 / gj_product_score_27700\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_27900 / gj_continuum_ready_27800 /\n"
            f" * gj_continuum_ready_27700 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_27900 / gj_smoke_soft_27800 / gj_smoke_soft_27700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_27900 / gj_dyn_soft_27800 / gj_dyn_soft_27700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_27900 / gj_milestone_tag_27800 / gj_milestone_tag_27700,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_27900 / gj_continuum_wave_27800 /\n"
            f" * gj_continuum_wave_27700, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch28100.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 28099
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 28091–28099; 28100 is capstone
