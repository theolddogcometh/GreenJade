...
"""
Soft continuum generator — MILESTONE 28200 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 28101–28200 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=28200   when parent has wired through graph_batch28200.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 28101 28200
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 28101 28200
...
graph batches ending at M. For M = **28200** the exclusive range is
**28101–28200** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 28101–28150  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 28151–28175  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 28176–28190  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 28191–28200  │ decade soft gates (Wave 140 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 28200 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_28200        1          graph_batch28191.c
  gj_libcgj_green_28200       1          graph_batch28192.c
  gj_bar3_ready_28200         0          graph_batch28193.c  (bar3 open)
  gj_product_score_28200      0          graph_batch28194.c  (score open)
  gj_continuum_ready_28200    1          graph_batch28195.c
  gj_smoke_soft_28200         1          graph_batch28196.c
  gj_dyn_soft_28200           1          graph_batch28197.c
  gj_milestone_tag_28200      28200      graph_batch28198.c
  gj_continuum_wave_28200     28200      graph_batch28199.c
  gj_batch_id_28200           28200      graph_batch28200.c
  gj_graph_milestone_28200    28200      graph_batch28200.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=28200`` means parent wired CGJ_SRCS through
    ``graph_batch28200.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_28200`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_28200`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M28200_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_28100`` / ``gj_*_28000`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 140 exclusive CREATE-ONLY lands the **markers** band
  (28191–28200) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_28200.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_28200.py

  # Restrict to marker band only (soft-gate TUs; Wave 140 exclusive):
  python3 user/libcgj/src/_gen_milestone_28200.py --band markers

  # Extension identity only (28101–28150):
  python3 user/libcgj/src/_gen_milestone_28200.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_28200.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_28100.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 28200 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 28200
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 28101
WAVE_HI = 28200  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 28101, 28150
IDENTITY_LO, IDENTITY_HI = 28151, 28175
FILL_LO, FILL_HI = 28176, 28190
MARKERS_LO, MARKERS_HI = 28191, 28200
...
# Stems become gj_<stem>_28200. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (28191, "shell_green", 1, "shell product green extract (wave 28200)"),
    (28192, "libcgj_green", 1, "libcgj product green extract (wave 28200)"),
    (28193, "bar3_ready", 0, "product bar3 readiness gate (wave 28200)"),
    (28194, "product_score", 0, "product readiness score tag (wave 28200)"),
    (28195, "continuum_ready", 1, "continuum readiness gate (wave 28200)"),
    (28196, "smoke_soft", 1, "smoke soft lamp (wave 28200)"),
    (28197, "dyn_soft", 1, "dyn soft lamp (wave 28200)"),
    (28198, "milestone_tag", MILESTONE, "milestone 28200 product tag"),
    (28199, "continuum_wave", MILESTONE, "milestone 28200 continuum wave identity"),
    # 28200 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        28176: "continuum wave fill soft unit + batch id",
        28180: "continuum bridge soft unit + batch id",
        28190: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (28191–28199), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 28193
    tag_batch = MARKERS_LO + 7  # 28198
    wave_batch = MARKERS_LO + 8  # 28199
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_28100 / gj_shell_green_28000 / gj_shell_green_27900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_28100 / gj_libcgj_green_28000 / gj_libcgj_green_27900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_28100 / gj_bar3_ready_28000 / gj_bar3_ready_27900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_28100 / gj_product_score_28000 / gj_product_score_27900\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_28100 / gj_continuum_ready_28000 /\n"
            f" * gj_continuum_ready_27900 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_28100 / gj_smoke_soft_28000 / gj_smoke_soft_27900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_28100 / gj_dyn_soft_28000 / gj_dyn_soft_27900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_28100 / gj_milestone_tag_28000 / gj_milestone_tag_27900,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_28100 / gj_continuum_wave_28000 /\n"
            f" * gj_continuum_wave_27900, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch28200.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 28199
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 28191–28199; 28200 is capstone
