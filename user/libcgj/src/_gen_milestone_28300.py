...
"""
Soft continuum generator — MILESTONE 28300 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 28201–28300 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=28300   when parent has wired through graph_batch28300.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 28201 28300
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 28201 28300
...
graph batches ending at M. For M = **28300** the exclusive range is
**28201–28300** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 28201–28250  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 28251–28275  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 28276–28290  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 28291–28300  │ decade soft gates (Wave 141 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 28300 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_28300        1          graph_batch28291.c
  gj_libcgj_green_28300       1          graph_batch28292.c
  gj_bar3_ready_28300         0          graph_batch28293.c  (bar3 open)
  gj_product_score_28300      0          graph_batch28294.c  (score open)
  gj_continuum_ready_28300    1          graph_batch28295.c
  gj_smoke_soft_28300         1          graph_batch28296.c
  gj_dyn_soft_28300           1          graph_batch28297.c
  gj_milestone_tag_28300      28300      graph_batch28298.c
  gj_continuum_wave_28300     28300      graph_batch28299.c
  gj_batch_id_28300           28300      graph_batch28300.c
  gj_graph_milestone_28300    28300      graph_batch28300.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=28300`` means parent wired CGJ_SRCS through
    ``graph_batch28300.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_28300`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_28300`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M28300_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_28200`` / ``gj_*_28100`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 141 exclusive CREATE-ONLY lands the **markers** band
  (28291–28300) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_28300.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_28300.py

  # Restrict to marker band only (soft-gate TUs; Wave 141 exclusive):
  python3 user/libcgj/src/_gen_milestone_28300.py --band markers

  # Extension identity only (28201–28250):
  python3 user/libcgj/src/_gen_milestone_28300.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_28300.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_28200.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 28300 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 28300
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 28201
WAVE_HI = 28300  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 28201, 28250
IDENTITY_LO, IDENTITY_HI = 28251, 28275
FILL_LO, FILL_HI = 28276, 28290
MARKERS_LO, MARKERS_HI = 28291, 28300
...
# Stems become gj_<stem>_28300. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (28291, "shell_green", 1, "shell product green extract (wave 28300)"),
    (28292, "libcgj_green", 1, "libcgj product green extract (wave 28300)"),
    (28293, "bar3_ready", 0, "product bar3 readiness gate (wave 28300)"),
    (28294, "product_score", 0, "product readiness score tag (wave 28300)"),
    (28295, "continuum_ready", 1, "continuum readiness gate (wave 28300)"),
    (28296, "smoke_soft", 1, "smoke soft lamp (wave 28300)"),
    (28297, "dyn_soft", 1, "dyn soft lamp (wave 28300)"),
    (28298, "milestone_tag", MILESTONE, "milestone 28300 product tag"),
    (28299, "continuum_wave", MILESTONE, "milestone 28300 continuum wave identity"),
    # 28300 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        28276: "continuum wave fill soft unit + batch id",
        28280: "continuum bridge soft unit + batch id",
        28290: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (28291–28299), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 28293
    tag_batch = MARKERS_LO + 7  # 28298
    wave_batch = MARKERS_LO + 8  # 28299
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_28200 / gj_shell_green_28100 / gj_shell_green_28000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_28200 / gj_libcgj_green_28100 / gj_libcgj_green_28000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_28200 / gj_bar3_ready_28100 / gj_bar3_ready_28000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_28200 / gj_product_score_28100 / gj_product_score_28000\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_28200 / gj_continuum_ready_28100 /\n"
            f" * gj_continuum_ready_28000 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_28200 / gj_smoke_soft_28100 / gj_smoke_soft_28000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_28200 / gj_dyn_soft_28100 / gj_dyn_soft_28000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_28200 / gj_milestone_tag_28100 / gj_milestone_tag_28000,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_28200 / gj_continuum_wave_28100 /\n"
            f" * gj_continuum_wave_28000, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch28300.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 28299
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 28291–28299; 28300 is capstone
