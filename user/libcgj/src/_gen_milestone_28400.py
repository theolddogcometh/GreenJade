...
"""
Soft continuum generator — MILESTONE 28400 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 28301–28400 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=28400   when parent has wired through graph_batch28400.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 28301 28400
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 28301 28400
...
graph batches ending at M. For M = **28400** the exclusive range is
**28301–28400** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 28301–28350  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 28351–28375  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 28376–28390  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 28391–28400  │ decade soft gates (Wave 142 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 28400 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_28400        1          graph_batch28391.c
  gj_libcgj_green_28400       1          graph_batch28392.c
  gj_bar3_ready_28400         0          graph_batch28393.c  (bar3 open)
  gj_product_score_28400      0          graph_batch28394.c  (score open)
  gj_continuum_ready_28400    1          graph_batch28395.c
  gj_smoke_soft_28400         1          graph_batch28396.c
  gj_dyn_soft_28400           1          graph_batch28397.c
  gj_milestone_tag_28400      28400      graph_batch28398.c
  gj_continuum_wave_28400     28400      graph_batch28399.c
  gj_batch_id_28400           28400      graph_batch28400.c
  gj_graph_milestone_28400    28400      graph_batch28400.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=28400`` means parent wired CGJ_SRCS through
    ``graph_batch28400.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_28400`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_28400`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M28400_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_28300`` / ``gj_*_28200`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 142 exclusive CREATE-ONLY lands the **markers** band
  (28391–28400) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_28400.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_28400.py

  # Restrict to marker band only (soft-gate TUs; Wave 142 exclusive):
  python3 user/libcgj/src/_gen_milestone_28400.py --band markers

  # Extension identity only (28301–28350):
  python3 user/libcgj/src/_gen_milestone_28400.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_28400.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_28300.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 28400 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 28400
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 28301
WAVE_HI = 28400  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 28301, 28350
IDENTITY_LO, IDENTITY_HI = 28351, 28375
FILL_LO, FILL_HI = 28376, 28390
MARKERS_LO, MARKERS_HI = 28391, 28400
...
# Stems become gj_<stem>_28400. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (28391, "shell_green", 1, "shell product green extract (wave 28400)"),
    (28392, "libcgj_green", 1, "libcgj product green extract (wave 28400)"),
    (28393, "bar3_ready", 0, "product bar3 readiness gate (wave 28400)"),
    (28394, "product_score", 0, "product readiness score tag (wave 28400)"),
    (28395, "continuum_ready", 1, "continuum readiness gate (wave 28400)"),
    (28396, "smoke_soft", 1, "smoke soft lamp (wave 28400)"),
    (28397, "dyn_soft", 1, "dyn soft lamp (wave 28400)"),
    (28398, "milestone_tag", MILESTONE, "milestone 28400 product tag"),
    (28399, "continuum_wave", MILESTONE, "milestone 28400 continuum wave identity"),
    # 28400 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        28376: "continuum wave fill soft unit + batch id",
        28380: "continuum bridge soft unit + batch id",
        28390: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (28391–28399), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 28393
    tag_batch = MARKERS_LO + 7  # 28398
    wave_batch = MARKERS_LO + 8  # 28399
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_28300 / gj_shell_green_28200 / gj_shell_green_28100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_28300 / gj_libcgj_green_28200 / gj_libcgj_green_28100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_28300 / gj_bar3_ready_28200 / gj_bar3_ready_28100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_28300 / gj_product_score_28200 / gj_product_score_28100\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_28300 / gj_continuum_ready_28200 /\n"
            f" * gj_continuum_ready_28100 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_28300 / gj_smoke_soft_28200 / gj_smoke_soft_28100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_28300 / gj_dyn_soft_28200 / gj_dyn_soft_28100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_28300 / gj_milestone_tag_28200 / gj_milestone_tag_28100,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_28300 / gj_continuum_wave_28200 /\n"
            f" * gj_continuum_wave_28100, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch28400.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 28399
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 28391–28399; 28400 is capstone
