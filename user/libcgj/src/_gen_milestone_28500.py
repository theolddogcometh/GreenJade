...
"""
Soft continuum generator — MILESTONE 28500 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 28401–28500 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=28500   when parent has wired through graph_batch28500.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 28401 28500
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 28401 28500
...
graph batches ending at M. For M = **28500** the exclusive range is
**28401–28500** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 28401–28450  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 28451–28475  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 28476–28490  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 28491–28500  │ decade soft gates (Wave 143 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 28500 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_28500        1          graph_batch28491.c
  gj_libcgj_green_28500       1          graph_batch28492.c
  gj_bar3_ready_28500         0          graph_batch28493.c  (bar3 open)
  gj_product_score_28500      0          graph_batch28494.c  (score open)
  gj_continuum_ready_28500    1          graph_batch28495.c
  gj_smoke_soft_28500         1          graph_batch28496.c
  gj_dyn_soft_28500           1          graph_batch28497.c
  gj_milestone_tag_28500      28500      graph_batch28498.c
  gj_continuum_wave_28500     28500      graph_batch28499.c
  gj_batch_id_28500           28500      graph_batch28500.c
  gj_graph_milestone_28500    28500      graph_batch28500.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=28500`` means parent wired CGJ_SRCS through
    ``graph_batch28500.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_28500`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_28500`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M28500_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_28400`` / ``gj_*_28300`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 143 exclusive CREATE-ONLY lands the **markers** band
  (28491–28500) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_28500.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_28500.py

  # Restrict to marker band only (soft-gate TUs; Wave 143 exclusive):
  python3 user/libcgj/src/_gen_milestone_28500.py --band markers

  # Extension identity only (28401–28450):
  python3 user/libcgj/src/_gen_milestone_28500.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_28500.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_28400.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 28500 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 28500
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 28401
WAVE_HI = 28500  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 28401, 28450
IDENTITY_LO, IDENTITY_HI = 28451, 28475
FILL_LO, FILL_HI = 28476, 28490
MARKERS_LO, MARKERS_HI = 28491, 28500
...
# Stems become gj_<stem>_28500. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (28491, "shell_green", 1, "shell product green extract (wave 28500)"),
    (28492, "libcgj_green", 1, "libcgj product green extract (wave 28500)"),
    (28493, "bar3_ready", 0, "product bar3 readiness gate (wave 28500)"),
    (28494, "product_score", 0, "product readiness score tag (wave 28500)"),
    (28495, "continuum_ready", 1, "continuum readiness gate (wave 28500)"),
    (28496, "smoke_soft", 1, "smoke soft lamp (wave 28500)"),
    (28497, "dyn_soft", 1, "dyn soft lamp (wave 28500)"),
    (28498, "milestone_tag", MILESTONE, "milestone 28500 product tag"),
    (28499, "continuum_wave", MILESTONE, "milestone 28500 continuum wave identity"),
    # 28500 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        28476: "continuum wave fill soft unit + batch id",
        28480: "continuum bridge soft unit + batch id",
        28490: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (28491–28499), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 28493
    tag_batch = MARKERS_LO + 7  # 28498
    wave_batch = MARKERS_LO + 8  # 28499
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_28400 / gj_shell_green_28300 / gj_shell_green_28200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_28400 / gj_libcgj_green_28300 / gj_libcgj_green_28200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_28400 / gj_bar3_ready_28300 / gj_bar3_ready_28200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_28400 / gj_product_score_28300 / gj_product_score_28200\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_28400 / gj_continuum_ready_28300 /\n"
            f" * gj_continuum_ready_28200 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_28400 / gj_smoke_soft_28300 / gj_smoke_soft_28200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_28400 / gj_dyn_soft_28300 / gj_dyn_soft_28200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_28400 / gj_milestone_tag_28300 / gj_milestone_tag_28200,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_28400 / gj_continuum_wave_28300 /\n"
            f" * gj_continuum_wave_28200, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch28500.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 28499
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 28491–28499; 28500 is capstone
