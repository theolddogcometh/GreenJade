...
"""
Soft continuum generator — MILESTONE 28600 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 28501–28600 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=28600   when parent has wired through graph_batch28600.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 28501 28600
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 28501 28600
...
graph batches ending at M. For M = **28600** the exclusive range is
**28501–28600** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 28501–28550  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 28551–28575  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 28576–28590  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 28591–28600  │ decade soft gates (Wave 144 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 28600 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_28600        1          graph_batch28591.c
  gj_libcgj_green_28600       1          graph_batch28592.c
  gj_bar3_ready_28600         0          graph_batch28593.c  (bar3 open)
  gj_product_score_28600      0          graph_batch28594.c  (score open)
  gj_continuum_ready_28600    1          graph_batch28595.c
  gj_smoke_soft_28600         1          graph_batch28596.c
  gj_dyn_soft_28600           1          graph_batch28597.c
  gj_milestone_tag_28600      28600      graph_batch28598.c
  gj_continuum_wave_28600     28600      graph_batch28599.c
  gj_batch_id_28600           28600      graph_batch28600.c
  gj_graph_milestone_28600    28600      graph_batch28600.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=28600`` means parent wired CGJ_SRCS through
    ``graph_batch28600.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_28600`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_28600`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M28600_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_28500`` / ``gj_*_28400`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 144 exclusive CREATE-ONLY lands the **markers** band
  (28591–28600) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_28600.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_28600.py

  # Restrict to marker band only (soft-gate TUs; Wave 144 exclusive):
  python3 user/libcgj/src/_gen_milestone_28600.py --band markers

  # Extension identity only (28501–28550):
  python3 user/libcgj/src/_gen_milestone_28600.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_28600.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_28500.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 28600 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 28600
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 28501
WAVE_HI = 28600  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 28501, 28550
IDENTITY_LO, IDENTITY_HI = 28551, 28575
FILL_LO, FILL_HI = 28576, 28590
MARKERS_LO, MARKERS_HI = 28591, 28600
...
# Stems become gj_<stem>_28600. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (28591, "shell_green", 1, "shell product green extract (wave 28600)"),
    (28592, "libcgj_green", 1, "libcgj product green extract (wave 28600)"),
    (28593, "bar3_ready", 0, "product bar3 readiness gate (wave 28600)"),
    (28594, "product_score", 0, "product readiness score tag (wave 28600)"),
    (28595, "continuum_ready", 1, "continuum readiness gate (wave 28600)"),
    (28596, "smoke_soft", 1, "smoke soft lamp (wave 28600)"),
    (28597, "dyn_soft", 1, "dyn soft lamp (wave 28600)"),
    (28598, "milestone_tag", MILESTONE, "milestone 28600 product tag"),
    (28599, "continuum_wave", MILESTONE, "milestone 28600 continuum wave identity"),
    # 28600 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        28576: "continuum wave fill soft unit + batch id",
        28580: "continuum bridge soft unit + batch id",
        28590: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (28591–28599), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 28593
    tag_batch = MARKERS_LO + 7  # 28598
    wave_batch = MARKERS_LO + 8  # 28599
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_28500 / gj_shell_green_28400 / gj_shell_green_28300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_28500 / gj_libcgj_green_28400 / gj_libcgj_green_28300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_28500 / gj_bar3_ready_28400 / gj_bar3_ready_28300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_28500 / gj_product_score_28400 / gj_product_score_28300\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_28500 / gj_continuum_ready_28400 /\n"
            f" * gj_continuum_ready_28300 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_28500 / gj_smoke_soft_28400 / gj_smoke_soft_28300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_28500 / gj_dyn_soft_28400 / gj_dyn_soft_28300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_28500 / gj_milestone_tag_28400 / gj_milestone_tag_28300,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_28500 / gj_continuum_wave_28400 /\n"
            f" * gj_continuum_wave_28300, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch28600.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 28599
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 28591–28599; 28600 is capstone
