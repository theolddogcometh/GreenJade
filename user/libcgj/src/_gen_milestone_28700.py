...
"""
Soft continuum generator — MILESTONE 28700 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 28601–28700 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=28700   when parent has wired through graph_batch28700.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 28601 28700
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 28601 28700
...
graph batches ending at M. For M = **28700** the exclusive range is
**28601–28700** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 28601–28650  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 28651–28675  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 28676–28690  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 28691–28700  │ decade soft gates (Wave 145 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 28700 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_28700        1          graph_batch28691.c
  gj_libcgj_green_28700       1          graph_batch28692.c
  gj_bar3_ready_28700         0          graph_batch28693.c  (bar3 open)
  gj_product_score_28700      0          graph_batch28694.c  (score open)
  gj_continuum_ready_28700    1          graph_batch28695.c
  gj_smoke_soft_28700         1          graph_batch28696.c
  gj_dyn_soft_28700           1          graph_batch28697.c
  gj_milestone_tag_28700      28700      graph_batch28698.c
  gj_continuum_wave_28700     28700      graph_batch28699.c
  gj_batch_id_28700           28700      graph_batch28700.c
  gj_graph_milestone_28700    28700      graph_batch28700.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=28700`` means parent wired CGJ_SRCS through
    ``graph_batch28700.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_28700`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_28700`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M28700_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_28600`` / ``gj_*_28500`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 145 exclusive CREATE-ONLY lands the **markers** band
  (28691–28700) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_28700.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_28700.py

  # Restrict to marker band only (soft-gate TUs; Wave 145 exclusive):
  python3 user/libcgj/src/_gen_milestone_28700.py --band markers

  # Extension identity only (28601–28650):
  python3 user/libcgj/src/_gen_milestone_28700.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_28700.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_28600.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 28700 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 28700
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 28601
WAVE_HI = 28700  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 28601, 28650
IDENTITY_LO, IDENTITY_HI = 28651, 28675
FILL_LO, FILL_HI = 28676, 28690
MARKERS_LO, MARKERS_HI = 28691, 28700
...
# Stems become gj_<stem>_28700. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (28691, "shell_green", 1, "shell product green extract (wave 28700)"),
    (28692, "libcgj_green", 1, "libcgj product green extract (wave 28700)"),
    (28693, "bar3_ready", 0, "product bar3 readiness gate (wave 28700)"),
    (28694, "product_score", 0, "product readiness score tag (wave 28700)"),
    (28695, "continuum_ready", 1, "continuum readiness gate (wave 28700)"),
    (28696, "smoke_soft", 1, "smoke soft lamp (wave 28700)"),
    (28697, "dyn_soft", 1, "dyn soft lamp (wave 28700)"),
    (28698, "milestone_tag", MILESTONE, "milestone 28700 product tag"),
    (28699, "continuum_wave", MILESTONE, "milestone 28700 continuum wave identity"),
    # 28700 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        28676: "continuum wave fill soft unit + batch id",
        28680: "continuum bridge soft unit + batch id",
        28690: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (28691–28699), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 28693
    tag_batch = MARKERS_LO + 7  # 28698
    wave_batch = MARKERS_LO + 8  # 28699
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_28600 / gj_shell_green_28500 / gj_shell_green_28400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_28600 / gj_libcgj_green_28500 / gj_libcgj_green_28400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_28600 / gj_bar3_ready_28500 / gj_bar3_ready_28400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_28600 / gj_product_score_28500 / gj_product_score_28400\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_28600 / gj_continuum_ready_28500 /\n"
            f" * gj_continuum_ready_28400 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_28600 / gj_smoke_soft_28500 / gj_smoke_soft_28400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_28600 / gj_dyn_soft_28500 / gj_dyn_soft_28400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_28600 / gj_milestone_tag_28500 / gj_milestone_tag_28400,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_28600 / gj_continuum_wave_28500 /\n"
            f" * gj_continuum_wave_28400, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch28700.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 28699
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 28691–28699; 28700 is capstone
