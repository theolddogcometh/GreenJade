...
"""
Soft continuum generator — MILESTONE 28800 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 28701–28800 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=28800   when parent has wired through graph_batch28800.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 28701 28800
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 28701 28800
...
graph batches ending at M. For M = **28800** the exclusive range is
**28701–28800** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 28701–28750  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 28751–28775  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 28776–28790  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 28791–28800  │ decade soft gates (Wave 146 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 28800 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_28800        1          graph_batch28791.c
  gj_libcgj_green_28800       1          graph_batch28792.c
  gj_bar3_ready_28800         0          graph_batch28793.c  (bar3 open)
  gj_product_score_28800      0          graph_batch28794.c  (score open)
  gj_continuum_ready_28800    1          graph_batch28795.c
  gj_smoke_soft_28800         1          graph_batch28796.c
  gj_dyn_soft_28800           1          graph_batch28797.c
  gj_milestone_tag_28800      28800      graph_batch28798.c
  gj_continuum_wave_28800     28800      graph_batch28799.c
  gj_batch_id_28800           28800      graph_batch28800.c
  gj_graph_milestone_28800    28800      graph_batch28800.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=28800`` means parent wired CGJ_SRCS through
    ``graph_batch28800.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_28800`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_28800`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M28800_DIRECT``.
...
--------------------
  ...
  Wave 146 exclusive CREATE-ONLY lands the **markers** band
  (28791–28800) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_28800.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_28800.py

  # Restrict to marker band only (soft-gate TUs; Wave 146 exclusive):
  python3 user/libcgj/src/_gen_milestone_28800.py --band markers

  # Extension identity only (28701–28750):
  python3 user/libcgj/src/_gen_milestone_28800.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_28800.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_28700.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 28800 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 28800
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 28701
WAVE_HI = 28800  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 28701, 28750
IDENTITY_LO, IDENTITY_HI = 28751, 28775
FILL_LO, FILL_HI = 28776, 28790
MARKERS_LO, MARKERS_HI = 28791, 28800
...
# Stems become gj_<stem>_28800. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (28791, "shell_green", 1, "shell product green extract (wave 28800)"),
    (28792, "libcgj_green", 1, "libcgj product green extract (wave 28800)"),
    (28793, "bar3_ready", 0, "product bar3 readiness gate (wave 28800)"),
    (28794, "product_score", 0, "product readiness score tag (wave 28800)"),
    (28795, "continuum_ready", 1, "continuum readiness gate (wave 28800)"),
    (28796, "smoke_soft", 1, "smoke soft lamp (wave 28800)"),
    (28797, "dyn_soft", 1, "dyn soft lamp (wave 28800)"),
    (28798, "milestone_tag", MILESTONE, "milestone 28800 product tag"),
    (28799, "continuum_wave", MILESTONE, "milestone 28800 continuum wave identity"),
    # 28800 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        28776: "continuum wave fill soft unit + batch id",
        28780: "continuum bridge soft unit + batch id",
        28790: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (28791–28799), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 28793
    tag_batch = MARKERS_LO + 7  # 28798
    wave_batch = MARKERS_LO + 8  # 28799
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_28700 / gj_bar3_ready_28600 / gj_bar3_ready_28500 and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch28800.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 28799
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 28791–28799; 28800 is capstone
