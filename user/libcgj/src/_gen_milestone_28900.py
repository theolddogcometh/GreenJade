...
"""
Soft continuum generator — MILESTONE 28900 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 28801–28900 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=28900   when parent has wired through graph_batch28900.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 28801 28900
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 28801 28900
...
graph batches ending at M. For M = **28900** the exclusive range is
**28801–28900** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 28801–28850  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 28851–28875  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 28876–28890  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 28891–28900  │ decade soft gates (Wave 147 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 28900 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_28900        1          graph_batch28891.c
  gj_libcgj_green_28900       1          graph_batch28892.c
  gj_bar3_ready_28900         0          graph_batch28893.c  (bar3 open)
  gj_product_score_28900      0          graph_batch28894.c  (score open)
  gj_continuum_ready_28900    1          graph_batch28895.c
  gj_smoke_soft_28900         1          graph_batch28896.c
  gj_dyn_soft_28900           1          graph_batch28897.c
  gj_milestone_tag_28900      28900      graph_batch28898.c
  gj_continuum_wave_28900     28900      graph_batch28899.c
  gj_batch_id_28900           28900      graph_batch28900.c
  gj_graph_milestone_28900    28900      graph_batch28900.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=28900`` means parent wired CGJ_SRCS through
    ``graph_batch28900.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_28900`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_28900`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M28900_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_28800`` / ``gj_*_28700`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 147 exclusive CREATE-ONLY lands the **markers** band
  (28891–28900) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_28900.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_28900.py

  # Restrict to marker band only (soft-gate TUs; Wave 147 exclusive):
  python3 user/libcgj/src/_gen_milestone_28900.py --band markers

  # Extension identity only (28801–28850):
  python3 user/libcgj/src/_gen_milestone_28900.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_28900.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_28800.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 28900 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 28900
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 28801
WAVE_HI = 28900  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 28801, 28850
IDENTITY_LO, IDENTITY_HI = 28851, 28875
FILL_LO, FILL_HI = 28876, 28890
MARKERS_LO, MARKERS_HI = 28891, 28900
...
# Stems become gj_<stem>_28900. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (28891, "shell_green", 1, "shell product green extract (wave 28900)"),
    (28892, "libcgj_green", 1, "libcgj product green extract (wave 28900)"),
    (28893, "bar3_ready", 0, "product bar3 readiness gate (wave 28900)"),
    (28894, "product_score", 0, "product readiness score tag (wave 28900)"),
    (28895, "continuum_ready", 1, "continuum readiness gate (wave 28900)"),
    (28896, "smoke_soft", 1, "smoke soft lamp (wave 28900)"),
    (28897, "dyn_soft", 1, "dyn soft lamp (wave 28900)"),
    (28898, "milestone_tag", MILESTONE, "milestone 28900 product tag"),
    (28899, "continuum_wave", MILESTONE, "milestone 28900 continuum wave identity"),
    # 28900 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        28876: "continuum wave fill soft unit + batch id",
        28880: "continuum bridge soft unit + batch id",
        28890: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (28891–28899), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 28893
    tag_batch = MARKERS_LO + 7  # 28898
    wave_batch = MARKERS_LO + 8  # 28899
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_28800 / gj_shell_green_28700 / gj_shell_green_28600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_28800 / gj_libcgj_green_28700 / gj_libcgj_green_28600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_28800 / gj_bar3_ready_28700 / gj_bar3_ready_28600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_28800 / gj_product_score_28700 / gj_product_score_28600\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_28800 / gj_continuum_ready_28700 /\n"
            f" * gj_continuum_ready_28600 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_28800 / gj_smoke_soft_28700 / gj_smoke_soft_28600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_28800 / gj_dyn_soft_28700 / gj_dyn_soft_28600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_28800 / gj_milestone_tag_28700 / gj_milestone_tag_28600,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_28800 / gj_continuum_wave_28700 /\n"
            f" * gj_continuum_wave_28600, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch28900.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 28899
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 28891–28899; 28800 is capstone
