...
"""
Soft continuum generator — MILESTONE 29000 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 28901–29000 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=29000   when parent has wired through graph_batch29000.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 28901 29000
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 28901 29000
...
graph batches ending at M. For M = **29000** the exclusive range is
**28901–29000** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 28901–28950  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 28951–28975  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 28976–28990  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 28991–29000  │ decade soft gates (Wave 148 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 29000 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_29000        1          graph_batch28991.c
  gj_libcgj_green_29000       1          graph_batch28992.c
  gj_bar3_ready_29000         0          graph_batch28993.c  (bar3 open)
  gj_product_score_29000      0          graph_batch28994.c  (score open)
  gj_continuum_ready_29000    1          graph_batch28995.c
  gj_smoke_soft_29000         1          graph_batch28996.c
  gj_dyn_soft_29000           1          graph_batch28997.c
  gj_milestone_tag_29000      29000      graph_batch28998.c
  gj_continuum_wave_29000     29000      graph_batch28999.c
  gj_batch_id_29000           29000      graph_batch29000.c
  gj_graph_milestone_29000    29000      graph_batch29000.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=29000`` means parent wired CGJ_SRCS through
    ``graph_batch29000.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_29000`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_29000`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M29000_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_28900`` / ``gj_*_28800`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 148 exclusive CREATE-ONLY lands the **markers** band
  (28991–29000) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_29000.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_29000.py

  # Restrict to marker band only (soft-gate TUs; Wave 148 exclusive):
  python3 user/libcgj/src/_gen_milestone_29000.py --band markers

  # Extension identity only (28901–28950):
  python3 user/libcgj/src/_gen_milestone_29000.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_29000.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_28900.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 29000 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 29000
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 28901
WAVE_HI = 29000  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 28901, 28950
IDENTITY_LO, IDENTITY_HI = 28951, 28975
FILL_LO, FILL_HI = 28976, 28990
MARKERS_LO, MARKERS_HI = 28991, 29000
...
# Stems become gj_<stem>_29000. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (28991, "shell_green", 1, "shell product green extract (wave 29000)"),
    (28992, "libcgj_green", 1, "libcgj product green extract (wave 29000)"),
    (28993, "bar3_ready", 0, "product bar3 readiness gate (wave 29000)"),
    (28994, "product_score", 0, "product readiness score tag (wave 29000)"),
    (28995, "continuum_ready", 1, "continuum readiness gate (wave 29000)"),
    (28996, "smoke_soft", 1, "smoke soft lamp (wave 29000)"),
    (28997, "dyn_soft", 1, "dyn soft lamp (wave 29000)"),
    (28998, "milestone_tag", MILESTONE, "milestone 29000 product tag"),
    (28999, "continuum_wave", MILESTONE, "milestone 29000 continuum wave identity"),
    # 29000 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        28976: "continuum wave fill soft unit + batch id",
        28980: "continuum bridge soft unit + batch id",
        28990: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (28991–28999), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 28993
    tag_batch = MARKERS_LO + 7  # 28998
    wave_batch = MARKERS_LO + 8  # 28999
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_28900 / gj_shell_green_28800 / gj_shell_green_28700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_28900 / gj_libcgj_green_28800 / gj_libcgj_green_28700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_28900 / gj_bar3_ready_28800 / gj_bar3_ready_28700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_28900 / gj_product_score_28800 / gj_product_score_28700\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_28900 / gj_continuum_ready_28800 /\n"
            f" * gj_continuum_ready_28700 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_28900 / gj_smoke_soft_28800 / gj_smoke_soft_28700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_28900 / gj_dyn_soft_28800 / gj_dyn_soft_28700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_28900 / gj_milestone_tag_28800 / gj_milestone_tag_28700,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_28900 / gj_continuum_wave_28800 /\n"
            f" * gj_continuum_wave_28700, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch29000.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 28999
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 28991–28999; 29000 is capstone
