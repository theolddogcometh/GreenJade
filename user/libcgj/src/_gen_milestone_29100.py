...
"""
Soft continuum generator — MILESTONE 29100 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 29001–29100 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=29100   when parent has wired through graph_batch29100.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 29001 29100
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 29001 29100
...
graph batches ending at M. For M = **29100** the exclusive range is
**29001–29100** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 29001–29050  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 29051–29075  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 29076–29090  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 29091–29100  │ decade soft gates (Wave 149 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 29100 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_29100        1          graph_batch29091.c
  gj_libcgj_green_29100       1          graph_batch29092.c
  gj_bar3_ready_29100         0          graph_batch29093.c  (bar3 open)
  gj_product_score_29100      0          graph_batch29094.c  (score open)
  gj_continuum_ready_29100    1          graph_batch29095.c
  gj_smoke_soft_29100         1          graph_batch29096.c
  gj_dyn_soft_29100           1          graph_batch29097.c
  gj_milestone_tag_29100      29100      graph_batch29098.c
  gj_continuum_wave_29100     29100      graph_batch29099.c
  gj_batch_id_29100           29100      graph_batch29100.c
  gj_graph_milestone_29100    29100      graph_batch29100.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=29100`` means parent wired CGJ_SRCS through
    ``graph_batch29100.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_29100`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_29100`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M29100_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_29000`` / ``gj_*_28900`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 149 exclusive CREATE-ONLY lands the **markers** band
  (29091–29100) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_29100.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_29100.py

  # Restrict to marker band only (soft-gate TUs; Wave 149 exclusive):
  python3 user/libcgj/src/_gen_milestone_29100.py --band markers

  # Extension identity only (29001–29050):
  python3 user/libcgj/src/_gen_milestone_29100.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_29100.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_29000.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 29100 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 29100
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 29001
WAVE_HI = 29100  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 29001, 29050
IDENTITY_LO, IDENTITY_HI = 29051, 29075
FILL_LO, FILL_HI = 29076, 29090
MARKERS_LO, MARKERS_HI = 29091, 29100
...
# Stems become gj_<stem>_29100. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (29091, "shell_green", 1, "shell product green extract (wave 29100)"),
    (29092, "libcgj_green", 1, "libcgj product green extract (wave 29100)"),
    (29093, "bar3_ready", 0, "product bar3 readiness gate (wave 29100)"),
    (29094, "product_score", 0, "product readiness score tag (wave 29100)"),
    (29095, "continuum_ready", 1, "continuum readiness gate (wave 29100)"),
    (29096, "smoke_soft", 1, "smoke soft lamp (wave 29100)"),
    (29097, "dyn_soft", 1, "dyn soft lamp (wave 29100)"),
    (29098, "milestone_tag", MILESTONE, "milestone 29100 product tag"),
    (29099, "continuum_wave", MILESTONE, "milestone 29100 continuum wave identity"),
    # 29100 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        29076: "continuum wave fill soft unit + batch id",
        29080: "continuum bridge soft unit + batch id",
        29090: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (29091–29099), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 29093
    tag_batch = MARKERS_LO + 7  # 29098
    wave_batch = MARKERS_LO + 8  # 29099
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_29000 / gj_shell_green_28900 / gj_shell_green_28800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_29000 / gj_libcgj_green_28900 / gj_libcgj_green_28800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_29000 / gj_bar3_ready_28900 / gj_bar3_ready_28800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_29000 / gj_product_score_28900 / gj_product_score_28800\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_29000 / gj_continuum_ready_28900 /\n"
            f" * gj_continuum_ready_28800 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_29000 / gj_smoke_soft_28900 / gj_smoke_soft_28800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_29000 / gj_dyn_soft_28900 / gj_dyn_soft_28800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_29000 / gj_milestone_tag_28900 / gj_milestone_tag_28800,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_29000 / gj_continuum_wave_28900 /\n"
            f" * gj_continuum_wave_28800, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch29100.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 29099
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 29091–29099; 29100 is capstone
