...
"""
Soft continuum generator — MILESTONE 29200 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 29101–29200 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=29200   when parent has wired through graph_batch29200.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 29101 29200
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 29101 29200
...
graph batches ending at M. For M = **29200** the exclusive range is
**29101–29200** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 29101–29150  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 29151–29175  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 29176–29190  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 29191–29200  │ decade soft gates (Wave 150 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 29200 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_29200        1          graph_batch29191.c
  gj_libcgj_green_29200       1          graph_batch29192.c
  gj_bar3_ready_29200         0          graph_batch29193.c  (bar3 open)
  gj_product_score_29200      0          graph_batch29194.c  (score open)
  gj_continuum_ready_29200    1          graph_batch29195.c
  gj_smoke_soft_29200         1          graph_batch29196.c
  gj_dyn_soft_29200           1          graph_batch29197.c
  gj_milestone_tag_29200      29200      graph_batch29198.c
  gj_continuum_wave_29200     29200      graph_batch29199.c
  gj_batch_id_29200           29200      graph_batch29200.c
  gj_graph_milestone_29200    29200      graph_batch29200.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=29200`` means parent wired CGJ_SRCS through
    ``graph_batch29200.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_29200`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_29200`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M29200_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_28700`` / ``gj_*_28600`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 150 exclusive CREATE-ONLY lands the **markers** band
  (29191–29200) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_29200.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_29200.py

  # Restrict to marker band only (soft-gate TUs; Wave 150 exclusive):
  python3 user/libcgj/src/_gen_milestone_29200.py --band markers

  # Extension identity only (29101–29150):
  python3 user/libcgj/src/_gen_milestone_29200.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_29200.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_29100.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 29200 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 29200
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 29101
WAVE_HI = 29200  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 29101, 29150
IDENTITY_LO, IDENTITY_HI = 29151, 29175
FILL_LO, FILL_HI = 29176, 29190
MARKERS_LO, MARKERS_HI = 29191, 29200
...
# Stems become gj_<stem>_29200. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (29191, "shell_green", 1, "shell product green extract (wave 29200)"),
    (29192, "libcgj_green", 1, "libcgj product green extract (wave 29200)"),
    (29193, "bar3_ready", 0, "product bar3 readiness gate (wave 29200)"),
    (29194, "product_score", 0, "product readiness score tag (wave 29200)"),
    (29195, "continuum_ready", 1, "continuum readiness gate (wave 29200)"),
    (29196, "smoke_soft", 1, "smoke soft lamp (wave 29200)"),
    (29197, "dyn_soft", 1, "dyn soft lamp (wave 29200)"),
    (29198, "milestone_tag", MILESTONE, "milestone 29200 product tag"),
    (29199, "continuum_wave", MILESTONE, "milestone 29200 continuum wave identity"),
    # 29200 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        29176: "continuum wave fill soft unit + batch id",
        29180: "continuum bridge soft unit + batch id",
        29190: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (29191–29199), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 29193
    tag_batch = MARKERS_LO + 7  # 29198
    wave_batch = MARKERS_LO + 8  # 29199
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_28700 / gj_shell_green_28600 / gj_shell_green_28500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_28700 / gj_libcgj_green_28600 / gj_libcgj_green_28500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_29100 / gj_bar3_ready_29000 / gj_bar3_ready_28900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_28700 / gj_product_score_28600 / gj_product_score_28500\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_28700 / gj_continuum_ready_28600 /\n"
            f" * gj_continuum_ready_28500 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_28700 / gj_smoke_soft_28600 / gj_smoke_soft_28500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_28700 / gj_dyn_soft_28600 / gj_dyn_soft_28500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_28700 / gj_milestone_tag_28600 / gj_milestone_tag_28500,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_28700 / gj_continuum_wave_28600 /\n"
            f" * gj_continuum_wave_28500, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch29200.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 29199
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 29191–29199; 29200 is capstone
