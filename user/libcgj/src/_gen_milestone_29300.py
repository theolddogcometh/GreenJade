...
"""
Soft continuum generator — MILESTONE 29300 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 29201–29300 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=29300   when parent has wired through graph_batch29300.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 29201 29300
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 29201 29300
...
graph batches ending at M. For M = **29300** the exclusive range is
**29201–29300** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 29201–29250  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 29251–29275  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 29276–29290  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 29291–29300  │ decade soft gates (Wave 151 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 29300 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_29300        1          graph_batch29291.c
  gj_libcgj_green_29300       1          graph_batch29292.c
  gj_bar3_ready_29300         0          graph_batch29293.c  (bar3 open)
  gj_product_score_29300      0          graph_batch29294.c  (score open)
  gj_continuum_ready_29300    1          graph_batch29295.c
  gj_smoke_soft_29300         1          graph_batch29296.c
  gj_dyn_soft_29300           1          graph_batch29297.c
  gj_milestone_tag_29300      29300      graph_batch29298.c
  gj_continuum_wave_29300     29300      graph_batch29299.c
  gj_batch_id_29300           29300      graph_batch29300.c
  gj_graph_milestone_29300    29300      graph_batch29300.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=29300`` means parent wired CGJ_SRCS through
    ``graph_batch29300.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_29300`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_29300`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M29300_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_29200`` / ``gj_*_29100`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 151 exclusive CREATE-ONLY lands the **markers** band
  (29291–29300) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_29300.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_29300.py

  # Restrict to marker band only (soft-gate TUs; Wave 151 exclusive):
  python3 user/libcgj/src/_gen_milestone_29300.py --band markers

  # Extension identity only (29201–29250):
  python3 user/libcgj/src/_gen_milestone_29300.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_29300.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_29200.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 29300 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 29300
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 29201
WAVE_HI = 29300  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 29201, 29250
IDENTITY_LO, IDENTITY_HI = 29251, 29275
FILL_LO, FILL_HI = 29276, 29290
MARKERS_LO, MARKERS_HI = 29291, 29300
...
# Stems become gj_<stem>_29300. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (29291, "shell_green", 1, "shell product green extract (wave 29300)"),
    (29292, "libcgj_green", 1, "libcgj product green extract (wave 29300)"),
    (29293, "bar3_ready", 0, "product bar3 readiness gate (wave 29300)"),
    (29294, "product_score", 0, "product readiness score tag (wave 29300)"),
    (29295, "continuum_ready", 1, "continuum readiness gate (wave 29300)"),
    (29296, "smoke_soft", 1, "smoke soft lamp (wave 29300)"),
    (29297, "dyn_soft", 1, "dyn soft lamp (wave 29300)"),
    (29298, "milestone_tag", MILESTONE, "milestone 29300 product tag"),
    (29299, "continuum_wave", MILESTONE, "milestone 29300 continuum wave identity"),
    # 29300 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        29276: "continuum wave fill soft unit + batch id",
        29280: "continuum bridge soft unit + batch id",
        29290: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (29291–29299), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 29293
    tag_batch = MARKERS_LO + 7  # 29298
    wave_batch = MARKERS_LO + 8  # 29299
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_29200 / gj_shell_green_29100 / gj_shell_green_29000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_29200 / gj_libcgj_green_29100 / gj_libcgj_green_29000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_29200 / gj_bar3_ready_29100 / gj_bar3_ready_29000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_29200 / gj_product_score_29100 / gj_product_score_29000\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_29200 / gj_continuum_ready_29100 /\n"
            f" * gj_continuum_ready_29000 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_29200 / gj_smoke_soft_29100 / gj_smoke_soft_29000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_29200 / gj_dyn_soft_29100 / gj_dyn_soft_29000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_29200 / gj_milestone_tag_29100 / gj_milestone_tag_29000,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_29200 / gj_continuum_wave_29100 /\n"
            f" * gj_continuum_wave_29000, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch29300.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 29299
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 29291–29299; 29300 is capstone
