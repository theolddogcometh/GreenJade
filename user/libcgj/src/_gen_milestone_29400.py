...
"""
Soft continuum generator — MILESTONE 29400 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 29301–29400 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=29400   when parent has wired through graph_batch29400.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 29301 29400
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 29301 29400
...
graph batches ending at M. For M = **29400** the exclusive range is
**29301–29400** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 29301–29350  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 29351–29375  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 29376–29390  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 29391–29400  │ decade soft gates (Wave 152 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 29400 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_29400        1          graph_batch29391.c
  gj_libcgj_green_29400       1          graph_batch29392.c
  gj_bar3_ready_29400         0          graph_batch29393.c  (bar3 open)
  gj_product_score_29400      0          graph_batch29394.c  (score open)
  gj_continuum_ready_29400    1          graph_batch29395.c
  gj_smoke_soft_29400         1          graph_batch29396.c
  gj_dyn_soft_29400           1          graph_batch29397.c
  gj_milestone_tag_29400      29400      graph_batch29398.c
  gj_continuum_wave_29400     29400      graph_batch29399.c
  gj_batch_id_29400           29400      graph_batch29400.c
  gj_graph_milestone_29400    29400      graph_batch29400.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=29400`` means parent wired CGJ_SRCS through
    ``graph_batch29400.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_29400`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_29400`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M29400_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_29300`` / ``gj_*_29200`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 152 exclusive CREATE-ONLY lands the **markers** band
  (29391–29400) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_29400.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_29400.py

  # Restrict to marker band only (soft-gate TUs; Wave 152 exclusive):
  python3 user/libcgj/src/_gen_milestone_29400.py --band markers

  # Extension identity only (29301–29350):
  python3 user/libcgj/src/_gen_milestone_29400.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_29400.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_29300.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 29400 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 29400
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 29301
WAVE_HI = 29400  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 29301, 29350
IDENTITY_LO, IDENTITY_HI = 29351, 29375
FILL_LO, FILL_HI = 29376, 29390
MARKERS_LO, MARKERS_HI = 29391, 29400
...
# Stems become gj_<stem>_29400. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (29391, "shell_green", 1, "shell product green extract (wave 29400)"),
    (29392, "libcgj_green", 1, "libcgj product green extract (wave 29400)"),
    (29393, "bar3_ready", 0, "product bar3 readiness gate (wave 29400)"),
    (29394, "product_score", 0, "product readiness score tag (wave 29400)"),
    (29395, "continuum_ready", 1, "continuum readiness gate (wave 29400)"),
    (29396, "smoke_soft", 1, "smoke soft lamp (wave 29400)"),
    (29397, "dyn_soft", 1, "dyn soft lamp (wave 29400)"),
    (29398, "milestone_tag", MILESTONE, "milestone 29400 product tag"),
    (29399, "continuum_wave", MILESTONE, "milestone 29400 continuum wave identity"),
    # 29400 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        29376: "continuum wave fill soft unit + batch id",
        29380: "continuum bridge soft unit + batch id",
        29390: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (29391–29399), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 29393
    tag_batch = MARKERS_LO + 7  # 29398
    wave_batch = MARKERS_LO + 8  # 29399
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_29300 / gj_shell_green_29200 / gj_shell_green_29100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_29300 / gj_libcgj_green_29200 / gj_libcgj_green_29100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_29300 / gj_bar3_ready_29200 / gj_bar3_ready_29100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_29300 / gj_product_score_29200 / gj_product_score_29100\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_29300 / gj_continuum_ready_29200 /\n"
            f" * gj_continuum_ready_29100 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_29300 / gj_smoke_soft_29200 / gj_smoke_soft_29100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_29300 / gj_dyn_soft_29200 / gj_dyn_soft_29100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_29300 / gj_milestone_tag_29200 / gj_milestone_tag_29100,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_29300 / gj_continuum_wave_29200 /\n"
            f" * gj_continuum_wave_29100, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch29400.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 29399
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 29391–29399; 29400 is capstone
