...
"""
Soft continuum generator — MILESTONE 29500 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 29401–29500 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=29500   when parent has wired through graph_batch29500.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 29401 29500
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 29401 29500
...
graph batches ending at M. For M = **29500** the exclusive range is
**29401–29500** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 29401–29450  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 29451–29475  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 29476–29490  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 29491–29500  │ decade soft gates (Wave 153 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 29500 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_29500        1          graph_batch29491.c
  gj_libcgj_green_29500       1          graph_batch29492.c
  gj_bar3_ready_29500         0          graph_batch29493.c  (bar3 open)
  gj_product_score_29500      0          graph_batch29494.c  (score open)
  gj_continuum_ready_29500    1          graph_batch29495.c
  gj_smoke_soft_29500         1          graph_batch29496.c
  gj_dyn_soft_29500           1          graph_batch29497.c
  gj_milestone_tag_29500      29500      graph_batch29498.c
  gj_continuum_wave_29500     29500      graph_batch29499.c
  gj_batch_id_29500           29500      graph_batch29500.c
  gj_graph_milestone_29500    29500      graph_batch29500.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=29500`` means parent wired CGJ_SRCS through
    ``graph_batch29500.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_29500`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_29500`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M29500_DIRECT``.
...
--------------------
  ...
  Wave 153 exclusive CREATE-ONLY lands the **markers** band
  (29491–29500) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_29500.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_29500.py

  # Restrict to marker band only (soft-gate TUs; Wave 153 exclusive):
  python3 user/libcgj/src/_gen_milestone_29500.py --band markers

  # Extension identity only (29401–29450):
  python3 user/libcgj/src/_gen_milestone_29500.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_29500.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_29400.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 29500 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 29500
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 29401
WAVE_HI = 29500  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 29401, 29450
IDENTITY_LO, IDENTITY_HI = 29451, 29475
FILL_LO, FILL_HI = 29476, 29490
MARKERS_LO, MARKERS_HI = 29491, 29500
...
# Stems become gj_<stem>_29500. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (29491, "shell_green", 1, "shell product green extract (wave 29500)"),
    (29492, "libcgj_green", 1, "libcgj product green extract (wave 29500)"),
    (29493, "bar3_ready", 0, "product bar3 readiness gate (wave 29500)"),
    (29494, "product_score", 0, "product readiness score tag (wave 29500)"),
    (29495, "continuum_ready", 1, "continuum readiness gate (wave 29500)"),
    (29496, "smoke_soft", 1, "smoke soft lamp (wave 29500)"),
    (29497, "dyn_soft", 1, "dyn soft lamp (wave 29500)"),
    (29498, "milestone_tag", MILESTONE, "milestone 29500 product tag"),
    (29499, "continuum_wave", MILESTONE, "milestone 29500 continuum wave identity"),
    # 29500 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        29476: "continuum wave fill soft unit + batch id",
        29480: "continuum bridge soft unit + batch id",
        29490: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (29491–29499), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 29493
    tag_batch = MARKERS_LO + 7  # 29498
    wave_batch = MARKERS_LO + 8  # 29499
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_29400 / gj_bar3_ready_29300 / gj_bar3_ready_29200 and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch29500.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 29499
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 29491–29499; 29500 is capstone
