...
"""
Soft continuum generator — MILESTONE 29600 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 29501–29600 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=29600   when parent has wired through graph_batch29600.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 29501 29600
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 29501 29600
...
graph batches ending at M. For M = **29600** the exclusive range is
**29501–29600** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 29501–29550  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 29551–29575  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 29576–29590  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 29591–29600  │ decade soft gates (Wave 154 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 29600 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_29600        1          graph_batch29591.c
  gj_libcgj_green_29600       1          graph_batch29592.c
  gj_bar3_ready_29600         0          graph_batch29593.c  (bar3 open)
  gj_product_score_29600      0          graph_batch29594.c  (score open)
  gj_continuum_ready_29600    1          graph_batch29595.c
  gj_smoke_soft_29600         1          graph_batch29596.c
  gj_dyn_soft_29600           1          graph_batch29597.c
  gj_milestone_tag_29600      29600      graph_batch29598.c
  gj_continuum_wave_29600     29600      graph_batch29599.c
  gj_batch_id_29600           29600      graph_batch29600.c
  gj_graph_milestone_29600    29600      graph_batch29600.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=29600`` means parent wired CGJ_SRCS through
    ``graph_batch29600.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_29600`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_29600`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M29600_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_29500`` / ``gj_*_29400`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 154 exclusive CREATE-ONLY lands the **markers** band
  (29591–29600) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_29600.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_29600.py

  # Restrict to marker band only (soft-gate TUs; Wave 154 exclusive):
  python3 user/libcgj/src/_gen_milestone_29600.py --band markers

  # Extension identity only (29501–29550):
  python3 user/libcgj/src/_gen_milestone_29600.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_29600.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_29500.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 29600 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 29600
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 29501
WAVE_HI = 29600  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 29501, 29550
IDENTITY_LO, IDENTITY_HI = 29551, 29575
FILL_LO, FILL_HI = 29576, 29590
MARKERS_LO, MARKERS_HI = 29591, 29600
...
# Stems become gj_<stem>_29600. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (29591, "shell_green", 1, "shell product green extract (wave 29600)"),
    (29592, "libcgj_green", 1, "libcgj product green extract (wave 29600)"),
    (29593, "bar3_ready", 0, "product bar3 readiness gate (wave 29600)"),
    (29594, "product_score", 0, "product readiness score tag (wave 29600)"),
    (29595, "continuum_ready", 1, "continuum readiness gate (wave 29600)"),
    (29596, "smoke_soft", 1, "smoke soft lamp (wave 29600)"),
    (29597, "dyn_soft", 1, "dyn soft lamp (wave 29600)"),
    (29598, "milestone_tag", MILESTONE, "milestone 29600 product tag"),
    (29599, "continuum_wave", MILESTONE, "milestone 29600 continuum wave identity"),
    # 29600 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        29576: "continuum wave fill soft unit + batch id",
        29580: "continuum bridge soft unit + batch id",
        29590: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (29591–29599), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 29593
    tag_batch = MARKERS_LO + 7  # 29598
    wave_batch = MARKERS_LO + 8  # 29599
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_29500 / gj_shell_green_29400 / gj_shell_green_29300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_29500 / gj_libcgj_green_29400 / gj_libcgj_green_29300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_29500 / gj_bar3_ready_29400 / gj_bar3_ready_29300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_29500 / gj_product_score_29400 / gj_product_score_29300\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_29500 / gj_continuum_ready_29400 /\n"
            f" * gj_continuum_ready_29300 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_29500 / gj_smoke_soft_29400 / gj_smoke_soft_29300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_29500 / gj_dyn_soft_29400 / gj_dyn_soft_29300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_29500 / gj_milestone_tag_29400 / gj_milestone_tag_29300,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_29500 / gj_continuum_wave_29400 /\n"
            f" * gj_continuum_wave_29300, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch29600.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 29599
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 29591–29599; 29600 is capstone
