...
"""
Soft continuum generator — MILESTONE 29700 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 29601–29700 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=29700   when parent has wired through graph_batch29700.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 29601 29700
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 29601 29700
...
graph batches ending at M. For M = **29700** the exclusive range is
**29601–29700** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 29601–29650  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 29651–29675  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 29676–29690  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 29691–29700  │ decade soft gates (Wave 155 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 29700 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_29700        1          graph_batch29691.c
  gj_libcgj_green_29700       1          graph_batch29692.c
  gj_bar3_ready_29700         0          graph_batch29693.c  (bar3 open)
  gj_product_score_29700      0          graph_batch29694.c  (score open)
  gj_continuum_ready_29700    1          graph_batch29695.c
  gj_smoke_soft_29700         1          graph_batch29696.c
  gj_dyn_soft_29700           1          graph_batch29697.c
  gj_milestone_tag_29700      29700      graph_batch29698.c
  gj_continuum_wave_29700     29700      graph_batch29699.c
  gj_batch_id_29700           29700      graph_batch29700.c
  gj_graph_milestone_29700    29700      graph_batch29700.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=29700`` means parent wired CGJ_SRCS through
    ``graph_batch29700.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_29700`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_29700`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M29700_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_29600`` / ``gj_*_29500`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 155 exclusive CREATE-ONLY lands the **markers** band
  (29691–29700) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_29700.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_29700.py

  # Restrict to marker band only (soft-gate TUs; Wave 155 exclusive):
  python3 user/libcgj/src/_gen_milestone_29700.py --band markers

  # Extension identity only (29601–29650):
  python3 user/libcgj/src/_gen_milestone_29700.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_29700.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_29600.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 29700 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 29700
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 29601
WAVE_HI = 29700  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 29601, 29650
IDENTITY_LO, IDENTITY_HI = 29651, 29675
FILL_LO, FILL_HI = 29676, 29690
MARKERS_LO, MARKERS_HI = 29691, 29700
...
# Stems become gj_<stem>_29700. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (29691, "shell_green", 1, "shell product green extract (wave 29700)"),
    (29692, "libcgj_green", 1, "libcgj product green extract (wave 29700)"),
    (29693, "bar3_ready", 0, "product bar3 readiness gate (wave 29700)"),
    (29694, "product_score", 0, "product readiness score tag (wave 29700)"),
    (29695, "continuum_ready", 1, "continuum readiness gate (wave 29700)"),
    (29696, "smoke_soft", 1, "smoke soft lamp (wave 29700)"),
    (29697, "dyn_soft", 1, "dyn soft lamp (wave 29700)"),
    (29698, "milestone_tag", MILESTONE, "milestone 29700 product tag"),
    (29699, "continuum_wave", MILESTONE, "milestone 29700 continuum wave identity"),
    # 29700 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        29676: "continuum wave fill soft unit + batch id",
        29680: "continuum bridge soft unit + batch id",
        29690: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (29691–29699), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 29693
    tag_batch = MARKERS_LO + 7  # 29698
    wave_batch = MARKERS_LO + 8  # 29699
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_29600 / gj_shell_green_29500 / gj_shell_green_29400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_29600 / gj_libcgj_green_29500 / gj_libcgj_green_29400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_29600 / gj_bar3_ready_29500 / gj_bar3_ready_29400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_29600 / gj_product_score_29500 / gj_product_score_29400\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_29600 / gj_continuum_ready_29500 /\n"
            f" * gj_continuum_ready_29400 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_29600 / gj_smoke_soft_29500 / gj_smoke_soft_29400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_29600 / gj_dyn_soft_29500 / gj_dyn_soft_29400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_29600 / gj_milestone_tag_29500 / gj_milestone_tag_29400,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_29600 / gj_continuum_wave_29500 /\n"
            f" * gj_continuum_wave_29400, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch29700.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 29699
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 29691–29699; 29700 is capstone
