...
"""
Soft continuum generator — MILESTONE 29800 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 29701–29800 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=29800   when parent has wired through graph_batch29800.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 29701 29800
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 29701 29800
...
graph batches ending at M. For M = **29800** the exclusive range is
**29701–29800** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 29701–29750  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 29751–29775  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 29776–29790  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 29791–29800  │ decade soft gates (Wave 156 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 29800 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_29800        1          graph_batch29791.c
  gj_libcgj_green_29800       1          graph_batch29792.c
  gj_bar3_ready_29800         0          graph_batch29793.c  (bar3 open)
  gj_product_score_29800      0          graph_batch29794.c  (score open)
  gj_continuum_ready_29800    1          graph_batch29795.c
  gj_smoke_soft_29800         1          graph_batch29796.c
  gj_dyn_soft_29800           1          graph_batch29797.c
  gj_milestone_tag_29800      29800      graph_batch29798.c
  gj_continuum_wave_29800     29800      graph_batch29799.c
  gj_batch_id_29800           29800      graph_batch29800.c
  gj_graph_milestone_29800    29800      graph_batch29800.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=29800`` means parent wired CGJ_SRCS through
    ``graph_batch29800.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_29800`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_29800`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M29800_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_29700`` / ``gj_*_29600`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 156 exclusive CREATE-ONLY lands the **markers** band
  (29791–29800) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_29800.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_29800.py

  # Restrict to marker band only (soft-gate TUs; Wave 156 exclusive):
  python3 user/libcgj/src/_gen_milestone_29800.py --band markers

  # Extension identity only (29701–29750):
  python3 user/libcgj/src/_gen_milestone_29800.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_29800.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_29700.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 29800 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 29800
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 29701
WAVE_HI = 29800  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 29701, 29750
IDENTITY_LO, IDENTITY_HI = 29751, 29775
FILL_LO, FILL_HI = 29776, 29790
MARKERS_LO, MARKERS_HI = 29791, 29800
...
# Stems become gj_<stem>_29800. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (29791, "shell_green", 1, "shell product green extract (wave 29800)"),
    (29792, "libcgj_green", 1, "libcgj product green extract (wave 29800)"),
    (29793, "bar3_ready", 0, "product bar3 readiness gate (wave 29800)"),
    (29794, "product_score", 0, "product readiness score tag (wave 29800)"),
    (29795, "continuum_ready", 1, "continuum readiness gate (wave 29800)"),
    (29796, "smoke_soft", 1, "smoke soft lamp (wave 29800)"),
    (29797, "dyn_soft", 1, "dyn soft lamp (wave 29800)"),
    (29798, "milestone_tag", MILESTONE, "milestone 29800 product tag"),
    (29799, "continuum_wave", MILESTONE, "milestone 29800 continuum wave identity"),
    # 29800 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        29776: "continuum wave fill soft unit + batch id",
        29780: "continuum bridge soft unit + batch id",
        29790: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (29791–29799), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 29793
    tag_batch = MARKERS_LO + 7  # 29798
    wave_batch = MARKERS_LO + 8  # 29799
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_29700 / gj_shell_green_29600 / gj_shell_green_29500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_29700 / gj_libcgj_green_29600 / gj_libcgj_green_29500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_29700 / gj_bar3_ready_29600 / gj_bar3_ready_29500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_29700 / gj_product_score_29600 / gj_product_score_29500\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_29700 / gj_continuum_ready_29600 /\n"
            f" * gj_continuum_ready_29500 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_29700 / gj_smoke_soft_29600 / gj_smoke_soft_29500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_29700 / gj_dyn_soft_29600 / gj_dyn_soft_29500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_29700 / gj_milestone_tag_29600 / gj_milestone_tag_29500,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_29700 / gj_continuum_wave_29600 /\n"
            f" * gj_continuum_wave_29500, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch29800.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 29799
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 29791–29799; 29800 is capstone
