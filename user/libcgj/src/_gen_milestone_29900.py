...
"""
Soft continuum generator — MILESTONE 29900 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 29801–29900 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=29900   when parent has wired through graph_batch29900.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 29801 29900
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 29801 29900
...
graph batches ending at M. For M = **29900** the exclusive range is
**29801–29900** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 29801–29850  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 29851–29875  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 29876–29890  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 29891–29900  │ decade soft gates (Wave 157 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 29900 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_29900        1          graph_batch29891.c
  gj_libcgj_green_29900       1          graph_batch29892.c
  gj_bar3_ready_29900         0          graph_batch29893.c  (bar3 open)
  gj_product_score_29900      0          graph_batch29894.c  (score open)
  gj_continuum_ready_29900    1          graph_batch29895.c
  gj_smoke_soft_29900         1          graph_batch29896.c
  gj_dyn_soft_29900           1          graph_batch29897.c
  gj_milestone_tag_29900      29900      graph_batch29898.c
  gj_continuum_wave_29900     29900      graph_batch29899.c
  gj_batch_id_29900           29900      graph_batch29900.c
  gj_graph_milestone_29900    29900      graph_batch29900.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=29900`` means parent wired CGJ_SRCS through
    ``graph_batch29900.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_29900`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_29900`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M29900_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_29800`` / ``gj_*_29700`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 157 exclusive CREATE-ONLY lands the **markers** band
  (29891–29900) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_29900.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_29900.py

  # Restrict to marker band only (soft-gate TUs; Wave 157 exclusive):
  python3 user/libcgj/src/_gen_milestone_29900.py --band markers

  # Extension identity only (29801–29850):
  python3 user/libcgj/src/_gen_milestone_29900.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_29900.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_29800.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 29900 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 29900
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 29801
WAVE_HI = 29900  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 29801, 29850
IDENTITY_LO, IDENTITY_HI = 29851, 29875
FILL_LO, FILL_HI = 29876, 29890
MARKERS_LO, MARKERS_HI = 29891, 29900
...
# Stems become gj_<stem>_29900. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (29891, "shell_green", 1, "shell product green extract (wave 29900)"),
    (29892, "libcgj_green", 1, "libcgj product green extract (wave 29900)"),
    (29893, "bar3_ready", 0, "product bar3 readiness gate (wave 29900)"),
    (29894, "product_score", 0, "product readiness score tag (wave 29900)"),
    (29895, "continuum_ready", 1, "continuum readiness gate (wave 29900)"),
    (29896, "smoke_soft", 1, "smoke soft lamp (wave 29900)"),
    (29897, "dyn_soft", 1, "dyn soft lamp (wave 29900)"),
    (29898, "milestone_tag", MILESTONE, "milestone 29900 product tag"),
    (29899, "continuum_wave", MILESTONE, "milestone 29900 continuum wave identity"),
    # 29900 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        29876: "continuum wave fill soft unit + batch id",
        29880: "continuum bridge soft unit + batch id",
        29890: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (29891–29899), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 29893
    tag_batch = MARKERS_LO + 7  # 29898
    wave_batch = MARKERS_LO + 8  # 29899
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_29800 / gj_shell_green_29700 / gj_shell_green_29600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_29800 / gj_libcgj_green_29700 / gj_libcgj_green_29600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_29800 / gj_bar3_ready_29700 / gj_bar3_ready_29600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_29800 / gj_product_score_29700 / gj_product_score_29600\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_29800 / gj_continuum_ready_29700 /\n"
            f" * gj_continuum_ready_29600 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_29800 / gj_smoke_soft_29700 / gj_smoke_soft_29600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_29800 / gj_dyn_soft_29700 / gj_dyn_soft_29600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_29800 / gj_milestone_tag_29700 / gj_milestone_tag_29600,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_29800 / gj_continuum_wave_29700 /\n"
            f" * gj_continuum_wave_29600, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch29900.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 29899
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 29891–29899; 29900 is capstone
