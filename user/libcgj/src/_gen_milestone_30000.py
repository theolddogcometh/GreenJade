...
"""
Soft continuum generator — MILESTONE 30000 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 29901–30000 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=30000   when parent has wired through graph_batch30000.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 29901 30000
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 29901 30000
...
graph batches ending at M. For M = **30000** the exclusive range is
**29901–30000** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 29901–29950  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 29951–29975  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 29976–29990  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 29991–30000  │ decade soft gates (Wave 158 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 30000 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_30000        1          graph_batch29991.c
  gj_libcgj_green_30000       1          graph_batch29992.c
  gj_bar3_ready_30000         0          graph_batch29993.c  (bar3 open)
  gj_product_score_30000      0          graph_batch29994.c  (score open)
  gj_continuum_ready_30000    1          graph_batch29995.c
  gj_smoke_soft_30000         1          graph_batch29996.c
  gj_dyn_soft_30000           1          graph_batch29997.c
  gj_milestone_tag_30000      30000      graph_batch29998.c
  gj_continuum_wave_30000     30000      graph_batch29999.c
  gj_batch_id_30000           30000      graph_batch30000.c
  gj_graph_milestone_30000    30000      graph_batch30000.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=30000`` means parent wired CGJ_SRCS through
    ``graph_batch30000.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_30000`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_30000`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M30000_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_29900`` / ``gj_*_29800`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 158 exclusive CREATE-ONLY lands the **markers** band
  (29991–30000) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_30000.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_30000.py

  # Restrict to marker band only (soft-gate TUs; Wave 158 exclusive):
  python3 user/libcgj/src/_gen_milestone_30000.py --band markers

  # Extension identity only (29901–29950):
  python3 user/libcgj/src/_gen_milestone_30000.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_30000.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_29900.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 30000 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 30000
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 29901
WAVE_HI = 30000  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 29901, 29950
IDENTITY_LO, IDENTITY_HI = 29951, 29975
FILL_LO, FILL_HI = 29976, 29990
MARKERS_LO, MARKERS_HI = 29991, 30000
...
# Stems become gj_<stem>_30000. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (29991, "shell_green", 1, "shell product green extract (wave 30000)"),
    (29992, "libcgj_green", 1, "libcgj product green extract (wave 30000)"),
    (29993, "bar3_ready", 0, "product bar3 readiness gate (wave 30000)"),
    (29994, "product_score", 0, "product readiness score tag (wave 30000)"),
    (29995, "continuum_ready", 1, "continuum readiness gate (wave 30000)"),
    (29996, "smoke_soft", 1, "smoke soft lamp (wave 30000)"),
    (29997, "dyn_soft", 1, "dyn soft lamp (wave 30000)"),
    (29998, "milestone_tag", MILESTONE, "milestone 30000 product tag"),
    (29999, "continuum_wave", MILESTONE, "milestone 30000 continuum wave identity"),
    # 30000 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        29976: "continuum wave fill soft unit + batch id",
        29980: "continuum bridge soft unit + batch id",
        29990: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (29991–29999), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 29993
    tag_batch = MARKERS_LO + 7  # 29998
    wave_batch = MARKERS_LO + 8  # 29999
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_29900 / gj_shell_green_29800 / gj_shell_green_29700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_29900 / gj_libcgj_green_29800 / gj_libcgj_green_29700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_29900 / gj_bar3_ready_29800 / gj_bar3_ready_29700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_29900 / gj_product_score_29800 / gj_product_score_29700\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_29900 / gj_continuum_ready_29800 /\n"
            f" * gj_continuum_ready_29700 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_29900 / gj_smoke_soft_29800 / gj_smoke_soft_29700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_29900 / gj_dyn_soft_29800 / gj_dyn_soft_29700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_29900 / gj_milestone_tag_29800 / gj_milestone_tag_29700,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_29900 / gj_continuum_wave_29800 /\n"
            f" * gj_continuum_wave_29700, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch30000.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 29999
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 29991–29999; 30000 is capstone
