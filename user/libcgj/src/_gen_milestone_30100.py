...
"""
Soft continuum generator — MILESTONE 30100 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 30001–30100 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=30100   when parent has wired through graph_batch30100.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 30001 30100
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 30001 30100
...
graph batches ending at M. For M = **30100** the exclusive range is
**30001–30100** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 30001–30050  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 30051–30075  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 30076–30090  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 30091–30100  │ decade soft gates (Wave 159 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 30100 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_30100        1          graph_batch30091.c
  gj_libcgj_green_30100       1          graph_batch30092.c
  gj_bar3_ready_30100         0          graph_batch30093.c  (bar3 open)
  gj_product_score_30100      0          graph_batch30094.c  (score open)
  gj_continuum_ready_30100    1          graph_batch30095.c
  gj_smoke_soft_30100         1          graph_batch30096.c
  gj_dyn_soft_30100           1          graph_batch30097.c
  gj_milestone_tag_30100      30100      graph_batch30098.c
  gj_continuum_wave_30100     30100      graph_batch30099.c
  gj_batch_id_30100           30100      graph_batch30100.c
  gj_graph_milestone_30100    30100      graph_batch30100.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=30100`` means parent wired CGJ_SRCS through
    ``graph_batch30100.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_30100`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_30100`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M30100_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_30000`` / ``gj_*_29900`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 159 exclusive CREATE-ONLY lands the **markers** band
  (30091–30100) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_30100.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_30100.py

  # Restrict to marker band only (soft-gate TUs; Wave 159 exclusive):
  python3 user/libcgj/src/_gen_milestone_30100.py --band markers

  # Extension identity only (30001–30050):
  python3 user/libcgj/src/_gen_milestone_30100.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_30100.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_30000.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 30100 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 30100
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 30001
WAVE_HI = 30100  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 30001, 30050
IDENTITY_LO, IDENTITY_HI = 30051, 30075
FILL_LO, FILL_HI = 30076, 30090
MARKERS_LO, MARKERS_HI = 30091, 30100
...
# Stems become gj_<stem>_30100. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (30091, "shell_green", 1, "shell product green extract (wave 30100)"),
    (30092, "libcgj_green", 1, "libcgj product green extract (wave 30100)"),
    (30093, "bar3_ready", 0, "product bar3 readiness gate (wave 30100)"),
    (30094, "product_score", 0, "product readiness score tag (wave 30100)"),
    (30095, "continuum_ready", 1, "continuum readiness gate (wave 30100)"),
    (30096, "smoke_soft", 1, "smoke soft lamp (wave 30100)"),
    (30097, "dyn_soft", 1, "dyn soft lamp (wave 30100)"),
    (30098, "milestone_tag", MILESTONE, "milestone 30100 product tag"),
    (30099, "continuum_wave", MILESTONE, "milestone 30100 continuum wave identity"),
    # 30100 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        30076: "continuum wave fill soft unit + batch id",
        30080: "continuum bridge soft unit + batch id",
        30090: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (30091–30099), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 30093
    tag_batch = MARKERS_LO + 7  # 30098
    wave_batch = MARKERS_LO + 8  # 30099
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_30000 / gj_shell_green_29900 / gj_shell_green_29800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_30000 / gj_libcgj_green_29900 / gj_libcgj_green_29800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_30000 / gj_bar3_ready_29900 / gj_bar3_ready_29800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_30000 / gj_product_score_29900 / gj_product_score_29800\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_30000 / gj_continuum_ready_29900 /\n"
            f" * gj_continuum_ready_29800 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_30000 / gj_smoke_soft_29900 / gj_smoke_soft_29800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_30000 / gj_dyn_soft_29900 / gj_dyn_soft_29800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_30000 / gj_milestone_tag_29900 / gj_milestone_tag_29800,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_30000 / gj_continuum_wave_29900 /\n"
            f" * gj_continuum_wave_29800, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch30100.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 30099
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 30091–30099; 30100 is capstone
