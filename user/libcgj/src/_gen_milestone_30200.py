...
"""
Soft continuum generator — MILESTONE 30200 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 30101–30200 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=30200   when parent has wired through graph_batch30200.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 30101 30200
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 30101 30200
...
graph batches ending at M. For M = **30200** the exclusive range is
**30101–30200** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 30101–30150  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 30151–30175  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 30176–30190  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 30191–30200  │ decade soft gates (Wave 160 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 30200 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_30200        1          graph_batch30191.c
  gj_libcgj_green_30200       1          graph_batch30192.c
  gj_bar3_ready_30200         0          graph_batch30193.c  (bar3 open)
  gj_product_score_30200      0          graph_batch30194.c  (score open)
  gj_continuum_ready_30200    1          graph_batch30195.c
  gj_smoke_soft_30200         1          graph_batch30196.c
  gj_dyn_soft_30200           1          graph_batch30197.c
  gj_milestone_tag_30200      30200      graph_batch30198.c
  gj_continuum_wave_30200     30200      graph_batch30199.c
  gj_batch_id_30200           30200      graph_batch30200.c
  gj_graph_milestone_30200    30200      graph_batch30200.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=30200`` means parent wired CGJ_SRCS through
    ``graph_batch30200.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_30200`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_30200`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M30200_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_30100`` / ``gj_*_30000`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 160 exclusive CREATE-ONLY lands the **markers** band
  (30191–30200) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_30200.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_30200.py

  # Restrict to marker band only (soft-gate TUs; Wave 160 exclusive):
  python3 user/libcgj/src/_gen_milestone_30200.py --band markers

  # Extension identity only (30101–30150):
  python3 user/libcgj/src/_gen_milestone_30200.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_30200.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_30100.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 30200 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 30200
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 30101
WAVE_HI = 30200  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 30101, 30150
IDENTITY_LO, IDENTITY_HI = 30151, 30175
FILL_LO, FILL_HI = 30176, 30190
MARKERS_LO, MARKERS_HI = 30191, 30200
...
# Stems become gj_<stem>_30200. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (30191, "shell_green", 1, "shell product green extract (wave 30200)"),
    (30192, "libcgj_green", 1, "libcgj product green extract (wave 30200)"),
    (30193, "bar3_ready", 0, "product bar3 readiness gate (wave 30200)"),
    (30194, "product_score", 0, "product readiness score tag (wave 30200)"),
    (30195, "continuum_ready", 1, "continuum readiness gate (wave 30200)"),
    (30196, "smoke_soft", 1, "smoke soft lamp (wave 30200)"),
    (30197, "dyn_soft", 1, "dyn soft lamp (wave 30200)"),
    (30198, "milestone_tag", MILESTONE, "milestone 30200 product tag"),
    (30199, "continuum_wave", MILESTONE, "milestone 30200 continuum wave identity"),
    # 30200 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        30176: "continuum wave fill soft unit + batch id",
        30180: "continuum bridge soft unit + batch id",
        30190: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (30191–30199), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 30193
    tag_batch = MARKERS_LO + 7  # 30198
    wave_batch = MARKERS_LO + 8  # 30199
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_30100 / gj_shell_green_30000 / gj_shell_green_29900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_30100 / gj_libcgj_green_30000 / gj_libcgj_green_29900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_30100 / gj_bar3_ready_30000 / gj_bar3_ready_29900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_30100 / gj_product_score_30000 / gj_product_score_29900\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_30100 / gj_continuum_ready_30000 /\n"
            f" * gj_continuum_ready_29900 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_30100 / gj_smoke_soft_30000 / gj_smoke_soft_29900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_30100 / gj_dyn_soft_30000 / gj_dyn_soft_29900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_30100 / gj_milestone_tag_30000 / gj_milestone_tag_29900,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_30100 / gj_continuum_wave_30000 /\n"
            f" * gj_continuum_wave_29900, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch30200.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 30199
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 30191–30199; 30200 is capstone
