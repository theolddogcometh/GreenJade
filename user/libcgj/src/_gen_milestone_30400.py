...
"""
Soft continuum generator — MILESTONE 30400 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 30301–30400 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=30400   when parent has wired through graph_batch30400.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 30301 30400
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 30301 30400
...
graph batches ending at M. For M = **30400** the exclusive range is
**30301–30400** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 30301–30350  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 30351–30375  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 30376–30390  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 30391–30400  │ decade soft gates (Wave 162 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 30400 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_30400        1          graph_batch30391.c
  gj_libcgj_green_30400       1          graph_batch30392.c
  gj_bar3_ready_30400         0          graph_batch30393.c  (bar3 open)
  gj_product_score_30400      0          graph_batch30394.c  (score open)
  gj_continuum_ready_30400    1          graph_batch30395.c
  gj_smoke_soft_30400         1          graph_batch30396.c
  gj_dyn_soft_30400           1          graph_batch30397.c
  gj_milestone_tag_30400      30400      graph_batch30398.c
  gj_continuum_wave_30400     30400      graph_batch30399.c
  gj_batch_id_30400           30400      graph_batch30400.c
  gj_graph_milestone_30400    30400      graph_batch30400.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=30400`` means parent wired CGJ_SRCS through
    ``graph_batch30400.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_30400`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_30400`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M30400_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_30300`` / ``gj_*_30200`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 162 exclusive CREATE-ONLY lands the **markers** band
  (30391–30400) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_30400.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_30400.py

  # Restrict to marker band only (soft-gate TUs; Wave 162 exclusive):
  python3 user/libcgj/src/_gen_milestone_30400.py --band markers

  # Extension identity only (30301–30350):
  python3 user/libcgj/src/_gen_milestone_30400.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_30400.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_30300.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 30400 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 30400
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 30301
WAVE_HI = 30400  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 30301, 30350
IDENTITY_LO, IDENTITY_HI = 30351, 30375
FILL_LO, FILL_HI = 30376, 30390
MARKERS_LO, MARKERS_HI = 30391, 30400
...
# Stems become gj_<stem>_30400. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (30391, "shell_green", 1, "shell product green extract (wave 30400)"),
    (30392, "libcgj_green", 1, "libcgj product green extract (wave 30400)"),
    (30393, "bar3_ready", 0, "product bar3 readiness gate (wave 30400)"),
    (30394, "product_score", 0, "product readiness score tag (wave 30400)"),
    (30395, "continuum_ready", 1, "continuum readiness gate (wave 30400)"),
    (30396, "smoke_soft", 1, "smoke soft lamp (wave 30400)"),
    (30397, "dyn_soft", 1, "dyn soft lamp (wave 30400)"),
    (30398, "milestone_tag", MILESTONE, "milestone 30400 product tag"),
    (30399, "continuum_wave", MILESTONE, "milestone 30400 continuum wave identity"),
    # 30400 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        30376: "continuum wave fill soft unit + batch id",
        30380: "continuum bridge soft unit + batch id",
        30390: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (30391–30399), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 30393
    tag_batch = MARKERS_LO + 7  # 30398
    wave_batch = MARKERS_LO + 8  # 30399
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_30300 / gj_shell_green_30200 / gj_shell_green_30100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_30300 / gj_libcgj_green_30200 / gj_libcgj_green_30100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_30300 / gj_bar3_ready_30200 / gj_bar3_ready_30100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_30300 / gj_product_score_30200 / gj_product_score_30100\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_30300 / gj_continuum_ready_30200 /\n"
            f" * gj_continuum_ready_30100 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_30300 / gj_smoke_soft_30200 / gj_smoke_soft_30100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_30300 / gj_dyn_soft_30200 / gj_dyn_soft_30100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_30300 / gj_milestone_tag_30200 / gj_milestone_tag_30100,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_30300 / gj_continuum_wave_30200 /\n"
            f" * gj_continuum_wave_30100, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch30400.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 30399
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 30391–30399; 30400 is capstone
