...
"""
Soft continuum generator — MILESTONE 30500 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 30401–30500 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=30500   when parent has wired through graph_batch30500.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 30401 30500
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 30401 30500
...
graph batches ending at M. For M = **30500** the exclusive range is
**30401–30500** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 30401–30450  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 30451–30475  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 30476–30490  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 30491–30500  │ decade soft gates (Wave 163 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 30500 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_30500        1          graph_batch30491.c
  gj_libcgj_green_30500       1          graph_batch30492.c
  gj_bar3_ready_30500         0          graph_batch30493.c  (bar3 open)
  gj_product_score_30500      0          graph_batch30494.c  (score open)
  gj_continuum_ready_30500    1          graph_batch30495.c
  gj_smoke_soft_30500         1          graph_batch30496.c
  gj_dyn_soft_30500           1          graph_batch30497.c
  gj_milestone_tag_30500      30500      graph_batch30498.c
  gj_continuum_wave_30500     30500      graph_batch30499.c
  gj_batch_id_30500           30500      graph_batch30500.c
  gj_graph_milestone_30500    30500      graph_batch30500.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=30500`` means parent wired CGJ_SRCS through
    ``graph_batch30500.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_30500`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_30500`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M30500_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_30400`` / ``gj_*_30300`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 163 exclusive CREATE-ONLY lands the **markers** band
  (30491–30500) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_30500.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_30500.py

  # Restrict to marker band only (soft-gate TUs; Wave 163 exclusive):
  python3 user/libcgj/src/_gen_milestone_30500.py --band markers

  # Extension identity only (30401–30450):
  python3 user/libcgj/src/_gen_milestone_30500.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_30500.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_30400.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 30500 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 30500
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 30401
WAVE_HI = 30500  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 30401, 30450
IDENTITY_LO, IDENTITY_HI = 30451, 30475
FILL_LO, FILL_HI = 30476, 30490
MARKERS_LO, MARKERS_HI = 30491, 30500
...
# Stems become gj_<stem>_30500. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (30491, "shell_green", 1, "shell product green extract (wave 30500)"),
    (30492, "libcgj_green", 1, "libcgj product green extract (wave 30500)"),
    (30493, "bar3_ready", 0, "product bar3 readiness gate (wave 30500)"),
    (30494, "product_score", 0, "product readiness score tag (wave 30500)"),
    (30495, "continuum_ready", 1, "continuum readiness gate (wave 30500)"),
    (30496, "smoke_soft", 1, "smoke soft lamp (wave 30500)"),
    (30497, "dyn_soft", 1, "dyn soft lamp (wave 30500)"),
    (30498, "milestone_tag", MILESTONE, "milestone 30500 product tag"),
    (30499, "continuum_wave", MILESTONE, "milestone 30500 continuum wave identity"),
    # 30500 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        30476: "continuum wave fill soft unit + batch id",
        30480: "continuum bridge soft unit + batch id",
        30490: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (30491–30499), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 30493
    tag_batch = MARKERS_LO + 7  # 30498
    wave_batch = MARKERS_LO + 8  # 30499
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_30400 / gj_shell_green_30300 / gj_shell_green_30200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_30400 / gj_libcgj_green_30300 / gj_libcgj_green_30200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_30400 / gj_bar3_ready_30300 / gj_bar3_ready_30200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_30400 / gj_product_score_30300 / gj_product_score_30200\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_30400 / gj_continuum_ready_30300 /\n"
            f" * gj_continuum_ready_30200 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_30400 / gj_smoke_soft_30300 / gj_smoke_soft_30200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_30400 / gj_dyn_soft_30300 / gj_dyn_soft_30200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_30400 / gj_milestone_tag_30300 / gj_milestone_tag_30200,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_30400 / gj_continuum_wave_30300 /\n"
            f" * gj_continuum_wave_30200, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch30500.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 30499
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 30491–30499; 30500 is capstone
