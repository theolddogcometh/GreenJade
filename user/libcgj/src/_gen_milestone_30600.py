...
"""
Soft continuum generator — MILESTONE 30600 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 30501–30600 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=30600   when parent has wired through graph_batch30600.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 30501 30600
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 30501 30600
...
graph batches ending at M. For M = **30600** the exclusive range is
**30501–30600** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 30501–30550  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 30551–30575  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 30576–30590  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 30591–30600  │ decade soft gates (Wave 164 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 30600 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_30600        1          graph_batch30591.c
  gj_libcgj_green_30600       1          graph_batch30592.c
  gj_bar3_ready_30600         0          graph_batch30593.c  (bar3 open)
  gj_product_score_30600      0          graph_batch30594.c  (score open)
  gj_continuum_ready_30600    1          graph_batch30595.c
  gj_smoke_soft_30600         1          graph_batch30596.c
  gj_dyn_soft_30600           1          graph_batch30597.c
  gj_milestone_tag_30600      30600      graph_batch30598.c
  gj_continuum_wave_30600     30600      graph_batch30599.c
  gj_batch_id_30600           30600      graph_batch30600.c
  gj_graph_milestone_30600    30600      graph_batch30600.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=30600`` means parent wired CGJ_SRCS through
    ``graph_batch30600.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_30600`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_30600`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M30600_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_30500`` / ``gj_*_30400`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 164 exclusive CREATE-ONLY lands the **markers** band
  (30591–30600) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_30600.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_30600.py

  # Restrict to marker band only (soft-gate TUs; Wave 164 exclusive):
  python3 user/libcgj/src/_gen_milestone_30600.py --band markers

  # Extension identity only (30501–30550):
  python3 user/libcgj/src/_gen_milestone_30600.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_30600.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_30500.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 30600 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 30600
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 30501
WAVE_HI = 30600  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 30501, 30550
IDENTITY_LO, IDENTITY_HI = 30551, 30575
FILL_LO, FILL_HI = 30576, 30590
MARKERS_LO, MARKERS_HI = 30591, 30600
...
# Stems become gj_<stem>_30600. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (30591, "shell_green", 1, "shell product green extract (wave 30600)"),
    (30592, "libcgj_green", 1, "libcgj product green extract (wave 30600)"),
    (30593, "bar3_ready", 0, "product bar3 readiness gate (wave 30600)"),
    (30594, "product_score", 0, "product readiness score tag (wave 30600)"),
    (30595, "continuum_ready", 1, "continuum readiness gate (wave 30600)"),
    (30596, "smoke_soft", 1, "smoke soft lamp (wave 30600)"),
    (30597, "dyn_soft", 1, "dyn soft lamp (wave 30600)"),
    (30598, "milestone_tag", MILESTONE, "milestone 30600 product tag"),
    (30599, "continuum_wave", MILESTONE, "milestone 30600 continuum wave identity"),
    # 30600 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        30576: "continuum wave fill soft unit + batch id",
        30580: "continuum bridge soft unit + batch id",
        30590: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (30591–30599), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 30593
    tag_batch = MARKERS_LO + 7  # 30598
    wave_batch = MARKERS_LO + 8  # 30599
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_30500 / gj_shell_green_30400 / gj_shell_green_30300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_30500 / gj_libcgj_green_30400 / gj_libcgj_green_30300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_30500 / gj_bar3_ready_30400 / gj_bar3_ready_30300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_30500 / gj_product_score_30400 / gj_product_score_30300\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_30500 / gj_continuum_ready_30400 /\n"
            f" * gj_continuum_ready_30300 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_30500 / gj_smoke_soft_30400 / gj_smoke_soft_30300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_30500 / gj_dyn_soft_30400 / gj_dyn_soft_30300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_30500 / gj_milestone_tag_30400 / gj_milestone_tag_30300,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_30500 / gj_continuum_wave_30400 /\n"
            f" * gj_continuum_wave_30300, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch30600.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 30599
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 30591–30599; 30600 is capstone
