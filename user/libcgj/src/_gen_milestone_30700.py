...
"""
Soft continuum generator — MILESTONE 30700 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 30601–30700 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=30700   when parent has wired through graph_batch30700.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 30601 30700
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 30601 30700
...
graph batches ending at M. For M = **30700** the exclusive range is
**30601–30700** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 30601–30650  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 30651–30675  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 30676–30690  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 30691–30700  │ decade soft gates (Wave 165 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 30700 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_30700        1          graph_batch30691.c
  gj_libcgj_green_30700       1          graph_batch30692.c
  gj_bar3_ready_30700         0          graph_batch30693.c  (bar3 open)
  gj_product_score_30700      0          graph_batch30694.c  (score open)
  gj_continuum_ready_30700    1          graph_batch30695.c
  gj_smoke_soft_30700         1          graph_batch30696.c
  gj_dyn_soft_30700           1          graph_batch30697.c
  gj_milestone_tag_30700      30700      graph_batch30698.c
  gj_continuum_wave_30700     30700      graph_batch30699.c
  gj_batch_id_30700           30700      graph_batch30700.c
  gj_graph_milestone_30700    30700      graph_batch30700.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=30700`` means parent wired CGJ_SRCS through
    ``graph_batch30700.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_30700`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_30700`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M30700_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_30600`` / ``gj_*_30500`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 165 exclusive CREATE-ONLY lands the **markers** band
  (30691–30700) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_30700.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_30700.py

  # Restrict to marker band only (soft-gate TUs; Wave 165 exclusive):
  python3 user/libcgj/src/_gen_milestone_30700.py --band markers

  # Extension identity only (30601–30650):
  python3 user/libcgj/src/_gen_milestone_30700.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_30700.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_30600.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 30700 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 30700
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 30601
WAVE_HI = 30700  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 30601, 30650
IDENTITY_LO, IDENTITY_HI = 30651, 30675
FILL_LO, FILL_HI = 30676, 30690
MARKERS_LO, MARKERS_HI = 30691, 30700
...
# Stems become gj_<stem>_30700. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (30691, "shell_green", 1, "shell product green extract (wave 30700)"),
    (30692, "libcgj_green", 1, "libcgj product green extract (wave 30700)"),
    (30693, "bar3_ready", 0, "product bar3 readiness gate (wave 30700)"),
    (30694, "product_score", 0, "product readiness score tag (wave 30700)"),
    (30695, "continuum_ready", 1, "continuum readiness gate (wave 30700)"),
    (30696, "smoke_soft", 1, "smoke soft lamp (wave 30700)"),
    (30697, "dyn_soft", 1, "dyn soft lamp (wave 30700)"),
    (30698, "milestone_tag", MILESTONE, "milestone 30700 product tag"),
    (30699, "continuum_wave", MILESTONE, "milestone 30700 continuum wave identity"),
    # 30700 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        30676: "continuum wave fill soft unit + batch id",
        30680: "continuum bridge soft unit + batch id",
        30690: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (30691–30699), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 30693
    tag_batch = MARKERS_LO + 7  # 30698
    wave_batch = MARKERS_LO + 8  # 30699
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_30600 / gj_shell_green_30500 / gj_shell_green_30400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_30600 / gj_libcgj_green_30500 / gj_libcgj_green_30400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_30600 / gj_bar3_ready_30500 / gj_bar3_ready_30400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_30600 / gj_product_score_30500 / gj_product_score_30400\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_30600 / gj_continuum_ready_30500 /\n"
            f" * gj_continuum_ready_30400 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_30600 / gj_smoke_soft_30500 / gj_smoke_soft_30400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_30600 / gj_dyn_soft_30500 / gj_dyn_soft_30400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_30600 / gj_milestone_tag_30500 / gj_milestone_tag_30400,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_30600 / gj_continuum_wave_30500 /\n"
            f" * gj_continuum_wave_30400, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch30700.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 30699
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 30691–30699; 30700 is capstone
