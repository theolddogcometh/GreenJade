...
"""
Soft continuum generator — MILESTONE 30800 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 30701–30800 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=30800   when parent has wired through graph_batch30800.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 30701 30800
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 30701 30800
...
graph batches ending at M. For M = **30800** the exclusive range is
**30701–30800** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 30701–30750  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 30751–30775  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 30776–30790  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 30791–30800  │ decade soft gates (Wave 166 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 30800 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_30800        1          graph_batch30791.c
  gj_libcgj_green_30800       1          graph_batch30792.c
  gj_bar3_ready_30800         0          graph_batch30793.c  (bar3 open)
  gj_product_score_30800      0          graph_batch30794.c  (score open)
  gj_continuum_ready_30800    1          graph_batch30795.c
  gj_smoke_soft_30800         1          graph_batch30796.c
  gj_dyn_soft_30800           1          graph_batch30797.c
  gj_milestone_tag_30800      30800      graph_batch30798.c
  gj_continuum_wave_30800     30800      graph_batch30799.c
  gj_batch_id_30800           30800      graph_batch30800.c
  gj_graph_milestone_30800    30800      graph_batch30800.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=30800`` means parent wired CGJ_SRCS through
    ``graph_batch30800.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_30800`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_30800`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M30800_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_30700`` / ``gj_*_30600`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 166 exclusive CREATE-ONLY lands the **markers** band
  (30791–30800) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_30800.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_30800.py

  # Restrict to marker band only (soft-gate TUs; Wave 166 exclusive):
  python3 user/libcgj/src/_gen_milestone_30800.py --band markers

  # Extension identity only (30701–30750):
  python3 user/libcgj/src/_gen_milestone_30800.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_30800.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_30700.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 30800 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 30800
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 30701
WAVE_HI = 30800  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 30701, 30750
IDENTITY_LO, IDENTITY_HI = 30751, 30775
FILL_LO, FILL_HI = 30776, 30790
MARKERS_LO, MARKERS_HI = 30791, 30800
...
# Stems become gj_<stem>_30800. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (30791, "shell_green", 1, "shell product green extract (wave 30800)"),
    (30792, "libcgj_green", 1, "libcgj product green extract (wave 30800)"),
    (30793, "bar3_ready", 0, "product bar3 readiness gate (wave 30800)"),
    (30794, "product_score", 0, "product readiness score tag (wave 30800)"),
    (30795, "continuum_ready", 1, "continuum readiness gate (wave 30800)"),
    (30796, "smoke_soft", 1, "smoke soft lamp (wave 30800)"),
    (30797, "dyn_soft", 1, "dyn soft lamp (wave 30800)"),
    (30798, "milestone_tag", MILESTONE, "milestone 30800 product tag"),
    (30799, "continuum_wave", MILESTONE, "milestone 30800 continuum wave identity"),
    # 30800 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        30776: "continuum wave fill soft unit + batch id",
        30780: "continuum bridge soft unit + batch id",
        30790: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (30791–30799), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 30793
    tag_batch = MARKERS_LO + 7  # 30798
    wave_batch = MARKERS_LO + 8  # 30799
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_30700 / gj_shell_green_30600 / gj_shell_green_30500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_30700 / gj_libcgj_green_30600 / gj_libcgj_green_30500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_30700 / gj_bar3_ready_30600 / gj_bar3_ready_30500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_30700 / gj_product_score_30600 / gj_product_score_30500\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_30700 / gj_continuum_ready_30600 /\n"
            f" * gj_continuum_ready_30500 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_30700 / gj_smoke_soft_30600 / gj_smoke_soft_30500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_30700 / gj_dyn_soft_30600 / gj_dyn_soft_30500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_30700 / gj_milestone_tag_30600 / gj_milestone_tag_30500,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_30700 / gj_continuum_wave_30600 /\n"
            f" * gj_continuum_wave_30500, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch30800.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 30799
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 30791–30799; 30800 is capstone
