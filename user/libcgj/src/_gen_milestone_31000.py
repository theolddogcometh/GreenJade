...
"""
Soft continuum generator — MILESTONE 31000 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 30901–31000 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=31000   when parent has wired through graph_batch31000.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 30901 31000
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 30901 31000
...
graph batches ending at M. For M = **31000** the exclusive range is
**30901–31000** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 30901–30950  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 30951–30975  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 30976–30990  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 30991–31000  │ decade soft gates (Wave 168 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 31000 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_31000        1          graph_batch30991.c
  gj_libcgj_green_31000       1          graph_batch30992.c
  gj_bar3_ready_31000         0          graph_batch30993.c  (bar3 open)
  gj_product_score_31000      0          graph_batch30994.c  (score open)
  gj_continuum_ready_31000    1          graph_batch30995.c
  gj_smoke_soft_31000         1          graph_batch30996.c
  gj_dyn_soft_31000           1          graph_batch30997.c
  gj_milestone_tag_31000      31000      graph_batch30998.c
  gj_continuum_wave_31000     31000      graph_batch30999.c
  gj_batch_id_31000           31000      graph_batch31000.c
  gj_graph_milestone_31000    31000      graph_batch31000.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=31000`` means parent wired CGJ_SRCS through
    ``graph_batch31000.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_31000`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_31000`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M31000_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_30900`` / ``gj_*_30800`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 168 exclusive CREATE-ONLY lands the **markers** band
  (30991–31000) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_31000.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_31000.py

  # Restrict to marker band only (soft-gate TUs; Wave 168 exclusive):
  python3 user/libcgj/src/_gen_milestone_31000.py --band markers

  # Extension identity only (30901–30950):
  python3 user/libcgj/src/_gen_milestone_31000.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_31000.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_30900.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 31000 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 31000
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 30901
WAVE_HI = 31000  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 30901, 30950
IDENTITY_LO, IDENTITY_HI = 30951, 30975
FILL_LO, FILL_HI = 30976, 30990
MARKERS_LO, MARKERS_HI = 30991, 31000
...
# Stems become gj_<stem>_31000. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (30991, "shell_green", 1, "shell product green extract (wave 31000)"),
    (30992, "libcgj_green", 1, "libcgj product green extract (wave 31000)"),
    (30993, "bar3_ready", 0, "product bar3 readiness gate (wave 31000)"),
    (30994, "product_score", 0, "product readiness score tag (wave 31000)"),
    (30995, "continuum_ready", 1, "continuum readiness gate (wave 31000)"),
    (30996, "smoke_soft", 1, "smoke soft lamp (wave 31000)"),
    (30997, "dyn_soft", 1, "dyn soft lamp (wave 31000)"),
    (30998, "milestone_tag", MILESTONE, "milestone 31000 product tag"),
    (30999, "continuum_wave", MILESTONE, "milestone 31000 continuum wave identity"),
    # 31000 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        30976: "continuum wave fill soft unit + batch id",
        30980: "continuum bridge soft unit + batch id",
        30990: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (30991–30999), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 30993
    tag_batch = MARKERS_LO + 7  # 30998
    wave_batch = MARKERS_LO + 8  # 30999
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_30900 / gj_shell_green_30800 / gj_shell_green_30700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_30900 / gj_libcgj_green_30800 / gj_libcgj_green_30700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_30900 / gj_bar3_ready_30800 / gj_bar3_ready_30700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_30900 / gj_product_score_30800 / gj_product_score_30700\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_30900 / gj_continuum_ready_30800 /\n"
            f" * gj_continuum_ready_30700 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_30900 / gj_smoke_soft_30800 / gj_smoke_soft_30700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_30900 / gj_dyn_soft_30800 / gj_dyn_soft_30700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_30900 / gj_milestone_tag_30800 / gj_milestone_tag_30700,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_30900 / gj_continuum_wave_30800 /\n"
            f" * gj_continuum_wave_30700, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch31000.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 30999
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 30991–30999; 31000 is capstone
