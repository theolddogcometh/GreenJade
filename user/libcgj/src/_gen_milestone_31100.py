...
"""
Soft continuum generator — MILESTONE 31100 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 31001–31100 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=31100   when parent has wired through graph_batch31100.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 31001 31100
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 31001 31100
...
graph batches ending at M. For M = **31100** the exclusive range is
**31001–31100** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 31001–31050  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 31051–31075  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 31076–31090  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 31091–31100  │ decade soft gates (Wave 169 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 31100 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_31100        1          graph_batch31091.c
  gj_libcgj_green_31100       1          graph_batch31092.c
  gj_bar3_ready_31100         0          graph_batch31093.c  (bar3 open)
  gj_product_score_31100      0          graph_batch31094.c  (score open)
  gj_continuum_ready_31100    1          graph_batch31095.c
  gj_smoke_soft_31100         1          graph_batch31096.c
  gj_dyn_soft_31100           1          graph_batch31097.c
  gj_milestone_tag_31100      31100      graph_batch31098.c
  gj_continuum_wave_31100     31100      graph_batch31099.c
  gj_batch_id_31100           31100      graph_batch31100.c
  gj_graph_milestone_31100    31100      graph_batch31100.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=31100`` means parent wired CGJ_SRCS through
    ``graph_batch31100.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_31100`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_31100`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M31100_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_31000`` / ``gj_*_30900`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 169 exclusive CREATE-ONLY lands the **markers** band
  (31091–31100) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_31100.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_31100.py

  # Restrict to marker band only (soft-gate TUs; Wave 169 exclusive):
  python3 user/libcgj/src/_gen_milestone_31100.py --band markers

  # Extension identity only (31001–31050):
  python3 user/libcgj/src/_gen_milestone_31100.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_31100.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_31000.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 31100 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 31100
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 31001
WAVE_HI = 31100  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 31001, 31050
IDENTITY_LO, IDENTITY_HI = 31051, 31075
FILL_LO, FILL_HI = 31076, 31090
MARKERS_LO, MARKERS_HI = 31091, 31100
...
# Stems become gj_<stem>_31100. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (31091, "shell_green", 1, "shell product green extract (wave 31100)"),
    (31092, "libcgj_green", 1, "libcgj product green extract (wave 31100)"),
    (31093, "bar3_ready", 0, "product bar3 readiness gate (wave 31100)"),
    (31094, "product_score", 0, "product readiness score tag (wave 31100)"),
    (31095, "continuum_ready", 1, "continuum readiness gate (wave 31100)"),
    (31096, "smoke_soft", 1, "smoke soft lamp (wave 31100)"),
    (31097, "dyn_soft", 1, "dyn soft lamp (wave 31100)"),
    (31098, "milestone_tag", MILESTONE, "milestone 31100 product tag"),
    (31099, "continuum_wave", MILESTONE, "milestone 31100 continuum wave identity"),
    # 31100 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        31076: "continuum wave fill soft unit + batch id",
        31080: "continuum bridge soft unit + batch id",
        31090: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (31091–31099), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 31093
    tag_batch = MARKERS_LO + 7  # 31098
    wave_batch = MARKERS_LO + 8  # 31099
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_31000 / gj_shell_green_30900 / gj_shell_green_30800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_31000 / gj_libcgj_green_30900 / gj_libcgj_green_30800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_31000 / gj_bar3_ready_30900 / gj_bar3_ready_30800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_31000 / gj_product_score_30900 / gj_product_score_30800\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_31000 / gj_continuum_ready_30900 /\n"
            f" * gj_continuum_ready_30800 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_31000 / gj_smoke_soft_30900 / gj_smoke_soft_30800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_31000 / gj_dyn_soft_30900 / gj_dyn_soft_30800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_31000 / gj_milestone_tag_30900 / gj_milestone_tag_30800,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_31000 / gj_continuum_wave_30900 /\n"
            f" * gj_continuum_wave_30800, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch31100.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 31099
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 31091–31099; 31100 is capstone
