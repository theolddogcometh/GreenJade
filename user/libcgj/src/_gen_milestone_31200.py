...
"""
Soft continuum generator — MILESTONE 31200 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 31101–31200 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=31200   when parent has wired through graph_batch31200.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 31101 31200
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 31101 31200
...
graph batches ending at M. For M = **31200** the exclusive range is
**31101–31200** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 31101–31150  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 31151–31175  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 31176–31190  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 31191–31200  │ decade soft gates (Wave 170 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 31200 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_31200        1          graph_batch31191.c
  gj_libcgj_green_31200       1          graph_batch31192.c
  gj_bar3_ready_31200         0          graph_batch31193.c  (bar3 open)
  gj_product_score_31200      0          graph_batch31194.c  (score open)
  gj_continuum_ready_31200    1          graph_batch31195.c
  gj_smoke_soft_31200         1          graph_batch31196.c
  gj_dyn_soft_31200           1          graph_batch31197.c
  gj_milestone_tag_31200      31200      graph_batch31198.c
  gj_continuum_wave_31200     31200      graph_batch31199.c
  gj_batch_id_31200           31200      graph_batch31200.c
  gj_graph_milestone_31200    31200      graph_batch31200.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=31200`` means parent wired CGJ_SRCS through
    ``graph_batch31200.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_31200`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_31200`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M31200_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_31100`` / ``gj_*_31000`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 170 exclusive CREATE-ONLY lands the **markers** band
  (31191–31200) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_31200.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_31200.py

  # Restrict to marker band only (soft-gate TUs; Wave 170 exclusive):
  python3 user/libcgj/src/_gen_milestone_31200.py --band markers

  # Extension identity only (31101–31150):
  python3 user/libcgj/src/_gen_milestone_31200.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_31200.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_31100.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 31200 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 31200
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 31101
WAVE_HI = 31200  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 31101, 31150
IDENTITY_LO, IDENTITY_HI = 31151, 31175
FILL_LO, FILL_HI = 31176, 31190
MARKERS_LO, MARKERS_HI = 31191, 31200
...
# Stems become gj_<stem>_31200. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (31191, "shell_green", 1, "shell product green extract (wave 31200)"),
    (31192, "libcgj_green", 1, "libcgj product green extract (wave 31200)"),
    (31193, "bar3_ready", 0, "product bar3 readiness gate (wave 31200)"),
    (31194, "product_score", 0, "product readiness score tag (wave 31200)"),
    (31195, "continuum_ready", 1, "continuum readiness gate (wave 31200)"),
    (31196, "smoke_soft", 1, "smoke soft lamp (wave 31200)"),
    (31197, "dyn_soft", 1, "dyn soft lamp (wave 31200)"),
    (31198, "milestone_tag", MILESTONE, "milestone 31200 product tag"),
    (31199, "continuum_wave", MILESTONE, "milestone 31200 continuum wave identity"),
    # 31200 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        31176: "continuum wave fill soft unit + batch id",
        31180: "continuum bridge soft unit + batch id",
        31190: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (31191–31199), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 31193
    tag_batch = MARKERS_LO + 7  # 31198
    wave_batch = MARKERS_LO + 8  # 31199
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_31100 / gj_shell_green_31000 / gj_shell_green_30900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_31100 / gj_libcgj_green_31000 / gj_libcgj_green_30900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_31100 / gj_bar3_ready_31000 / gj_bar3_ready_30900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_31100 / gj_product_score_31000 / gj_product_score_30900\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_31100 / gj_continuum_ready_31000 /\n"
            f" * gj_continuum_ready_30900 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_31100 / gj_smoke_soft_31000 / gj_smoke_soft_30900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_31100 / gj_dyn_soft_31000 / gj_dyn_soft_30900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_31100 / gj_milestone_tag_31000 / gj_milestone_tag_30900,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_31100 / gj_continuum_wave_31000 /\n"
            f" * gj_continuum_wave_30900, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch31200.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 31199
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 31191–31199; 31200 is capstone
