...
"""
Soft continuum generator — MILESTONE 31300 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 31201–31300 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=31300   when parent has wired through graph_batch31300.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 31201 31300
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 31201 31300
...
graph batches ending at M. For M = **31300** the exclusive range is
**31201–31300** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 31201–31250  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 31251–31275  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 31276–31290  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 31291–31300  │ decade soft gates (Wave 171 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 31300 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_31300        1          graph_batch31291.c
  gj_libcgj_green_31300       1          graph_batch31292.c
  gj_bar3_ready_31300         0          graph_batch31293.c  (bar3 open)
  gj_product_score_31300      0          graph_batch31294.c  (score open)
  gj_continuum_ready_31300    1          graph_batch31295.c
  gj_smoke_soft_31300         1          graph_batch31296.c
  gj_dyn_soft_31300           1          graph_batch31297.c
  gj_milestone_tag_31300      31300      graph_batch31298.c
  gj_continuum_wave_31300     31300      graph_batch31299.c
  gj_batch_id_31300           31300      graph_batch31300.c
  gj_graph_milestone_31300    31300      graph_batch31300.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=31300`` means parent wired CGJ_SRCS through
    ``graph_batch31300.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_31300`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_31300`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M31300_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_31200`` / ``gj_*_31100`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 171 exclusive CREATE-ONLY lands the **markers** band
  (31291–31300) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_31300.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_31300.py

  # Restrict to marker band only (soft-gate TUs; Wave 171 exclusive):
  python3 user/libcgj/src/_gen_milestone_31300.py --band markers

  # Extension identity only (31201–31250):
  python3 user/libcgj/src/_gen_milestone_31300.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_31300.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_31200.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 31300 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 31300
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 31201
WAVE_HI = 31300  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 31201, 31250
IDENTITY_LO, IDENTITY_HI = 31251, 31275
FILL_LO, FILL_HI = 31276, 31290
MARKERS_LO, MARKERS_HI = 31291, 31300
...
# Stems become gj_<stem>_31300. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (31291, "shell_green", 1, "shell product green extract (wave 31300)"),
    (31292, "libcgj_green", 1, "libcgj product green extract (wave 31300)"),
    (31293, "bar3_ready", 0, "product bar3 readiness gate (wave 31300)"),
    (31294, "product_score", 0, "product readiness score tag (wave 31300)"),
    (31295, "continuum_ready", 1, "continuum readiness gate (wave 31300)"),
    (31296, "smoke_soft", 1, "smoke soft lamp (wave 31300)"),
    (31297, "dyn_soft", 1, "dyn soft lamp (wave 31300)"),
    (31298, "milestone_tag", MILESTONE, "milestone 31300 product tag"),
    (31299, "continuum_wave", MILESTONE, "milestone 31300 continuum wave identity"),
    # 31300 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        31276: "continuum wave fill soft unit + batch id",
        31280: "continuum bridge soft unit + batch id",
        31290: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (31291–31299), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 31293
    tag_batch = MARKERS_LO + 7  # 31298
    wave_batch = MARKERS_LO + 8  # 31299
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_31200 / gj_shell_green_31100 / gj_shell_green_31000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_31200 / gj_libcgj_green_31100 / gj_libcgj_green_31000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_31200 / gj_bar3_ready_31100 / gj_bar3_ready_31000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_31200 / gj_product_score_31100 / gj_product_score_31000\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_31200 / gj_continuum_ready_31100 /\n"
            f" * gj_continuum_ready_31000 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_31200 / gj_smoke_soft_31100 / gj_smoke_soft_31000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_31200 / gj_dyn_soft_31100 / gj_dyn_soft_31000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_31200 / gj_milestone_tag_31100 / gj_milestone_tag_31000,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_31200 / gj_continuum_wave_31100 /\n"
            f" * gj_continuum_wave_31000, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch31300.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 31299
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 31291–31299; 31300 is capstone
