...
"""
Soft continuum generator — MILESTONE 31400 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 31301–31400 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=31400   when parent has wired through graph_batch31400.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 31301 31400
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 31301 31400
...
graph batches ending at M. For M = **31400** the exclusive range is
**31301–31400** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 31301–31350  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 31351–31375  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 31376–31390  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 31391–31400  │ decade soft gates (Wave 172 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 31400 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_31400        1          graph_batch31391.c
  gj_libcgj_green_31400       1          graph_batch31392.c
  gj_bar3_ready_31400         0          graph_batch31393.c  (bar3 open)
  gj_product_score_31400      0          graph_batch31394.c  (score open)
  gj_continuum_ready_31400    1          graph_batch31395.c
  gj_smoke_soft_31400         1          graph_batch31396.c
  gj_dyn_soft_31400           1          graph_batch31397.c
  gj_milestone_tag_31400      31400      graph_batch31398.c
  gj_continuum_wave_31400     31400      graph_batch31399.c
  gj_batch_id_31400           31400      graph_batch31400.c
  gj_graph_milestone_31400    31400      graph_batch31400.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=31400`` means parent wired CGJ_SRCS through
    ``graph_batch31400.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_31400`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_31400`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M31400_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_31300`` / ``gj_*_31200`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 172 exclusive CREATE-ONLY lands the **markers** band
  (31391–31400) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_31400.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_31400.py

  # Restrict to marker band only (soft-gate TUs; Wave 172 exclusive):
  python3 user/libcgj/src/_gen_milestone_31400.py --band markers

  # Extension identity only (31301–31350):
  python3 user/libcgj/src/_gen_milestone_31400.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_31400.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_31300.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 31400 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 31400
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 31301
WAVE_HI = 31400  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 31301, 31350
IDENTITY_LO, IDENTITY_HI = 31351, 31375
FILL_LO, FILL_HI = 31376, 31390
MARKERS_LO, MARKERS_HI = 31391, 31400
...
# Stems become gj_<stem>_31400. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (31391, "shell_green", 1, "shell product green extract (wave 31400)"),
    (31392, "libcgj_green", 1, "libcgj product green extract (wave 31400)"),
    (31393, "bar3_ready", 0, "product bar3 readiness gate (wave 31400)"),
    (31394, "product_score", 0, "product readiness score tag (wave 31400)"),
    (31395, "continuum_ready", 1, "continuum readiness gate (wave 31400)"),
    (31396, "smoke_soft", 1, "smoke soft lamp (wave 31400)"),
    (31397, "dyn_soft", 1, "dyn soft lamp (wave 31400)"),
    (31398, "milestone_tag", MILESTONE, "milestone 31400 product tag"),
    (31399, "continuum_wave", MILESTONE, "milestone 31400 continuum wave identity"),
    # 31400 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        31376: "continuum wave fill soft unit + batch id",
        31380: "continuum bridge soft unit + batch id",
        31390: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (31391–31399), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 31393
    tag_batch = MARKERS_LO + 7  # 31398
    wave_batch = MARKERS_LO + 8  # 31399
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_31300 / gj_shell_green_31200 / gj_shell_green_31100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_31300 / gj_libcgj_green_31200 / gj_libcgj_green_31100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_31300 / gj_bar3_ready_31200 / gj_bar3_ready_31100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_31300 / gj_product_score_31200 / gj_product_score_31100\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_31300 / gj_continuum_ready_31200 /\n"
            f" * gj_continuum_ready_31100 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_31300 / gj_smoke_soft_31200 / gj_smoke_soft_31100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_31300 / gj_dyn_soft_31200 / gj_dyn_soft_31100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_31300 / gj_milestone_tag_31200 / gj_milestone_tag_31100,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_31300 / gj_continuum_wave_31200 /\n"
            f" * gj_continuum_wave_31100, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch31400.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 31399
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 31391–31399; 31400 is capstone
