...
"""
Soft continuum generator — MILESTONE 31600 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 31501–31600 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=31600   when parent has wired through graph_batch31600.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 31501 31600
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 31501 31600
...
graph batches ending at M. For M = **31600** the exclusive range is
**31501–31600** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 31501–31550  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 31551–31575  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 31576–31590  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 31591–31600  │ decade soft gates (Wave 174 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 31600 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_31600        1          graph_batch31591.c
  gj_libcgj_green_31600       1          graph_batch31592.c
  gj_bar3_ready_31600         0          graph_batch31593.c  (bar3 open)
  gj_product_score_31600      0          graph_batch31594.c  (score open)
  gj_continuum_ready_31600    1          graph_batch31595.c
  gj_smoke_soft_31600         1          graph_batch31596.c
  gj_dyn_soft_31600           1          graph_batch31597.c
  gj_milestone_tag_31600      31600      graph_batch31598.c
  gj_continuum_wave_31600     31600      graph_batch31599.c
  gj_batch_id_31600           31600      graph_batch31600.c
  gj_graph_milestone_31600    31600      graph_batch31600.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=31600`` means parent wired CGJ_SRCS through
    ``graph_batch31600.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_31600`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_31600`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M31600_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_31500`` / ``gj_*_31400`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 174 exclusive CREATE-ONLY lands the **markers** band
  (31591–31600) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_31600.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_31600.py

  # Restrict to marker band only (soft-gate TUs; Wave 174 exclusive):
  python3 user/libcgj/src/_gen_milestone_31600.py --band markers

  # Extension identity only (31501–31550):
  python3 user/libcgj/src/_gen_milestone_31600.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_31600.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_31500.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 31600 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 31600
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 31501
WAVE_HI = 31600  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 31501, 31550
IDENTITY_LO, IDENTITY_HI = 31551, 31575
FILL_LO, FILL_HI = 31576, 31590
MARKERS_LO, MARKERS_HI = 31591, 31600
...
# Stems become gj_<stem>_31600. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (31591, "shell_green", 1, "shell product green extract (wave 31600)"),
    (31592, "libcgj_green", 1, "libcgj product green extract (wave 31600)"),
    (31593, "bar3_ready", 0, "product bar3 readiness gate (wave 31600)"),
    (31594, "product_score", 0, "product readiness score tag (wave 31600)"),
    (31595, "continuum_ready", 1, "continuum readiness gate (wave 31600)"),
    (31596, "smoke_soft", 1, "smoke soft lamp (wave 31600)"),
    (31597, "dyn_soft", 1, "dyn soft lamp (wave 31600)"),
    (31598, "milestone_tag", MILESTONE, "milestone 31600 product tag"),
    (31599, "continuum_wave", MILESTONE, "milestone 31600 continuum wave identity"),
    # 31600 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        31576: "continuum wave fill soft unit + batch id",
        31580: "continuum bridge soft unit + batch id",
        31590: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (31591–31599), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 31593
    tag_batch = MARKERS_LO + 7  # 31598
    wave_batch = MARKERS_LO + 8  # 31599
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_31500 / gj_shell_green_31400 / gj_shell_green_31300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_31500 / gj_libcgj_green_31400 / gj_libcgj_green_31300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_31500 / gj_bar3_ready_31400 / gj_bar3_ready_31300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_31500 / gj_product_score_31400 / gj_product_score_31300\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_31500 / gj_continuum_ready_31400 /\n"
            f" * gj_continuum_ready_31300 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_31500 / gj_smoke_soft_31400 / gj_smoke_soft_31300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_31500 / gj_dyn_soft_31400 / gj_dyn_soft_31300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_31500 / gj_milestone_tag_31400 / gj_milestone_tag_31300,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_31500 / gj_continuum_wave_31400 /\n"
            f" * gj_continuum_wave_31300, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch31600.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 31599
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 31591–31599; 31600 is capstone
