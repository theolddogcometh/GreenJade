...
"""
Soft continuum generator — MILESTONE 31700 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 31601–31700 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=31700   when parent has wired through graph_batch31700.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 31601 31700
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 31601 31700
...
graph batches ending at M. For M = **31700** the exclusive range is
**31601–31700** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 31601–31650  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 31651–31675  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 31676–31690  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 31691–31700  │ decade soft gates (Wave 175 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 31700 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_31700        1          graph_batch31691.c
  gj_libcgj_green_31700       1          graph_batch31692.c
  gj_bar3_ready_31700         0          graph_batch31693.c  (bar3 open)
  gj_product_score_31700      0          graph_batch31694.c  (score open)
  gj_continuum_ready_31700    1          graph_batch31695.c
  gj_smoke_soft_31700         1          graph_batch31696.c
  gj_dyn_soft_31700           1          graph_batch31697.c
  gj_milestone_tag_31700      31700      graph_batch31698.c
  gj_continuum_wave_31700     31700      graph_batch31699.c
  gj_batch_id_31700           31700      graph_batch31700.c
  gj_graph_milestone_31700    31700      graph_batch31700.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=31700`` means parent wired CGJ_SRCS through
    ``graph_batch31700.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_31700`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_31700`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M31700_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_31600`` / ``gj_*_31500`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 175 exclusive CREATE-ONLY lands the **markers** band
  (31691–31700) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_31700.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_31700.py

  # Restrict to marker band only (soft-gate TUs; Wave 175 exclusive):
  python3 user/libcgj/src/_gen_milestone_31700.py --band markers

  # Extension identity only (31601–31650):
  python3 user/libcgj/src/_gen_milestone_31700.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_31700.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_31600.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 31700 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 31700
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 31601
WAVE_HI = 31700  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 31601, 31650
IDENTITY_LO, IDENTITY_HI = 31651, 31675
FILL_LO, FILL_HI = 31676, 31690
MARKERS_LO, MARKERS_HI = 31691, 31700
...
# Stems become gj_<stem>_31700. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (31691, "shell_green", 1, "shell product green extract (wave 31700)"),
    (31692, "libcgj_green", 1, "libcgj product green extract (wave 31700)"),
    (31693, "bar3_ready", 0, "product bar3 readiness gate (wave 31700)"),
    (31694, "product_score", 0, "product readiness score tag (wave 31700)"),
    (31695, "continuum_ready", 1, "continuum readiness gate (wave 31700)"),
    (31696, "smoke_soft", 1, "smoke soft lamp (wave 31700)"),
    (31697, "dyn_soft", 1, "dyn soft lamp (wave 31700)"),
    (31698, "milestone_tag", MILESTONE, "milestone 31700 product tag"),
    (31699, "continuum_wave", MILESTONE, "milestone 31700 continuum wave identity"),
    # 31700 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        31676: "continuum wave fill soft unit + batch id",
        31680: "continuum bridge soft unit + batch id",
        31690: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (31691–31699), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 31693
    tag_batch = MARKERS_LO + 7  # 31698
    wave_batch = MARKERS_LO + 8  # 31699
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_31600 / gj_shell_green_31500 / gj_shell_green_31400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_31600 / gj_libcgj_green_31500 / gj_libcgj_green_31400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_31600 / gj_bar3_ready_31500 / gj_bar3_ready_31400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_31600 / gj_product_score_31500 / gj_product_score_31400\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_31600 / gj_continuum_ready_31500 /\n"
            f" * gj_continuum_ready_31400 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_31600 / gj_smoke_soft_31500 / gj_smoke_soft_31400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_31600 / gj_dyn_soft_31500 / gj_dyn_soft_31400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_31600 / gj_milestone_tag_31500 / gj_milestone_tag_31400,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_31600 / gj_continuum_wave_31500 /\n"
            f" * gj_continuum_wave_31400, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch31700.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 31699
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 31691–31699; 31700 is capstone
