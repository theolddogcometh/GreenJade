...
"""
Soft continuum generator — MILESTONE 31800 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 31701–31800 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=31800   when parent has wired through graph_batch31800.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 31701 31800
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 31701 31800
...
graph batches ending at M. For M = **31800** the exclusive range is
**31701–31800** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 31701–31750  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 31751–31775  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 31776–31790  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 31791–31800  │ decade soft gates (Wave 176 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 31800 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_31800        1          graph_batch31791.c
  gj_libcgj_green_31800       1          graph_batch31792.c
  gj_bar3_ready_31800         0          graph_batch31793.c  (bar3 open)
  gj_product_score_31800      0          graph_batch31794.c  (score open)
  gj_continuum_ready_31800    1          graph_batch31795.c
  gj_smoke_soft_31800         1          graph_batch31796.c
  gj_dyn_soft_31800           1          graph_batch31797.c
  gj_milestone_tag_31800      31800      graph_batch31798.c
  gj_continuum_wave_31800     31800      graph_batch31799.c
  gj_batch_id_31800           31800      graph_batch31800.c
  gj_graph_milestone_31800    31800      graph_batch31800.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=31800`` means parent wired CGJ_SRCS through
    ``graph_batch31800.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_31800`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_31800`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M31800_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_31700`` / ``gj_*_31600`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 176 exclusive CREATE-ONLY lands the **markers** band
  (31791–31800) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_31800.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_31800.py

  # Restrict to marker band only (soft-gate TUs; Wave 176 exclusive):
  python3 user/libcgj/src/_gen_milestone_31800.py --band markers

  # Extension identity only (31701–31750):
  python3 user/libcgj/src/_gen_milestone_31800.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_31800.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_31700.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 31800 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 31800
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 31701
WAVE_HI = 31800  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 31701, 31750
IDENTITY_LO, IDENTITY_HI = 31751, 31775
FILL_LO, FILL_HI = 31776, 31790
MARKERS_LO, MARKERS_HI = 31791, 31800
...
# Stems become gj_<stem>_31800. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (31791, "shell_green", 1, "shell product green extract (wave 31800)"),
    (31792, "libcgj_green", 1, "libcgj product green extract (wave 31800)"),
    (31793, "bar3_ready", 0, "product bar3 readiness gate (wave 31800)"),
    (31794, "product_score", 0, "product readiness score tag (wave 31800)"),
    (31795, "continuum_ready", 1, "continuum readiness gate (wave 31800)"),
    (31796, "smoke_soft", 1, "smoke soft lamp (wave 31800)"),
    (31797, "dyn_soft", 1, "dyn soft lamp (wave 31800)"),
    (31798, "milestone_tag", MILESTONE, "milestone 31800 product tag"),
    (31799, "continuum_wave", MILESTONE, "milestone 31800 continuum wave identity"),
    # 31800 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        31776: "continuum wave fill soft unit + batch id",
        31780: "continuum bridge soft unit + batch id",
        31790: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (31791–31799), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 31793
    tag_batch = MARKERS_LO + 7  # 31798
    wave_batch = MARKERS_LO + 8  # 31799
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_31700 / gj_shell_green_31600 / gj_shell_green_31500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_31700 / gj_libcgj_green_31600 / gj_libcgj_green_31500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_31700 / gj_bar3_ready_31600 / gj_bar3_ready_31500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_31700 / gj_product_score_31600 / gj_product_score_31500\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_31700 / gj_continuum_ready_31600 /\n"
            f" * gj_continuum_ready_31500 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_31700 / gj_smoke_soft_31600 / gj_smoke_soft_31500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_31700 / gj_dyn_soft_31600 / gj_dyn_soft_31500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_31700 / gj_milestone_tag_31600 / gj_milestone_tag_31500,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_31700 / gj_continuum_wave_31600 /\n"
            f" * gj_continuum_wave_31500, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch31800.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 31799
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 31791–31799; 31800 is capstone
