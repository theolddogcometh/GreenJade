...
"""
Soft continuum generator — MILESTONE 31900 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 31801–31900 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=31900   when parent has wired through graph_batch31600.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 31801 31900
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 31801 31900
...
graph batches ending at M. For M = **31900** the exclusive range is
**31801–31900** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 31801–31850  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 31851–31875  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 31876–31890  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 31891–31900  │ decade soft gates (Wave 177 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 31900 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_31900        1          graph_batch31891.c
  gj_libcgj_green_31900       1          graph_batch31892.c
  gj_bar3_ready_31900         0          graph_batch31893.c  (bar3 open)
  gj_product_score_31900      0          graph_batch31894.c  (score open)
  gj_continuum_ready_31900    1          graph_batch31895.c
  gj_smoke_soft_31900         1          graph_batch31896.c
  gj_dyn_soft_31900           1          graph_batch31897.c
  gj_milestone_tag_31900      31900      graph_batch31898.c
  gj_continuum_wave_31900     31900      graph_batch31899.c
  gj_batch_id_31900           31900      graph_batch31900.c
  gj_graph_milestone_31900    31900      graph_batch31900.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=31900`` means parent wired CGJ_SRCS through
    ``graph_batch31900.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_31900`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_31900`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M31900_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_31800`` / ``gj_*_31700`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 177 exclusive CREATE-ONLY lands the **markers** band
  (31891–31900) first; extension/identity/fill may be emitted later via
...
-----
  ...
  # Restrict to marker band only (soft-gate TUs; Wave 177 exclusive):
  python3 user/libcgj/src/_gen_milestone_31600.py --band markers

  # Extension identity only (31801–31850):
...
See also
--------
  user/libcgj/src/_gen_milestone_31800.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 31900 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 31900
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 31801
WAVE_HI = 31900  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 31801, 31850
IDENTITY_LO, IDENTITY_HI = 31851, 31875
FILL_LO, FILL_HI = 31876, 31890
MARKERS_LO, MARKERS_HI = 31891, 31900
...
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (31891, "shell_green", 1, "shell product green extract (wave 31900)"),
    (31892, "libcgj_green", 1, "libcgj product green extract (wave 31900)"),
    (31893, "bar3_ready", 0, "product bar3 readiness gate (wave 31900)"),
    (31894, "product_score", 0, "product readiness score tag (wave 31900)"),
    (31895, "continuum_ready", 1, "continuum readiness gate (wave 31900)"),
    (31896, "smoke_soft", 1, "smoke soft lamp (wave 31900)"),
    (31897, "dyn_soft", 1, "dyn soft lamp (wave 31900)"),
    (31898, "milestone_tag", MILESTONE, "milestone 31900 product tag"),
    (31899, "continuum_wave", MILESTONE, "milestone 31900 continuum wave identity"),
    # 31900 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        31876: "continuum wave fill soft unit + batch id",
        31880: "continuum bridge soft unit + batch id",
        31890: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (31891–31899), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 31893
    tag_batch = MARKERS_LO + 7  # 31898
    wave_batch = MARKERS_LO + 8  # 31899
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_31800 / gj_shell_green_31700 / gj_shell_green_31600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_31800 / gj_libcgj_green_31700 / gj_libcgj_green_31600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_31800 / gj_bar3_ready_31700 / gj_bar3_ready_31600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_31800 / gj_product_score_31700 / gj_product_score_31600\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_31800 / gj_continuum_ready_31700 /\n"
            f" * gj_continuum_ready_31600 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_31800 / gj_smoke_soft_31700 / gj_smoke_soft_31600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_31800 / gj_dyn_soft_31700 / gj_dyn_soft_31600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_31800 / gj_milestone_tag_31700 / gj_milestone_tag_31600,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_31800 / gj_continuum_wave_31700 /\n"
            f" * gj_continuum_wave_31600, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch31900.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 31899
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 31891–31899; 31900 is capstone
