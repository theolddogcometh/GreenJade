...
"""
Soft continuum generator — MILESTONE 32000 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 31901–32000 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=32000   when parent has wired through graph_batch32000.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 31901 32000
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 31901 32000
...
graph batches ending at M. For M = **32000** the exclusive range is
**31901–32000** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 31901–31950  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 31951–31975  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 31976–31990  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 31991–32000  │ decade soft gates (Wave 178 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 32000 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_32000        1          graph_batch31591.c
  gj_libcgj_green_32000       1          graph_batch31592.c
  gj_bar3_ready_32000         0          graph_batch31593.c  (bar3 open)
  gj_product_score_32000      0          graph_batch31594.c  (score open)
  gj_continuum_ready_32000    1          graph_batch31595.c
  gj_smoke_soft_32000         1          graph_batch31596.c
  gj_dyn_soft_32000           1          graph_batch31597.c
  gj_milestone_tag_32000      32000      graph_batch31598.c
  gj_continuum_wave_32000     32000      graph_batch31599.c
  gj_batch_id_32000           32000      graph_batch32000.c
  gj_graph_milestone_32000    32000      graph_batch32000.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=32000`` means parent wired CGJ_SRCS through
    ``graph_batch32000.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_32000`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_32000`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M32000_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_31900`` / ``gj_*_31800`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 178 exclusive CREATE-ONLY lands the **markers** band
  (31991–32000) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_32000.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_32000.py

  # Restrict to marker band only (soft-gate TUs; Wave 178 exclusive):
  python3 user/libcgj/src/_gen_milestone_32000.py --band markers

  # Extension identity only (31901–31950):
  python3 user/libcgj/src/_gen_milestone_32000.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_32000.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_31900.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 32000 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 32000
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 31901
WAVE_HI = 32000  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 31901, 31950
IDENTITY_LO, IDENTITY_HI = 31951, 31975
FILL_LO, FILL_HI = 31976, 31990
MARKERS_LO, MARKERS_HI = 31991, 32000
...
# Stems become gj_<stem>_32000. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (31991, "shell_green", 1, "shell product green extract (wave 32000)"),
    (31992, "libcgj_green", 1, "libcgj product green extract (wave 32000)"),
    (31993, "bar3_ready", 0, "product bar3 readiness gate (wave 32000)"),
    (31994, "product_score", 0, "product readiness score tag (wave 32000)"),
    (31995, "continuum_ready", 1, "continuum readiness gate (wave 32000)"),
    (31996, "smoke_soft", 1, "smoke soft lamp (wave 32000)"),
    (31997, "dyn_soft", 1, "dyn soft lamp (wave 32000)"),
    (31998, "milestone_tag", MILESTONE, "milestone 32000 product tag"),
    (31999, "continuum_wave", MILESTONE, "milestone 32000 continuum wave identity"),
    # 32000 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        31976: "continuum wave fill soft unit + batch id",
        31980: "continuum bridge soft unit + batch id",
        31990: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (31991–31999), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 31993
    tag_batch = MARKERS_LO + 7  # 31998
    wave_batch = MARKERS_LO + 8  # 31999
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_31900 / gj_shell_green_31800 / gj_shell_green_31700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_31900 / gj_libcgj_green_31800 / gj_libcgj_green_31700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_31900 / gj_bar3_ready_31800 / gj_bar3_ready_31700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_31900 / gj_product_score_31800 / gj_product_score_31700\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_31900 / gj_continuum_ready_31800 /\n"
            f" * gj_continuum_ready_31700 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_31900 / gj_smoke_soft_31800 / gj_smoke_soft_31700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_31900 / gj_dyn_soft_31800 / gj_dyn_soft_31700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_31900 / gj_milestone_tag_31800 / gj_milestone_tag_31700,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_31900 / gj_continuum_wave_31800 /\n"
            f" * gj_continuum_wave_31700, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch32000.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 31999
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 31991–31999; 32000 is capstone
