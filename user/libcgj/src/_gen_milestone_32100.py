...
"""
Soft continuum generator — MILESTONE 32100 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 32001–32100 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=32100   when parent has wired through graph_batch32100.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 32001 32100
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 32001 32100
...
graph batches ending at M. For M = **32100** the exclusive range is
**32001–32100** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 32001–32050  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 32051–32075  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 32076–32090  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 32091–32100  │ decade soft gates (Wave 179 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 32100 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_32100        1          graph_batch32091.c
  gj_libcgj_green_32100       1          graph_batch32092.c
  gj_bar3_ready_32100         0          graph_batch32093.c  (bar3 open)
  gj_product_score_32100      0          graph_batch32094.c  (score open)
  gj_continuum_ready_32100    1          graph_batch32095.c
  gj_smoke_soft_32100         1          graph_batch32096.c
  gj_dyn_soft_32100           1          graph_batch32097.c
  gj_milestone_tag_32100      32100      graph_batch32098.c
  gj_continuum_wave_32100     32100      graph_batch32099.c
  gj_batch_id_32100           32100      graph_batch32100.c
  gj_graph_milestone_32100    32100      graph_batch32100.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=32100`` means parent wired CGJ_SRCS through
    ``graph_batch32100.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_32100`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_32100`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M32100_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_32000`` / ``gj_*_31900`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 179 exclusive CREATE-ONLY lands the **markers** band
  (32091–32100) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_32100.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_32100.py

  # Restrict to marker band only (soft-gate TUs; Wave 179 exclusive):
  python3 user/libcgj/src/_gen_milestone_32100.py --band markers

  # Extension identity only (32001–32050):
  python3 user/libcgj/src/_gen_milestone_32100.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_32100.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_32000.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 32100 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 32100
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 32001
WAVE_HI = 32100  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 32001, 32050
IDENTITY_LO, IDENTITY_HI = 32051, 32075
FILL_LO, FILL_HI = 32076, 32090
MARKERS_LO, MARKERS_HI = 32091, 32100
...
# Stems become gj_<stem>_32100. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (32091, "shell_green", 1, "shell product green extract (wave 32100)"),
    (32092, "libcgj_green", 1, "libcgj product green extract (wave 32100)"),
    (32093, "bar3_ready", 0, "product bar3 readiness gate (wave 32100)"),
    (32094, "product_score", 0, "product readiness score tag (wave 32100)"),
    (32095, "continuum_ready", 1, "continuum readiness gate (wave 32100)"),
    (32096, "smoke_soft", 1, "smoke soft lamp (wave 32100)"),
    (32097, "dyn_soft", 1, "dyn soft lamp (wave 32100)"),
    (32098, "milestone_tag", MILESTONE, "milestone 32100 product tag"),
    (32099, "continuum_wave", MILESTONE, "milestone 32100 continuum wave identity"),
    # 32100 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        32076: "continuum wave fill soft unit + batch id",
        32080: "continuum bridge soft unit + batch id",
        32090: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (32091–32099), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 32093
    tag_batch = MARKERS_LO + 7  # 32098
    wave_batch = MARKERS_LO + 8  # 32099
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_32000 / gj_shell_green_31900 / gj_shell_green_31800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_32000 / gj_libcgj_green_31900 / gj_libcgj_green_31800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_32000 / gj_bar3_ready_31900 / gj_bar3_ready_31800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_32000 / gj_product_score_31900 / gj_product_score_31800\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_32000 / gj_continuum_ready_31900 /\n"
            f" * gj_continuum_ready_31800 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_32000 / gj_smoke_soft_31900 / gj_smoke_soft_31800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_32000 / gj_dyn_soft_31900 / gj_dyn_soft_31800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_32000 / gj_milestone_tag_31900 / gj_milestone_tag_31800,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_32000 / gj_continuum_wave_31900 /\n"
            f" * gj_continuum_wave_31800, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch32100.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 32099
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 32091–32099; 32100 is capstone
