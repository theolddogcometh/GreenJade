...
"""
Soft continuum generator — MILESTONE 32200 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 32101–32200 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=32200   when parent has wired through graph_batch32200.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 32101 32200
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 32101 32200
...
graph batches ending at M. For M = **32200** the exclusive range is
**32101–32200** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 32101–32150  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 32151–32175  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 32176–32190  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 32191–32200  │ decade soft gates (Wave 180 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 32200 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_32200        1          graph_batch32191.c
  gj_libcgj_green_32200       1          graph_batch32192.c
  gj_bar3_ready_32200         0          graph_batch32193.c  (bar3 open)
  gj_product_score_32200      0          graph_batch32194.c  (score open)
  gj_continuum_ready_32200    1          graph_batch32195.c
  gj_smoke_soft_32200         1          graph_batch32196.c
  gj_dyn_soft_32200           1          graph_batch32197.c
  gj_milestone_tag_32200      32200      graph_batch32198.c
  gj_continuum_wave_32200     32200      graph_batch32199.c
  gj_batch_id_32200           32200      graph_batch32200.c
  gj_graph_milestone_32200    32200      graph_batch32200.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=32200`` means parent wired CGJ_SRCS through
    ``graph_batch32200.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_32200`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_32200`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M32200_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_32100`` / ``gj_*_32000`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 180 exclusive CREATE-ONLY lands the **markers** band
  (32191–32200) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_32200.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_32200.py

  # Restrict to marker band only (soft-gate TUs; Wave 180 exclusive):
  python3 user/libcgj/src/_gen_milestone_32200.py --band markers

  # Extension identity only (32101–32150):
  python3 user/libcgj/src/_gen_milestone_32200.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_32200.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_32100.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 32200 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 32200
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 32101
WAVE_HI = 32200  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 32101, 32150
IDENTITY_LO, IDENTITY_HI = 32151, 32175
FILL_LO, FILL_HI = 32176, 32190
MARKERS_LO, MARKERS_HI = 32191, 32200
...
# Stems become gj_<stem>_32200. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (32191, "shell_green", 1, "shell product green extract (wave 32200)"),
    (32192, "libcgj_green", 1, "libcgj product green extract (wave 32200)"),
    (32193, "bar3_ready", 0, "product bar3 readiness gate (wave 32200)"),
    (32194, "product_score", 0, "product readiness score tag (wave 32200)"),
    (32195, "continuum_ready", 1, "continuum readiness gate (wave 32200)"),
    (32196, "smoke_soft", 1, "smoke soft lamp (wave 32200)"),
    (32197, "dyn_soft", 1, "dyn soft lamp (wave 32200)"),
    (32198, "milestone_tag", MILESTONE, "milestone 32200 product tag"),
    (32199, "continuum_wave", MILESTONE, "milestone 32200 continuum wave identity"),
    # 32200 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        32176: "continuum wave fill soft unit + batch id",
        32180: "continuum bridge soft unit + batch id",
        32190: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (32191–32199), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 32193
    tag_batch = MARKERS_LO + 7  # 32198
    wave_batch = MARKERS_LO + 8  # 32199
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_32100 / gj_shell_green_32000 / gj_shell_green_31900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_32100 / gj_libcgj_green_32000 / gj_libcgj_green_31900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_32100 / gj_bar3_ready_32000 / gj_bar3_ready_31900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_32100 / gj_product_score_32000 / gj_product_score_31900\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_32100 / gj_continuum_ready_32000 /\n"
            f" * gj_continuum_ready_31900 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_32100 / gj_smoke_soft_32000 / gj_smoke_soft_31900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_32100 / gj_dyn_soft_32000 / gj_dyn_soft_31900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_32100 / gj_milestone_tag_32000 / gj_milestone_tag_31900,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_32100 / gj_continuum_wave_32000 /\n"
            f" * gj_continuum_wave_31900, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch32200.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 32199
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 32191–32199; 32200 is capstone
