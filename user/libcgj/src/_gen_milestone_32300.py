...
"""
Soft continuum generator — MILESTONE 32300 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 32201–32300 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=32300   when parent has wired through graph_batch32300.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 32201 32300
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 32201 32300
...
graph batches ending at M. For M = **32300** the exclusive range is
**32201–32300** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 32201–32250  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 32251–32275  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 32276–32290  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 32291–32300  │ decade soft gates (Wave 181 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 32300 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_32300        1          graph_batch32291.c
  gj_libcgj_green_32300       1          graph_batch32292.c
  gj_bar3_ready_32300         0          graph_batch32293.c  (bar3 open)
  gj_product_score_32300      0          graph_batch32294.c  (score open)
  gj_continuum_ready_32300    1          graph_batch32295.c
  gj_smoke_soft_32300         1          graph_batch32296.c
  gj_dyn_soft_32300           1          graph_batch32297.c
  gj_milestone_tag_32300      32300      graph_batch32298.c
  gj_continuum_wave_32300     32300      graph_batch32299.c
  gj_batch_id_32300           32300      graph_batch32300.c
  gj_graph_milestone_32300    32300      graph_batch32300.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=32300`` means parent wired CGJ_SRCS through
    ``graph_batch32300.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_32300`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_32300`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M32300_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_32200`` / ``gj_*_32100`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 181 exclusive CREATE-ONLY lands the **markers** band
  (32291–32300) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_32300.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_32300.py

  # Restrict to marker band only (soft-gate TUs; Wave 181 exclusive):
  python3 user/libcgj/src/_gen_milestone_32300.py --band markers

  # Extension identity only (32201–32250):
  python3 user/libcgj/src/_gen_milestone_32300.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_32300.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_32200.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 32300 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 32300
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 32201
WAVE_HI = 32300  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 32201, 32250
IDENTITY_LO, IDENTITY_HI = 32251, 32275
FILL_LO, FILL_HI = 32276, 32290
MARKERS_LO, MARKERS_HI = 32291, 32300
...
# Stems become gj_<stem>_32300. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (32291, "shell_green", 1, "shell product green extract (wave 32300)"),
    (32292, "libcgj_green", 1, "libcgj product green extract (wave 32300)"),
    (32293, "bar3_ready", 0, "product bar3 readiness gate (wave 32300)"),
    (32294, "product_score", 0, "product readiness score tag (wave 32300)"),
    (32295, "continuum_ready", 1, "continuum readiness gate (wave 32300)"),
    (32296, "smoke_soft", 1, "smoke soft lamp (wave 32300)"),
    (32297, "dyn_soft", 1, "dyn soft lamp (wave 32300)"),
    (32298, "milestone_tag", MILESTONE, "milestone 32300 product tag"),
    (32299, "continuum_wave", MILESTONE, "milestone 32300 continuum wave identity"),
    # 32300 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        32276: "continuum wave fill soft unit + batch id",
        32280: "continuum bridge soft unit + batch id",
        32290: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (32291–32299), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 32293
    tag_batch = MARKERS_LO + 7  # 32298
    wave_batch = MARKERS_LO + 8  # 32299
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_32200 / gj_shell_green_32100 / gj_shell_green_32000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_32200 / gj_libcgj_green_32100 / gj_libcgj_green_32000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_32200 / gj_bar3_ready_32100 / gj_bar3_ready_32000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_32200 / gj_product_score_32100 / gj_product_score_32000\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_32200 / gj_continuum_ready_32100 /\n"
            f" * gj_continuum_ready_32000 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_32200 / gj_smoke_soft_32100 / gj_smoke_soft_32000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_32200 / gj_dyn_soft_32100 / gj_dyn_soft_32000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_32200 / gj_milestone_tag_32100 / gj_milestone_tag_32000,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_32200 / gj_continuum_wave_32100 /\n"
            f" * gj_continuum_wave_32000, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch32300.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 32299
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 32291–32299; 32300 is capstone
