...
"""
Soft continuum generator — MILESTONE 32400 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 32301–32400 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=32400   when parent has wired through graph_batch32400.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 32301 32400
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 32301 32400
...
graph batches ending at M. For M = **32400** the exclusive range is
**32301–32400** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 32301–32350  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 32351–32375  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 32376–32390  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 32391–32400  │ decade soft gates (Wave 182 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 32400 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_32400        1          graph_batch32391.c
  gj_libcgj_green_32400       1          graph_batch32392.c
  gj_bar3_ready_32400         0          graph_batch32393.c  (bar3 open)
  gj_product_score_32400      0          graph_batch32394.c  (score open)
  gj_continuum_ready_32400    1          graph_batch32395.c
  gj_smoke_soft_32400         1          graph_batch32396.c
  gj_dyn_soft_32400           1          graph_batch32397.c
  gj_milestone_tag_32400      32400      graph_batch32398.c
  gj_continuum_wave_32400     32400      graph_batch32399.c
  gj_batch_id_32400           32400      graph_batch32400.c
  gj_graph_milestone_32400    32400      graph_batch32400.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=32400`` means parent wired CGJ_SRCS through
    ``graph_batch32400.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_32400`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_32400`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M32400_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_32300`` / ``gj_*_32200`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 182 exclusive CREATE-ONLY lands the **markers** band
  (32391–32400) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_32400.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_32400.py

  # Restrict to marker band only (soft-gate TUs; Wave 182 exclusive):
  python3 user/libcgj/src/_gen_milestone_32400.py --band markers

  # Extension identity only (32301–32350):
  python3 user/libcgj/src/_gen_milestone_32400.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_32400.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_32300.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 32400 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 32400
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 32301
WAVE_HI = 32400  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 32301, 32350
IDENTITY_LO, IDENTITY_HI = 32351, 32375
FILL_LO, FILL_HI = 32376, 32390
MARKERS_LO, MARKERS_HI = 32391, 32400
...
# Stems become gj_<stem>_32400. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (32391, "shell_green", 1, "shell product green extract (wave 32400)"),
    (32392, "libcgj_green", 1, "libcgj product green extract (wave 32400)"),
    (32393, "bar3_ready", 0, "product bar3 readiness gate (wave 32400)"),
    (32394, "product_score", 0, "product readiness score tag (wave 32400)"),
    (32395, "continuum_ready", 1, "continuum readiness gate (wave 32400)"),
    (32396, "smoke_soft", 1, "smoke soft lamp (wave 32400)"),
    (32397, "dyn_soft", 1, "dyn soft lamp (wave 32400)"),
    (32398, "milestone_tag", MILESTONE, "milestone 32400 product tag"),
    (32399, "continuum_wave", MILESTONE, "milestone 32400 continuum wave identity"),
    # 32400 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        32376: "continuum wave fill soft unit + batch id",
        32380: "continuum bridge soft unit + batch id",
        32390: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (32391–32399), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 32393
    tag_batch = MARKERS_LO + 7  # 32398
    wave_batch = MARKERS_LO + 8  # 32399
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_32300 / gj_shell_green_32200 / gj_shell_green_32100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_32300 / gj_libcgj_green_32200 / gj_libcgj_green_32100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_32300 / gj_bar3_ready_32200 / gj_bar3_ready_32100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_32300 / gj_product_score_32200 / gj_product_score_32100\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_32300 / gj_smoke_soft_32200 / gj_smoke_soft_32100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_32300 / gj_dyn_soft_32200 / gj_dyn_soft_32100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_32300 / gj_milestone_tag_32200 / gj_milestone_tag_32100,\n"
...
def render_milestone_capstone() -> str:
    """graph_batch32400.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 32399
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 32391–32399; 32400 is capstone
