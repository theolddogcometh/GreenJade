...
"""
Soft continuum generator — MILESTONE 32500 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 32401–32500 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=32500   when parent has wired through graph_batch32500.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 32401 32500
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 32401 32500
...
graph batches ending at M. For M = **32500** the exclusive range is
**32401–32500** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 32401–32450  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 32451–32475  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 32476–32490  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 32491–32500  │ decade soft gates (Wave 183 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 32500 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_32500        1          graph_batch32491.c
  gj_libcgj_green_32500       1          graph_batch32492.c
  gj_bar3_ready_32500         0          graph_batch32493.c  (bar3 open)
  gj_product_score_32500      0          graph_batch32494.c  (score open)
  gj_continuum_ready_32500    1          graph_batch32495.c
  gj_smoke_soft_32500         1          graph_batch32496.c
  gj_dyn_soft_32500           1          graph_batch32497.c
  gj_milestone_tag_32500      32500      graph_batch32498.c
  gj_continuum_wave_32500     32500      graph_batch32499.c
  gj_batch_id_32500           32500      graph_batch32500.c
  gj_graph_milestone_32500    32500      graph_batch32500.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=32500`` means parent wired CGJ_SRCS through
    ``graph_batch32500.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_32500`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_32500`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M32500_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_32400`` / ``gj_*_32300`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 183 exclusive CREATE-ONLY lands the **markers** band
  (32491–32500) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_32500.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_32500.py

  # Restrict to marker band only (soft-gate TUs; Wave 183 exclusive):
  python3 user/libcgj/src/_gen_milestone_32500.py --band markers

  # Extension identity only (32401–32450):
  python3 user/libcgj/src/_gen_milestone_32500.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_32500.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_32400.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 32500 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 32500
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 32401
WAVE_HI = 32500  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 32401, 32450
IDENTITY_LO, IDENTITY_HI = 32451, 32475
FILL_LO, FILL_HI = 32476, 32490
MARKERS_LO, MARKERS_HI = 32491, 32500
...
# Stems become gj_<stem>_32500. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (32491, "shell_green", 1, "shell product green extract (wave 32500)"),
    (32492, "libcgj_green", 1, "libcgj product green extract (wave 32500)"),
    (32493, "bar3_ready", 0, "product bar3 readiness gate (wave 32500)"),
    (32494, "product_score", 0, "product readiness score tag (wave 32500)"),
    (32495, "continuum_ready", 1, "continuum readiness gate (wave 32500)"),
    (32496, "smoke_soft", 1, "smoke soft lamp (wave 32500)"),
    (32497, "dyn_soft", 1, "dyn soft lamp (wave 32500)"),
    (32498, "milestone_tag", MILESTONE, "milestone 32500 product tag"),
    (32499, "continuum_wave", MILESTONE, "milestone 32500 continuum wave identity"),
    # 32500 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        32476: "continuum wave fill soft unit + batch id",
        32480: "continuum bridge soft unit + batch id",
        32490: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (32491–32499), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 32493
    tag_batch = MARKERS_LO + 7  # 32498
    wave_batch = MARKERS_LO + 8  # 32499
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_32400 / gj_shell_green_32300 / gj_shell_green_32200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_32400 / gj_libcgj_green_32300 / gj_libcgj_green_32200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_32400 / gj_bar3_ready_32300 / gj_bar3_ready_32200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_32400 / gj_product_score_32300 / gj_product_score_32200\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_32400 / gj_continuum_ready_32300 /\n"
            f" * gj_continuum_ready_32200 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_32400 / gj_smoke_soft_32300 / gj_smoke_soft_32200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_32400 / gj_dyn_soft_32300 / gj_dyn_soft_32200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_32400 / gj_milestone_tag_32300 / gj_milestone_tag_32200,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_32400 / gj_continuum_wave_32300 /\n"
            f" * gj_continuum_wave_32200, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch32500.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 32499
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 32491–32499; 32500 is capstone
