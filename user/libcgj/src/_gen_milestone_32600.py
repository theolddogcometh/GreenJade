...
"""
Soft continuum generator — MILESTONE 32600 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 32501–32600 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=32600   when parent has wired through graph_batch32600.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 32501 32600
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 32501 32600
...
graph batches ending at M. For M = **32600** the exclusive range is
**32501–32600** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 32501–32550  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 32551–32575  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 32576–32590  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 32591–32600  │ decade soft gates (Wave 184 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 32600 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_32600        1          graph_batch32591.c
  gj_libcgj_green_32600       1          graph_batch32592.c
  gj_bar3_ready_32600         0          graph_batch32593.c  (bar3 open)
  gj_product_score_32600      0          graph_batch32594.c  (score open)
  gj_continuum_ready_32600    1          graph_batch32595.c
  gj_smoke_soft_32600         1          graph_batch32596.c
  gj_dyn_soft_32600           1          graph_batch32597.c
  gj_milestone_tag_32600      32600      graph_batch32598.c
  gj_continuum_wave_32600     32600      graph_batch32599.c
  gj_batch_id_32600           32600      graph_batch32600.c
  gj_graph_milestone_32600    32600      graph_batch32600.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=32600`` means parent wired CGJ_SRCS through
    ``graph_batch32600.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_32600`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_32600`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M32600_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_32500`` / ``gj_*_32400`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 184 exclusive CREATE-ONLY lands the **markers** band
  (32591–32600) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_32600.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_32600.py

  # Restrict to marker band only (soft-gate TUs; Wave 184 exclusive):
  python3 user/libcgj/src/_gen_milestone_32600.py --band markers

  # Extension identity only (32501–32550):
  python3 user/libcgj/src/_gen_milestone_32600.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_32600.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_32500.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 32600 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 32600
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 32501
WAVE_HI = 32600  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 32501, 32550
IDENTITY_LO, IDENTITY_HI = 32551, 32575
FILL_LO, FILL_HI = 32576, 32590
MARKERS_LO, MARKERS_HI = 32591, 32600
...
# Stems become gj_<stem>_32600. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (32591, "shell_green", 1, "shell product green extract (wave 32600)"),
    (32592, "libcgj_green", 1, "libcgj product green extract (wave 32600)"),
    (32593, "bar3_ready", 0, "product bar3 readiness gate (wave 32600)"),
    (32594, "product_score", 0, "product readiness score tag (wave 32600)"),
    (32595, "continuum_ready", 1, "continuum readiness gate (wave 32600)"),
    (32596, "smoke_soft", 1, "smoke soft lamp (wave 32600)"),
    (32597, "dyn_soft", 1, "dyn soft lamp (wave 32600)"),
    (32598, "milestone_tag", MILESTONE, "milestone 32600 product tag"),
    (32599, "continuum_wave", MILESTONE, "milestone 32600 continuum wave identity"),
    # 32600 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        32576: "continuum wave fill soft unit + batch id",
        32580: "continuum bridge soft unit + batch id",
        32590: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (32591-32599), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 32593
    tag_batch = MARKERS_LO + 7  # 32598
    wave_batch = MARKERS_LO + 8  # 32599
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_32500 / gj_shell_green_32400 / gj_shell_green_32300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_32500 / gj_libcgj_green_32400 / gj_libcgj_green_32300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_32500 / gj_bar3_ready_32400 / gj_bar3_ready_32300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_32500 / gj_product_score_32400 / gj_product_score_32300\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_32500 / gj_continuum_ready_32400 /\n"
            f" * gj_continuum_ready_32300 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_32500 / gj_smoke_soft_32400 / gj_smoke_soft_32300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_32500 / gj_dyn_soft_32400 / gj_dyn_soft_32300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_32500 / gj_milestone_tag_32400 / gj_milestone_tag_32300,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_32500 / gj_continuum_wave_32400 /\n"
            f" * gj_continuum_wave_32300, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch32600.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 32599
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 32591–32599; 32600 is capstone
