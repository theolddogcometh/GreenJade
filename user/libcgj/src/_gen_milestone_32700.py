...
"""
Soft continuum generator — MILESTONE 32700 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 32601–32700 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=32700   when parent has wired through graph_batch32700.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 32601 32700
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 32601 32700
...
graph batches ending at M. For M = **32700** the exclusive range is
**32601–32700** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 32601–32650  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 32651–32675  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 32676–32690  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 32691–32700  │ decade soft gates (Wave 185 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 32700 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_32700        1          graph_batch32691.c
  gj_libcgj_green_32700       1          graph_batch32692.c
  gj_bar3_ready_32700         0          graph_batch32693.c  (bar3 open)
  gj_product_score_32700      0          graph_batch32694.c  (score open)
  gj_continuum_ready_32700    1          graph_batch32695.c
  gj_smoke_soft_32700         1          graph_batch32696.c
  gj_dyn_soft_32700           1          graph_batch32697.c
  gj_milestone_tag_32700      32700      graph_batch32698.c
  gj_continuum_wave_32700     32700      graph_batch32699.c
  gj_batch_id_32700           32700      graph_batch32700.c
  gj_graph_milestone_32700    32700      graph_batch32700.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=32700`` means parent wired CGJ_SRCS through
    ``graph_batch32700.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_32700`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_32700`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M32700_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_32600`` / ``gj_*_32500`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 185 exclusive CREATE-ONLY lands the **markers** band
  (32691–32700) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_32700.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_32700.py

  # Restrict to marker band only (soft-gate TUs; Wave 185 exclusive):
  python3 user/libcgj/src/_gen_milestone_32700.py --band markers

  # Extension identity only (32601–32650):
  python3 user/libcgj/src/_gen_milestone_32700.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_32700.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_32600.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 32700 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 32700
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 32601
WAVE_HI = 32700  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 32601, 32650
IDENTITY_LO, IDENTITY_HI = 32651, 32675
FILL_LO, FILL_HI = 32676, 32690
MARKERS_LO, MARKERS_HI = 32691, 32700
...
# Stems become gj_<stem>_32700. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (32691, "shell_green", 1, "shell product green extract (wave 32700)"),
    (32692, "libcgj_green", 1, "libcgj product green extract (wave 32700)"),
    (32693, "bar3_ready", 0, "product bar3 readiness gate (wave 32700)"),
    (32694, "product_score", 0, "product readiness score tag (wave 32700)"),
    (32695, "continuum_ready", 1, "continuum readiness gate (wave 32700)"),
    (32696, "smoke_soft", 1, "smoke soft lamp (wave 32700)"),
    (32697, "dyn_soft", 1, "dyn soft lamp (wave 32700)"),
    (32698, "milestone_tag", MILESTONE, "milestone 32700 product tag"),
    (32699, "continuum_wave", MILESTONE, "milestone 32700 continuum wave identity"),
    # 32700 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        32676: "continuum wave fill soft unit + batch id",
        32680: "continuum bridge soft unit + batch id",
        32690: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (32691–32699), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 32693
    tag_batch = MARKERS_LO + 7  # 32698
    wave_batch = MARKERS_LO + 8  # 32699
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_32600 / gj_shell_green_32500 / gj_shell_green_32400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_32600 / gj_libcgj_green_32500 / gj_libcgj_green_32400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_32600 / gj_bar3_ready_32500 / gj_bar3_ready_32400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_32600 / gj_product_score_32500 / gj_product_score_32400\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_32600 / gj_continuum_ready_32500 /\n"
            f" * gj_continuum_ready_32400 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_32600 / gj_smoke_soft_32500 / gj_smoke_soft_32400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_32600 / gj_dyn_soft_32500 / gj_dyn_soft_32400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_32600 / gj_milestone_tag_32500 / gj_milestone_tag_32400,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_32600 / gj_continuum_wave_32500 /\n"
            f" * gj_continuum_wave_32400, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch32700.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 32699
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 32691–32699; 32700 is capstone
