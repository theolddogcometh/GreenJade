...
"""
Soft continuum generator — MILESTONE 32800 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 32701–32800 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=32800   when parent has wired through graph_batch32800.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 32701 32800
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 32701 32800
...
graph batches ending at M. For M = **32800** the exclusive range is
**32701–32800** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 32701–32750  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 32751–32775  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 32776–32790  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 32791–32800  │ decade soft gates (Wave 186 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 32800 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_32800        1          graph_batch32791.c
  gj_libcgj_green_32800       1          graph_batch32392.c
  gj_bar3_ready_32800         0          graph_batch32793.c  (bar3 open)
  gj_product_score_32800      0          graph_batch32394.c  (score open)
  gj_continuum_ready_32800    1          graph_batch32395.c
  gj_smoke_soft_32800         1          graph_batch32396.c
  gj_dyn_soft_32800           1          graph_batch32397.c
  gj_milestone_tag_32800      32800      graph_batch32798.c
  gj_continuum_wave_32800     32800      graph_batch32799.c
  gj_batch_id_32800           32800      graph_batch32800.c
  gj_graph_milestone_32800    32800      graph_batch32800.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=32800`` means parent wired CGJ_SRCS through
    ``graph_batch32800.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_32800`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_32800`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M32800_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_32700`` / ``gj_*_32600`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 186 exclusive CREATE-ONLY lands the **markers** band
  (32791–32800) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_32800.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_32800.py

  # Restrict to marker band only (soft-gate TUs; Wave 186 exclusive):
  python3 user/libcgj/src/_gen_milestone_32800.py --band markers

  # Extension identity only (32701–32750):
  python3 user/libcgj/src/_gen_milestone_32800.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_32800.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_32700.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 32800 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 32800
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 32701
WAVE_HI = 32800  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 32701, 32750
IDENTITY_LO, IDENTITY_HI = 32751, 32775
FILL_LO, FILL_HI = 32776, 32790
MARKERS_LO, MARKERS_HI = 32791, 32800
...
# Stems become gj_<stem>_32800. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (32791, "shell_green", 1, "shell product green extract (wave 32800)"),
    (32792, "libcgj_green", 1, "libcgj product green extract (wave 32800)"),
    (32793, "bar3_ready", 0, "product bar3 readiness gate (wave 32800)"),
    (32794, "product_score", 0, "product readiness score tag (wave 32800)"),
    (32795, "continuum_ready", 1, "continuum readiness gate (wave 32800)"),
    (32796, "smoke_soft", 1, "smoke soft lamp (wave 32800)"),
    (32797, "dyn_soft", 1, "dyn soft lamp (wave 32800)"),
    (32798, "milestone_tag", MILESTONE, "milestone 32800 product tag"),
    (32799, "continuum_wave", MILESTONE, "milestone 32800 continuum wave identity"),
    # 32800 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        32776: "continuum wave fill soft unit + batch id",
        32780: "continuum bridge soft unit + batch id",
        32790: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (32791–32799), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 32793
    tag_batch = MARKERS_LO + 7  # 32798
    wave_batch = MARKERS_LO + 8  # 32799
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_32700 / gj_shell_green_32600 / gj_shell_green_32500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_32700 / gj_libcgj_green_32600 / gj_libcgj_green_32500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_32700 / gj_bar3_ready_32600 / gj_bar3_ready_32500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_32700 / gj_product_score_32600 / gj_product_score_32500\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_32300 / gj_continuum_ready_32200 /\n"
            f" * gj_continuum_ready_32100 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_32700 / gj_smoke_soft_32600 / gj_smoke_soft_32500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_32700 / gj_dyn_soft_32600 / gj_dyn_soft_32500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_32700 / gj_milestone_tag_32600 / gj_milestone_tag_32500,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_32300 / gj_continuum_wave_32200 /\n"
            f" * gj_continuum_wave_32100, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch32800.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 32799
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 32791–32799; 32800 is capstone
