...
"""
Soft continuum generator — MILESTONE 32900 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 32801–32900 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=32900   when parent has wired through graph_batch32900.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 32801 32900
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 32801 32900
...
graph batches ending at M. For M = **32900** the exclusive range is
**32801–32900** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 32801–32850  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 32851–32875  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 32876–32890  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 32891–32900  │ decade soft gates (Wave 187 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 32900 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_32900        1          graph_batch32891.c
  gj_libcgj_green_32900       1          graph_batch32892.c
  gj_bar3_ready_32900         0          graph_batch32893.c  (bar3 open)
  gj_product_score_32900      0          graph_batch32894.c  (score open)
  gj_continuum_ready_32900    1          graph_batch32895.c
  gj_smoke_soft_32900         1          graph_batch32896.c
  gj_dyn_soft_32900           1          graph_batch32897.c
  gj_milestone_tag_32900      32900      graph_batch32898.c
  gj_continuum_wave_32900     32900      graph_batch32899.c
  gj_batch_id_32900           32900      graph_batch32900.c
  gj_graph_milestone_32900    32900      graph_batch32900.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=32900`` means parent wired CGJ_SRCS through
    ``graph_batch32900.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_32900`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_32900`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M32900_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_32800`` / ``gj_*_32700`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 187 exclusive CREATE-ONLY lands the **markers** band
  (32891–32900) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_32900.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_32900.py

  # Restrict to marker band only (soft-gate TUs; Wave 187 exclusive):
  python3 user/libcgj/src/_gen_milestone_32900.py --band markers

  # Extension identity only (32801–32850):
  python3 user/libcgj/src/_gen_milestone_32900.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_32900.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_32800.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 32900 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 32900
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 32801
WAVE_HI = 32900  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 32801, 32850
IDENTITY_LO, IDENTITY_HI = 32851, 32875
FILL_LO, FILL_HI = 32876, 32890
MARKERS_LO, MARKERS_HI = 32891, 32900
...
# Stems become gj_<stem>_32900. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (32891, "shell_green", 1, "shell product green extract (wave 32900)"),
    (32892, "libcgj_green", 1, "libcgj product green extract (wave 32900)"),
    (32893, "bar3_ready", 0, "product bar3 readiness gate (wave 32900)"),
    (32894, "product_score", 0, "product readiness score tag (wave 32900)"),
    (32895, "continuum_ready", 1, "continuum readiness gate (wave 32900)"),
    (32896, "smoke_soft", 1, "smoke soft lamp (wave 32900)"),
    (32897, "dyn_soft", 1, "dyn soft lamp (wave 32900)"),
    (32898, "milestone_tag", MILESTONE, "milestone 32900 product tag"),
    (32899, "continuum_wave", MILESTONE, "milestone 32900 continuum wave identity"),
    # 32900 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        32876: "continuum wave fill soft unit + batch id",
        32880: "continuum bridge soft unit + batch id",
        32890: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (32891–32899), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 32893
    tag_batch = MARKERS_LO + 7  # 32898
    wave_batch = MARKERS_LO + 8  # 32899
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_32800 / gj_shell_green_32700 / gj_shell_green_32600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_32800 / gj_libcgj_green_32700 / gj_libcgj_green_32600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_32800 / gj_bar3_ready_32700 / gj_bar3_ready_32600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_32800 / gj_product_score_32700 / gj_product_score_32600\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_32800 / gj_continuum_ready_32700 /\n"
            f" * gj_continuum_ready_32600 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_32800 / gj_smoke_soft_32700 / gj_smoke_soft_32600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_32800 / gj_dyn_soft_32700 / gj_dyn_soft_32600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_32800 / gj_milestone_tag_32700 / gj_milestone_tag_32600,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_32800 / gj_continuum_wave_32700 /\n"
            f" * gj_continuum_wave_32600, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch32900.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 32899
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 32891–32899; 32900 is capstone
