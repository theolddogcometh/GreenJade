...
"""
Soft continuum generator — MILESTONE 33000 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 32901–33000 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=33000   when parent has wired through graph_batch33000.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 32901 33000
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 32901 33000
...
graph batches ending at M. For M = **33000** the exclusive range is
**32901–33000** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 32901–32950  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 32951–32975  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 32976–32990  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 32991–33000  │ decade soft gates (Wave 188 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 33000 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_33000        1          graph_batch32991.c
  gj_libcgj_green_33000       1          graph_batch32992.c
  gj_bar3_ready_33000         0          graph_batch32993.c  (bar3 open)
  gj_product_score_33000      0          graph_batch32994.c  (score open)
  gj_continuum_ready_33000    1          graph_batch32995.c
  gj_smoke_soft_33000         1          graph_batch32996.c
  gj_dyn_soft_33000           1          graph_batch32997.c
  gj_milestone_tag_33000      33000      graph_batch32998.c
  gj_continuum_wave_33000     33000      graph_batch32999.c
  gj_batch_id_33000           33000      graph_batch33000.c
  gj_graph_milestone_33000    33000      graph_batch33000.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=33000`` means parent wired CGJ_SRCS through
    ``graph_batch33000.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_33000`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_33000`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M33000_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_32900`` / ``gj_*_32800`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 188 exclusive CREATE-ONLY lands the **markers** band
  (32991–33000) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_33000.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_33000.py

  # Restrict to marker band only (soft-gate TUs; Wave 188 exclusive):
  python3 user/libcgj/src/_gen_milestone_33000.py --band markers

  # Extension identity only (32901–32950):
  python3 user/libcgj/src/_gen_milestone_33000.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_33000.py --force
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 33000 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 33000
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 32901
WAVE_HI = 33000  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 32901, 32950
IDENTITY_LO, IDENTITY_HI = 32951, 32975
FILL_LO, FILL_HI = 32976, 32990
MARKERS_LO, MARKERS_HI = 32991, 33000
...
# Stems become gj_<stem>_33000. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (32991, "shell_green", 1, "shell product green extract (wave 33000)"),
    (32992, "libcgj_green", 1, "libcgj product green extract (wave 33000)"),
    (32993, "bar3_ready", 0, "product bar3 readiness gate (wave 33000)"),
    (32994, "product_score", 0, "product readiness score tag (wave 33000)"),
    (32995, "continuum_ready", 1, "continuum readiness gate (wave 33000)"),
    (32996, "smoke_soft", 1, "smoke soft lamp (wave 33000)"),
    (32997, "dyn_soft", 1, "dyn soft lamp (wave 33000)"),
    (32998, "milestone_tag", MILESTONE, "milestone 33000 product tag"),
    (32999, "continuum_wave", MILESTONE, "milestone 33000 continuum wave identity"),
    # 33000 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        32976: "continuum wave fill soft unit + batch id",
        32980: "continuum bridge soft unit + batch id",
        32990: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (32991–32999), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 32993
    tag_batch = MARKERS_LO + 7  # 32998
    wave_batch = MARKERS_LO + 8  # 32999
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_32900 / gj_shell_green_32800 / gj_shell_green_32700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_32900 / gj_libcgj_green_32800 / gj_libcgj_green_32700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_32900 / gj_bar3_ready_32800 / gj_bar3_ready_32700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_32900 / gj_product_score_32800 / gj_product_score_32700\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_32900 / gj_smoke_soft_32800 / gj_smoke_soft_32700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_32900 / gj_dyn_soft_32800 / gj_dyn_soft_32700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_32900 / gj_milestone_tag_32800 / gj_milestone_tag_32700,\n"
...
def render_milestone_capstone() -> str:
    """graph_batch33000.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 32999
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 32991–32999; 33000 is capstone
