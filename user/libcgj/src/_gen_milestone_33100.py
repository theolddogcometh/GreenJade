...
"""
Soft continuum generator — MILESTONE 33100 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 33001–33100 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=33100   when parent has wired through graph_batch33100.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 33001 33100
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 33001 33100
...
graph batches ending at M. For M = **33100** the exclusive range is
**33001–33100** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 33001–33050  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 33051–33075  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 33076–33090  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 33091–33100  │ decade soft gates (Wave 189 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 33100 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_33100        1          graph_batch33091.c
  gj_libcgj_green_33100       1          graph_batch33092.c
  gj_bar3_ready_33100         0          graph_batch33093.c  (bar3 open)
  gj_product_score_33100      0          graph_batch33094.c  (score open)
  gj_continuum_ready_33100    1          graph_batch33095.c
  gj_smoke_soft_33100         1          graph_batch33096.c
  gj_dyn_soft_33100           1          graph_batch33097.c
  gj_milestone_tag_33100      33100      graph_batch33098.c
  gj_continuum_wave_33100     33100      graph_batch33099.c
  gj_batch_id_33100           33100      graph_batch33100.c
  gj_graph_milestone_33100    33100      graph_batch33100.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=33100`` means parent wired CGJ_SRCS through
    ``graph_batch33100.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_33100`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_33100`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M33100_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_33000`` / ``gj_*_32900`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 189 exclusive CREATE-ONLY lands the **markers** band
  (33091–33100) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_33100.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_33100.py

  # Restrict to marker band only (soft-gate TUs; Wave 189 exclusive):
  python3 user/libcgj/src/_gen_milestone_33100.py --band markers

  # Extension identity only (33001–33050):
  python3 user/libcgj/src/_gen_milestone_33100.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_33100.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_33000.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 33100 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 33100
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 33001
WAVE_HI = 33100  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 33001, 33050
IDENTITY_LO, IDENTITY_HI = 33051, 33075
FILL_LO, FILL_HI = 33076, 33090
MARKERS_LO, MARKERS_HI = 33091, 33100
...
# Stems become gj_<stem>_33100. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (33091, "shell_green", 1, "shell product green extract (wave 33100)"),
    (33092, "libcgj_green", 1, "libcgj product green extract (wave 33100)"),
    (33093, "bar3_ready", 0, "product bar3 readiness gate (wave 33100)"),
    (33094, "product_score", 0, "product readiness score tag (wave 33100)"),
    (33095, "continuum_ready", 1, "continuum readiness gate (wave 33100)"),
    (33096, "smoke_soft", 1, "smoke soft lamp (wave 33100)"),
    (33097, "dyn_soft", 1, "dyn soft lamp (wave 33100)"),
    (33098, "milestone_tag", MILESTONE, "milestone 33100 product tag"),
    (33099, "continuum_wave", MILESTONE, "milestone 33100 continuum wave identity"),
    # 33100 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        33076: "continuum wave fill soft unit + batch id",
        33080: "continuum bridge soft unit + batch id",
        33090: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (33091–33099), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 33093
    tag_batch = MARKERS_LO + 7  # 33098
    wave_batch = MARKERS_LO + 8  # 33099
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_33000 / gj_shell_green_32900 / gj_shell_green_32800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_33000 / gj_libcgj_green_32900 / gj_libcgj_green_32800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_33000 / gj_bar3_ready_32900 / gj_bar3_ready_32800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_33000 / gj_product_score_32900 / gj_product_score_32800\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_33000 / gj_continuum_ready_32900 /\n"
            f" * gj_continuum_ready_32800 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_33000 / gj_smoke_soft_32900 / gj_smoke_soft_32800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_33000 / gj_dyn_soft_32900 / gj_dyn_soft_32800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_33000 / gj_milestone_tag_32900 / gj_milestone_tag_32800,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_33000 / gj_continuum_wave_32900 /\n"
            f" * gj_continuum_wave_32800, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch33100.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 33099
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 33091–33099; 33100 is capstone
