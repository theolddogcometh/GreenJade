...
"""
Soft continuum generator — MILESTONE 33200 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 33101–33200 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=33200   when parent has wired through graph_batch33200.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 33101 33200
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 33101 33200
...
graph batches ending at M. For M = **33200** the exclusive range is
**33101–33200** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 33101–33150  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 33151–33175  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 33176–33190  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 33191–33200  │ decade soft gates (Wave 190 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 33200 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_33200        1          graph_batch33191.c
  gj_libcgj_green_33200       1          graph_batch33192.c
  gj_bar3_ready_33200         0          graph_batch33193.c  (bar3 open)
  gj_product_score_33200      0          graph_batch33194.c  (score open)
  gj_continuum_ready_33200    1          graph_batch33195.c
  gj_smoke_soft_33200         1          graph_batch33196.c
  gj_dyn_soft_33200           1          graph_batch33197.c
  gj_milestone_tag_33200      33200      graph_batch33198.c
  gj_continuum_wave_33200     33200      graph_batch33199.c
  gj_batch_id_33200           33200      graph_batch33200.c
  gj_graph_milestone_33200    33200      graph_batch33200.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=33200`` means parent wired CGJ_SRCS through
    ``graph_batch33200.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_33200`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_33200`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M33200_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_33100`` / ``gj_*_33000`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 190 exclusive CREATE-ONLY lands the **markers** band
  (33191–33200) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_33200.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_33200.py

  # Restrict to marker band only (soft-gate TUs; Wave 190 exclusive):
  python3 user/libcgj/src/_gen_milestone_33200.py --band markers

  # Extension identity only (33101–33150):
  python3 user/libcgj/src/_gen_milestone_33200.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_33200.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_33100.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 33200 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 33200
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 33101
WAVE_HI = 33200  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 33101, 33150
IDENTITY_LO, IDENTITY_HI = 33151, 33175
FILL_LO, FILL_HI = 33176, 33190
MARKERS_LO, MARKERS_HI = 33191, 33200
...
# Stems become gj_<stem>_33200. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (33191, "shell_green", 1, "shell product green extract (wave 33200)"),
    (33192, "libcgj_green", 1, "libcgj product green extract (wave 33200)"),
    (33193, "bar3_ready", 0, "product bar3 readiness gate (wave 33200)"),
    (33194, "product_score", 0, "product readiness score tag (wave 33200)"),
    (33195, "continuum_ready", 1, "continuum readiness gate (wave 33200)"),
    (33196, "smoke_soft", 1, "smoke soft lamp (wave 33200)"),
    (33197, "dyn_soft", 1, "dyn soft lamp (wave 33200)"),
    (33198, "milestone_tag", MILESTONE, "milestone 33200 product tag"),
    (33199, "continuum_wave", MILESTONE, "milestone 33200 continuum wave identity"),
    # 33200 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        33176: "continuum wave fill soft unit + batch id",
        33180: "continuum bridge soft unit + batch id",
        33190: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (33191–33199), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 33193
    tag_batch = MARKERS_LO + 7  # 33198
    wave_batch = MARKERS_LO + 8  # 33199
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_33100 / gj_shell_green_33000 / gj_shell_green_32900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_33100 / gj_libcgj_green_33000 / gj_libcgj_green_32900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_33100 / gj_bar3_ready_33000 / gj_bar3_ready_32900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_33100 / gj_product_score_33000 / gj_product_score_32900\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_32700 / gj_continuum_ready_32600 /\n"
            f" * gj_continuum_ready_32500 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_33100 / gj_smoke_soft_33000 / gj_smoke_soft_32900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_33100 / gj_dyn_soft_33000 / gj_dyn_soft_32900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_33100 / gj_milestone_tag_33000 / gj_milestone_tag_32900,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_32700 / gj_continuum_wave_32600 /\n"
            f" * gj_continuum_wave_32500, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch33200.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 33199
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 33191–33199; 33200 is capstone
