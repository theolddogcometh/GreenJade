...
"""
Soft continuum generator — MILESTONE 33300 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 33201–33300 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=33300   when parent has wired through graph_batch33300.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 33201 33300
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 33201 33300
...
graph batches ending at M. For M = **33300** the exclusive range is
**33201–33300** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 33201–33250  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 33251–33275  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 33276–33290  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 33291–33300  │ decade soft gates (Wave 191 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 33300 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_33300        1          graph_batch33291.c
  gj_libcgj_green_33300       1          graph_batch33292.c
  gj_bar3_ready_33300         0          graph_batch33293.c  (bar3 open)
  gj_product_score_33300      0          graph_batch33294.c  (score open)
  gj_continuum_ready_33300    1          graph_batch33295.c
  gj_smoke_soft_33300         1          graph_batch33296.c
  gj_dyn_soft_33300           1          graph_batch33297.c
  gj_milestone_tag_33300      33300      graph_batch33298.c
  gj_continuum_wave_33300     33300      graph_batch33299.c
  gj_batch_id_33300           33300      graph_batch33300.c
  gj_graph_milestone_33300    33300      graph_batch33300.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=33300`` means parent wired CGJ_SRCS through
    ``graph_batch33300.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_33300`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_33300`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M33300_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_33200`` / ``gj_*_33100`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 191 exclusive CREATE-ONLY lands the **markers** band
  (33291–33300) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_33300.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_33300.py

  # Restrict to marker band only (soft-gate TUs; Wave 191 exclusive):
  python3 user/libcgj/src/_gen_milestone_33300.py --band markers

  # Extension identity only (33201–33250):
  python3 user/libcgj/src/_gen_milestone_33300.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_33300.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_33200.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 33300 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 33300
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 33201
WAVE_HI = 33300  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 33201, 33250
IDENTITY_LO, IDENTITY_HI = 33251, 33275
FILL_LO, FILL_HI = 33276, 33290
MARKERS_LO, MARKERS_HI = 33291, 33300
...
# Stems become gj_<stem>_33300. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (33291, "shell_green", 1, "shell product green extract (wave 33300)"),
    (33292, "libcgj_green", 1, "libcgj product green extract (wave 33300)"),
    (33293, "bar3_ready", 0, "product bar3 readiness gate (wave 33300)"),
    (33294, "product_score", 0, "product readiness score tag (wave 33300)"),
    (33295, "continuum_ready", 1, "continuum readiness gate (wave 33300)"),
    (33296, "smoke_soft", 1, "smoke soft lamp (wave 33300)"),
    (33297, "dyn_soft", 1, "dyn soft lamp (wave 33300)"),
    (33298, "milestone_tag", MILESTONE, "milestone 33300 product tag"),
    (33299, "continuum_wave", MILESTONE, "milestone 33300 continuum wave identity"),
    # 33300 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        33276: "continuum wave fill soft unit + batch id",
        33280: "continuum bridge soft unit + batch id",
        33290: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (33291–33299), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 33293
    tag_batch = MARKERS_LO + 7  # 33298
    wave_batch = MARKERS_LO + 8  # 33299
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_33200 / gj_shell_green_33100 / gj_shell_green_33000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_33200 / gj_libcgj_green_33100 / gj_libcgj_green_33000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_33200 / gj_bar3_ready_33100 / gj_bar3_ready_33000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_33200 / gj_product_score_33100 / gj_product_score_33000\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_33200 / gj_smoke_soft_33100 / gj_smoke_soft_33000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_33200 / gj_dyn_soft_33100 / gj_dyn_soft_33000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_33200 / gj_milestone_tag_33100 / gj_milestone_tag_33000,\n"
...
def render_milestone_capstone() -> str:
    """graph_batch33300.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 33299
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 33291–33299; 33300 is capstone
