...
"""
Soft continuum generator — MILESTONE 33400 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 33301–33400 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=33400   when parent has wired through graph_batch33400.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 33301 33400
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 33301 33400
...
graph batches ending at M. For M = **33400** the exclusive range is
**33301–33400** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 33301–33350  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 33351–33375  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 33376–33390  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 33391–33400  │ decade soft gates (Wave 192 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 33400 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_33400        1          graph_batch33391.c
  gj_libcgj_green_33400       1          graph_batch33392.c
  gj_bar3_ready_33400         0          graph_batch33393.c  (bar3 open)
  gj_product_score_33400      0          graph_batch33394.c  (score open)
  gj_continuum_ready_33400    1          graph_batch33395.c
  gj_smoke_soft_33400         1          graph_batch33396.c
  gj_dyn_soft_33400           1          graph_batch33397.c
  gj_milestone_tag_33400      33400      graph_batch33398.c
  gj_continuum_wave_33400     33400      graph_batch33399.c
  gj_batch_id_33400           33400      graph_batch33400.c
  gj_graph_milestone_33400    33400      graph_batch33400.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=33400`` means parent wired CGJ_SRCS through
    ``graph_batch33400.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_33400`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_33400`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M33400_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_33300`` / ``gj_*_33200`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 192 exclusive CREATE-ONLY lands the **markers** band
  (33391–33400) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_33400.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_33400.py

  # Restrict to marker band only (soft-gate TUs; Wave 192 exclusive):
  python3 user/libcgj/src/_gen_milestone_33400.py --band markers

  # Extension identity only (33301–33350):
  python3 user/libcgj/src/_gen_milestone_33400.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_33400.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_33300.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 33400 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 33400
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 33301
WAVE_HI = 33400  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 33301, 33350
IDENTITY_LO, IDENTITY_HI = 33351, 33375
FILL_LO, FILL_HI = 33376, 33390
MARKERS_LO, MARKERS_HI = 33391, 33400
...
# Stems become gj_<stem>_33400. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (33391, "shell_green", 1, "shell product green extract (wave 33400)"),
    (33392, "libcgj_green", 1, "libcgj product green extract (wave 33400)"),
    (33393, "bar3_ready", 0, "product bar3 readiness gate (wave 33400)"),
    (33394, "product_score", 0, "product readiness score tag (wave 33400)"),
    (33395, "continuum_ready", 1, "continuum readiness gate (wave 33400)"),
    (33396, "smoke_soft", 1, "smoke soft lamp (wave 33400)"),
    (33397, "dyn_soft", 1, "dyn soft lamp (wave 33400)"),
    (33398, "milestone_tag", MILESTONE, "milestone 33400 product tag"),
    (33399, "continuum_wave", MILESTONE, "milestone 33400 continuum wave identity"),
    # 33400 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        33376: "continuum wave fill soft unit + batch id",
        33380: "continuum bridge soft unit + batch id",
        33390: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (33391–33399), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 33393
    tag_batch = MARKERS_LO + 7  # 33398
    wave_batch = MARKERS_LO + 8  # 33399
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_33300 / gj_shell_green_33200 / gj_shell_green_33100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_33300 / gj_libcgj_green_33200 / gj_libcgj_green_33100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_33300 / gj_bar3_ready_33200 / gj_bar3_ready_33100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_33300 / gj_product_score_33200 / gj_product_score_33100\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_33300 / gj_continuum_ready_33200 /\n"
            f" * gj_continuum_ready_33100 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_33300 / gj_smoke_soft_33200 / gj_smoke_soft_33100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_33300 / gj_dyn_soft_33200 / gj_dyn_soft_33100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_33300 / gj_milestone_tag_33200 / gj_milestone_tag_33100,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_33300 / gj_continuum_wave_33200 /\n"
            f" * gj_continuum_wave_33100, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch33400.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 33399
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 33391–33399; 33400 is capstone
