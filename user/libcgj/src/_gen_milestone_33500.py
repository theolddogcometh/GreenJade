...
"""
Soft continuum generator — MILESTONE 33500 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 33401–33500 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=33500   when parent has wired through graph_batch33500.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 33401 33500
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 33401 33500
...
graph batches ending at M. For M = **33500** the exclusive range is
**33401–33500** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 33401–33450  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 33451–33475  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 33476–33490  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 33491–33500  │ decade soft gates (Wave 193 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 33500 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_33500        1          graph_batch33491.c
  gj_libcgj_green_33500       1          graph_batch33492.c
  gj_bar3_ready_33500         0          graph_batch33493.c  (bar3 open)
  gj_product_score_33500      0          graph_batch33494.c  (score open)
  gj_continuum_ready_33500    1          graph_batch33495.c
  gj_smoke_soft_33500         1          graph_batch33496.c
  gj_dyn_soft_33500           1          graph_batch33497.c
  gj_milestone_tag_33500      33500      graph_batch33498.c
  gj_continuum_wave_33500     33500      graph_batch33499.c
  gj_batch_id_33500           33500      graph_batch33500.c
  gj_graph_milestone_33500    33500      graph_batch33500.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=33500`` means parent wired CGJ_SRCS through
    ``graph_batch33500.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_33500`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_33500`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M33500_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_33400`` / ``gj_*_33300`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 193 exclusive CREATE-ONLY lands the **markers** band
  (33491–33500) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_33500.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_33500.py

  # Restrict to marker band only (soft-gate TUs; Wave 193 exclusive):
  python3 user/libcgj/src/_gen_milestone_33500.py --band markers

  # Extension identity only (33401–33450):
  python3 user/libcgj/src/_gen_milestone_33500.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_33500.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_33400.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 33500 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 33500
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 33401
WAVE_HI = 33500  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 33401, 33450
IDENTITY_LO, IDENTITY_HI = 33451, 33475
FILL_LO, FILL_HI = 33476, 33490
MARKERS_LO, MARKERS_HI = 33491, 33500
...
# Stems become gj_<stem>_33500. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (33491, "shell_green", 1, "shell product green extract (wave 33500)"),
    (33492, "libcgj_green", 1, "libcgj product green extract (wave 33500)"),
    (33493, "bar3_ready", 0, "product bar3 readiness gate (wave 33500)"),
    (33494, "product_score", 0, "product readiness score tag (wave 33500)"),
    (33495, "continuum_ready", 1, "continuum readiness gate (wave 33500)"),
    (33496, "smoke_soft", 1, "smoke soft lamp (wave 33500)"),
    (33497, "dyn_soft", 1, "dyn soft lamp (wave 33500)"),
    (33498, "milestone_tag", MILESTONE, "milestone 33500 product tag"),
    (33499, "continuum_wave", MILESTONE, "milestone 33500 continuum wave identity"),
    # 33500 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        33476: "continuum wave fill soft unit + batch id",
        33480: "continuum bridge soft unit + batch id",
        33490: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (33491–33499), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 33493
    tag_batch = MARKERS_LO + 7  # 33498
    wave_batch = MARKERS_LO + 8  # 33499
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_33400 / gj_shell_green_33300 / gj_shell_green_33500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_33400 / gj_libcgj_green_33300 / gj_libcgj_green_33500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_33400 / gj_bar3_ready_33300 / gj_bar3_ready_33500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_33400 / gj_product_score_33300 / gj_product_score_33500\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_33400 / gj_continuum_ready_33300 /\n"
            f" * gj_continuum_ready_33200 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_33400 / gj_smoke_soft_33300 / gj_smoke_soft_33500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_33400 / gj_dyn_soft_33300 / gj_dyn_soft_33500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_33400 / gj_milestone_tag_33300 / gj_milestone_tag_33500,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_33400 / gj_continuum_wave_33300 /\n"
            f" * gj_continuum_wave_33200, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch33500.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 33499
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 33491–33499; 33500 is capstone
