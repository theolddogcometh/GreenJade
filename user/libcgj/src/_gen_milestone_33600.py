...
"""
Soft continuum generator — MILESTONE 33600 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 33501–33600 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=33600   when parent has wired through graph_batch33600.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 33501 33600
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 33501 33600
...
graph batches ending at M. For M = **33600** the exclusive range is
**33501–33600** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 33501–33550  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 33551–33575  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 33576–33590  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 33591–33600  │ decade soft gates (Wave 194 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 33600 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_33600        1          graph_batch33591.c
  gj_libcgj_green_33600       1          graph_batch33592.c
  gj_bar3_ready_33600         0          graph_batch33593.c  (bar3 open)
  gj_product_score_33600      0          graph_batch33594.c  (score open)
  gj_continuum_ready_33600    1          graph_batch33595.c
  gj_smoke_soft_33600         1          graph_batch33596.c
  gj_dyn_soft_33600           1          graph_batch33597.c
  gj_milestone_tag_33600      33600      graph_batch33598.c
  gj_continuum_wave_33600     33600      graph_batch33599.c
  gj_batch_id_33600           33600      graph_batch33600.c
  gj_graph_milestone_33600    33600      graph_batch33600.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=33600`` means parent wired CGJ_SRCS through
    ``graph_batch33600.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_33600`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_33600`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M33600_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_33500`` / ``gj_*_33400`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 194 exclusive CREATE-ONLY lands the **markers** band
  (33591–33600) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_33600.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_33600.py

  # Restrict to marker band only (soft-gate TUs; Wave 194 exclusive):
  python3 user/libcgj/src/_gen_milestone_33600.py --band markers

  # Extension identity only (33501–33550):
  python3 user/libcgj/src/_gen_milestone_33600.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_33600.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_33500.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 33600 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 33600
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 33501
WAVE_HI = 33600  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 33501, 33550
IDENTITY_LO, IDENTITY_HI = 33551, 33575
FILL_LO, FILL_HI = 33576, 33590
MARKERS_LO, MARKERS_HI = 33591, 33600
...
# Stems become gj_<stem>_33600. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (33591, "shell_green", 1, "shell product green extract (wave 33600)"),
    (33592, "libcgj_green", 1, "libcgj product green extract (wave 33600)"),
    (33593, "bar3_ready", 0, "product bar3 readiness gate (wave 33600)"),
    (33594, "product_score", 0, "product readiness score tag (wave 33600)"),
    (33595, "continuum_ready", 1, "continuum readiness gate (wave 33600)"),
    (33596, "smoke_soft", 1, "smoke soft lamp (wave 33600)"),
    (33597, "dyn_soft", 1, "dyn soft lamp (wave 33600)"),
    (33598, "milestone_tag", MILESTONE, "milestone 33600 product tag"),
    (33599, "continuum_wave", MILESTONE, "milestone 33600 continuum wave identity"),
    # 33600 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        33576: "continuum wave fill soft unit + batch id",
        33580: "continuum bridge soft unit + batch id",
        33590: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (33591–33599), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 33593
    tag_batch = MARKERS_LO + 7  # 33598
    wave_batch = MARKERS_LO + 8  # 33599
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_33500 / gj_shell_green_33400 / gj_shell_green_33300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_33500 / gj_libcgj_green_33400 / gj_libcgj_green_33300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_33500 / gj_bar3_ready_33400 / gj_bar3_ready_33300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_33500 / gj_product_score_33400 / gj_product_score_33300\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_33500 / gj_continuum_ready_33400 /\n"
            f" * gj_continuum_ready_33300 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_33500 / gj_smoke_soft_33400 / gj_smoke_soft_33300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_33500 / gj_dyn_soft_33400 / gj_dyn_soft_33300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_33500 / gj_milestone_tag_33400 / gj_milestone_tag_33300,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_33500 / gj_continuum_wave_33400 /\n"
            f" * gj_continuum_wave_33300, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch33600.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 33599
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 33591–33599; 33600 is capstone
