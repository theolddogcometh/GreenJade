...
"""
Soft continuum generator — MILESTONE 33700 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 33601–33700 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=33700   when parent has wired through graph_batch33700.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 33601 33700
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 33601 33700
...
graph batches ending at M. For M = **33700** the exclusive range is
**33601–33700** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 33601–33650  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 33651–33675  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 33676–33690  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 33691–33700  │ decade soft gates (Wave 195 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 33700 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_33700        1          graph_batch33691.c
  gj_libcgj_green_33700       1          graph_batch33692.c
  gj_bar3_ready_33700         0          graph_batch33693.c  (bar3 open)
  gj_product_score_33700      0          graph_batch33694.c  (score open)
  gj_continuum_ready_33700    1          graph_batch33695.c
  gj_smoke_soft_33700         1          graph_batch33696.c
  gj_dyn_soft_33700           1          graph_batch33697.c
  gj_milestone_tag_33700      33700      graph_batch33698.c
  gj_continuum_wave_33700     33700      graph_batch33699.c
  gj_batch_id_33700           33700      graph_batch33700.c
  gj_graph_milestone_33700    33700      graph_batch33700.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=33700`` means parent wired CGJ_SRCS through
    ``graph_batch33700.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_33700`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_33700`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M33700_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_33600`` / ``gj_*_33500`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 195 exclusive CREATE-ONLY lands the **markers** band
  (33691–33700) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_33700.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_33700.py

  # Restrict to marker band only (soft-gate TUs; Wave 195 exclusive):
  python3 user/libcgj/src/_gen_milestone_33700.py --band markers

  # Extension identity only (33601–33650):
  python3 user/libcgj/src/_gen_milestone_33700.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_33700.py --force
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 33700 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 33700
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 33601
WAVE_HI = 33700  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 33601, 33650
IDENTITY_LO, IDENTITY_HI = 33651, 33675
FILL_LO, FILL_HI = 33676, 33690
MARKERS_LO, MARKERS_HI = 33691, 33700
...
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (33691, "shell_green", 1, "shell product green extract (wave 33700)"),
    (33692, "libcgj_green", 1, "libcgj product green extract (wave 33700)"),
    (33693, "bar3_ready", 0, "product bar3 readiness gate (wave 33700)"),
    (33694, "product_score", 0, "product readiness score tag (wave 33700)"),
    (33695, "continuum_ready", 1, "continuum readiness gate (wave 33700)"),
    (33696, "smoke_soft", 1, "smoke soft lamp (wave 33700)"),
    (33697, "dyn_soft", 1, "dyn soft lamp (wave 33700)"),
    (33698, "milestone_tag", MILESTONE, "milestone 33700 product tag"),
    (33699, "continuum_wave", MILESTONE, "milestone 33700 continuum wave identity"),
    # 33700 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        33676: "continuum wave fill soft unit + batch id",
        33680: "continuum bridge soft unit + batch id",
        33690: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (33691–33699), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 33693
    tag_batch = MARKERS_LO + 7  # 33698
    wave_batch = MARKERS_LO + 8  # 33699
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_33600 / gj_shell_green_33500 / gj_shell_green_33400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_33600 / gj_libcgj_green_33500 / gj_libcgj_green_33400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_33600 / gj_bar3_ready_33500 / gj_bar3_ready_33400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_33600 / gj_product_score_33500 / gj_product_score_33400\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_33600 / gj_continuum_ready_33500 /\n"
            f" * gj_continuum_ready_33400 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_33600 / gj_smoke_soft_33500 / gj_smoke_soft_33400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_33600 / gj_dyn_soft_33500 / gj_dyn_soft_33400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_33600 / gj_milestone_tag_33500 / gj_milestone_tag_33400,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_33600 / gj_continuum_wave_33500 /\n"
            f" * gj_continuum_wave_33400, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch33700.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 33699
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 33691–33699; 33700 is capstone
