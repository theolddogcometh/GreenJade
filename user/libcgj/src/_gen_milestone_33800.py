...
"""
Soft continuum generator — MILESTONE 33800 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 33701–33800 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=33800   when parent has wired through graph_batch33800.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 33701 33800
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 33701 33800
...
graph batches ending at M. For M = **33800** the exclusive range is
**33701–33800** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 33701–33750  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 33751–33775  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 33776–33790  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 33791–33800  │ decade soft gates (Wave 196 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 33800 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_33800        1          graph_batch33791.c
  gj_libcgj_green_33800       1          graph_batch33792.c
  gj_bar3_ready_33800         0          graph_batch33793.c  (bar3 open)
  gj_product_score_33800      0          graph_batch33794.c  (score open)
  gj_continuum_ready_33800    1          graph_batch33795.c
  gj_smoke_soft_33800         1          graph_batch33796.c
  gj_dyn_soft_33800           1          graph_batch33797.c
  gj_milestone_tag_33800      33800      graph_batch33798.c
  gj_continuum_wave_33800     33800      graph_batch33799.c
  gj_batch_id_33800           33800      graph_batch33800.c
  gj_graph_milestone_33800    33800      graph_batch33800.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=33800`` means parent wired CGJ_SRCS through
    ``graph_batch33800.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_33800`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_33800`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M33800_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_33700`` / ``gj_*_33600`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 196 exclusive CREATE-ONLY lands the **markers** band
  (33791–33800) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_33800.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_33800.py

  # Restrict to marker band only (soft-gate TUs; Wave 196 exclusive):
  python3 user/libcgj/src/_gen_milestone_33800.py --band markers

  # Extension identity only (33701–33750):
  python3 user/libcgj/src/_gen_milestone_33800.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_33800.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_33700.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 33800 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 33800
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 33701
WAVE_HI = 33800  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 33701, 33750
IDENTITY_LO, IDENTITY_HI = 33751, 33775
FILL_LO, FILL_HI = 33776, 33790
MARKERS_LO, MARKERS_HI = 33791, 33800
...
# Stems become gj_<stem>_33800. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (33791, "shell_green", 1, "shell product green extract (wave 33800)"),
    (33792, "libcgj_green", 1, "libcgj product green extract (wave 33800)"),
    (33793, "bar3_ready", 0, "product bar3 readiness gate (wave 33800)"),
    (33794, "product_score", 0, "product readiness score tag (wave 33800)"),
    (33795, "continuum_ready", 1, "continuum readiness gate (wave 33800)"),
    (33796, "smoke_soft", 1, "smoke soft lamp (wave 33800)"),
    (33797, "dyn_soft", 1, "dyn soft lamp (wave 33800)"),
    (33798, "milestone_tag", MILESTONE, "milestone 33800 product tag"),
    (33799, "continuum_wave", MILESTONE, "milestone 33800 continuum wave identity"),
    # 33800 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        33776: "continuum wave fill soft unit + batch id",
        33780: "continuum bridge soft unit + batch id",
        33790: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (33791–33799), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 33793
    tag_batch = MARKERS_LO + 7  # 33798
    wave_batch = MARKERS_LO + 8  # 33799
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_33700 / gj_shell_green_33600 / gj_shell_green_33500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_33700 / gj_libcgj_green_33600 / gj_libcgj_green_33500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_33700 / gj_bar3_ready_33600 / gj_bar3_ready_33500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_33700 / gj_product_score_33600 / gj_product_score_33500\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_33700 / gj_continuum_ready_33600 /\n"
            f" * gj_continuum_ready_33500 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_33700 / gj_smoke_soft_33600 / gj_smoke_soft_33500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_33700 / gj_dyn_soft_33600 / gj_dyn_soft_33500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_33700 / gj_milestone_tag_33600 / gj_milestone_tag_33500,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_33700 / gj_continuum_wave_33600 /\n"
            f" * gj_continuum_wave_33500, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch33800.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 33799
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 33791–33799; 33800 is capstone
