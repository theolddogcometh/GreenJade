...
"""
Soft continuum generator — MILESTONE 33900 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 33801–33900 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=33900   when parent has wired through graph_batch33900.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 33801 33900
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 33801 33900
...
graph batches ending at M. For M = **33900** the exclusive range is
**33801–33900** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 33801–33850  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 33851–33875  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 33876–33890  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 33891–33900  │ decade soft gates (Wave 197 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 33900 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_33900        1          graph_batch33891.c
  gj_libcgj_green_33900       1          graph_batch33892.c
  gj_bar3_ready_33900         0          graph_batch33893.c  (bar3 open)
  gj_product_score_33900      0          graph_batch33894.c  (score open)
  gj_continuum_ready_33900    1          graph_batch33895.c
  gj_smoke_soft_33900         1          graph_batch33896.c
  gj_dyn_soft_33900           1          graph_batch33897.c
  gj_milestone_tag_33900      33900      graph_batch33898.c
  gj_continuum_wave_33900     33900      graph_batch33899.c
  gj_batch_id_33900           33900      graph_batch33900.c
  gj_graph_milestone_33900    33900      graph_batch33900.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=33900`` means parent wired CGJ_SRCS through
    ``graph_batch33900.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_33900`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_33900`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M33900_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_33800`` / ``gj_*_33700`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 197 exclusive CREATE-ONLY lands the **markers** band
  (33891–33900) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_33900.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_33900.py

  # Restrict to marker band only (soft-gate TUs; Wave 197 exclusive):
  python3 user/libcgj/src/_gen_milestone_33900.py --band markers

  # Extension identity only (33801–33850):
  python3 user/libcgj/src/_gen_milestone_33900.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_33900.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_33800.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 33900 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 33900
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 33801
WAVE_HI = 33900  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 33801, 33850
IDENTITY_LO, IDENTITY_HI = 33851, 33875
FILL_LO, FILL_HI = 33876, 33890
MARKERS_LO, MARKERS_HI = 33891, 33900
...
# Stems become gj_<stem>_33900. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (33891, "shell_green", 1, "shell product green extract (wave 33900)"),
    (33892, "libcgj_green", 1, "libcgj product green extract (wave 33900)"),
    (33893, "bar3_ready", 0, "product bar3 readiness gate (wave 33900)"),
    (33894, "product_score", 0, "product readiness score tag (wave 33900)"),
    (33895, "continuum_ready", 1, "continuum readiness gate (wave 33900)"),
    (33896, "smoke_soft", 1, "smoke soft lamp (wave 33900)"),
    (33897, "dyn_soft", 1, "dyn soft lamp (wave 33900)"),
    (33898, "milestone_tag", MILESTONE, "milestone 33900 product tag"),
    (33899, "continuum_wave", MILESTONE, "milestone 33900 continuum wave identity"),
    # 33900 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        33876: "continuum wave fill soft unit + batch id",
        33880: "continuum bridge soft unit + batch id",
        33890: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (33891–33899), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 33893
    tag_batch = MARKERS_LO + 7  # 33898
    wave_batch = MARKERS_LO + 8  # 33899
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_33800 / gj_shell_green_33700 / gj_shell_green_33600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_33800 / gj_libcgj_green_33700 / gj_libcgj_green_33600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_33800 / gj_bar3_ready_33700 / gj_bar3_ready_33600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_33800 / gj_product_score_33700 / gj_product_score_33600\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_33800 / gj_continuum_ready_33700 /\n"
            f" * gj_continuum_ready_33600 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_33800 / gj_smoke_soft_33700 / gj_smoke_soft_33600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_33800 / gj_dyn_soft_33700 / gj_dyn_soft_33600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_33800 / gj_milestone_tag_33700 / gj_milestone_tag_33600,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_33800 / gj_continuum_wave_33700 /\n"
            f" * gj_continuum_wave_33600, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch33900.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 33899
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 33891–33899; 33900 is capstone
