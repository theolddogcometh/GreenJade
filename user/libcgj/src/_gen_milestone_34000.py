...
"""
Soft continuum generator — MILESTONE 34000 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 33901–34000 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=34000   when parent has wired through graph_batch34000.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 33901 34000
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 33901 34000
...
graph batches ending at M. For M = **34000** the exclusive range is
**33901–34000** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 33901–33950  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 33951–33975  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 33976–33990  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 33991–34000  │ decade soft gates (Wave 198 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 34000 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_34000        1          graph_batch33991.c
  gj_libcgj_green_34000       1          graph_batch33992.c
  gj_bar3_ready_34000         0          graph_batch33993.c  (bar3 open)
  gj_product_score_34000      0          graph_batch33994.c  (score open)
  gj_continuum_ready_34000    1          graph_batch33995.c
  gj_smoke_soft_34000         1          graph_batch33996.c
  gj_dyn_soft_34000           1          graph_batch33997.c
  gj_milestone_tag_34000      34000      graph_batch33998.c
  gj_continuum_wave_34000     34000      graph_batch33999.c
  gj_batch_id_34000           34000      graph_batch34000.c
  gj_graph_milestone_34000    34000      graph_batch34000.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=34000`` means parent wired CGJ_SRCS through
    ``graph_batch34000.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_34000`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_34000`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M34000_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_33900`` / ``gj_*_33800`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 198 exclusive CREATE-ONLY lands the **markers** band
  (33991–34000) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_34000.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_34000.py

  # Restrict to marker band only (soft-gate TUs; Wave 198 exclusive):
  python3 user/libcgj/src/_gen_milestone_34000.py --band markers

  # Extension identity only (33901–33950):
  python3 user/libcgj/src/_gen_milestone_34000.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_34000.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_33900.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 34000 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 34000
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 33901
WAVE_HI = 34000  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 33901, 33950
IDENTITY_LO, IDENTITY_HI = 33951, 33975
FILL_LO, FILL_HI = 33976, 33990
MARKERS_LO, MARKERS_HI = 33991, 34000
...
# Stems become gj_<stem>_34000. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (33991, "shell_green", 1, "shell product green extract (wave 34000)"),
    (33992, "libcgj_green", 1, "libcgj product green extract (wave 34000)"),
    (33993, "bar3_ready", 0, "product bar3 readiness gate (wave 34000)"),
    (33994, "product_score", 0, "product readiness score tag (wave 34000)"),
    (33995, "continuum_ready", 1, "continuum readiness gate (wave 34000)"),
    (33996, "smoke_soft", 1, "smoke soft lamp (wave 34000)"),
    (33997, "dyn_soft", 1, "dyn soft lamp (wave 34000)"),
    (33998, "milestone_tag", MILESTONE, "milestone 34000 product tag"),
    (33999, "continuum_wave", MILESTONE, "milestone 34000 continuum wave identity"),
    # 34000 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        33976: "continuum wave fill soft unit + batch id",
        33980: "continuum bridge soft unit + batch id",
        33990: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (33991–33999), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 33993
    tag_batch = MARKERS_LO + 7  # 33998
    wave_batch = MARKERS_LO + 8  # 33999
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_33900 / gj_shell_green_33800 / gj_shell_green_33700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_33900 / gj_libcgj_green_33800 / gj_libcgj_green_33700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_33900 / gj_bar3_ready_33800 / gj_bar3_ready_33700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_33900 / gj_product_score_33800 / gj_product_score_33700\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_33900 / gj_continuum_ready_33800 /\n"
            f" * gj_continuum_ready_33700 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_33900 / gj_smoke_soft_33800 / gj_smoke_soft_33700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_33900 / gj_dyn_soft_33800 / gj_dyn_soft_33700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_33900 / gj_milestone_tag_33800 / gj_milestone_tag_33700,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_33900 / gj_continuum_wave_33800 /\n"
            f" * gj_continuum_wave_33700, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch34000.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 33999
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 33991–33999; 34000 is capstone
