...
"""
Soft continuum generator — MILESTONE 34100 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 34001–34100 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=34100   when parent has wired through graph_batch34100.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 34001 34100
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 34001 34100
...
graph batches ending at M. For M = **34100** the exclusive range is
**34001–34100** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 34001–34050  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 34051–34075  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 34076–34090  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 34091–34100  │ decade soft gates (Wave 199 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 34100 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_34100        1          graph_batch34091.c
  gj_libcgj_green_34100       1          graph_batch34092.c
  gj_bar3_ready_34100         0          graph_batch34093.c  (bar3 open)
  gj_product_score_34100      0          graph_batch34094.c  (score open)
  gj_continuum_ready_34100    1          graph_batch34095.c
  gj_smoke_soft_34100         1          graph_batch34096.c
  gj_dyn_soft_34100           1          graph_batch34097.c
  gj_milestone_tag_34100      34100      graph_batch34098.c
  gj_continuum_wave_34100     34100      graph_batch34099.c
  gj_batch_id_34100           34100      graph_batch34100.c
  gj_graph_milestone_34100    34100      graph_batch34100.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=34100`` means parent wired CGJ_SRCS through
    ``graph_batch34100.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_34100`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_34100`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M34100_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_34000`` / ``gj_*_33900`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 199 exclusive CREATE-ONLY lands the **markers** band
  (34091–34100) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_34100.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_34100.py

  # Restrict to marker band only (soft-gate TUs; Wave 199 exclusive):
  python3 user/libcgj/src/_gen_milestone_34100.py --band markers

  # Extension identity only (34001–34050):
  python3 user/libcgj/src/_gen_milestone_34100.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_34100.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_34000.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 34100 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 34100
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 34001
WAVE_HI = 34100  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 34001, 34050
IDENTITY_LO, IDENTITY_HI = 34051, 34075
FILL_LO, FILL_HI = 34076, 34090
MARKERS_LO, MARKERS_HI = 34091, 34100
...
# Stems become gj_<stem>_34100. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (34091, "shell_green", 1, "shell product green extract (wave 34100)"),
    (34092, "libcgj_green", 1, "libcgj product green extract (wave 34100)"),
    (34093, "bar3_ready", 0, "product bar3 readiness gate (wave 34100)"),
    (34094, "product_score", 0, "product readiness score tag (wave 34100)"),
    (34095, "continuum_ready", 1, "continuum readiness gate (wave 34100)"),
    (34096, "smoke_soft", 1, "smoke soft lamp (wave 34100)"),
    (34097, "dyn_soft", 1, "dyn soft lamp (wave 34100)"),
    (34098, "milestone_tag", MILESTONE, "milestone 34100 product tag"),
    (34099, "continuum_wave", MILESTONE, "milestone 34100 continuum wave identity"),
    # 34100 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        34076: "continuum wave fill soft unit + batch id",
        34080: "continuum bridge soft unit + batch id",
        34090: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (34091–34099), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 34093
    tag_batch = MARKERS_LO + 7  # 34098
    wave_batch = MARKERS_LO + 8  # 34099
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_34000 / gj_shell_green_33900 / gj_shell_green_33800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_34000 / gj_libcgj_green_33900 / gj_libcgj_green_33800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_34000 / gj_bar3_ready_33900 / gj_bar3_ready_33800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_34000 / gj_product_score_33900 / gj_product_score_33800\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_34000 / gj_continuum_ready_33900 /\n"
            f" * gj_continuum_ready_33800 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_34000 / gj_smoke_soft_33900 / gj_smoke_soft_33800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_34000 / gj_dyn_soft_33900 / gj_dyn_soft_33800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_34000 / gj_milestone_tag_33900 / gj_milestone_tag_33800,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_34000 / gj_continuum_wave_33900 /\n"
            f" * gj_continuum_wave_33800, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch34100.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 34099
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 34091–34099; 34100 is capstone
