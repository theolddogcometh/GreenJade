...
"""
Soft continuum generator — MILESTONE 34200 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 34101–34200 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=34200   when parent has wired through graph_batch34200.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 34101 34200
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 34101 34200
...
graph batches ending at M. For M = **34200** the exclusive range is
**34101–34200** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 34101–34150  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 34151–34175  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 34176–34190  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 34191–34200  │ decade soft gates (Wave 200 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 34200 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_34200        1          graph_batch34191.c
  gj_libcgj_green_34200       1          graph_batch34192.c
  gj_bar3_ready_34200         0          graph_batch34193.c  (bar3 open)
  gj_product_score_34200      0          graph_batch34194.c  (score open)
  gj_continuum_ready_34200    1          graph_batch34195.c
  gj_smoke_soft_34200         1          graph_batch34196.c
  gj_dyn_soft_34200           1          graph_batch34197.c
  gj_milestone_tag_34200      34200      graph_batch34198.c
  gj_continuum_wave_34200     34200      graph_batch34199.c
  gj_batch_id_34200           34200      graph_batch34200.c
  gj_graph_milestone_34200    34200      graph_batch34200.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=34200`` means parent wired CGJ_SRCS through
    ``graph_batch34200.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_34200`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_34200`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M34200_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_34100`` / ``gj_*_34000`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 200 exclusive CREATE-ONLY lands the **markers** band
  (34191–34200) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_34200.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_34200.py

  # Restrict to marker band only (soft-gate TUs; Wave 200 exclusive):
  python3 user/libcgj/src/_gen_milestone_34200.py --band markers

  # Extension identity only (34101–34150):
  python3 user/libcgj/src/_gen_milestone_34200.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_34200.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_34100.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 34200 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 34200
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 34101
WAVE_HI = 34200  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 34101, 34150
IDENTITY_LO, IDENTITY_HI = 34151, 34175
FILL_LO, FILL_HI = 34176, 34190
MARKERS_LO, MARKERS_HI = 34191, 34200
...
# Stems become gj_<stem>_34200. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (34191, "shell_green", 1, "shell product green extract (wave 34200)"),
    (34192, "libcgj_green", 1, "libcgj product green extract (wave 34200)"),
    (34193, "bar3_ready", 0, "product bar3 readiness gate (wave 34200)"),
    (34194, "product_score", 0, "product readiness score tag (wave 34200)"),
    (34195, "continuum_ready", 1, "continuum readiness gate (wave 34200)"),
    (34196, "smoke_soft", 1, "smoke soft lamp (wave 34200)"),
    (34197, "dyn_soft", 1, "dyn soft lamp (wave 34200)"),
    (34198, "milestone_tag", MILESTONE, "milestone 34200 product tag"),
    (34199, "continuum_wave", MILESTONE, "milestone 34200 continuum wave identity"),
    # 34200 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        34176: "continuum wave fill soft unit + batch id",
        34180: "continuum bridge soft unit + batch id",
        34190: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (34191–34199), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 34193
    tag_batch = MARKERS_LO + 7  # 34198
    wave_batch = MARKERS_LO + 8  # 34199
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_34100 / gj_shell_green_34000 / gj_shell_green_33900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_34100 / gj_libcgj_green_34000 / gj_libcgj_green_33900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_34100 / gj_bar3_ready_34000 / gj_bar3_ready_33900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_34100 / gj_product_score_34000 / gj_product_score_33900\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_34100 / gj_continuum_ready_34000 /\n"
            f" * gj_continuum_ready_33900 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_34100 / gj_smoke_soft_34000 / gj_smoke_soft_33900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_34100 / gj_dyn_soft_34000 / gj_dyn_soft_33900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_34100 / gj_milestone_tag_34000 / gj_milestone_tag_33900,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_34100 / gj_continuum_wave_34000 /\n"
            f" * gj_continuum_wave_33900, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch34200.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 34199
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 34191–34199; 34200 is capstone
