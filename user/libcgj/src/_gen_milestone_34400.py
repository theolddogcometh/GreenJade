...
"""
Soft continuum generator — MILESTONE 34400 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 34301–34400 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=34400   when parent has wired through graph_batch34400.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 34301 34400
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 34301 34400
...
graph batches ending at M. For M = **34400** the exclusive range is
**34301–34400** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 34301–34350  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 34351–34375  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 34376–34390  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 34391–34400  │ decade soft gates (Wave 202 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 34400 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_34400        1          graph_batch34391.c
  gj_libcgj_green_34400       1          graph_batch34392.c
  gj_bar3_ready_34400         0          graph_batch34393.c  (bar3 open)
  gj_product_score_34400      0          graph_batch34394.c  (score open)
  gj_continuum_ready_34400    1          graph_batch34395.c
  gj_smoke_soft_34400         1          graph_batch34396.c
  gj_dyn_soft_34400           1          graph_batch34397.c
  gj_milestone_tag_34400      34400      graph_batch34398.c
  gj_continuum_wave_34400     34400      graph_batch34399.c
  gj_batch_id_34400           34400      graph_batch34400.c
  gj_graph_milestone_34400    34400      graph_batch34400.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=34400`` means parent wired CGJ_SRCS through
    ``graph_batch34400.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_34400`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_34400`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M34400_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_34300`` / ``gj_*_34200`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 202 exclusive CREATE-ONLY lands the **markers** band
  (34391–34400) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_34400.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_34400.py

  # Restrict to marker band only (soft-gate TUs; Wave 202 exclusive):
  python3 user/libcgj/src/_gen_milestone_34400.py --band markers

  # Extension identity only (34301–34350):
  python3 user/libcgj/src/_gen_milestone_34400.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_34400.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_34300.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 34400 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 34400
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 34301
WAVE_HI = 34400  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 34301, 34350
IDENTITY_LO, IDENTITY_HI = 34351, 34375
FILL_LO, FILL_HI = 34376, 34390
MARKERS_LO, MARKERS_HI = 34391, 34400
...
# Stems become gj_<stem>_34400. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (34391, "shell_green", 1, "shell product green extract (wave 34400)"),
    (34392, "libcgj_green", 1, "libcgj product green extract (wave 34400)"),
    (34393, "bar3_ready", 0, "product bar3 readiness gate (wave 34400)"),
    (34394, "product_score", 0, "product readiness score tag (wave 34400)"),
    (34395, "continuum_ready", 1, "continuum readiness gate (wave 34400)"),
    (34396, "smoke_soft", 1, "smoke soft lamp (wave 34400)"),
    (34397, "dyn_soft", 1, "dyn soft lamp (wave 34400)"),
    (34398, "milestone_tag", MILESTONE, "milestone 34400 product tag"),
    (34399, "continuum_wave", MILESTONE, "milestone 34400 continuum wave identity"),
    # 34400 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        34376: "continuum wave fill soft unit + batch id",
        34380: "continuum bridge soft unit + batch id",
        34390: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (34391–34399), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 34393
    tag_batch = MARKERS_LO + 7  # 34398
    wave_batch = MARKERS_LO + 8  # 34399
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_34300 / gj_shell_green_34200 / gj_shell_green_34100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_34300 / gj_libcgj_green_34200 / gj_libcgj_green_34100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_34300 / gj_bar3_ready_34200 / gj_bar3_ready_34100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_34300 / gj_product_score_34200 / gj_product_score_34100\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_34300 / gj_continuum_ready_34200 /\n"
            f" * gj_continuum_ready_34100 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_34300 / gj_smoke_soft_34200 / gj_smoke_soft_34100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_34300 / gj_dyn_soft_34200 / gj_dyn_soft_34100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_34300 / gj_milestone_tag_34200 / gj_milestone_tag_34100,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_34300 / gj_continuum_wave_34200 /\n"
            f" * gj_continuum_wave_34100, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch34400.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 34399
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 34391–34399; 34400 is capstone
