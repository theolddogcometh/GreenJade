...
"""
Soft continuum generator — MILESTONE 34500 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 34401–34500 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=34500   when parent has wired through graph_batch34500.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 34401 34500
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 34401 34500
...
graph batches ending at M. For M = **34500** the exclusive range is
**34401–34500** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 34401–34450  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 34451–34475  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 34476–34490  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 34491–34500  │ decade soft gates (Wave 203 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 34500 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_34500        1          graph_batch34491.c
  gj_libcgj_green_34500       1          graph_batch34492.c
  gj_bar3_ready_34500         0          graph_batch34493.c  (bar3 open)
  gj_product_score_34500      0          graph_batch34494.c  (score open)
  gj_continuum_ready_34500    1          graph_batch34495.c
  gj_smoke_soft_34500         1          graph_batch34496.c
  gj_dyn_soft_34500           1          graph_batch34497.c
  gj_milestone_tag_34500      34500      graph_batch34498.c
  gj_continuum_wave_34500     34500      graph_batch34499.c
  gj_batch_id_34500           34500      graph_batch34500.c
  gj_graph_milestone_34500    34500      graph_batch34500.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=34500`` means parent wired CGJ_SRCS through
    ``graph_batch34500.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_34500`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_34500`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M34500_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_34400`` / ``gj_*_34300`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 203 exclusive CREATE-ONLY lands the **markers** band
  (34491–34500) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_34500.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_34500.py

  # Restrict to marker band only (soft-gate TUs; Wave 203 exclusive):
  python3 user/libcgj/src/_gen_milestone_34500.py --band markers

  # Extension identity only (34401–34450):
  python3 user/libcgj/src/_gen_milestone_34500.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_34500.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_34400.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 34500 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 34500
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 34401
WAVE_HI = 34500  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 34401, 34450
IDENTITY_LO, IDENTITY_HI = 34451, 34475
FILL_LO, FILL_HI = 34476, 34490
MARKERS_LO, MARKERS_HI = 34491, 34500
...
# Stems become gj_<stem>_34500. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (34491, "shell_green", 1, "shell product green extract (wave 34500)"),
    (34492, "libcgj_green", 1, "libcgj product green extract (wave 34500)"),
    (34493, "bar3_ready", 0, "product bar3 readiness gate (wave 34500)"),
    (34494, "product_score", 0, "product readiness score tag (wave 34500)"),
    (34495, "continuum_ready", 1, "continuum readiness gate (wave 34500)"),
    (34496, "smoke_soft", 1, "smoke soft lamp (wave 34500)"),
    (34497, "dyn_soft", 1, "dyn soft lamp (wave 34500)"),
    (34498, "milestone_tag", MILESTONE, "milestone 34500 product tag"),
    (34499, "continuum_wave", MILESTONE, "milestone 34500 continuum wave identity"),
    # 34500 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        34476: "continuum wave fill soft unit + batch id",
        34480: "continuum bridge soft unit + batch id",
        34490: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (34491–34499), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 34493
    tag_batch = MARKERS_LO + 7  # 34498
    wave_batch = MARKERS_LO + 8  # 34499
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_34400 / gj_shell_green_34300 / gj_shell_green_34200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_34400 / gj_libcgj_green_34300 / gj_libcgj_green_34200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_34400 / gj_bar3_ready_34300 / gj_bar3_ready_34200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_34400 / gj_product_score_34300 / gj_product_score_34200\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_34400 / gj_continuum_ready_34300 /\n"
            f" * gj_continuum_ready_34200 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_34400 / gj_smoke_soft_34300 / gj_smoke_soft_34200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_34400 / gj_dyn_soft_34300 / gj_dyn_soft_34200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_34400 / gj_milestone_tag_34300 / gj_milestone_tag_34200,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_34400 / gj_continuum_wave_34300 /\n"
            f" * gj_continuum_wave_34200, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch34500.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 34499
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 34491–34499; 34500 is capstone
