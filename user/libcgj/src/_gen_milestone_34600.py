...
"""
Soft continuum generator — MILESTONE 34600 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 34501–34600 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=34600   when parent has wired through graph_batch34600.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 34501 34600
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 34501 34600
...
graph batches ending at M. For M = **34600** the exclusive range is
**34501–34600** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 34501–34550  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 34551–34575  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 34576–34590  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 34591–34600  │ decade soft gates (Wave 204 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 34600 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_34600        1          graph_batch34591.c
  gj_libcgj_green_34600       1          graph_batch34592.c
  gj_bar3_ready_34600         0          graph_batch34593.c  (bar3 open)
  gj_product_score_34600      0          graph_batch34594.c  (score open)
  gj_continuum_ready_34600    1          graph_batch34595.c
  gj_smoke_soft_34600         1          graph_batch34596.c
  gj_dyn_soft_34600           1          graph_batch34597.c
  gj_milestone_tag_34600      34600      graph_batch34598.c
  gj_continuum_wave_34600     34600      graph_batch34599.c
  gj_batch_id_34600           34600      graph_batch34600.c
  gj_graph_milestone_34600    34600      graph_batch34600.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=34600`` means parent wired CGJ_SRCS through
    ``graph_batch34600.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_34600`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_34600`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M34600_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_34500`` / ``gj_*_34400`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 204 exclusive CREATE-ONLY lands the **markers** band
  (34591–34600) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_34600.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_34600.py

  # Restrict to marker band only (soft-gate TUs; Wave 204 exclusive):
  python3 user/libcgj/src/_gen_milestone_34600.py --band markers

  # Extension identity only (34501–34550):
  python3 user/libcgj/src/_gen_milestone_34600.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_34600.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_34500.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 34600 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 34600
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 34501
WAVE_HI = 34600  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 34501, 34550
IDENTITY_LO, IDENTITY_HI = 34551, 34575
FILL_LO, FILL_HI = 34576, 34590
MARKERS_LO, MARKERS_HI = 34591, 34600
...
# Stems become gj_<stem>_34600. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (34591, "shell_green", 1, "shell product green extract (wave 34600)"),
    (34592, "libcgj_green", 1, "libcgj product green extract (wave 34600)"),
    (34593, "bar3_ready", 0, "product bar3 readiness gate (wave 34600)"),
    (34594, "product_score", 0, "product readiness score tag (wave 34600)"),
    (34595, "continuum_ready", 1, "continuum readiness gate (wave 34600)"),
    (34596, "smoke_soft", 1, "smoke soft lamp (wave 34600)"),
    (34597, "dyn_soft", 1, "dyn soft lamp (wave 34600)"),
    (34598, "milestone_tag", MILESTONE, "milestone 34600 product tag"),
    (34599, "continuum_wave", MILESTONE, "milestone 34600 continuum wave identity"),
    # 34600 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        34576: "continuum wave fill soft unit + batch id",
        34580: "continuum bridge soft unit + batch id",
        34590: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (34591–34599), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 34593
    tag_batch = MARKERS_LO + 7  # 34598
    wave_batch = MARKERS_LO + 8  # 34599
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_34500 / gj_shell_green_34400 / gj_shell_green_34300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_34500 / gj_libcgj_green_34400 / gj_libcgj_green_34300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_34500 / gj_bar3_ready_34400 / gj_bar3_ready_34300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_34500 / gj_product_score_34400 / gj_product_score_34300\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_34500 / gj_continuum_ready_34400 /\n"
            f" * gj_continuum_ready_34300 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_34500 / gj_smoke_soft_34400 / gj_smoke_soft_34300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_34500 / gj_dyn_soft_34400 / gj_dyn_soft_34300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_34500 / gj_milestone_tag_34400 / gj_milestone_tag_34300,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_34500 / gj_continuum_wave_34400 /\n"
            f" * gj_continuum_wave_34300, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch34600.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 34599
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 34591–34599; 34600 is capstone
