...
"""
Soft continuum generator — MILESTONE 34700 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 34601–34700 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=34700   when parent has wired through graph_batch34700.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 34601 34700
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 34601 34700
...
graph batches ending at M. For M = **34700** the exclusive range is
**34601–34700** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 34601–34650  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 34651–34675  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 34676–34690  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 34691–34700  │ decade soft gates (Wave 205 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 34700 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_34700        1          graph_batch34691.c
  gj_libcgj_green_34700       1          graph_batch34692.c
  gj_bar3_ready_34700         0          graph_batch34693.c  (bar3 open)
  gj_product_score_34700      0          graph_batch34694.c  (score open)
  gj_continuum_ready_34700    1          graph_batch34695.c
  gj_smoke_soft_34700         1          graph_batch34696.c
  gj_dyn_soft_34700           1          graph_batch34697.c
  gj_milestone_tag_34700      34700      graph_batch34698.c
  gj_continuum_wave_34700     34700      graph_batch34699.c
  gj_batch_id_34700           34700      graph_batch34700.c
  gj_graph_milestone_34700    34700      graph_batch34700.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=34700`` means parent wired CGJ_SRCS through
    ``graph_batch34700.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_34700`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_34700`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M34700_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_34600`` / ``gj_*_34500`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 205 exclusive CREATE-ONLY lands the **markers** band
  (34691–34700) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_34700.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_34700.py

  # Restrict to marker band only (soft-gate TUs; Wave 205 exclusive):
  python3 user/libcgj/src/_gen_milestone_34700.py --band markers

  # Extension identity only (34601–34650):
  python3 user/libcgj/src/_gen_milestone_34700.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_34700.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_34600.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 34700 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 34700
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 34601
WAVE_HI = 34700  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 34601, 34650
IDENTITY_LO, IDENTITY_HI = 34651, 34675
FILL_LO, FILL_HI = 34676, 34690
MARKERS_LO, MARKERS_HI = 34691, 34700
...
# Stems become gj_<stem>_34700. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (34691, "shell_green", 1, "shell product green extract (wave 34700)"),
    (34692, "libcgj_green", 1, "libcgj product green extract (wave 34700)"),
    (34693, "bar3_ready", 0, "product bar3 readiness gate (wave 34700)"),
    (34694, "product_score", 0, "product readiness score tag (wave 34700)"),
    (34695, "continuum_ready", 1, "continuum readiness gate (wave 34700)"),
    (34696, "smoke_soft", 1, "smoke soft lamp (wave 34700)"),
    (34697, "dyn_soft", 1, "dyn soft lamp (wave 34700)"),
    (34698, "milestone_tag", MILESTONE, "milestone 34700 product tag"),
    (34699, "continuum_wave", MILESTONE, "milestone 34700 continuum wave identity"),
    # 34700 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        34676: "continuum wave fill soft unit + batch id",
        34680: "continuum bridge soft unit + batch id",
        34690: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (34691–34699), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 34693
    tag_batch = MARKERS_LO + 7  # 34698
    wave_batch = MARKERS_LO + 8  # 34699
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_34600 / gj_shell_green_34500 / gj_shell_green_34400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_34600 / gj_libcgj_green_34500 / gj_libcgj_green_34400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_34600 / gj_bar3_ready_34500 / gj_bar3_ready_34400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_34600 / gj_product_score_34500 / gj_product_score_34400\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_34600 / gj_continuum_ready_34500 /\n"
            f" * gj_continuum_ready_34400 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_34600 / gj_smoke_soft_34500 / gj_smoke_soft_34400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_34600 / gj_dyn_soft_34500 / gj_dyn_soft_34400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_34600 / gj_milestone_tag_34500 / gj_milestone_tag_34400,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_34600 / gj_continuum_wave_34500 /\n"
            f" * gj_continuum_wave_34400, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch34700.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 34699
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 34691–34699; 34700 is capstone
