...
"""
Soft continuum generator — MILESTONE 34800 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 34701–34800 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=34800   when parent has wired through graph_batch34800.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 34701 34800
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 34701 34800
...
graph batches ending at M. For M = **34800** the exclusive range is
**34701–34800** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 34701–34750  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 34751–34775  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 34776–34790  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 34791–34800  │ decade soft gates (Wave 206 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 34800 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_34800        1          graph_batch34791.c
  gj_libcgj_green_34800       1          graph_batch34792.c
  gj_bar3_ready_34800         0          graph_batch34793.c  (bar3 open)
  gj_product_score_34800      0          graph_batch34794.c  (score open)
  gj_continuum_ready_34800    1          graph_batch34795.c
  gj_smoke_soft_34800         1          graph_batch34796.c
  gj_dyn_soft_34800           1          graph_batch34797.c
  gj_milestone_tag_34800      34800      graph_batch34798.c
  gj_continuum_wave_34800     34800      graph_batch34799.c
  gj_batch_id_34800           34800      graph_batch34800.c
  gj_graph_milestone_34800    34800      graph_batch34800.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=34800`` means parent wired CGJ_SRCS through
    ``graph_batch34800.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_34800`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_34800`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M34800_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_34700`` / ``gj_*_34600`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 206 exclusive CREATE-ONLY lands the **markers** band
  (34791–34800) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_34800.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_34800.py

  # Restrict to marker band only (soft-gate TUs; Wave 206 exclusive):
  python3 user/libcgj/src/_gen_milestone_34800.py --band markers

  # Extension identity only (34701–34750):
  python3 user/libcgj/src/_gen_milestone_34800.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_34800.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_34700.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 34800 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 34800
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 34701
WAVE_HI = 34800  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 34701, 34750
IDENTITY_LO, IDENTITY_HI = 34751, 34775
FILL_LO, FILL_HI = 34776, 34790
MARKERS_LO, MARKERS_HI = 34791, 34800
...
# Stems become gj_<stem>_34800. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (34791, "shell_green", 1, "shell product green extract (wave 34800)"),
    (34792, "libcgj_green", 1, "libcgj product green extract (wave 34800)"),
    (34793, "bar3_ready", 0, "product bar3 readiness gate (wave 34800)"),
    (34794, "product_score", 0, "product readiness score tag (wave 34800)"),
    (34795, "continuum_ready", 1, "continuum readiness gate (wave 34800)"),
    (34796, "smoke_soft", 1, "smoke soft lamp (wave 34800)"),
    (34797, "dyn_soft", 1, "dyn soft lamp (wave 34800)"),
    (34798, "milestone_tag", MILESTONE, "milestone 34800 product tag"),
    (34799, "continuum_wave", MILESTONE, "milestone 34800 continuum wave identity"),
    # 34800 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        34776: "continuum wave fill soft unit + batch id",
        34780: "continuum bridge soft unit + batch id",
        34790: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (34791–34799), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 34793
    tag_batch = MARKERS_LO + 7  # 34798
    wave_batch = MARKERS_LO + 8  # 34799
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_34700 / gj_shell_green_34600 / gj_shell_green_34500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_34700 / gj_libcgj_green_34600 / gj_libcgj_green_34500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_34700 / gj_bar3_ready_34600 / gj_bar3_ready_34500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_34700 / gj_product_score_34600 / gj_product_score_34500\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_34700 / gj_continuum_ready_34600 /\n"
            f" * gj_continuum_ready_34500 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_34700 / gj_smoke_soft_34600 / gj_smoke_soft_34500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_34700 / gj_dyn_soft_34600 / gj_dyn_soft_34500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_34700 / gj_milestone_tag_34600 / gj_milestone_tag_34500,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_34700 / gj_continuum_wave_34600 /\n"
            f" * gj_continuum_wave_34500, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch34800.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 34799
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 34791–34799; 34800 is capstone
