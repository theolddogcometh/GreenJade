...
"""
Soft continuum generator — MILESTONE 34900 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 34801–34900 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=34900   when parent has wired through graph_batch34800.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 34801 34900
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 34801 34900
...
graph batches ending at M. For M = **34900** the exclusive range is
**34801–34900** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 34801–34850  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 34851–34875  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 34876–34890  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 34891–34900  │ decade soft gates (Wave 207 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 34900 marker KATs (soft host probe expectations)
----------------------------------------------------------
  ...
  gj_milestone_tag_34800      34900      graph_batch34798.c
  gj_continuum_wave_34800     34900      graph_batch34799.c
  gj_batch_id_34800           34900      graph_batch34800.c
  gj_graph_milestone_34800    34900      graph_batch34800.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=34900`` means parent wired CGJ_SRCS through
  ...
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M34900_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_34800`` / ``gj_*_34700`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 207 exclusive CREATE-ONLY lands the **markers** band
  (34891–34900) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_34900.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_34900.py

  # Restrict to marker band only (soft-gate TUs; Wave 207 exclusive):
  python3 user/libcgj/src/_gen_milestone_34900.py --band markers

  # Extension identity only (34801–34850):
  python3 user/libcgj/src/_gen_milestone_34900.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_34900.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_34800.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 34900 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 34900
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 34801
WAVE_HI = 34900  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 34801, 34850
IDENTITY_LO, IDENTITY_HI = 34851, 34875
FILL_LO, FILL_HI = 34876, 34890
MARKERS_LO, MARKERS_HI = 34891, 34900
...
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (34891, "shell_green", 1, "shell product green extract (wave 34900)"),
    (34892, "libcgj_green", 1, "libcgj product green extract (wave 34900)"),
    (34893, "bar3_ready", 0, "product bar3 readiness gate (wave 34900)"),
    (34894, "product_score", 0, "product readiness score tag (wave 34900)"),
    (34895, "continuum_ready", 1, "continuum readiness gate (wave 34900)"),
    (34896, "smoke_soft", 1, "smoke soft lamp (wave 34900)"),
    (34897, "dyn_soft", 1, "dyn soft lamp (wave 34900)"),
    (34898, "milestone_tag", MILESTONE, "milestone 34900 product tag"),
    (34899, "continuum_wave", MILESTONE, "milestone 34900 continuum wave identity"),
    # 34900 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        34876: "continuum wave fill soft unit + batch id",
        34880: "continuum bridge soft unit + batch id",
        34890: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (34891–34899), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 34893
    tag_batch = MARKERS_LO + 7  # 34898
    wave_batch = MARKERS_LO + 8  # 34899
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_34800 / gj_shell_green_34700 / gj_shell_green_34600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_34800 / gj_libcgj_green_34700 / gj_libcgj_green_34600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_34800 / gj_bar3_ready_34700 / gj_bar3_ready_34600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_34800 / gj_product_score_34700 / gj_product_score_34600\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_34800 / gj_continuum_ready_34700 /\n"
            f" * gj_continuum_ready_34600 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_34800 / gj_smoke_soft_34700 / gj_smoke_soft_34600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_34800 / gj_dyn_soft_34700 / gj_dyn_soft_34600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_34800 / gj_milestone_tag_34700 / gj_milestone_tag_34600,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_34800 / gj_continuum_wave_34700 /\n"
            f" * gj_continuum_wave_34600, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    ...
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 34899
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 34891–34899; 34900 is capstone
