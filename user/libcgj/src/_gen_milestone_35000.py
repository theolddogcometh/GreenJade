...
"""
Soft continuum generator — MILESTONE 35000 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 34901–35000 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=35000   when parent has wired through graph_batch35000.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 34901 35000
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 34901 35000
...
graph batches ending at M. For M = **35000** the exclusive range is
**34901–35000** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 34901–34950  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 34951–34975  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 34976–34990  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 34991–35000  │ decade soft gates (Wave 208 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 35000 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_35000        1          graph_batch34991.c
  gj_libcgj_green_35000       1          graph_batch34992.c
  gj_bar3_ready_35000         0          graph_batch34993.c  (bar3 open)
  gj_product_score_35000      0          graph_batch34994.c  (score open)
  gj_continuum_ready_35000    1          graph_batch34995.c
  gj_smoke_soft_35000         1          graph_batch34996.c
  gj_dyn_soft_35000           1          graph_batch34997.c
  gj_milestone_tag_35000      35000      graph_batch34998.c
  gj_continuum_wave_35000     35000      graph_batch34999.c
  gj_batch_id_35000           35000      graph_batch35000.c
  gj_graph_milestone_35000    35000      graph_batch35000.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=35000`` means parent wired CGJ_SRCS through
    ``graph_batch35000.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_35000`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_35000`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M35000_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_34900`` / ``gj_*_34800`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 208 exclusive CREATE-ONLY lands the **markers** band
  (34991–35000) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_35000.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_35000.py

  # Restrict to marker band only (soft-gate TUs; Wave 208 exclusive):
  python3 user/libcgj/src/_gen_milestone_35000.py --band markers

  # Extension identity only (34901–34950):
  python3 user/libcgj/src/_gen_milestone_35000.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_35000.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_34900.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 35000 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 35000
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 34901
WAVE_HI = 35000  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 34901, 34950
IDENTITY_LO, IDENTITY_HI = 34951, 34975
FILL_LO, FILL_HI = 34976, 34990
MARKERS_LO, MARKERS_HI = 34991, 35000
...
# Stems become gj_<stem>_35000. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (34991, "shell_green", 1, "shell product green extract (wave 35000)"),
    (34992, "libcgj_green", 1, "libcgj product green extract (wave 35000)"),
    (34993, "bar3_ready", 0, "product bar3 readiness gate (wave 35000)"),
    (34994, "product_score", 0, "product readiness score tag (wave 35000)"),
    (34995, "continuum_ready", 1, "continuum readiness gate (wave 35000)"),
    (34996, "smoke_soft", 1, "smoke soft lamp (wave 35000)"),
    (34997, "dyn_soft", 1, "dyn soft lamp (wave 35000)"),
    (34998, "milestone_tag", MILESTONE, "milestone 35000 product tag"),
    (34999, "continuum_wave", MILESTONE, "milestone 35000 continuum wave identity"),
    # 35000 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        34976: "continuum wave fill soft unit + batch id",
        34980: "continuum bridge soft unit + batch id",
        34990: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (34991–34999), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 34993
    tag_batch = MARKERS_LO + 7  # 34998
    wave_batch = MARKERS_LO + 8  # 34999
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_34900 / gj_shell_green_34800 / gj_shell_green_34700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_34900 / gj_libcgj_green_34800 / gj_libcgj_green_34700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_34900 / gj_bar3_ready_34800 / gj_bar3_ready_34700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_34900 / gj_product_score_34800 / gj_product_score_34700\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_34900 / gj_continuum_ready_34800 /\n"
            f" * gj_continuum_ready_34700 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_34900 / gj_smoke_soft_34800 / gj_smoke_soft_34700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_34900 / gj_dyn_soft_34800 / gj_dyn_soft_34700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_34900 / gj_milestone_tag_34800 / gj_milestone_tag_34700,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_34900 / gj_continuum_wave_34800 /\n"
            f" * gj_continuum_wave_34700, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch35000.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 34999
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 34991–34999; 35000 is capstone
