...
"""
Soft continuum generator — MILESTONE 35100 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 35001–35100 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=35100   when parent has wired through graph_batch35100.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 35001 35100
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 35001 35100
...
graph batches ending at M. For M = **35100** the exclusive range is
**35001–35100** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 35001–35050  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 35051–35075  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 35076–35090  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 35091–35100  │ decade soft gates (Wave 209 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 35100 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_35100        1          graph_batch35091.c
  gj_libcgj_green_35100       1          graph_batch35092.c
  gj_bar3_ready_35100         0          graph_batch35093.c  (bar3 open)
  gj_product_score_35100      0          graph_batch35094.c  (score open)
  gj_continuum_ready_35100    1          graph_batch35095.c
  gj_smoke_soft_35100         1          graph_batch35096.c
  gj_dyn_soft_35100           1          graph_batch35097.c
  gj_milestone_tag_35100      35100      graph_batch35098.c
  gj_continuum_wave_35100     35100      graph_batch35099.c
  gj_batch_id_35100           35100      graph_batch35100.c
  gj_graph_milestone_35100    35100      graph_batch35100.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=35100`` means parent wired CGJ_SRCS through
    ``graph_batch35100.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_35100`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_35100`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M35100_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_35000`` / ``gj_*_34900`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 209 exclusive CREATE-ONLY lands the **markers** band
  (35091–35100) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_35100.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_35100.py

  # Restrict to marker band only (soft-gate TUs; Wave 209 exclusive):
  python3 user/libcgj/src/_gen_milestone_35100.py --band markers

  # Extension identity only (35001–35050):
  python3 user/libcgj/src/_gen_milestone_35100.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_35100.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_35000.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 35100 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 35100
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 35001
WAVE_HI = 35100  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 35001, 35050
IDENTITY_LO, IDENTITY_HI = 35051, 35075
FILL_LO, FILL_HI = 35076, 35090
MARKERS_LO, MARKERS_HI = 35091, 35100
...
# Stems become gj_<stem>_35100. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (35091, "shell_green", 1, "shell product green extract (wave 35100)"),
    (35092, "libcgj_green", 1, "libcgj product green extract (wave 35100)"),
    (35093, "bar3_ready", 0, "product bar3 readiness gate (wave 35100)"),
    (35094, "product_score", 0, "product readiness score tag (wave 35100)"),
    (35095, "continuum_ready", 1, "continuum readiness gate (wave 35100)"),
    (35096, "smoke_soft", 1, "smoke soft lamp (wave 35100)"),
    (35097, "dyn_soft", 1, "dyn soft lamp (wave 35100)"),
    (35098, "milestone_tag", MILESTONE, "milestone 35100 product tag"),
    (35099, "continuum_wave", MILESTONE, "milestone 35100 continuum wave identity"),
    # 35100 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        35076: "continuum wave fill soft unit + batch id",
        35080: "continuum bridge soft unit + batch id",
        35090: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (35091–35099), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 35093
    tag_batch = MARKERS_LO + 7  # 35098
    wave_batch = MARKERS_LO + 8  # 35099
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_35000 / gj_shell_green_34900 / gj_shell_green_34800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_35000 / gj_libcgj_green_34900 / gj_libcgj_green_34800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_35000 / gj_bar3_ready_34900 / gj_bar3_ready_34800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_35000 / gj_product_score_34900 / gj_product_score_34800\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_35000 / gj_continuum_ready_34900 /\n"
            f" * gj_continuum_ready_34800 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_35000 / gj_smoke_soft_34900 / gj_smoke_soft_34800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_35000 / gj_dyn_soft_34900 / gj_dyn_soft_34800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_35000 / gj_milestone_tag_34900 / gj_milestone_tag_34800,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_35000 / gj_continuum_wave_34900 /\n"
            f" * gj_continuum_wave_34800, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch35100.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 35099
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 35091–35099; 35100 is capstone
