...
"""
Soft continuum generator — MILESTONE 35200 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 35101–35200 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=35200   when parent has wired through graph_batch35200.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 35101 35200
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 35101 35200
...
graph batches ending at M. For M = **35200** the exclusive range is
**35101–35200** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 35101–35150  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 35151–35175  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 35176–35190  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 35191–35200  │ decade soft gates (Wave 210 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 35200 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_35200        1          graph_batch35191.c
  gj_libcgj_green_35200       1          graph_batch35192.c
  gj_bar3_ready_35200         0          graph_batch35193.c  (bar3 open)
  gj_product_score_35200      0          graph_batch35194.c  (score open)
  gj_continuum_ready_35200    1          graph_batch35195.c
  gj_smoke_soft_35200         1          graph_batch35196.c
  gj_dyn_soft_35200           1          graph_batch35197.c
  gj_milestone_tag_35200      35200      graph_batch35198.c
  gj_continuum_wave_35200     35200      graph_batch35199.c
  gj_batch_id_35200           35200      graph_batch35200.c
  gj_graph_milestone_35200    35200      graph_batch35200.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=35200`` means parent wired CGJ_SRCS through
    ``graph_batch35200.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_35200`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_35200`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M35200_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_35100`` / ``gj_*_35000`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 210 exclusive CREATE-ONLY lands the **markers** band
  (35191–35200) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_35200.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_35200.py

  # Restrict to marker band only (soft-gate TUs; Wave 210 exclusive):
  python3 user/libcgj/src/_gen_milestone_35200.py --band markers

  # Extension identity only (35101–35150):
  python3 user/libcgj/src/_gen_milestone_35200.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_35200.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_35100.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 35200 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 35200
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 35101
WAVE_HI = 35200  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 35101, 35150
IDENTITY_LO, IDENTITY_HI = 35151, 35175
FILL_LO, FILL_HI = 35176, 35190
MARKERS_LO, MARKERS_HI = 35191, 35200
...
# Stems become gj_<stem>_35200. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (35191, "shell_green", 1, "shell product green extract (wave 35200)"),
    (35192, "libcgj_green", 1, "libcgj product green extract (wave 35200)"),
    (35193, "bar3_ready", 0, "product bar3 readiness gate (wave 35200)"),
    (35194, "product_score", 0, "product readiness score tag (wave 35200)"),
    (35195, "continuum_ready", 1, "continuum readiness gate (wave 35200)"),
    (35196, "smoke_soft", 1, "smoke soft lamp (wave 35200)"),
    (35197, "dyn_soft", 1, "dyn soft lamp (wave 35200)"),
    (35198, "milestone_tag", MILESTONE, "milestone 35200 product tag"),
    (35199, "continuum_wave", MILESTONE, "milestone 35200 continuum wave identity"),
    # 35200 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        35176: "continuum wave fill soft unit + batch id",
        35180: "continuum bridge soft unit + batch id",
        35190: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (35191–35199), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 35193
    tag_batch = MARKERS_LO + 7  # 35198
    wave_batch = MARKERS_LO + 8  # 35199
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_35100 / gj_shell_green_35000 / gj_shell_green_34900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_35100 / gj_libcgj_green_35000 / gj_libcgj_green_34900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_35100 / gj_bar3_ready_35000 / gj_bar3_ready_34900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_35100 / gj_product_score_35000 / gj_product_score_34900\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_35100 / gj_continuum_ready_35000 /\n"
            f" * gj_continuum_ready_34900 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_35100 / gj_smoke_soft_35000 / gj_smoke_soft_34900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_35100 / gj_dyn_soft_35000 / gj_dyn_soft_34900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_35100 / gj_milestone_tag_35000 / gj_milestone_tag_34900,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_35100 / gj_continuum_wave_35000 /\n"
            f" * gj_continuum_wave_34900, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch35200.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 35199
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 35191–35199; 35200 is capstone
