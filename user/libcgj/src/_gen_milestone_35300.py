...
"""
Soft continuum generator — MILESTONE 35300 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 35201–35300 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=35300   when parent has wired through graph_batch35300.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 35201 35300
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 35201 35300
...
graph batches ending at M. For M = **35300** the exclusive range is
**35201–35300** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 35201–35250  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 35251–35275  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 35276–35290  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 35291–35300  │ decade soft gates (Wave 211 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 35300 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_35300        1          graph_batch35291.c
  gj_libcgj_green_35300       1          graph_batch35292.c
  gj_bar3_ready_35300         0          graph_batch35293.c  (bar3 open)
  gj_product_score_35300      0          graph_batch35294.c  (score open)
  gj_continuum_ready_35300    1          graph_batch35295.c
  gj_smoke_soft_35300         1          graph_batch35296.c
  gj_dyn_soft_35300           1          graph_batch35297.c
  gj_milestone_tag_35300      35300      graph_batch35298.c
  gj_continuum_wave_35300     35300      graph_batch35299.c
  gj_batch_id_35300           35300      graph_batch35300.c
  gj_graph_milestone_35300    35300      graph_batch35300.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=35300`` means parent wired CGJ_SRCS through
    ``graph_batch35300.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_35300`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_35300`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M35300_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_35200`` / ``gj_*_35100`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 211 exclusive CREATE-ONLY lands the **markers** band
  (35291–35300) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_35300.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_35300.py

  # Restrict to marker band only (soft-gate TUs; Wave 211 exclusive):
  python3 user/libcgj/src/_gen_milestone_35300.py --band markers

  # Extension identity only (35201–35250):
  python3 user/libcgj/src/_gen_milestone_35300.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_35300.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_35200.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 35300 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 35300
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 35201
WAVE_HI = 35300  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 35201, 35250
IDENTITY_LO, IDENTITY_HI = 35251, 35275
FILL_LO, FILL_HI = 35276, 35290
MARKERS_LO, MARKERS_HI = 35291, 35300
...
# Stems become gj_<stem>_35300. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (35291, "shell_green", 1, "shell product green extract (wave 35300)"),
    (35292, "libcgj_green", 1, "libcgj product green extract (wave 35300)"),
    (35293, "bar3_ready", 0, "product bar3 readiness gate (wave 35300)"),
    (35294, "product_score", 0, "product readiness score tag (wave 35300)"),
    (35295, "continuum_ready", 1, "continuum readiness gate (wave 35300)"),
    (35296, "smoke_soft", 1, "smoke soft lamp (wave 35300)"),
    (35297, "dyn_soft", 1, "dyn soft lamp (wave 35300)"),
    (35298, "milestone_tag", MILESTONE, "milestone 35300 product tag"),
    (35299, "continuum_wave", MILESTONE, "milestone 35300 continuum wave identity"),
    # 35300 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        35276: "continuum wave fill soft unit + batch id",
        35280: "continuum bridge soft unit + batch id",
        35290: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (35291–35299), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 35293
    tag_batch = MARKERS_LO + 7  # 35298
    wave_batch = MARKERS_LO + 8  # 35299
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_35200 / gj_shell_green_35100 / gj_shell_green_35000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_35200 / gj_libcgj_green_35100 / gj_libcgj_green_35000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_35200 / gj_bar3_ready_35100 / gj_bar3_ready_35000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_35200 / gj_product_score_35100 / gj_product_score_35000\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_35200 / gj_continuum_ready_35100 /\n"
            f" * gj_continuum_ready_35000 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_35200 / gj_smoke_soft_35100 / gj_smoke_soft_35000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_35200 / gj_dyn_soft_35100 / gj_dyn_soft_35000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_35200 / gj_milestone_tag_35100 / gj_milestone_tag_35000,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_35200 / gj_continuum_wave_35100 /\n"
            f" * gj_continuum_wave_35000, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch35300.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 35299
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 35291–35299; 35300 is capstone
