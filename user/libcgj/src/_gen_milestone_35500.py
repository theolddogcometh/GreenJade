...
"""
Soft continuum generator — MILESTONE 35500 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 35401–35500 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=35500   when parent has wired through graph_batch35500.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 35401 35500
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 35401 35500
...
graph batches ending at M. For M = **35500** the exclusive range is
**35401–35500** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 35401–35450  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 35451–35475  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 35476–35490  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 35491–35500  │ decade soft gates (Wave 213 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 35500 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_35500        1          graph_batch35491.c
  gj_libcgj_green_35500       1          graph_batch35492.c
  gj_bar3_ready_35500         0          graph_batch35493.c  (bar3 open)
  gj_product_score_35500      0          graph_batch35494.c  (score open)
  gj_continuum_ready_35500    1          graph_batch35495.c
  gj_smoke_soft_35500         1          graph_batch35496.c
  gj_dyn_soft_35500           1          graph_batch35497.c
  gj_milestone_tag_35500      35500      graph_batch35498.c
  gj_continuum_wave_35500     35500      graph_batch35499.c
  gj_batch_id_35500           35500      graph_batch35500.c
  gj_graph_milestone_35500    35500      graph_batch35500.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=35500`` means parent wired CGJ_SRCS through
    ``graph_batch35500.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_35500`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_35500`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M35500_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_35400`` / ``gj_*_35300`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 213 exclusive CREATE-ONLY lands the **markers** band
  (35491–35500) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_35500.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_35500.py

  # Restrict to marker band only (soft-gate TUs; Wave 213 exclusive):
  python3 user/libcgj/src/_gen_milestone_35500.py --band markers

  # Extension identity only (35401–35450):
  python3 user/libcgj/src/_gen_milestone_35500.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_35500.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_35400.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 35500 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 35500
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 35401
WAVE_HI = 35500  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 35401, 35450
IDENTITY_LO, IDENTITY_HI = 35451, 35475
FILL_LO, FILL_HI = 35476, 35490
MARKERS_LO, MARKERS_HI = 35491, 35500
...
# Stems become gj_<stem>_35500. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (35491, "shell_green", 1, "shell product green extract (wave 35500)"),
    (35492, "libcgj_green", 1, "libcgj product green extract (wave 35500)"),
    (35493, "bar3_ready", 0, "product bar3 readiness gate (wave 35500)"),
    (35494, "product_score", 0, "product readiness score tag (wave 35500)"),
    (35495, "continuum_ready", 1, "continuum readiness gate (wave 35500)"),
    (35496, "smoke_soft", 1, "smoke soft lamp (wave 35500)"),
    (35497, "dyn_soft", 1, "dyn soft lamp (wave 35500)"),
    (35498, "milestone_tag", MILESTONE, "milestone 35500 product tag"),
    (35499, "continuum_wave", MILESTONE, "milestone 35500 continuum wave identity"),
    # 35500 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        35476: "continuum wave fill soft unit + batch id",
        35480: "continuum bridge soft unit + batch id",
        35490: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (35491–35499), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 35493
    tag_batch = MARKERS_LO + 7  # 35498
    wave_batch = MARKERS_LO + 8  # 35499
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_35400 / gj_shell_green_35300 / gj_shell_green_35200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_35400 / gj_libcgj_green_35300 / gj_libcgj_green_35200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_35400 / gj_bar3_ready_35300 / gj_bar3_ready_35200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_35400 / gj_product_score_35300 / gj_product_score_35200\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_35400 / gj_continuum_ready_35300 /\n"
            f" * gj_continuum_ready_35200 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_35400 / gj_smoke_soft_35300 / gj_smoke_soft_35200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_35400 / gj_dyn_soft_35300 / gj_dyn_soft_35200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_35400 / gj_milestone_tag_35300 / gj_milestone_tag_35200,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_35400 / gj_continuum_wave_35300 /\n"
            f" * gj_continuum_wave_35200, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch35500.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 35499
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 35491–35499; 35500 is capstone
