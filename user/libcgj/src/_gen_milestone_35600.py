...
"""
Soft continuum generator — MILESTONE 35600 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 35501–35600 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=35600   when parent has wired through graph_batch35600.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 35501 35600
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 35501 35600
...
graph batches ending at M. For M = **35600** the exclusive range is
**35501–35600** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 35501–35550  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 35551–35575  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 35576–35590  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 35591–35600  │ decade soft gates (Wave 214 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 35600 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_35600        1          graph_batch35591.c
  gj_libcgj_green_35600       1          graph_batch35592.c
  gj_bar3_ready_35600         0          graph_batch35593.c  (bar3 open)
  gj_product_score_35600      0          graph_batch35594.c  (score open)
  gj_continuum_ready_35600    1          graph_batch35595.c
  gj_smoke_soft_35600         1          graph_batch35596.c
  gj_dyn_soft_35600           1          graph_batch35597.c
  gj_milestone_tag_35600      35600      graph_batch35598.c
  gj_continuum_wave_35600     35600      graph_batch35599.c
  gj_batch_id_35600           35600      graph_batch35600.c
  gj_graph_milestone_35600    35600      graph_batch35600.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=35600`` means parent wired CGJ_SRCS through
    ``graph_batch35600.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_35600`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_35600`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M35600_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_35500`` / ``gj_*_35400`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 214 exclusive CREATE-ONLY lands the **markers** band
  (35591–35600) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_35600.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_35600.py

  # Restrict to marker band only (soft-gate TUs; Wave 214 exclusive):
  python3 user/libcgj/src/_gen_milestone_35600.py --band markers

  # Extension identity only (35501–35550):
  python3 user/libcgj/src/_gen_milestone_35600.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_35600.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_35500.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 35600 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 35600
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 35501
WAVE_HI = 35600  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 35501, 35550
IDENTITY_LO, IDENTITY_HI = 35551, 35575
FILL_LO, FILL_HI = 35576, 35590
MARKERS_LO, MARKERS_HI = 35591, 35600
...
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (35591, "shell_green", 1, "shell product green extract (wave 35600)"),
    (35592, "libcgj_green", 1, "libcgj product green extract (wave 35600)"),
    (35593, "bar3_ready", 0, "product bar3 readiness gate (wave 35600)"),
    (35594, "product_score", 0, "product readiness score tag (wave 35600)"),
    (35595, "continuum_ready", 1, "continuum readiness gate (wave 35600)"),
    (35596, "smoke_soft", 1, "smoke soft lamp (wave 35600)"),
    (35597, "dyn_soft", 1, "dyn soft lamp (wave 35600)"),
    (35598, "milestone_tag", MILESTONE, "milestone 35600 product tag"),
    (35599, "continuum_wave", MILESTONE, "milestone 35600 continuum wave identity"),
    # 35600 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        35576: "continuum wave fill soft unit + batch id",
        35580: "continuum bridge soft unit + batch id",
        35590: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (35591–35599), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 35593
    tag_batch = MARKERS_LO + 7  # 35598
    wave_batch = MARKERS_LO + 8  # 35599
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_35500 / gj_shell_green_35400 / gj_shell_green_35300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_35500 / gj_libcgj_green_35400 / gj_libcgj_green_35300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_35500 / gj_bar3_ready_35400 / gj_bar3_ready_35300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_35500 / gj_product_score_35400 / gj_product_score_35300\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_35500 / gj_continuum_ready_35400 /\n"
            f" * gj_continuum_ready_35300 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_35500 / gj_smoke_soft_35400 / gj_smoke_soft_35300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_35500 / gj_dyn_soft_35400 / gj_dyn_soft_35300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_35500 / gj_milestone_tag_35400 / gj_milestone_tag_35300,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_35500 / gj_continuum_wave_35400 /\n"
            f" * gj_continuum_wave_35300, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch35600.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 35599
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 35591–35599; 35600 is capstone
