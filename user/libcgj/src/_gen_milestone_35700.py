...
"""
Soft continuum generator — MILESTONE 35700 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 35601–35700 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=35700   when parent has wired through graph_batch35700.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 35601 35700
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 35601 35700
...
graph batches ending at M. For M = **35700** the exclusive range is
**35601–35700** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 35601–35650  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 35651–35675  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 35676–35690  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 35691–35700  │ decade soft gates (Wave 215 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 35700 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_35700        1          graph_batch35691.c
  gj_libcgj_green_35700       1          graph_batch35692.c
  gj_bar3_ready_35700         0          graph_batch35693.c  (bar3 open)
  gj_product_score_35700      0          graph_batch35694.c  (score open)
  gj_continuum_ready_35700    1          graph_batch35695.c
  gj_smoke_soft_35700         1          graph_batch35696.c
  gj_dyn_soft_35700           1          graph_batch35697.c
  gj_milestone_tag_35700      35700      graph_batch35698.c
  gj_continuum_wave_35700     35700      graph_batch35699.c
  gj_batch_id_35700           35700      graph_batch35700.c
  gj_graph_milestone_35700    35700      graph_batch35700.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=35700`` means parent wired CGJ_SRCS through
    ``graph_batch35700.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_35700`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_35700`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M35700_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_35600`` / ``gj_*_35500`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 215 exclusive CREATE-ONLY lands the **markers** band
  (35691–35700) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_35700.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_35700.py

  # Restrict to marker band only (soft-gate TUs; Wave 215 exclusive):
  python3 user/libcgj/src/_gen_milestone_35700.py --band markers

  # Extension identity only (35601–35650):
  python3 user/libcgj/src/_gen_milestone_35700.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_35700.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_35600.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 35700 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 35700
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 35601
WAVE_HI = 35700  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 35601, 35650
IDENTITY_LO, IDENTITY_HI = 35651, 35675
FILL_LO, FILL_HI = 35676, 35690
MARKERS_LO, MARKERS_HI = 35691, 35700
...
# Stems become gj_<stem>_35600. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (35691, "shell_green", 1, "shell product green extract (wave 35700)"),
    (35692, "libcgj_green", 1, "libcgj product green extract (wave 35700)"),
    (35693, "bar3_ready", 0, "product bar3 readiness gate (wave 35700)"),
    (35694, "product_score", 0, "product readiness score tag (wave 35700)"),
    (35695, "continuum_ready", 1, "continuum readiness gate (wave 35700)"),
    (35696, "smoke_soft", 1, "smoke soft lamp (wave 35700)"),
    (35697, "dyn_soft", 1, "dyn soft lamp (wave 35700)"),
    (35698, "milestone_tag", MILESTONE, "milestone 35700 product tag"),
    (35699, "continuum_wave", MILESTONE, "milestone 35700 continuum wave identity"),
    # 35700 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        35676: "continuum wave fill soft unit + batch id",
        35680: "continuum bridge soft unit + batch id",
        35690: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (35691–35699), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 35693
    tag_batch = MARKERS_LO + 7  # 35698
    wave_batch = MARKERS_LO + 8  # 35699
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_35600 / gj_shell_green_35500 / gj_shell_green_35400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_35600 / gj_libcgj_green_35500 / gj_libcgj_green_35400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_35600 / gj_bar3_ready_35500 / gj_bar3_ready_35400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_35600 / gj_product_score_35500 / gj_product_score_35400\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_35600 / gj_continuum_ready_35500 /\n"
            f" * gj_continuum_ready_35400 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_35600 / gj_smoke_soft_35500 / gj_smoke_soft_35400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_35600 / gj_dyn_soft_35500 / gj_dyn_soft_35400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_35600 / gj_milestone_tag_35500 / gj_milestone_tag_35400,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_35600 / gj_continuum_wave_35500 /\n"
            f" * gj_continuum_wave_35400, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch35700.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 35699
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 35691–35699; 35600 is capstone
