...
"""
Soft continuum generator — MILESTONE 35800 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 35701–35800 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=35800   when parent has wired through graph_batch35800.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 35701 35800
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 35701 35800
...
graph batches ending at M. For M = **35800** the exclusive range is
**35701–35800** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 35701–35750  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 35751–35775  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 35776–35790  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 35791–35800  │ decade soft gates (Wave 216 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 35800 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_35800        1          graph_batch35791.c
  gj_libcgj_green_35800       1          graph_batch35792.c
  gj_bar3_ready_35800         0          graph_batch35793.c  (bar3 open)
  gj_product_score_35800      0          graph_batch35794.c  (score open)
  gj_continuum_ready_35800    1          graph_batch35795.c
  gj_smoke_soft_35800         1          graph_batch35796.c
  gj_dyn_soft_35800           1          graph_batch35797.c
  gj_milestone_tag_35800      35800      graph_batch35798.c
  gj_continuum_wave_35800     35800      graph_batch35799.c
  gj_batch_id_35800           35800      graph_batch35800.c
  gj_graph_milestone_35800    35800      graph_batch35800.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=35800`` means parent wired CGJ_SRCS through
    ``graph_batch35800.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_35800`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_35800`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M35800_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_35700`` / ``gj_*_35600`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 216 exclusive CREATE-ONLY lands the **markers** band
  (35791–35800) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_35800.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_35800.py

  # Restrict to marker band only (soft-gate TUs; Wave 216 exclusive):
  python3 user/libcgj/src/_gen_milestone_35800.py --band markers

  # Extension identity only (35701–35750):
  python3 user/libcgj/src/_gen_milestone_35800.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_35800.py --force
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 35800 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 35800
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 35701
WAVE_HI = 35800  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 35701, 35750
IDENTITY_LO, IDENTITY_HI = 35751, 35775
FILL_LO, FILL_HI = 35776, 35790
MARKERS_LO, MARKERS_HI = 35791, 35800
...
# Stems become gj_<stem>_35800. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (35791, "shell_green", 1, "shell product green extract (wave 35800)"),
    (35792, "libcgj_green", 1, "libcgj product green extract (wave 35800)"),
    (35793, "bar3_ready", 0, "product bar3 readiness gate (wave 35800)"),
    (35794, "product_score", 0, "product readiness score tag (wave 35800)"),
    (35795, "continuum_ready", 1, "continuum readiness gate (wave 35800)"),
    (35796, "smoke_soft", 1, "smoke soft lamp (wave 35800)"),
    (35797, "dyn_soft", 1, "dyn soft lamp (wave 35800)"),
    (35798, "milestone_tag", MILESTONE, "milestone 35800 product tag"),
    (35799, "continuum_wave", MILESTONE, "milestone 35800 continuum wave identity"),
    # 35800 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        35776: "continuum wave fill soft unit + batch id",
        35780: "continuum bridge soft unit + batch id",
        35790: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (35791–35799), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 35793
    tag_batch = MARKERS_LO + 7  # 35798
    wave_batch = MARKERS_LO + 8  # 35799
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_35700 / gj_shell_green_35600 / gj_shell_green_35500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_35700 / gj_libcgj_green_35600 / gj_libcgj_green_35500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_35700 / gj_bar3_ready_35600 / gj_bar3_ready_35500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_35700 / gj_product_score_35600 / gj_product_score_35500\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_35700 / gj_continuum_ready_35600 /\n"
            f" * gj_continuum_ready_35500 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_35700 / gj_smoke_soft_35600 / gj_smoke_soft_35500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_35700 / gj_dyn_soft_35600 / gj_dyn_soft_35500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_35700 / gj_milestone_tag_35600 / gj_milestone_tag_35500,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_35700 / gj_continuum_wave_35600 /\n"
            f" * gj_continuum_wave_35500, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch35800.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 35799
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 35791–35799; 35800 is capstone
