...
"""
Soft continuum generator — MILESTONE 35900 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 35801–35900 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=35900   when parent has wired through graph_batch35900.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 35801 35900
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 35801 35900
...
graph batches ending at M. For M = **35900** the exclusive range is
**35801–35900** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 35801–35850  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 35851–35875  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 35876–35890  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 35891–35900  │ decade soft gates (Wave 217 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 35900 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_35900        1          graph_batch35891.c
  gj_libcgj_green_35900       1          graph_batch35892.c
  gj_bar3_ready_35900         0          graph_batch35893.c  (bar3 open)
  gj_product_score_35900      0          graph_batch35894.c  (score open)
  gj_continuum_ready_35900    1          graph_batch35895.c
  gj_smoke_soft_35900         1          graph_batch35896.c
  gj_dyn_soft_35900           1          graph_batch35897.c
  gj_milestone_tag_35900      35900      graph_batch35898.c
  gj_continuum_wave_35900     35900      graph_batch35899.c
  gj_batch_id_35900           35900      graph_batch35900.c
  gj_graph_milestone_35900    35900      graph_batch35900.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=35900`` means parent wired CGJ_SRCS through
    ``graph_batch35900.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_35900`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_35900`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M35900_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_35800`` / ``gj_*_35700`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 217 exclusive CREATE-ONLY lands the **markers** band
  (35891–35900) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_35900.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_35900.py

  # Restrict to marker band only (soft-gate TUs; Wave 217 exclusive):
  python3 user/libcgj/src/_gen_milestone_35900.py --band markers

  # Extension identity only (35801–35850):
  python3 user/libcgj/src/_gen_milestone_35900.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_35900.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_35800.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 35900 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 35900
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 35801
WAVE_HI = 35900  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 35801, 35850
IDENTITY_LO, IDENTITY_HI = 35851, 35875
FILL_LO, FILL_HI = 35876, 35890
MARKERS_LO, MARKERS_HI = 35891, 35900
...
# Stems become gj_<stem>_35900. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (35891, "shell_green", 1, "shell product green extract (wave 35900)"),
    (35892, "libcgj_green", 1, "libcgj product green extract (wave 35900)"),
    (35893, "bar3_ready", 0, "product bar3 readiness gate (wave 35900)"),
    (35894, "product_score", 0, "product readiness score tag (wave 35900)"),
    (35895, "continuum_ready", 1, "continuum readiness gate (wave 35900)"),
    (35896, "smoke_soft", 1, "smoke soft lamp (wave 35900)"),
    (35897, "dyn_soft", 1, "dyn soft lamp (wave 35900)"),
    (35898, "milestone_tag", MILESTONE, "milestone 35900 product tag"),
    (35899, "continuum_wave", MILESTONE, "milestone 35900 continuum wave identity"),
    # 35900 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        35876: "continuum wave fill soft unit + batch id",
        35880: "continuum bridge soft unit + batch id",
        35890: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (35891–35899), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 35893
    tag_batch = MARKERS_LO + 7  # 35898
    wave_batch = MARKERS_LO + 8  # 35899
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_35800 / gj_shell_green_35700 / gj_shell_green_35900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_35800 / gj_libcgj_green_35700 / gj_libcgj_green_35900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_35800 / gj_bar3_ready_35700 / gj_bar3_ready_35900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_35800 / gj_product_score_35700 / gj_product_score_35900\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_35800 / gj_continuum_ready_35700 /\n"
            f" * gj_continuum_ready_35600 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_35800 / gj_smoke_soft_35700 / gj_smoke_soft_35900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_35800 / gj_dyn_soft_35700 / gj_dyn_soft_35900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_35800 / gj_milestone_tag_35700 / gj_milestone_tag_35900,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_35800 / gj_continuum_wave_35700 /\n"
            f" * gj_continuum_wave_35600, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch35900.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 35899
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 35891–35899; 35900 is capstone
