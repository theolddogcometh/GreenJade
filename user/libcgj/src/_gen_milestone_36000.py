...
"""
Soft continuum generator — MILESTONE 36000 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 35901–36000 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=36000   when parent has wired through graph_batch36000.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 35901 36000
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 35901 36000
...
graph batches ending at M. For M = **36000** the exclusive range is
**35901–36000** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 35901–35950  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 35951–35975  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 35976–35990  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 35991–36000  │ decade soft gates (Wave 218 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 36000 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_36000        1          graph_batch35991.c
  gj_libcgj_green_36000       1          graph_batch35992.c
  gj_bar3_ready_36000         0          graph_batch35993.c  (bar3 open)
  gj_product_score_36000      0          graph_batch35994.c  (score open)
  gj_continuum_ready_36000    1          graph_batch35995.c
  gj_smoke_soft_36000         1          graph_batch35996.c
  gj_dyn_soft_36000           1          graph_batch35997.c
  gj_milestone_tag_36000      36000      graph_batch35998.c
  gj_continuum_wave_36000     36000      graph_batch35999.c
  gj_batch_id_36000           36000      graph_batch36000.c
  gj_graph_milestone_36000    36000      graph_batch36000.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=36000`` means parent wired CGJ_SRCS through
    ``graph_batch36000.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_36000`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_36000`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M36000_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_35900`` / ``gj_*_35800`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 218 exclusive CREATE-ONLY lands the **markers** band
  (35991–36000) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_36000.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_36000.py

  # Restrict to marker band only (soft-gate TUs; Wave 218 exclusive):
  python3 user/libcgj/src/_gen_milestone_36000.py --band markers

  # Extension identity only (35901–35950):
  python3 user/libcgj/src/_gen_milestone_36000.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_36000.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_35900.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 36000 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 36000
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 35901
WAVE_HI = 36000  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 35901, 35950
IDENTITY_LO, IDENTITY_HI = 35951, 35975
FILL_LO, FILL_HI = 35976, 35990
MARKERS_LO, MARKERS_HI = 35991, 36000
...
# Stems become gj_<stem>_36000. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (35991, "shell_green", 1, "shell product green extract (wave 36000)"),
    (35992, "libcgj_green", 1, "libcgj product green extract (wave 36000)"),
    (35993, "bar3_ready", 0, "product bar3 readiness gate (wave 36000)"),
    (35994, "product_score", 0, "product readiness score tag (wave 36000)"),
    (35995, "continuum_ready", 1, "continuum readiness gate (wave 36000)"),
    (35996, "smoke_soft", 1, "smoke soft lamp (wave 36000)"),
    (35997, "dyn_soft", 1, "dyn soft lamp (wave 36000)"),
    (35998, "milestone_tag", MILESTONE, "milestone 36000 product tag"),
    (35999, "continuum_wave", MILESTONE, "milestone 36000 continuum wave identity"),
    # 36000 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        35976: "continuum wave fill soft unit + batch id",
        35980: "continuum bridge soft unit + batch id",
        35990: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (35991–35999), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 35993
    tag_batch = MARKERS_LO + 7  # 35998
    wave_batch = MARKERS_LO + 8  # 35999
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_35900 / gj_shell_green_35800 / gj_shell_green_35700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_35900 / gj_libcgj_green_35800 / gj_libcgj_green_35700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_35900 / gj_bar3_ready_35800 / gj_bar3_ready_35700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_35900 / gj_product_score_35800 / gj_product_score_35700\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_35900 / gj_continuum_ready_35800 /\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_35900 / gj_smoke_soft_35800 / gj_smoke_soft_35700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_35900 / gj_dyn_soft_35800 / gj_dyn_soft_35700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_35900 / gj_milestone_tag_35800 / gj_milestone_tag_35700,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_35900 / gj_continuum_wave_35800 /\n"
...
def render_milestone_capstone() -> str:
    """graph_batch36000.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 35999
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 35991–35999; 36000 is capstone
