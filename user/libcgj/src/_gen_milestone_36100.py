...
"""
Soft continuum generator — MILESTONE 36100 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 36001–36100 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=36100   when parent has wired through graph_batch36100.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 36001 36100
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 36001 36100
...
graph batches ending at M. For M = **36100** the exclusive range is
**36001–36100** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 36001–36050  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 36051–36075  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 36076–36090  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 36091–36100  │ decade soft gates (Wave 219 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 36100 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_36100        1          graph_batch36091.c
  gj_libcgj_green_36100       1          graph_batch36092.c
  gj_bar3_ready_36100         0          graph_batch36093.c  (bar3 open)
  gj_product_score_36100      0          graph_batch36094.c  (score open)
  gj_continuum_ready_36100    1          graph_batch36095.c
  gj_smoke_soft_36100         1          graph_batch36096.c
  gj_dyn_soft_36100           1          graph_batch36097.c
  gj_milestone_tag_36100      36100      graph_batch36098.c
  gj_continuum_wave_36100     36100      graph_batch36099.c
  gj_batch_id_36100           36100      graph_batch36100.c
  gj_graph_milestone_36100    36100      graph_batch36100.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=36100`` means parent wired CGJ_SRCS through
    ``graph_batch36100.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_36100`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_36100`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M36100_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_36000`` / ``gj_*_35900`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 219 exclusive CREATE-ONLY lands the **markers** band
  (36091–36100) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_36100.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_36100.py

  # Restrict to marker band only (soft-gate TUs; Wave 219 exclusive):
  python3 user/libcgj/src/_gen_milestone_36100.py --band markers

  # Extension identity only (36001–36050):
  python3 user/libcgj/src/_gen_milestone_36100.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_36100.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_36000.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 36100 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 36100
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 36001
WAVE_HI = 36100  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 36001, 36050
IDENTITY_LO, IDENTITY_HI = 36051, 36075
FILL_LO, FILL_HI = 36076, 36090
MARKERS_LO, MARKERS_HI = 36091, 36100
...
# Stems become gj_<stem>_36100. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (36091, "shell_green", 1, "shell product green extract (wave 36100)"),
    (36092, "libcgj_green", 1, "libcgj product green extract (wave 36100)"),
    (36093, "bar3_ready", 0, "product bar3 readiness gate (wave 36100)"),
    (36094, "product_score", 0, "product readiness score tag (wave 36100)"),
    (36095, "continuum_ready", 1, "continuum readiness gate (wave 36100)"),
    (36096, "smoke_soft", 1, "smoke soft lamp (wave 36100)"),
    (36097, "dyn_soft", 1, "dyn soft lamp (wave 36100)"),
    (36098, "milestone_tag", MILESTONE, "milestone 36100 product tag"),
    (36099, "continuum_wave", MILESTONE, "milestone 36100 continuum wave identity"),
    # 36100 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        36076: "continuum wave fill soft unit + batch id",
        36080: "continuum bridge soft unit + batch id",
        36090: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (36091–36099), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 36093
    tag_batch = MARKERS_LO + 7  # 36098
    wave_batch = MARKERS_LO + 8  # 36099
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_36000 / gj_shell_green_360002 / gj_shell_green_35700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_36000 / gj_libcgj_green_360002 / gj_libcgj_green_35700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_36000 / gj_bar3_ready_360002 / gj_bar3_ready_35700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_36000 / gj_product_score_360002 / gj_product_score_35700\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_36000 / gj_continuum_ready_360002 /\n"
            f" * gj_continuum_ready_35700 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_36000 / gj_smoke_soft_360002 / gj_smoke_soft_35700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_36000 / gj_dyn_soft_360002 / gj_dyn_soft_35700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_36000 / gj_milestone_tag_360002 / gj_milestone_tag_35700,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_36000 / gj_continuum_wave_360002 /\n"
            f" * gj_continuum_wave_35700, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch36100.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 36099
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 36091–36099; 36100 is capstone
