...
"""
Soft continuum generator — MILESTONE 36200 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 36101–36200 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=36200   when parent has wired through graph_batch36200.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 36101 36200
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 36101 36200
...
graph batches ending at M. For M = **36200** the exclusive range is
**36101–36200** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 36101–36150  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 36151–36175  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 36176–36190  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 36191–36200  │ decade soft gates (Wave 220 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 36200 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_36200        1          graph_batch36191.c
  gj_libcgj_green_36200       1          graph_batch36192.c
  gj_bar3_ready_36200         0          graph_batch36193.c  (bar3 open)
  gj_product_score_36200      0          graph_batch36194.c  (score open)
  gj_continuum_ready_36200    1          graph_batch36195.c
  gj_smoke_soft_36200         1          graph_batch36196.c
  gj_dyn_soft_36200           1          graph_batch36197.c
  gj_milestone_tag_36200      36200      graph_batch36198.c
  gj_continuum_wave_36200     36200      graph_batch36199.c
  gj_batch_id_36200           36200      graph_batch36200.c
  gj_graph_milestone_36200    36200      graph_batch36200.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=36200`` means parent wired CGJ_SRCS through
    ``graph_batch36200.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_36200`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_36200`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M36200_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_36100`` / ``gj_*_36000`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 220 exclusive CREATE-ONLY lands the **markers** band
  (36191–36200) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_36200.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_36200.py

  # Restrict to marker band only (soft-gate TUs; Wave 220 exclusive):
  python3 user/libcgj/src/_gen_milestone_36200.py --band markers

  # Extension identity only (36101–36150):
  python3 user/libcgj/src/_gen_milestone_36200.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_36200.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_36100.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 36200 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 36200
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 36101
WAVE_HI = 36200  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 36101, 36150
IDENTITY_LO, IDENTITY_HI = 36151, 36175
FILL_LO, FILL_HI = 36176, 36190
MARKERS_LO, MARKERS_HI = 36191, 36200
...
# Stems become gj_<stem>_36200. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (36191, "shell_green", 1, "shell product green extract (wave 36200)"),
    (36192, "libcgj_green", 1, "libcgj product green extract (wave 36200)"),
    (36193, "bar3_ready", 0, "product bar3 readiness gate (wave 36200)"),
    (36194, "product_score", 0, "product readiness score tag (wave 36200)"),
    (36195, "continuum_ready", 1, "continuum readiness gate (wave 36200)"),
    (36196, "smoke_soft", 1, "smoke soft lamp (wave 36200)"),
    (36197, "dyn_soft", 1, "dyn soft lamp (wave 36200)"),
    (36198, "milestone_tag", MILESTONE, "milestone 36200 product tag"),
    (36199, "continuum_wave", MILESTONE, "milestone 36200 continuum wave identity"),
    # 36200 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        36176: "continuum wave fill soft unit + batch id",
        36180: "continuum bridge soft unit + batch id",
        36190: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (36191–36199), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 36193
    tag_batch = MARKERS_LO + 7  # 36198
    wave_batch = MARKERS_LO + 8  # 36199
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_36100 / gj_shell_green_36000 / gj_shell_green_35900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_36100 / gj_libcgj_green_36000 / gj_libcgj_green_35900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_36100 / gj_bar3_ready_36000 / gj_bar3_ready_35900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_36100 / gj_product_score_36000 / gj_product_score_35900\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_36100 / gj_continuum_ready_36000 /\n"
            f" * gj_continuum_ready_35900 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_36100 / gj_smoke_soft_36000 / gj_smoke_soft_35900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_36100 / gj_dyn_soft_36000 / gj_dyn_soft_35900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_36100 / gj_milestone_tag_36000 / gj_milestone_tag_35900,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_36100 / gj_continuum_wave_36000 /\n"
            f" * gj_continuum_wave_35900, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch36200.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 36199
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 36191–36199; 36200 is capstone
