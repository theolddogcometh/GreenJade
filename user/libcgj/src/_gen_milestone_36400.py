...
"""
Soft continuum generator — MILESTONE 36400 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 36301–36400 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=36400   when parent has wired through graph_batch36400.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 36301 36400
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 36301 36400
...
graph batches ending at M. For M = **36400** the exclusive range is
**36301–36400** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 36301–36350  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 36351–36375  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 36376–36390  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 36391–36400  │ decade soft gates (Wave 222 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 36400 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_36400        1          graph_batch36391.c
  gj_libcgj_green_36400       1          graph_batch36392.c
  gj_bar3_ready_36400         0          graph_batch36393.c  (bar3 open)
  gj_product_score_36400      0          graph_batch36394.c  (score open)
  gj_continuum_ready_36400    1          graph_batch36395.c
  gj_smoke_soft_36400         1          graph_batch36396.c
  gj_dyn_soft_36400           1          graph_batch36397.c
  gj_milestone_tag_36400      36400      graph_batch36398.c
  gj_continuum_wave_36400     36400      graph_batch36399.c
  gj_batch_id_36400           36400      graph_batch36400.c
  gj_graph_milestone_36400    36400      graph_batch36400.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=36400`` means parent wired CGJ_SRCS through
    ``graph_batch36400.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_36400`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_36400`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M36400_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_36300`` / ``gj_*_36200`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 222 exclusive CREATE-ONLY lands the **markers** band
  (36391–36400) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_36400.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_36400.py

  # Restrict to marker band only (soft-gate TUs; Wave 222 exclusive):
  python3 user/libcgj/src/_gen_milestone_36400.py --band markers

  # Extension identity only (36301–36350):
  python3 user/libcgj/src/_gen_milestone_36400.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_36400.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_36300.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 36400 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 36400
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 36301
WAVE_HI = 36400  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 36301, 36350
IDENTITY_LO, IDENTITY_HI = 36351, 36375
FILL_LO, FILL_HI = 36376, 36390
MARKERS_LO, MARKERS_HI = 36391, 36400
...
# Stems become gj_<stem>_36400. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (36391, "shell_green", 1, "shell product green extract (wave 36400)"),
    (36392, "libcgj_green", 1, "libcgj product green extract (wave 36400)"),
    (36393, "bar3_ready", 0, "product bar3 readiness gate (wave 36400)"),
    (36394, "product_score", 0, "product readiness score tag (wave 36400)"),
    (36395, "continuum_ready", 1, "continuum readiness gate (wave 36400)"),
    (36396, "smoke_soft", 1, "smoke soft lamp (wave 36400)"),
    (36397, "dyn_soft", 1, "dyn soft lamp (wave 36400)"),
    (36398, "milestone_tag", MILESTONE, "milestone 36400 product tag"),
    (36399, "continuum_wave", MILESTONE, "milestone 36400 continuum wave identity"),
    # 36400 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        36376: "continuum wave fill soft unit + batch id",
        36380: "continuum bridge soft unit + batch id",
        36390: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (36391–36399), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 36393
    tag_batch = MARKERS_LO + 7  # 36398
    wave_batch = MARKERS_LO + 8  # 36399
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_36300 / gj_shell_green_36200 / gj_shell_green_36100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_36300 / gj_libcgj_green_36200 / gj_libcgj_green_36100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_36300 / gj_bar3_ready_36200 / gj_bar3_ready_36100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_36300 / gj_product_score_36200 / gj_product_score_36100\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_36300 / gj_continuum_ready_36200 /\n"
            f" * gj_continuum_ready_36100 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_36300 / gj_smoke_soft_36200 / gj_smoke_soft_36100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_36300 / gj_dyn_soft_36200 / gj_dyn_soft_36100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_36300 / gj_milestone_tag_36200 / gj_milestone_tag_36100,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_36300 / gj_continuum_wave_36200 /\n"
            f" * gj_continuum_wave_36100, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch36400.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 36399
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 36391–36399; 36400 is capstone
