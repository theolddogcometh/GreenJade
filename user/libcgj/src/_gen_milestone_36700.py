...
"""
Soft continuum generator — MILESTONE 36700 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 36601–36700 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=36700   when parent has wired through graph_batch36700.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 36601 36700
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 36601 36700
...
graph batches ending at M. For M = **36700** the exclusive range is
**36601–36700** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 36601–36650  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 36651–36675  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 36676–36690  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 36691–36700  │ decade soft gates (Wave 225 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 36700 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_36700        1          graph_batch36691.c
  gj_libcgj_green_36700       1          graph_batch36692.c
  gj_bar3_ready_36700         0          graph_batch36693.c  (bar3 open)
  gj_product_score_36700      0          graph_batch36694.c  (score open)
  gj_continuum_ready_36700    1          graph_batch36695.c
  gj_smoke_soft_36700         1          graph_batch36696.c
  gj_dyn_soft_36700           1          graph_batch36697.c
  gj_milestone_tag_36700      36700      graph_batch36698.c
  gj_continuum_wave_36700     36700      graph_batch36699.c
  gj_batch_id_36700           36700      graph_batch36700.c
  gj_graph_milestone_36700    36700      graph_batch36700.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=36700`` means parent wired CGJ_SRCS through
    ``graph_batch36700.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_36700`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_36700`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M36700_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_36600`` / ``gj_*_36500`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 225 exclusive CREATE-ONLY lands the **markers** band
  (36691–36700) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_36700.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_36700.py

  # Restrict to marker band only (soft-gate TUs; Wave 225 exclusive):
  python3 user/libcgj/src/_gen_milestone_36700.py --band markers

  # Extension identity only (36601–36650):
  python3 user/libcgj/src/_gen_milestone_36700.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_36700.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_36600.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 36700 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 36700
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 36601
WAVE_HI = 36700  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 36601, 36650
IDENTITY_LO, IDENTITY_HI = 36651, 36675
FILL_LO, FILL_HI = 36676, 36690
MARKERS_LO, MARKERS_HI = 36691, 36700
...
# Stems become gj_<stem>_36700. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (36691, "shell_green", 1, "shell product green extract (wave 36700)"),
    (36692, "libcgj_green", 1, "libcgj product green extract (wave 36700)"),
    (36693, "bar3_ready", 0, "product bar3 readiness gate (wave 36700)"),
    (36694, "product_score", 0, "product readiness score tag (wave 36700)"),
    (36695, "continuum_ready", 1, "continuum readiness gate (wave 36700)"),
    (36696, "smoke_soft", 1, "smoke soft lamp (wave 36700)"),
    (36697, "dyn_soft", 1, "dyn soft lamp (wave 36700)"),
    (36698, "milestone_tag", MILESTONE, "milestone 36700 product tag"),
    (36699, "continuum_wave", MILESTONE, "milestone 36700 continuum wave identity"),
    # 36700 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        36676: "continuum wave fill soft unit + batch id",
        36680: "continuum bridge soft unit + batch id",
        36690: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (36691–36699), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 36693
    tag_batch = MARKERS_LO + 7  # 36698
    wave_batch = MARKERS_LO + 8  # 36699
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_36600 / gj_shell_green_36500 / gj_shell_green_36400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_36600 / gj_libcgj_green_36500 / gj_libcgj_green_36400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_36600 / gj_bar3_ready_36500 / gj_bar3_ready_36400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_36600 / gj_product_score_36500 / gj_product_score_36400\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_36600 / gj_continuum_ready_36500 /\n"
            f" * gj_continuum_ready_36400 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_36600 / gj_smoke_soft_36500 / gj_smoke_soft_36400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_36600 / gj_dyn_soft_36500 / gj_dyn_soft_36400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_36600 / gj_milestone_tag_36500 / gj_milestone_tag_36400,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_36600 / gj_continuum_wave_36500 /\n"
            f" * gj_continuum_wave_36400, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch36700.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 36699
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 36691–36699; 36700 is capstone
