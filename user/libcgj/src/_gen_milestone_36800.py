...
"""
Soft continuum generator — MILESTONE 36800 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 36701–36800 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=36800   when parent has wired through graph_batch36800.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 36701 36800
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 36701 36800
...
graph batches ending at M. For M = **36800** the exclusive range is
**36701–36800** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 36701–36750  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 36751–36775  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 36776–36790  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 36791–36800  │ decade soft gates (Wave 226 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 36800 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_36800        1          graph_batch36791.c
  gj_libcgj_green_36800       1          graph_batch36792.c
  gj_bar3_ready_36800         0          graph_batch36793.c  (bar3 open)
  gj_product_score_36800      0          graph_batch36794.c  (score open)
  gj_continuum_ready_36800    1          graph_batch36795.c
  gj_smoke_soft_36800         1          graph_batch36796.c
  gj_dyn_soft_36800           1          graph_batch36797.c
  gj_milestone_tag_36800      36800      graph_batch36798.c
  gj_continuum_wave_36800     36800      graph_batch36799.c
  gj_batch_id_36800           36800      graph_batch36800.c
  gj_graph_milestone_36800    36800      graph_batch36800.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=36800`` means parent wired CGJ_SRCS through
    ``graph_batch36800.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_36800`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_36800`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M36800_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_36700`` / ``gj_*_36600`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 226 exclusive CREATE-ONLY lands the **markers** band
  (36791–36800) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_36800.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_36800.py

  # Restrict to marker band only (soft-gate TUs; Wave 226 exclusive):
  python3 user/libcgj/src/_gen_milestone_36800.py --band markers

  # Extension identity only (36701–36750):
  python3 user/libcgj/src/_gen_milestone_36800.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_36800.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_36700.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 36800 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 36800
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 36701
WAVE_HI = 36800  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 36701, 36750
IDENTITY_LO, IDENTITY_HI = 36751, 36775
FILL_LO, FILL_HI = 36776, 36790
MARKERS_LO, MARKERS_HI = 36791, 36800
...
# Stems become gj_<stem>_36800. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (36791, "shell_green", 1, "shell product green extract (wave 36800)"),
    (36792, "libcgj_green", 1, "libcgj product green extract (wave 36800)"),
    (36793, "bar3_ready", 0, "product bar3 readiness gate (wave 36800)"),
    (36794, "product_score", 0, "product readiness score tag (wave 36800)"),
    (36795, "continuum_ready", 1, "continuum readiness gate (wave 36800)"),
    (36796, "smoke_soft", 1, "smoke soft lamp (wave 36800)"),
    (36797, "dyn_soft", 1, "dyn soft lamp (wave 36800)"),
    (36798, "milestone_tag", MILESTONE, "milestone 36800 product tag"),
    (36799, "continuum_wave", MILESTONE, "milestone 36800 continuum wave identity"),
    # 36800 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        36776: "continuum wave fill soft unit + batch id",
        36780: "continuum bridge soft unit + batch id",
        36790: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (36791–36799), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 36793
    tag_batch = MARKERS_LO + 7  # 36798
    wave_batch = MARKERS_LO + 8  # 36799
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_36700 / gj_shell_green_36600 / gj_shell_green_36500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_36700 / gj_libcgj_green_36600 / gj_libcgj_green_36500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_36700 / gj_bar3_ready_36600 / gj_bar3_ready_36500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_36700 / gj_product_score_36600 / gj_product_score_36500\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_36700 / gj_continuum_ready_36600 /\n"
            f" * gj_continuum_ready_36500 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_36700 / gj_smoke_soft_36600 / gj_smoke_soft_36500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_36700 / gj_dyn_soft_36600 / gj_dyn_soft_36500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_36700 / gj_milestone_tag_36600 / gj_milestone_tag_36500,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_36700 / gj_continuum_wave_36600 /\n"
            f" * gj_continuum_wave_36500, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch36800.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 36799
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 36791–36799; 36800 is capstone
