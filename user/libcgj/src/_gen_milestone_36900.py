...
"""
Soft continuum generator — MILESTONE 36900 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 36801–36900 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=36900   when parent has wired through graph_batch36900.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 36801 36900
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 36801 36900
...
graph batches ending at M. For M = **36900** the exclusive range is
**36801–36900** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 36801–36850  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 36851–36875  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 36876–36890  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 36891–36900  │ decade soft gates (Wave 227 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 36900 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_36900        1          graph_batch36891.c
  gj_libcgj_green_36900       1          graph_batch36892.c
  gj_bar3_ready_36900         0          graph_batch36893.c  (bar3 open)
  gj_product_score_36900      0          graph_batch36894.c  (score open)
  gj_continuum_ready_36900    1          graph_batch36895.c
  gj_smoke_soft_36900         1          graph_batch36896.c
  gj_dyn_soft_36900           1          graph_batch36897.c
  gj_milestone_tag_36900      36900      graph_batch36898.c
  gj_continuum_wave_36900     36900      graph_batch36899.c
  gj_batch_id_36900           36900      graph_batch36900.c
  gj_graph_milestone_36900    36900      graph_batch36900.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=36900`` means parent wired CGJ_SRCS through
    ``graph_batch36900.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_36900`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_36900`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M36900_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_36800`` / ``gj_*_36700`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 227 exclusive CREATE-ONLY lands the **markers** band
  (36891–36900) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_36900.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_36900.py

  # Restrict to marker band only (soft-gate TUs; Wave 227 exclusive):
  python3 user/libcgj/src/_gen_milestone_36900.py --band markers

  # Extension identity only (36801–36850):
  python3 user/libcgj/src/_gen_milestone_36900.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_36900.py --force
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 36900 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 36900
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 36801
WAVE_HI = 36900  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 36801, 36850
IDENTITY_LO, IDENTITY_HI = 36851, 36875
FILL_LO, FILL_HI = 36876, 36890
MARKERS_LO, MARKERS_HI = 36891, 36900
...
# Stems become gj_<stem>_36900. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (36891, "shell_green", 1, "shell product green extract (wave 36900)"),
    (36892, "libcgj_green", 1, "libcgj product green extract (wave 36900)"),
    (36893, "bar3_ready", 0, "product bar3 readiness gate (wave 36900)"),
    (36894, "product_score", 0, "product readiness score tag (wave 36900)"),
    (36895, "continuum_ready", 1, "continuum readiness gate (wave 36900)"),
    (36896, "smoke_soft", 1, "smoke soft lamp (wave 36900)"),
    (36897, "dyn_soft", 1, "dyn soft lamp (wave 36900)"),
    (36898, "milestone_tag", MILESTONE, "milestone 36900 product tag"),
    (36899, "continuum_wave", MILESTONE, "milestone 36900 continuum wave identity"),
    # 36900 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        36876: "continuum wave fill soft unit + batch id",
        36880: "continuum bridge soft unit + batch id",
        36890: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (36891–36899), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 36893
    tag_batch = MARKERS_LO + 7  # 36898
    wave_batch = MARKERS_LO + 8  # 36899
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_36800 / gj_shell_green_36700 / gj_shell_green_36600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_36800 / gj_libcgj_green_36700 / gj_libcgj_green_36600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_36800 / gj_bar3_ready_36700 / gj_bar3_ready_36600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_36800 / gj_product_score_36700 / gj_product_score_36600\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_36800 / gj_continuum_ready_36700 /\n"
            f" * gj_continuum_ready_36600 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_36800 / gj_smoke_soft_36700 / gj_smoke_soft_36600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_36800 / gj_dyn_soft_36700 / gj_dyn_soft_36600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_36800 / gj_milestone_tag_36700 / gj_milestone_tag_36600,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_36800 / gj_continuum_wave_36700 /\n"
            f" * gj_continuum_wave_36600, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch36900.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 36899
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 36891–36899; 36900 is capstone
