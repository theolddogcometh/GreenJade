...
"""
Soft continuum generator — MILESTONE 37000 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 36901–37000 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=37000   when parent has wired through graph_batch37000.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 36901 37000
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 36901 37000
...
graph batches ending at M. For M = **37000** the exclusive range is
**36901–37000** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 36901–36950  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 36951–36975  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 36976–36990  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 36991–37000  │ decade soft gates (Wave 228 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 37000 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_37000        1          graph_batch36991.c
  gj_libcgj_green_37000       1          graph_batch36992.c
  gj_bar3_ready_37000         0          graph_batch36993.c  (bar3 open)
  gj_product_score_37000      0          graph_batch36994.c  (score open)
  gj_continuum_ready_37000    1          graph_batch36995.c
  gj_smoke_soft_37000         1          graph_batch36996.c
  gj_dyn_soft_37000           1          graph_batch36997.c
  gj_milestone_tag_37000      37000      graph_batch36998.c
  gj_continuum_wave_37000     37000      graph_batch36999.c
  gj_batch_id_37000           37000      graph_batch37000.c
  gj_graph_milestone_37000    37000      graph_batch37000.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=37000`` means parent wired CGJ_SRCS through
    ``graph_batch37000.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_37000`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_37000`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M37000_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_36900`` / ``gj_*_36800`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 228 exclusive CREATE-ONLY lands the **markers** band
  (36991–37000) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_37000.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_37000.py

  # Restrict to marker band only (soft-gate TUs; Wave 228 exclusive):
  python3 user/libcgj/src/_gen_milestone_37000.py --band markers

  # Extension identity only (36901–36950):
  python3 user/libcgj/src/_gen_milestone_37000.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_37000.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_36800.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 37000 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 37000
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 36901
WAVE_HI = 37000  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 36901, 36950
IDENTITY_LO, IDENTITY_HI = 36951, 36975
FILL_LO, FILL_HI = 36976, 36990
MARKERS_LO, MARKERS_HI = 36991, 37000
...
# Stems become gj_<stem>_37000. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (36991, "shell_green", 1, "shell product green extract (wave 37000)"),
    (36992, "libcgj_green", 1, "libcgj product green extract (wave 37000)"),
    (36993, "bar3_ready", 0, "product bar3 readiness gate (wave 37000)"),
    (36994, "product_score", 0, "product readiness score tag (wave 37000)"),
    (36995, "continuum_ready", 1, "continuum readiness gate (wave 37000)"),
    (36996, "smoke_soft", 1, "smoke soft lamp (wave 37000)"),
    (36997, "dyn_soft", 1, "dyn soft lamp (wave 37000)"),
    (36998, "milestone_tag", MILESTONE, "milestone 37000 product tag"),
    (36999, "continuum_wave", MILESTONE, "milestone 37000 continuum wave identity"),
    # 37000 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        36976: "continuum wave fill soft unit + batch id",
        36980: "continuum bridge soft unit + batch id",
        36990: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (36991–36999), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 36993
    tag_batch = MARKERS_LO + 7  # 36998
    wave_batch = MARKERS_LO + 8  # 36999
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_36900 / gj_shell_green_36800 / gj_shell_green_36700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_36900 / gj_libcgj_green_36800 / gj_libcgj_green_36700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_36900 / gj_bar3_ready_36800 / gj_bar3_ready_36700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_36900 / gj_product_score_36800 / gj_product_score_36700\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_36900 / gj_continuum_ready_36800 /\n"
            f" * gj_continuum_ready_36700 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_36900 / gj_smoke_soft_36800 / gj_smoke_soft_36700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_36900 / gj_dyn_soft_36800 / gj_dyn_soft_36700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_36900 / gj_milestone_tag_36800 / gj_milestone_tag_36700,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_36900 / gj_continuum_wave_36800 /\n"
            f" * gj_continuum_wave_36700, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch37000.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 36999
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 36991–36999; 37000 is capstone
