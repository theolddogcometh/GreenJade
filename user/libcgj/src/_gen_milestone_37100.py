...
"""
Soft continuum generator — MILESTONE 37100 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 37001–37100 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=37100   when parent has wired through graph_batch37100.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 37001 37100
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 37001 37100
...
graph batches ending at M. For M = **37100** the exclusive range is
**37001–37100** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 37001–37050  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 37051–37075  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 37076–37090  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 37091–37100  │ decade soft gates (Wave 229 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 37100 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_37100        1          graph_batch37091.c
  gj_libcgj_green_37100       1          graph_batch37092.c
  gj_bar3_ready_37100         0          graph_batch37093.c  (bar3 open)
  gj_product_score_37100      0          graph_batch37094.c  (score open)
  gj_continuum_ready_37100    1          graph_batch37095.c
  gj_smoke_soft_37100         1          graph_batch37096.c
  gj_dyn_soft_37100           1          graph_batch37097.c
  gj_milestone_tag_37100      37100      graph_batch37098.c
  gj_continuum_wave_37100     37100      graph_batch37099.c
  gj_batch_id_37100           37100      graph_batch37100.c
  gj_graph_milestone_37100    37100      graph_batch37100.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=37100`` means parent wired CGJ_SRCS through
    ``graph_batch37100.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_37100`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_37100`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M37100_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_37000`` / ``gj_*_36900`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 229 exclusive CREATE-ONLY lands the **markers** band
  (37091–37100) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_37100.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_37100.py

  # Restrict to marker band only (soft-gate TUs; Wave 229 exclusive):
  python3 user/libcgj/src/_gen_milestone_37100.py --band markers

  # Extension identity only (37001–37050):
  python3 user/libcgj/src/_gen_milestone_37100.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_37100.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_37000.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 37100 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 37100
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 37001
WAVE_HI = 37100  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 37001, 37050
IDENTITY_LO, IDENTITY_HI = 37051, 37075
FILL_LO, FILL_HI = 37076, 37090
MARKERS_LO, MARKERS_HI = 37091, 37100
...
# Stems become gj_<stem>_37100. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (37091, "shell_green", 1, "shell product green extract (wave 37100)"),
    (37092, "libcgj_green", 1, "libcgj product green extract (wave 37100)"),
    (37093, "bar3_ready", 0, "product bar3 readiness gate (wave 37100)"),
    (37094, "product_score", 0, "product readiness score tag (wave 37100)"),
    (37095, "continuum_ready", 1, "continuum readiness gate (wave 37100)"),
    (37096, "smoke_soft", 1, "smoke soft lamp (wave 37100)"),
    (37097, "dyn_soft", 1, "dyn soft lamp (wave 37100)"),
    (37098, "milestone_tag", MILESTONE, "milestone 37100 product tag"),
    (37099, "continuum_wave", MILESTONE, "milestone 37100 continuum wave identity"),
    # 37100 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        37076: "continuum wave fill soft unit + batch id",
        37080: "continuum bridge soft unit + batch id",
        37090: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (37091–37099), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 37093
    tag_batch = MARKERS_LO + 7  # 37098
    wave_batch = MARKERS_LO + 8  # 37099
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_37000 / gj_shell_green_36900 / gj_shell_green_36800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_37000 / gj_libcgj_green_36900 / gj_libcgj_green_36800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_37000 / gj_bar3_ready_36900 / gj_bar3_ready_36800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_37000 / gj_product_score_36900 / gj_product_score_36800\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_37000 / gj_continuum_ready_36900 /\n"
            f" * gj_continuum_ready_36800 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_37000 / gj_smoke_soft_36900 / gj_smoke_soft_36800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_37000 / gj_dyn_soft_36900 / gj_dyn_soft_36800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_37000 / gj_milestone_tag_36900 / gj_milestone_tag_36800,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_37000 / gj_continuum_wave_36900 /\n"
            f" * gj_continuum_wave_36800, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch37100.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 37099
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 37091–37099; 37100 is capstone
