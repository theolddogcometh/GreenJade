...
"""
Soft continuum generator — MILESTONE 37200 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 37101–37200 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=37200   when parent has wired through graph_batch37200.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 37101 37200
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 37101 37200
...
graph batches ending at M. For M = **37200** the exclusive range is
**37101–37200** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 37101–37150  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 37151–37175  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 37176–37190  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 37191–37200  │ decade soft gates (Wave 230 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 37200 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_37200        1          graph_batch37191.c
  gj_libcgj_green_37200       1          graph_batch37192.c
  gj_bar3_ready_37200         0          graph_batch37193.c  (bar3 open)
  gj_product_score_37200      0          graph_batch37194.c  (score open)
  gj_continuum_ready_37200    1          graph_batch37195.c
  gj_smoke_soft_37200         1          graph_batch37196.c
  gj_dyn_soft_37200           1          graph_batch37197.c
  gj_milestone_tag_37200      37200      graph_batch37198.c
  gj_continuum_wave_37200     37200      graph_batch37199.c
  gj_batch_id_37200           37200      graph_batch37200.c
  gj_graph_milestone_37200    37200      graph_batch37200.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=37200`` means parent wired CGJ_SRCS through
    ``graph_batch37200.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_37200`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_37200`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M37200_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_37100`` / ``gj_*_37000`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 230 exclusive CREATE-ONLY lands the **markers** band
  (37191–37200) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_37200.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_37200.py

  # Restrict to marker band only (soft-gate TUs; Wave 230 exclusive):
  python3 user/libcgj/src/_gen_milestone_37200.py --band markers

  # Extension identity only (37101–37150):
  python3 user/libcgj/src/_gen_milestone_37200.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_37200.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_37100.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 37200 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 37200
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 37101
WAVE_HI = 37200  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 37101, 37150
IDENTITY_LO, IDENTITY_HI = 37151, 37175
FILL_LO, FILL_HI = 37176, 37190
MARKERS_LO, MARKERS_HI = 37191, 37200
...
# Stems become gj_<stem>_37200. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (37191, "shell_green", 1, "shell product green extract (wave 37200)"),
    (37192, "libcgj_green", 1, "libcgj product green extract (wave 37200)"),
    (37193, "bar3_ready", 0, "product bar3 readiness gate (wave 37200)"),
    (37194, "product_score", 0, "product readiness score tag (wave 37200)"),
    (37195, "continuum_ready", 1, "continuum readiness gate (wave 37200)"),
    (37196, "smoke_soft", 1, "smoke soft lamp (wave 37200)"),
    (37197, "dyn_soft", 1, "dyn soft lamp (wave 37200)"),
    (37198, "milestone_tag", MILESTONE, "milestone 37200 product tag"),
    (37199, "continuum_wave", MILESTONE, "milestone 37200 continuum wave identity"),
    # 37200 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        37176: "continuum wave fill soft unit + batch id",
        37180: "continuum bridge soft unit + batch id",
        37190: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (37191–37199), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 37193
    tag_batch = MARKERS_LO + 7  # 37198
    wave_batch = MARKERS_LO + 8  # 37199
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_37100 / gj_shell_green_37000 / gj_shell_green_36900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_37100 / gj_libcgj_green_37000 / gj_libcgj_green_36900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_37100 / gj_bar3_ready_37000 / gj_bar3_ready_36900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_37100 / gj_product_score_37000 / gj_product_score_36900\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_37100 / gj_continuum_ready_37000 /\n"
            f" * gj_continuum_ready_36900 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_37100 / gj_smoke_soft_37000 / gj_smoke_soft_36900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_37100 / gj_dyn_soft_37000 / gj_dyn_soft_36900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_37100 / gj_milestone_tag_37000 / gj_milestone_tag_36900,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_37100 / gj_continuum_wave_37000 /\n"
            f" * gj_continuum_wave_36900, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch37200.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 37199
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 37191–37199; 37200 is capstone
