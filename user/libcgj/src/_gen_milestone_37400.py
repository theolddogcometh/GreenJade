...
"""
Soft continuum generator — MILESTONE 37400 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 37301–37400 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=37400   when parent has wired through graph_batch37400.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 37301 37400
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 37301 37400
...
graph batches ending at M. For M = **37400** the exclusive range is
**37301–37400** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 37301–37350  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 37351–37375  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 37376–37390  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 37391–37400  │ decade soft gates (Wave 232 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 37400 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_37400        1          graph_batch37391.c
  gj_libcgj_green_37400       1          graph_batch37392.c
  gj_bar3_ready_37400         0          graph_batch37393.c  (bar3 open)
  gj_product_score_37400      0          graph_batch37394.c  (score open)
  gj_continuum_ready_37400    1          graph_batch37395.c
  gj_smoke_soft_37400         1          graph_batch37396.c
  gj_dyn_soft_37400           1          graph_batch37397.c
  gj_milestone_tag_37400      37400      graph_batch37398.c
  gj_continuum_wave_37400     37400      graph_batch37399.c
  gj_batch_id_37400           37400      graph_batch37400.c
  gj_graph_milestone_37400    37400      graph_batch37400.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=37400`` means parent wired CGJ_SRCS through
    ``graph_batch37400.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_37400`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_37400`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M37400_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_37300`` / ``gj_*_37200`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 232 exclusive CREATE-ONLY lands the **markers** band
  (37391–37400) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_37400.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_37400.py

  # Restrict to marker band only (soft-gate TUs; Wave 232 exclusive):
  python3 user/libcgj/src/_gen_milestone_37400.py --band markers

  # Extension identity only (37301–37350):
  python3 user/libcgj/src/_gen_milestone_37400.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_37400.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_37300.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 37400 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 37400
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 37301
WAVE_HI = 37400  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 37301, 37350
IDENTITY_LO, IDENTITY_HI = 37351, 37375
FILL_LO, FILL_HI = 37376, 37390
MARKERS_LO, MARKERS_HI = 37391, 37400
...
# Stems become gj_<stem>_37400. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (37391, "shell_green", 1, "shell product green extract (wave 37400)"),
    (37392, "libcgj_green", 1, "libcgj product green extract (wave 37400)"),
    (37393, "bar3_ready", 0, "product bar3 readiness gate (wave 37400)"),
    (37394, "product_score", 0, "product readiness score tag (wave 37400)"),
    (37395, "continuum_ready", 1, "continuum readiness gate (wave 37400)"),
    (37396, "smoke_soft", 1, "smoke soft lamp (wave 37400)"),
    (37397, "dyn_soft", 1, "dyn soft lamp (wave 37400)"),
    (37398, "milestone_tag", MILESTONE, "milestone 37400 product tag"),
    (37399, "continuum_wave", MILESTONE, "milestone 37400 continuum wave identity"),
    # 37400 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        37376: "continuum wave fill soft unit + batch id",
        37380: "continuum bridge soft unit + batch id",
        37390: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (37391–37399), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 37393
    tag_batch = MARKERS_LO + 7  # 37398
    wave_batch = MARKERS_LO + 8  # 37399
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_37300 / gj_shell_green_37200 / gj_shell_green_37100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_37300 / gj_libcgj_green_37200 / gj_libcgj_green_37100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_37300 / gj_bar3_ready_37200 / gj_bar3_ready_37100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_37300 / gj_product_score_37200 / gj_product_score_37100\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_37300 / gj_continuum_ready_37200 /\n"
            f" * gj_continuum_ready_37100 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_37300 / gj_smoke_soft_37200 / gj_smoke_soft_37100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_37300 / gj_dyn_soft_37200 / gj_dyn_soft_37100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_37300 / gj_milestone_tag_37200 / gj_milestone_tag_37100,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_37300 / gj_continuum_wave_37200 /\n"
            f" * gj_continuum_wave_37100, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch37400.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 37399
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 37391–37399; 37400 is capstone
