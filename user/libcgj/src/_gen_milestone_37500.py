...
"""
Soft continuum generator — MILESTONE 37500 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 37401–37500 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=37500   when parent has wired through graph_batch37500.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 37401 37500
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 37401 37500
...
graph batches ending at M. For M = **37500** the exclusive range is
**37401–37500** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 37401–37450  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 37451–37475  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 37476–37490  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 37491–37500  │ decade soft gates (Wave 233 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 37500 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_37500        1          graph_batch37491.c
  gj_libcgj_green_37500       1          graph_batch37492.c
  gj_bar3_ready_37500         0          graph_batch37493.c  (bar3 open)
  gj_product_score_37500      0          graph_batch37494.c  (score open)
  gj_continuum_ready_37500    1          graph_batch37495.c
  gj_smoke_soft_37500         1          graph_batch37496.c
  gj_dyn_soft_37500           1          graph_batch37497.c
  gj_milestone_tag_37500      37500      graph_batch37498.c
  gj_continuum_wave_37500     37500      graph_batch37499.c
  gj_batch_id_37500           37500      graph_batch37500.c
  gj_graph_milestone_37500    37500      graph_batch37500.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=37500`` means parent wired CGJ_SRCS through
    ``graph_batch37500.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_37500`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_37500`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M37500_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_37400`` / ``gj_*_37300`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 233 exclusive CREATE-ONLY lands the **markers** band
  (37491–37500) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_37500.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_37500.py

  # Restrict to marker band only (soft-gate TUs; Wave 233 exclusive):
  python3 user/libcgj/src/_gen_milestone_37500.py --band markers

  # Extension identity only (37401–37450):
  python3 user/libcgj/src/_gen_milestone_37500.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_37500.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_37400.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 37500 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 37500
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 37401
WAVE_HI = 37500  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 37401, 37450
IDENTITY_LO, IDENTITY_HI = 37451, 37475
FILL_LO, FILL_HI = 37476, 37490
MARKERS_LO, MARKERS_HI = 37491, 37500
...
# Stems become gj_<stem>_37500. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (37491, "shell_green", 1, "shell product green extract (wave 37500)"),
    (37492, "libcgj_green", 1, "libcgj product green extract (wave 37500)"),
    (37493, "bar3_ready", 0, "product bar3 readiness gate (wave 37500)"),
    (37494, "product_score", 0, "product readiness score tag (wave 37500)"),
    (37495, "continuum_ready", 1, "continuum readiness gate (wave 37500)"),
    (37496, "smoke_soft", 1, "smoke soft lamp (wave 37500)"),
    (37497, "dyn_soft", 1, "dyn soft lamp (wave 37500)"),
    (37498, "milestone_tag", MILESTONE, "milestone 37500 product tag"),
    (37499, "continuum_wave", MILESTONE, "milestone 37500 continuum wave identity"),
    # 37500 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        37476: "continuum wave fill soft unit + batch id",
        37480: "continuum bridge soft unit + batch id",
        37490: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (37491–37499), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 37493
    tag_batch = MARKERS_LO + 7  # 37498
    wave_batch = MARKERS_LO + 8  # 37499
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_37400 / gj_shell_green_37300 / gj_shell_green_37200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_37400 / gj_libcgj_green_37300 / gj_libcgj_green_37200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_37400 / gj_bar3_ready_37300 / gj_bar3_ready_37200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_37400 / gj_product_score_37300 / gj_product_score_37200\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_37400 / gj_continuum_ready_37300 /\n"
            f" * gj_continuum_ready_37200 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_37400 / gj_smoke_soft_37300 / gj_smoke_soft_37200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_37400 / gj_dyn_soft_37300 / gj_dyn_soft_37200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_37400 / gj_milestone_tag_37300 / gj_milestone_tag_37200,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_37400 / gj_continuum_wave_37300 /\n"
            f" * gj_continuum_wave_37200, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch37500.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 37499
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 37491–37499; 37500 is capstone
