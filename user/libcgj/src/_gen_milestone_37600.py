...
"""
Soft continuum generator — MILESTONE 37600 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 37501–37600 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=37600   when parent has wired through graph_batch37600.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 37501 37600
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 37501 37600
...
graph batches ending at M. For M = **37600** the exclusive range is
**37501–37600** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 37501–37550  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 37551–37575  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 37576–37590  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 37591–37600  │ decade soft gates (Wave 234 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 37600 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_37600        1          graph_batch37591.c
  gj_libcgj_green_37600       1          graph_batch37592.c
  gj_bar3_ready_37600         0          graph_batch37593.c  (bar3 open)
  gj_product_score_37600      0          graph_batch37594.c  (score open)
  gj_continuum_ready_37600    1          graph_batch37595.c
  gj_smoke_soft_37600         1          graph_batch37596.c
  gj_dyn_soft_37600           1          graph_batch37597.c
  gj_milestone_tag_37600      37600      graph_batch37598.c
  gj_continuum_wave_37600     37600      graph_batch37599.c
  gj_batch_id_37600           37600      graph_batch37600.c
  gj_graph_milestone_37600    37600      graph_batch37600.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=37600`` means parent wired CGJ_SRCS through
    ``graph_batch37600.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_37600`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_37600`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M37600_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_37500`` / ``gj_*_37400`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 234 exclusive CREATE-ONLY lands the **markers** band
  (37591–37600) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_37600.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_37600.py

  # Restrict to marker band only (soft-gate TUs; Wave 234 exclusive):
  python3 user/libcgj/src/_gen_milestone_37600.py --band markers

  # Extension identity only (37501–37550):
  python3 user/libcgj/src/_gen_milestone_37600.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_37600.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_37500.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 37600 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 37600
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 37501
WAVE_HI = 37600  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 37501, 37550
IDENTITY_LO, IDENTITY_HI = 37551, 37575
FILL_LO, FILL_HI = 37576, 37590
MARKERS_LO, MARKERS_HI = 37591, 37600
...
# Stems become gj_<stem>_37600. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (37591, "shell_green", 1, "shell product green extract (wave 37600)"),
    (37592, "libcgj_green", 1, "libcgj product green extract (wave 37600)"),
    (37593, "bar3_ready", 0, "product bar3 readiness gate (wave 37600)"),
    (37594, "product_score", 0, "product readiness score tag (wave 37600)"),
    (37595, "continuum_ready", 1, "continuum readiness gate (wave 37600)"),
    (37596, "smoke_soft", 1, "smoke soft lamp (wave 37600)"),
    (37597, "dyn_soft", 1, "dyn soft lamp (wave 37600)"),
    (37598, "milestone_tag", MILESTONE, "milestone 37600 product tag"),
    (37599, "continuum_wave", MILESTONE, "milestone 37600 continuum wave identity"),
    # 37600 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        37576: "continuum wave fill soft unit + batch id",
        37580: "continuum bridge soft unit + batch id",
        37590: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (37591–37599), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 37593
    tag_batch = MARKERS_LO + 7  # 37598
    wave_batch = MARKERS_LO + 8  # 37599
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_37500 / gj_shell_green_37400 / gj_shell_green_37300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_37500 / gj_libcgj_green_37400 / gj_libcgj_green_37300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_37500 / gj_bar3_ready_37400 / gj_bar3_ready_37300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_37500 / gj_product_score_37400 / gj_product_score_37300\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_37500 / gj_continuum_ready_37400 /\n"
            f" * gj_continuum_ready_37300 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_37500 / gj_smoke_soft_37400 / gj_smoke_soft_37300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_37500 / gj_dyn_soft_37400 / gj_dyn_soft_37300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_37500 / gj_milestone_tag_37400 / gj_milestone_tag_37300,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_37500 / gj_continuum_wave_37400 /\n"
            f" * gj_continuum_wave_37300, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch37600.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 37599
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 37591–37599; 37600 is capstone
