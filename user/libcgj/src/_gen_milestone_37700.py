...
"""
Soft continuum generator — MILESTONE 37700 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 37601–37700 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=37700   when parent has wired through graph_batch37700.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 37601 37700
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 37601 37700
...
graph batches ending at M. For M = **37700** the exclusive range is
**37601–37700** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 37601–37650  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 37651–37675  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 37676–37690  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 37691–37700  │ decade soft gates (Wave 235 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 37700 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_37700        1          graph_batch37691.c
  gj_libcgj_green_37700       1          graph_batch37692.c
  gj_bar3_ready_37700         0          graph_batch37693.c  (bar3 open)
  gj_product_score_37700      0          graph_batch37694.c  (score open)
  gj_continuum_ready_37700    1          graph_batch37695.c
  gj_smoke_soft_37700         1          graph_batch37696.c
  gj_dyn_soft_37700           1          graph_batch37697.c
  gj_milestone_tag_37700      37700      graph_batch37698.c
  gj_continuum_wave_37700     37700      graph_batch37699.c
  gj_batch_id_37700           37700      graph_batch37700.c
  gj_graph_milestone_37700    37700      graph_batch37700.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=37700`` means parent wired CGJ_SRCS through
    ``graph_batch37700.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_37700`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_37700`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M37700_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_37600`` / ``gj_*_37500`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 235 exclusive CREATE-ONLY lands the **markers** band
  (37691–37700) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_37700.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_37700.py

  # Restrict to marker band only (soft-gate TUs; Wave 235 exclusive):
  python3 user/libcgj/src/_gen_milestone_37700.py --band markers

  # Extension identity only (37601–37650):
  python3 user/libcgj/src/_gen_milestone_37700.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_37700.py --force
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 37700 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 37700
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 37601
WAVE_HI = 37700  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 37601, 37650
IDENTITY_LO, IDENTITY_HI = 37651, 37675
FILL_LO, FILL_HI = 37676, 37690
MARKERS_LO, MARKERS_HI = 37691, 37700
...
# Stems become gj_<stem>_37700. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (37691, "shell_green", 1, "shell product green extract (wave 37700)"),
    (37692, "libcgj_green", 1, "libcgj product green extract (wave 37700)"),
    (37693, "bar3_ready", 0, "product bar3 readiness gate (wave 37700)"),
    (37694, "product_score", 0, "product readiness score tag (wave 37700)"),
    (37695, "continuum_ready", 1, "continuum readiness gate (wave 37700)"),
    (37696, "smoke_soft", 1, "smoke soft lamp (wave 37700)"),
    (37697, "dyn_soft", 1, "dyn soft lamp (wave 37700)"),
    (37698, "milestone_tag", MILESTONE, "milestone 37700 product tag"),
    (37699, "continuum_wave", MILESTONE, "milestone 37700 continuum wave identity"),
    # 37700 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        37676: "continuum wave fill soft unit + batch id",
        37680: "continuum bridge soft unit + batch id",
        37690: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (37691–37699), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 37693
    tag_batch = MARKERS_LO + 7  # 37698
    wave_batch = MARKERS_LO + 8  # 37699
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_37600 / gj_shell_green_37500 / gj_shell_green_37400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_37600 / gj_libcgj_green_37500 / gj_libcgj_green_37400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_37600 / gj_bar3_ready_37500 / gj_bar3_ready_37400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_37600 / gj_product_score_37500 / gj_product_score_37400\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_37600 / gj_continuum_ready_37500 /\n"
            f" * gj_continuum_ready_37400 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_37600 / gj_smoke_soft_37500 / gj_smoke_soft_37400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_37600 / gj_dyn_soft_37500 / gj_dyn_soft_37400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_37600 / gj_milestone_tag_37500 / gj_milestone_tag_37400,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_37600 / gj_continuum_wave_37500 /\n"
            f" * gj_continuum_wave_37400, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch37700.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 37699
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 37691–37699; 37700 is capstone
