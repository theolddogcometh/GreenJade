...
"""
Soft continuum generator — MILESTONE 37800 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 37701–37800 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=37800   when parent has wired through graph_batch37800.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 37701 37800
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 37701 37800
...
graph batches ending at M. For M = **37800** the exclusive range is
**37701–37800** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 37701–37750  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 37751–37775  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 37776–37790  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 37791–37800  │ decade soft gates (Wave 236 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 37800 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_37800        1          graph_batch37791.c
  gj_libcgj_green_37800       1          graph_batch37792.c
  gj_bar3_ready_37800         0          graph_batch37793.c  (bar3 open)
  gj_product_score_37800      0          graph_batch37794.c  (score open)
  gj_continuum_ready_37800    1          graph_batch37795.c
  gj_smoke_soft_37800         1          graph_batch37796.c
  gj_dyn_soft_37800           1          graph_batch37797.c
  gj_milestone_tag_37800      37800      graph_batch37798.c
  gj_continuum_wave_37800     37800      graph_batch37799.c
  gj_batch_id_37800           37800      graph_batch37800.c
  gj_graph_milestone_37800    37800      graph_batch37800.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=37800`` means parent wired CGJ_SRCS through
    ``graph_batch37800.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_37800`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_37800`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M37800_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_37700`` / ``gj_*_37600`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 236 exclusive CREATE-ONLY lands the **markers** band
  (37791–37800) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_37800.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_37800.py

  # Restrict to marker band only (soft-gate TUs; Wave 236 exclusive):
  python3 user/libcgj/src/_gen_milestone_37800.py --band markers

  # Extension identity only (37701–37750):
  python3 user/libcgj/src/_gen_milestone_37800.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_37800.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_37700.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 37800 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 37800
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 37701
WAVE_HI = 37800  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 37701, 37750
IDENTITY_LO, IDENTITY_HI = 37751, 37775
FILL_LO, FILL_HI = 37776, 37790
MARKERS_LO, MARKERS_HI = 37791, 37800
...
# Stems become gj_<stem>_37800. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (37791, "shell_green", 1, "shell product green extract (wave 37800)"),
    (37792, "libcgj_green", 1, "libcgj product green extract (wave 37800)"),
    (37793, "bar3_ready", 0, "product bar3 readiness gate (wave 37800)"),
    (37794, "product_score", 0, "product readiness score tag (wave 37800)"),
    (37795, "continuum_ready", 1, "continuum readiness gate (wave 37800)"),
    (37796, "smoke_soft", 1, "smoke soft lamp (wave 37800)"),
    (37797, "dyn_soft", 1, "dyn soft lamp (wave 37800)"),
    (37798, "milestone_tag", MILESTONE, "milestone 37800 product tag"),
    (37799, "continuum_wave", MILESTONE, "milestone 37800 continuum wave identity"),
    # 37800 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        37776: "continuum wave fill soft unit + batch id",
        37780: "continuum bridge soft unit + batch id",
        37790: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (37791–37799), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 37793
    tag_batch = MARKERS_LO + 7  # 37798
    wave_batch = MARKERS_LO + 8  # 37799
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_37700 / gj_shell_green_37600 / gj_shell_green_37500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_37700 / gj_libcgj_green_37600 / gj_libcgj_green_37500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_37700 / gj_bar3_ready_37600 / gj_bar3_ready_37500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_37700 / gj_product_score_37600 / gj_product_score_37500\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_37700 / gj_continuum_ready_37600 /\n"
            f" * gj_continuum_ready_37500 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_37700 / gj_smoke_soft_37600 / gj_smoke_soft_37500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_37700 / gj_dyn_soft_37600 / gj_dyn_soft_37500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_37700 / gj_milestone_tag_37600 / gj_milestone_tag_37500,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_37700 / gj_continuum_wave_37600 /\n"
            f" * gj_continuum_wave_37500, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch37800.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 37799
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 37791–37799; 37800 is capstone
