...
"""
Soft continuum generator — MILESTONE 37900 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 37801–37900 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=37900   when parent has wired through graph_batch37900.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 37801 37900
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 37801 37900
...
graph batches ending at M. For M = **37900** the exclusive range is
**37801–37900** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 37801–37850  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 37851–37875  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 37876–37890  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 37891–37900  │ decade soft gates (Wave 237 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 37900 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_37900        1          graph_batch37891.c
  gj_libcgj_green_37900       1          graph_batch37892.c
  gj_bar3_ready_37900         0          graph_batch37893.c  (bar3 open)
  gj_product_score_37900      0          graph_batch37894.c  (score open)
  gj_continuum_ready_37900    1          graph_batch37895.c
  gj_smoke_soft_37900         1          graph_batch37896.c
  gj_dyn_soft_37900           1          graph_batch37897.c
  gj_milestone_tag_37900      37900      graph_batch37898.c
  gj_continuum_wave_37900     37900      graph_batch37899.c
  gj_batch_id_37900           37900      graph_batch37900.c
  gj_graph_milestone_37900    37900      graph_batch37900.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=37900`` means parent wired CGJ_SRCS through
    ``graph_batch37900.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_37900`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_37900`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M37900_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_37800`` / ``gj_*_37700`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 237 exclusive CREATE-ONLY lands the **markers** band
  (37891–37900) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_37900.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_37900.py

  # Restrict to marker band only (soft-gate TUs; Wave 237 exclusive):
  python3 user/libcgj/src/_gen_milestone_37900.py --band markers

  # Extension identity only (37801–37850):
  python3 user/libcgj/src/_gen_milestone_37900.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_37900.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_37600.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 37900 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 37900
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 37801
WAVE_HI = 37900  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 37801, 37850
IDENTITY_LO, IDENTITY_HI = 37851, 37875
FILL_LO, FILL_HI = 37876, 37890
MARKERS_LO, MARKERS_HI = 37891, 37900
...
# Stems become gj_<stem>_37900. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (37891, "shell_green", 1, "shell product green extract (wave 37900)"),
    (37892, "libcgj_green", 1, "libcgj product green extract (wave 37900)"),
    (37893, "bar3_ready", 0, "product bar3 readiness gate (wave 37900)"),
    (37894, "product_score", 0, "product readiness score tag (wave 37900)"),
    (37895, "continuum_ready", 1, "continuum readiness gate (wave 37900)"),
    (37896, "smoke_soft", 1, "smoke soft lamp (wave 37900)"),
    (37897, "dyn_soft", 1, "dyn soft lamp (wave 37900)"),
    (37898, "milestone_tag", MILESTONE, "milestone 37900 product tag"),
    (37899, "continuum_wave", MILESTONE, "milestone 37900 continuum wave identity"),
    # 37900 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        37876: "continuum wave fill soft unit + batch id",
        37880: "continuum bridge soft unit + batch id",
        37890: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (37891–37899), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 37893
    tag_batch = MARKERS_LO + 7  # 37898
    wave_batch = MARKERS_LO + 8  # 37899
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_37800 / gj_shell_green_37700 / gj_shell_green_37900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_37800 / gj_libcgj_green_37700 / gj_libcgj_green_37900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_37800 / gj_bar3_ready_37700 / gj_bar3_ready_37900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_37800 / gj_product_score_37700 / gj_product_score_37900\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_37800 / gj_continuum_ready_37700 /\n"
            f" * gj_continuum_ready_37600 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_37800 / gj_smoke_soft_37700 / gj_smoke_soft_37900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_37800 / gj_dyn_soft_37700 / gj_dyn_soft_37900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_37800 / gj_milestone_tag_37700 / gj_milestone_tag_37900,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_37800 / gj_continuum_wave_37700 /\n"
            f" * gj_continuum_wave_37600, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch37900.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 37899
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 37891–37899; 37900 is capstone
