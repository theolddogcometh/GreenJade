...
"""
Soft continuum generator — MILESTONE 38000 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 37901–38000 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=38000   when parent has wired through graph_batch38000.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 37901 38000
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 37901 38000
...
graph batches ending at M. For M = **38000** the exclusive range is
**37901–38000** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 37901–37950  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 37951–37975  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 37976–37990  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 37991–38000  │ decade soft gates (Wave 238 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 38000 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_38000        1          graph_batch37991.c
  gj_libcgj_green_38000       1          graph_batch37992.c
  gj_bar3_ready_38000         0          graph_batch37993.c  (bar3 open)
  gj_product_score_38000      0          graph_batch37994.c  (score open)
  gj_continuum_ready_38000    1          graph_batch37995.c
  gj_smoke_soft_38000         1          graph_batch37996.c
  gj_dyn_soft_38000           1          graph_batch37997.c
  gj_milestone_tag_38000      38000      graph_batch37998.c
  gj_continuum_wave_38000     38000      graph_batch37999.c
  gj_batch_id_38000           38000      graph_batch38000.c
  gj_graph_milestone_38000    38000      graph_batch38000.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=38000`` means parent wired CGJ_SRCS through
    ``graph_batch38000.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_38000`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_38000`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M38000_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_37900`` / ``gj_*_37800`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 238 exclusive CREATE-ONLY lands the **markers** band
  (37991–38000) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_38000.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_38000.py

  # Restrict to marker band only (soft-gate TUs; Wave 238 exclusive):
  python3 user/libcgj/src/_gen_milestone_38000.py --band markers

  # Extension identity only (37901–37950):
  python3 user/libcgj/src/_gen_milestone_38000.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_38000.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_37900.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 38000 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 38000
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 37901
WAVE_HI = 38000  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 37901, 37950
IDENTITY_LO, IDENTITY_HI = 37951, 37975
FILL_LO, FILL_HI = 37976, 37990
MARKERS_LO, MARKERS_HI = 37991, 38000
...
# Stems become gj_<stem>_38000. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (37991, "shell_green", 1, "shell product green extract (wave 38000)"),
    (37992, "libcgj_green", 1, "libcgj product green extract (wave 38000)"),
    (37993, "bar3_ready", 0, "product bar3 readiness gate (wave 38000)"),
    (37994, "product_score", 0, "product readiness score tag (wave 38000)"),
    (37995, "continuum_ready", 1, "continuum readiness gate (wave 38000)"),
    (37996, "smoke_soft", 1, "smoke soft lamp (wave 38000)"),
    (37997, "dyn_soft", 1, "dyn soft lamp (wave 38000)"),
    (37998, "milestone_tag", MILESTONE, "milestone 38000 product tag"),
    (37999, "continuum_wave", MILESTONE, "milestone 38000 continuum wave identity"),
    # 38000 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        37976: "continuum wave fill soft unit + batch id",
        37980: "continuum bridge soft unit + batch id",
        37990: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (37991–37999), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 37993
    tag_batch = MARKERS_LO + 7  # 37998
    wave_batch = MARKERS_LO + 8  # 37999
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_37900 / gj_shell_green_37800 / gj_shell_green_37700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_37900 / gj_libcgj_green_37800 / gj_libcgj_green_37700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_37900 / gj_bar3_ready_37800 / gj_bar3_ready_37700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_37900 / gj_product_score_37800 / gj_product_score_37700\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_37900 / gj_continuum_ready_37800 /\n"
            f" * gj_continuum_ready_37700 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_37900 / gj_smoke_soft_37800 / gj_smoke_soft_37700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_37900 / gj_dyn_soft_37800 / gj_dyn_soft_37700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_37900 / gj_milestone_tag_37800 / gj_milestone_tag_37700,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_37900 / gj_continuum_wave_37800 /\n"
            f" * gj_continuum_wave_37700, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch38000.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 37999
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 37991–37999; 38000 is capstone
