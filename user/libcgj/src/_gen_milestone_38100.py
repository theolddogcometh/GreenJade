...
"""
Soft continuum generator — MILESTONE 38100 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 38001–38100 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=38100   when parent has wired through graph_batch38100.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 38001 38100
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 38001 38100
...
graph batches ending at M. For M = **38100** the exclusive range is
**38001–38100** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 38001–38050  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 38051–38075  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 38076–38090  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 38091–38100  │ decade soft gates (Wave 239 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 38100 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_38100        1          graph_batch38091.c
  gj_libcgj_green_38100       1          graph_batch38092.c
  gj_bar3_ready_38100         0          graph_batch38093.c  (bar3 open)
  gj_product_score_38100      0          graph_batch38094.c  (score open)
  gj_continuum_ready_38100    1          graph_batch38095.c
  gj_smoke_soft_38100         1          graph_batch38096.c
  gj_dyn_soft_38100           1          graph_batch38097.c
  gj_milestone_tag_38100      38100      graph_batch38098.c
  gj_continuum_wave_38100     38100      graph_batch38099.c
  gj_batch_id_38100           38100      graph_batch38100.c
  gj_graph_milestone_38100    38100      graph_batch38100.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=38100`` means parent wired CGJ_SRCS through
    ``graph_batch38100.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_38100`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_38100`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M38100_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_38000`` / ``gj_*_37900`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 239 exclusive CREATE-ONLY lands the **markers** band
  (38091–38100) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_38100.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_38100.py

  # Restrict to marker band only (soft-gate TUs; Wave 239 exclusive):
  python3 user/libcgj/src/_gen_milestone_38100.py --band markers

  # Extension identity only (38001–38050):
  python3 user/libcgj/src/_gen_milestone_38100.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_38100.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_38000.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 38100 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 38100
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 38001
WAVE_HI = 38100  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 38001, 38050
IDENTITY_LO, IDENTITY_HI = 38051, 38075
FILL_LO, FILL_HI = 38076, 38090
MARKERS_LO, MARKERS_HI = 38091, 38100
...
# Stems become gj_<stem>_38100. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (38091, "shell_green", 1, "shell product green extract (wave 38100)"),
    (38092, "libcgj_green", 1, "libcgj product green extract (wave 38100)"),
    (38093, "bar3_ready", 0, "product bar3 readiness gate (wave 38100)"),
    (38094, "product_score", 0, "product readiness score tag (wave 38100)"),
    (38095, "continuum_ready", 1, "continuum readiness gate (wave 38100)"),
    (38096, "smoke_soft", 1, "smoke soft lamp (wave 38100)"),
    (38097, "dyn_soft", 1, "dyn soft lamp (wave 38100)"),
    (38098, "milestone_tag", MILESTONE, "milestone 38100 product tag"),
    (38099, "continuum_wave", MILESTONE, "milestone 38100 continuum wave identity"),
    # 38100 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        38076: "continuum wave fill soft unit + batch id",
        38080: "continuum bridge soft unit + batch id",
        38090: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (38091–38099), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 38093
    tag_batch = MARKERS_LO + 7  # 38098
    wave_batch = MARKERS_LO + 8  # 38099
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_38000 / gj_shell_green_37900 / gj_shell_green_37800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_38000 / gj_libcgj_green_37900 / gj_libcgj_green_37800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_38000 / gj_bar3_ready_37900 / gj_bar3_ready_37800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_38000 / gj_product_score_37900 / gj_product_score_37800\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_38000 / gj_continuum_ready_37900 /\n"
            f" * gj_continuum_ready_37800 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_38000 / gj_smoke_soft_37900 / gj_smoke_soft_37800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_38000 / gj_dyn_soft_37900 / gj_dyn_soft_37800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_38000 / gj_milestone_tag_37900 / gj_milestone_tag_37800,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_38000 / gj_continuum_wave_37900 /\n"
            f" * gj_continuum_wave_37800, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch38100.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 38099
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 38091–38099; 38100 is capstone
