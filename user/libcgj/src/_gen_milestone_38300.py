...
"""
Soft continuum generator — MILESTONE 38300 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 38201–38300 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=38300   when parent has wired through graph_batch38300.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 38201 38300
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 38201 38300
...
graph batches ending at M. For M = **38300** the exclusive range is
**38201–38300** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 38201–38250  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 38251–38275  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 38276–38290  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 38291–38300  │ decade soft gates (Wave 241 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 38300 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_38300        1          graph_batch38291.c
  gj_libcgj_green_38300       1          graph_batch38292.c
  gj_bar3_ready_38300         0          graph_batch38293.c  (bar3 open)
  gj_product_score_38300      0          graph_batch38294.c  (score open)
  gj_continuum_ready_38300    1          graph_batch38295.c
  gj_smoke_soft_38300         1          graph_batch38296.c
  gj_dyn_soft_38300           1          graph_batch38297.c
  gj_milestone_tag_38300      38300      graph_batch38298.c
  gj_continuum_wave_38300     38300      graph_batch38299.c
  gj_batch_id_38300           38300      graph_batch38300.c
  gj_graph_milestone_38300    38300      graph_batch38300.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=38300`` means parent wired CGJ_SRCS through
    ``graph_batch38300.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_38300`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_38300`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M38300_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_38200`` / ``gj_*_38100`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 241 exclusive CREATE-ONLY lands the **markers** band
  (38291–38300) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_38300.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_38300.py

  # Restrict to marker band only (soft-gate TUs; Wave 241 exclusive):
  python3 user/libcgj/src/_gen_milestone_38300.py --band markers

  # Extension identity only (38201–38250):
  python3 user/libcgj/src/_gen_milestone_38300.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_38300.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_38200.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 38300 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 38300
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 38201
WAVE_HI = 38300  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 38201, 38250
IDENTITY_LO, IDENTITY_HI = 38251, 38275
FILL_LO, FILL_HI = 38276, 38290
MARKERS_LO, MARKERS_HI = 38291, 38300
...
# Stems become gj_<stem>_38300. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (38291, "shell_green", 1, "shell product green extract (wave 38300)"),
    (38292, "libcgj_green", 1, "libcgj product green extract (wave 38300)"),
    (38293, "bar3_ready", 0, "product bar3 readiness gate (wave 38300)"),
    (38294, "product_score", 0, "product readiness score tag (wave 38300)"),
    (38295, "continuum_ready", 1, "continuum readiness gate (wave 38300)"),
    (38296, "smoke_soft", 1, "smoke soft lamp (wave 38300)"),
    (38297, "dyn_soft", 1, "dyn soft lamp (wave 38300)"),
    (38298, "milestone_tag", MILESTONE, "milestone 38300 product tag"),
    (38299, "continuum_wave", MILESTONE, "milestone 38300 continuum wave identity"),
    # 38300 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        38276: "continuum wave fill soft unit + batch id",
        38280: "continuum bridge soft unit + batch id",
        38290: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (38291–38299), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 38293
    tag_batch = MARKERS_LO + 7  # 38298
    wave_batch = MARKERS_LO + 8  # 38299
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_38200 / gj_shell_green_38100 / gj_shell_green_38000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_38200 / gj_libcgj_green_38100 / gj_libcgj_green_38000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_38200 / gj_bar3_ready_38100 / gj_bar3_ready_38000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_38200 / gj_product_score_38100 / gj_product_score_38000\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_38200 / gj_continuum_ready_38100 /\n"
            f" * gj_continuum_ready_38000 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_38200 / gj_smoke_soft_38100 / gj_smoke_soft_38000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_38200 / gj_dyn_soft_38100 / gj_dyn_soft_38000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_38200 / gj_milestone_tag_38100 / gj_milestone_tag_38000,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_38200 / gj_continuum_wave_38100 /\n"
            f" * gj_continuum_wave_38000, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch38300.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 38299
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 38291–38299; 38300 is capstone
