...
"""
Soft continuum generator — MILESTONE 38400 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 38301–38400 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=38400   when parent has wired through graph_batch38400.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 38301 38400
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 38301 38400
...
graph batches ending at M. For M = **38400** the exclusive range is
**38301–38400** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 38301–38350  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 38351–38375  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 38376–38390  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 38391–38400  │ decade soft gates (Wave 242 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 38400 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_38400        1          graph_batch38391.c
  gj_libcgj_green_38400       1          graph_batch38392.c
  gj_bar3_ready_38400         0          graph_batch38393.c  (bar3 open)
  gj_product_score_38400      0          graph_batch38394.c  (score open)
  gj_continuum_ready_38400    1          graph_batch38395.c
  gj_smoke_soft_38400         1          graph_batch38396.c
  gj_dyn_soft_38400           1          graph_batch38397.c
  gj_milestone_tag_38400      38400      graph_batch38398.c
  gj_continuum_wave_38400     38400      graph_batch38399.c
  gj_batch_id_38400           38400      graph_batch38400.c
  gj_graph_milestone_38400    38400      graph_batch38400.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=38400`` means parent wired CGJ_SRCS through
    ``graph_batch38400.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_38400`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_38400`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M38400_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_38300`` / ``gj_*_38200`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 242 exclusive CREATE-ONLY lands the **markers** band
  (38391–38400) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_38400.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_38400.py

  # Restrict to marker band only (soft-gate TUs; Wave 242 exclusive):
  python3 user/libcgj/src/_gen_milestone_38400.py --band markers

  # Extension identity only (38301–38350):
  python3 user/libcgj/src/_gen_milestone_38400.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_38400.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_38300.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 38400 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 38400
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 38301
WAVE_HI = 38400  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 38301, 38350
IDENTITY_LO, IDENTITY_HI = 38351, 38375
FILL_LO, FILL_HI = 38376, 38390
MARKERS_LO, MARKERS_HI = 38391, 38400
...
# Stems become gj_<stem>_38400. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (38391, "shell_green", 1, "shell product green extract (wave 38400)"),
    (38392, "libcgj_green", 1, "libcgj product green extract (wave 38400)"),
    (38393, "bar3_ready", 0, "product bar3 readiness gate (wave 38400)"),
    (38394, "product_score", 0, "product readiness score tag (wave 38400)"),
    (38395, "continuum_ready", 1, "continuum readiness gate (wave 38400)"),
    (38396, "smoke_soft", 1, "smoke soft lamp (wave 38400)"),
    (38397, "dyn_soft", 1, "dyn soft lamp (wave 38400)"),
    (38398, "milestone_tag", MILESTONE, "milestone 38400 product tag"),
    (38399, "continuum_wave", MILESTONE, "milestone 38400 continuum wave identity"),
    # 38400 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        38376: "continuum wave fill soft unit + batch id",
        38380: "continuum bridge soft unit + batch id",
        38390: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (38391–38399), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 38393
    tag_batch = MARKERS_LO + 7  # 38398
    wave_batch = MARKERS_LO + 8  # 38399
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_38300 / gj_shell_green_38200 / gj_shell_green_38100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_38300 / gj_libcgj_green_38200 / gj_libcgj_green_38100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_38300 / gj_bar3_ready_38200 / gj_bar3_ready_38100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_38300 / gj_product_score_38200 / gj_product_score_38100\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_38300 / gj_continuum_ready_38200 /\n"
            f" * gj_continuum_ready_38100 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_38300 / gj_smoke_soft_38200 / gj_smoke_soft_38100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_38300 / gj_dyn_soft_38200 / gj_dyn_soft_38100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_38300 / gj_milestone_tag_38200 / gj_milestone_tag_38100,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_38300 / gj_continuum_wave_38200 /\n"
            f" * gj_continuum_wave_38100, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch38400.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 38399
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 38391–38399; 38400 is capstone
