...
"""
Soft continuum generator — MILESTONE 38500 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 38401–38500 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=38500   when parent has wired through graph_batch38500.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 38401 38500
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 38401 38500
...
graph batches ending at M. For M = **38500** the exclusive range is
**38401–38500** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 38401–38450  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 38451–38475  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 38476–38490  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 38491–38500  │ decade soft gates (Wave 243 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 38500 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_38500        1          graph_batch38491.c
  gj_libcgj_green_38500       1          graph_batch38492.c
  gj_bar3_ready_38500         0          graph_batch38493.c  (bar3 open)
  gj_product_score_38500      0          graph_batch38494.c  (score open)
  gj_continuum_ready_38500    1          graph_batch38495.c
  gj_smoke_soft_38500         1          graph_batch38496.c
  gj_dyn_soft_38500           1          graph_batch38497.c
  gj_milestone_tag_38500      38500      graph_batch38498.c
  gj_continuum_wave_38500     38500      graph_batch38499.c
  gj_batch_id_38500           38500      graph_batch38500.c
  gj_graph_milestone_38500    38500      graph_batch38500.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=38500`` means parent wired CGJ_SRCS through
    ``graph_batch38500.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_38500`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_38500`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M38500_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_38400`` / ``gj_*_38300`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 243 exclusive CREATE-ONLY lands the **markers** band
  (38491–38500) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_38500.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_38500.py

  # Restrict to marker band only (soft-gate TUs; Wave 243 exclusive):
  python3 user/libcgj/src/_gen_milestone_38500.py --band markers

  # Extension identity only (38401–38450):
  python3 user/libcgj/src/_gen_milestone_38500.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_38500.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_38400.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 38500 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 38500
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 38401
WAVE_HI = 38500  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 38401, 38450
IDENTITY_LO, IDENTITY_HI = 38451, 38475
FILL_LO, FILL_HI = 38476, 38490
MARKERS_LO, MARKERS_HI = 38491, 38500
...
# Stems become gj_<stem>_38500. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (38491, "shell_green", 1, "shell product green extract (wave 38500)"),
    (38492, "libcgj_green", 1, "libcgj product green extract (wave 38500)"),
    (38493, "bar3_ready", 0, "product bar3 readiness gate (wave 38500)"),
    (38494, "product_score", 0, "product readiness score tag (wave 38500)"),
    (38495, "continuum_ready", 1, "continuum readiness gate (wave 38500)"),
    (38496, "smoke_soft", 1, "smoke soft lamp (wave 38500)"),
    (38497, "dyn_soft", 1, "dyn soft lamp (wave 38500)"),
    (38498, "milestone_tag", MILESTONE, "milestone 38500 product tag"),
    (38499, "continuum_wave", MILESTONE, "milestone 38500 continuum wave identity"),
    # 38500 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        38476: "continuum wave fill soft unit + batch id",
        38480: "continuum bridge soft unit + batch id",
        38490: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (38491–38499), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 38493
    tag_batch = MARKERS_LO + 7  # 38498
    wave_batch = MARKERS_LO + 8  # 38499
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_38400 / gj_shell_green_38300 / gj_shell_green_38200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_38400 / gj_libcgj_green_38300 / gj_libcgj_green_38200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_38400 / gj_bar3_ready_38300 / gj_bar3_ready_38200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_38400 / gj_product_score_38300 / gj_product_score_38200\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_38400 / gj_continuum_ready_38300 /\n"
            f" * gj_continuum_ready_38200 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_38400 / gj_smoke_soft_38300 / gj_smoke_soft_38200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_38400 / gj_dyn_soft_38300 / gj_dyn_soft_38200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_38400 / gj_milestone_tag_38300 / gj_milestone_tag_38200,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_38400 / gj_continuum_wave_38300 /\n"
            f" * gj_continuum_wave_38200, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch38500.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 38499
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 38491–38499; 38500 is capstone
