...
"""
Soft continuum generator — MILESTONE 38600 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 38501–38600 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=38600   when parent has wired through graph_batch38600.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 38501 38600
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 38501 38600
...
graph batches ending at M. For M = **38600** the exclusive range is
**38501–38600** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 38501–38550  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 38551–38575  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 38576–38590  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 38591–38600  │ decade soft gates (Wave 244 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 38600 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_38600        1          graph_batch38591.c
  gj_libcgj_green_38600       1          graph_batch38592.c
  gj_bar3_ready_38600         0          graph_batch38593.c  (bar3 open)
  gj_product_score_38600      0          graph_batch38594.c  (score open)
  gj_continuum_ready_38600    1          graph_batch38595.c
  gj_smoke_soft_38600         1          graph_batch38596.c
  gj_dyn_soft_38600           1          graph_batch38597.c
  gj_milestone_tag_38600      38600      graph_batch38598.c
  gj_continuum_wave_38600     38600      graph_batch38599.c
  gj_batch_id_38600           38600      graph_batch38600.c
  gj_graph_milestone_38600    38600      graph_batch38600.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=38600`` means parent wired CGJ_SRCS through
    ``graph_batch38600.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_38600`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_38600`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M38600_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_38500`` / ``gj_*_38400`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 244 exclusive CREATE-ONLY lands the **markers** band
  (38591–38600) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_38600.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_38600.py

  # Restrict to marker band only (soft-gate TUs; Wave 244 exclusive):
  python3 user/libcgj/src/_gen_milestone_38600.py --band markers

  # Extension identity only (38501–38550):
  python3 user/libcgj/src/_gen_milestone_38600.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_38600.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_38500.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 38600 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 38600
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 38501
WAVE_HI = 38600  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 38501, 38550
IDENTITY_LO, IDENTITY_HI = 38551, 38575
FILL_LO, FILL_HI = 38576, 38590
MARKERS_LO, MARKERS_HI = 38591, 38600
...
# Stems become gj_<stem>_38600. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (38591, "shell_green", 1, "shell product green extract (wave 38600)"),
    (38592, "libcgj_green", 1, "libcgj product green extract (wave 38600)"),
    (38593, "bar3_ready", 0, "product bar3 readiness gate (wave 38600)"),
    (38594, "product_score", 0, "product readiness score tag (wave 38600)"),
    (38595, "continuum_ready", 1, "continuum readiness gate (wave 38600)"),
    (38596, "smoke_soft", 1, "smoke soft lamp (wave 38600)"),
    (38597, "dyn_soft", 1, "dyn soft lamp (wave 38600)"),
    (38598, "milestone_tag", MILESTONE, "milestone 38600 product tag"),
    (38599, "continuum_wave", MILESTONE, "milestone 38600 continuum wave identity"),
    # 38600 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        38576: "continuum wave fill soft unit + batch id",
        38580: "continuum bridge soft unit + batch id",
        38590: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (38591–38599), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 38593
    tag_batch = MARKERS_LO + 7  # 38598
    wave_batch = MARKERS_LO + 8  # 38599
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_38500 / gj_shell_green_38400 / gj_shell_green_38300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_38500 / gj_libcgj_green_38400 / gj_libcgj_green_38300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_38500 / gj_bar3_ready_38400 / gj_bar3_ready_38300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_38500 / gj_product_score_38400 / gj_product_score_38300\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_38500 / gj_continuum_ready_38400 /\n"
            f" * gj_continuum_ready_38300 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_38500 / gj_smoke_soft_38400 / gj_smoke_soft_38300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_38500 / gj_dyn_soft_38400 / gj_dyn_soft_38300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_38500 / gj_milestone_tag_38400 / gj_milestone_tag_38300,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_38500 / gj_continuum_wave_38400 /\n"
            f" * gj_continuum_wave_38300, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch38600.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 38599
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 38591–38599; 38600 is capstone
