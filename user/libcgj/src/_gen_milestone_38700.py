...
"""
Soft continuum generator — MILESTONE 38700 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 38601–38700 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=38700   when parent has wired through graph_batch38700.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 38601 38700
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 38601 38700
...
graph batches ending at M. For M = **38700** the exclusive range is
**38601–38700** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 38601–38650  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 38651–38675  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 38676–38690  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 38691–38700  │ decade soft gates (Wave 245 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 38700 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_38700        1          graph_batch38691.c
  gj_libcgj_green_38700       1          graph_batch38692.c
  gj_bar3_ready_38700         0          graph_batch38693.c  (bar3 open)
  gj_product_score_38700      0          graph_batch38694.c  (score open)
  gj_continuum_ready_38700    1          graph_batch38695.c
  gj_smoke_soft_38700         1          graph_batch38696.c
  gj_dyn_soft_38700           1          graph_batch38697.c
  gj_milestone_tag_38700      38700      graph_batch38698.c
  gj_continuum_wave_38700     38700      graph_batch38699.c
  gj_batch_id_38700           38700      graph_batch38700.c
  gj_graph_milestone_38700    38700      graph_batch38700.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=38700`` means parent wired CGJ_SRCS through
    ``graph_batch38700.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_38700`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_38700`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M38700_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_38600`` / ``gj_*_38500`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 245 exclusive CREATE-ONLY lands the **markers** band
  (38691–38700) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_38700.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_38700.py

  # Restrict to marker band only (soft-gate TUs; Wave 245 exclusive):
  python3 user/libcgj/src/_gen_milestone_38700.py --band markers

  # Extension identity only (38601–38650):
  python3 user/libcgj/src/_gen_milestone_38700.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_38700.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_38600.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 38700 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 38700
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 38601
WAVE_HI = 38700  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 38601, 38650
IDENTITY_LO, IDENTITY_HI = 38651, 38675
FILL_LO, FILL_HI = 38676, 38690
MARKERS_LO, MARKERS_HI = 38691, 38700
...
# Stems become gj_<stem>_38700. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (38691, "shell_green", 1, "shell product green extract (wave 38700)"),
    (38692, "libcgj_green", 1, "libcgj product green extract (wave 38700)"),
    (38693, "bar3_ready", 0, "product bar3 readiness gate (wave 38700)"),
    (38694, "product_score", 0, "product readiness score tag (wave 38700)"),
    (38695, "continuum_ready", 1, "continuum readiness gate (wave 38700)"),
    (38696, "smoke_soft", 1, "smoke soft lamp (wave 38700)"),
    (38697, "dyn_soft", 1, "dyn soft lamp (wave 38700)"),
    (38698, "milestone_tag", MILESTONE, "milestone 38700 product tag"),
    (38699, "continuum_wave", MILESTONE, "milestone 38700 continuum wave identity"),
    # 38700 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        38676: "continuum wave fill soft unit + batch id",
        38680: "continuum bridge soft unit + batch id",
        38690: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (38691–38699), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 38693
    tag_batch = MARKERS_LO + 7  # 38698
    wave_batch = MARKERS_LO + 8  # 38699
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_38600 / gj_shell_green_38500 / gj_shell_green_38400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_38600 / gj_libcgj_green_38500 / gj_libcgj_green_38400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_38600 / gj_bar3_ready_38500 / gj_bar3_ready_38400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_38600 / gj_product_score_38500 / gj_product_score_38400\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_38600 / gj_continuum_ready_38500 /\n"
            f" * gj_continuum_ready_38400 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_38600 / gj_smoke_soft_38500 / gj_smoke_soft_38400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_38600 / gj_dyn_soft_38500 / gj_dyn_soft_38400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_38600 / gj_milestone_tag_38500 / gj_milestone_tag_38400,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_38600 / gj_continuum_wave_38500 /\n"
            f" * gj_continuum_wave_38400, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch38700.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 38699
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 38691–38699; 38700 is capstone
