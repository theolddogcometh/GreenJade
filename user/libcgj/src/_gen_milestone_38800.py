...
"""
Soft continuum generator — MILESTONE 38800 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 38701–38800 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=38800   when parent has wired through graph_batch38800.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 38701 38800
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 38701 38800
...
graph batches ending at M. For M = **38800** the exclusive range is
**38701–38800** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 38701–38750  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 38751–38775  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 38776–38790  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 38791–38800  │ decade soft gates (Wave 246 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 38800 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_38800        1          graph_batch38791.c
  gj_libcgj_green_38800       1          graph_batch38792.c
  gj_bar3_ready_38800         0          graph_batch38793.c  (bar3 open)
  gj_product_score_38800      0          graph_batch38794.c  (score open)
  gj_continuum_ready_38800    1          graph_batch38795.c
  gj_smoke_soft_38800         1          graph_batch38796.c
  gj_dyn_soft_38800           1          graph_batch38797.c
  gj_milestone_tag_38800      38800      graph_batch38798.c
  gj_continuum_wave_38800     38800      graph_batch38799.c
  gj_batch_id_38800           38800      graph_batch38800.c
  gj_graph_milestone_38800    38800      graph_batch38800.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=38800`` means parent wired CGJ_SRCS through
    ``graph_batch38800.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_38800`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_38800`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M38800_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_38700`` / ``gj_*_38600`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 246 exclusive CREATE-ONLY lands the **markers** band
  (38791–38800) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_38800.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_38800.py

  # Restrict to marker band only (soft-gate TUs; Wave 246 exclusive):
  python3 user/libcgj/src/_gen_milestone_38800.py --band markers

  # Extension identity only (38701–38750):
  python3 user/libcgj/src/_gen_milestone_38800.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_38800.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_38700.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 38800 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 38800
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 38701
WAVE_HI = 38800  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 38701, 38750
IDENTITY_LO, IDENTITY_HI = 38751, 38775
FILL_LO, FILL_HI = 38776, 38790
MARKERS_LO, MARKERS_HI = 38791, 38800
...
# Stems become gj_<stem>_38800. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (38791, "shell_green", 1, "shell product green extract (wave 38800)"),
    (38792, "libcgj_green", 1, "libcgj product green extract (wave 38800)"),
    (38793, "bar3_ready", 0, "product bar3 readiness gate (wave 38800)"),
    (38794, "product_score", 0, "product readiness score tag (wave 38800)"),
    (38795, "continuum_ready", 1, "continuum readiness gate (wave 38800)"),
    (38796, "smoke_soft", 1, "smoke soft lamp (wave 38800)"),
    (38797, "dyn_soft", 1, "dyn soft lamp (wave 38800)"),
    (38798, "milestone_tag", MILESTONE, "milestone 38800 product tag"),
    (38799, "continuum_wave", MILESTONE, "milestone 38800 continuum wave identity"),
    # 38800 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        38776: "continuum wave fill soft unit + batch id",
        38780: "continuum bridge soft unit + batch id",
        38790: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (38791–38799), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 38793
    tag_batch = MARKERS_LO + 7  # 38798
    wave_batch = MARKERS_LO + 8  # 38799
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_38700 / gj_shell_green_38600 / gj_shell_green_38500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_38700 / gj_libcgj_green_38600 / gj_libcgj_green_38500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_38700 / gj_bar3_ready_38600 / gj_bar3_ready_38500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_38700 / gj_product_score_38600 / gj_product_score_38500\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_38700 / gj_continuum_ready_38600 /\n"
            f" * gj_continuum_ready_38500 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_38700 / gj_smoke_soft_38600 / gj_smoke_soft_38500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_38700 / gj_dyn_soft_38600 / gj_dyn_soft_38500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_38700 / gj_milestone_tag_38600 / gj_milestone_tag_38500,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_38700 / gj_continuum_wave_38600 /\n"
            f" * gj_continuum_wave_38500, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch38800.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 38799
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 38791–38799; 38800 is capstone
