...
"""
Soft continuum generator — MILESTONE 38900 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 38801–38900 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=38900   when parent has wired through graph_batch38900.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 38801 38900
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 38801 38900
...
graph batches ending at M. For M = **38900** the exclusive range is
**38801–38900** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 38801–38850  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 38851–38875  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 38876–38890  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 38891–38900  │ decade soft gates (Wave 247 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 38900 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_38900        1          graph_batch38891.c
  gj_libcgj_green_38900       1          graph_batch38892.c
  gj_bar3_ready_38900         0          graph_batch38893.c  (bar3 open)
  gj_product_score_38900      0          graph_batch38894.c  (score open)
  gj_continuum_ready_38900    1          graph_batch38895.c
  gj_smoke_soft_38900         1          graph_batch38896.c
  gj_dyn_soft_38900           1          graph_batch38897.c
  gj_milestone_tag_38900      38900      graph_batch38898.c
  gj_continuum_wave_38900     38900      graph_batch38899.c
  gj_batch_id_38900           38900      graph_batch38900.c
  gj_graph_milestone_38900    38900      graph_batch38900.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=38900`` means parent wired CGJ_SRCS through
    ``graph_batch38900.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_38900`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_38900`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M38900_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_38800`` / ``gj_*_38700`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 247 exclusive CREATE-ONLY lands the **markers** band
  (38891–38900) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_38900.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_38900.py

  # Restrict to marker band only (soft-gate TUs; Wave 247 exclusive):
  python3 user/libcgj/src/_gen_milestone_38900.py --band markers

  # Extension identity only (38801–38850):
  python3 user/libcgj/src/_gen_milestone_38900.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_38900.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_38800.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 38900 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 38900
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 38801
WAVE_HI = 38900  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 38801, 38850
IDENTITY_LO, IDENTITY_HI = 38851, 38875
FILL_LO, FILL_HI = 38876, 38890
MARKERS_LO, MARKERS_HI = 38891, 38900
...
# Stems become gj_<stem>_38900. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (38891, "shell_green", 1, "shell product green extract (wave 38900)"),
    (38892, "libcgj_green", 1, "libcgj product green extract (wave 38900)"),
    (38893, "bar3_ready", 0, "product bar3 readiness gate (wave 38900)"),
    (38894, "product_score", 0, "product readiness score tag (wave 38900)"),
    (38895, "continuum_ready", 1, "continuum readiness gate (wave 38900)"),
    (38896, "smoke_soft", 1, "smoke soft lamp (wave 38900)"),
    (38897, "dyn_soft", 1, "dyn soft lamp (wave 38900)"),
    (38898, "milestone_tag", MILESTONE, "milestone 38900 product tag"),
    (38899, "continuum_wave", MILESTONE, "milestone 38900 continuum wave identity"),
    # 38900 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        38876: "continuum wave fill soft unit + batch id",
        38880: "continuum bridge soft unit + batch id",
        38890: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (38891–38899), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 38893
    tag_batch = MARKERS_LO + 7  # 38898
    wave_batch = MARKERS_LO + 8  # 38899
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_38800 / gj_shell_green_38700 / gj_shell_green_38600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_38800 / gj_libcgj_green_38700 / gj_libcgj_green_38600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_38800 / gj_bar3_ready_38700 / gj_bar3_ready_38600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_38800 / gj_product_score_38700 / gj_product_score_38600\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_38800 / gj_continuum_ready_38700 /\n"
            f" * gj_continuum_ready_38600 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_38800 / gj_smoke_soft_38700 / gj_smoke_soft_38600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_38800 / gj_dyn_soft_38700 / gj_dyn_soft_38600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_38800 / gj_milestone_tag_38700 / gj_milestone_tag_38600,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_38800 / gj_continuum_wave_38700 /\n"
            f" * gj_continuum_wave_38600, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch38900.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 38899
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 38891–38899; 38900 is capstone
