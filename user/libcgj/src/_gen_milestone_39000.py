...
"""
Soft continuum generator — MILESTONE 39000 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 38901–39000 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=39000   when parent has wired through graph_batch39000.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 38901 39000
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 38901 39000
...
graph batches ending at M. For M = **39000** the exclusive range is
**38901–39000** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 38901–38950  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 38951–38975  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 38976–38990  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 38991–39000  │ decade soft gates (Wave 248 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 39000 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_39000        1          graph_batch38991.c
  gj_libcgj_green_39000       1          graph_batch38992.c
  gj_bar3_ready_39000         0          graph_batch38993.c  (bar3 open)
  gj_product_score_39000      0          graph_batch38994.c  (score open)
  gj_continuum_ready_39000    1          graph_batch38995.c
  gj_smoke_soft_39000         1          graph_batch38996.c
  gj_dyn_soft_39000           1          graph_batch38997.c
  gj_milestone_tag_39000      39000      graph_batch38998.c
  gj_continuum_wave_39000     39000      graph_batch38999.c
  gj_batch_id_39000           39000      graph_batch39000.c
  gj_graph_milestone_39000    39000      graph_batch39000.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=39000`` means parent wired CGJ_SRCS through
    ``graph_batch39000.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_39000`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_39000`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M39000_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_38900`` / ``gj_*_38800`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 248 exclusive CREATE-ONLY lands the **markers** band
  (38991–39000) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_39000.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_39000.py

  # Restrict to marker band only (soft-gate TUs; Wave 248 exclusive):
  python3 user/libcgj/src/_gen_milestone_39000.py --band markers

  # Extension identity only (38901–38950):
  python3 user/libcgj/src/_gen_milestone_39000.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_39000.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_38900.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 39000 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 39000
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 38901
WAVE_HI = 39000  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 38901, 38950
IDENTITY_LO, IDENTITY_HI = 38951, 38975
FILL_LO, FILL_HI = 38976, 38990
MARKERS_LO, MARKERS_HI = 38991, 39000
...
# Stems become gj_<stem>_39000. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (38991, "shell_green", 1, "shell product green extract (wave 39000)"),
    (38992, "libcgj_green", 1, "libcgj product green extract (wave 39000)"),
    (38993, "bar3_ready", 0, "product bar3 readiness gate (wave 39000)"),
    (38994, "product_score", 0, "product readiness score tag (wave 39000)"),
    (38995, "continuum_ready", 1, "continuum readiness gate (wave 39000)"),
    (38996, "smoke_soft", 1, "smoke soft lamp (wave 39000)"),
    (38997, "dyn_soft", 1, "dyn soft lamp (wave 39000)"),
    (38998, "milestone_tag", MILESTONE, "milestone 39000 product tag"),
    (38999, "continuum_wave", MILESTONE, "milestone 39000 continuum wave identity"),
    # 39000 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        38976: "continuum wave fill soft unit + batch id",
        38980: "continuum bridge soft unit + batch id",
        38990: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (38991–38999), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 38993
    tag_batch = MARKERS_LO + 7  # 38998
    wave_batch = MARKERS_LO + 8  # 38999
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_38900 / gj_shell_green_38800 / gj_shell_green_38700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_38900 / gj_libcgj_green_38800 / gj_libcgj_green_38700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_38900 / gj_bar3_ready_38800 / gj_bar3_ready_38700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_38900 / gj_product_score_38800 / gj_product_score_38700\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_38900 / gj_continuum_ready_38800 /\n"
            f" * gj_continuum_ready_38700 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_38900 / gj_smoke_soft_38800 / gj_smoke_soft_38700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_38900 / gj_dyn_soft_38800 / gj_dyn_soft_38700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_38900 / gj_milestone_tag_38800 / gj_milestone_tag_38700,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_38900 / gj_continuum_wave_38800 /\n"
            f" * gj_continuum_wave_38700, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch39000.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 38999
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 38991–38999; 39000 is capstone
