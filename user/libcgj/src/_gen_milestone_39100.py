...
"""
Soft continuum generator — MILESTONE 39100 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 39001–39100 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=39100   when parent has wired through graph_batch39100.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 39001 39100
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 39001 39100
...
graph batches ending at M. For M = **39100** the exclusive range is
**39001–39100** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 39001–39050  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 39051–39075  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 39076–39090  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 39091–39100  │ decade soft gates (Wave 249 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 39100 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_39100        1          graph_batch39091.c
  gj_libcgj_green_39100       1          graph_batch39092.c
  gj_bar3_ready_39100         0          graph_batch39093.c  (bar3 open)
  gj_product_score_39100      0          graph_batch39094.c  (score open)
  gj_continuum_ready_39100    1          graph_batch39095.c
  gj_smoke_soft_39100         1          graph_batch39096.c
  gj_dyn_soft_39100           1          graph_batch39097.c
  gj_milestone_tag_39100      39100      graph_batch39098.c
  gj_continuum_wave_39100     39100      graph_batch39099.c
  gj_batch_id_39100           39100      graph_batch39100.c
  gj_graph_milestone_39100    39100      graph_batch39100.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=39100`` means parent wired CGJ_SRCS through
    ``graph_batch39100.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_39100`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_39100`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M39100_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_39000`` / ``gj_*_38900`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 249 exclusive CREATE-ONLY lands the **markers** band
  (39091–39100) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_39100.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_39100.py

  # Restrict to marker band only (soft-gate TUs; Wave 249 exclusive):
  python3 user/libcgj/src/_gen_milestone_39100.py --band markers

  # Extension identity only (39001–39050):
  python3 user/libcgj/src/_gen_milestone_39100.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_39100.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_39000.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 39100 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 39100
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 39001
WAVE_HI = 39100  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 39001, 39050
IDENTITY_LO, IDENTITY_HI = 39051, 39075
FILL_LO, FILL_HI = 39076, 39090
MARKERS_LO, MARKERS_HI = 39091, 39100
...
# Stems become gj_<stem>_39100. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (39091, "shell_green", 1, "shell product green extract (wave 39100)"),
    (39092, "libcgj_green", 1, "libcgj product green extract (wave 39100)"),
    (39093, "bar3_ready", 0, "product bar3 readiness gate (wave 39100)"),
    (39094, "product_score", 0, "product readiness score tag (wave 39100)"),
    (39095, "continuum_ready", 1, "continuum readiness gate (wave 39100)"),
    (39096, "smoke_soft", 1, "smoke soft lamp (wave 39100)"),
    (39097, "dyn_soft", 1, "dyn soft lamp (wave 39100)"),
    (39098, "milestone_tag", MILESTONE, "milestone 39100 product tag"),
    (39099, "continuum_wave", MILESTONE, "milestone 39100 continuum wave identity"),
    # 39100 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        39076: "continuum wave fill soft unit + batch id",
        39080: "continuum bridge soft unit + batch id",
        39090: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (39091–39099), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 39093
    tag_batch = MARKERS_LO + 7  # 39098
    wave_batch = MARKERS_LO + 8  # 39099
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_39000 / gj_shell_green_38900 / gj_shell_green_38800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_39000 / gj_libcgj_green_38900 / gj_libcgj_green_38800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_39000 / gj_bar3_ready_38900 / gj_bar3_ready_38800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_39000 / gj_product_score_38900 / gj_product_score_38800\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_39000 / gj_continuum_ready_38900 /\n"
            f" * gj_continuum_ready_38800 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_39000 / gj_smoke_soft_38900 / gj_smoke_soft_38800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_39000 / gj_dyn_soft_38900 / gj_dyn_soft_38800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_39000 / gj_milestone_tag_38900 / gj_milestone_tag_38800,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_39000 / gj_continuum_wave_38900 /\n"
            f" * gj_continuum_wave_38800, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch39100.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 39099
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 39091–39099; 39100 is capstone
