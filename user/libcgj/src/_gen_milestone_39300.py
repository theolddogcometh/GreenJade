...
"""
Soft continuum generator — MILESTONE 39300 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 39201–39300 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=39300   when parent has wired through graph_batch39300.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 39201 39300
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 39201 39300
...
graph batches ending at M. For M = **39300** the exclusive range is
**39201–39300** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 39201–39250  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 39251–39275  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 39276–39290  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 39291–39300  │ decade soft gates (Wave 251 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 39300 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_39300        1          graph_batch39291.c
  gj_libcgj_green_39300       1          graph_batch39292.c
  gj_bar3_ready_39300         0          graph_batch39293.c  (bar3 open)
  gj_product_score_39300      0          graph_batch39294.c  (score open)
  gj_continuum_ready_39300    1          graph_batch39295.c
  gj_smoke_soft_39300         1          graph_batch39296.c
  gj_dyn_soft_39300           1          graph_batch39297.c
  gj_milestone_tag_39300      39300      graph_batch39298.c
  gj_continuum_wave_39300     39300      graph_batch39299.c
  gj_batch_id_39300           39300      graph_batch39300.c
  gj_graph_milestone_39300    39300      graph_batch39300.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=39300`` means parent wired CGJ_SRCS through
    ``graph_batch39300.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_39300`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_39300`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M39300_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_39200`` / ``gj_*_39100`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 251 exclusive CREATE-ONLY lands the **markers** band
  (39291–39300) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_39300.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_39300.py

  # Restrict to marker band only (soft-gate TUs; Wave 251 exclusive):
  python3 user/libcgj/src/_gen_milestone_39300.py --band markers

  # Extension identity only (39201–39250):
  python3 user/libcgj/src/_gen_milestone_39300.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_39300.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_39200.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 39300 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 39300
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 39201
WAVE_HI = 39300  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 39201, 39250
IDENTITY_LO, IDENTITY_HI = 39251, 39275
FILL_LO, FILL_HI = 39276, 39290
MARKERS_LO, MARKERS_HI = 39291, 39300
...
# Stems become gj_<stem>_39300. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (39291, "shell_green", 1, "shell product green extract (wave 39300)"),
    (39292, "libcgj_green", 1, "libcgj product green extract (wave 39300)"),
    (39293, "bar3_ready", 0, "product bar3 readiness gate (wave 39300)"),
    (39294, "product_score", 0, "product readiness score tag (wave 39300)"),
    (39295, "continuum_ready", 1, "continuum readiness gate (wave 39300)"),
    (39296, "smoke_soft", 1, "smoke soft lamp (wave 39300)"),
    (39297, "dyn_soft", 1, "dyn soft lamp (wave 39300)"),
    (39298, "milestone_tag", MILESTONE, "milestone 39300 product tag"),
    (39299, "continuum_wave", MILESTONE, "milestone 39300 continuum wave identity"),
    # 39300 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        39276: "continuum wave fill soft unit + batch id",
        39280: "continuum bridge soft unit + batch id",
        39290: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (39291–39299), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 39293
    tag_batch = MARKERS_LO + 7  # 39298
    wave_batch = MARKERS_LO + 8  # 39299
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_39200 / gj_shell_green_39100 / gj_shell_green_39000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_39200 / gj_libcgj_green_39100 / gj_libcgj_green_39000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_39200 / gj_bar3_ready_39100 / gj_bar3_ready_39000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_39200 / gj_product_score_39100 / gj_product_score_39000\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_39200 / gj_continuum_ready_39100 /\n"
            f" * gj_continuum_ready_39000 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_39200 / gj_smoke_soft_39100 / gj_smoke_soft_39000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_39200 / gj_dyn_soft_39100 / gj_dyn_soft_39000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_39200 / gj_milestone_tag_39100 / gj_milestone_tag_39000,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_39200 / gj_continuum_wave_39100 /\n"
            f" * gj_continuum_wave_39000, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch39300.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 39299
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 39291–39299; 39300 is capstone
