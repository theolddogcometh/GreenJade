...
"""
Soft continuum generator — MILESTONE 39400 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 39301–39400 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=39400   when parent has wired through graph_batch39400.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 39301 39400
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 39301 39400
...
graph batches ending at M. For M = **39400** the exclusive range is
**39301–39400** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 39301–39350  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 39351–39375  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 39376–39390  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 39391–39400  │ decade soft gates (Wave 252 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 39400 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_39400        1          graph_batch39391.c
  gj_libcgj_green_39400       1          graph_batch39392.c
  gj_bar3_ready_39400         0          graph_batch39393.c  (bar3 open)
  gj_product_score_39400      0          graph_batch39394.c  (score open)
  gj_continuum_ready_39400    1          graph_batch39395.c
  gj_smoke_soft_39400         1          graph_batch39396.c
  gj_dyn_soft_39400           1          graph_batch39397.c
  gj_milestone_tag_39400      39400      graph_batch39398.c
  gj_continuum_wave_39400     39400      graph_batch39399.c
  gj_batch_id_39400           39400      graph_batch39400.c
  gj_graph_milestone_39400    39400      graph_batch39400.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=39400`` means parent wired CGJ_SRCS through
    ``graph_batch39400.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_39400`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_39400`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M39400_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_39300`` / ``gj_*_39200`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 252 exclusive CREATE-ONLY lands the **markers** band
  (39391–39400) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_39400.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_39400.py

  # Restrict to marker band only (soft-gate TUs; Wave 252 exclusive):
  python3 user/libcgj/src/_gen_milestone_39400.py --band markers

  # Extension identity only (39301–39350):
  python3 user/libcgj/src/_gen_milestone_39400.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_39400.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_39300.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 39400 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 39400
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 39301
WAVE_HI = 39400  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 39301, 39350
IDENTITY_LO, IDENTITY_HI = 39351, 39375
FILL_LO, FILL_HI = 39376, 39390
MARKERS_LO, MARKERS_HI = 39391, 39400
...
# Stems become gj_<stem>_39400. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (39391, "shell_green", 1, "shell product green extract (wave 39400)"),
    (39392, "libcgj_green", 1, "libcgj product green extract (wave 39400)"),
    (39393, "bar3_ready", 0, "product bar3 readiness gate (wave 39400)"),
    (39394, "product_score", 0, "product readiness score tag (wave 39400)"),
    (39395, "continuum_ready", 1, "continuum readiness gate (wave 39400)"),
    (39396, "smoke_soft", 1, "smoke soft lamp (wave 39400)"),
    (39397, "dyn_soft", 1, "dyn soft lamp (wave 39400)"),
    (39398, "milestone_tag", MILESTONE, "milestone 39400 product tag"),
    (39399, "continuum_wave", MILESTONE, "milestone 39400 continuum wave identity"),
    # 39400 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        39376: "continuum wave fill soft unit + batch id",
        39380: "continuum bridge soft unit + batch id",
        39390: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (39391–39399), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 39393
    tag_batch = MARKERS_LO + 7  # 39398
    wave_batch = MARKERS_LO + 8  # 39399
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_39300 / gj_shell_green_39200 / gj_shell_green_39100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_39300 / gj_libcgj_green_39200 / gj_libcgj_green_39100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_39300 / gj_bar3_ready_39200 / gj_bar3_ready_39100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_39300 / gj_product_score_39200 / gj_product_score_39100\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_39300 / gj_continuum_ready_39200 /\n"
            f" * gj_continuum_ready_39100 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_39300 / gj_smoke_soft_39200 / gj_smoke_soft_39100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_39300 / gj_dyn_soft_39200 / gj_dyn_soft_39100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_39300 / gj_milestone_tag_39200 / gj_milestone_tag_39100,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_39300 / gj_continuum_wave_39200 /\n"
            f" * gj_continuum_wave_39100, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch39400.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 39399
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 39391–39399; 39400 is capstone
