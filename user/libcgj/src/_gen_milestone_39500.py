...
"""
Soft continuum generator — MILESTONE 39500 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 39401–39500 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=39500   when parent has wired through graph_batch39500.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 39401 39500
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 39401 39500
...
graph batches ending at M. For M = **39500** the exclusive range is
**39401–39500** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 39401–39450  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 39451–39475  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 39476–39490  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 39491–39500  │ decade soft gates (Wave 253 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 39500 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_39500        1          graph_batch39491.c
  gj_libcgj_green_39500       1          graph_batch39492.c
  gj_bar3_ready_39500         0          graph_batch39493.c  (bar3 open)
  gj_product_score_39500      0          graph_batch39494.c  (score open)
  gj_continuum_ready_39500    1          graph_batch39495.c
  gj_smoke_soft_39500         1          graph_batch39496.c
  gj_dyn_soft_39500           1          graph_batch39497.c
  gj_milestone_tag_39500      39500      graph_batch39498.c
  gj_continuum_wave_39500     39500      graph_batch39499.c
  gj_batch_id_39500           39500      graph_batch39500.c
  gj_graph_milestone_39500    39500      graph_batch39500.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=39500`` means parent wired CGJ_SRCS through
    ``graph_batch39500.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_39500`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_39500`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M39500_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_39400`` / ``gj_*_39300`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 253 exclusive CREATE-ONLY lands the **markers** band
  (39491–39500) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_39500.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_39500.py

  # Restrict to marker band only (soft-gate TUs; Wave 253 exclusive):
  python3 user/libcgj/src/_gen_milestone_39500.py --band markers

  # Extension identity only (39401–39450):
  python3 user/libcgj/src/_gen_milestone_39500.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_39500.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_39400.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 39500 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 39500
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 39401
WAVE_HI = 39500  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 39401, 39450
IDENTITY_LO, IDENTITY_HI = 39451, 39475
FILL_LO, FILL_HI = 39476, 39490
MARKERS_LO, MARKERS_HI = 39491, 39500
...
# Stems become gj_<stem>_39500. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (39491, "shell_green", 1, "shell product green extract (wave 39500)"),
    (39492, "libcgj_green", 1, "libcgj product green extract (wave 39500)"),
    (39493, "bar3_ready", 0, "product bar3 readiness gate (wave 39500)"),
    (39494, "product_score", 0, "product readiness score tag (wave 39500)"),
    (39495, "continuum_ready", 1, "continuum readiness gate (wave 39500)"),
    (39496, "smoke_soft", 1, "smoke soft lamp (wave 39500)"),
    (39497, "dyn_soft", 1, "dyn soft lamp (wave 39500)"),
    (39498, "milestone_tag", MILESTONE, "milestone 39500 product tag"),
    (39499, "continuum_wave", MILESTONE, "milestone 39500 continuum wave identity"),
    # 39500 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        39476: "continuum wave fill soft unit + batch id",
        39480: "continuum bridge soft unit + batch id",
        39490: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (39491–39499), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 39493
    tag_batch = MARKERS_LO + 7  # 39498
    wave_batch = MARKERS_LO + 8  # 39499
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_39400 / gj_shell_green_39300 / gj_shell_green_39200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_39400 / gj_libcgj_green_39300 / gj_libcgj_green_39200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_39400 / gj_bar3_ready_39300 / gj_bar3_ready_39200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_39400 / gj_product_score_39300 / gj_product_score_39200\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_39400 / gj_continuum_ready_39300 /\n"
            f" * gj_continuum_ready_39200 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_39400 / gj_smoke_soft_39300 / gj_smoke_soft_39200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_39400 / gj_dyn_soft_39300 / gj_dyn_soft_39200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_39400 / gj_milestone_tag_39300 / gj_milestone_tag_39200,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_39400 / gj_continuum_wave_39300 /\n"
            f" * gj_continuum_wave_39200, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch39500.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 39499
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 39491–39499; 39500 is capstone
