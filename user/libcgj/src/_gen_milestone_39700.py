...
"""
Soft continuum generator — MILESTONE 39700 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 39601–39700 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=39700   when parent has wired through graph_batch39700.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 39601 39700
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 39601 39700
...
graph batches ending at M. For M = **39700** the exclusive range is
**39601–39700** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 39601–39650  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 39651–39675  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 39676–39690  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 39691–39700  │ decade soft gates (Wave 255 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 39700 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_39700        1          graph_batch39691.c
  gj_libcgj_green_39700       1          graph_batch39692.c
  gj_bar3_ready_39700         0          graph_batch39693.c  (bar3 open)
  gj_product_score_39700      0          graph_batch39694.c  (score open)
  gj_continuum_ready_39700    1          graph_batch39695.c
  gj_smoke_soft_39700         1          graph_batch39696.c
  gj_dyn_soft_39700           1          graph_batch39697.c
  gj_milestone_tag_39700      39700      graph_batch39698.c
  gj_continuum_wave_39700     39700      graph_batch39699.c
  gj_batch_id_39700           39700      graph_batch39700.c
  gj_graph_milestone_39700    39700      graph_batch39700.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=39700`` means parent wired CGJ_SRCS through
    ``graph_batch39700.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_39700`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_39700`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M39700_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_39600`` / ``gj_*_39500`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 255 exclusive CREATE-ONLY lands the **markers** band
  (39691–39700) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_39700.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_39700.py

  # Restrict to marker band only (soft-gate TUs; Wave 255 exclusive):
  python3 user/libcgj/src/_gen_milestone_39700.py --band markers

  # Extension identity only (39601–39650):
  python3 user/libcgj/src/_gen_milestone_39700.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_39700.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_39600.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 39700 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 39700
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 39601
WAVE_HI = 39700  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 39601, 39650
IDENTITY_LO, IDENTITY_HI = 39651, 39675
FILL_LO, FILL_HI = 39676, 39690
MARKERS_LO, MARKERS_HI = 39691, 39700
...
# Stems become gj_<stem>_39700. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (39691, "shell_green", 1, "shell product green extract (wave 39700)"),
    (39692, "libcgj_green", 1, "libcgj product green extract (wave 39700)"),
    (39693, "bar3_ready", 0, "product bar3 readiness gate (wave 39700)"),
    (39694, "product_score", 0, "product readiness score tag (wave 39700)"),
    (39695, "continuum_ready", 1, "continuum readiness gate (wave 39700)"),
    (39696, "smoke_soft", 1, "smoke soft lamp (wave 39700)"),
    (39697, "dyn_soft", 1, "dyn soft lamp (wave 39700)"),
    (39698, "milestone_tag", MILESTONE, "milestone 39700 product tag"),
    (39699, "continuum_wave", MILESTONE, "milestone 39700 continuum wave identity"),
    # 39700 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        39676: "continuum wave fill soft unit + batch id",
        39680: "continuum bridge soft unit + batch id",
        39690: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (39691–39699), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 39693
    tag_batch = MARKERS_LO + 7  # 39698
    wave_batch = MARKERS_LO + 8  # 39699
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_39600 / gj_shell_green_39500 / gj_shell_green_39400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_39600 / gj_libcgj_green_39500 / gj_libcgj_green_39400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_39600 / gj_bar3_ready_39500 / gj_bar3_ready_39400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_39600 / gj_product_score_39500 / gj_product_score_39400\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_39600 / gj_continuum_ready_39500 /\n"
            f" * gj_continuum_ready_39400 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_39600 / gj_smoke_soft_39500 / gj_smoke_soft_39400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_39600 / gj_dyn_soft_39500 / gj_dyn_soft_39400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_39600 / gj_milestone_tag_39500 / gj_milestone_tag_39400,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_39600 / gj_continuum_wave_39500 /\n"
            f" * gj_continuum_wave_39400, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch39700.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 39699
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 39691–39699; 39700 is capstone
