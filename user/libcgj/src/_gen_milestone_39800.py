...
"""
Soft continuum generator — MILESTONE 39800 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 39701–39800 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=39800   when parent has wired through graph_batch39800.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 39701 39800
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 39701 39800
...
graph batches ending at M. For M = **39800** the exclusive range is
**39701–39800** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 39701–39750  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 39751–39775  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 39776–39790  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 39791–39800  │ decade soft gates (Wave 256 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 39800 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_39800        1          graph_batch39791.c
  gj_libcgj_green_39800       1          graph_batch39792.c
  gj_bar3_ready_39800         0          graph_batch39793.c  (bar3 open)
  gj_product_score_39800      0          graph_batch39794.c  (score open)
  gj_continuum_ready_39800    1          graph_batch39795.c
  gj_smoke_soft_39800         1          graph_batch39796.c
  gj_dyn_soft_39800           1          graph_batch39797.c
  gj_milestone_tag_39800      39800      graph_batch39798.c
  gj_continuum_wave_39800     39800      graph_batch39799.c
  gj_batch_id_39800           39800      graph_batch39800.c
  gj_graph_milestone_39800    39800      graph_batch39800.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=39800`` means parent wired CGJ_SRCS through
    ``graph_batch39800.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_39800`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_39800`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M39800_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_39700`` / ``gj_*_39600`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 256 exclusive CREATE-ONLY lands the **markers** band
  (39791–39800) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_39800.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_39800.py

  # Restrict to marker band only (soft-gate TUs; Wave 256 exclusive):
  python3 user/libcgj/src/_gen_milestone_39800.py --band markers

  # Extension identity only (39701–39750):
  python3 user/libcgj/src/_gen_milestone_39800.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_39800.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_39700.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 39800 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 39800
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 39701
WAVE_HI = 39800  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 39701, 39750
IDENTITY_LO, IDENTITY_HI = 39751, 39775
FILL_LO, FILL_HI = 39776, 39790
MARKERS_LO, MARKERS_HI = 39791, 39800
...
# Stems become gj_<stem>_39800. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (39791, "shell_green", 1, "shell product green extract (wave 39800)"),
    (39792, "libcgj_green", 1, "libcgj product green extract (wave 39800)"),
    (39793, "bar3_ready", 0, "product bar3 readiness gate (wave 39800)"),
    (39794, "product_score", 0, "product readiness score tag (wave 39800)"),
    (39795, "continuum_ready", 1, "continuum readiness gate (wave 39800)"),
    (39796, "smoke_soft", 1, "smoke soft lamp (wave 39800)"),
    (39797, "dyn_soft", 1, "dyn soft lamp (wave 39800)"),
    (39798, "milestone_tag", MILESTONE, "milestone 39800 product tag"),
    (39799, "continuum_wave", MILESTONE, "milestone 39800 continuum wave identity"),
    # 39800 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        39776: "continuum wave fill soft unit + batch id",
        39780: "continuum bridge soft unit + batch id",
        39790: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (39791–39799), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 39793
    tag_batch = MARKERS_LO + 7  # 39798
    wave_batch = MARKERS_LO + 8  # 39799
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_39700 / gj_shell_green_39600 / gj_shell_green_39500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_39700 / gj_libcgj_green_39600 / gj_libcgj_green_39500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_39700 / gj_bar3_ready_39600 / gj_bar3_ready_39500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_39700 / gj_product_score_39600 / gj_product_score_39500\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_39700 / gj_continuum_ready_39600 /\n"
            f" * gj_continuum_ready_39500 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_39700 / gj_smoke_soft_39600 / gj_smoke_soft_39500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_39700 / gj_dyn_soft_39600 / gj_dyn_soft_39500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_39700 / gj_milestone_tag_39600 / gj_milestone_tag_39500,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_39700 / gj_continuum_wave_39600 /\n"
            f" * gj_continuum_wave_39500, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch39800.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 39799
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 39791–39799; 39800 is capstone
