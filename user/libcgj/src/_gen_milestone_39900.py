...
"""
Soft continuum generator — MILESTONE 39900 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 39801–39900 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=39900   when parent has wired through graph_batch39900.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 39801 39900
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 39801 39900
...
graph batches ending at M. For M = **39900** the exclusive range is
**39801–39900** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 39801–39850  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 39851–39875  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 39876–39890  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 39891–39900  │ decade soft gates (Wave 257 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 39900 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_39900        1          graph_batch39891.c
  gj_libcgj_green_39900       1          graph_batch39892.c
  gj_bar3_ready_39900         0          graph_batch39893.c  (bar3 open)
  gj_product_score_39900      0          graph_batch39894.c  (score open)
  gj_continuum_ready_39900    1          graph_batch39895.c
  gj_smoke_soft_39900         1          graph_batch39896.c
  gj_dyn_soft_39900           1          graph_batch39897.c
  gj_milestone_tag_39900      39900      graph_batch39898.c
  gj_continuum_wave_39900     39900      graph_batch39899.c
  gj_batch_id_39900           39900      graph_batch39900.c
  gj_graph_milestone_39900    39900      graph_batch39900.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=39900`` means parent wired CGJ_SRCS through
    ``graph_batch39900.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_39900`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_39900`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M39900_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_39800`` / ``gj_*_39700`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 257 exclusive CREATE-ONLY lands the **markers** band
  (39891–39900) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_39900.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_39900.py

  # Restrict to marker band only (soft-gate TUs; Wave 257 exclusive):
  python3 user/libcgj/src/_gen_milestone_39900.py --band markers

  # Extension identity only (39801–39850):
  python3 user/libcgj/src/_gen_milestone_39900.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_39900.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_39800.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 39900 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 39900
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 39801
WAVE_HI = 39900  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 39801, 39850
IDENTITY_LO, IDENTITY_HI = 39851, 39875
FILL_LO, FILL_HI = 39876, 39890
MARKERS_LO, MARKERS_HI = 39891, 39900
...
# Stems become gj_<stem>_39900. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (39891, "shell_green", 1, "shell product green extract (wave 39900)"),
    (39892, "libcgj_green", 1, "libcgj product green extract (wave 39900)"),
    (39893, "bar3_ready", 0, "product bar3 readiness gate (wave 39900)"),
    (39894, "product_score", 0, "product readiness score tag (wave 39900)"),
    (39895, "continuum_ready", 1, "continuum readiness gate (wave 39900)"),
    (39896, "smoke_soft", 1, "smoke soft lamp (wave 39900)"),
    (39897, "dyn_soft", 1, "dyn soft lamp (wave 39900)"),
    (39898, "milestone_tag", MILESTONE, "milestone 39900 product tag"),
    (39899, "continuum_wave", MILESTONE, "milestone 39900 continuum wave identity"),
    # 39900 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        39876: "continuum wave fill soft unit + batch id",
        39880: "continuum bridge soft unit + batch id",
        39890: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (39891–39899), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 39893
    tag_batch = MARKERS_LO + 7  # 39898
    wave_batch = MARKERS_LO + 8  # 39899
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_39800 / gj_shell_green_39700 / gj_shell_green_39900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_39800 / gj_libcgj_green_39700 / gj_libcgj_green_39900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_39800 / gj_bar3_ready_39700 / gj_bar3_ready_39900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_39800 / gj_product_score_39700 / gj_product_score_39900\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_39800 / gj_continuum_ready_39700 /\n"
            f" * gj_continuum_ready_39600 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_39800 / gj_smoke_soft_39700 / gj_smoke_soft_39900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_39800 / gj_dyn_soft_39700 / gj_dyn_soft_39900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_39800 / gj_milestone_tag_39700 / gj_milestone_tag_39900,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_39800 / gj_continuum_wave_39700 /\n"
            f" * gj_continuum_wave_39600, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch39900.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 39899
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 39891–39899; 39900 is capstone
