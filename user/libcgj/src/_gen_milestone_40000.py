...
"""
Soft continuum generator — MILESTONE 40000 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 39901–40000 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=40000   when parent has wired through graph_batch40000.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 39901 40000
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 39901 40000
...
graph batches ending at M. For M = **40000** the exclusive range is
**39901–40000** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 39901–39950  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 39951–39975  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 39976–39990  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 39991–40000  │ decade soft gates (Wave 258 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 40000 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_40000        1          graph_batch39991.c
  gj_libcgj_green_40000       1          graph_batch39992.c
  gj_bar3_ready_40000         0          graph_batch39993.c  (bar3 open)
  gj_product_score_40000      0          graph_batch39994.c  (score open)
  gj_continuum_ready_40000    1          graph_batch39995.c
  gj_smoke_soft_40000         1          graph_batch39996.c
  gj_dyn_soft_40000           1          graph_batch39997.c
  gj_milestone_tag_40000      40000      graph_batch39998.c
  gj_continuum_wave_40000     40000      graph_batch39999.c
  gj_batch_id_40000           40000      graph_batch40000.c
  gj_graph_milestone_40000    40000      graph_batch40000.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=40000`` means parent wired CGJ_SRCS through
    ``graph_batch40000.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_40000`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_40000`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M40000_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_39900`` / ``gj_*_39800`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 258 exclusive CREATE-ONLY lands the **markers** band
  (39991–40000) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_40000.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_40000.py

  # Restrict to marker band only (soft-gate TUs; Wave 258 exclusive):
  python3 user/libcgj/src/_gen_milestone_40000.py --band markers

  # Extension identity only (39901–39950):
  python3 user/libcgj/src/_gen_milestone_40000.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_40000.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_39900.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 40000 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 40000
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 39901
WAVE_HI = 40000  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 39901, 39950
IDENTITY_LO, IDENTITY_HI = 39951, 39975
FILL_LO, FILL_HI = 39976, 39990
MARKERS_LO, MARKERS_HI = 39991, 40000
...
# Stems become gj_<stem>_40000. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (39991, "shell_green", 1, "shell product green extract (wave 40000)"),
    (39992, "libcgj_green", 1, "libcgj product green extract (wave 40000)"),
    (39993, "bar3_ready", 0, "product bar3 readiness gate (wave 40000)"),
    (39994, "product_score", 0, "product readiness score tag (wave 40000)"),
    (39995, "continuum_ready", 1, "continuum readiness gate (wave 40000)"),
    (39996, "smoke_soft", 1, "smoke soft lamp (wave 40000)"),
    (39997, "dyn_soft", 1, "dyn soft lamp (wave 40000)"),
    (39998, "milestone_tag", MILESTONE, "milestone 40000 product tag"),
    (39999, "continuum_wave", MILESTONE, "milestone 40000 continuum wave identity"),
    # 40000 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        39976: "continuum wave fill soft unit + batch id",
        39980: "continuum bridge soft unit + batch id",
        39990: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (39991–39999), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 39993
    tag_batch = MARKERS_LO + 7  # 39998
    wave_batch = MARKERS_LO + 8  # 39999
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_39900 / gj_shell_green_39800 / gj_shell_green_39700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_39900 / gj_libcgj_green_39800 / gj_libcgj_green_39700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_39900 / gj_bar3_ready_39800 / gj_bar3_ready_39700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_39900 / gj_product_score_39800 / gj_product_score_39700\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_39900 / gj_continuum_ready_39800 /\n"
            f" * gj_continuum_ready_39700 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_39900 / gj_smoke_soft_39800 / gj_smoke_soft_39700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_39900 / gj_dyn_soft_39800 / gj_dyn_soft_39700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_39900 / gj_milestone_tag_39800 / gj_milestone_tag_39700,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_39900 / gj_continuum_wave_39800 /\n"
            f" * gj_continuum_wave_39700, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch40000.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 39999
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 39991–39999; 40000 is capstone
