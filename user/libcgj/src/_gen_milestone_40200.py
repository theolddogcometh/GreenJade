...
"""
Soft continuum generator — MILESTONE 40200 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 40101–40200 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=40200   when parent has wired through graph_batch40200.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 40101 40200
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 40101 40200
...
graph batches ending at M. For M = **40200** the exclusive range is
**40101–40200** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 40101–40150  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 40151–40175  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 40176–40190  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 40191–40200  │ decade soft gates (Wave 260 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 40200 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_40200        1          graph_batch40191.c
  gj_libcgj_green_40200       1          graph_batch40192.c
  gj_bar3_ready_40200         0          graph_batch40193.c  (bar3 open)
  gj_product_score_40200      0          graph_batch40194.c  (score open)
  gj_continuum_ready_40200    1          graph_batch40195.c
  gj_smoke_soft_40200         1          graph_batch40196.c
  gj_dyn_soft_40200           1          graph_batch40197.c
  gj_milestone_tag_40200      40200      graph_batch40198.c
  gj_continuum_wave_40200     40200      graph_batch40199.c
  gj_batch_id_40200           40200      graph_batch40200.c
  gj_graph_milestone_40200    40200      graph_batch40200.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=40200`` means parent wired CGJ_SRCS through
    ``graph_batch40200.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_40200`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_40200`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M40200_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_40100`` / ``gj_*_40000`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 260 exclusive CREATE-ONLY lands the **markers** band
  (40191–40200) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_40200.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_40200.py

  # Restrict to marker band only (soft-gate TUs; Wave 260 exclusive):
  python3 user/libcgj/src/_gen_milestone_40200.py --band markers

  # Extension identity only (40101–40150):
  python3 user/libcgj/src/_gen_milestone_40200.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_40200.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_40100.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 40200 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 40200
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 40101
WAVE_HI = 40200  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 40101, 40150
IDENTITY_LO, IDENTITY_HI = 40151, 40175
FILL_LO, FILL_HI = 40176, 40190
MARKERS_LO, MARKERS_HI = 40191, 40200
...
# Stems become gj_<stem>_40200. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (40191, "shell_green", 1, "shell product green extract (wave 40200)"),
    (40192, "libcgj_green", 1, "libcgj product green extract (wave 40200)"),
    (40193, "bar3_ready", 0, "product bar3 readiness gate (wave 40200)"),
    (40194, "product_score", 0, "product readiness score tag (wave 40200)"),
    (40195, "continuum_ready", 1, "continuum readiness gate (wave 40200)"),
    (40196, "smoke_soft", 1, "smoke soft lamp (wave 40200)"),
    (40197, "dyn_soft", 1, "dyn soft lamp (wave 40200)"),
    (40198, "milestone_tag", MILESTONE, "milestone 40200 product tag"),
    (40199, "continuum_wave", MILESTONE, "milestone 40200 continuum wave identity"),
    # 40200 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        40176: "continuum wave fill soft unit + batch id",
        40180: "continuum bridge soft unit + batch id",
        40190: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (40191–40199), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 40193
    tag_batch = MARKERS_LO + 7  # 40198
    wave_batch = MARKERS_LO + 8  # 40199
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_40100 / gj_shell_green_40000 / gj_shell_green_39900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_40100 / gj_libcgj_green_40000 / gj_libcgj_green_39900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_40100 / gj_bar3_ready_40000 / gj_bar3_ready_39900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_40100 / gj_product_score_40000 / gj_product_score_39900\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_40100 / gj_continuum_ready_40000 /\n"
            f" * gj_continuum_ready_39900 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_40100 / gj_smoke_soft_40000 / gj_smoke_soft_39900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_40100 / gj_dyn_soft_40000 / gj_dyn_soft_39900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_40100 / gj_milestone_tag_40000 / gj_milestone_tag_39900,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_40100 / gj_continuum_wave_40000 /\n"
            f" * gj_continuum_wave_39900, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch40200.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 40199
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 40191–40199; 40200 is capstone
