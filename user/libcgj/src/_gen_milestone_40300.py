...
"""
Soft continuum generator — MILESTONE 40300 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 40201–40300 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=40300   when parent has wired through graph_batch40300.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 40201 40300
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 40201 40300
...
graph batches ending at M. For M = **40300** the exclusive range is
**40201–40300** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 40201–40250  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 40251–40275  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 40276–40290  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 40291–40300  │ decade soft gates (Wave 261 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 40300 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_40300        1          graph_batch40291.c
  gj_libcgj_green_40300       1          graph_batch40292.c
  gj_bar3_ready_40300         0          graph_batch40293.c  (bar3 open)
  gj_product_score_40300      0          graph_batch40294.c  (score open)
  gj_continuum_ready_40300    1          graph_batch40295.c
  gj_smoke_soft_40300         1          graph_batch40296.c
  gj_dyn_soft_40300           1          graph_batch40297.c
  gj_milestone_tag_40300      40300      graph_batch40298.c
  gj_continuum_wave_40300     40300      graph_batch40299.c
  gj_batch_id_40300           40300      graph_batch40300.c
  gj_graph_milestone_40300    40300      graph_batch40300.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=40300`` means parent wired CGJ_SRCS through
    ``graph_batch40300.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_40300`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_40300`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M40300_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_40200`` / ``gj_*_40100`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 261 exclusive CREATE-ONLY lands the **markers** band
  (40291–40300) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_40300.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_40300.py

  # Restrict to marker band only (soft-gate TUs; Wave 261 exclusive):
  python3 user/libcgj/src/_gen_milestone_40300.py --band markers

  # Extension identity only (40201–40250):
  python3 user/libcgj/src/_gen_milestone_40300.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_40300.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_40200.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 40300 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 40300
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 40201
WAVE_HI = 40300  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 40201, 40250
IDENTITY_LO, IDENTITY_HI = 40251, 40275
FILL_LO, FILL_HI = 40276, 40290
MARKERS_LO, MARKERS_HI = 40291, 40300
...
# Stems become gj_<stem>_40300. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (40291, "shell_green", 1, "shell product green extract (wave 40300)"),
    (40292, "libcgj_green", 1, "libcgj product green extract (wave 40300)"),
    (40293, "bar3_ready", 0, "product bar3 readiness gate (wave 40300)"),
    (40294, "product_score", 0, "product readiness score tag (wave 40300)"),
    (40295, "continuum_ready", 1, "continuum readiness gate (wave 40300)"),
    (40296, "smoke_soft", 1, "smoke soft lamp (wave 40300)"),
    (40297, "dyn_soft", 1, "dyn soft lamp (wave 40300)"),
    (40298, "milestone_tag", MILESTONE, "milestone 40300 product tag"),
    (40299, "continuum_wave", MILESTONE, "milestone 40300 continuum wave identity"),
    # 40300 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        40276: "continuum wave fill soft unit + batch id",
        40280: "continuum bridge soft unit + batch id",
        40290: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (40291–40299), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 40293
    tag_batch = MARKERS_LO + 7  # 40298
    wave_batch = MARKERS_LO + 8  # 40299
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_40200 / gj_shell_green_40100 / gj_shell_green_40000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_40200 / gj_libcgj_green_40100 / gj_libcgj_green_40000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_40200 / gj_bar3_ready_40100 / gj_bar3_ready_40000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_40200 / gj_product_score_40100 / gj_product_score_40000\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_40200 / gj_continuum_ready_40100 /\n"
            f" * gj_continuum_ready_40000 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_40200 / gj_smoke_soft_40100 / gj_smoke_soft_40000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_40200 / gj_dyn_soft_40100 / gj_dyn_soft_40000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_40200 / gj_milestone_tag_40100 / gj_milestone_tag_40000,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_40200 / gj_continuum_wave_40100 /\n"
            f" * gj_continuum_wave_40000, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch40300.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 40299
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 40291–40299; 40300 is capstone
