...
"""
Soft continuum generator — MILESTONE 40400 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 40301–40400 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=40400   when parent has wired through graph_batch40400.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 40301 40400
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 40301 40400
...
graph batches ending at M. For M = **40400** the exclusive range is
**40301–40400** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 40301–40350  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 40351–40375  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 40376–40390  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 40391–40400  │ decade soft gates (Wave 262 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 40400 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_40400        1          graph_batch40391.c
  gj_libcgj_green_40400       1          graph_batch40392.c
  gj_bar3_ready_40400         0          graph_batch40393.c  (bar3 open)
  gj_product_score_40400      0          graph_batch40394.c  (score open)
  gj_continuum_ready_40400    1          graph_batch40395.c
  gj_smoke_soft_40400         1          graph_batch40396.c
  gj_dyn_soft_40400           1          graph_batch40397.c
  gj_milestone_tag_40400      40400      graph_batch40398.c
  gj_continuum_wave_40400     40400      graph_batch40399.c
  gj_batch_id_40400           40400      graph_batch40400.c
  gj_graph_milestone_40400    40400      graph_batch40400.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=40400`` means parent wired CGJ_SRCS through
    ``graph_batch40400.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_40400`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_40400`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M40400_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_40300`` / ``gj_*_40200`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 262 exclusive CREATE-ONLY lands the **markers** band
  (40391–40400) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_40400.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_40400.py

  # Restrict to marker band only (soft-gate TUs; Wave 262 exclusive):
  python3 user/libcgj/src/_gen_milestone_40400.py --band markers

  # Extension identity only (40301–40350):
  python3 user/libcgj/src/_gen_milestone_40400.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_40400.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_40300.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 40400 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 40400
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 40301
WAVE_HI = 40400  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 40301, 40350
IDENTITY_LO, IDENTITY_HI = 40351, 40375
FILL_LO, FILL_HI = 40376, 40390
MARKERS_LO, MARKERS_HI = 40391, 40400
...
# Stems become gj_<stem>_40400. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (40391, "shell_green", 1, "shell product green extract (wave 40400)"),
    (40392, "libcgj_green", 1, "libcgj product green extract (wave 40400)"),
    (40393, "bar3_ready", 0, "product bar3 readiness gate (wave 40400)"),
    (40394, "product_score", 0, "product readiness score tag (wave 40400)"),
    (40395, "continuum_ready", 1, "continuum readiness gate (wave 40400)"),
    (40396, "smoke_soft", 1, "smoke soft lamp (wave 40400)"),
    (40397, "dyn_soft", 1, "dyn soft lamp (wave 40400)"),
    (40398, "milestone_tag", MILESTONE, "milestone 40400 product tag"),
    (40399, "continuum_wave", MILESTONE, "milestone 40400 continuum wave identity"),
    # 40400 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        40376: "continuum wave fill soft unit + batch id",
        40380: "continuum bridge soft unit + batch id",
        40390: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (40391–40399), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 40393
    tag_batch = MARKERS_LO + 7  # 40398
    wave_batch = MARKERS_LO + 8  # 40399
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_40300 / gj_shell_green_40200 / gj_shell_green_40100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_40300 / gj_libcgj_green_40200 / gj_libcgj_green_40100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_40300 / gj_bar3_ready_40200 / gj_bar3_ready_40100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_40300 / gj_product_score_40200 / gj_product_score_40100\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_40300 / gj_continuum_ready_40200 /\n"
            f" * gj_continuum_ready_40100 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_40300 / gj_smoke_soft_40200 / gj_smoke_soft_40100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_40300 / gj_dyn_soft_40200 / gj_dyn_soft_40100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_40300 / gj_milestone_tag_40200 / gj_milestone_tag_40100,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_40300 / gj_continuum_wave_40200 /\n"
            f" * gj_continuum_wave_40100, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch40400.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 40399
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 40391–40399; 40400 is capstone
