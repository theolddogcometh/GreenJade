...
"""
Soft continuum generator — MILESTONE 40500 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 40401–40500 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=40500   when parent has wired through graph_batch40500.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 40401 40500
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 40401 40500
...
graph batches ending at M. For M = **40500** the exclusive range is
**40401–40500** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 40401–40450  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 40451–40475  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 40476–40490  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 40491–40500  │ decade soft gates (Wave 263 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 40500 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_40500        1          graph_batch40491.c
  gj_libcgj_green_40500       1          graph_batch40492.c
  gj_bar3_ready_40500         0          graph_batch40493.c  (bar3 open)
  gj_product_score_40500      0          graph_batch40494.c  (score open)
  gj_continuum_ready_40500    1          graph_batch40495.c
  gj_smoke_soft_40500         1          graph_batch40496.c
  gj_dyn_soft_40500           1          graph_batch40497.c
  gj_milestone_tag_40500      40500      graph_batch40498.c
  gj_continuum_wave_40500     40500      graph_batch40499.c
  gj_batch_id_40500           40500      graph_batch40500.c
  gj_graph_milestone_40500    40500      graph_batch40500.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=40500`` means parent wired CGJ_SRCS through
    ``graph_batch40500.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_40500`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_40500`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M40500_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_40400`` / ``gj_*_40300`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 263 exclusive CREATE-ONLY lands the **markers** band
  (40491–40500) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_40500.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_40500.py

  # Restrict to marker band only (soft-gate TUs; Wave 263 exclusive):
  python3 user/libcgj/src/_gen_milestone_40500.py --band markers

  # Extension identity only (40401–40450):
  python3 user/libcgj/src/_gen_milestone_40500.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_40500.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_40400.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 40500 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 40500
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 40401
WAVE_HI = 40500  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 40401, 40450
IDENTITY_LO, IDENTITY_HI = 40451, 40475
FILL_LO, FILL_HI = 40476, 40490
MARKERS_LO, MARKERS_HI = 40491, 40500
...
# Stems become gj_<stem>_40500. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (40491, "shell_green", 1, "shell product green extract (wave 40500)"),
    (40492, "libcgj_green", 1, "libcgj product green extract (wave 40500)"),
    (40493, "bar3_ready", 0, "product bar3 readiness gate (wave 40500)"),
    (40494, "product_score", 0, "product readiness score tag (wave 40500)"),
    (40495, "continuum_ready", 1, "continuum readiness gate (wave 40500)"),
    (40496, "smoke_soft", 1, "smoke soft lamp (wave 40500)"),
    (40497, "dyn_soft", 1, "dyn soft lamp (wave 40500)"),
    (40498, "milestone_tag", MILESTONE, "milestone 40500 product tag"),
    (40499, "continuum_wave", MILESTONE, "milestone 40500 continuum wave identity"),
    # 40500 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        40476: "continuum wave fill soft unit + batch id",
        40480: "continuum bridge soft unit + batch id",
        40490: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (40491–40499), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 40493
    tag_batch = MARKERS_LO + 7  # 40498
    wave_batch = MARKERS_LO + 8  # 40499
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_40400 / gj_shell_green_40300 / gj_shell_green_40200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_40400 / gj_libcgj_green_40300 / gj_libcgj_green_40200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_40400 / gj_bar3_ready_40300 / gj_bar3_ready_40200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_40400 / gj_product_score_40300 / gj_product_score_40200\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_40400 / gj_continuum_ready_40300 /\n"
            f" * gj_continuum_ready_40200 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_40400 / gj_smoke_soft_40300 / gj_smoke_soft_40200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_40400 / gj_dyn_soft_40300 / gj_dyn_soft_40200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_40400 / gj_milestone_tag_40300 / gj_milestone_tag_40200,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_40400 / gj_continuum_wave_40300 /\n"
            f" * gj_continuum_wave_40200, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    ...
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 40499
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 40491–40499; 40500 is capstone
