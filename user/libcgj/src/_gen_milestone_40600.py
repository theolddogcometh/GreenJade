...
"""
Soft continuum generator — MILESTONE 40600 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 40501–40600 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=40600   when parent has wired through graph_batch40600.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 40501 40600
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 40501 40600
...
graph batches ending at M. For M = **40600** the exclusive range is
**40501–40600** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 40501–40550  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 40551–40575  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 40576–40590  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 40591–40600  │ decade soft gates (Wave 264 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 40600 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_40600        1          graph_batch40591.c
  gj_libcgj_green_40600       1          graph_batch40592.c
  gj_bar3_ready_40600         0          graph_batch40593.c  (bar3 open)
  gj_product_score_40600      0          graph_batch40594.c  (score open)
  gj_continuum_ready_40600    1          graph_batch40595.c
  gj_smoke_soft_40600         1          graph_batch40596.c
  gj_dyn_soft_40600           1          graph_batch40597.c
  gj_milestone_tag_40600      40600      graph_batch40598.c
  gj_continuum_wave_40600     40600      graph_batch40599.c
  gj_batch_id_40600           40600      graph_batch40600.c
  gj_graph_milestone_40600    40600      graph_batch40600.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=40600`` means parent wired CGJ_SRCS through
    ``graph_batch40600.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_40600`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_40600`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M40600_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_40500`` / ``gj_*_40400`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 264 exclusive CREATE-ONLY lands the **markers** band
  (40591–40600) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_40600.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_40600.py

  # Restrict to marker band only (soft-gate TUs; Wave 264 exclusive):
  python3 user/libcgj/src/_gen_milestone_40600.py --band markers

  # Extension identity only (40501–40550):
  python3 user/libcgj/src/_gen_milestone_40600.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_40600.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_40500.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 40600 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 40600
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 40501
WAVE_HI = 40600  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 40501, 40550
IDENTITY_LO, IDENTITY_HI = 40551, 40575
FILL_LO, FILL_HI = 40576, 40590
MARKERS_LO, MARKERS_HI = 40591, 40600
...
# Stems become gj_<stem>_40600. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (40591, "shell_green", 1, "shell product green extract (wave 40600)"),
    (40592, "libcgj_green", 1, "libcgj product green extract (wave 40600)"),
    (40593, "bar3_ready", 0, "product bar3 readiness gate (wave 40600)"),
    (40594, "product_score", 0, "product readiness score tag (wave 40600)"),
    (40595, "continuum_ready", 1, "continuum readiness gate (wave 40600)"),
    (40596, "smoke_soft", 1, "smoke soft lamp (wave 40600)"),
    (40597, "dyn_soft", 1, "dyn soft lamp (wave 40600)"),
    (40598, "milestone_tag", MILESTONE, "milestone 40600 product tag"),
    (40599, "continuum_wave", MILESTONE, "milestone 40600 continuum wave identity"),
    # 40600 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        40576: "continuum wave fill soft unit + batch id",
        40580: "continuum bridge soft unit + batch id",
        40590: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (40591–40599), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 40593
    tag_batch = MARKERS_LO + 7  # 40598
    wave_batch = MARKERS_LO + 8  # 40599
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_40500 / gj_shell_green_40400 / gj_shell_green_40300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_40500 / gj_libcgj_green_40400 / gj_libcgj_green_40300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_40500 / gj_bar3_ready_40400 / gj_bar3_ready_40300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_40500 / gj_product_score_40400 / gj_product_score_40300\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_40500 / gj_continuum_ready_40400 /\n"
            f" * gj_continuum_ready_40300 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_40500 / gj_smoke_soft_40400 / gj_smoke_soft_40300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_40500 / gj_dyn_soft_40400 / gj_dyn_soft_40300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_40500 / gj_milestone_tag_40400 / gj_milestone_tag_40300,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_40500 / gj_continuum_wave_40400 /\n"
            f" * gj_continuum_wave_40300, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch40600.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 40599
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 40591–40599; 40600 is capstone
