...
"""
Soft continuum generator — MILESTONE 40700 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 40601–40700 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=40700   when parent has wired through graph_batch40700.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 40601 40700
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 40601 40700
...
graph batches ending at M. For M = **40700** the exclusive range is
**40601–40700** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 40601–40650  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 40651–40675  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 40676–40690  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 40691–40700  │ decade soft gates (Wave 265 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 40700 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_40700        1          graph_batch40691.c
  gj_libcgj_green_40700       1          graph_batch40692.c
  gj_bar3_ready_40700         0          graph_batch40693.c  (bar3 open)
  gj_product_score_40700      0          graph_batch40694.c  (score open)
  gj_continuum_ready_40700    1          graph_batch40695.c
  gj_smoke_soft_40700         1          graph_batch40696.c
  gj_dyn_soft_40700           1          graph_batch40697.c
  gj_milestone_tag_40700      40700      graph_batch40698.c
  gj_continuum_wave_40700     40700      graph_batch40699.c
  gj_batch_id_40700           40700      graph_batch40700.c
  gj_graph_milestone_40700    40700      graph_batch40700.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=40700`` means parent wired CGJ_SRCS through
    ``graph_batch40700.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_40700`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_40700`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M40700_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_40600`` / ``gj_*_40500`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 265 exclusive CREATE-ONLY lands the **markers** band
  (40691–40700) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_40700.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_40700.py

  # Restrict to marker band only (soft-gate TUs; Wave 265 exclusive):
  python3 user/libcgj/src/_gen_milestone_40700.py --band markers

  # Extension identity only (40601–40650):
  python3 user/libcgj/src/_gen_milestone_40700.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_40700.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_40600.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 40700 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 40700
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 40601
WAVE_HI = 40700  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 40601, 40650
IDENTITY_LO, IDENTITY_HI = 40651, 40675
FILL_LO, FILL_HI = 40676, 40690
MARKERS_LO, MARKERS_HI = 40691, 40700
...
# Stems become gj_<stem>_40700. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (40691, "shell_green", 1, "shell product green extract (wave 40700)"),
    (40692, "libcgj_green", 1, "libcgj product green extract (wave 40700)"),
    (40693, "bar3_ready", 0, "product bar3 readiness gate (wave 40700)"),
    (40694, "product_score", 0, "product readiness score tag (wave 40700)"),
    (40695, "continuum_ready", 1, "continuum readiness gate (wave 40700)"),
    (40696, "smoke_soft", 1, "smoke soft lamp (wave 40700)"),
    (40697, "dyn_soft", 1, "dyn soft lamp (wave 40700)"),
    (40698, "milestone_tag", MILESTONE, "milestone 40700 product tag"),
    (40699, "continuum_wave", MILESTONE, "milestone 40700 continuum wave identity"),
    # 40700 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        40676: "continuum wave fill soft unit + batch id",
        40680: "continuum bridge soft unit + batch id",
        40690: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (40691–40699), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 40693
    tag_batch = MARKERS_LO + 7  # 40698
    wave_batch = MARKERS_LO + 8  # 40699
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_40600 / gj_shell_green_40500 / gj_shell_green_40400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_40600 / gj_libcgj_green_40500 / gj_libcgj_green_40400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_40600 / gj_bar3_ready_40500 / gj_bar3_ready_40400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_40600 / gj_product_score_40500 / gj_product_score_40400\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_40600 / gj_continuum_ready_40500 /\n"
            f" * gj_continuum_ready_40400 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_40600 / gj_smoke_soft_40500 / gj_smoke_soft_40400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_40600 / gj_dyn_soft_40500 / gj_dyn_soft_40400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_40600 / gj_milestone_tag_40500 / gj_milestone_tag_40400,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_40600 / gj_continuum_wave_40500 /\n"
            f" * gj_continuum_wave_40400, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch40700.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 40699
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 40691–40699; 40700 is capstone
