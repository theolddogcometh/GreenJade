...
"""
Soft continuum generator — MILESTONE 40800 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 40701–40800 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=40800   when parent has wired through graph_batch40800.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 40701 40800
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 40701 40800
...
graph batches ending at M. For M = **40800** the exclusive range is
**40701–40800** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 40701–40750  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 40751–40775  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 40776–40790  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 40791–40800  │ decade soft gates (Wave 266 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 40800 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_40800        1          graph_batch40791.c
  gj_libcgj_green_40800       1          graph_batch40792.c
  gj_bar3_ready_40800         0          graph_batch40793.c  (bar3 open)
  gj_product_score_40800      0          graph_batch40794.c  (score open)
  gj_continuum_ready_40800    1          graph_batch40795.c
  gj_smoke_soft_40800         1          graph_batch40796.c
  gj_dyn_soft_40800           1          graph_batch40797.c
  gj_milestone_tag_40800      40800      graph_batch40798.c
  gj_continuum_wave_40800     40800      graph_batch40799.c
  gj_batch_id_40800           40800      graph_batch40800.c
  gj_graph_milestone_40800    40800      graph_batch40800.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=40800`` means parent wired CGJ_SRCS through
    ``graph_batch40800.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_40800`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_40800`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M40800_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_40700`` / ``gj_*_40600`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 266 exclusive CREATE-ONLY lands the **markers** band
  (40791–40800) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_40800.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_40800.py

  # Restrict to marker band only (soft-gate TUs; Wave 266 exclusive):
  python3 user/libcgj/src/_gen_milestone_40800.py --band markers

  # Extension identity only (40701–40750):
  python3 user/libcgj/src/_gen_milestone_40800.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_40800.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_40700.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 40800 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 40800
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 40701
WAVE_HI = 40800  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 40701, 40750
IDENTITY_LO, IDENTITY_HI = 40751, 40775
FILL_LO, FILL_HI = 40776, 40790
MARKERS_LO, MARKERS_HI = 40791, 40800
...
# Stems become gj_<stem>_40800. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (40791, "shell_green", 1, "shell product green extract (wave 40800)"),
    (40792, "libcgj_green", 1, "libcgj product green extract (wave 40800)"),
    (40793, "bar3_ready", 0, "product bar3 readiness gate (wave 40800)"),
    (40794, "product_score", 0, "product readiness score tag (wave 40800)"),
    (40795, "continuum_ready", 1, "continuum readiness gate (wave 40800)"),
    (40796, "smoke_soft", 1, "smoke soft lamp (wave 40800)"),
    (40797, "dyn_soft", 1, "dyn soft lamp (wave 40800)"),
    (40798, "milestone_tag", MILESTONE, "milestone 40800 product tag"),
    (40799, "continuum_wave", MILESTONE, "milestone 40800 continuum wave identity"),
    # 40800 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        40776: "continuum wave fill soft unit + batch id",
        40780: "continuum bridge soft unit + batch id",
        40790: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (40791–40799), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 40793
    tag_batch = MARKERS_LO + 7  # 40798
    wave_batch = MARKERS_LO + 8  # 40799
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_40700 / gj_shell_green_40600 / gj_shell_green_40500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_40700 / gj_libcgj_green_40600 / gj_libcgj_green_40500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_40700 / gj_bar3_ready_40600 / gj_bar3_ready_40500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_40700 / gj_product_score_40600 / gj_product_score_40500\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_40700 / gj_continuum_ready_40600 /\n"
            f" * gj_continuum_ready_40500 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_40700 / gj_smoke_soft_40600 / gj_smoke_soft_40500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_40700 / gj_dyn_soft_40600 / gj_dyn_soft_40500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_40700 / gj_milestone_tag_40600 / gj_milestone_tag_40500,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_40700 / gj_continuum_wave_40600 /\n"
            f" * gj_continuum_wave_40500, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch40800.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 40799
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 40791–40799; 40800 is capstone
