...
"""
Soft continuum generator — MILESTONE 40900 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 40801–40900 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=40900   when parent has wired through graph_batch40900.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 40801 40900
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 40801 40900
...
graph batches ending at M. For M = **40900** the exclusive range is
**40801–40900** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 40801–40850  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 40851–40875  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 40876–40890  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 40891–40900  │ decade soft gates (Wave 267 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 40900 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_40900        1          graph_batch40891.c
  gj_libcgj_green_40900       1          graph_batch40892.c
  gj_bar3_ready_40900         0          graph_batch40893.c  (bar3 open)
  gj_product_score_40900      0          graph_batch40894.c  (score open)
  gj_continuum_ready_40900    1          graph_batch40895.c
  gj_smoke_soft_40900         1          graph_batch40896.c
  gj_dyn_soft_40900           1          graph_batch40897.c
  gj_milestone_tag_40900      40900      graph_batch40898.c
  gj_continuum_wave_40900     40900      graph_batch40899.c
  gj_batch_id_40900           40900      graph_batch40900.c
  gj_graph_milestone_40900    40900      graph_batch40900.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=40900`` means parent wired CGJ_SRCS through
    ``graph_batch40900.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_40900`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_40900`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M40900_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_40800`` / ``gj_*_40700`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 267 exclusive CREATE-ONLY lands the **markers** band
  (40891–40900) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_40900.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_40900.py

  # Restrict to marker band only (soft-gate TUs; Wave 267 exclusive):
  python3 user/libcgj/src/_gen_milestone_40900.py --band markers

  # Extension identity only (40801–40850):
  python3 user/libcgj/src/_gen_milestone_40900.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_40900.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_40800.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 40900 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 40900
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 40801
WAVE_HI = 40900  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 40801, 40850
IDENTITY_LO, IDENTITY_HI = 40851, 40875
FILL_LO, FILL_HI = 40876, 40890
MARKERS_LO, MARKERS_HI = 40891, 40900
...
# Stems become gj_<stem>_40900. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (40891, "shell_green", 1, "shell product green extract (wave 40900)"),
    (40892, "libcgj_green", 1, "libcgj product green extract (wave 40900)"),
    (40893, "bar3_ready", 0, "product bar3 readiness gate (wave 40900)"),
    (40894, "product_score", 0, "product readiness score tag (wave 40900)"),
    (40895, "continuum_ready", 1, "continuum readiness gate (wave 40900)"),
    (40896, "smoke_soft", 1, "smoke soft lamp (wave 40900)"),
    (40897, "dyn_soft", 1, "dyn soft lamp (wave 40900)"),
    (40898, "milestone_tag", MILESTONE, "milestone 40900 product tag"),
    (40899, "continuum_wave", MILESTONE, "milestone 40900 continuum wave identity"),
    # 40900 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        40876: "continuum wave fill soft unit + batch id",
        40880: "continuum bridge soft unit + batch id",
        40890: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (40891–40899), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 40893
    tag_batch = MARKERS_LO + 7  # 40898
    wave_batch = MARKERS_LO + 8  # 40899
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_40800 / gj_shell_green_40700 / gj_shell_green_40600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_40800 / gj_libcgj_green_40700 / gj_libcgj_green_40600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_40800 / gj_bar3_ready_40700 / gj_bar3_ready_40600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_40800 / gj_product_score_40700 / gj_product_score_40600\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_40800 / gj_continuum_ready_40700 /\n"
            f" * gj_continuum_ready_40600 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_40800 / gj_smoke_soft_40700 / gj_smoke_soft_40600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_40800 / gj_dyn_soft_40700 / gj_dyn_soft_40600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_40800 / gj_milestone_tag_40700 / gj_milestone_tag_40600,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_40800 / gj_continuum_wave_40700 /\n"
            f" * gj_continuum_wave_40600, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch40900.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 40899
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 40891–40899; 40900 is capstone
