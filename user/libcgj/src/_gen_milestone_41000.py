...
"""
Soft continuum generator — MILESTONE 41000 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 40901–41000 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=41000   when parent has wired through graph_batch41000.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 40901 41000
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 40901 41000
...
graph batches ending at M. For M = **41000** the exclusive range is
**40901–41000** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 40901–40950  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 40951–40975  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 40976–40990  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 40991–41000  │ decade soft gates (Wave 268 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 41000 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_41000        1          graph_batch40991.c
  gj_libcgj_green_41000       1          graph_batch40992.c
  gj_bar3_ready_41000         0          graph_batch40993.c  (bar3 open)
  gj_product_score_41000      0          graph_batch40994.c  (score open)
  gj_continuum_ready_41000    1          graph_batch40995.c
  gj_smoke_soft_41000         1          graph_batch40996.c
  gj_dyn_soft_41000           1          graph_batch40997.c
  gj_milestone_tag_41000      41000      graph_batch40998.c
  gj_continuum_wave_41000     41000      graph_batch40999.c
  gj_batch_id_41000           41000      graph_batch41000.c
  gj_graph_milestone_41000    41000      graph_batch41000.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=41000`` means parent wired CGJ_SRCS through
    ``graph_batch41000.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_41000`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_41000`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M41000_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_40900`` / ``gj_*_40800`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 268 exclusive CREATE-ONLY lands the **markers** band
  (40991–41000) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_41000.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_41000.py

  # Restrict to marker band only (soft-gate TUs; Wave 268 exclusive):
  python3 user/libcgj/src/_gen_milestone_41000.py --band markers

  # Extension identity only (40901–40950):
  python3 user/libcgj/src/_gen_milestone_41000.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_41000.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_40900.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 41000 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 41000
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 40901
WAVE_HI = 41000  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 40901, 40950
IDENTITY_LO, IDENTITY_HI = 40951, 40975
FILL_LO, FILL_HI = 40976, 40990
MARKERS_LO, MARKERS_HI = 40991, 41000
...
# Stems become gj_<stem>_41000. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (40991, "shell_green", 1, "shell product green extract (wave 41000)"),
    (40992, "libcgj_green", 1, "libcgj product green extract (wave 41000)"),
    (40993, "bar3_ready", 0, "product bar3 readiness gate (wave 41000)"),
    (40994, "product_score", 0, "product readiness score tag (wave 41000)"),
    (40995, "continuum_ready", 1, "continuum readiness gate (wave 41000)"),
    (40996, "smoke_soft", 1, "smoke soft lamp (wave 41000)"),
    (40997, "dyn_soft", 1, "dyn soft lamp (wave 41000)"),
    (40998, "milestone_tag", MILESTONE, "milestone 41000 product tag"),
    (40999, "continuum_wave", MILESTONE, "milestone 41000 continuum wave identity"),
    # 41000 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        40976: "continuum wave fill soft unit + batch id",
        40980: "continuum bridge soft unit + batch id",
        40990: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (40991–40999), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 40993
    tag_batch = MARKERS_LO + 7  # 40998
    wave_batch = MARKERS_LO + 8  # 40999
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_40900 / gj_shell_green_40800 / gj_shell_green_40700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_40900 / gj_libcgj_green_40800 / gj_libcgj_green_40700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_40900 / gj_bar3_ready_40800 / gj_bar3_ready_40700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_40900 / gj_product_score_40800 / gj_product_score_40700\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_40900 / gj_continuum_ready_40800 /\n"
            f" * gj_continuum_ready_40700 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_40900 / gj_smoke_soft_40800 / gj_smoke_soft_40700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_40900 / gj_dyn_soft_40800 / gj_dyn_soft_40700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_40900 / gj_milestone_tag_40800 / gj_milestone_tag_40700,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_40900 / gj_continuum_wave_40800 /\n"
            f" * gj_continuum_wave_40700, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch41000.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 40999
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 40991–40999; 41000 is capstone
