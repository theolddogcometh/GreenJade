...
"""
Soft continuum generator — MILESTONE 41100 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 41001–41100 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=41100   when parent has wired through graph_batch41100.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 41001 41100
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 41001 41100
...
graph batches ending at M. For M = **41100** the exclusive range is
**41001–41100** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 41001–41050  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 41051–41075  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 41076–41090  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 41091–41100  │ decade soft gates (Wave 269 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 41100 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_41100        1          graph_batch41091.c
  gj_libcgj_green_41100       1          graph_batch41092.c
  gj_bar3_ready_41100         0          graph_batch41093.c  (bar3 open)
  gj_product_score_41100      0          graph_batch41094.c  (score open)
  gj_continuum_ready_41100    1          graph_batch41095.c
  gj_smoke_soft_41100         1          graph_batch41096.c
  gj_dyn_soft_41100           1          graph_batch41097.c
  gj_milestone_tag_41100      41100      graph_batch41098.c
  gj_continuum_wave_41100     41100      graph_batch41099.c
  gj_batch_id_41100           41100      graph_batch41100.c
  gj_graph_milestone_41100    41100      graph_batch41100.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=41100`` means parent wired CGJ_SRCS through
    ``graph_batch41100.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_41100`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_41100`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M41100_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_41000`` / ``gj_*_40900`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 269 exclusive CREATE-ONLY lands the **markers** band
  (41091–41100) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_41100.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_41100.py

  # Restrict to marker band only (soft-gate TUs; Wave 269 exclusive):
  python3 user/libcgj/src/_gen_milestone_41100.py --band markers

  # Extension identity only (41001–41050):
  python3 user/libcgj/src/_gen_milestone_41100.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_41100.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_41000.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 41100 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 41100
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 41001
WAVE_HI = 41100  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 41001, 41050
IDENTITY_LO, IDENTITY_HI = 41051, 41075
FILL_LO, FILL_HI = 41076, 41090
MARKERS_LO, MARKERS_HI = 41091, 41100
...
# Stems become gj_<stem>_41100. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (41091, "shell_green", 1, "shell product green extract (wave 41100)"),
    (41092, "libcgj_green", 1, "libcgj product green extract (wave 41100)"),
    (41093, "bar3_ready", 0, "product bar3 readiness gate (wave 41100)"),
    (41094, "product_score", 0, "product readiness score tag (wave 41100)"),
    (41095, "continuum_ready", 1, "continuum readiness gate (wave 41100)"),
    (41096, "smoke_soft", 1, "smoke soft lamp (wave 41100)"),
    (41097, "dyn_soft", 1, "dyn soft lamp (wave 41100)"),
    (41098, "milestone_tag", MILESTONE, "milestone 41100 product tag"),
    (41099, "continuum_wave", MILESTONE, "milestone 41100 continuum wave identity"),
    # 41100 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        41076: "continuum wave fill soft unit + batch id",
        41080: "continuum bridge soft unit + batch id",
        41090: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (41091–41099), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 41093
    tag_batch = MARKERS_LO + 7  # 41098
    wave_batch = MARKERS_LO + 8  # 41099
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_41000 / gj_shell_green_40900 / gj_shell_green_40800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_41000 / gj_libcgj_green_40900 / gj_libcgj_green_40800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_41000 / gj_bar3_ready_40900 / gj_bar3_ready_40800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_41000 / gj_product_score_40900 / gj_product_score_40800\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_41000 / gj_continuum_ready_40900 /\n"
            f" * gj_continuum_ready_40800 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_41000 / gj_smoke_soft_40900 / gj_smoke_soft_40800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_41000 / gj_dyn_soft_40900 / gj_dyn_soft_40800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_41000 / gj_milestone_tag_40900 / gj_milestone_tag_40800,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_41000 / gj_continuum_wave_40900 /\n"
            f" * gj_continuum_wave_40800, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch41100.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 41099
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 41091–41099; 41100 is capstone
