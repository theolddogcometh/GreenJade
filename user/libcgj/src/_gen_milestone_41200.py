...
"""
Soft continuum generator — MILESTONE 41200 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 41101–41200 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=41200   when parent has wired through graph_batch41200.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 41101 41200
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 41101 41200
...
graph batches ending at M. For M = **41200** the exclusive range is
**41101–41200** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 41101–41150  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 41151–41175  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 41176–41190  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 41191–41200  │ decade soft gates (Wave 270 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 41200 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_41200        1          graph_batch41191.c
  gj_libcgj_green_41200       1          graph_batch41192.c
  gj_bar3_ready_41200         0          graph_batch41193.c  (bar3 open)
  gj_product_score_41200      0          graph_batch41194.c  (score open)
  gj_continuum_ready_41200    1          graph_batch41195.c
  gj_smoke_soft_41200         1          graph_batch41196.c
  gj_dyn_soft_41200           1          graph_batch41197.c
  gj_milestone_tag_41200      41200      graph_batch41198.c
  gj_continuum_wave_41200     41200      graph_batch41199.c
  gj_batch_id_41200           41200      graph_batch41200.c
  gj_graph_milestone_41200    41200      graph_batch41200.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=41200`` means parent wired CGJ_SRCS through
    ``graph_batch41200.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_41200`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_41200`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M41200_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_41100`` / ``gj_*_41000`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 270 exclusive CREATE-ONLY lands the **markers** band
  (41191–41200) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_41200.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_41200.py

  # Restrict to marker band only (soft-gate TUs; Wave 270 exclusive):
  python3 user/libcgj/src/_gen_milestone_41200.py --band markers

  # Extension identity only (41101–41150):
  python3 user/libcgj/src/_gen_milestone_41200.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_41200.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_41100.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 41200 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 41200
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 41101
WAVE_HI = 41200  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 41101, 41150
IDENTITY_LO, IDENTITY_HI = 41151, 41175
FILL_LO, FILL_HI = 41176, 41190
MARKERS_LO, MARKERS_HI = 41191, 41200
...
# Stems become gj_<stem>_41200. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (41191, "shell_green", 1, "shell product green extract (wave 41200)"),
    (41192, "libcgj_green", 1, "libcgj product green extract (wave 41200)"),
    (41193, "bar3_ready", 0, "product bar3 readiness gate (wave 41200)"),
    (41194, "product_score", 0, "product readiness score tag (wave 41200)"),
    (41195, "continuum_ready", 1, "continuum readiness gate (wave 41200)"),
    (41196, "smoke_soft", 1, "smoke soft lamp (wave 41200)"),
    (41197, "dyn_soft", 1, "dyn soft lamp (wave 41200)"),
    (41198, "milestone_tag", MILESTONE, "milestone 41200 product tag"),
    (41199, "continuum_wave", MILESTONE, "milestone 41200 continuum wave identity"),
    # 41200 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        41176: "continuum wave fill soft unit + batch id",
        41180: "continuum bridge soft unit + batch id",
        41190: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (41191–41199), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 41193
    tag_batch = MARKERS_LO + 7  # 41198
    wave_batch = MARKERS_LO + 8  # 41199
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_41100 / gj_shell_green_41000 / gj_shell_green_40900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_41100 / gj_libcgj_green_41000 / gj_libcgj_green_40900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_41100 / gj_bar3_ready_41000 / gj_bar3_ready_40900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_41100 / gj_product_score_41000 / gj_product_score_40900\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_41100 / gj_continuum_ready_41000 /\n"
            f" * gj_continuum_ready_40900 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_41100 / gj_smoke_soft_41000 / gj_smoke_soft_40900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_41100 / gj_dyn_soft_41000 / gj_dyn_soft_40900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_41100 / gj_milestone_tag_41000 / gj_milestone_tag_40900,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_41100 / gj_continuum_wave_41000 /\n"
            f" * gj_continuum_wave_40900, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch41200.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 41199
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 41191–41199; 41200 is capstone
