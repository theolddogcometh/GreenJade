...
"""
Soft continuum generator — MILESTONE 41300 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 41201–41300 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=41300   when parent has wired through graph_batch41300.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 41201 41300
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 41201 41300
...
graph batches ending at M. For M = **41300** the exclusive range is
**41201–41300** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 41201–41250  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 41251–41275  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 41276–41290  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 41291–41300  │ decade soft gates (Wave 271 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 41300 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_41300        1          graph_batch41291.c
  gj_libcgj_green_41300       1          graph_batch41292.c
  gj_bar3_ready_41300         0          graph_batch41293.c  (bar3 open)
  gj_product_score_41300      0          graph_batch41294.c  (score open)
  gj_continuum_ready_41300    1          graph_batch41295.c
  gj_smoke_soft_41300         1          graph_batch41296.c
  gj_dyn_soft_41300           1          graph_batch41297.c
  gj_milestone_tag_41300      41300      graph_batch41298.c
  gj_continuum_wave_41300     41300      graph_batch41299.c
  gj_batch_id_41300           41300      graph_batch41300.c
  gj_graph_milestone_41300    41300      graph_batch41300.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=41300`` means parent wired CGJ_SRCS through
    ``graph_batch41300.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_41300`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_41300`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M41300_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_41200`` / ``gj_*_41100`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 271 exclusive CREATE-ONLY lands the **markers** band
  (41291–41300) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_41300.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_41300.py

  # Restrict to marker band only (soft-gate TUs; Wave 271 exclusive):
  python3 user/libcgj/src/_gen_milestone_41300.py --band markers

  # Extension identity only (41201–41250):
  python3 user/libcgj/src/_gen_milestone_41300.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_41300.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_41200.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 41300 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 41300
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 41201
WAVE_HI = 41300  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 41201, 41250
IDENTITY_LO, IDENTITY_HI = 41251, 41275
FILL_LO, FILL_HI = 41276, 41290
MARKERS_LO, MARKERS_HI = 41291, 41300
...
# Stems become gj_<stem>_41300. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (41291, "shell_green", 1, "shell product green extract (wave 41300)"),
    (41292, "libcgj_green", 1, "libcgj product green extract (wave 41300)"),
    (41293, "bar3_ready", 0, "product bar3 readiness gate (wave 41300)"),
    (41294, "product_score", 0, "product readiness score tag (wave 41300)"),
    (41295, "continuum_ready", 1, "continuum readiness gate (wave 41300)"),
    (41296, "smoke_soft", 1, "smoke soft lamp (wave 41300)"),
    (41297, "dyn_soft", 1, "dyn soft lamp (wave 41300)"),
    (41298, "milestone_tag", MILESTONE, "milestone 41300 product tag"),
    (41299, "continuum_wave", MILESTONE, "milestone 41300 continuum wave identity"),
    # 41300 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        41276: "continuum wave fill soft unit + batch id",
        41280: "continuum bridge soft unit + batch id",
        41290: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (41291–41299), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 41293
    tag_batch = MARKERS_LO + 7  # 41298
    wave_batch = MARKERS_LO + 8  # 41299
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_41200 / gj_shell_green_41100 / gj_shell_green_41000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_41200 / gj_libcgj_green_41100 / gj_libcgj_green_41000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_41200 / gj_bar3_ready_41100 / gj_bar3_ready_41000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_41200 / gj_product_score_41100 / gj_product_score_41000\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_41200 / gj_continuum_ready_41100 /\n"
            f" * gj_continuum_ready_41000 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_41200 / gj_smoke_soft_41100 / gj_smoke_soft_41000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_41200 / gj_dyn_soft_41100 / gj_dyn_soft_41000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_41200 / gj_milestone_tag_41100 / gj_milestone_tag_41000,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_41200 / gj_continuum_wave_41100 /\n"
            f" * gj_continuum_wave_41000, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch41300.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 41299
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 41291–41299; 41300 is capstone
