...
"""
Soft continuum generator — MILESTONE 41500 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 41401–41500 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=41500   when parent has wired through graph_batch41500.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 41401 41500
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 41401 41500
...
graph batches ending at M. For M = **41500** the exclusive range is
**41401–41500** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 41401–41450  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 41451–41475  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 41476–41490  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 41491–41500  │ decade soft gates (Wave 273 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 41500 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_41500        1          graph_batch41491.c
  gj_libcgj_green_41500       1          graph_batch41492.c
  gj_bar3_ready_41500         0          graph_batch41493.c  (bar3 open)
  gj_product_score_41500      0          graph_batch41494.c  (score open)
  gj_continuum_ready_41500    1          graph_batch41495.c
  gj_smoke_soft_41500         1          graph_batch41496.c
  gj_dyn_soft_41500           1          graph_batch41497.c
  gj_milestone_tag_41500      41500      graph_batch41498.c
  gj_continuum_wave_41500     41500      graph_batch41499.c
  gj_batch_id_41500           41500      graph_batch41500.c
  gj_graph_milestone_41500    41500      graph_batch41500.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=41500`` means parent wired CGJ_SRCS through
    ``graph_batch41500.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_41500`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_41500`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M41500_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_41400`` / ``gj_*_41300`` / ``gj_*_41200`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 273 exclusive CREATE-ONLY lands the **markers** band
  (41491–41500) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_41500.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_41500.py

  # Restrict to marker band only (soft-gate TUs; Wave 273 exclusive):
  python3 user/libcgj/src/_gen_milestone_41500.py --band markers

  # Extension identity only (41401–41450):
  python3 user/libcgj/src/_gen_milestone_41500.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_41500.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_41400.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 41500 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 41500
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 41401
WAVE_HI = 41500  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 41401, 41450
IDENTITY_LO, IDENTITY_HI = 41451, 41475
FILL_LO, FILL_HI = 41476, 41490
MARKERS_LO, MARKERS_HI = 41491, 41500
...
# Stems become gj_<stem>_41500. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (41491, "shell_green", 1, "shell product green extract (wave 41500)"),
    (41492, "libcgj_green", 1, "libcgj product green extract (wave 41500)"),
    (41493, "bar3_ready", 0, "product bar3 readiness gate (wave 41500)"),
    (41494, "product_score", 0, "product readiness score tag (wave 41500)"),
    (41495, "continuum_ready", 1, "continuum readiness gate (wave 41500)"),
    (41496, "smoke_soft", 1, "smoke soft lamp (wave 41500)"),
    (41497, "dyn_soft", 1, "dyn soft lamp (wave 41500)"),
    (41498, "milestone_tag", MILESTONE, "milestone 41500 product tag"),
    (41499, "continuum_wave", MILESTONE, "milestone 41500 continuum wave identity"),
    # 41500 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        41476: "continuum wave fill soft unit + batch id",
        41480: "continuum bridge soft unit + batch id",
        41490: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (41491–41499), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 41493
    tag_batch = MARKERS_LO + 7  # 41498
    wave_batch = MARKERS_LO + 8  # 41499
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_41400 / gj_shell_green_41300 / gj_shell_green_41200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_41400 / gj_libcgj_green_41300 / gj_libcgj_green_41200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_41400 / gj_bar3_ready_41300 / gj_bar3_ready_41200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_41400 / gj_product_score_41300 / gj_product_score_41200\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_41400 / gj_continuum_ready_41300 /\n"
            f" * gj_continuum_ready_41200 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_41400 / gj_smoke_soft_41300 / gj_smoke_soft_41200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_41400 / gj_dyn_soft_41300 / gj_dyn_soft_41200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_41400 / gj_milestone_tag_41300 / gj_milestone_tag_41200,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_41400 / gj_continuum_wave_41300 /\n"
            f" * gj_continuum_wave_41200, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch41500.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 41499
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 41491–41499; 41500 is capstone
