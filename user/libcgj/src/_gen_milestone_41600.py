...
"""
Soft continuum generator — MILESTONE 41600 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 41501–41600 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=41600   when parent has wired through graph_batch41600.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 41501 41600
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 41501 41600
...
graph batches ending at M. For M = **41600** the exclusive range is
**41501–41600** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 41501–41550  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 41551–41575  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 41576–41590  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 41591–41600  │ decade soft gates (Wave 274 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 41600 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_41600        1          graph_batch41591.c
  gj_libcgj_green_41600       1          graph_batch41592.c
  gj_bar3_ready_41600         0          graph_batch41593.c  (bar3 open)
  gj_product_score_41600      0          graph_batch41594.c  (score open)
  gj_continuum_ready_41600    1          graph_batch41595.c
  gj_smoke_soft_41600         1          graph_batch41596.c
  gj_dyn_soft_41600           1          graph_batch41597.c
  gj_milestone_tag_41600      41600      graph_batch41598.c
  gj_continuum_wave_41600     41600      graph_batch41599.c
  gj_batch_id_41600           41600      graph_batch41600.c
  gj_graph_milestone_41600    41600      graph_batch41600.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=41600`` means parent wired CGJ_SRCS through
    ``graph_batch41600.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_41600`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_41600`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M41600_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_41500`` / ``gj_*_41400`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 274 exclusive CREATE-ONLY lands the **markers** band
  (41591–41600) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_41600.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_41600.py

  # Restrict to marker band only (soft-gate TUs; Wave 274 exclusive):
  python3 user/libcgj/src/_gen_milestone_41600.py --band markers

  # Extension identity only (41501–41550):
  python3 user/libcgj/src/_gen_milestone_41600.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_41600.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_41500.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 41600 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 41600
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 41501
WAVE_HI = 41600  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 41501, 41550
IDENTITY_LO, IDENTITY_HI = 41551, 41575
FILL_LO, FILL_HI = 41576, 41590
MARKERS_LO, MARKERS_HI = 41591, 41600
...
# Stems become gj_<stem>_41600. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (41591, "shell_green", 1, "shell product green extract (wave 41600)"),
    (41592, "libcgj_green", 1, "libcgj product green extract (wave 41600)"),
    (41593, "bar3_ready", 0, "product bar3 readiness gate (wave 41600)"),
    (41594, "product_score", 0, "product readiness score tag (wave 41600)"),
    (41595, "continuum_ready", 1, "continuum readiness gate (wave 41600)"),
    (41596, "smoke_soft", 1, "smoke soft lamp (wave 41600)"),
    (41597, "dyn_soft", 1, "dyn soft lamp (wave 41600)"),
    (41598, "milestone_tag", MILESTONE, "milestone 41600 product tag"),
    (41599, "continuum_wave", MILESTONE, "milestone 41600 continuum wave identity"),
    # 41600 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        41576: "continuum wave fill soft unit + batch id",
        41580: "continuum bridge soft unit + batch id",
        41590: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (41591–41599), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 41593
    tag_batch = MARKERS_LO + 7  # 41598
    wave_batch = MARKERS_LO + 8  # 41599
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_41500 / gj_shell_green_41400 / gj_shell_green_41300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_41500 / gj_libcgj_green_41400 / gj_libcgj_green_41300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_41500 / gj_bar3_ready_41400 / gj_bar3_ready_41300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_41500 / gj_product_score_41400 / gj_product_score_41300\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_41500 / gj_continuum_ready_41400 /\n"
            f" * gj_continuum_ready_41300 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_41500 / gj_smoke_soft_41400 / gj_smoke_soft_41300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_41500 / gj_dyn_soft_41400 / gj_dyn_soft_41300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_41500 / gj_milestone_tag_41400 / gj_milestone_tag_41300,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_41500 / gj_continuum_wave_41400 /\n"
            f" * gj_continuum_wave_41300, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch41600.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 41599
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 41591–41599; 41600 is capstone
