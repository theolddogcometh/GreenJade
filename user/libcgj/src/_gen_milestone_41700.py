...
"""
Soft continuum generator — MILESTONE 41700 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 41601–41700 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=41700   when parent has wired through graph_batch41700.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 41601 41700
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 41601 41700
...
graph batches ending at M. For M = **41700** the exclusive range is
**41601–41700** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 41601–41650  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 41651–41675  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 41676–41690  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 41691–41700  │ decade soft gates (Wave 275 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 41700 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_41700        1          graph_batch41691.c
  gj_libcgj_green_41700       1          graph_batch41692.c
  gj_bar3_ready_41700         0          graph_batch41693.c  (bar3 open)
  gj_product_score_41700      0          graph_batch41694.c  (score open)
  gj_continuum_ready_41700    1          graph_batch41695.c
  gj_smoke_soft_41700         1          graph_batch41696.c
  gj_dyn_soft_41700           1          graph_batch41697.c
  gj_milestone_tag_41700      41700      graph_batch41698.c
  gj_continuum_wave_41700     41700      graph_batch41699.c
  gj_batch_id_41700           41700      graph_batch41700.c
  gj_graph_milestone_41700    41700      graph_batch41700.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=41700`` means parent wired CGJ_SRCS through
    ``graph_batch41700.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_41700`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_41700`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M41700_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_41600`` / ``gj_*_41500`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 275 exclusive CREATE-ONLY lands the **markers** band
  (41691–41700) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_41700.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_41700.py

  # Restrict to marker band only (soft-gate TUs; Wave 275 exclusive):
  python3 user/libcgj/src/_gen_milestone_41700.py --band markers

  # Extension identity only (41601–41650):
  python3 user/libcgj/src/_gen_milestone_41700.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_41700.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_41600.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 41700 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 41700
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 41601
WAVE_HI = 41700  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 41601, 41650
IDENTITY_LO, IDENTITY_HI = 41651, 41675
FILL_LO, FILL_HI = 41676, 41690
MARKERS_LO, MARKERS_HI = 41691, 41700
...
# Stems become gj_<stem>_41700. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (41691, "shell_green", 1, "shell product green extract (wave 41700)"),
    (41692, "libcgj_green", 1, "libcgj product green extract (wave 41700)"),
    (41693, "bar3_ready", 0, "product bar3 readiness gate (wave 41700)"),
    (41694, "product_score", 0, "product readiness score tag (wave 41700)"),
    (41695, "continuum_ready", 1, "continuum readiness gate (wave 41700)"),
    (41696, "smoke_soft", 1, "smoke soft lamp (wave 41700)"),
    (41697, "dyn_soft", 1, "dyn soft lamp (wave 41700)"),
    (41698, "milestone_tag", MILESTONE, "milestone 41700 product tag"),
    (41699, "continuum_wave", MILESTONE, "milestone 41700 continuum wave identity"),
    # 41700 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        41676: "continuum wave fill soft unit + batch id",
        41680: "continuum bridge soft unit + batch id",
        41690: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (41691–41699), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 41693
    tag_batch = MARKERS_LO + 7  # 41698
    wave_batch = MARKERS_LO + 8  # 41699
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_41600 / gj_shell_green_41500 / gj_shell_green_41400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_41600 / gj_libcgj_green_41500 / gj_libcgj_green_41400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_41600 / gj_bar3_ready_41500 / gj_bar3_ready_41400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_41600 / gj_product_score_41500 / gj_product_score_41400\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_41600 / gj_continuum_ready_41500 /\n"
            f" * gj_continuum_ready_41400 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_41600 / gj_smoke_soft_41500 / gj_smoke_soft_41400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_41600 / gj_dyn_soft_41500 / gj_dyn_soft_41400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_41600 / gj_milestone_tag_41500 / gj_milestone_tag_41400,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_41600 / gj_continuum_wave_41500 /\n"
            f" * gj_continuum_wave_41400, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch41700.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 41699
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 41691–41699; 41700 is capstone
