...
"""
Soft continuum generator — MILESTONE 41800 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 41701–41800 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=41800   when parent has wired through graph_batch41800.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 41701 41800
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 41701 41800
...
graph batches ending at M. For M = **41800** the exclusive range is
**41701–41800** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 41701–41750  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 41751–41775  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 41776–41790  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 41791–41800  │ decade soft gates (Wave 276 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 41800 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_41800        1          graph_batch41791.c
  gj_libcgj_green_41800       1          graph_batch41792.c
  gj_bar3_ready_41800         0          graph_batch41793.c  (bar3 open)
  gj_product_score_41800      0          graph_batch41794.c  (score open)
  gj_continuum_ready_41800    1          graph_batch41795.c
  gj_smoke_soft_41800         1          graph_batch41796.c
  gj_dyn_soft_41800           1          graph_batch41797.c
  gj_milestone_tag_41800      41800      graph_batch41798.c
  gj_continuum_wave_41800     41800      graph_batch41799.c
  gj_batch_id_41800           41800      graph_batch41800.c
  gj_graph_milestone_41800    41800      graph_batch41800.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=41800`` means parent wired CGJ_SRCS through
    ``graph_batch41800.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_41800`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_41800`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M41800_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_41700`` / ``gj_*_41600`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 276 exclusive CREATE-ONLY lands the **markers** band
  (41791–41800) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_41800.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_41800.py

  # Restrict to marker band only (soft-gate TUs; Wave 276 exclusive):
  python3 user/libcgj/src/_gen_milestone_41800.py --band markers

  # Extension identity only (41701–41750):
  python3 user/libcgj/src/_gen_milestone_41800.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_41800.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_41700.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 41800 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 41800
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 41701
WAVE_HI = 41800  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 41701, 41750
IDENTITY_LO, IDENTITY_HI = 41751, 41775
FILL_LO, FILL_HI = 41776, 41790
MARKERS_LO, MARKERS_HI = 41791, 41800
...
# Stems become gj_<stem>_41800. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (41791, "shell_green", 1, "shell product green extract (wave 41800)"),
    (41792, "libcgj_green", 1, "libcgj product green extract (wave 41800)"),
    (41793, "bar3_ready", 0, "product bar3 readiness gate (wave 41800)"),
    (41794, "product_score", 0, "product readiness score tag (wave 41800)"),
    (41795, "continuum_ready", 1, "continuum readiness gate (wave 41800)"),
    (41796, "smoke_soft", 1, "smoke soft lamp (wave 41800)"),
    (41797, "dyn_soft", 1, "dyn soft lamp (wave 41800)"),
    (41798, "milestone_tag", MILESTONE, "milestone 41800 product tag"),
    (41799, "continuum_wave", MILESTONE, "milestone 41800 continuum wave identity"),
    # 41800 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        41776: "continuum wave fill soft unit + batch id",
        41780: "continuum bridge soft unit + batch id",
        41790: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (41791–41799), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 41793
    tag_batch = MARKERS_LO + 7  # 41798
    wave_batch = MARKERS_LO + 8  # 41799
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_41700 / gj_shell_green_41600 / gj_shell_green_41500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_41700 / gj_libcgj_green_41600 / gj_libcgj_green_41500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_41700 / gj_bar3_ready_41600 / gj_bar3_ready_41500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_41700 / gj_product_score_41600 / gj_product_score_41500\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_41700 / gj_continuum_ready_41600 /\n"
            f" * gj_continuum_ready_41500 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_41700 / gj_smoke_soft_41600 / gj_smoke_soft_41500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_41700 / gj_dyn_soft_41600 / gj_dyn_soft_41500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_41700 / gj_milestone_tag_41600 / gj_milestone_tag_41500,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_41700 / gj_continuum_wave_41600 /\n"
            f" * gj_continuum_wave_41500, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch41800.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 41799
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 41791–41799; 41800 is capstone
