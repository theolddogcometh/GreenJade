...
"""
Soft continuum generator — MILESTONE 41900 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 41801–41900 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=41900   when parent has wired through graph_batch41900.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 41801 41900
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 41801 41900
...
graph batches ending at M. For M = **41900** the exclusive range is
**41801–41900** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 41801–41850  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 41851–41875  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 41876–41890  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 41891–41900  │ decade soft gates (Wave 277 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 41900 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_41900        1          graph_batch41891.c
  gj_libcgj_green_41900       1          graph_batch41892.c
  gj_bar3_ready_41900         0          graph_batch41893.c  (bar3 open)
  gj_product_score_41900      0          graph_batch41894.c  (score open)
  gj_continuum_ready_41900    1          graph_batch41895.c
  gj_smoke_soft_41900         1          graph_batch41896.c
  gj_dyn_soft_41900           1          graph_batch41897.c
  gj_milestone_tag_41900      41900      graph_batch41898.c
  gj_continuum_wave_41900     41900      graph_batch41899.c
  gj_batch_id_41900           41900      graph_batch41900.c
  gj_graph_milestone_41900    41900      graph_batch41900.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=41900`` means parent wired CGJ_SRCS through
    ``graph_batch41900.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_41900`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_41900`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M41900_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_41800`` / ``gj_*_41700`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 277 exclusive CREATE-ONLY lands the **markers** band
  (41891–41900) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_41900.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_41900.py

  # Restrict to marker band only (soft-gate TUs; Wave 277 exclusive):
  python3 user/libcgj/src/_gen_milestone_41900.py --band markers

  # Extension identity only (41801–41850):
  python3 user/libcgj/src/_gen_milestone_41900.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_41900.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_41800.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 41900 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 41900
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 41801
WAVE_HI = 41900  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 41801, 41850
IDENTITY_LO, IDENTITY_HI = 41851, 41875
FILL_LO, FILL_HI = 41876, 41890
MARKERS_LO, MARKERS_HI = 41891, 41900
...
# Stems become gj_<stem>_41900. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (41891, "shell_green", 1, "shell product green extract (wave 41900)"),
    (41892, "libcgj_green", 1, "libcgj product green extract (wave 41900)"),
    (41893, "bar3_ready", 0, "product bar3 readiness gate (wave 41900)"),
    (41894, "product_score", 0, "product readiness score tag (wave 41900)"),
    (41895, "continuum_ready", 1, "continuum readiness gate (wave 41900)"),
    (41896, "smoke_soft", 1, "smoke soft lamp (wave 41900)"),
    (41897, "dyn_soft", 1, "dyn soft lamp (wave 41900)"),
    (41898, "milestone_tag", MILESTONE, "milestone 41900 product tag"),
    (41899, "continuum_wave", MILESTONE, "milestone 41900 continuum wave identity"),
    # 41900 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        41876: "continuum wave fill soft unit + batch id",
        41880: "continuum bridge soft unit + batch id",
        41890: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (41891–41899), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 41893
    tag_batch = MARKERS_LO + 7  # 41898
    wave_batch = MARKERS_LO + 8  # 41899
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_41800 / gj_shell_green_41700 / gj_shell_green_41600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_41800 / gj_libcgj_green_41700 / gj_libcgj_green_41600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_41800 / gj_bar3_ready_41700 / gj_bar3_ready_41600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_41800 / gj_product_score_41700 / gj_product_score_41600\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_41800 / gj_continuum_ready_41700 /\n"
            f" * gj_continuum_ready_41600 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_41800 / gj_smoke_soft_41700 / gj_smoke_soft_41600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_41800 / gj_dyn_soft_41700 / gj_dyn_soft_41600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_41800 / gj_milestone_tag_41700 / gj_milestone_tag_41600,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_41800 / gj_continuum_wave_41700 /\n"
            f" * gj_continuum_wave_41600, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch41900.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 41899
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 41891–41899; 41900 is capstone
