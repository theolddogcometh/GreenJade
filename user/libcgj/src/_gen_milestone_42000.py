...
"""
Soft continuum generator — MILESTONE 42000 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 41901–42000 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=42000   when parent has wired through graph_batch42000.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 41901 42000
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 41901 42000
...
graph batches ending at M. For M = **42000** the exclusive range is
**41901–42000** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 41901–41950  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 41951–41975  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 41976–41990  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 41991–42000  │ decade soft gates (Wave 278 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 42000 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_42000        1          graph_batch41991.c
  gj_libcgj_green_42000       1          graph_batch41992.c
  gj_bar3_ready_42000         0          graph_batch41993.c  (bar3 open)
  gj_product_score_42000      0          graph_batch41994.c  (score open)
  gj_continuum_ready_42000    1          graph_batch41995.c
  gj_smoke_soft_42000         1          graph_batch41996.c
  gj_dyn_soft_42000           1          graph_batch41997.c
  gj_milestone_tag_42000      42000      graph_batch41998.c
  gj_continuum_wave_42000     42000      graph_batch41999.c
  gj_batch_id_42000           42000      graph_batch42000.c
  gj_graph_milestone_42000    42000      graph_batch42000.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=42000`` means parent wired CGJ_SRCS through
    ``graph_batch42000.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_42000`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_42000`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M42000_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_41900`` / ``gj_*_41800`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 278 exclusive CREATE-ONLY lands the **markers** band
  (41991–42000) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_42000.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_42000.py

  # Restrict to marker band only (soft-gate TUs; Wave 278 exclusive):
  python3 user/libcgj/src/_gen_milestone_42000.py --band markers

  # Extension identity only (41901–41950):
  python3 user/libcgj/src/_gen_milestone_42000.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_42000.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_41900.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 42000 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 42000
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 41901
WAVE_HI = 42000  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 41901, 41950
IDENTITY_LO, IDENTITY_HI = 41951, 41975
FILL_LO, FILL_HI = 41976, 41990
MARKERS_LO, MARKERS_HI = 41991, 42000
...
# Stems become gj_<stem>_42000. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (41991, "shell_green", 1, "shell product green extract (wave 42000)"),
    (41992, "libcgj_green", 1, "libcgj product green extract (wave 42000)"),
    (41993, "bar3_ready", 0, "product bar3 readiness gate (wave 42000)"),
    (41994, "product_score", 0, "product readiness score tag (wave 42000)"),
    (41995, "continuum_ready", 1, "continuum readiness gate (wave 42000)"),
    (41996, "smoke_soft", 1, "smoke soft lamp (wave 42000)"),
    (41997, "dyn_soft", 1, "dyn soft lamp (wave 42000)"),
    (41998, "milestone_tag", MILESTONE, "milestone 42000 product tag"),
    (41999, "continuum_wave", MILESTONE, "milestone 42000 continuum wave identity"),
    # 42000 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        41976: "continuum wave fill soft unit + batch id",
        41980: "continuum bridge soft unit + batch id",
        41990: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (41991–41999), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 41993
    tag_batch = MARKERS_LO + 7  # 41998
    wave_batch = MARKERS_LO + 8  # 41999
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_41900 / gj_shell_green_41800 / gj_shell_green_41700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_41900 / gj_libcgj_green_41800 / gj_libcgj_green_41700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_41900 / gj_bar3_ready_41800 / gj_bar3_ready_41700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_41900 / gj_product_score_41800 / gj_product_score_41700\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_41900 / gj_continuum_ready_41800 /\n"
            f" * gj_continuum_ready_41700 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_41900 / gj_smoke_soft_41800 / gj_smoke_soft_41700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_41900 / gj_dyn_soft_41800 / gj_dyn_soft_41700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_41900 / gj_milestone_tag_41800 / gj_milestone_tag_41700,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_41900 / gj_continuum_wave_41800 /\n"
            f" * gj_continuum_wave_41700, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch42000.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 41999
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 41991–41999; 42000 is capstone
