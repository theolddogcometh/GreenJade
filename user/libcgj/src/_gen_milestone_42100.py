...
"""
Soft continuum generator — MILESTONE 42100 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 42001–42100 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=42100   when parent has wired through graph_batch42100.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 42001 42100
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 42001 42100
...
graph batches ending at M. For M = **42100** the exclusive range is
**42001–42100** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 42001–42050  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 42051–42075  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 42076–42090  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 42091–42100  │ decade soft gates (Wave 279 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 42100 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_42100        1          graph_batch42091.c
  gj_libcgj_green_42100       1          graph_batch42092.c
  gj_bar3_ready_42100         0          graph_batch42093.c  (bar3 open)
  gj_product_score_42100      0          graph_batch42094.c  (score open)
  gj_continuum_ready_42100    1          graph_batch42095.c
  gj_smoke_soft_42100         1          graph_batch42096.c
  gj_dyn_soft_42100           1          graph_batch42097.c
  gj_milestone_tag_42100      42100      graph_batch42098.c
  gj_continuum_wave_42100     42100      graph_batch42099.c
  gj_batch_id_42100           42100      graph_batch42100.c
  gj_graph_milestone_42100    42100      graph_batch42100.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=42100`` means parent wired CGJ_SRCS through
    ``graph_batch42100.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_42100`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_42100`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M42100_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_42000`` / ``gj_*_41900`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 279 exclusive CREATE-ONLY lands the **markers** band
  (42091–42100) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_42100.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_42100.py

  # Restrict to marker band only (soft-gate TUs; Wave 279 exclusive):
  python3 user/libcgj/src/_gen_milestone_42100.py --band markers

  # Extension identity only (42001–42050):
  python3 user/libcgj/src/_gen_milestone_42100.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_42100.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_42000.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 42100 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 42100
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 42001
WAVE_HI = 42100  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 42001, 42050
IDENTITY_LO, IDENTITY_HI = 42051, 42075
FILL_LO, FILL_HI = 42076, 42090
MARKERS_LO, MARKERS_HI = 42091, 42100
...
# Stems become gj_<stem>_42100. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (42091, "shell_green", 1, "shell product green extract (wave 42100)"),
    (42092, "libcgj_green", 1, "libcgj product green extract (wave 42100)"),
    (42093, "bar3_ready", 0, "product bar3 readiness gate (wave 42100)"),
    (42094, "product_score", 0, "product readiness score tag (wave 42100)"),
    (42095, "continuum_ready", 1, "continuum readiness gate (wave 42100)"),
    (42096, "smoke_soft", 1, "smoke soft lamp (wave 42100)"),
    (42097, "dyn_soft", 1, "dyn soft lamp (wave 42100)"),
    (42098, "milestone_tag", MILESTONE, "milestone 42100 product tag"),
    (42099, "continuum_wave", MILESTONE, "milestone 42100 continuum wave identity"),
    # 42100 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        42076: "continuum wave fill soft unit + batch id",
        42080: "continuum bridge soft unit + batch id",
        42090: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (42091–42099), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 42093
    tag_batch = MARKERS_LO + 7  # 42098
    wave_batch = MARKERS_LO + 8  # 42099
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_42000 / gj_shell_green_41900 / gj_shell_green_41800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_42000 / gj_libcgj_green_41900 / gj_libcgj_green_41800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_42000 / gj_bar3_ready_41900 / gj_bar3_ready_41800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_42000 / gj_product_score_41900 / gj_product_score_41800\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_42000 / gj_continuum_ready_41900 /\n"
            f" * gj_continuum_ready_41800 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_42000 / gj_smoke_soft_41900 / gj_smoke_soft_41800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_42000 / gj_dyn_soft_41900 / gj_dyn_soft_41800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_42000 / gj_milestone_tag_41900 / gj_milestone_tag_41800,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_42000 / gj_continuum_wave_41900 /\n"
            f" * gj_continuum_wave_41800, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch42100.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 42099
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 42091–42099; 42100 is capstone
