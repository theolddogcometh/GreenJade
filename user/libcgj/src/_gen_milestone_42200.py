...
"""
Soft continuum generator — MILESTONE 42200 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 42101–42200 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=42200   when parent has wired through graph_batch42200.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 42101 42200
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 42101 42200
...
graph batches ending at M. For M = **42200** the exclusive range is
**42101–42200** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 42101–42150  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 42151–42175  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 42176–42190  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 42191–42200  │ decade soft gates (Wave 280 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 42200 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_42200        1          graph_batch42191.c
  gj_libcgj_green_42200       1          graph_batch42192.c
  gj_bar3_ready_42200         0          graph_batch42193.c  (bar3 open)
  gj_product_score_42200      0          graph_batch42194.c  (score open)
  gj_continuum_ready_42200    1          graph_batch42195.c
  gj_smoke_soft_42200         1          graph_batch42196.c
  gj_dyn_soft_42200           1          graph_batch42197.c
  gj_milestone_tag_42200      42200      graph_batch42198.c
  gj_continuum_wave_42200     42200      graph_batch42199.c
  gj_batch_id_42200           42200      graph_batch42200.c
  gj_graph_milestone_42200    42200      graph_batch42200.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=42200`` means parent wired CGJ_SRCS through
    ``graph_batch42200.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_42200`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_42200`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M42200_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_42100`` / ``gj_*_42000`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 280 exclusive CREATE-ONLY lands the **markers** band
  (42191–42200) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_42200.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_42200.py

  # Restrict to marker band only (soft-gate TUs; Wave 280 exclusive):
  python3 user/libcgj/src/_gen_milestone_42200.py --band markers

  # Extension identity only (42101–42150):
  python3 user/libcgj/src/_gen_milestone_42200.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_42200.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_42100.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 42200 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 42200
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 42101
WAVE_HI = 42200  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 42101, 42150
IDENTITY_LO, IDENTITY_HI = 42151, 42175
FILL_LO, FILL_HI = 42176, 42190
MARKERS_LO, MARKERS_HI = 42191, 42200
...
# Stems become gj_<stem>_42200. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (42191, "shell_green", 1, "shell product green extract (wave 42200)"),
    (42192, "libcgj_green", 1, "libcgj product green extract (wave 42200)"),
    (42193, "bar3_ready", 0, "product bar3 readiness gate (wave 42200)"),
    (42194, "product_score", 0, "product readiness score tag (wave 42200)"),
    (42195, "continuum_ready", 1, "continuum readiness gate (wave 42200)"),
    (42196, "smoke_soft", 1, "smoke soft lamp (wave 42200)"),
    (42197, "dyn_soft", 1, "dyn soft lamp (wave 42200)"),
    (42198, "milestone_tag", MILESTONE, "milestone 42200 product tag"),
    (42199, "continuum_wave", MILESTONE, "milestone 42200 continuum wave identity"),
    # 42200 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        42176: "continuum wave fill soft unit + batch id",
        42180: "continuum bridge soft unit + batch id",
        42190: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (42191–42199), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 42193
    tag_batch = MARKERS_LO + 7  # 42198
    wave_batch = MARKERS_LO + 8  # 42199
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_42100 / gj_shell_green_42000 / gj_shell_green_41900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_42100 / gj_libcgj_green_42000 / gj_libcgj_green_41900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_42100 / gj_bar3_ready_42000 / gj_bar3_ready_41900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_42100 / gj_product_score_42000 / gj_product_score_41900\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_42100 / gj_continuum_ready_42000 /\n"
            f" * gj_continuum_ready_41900 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_42100 / gj_smoke_soft_42000 / gj_smoke_soft_41900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_42100 / gj_dyn_soft_42000 / gj_dyn_soft_41900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_42100 / gj_milestone_tag_42000 / gj_milestone_tag_41900,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_42100 / gj_continuum_wave_42000 /\n"
            f" * gj_continuum_wave_41900, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch42200.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 42199
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 42191–42199; 42200 is capstone
