...
"""
Soft continuum generator — MILESTONE 42300 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 42201–42300 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=42300   when parent has wired through graph_batch42300.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 42201 42300
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 42201 42300
...
graph batches ending at M. For M = **42300** the exclusive range is
**42201–42300** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 42201–42250  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 42251–42275  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 42276–42290  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 42291–42300  │ decade soft gates (Wave 281 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 42300 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_42300        1          graph_batch42291.c
  gj_libcgj_green_42300       1          graph_batch42292.c
  gj_bar3_ready_42300         0          graph_batch42293.c  (bar3 open)
  gj_product_score_42300      0          graph_batch42294.c  (score open)
  gj_continuum_ready_42300    1          graph_batch42295.c
  gj_smoke_soft_42300         1          graph_batch42296.c
  gj_dyn_soft_42300           1          graph_batch42297.c
  gj_milestone_tag_42300      42300      graph_batch42298.c
  gj_continuum_wave_42300     42300      graph_batch42299.c
  gj_batch_id_42300           42300      graph_batch42300.c
  gj_graph_milestone_42300    42300      graph_batch42300.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=42300`` means parent wired CGJ_SRCS through
    ``graph_batch42300.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_42300`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_42300`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M42300_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_42200`` / ``gj_*_42100`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 281 exclusive CREATE-ONLY lands the **markers** band
  (42291–42300) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_42300.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_42300.py

  # Restrict to marker band only (soft-gate TUs; Wave 281 exclusive):
  python3 user/libcgj/src/_gen_milestone_42300.py --band markers

  # Extension identity only (42201–42250):
  python3 user/libcgj/src/_gen_milestone_42300.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_42300.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_42200.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 42300 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 42300
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 42201
WAVE_HI = 42300  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 42201, 42250
IDENTITY_LO, IDENTITY_HI = 42251, 42275
FILL_LO, FILL_HI = 42276, 42290
MARKERS_LO, MARKERS_HI = 42291, 42300
...
# Stems become gj_<stem>_42300. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (42291, "shell_green", 1, "shell product green extract (wave 42300)"),
    (42292, "libcgj_green", 1, "libcgj product green extract (wave 42300)"),
    (42293, "bar3_ready", 0, "product bar3 readiness gate (wave 42300)"),
    (42294, "product_score", 0, "product readiness score tag (wave 42300)"),
    (42295, "continuum_ready", 1, "continuum readiness gate (wave 42300)"),
    (42296, "smoke_soft", 1, "smoke soft lamp (wave 42300)"),
    (42297, "dyn_soft", 1, "dyn soft lamp (wave 42300)"),
    (42298, "milestone_tag", MILESTONE, "milestone 42300 product tag"),
    (42299, "continuum_wave", MILESTONE, "milestone 42300 continuum wave identity"),
    # 42300 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        42276: "continuum wave fill soft unit + batch id",
        42280: "continuum bridge soft unit + batch id",
        42290: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (42291–42299), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 42293
    tag_batch = MARKERS_LO + 7  # 42298
    wave_batch = MARKERS_LO + 8  # 42299
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_42200 / gj_shell_green_42100 / gj_shell_green_42000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_42200 / gj_libcgj_green_42100 / gj_libcgj_green_42000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_42200 / gj_bar3_ready_42100 / gj_bar3_ready_42000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_42200 / gj_product_score_42100 / gj_product_score_42000\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_42200 / gj_continuum_ready_42100 /\n"
            f" * gj_continuum_ready_42000 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_42200 / gj_smoke_soft_42100 / gj_smoke_soft_42000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_42200 / gj_dyn_soft_42100 / gj_dyn_soft_42000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_42200 / gj_milestone_tag_42100 / gj_milestone_tag_42000,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_42200 / gj_continuum_wave_42100 /\n"
            f" * gj_continuum_wave_42000, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch42300.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 42299
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 42291–42299; 42300 is capstone
