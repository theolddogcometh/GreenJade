...
"""
Soft continuum generator — MILESTONE 42500 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 42401–42500 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=42500   when parent has wired through graph_batch42500.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 42401 42500
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 42401 42500
...
graph batches ending at M. For M = **42500** the exclusive range is
**42401–42500** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 42401–42450  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 42451–42475  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 42476–42490  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 42491–42500  │ decade soft gates (Wave 282 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 42500 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_42500        1          graph_batch42491.c
  gj_libcgj_green_42500       1          graph_batch42492.c
  gj_bar3_ready_42500         0          graph_batch42493.c  (bar3 open)
  gj_product_score_42500      0          graph_batch42494.c  (score open)
  gj_continuum_ready_42500    1          graph_batch42495.c
  gj_smoke_soft_42500         1          graph_batch42496.c
  gj_dyn_soft_42500           1          graph_batch42497.c
  gj_milestone_tag_42500      42500      graph_batch42498.c
  gj_continuum_wave_42500     42500      graph_batch42499.c
  gj_batch_id_42500           42500      graph_batch42500.c
  gj_graph_milestone_42500    42500      graph_batch42500.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=42500`` means parent wired CGJ_SRCS through
    ``graph_batch42500.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_42500`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_42500`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M42500_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_42400`` / ``gj_*_42300`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  (42491–42500) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_42500.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_42500.py

  # Restrict to marker band only (soft-gate TUs; Wave 282 exclusive):
  python3 user/libcgj/src/_gen_milestone_42500.py --band markers

  # Extension identity only (42401–42450):
  python3 user/libcgj/src/_gen_milestone_42500.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_42500.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_42500.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 42500 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 42500
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 42401
WAVE_HI = 42500  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 42401, 42450
IDENTITY_LO, IDENTITY_HI = 42451, 42475
FILL_LO, FILL_HI = 42476, 42490
MARKERS_LO, MARKERS_HI = 42491, 42500
...
# Stems become gj_<stem>_42500. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (42491, "shell_green", 1, "shell product green extract (wave 42500)"),
    (42492, "libcgj_green", 1, "libcgj product green extract (wave 42500)"),
    (42493, "bar3_ready", 0, "product bar3 readiness gate (wave 42500)"),
    (42494, "product_score", 0, "product readiness score tag (wave 42500)"),
    (42495, "continuum_ready", 1, "continuum readiness gate (wave 42500)"),
    (42496, "smoke_soft", 1, "smoke soft lamp (wave 42500)"),
    (42497, "dyn_soft", 1, "dyn soft lamp (wave 42500)"),
    (42498, "milestone_tag", MILESTONE, "milestone 42500 product tag"),
    (42499, "continuum_wave", MILESTONE, "milestone 42500 continuum wave identity"),
    # 42500 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_42400 / gj_shell_green_42300 / gj_shell_green_42200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_42400 / gj_libcgj_green_42300 / gj_libcgj_green_42200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_42400 / gj_bar3_ready_42300 / gj_bar3_ready_42200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_42400 / gj_product_score_42300 / gj_product_score_42200\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_42400 / gj_continuum_ready_42300 /\n"
            f" * gj_continuum_ready_42200 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_42400 / gj_smoke_soft_42300 / gj_smoke_soft_42200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_42400 / gj_dyn_soft_42300 / gj_dyn_soft_42200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_42400 / gj_milestone_tag_42300 / gj_milestone_tag_42200,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_42400 / gj_continuum_wave_42300 /\n"
            f" * gj_continuum_wave_42200, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch42500.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 42491–42499; 42500 is capstone
