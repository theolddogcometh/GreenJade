...
"""
Soft continuum generator — MILESTONE 42600 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 42501–42600 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=42600   when parent has wired through graph_batch42600.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 42501 42600
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 42501 42600
...
graph batches ending at M. For M = **42600** the exclusive range is
**42501–42600** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 42501–42550  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 42551–42575  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 42576–42590  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 42591–42600  │ decade soft gates (Wave 284 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 42600 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_42600        1          graph_batch42391.c
  gj_libcgj_green_42600       1          graph_batch42392.c
  gj_bar3_ready_42600         0          graph_batch42393.c  (bar3 open)
  gj_product_score_42600      0          graph_batch42394.c  (score open)
  gj_continuum_ready_42600    1          graph_batch42395.c
  gj_smoke_soft_42600         1          graph_batch42396.c
  gj_dyn_soft_42600           1          graph_batch42397.c
  gj_milestone_tag_42600      42600      graph_batch42398.c
  gj_continuum_wave_42600     42600      graph_batch42399.c
  gj_batch_id_42600           42600      graph_batch42600.c
  gj_graph_milestone_42600    42600      graph_batch42600.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=42600`` means parent wired CGJ_SRCS through
    ``graph_batch42600.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_42600`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_42600`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M42600_DIRECT``.
...
--------------------
  ...
  (42591–42600) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_42600.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_42600.py

  # Restrict to marker band only (soft-gate TUs; Wave 284 exclusive):
  python3 user/libcgj/src/_gen_milestone_42600.py --band markers

  # Extension identity only (42501–42550):
  python3 user/libcgj/src/_gen_milestone_42600.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_42600.py --force
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 42600 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 42600
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 42501
WAVE_HI = 42600  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 42501, 42550
IDENTITY_LO, IDENTITY_HI = 42551, 42575
FILL_LO, FILL_HI = 42576, 42590
MARKERS_LO, MARKERS_HI = 42591, 42600
...
# Stems become gj_<stem>_42600. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (42591, "shell_green", 1, "shell product green extract (wave 42600)"),
    (42592, "libcgj_green", 1, "libcgj product green extract (wave 42600)"),
    (42593, "bar3_ready", 0, "product bar3 readiness gate (wave 42600)"),
    (42594, "product_score", 0, "product readiness score tag (wave 42600)"),
    (42595, "continuum_ready", 1, "continuum readiness gate (wave 42600)"),
    (42596, "smoke_soft", 1, "smoke soft lamp (wave 42600)"),
    (42597, "dyn_soft", 1, "dyn soft lamp (wave 42600)"),
    (42598, "milestone_tag", MILESTONE, "milestone 42600 product tag"),
    (42599, "continuum_wave", MILESTONE, "milestone 42600 continuum wave identity"),
    # 42600 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        42576: "continuum wave fill soft unit + batch id",
        42580: "continuum bridge soft unit + batch id",
        42590: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (42591–42599), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 42593
    tag_batch = MARKERS_LO + 7  # 42598
    wave_batch = MARKERS_LO + 8  # 42599
...
def render_milestone_capstone() -> str:
    """graph_batch42600.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 42599
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 42591–42599; 42600 is capstone
