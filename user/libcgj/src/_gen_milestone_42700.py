...
"""
Soft continuum generator — MILESTONE 42700 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 42601–42700 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=42700   when parent has wired through graph_batch42700.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 42601 42700
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 42601 42700
...
graph batches ending at M. For M = **42700** the exclusive range is
**42601–42700** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 42601–42650  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 42651–42675  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 42676–42690  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 42691–42700  │ decade soft gates (Wave 285 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 42700 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_42700        1          graph_batch42691.c
  gj_libcgj_green_42700       1          graph_batch42692.c
  gj_bar3_ready_42700         0          graph_batch42693.c  (bar3 open)
  gj_product_score_42700      0          graph_batch42694.c  (score open)
  gj_continuum_ready_42700    1          graph_batch42695.c
  gj_smoke_soft_42700         1          graph_batch42696.c
  gj_dyn_soft_42700           1          graph_batch42697.c
  gj_milestone_tag_42700      42700      graph_batch42698.c
  gj_continuum_wave_42700     42700      graph_batch42699.c
  gj_batch_id_42700           42700      graph_batch42700.c
  gj_graph_milestone_42700    42700      graph_batch42700.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=42700`` means parent wired CGJ_SRCS through
    ``graph_batch42700.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_42700`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_42700`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M42700_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_42600`` / ``gj_*_42500`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 285 exclusive CREATE-ONLY lands the **markers** band
  (42691–42700) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_42700.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_42700.py

  # Restrict to marker band only (soft-gate TUs; Wave 285 exclusive):
  python3 user/libcgj/src/_gen_milestone_42700.py --band markers

  # Extension identity only (42601–42650):
  python3 user/libcgj/src/_gen_milestone_42700.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_42700.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_42600.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 42700 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 42700
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 42601
WAVE_HI = 42700  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 42601, 42650
IDENTITY_LO, IDENTITY_HI = 42651, 42675
FILL_LO, FILL_HI = 42676, 42690
MARKERS_LO, MARKERS_HI = 42691, 42700
...
# Stems become gj_<stem>_42700. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (42691, "shell_green", 1, "shell product green extract (wave 42700)"),
    (42692, "libcgj_green", 1, "libcgj product green extract (wave 42700)"),
    (42693, "bar3_ready", 0, "product bar3 readiness gate (wave 42700)"),
    (42694, "product_score", 0, "product readiness score tag (wave 42700)"),
    (42695, "continuum_ready", 1, "continuum readiness gate (wave 42700)"),
    (42696, "smoke_soft", 1, "smoke soft lamp (wave 42700)"),
    (42697, "dyn_soft", 1, "dyn soft lamp (wave 42700)"),
    (42698, "milestone_tag", MILESTONE, "milestone 42700 product tag"),
    (42699, "continuum_wave", MILESTONE, "milestone 42700 continuum wave identity"),
    # 42700 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        42676: "continuum wave fill soft unit + batch id",
        42680: "continuum bridge soft unit + batch id",
        42690: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (42691–42699), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 42693
    tag_batch = MARKERS_LO + 7  # 42698
    wave_batch = MARKERS_LO + 8  # 42699
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_42600 / gj_shell_green_42500 / gj_shell_green_42400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_42600 / gj_libcgj_green_42500 / gj_libcgj_green_42400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_42600 / gj_bar3_ready_42500 / gj_bar3_ready_42400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_42600 / gj_product_score_42500 / gj_product_score_42400\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_42600 / gj_continuum_ready_42500 /\n"
            f" * gj_continuum_ready_42400 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_42600 / gj_smoke_soft_42500 / gj_smoke_soft_42400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_42600 / gj_dyn_soft_42500 / gj_dyn_soft_42400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_42600 / gj_milestone_tag_42500 / gj_milestone_tag_42400,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_42600 / gj_continuum_wave_42500 /\n"
            f" * gj_continuum_wave_42400, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch42700.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 42699
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 42691–42699; 42700 is capstone
