...
"""
Soft continuum generator — MILESTONE 42800 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 42701–42800 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=42800   when parent has wired through graph_batch42800.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 42701 42800
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 42701 42800
...
graph batches ending at M. For M = **42800** the exclusive range is
**42701–42800** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 42701–42750  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 42751–42775  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 42776–42790  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 42791–42800  │ decade soft gates (Wave 282 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 42800 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_42800        1          graph_batch42791.c
  gj_libcgj_green_42800       1          graph_batch42792.c
  gj_bar3_ready_42800         0          graph_batch42793.c  (bar3 open)
  gj_product_score_42800      0          graph_batch42794.c  (score open)
  gj_continuum_ready_42800    1          graph_batch42795.c
  gj_smoke_soft_42800         1          graph_batch42796.c
  gj_dyn_soft_42800           1          graph_batch42797.c
  gj_milestone_tag_42800      42800      graph_batch42798.c
  gj_continuum_wave_42800     42800      graph_batch42799.c
  gj_batch_id_42800           42800      graph_batch42800.c
  gj_graph_milestone_42800    42800      graph_batch42800.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=42800`` means parent wired CGJ_SRCS through
    ``graph_batch42800.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_42800`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_42800`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M42800_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_42700`` / ``gj_*_42600`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  (42791–42800) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_42800.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_42800.py

  # Restrict to marker band only (soft-gate TUs; Wave 282 exclusive):
  python3 user/libcgj/src/_gen_milestone_42800.py --band markers

  # Extension identity only (42701–42750):
  python3 user/libcgj/src/_gen_milestone_42800.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_42800.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_42700.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 42800 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 42800
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 42701
WAVE_HI = 42800  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 42701, 42750
IDENTITY_LO, IDENTITY_HI = 42751, 42775
FILL_LO, FILL_HI = 42776, 42790
MARKERS_LO, MARKERS_HI = 42791, 42800
...
# Stems become gj_<stem>_42800. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (42791, "shell_green", 1, "shell product green extract (wave 42800)"),
    (42792, "libcgj_green", 1, "libcgj product green extract (wave 42800)"),
    (42793, "bar3_ready", 0, "product bar3 readiness gate (wave 42800)"),
    (42794, "product_score", 0, "product readiness score tag (wave 42800)"),
    (42795, "continuum_ready", 1, "continuum readiness gate (wave 42800)"),
    (42796, "smoke_soft", 1, "smoke soft lamp (wave 42800)"),
    (42797, "dyn_soft", 1, "dyn soft lamp (wave 42800)"),
    (42798, "milestone_tag", MILESTONE, "milestone 42800 product tag"),
    (42799, "continuum_wave", MILESTONE, "milestone 42800 continuum wave identity"),
    # 42800 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        42776: "continuum wave fill soft unit + batch id",
        42780: "continuum bridge soft unit + batch id",
        42790: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (42791–42799), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 42793
    tag_batch = MARKERS_LO + 7  # 42798
    wave_batch = MARKERS_LO + 8  # 42799
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_42700 / gj_shell_green_42600 / gj_shell_green_42500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_42700 / gj_libcgj_green_42600 / gj_libcgj_green_42500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_42700 / gj_bar3_ready_42600 / gj_bar3_ready_42500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_42700 / gj_product_score_42600 / gj_product_score_42500\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_42700 / gj_continuum_ready_42600 /\n"
            f" * gj_continuum_ready_42500 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_42700 / gj_smoke_soft_42600 / gj_smoke_soft_42500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_42700 / gj_dyn_soft_42600 / gj_dyn_soft_42500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_42700 / gj_milestone_tag_42600 / gj_milestone_tag_42500,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_42700 / gj_continuum_wave_42600 /\n"
            f" * gj_continuum_wave_42500, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch42800.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 42799
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 42791–42799; 42800 is capstone
