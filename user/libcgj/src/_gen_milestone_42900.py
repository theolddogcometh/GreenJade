...
"""
Soft continuum generator — MILESTONE 42900 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 42801–42900 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=42900   when parent has wired through graph_batch42900.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 42801 42900
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 42801 42900
...
graph batches ending at M. For M = **42900** the exclusive range is
**42801–42900** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 42801–42850  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 42851–42875  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 42876–42890  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 42891–42900  │ decade soft gates (Wave 283 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 42900 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_42900        1          graph_batch42891.c
  gj_libcgj_green_42900       1          graph_batch42892.c
  gj_bar3_ready_42900         0          graph_batch42893.c  (bar3 open)
  gj_product_score_42900      0          graph_batch42894.c  (score open)
  gj_continuum_ready_42900    1          graph_batch42895.c
  gj_smoke_soft_42900         1          graph_batch42896.c
  gj_dyn_soft_42900           1          graph_batch42897.c
  gj_milestone_tag_42900      42900      graph_batch42898.c
  gj_continuum_wave_42900     42900      graph_batch42899.c
  gj_batch_id_42900           42900      graph_batch42900.c
  gj_graph_milestone_42900    42900      graph_batch42900.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=42900`` means parent wired CGJ_SRCS through
    ``graph_batch42900.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_42900`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_42900`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M42900_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_42800`` / ``gj_*_42700`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 283 exclusive CREATE-ONLY lands the **markers** band
  (42891–42900) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_42900.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_42900.py

  # Restrict to marker band only (soft-gate TUs; Wave 283 exclusive):
  python3 user/libcgj/src/_gen_milestone_42900.py --band markers

  # Extension identity only (42801–42850):
  python3 user/libcgj/src/_gen_milestone_42900.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_42900.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_42800.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 42900 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 42900
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 42801
WAVE_HI = 42900  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 42801, 42850
IDENTITY_LO, IDENTITY_HI = 42851, 42875
FILL_LO, FILL_HI = 42876, 42890
MARKERS_LO, MARKERS_HI = 42891, 42900
...
# Stems become gj_<stem>_42900. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (42891, "shell_green", 1, "shell product green extract (wave 42900)"),
    (42892, "libcgj_green", 1, "libcgj product green extract (wave 42900)"),
    (42893, "bar3_ready", 0, "product bar3 readiness gate (wave 42900)"),
    (42894, "product_score", 0, "product readiness score tag (wave 42900)"),
    (42895, "continuum_ready", 1, "continuum readiness gate (wave 42900)"),
    (42896, "smoke_soft", 1, "smoke soft lamp (wave 42900)"),
    (42897, "dyn_soft", 1, "dyn soft lamp (wave 42900)"),
    (42898, "milestone_tag", MILESTONE, "milestone 42900 product tag"),
    (42899, "continuum_wave", MILESTONE, "milestone 42900 continuum wave identity"),
    # 42900 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        42876: "continuum wave fill soft unit + batch id",
        42880: "continuum bridge soft unit + batch id",
        42890: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (42891–42899), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 42893
    tag_batch = MARKERS_LO + 7  # 42898
    wave_batch = MARKERS_LO + 8  # 42899
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_42800 / gj_shell_green_42700 / gj_shell_green_42600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_42800 / gj_libcgj_green_42700 / gj_libcgj_green_42600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_42800 / gj_bar3_ready_42700 / gj_bar3_ready_42600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_42800 / gj_product_score_42700 / gj_product_score_42600\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_42800 / gj_continuum_ready_42700 /\n"
            f" * gj_continuum_ready_42600 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_42800 / gj_smoke_soft_42700 / gj_smoke_soft_42600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_42800 / gj_dyn_soft_42700 / gj_dyn_soft_42600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_42800 / gj_milestone_tag_42700 / gj_milestone_tag_42600,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_42800 / gj_continuum_wave_42700 /\n"
            f" * gj_continuum_wave_42600, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch42900.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 42899
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 42891–42899; 42900 is capstone
