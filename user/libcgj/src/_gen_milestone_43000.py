...
"""
Soft continuum generator — MILESTONE 43000 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 42901–43000 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=43000   when parent has wired through graph_batch43000.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 42901 43000
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 42901 43000
...
graph batches ending at M. For M = **43000** the exclusive range is
**42901–43000** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 42901–42950  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 42951–42975  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 42976–42990  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 42991–43000  │ decade soft gates (Wave 284 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 43000 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_43000        1          graph_batch42991.c
  gj_libcgj_green_43000       1          graph_batch42992.c
  gj_bar3_ready_43000         0          graph_batch42993.c  (bar3 open)
  gj_product_score_43000      0          graph_batch42994.c  (score open)
  gj_continuum_ready_43000    1          graph_batch42995.c
  gj_smoke_soft_43000         1          graph_batch42996.c
  gj_dyn_soft_43000           1          graph_batch42997.c
  gj_milestone_tag_43000      43000      graph_batch42998.c
  gj_continuum_wave_43000     43000      graph_batch42999.c
  gj_batch_id_43000           43000      graph_batch43000.c
  gj_graph_milestone_43000    43000      graph_batch43000.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=43000`` means parent wired CGJ_SRCS through
    ``graph_batch43000.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_43000`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_43000`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M43000_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_42900`` / ``gj_*_42800`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 284 exclusive CREATE-ONLY lands the **markers** band
  (42991–43000) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_43000.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_43000.py

  # Restrict to marker band only (soft-gate TUs; Wave 284 exclusive):
  python3 user/libcgj/src/_gen_milestone_43000.py --band markers

  # Extension identity only (42901–42950):
  python3 user/libcgj/src/_gen_milestone_43000.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_43000.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_42900.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 43000 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 43000
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 42901
WAVE_HI = 43000  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 42901, 42950
IDENTITY_LO, IDENTITY_HI = 42951, 42975
FILL_LO, FILL_HI = 42976, 42990
MARKERS_LO, MARKERS_HI = 42991, 43000
...
# Stems become gj_<stem>_43000. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (42991, "shell_green", 1, "shell product green extract (wave 43000)"),
    (42992, "libcgj_green", 1, "libcgj product green extract (wave 43000)"),
    (42993, "bar3_ready", 0, "product bar3 readiness gate (wave 43000)"),
    (42994, "product_score", 0, "product readiness score tag (wave 43000)"),
    (42995, "continuum_ready", 1, "continuum readiness gate (wave 43000)"),
    (42996, "smoke_soft", 1, "smoke soft lamp (wave 43000)"),
    (42997, "dyn_soft", 1, "dyn soft lamp (wave 43000)"),
    (42998, "milestone_tag", MILESTONE, "milestone 43000 product tag"),
    (42999, "continuum_wave", MILESTONE, "milestone 43000 continuum wave identity"),
    # 43000 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        42976: "continuum wave fill soft unit + batch id",
        42980: "continuum bridge soft unit + batch id",
        42990: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (42991–42999), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 42993
    tag_batch = MARKERS_LO + 7  # 42998
    wave_batch = MARKERS_LO + 8  # 42999
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_42900 / gj_shell_green_42800 / gj_shell_green_42700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_42900 / gj_libcgj_green_42800 / gj_libcgj_green_42700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_42900 / gj_bar3_ready_42800 / gj_bar3_ready_42700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_42900 / gj_product_score_42800 / gj_product_score_42700\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_42900 / gj_continuum_ready_42800 /\n"
            f" * gj_continuum_ready_42700 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_42900 / gj_smoke_soft_42800 / gj_smoke_soft_42700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_42900 / gj_dyn_soft_42800 / gj_dyn_soft_42700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_42900 / gj_milestone_tag_42800 / gj_milestone_tag_42700,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_42900 / gj_continuum_wave_42800 /\n"
            f" * gj_continuum_wave_42700, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch43000.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 42999
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 42991–42999; 43000 is capstone
