...
"""
Soft continuum generator — MILESTONE 43100 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 43001–43100 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=43100   when parent has wired through graph_batch43100.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 43001 43100
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 43001 43100
...
graph batches ending at M. For M = **43100** the exclusive range is
**43001–43100** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 43001–43050  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 43051–43075  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 43076–43090  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 43091–43100  │ decade soft gates (Wave 285 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 43100 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_43100        1          graph_batch43091.c
  gj_libcgj_green_43100       1          graph_batch43092.c
  gj_bar3_ready_43100         0          graph_batch43093.c  (bar3 open)
  gj_product_score_43100      0          graph_batch43094.c  (score open)
  gj_continuum_ready_43100    1          graph_batch43095.c
  gj_smoke_soft_43100         1          graph_batch43096.c
  gj_dyn_soft_43100           1          graph_batch43097.c
  gj_milestone_tag_43100      43100      graph_batch43098.c
  gj_continuum_wave_43100     43100      graph_batch43099.c
  gj_batch_id_43100           43100      graph_batch43100.c
  gj_graph_milestone_43100    43100      graph_batch43100.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=43100`` means parent wired CGJ_SRCS through
    ``graph_batch43100.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_43100`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_43100`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M43100_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_43000`` / ``gj_*_42900`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  (43091–43100) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_43100.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_43100.py

  # Restrict to marker band only (soft-gate TUs; Wave 285 exclusive):
  python3 user/libcgj/src/_gen_milestone_43100.py --band markers

  # Extension identity only (43001–43050):
  python3 user/libcgj/src/_gen_milestone_43100.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_43100.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_43000.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 43100 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 43100
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 43001
WAVE_HI = 43100  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 43001, 43050
IDENTITY_LO, IDENTITY_HI = 43051, 43075
FILL_LO, FILL_HI = 43076, 43090
MARKERS_LO, MARKERS_HI = 43091, 43100
...
# Stems become gj_<stem>_43100. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (43091, "shell_green", 1, "shell product green extract (wave 43100)"),
    (43092, "libcgj_green", 1, "libcgj product green extract (wave 43100)"),
    (43093, "bar3_ready", 0, "product bar3 readiness gate (wave 43100)"),
    (43094, "product_score", 0, "product readiness score tag (wave 43100)"),
    (43095, "continuum_ready", 1, "continuum readiness gate (wave 43100)"),
    (43096, "smoke_soft", 1, "smoke soft lamp (wave 43100)"),
    (43097, "dyn_soft", 1, "dyn soft lamp (wave 43100)"),
    (43098, "milestone_tag", MILESTONE, "milestone 43100 product tag"),
    (43099, "continuum_wave", MILESTONE, "milestone 43100 continuum wave identity"),
    # 43100 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        43076: "continuum wave fill soft unit + batch id",
        43080: "continuum bridge soft unit + batch id",
        43090: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (43091–43099), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 43093
    tag_batch = MARKERS_LO + 7  # 43098
    wave_batch = MARKERS_LO + 8  # 43099
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_43000 / gj_shell_green_42900 / gj_shell_green_42800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_43000 / gj_libcgj_green_42900 / gj_libcgj_green_42800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_43000 / gj_bar3_ready_42900 / gj_bar3_ready_42800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_43000 / gj_product_score_42900 / gj_product_score_42800\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_43000 / gj_continuum_ready_42900 /\n"
            f" * gj_continuum_ready_42800 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_43000 / gj_smoke_soft_42900 / gj_smoke_soft_42800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_43000 / gj_dyn_soft_42900 / gj_dyn_soft_42800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_43000 / gj_milestone_tag_42900 / gj_milestone_tag_42800,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_43000 / gj_continuum_wave_42900 /\n"
            f" * gj_continuum_wave_42800, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch43100.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 43099
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 43091–43099; 43100 is capstone
