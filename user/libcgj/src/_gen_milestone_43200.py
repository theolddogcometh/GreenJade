...
"""
Soft continuum generator — MILESTONE 43200 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 43101–43200 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=43200   when parent has wired through graph_batch43200.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 43101 43200
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 43101 43200
...
graph batches ending at M. For M = **43200** the exclusive range is
**43101–43200** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 43101–43150  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 43151–43175  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 43176–43190  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 43191–43200  │ decade soft gates (Wave 286 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 43200 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_43200        1          graph_batch43191.c
  gj_libcgj_green_43200       1          graph_batch43192.c
  gj_bar3_ready_43200         0          graph_batch43193.c  (bar3 open)
  gj_product_score_43200      0          graph_batch43194.c  (score open)
  gj_continuum_ready_43200    1          graph_batch43195.c
  gj_smoke_soft_43200         1          graph_batch43196.c
  gj_dyn_soft_43200           1          graph_batch43197.c
  gj_milestone_tag_43200      43200      graph_batch43198.c
  gj_continuum_wave_43200     43200      graph_batch43199.c
  gj_batch_id_43200           43200      graph_batch43200.c
  gj_graph_milestone_43200    43200      graph_batch43200.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=43200`` means parent wired CGJ_SRCS through
    ``graph_batch43200.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_43200`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_43200`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M43200_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_43100`` / ``gj_*_43000`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 286 exclusive CREATE-ONLY lands the **markers** band
  (43191–43200) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_43200.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_43200.py

  # Restrict to marker band only (soft-gate TUs; Wave 286 exclusive):
  python3 user/libcgj/src/_gen_milestone_43200.py --band markers

  # Extension identity only (43101–43150):
  python3 user/libcgj/src/_gen_milestone_43200.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_43200.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_43100.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 43200 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 43200
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 43101
WAVE_HI = 43200  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 43101, 43150
IDENTITY_LO, IDENTITY_HI = 43151, 43175
FILL_LO, FILL_HI = 43176, 43190
MARKERS_LO, MARKERS_HI = 43191, 43200
...
# Stems become gj_<stem>_43200. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (43191, "shell_green", 1, "shell product green extract (wave 43200)"),
    (43192, "libcgj_green", 1, "libcgj product green extract (wave 43200)"),
    (43193, "bar3_ready", 0, "product bar3 readiness gate (wave 43200)"),
    (43194, "product_score", 0, "product readiness score tag (wave 43200)"),
    (43195, "continuum_ready", 1, "continuum readiness gate (wave 43200)"),
    (43196, "smoke_soft", 1, "smoke soft lamp (wave 43200)"),
    (43197, "dyn_soft", 1, "dyn soft lamp (wave 43200)"),
    (43198, "milestone_tag", MILESTONE, "milestone 43200 product tag"),
    (43199, "continuum_wave", MILESTONE, "milestone 43200 continuum wave identity"),
    # 43200 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        43176: "continuum wave fill soft unit + batch id",
        43180: "continuum bridge soft unit + batch id",
        43190: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (43191–43199), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 43193
    tag_batch = MARKERS_LO + 7  # 43198
    wave_batch = MARKERS_LO + 8  # 43199
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_43100 / gj_shell_green_43000 / gj_shell_green_42900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_43100 / gj_libcgj_green_43000 / gj_libcgj_green_42900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_43100 / gj_bar3_ready_43000 / gj_bar3_ready_42900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_43100 / gj_product_score_43000 / gj_product_score_42900\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_43100 / gj_continuum_ready_43000 /\n"
            f" * gj_continuum_ready_42900 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_43100 / gj_smoke_soft_43000 / gj_smoke_soft_42900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_43100 / gj_dyn_soft_43000 / gj_dyn_soft_42900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_43100 / gj_milestone_tag_43000 / gj_milestone_tag_42900,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_43100 / gj_continuum_wave_43000 /\n"
            f" * gj_continuum_wave_42900, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch43200.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 43199
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 43191–43199; 43200 is capstone
