...
"""
Soft continuum generator — MILESTONE 43300 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 43201–43300 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=43300   when parent has wired through graph_batch43300.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 43201 43300
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 43201 43300
...
graph batches ending at M. For M = **43300** the exclusive range is
**43201–43300** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 43201–43250  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 43251–43275  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 43276–43290  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 43291–43300  │ decade soft gates (Wave 287 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 43300 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_43300        1          graph_batch43291.c
  gj_libcgj_green_43300       1          graph_batch43292.c
  gj_bar3_ready_43300         0          graph_batch43293.c  (bar3 open)
  gj_product_score_43300      0          graph_batch43294.c  (score open)
  gj_continuum_ready_43300    1          graph_batch43295.c
  gj_smoke_soft_43300         1          graph_batch43296.c
  gj_dyn_soft_43300           1          graph_batch43297.c
  gj_milestone_tag_43300      43300      graph_batch43298.c
  gj_continuum_wave_43300     43300      graph_batch43299.c
  gj_batch_id_43300           43300      graph_batch43300.c
  gj_graph_milestone_43300    43300      graph_batch43300.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=43300`` means parent wired CGJ_SRCS through
    ``graph_batch43300.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_43300`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_43300`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M43300_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_43200`` / ``gj_*_43100`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 287 exclusive CREATE-ONLY lands the **markers** band
  (43291–43300) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_43300.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_43300.py

  # Restrict to marker band only (soft-gate TUs; Wave 287 exclusive):
  python3 user/libcgj/src/_gen_milestone_43300.py --band markers

  # Extension identity only (43201–43250):
  python3 user/libcgj/src/_gen_milestone_43300.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_43300.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_43200.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 43300 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 43300
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 43201
WAVE_HI = 43300  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 43201, 43250
IDENTITY_LO, IDENTITY_HI = 43251, 43275
FILL_LO, FILL_HI = 43276, 43290
MARKERS_LO, MARKERS_HI = 43291, 43300
...
# Stems become gj_<stem>_43300. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (43291, "shell_green", 1, "shell product green extract (wave 43300)"),
    (43292, "libcgj_green", 1, "libcgj product green extract (wave 43300)"),
    (43293, "bar3_ready", 0, "product bar3 readiness gate (wave 43300)"),
    (43294, "product_score", 0, "product readiness score tag (wave 43300)"),
    (43295, "continuum_ready", 1, "continuum readiness gate (wave 43300)"),
    (43296, "smoke_soft", 1, "smoke soft lamp (wave 43300)"),
    (43297, "dyn_soft", 1, "dyn soft lamp (wave 43300)"),
    (43298, "milestone_tag", MILESTONE, "milestone 43300 product tag"),
    (43299, "continuum_wave", MILESTONE, "milestone 43300 continuum wave identity"),
    # 43300 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        43276: "continuum wave fill soft unit + batch id",
        43280: "continuum bridge soft unit + batch id",
        43290: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (43291–43299), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 43293
    tag_batch = MARKERS_LO + 7  # 43298
    wave_batch = MARKERS_LO + 8  # 43299
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_43200 / gj_shell_green_43100 / gj_shell_green_43000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_43200 / gj_libcgj_green_43100 / gj_libcgj_green_43000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_43200 / gj_bar3_ready_43100 / gj_bar3_ready_43000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_43200 / gj_product_score_43100 / gj_product_score_43000\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_43200 / gj_continuum_ready_43100 /\n"
            f" * gj_continuum_ready_43000 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_43200 / gj_smoke_soft_43100 / gj_smoke_soft_43000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_43200 / gj_dyn_soft_43100 / gj_dyn_soft_43000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_43200 / gj_milestone_tag_43100 / gj_milestone_tag_43000,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_43200 / gj_continuum_wave_43100 /\n"
            f" * gj_continuum_wave_43000, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch43300.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 43299
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 43291–43299; 43300 is capstone
