...
"""
Soft continuum generator — MILESTONE 43400 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 43301–43400 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=43400   when parent has wired through graph_batch43400.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 43301 43400
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 43301 43400
...
graph batches ending at M. For M = **43400** the exclusive range is
**43301–43400** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 43301–43350  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 43351–43375  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 43376–43390  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 43391–43400  │ decade soft gates (Wave 286 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 43400 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_43400        1          graph_batch43391.c
  gj_libcgj_green_43400       1          graph_batch43392.c
  gj_bar3_ready_43400         0          graph_batch43393.c  (bar3 open)
  gj_product_score_43400      0          graph_batch43394.c  (score open)
  gj_continuum_ready_43400    1          graph_batch43395.c
  gj_smoke_soft_43400         1          graph_batch43396.c
  gj_dyn_soft_43400           1          graph_batch43397.c
  gj_milestone_tag_43400      43400      graph_batch43398.c
  gj_continuum_wave_43400     43400      graph_batch43399.c
  gj_batch_id_43400           43400      graph_batch43400.c
  gj_graph_milestone_43400    43400      graph_batch43400.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=43400`` means parent wired CGJ_SRCS through
    ``graph_batch43400.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_43400`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_43400`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M43400_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_43300`` / ``gj_*_43200`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  (43391–43400) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_43400.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_43400.py

  # Restrict to marker band only (soft-gate TUs; Wave 286 exclusive):
  python3 user/libcgj/src/_gen_milestone_43400.py --band markers

  # Extension identity only (43301–43350):
  python3 user/libcgj/src/_gen_milestone_43400.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_43400.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_43300.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 43400 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 43400
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 43301
WAVE_HI = 43400  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 43301, 43350
IDENTITY_LO, IDENTITY_HI = 43351, 43375
FILL_LO, FILL_HI = 43376, 43390
MARKERS_LO, MARKERS_HI = 43391, 43400
...
# Stems become gj_<stem>_43400. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (43391, "shell_green", 1, "shell product green extract (wave 43400)"),
    (43392, "libcgj_green", 1, "libcgj product green extract (wave 43400)"),
    (43393, "bar3_ready", 0, "product bar3 readiness gate (wave 43400)"),
    (43394, "product_score", 0, "product readiness score tag (wave 43400)"),
    (43395, "continuum_ready", 1, "continuum readiness gate (wave 43400)"),
    (43396, "smoke_soft", 1, "smoke soft lamp (wave 43400)"),
    (43397, "dyn_soft", 1, "dyn soft lamp (wave 43400)"),
    (43398, "milestone_tag", MILESTONE, "milestone 43400 product tag"),
    (43399, "continuum_wave", MILESTONE, "milestone 43400 continuum wave identity"),
    # 43400 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        43376: "continuum wave fill soft unit + batch id",
        43380: "continuum bridge soft unit + batch id",
        43390: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (43391–43399), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 43393
    tag_batch = MARKERS_LO + 7  # 43398
    wave_batch = MARKERS_LO + 8  # 43399
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_43300 / gj_shell_green_43200 / gj_shell_green_43100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_43300 / gj_libcgj_green_43200 / gj_libcgj_green_43100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_43300 / gj_bar3_ready_43200 / gj_bar3_ready_43100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_43300 / gj_product_score_43200 / gj_product_score_43100\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_43300 / gj_continuum_ready_43200 /\n"
            f" * gj_continuum_ready_43100 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_43300 / gj_smoke_soft_43200 / gj_smoke_soft_43100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_43300 / gj_dyn_soft_43200 / gj_dyn_soft_43100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_43300 / gj_milestone_tag_43200 / gj_milestone_tag_43100,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_43300 / gj_continuum_wave_43200 /\n"
            f" * gj_continuum_wave_43100, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch43400.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 43399
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 43391–43399; 43400 is capstone
