...
"""
Soft continuum generator — MILESTONE 43500 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 43401–43500 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=43500   when parent has wired through graph_batch43500.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 43401 43500
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 43401 43500
...
graph batches ending at M. For M = **43500** the exclusive range is
**43401–43500** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 43401–43450  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 43451–43475  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 43476–43490  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 43491–43500  │ decade soft gates (Wave 289 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 43500 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_43500        1          graph_batch43491.c
  gj_libcgj_green_43500       1          graph_batch43492.c
  gj_bar3_ready_43500         0          graph_batch43493.c  (bar3 open)
  gj_product_score_43500      0          graph_batch43494.c  (score open)
  gj_continuum_ready_43500    1          graph_batch43495.c
  gj_smoke_soft_43500         1          graph_batch43496.c
  gj_dyn_soft_43500           1          graph_batch43497.c
  gj_milestone_tag_43500      43500      graph_batch43498.c
  gj_continuum_wave_43500     43500      graph_batch43499.c
  gj_batch_id_43500           43500      graph_batch43500.c
  gj_graph_milestone_43500    43500      graph_batch43500.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=43500`` means parent wired CGJ_SRCS through
    ``graph_batch43500.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_43500`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_43500`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M43500_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_43400`` / ``gj_*_43300`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 289 exclusive CREATE-ONLY lands the **markers** band
  (43491–43500) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_43500.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_43500.py

  # Restrict to marker band only (soft-gate TUs; Wave 289 exclusive):
  python3 user/libcgj/src/_gen_milestone_43500.py --band markers

  # Extension identity only (43401–43450):
  python3 user/libcgj/src/_gen_milestone_43500.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_43500.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_43400.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 43500 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 43500
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 43401
WAVE_HI = 43500  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 43401, 43450
IDENTITY_LO, IDENTITY_HI = 43451, 43475
FILL_LO, FILL_HI = 43476, 43490
MARKERS_LO, MARKERS_HI = 43491, 43500
...
# Stems become gj_<stem>_43500. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (43491, "shell_green", 1, "shell product green extract (wave 43500)"),
    (43492, "libcgj_green", 1, "libcgj product green extract (wave 43500)"),
    (43493, "bar3_ready", 0, "product bar3 readiness gate (wave 43500)"),
    (43494, "product_score", 0, "product readiness score tag (wave 43500)"),
    (43495, "continuum_ready", 1, "continuum readiness gate (wave 43500)"),
    (43496, "smoke_soft", 1, "smoke soft lamp (wave 43500)"),
    (43497, "dyn_soft", 1, "dyn soft lamp (wave 43500)"),
    (43498, "milestone_tag", MILESTONE, "milestone 43500 product tag"),
    (43499, "continuum_wave", MILESTONE, "milestone 43500 continuum wave identity"),
    # 43500 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        43476: "continuum wave fill soft unit + batch id",
        43480: "continuum bridge soft unit + batch id",
        43490: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (43491–43499), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 43493
    tag_batch = MARKERS_LO + 7  # 43498
    wave_batch = MARKERS_LO + 8  # 43499
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_43400 / gj_shell_green_43300 / gj_shell_green_43500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_43400 / gj_libcgj_green_43300 / gj_libcgj_green_43500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_43400 / gj_bar3_ready_43300 / gj_bar3_ready_43500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_43400 / gj_product_score_43300 / gj_product_score_43500\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_43400 / gj_continuum_ready_43300 /\n"
            f" * gj_continuum_ready_43200 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_43400 / gj_smoke_soft_43300 / gj_smoke_soft_43500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_43400 / gj_dyn_soft_43300 / gj_dyn_soft_43500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_43400 / gj_milestone_tag_43300 / gj_milestone_tag_43500,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_43400 / gj_continuum_wave_43300 /\n"
            f" * gj_continuum_wave_43200, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch43500.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 43499
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 43491–43499; 43500 is capstone
