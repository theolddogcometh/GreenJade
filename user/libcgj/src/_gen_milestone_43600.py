...
"""
Soft continuum generator — MILESTONE 43600 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 43501–43600 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=43600   when parent has wired through graph_batch43600.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 43501 43600
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 43501 43600
...
graph batches ending at M. For M = **43600** the exclusive range is
**43501–43600** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 43501–43550  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 43551–43575  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 43576–43590  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 43591–43600  │ decade soft gates (Wave 290 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 43600 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_43600        1          graph_batch43591.c
  gj_libcgj_green_43600       1          graph_batch43592.c
  gj_bar3_ready_43600         0          graph_batch43593.c  (bar3 open)
  gj_product_score_43600      0          graph_batch43594.c  (score open)
  gj_continuum_ready_43600    1          graph_batch43595.c
  gj_smoke_soft_43600         1          graph_batch43596.c
  gj_dyn_soft_43600           1          graph_batch43597.c
  gj_milestone_tag_43600      43600      graph_batch43598.c
  gj_continuum_wave_43600     43600      graph_batch43599.c
  gj_batch_id_43600           43600      graph_batch43600.c
  gj_graph_milestone_43600    43600      graph_batch43600.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=43600`` means parent wired CGJ_SRCS through
    ``graph_batch43600.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_43600`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_43600`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M43600_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_43500`` / ``gj_*_43400`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 290 exclusive CREATE-ONLY lands the **markers** band
  (43591–43600) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_43600.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_43600.py

  # Restrict to marker band only (soft-gate TUs; Wave 290 exclusive):
  python3 user/libcgj/src/_gen_milestone_43600.py --band markers

  # Extension identity only (43501–43550):
  python3 user/libcgj/src/_gen_milestone_43600.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_43600.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_43500.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 43600 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 43600
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 43501
WAVE_HI = 43600  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 43501, 43550
IDENTITY_LO, IDENTITY_HI = 43551, 43575
FILL_LO, FILL_HI = 43576, 43590
MARKERS_LO, MARKERS_HI = 43591, 43600
...
# Stems become gj_<stem>_43600. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (43591, "shell_green", 1, "shell product green extract (wave 43600)"),
    (43592, "libcgj_green", 1, "libcgj product green extract (wave 43600)"),
    (43593, "bar3_ready", 0, "product bar3 readiness gate (wave 43600)"),
    (43594, "product_score", 0, "product readiness score tag (wave 43600)"),
    (43595, "continuum_ready", 1, "continuum readiness gate (wave 43600)"),
    (43596, "smoke_soft", 1, "smoke soft lamp (wave 43600)"),
    (43597, "dyn_soft", 1, "dyn soft lamp (wave 43600)"),
    (43598, "milestone_tag", MILESTONE, "milestone 43600 product tag"),
    (43599, "continuum_wave", MILESTONE, "milestone 43600 continuum wave identity"),
    # 43600 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        43576: "continuum wave fill soft unit + batch id",
        43580: "continuum bridge soft unit + batch id",
        43590: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (43591–43599), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 43593
    tag_batch = MARKERS_LO + 7  # 43598
    wave_batch = MARKERS_LO + 8  # 43599
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_43500 / gj_shell_green_43400 / gj_shell_green_43300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_43500 / gj_libcgj_green_43400 / gj_libcgj_green_43300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_43500 / gj_bar3_ready_43400 / gj_bar3_ready_43300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_43500 / gj_product_score_43400 / gj_product_score_43300\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_43500 / gj_continuum_ready_43400 /\n"
            f" * gj_continuum_ready_43300 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_43500 / gj_smoke_soft_43400 / gj_smoke_soft_43300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_43500 / gj_dyn_soft_43400 / gj_dyn_soft_43300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_43500 / gj_milestone_tag_43400 / gj_milestone_tag_43300,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_43500 / gj_continuum_wave_43400 /\n"
            f" * gj_continuum_wave_43300, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch43600.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 43599
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 43591–43599; 43600 is capstone
