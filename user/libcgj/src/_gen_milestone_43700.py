...
"""
Soft continuum generator — MILESTONE 43700 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 43601–43700 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=43700   when parent has wired through graph_batch43700.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 43601 43700
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 43601 43700
...
graph batches ending at M. For M = **43700** the exclusive range is
**43601–43700** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 43601–43650  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 43651–43675  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 43676–43690  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 43691–43700  │ decade soft gates (Wave 291 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 43700 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_43700        1          graph_batch43691.c
  gj_libcgj_green_43700       1          graph_batch43692.c
  gj_bar3_ready_43700         0          graph_batch43693.c  (bar3 open)
  gj_product_score_43700      0          graph_batch43694.c  (score open)
  gj_continuum_ready_43700    1          graph_batch43695.c
  gj_smoke_soft_43700         1          graph_batch43696.c
  gj_dyn_soft_43700           1          graph_batch43697.c
  gj_milestone_tag_43700      43700      graph_batch43698.c
  gj_continuum_wave_43700     43700      graph_batch43699.c
  gj_batch_id_43700           43700      graph_batch43700.c
  gj_graph_milestone_43700    43700      graph_batch43700.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=43700`` means parent wired CGJ_SRCS through
    ``graph_batch43700.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_43700`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_43700`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M43700_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_43600`` / ``gj_*_43500`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 291 exclusive CREATE-ONLY lands the **markers** band
  (43691–43700) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_43700.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_43700.py

  # Restrict to marker band only (soft-gate TUs; Wave 291 exclusive):
  python3 user/libcgj/src/_gen_milestone_43700.py --band markers

  # Extension identity only (43601–43650):
  python3 user/libcgj/src/_gen_milestone_43700.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_43700.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_43600.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 43700 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 43700
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 43601
WAVE_HI = 43700  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 43601, 43650
IDENTITY_LO, IDENTITY_HI = 43651, 43675
FILL_LO, FILL_HI = 43676, 43690
MARKERS_LO, MARKERS_HI = 43691, 43700
...
# Stems become gj_<stem>_43700. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (43691, "shell_green", 1, "shell product green extract (wave 43700)"),
    (43692, "libcgj_green", 1, "libcgj product green extract (wave 43700)"),
    (43693, "bar3_ready", 0, "product bar3 readiness gate (wave 43700)"),
    (43694, "product_score", 0, "product readiness score tag (wave 43700)"),
    (43695, "continuum_ready", 1, "continuum readiness gate (wave 43700)"),
    (43696, "smoke_soft", 1, "smoke soft lamp (wave 43700)"),
    (43697, "dyn_soft", 1, "dyn soft lamp (wave 43700)"),
    (43698, "milestone_tag", MILESTONE, "milestone 43700 product tag"),
    (43699, "continuum_wave", MILESTONE, "milestone 43700 continuum wave identity"),
    # 43700 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        43676: "continuum wave fill soft unit + batch id",
        43680: "continuum bridge soft unit + batch id",
        43690: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (43691–43699), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 43693
    tag_batch = MARKERS_LO + 7  # 43698
    wave_batch = MARKERS_LO + 8  # 43699
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_43600 / gj_shell_green_43500 / gj_shell_green_43400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_43600 / gj_libcgj_green_43500 / gj_libcgj_green_43400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_43600 / gj_bar3_ready_43500 / gj_bar3_ready_43400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_43600 / gj_product_score_43500 / gj_product_score_43400\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_43600 / gj_continuum_ready_43500 /\n"
            f" * gj_continuum_ready_43400 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_43600 / gj_smoke_soft_43500 / gj_smoke_soft_43400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_43600 / gj_dyn_soft_43500 / gj_dyn_soft_43400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_43600 / gj_milestone_tag_43500 / gj_milestone_tag_43400,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_43600 / gj_continuum_wave_43500 /\n"
            f" * gj_continuum_wave_43400, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch43700.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 43699
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 43691–43699; 43700 is capstone
