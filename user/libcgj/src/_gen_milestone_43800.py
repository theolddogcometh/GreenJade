...
"""
Soft continuum generator — MILESTONE 43800 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 43701–43800 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=43800   when parent has wired through graph_batch43800.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 43701 43800
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 43701 43800
...
graph batches ending at M. For M = **43800** the exclusive range is
**43701–43800** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 43701–43750  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 43751–43775  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 43776–43790  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 43791–43800  │ decade soft gates (Wave 292 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 43800 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_43800        1          graph_batch43791.c
  gj_libcgj_green_43800       1          graph_batch43792.c
  gj_bar3_ready_43800         0          graph_batch43793.c  (bar3 open)
  gj_product_score_43800      0          graph_batch43794.c  (score open)
  gj_continuum_ready_43800    1          graph_batch43795.c
  gj_smoke_soft_43800         1          graph_batch43796.c
  gj_dyn_soft_43800           1          graph_batch43797.c
  gj_milestone_tag_43800      43800      graph_batch43798.c
  gj_continuum_wave_43800     43800      graph_batch43799.c
  gj_batch_id_43800           43800      graph_batch43800.c
  gj_graph_milestone_43800    43800      graph_batch43800.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=43800`` means parent wired CGJ_SRCS through
    ``graph_batch43800.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_43800`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_43800`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M43800_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_43700`` / ``gj_*_43600`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 292 exclusive CREATE-ONLY lands the **markers** band
  (43791–43800) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_43800.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_43800.py

  # Restrict to marker band only (soft-gate TUs; Wave 292 exclusive):
  python3 user/libcgj/src/_gen_milestone_43800.py --band markers

  # Extension identity only (43701–43750):
  python3 user/libcgj/src/_gen_milestone_43800.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_43800.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_43700.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 43800 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 43800
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 43701
WAVE_HI = 43800  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 43701, 43750
IDENTITY_LO, IDENTITY_HI = 43751, 43775
FILL_LO, FILL_HI = 43776, 43790
MARKERS_LO, MARKERS_HI = 43791, 43800
...
# Stems become gj_<stem>_43800. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (43791, "shell_green", 1, "shell product green extract (wave 43800)"),
    (43792, "libcgj_green", 1, "libcgj product green extract (wave 43800)"),
    (43793, "bar3_ready", 0, "product bar3 readiness gate (wave 43800)"),
    (43794, "product_score", 0, "product readiness score tag (wave 43800)"),
    (43795, "continuum_ready", 1, "continuum readiness gate (wave 43800)"),
    (43796, "smoke_soft", 1, "smoke soft lamp (wave 43800)"),
    (43797, "dyn_soft", 1, "dyn soft lamp (wave 43800)"),
    (43798, "milestone_tag", MILESTONE, "milestone 43800 product tag"),
    (43799, "continuum_wave", MILESTONE, "milestone 43800 continuum wave identity"),
    # 43800 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        43776: "continuum wave fill soft unit + batch id",
        43780: "continuum bridge soft unit + batch id",
        43790: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (43791–43799), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 43793
    tag_batch = MARKERS_LO + 7  # 43798
    wave_batch = MARKERS_LO + 8  # 43799
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_43700 / gj_shell_green_43600 / gj_shell_green_43500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_43700 / gj_libcgj_green_43600 / gj_libcgj_green_43500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_43700 / gj_bar3_ready_43600 / gj_bar3_ready_43500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_43700 / gj_product_score_43600 / gj_product_score_43500\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_43700 / gj_continuum_ready_43600 /\n"
            f" * gj_continuum_ready_43500 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_43700 / gj_smoke_soft_43600 / gj_smoke_soft_43500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_43700 / gj_dyn_soft_43600 / gj_dyn_soft_43500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_43700 / gj_milestone_tag_43600 / gj_milestone_tag_43500,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_43700 / gj_continuum_wave_43600 /\n"
            f" * gj_continuum_wave_43500, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch43800.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 43799
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 43791–43799; 43800 is capstone
