...
"""
Soft continuum generator — MILESTONE 43900 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 43801–43900 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=43900   when parent has wired through graph_batch43900.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 43801 43900
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 43801 43900
...
graph batches ending at M. For M = **43900** the exclusive range is
**43801–43900** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 43801–43850  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 43851–43875  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 43876–43890  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 43891–43900  │ decade soft gates (Wave 293 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 43900 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_43900        1          graph_batch43891.c
  gj_libcgj_green_43900       1          graph_batch43892.c
  gj_bar3_ready_43900         0          graph_batch43893.c  (bar3 open)
  gj_product_score_43900      0          graph_batch43894.c  (score open)
  gj_continuum_ready_43900    1          graph_batch43895.c
  gj_smoke_soft_43900         1          graph_batch43896.c
  gj_dyn_soft_43900           1          graph_batch43897.c
  gj_milestone_tag_43900      43900      graph_batch43898.c
  gj_continuum_wave_43900     43900      graph_batch43899.c
  gj_batch_id_43900           43900      graph_batch43900.c
  gj_graph_milestone_43900    43900      graph_batch43900.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=43900`` means parent wired CGJ_SRCS through
    ``graph_batch43900.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_43900`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_43900`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M43900_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_43800`` / ``gj_*_43700`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 293 exclusive CREATE-ONLY lands the **markers** band
  (43891–43900) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_43900.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_43900.py

  # Restrict to marker band only (soft-gate TUs; Wave 293 exclusive):
  python3 user/libcgj/src/_gen_milestone_43900.py --band markers

  # Extension identity only (43801–43850):
  python3 user/libcgj/src/_gen_milestone_43900.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_43900.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_43800.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 43900 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 43900
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 43801
WAVE_HI = 43900  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 43801, 43850
IDENTITY_LO, IDENTITY_HI = 43851, 43875
FILL_LO, FILL_HI = 43876, 43890
MARKERS_LO, MARKERS_HI = 43891, 43900
...
# Stems become gj_<stem>_43900. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (43891, "shell_green", 1, "shell product green extract (wave 43900)"),
    (43892, "libcgj_green", 1, "libcgj product green extract (wave 43900)"),
    (43893, "bar3_ready", 0, "product bar3 readiness gate (wave 43900)"),
    (43894, "product_score", 0, "product readiness score tag (wave 43900)"),
    (43895, "continuum_ready", 1, "continuum readiness gate (wave 43900)"),
    (43896, "smoke_soft", 1, "smoke soft lamp (wave 43900)"),
    (43897, "dyn_soft", 1, "dyn soft lamp (wave 43900)"),
    (43898, "milestone_tag", MILESTONE, "milestone 43900 product tag"),
    (43899, "continuum_wave", MILESTONE, "milestone 43900 continuum wave identity"),
    # 43900 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        43876: "continuum wave fill soft unit + batch id",
        43880: "continuum bridge soft unit + batch id",
        43890: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (43891–43899), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 43893
    tag_batch = MARKERS_LO + 7  # 43898
    wave_batch = MARKERS_LO + 8  # 43899
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_43800 / gj_shell_green_43700 / gj_shell_green_43600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_43800 / gj_libcgj_green_43700 / gj_libcgj_green_43600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_43800 / gj_bar3_ready_43700 / gj_bar3_ready_43600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_43800 / gj_product_score_43700 / gj_product_score_43600\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_43800 / gj_continuum_ready_43700 /\n"
            f" * gj_continuum_ready_43600 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_43800 / gj_smoke_soft_43700 / gj_smoke_soft_43600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_43800 / gj_dyn_soft_43700 / gj_dyn_soft_43600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_43800 / gj_milestone_tag_43700 / gj_milestone_tag_43600,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_43800 / gj_continuum_wave_43700 /\n"
            f" * gj_continuum_wave_43600, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch43900.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 43899
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 43891–43899; 43900 is capstone
