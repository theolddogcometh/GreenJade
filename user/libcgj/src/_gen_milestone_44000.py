...
"""
Soft continuum generator — MILESTONE 44000 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 43901–44000 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=44000   when parent has wired through graph_batch44000.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 43901 44000
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 43901 44000
...
graph batches ending at M. For M = **44000** the exclusive range is
**43901–44000** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 43901–43950  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 43951–43975  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 43976–43990  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 43991–44000  │ decade soft gates (Wave 294 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 44000 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_44000        1          graph_batch43991.c
  gj_libcgj_green_44000       1          graph_batch43992.c
  gj_bar3_ready_44000         0          graph_batch43993.c  (bar3 open)
  gj_product_score_44000      0          graph_batch43994.c  (score open)
  gj_continuum_ready_44000    1          graph_batch43995.c
  gj_smoke_soft_44000         1          graph_batch43996.c
  gj_dyn_soft_44000           1          graph_batch43997.c
  gj_milestone_tag_44000      44000      graph_batch43998.c
  gj_continuum_wave_44000     44000      graph_batch43999.c
  gj_batch_id_44000           44000      graph_batch44000.c
  gj_graph_milestone_44000    44000      graph_batch44000.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=44000`` means parent wired CGJ_SRCS through
    ``graph_batch44000.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_44000`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_44000`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M44000_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_43900`` / ``gj_*_43800`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 294 exclusive CREATE-ONLY lands the **markers** band
  (43991–44000) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_44000.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_44000.py

  # Restrict to marker band only (soft-gate TUs; Wave 294 exclusive):
  python3 user/libcgj/src/_gen_milestone_44000.py --band markers

  # Extension identity only (43901–43950):
  python3 user/libcgj/src/_gen_milestone_44000.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_44000.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_43900.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 44000 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 44000
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 43901
WAVE_HI = 44000  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 43901, 43950
IDENTITY_LO, IDENTITY_HI = 43951, 43975
FILL_LO, FILL_HI = 43976, 43990
MARKERS_LO, MARKERS_HI = 43991, 44000
...
# Stems become gj_<stem>_44000. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (43991, "shell_green", 1, "shell product green extract (wave 44000)"),
    (43992, "libcgj_green", 1, "libcgj product green extract (wave 44000)"),
    (43993, "bar3_ready", 0, "product bar3 readiness gate (wave 44000)"),
    (43994, "product_score", 0, "product readiness score tag (wave 44000)"),
    (43995, "continuum_ready", 1, "continuum readiness gate (wave 44000)"),
    (43996, "smoke_soft", 1, "smoke soft lamp (wave 44000)"),
    (43997, "dyn_soft", 1, "dyn soft lamp (wave 44000)"),
    (43998, "milestone_tag", MILESTONE, "milestone 44000 product tag"),
    (43999, "continuum_wave", MILESTONE, "milestone 44000 continuum wave identity"),
    # 44000 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        43976: "continuum wave fill soft unit + batch id",
        43980: "continuum bridge soft unit + batch id",
        43990: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (43991–43999), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 43993
    tag_batch = MARKERS_LO + 7  # 43998
    wave_batch = MARKERS_LO + 8  # 43999
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_43900 / gj_shell_green_43800 / gj_shell_green_43700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_43900 / gj_libcgj_green_43800 / gj_libcgj_green_43700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_43900 / gj_bar3_ready_43800 / gj_bar3_ready_43700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_43900 / gj_product_score_43800 / gj_product_score_43700\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_43900 / gj_continuum_ready_43800 /\n"
            f" * gj_continuum_ready_43700 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_43900 / gj_smoke_soft_43800 / gj_smoke_soft_43700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_43900 / gj_dyn_soft_43800 / gj_dyn_soft_43700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_43900 / gj_milestone_tag_43800 / gj_milestone_tag_43700,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_43900 / gj_continuum_wave_43800 /\n"
            f" * gj_continuum_wave_43700, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch44000.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 43999
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 43991–43999; 44000 is capstone
