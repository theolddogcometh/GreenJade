...
"""
Soft continuum generator — MILESTONE 44100 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 44001–44100 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=44100   when parent has wired through graph_batch44100.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 44001 44100
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 44001 44100
...
graph batches ending at M. For M = **44100** the exclusive range is
**44001–44100** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 44001–44050  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 44051–44075  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 44076–44090  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 44091–44100  │ decade soft gates (Wave 295 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 44100 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_44100        1          graph_batch44091.c
  gj_libcgj_green_44100       1          graph_batch44092.c
  gj_bar3_ready_44100         0          graph_batch44093.c  (bar3 open)
  gj_product_score_44100      0          graph_batch44094.c  (score open)
  gj_continuum_ready_44100    1          graph_batch44095.c
  gj_smoke_soft_44100         1          graph_batch44096.c
  gj_dyn_soft_44100           1          graph_batch44097.c
  gj_milestone_tag_44100      44100      graph_batch44098.c
  gj_continuum_wave_44100     44100      graph_batch44099.c
  gj_batch_id_44100           44100      graph_batch44100.c
  gj_graph_milestone_44100    44100      graph_batch44100.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=44100`` means parent wired CGJ_SRCS through
    ``graph_batch44100.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_44100`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_44100`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M44100_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_44000`` / ``gj_*_43900`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 295 exclusive CREATE-ONLY lands the **markers** band
  (44091–44100) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_44100.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_44100.py

  # Restrict to marker band only (soft-gate TUs; Wave 295 exclusive):
  python3 user/libcgj/src/_gen_milestone_44100.py --band markers

  # Extension identity only (44001–44050):
  python3 user/libcgj/src/_gen_milestone_44100.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_44100.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_44000.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 44100 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 44100
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 44001
WAVE_HI = 44100  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 44001, 44050
IDENTITY_LO, IDENTITY_HI = 44051, 44075
FILL_LO, FILL_HI = 44076, 44090
MARKERS_LO, MARKERS_HI = 44091, 44100
...
# Stems become gj_<stem>_44100. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (44091, "shell_green", 1, "shell product green extract (wave 44100)"),
    (44092, "libcgj_green", 1, "libcgj product green extract (wave 44100)"),
    (44093, "bar3_ready", 0, "product bar3 readiness gate (wave 44100)"),
    (44094, "product_score", 0, "product readiness score tag (wave 44100)"),
    (44095, "continuum_ready", 1, "continuum readiness gate (wave 44100)"),
    (44096, "smoke_soft", 1, "smoke soft lamp (wave 44100)"),
    (44097, "dyn_soft", 1, "dyn soft lamp (wave 44100)"),
    (44098, "milestone_tag", MILESTONE, "milestone 44100 product tag"),
    (44099, "continuum_wave", MILESTONE, "milestone 44100 continuum wave identity"),
    # 44100 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        44076: "continuum wave fill soft unit + batch id",
        44080: "continuum bridge soft unit + batch id",
        44090: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (44091–44099), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 44093
    tag_batch = MARKERS_LO + 7  # 44098
    wave_batch = MARKERS_LO + 8  # 44099
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_44000 / gj_shell_green_43900 / gj_shell_green_43800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_44000 / gj_libcgj_green_43900 / gj_libcgj_green_43800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_44000 / gj_bar3_ready_43900 / gj_bar3_ready_43800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_44000 / gj_product_score_43900 / gj_product_score_43800\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_44000 / gj_continuum_ready_43900 /\n"
            f" * gj_continuum_ready_43800 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_44000 / gj_smoke_soft_43900 / gj_smoke_soft_43800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_44000 / gj_dyn_soft_43900 / gj_dyn_soft_43800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_44000 / gj_milestone_tag_43900 / gj_milestone_tag_43800,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_44000 / gj_continuum_wave_43900 /\n"
            f" * gj_continuum_wave_43800, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch44100.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 44099
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 44091–44099; 44100 is capstone
