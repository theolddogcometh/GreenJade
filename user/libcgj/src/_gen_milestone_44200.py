...
"""
Soft continuum generator — MILESTONE 44200 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 44101–44200 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=44200   when parent has wired through graph_batch44200.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 44101 44200
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 44101 44200
...
graph batches ending at M. For M = **44200** the exclusive range is
**44101–44200** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 44101–44150  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 44151–44175  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 44176–44190  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 44191–44200  │ decade soft gates (Wave 296 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 44200 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_44200        1          graph_batch44191.c
  gj_libcgj_green_44200       1          graph_batch44192.c
  gj_bar3_ready_44200         0          graph_batch44193.c  (bar3 open)
  gj_product_score_44200      0          graph_batch44194.c  (score open)
  gj_continuum_ready_44200    1          graph_batch44195.c
  gj_smoke_soft_44200         1          graph_batch44196.c
  gj_dyn_soft_44200           1          graph_batch44197.c
  gj_milestone_tag_44200      44200      graph_batch44198.c
  gj_continuum_wave_44200     44200      graph_batch44199.c
  gj_batch_id_44200           44200      graph_batch44200.c
  gj_graph_milestone_44200    44200      graph_batch44200.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=44200`` means parent wired CGJ_SRCS through
    ``graph_batch44200.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_44200`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_44200`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M44200_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_44100`` / ``gj_*_44000`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 296 exclusive CREATE-ONLY lands the **markers** band
  (44191–44200) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_44200.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_44200.py

  # Restrict to marker band only (soft-gate TUs; Wave 296 exclusive):
  python3 user/libcgj/src/_gen_milestone_44200.py --band markers

  # Extension identity only (44101–44150):
  python3 user/libcgj/src/_gen_milestone_44200.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_44200.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_44100.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 44200 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 44200
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 44101
WAVE_HI = 44200  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 44101, 44150
IDENTITY_LO, IDENTITY_HI = 44151, 44175
FILL_LO, FILL_HI = 44176, 44190
MARKERS_LO, MARKERS_HI = 44191, 44200
...
# Stems become gj_<stem>_44200. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (44191, "shell_green", 1, "shell product green extract (wave 44200)"),
    (44192, "libcgj_green", 1, "libcgj product green extract (wave 44200)"),
    (44193, "bar3_ready", 0, "product bar3 readiness gate (wave 44200)"),
    (44194, "product_score", 0, "product readiness score tag (wave 44200)"),
    (44195, "continuum_ready", 1, "continuum readiness gate (wave 44200)"),
    (44196, "smoke_soft", 1, "smoke soft lamp (wave 44200)"),
    (44197, "dyn_soft", 1, "dyn soft lamp (wave 44200)"),
    (44198, "milestone_tag", MILESTONE, "milestone 44200 product tag"),
    (44199, "continuum_wave", MILESTONE, "milestone 44200 continuum wave identity"),
    # 44200 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        44176: "continuum wave fill soft unit + batch id",
        44180: "continuum bridge soft unit + batch id",
        44190: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (44191–44199), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 44193
    tag_batch = MARKERS_LO + 7  # 44198
    wave_batch = MARKERS_LO + 8  # 44199
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_44100 / gj_shell_green_44000 / gj_shell_green_43900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_44100 / gj_libcgj_green_44000 / gj_libcgj_green_43900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_44100 / gj_bar3_ready_44000 / gj_bar3_ready_43900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_44100 / gj_product_score_44000 / gj_product_score_43900\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_44100 / gj_continuum_ready_44000 /\n"
            f" * gj_continuum_ready_43900 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_44100 / gj_smoke_soft_44000 / gj_smoke_soft_43900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_44100 / gj_dyn_soft_44000 / gj_dyn_soft_43900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_44100 / gj_milestone_tag_44000 / gj_milestone_tag_43900,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_44100 / gj_continuum_wave_44000 /\n"
            f" * gj_continuum_wave_43900, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch44200.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 44199
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 44191–44199; 44200 is capstone
