...
"""
Soft continuum generator — MILESTONE 44300 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 44201–44300 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=44300   when parent has wired through graph_batch44300.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 44201 44300
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 44201 44300
...
graph batches ending at M. For M = **44300** the exclusive range is
**44201–44300** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 44201–44250  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 44251–44275  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 44276–44290  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 44291–44300  │ decade soft gates (Wave 297 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 44300 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_44300        1          graph_batch44291.c
  gj_libcgj_green_44300       1          graph_batch44292.c
  gj_bar3_ready_44300         0          graph_batch44293.c  (bar3 open)
  gj_product_score_44300      0          graph_batch44294.c  (score open)
  gj_continuum_ready_44300    1          graph_batch44295.c
  gj_smoke_soft_44300         1          graph_batch44296.c
  gj_dyn_soft_44300           1          graph_batch44297.c
  gj_milestone_tag_44300      44300      graph_batch44298.c
  gj_continuum_wave_44300     44300      graph_batch44299.c
  gj_batch_id_44300           44300      graph_batch44300.c
  gj_graph_milestone_44300    44300      graph_batch44300.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=44300`` means parent wired CGJ_SRCS through
    ``graph_batch44300.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_44300`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_44300`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M44300_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_44200`` / ``gj_*_44100`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 297 exclusive CREATE-ONLY lands the **markers** band
  (44291–44300) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_44300.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_44300.py

  # Restrict to marker band only (soft-gate TUs; Wave 297 exclusive):
  python3 user/libcgj/src/_gen_milestone_44300.py --band markers

  # Extension identity only (44201–44250):
  python3 user/libcgj/src/_gen_milestone_44300.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_44300.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_44200.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 44300 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 44300
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 44201
WAVE_HI = 44300  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 44201, 44250
IDENTITY_LO, IDENTITY_HI = 44251, 44275
FILL_LO, FILL_HI = 44276, 44290
MARKERS_LO, MARKERS_HI = 44291, 44300
...
# Stems become gj_<stem>_44300. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (44291, "shell_green", 1, "shell product green extract (wave 44300)"),
    (44292, "libcgj_green", 1, "libcgj product green extract (wave 44300)"),
    (44293, "bar3_ready", 0, "product bar3 readiness gate (wave 44300)"),
    (44294, "product_score", 0, "product readiness score tag (wave 44300)"),
    (44295, "continuum_ready", 1, "continuum readiness gate (wave 44300)"),
    (44296, "smoke_soft", 1, "smoke soft lamp (wave 44300)"),
    (44297, "dyn_soft", 1, "dyn soft lamp (wave 44300)"),
    (44298, "milestone_tag", MILESTONE, "milestone 44300 product tag"),
    (44299, "continuum_wave", MILESTONE, "milestone 44300 continuum wave identity"),
    # 44300 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        44276: "continuum wave fill soft unit + batch id",
        44280: "continuum bridge soft unit + batch id",
        44290: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (44291–44299), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 44293
    tag_batch = MARKERS_LO + 7  # 44298
    wave_batch = MARKERS_LO + 8  # 44299
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_44200 / gj_shell_green_44100 / gj_shell_green_44000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_44200 / gj_libcgj_green_44100 / gj_libcgj_green_44000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_44200 / gj_bar3_ready_44100 / gj_bar3_ready_44000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_44200 / gj_product_score_44100 / gj_product_score_44000\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_44200 / gj_continuum_ready_44100 /\n"
            f" * gj_continuum_ready_44000 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_44200 / gj_smoke_soft_44100 / gj_smoke_soft_44000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_44200 / gj_dyn_soft_44100 / gj_dyn_soft_44000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_44200 / gj_milestone_tag_44100 / gj_milestone_tag_44000,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_44200 / gj_continuum_wave_44100 /\n"
            f" * gj_continuum_wave_44000, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch44300.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 44299
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 44291–44299; 44300 is capstone
