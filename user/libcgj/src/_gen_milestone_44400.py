...
"""
Soft continuum generator — MILESTONE 44400 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 44301–44400 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=44400   when parent has wired through graph_batch44400.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 44301 44400
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 44301 44400
...
graph batches ending at M. For M = **44400** the exclusive range is
**44301–44400** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 44301–44350  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 44351–44375  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 44376–44390  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 44391–44400  │ decade soft gates (Wave 298 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 44400 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_44400        1          graph_batch44391.c
  gj_libcgj_green_44400       1          graph_batch44392.c
  gj_bar3_ready_44400         0          graph_batch44393.c  (bar3 open)
  gj_product_score_44400      0          graph_batch44394.c  (score open)
  gj_continuum_ready_44400    1          graph_batch44395.c
  gj_smoke_soft_44400         1          graph_batch44396.c
  gj_dyn_soft_44400           1          graph_batch44397.c
  gj_milestone_tag_44400      44400      graph_batch44398.c
  gj_continuum_wave_44400     44400      graph_batch44399.c
  gj_batch_id_44400           44400      graph_batch44400.c
  gj_graph_milestone_44400    44400      graph_batch44400.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=44400`` means parent wired CGJ_SRCS through
    ``graph_batch44400.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_44400`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_44400`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M44400_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_44300`` / ``gj_*_44200`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 298 exclusive CREATE-ONLY lands the **markers** band
  (44391–44400) first; extension/identity/fill may be emitted later via
...
-----
  ...
  # Restrict to marker band only (soft-gate TUs; Wave 298 exclusive):
  python3 user/libcgj/src/_gen_milestone_44400.py --band markers

  # Extension identity only (44301–44350):
...
See also
--------
  user/libcgj/src/_gen_milestone_44300.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 44400 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 44400
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 44301
WAVE_HI = 44400  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 44301, 44350
IDENTITY_LO, IDENTITY_HI = 44351, 44375
FILL_LO, FILL_HI = 44376, 44390
MARKERS_LO, MARKERS_HI = 44391, 44400
...
# Stems become gj_<stem>_44400. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (44391, "shell_green", 1, "shell product green extract (wave 44400)"),
    (44392, "libcgj_green", 1, "libcgj product green extract (wave 44400)"),
    (44393, "bar3_ready", 0, "product bar3 readiness gate (wave 44400)"),
    (44394, "product_score", 0, "product readiness score tag (wave 44400)"),
    (44395, "continuum_ready", 1, "continuum readiness gate (wave 44400)"),
    (44396, "smoke_soft", 1, "smoke soft lamp (wave 44400)"),
    (44397, "dyn_soft", 1, "dyn soft lamp (wave 44400)"),
    (44398, "milestone_tag", MILESTONE, "milestone 44400 product tag"),
    (44399, "continuum_wave", MILESTONE, "milestone 44400 continuum wave identity"),
    # 44400 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        44376: "continuum wave fill soft unit + batch id",
        44380: "continuum bridge soft unit + batch id",
        44390: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (44391–44399), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 44393
    tag_batch = MARKERS_LO + 7  # 44398
    wave_batch = MARKERS_LO + 8  # 44399
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_44300 / gj_shell_green_44200 / gj_shell_green_44100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_44300 / gj_libcgj_green_44200 / gj_libcgj_green_44100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_44300 / gj_bar3_ready_44200 / gj_bar3_ready_44100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_44300 / gj_product_score_44200 / gj_product_score_44100\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_44300 / gj_continuum_ready_44200 /\n"
            f" * gj_continuum_ready_44100 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_44300 / gj_smoke_soft_44200 / gj_smoke_soft_44100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_44300 / gj_dyn_soft_44200 / gj_dyn_soft_44100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_44300 / gj_milestone_tag_44200 / gj_milestone_tag_44100,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_44300 / gj_continuum_wave_44200 /\n"
            f" * gj_continuum_wave_44100, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch44400.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 44399
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 44391–44399; 44400 is capstone
