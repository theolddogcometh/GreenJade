...
"""
Soft continuum generator — MILESTONE 44500 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 44401–44500 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=44500   when parent has wired through graph_batch44500.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 44401 44500
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 44401 44500
...
graph batches ending at M. For M = **44500** the exclusive range is
**44401–44500** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 44401–44450  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 44451–44475  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 44476–44490  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 44491–44500  │ decade soft gates (Wave 299 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 44500 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_44500        1          graph_batch44491.c
  gj_libcgj_green_44500       1          graph_batch44492.c
  gj_bar3_ready_44500         0          graph_batch44493.c  (bar3 open)
  gj_product_score_44500      0          graph_batch44494.c  (score open)
  gj_continuum_ready_44500    1          graph_batch44495.c
  gj_smoke_soft_44500         1          graph_batch44496.c
  gj_dyn_soft_44500           1          graph_batch44497.c
  gj_milestone_tag_44500      44500      graph_batch44498.c
  gj_continuum_wave_44500     44500      graph_batch44499.c
  gj_batch_id_44500           44500      graph_batch44500.c
  gj_graph_milestone_44500    44500      graph_batch44500.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=44500`` means parent wired CGJ_SRCS through
    ``graph_batch44500.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_44500`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_44500`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M44500_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_44400`` / ``gj_*_44300`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 299 exclusive CREATE-ONLY lands the **markers** band
  (44491–44500) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_44400.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_44400.py

  # Restrict to marker band only (soft-gate TUs; Wave 299 exclusive):
  python3 user/libcgj/src/_gen_milestone_44400.py --band markers

  # Extension identity only (44401–44450):
  python3 user/libcgj/src/_gen_milestone_44400.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_44400.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_44400.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 44500 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 44500
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 44401
WAVE_HI = 44500  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 44401, 44450
IDENTITY_LO, IDENTITY_HI = 44451, 44475
FILL_LO, FILL_HI = 44476, 44490
MARKERS_LO, MARKERS_HI = 44491, 44500
...
# Stems become gj_<stem>_44500. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (44491, "shell_green", 1, "shell product green extract (wave 44500)"),
    (44492, "libcgj_green", 1, "libcgj product green extract (wave 44500)"),
    (44493, "bar3_ready", 0, "product bar3 readiness gate (wave 44500)"),
    (44494, "product_score", 0, "product readiness score tag (wave 44500)"),
    (44495, "continuum_ready", 1, "continuum readiness gate (wave 44500)"),
    (44496, "smoke_soft", 1, "smoke soft lamp (wave 44500)"),
    (44497, "dyn_soft", 1, "dyn soft lamp (wave 44500)"),
    (44498, "milestone_tag", MILESTONE, "milestone 44500 product tag"),
    (44499, "continuum_wave", MILESTONE, "milestone 44500 continuum wave identity"),
    # 44500 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        44476: "continuum wave fill soft unit + batch id",
        44480: "continuum bridge soft unit + batch id",
        44490: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (44491–44499), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 44493
    tag_batch = MARKERS_LO + 7  # 44498
    wave_batch = MARKERS_LO + 8  # 44499
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_44400 / gj_shell_green_44300 / gj_shell_green_44200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_44400 / gj_libcgj_green_44300 / gj_libcgj_green_44200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_44400 / gj_bar3_ready_44300 / gj_bar3_ready_44200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_44400 / gj_product_score_44300 / gj_product_score_44200\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_44400 / gj_continuum_ready_44300 /\n"
            f" * gj_continuum_ready_44200 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_44400 / gj_smoke_soft_44300 / gj_smoke_soft_44200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_44400 / gj_dyn_soft_44300 / gj_dyn_soft_44200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_44400 / gj_milestone_tag_44300 / gj_milestone_tag_44200,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_44400 / gj_continuum_wave_44300 /\n"
            f" * gj_continuum_wave_44200, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch44500.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 44499
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 44491–44499; 44500 is capstone
