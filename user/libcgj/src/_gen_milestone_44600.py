...
"""
Soft continuum generator — MILESTONE 44600 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 44501–44600 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=44600   when parent has wired through graph_batch44600.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 44501 44600
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 44501 44600
...
graph batches ending at M. For M = **44600** the exclusive range is
**44501–44600** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 44501–44550  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 44551–44575  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 44576–44590  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 44591–44600  │ decade soft gates (Wave 300 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 44600 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_44600        1          graph_batch44591.c
  gj_libcgj_green_44600       1          graph_batch44592.c
  gj_bar3_ready_44600         0          graph_batch44593.c  (bar3 open)
  gj_product_score_44600      0          graph_batch44594.c  (score open)
  gj_continuum_ready_44600    1          graph_batch44595.c
  gj_smoke_soft_44600         1          graph_batch44596.c
  gj_dyn_soft_44600           1          graph_batch44597.c
  gj_milestone_tag_44600      44600      graph_batch44598.c
  gj_continuum_wave_44600     44600      graph_batch44599.c
  gj_batch_id_44600           44600      graph_batch44600.c
  gj_graph_milestone_44600    44600      graph_batch44600.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=44600`` means parent wired CGJ_SRCS through
    ``graph_batch44600.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_44600`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_44600`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M44600_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_44500`` / ``gj_*_44400`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 300 exclusive CREATE-ONLY lands the **markers** band
  (44591–44600) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_44600.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_44600.py

  # Restrict to marker band only (soft-gate TUs; Wave 300 exclusive):
  python3 user/libcgj/src/_gen_milestone_44600.py --band markers

  # Extension identity only (44501–44550):
  python3 user/libcgj/src/_gen_milestone_44600.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_44600.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_44500.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 44600 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 44600
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 44501
WAVE_HI = 44600  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 44501, 44550
IDENTITY_LO, IDENTITY_HI = 44551, 44575
FILL_LO, FILL_HI = 44576, 44590
MARKERS_LO, MARKERS_HI = 44591, 44600
...
# Stems become gj_<stem>_44600. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (44591, "shell_green", 1, "shell product green extract (wave 44600)"),
    (44592, "libcgj_green", 1, "libcgj product green extract (wave 44600)"),
    (44593, "bar3_ready", 0, "product bar3 readiness gate (wave 44600)"),
    (44594, "product_score", 0, "product readiness score tag (wave 44600)"),
    (44595, "continuum_ready", 1, "continuum readiness gate (wave 44600)"),
    (44596, "smoke_soft", 1, "smoke soft lamp (wave 44600)"),
    (44597, "dyn_soft", 1, "dyn soft lamp (wave 44600)"),
    (44598, "milestone_tag", MILESTONE, "milestone 44600 product tag"),
    (44599, "continuum_wave", MILESTONE, "milestone 44600 continuum wave identity"),
    # 44600 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        44576: "continuum wave fill soft unit + batch id",
        44580: "continuum bridge soft unit + batch id",
        44590: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (44591–44599), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 44593
    tag_batch = MARKERS_LO + 7  # 44598
    wave_batch = MARKERS_LO + 8  # 44599
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_44500 / gj_shell_green_44400 / gj_shell_green_44300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_44500 / gj_libcgj_green_44400 / gj_libcgj_green_44300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_44500 / gj_bar3_ready_44400 / gj_bar3_ready_44300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_44500 / gj_product_score_44400 / gj_product_score_44300\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_44500 / gj_continuum_ready_44400 /\n"
            f" * gj_continuum_ready_44300 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_44500 / gj_smoke_soft_44400 / gj_smoke_soft_44300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_44500 / gj_dyn_soft_44400 / gj_dyn_soft_44300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_44500 / gj_milestone_tag_44400 / gj_milestone_tag_44300,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_44500 / gj_continuum_wave_44400 /\n"
            f" * gj_continuum_wave_44300, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch44600.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 44599
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 44591–44599; 44600 is capstone
