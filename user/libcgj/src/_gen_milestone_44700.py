...
"""
Soft continuum generator — MILESTONE 44700 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 44601–44700 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=44700   when parent has wired through graph_batch44700.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 44601 44700
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 44601 44700
...
graph batches ending at M. For M = **44700** the exclusive range is
**44601–44700** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 44601–44650  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 44651–44675  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 44676–44690  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 44691–44700  │ decade soft gates (Wave 301 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 44700 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_44700        1          graph_batch44691.c
  gj_libcgj_green_44700       1          graph_batch44692.c
  gj_bar3_ready_44700         0          graph_batch44693.c  (bar3 open)
  gj_product_score_44700      0          graph_batch44694.c  (score open)
  gj_continuum_ready_44700    1          graph_batch44695.c
  gj_smoke_soft_44700         1          graph_batch44696.c
  gj_dyn_soft_44700           1          graph_batch44697.c
  gj_milestone_tag_44700      44700      graph_batch44698.c
  gj_continuum_wave_44700     44700      graph_batch44699.c
  gj_batch_id_44700           44700      graph_batch44700.c
  gj_graph_milestone_44700    44700      graph_batch44700.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=44700`` means parent wired CGJ_SRCS through
    ``graph_batch44700.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_44700`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_44700`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M44700_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_44600`` / ``gj_*_44500`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 301 exclusive CREATE-ONLY lands the **markers** band
  (44691–44700) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_44700.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_44700.py

  # Restrict to marker band only (soft-gate TUs; Wave 301 exclusive):
  python3 user/libcgj/src/_gen_milestone_44700.py --band markers

  # Extension identity only (44601–44650):
  python3 user/libcgj/src/_gen_milestone_44700.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_44700.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_44600.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 44700 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 44700
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 44601
WAVE_HI = 44700  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 44601, 44650
IDENTITY_LO, IDENTITY_HI = 44651, 44675
FILL_LO, FILL_HI = 44676, 44690
MARKERS_LO, MARKERS_HI = 44691, 44700
...
# Stems become gj_<stem>_44700. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (44691, "shell_green", 1, "shell product green extract (wave 44700)"),
    (44692, "libcgj_green", 1, "libcgj product green extract (wave 44700)"),
    (44693, "bar3_ready", 0, "product bar3 readiness gate (wave 44700)"),
    (44694, "product_score", 0, "product readiness score tag (wave 44700)"),
    (44695, "continuum_ready", 1, "continuum readiness gate (wave 44700)"),
    (44696, "smoke_soft", 1, "smoke soft lamp (wave 44700)"),
    (44697, "dyn_soft", 1, "dyn soft lamp (wave 44700)"),
    (44698, "milestone_tag", MILESTONE, "milestone 44700 product tag"),
    (44699, "continuum_wave", MILESTONE, "milestone 44700 continuum wave identity"),
    # 44700 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        44676: "continuum wave fill soft unit + batch id",
        44680: "continuum bridge soft unit + batch id",
        44690: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (44691–44699), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 44693
    tag_batch = MARKERS_LO + 7  # 44698
    wave_batch = MARKERS_LO + 8  # 44699
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_44600 / gj_shell_green_44500 / gj_shell_green_44400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_44600 / gj_libcgj_green_44500 / gj_libcgj_green_44400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_44600 / gj_bar3_ready_44500 / gj_bar3_ready_44400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_44600 / gj_product_score_44500 / gj_product_score_44400\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_44600 / gj_continuum_ready_44500 /\n"
            f" * gj_continuum_ready_44400 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_44600 / gj_smoke_soft_44500 / gj_smoke_soft_44400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_44600 / gj_dyn_soft_44500 / gj_dyn_soft_44400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_44600 / gj_milestone_tag_44500 / gj_milestone_tag_44400,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_44600 / gj_continuum_wave_44500 /\n"
            f" * gj_continuum_wave_44400, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch44700.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 44699
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 44691–44699; 44700 is capstone
