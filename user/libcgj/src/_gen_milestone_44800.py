...
"""
Soft continuum generator — MILESTONE 44800 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 44701–44800 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=44800   when parent has wired through graph_batch44800.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 44701 44800
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 44701 44800
...
graph batches ending at M. For M = **44800** the exclusive range is
**44701–44800** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 44701–44750  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 44751–44775  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 44776–44790  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 44791–44800  │ decade soft gates (Wave 302 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 44800 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_44800        1          graph_batch44791.c
  gj_libcgj_green_44800       1          graph_batch44792.c
  gj_bar3_ready_44800         0          graph_batch44793.c  (bar3 open)
  gj_product_score_44800      0          graph_batch44794.c  (score open)
  gj_continuum_ready_44800    1          graph_batch44795.c
  gj_smoke_soft_44800         1          graph_batch44796.c
  gj_dyn_soft_44800           1          graph_batch44797.c
  gj_milestone_tag_44800      44800      graph_batch44798.c
  gj_continuum_wave_44800     44800      graph_batch44799.c
  gj_batch_id_44800           44800      graph_batch44800.c
  gj_graph_milestone_44800    44800      graph_batch44800.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=44800`` means parent wired CGJ_SRCS through
    ``graph_batch44800.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_44800`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_44800`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M44800_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_44700`` / ``gj_*_44600`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 302 exclusive CREATE-ONLY lands the **markers** band
  (44791–44800) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_44800.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_44800.py

  # Restrict to marker band only (soft-gate TUs; Wave 302 exclusive):
  python3 user/libcgj/src/_gen_milestone_44800.py --band markers

  # Extension identity only (44701–44750):
  python3 user/libcgj/src/_gen_milestone_44800.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_44800.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_44700.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 44800 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 44800
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 44701
WAVE_HI = 44800  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 44701, 44750
IDENTITY_LO, IDENTITY_HI = 44751, 44775
FILL_LO, FILL_HI = 44776, 44790
MARKERS_LO, MARKERS_HI = 44791, 44800
...
# Stems become gj_<stem>_44800. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (44791, "shell_green", 1, "shell product green extract (wave 44800)"),
    (44792, "libcgj_green", 1, "libcgj product green extract (wave 44800)"),
    (44793, "bar3_ready", 0, "product bar3 readiness gate (wave 44800)"),
    (44794, "product_score", 0, "product readiness score tag (wave 44800)"),
    (44795, "continuum_ready", 1, "continuum readiness gate (wave 44800)"),
    (44796, "smoke_soft", 1, "smoke soft lamp (wave 44800)"),
    (44797, "dyn_soft", 1, "dyn soft lamp (wave 44800)"),
    (44798, "milestone_tag", MILESTONE, "milestone 44800 product tag"),
    (44799, "continuum_wave", MILESTONE, "milestone 44800 continuum wave identity"),
    # 44800 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        44776: "continuum wave fill soft unit + batch id",
        44780: "continuum bridge soft unit + batch id",
        44790: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (44791–44799), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 44793
    tag_batch = MARKERS_LO + 7  # 44798
    wave_batch = MARKERS_LO + 8  # 44799
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_44700 / gj_shell_green_44600 / gj_shell_green_44500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_44700 / gj_libcgj_green_44600 / gj_libcgj_green_44500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_44700 / gj_bar3_ready_44600 / gj_bar3_ready_44500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_44700 / gj_product_score_44600 / gj_product_score_44500\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_44700 / gj_continuum_ready_44600 /\n"
            f" * gj_continuum_ready_44500 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_44700 / gj_smoke_soft_44600 / gj_smoke_soft_44500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_44700 / gj_dyn_soft_44600 / gj_dyn_soft_44500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_44700 / gj_milestone_tag_44600 / gj_milestone_tag_44500,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_44700 / gj_continuum_wave_44600 /\n"
            f" * gj_continuum_wave_44500, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch44800.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 44799
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 44791–44799; 44800 is capstone
