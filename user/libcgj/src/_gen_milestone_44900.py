...
"""
Soft continuum generator — MILESTONE 44900 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 44801–44900 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=44900   when parent has wired through graph_batch44900.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 44801 44900
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 44801 44900
...
graph batches ending at M. For M = **44900** the exclusive range is
**44801–44900** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 44801–44850  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 44851–44875  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 44876–44890  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 44891–44900  │ decade soft gates (Wave 303 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 44900 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_44900        1          graph_batch44891.c
  gj_libcgj_green_44900       1          graph_batch44892.c
  gj_bar3_ready_44900         0          graph_batch44893.c  (bar3 open)
  gj_product_score_44900      0          graph_batch44894.c  (score open)
  gj_continuum_ready_44900    1          graph_batch44895.c
  gj_smoke_soft_44900         1          graph_batch44896.c
  gj_dyn_soft_44900           1          graph_batch44897.c
  gj_milestone_tag_44900      44900      graph_batch44898.c
  gj_continuum_wave_44900     44900      graph_batch44899.c
  gj_batch_id_44900           44900      graph_batch44900.c
  gj_graph_milestone_44900    44900      graph_batch44900.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=44900`` means parent wired CGJ_SRCS through
    ``graph_batch44900.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_44900`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_44900`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M44900_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_44800`` / ``gj_*_44700`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 303 exclusive CREATE-ONLY lands the **markers** band
  (44891–44900) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_44900.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_44900.py

  # Restrict to marker band only (soft-gate TUs; Wave 303 exclusive):
  python3 user/libcgj/src/_gen_milestone_44900.py --band markers

  # Extension identity only (44801–44850):
  python3 user/libcgj/src/_gen_milestone_44900.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_44900.py --force
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 44900 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 44900
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 44801
WAVE_HI = 44900  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 44801, 44850
IDENTITY_LO, IDENTITY_HI = 44851, 44875
FILL_LO, FILL_HI = 44876, 44890
MARKERS_LO, MARKERS_HI = 44891, 44900
...
# Stems become gj_<stem>_44900. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (44891, "shell_green", 1, "shell product green extract (wave 44900)"),
    (44892, "libcgj_green", 1, "libcgj product green extract (wave 44900)"),
    (44893, "bar3_ready", 0, "product bar3 readiness gate (wave 44900)"),
    (44894, "product_score", 0, "product readiness score tag (wave 44900)"),
    (44895, "continuum_ready", 1, "continuum readiness gate (wave 44900)"),
    (44896, "smoke_soft", 1, "smoke soft lamp (wave 44900)"),
    (44897, "dyn_soft", 1, "dyn soft lamp (wave 44900)"),
    (44898, "milestone_tag", MILESTONE, "milestone 44900 product tag"),
    (44899, "continuum_wave", MILESTONE, "milestone 44900 continuum wave identity"),
    # 44900 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        44876: "continuum wave fill soft unit + batch id",
        44880: "continuum bridge soft unit + batch id",
        44890: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (44891–44899), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 44893
    tag_batch = MARKERS_LO + 7  # 44898
    wave_batch = MARKERS_LO + 8  # 44899
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_44800 / gj_shell_green_44700 / gj_shell_green_44600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_44800 / gj_libcgj_green_44700 / gj_libcgj_green_44600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_44800 / gj_bar3_ready_44700 / gj_bar3_ready_44600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_44800 / gj_product_score_44700 / gj_product_score_44600\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_44800 / gj_continuum_ready_44700 /\n"
            f" * gj_continuum_ready_44600 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_44800 / gj_smoke_soft_44700 / gj_smoke_soft_44600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_44800 / gj_dyn_soft_44700 / gj_dyn_soft_44600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_44800 / gj_milestone_tag_44700 / gj_milestone_tag_44600,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_44800 / gj_continuum_wave_44700 /\n"
            f" * gj_continuum_wave_44600, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch44900.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 44899
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 44891–44899; 44900 is capstone
