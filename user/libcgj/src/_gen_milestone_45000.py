...
"""
Soft continuum generator — MILESTONE 45000 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 44901–45000 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=45000   when parent has wired through graph_batch45000.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 44901 45000
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 44901 45000
...
graph batches ending at M. For M = **45000** the exclusive range is
**44901–45000** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 44901–44950  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 44951–44975  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 44976–44990  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 44991–45000  │ decade soft gates (Wave 304 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 45000 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_45000        1          graph_batch44991.c
  gj_libcgj_green_45000       1          graph_batch44992.c
  gj_bar3_ready_45000         0          graph_batch44993.c  (bar3 open)
  gj_product_score_45000      0          graph_batch44994.c  (score open)
  gj_continuum_ready_45000    1          graph_batch44995.c
  gj_smoke_soft_45000         1          graph_batch44996.c
  gj_dyn_soft_45000           1          graph_batch44997.c
  gj_milestone_tag_45000      45000      graph_batch44998.c
  gj_continuum_wave_45000     45000      graph_batch44999.c
  gj_batch_id_45000           45000      graph_batch45000.c
  gj_graph_milestone_45000    45000      graph_batch45000.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=45000`` means parent wired CGJ_SRCS through
    ``graph_batch45000.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_45000`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_45000`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M45000_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_44900`` / ``gj_*_44800`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 304 exclusive CREATE-ONLY lands the **markers** band
  (44991–45000) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_45000.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_45000.py

  # Restrict to marker band only (soft-gate TUs; Wave 304 exclusive):
  python3 user/libcgj/src/_gen_milestone_45000.py --band markers

  # Extension identity only (44901–44950):
  python3 user/libcgj/src/_gen_milestone_45000.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_45000.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_44900.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 45000 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 45000
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 44901
WAVE_HI = 45000  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 44901, 44950
IDENTITY_LO, IDENTITY_HI = 44951, 44975
FILL_LO, FILL_HI = 44976, 44990
MARKERS_LO, MARKERS_HI = 44991, 45000
...
# Stems become gj_<stem>_45000. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (44991, "shell_green", 1, "shell product green extract (wave 45000)"),
    (44992, "libcgj_green", 1, "libcgj product green extract (wave 45000)"),
    (44993, "bar3_ready", 0, "product bar3 readiness gate (wave 45000)"),
    (44994, "product_score", 0, "product readiness score tag (wave 45000)"),
    (44995, "continuum_ready", 1, "continuum readiness gate (wave 45000)"),
    (44996, "smoke_soft", 1, "smoke soft lamp (wave 45000)"),
    (44997, "dyn_soft", 1, "dyn soft lamp (wave 45000)"),
    (44998, "milestone_tag", MILESTONE, "milestone 45000 product tag"),
    (44999, "continuum_wave", MILESTONE, "milestone 45000 continuum wave identity"),
    # 45000 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        44976: "continuum wave fill soft unit + batch id",
        44980: "continuum bridge soft unit + batch id",
        44990: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (44991–44999), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 44993
    tag_batch = MARKERS_LO + 7  # 44998
    wave_batch = MARKERS_LO + 8  # 44999
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_44900 / gj_shell_green_44800 / gj_shell_green_44700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_44900 / gj_libcgj_green_44800 / gj_libcgj_green_44700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_44900 / gj_bar3_ready_44800 / gj_bar3_ready_44700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_44900 / gj_product_score_44800 / gj_product_score_44700\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_44900 / gj_continuum_ready_44800 /\n"
            f" * gj_continuum_ready_44700 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_44900 / gj_smoke_soft_44800 / gj_smoke_soft_44700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_44900 / gj_dyn_soft_44800 / gj_dyn_soft_44700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_44900 / gj_milestone_tag_44800 / gj_milestone_tag_44700,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_44900 / gj_continuum_wave_44800 /\n"
            f" * gj_continuum_wave_44700, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch45000.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 44999
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 44991–44999; 45000 is capstone
