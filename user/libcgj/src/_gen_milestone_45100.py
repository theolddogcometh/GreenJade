...
"""
Soft continuum generator — MILESTONE 45100 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 45001–45100 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=45100   when parent has wired through graph_batch45100.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 45001 45100
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 45001 45100
...
graph batches ending at M. For M = **45100** the exclusive range is
**45001–45100** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 45001–45050  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 45051–45075  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 45076–45090  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 45091–45100  │ decade soft gates (Wave 305 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 45100 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_45100        1          graph_batch45091.c
  gj_libcgj_green_45100       1          graph_batch45092.c
  gj_bar3_ready_45100         0          graph_batch45093.c  (bar3 open)
  gj_product_score_45100      0          graph_batch45094.c  (score open)
  gj_continuum_ready_45100    1          graph_batch45095.c
  gj_smoke_soft_45100         1          graph_batch45096.c
  gj_dyn_soft_45100           1          graph_batch45097.c
  gj_milestone_tag_45100      45100      graph_batch45098.c
  gj_continuum_wave_45100     45100      graph_batch45099.c
  gj_batch_id_45100           45100      graph_batch45100.c
  gj_graph_milestone_45100    45100      graph_batch45100.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=45100`` means parent wired CGJ_SRCS through
    ``graph_batch45100.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_45100`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_45100`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M45100_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_45000`` / ``gj_*_44900`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 305 exclusive CREATE-ONLY lands the **markers** band
  (45091–45100) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_45100.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_45100.py

  # Restrict to marker band only (soft-gate TUs; Wave 305 exclusive):
  python3 user/libcgj/src/_gen_milestone_45100.py --band markers

  # Extension identity only (45001–45050):
  python3 user/libcgj/src/_gen_milestone_45100.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_45100.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_45000.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 45100 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 45100
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 45001
WAVE_HI = 45100  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 45001, 45050
IDENTITY_LO, IDENTITY_HI = 45051, 45075
FILL_LO, FILL_HI = 45076, 45090
MARKERS_LO, MARKERS_HI = 45091, 45100
...
# Stems become gj_<stem>_45100. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (45091, "shell_green", 1, "shell product green extract (wave 45100)"),
    (45092, "libcgj_green", 1, "libcgj product green extract (wave 45100)"),
    (45093, "bar3_ready", 0, "product bar3 readiness gate (wave 45100)"),
    (45094, "product_score", 0, "product readiness score tag (wave 45100)"),
    (45095, "continuum_ready", 1, "continuum readiness gate (wave 45100)"),
    (45096, "smoke_soft", 1, "smoke soft lamp (wave 45100)"),
    (45097, "dyn_soft", 1, "dyn soft lamp (wave 45100)"),
    (45098, "milestone_tag", MILESTONE, "milestone 45100 product tag"),
    (45099, "continuum_wave", MILESTONE, "milestone 45100 continuum wave identity"),
    # 45100 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        45076: "continuum wave fill soft unit + batch id",
        45080: "continuum bridge soft unit + batch id",
        45090: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (45091–45099), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 45093
    tag_batch = MARKERS_LO + 7  # 45098
    wave_batch = MARKERS_LO + 8  # 45099
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_45000 / gj_shell_green_44900 / gj_shell_green_44800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_45000 / gj_libcgj_green_44900 / gj_libcgj_green_44800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_45000 / gj_bar3_ready_44900 / gj_bar3_ready_44800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_45000 / gj_product_score_44900 / gj_product_score_44800\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_45000 / gj_continuum_ready_44900 /\n"
            f" * gj_continuum_ready_44800 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_45000 / gj_smoke_soft_44900 / gj_smoke_soft_44800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_45000 / gj_dyn_soft_44900 / gj_dyn_soft_44800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_45000 / gj_milestone_tag_44900 / gj_milestone_tag_44800,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_45000 / gj_continuum_wave_44900 /\n"
            f" * gj_continuum_wave_44800, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch45100.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 45099
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 45091–45099; 45100 is capstone
