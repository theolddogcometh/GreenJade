...
"""
Soft continuum generator — MILESTONE 45200 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 45101–45200 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=45200   when parent has wired through graph_batch45200.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 45101 45200
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 45101 45200
...
graph batches ending at M. For M = **45200** the exclusive range is
**45101–45200** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 45101–45150  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 45151–45175  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 45176–45190  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 45191–45200  │ decade soft gates (Wave 306 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 45200 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_45200        1          graph_batch45191.c
  gj_libcgj_green_45200       1          graph_batch45192.c
  gj_bar3_ready_45200         0          graph_batch45193.c  (bar3 open)
  gj_product_score_45200      0          graph_batch45194.c  (score open)
  gj_continuum_ready_45200    1          graph_batch45195.c
  gj_smoke_soft_45200         1          graph_batch45196.c
  gj_dyn_soft_45200           1          graph_batch45197.c
  gj_milestone_tag_45200      45200      graph_batch45198.c
  gj_continuum_wave_45200     45200      graph_batch45199.c
  gj_batch_id_45200           45200      graph_batch45200.c
  gj_graph_milestone_45200    45200      graph_batch45200.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=45200`` means parent wired CGJ_SRCS through
    ``graph_batch45200.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_45200`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_45200`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M45200_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_45100`` / ``gj_*_45000`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 306 exclusive CREATE-ONLY lands the **markers** band
  (45191–45200) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_45200.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_45200.py

  # Restrict to marker band only (soft-gate TUs; Wave 306 exclusive):
  python3 user/libcgj/src/_gen_milestone_45200.py --band markers

  # Extension identity only (45101–45150):
  python3 user/libcgj/src/_gen_milestone_45200.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_45200.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_45100.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 45200 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 45200
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 45101
WAVE_HI = 45200  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 45101, 45150
IDENTITY_LO, IDENTITY_HI = 45151, 45175
FILL_LO, FILL_HI = 45176, 45190
MARKERS_LO, MARKERS_HI = 45191, 45200
...
# Stems become gj_<stem>_45200. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (45191, "shell_green", 1, "shell product green extract (wave 45200)"),
    (45192, "libcgj_green", 1, "libcgj product green extract (wave 45200)"),
    (45193, "bar3_ready", 0, "product bar3 readiness gate (wave 45200)"),
    (45194, "product_score", 0, "product readiness score tag (wave 45200)"),
    (45195, "continuum_ready", 1, "continuum readiness gate (wave 45200)"),
    (45196, "smoke_soft", 1, "smoke soft lamp (wave 45200)"),
    (45197, "dyn_soft", 1, "dyn soft lamp (wave 45200)"),
    (45198, "milestone_tag", MILESTONE, "milestone 45200 product tag"),
    (45199, "continuum_wave", MILESTONE, "milestone 45200 continuum wave identity"),
    # 45200 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        45176: "continuum wave fill soft unit + batch id",
        45180: "continuum bridge soft unit + batch id",
        45190: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (45191–45199), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 45193
    tag_batch = MARKERS_LO + 7  # 45198
    wave_batch = MARKERS_LO + 8  # 45199
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_45100 / gj_shell_green_45000 / gj_shell_green_44900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_45100 / gj_libcgj_green_45000 / gj_libcgj_green_44900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_45100 / gj_bar3_ready_45000 / gj_bar3_ready_44900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_45100 / gj_product_score_45000 / gj_product_score_44900\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_45100 / gj_continuum_ready_45000 /\n"
            f" * gj_continuum_ready_44900 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_45100 / gj_smoke_soft_45000 / gj_smoke_soft_44900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_45100 / gj_dyn_soft_45000 / gj_dyn_soft_44900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_45100 / gj_milestone_tag_45000 / gj_milestone_tag_44900,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_45100 / gj_continuum_wave_45000 /\n"
            f" * gj_continuum_wave_44900, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch45200.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 45199
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 45191–45199; 45200 is capstone
