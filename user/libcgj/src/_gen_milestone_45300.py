...
"""
Soft continuum generator — MILESTONE 45300 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 45201–45300 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=45300   when parent has wired through graph_batch45300.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 45201 45300
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 45201 45300
...
graph batches ending at M. For M = **45300** the exclusive range is
**45201–45300** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 45201–45250  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 45251–45275  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 45276–45290  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 45291–45300  │ decade soft gates (Wave 307 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 45300 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_45300        1          graph_batch45291.c
  gj_libcgj_green_45300       1          graph_batch45292.c
  gj_bar3_ready_45300         0          graph_batch45293.c  (bar3 open)
  gj_product_score_45300      0          graph_batch45294.c  (score open)
  gj_continuum_ready_45300    1          graph_batch45295.c
  gj_smoke_soft_45300         1          graph_batch45296.c
  gj_dyn_soft_45300           1          graph_batch45297.c
  gj_milestone_tag_45300      45300      graph_batch45298.c
  gj_continuum_wave_45300     45300      graph_batch45299.c
  gj_batch_id_45300           45300      graph_batch45300.c
  gj_graph_milestone_45300    45300      graph_batch45300.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=45300`` means parent wired CGJ_SRCS through
    ``graph_batch45300.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_45300`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_45300`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M45300_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_45200`` / ``gj_*_45100`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 307 exclusive CREATE-ONLY lands the **markers** band
  (45291–45300) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_45300.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_45300.py

  # Restrict to marker band only (soft-gate TUs; Wave 307 exclusive):
  python3 user/libcgj/src/_gen_milestone_45300.py --band markers

  # Extension identity only (45201–45250):
  python3 user/libcgj/src/_gen_milestone_45300.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_45300.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_45200.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 45300 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 45300
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 45201
WAVE_HI = 45300  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 45201, 45250
IDENTITY_LO, IDENTITY_HI = 45251, 45275
FILL_LO, FILL_HI = 45276, 45290
MARKERS_LO, MARKERS_HI = 45291, 45300
...
# Stems become gj_<stem>_45300. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (45291, "shell_green", 1, "shell product green extract (wave 45300)"),
    (45292, "libcgj_green", 1, "libcgj product green extract (wave 45300)"),
    (45293, "bar3_ready", 0, "product bar3 readiness gate (wave 45300)"),
    (45294, "product_score", 0, "product readiness score tag (wave 45300)"),
    (45295, "continuum_ready", 1, "continuum readiness gate (wave 45300)"),
    (45296, "smoke_soft", 1, "smoke soft lamp (wave 45300)"),
    (45297, "dyn_soft", 1, "dyn soft lamp (wave 45300)"),
    (45298, "milestone_tag", MILESTONE, "milestone 45300 product tag"),
    (45299, "continuum_wave", MILESTONE, "milestone 45300 continuum wave identity"),
    # 45300 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        45276: "continuum wave fill soft unit + batch id",
        45280: "continuum bridge soft unit + batch id",
        45290: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (45291–45299), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 45293
    tag_batch = MARKERS_LO + 7  # 45298
    wave_batch = MARKERS_LO + 8  # 45299
...
def render_milestone_capstone() -> str:
    """graph_batch45300.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 45299
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 45291–45299; 45300 is capstone
