...
"""
Soft continuum generator — MILESTONE 45400 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 45301–45400 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=45400   when parent has wired through graph_batch45400.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 45301 45400
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 45301 45400
...
graph batches ending at M. For M = **45400** the exclusive range is
**45301–45400** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 45301–45350  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 45351–45375  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 45376–45390  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 45391–45400  │ decade soft gates (Wave 308 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 45400 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_45400        1          graph_batch45391.c
  gj_libcgj_green_45400       1          graph_batch45392.c
  gj_bar3_ready_45400         0          graph_batch45393.c  (bar3 open)
  gj_product_score_45400      0          graph_batch45394.c  (score open)
  gj_continuum_ready_45400    1          graph_batch45395.c
  gj_smoke_soft_45400         1          graph_batch45396.c
  gj_dyn_soft_45400           1          graph_batch45397.c
  gj_milestone_tag_45400      45400      graph_batch45398.c
  gj_continuum_wave_45400     45400      graph_batch45399.c
  gj_batch_id_45400           45400      graph_batch45400.c
  gj_graph_milestone_45400    45400      graph_batch45400.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=45400`` means parent wired CGJ_SRCS through
    ``graph_batch45400.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_45400`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_45400`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M45400_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_45300`` / ``gj_*_45200`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 308 exclusive CREATE-ONLY lands the **markers** band
  (45391–45400) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_45400.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_45400.py

  # Restrict to marker band only (soft-gate TUs; Wave 308 exclusive):
  python3 user/libcgj/src/_gen_milestone_45400.py --band markers

  # Extension identity only (45301–45350):
  python3 user/libcgj/src/_gen_milestone_45400.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_45400.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_45300.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 45400 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 45400
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 45301
WAVE_HI = 45400  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 45301, 45350
IDENTITY_LO, IDENTITY_HI = 45351, 45375
FILL_LO, FILL_HI = 45376, 45390
MARKERS_LO, MARKERS_HI = 45391, 45400
...
# Stems become gj_<stem>_45400. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (45391, "shell_green", 1, "shell product green extract (wave 45400)"),
    (45392, "libcgj_green", 1, "libcgj product green extract (wave 45400)"),
    (45393, "bar3_ready", 0, "product bar3 readiness gate (wave 45400)"),
    (45394, "product_score", 0, "product readiness score tag (wave 45400)"),
    (45395, "continuum_ready", 1, "continuum readiness gate (wave 45400)"),
    (45396, "smoke_soft", 1, "smoke soft lamp (wave 45400)"),
    (45397, "dyn_soft", 1, "dyn soft lamp (wave 45400)"),
    (45398, "milestone_tag", MILESTONE, "milestone 45400 product tag"),
    (45399, "continuum_wave", MILESTONE, "milestone 45400 continuum wave identity"),
    # 45400 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        45376: "continuum wave fill soft unit + batch id",
        45380: "continuum bridge soft unit + batch id",
        45390: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (45391–45399), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 45393
    tag_batch = MARKERS_LO + 7  # 45398
    wave_batch = MARKERS_LO + 8  # 45399
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_45300 / gj_shell_green_45200 / gj_shell_green_45100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_45300 / gj_libcgj_green_45200 / gj_libcgj_green_45100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_45300 / gj_bar3_ready_45200 / gj_bar3_ready_45100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_45300 / gj_product_score_45200 / gj_product_score_45100\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_45300 / gj_continuum_ready_45200 /\n"
            f" * gj_continuum_ready_45100 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_45300 / gj_smoke_soft_45200 / gj_smoke_soft_45100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_45300 / gj_dyn_soft_45200 / gj_dyn_soft_45100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_45300 / gj_milestone_tag_45200 / gj_milestone_tag_45100,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_45300 / gj_continuum_wave_45200 /\n"
            f" * gj_continuum_wave_45100, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch45400.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 45399
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 45391–45399; 45400 is capstone
