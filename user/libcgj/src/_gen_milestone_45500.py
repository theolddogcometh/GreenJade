...
"""
Soft continuum generator — MILESTONE 45500 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 45401–45500 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=45500   when parent has wired through graph_batch45500.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 45401 45500
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 45401 45500
...
graph batches ending at M. For M = **45500** the exclusive range is
**45401–45500** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 45401–45450  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 45451–45475  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 45476–45490  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 45491–45500  │ decade soft gates (Wave 309 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 45500 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_45500        1          graph_batch45491.c
  gj_libcgj_green_45500       1          graph_batch45492.c
  gj_bar3_ready_45500         0          graph_batch45493.c  (bar3 open)
  gj_product_score_45500      0          graph_batch45494.c  (score open)
  gj_continuum_ready_45500    1          graph_batch45495.c
  gj_smoke_soft_45500         1          graph_batch45496.c
  gj_dyn_soft_45500           1          graph_batch45497.c
  gj_milestone_tag_45500      45500      graph_batch45498.c
  gj_continuum_wave_45500     45500      graph_batch45499.c
  gj_batch_id_45500           45500      graph_batch45500.c
  gj_graph_milestone_45500    45500      graph_batch45500.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=45500`` means parent wired CGJ_SRCS through
    ``graph_batch45500.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_45500`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_45500`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M45500_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_45400`` / ``gj_*_45300`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 309 exclusive CREATE-ONLY lands the **markers** band
  (45491–45500) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_45500.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_45500.py

  # Restrict to marker band only (soft-gate TUs; Wave 309 exclusive):
  python3 user/libcgj/src/_gen_milestone_45500.py --band markers

  # Extension identity only (45401–45450):
  python3 user/libcgj/src/_gen_milestone_45500.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_45500.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_45400.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 45500 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 45500
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 45401
WAVE_HI = 45500  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 45401, 45450
IDENTITY_LO, IDENTITY_HI = 45451, 45475
FILL_LO, FILL_HI = 45476, 45490
MARKERS_LO, MARKERS_HI = 45491, 45500
...
# Stems become gj_<stem>_45500. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (45491, "shell_green", 1, "shell product green extract (wave 45500)"),
    (45492, "libcgj_green", 1, "libcgj product green extract (wave 45500)"),
    (45493, "bar3_ready", 0, "product bar3 readiness gate (wave 45500)"),
    (45494, "product_score", 0, "product readiness score tag (wave 45500)"),
    (45495, "continuum_ready", 1, "continuum readiness gate (wave 45500)"),
    (45496, "smoke_soft", 1, "smoke soft lamp (wave 45500)"),
    (45497, "dyn_soft", 1, "dyn soft lamp (wave 45500)"),
    (45498, "milestone_tag", MILESTONE, "milestone 45500 product tag"),
    (45499, "continuum_wave", MILESTONE, "milestone 45500 continuum wave identity"),
    # 45500 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        45476: "continuum wave fill soft unit + batch id",
        45480: "continuum bridge soft unit + batch id",
        45490: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (45491–45499), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 45493
    tag_batch = MARKERS_LO + 7  # 45498
    wave_batch = MARKERS_LO + 8  # 45499
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_45400 / gj_shell_green_45300 / gj_shell_green_45200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_45400 / gj_libcgj_green_45300 / gj_libcgj_green_45200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_45400 / gj_bar3_ready_45300 / gj_bar3_ready_45200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_45400 / gj_product_score_45300 / gj_product_score_45200\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_45400 / gj_continuum_ready_45300 /\n"
            f" * gj_continuum_ready_45200 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_45400 / gj_smoke_soft_45300 / gj_smoke_soft_45200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_45400 / gj_dyn_soft_45300 / gj_dyn_soft_45200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_45400 / gj_milestone_tag_45300 / gj_milestone_tag_45200,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_45400 / gj_continuum_wave_45300 /\n"
            f" * gj_continuum_wave_45200, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch45500.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 45499
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 45491–45499; 45500 is capstone
