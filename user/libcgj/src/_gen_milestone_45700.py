...
"""
Soft continuum generator — MILESTONE 45700 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 45601–45700 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=45700   when parent has wired through graph_batch45700.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 45601 45700
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 45601 45700
...
graph batches ending at M. For M = **45700** the exclusive range is
**45601–45700** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 45601–45650  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 45651–45675  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 45676–45690  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 45691–45700  │ decade soft gates (Wave 311 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 45700 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_45700        1          graph_batch45691.c
  gj_libcgj_green_45700       1          graph_batch45692.c
  gj_bar3_ready_45700         0          graph_batch45693.c  (bar3 open)
  gj_product_score_45700      0          graph_batch45694.c  (score open)
  gj_continuum_ready_45700    1          graph_batch45695.c
  gj_smoke_soft_45700         1          graph_batch45696.c
  gj_dyn_soft_45700           1          graph_batch45697.c
  gj_milestone_tag_45700      45700      graph_batch45698.c
  gj_continuum_wave_45700     45700      graph_batch45699.c
  gj_batch_id_45700           45700      graph_batch45700.c
  gj_graph_milestone_45700    45700      graph_batch45700.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=45700`` means parent wired CGJ_SRCS through
    ``graph_batch45700.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_45700`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_45700`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M45700_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_45600`` / ``gj_*_45500`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 311 exclusive CREATE-ONLY lands the **markers** band
  (45691–45700) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_45700.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_45700.py

  # Restrict to marker band only (soft-gate TUs; Wave 311 exclusive):
  python3 user/libcgj/src/_gen_milestone_45700.py --band markers

  # Extension identity only (45601–45650):
  python3 user/libcgj/src/_gen_milestone_45700.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_45700.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_45600.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 45700 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 45700
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 45601
WAVE_HI = 45700  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 45601, 45650
IDENTITY_LO, IDENTITY_HI = 45651, 45675
FILL_LO, FILL_HI = 45676, 45690
MARKERS_LO, MARKERS_HI = 45691, 45700
...
# Stems become gj_<stem>_45700. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (45691, "shell_green", 1, "shell product green extract (wave 45700)"),
    (45692, "libcgj_green", 1, "libcgj product green extract (wave 45700)"),
    (45693, "bar3_ready", 0, "product bar3 readiness gate (wave 45700)"),
    (45694, "product_score", 0, "product readiness score tag (wave 45700)"),
    (45695, "continuum_ready", 1, "continuum readiness gate (wave 45700)"),
    (45696, "smoke_soft", 1, "smoke soft lamp (wave 45700)"),
    (45697, "dyn_soft", 1, "dyn soft lamp (wave 45700)"),
    (45698, "milestone_tag", MILESTONE, "milestone 45700 product tag"),
    (45699, "continuum_wave", MILESTONE, "milestone 45700 continuum wave identity"),
    # 45700 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        45676: "continuum wave fill soft unit + batch id",
        45680: "continuum bridge soft unit + batch id",
        45690: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (45691–45699), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 45693
    tag_batch = MARKERS_LO + 7  # 45698
    wave_batch = MARKERS_LO + 8  # 45699
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_45600 / gj_shell_green_45500 / gj_shell_green_45400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_45600 / gj_libcgj_green_45500 / gj_libcgj_green_45400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_45600 / gj_bar3_ready_45500 / gj_bar3_ready_45400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_45600 / gj_product_score_45500 / gj_product_score_45400\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_45600 / gj_continuum_ready_45500 /\n"
            f" * gj_continuum_ready_45400 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_45600 / gj_smoke_soft_45500 / gj_smoke_soft_45400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_45600 / gj_dyn_soft_45500 / gj_dyn_soft_45400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_45600 / gj_milestone_tag_45500 / gj_milestone_tag_45400,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_45600 / gj_continuum_wave_45500 /\n"
            f" * gj_continuum_wave_45400, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch45700.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 45699
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 45691–45699; 45700 is capstone
