...
"""
Soft continuum generator — MILESTONE 45800 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 45701–45800 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=45800   when parent has wired through graph_batch45800.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 45701 45800
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 45701 45800
...
graph batches ending at M. For M = **45800** the exclusive range is
**45701–45800** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 45701–45750  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 45751–45775  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 45776–45790  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 45791–45800  │ decade soft gates (Wave 312 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 45800 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_45800        1          graph_batch45791.c
  gj_libcgj_green_45800       1          graph_batch45792.c
  gj_bar3_ready_45800         0          graph_batch45793.c  (bar3 open)
  gj_product_score_45800      0          graph_batch45794.c  (score open)
  gj_continuum_ready_45800    1          graph_batch45795.c
  gj_smoke_soft_45800         1          graph_batch45796.c
  gj_dyn_soft_45800           1          graph_batch45797.c
  gj_milestone_tag_45800      45800      graph_batch45798.c
  gj_continuum_wave_45800     45800      graph_batch45799.c
  gj_batch_id_45800           45800      graph_batch45800.c
  gj_graph_milestone_45800    45800      graph_batch45800.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=45800`` means parent wired CGJ_SRCS through
    ``graph_batch45800.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_45800`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_45800`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M45800_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_45700`` / ``gj_*_45600`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 312 exclusive CREATE-ONLY lands the **markers** band
  (45791–45800) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_45800.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_45800.py

  # Restrict to marker band only (soft-gate TUs; Wave 312 exclusive):
  python3 user/libcgj/src/_gen_milestone_45800.py --band markers

  # Extension identity only (45701–45750):
  python3 user/libcgj/src/_gen_milestone_45800.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_45800.py --force
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 45800 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 45800
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 45701
WAVE_HI = 45800  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 45701, 45750
IDENTITY_LO, IDENTITY_HI = 45751, 45775
FILL_LO, FILL_HI = 45776, 45790
MARKERS_LO, MARKERS_HI = 45791, 45800
...
# Stems become gj_<stem>_45800. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (45791, "shell_green", 1, "shell product green extract (wave 45800)"),
    (45792, "libcgj_green", 1, "libcgj product green extract (wave 45800)"),
    (45793, "bar3_ready", 0, "product bar3 readiness gate (wave 45800)"),
    (45794, "product_score", 0, "product readiness score tag (wave 45800)"),
    (45795, "continuum_ready", 1, "continuum readiness gate (wave 45800)"),
    (45796, "smoke_soft", 1, "smoke soft lamp (wave 45800)"),
    (45797, "dyn_soft", 1, "dyn soft lamp (wave 45800)"),
    (45798, "milestone_tag", MILESTONE, "milestone 45800 product tag"),
    (45799, "continuum_wave", MILESTONE, "milestone 45800 continuum wave identity"),
    # 45800 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        45776: "continuum wave fill soft unit + batch id",
        45780: "continuum bridge soft unit + batch id",
        45790: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (45791–45799), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 45793
    tag_batch = MARKERS_LO + 7  # 45798
    wave_batch = MARKERS_LO + 8  # 45799
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_45700 / gj_shell_green_45600 / gj_shell_green_45500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_45700 / gj_libcgj_green_45600 / gj_libcgj_green_45500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_45700 / gj_bar3_ready_45600 / gj_bar3_ready_45500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_45700 / gj_product_score_45600 / gj_product_score_45500\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_45700 / gj_continuum_ready_45600 /\n"
            f" * gj_continuum_ready_45500 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_45700 / gj_smoke_soft_45600 / gj_smoke_soft_45500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_45700 / gj_dyn_soft_45600 / gj_dyn_soft_45500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_45700 / gj_milestone_tag_45600 / gj_milestone_tag_45500,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_45700 / gj_continuum_wave_45600 /\n"
            f" * gj_continuum_wave_45500, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch45800.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 45799
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 45791–45799; 45800 is capstone
