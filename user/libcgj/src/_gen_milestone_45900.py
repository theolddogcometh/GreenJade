...
"""
Soft continuum generator — MILESTONE 45900 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 45801–45900 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=45900   when parent has wired through graph_batch45900.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 45801 45900
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 45801 45900
...
graph batches ending at M. For M = **45900** the exclusive range is
**45801–45900** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 45801–45850  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 45851–45875  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 45876–45890  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 45891–45900  │ decade soft gates (Wave 313 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 45900 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_45900        1          graph_batch45891.c
  gj_libcgj_green_45900       1          graph_batch45892.c
  gj_bar3_ready_45900         0          graph_batch45893.c  (bar3 open)
  gj_product_score_45900      0          graph_batch45894.c  (score open)
  gj_continuum_ready_45900    1          graph_batch45895.c
  gj_smoke_soft_45900         1          graph_batch45896.c
  gj_dyn_soft_45900           1          graph_batch45897.c
  gj_milestone_tag_45900      45900      graph_batch45898.c
  gj_continuum_wave_45900     45900      graph_batch45899.c
  gj_batch_id_45900           45900      graph_batch45900.c
  gj_graph_milestone_45900    45900      graph_batch45900.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=45900`` means parent wired CGJ_SRCS through
    ``graph_batch45900.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_45900`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_45900`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M45900_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_45800`` / ``gj_*_45700`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 313 exclusive CREATE-ONLY lands the **markers** band
  (45891–45900) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_45900.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_45900.py

  # Restrict to marker band only (soft-gate TUs; Wave 313 exclusive):
  python3 user/libcgj/src/_gen_milestone_45900.py --band markers

  # Extension identity only (45801–45850):
  python3 user/libcgj/src/_gen_milestone_45900.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_45900.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_45800.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 45900 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 45900
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 45801
WAVE_HI = 45900  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 45801, 45850
IDENTITY_LO, IDENTITY_HI = 45851, 45875
FILL_LO, FILL_HI = 45876, 45890
MARKERS_LO, MARKERS_HI = 45891, 45900
...
# Stems become gj_<stem>_45900. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (45891, "shell_green", 1, "shell product green extract (wave 45900)"),
    (45892, "libcgj_green", 1, "libcgj product green extract (wave 45900)"),
    (45893, "bar3_ready", 0, "product bar3 readiness gate (wave 45900)"),
    (45894, "product_score", 0, "product readiness score tag (wave 45900)"),
    (45895, "continuum_ready", 1, "continuum readiness gate (wave 45900)"),
    (45896, "smoke_soft", 1, "smoke soft lamp (wave 45900)"),
    (45897, "dyn_soft", 1, "dyn soft lamp (wave 45900)"),
    (45898, "milestone_tag", MILESTONE, "milestone 45900 product tag"),
    (45899, "continuum_wave", MILESTONE, "milestone 45900 continuum wave identity"),
    # 45900 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        45876: "continuum wave fill soft unit + batch id",
        45880: "continuum bridge soft unit + batch id",
        45890: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (45891–45899), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 45893
    tag_batch = MARKERS_LO + 7  # 45898
    wave_batch = MARKERS_LO + 8  # 45899
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_45800 / gj_shell_green_45700 / gj_shell_green_45600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_45800 / gj_libcgj_green_45700 / gj_libcgj_green_45600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_45800 / gj_bar3_ready_45700 / gj_bar3_ready_45600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_45800 / gj_product_score_45700 / gj_product_score_45600\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_45800 / gj_continuum_ready_45700 /\n"
            f" * gj_continuum_ready_45600 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_45800 / gj_smoke_soft_45700 / gj_smoke_soft_45600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_45800 / gj_dyn_soft_45700 / gj_dyn_soft_45600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_45800 / gj_milestone_tag_45700 / gj_milestone_tag_45600,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_45800 / gj_continuum_wave_45700 /\n"
            f" * gj_continuum_wave_45600, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch45900.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 45899
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 45891–45899; 45900 is capstone
