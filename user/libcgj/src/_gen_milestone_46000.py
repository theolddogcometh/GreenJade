...
"""
Soft continuum generator — MILESTONE 46000 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 45901–46000 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=46000   when parent has wired through graph_batch46000.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 45901 46000
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 45901 46000
...
graph batches ending at M. For M = **46000** the exclusive range is
**45901–46000** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 45901–45950  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 45951–45975  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 45976–45990  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 45991–46000  │ decade soft gates (Wave 314 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 46000 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_46000        1          graph_batch45991.c
  gj_libcgj_green_46000       1          graph_batch45992.c
  gj_bar3_ready_46000         0          graph_batch45993.c  (bar3 open)
  gj_product_score_46000      0          graph_batch45994.c  (score open)
  gj_continuum_ready_46000    1          graph_batch45995.c
  gj_smoke_soft_46000         1          graph_batch45996.c
  gj_dyn_soft_46000           1          graph_batch45997.c
  gj_milestone_tag_46000      46000      graph_batch45998.c
  gj_continuum_wave_46000     46000      graph_batch45999.c
  gj_batch_id_46000           46000      graph_batch46000.c
  gj_graph_milestone_46000    46000      graph_batch46000.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=46000`` means parent wired CGJ_SRCS through
    ``graph_batch46000.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_46000`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_46000`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M46000_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_45900`` / ``gj_*_45800`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 314 exclusive CREATE-ONLY lands the **markers** band
  (45991–46000) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_46000.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_46000.py

  # Restrict to marker band only (soft-gate TUs; Wave 314 exclusive):
  python3 user/libcgj/src/_gen_milestone_46000.py --band markers

  # Extension identity only (45901–45950):
  python3 user/libcgj/src/_gen_milestone_46000.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_46000.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_45900.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 46000 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 46000
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 45901
WAVE_HI = 46000  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 45901, 45950
IDENTITY_LO, IDENTITY_HI = 45951, 45975
FILL_LO, FILL_HI = 45976, 45990
MARKERS_LO, MARKERS_HI = 45991, 46000
...
# Stems become gj_<stem>_46000. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (45991, "shell_green", 1, "shell product green extract (wave 46000)"),
    (45992, "libcgj_green", 1, "libcgj product green extract (wave 46000)"),
    (45993, "bar3_ready", 0, "product bar3 readiness gate (wave 46000)"),
    (45994, "product_score", 0, "product readiness score tag (wave 46000)"),
    (45995, "continuum_ready", 1, "continuum readiness gate (wave 46000)"),
    (45996, "smoke_soft", 1, "smoke soft lamp (wave 46000)"),
    (45997, "dyn_soft", 1, "dyn soft lamp (wave 46000)"),
    (45998, "milestone_tag", MILESTONE, "milestone 46000 product tag"),
    (45999, "continuum_wave", MILESTONE, "milestone 46000 continuum wave identity"),
    # 46000 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        45976: "continuum wave fill soft unit + batch id",
        45980: "continuum bridge soft unit + batch id",
        45990: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (45991–45999), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 45993
    tag_batch = MARKERS_LO + 7  # 45998
    wave_batch = MARKERS_LO + 8  # 45999
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_45900 / gj_shell_green_45800 / gj_shell_green_45700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_45900 / gj_libcgj_green_45800 / gj_libcgj_green_45700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_45900 / gj_bar3_ready_45800 / gj_bar3_ready_45700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_45900 / gj_product_score_45800 / gj_product_score_45700\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_45900 / gj_continuum_ready_45800 /\n"
            f" * gj_continuum_ready_45700 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_45900 / gj_smoke_soft_45800 / gj_smoke_soft_45700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_45900 / gj_dyn_soft_45800 / gj_dyn_soft_45700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_45900 / gj_milestone_tag_45800 / gj_milestone_tag_45700,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_45900 / gj_continuum_wave_45800 /\n"
            f" * gj_continuum_wave_45700, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch46000.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 45999
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 45991–45999; 46000 is capstone
