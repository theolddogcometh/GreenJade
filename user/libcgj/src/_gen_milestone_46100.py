...
"""
Soft continuum generator — MILESTONE 46100 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 46001–46100 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=46100   when parent has wired through graph_batch46100.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 46001 46100
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 46001 46100
...
graph batches ending at M. For M = **46100** the exclusive range is
**46001–46100** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 46001–46050  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 46051–46075  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 46076–46090  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 46091–46100  │ decade soft gates (Wave 315 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 46100 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_46100        1          graph_batch46091.c
  gj_libcgj_green_46100       1          graph_batch46092.c
  gj_bar3_ready_46100         0          graph_batch46093.c  (bar3 open)
  gj_product_score_46100      0          graph_batch46094.c  (score open)
  gj_continuum_ready_46100    1          graph_batch46095.c
  gj_smoke_soft_46100         1          graph_batch46096.c
  gj_dyn_soft_46100           1          graph_batch46097.c
  gj_milestone_tag_46100      46100      graph_batch46098.c
  gj_continuum_wave_46100     46100      graph_batch46099.c
  gj_batch_id_46100           46100      graph_batch46100.c
  gj_graph_milestone_46100    46100      graph_batch46100.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=46100`` means parent wired CGJ_SRCS through
    ``graph_batch46100.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_46100`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_46100`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M46100_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_46000`` / ``gj_*_45900`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 315 exclusive CREATE-ONLY lands the **markers** band
  (46091–46100) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_46100.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_46100.py

  # Restrict to marker band only (soft-gate TUs; Wave 315 exclusive):
  python3 user/libcgj/src/_gen_milestone_46100.py --band markers

  # Extension identity only (46001–46050):
  python3 user/libcgj/src/_gen_milestone_46100.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_46100.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_46000.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 46100 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 46100
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 46001
WAVE_HI = 46100  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 46001, 46050
IDENTITY_LO, IDENTITY_HI = 46051, 46075
FILL_LO, FILL_HI = 46076, 46090
MARKERS_LO, MARKERS_HI = 46091, 46100
...
# Stems become gj_<stem>_46100. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (46091, "shell_green", 1, "shell product green extract (wave 46100)"),
    (46092, "libcgj_green", 1, "libcgj product green extract (wave 46100)"),
    (46093, "bar3_ready", 0, "product bar3 readiness gate (wave 46100)"),
    (46094, "product_score", 0, "product readiness score tag (wave 46100)"),
    (46095, "continuum_ready", 1, "continuum readiness gate (wave 46100)"),
    (46096, "smoke_soft", 1, "smoke soft lamp (wave 46100)"),
    (46097, "dyn_soft", 1, "dyn soft lamp (wave 46100)"),
    (46098, "milestone_tag", MILESTONE, "milestone 46100 product tag"),
    (46099, "continuum_wave", MILESTONE, "milestone 46100 continuum wave identity"),
    # 46100 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        46076: "continuum wave fill soft unit + batch id",
        46080: "continuum bridge soft unit + batch id",
        46090: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (46091–46099), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 46093
    tag_batch = MARKERS_LO + 7  # 46098
    wave_batch = MARKERS_LO + 8  # 46099
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_46000 / gj_shell_green_45900 / gj_shell_green_45800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_46000 / gj_libcgj_green_45900 / gj_libcgj_green_45800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_46000 / gj_bar3_ready_45900 / gj_bar3_ready_45800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_46000 / gj_product_score_45900 / gj_product_score_45800\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_46000 / gj_continuum_ready_45900 /\n"
            f" * gj_continuum_ready_45800 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_46000 / gj_smoke_soft_45900 / gj_smoke_soft_45800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_46000 / gj_dyn_soft_45900 / gj_dyn_soft_45800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_46000 / gj_milestone_tag_45900 / gj_milestone_tag_45800,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_46000 / gj_continuum_wave_45900 /\n"
            f" * gj_continuum_wave_45800, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch46100.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 46099
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 46091–46099; 46100 is capstone
