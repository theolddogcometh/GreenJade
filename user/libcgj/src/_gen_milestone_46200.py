...
"""
Soft continuum generator — MILESTONE 46200 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 46101–46200 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=46200   when parent has wired through graph_batch46200.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 46101 46200
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 46101 46200
...
graph batches ending at M. For M = **46200** the exclusive range is
**46101–46200** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 46101–46150  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 46151–46175  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 46176–46190  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 46191–46200  │ decade soft gates (Wave 316 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 46200 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_46200        1          graph_batch46191.c
  gj_libcgj_green_46200       1          graph_batch46192.c
  gj_bar3_ready_46200         0          graph_batch46193.c  (bar3 open)
  gj_product_score_46200      0          graph_batch46194.c  (score open)
  gj_continuum_ready_46200    1          graph_batch46195.c
  gj_smoke_soft_46200         1          graph_batch46196.c
  gj_dyn_soft_46200           1          graph_batch46197.c
  gj_milestone_tag_46200      46200      graph_batch46198.c
  gj_continuum_wave_46200     46200      graph_batch46199.c
  gj_batch_id_46200           46200      graph_batch46200.c
  gj_graph_milestone_46200    46200      graph_batch46200.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=46200`` means parent wired CGJ_SRCS through
    ``graph_batch46200.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_46200`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_46200`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M46200_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_46100`` / ``gj_*_46000`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 316 exclusive CREATE-ONLY lands the **markers** band
  (46191–46200) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_46200.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_46200.py

  # Restrict to marker band only (soft-gate TUs; Wave 316 exclusive):
  python3 user/libcgj/src/_gen_milestone_46200.py --band markers

  # Extension identity only (46101–46150):
  python3 user/libcgj/src/_gen_milestone_46200.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_46200.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_46100.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 46200 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 46200
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 46101
WAVE_HI = 46200  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 46101, 46150
IDENTITY_LO, IDENTITY_HI = 46151, 46175
FILL_LO, FILL_HI = 46176, 46190
MARKERS_LO, MARKERS_HI = 46191, 46200
...
# Stems become gj_<stem>_46200. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (46191, "shell_green", 1, "shell product green extract (wave 46200)"),
    (46192, "libcgj_green", 1, "libcgj product green extract (wave 46200)"),
    (46193, "bar3_ready", 0, "product bar3 readiness gate (wave 46200)"),
    (46194, "product_score", 0, "product readiness score tag (wave 46200)"),
    (46195, "continuum_ready", 1, "continuum readiness gate (wave 46200)"),
    (46196, "smoke_soft", 1, "smoke soft lamp (wave 46200)"),
    (46197, "dyn_soft", 1, "dyn soft lamp (wave 46200)"),
    (46198, "milestone_tag", MILESTONE, "milestone 46200 product tag"),
    (46199, "continuum_wave", MILESTONE, "milestone 46200 continuum wave identity"),
    # 46200 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        46176: "continuum wave fill soft unit + batch id",
        46180: "continuum bridge soft unit + batch id",
        46190: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (46191–46199), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 46193
    tag_batch = MARKERS_LO + 7  # 46198
    wave_batch = MARKERS_LO + 8  # 46199
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_46100 / gj_shell_green_46000 / gj_shell_green_45900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_46100 / gj_libcgj_green_46000 / gj_libcgj_green_45900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_46100 / gj_bar3_ready_46000 / gj_bar3_ready_45900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_46100 / gj_product_score_46000 / gj_product_score_45900\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_46100 / gj_continuum_ready_46000 /\n"
            f" * gj_continuum_ready_45900 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_46100 / gj_smoke_soft_46000 / gj_smoke_soft_45900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_46100 / gj_dyn_soft_46000 / gj_dyn_soft_45900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_46100 / gj_milestone_tag_46000 / gj_milestone_tag_45900,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_46100 / gj_continuum_wave_46000 /\n"
            f" * gj_continuum_wave_45900, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch46200.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 46199
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 46191–46199; 46200 is capstone
