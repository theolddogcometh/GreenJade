...
"""
Soft continuum generator — MILESTONE 46300 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 46201–46300 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=46300   when parent has wired through graph_batch46300.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 46201 46300
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 46201 46300
...
graph batches ending at M. For M = **46300** the exclusive range is
**46201–46300** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 46201–46250  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 46251–46275  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 46276–46290  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 46291–46300  │ decade soft gates (Wave 317 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 46300 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_46300        1          graph_batch46291.c
  gj_libcgj_green_46300       1          graph_batch46292.c
  gj_bar3_ready_46300         0          graph_batch46293.c  (bar3 open)
  gj_product_score_46300      0          graph_batch46294.c  (score open)
  gj_continuum_ready_46300    1          graph_batch46295.c
  gj_smoke_soft_46300         1          graph_batch46296.c
  gj_dyn_soft_46300           1          graph_batch46297.c
  gj_milestone_tag_46300      46300      graph_batch46298.c
  gj_continuum_wave_46300     46300      graph_batch46299.c
  gj_batch_id_46300           46300      graph_batch46300.c
  gj_graph_milestone_46300    46300      graph_batch46300.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=46300`` means parent wired CGJ_SRCS through
    ``graph_batch46300.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_46300`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_46300`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M46300_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_46200`` / ``gj_*_46100`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 317 exclusive CREATE-ONLY lands the **markers** band
  (46291–46300) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_46300.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_46300.py

  # Restrict to marker band only (soft-gate TUs; Wave 317 exclusive):
  python3 user/libcgj/src/_gen_milestone_46300.py --band markers

  # Extension identity only (46201–46250):
  python3 user/libcgj/src/_gen_milestone_46300.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_46300.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_46200.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 46300 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 46300
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 46201
WAVE_HI = 46300  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 46201, 46250
IDENTITY_LO, IDENTITY_HI = 46251, 46275
FILL_LO, FILL_HI = 46276, 46290
MARKERS_LO, MARKERS_HI = 46291, 46300
...
# Stems become gj_<stem>_46300. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (46291, "shell_green", 1, "shell product green extract (wave 46300)"),
    (46292, "libcgj_green", 1, "libcgj product green extract (wave 46300)"),
    (46293, "bar3_ready", 0, "product bar3 readiness gate (wave 46300)"),
    (46294, "product_score", 0, "product readiness score tag (wave 46300)"),
    (46295, "continuum_ready", 1, "continuum readiness gate (wave 46300)"),
    (46296, "smoke_soft", 1, "smoke soft lamp (wave 46300)"),
    (46297, "dyn_soft", 1, "dyn soft lamp (wave 46300)"),
    (46298, "milestone_tag", MILESTONE, "milestone 46300 product tag"),
    (46299, "continuum_wave", MILESTONE, "milestone 46300 continuum wave identity"),
    # 46300 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        46276: "continuum wave fill soft unit + batch id",
        46280: "continuum bridge soft unit + batch id",
        46290: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (46291–46299), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 46293
    tag_batch = MARKERS_LO + 7  # 46298
    wave_batch = MARKERS_LO + 8  # 46299
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_46200 / gj_shell_green_46100 / gj_shell_green_46000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_46200 / gj_libcgj_green_46100 / gj_libcgj_green_46000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_46200 / gj_bar3_ready_46100 / gj_bar3_ready_46000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_46200 / gj_product_score_46100 / gj_product_score_46000\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_46200 / gj_continuum_ready_46100 /\n"
            f" * gj_continuum_ready_46000 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_46200 / gj_smoke_soft_46100 / gj_smoke_soft_46000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_46200 / gj_dyn_soft_46100 / gj_dyn_soft_46000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_46200 / gj_milestone_tag_46100 / gj_milestone_tag_46000,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_46200 / gj_continuum_wave_46100 /\n"
            f" * gj_continuum_wave_46000, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch46300.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 46299
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 46291–46299; 46300 is capstone
