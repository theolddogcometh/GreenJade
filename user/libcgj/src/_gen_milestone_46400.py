...
"""
Soft continuum generator — MILESTONE 46400 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 46301–46400 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=46400   when parent has wired through graph_batch46000.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 46301 46400
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 46301 46400
...
graph batches ending at M. For M = **46400** the exclusive range is
**46301–46400** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 46301–46350  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 46351–46375  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 46376–46390  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 46391–46400  │ decade soft gates (Wave 318 exclusive)│
...
----------------------------------------------------------
  ...
  gj_milestone_tag_46000      46400      graph_batch45998.c
  gj_continuum_wave_46000     46400      graph_batch45999.c
  gj_batch_id_46000           46400      graph_batch46000.c
  gj_graph_milestone_46000    46400      graph_batch46000.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=46400`` means parent wired CGJ_SRCS through
  ...
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_46300`` / ``gj_*_46200`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  (46391–46400) first; extension/identity/fill may be emitted later via
...
-----
  ...
  # Extension identity only (46301–46350):
...
See also
--------
  user/libcgj/src/_gen_milestone_46300.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 46400 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 46400
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 46301
WAVE_HI = 46400  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 46301, 46350
IDENTITY_LO, IDENTITY_HI = 46351, 46375
FILL_LO, FILL_HI = 46376, 46390
MARKERS_LO, MARKERS_HI = 46391, 46400
...
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (46391, "shell_green", 1, "shell product green extract (wave 46400)"),
    (46392, "libcgj_green", 1, "libcgj product green extract (wave 46400)"),
    (46393, "bar3_ready", 0, "product bar3 readiness gate (wave 46400)"),
    (46394, "product_score", 0, "product readiness score tag (wave 46400)"),
    (46395, "continuum_ready", 1, "continuum readiness gate (wave 46400)"),
    (46396, "smoke_soft", 1, "smoke soft lamp (wave 46400)"),
    (46397, "dyn_soft", 1, "dyn soft lamp (wave 46400)"),
    (46398, "milestone_tag", MILESTONE, "milestone 46400 product tag"),
    (46399, "continuum_wave", MILESTONE, "milestone 46400 continuum wave identity"),
    # 46400 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        46376: "continuum wave fill soft unit + batch id",
        46380: "continuum bridge soft unit + batch id",
        46390: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (46391–46399), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 46393
    tag_batch = MARKERS_LO + 7  # 46398
    wave_batch = MARKERS_LO + 8  # 46399
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_46300 / gj_shell_green_46200 / gj_shell_green_46100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_46300 / gj_libcgj_green_46200 / gj_libcgj_green_46100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_46300 / gj_bar3_ready_46200 / gj_bar3_ready_46100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_46300 / gj_product_score_46200 / gj_product_score_46100\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_46300 / gj_continuum_ready_46200 /\n"
            f" * gj_continuum_ready_46100 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_46300 / gj_smoke_soft_46200 / gj_smoke_soft_46100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_46300 / gj_dyn_soft_46200 / gj_dyn_soft_46100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_46300 / gj_milestone_tag_46200 / gj_milestone_tag_46100,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_46300 / gj_continuum_wave_46200 /\n"
            f" * gj_continuum_wave_46100, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    ...
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 46399
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 46391–46399; 46400 is capstone
