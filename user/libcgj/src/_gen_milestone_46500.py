...
"""
Soft continuum generator — MILESTONE 46500 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 46401–46500 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=46500   when parent has wired through graph_batch46500.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 46401 46500
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 46401 46500
...
graph batches ending at M. For M = **46500** the exclusive range is
**46401–46500** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 46401–46450  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 46451–46475  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 46476–46490  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 46491–46500  │ decade soft gates (Wave 318 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 46400 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_46500        1          graph_batch46491.c
  gj_libcgj_green_46500       1          graph_batch46492.c
  gj_bar3_ready_46500         0          graph_batch46493.c  (bar3 open)
  gj_product_score_46500      0          graph_batch46494.c  (score open)
  gj_continuum_ready_46500    1          graph_batch46495.c
  gj_smoke_soft_46500         1          graph_batch46496.c
  gj_dyn_soft_46500           1          graph_batch46497.c
  gj_milestone_tag_46500      46500      graph_batch46498.c
  gj_continuum_wave_46500     46500      graph_batch46499.c
  gj_batch_id_46500           46500      graph_batch46500.c
  gj_graph_milestone_46500    46500      graph_batch46500.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=46500`` means parent wired CGJ_SRCS through
    ``graph_batch46500.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_46500`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_46500`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M46500_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_46400`` / ``gj_*_46300`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 318 exclusive CREATE-ONLY lands the **markers** band
  (46491–46500) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_46500.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_46500.py

  # Restrict to marker band only (soft-gate TUs; Wave 318 exclusive):
  python3 user/libcgj/src/_gen_milestone_46500.py --band markers

  # Extension identity only (46401–46450):
  python3 user/libcgj/src/_gen_milestone_46500.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_46500.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_46400.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 46500 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 46500
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 46401
WAVE_HI = 46500  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 46401, 46450
IDENTITY_LO, IDENTITY_HI = 46451, 46475
FILL_LO, FILL_HI = 46476, 46490
MARKERS_LO, MARKERS_HI = 46491, 46500
...
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (46491, "shell_green", 1, "shell product green extract (wave 46400)"),
    (46492, "libcgj_green", 1, "libcgj product green extract (wave 46400)"),
    (46493, "bar3_ready", 0, "product bar3 readiness gate (wave 46400)"),
    (46494, "product_score", 0, "product readiness score tag (wave 46400)"),
    (46495, "continuum_ready", 1, "continuum readiness gate (wave 46400)"),
    (46496, "smoke_soft", 1, "smoke soft lamp (wave 46400)"),
    (46497, "dyn_soft", 1, "dyn soft lamp (wave 46400)"),
    (46498, "milestone_tag", MILESTONE, "milestone 46500 product tag"),
    (46499, "continuum_wave", MILESTONE, "milestone 46500 continuum wave identity"),
    # 46500 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        46476: "continuum wave fill soft unit + batch id",
        46480: "continuum bridge soft unit + batch id",
        46490: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (46491–46499), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 46493
    tag_batch = MARKERS_LO + 7  # 46498
    wave_batch = MARKERS_LO + 8  # 46499
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_46400 / gj_shell_green_46300 / gj_shell_green_46200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_46400 / gj_libcgj_green_46300 / gj_libcgj_green_46200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_46400 / gj_bar3_ready_46300 / gj_bar3_ready_46200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_46400 / gj_product_score_46300 / gj_product_score_46200\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_46400 / gj_continuum_ready_46300 /\n"
            f" * gj_continuum_ready_46200 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_46400 / gj_smoke_soft_46300 / gj_smoke_soft_46200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_46400 / gj_dyn_soft_46300 / gj_dyn_soft_46200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_46400 / gj_milestone_tag_46300 / gj_milestone_tag_46200,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_46400 / gj_continuum_wave_46300 /\n"
            f" * gj_continuum_wave_46200, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch46500.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 46499
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 46491–46499; 46500 is capstone
