...
"""
Soft continuum generator — MILESTONE 46600 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 46501–46600 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=46600   when parent has wired through graph_batch46600.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 46501 46600
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 46501 46600
...
graph batches ending at M. For M = **46600** the exclusive range is
**46501–46600** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 46501–46550  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 46551–46575  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 46576–46590  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 46591–46600  │ decade soft gates (Wave exclusive)    │
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 46600 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_46600        1          graph_batch46591.c
  gj_libcgj_green_46600       1          graph_batch46592.c
  gj_bar3_ready_46600         0          graph_batch46593.c  (bar3 open)
  gj_product_score_46600      0          graph_batch46594.c  (score open)
  gj_continuum_ready_46600    1          graph_batch46595.c
  gj_smoke_soft_46600         1          graph_batch46596.c
  gj_dyn_soft_46600           1          graph_batch46597.c
  gj_milestone_tag_46600      46600      graph_batch46598.c
  gj_continuum_wave_46600     46600      graph_batch46599.c
  gj_batch_id_46600           46600      graph_batch46600.c
  gj_graph_milestone_46600    46600      graph_batch46600.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=46600`` means parent wired CGJ_SRCS through
    ``graph_batch46600.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_46600`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_46600`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M46600_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_46500`` / ``gj_*_46400`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave exclusive CREATE-ONLY lands the **markers** band
  (46591–46600) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_46600.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_46600.py

  # Restrict to marker band only (soft-gate TUs; Wave exclusive):
  python3 user/libcgj/src/_gen_milestone_46600.py --band markers

  # Extension identity only (46501–46550):
  python3 user/libcgj/src/_gen_milestone_46600.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_46600.py --force
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 46600 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 46600
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 46501
WAVE_HI = 46600  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 46501, 46550
IDENTITY_LO, IDENTITY_HI = 46551, 46575
FILL_LO, FILL_HI = 46576, 46590
MARKERS_LO, MARKERS_HI = 46591, 46600
...
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (46591, "shell_green", 1, "shell product green extract (wave 46600)"),
    (46592, "libcgj_green", 1, "libcgj product green extract (wave 46600)"),
    (46593, "bar3_ready", 0, "product bar3 readiness gate (wave 46600)"),
    (46594, "product_score", 0, "product readiness score tag (wave 46600)"),
    (46595, "continuum_ready", 1, "continuum readiness gate (wave 46600)"),
    (46596, "smoke_soft", 1, "smoke soft lamp (wave 46600)"),
    (46597, "dyn_soft", 1, "dyn soft lamp (wave 46600)"),
    (46598, "milestone_tag", MILESTONE, "milestone 46600 product tag"),
    (46599, "continuum_wave", MILESTONE, "milestone 46600 continuum wave identity"),
    # 46600 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        46576: "continuum wave fill soft unit + batch id",
        46580: "continuum bridge soft unit + batch id",
        46590: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (46591–46599), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 46593
    tag_batch = MARKERS_LO + 7  # 46598
    wave_batch = MARKERS_LO + 8  # 46599
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_46500 / gj_shell_green_46400 / gj_shell_green_46300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_46500 / gj_libcgj_green_46400 / gj_libcgj_green_46300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_46500 / gj_bar3_ready_46400 / gj_bar3_ready_46300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_46500 / gj_product_score_46400 / gj_product_score_46300\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_46500 / gj_continuum_ready_46400 /\n"
            f" * gj_continuum_ready_46300 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_46500 / gj_smoke_soft_46400 / gj_smoke_soft_46300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_46500 / gj_dyn_soft_46400 / gj_dyn_soft_46300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_46500 / gj_milestone_tag_46400 / gj_milestone_tag_46300,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_46500 / gj_continuum_wave_46400 /\n"
            f" * gj_continuum_wave_46300, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    ...
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 46599
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 46591–46599; 46600 is capstone
