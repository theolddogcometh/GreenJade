...
"""
Soft continuum generator — MILESTONE 46700 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 46601–46700 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=46700   when parent has wired through graph_batch46700.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 46601 46700
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 46601 46700
...
graph batches ending at M. For M = **46700** the exclusive range is
**46601–46700** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 46601–46650  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 46651–46675  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 46676–46690  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 46691–46700  │ decade soft gates (Wave 318 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 46700 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_46700        1          graph_batch46691.c
  gj_libcgj_green_46700       1          graph_batch46692.c
  gj_bar3_ready_46700         0          graph_batch46693.c  (bar3 open)
  gj_product_score_46700      0          graph_batch46694.c  (score open)
  gj_continuum_ready_46700    1          graph_batch46695.c
  gj_smoke_soft_46700         1          graph_batch46696.c
  gj_dyn_soft_46700           1          graph_batch46697.c
  gj_milestone_tag_46700      46700      graph_batch46698.c
  gj_continuum_wave_46700     46700      graph_batch46699.c
  gj_batch_id_46700           46700      graph_batch46700.c
  gj_graph_milestone_46700    46700      graph_batch46700.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=46700`` means parent wired CGJ_SRCS through
    ``graph_batch46700.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_46700`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_46700`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M46700_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_46600`` / ``gj_*_46500`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  (46691–46700) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_46700.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_46700.py

  # Restrict to marker band only (soft-gate TUs; Wave 318 exclusive):
  python3 user/libcgj/src/_gen_milestone_46700.py --band markers

  # Extension identity only (46601–46650):
  python3 user/libcgj/src/_gen_milestone_46700.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_46700.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_46600.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 46700 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 46700
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 46601
WAVE_HI = 46700  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 46601, 46650
IDENTITY_LO, IDENTITY_HI = 46651, 46675
FILL_LO, FILL_HI = 46676, 46690
MARKERS_LO, MARKERS_HI = 46691, 46700
...
# Stems become gj_<stem>_46700. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (46691, "shell_green", 1, "shell product green extract (wave 46700)"),
    (46692, "libcgj_green", 1, "libcgj product green extract (wave 46700)"),
    (46693, "bar3_ready", 0, "product bar3 readiness gate (wave 46700)"),
    (46694, "product_score", 0, "product readiness score tag (wave 46700)"),
    (46695, "continuum_ready", 1, "continuum readiness gate (wave 46700)"),
    (46696, "smoke_soft", 1, "smoke soft lamp (wave 46700)"),
    (46697, "dyn_soft", 1, "dyn soft lamp (wave 46700)"),
    (46698, "milestone_tag", MILESTONE, "milestone 46700 product tag"),
    (46699, "continuum_wave", MILESTONE, "milestone 46700 continuum wave identity"),
    # 46700 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        46676: "continuum wave fill soft unit + batch id",
        46680: "continuum bridge soft unit + batch id",
        46690: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (46691–46699), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 46693
    tag_batch = MARKERS_LO + 7  # 46698
    wave_batch = MARKERS_LO + 8  # 46699
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_46600 / gj_shell_green_46500 / gj_shell_green_46400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_46600 / gj_libcgj_green_46500 / gj_libcgj_green_46400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_46600 / gj_bar3_ready_46500 / gj_bar3_ready_46400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_46600 / gj_product_score_46500 / gj_product_score_46400\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_46600 / gj_continuum_ready_46500 /\n"
            f" * gj_continuum_ready_46400 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_46600 / gj_smoke_soft_46500 / gj_smoke_soft_46400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_46600 / gj_dyn_soft_46500 / gj_dyn_soft_46400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_46600 / gj_milestone_tag_46500 / gj_milestone_tag_46400,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_46600 / gj_continuum_wave_46500 /\n"
            f" * gj_continuum_wave_46400, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch46700.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 46699
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 46691–46699; 46700 is capstone
