...
"""
Soft continuum generator — MILESTONE 46800 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 46701–46800 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=46800   when parent has wired through graph_batch46800.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 46701 46800
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 46701 46800
...
graph batches ending at M. For M = **46800** the exclusive range is
**46701–46800** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 46701–46750  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 46751–46775  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 46776–46790  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 46791–46800  │ decade soft gates (Wave 322 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 46800 marker KATs (soft host probe expectations)
----------------------------------------------------------
  ...
  gj_milestone_tag_46800      46800      graph_batch46798.c
  gj_continuum_wave_46800     46800      graph_batch46799.c
  gj_batch_id_46800           46800      graph_batch46800.c
  gj_graph_milestone_46800    46800      graph_batch46800.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=46800`` means parent wired CGJ_SRCS through
...
--------------------
  ...
  Wave 322 exclusive CREATE-ONLY lands the **markers** band
  (46791–46800) first; extension/identity/fill may be emitted later via
...
-----
  ...
  # Restrict to marker band only (soft-gate TUs; Wave 322 exclusive):
  python3 user/libcgj/src/_gen_milestone_46800.py --band markers

  # Extension identity only (46701–46750):
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 46800 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 46800
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 46701
WAVE_HI = 46800  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 46701, 46750
IDENTITY_LO, IDENTITY_HI = 46751, 46775
FILL_LO, FILL_HI = 46776, 46790
MARKERS_LO, MARKERS_HI = 46791, 46800
...
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (46791, "shell_green", 1, "shell product green extract (wave 46800)"),
    (46792, "libcgj_green", 1, "libcgj product green extract (wave 46800)"),
    (46793, "bar3_ready", 0, "product bar3 readiness gate (wave 46800)"),
    (46794, "product_score", 0, "product readiness score tag (wave 46800)"),
    (46795, "continuum_ready", 1, "continuum readiness gate (wave 46800)"),
    (46796, "smoke_soft", 1, "smoke soft lamp (wave 46800)"),
    (46797, "dyn_soft", 1, "dyn soft lamp (wave 46800)"),
    (46798, "milestone_tag", MILESTONE, "milestone 46800 product tag"),
    (46799, "continuum_wave", MILESTONE, "milestone 46800 continuum wave identity"),
    # 46800 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        46776: "continuum wave fill soft unit + batch id",
        46780: "continuum bridge soft unit + batch id",
        46790: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (46791–46799), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 46793
    tag_batch = MARKERS_LO + 7  # 46798
    wave_batch = MARKERS_LO + 8  # 46799
...
def render_milestone_capstone() -> str:
    ...
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 46799
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 46791–46799; 46800 is capstone
