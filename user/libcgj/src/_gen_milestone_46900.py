...
"""
Soft continuum generator — MILESTONE 46900 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 46801–46900 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=46900   when parent has wired through graph_batch46900.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 46801 46900
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 46801 46900
...
graph batches ending at M. For M = **46900** the exclusive range is
**46801–46900** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 46801–46850  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 46851–46875  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 46876–46890  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 46891–46900  │ decade soft gates (Wave 323 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 46900 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_46900        1          graph_batch46891.c
  gj_libcgj_green_46900       1          graph_batch46892.c
  gj_bar3_ready_46900         0          graph_batch46893.c  (bar3 open)
  gj_product_score_46900      0          graph_batch46894.c  (score open)
  gj_continuum_ready_46900    1          graph_batch46895.c
  gj_smoke_soft_46900         1          graph_batch46896.c
  gj_dyn_soft_46900           1          graph_batch46897.c
  gj_milestone_tag_46900      46900      graph_batch46898.c
  gj_continuum_wave_46900     46900      graph_batch46899.c
  gj_batch_id_46900           46900      graph_batch46900.c
  gj_graph_milestone_46900    46900      graph_batch46900.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=46900`` means parent wired CGJ_SRCS through
    ``graph_batch46900.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_46900`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_46900`` is intentionally **0**.
  ...
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_46800`` / ``gj_*_46700`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 323 exclusive CREATE-ONLY lands the **markers** band
  (46891–46900) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_46900.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_46900.py

  # Restrict to marker band only (soft-gate TUs; Wave 323 exclusive):
  python3 user/libcgj/src/_gen_milestone_46900.py --band markers

  # Extension identity only (46801–46850):
  python3 user/libcgj/src/_gen_milestone_46900.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_46900.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_46800.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 46900 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 46900
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 46801
WAVE_HI = 46900  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 46801, 46850
IDENTITY_LO, IDENTITY_HI = 46851, 46875
FILL_LO, FILL_HI = 46876, 46890
MARKERS_LO, MARKERS_HI = 46891, 46900
...
# Stems become gj_<stem>_46900. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (46891, "shell_green", 1, "shell product green extract (wave 46900)"),
    (46892, "libcgj_green", 1, "libcgj product green extract (wave 46900)"),
    (46893, "bar3_ready", 0, "product bar3 readiness gate (wave 46900)"),
    (46894, "product_score", 0, "product readiness score tag (wave 46900)"),
    (46895, "continuum_ready", 1, "continuum readiness gate (wave 46900)"),
    (46896, "smoke_soft", 1, "smoke soft lamp (wave 46900)"),
    (46897, "dyn_soft", 1, "dyn soft lamp (wave 46900)"),
    (46898, "milestone_tag", MILESTONE, "milestone 46900 product tag"),
    (46899, "continuum_wave", MILESTONE, "milestone 46900 continuum wave identity"),
    # 46900 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        46876: "continuum wave fill soft unit + batch id",
        46880: "continuum bridge soft unit + batch id",
        46890: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (46891–46899), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 46893
    tag_batch = MARKERS_LO + 7  # 46898
    wave_batch = MARKERS_LO + 8  # 46899
...
def render_milestone_capstone() -> str:
    """graph_batch46900.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 46899
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 46891–46899; 46900 is capstone
