...
"""
Soft continuum generator — MILESTONE 47000 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 46901–47000 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=47000   when parent has wired through graph_batch47000.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 46901 47000
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 46901 47000
...
graph batches ending at M. For M = **47000** the exclusive range is
**46901–47000** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 46901–46950  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 46951–46975  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 46976–46990  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 46991–47000  │ decade soft gates (Wave 324 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 47000 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_47000        1          graph_batch46991.c
  gj_libcgj_green_47000       1          graph_batch46992.c
  gj_bar3_ready_47000         0          graph_batch46993.c  (bar3 open)
  gj_product_score_47000      0          graph_batch46994.c  (score open)
  gj_continuum_ready_47000    1          graph_batch46995.c
  gj_smoke_soft_47000         1          graph_batch46996.c
  gj_dyn_soft_47000           1          graph_batch46997.c
  gj_milestone_tag_47000      47000      graph_batch46998.c
  gj_continuum_wave_47000     47000      graph_batch46999.c
  gj_batch_id_47000           47000      graph_batch47000.c
  gj_graph_milestone_47000    47000      graph_batch47000.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=47000`` means parent wired CGJ_SRCS through
    ``graph_batch47000.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_47000`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_47000`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M47000_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_46900`` / ``gj_*_46800`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 324 exclusive CREATE-ONLY lands the **markers** band
  (46991–47000) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_47000.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_47000.py

  # Restrict to marker band only (soft-gate TUs; Wave 324 exclusive):
  python3 user/libcgj/src/_gen_milestone_47000.py --band markers

  # Extension identity only (46901–46950):
  python3 user/libcgj/src/_gen_milestone_47000.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_47000.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_46900.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 47000 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 47000
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 46901
WAVE_HI = 47000  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 46901, 46950
IDENTITY_LO, IDENTITY_HI = 46951, 46975
FILL_LO, FILL_HI = 46976, 46990
MARKERS_LO, MARKERS_HI = 46991, 47000
...
# Stems become gj_<stem>_47000. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (46991, "shell_green", 1, "shell product green extract (wave 47000)"),
    (46992, "libcgj_green", 1, "libcgj product green extract (wave 47000)"),
    (46993, "bar3_ready", 0, "product bar3 readiness gate (wave 47000)"),
    (46994, "product_score", 0, "product readiness score tag (wave 47000)"),
    (46995, "continuum_ready", 1, "continuum readiness gate (wave 47000)"),
    (46996, "smoke_soft", 1, "smoke soft lamp (wave 47000)"),
    (46997, "dyn_soft", 1, "dyn soft lamp (wave 47000)"),
    (46998, "milestone_tag", MILESTONE, "milestone 47000 product tag"),
    (46999, "continuum_wave", MILESTONE, "milestone 47000 continuum wave identity"),
    # 47000 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        46976: "continuum wave fill soft unit + batch id",
        46980: "continuum bridge soft unit + batch id",
        46990: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (46991–46999), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 46993
    tag_batch = MARKERS_LO + 7  # 46998
    wave_batch = MARKERS_LO + 8  # 46999
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_46900 / gj_shell_green_46800 / gj_shell_green_46700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_46900 / gj_libcgj_green_46800 / gj_libcgj_green_46700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_46900 / gj_bar3_ready_46800 / gj_bar3_ready_46700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_46900 / gj_product_score_46800 / gj_product_score_46700\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_46900 / gj_continuum_ready_46800 /\n"
            f" * gj_continuum_ready_46700 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_46900 / gj_smoke_soft_46800 / gj_smoke_soft_46700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_46900 / gj_dyn_soft_46800 / gj_dyn_soft_46700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_46900 / gj_milestone_tag_46800 / gj_milestone_tag_46700,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_46900 / gj_continuum_wave_46800 /\n"
            f" * gj_continuum_wave_46700, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch47000.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 46999
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 46991–46999; 47000 is capstone
