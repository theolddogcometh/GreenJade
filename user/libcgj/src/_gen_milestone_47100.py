...
"""
Soft continuum generator — MILESTONE 47100 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 47001–47100 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=47100   when parent has wired through graph_batch46800.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 47001 47100
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 47001 47100
...
graph batches ending at M. For M = **47100** the exclusive range is
**47001–47100** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 47001–47050  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 47051–47075  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 47076–47090  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 47091–47100  │ decade soft gates (Wave 334 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 47100 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_46800        1          graph_batch46791.c
  gj_libcgj_green_46800       1          graph_batch46792.c
  gj_bar3_ready_46800         0          graph_batch46793.c  (bar3 open)
  gj_product_score_46800      0          graph_batch46794.c  (score open)
  gj_continuum_ready_46800    1          graph_batch46795.c
  gj_smoke_soft_46800         1          graph_batch46796.c
  gj_dyn_soft_46800           1          graph_batch46797.c
  gj_milestone_tag_46800      47100      graph_batch46798.c
  gj_continuum_wave_46800     47100      graph_batch46799.c
  gj_batch_id_46800           47100      graph_batch46800.c
  gj_graph_milestone_46800    47100      graph_batch46800.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=47100`` means parent wired CGJ_SRCS through
    ``graph_batch46800.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_46800`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_46800`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M47100_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_46700`` / ``gj_*_46600`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  (47091–47100) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_46800.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_46800.py

  # Restrict to marker band only (soft-gate TUs; Wave 334 exclusive):
  python3 user/libcgj/src/_gen_milestone_46800.py --band markers

  # Extension identity only (47001–47050):
  python3 user/libcgj/src/_gen_milestone_46800.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_46800.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_46700.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 47100 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 47100
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 47001
WAVE_HI = 47100  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 47001, 47050
IDENTITY_LO, IDENTITY_HI = 47051, 47075
FILL_LO, FILL_HI = 47076, 47090
MARKERS_LO, MARKERS_HI = 47091, 47100
...
# Stems become gj_<stem>_46800. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (47091, "shell_green", 1, "shell product green extract (wave 47100)"),
    (47092, "libcgj_green", 1, "libcgj product green extract (wave 47100)"),
    (47093, "bar3_ready", 0, "product bar3 readiness gate (wave 47100)"),
    (47094, "product_score", 0, "product readiness score tag (wave 47100)"),
    (47095, "continuum_ready", 1, "continuum readiness gate (wave 47100)"),
    (47096, "smoke_soft", 1, "smoke soft lamp (wave 47100)"),
    (47097, "dyn_soft", 1, "dyn soft lamp (wave 47100)"),
    (47098, "milestone_tag", MILESTONE, "milestone 47100 product tag"),
    (47099, "continuum_wave", MILESTONE, "milestone 47100 continuum wave identity"),
    # 47100 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        47076: "continuum wave fill soft unit + batch id",
        47080: "continuum bridge soft unit + batch id",
        47090: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (47091–47099), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 47093
    tag_batch = MARKERS_LO + 7  # 47098
    wave_batch = MARKERS_LO + 8  # 47099
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_46700 / gj_shell_green_46600 / gj_shell_green_46500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_46700 / gj_libcgj_green_46600 / gj_libcgj_green_46500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_46700 / gj_bar3_ready_46600 / gj_bar3_ready_46500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_46700 / gj_product_score_46600 / gj_product_score_46500\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_46700 / gj_continuum_ready_46600 /\n"
            f" * gj_continuum_ready_46500 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_46700 / gj_smoke_soft_46600 / gj_smoke_soft_46500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_46700 / gj_dyn_soft_46600 / gj_dyn_soft_46500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_46700 / gj_milestone_tag_46600 / gj_milestone_tag_46500,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_46700 / gj_continuum_wave_46600 /\n"
            f" * gj_continuum_wave_46500, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch46800.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 47099
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 47091–47099; 47100 is capstone
