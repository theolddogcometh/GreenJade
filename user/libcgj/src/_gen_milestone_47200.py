...
"""
Soft continuum generator — MILESTONE 47200 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 47101–47200 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=47200   when parent has wired through graph_batch47200.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 47101 47200
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 47101 47200
...
graph batches ending at M. For M = **47200** the exclusive range is
**47101–47200** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 47101–47150  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 47151–47175  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 47176–47190  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 47191–47200  │ decade soft gates (Wave 330 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 47200 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_47200        1          graph_batch47191.c
  gj_libcgj_green_47200       1          graph_batch47192.c
  gj_bar3_ready_47200         0          graph_batch47193.c  (bar3 open)
  gj_product_score_47200      0          graph_batch47194.c  (score open)
  gj_continuum_ready_47200    1          graph_batch47195.c
  gj_smoke_soft_47200         1          graph_batch47196.c
  gj_dyn_soft_47200           1          graph_batch47197.c
  gj_milestone_tag_47200      47200      graph_batch47198.c
  gj_continuum_wave_47200     47200      graph_batch47199.c
  gj_batch_id_47200           47200      graph_batch47200.c
  gj_graph_milestone_47200    47200      graph_batch47200.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=47200`` means parent wired CGJ_SRCS through
    ``graph_batch47200.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_47200`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_47200`` is intentionally **0**.
  ...
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_47100`` / ``gj_*_47000`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 330 exclusive CREATE-ONLY lands the **markers** band
  (47191–47200) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_47200.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_47200.py

  # Restrict to marker band only (soft-gate TUs; Wave 330 exclusive):
  python3 user/libcgj/src/_gen_milestone_47200.py --band markers

  # Extension identity only (47101–47150):
  python3 user/libcgj/src/_gen_milestone_47200.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_47200.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_47100.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 47200 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 47200
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 47101
WAVE_HI = 47200  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 47101, 47150
IDENTITY_LO, IDENTITY_HI = 47151, 47175
FILL_LO, FILL_HI = 47176, 47190
MARKERS_LO, MARKERS_HI = 47191, 47200
...
# Stems become gj_<stem>_47200. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (47191, "shell_green", 1, "shell product green extract (wave 47200)"),
    (47192, "libcgj_green", 1, "libcgj product green extract (wave 47200)"),
    (47193, "bar3_ready", 0, "product bar3 readiness gate (wave 47200)"),
    (47194, "product_score", 0, "product readiness score tag (wave 47200)"),
    (47195, "continuum_ready", 1, "continuum readiness gate (wave 47200)"),
    (47196, "smoke_soft", 1, "smoke soft lamp (wave 47200)"),
    (47197, "dyn_soft", 1, "dyn soft lamp (wave 47200)"),
    (47198, "milestone_tag", MILESTONE, "milestone 47200 product tag"),
    (47199, "continuum_wave", MILESTONE, "milestone 47200 continuum wave identity"),
    # 47200 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        47176: "continuum wave fill soft unit + batch id",
        47180: "continuum bridge soft unit + batch id",
        47190: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (47191–47199), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 47193
    tag_batch = MARKERS_LO + 7  # 47198
    wave_batch = MARKERS_LO + 8  # 47199
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_47100 / gj_shell_green_47000 / gj_shell_green_46900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_47100 / gj_libcgj_green_47000 / gj_libcgj_green_46900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_47100 / gj_bar3_ready_47000 / gj_bar3_ready_46900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_47100 / gj_product_score_47000 / gj_product_score_46900\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_47100 / gj_continuum_ready_47000 /\n"
            f" * gj_continuum_ready_46900 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_47100 / gj_smoke_soft_47000 / gj_smoke_soft_46900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_47100 / gj_dyn_soft_47000 / gj_dyn_soft_46900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_47100 / gj_milestone_tag_47000 / gj_milestone_tag_46900,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_47100 / gj_continuum_wave_47000 /\n"
            f" * gj_continuum_wave_46900, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch47200.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 47199
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 47191–47199; 47200 is capstone
