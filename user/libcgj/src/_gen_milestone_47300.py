...
"""
Soft continuum generator — MILESTONE 47300 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 47201–47300 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=47300   when parent has wired through graph_batch47300.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 47201 47300
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 47201 47300
...
graph batches ending at M. For M = **47300** the exclusive range is
**47201–47300** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 47201–47250  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 47251–47275  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 47276–47290  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 47291–47300  │ decade soft gates (Wave 331 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 47300 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_47300        1          graph_batch47291.c
  gj_libcgj_green_47300       1          graph_batch47292.c
  gj_bar3_ready_47300         0          graph_batch47293.c  (bar3 open)
  gj_product_score_47300      0          graph_batch47294.c  (score open)
  gj_continuum_ready_47300    1          graph_batch47295.c
  gj_smoke_soft_47300         1          graph_batch47296.c
  gj_dyn_soft_47300           1          graph_batch47297.c
  gj_milestone_tag_47300      47300      graph_batch47298.c
  gj_continuum_wave_47300     47300      graph_batch47299.c
  gj_batch_id_47300           47300      graph_batch47300.c
  gj_graph_milestone_47300    47300      graph_batch47300.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=47300`` means parent wired CGJ_SRCS through
    ``graph_batch47300.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_47300`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_47300`` is intentionally **0**.
  ...
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_47200`` / ``gj_*_47100`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 331 exclusive CREATE-ONLY lands the **markers** band
  (47291–47300) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_47300.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_47300.py

  # Restrict to marker band only (soft-gate TUs; Wave 331 exclusive):
  python3 user/libcgj/src/_gen_milestone_47300.py --band markers

  # Extension identity only (47201–47250):
  python3 user/libcgj/src/_gen_milestone_47300.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_47300.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_47200.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 47300 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 47300
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 47201
WAVE_HI = 47300  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 47201, 47250
IDENTITY_LO, IDENTITY_HI = 47251, 47275
FILL_LO, FILL_HI = 47276, 47290
MARKERS_LO, MARKERS_HI = 47291, 47300
...
# Stems become gj_<stem>_47300. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (47291, "shell_green", 1, "shell product green extract (wave 47300)"),
    (47292, "libcgj_green", 1, "libcgj product green extract (wave 47300)"),
    (47293, "bar3_ready", 0, "product bar3 readiness gate (wave 47300)"),
    (47294, "product_score", 0, "product readiness score tag (wave 47300)"),
    (47295, "continuum_ready", 1, "continuum readiness gate (wave 47300)"),
    (47296, "smoke_soft", 1, "smoke soft lamp (wave 47300)"),
    (47297, "dyn_soft", 1, "dyn soft lamp (wave 47300)"),
    (47298, "milestone_tag", MILESTONE, "milestone 47300 product tag"),
    (47299, "continuum_wave", MILESTONE, "milestone 47300 continuum wave identity"),
    # 47300 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        47276: "continuum wave fill soft unit + batch id",
        47280: "continuum bridge soft unit + batch id",
        47290: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (47291–47299), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 47293
    tag_batch = MARKERS_LO + 7  # 47298
    wave_batch = MARKERS_LO + 8  # 47299
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_47200 / gj_shell_green_47100 / gj_shell_green_47000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_47200 / gj_libcgj_green_47100 / gj_libcgj_green_47000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_47200 / gj_bar3_ready_47100 / gj_bar3_ready_47000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_47200 / gj_product_score_47100 / gj_product_score_47000\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_47200 / gj_continuum_ready_47100 /\n"
            f" * gj_continuum_ready_47000 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_47200 / gj_smoke_soft_47100 / gj_smoke_soft_47000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_47200 / gj_dyn_soft_47100 / gj_dyn_soft_47000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_47200 / gj_milestone_tag_47100 / gj_milestone_tag_47000,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_47200 / gj_continuum_wave_47100 /\n"
            f" * gj_continuum_wave_47000, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch47300.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 47299
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 47291–47299; 47300 is capstone
