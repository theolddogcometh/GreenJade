...
"""
Soft continuum generator — MILESTONE 47400 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 47301–47400 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=47400   when parent has wired through graph_batch47400.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 47301 47400
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 47301 47400
...
graph batches ending at M. For M = **47400** the exclusive range is
**47301–47400** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 47301–47350  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 47351–47375  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 47376–47390  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 47391–47400  │ decade soft gates (Wave 332 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 47400 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_47400        1          graph_batch47191.c
  gj_libcgj_green_47400       1          graph_batch47192.c
  gj_bar3_ready_47400         0          graph_batch47193.c  (bar3 open)
  gj_product_score_47400      0          graph_batch47194.c  (score open)
  gj_continuum_ready_47400    1          graph_batch47195.c
  gj_smoke_soft_47400         1          graph_batch47196.c
  gj_dyn_soft_47400           1          graph_batch47197.c
  gj_milestone_tag_47400      47400      graph_batch47198.c
  gj_continuum_wave_47400     47400      graph_batch47199.c
  gj_batch_id_47400           47400      graph_batch47400.c
  gj_graph_milestone_47400    47400      graph_batch47400.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=47400`` means parent wired CGJ_SRCS through
    ``graph_batch47400.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_47400`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_47400`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M47400_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_47300`` / ``gj_*_47400`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 332 exclusive CREATE-ONLY lands the **markers** band
  (47391–47400) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_47400.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_47400.py

  # Restrict to marker band only (soft-gate TUs; Wave 332 exclusive):
  python3 user/libcgj/src/_gen_milestone_47400.py --band markers

  # Extension identity only (47301–47350):
  python3 user/libcgj/src/_gen_milestone_47400.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_47400.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_47300.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 47400 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 47400
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 47301
WAVE_HI = 47400  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 47301, 47350
IDENTITY_LO, IDENTITY_HI = 47351, 47375
FILL_LO, FILL_HI = 47376, 47390
MARKERS_LO, MARKERS_HI = 47391, 47400
...
# Stems become gj_<stem>_47400. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (47391, "shell_green", 1, "shell product green extract (wave 47400)"),
    (47392, "libcgj_green", 1, "libcgj product green extract (wave 47400)"),
    (47393, "bar3_ready", 0, "product bar3 readiness gate (wave 47400)"),
    (47394, "product_score", 0, "product readiness score tag (wave 47400)"),
    (47395, "continuum_ready", 1, "continuum readiness gate (wave 47400)"),
    (47396, "smoke_soft", 1, "smoke soft lamp (wave 47400)"),
    (47397, "dyn_soft", 1, "dyn soft lamp (wave 47400)"),
    (47398, "milestone_tag", MILESTONE, "milestone 47400 product tag"),
    (47399, "continuum_wave", MILESTONE, "milestone 47400 continuum wave identity"),
    # 47400 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        47376: "continuum wave fill soft unit + batch id",
        47380: "continuum bridge soft unit + batch id",
        47390: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (47391–47399), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 47393
    tag_batch = MARKERS_LO + 7  # 47398
    wave_batch = MARKERS_LO + 8  # 47399
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_47300 / gj_shell_green_47400 / gj_shell_green_47100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_47300 / gj_libcgj_green_47400 / gj_libcgj_green_47100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_47300 / gj_bar3_ready_47400 / gj_bar3_ready_47100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_47300 / gj_product_score_47400 / gj_product_score_47100\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_47300 / gj_continuum_ready_47200 /\n"
            f" * gj_continuum_ready_47100 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_47300 / gj_smoke_soft_47400 / gj_smoke_soft_47100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_47300 / gj_dyn_soft_47400 / gj_dyn_soft_47100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_47300 / gj_milestone_tag_47400 / gj_milestone_tag_47100,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_47300 / gj_continuum_wave_47200 /\n"
            f" * gj_continuum_wave_47100, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch47400.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 47399
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 47391–47399; 47400 is capstone
