...
"""
Soft continuum generator — MILESTONE 47500 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 47401–47500 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=47500   when parent has wired through graph_batch47500.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 47401 47500
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 47401 47500
...
graph batches ending at M. For M = **47500** the exclusive range is
**47401–47500** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 47401–47450  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 47451–47475  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 47476–47490  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 47491–47500  │ decade soft gates (Wave 333 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 47500 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_47500        1          graph_batch47491.c
  gj_libcgj_green_47500       1          graph_batch47492.c
  gj_bar3_ready_47500         0          graph_batch47493.c  (bar3 open)
  gj_product_score_47500      0          graph_batch47494.c  (score open)
  gj_continuum_ready_47500    1          graph_batch47495.c
  gj_smoke_soft_47500         1          graph_batch47496.c
  gj_dyn_soft_47500           1          graph_batch47497.c
  gj_milestone_tag_47500      47500      graph_batch47498.c
  gj_continuum_wave_47500     47500      graph_batch47499.c
  gj_batch_id_47500           47500      graph_batch47500.c
  gj_graph_milestone_47500    47500      graph_batch47500.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=47500`` means parent wired CGJ_SRCS through
    ``graph_batch47500.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_47500`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_47500`` is intentionally **0**.
  ...
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_47400`` / ``gj_*_47300`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 333 exclusive CREATE-ONLY lands the **markers** band
  (47491–47500) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_47500.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_47500.py

  # Restrict to marker band only (soft-gate TUs; Wave 333 exclusive):
  python3 user/libcgj/src/_gen_milestone_47500.py --band markers

  # Extension identity only (47401–47450):
  python3 user/libcgj/src/_gen_milestone_47500.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_47500.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_47400.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 47500 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 47500
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 47401
WAVE_HI = 47500  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 47401, 47450
IDENTITY_LO, IDENTITY_HI = 47451, 47475
FILL_LO, FILL_HI = 47476, 47490
MARKERS_LO, MARKERS_HI = 47491, 47500
...
# Stems become gj_<stem>_47500. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (47491, "shell_green", 1, "shell product green extract (wave 47500)"),
    (47492, "libcgj_green", 1, "libcgj product green extract (wave 47500)"),
    (47493, "bar3_ready", 0, "product bar3 readiness gate (wave 47500)"),
    (47494, "product_score", 0, "product readiness score tag (wave 47500)"),
    (47495, "continuum_ready", 1, "continuum readiness gate (wave 47500)"),
    (47496, "smoke_soft", 1, "smoke soft lamp (wave 47500)"),
    (47497, "dyn_soft", 1, "dyn soft lamp (wave 47500)"),
    (47498, "milestone_tag", MILESTONE, "milestone 47500 product tag"),
    (47499, "continuum_wave", MILESTONE, "milestone 47500 continuum wave identity"),
    # 47500 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        47476: "continuum wave fill soft unit + batch id",
        47480: "continuum bridge soft unit + batch id",
        47490: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (47491–47499), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 47493
    tag_batch = MARKERS_LO + 7  # 47498
    wave_batch = MARKERS_LO + 8  # 47499
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_47400 / gj_shell_green_47300 / gj_shell_green_47200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_47400 / gj_libcgj_green_47300 / gj_libcgj_green_47200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_47400 / gj_bar3_ready_47300 / gj_bar3_ready_47200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_47400 / gj_product_score_47300 / gj_product_score_47200\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_47400 / gj_continuum_ready_47300 /\n"
            f" * gj_continuum_ready_47200 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_47400 / gj_smoke_soft_47300 / gj_smoke_soft_47200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_47400 / gj_dyn_soft_47300 / gj_dyn_soft_47200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_47400 / gj_milestone_tag_47300 / gj_milestone_tag_47200,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_47400 / gj_continuum_wave_47300 /\n"
            f" * gj_continuum_wave_47200, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch47500.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 47499
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 47491–47499; 47500 is capstone
