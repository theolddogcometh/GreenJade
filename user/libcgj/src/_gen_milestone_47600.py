...
"""
Soft continuum generator — MILESTONE 47600 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 47501–47600 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=47600   when parent has wired through graph_batch47600.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 47501 47600
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 47501 47600
...
graph batches ending at M. For M = **47600** the exclusive range is
**47501–47600** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 47501–47550  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 47551–47575  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 47576–47590  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 47591–47600  │ decade soft gates (Wave 334 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 47600 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_47600        1          graph_batch47591.c
  gj_libcgj_green_47600       1          graph_batch47592.c
  gj_bar3_ready_47600         0          graph_batch47593.c  (bar3 open)
  gj_product_score_47600      0          graph_batch47594.c  (score open)
  gj_continuum_ready_47600    1          graph_batch47595.c
  gj_smoke_soft_47600         1          graph_batch47596.c
  gj_dyn_soft_47600           1          graph_batch47597.c
  gj_milestone_tag_47600      47600      graph_batch47598.c
  gj_continuum_wave_47600     47600      graph_batch47599.c
  gj_batch_id_47600           47600      graph_batch47600.c
  gj_graph_milestone_47600    47600      graph_batch47600.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=47600`` means parent wired CGJ_SRCS through
    ``graph_batch47600.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_47600`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_47600`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M46400_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_47500`` / ``gj_*_47400`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 334 exclusive CREATE-ONLY lands the **markers** band
  (47591–47600) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_47600.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_47600.py

  # Restrict to marker band only (soft-gate TUs; Wave 334 exclusive):
  python3 user/libcgj/src/_gen_milestone_47600.py --band markers

  # Extension identity only (47501–47550):
  python3 user/libcgj/src/_gen_milestone_47600.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_47600.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_47500.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 47600 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 47600
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 47501
WAVE_HI = 47600  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 47501, 47550
IDENTITY_LO, IDENTITY_HI = 47551, 47575
FILL_LO, FILL_HI = 47576, 47590
MARKERS_LO, MARKERS_HI = 47591, 47600
...
# Stems become gj_<stem>_47600. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (47591, "shell_green", 1, "shell product green extract (wave 47600)"),
    (47592, "libcgj_green", 1, "libcgj product green extract (wave 47600)"),
    (47593, "bar3_ready", 0, "product bar3 readiness gate (wave 47600)"),
    (47594, "product_score", 0, "product readiness score tag (wave 47600)"),
    (47595, "continuum_ready", 1, "continuum readiness gate (wave 47600)"),
    (47596, "smoke_soft", 1, "smoke soft lamp (wave 47600)"),
    (47597, "dyn_soft", 1, "dyn soft lamp (wave 47600)"),
    (47598, "milestone_tag", MILESTONE, "milestone 47600 product tag"),
    (47599, "continuum_wave", MILESTONE, "milestone 47600 continuum wave identity"),
    # 47600 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        47576: "continuum wave fill soft unit + batch id",
        47580: "continuum bridge soft unit + batch id",
        47590: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (47591–47599), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 47593
    tag_batch = MARKERS_LO + 7  # 47598
    wave_batch = MARKERS_LO + 8  # 47599
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_47500 / gj_shell_green_47400 / gj_shell_green_47300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_47500 / gj_libcgj_green_47400 / gj_libcgj_green_47300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_47500 / gj_bar3_ready_47400 / gj_bar3_ready_47300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_47500 / gj_product_score_47400 / gj_product_score_47300\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_47500 / gj_continuum_ready_47400 /\n"
            f" * gj_continuum_ready_47300 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_47500 / gj_smoke_soft_47400 / gj_smoke_soft_47300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_47500 / gj_dyn_soft_47400 / gj_dyn_soft_47300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_47500 / gj_milestone_tag_47400 / gj_milestone_tag_47300,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_47500 / gj_continuum_wave_47400 /\n"
            f" * gj_continuum_wave_47300, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch47600.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 47599
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 47591–47599; 47600 is capstone
