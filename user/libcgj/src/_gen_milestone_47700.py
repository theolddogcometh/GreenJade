...
"""
Soft continuum generator — MILESTONE 47700 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 47601–47700 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=47700   when parent has wired through graph_batch47700.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 47601 47700
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 47601 47700
...
graph batches ending at M. For M = **47700** the exclusive range is
**47601–47700** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 47601–47650  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 47651–47675  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 47676–47690  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 47691–47700  │ decade soft gates (Wave 335 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 47700 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_47700        1          graph_batch47691.c
  gj_libcgj_green_47700       1          graph_batch47692.c
  gj_bar3_ready_47700         0          graph_batch47693.c  (bar3 open)
  gj_product_score_47700      0          graph_batch47694.c  (score open)
  gj_continuum_ready_47700    1          graph_batch47695.c
  gj_smoke_soft_47700         1          graph_batch47696.c
  gj_dyn_soft_47700           1          graph_batch47697.c
  gj_milestone_tag_47700      47700      graph_batch47698.c
  gj_continuum_wave_47700     47700      graph_batch47699.c
  gj_batch_id_47700           47700      graph_batch47700.c
  gj_graph_milestone_47700    47700      graph_batch47700.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=47700`` means parent wired CGJ_SRCS through
    ``graph_batch47700.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_47700`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_47700`` is intentionally **0**.
  ...
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_47600`` / ``gj_*_47500`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 335 exclusive CREATE-ONLY lands the **markers** band
  (47691–47700) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_47700.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_47700.py

  # Restrict to marker band only (soft-gate TUs; Wave 335 exclusive):
  python3 user/libcgj/src/_gen_milestone_47700.py --band markers

  # Extension identity only (47601–47650):
  python3 user/libcgj/src/_gen_milestone_47700.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_47700.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_47600.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 47700 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 47700
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 47601
WAVE_HI = 47700  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 47601, 47650
IDENTITY_LO, IDENTITY_HI = 47651, 47675
FILL_LO, FILL_HI = 47676, 47690
MARKERS_LO, MARKERS_HI = 47691, 47700
...
# Stems become gj_<stem>_47700. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (47691, "shell_green", 1, "shell product green extract (wave 47700)"),
    (47692, "libcgj_green", 1, "libcgj product green extract (wave 47700)"),
    (47693, "bar3_ready", 0, "product bar3 readiness gate (wave 47700)"),
    (47694, "product_score", 0, "product readiness score tag (wave 47700)"),
    (47695, "continuum_ready", 1, "continuum readiness gate (wave 47700)"),
    (47696, "smoke_soft", 1, "smoke soft lamp (wave 47700)"),
    (47697, "dyn_soft", 1, "dyn soft lamp (wave 47700)"),
    (47698, "milestone_tag", MILESTONE, "milestone 47700 product tag"),
    (47699, "continuum_wave", MILESTONE, "milestone 47700 continuum wave identity"),
    # 47700 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        47676: "continuum wave fill soft unit + batch id",
        47680: "continuum bridge soft unit + batch id",
        47690: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (47691–47699), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 47693
    tag_batch = MARKERS_LO + 7  # 47698
    wave_batch = MARKERS_LO + 8  # 47699
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_47600 / gj_shell_green_47500 / gj_shell_green_47400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_47600 / gj_libcgj_green_47500 / gj_libcgj_green_47400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_47600 / gj_bar3_ready_47500 / gj_bar3_ready_47400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_47600 / gj_product_score_47500 / gj_product_score_47400\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_47600 / gj_continuum_ready_47500 /\n"
            f" * gj_continuum_ready_47400 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_47600 / gj_smoke_soft_47500 / gj_smoke_soft_47400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_47600 / gj_dyn_soft_47500 / gj_dyn_soft_47400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_47600 / gj_milestone_tag_47500 / gj_milestone_tag_47400,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_47600 / gj_continuum_wave_47500 /\n"
            f" * gj_continuum_wave_47400, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch47700.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 47699
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 47691–47699; 47700 is capstone
