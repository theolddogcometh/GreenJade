...
"""
Soft continuum generator — MILESTONE 47800 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 47701–47800 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=47800   when parent has wired through graph_batch47800.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 47701 47800
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 47701 47800
...
graph batches ending at M. For M = **47800** the exclusive range is
**47701–47800** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 47701–47750  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 47751–47775  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 47776–47790  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 47791–47800  │ decade soft gates (Wave 336 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 47800 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_47800        1          graph_batch47791.c
  gj_libcgj_green_47800       1          graph_batch47792.c
  gj_bar3_ready_47800         0          graph_batch47793.c  (bar3 open)
  gj_product_score_47800      0          graph_batch47794.c  (score open)
  gj_continuum_ready_47800    1          graph_batch47795.c
  gj_smoke_soft_47800         1          graph_batch47796.c
  gj_dyn_soft_47800           1          graph_batch47797.c
  gj_milestone_tag_47800      47800      graph_batch47798.c
  gj_continuum_wave_47800     47800      graph_batch47799.c
  gj_batch_id_47800           47800      graph_batch47800.c
  gj_graph_milestone_47800    47800      graph_batch47800.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=47800`` means parent wired CGJ_SRCS through
    ``graph_batch47800.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_47800`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_47800`` is intentionally **0**.
  ...
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_47700`` / ``gj_*_47600`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 336 exclusive CREATE-ONLY lands the **markers** band
  (47791–47800) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_47800.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_47800.py

  # Restrict to marker band only (soft-gate TUs; Wave 336 exclusive):
  python3 user/libcgj/src/_gen_milestone_47800.py --band markers

  # Extension identity only (47701–47750):
  python3 user/libcgj/src/_gen_milestone_47800.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_47800.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_47700.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 47800 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 47800
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 47701
WAVE_HI = 47800  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 47701, 47750
IDENTITY_LO, IDENTITY_HI = 47751, 47775
FILL_LO, FILL_HI = 47776, 47790
MARKERS_LO, MARKERS_HI = 47791, 47800
...
# Stems become gj_<stem>_47800. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (47791, "shell_green", 1, "shell product green extract (wave 47800)"),
    (47792, "libcgj_green", 1, "libcgj product green extract (wave 47800)"),
    (47793, "bar3_ready", 0, "product bar3 readiness gate (wave 47800)"),
    (47794, "product_score", 0, "product readiness score tag (wave 47800)"),
    (47795, "continuum_ready", 1, "continuum readiness gate (wave 47800)"),
    (47796, "smoke_soft", 1, "smoke soft lamp (wave 47800)"),
    (47797, "dyn_soft", 1, "dyn soft lamp (wave 47800)"),
    (47798, "milestone_tag", MILESTONE, "milestone 47800 product tag"),
    (47799, "continuum_wave", MILESTONE, "milestone 47800 continuum wave identity"),
    # 47800 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        47776: "continuum wave fill soft unit + batch id",
        47780: "continuum bridge soft unit + batch id",
        47790: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (47791–47799), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 47793
    tag_batch = MARKERS_LO + 7  # 47798
    wave_batch = MARKERS_LO + 8  # 47799
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_47700 / gj_shell_green_47600 / gj_shell_green_47500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_47700 / gj_libcgj_green_47600 / gj_libcgj_green_47500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_47700 / gj_bar3_ready_47600 / gj_bar3_ready_47500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_47700 / gj_product_score_47600 / gj_product_score_47500\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_47700 / gj_continuum_ready_47600 /\n"
            f" * gj_continuum_ready_47500 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_47700 / gj_smoke_soft_47600 / gj_smoke_soft_47500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_47700 / gj_dyn_soft_47600 / gj_dyn_soft_47500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_47700 / gj_milestone_tag_47600 / gj_milestone_tag_47500,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_47700 / gj_continuum_wave_47600 /\n"
            f" * gj_continuum_wave_47500, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch47800.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 47799
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 47791–47799; 47800 is capstone
