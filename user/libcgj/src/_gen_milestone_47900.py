...
"""
Soft continuum generator — MILESTONE 47900 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 47801–47900 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=47900   when parent has wired through graph_batch47900.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 47801 47900
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 47801 47900
...
graph batches ending at M. For M = **47900** the exclusive range is
**47801–47900** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 47801–47850  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 47851–47875  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 47876–47890  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 47891–47900  │ decade soft gates (Wave 337 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 47900 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_47900        1          graph_batch47891.c
  gj_libcgj_green_47900       1          graph_batch47892.c
  gj_bar3_ready_47900         0          graph_batch47893.c  (bar3 open)
  gj_product_score_47900      0          graph_batch47894.c  (score open)
  gj_continuum_ready_47900    1          graph_batch47895.c
  gj_smoke_soft_47900         1          graph_batch47896.c
  gj_dyn_soft_47900           1          graph_batch47897.c
  gj_milestone_tag_47900      47900      graph_batch47898.c
  gj_continuum_wave_47900     47900      graph_batch47899.c
  gj_batch_id_47900           47900      graph_batch47900.c
  gj_graph_milestone_47900    47900      graph_batch47900.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=47900`` means parent wired CGJ_SRCS through
    ``graph_batch47900.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_47900`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_47900`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M47900_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_47800`` / ``gj_*_47700`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 337 exclusive CREATE-ONLY lands the **markers** band
  (47891–47900) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_47900.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_47900.py

  # Restrict to marker band only (soft-gate TUs; Wave 337 exclusive):
  python3 user/libcgj/src/_gen_milestone_47900.py --band markers

  # Extension identity only (47801–47850):
  python3 user/libcgj/src/_gen_milestone_47900.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_47900.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_47800.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 47900 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 47900
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 47801
WAVE_HI = 47900  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 47801, 47850
IDENTITY_LO, IDENTITY_HI = 47851, 47875
FILL_LO, FILL_HI = 47876, 47890
MARKERS_LO, MARKERS_HI = 47891, 47900
...
# Stems become gj_<stem>_47900. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (47891, "shell_green", 1, "shell product green extract (wave 47900)"),
    (47892, "libcgj_green", 1, "libcgj product green extract (wave 47900)"),
    (47893, "bar3_ready", 0, "product bar3 readiness gate (wave 47900)"),
    (47894, "product_score", 0, "product readiness score tag (wave 47900)"),
    (47895, "continuum_ready", 1, "continuum readiness gate (wave 47900)"),
    (47896, "smoke_soft", 1, "smoke soft lamp (wave 47900)"),
    (47897, "dyn_soft", 1, "dyn soft lamp (wave 47900)"),
    (47898, "milestone_tag", MILESTONE, "milestone 47900 product tag"),
    (47899, "continuum_wave", MILESTONE, "milestone 47900 continuum wave identity"),
    # 47900 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        47876: "continuum wave fill soft unit + batch id",
        47880: "continuum bridge soft unit + batch id",
        47890: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (47891–47899), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 47893
    tag_batch = MARKERS_LO + 7  # 47898
    wave_batch = MARKERS_LO + 8  # 47899
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_47800 / gj_shell_green_47700 / gj_shell_green_47600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_47800 / gj_libcgj_green_47700 / gj_libcgj_green_47600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_47800 / gj_bar3_ready_47700 / gj_bar3_ready_47600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_47800 / gj_product_score_47700 / gj_product_score_47600\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_47800 / gj_continuum_ready_47700 /\n"
            f" * gj_continuum_ready_47600 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_47800 / gj_smoke_soft_47700 / gj_smoke_soft_47600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_47800 / gj_dyn_soft_47700 / gj_dyn_soft_47600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_47800 / gj_milestone_tag_47700 / gj_milestone_tag_47600,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_47800 / gj_continuum_wave_47700 /\n"
            f" * gj_continuum_wave_47600, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch47900.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 47899
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 47891–47899; 47900 is capstone
