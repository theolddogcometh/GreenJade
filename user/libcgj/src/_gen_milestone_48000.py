...
"""
Soft continuum generator — MILESTONE 48000 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 47901–48000 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=48000   when parent has wired through graph_batch48000.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 47901 48000
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 47901 48000
...
graph batches ending at M. For M = **48000** the exclusive range is
**47901–48000** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 47901–47950  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 47951–47975  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 47976–47990  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 47991–48000  │ decade soft gates (Wave 338 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 48000 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_48000        1          graph_batch47991.c
  gj_libcgj_green_48000       1          graph_batch47992.c
  gj_bar3_ready_48000         0          graph_batch47993.c  (bar3 open)
  gj_product_score_48000      0          graph_batch47994.c  (score open)
  gj_continuum_ready_48000    1          graph_batch47995.c
  gj_smoke_soft_48000         1          graph_batch47996.c
  gj_dyn_soft_48000           1          graph_batch47997.c
  gj_milestone_tag_48000      48000      graph_batch47998.c
  gj_continuum_wave_48000     48000      graph_batch47999.c
  gj_batch_id_48000           48000      graph_batch48000.c
  gj_graph_milestone_48000    48000      graph_batch48000.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=48000`` means parent wired CGJ_SRCS through
    ``graph_batch48000.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_48000`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_48000`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M48000_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_47900`` / ``gj_*_47800`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 338 exclusive CREATE-ONLY lands the **markers** band
  (47991–48000) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_48000.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_48000.py

  # Restrict to marker band only (soft-gate TUs; Wave 338 exclusive):
  python3 user/libcgj/src/_gen_milestone_48000.py --band markers

  # Extension identity only (47901–47950):
  python3 user/libcgj/src/_gen_milestone_48000.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_48000.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_47900.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 48000 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 48000
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 47901
WAVE_HI = 48000  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 47901, 47950
IDENTITY_LO, IDENTITY_HI = 47951, 47975
FILL_LO, FILL_HI = 47976, 47990
MARKERS_LO, MARKERS_HI = 47991, 48000
...
# Stems become gj_<stem>_48000. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (47991, "shell_green", 1, "shell product green extract (wave 48000)"),
    (47992, "libcgj_green", 1, "libcgj product green extract (wave 48000)"),
    (47993, "bar3_ready", 0, "product bar3 readiness gate (wave 48000)"),
    (47994, "product_score", 0, "product readiness score tag (wave 48000)"),
    (47995, "continuum_ready", 1, "continuum readiness gate (wave 48000)"),
    (47996, "smoke_soft", 1, "smoke soft lamp (wave 48000)"),
    (47997, "dyn_soft", 1, "dyn soft lamp (wave 48000)"),
    (47998, "milestone_tag", MILESTONE, "milestone 48000 product tag"),
    (47999, "continuum_wave", MILESTONE, "milestone 48000 continuum wave identity"),
    # 48000 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        47976: "continuum wave fill soft unit + batch id",
        47980: "continuum bridge soft unit + batch id",
        47990: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (47991–47999), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 47993
    tag_batch = MARKERS_LO + 7  # 47998
    wave_batch = MARKERS_LO + 8  # 47999
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_47900 / gj_shell_green_47800 / gj_shell_green_47700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_47900 / gj_libcgj_green_47800 / gj_libcgj_green_47700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_47900 / gj_bar3_ready_47800 / gj_bar3_ready_47700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_47900 / gj_product_score_47800 / gj_product_score_47700\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_47900 / gj_continuum_ready_47800 /\n"
            f" * gj_continuum_ready_47700 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_47900 / gj_smoke_soft_47800 / gj_smoke_soft_47700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_47900 / gj_dyn_soft_47800 / gj_dyn_soft_47700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_47900 / gj_milestone_tag_47800 / gj_milestone_tag_47700,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_47900 / gj_continuum_wave_47800 /\n"
            f" * gj_continuum_wave_47700, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch48000.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 47999
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 47991–47999; 48000 is capstone
