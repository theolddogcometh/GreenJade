...
"""
Soft continuum generator — MILESTONE 48100 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 48001–48100 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=48100   when parent has wired through graph_batch48100.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 48001 48100
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 48001 48100
...
graph batches ending at M. For M = **48100** the exclusive range is
**48001–48100** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 48001–48050  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 48051–48075  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 48076–48090  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 48091–48100  │ decade soft gates (Wave 339 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 48100 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_48100        1          graph_batch48091.c
  gj_libcgj_green_48100       1          graph_batch48092.c
  gj_bar3_ready_48100         0          graph_batch48093.c  (bar3 open)
  gj_product_score_48100      0          graph_batch48094.c  (score open)
  gj_continuum_ready_48100    1          graph_batch48095.c
  gj_smoke_soft_48100         1          graph_batch48096.c
  gj_dyn_soft_48100           1          graph_batch48097.c
  gj_milestone_tag_48100      48100      graph_batch48098.c
  gj_continuum_wave_48100     48100      graph_batch48099.c
  gj_batch_id_48100           48100      graph_batch48100.c
  gj_graph_milestone_48100    48100      graph_batch48100.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=48100`` means parent wired CGJ_SRCS through
    ``graph_batch48100.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_48100`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_48100`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M48100_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_48000`` / ``gj_*_47900`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 339 exclusive CREATE-ONLY lands the **markers** band
  (48091–48100) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_48100.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_48100.py

  # Restrict to marker band only (soft-gate TUs; Wave 339 exclusive):
  python3 user/libcgj/src/_gen_milestone_48100.py --band markers

  # Extension identity only (48001–48050):
  python3 user/libcgj/src/_gen_milestone_48100.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_48100.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_48000.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 48100 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 48100
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 48001
WAVE_HI = 48100  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 48001, 48050
IDENTITY_LO, IDENTITY_HI = 48051, 48075
FILL_LO, FILL_HI = 48076, 48090
MARKERS_LO, MARKERS_HI = 48091, 48100
...
# Stems become gj_<stem>_48100. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (48091, "shell_green", 1, "shell product green extract (wave 48100)"),
    (48092, "libcgj_green", 1, "libcgj product green extract (wave 48100)"),
    (48093, "bar3_ready", 0, "product bar3 readiness gate (wave 48100)"),
    (48094, "product_score", 0, "product readiness score tag (wave 48100)"),
    (48095, "continuum_ready", 1, "continuum readiness gate (wave 48100)"),
    (48096, "smoke_soft", 1, "smoke soft lamp (wave 48100)"),
    (48097, "dyn_soft", 1, "dyn soft lamp (wave 48100)"),
    (48098, "milestone_tag", MILESTONE, "milestone 48100 product tag"),
    (48099, "continuum_wave", MILESTONE, "milestone 48100 continuum wave identity"),
    # 48100 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        48076: "continuum wave fill soft unit + batch id",
        48080: "continuum bridge soft unit + batch id",
        48090: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (48091–48099), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 48093
    tag_batch = MARKERS_LO + 7  # 48098
    wave_batch = MARKERS_LO + 8  # 48099
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_48000 / gj_shell_green_47900 / gj_shell_green_47800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_48000 / gj_libcgj_green_47900 / gj_libcgj_green_47800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_48000 / gj_bar3_ready_47900 / gj_bar3_ready_47800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_48000 / gj_product_score_47900 / gj_product_score_47800\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_48000 / gj_continuum_ready_47900 /\n"
            f" * gj_continuum_ready_47800 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_48000 / gj_smoke_soft_47900 / gj_smoke_soft_47800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_48000 / gj_dyn_soft_47900 / gj_dyn_soft_47800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_48000 / gj_milestone_tag_47900 / gj_milestone_tag_47800,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_48000 / gj_continuum_wave_47900 /\n"
            f" * gj_continuum_wave_47800, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch48100.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 48099
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 48091–48099; 48100 is capstone
