...
"""
Soft continuum generator — MILESTONE 48200 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 48101–48200 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=48200   when parent has wired through graph_batch48200.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 48101 48200
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 48101 48200
...
graph batches ending at M. For M = **48200** the exclusive range is
**48101–48200** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 48101–48150  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 48151–48175  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 48176–48190  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 48191–48200  │ decade soft gates (Wave 340 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 48200 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_48200        1          graph_batch48191.c
  gj_libcgj_green_48200       1          graph_batch48192.c
  gj_bar3_ready_48200         0          graph_batch48193.c  (bar3 open)
  gj_product_score_48200      0          graph_batch48194.c  (score open)
  gj_continuum_ready_48200    1          graph_batch48195.c
  gj_smoke_soft_48200         1          graph_batch48196.c
  gj_dyn_soft_48200           1          graph_batch48197.c
  gj_milestone_tag_48200      48200      graph_batch48198.c
  gj_continuum_wave_48200     48200      graph_batch48199.c
  gj_batch_id_48200           48200      graph_batch48200.c
  gj_graph_milestone_48200    48200      graph_batch48200.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=48200`` means parent wired CGJ_SRCS through
    ``graph_batch48200.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_48200`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_48200`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M48200_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_48100`` / ``gj_*_48000`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 340 exclusive CREATE-ONLY lands the **markers** band
  (48191–48200) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_48200.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_48200.py

  # Restrict to marker band only (soft-gate TUs; Wave 340 exclusive):
  python3 user/libcgj/src/_gen_milestone_48200.py --band markers

  # Extension identity only (48101–48150):
  python3 user/libcgj/src/_gen_milestone_48200.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_48200.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_48100.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 48200 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 48200
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 48101
WAVE_HI = 48200  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 48101, 48150
IDENTITY_LO, IDENTITY_HI = 48151, 48175
FILL_LO, FILL_HI = 48176, 48190
MARKERS_LO, MARKERS_HI = 48191, 48200
...
# Stems become gj_<stem>_48200. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (48191, "shell_green", 1, "shell product green extract (wave 48200)"),
    (48192, "libcgj_green", 1, "libcgj product green extract (wave 48200)"),
    (48193, "bar3_ready", 0, "product bar3 readiness gate (wave 48200)"),
    (48194, "product_score", 0, "product readiness score tag (wave 48200)"),
    (48195, "continuum_ready", 1, "continuum readiness gate (wave 48200)"),
    (48196, "smoke_soft", 1, "smoke soft lamp (wave 48200)"),
    (48197, "dyn_soft", 1, "dyn soft lamp (wave 48200)"),
    (48198, "milestone_tag", MILESTONE, "milestone 48200 product tag"),
    (48199, "continuum_wave", MILESTONE, "milestone 48200 continuum wave identity"),
    # 48200 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        48176: "continuum wave fill soft unit + batch id",
        48180: "continuum bridge soft unit + batch id",
        48190: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (48191–48199), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 48193
    tag_batch = MARKERS_LO + 7  # 48198
    wave_batch = MARKERS_LO + 8  # 48199
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_48100 / gj_shell_green_48000 / gj_shell_green_47900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_48100 / gj_libcgj_green_48000 / gj_libcgj_green_47900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_48100 / gj_bar3_ready_48000 / gj_bar3_ready_47900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_48100 / gj_product_score_48000 / gj_product_score_47900\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_48100 / gj_continuum_ready_48000 /\n"
            f" * gj_continuum_ready_47900 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_48100 / gj_smoke_soft_48000 / gj_smoke_soft_47900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_48100 / gj_dyn_soft_48000 / gj_dyn_soft_47900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_48100 / gj_milestone_tag_48000 / gj_milestone_tag_47900,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_48100 / gj_continuum_wave_48000 /\n"
            f" * gj_continuum_wave_47900, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch48200.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 48199
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 48191–48199; 48200 is capstone
