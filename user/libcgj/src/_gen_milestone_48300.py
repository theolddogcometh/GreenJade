...
"""
Soft continuum generator — MILESTONE 48300 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 48201–48300 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=48300   when parent has wired through graph_batch48300.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 48201 48300
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 48201 48300
...
graph batches ending at M. For M = **48300** the exclusive range is
**48201–48300** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 48201–48250  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 48251–48275  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 48276–48290  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 48291–48300  │ decade soft gates (Wave 341 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 48300 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_48300        1          graph_batch48291.c
  gj_libcgj_green_48300       1          graph_batch48292.c
  gj_bar3_ready_48300         0          graph_batch48293.c  (bar3 open)
  gj_product_score_48300      0          graph_batch48294.c  (score open)
  gj_continuum_ready_48300    1          graph_batch48295.c
  gj_smoke_soft_48300         1          graph_batch48296.c
  gj_dyn_soft_48300           1          graph_batch48297.c
  gj_milestone_tag_48300      48300      graph_batch48298.c
  gj_continuum_wave_48300     48300      graph_batch48299.c
  gj_batch_id_48300           48300      graph_batch48300.c
  gj_graph_milestone_48300    48300      graph_batch48300.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=48300`` means parent wired CGJ_SRCS through
    ``graph_batch48300.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_48300`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_48300`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M48300_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_48200`` / ``gj_*_48100`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 341 exclusive CREATE-ONLY lands the **markers** band
  (48291–48300) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_48300.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_48300.py

  # Restrict to marker band only (soft-gate TUs; Wave 341 exclusive):
  python3 user/libcgj/src/_gen_milestone_48300.py --band markers

  # Extension identity only (48201–48250):
  python3 user/libcgj/src/_gen_milestone_48300.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_48300.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_48200.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 48300 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 48300
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 48201
WAVE_HI = 48300  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 48201, 48250
IDENTITY_LO, IDENTITY_HI = 48251, 48275
FILL_LO, FILL_HI = 48276, 48290
MARKERS_LO, MARKERS_HI = 48291, 48300
...
# Stems become gj_<stem>_48300. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (48291, "shell_green", 1, "shell product green extract (wave 48300)"),
    (48292, "libcgj_green", 1, "libcgj product green extract (wave 48300)"),
    (48293, "bar3_ready", 0, "product bar3 readiness gate (wave 48300)"),
    (48294, "product_score", 0, "product readiness score tag (wave 48300)"),
    (48295, "continuum_ready", 1, "continuum readiness gate (wave 48300)"),
    (48296, "smoke_soft", 1, "smoke soft lamp (wave 48300)"),
    (48297, "dyn_soft", 1, "dyn soft lamp (wave 48300)"),
    (48298, "milestone_tag", MILESTONE, "milestone 48300 product tag"),
    (48299, "continuum_wave", MILESTONE, "milestone 48300 continuum wave identity"),
    # 48300 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        48276: "continuum wave fill soft unit + batch id",
        48280: "continuum bridge soft unit + batch id",
        48290: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (48291–48299), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 48293
    tag_batch = MARKERS_LO + 7  # 48298
    wave_batch = MARKERS_LO + 8  # 48299
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_48200 / gj_shell_green_48100 / gj_shell_green_48000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_48200 / gj_libcgj_green_48100 / gj_libcgj_green_48000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_48200 / gj_bar3_ready_48100 / gj_bar3_ready_48000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_48200 / gj_product_score_48100 / gj_product_score_48000\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_48200 / gj_continuum_ready_48100 /\n"
            f" * gj_continuum_ready_48000 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_48200 / gj_smoke_soft_48100 / gj_smoke_soft_48000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_48200 / gj_dyn_soft_48100 / gj_dyn_soft_48000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_48200 / gj_milestone_tag_48100 / gj_milestone_tag_48000,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_48200 / gj_continuum_wave_48100 /\n"
            f" * gj_continuum_wave_48000, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch48300.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 48299
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 48291–48299; 48300 is capstone
