...
"""
Soft continuum generator — MILESTONE 48400 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 48301–48400 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=48400   when parent has wired through graph_batch48400.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 48301 48400
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 48301 48400
...
graph batches ending at M. For M = **48400** the exclusive range is
**48301–48400** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 48301–48350  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 48351–48375  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 48376–48390  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 48391–48400  │ decade soft gates (Wave 342 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 48400 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_48400        1          graph_batch48391.c
  gj_libcgj_green_48400       1          graph_batch48392.c
  gj_bar3_ready_48400         0          graph_batch48393.c  (bar3 open)
  gj_product_score_48400      0          graph_batch48394.c  (score open)
  gj_continuum_ready_48400    1          graph_batch48395.c
  gj_smoke_soft_48400         1          graph_batch48396.c
  gj_dyn_soft_48400           1          graph_batch48397.c
  gj_milestone_tag_48400      48400      graph_batch48398.c
  gj_continuum_wave_48400     48400      graph_batch48399.c
  gj_batch_id_48400           48400      graph_batch48400.c
  gj_graph_milestone_48400    48400      graph_batch48400.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=48400`` means parent wired CGJ_SRCS through
    ``graph_batch48400.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_48400`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_48400`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M48400_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_48300`` / ``gj_*_48200`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  (48391–48400) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_48400.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_48400.py

  # Restrict to marker band only (soft-gate TUs; Wave 342 exclusive):
  python3 user/libcgj/src/_gen_milestone_48400.py --band markers

  # Extension identity only (48301–48350):
  python3 user/libcgj/src/_gen_milestone_48400.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_48400.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_48300.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 48400 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 48400
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 48301
WAVE_HI = 48400  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 48301, 48350
IDENTITY_LO, IDENTITY_HI = 48351, 48375
FILL_LO, FILL_HI = 48376, 48390
MARKERS_LO, MARKERS_HI = 48391, 48400
...
# Stems become gj_<stem>_48400. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (48391, "shell_green", 1, "shell product green extract (wave 48400)"),
    (48392, "libcgj_green", 1, "libcgj product green extract (wave 48400)"),
    (48393, "bar3_ready", 0, "product bar3 readiness gate (wave 48400)"),
    (48394, "product_score", 0, "product readiness score tag (wave 48400)"),
    (48395, "continuum_ready", 1, "continuum readiness gate (wave 48400)"),
    (48396, "smoke_soft", 1, "smoke soft lamp (wave 48400)"),
    (48397, "dyn_soft", 1, "dyn soft lamp (wave 48400)"),
    (48398, "milestone_tag", MILESTONE, "milestone 48400 product tag"),
    (48399, "continuum_wave", MILESTONE, "milestone 48400 continuum wave identity"),
    # 48400 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        48376: "continuum wave fill soft unit + batch id",
        48380: "continuum bridge soft unit + batch id",
        48390: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (48391–48399), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 48393
    tag_batch = MARKERS_LO + 7  # 48398
    wave_batch = MARKERS_LO + 8  # 48399
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_48300 / gj_shell_green_48200 / gj_shell_green_48100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_48300 / gj_libcgj_green_48200 / gj_libcgj_green_48100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_48300 / gj_bar3_ready_48200 / gj_bar3_ready_48100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_48300 / gj_product_score_48200 / gj_product_score_48100\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_48300 / gj_continuum_ready_48200 /\n"
            f" * gj_continuum_ready_48100 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_48300 / gj_smoke_soft_48200 / gj_smoke_soft_48100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_48300 / gj_dyn_soft_48200 / gj_dyn_soft_48100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_48300 / gj_milestone_tag_48200 / gj_milestone_tag_48100,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_48300 / gj_continuum_wave_48200 /\n"
            f" * gj_continuum_wave_48100, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch48400.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 48399
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 48391–48399; 48400 is capstone
