...
"""
Soft continuum generator — MILESTONE 48500 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 48401–48500 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=48500   when parent has wired through graph_batch48500.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 48401 48500
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 48401 48500
...
graph batches ending at M. For M = **48500** the exclusive range is
**48401–48500** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 48401–48450  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 48451–48475  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 48476–48490  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 48491–48500  │ decade soft gates (Wave 342 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 48500 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_48500        1          graph_batch48491.c
  gj_libcgj_green_48500       1          graph_batch48492.c
  gj_bar3_ready_48500         0          graph_batch48493.c  (bar3 open)
  gj_product_score_48500      0          graph_batch48494.c  (score open)
  gj_continuum_ready_48500    1          graph_batch48495.c
  gj_smoke_soft_48500         1          graph_batch48496.c
  gj_dyn_soft_48500           1          graph_batch48497.c
  gj_milestone_tag_48500      48500      graph_batch48498.c
  gj_continuum_wave_48500     48500      graph_batch48499.c
  gj_batch_id_48500           48500      graph_batch48500.c
  gj_graph_milestone_48500    48500      graph_batch48500.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=48500`` means parent wired CGJ_SRCS through
    ``graph_batch48500.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_48500`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_48500`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M48500_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_48400`` / ``gj_*_48300`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 342 exclusive CREATE-ONLY lands the **markers** band
  (48491–48500) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_48500.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_48500.py

  # Restrict to marker band only (soft-gate TUs; Wave 342 exclusive):
  python3 user/libcgj/src/_gen_milestone_48500.py --band markers

  # Extension identity only (48401–48450):
  python3 user/libcgj/src/_gen_milestone_48500.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_48500.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_48400.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 48500 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 48500
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 48401
WAVE_HI = 48500  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 48401, 48450
IDENTITY_LO, IDENTITY_HI = 48451, 48475
FILL_LO, FILL_HI = 48476, 48490
MARKERS_LO, MARKERS_HI = 48491, 48500
...
# Stems become gj_<stem>_48500. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (48491, "shell_green", 1, "shell product green extract (wave 48500)"),
    (48492, "libcgj_green", 1, "libcgj product green extract (wave 48500)"),
    (48493, "bar3_ready", 0, "product bar3 readiness gate (wave 48500)"),
    (48494, "product_score", 0, "product readiness score tag (wave 48500)"),
    (48495, "continuum_ready", 1, "continuum readiness gate (wave 48500)"),
    (48496, "smoke_soft", 1, "smoke soft lamp (wave 48500)"),
    (48497, "dyn_soft", 1, "dyn soft lamp (wave 48500)"),
    (48498, "milestone_tag", MILESTONE, "milestone 48500 product tag"),
    (48499, "continuum_wave", MILESTONE, "milestone 48500 continuum wave identity"),
    # 48500 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        48476: "continuum wave fill soft unit + batch id",
        48480: "continuum bridge soft unit + batch id",
        48490: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (48491–48499), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 48493
    tag_batch = MARKERS_LO + 7  # 48498
    wave_batch = MARKERS_LO + 8  # 48499
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_48400 / gj_shell_green_48300 / gj_shell_green_48200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_48400 / gj_libcgj_green_48300 / gj_libcgj_green_48200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_48400 / gj_bar3_ready_48300 / gj_bar3_ready_48200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_48400 / gj_product_score_48300 / gj_product_score_48200\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_48400 / gj_continuum_ready_48300 /\n"
            f" * gj_continuum_ready_48200 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_48400 / gj_smoke_soft_48300 / gj_smoke_soft_48200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_48400 / gj_dyn_soft_48300 / gj_dyn_soft_48200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_48400 / gj_milestone_tag_48300 / gj_milestone_tag_48200,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_48400 / gj_continuum_wave_48300 /\n"
            f" * gj_continuum_wave_48200, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch48500.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 48499
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 48491–48499; 48500 is capstone
