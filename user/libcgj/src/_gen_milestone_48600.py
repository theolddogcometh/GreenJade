...
"""
Soft continuum generator — MILESTONE 48600 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 48501–48600 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=48600   when parent has wired through graph_batch48600.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 48501 48600
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 48501 48600
...
graph batches ending at M. For M = **48600** the exclusive range is
**48501–48600** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 48501–48550  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 48551–48575  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 48576–48590  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 48591–48600  │ decade soft gates (Wave 344 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 48600 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_48600        1          graph_batch48591.c
  gj_libcgj_green_48600       1          graph_batch48592.c
  gj_bar3_ready_48600         0          graph_batch48593.c  (bar3 open)
  gj_product_score_48600      0          graph_batch48594.c  (score open)
  gj_continuum_ready_48600    1          graph_batch48595.c
  gj_smoke_soft_48600         1          graph_batch48596.c
  gj_dyn_soft_48600           1          graph_batch48597.c
  gj_milestone_tag_48600      48600      graph_batch48598.c
  gj_continuum_wave_48600     48600      graph_batch48599.c
  gj_batch_id_48600           48600      graph_batch48600.c
  gj_graph_milestone_48600    48600      graph_batch48600.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=48600`` means parent wired CGJ_SRCS through
    ``graph_batch48600.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_48600`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_48600`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M48600_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_48500`` / ``gj_*_48400`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 344 exclusive CREATE-ONLY lands the **markers** band
  (48591–48600) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_48600.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_48600.py

  # Restrict to marker band only (soft-gate TUs; Wave 344 exclusive):
  python3 user/libcgj/src/_gen_milestone_48600.py --band markers

  # Extension identity only (48501–48550):
  python3 user/libcgj/src/_gen_milestone_48600.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_48600.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_48500.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 48600 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 48600
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 48501
WAVE_HI = 48600  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 48501, 48550
IDENTITY_LO, IDENTITY_HI = 48551, 48575
FILL_LO, FILL_HI = 48576, 48590
MARKERS_LO, MARKERS_HI = 48591, 48600
...
# Stems become gj_<stem>_48600. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (48591, "shell_green", 1, "shell product green extract (wave 48600)"),
    (48592, "libcgj_green", 1, "libcgj product green extract (wave 48600)"),
    (48593, "bar3_ready", 0, "product bar3 readiness gate (wave 48600)"),
    (48594, "product_score", 0, "product readiness score tag (wave 48600)"),
    (48595, "continuum_ready", 1, "continuum readiness gate (wave 48600)"),
    (48596, "smoke_soft", 1, "smoke soft lamp (wave 48600)"),
    (48597, "dyn_soft", 1, "dyn soft lamp (wave 48600)"),
    (48598, "milestone_tag", MILESTONE, "milestone 48600 product tag"),
    (48599, "continuum_wave", MILESTONE, "milestone 48600 continuum wave identity"),
    # 48600 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        48576: "continuum wave fill soft unit + batch id",
        48580: "continuum bridge soft unit + batch id",
        48590: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (48591–48599), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 48593
    tag_batch = MARKERS_LO + 7  # 48598
    wave_batch = MARKERS_LO + 8  # 48599
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_48500 / gj_shell_green_48400 / gj_shell_green_48300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_48500 / gj_libcgj_green_48400 / gj_libcgj_green_48300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_48500 / gj_bar3_ready_48400 / gj_bar3_ready_48300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_48500 / gj_product_score_48400 / gj_product_score_48300\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_48500 / gj_continuum_ready_48400 /\n"
            f" * gj_continuum_ready_48300 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_48500 / gj_smoke_soft_48400 / gj_smoke_soft_48300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_48500 / gj_dyn_soft_48400 / gj_dyn_soft_48300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_48500 / gj_milestone_tag_48400 / gj_milestone_tag_48300,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_48500 / gj_continuum_wave_48400 /\n"
            f" * gj_continuum_wave_48300, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch48600.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 48599
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 48591–48599; 48600 is capstone
