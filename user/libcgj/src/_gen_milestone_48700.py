...
"""
Soft continuum generator — MILESTONE 48700 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 48601–48700 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=48700   when parent has wired through graph_batch48700.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 48601 48700
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 48601 48700
...
graph batches ending at M. For M = **48700** the exclusive range is
**48601–48700** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 48601–48650  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 48651–48675  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 48676–48690  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 48691–48700  │ decade soft gates (Wave 345 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 48700 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_48700        1          graph_batch48691.c
  gj_libcgj_green_48700       1          graph_batch48692.c
  gj_bar3_ready_48700         0          graph_batch48693.c  (bar3 open)
  gj_product_score_48700      0          graph_batch48694.c  (score open)
  gj_continuum_ready_48700    1          graph_batch48695.c
  gj_smoke_soft_48700         1          graph_batch48696.c
  gj_dyn_soft_48700           1          graph_batch48697.c
  gj_milestone_tag_48700      48700      graph_batch48698.c
  gj_continuum_wave_48700     48700      graph_batch48699.c
  gj_batch_id_48700           48700      graph_batch48700.c
  gj_graph_milestone_48700    48700      graph_batch48700.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=48700`` means parent wired CGJ_SRCS through
    ``graph_batch48700.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_48700`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_48700`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M48700_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_48600`` / ``gj_*_48500`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 345 exclusive CREATE-ONLY lands the **markers** band
  (48691–48700) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_48700.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_48700.py

  # Restrict to marker band only (soft-gate TUs; Wave 345 exclusive):
  python3 user/libcgj/src/_gen_milestone_48700.py --band markers

  # Extension identity only (48601–48650):
  python3 user/libcgj/src/_gen_milestone_48700.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_48700.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_48600.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 48700 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 48700
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 48601
WAVE_HI = 48700  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 48601, 48650
IDENTITY_LO, IDENTITY_HI = 48651, 48675
FILL_LO, FILL_HI = 48676, 48690
MARKERS_LO, MARKERS_HI = 48691, 48700
...
# Stems become gj_<stem>_48700. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (48691, "shell_green", 1, "shell product green extract (wave 48700)"),
    (48692, "libcgj_green", 1, "libcgj product green extract (wave 48700)"),
    (48693, "bar3_ready", 0, "product bar3 readiness gate (wave 48700)"),
    (48694, "product_score", 0, "product readiness score tag (wave 48700)"),
    (48695, "continuum_ready", 1, "continuum readiness gate (wave 48700)"),
    (48696, "smoke_soft", 1, "smoke soft lamp (wave 48700)"),
    (48697, "dyn_soft", 1, "dyn soft lamp (wave 48700)"),
    (48698, "milestone_tag", MILESTONE, "milestone 48700 product tag"),
    (48699, "continuum_wave", MILESTONE, "milestone 48700 continuum wave identity"),
    # 48700 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        48676: "continuum wave fill soft unit + batch id",
        48680: "continuum bridge soft unit + batch id",
        48690: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (48691–48699), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 48693
    tag_batch = MARKERS_LO + 7  # 48698
    wave_batch = MARKERS_LO + 8  # 48699
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_48600 / gj_shell_green_48500 / gj_shell_green_48700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_48600 / gj_libcgj_green_48500 / gj_libcgj_green_48700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_48600 / gj_bar3_ready_48500 / gj_bar3_ready_48700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_48600 / gj_product_score_48500 / gj_product_score_48700\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_48600 / gj_continuum_ready_48500 /\n"
            f" * gj_continuum_ready_48400 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_48600 / gj_smoke_soft_48500 / gj_smoke_soft_48700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_48600 / gj_dyn_soft_48500 / gj_dyn_soft_48700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_48600 / gj_milestone_tag_48500 / gj_milestone_tag_48700,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_48600 / gj_continuum_wave_48500 /\n"
            f" * gj_continuum_wave_48400, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch48700.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 48699
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 48691–48699; 48700 is capstone
