...
"""
Soft continuum generator — MILESTONE 48800 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 48701–48800 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=48800   when parent has wired through graph_batch48800.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 48701 48800
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 48701 48800
...
graph batches ending at M. For M = **48800** the exclusive range is
**48701–48800** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 48701–48750  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 48751–48775  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 48776–48790  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 48791–48800  │ decade soft gates (Wave 346 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 48800 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_48800        1          graph_batch48791.c
  gj_libcgj_green_48800       1          graph_batch48792.c
  gj_bar3_ready_48800         0          graph_batch48793.c  (bar3 open)
  gj_product_score_48800      0          graph_batch48794.c  (score open)
  gj_continuum_ready_48800    1          graph_batch48795.c
  gj_smoke_soft_48800         1          graph_batch48796.c
  gj_dyn_soft_48800           1          graph_batch48797.c
  gj_milestone_tag_48800      48800      graph_batch48798.c
  gj_continuum_wave_48800     48800      graph_batch48799.c
  gj_batch_id_48800           48800      graph_batch48800.c
  gj_graph_milestone_48800    48800      graph_batch48800.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=48800`` means parent wired CGJ_SRCS through
    ``graph_batch48800.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_48800`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_48800`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M48800_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_48700`` / ``gj_*_48600`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  (48791–48800) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_48800.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_48800.py

  # Restrict to marker band only (soft-gate TUs; Wave 346 exclusive):
  python3 user/libcgj/src/_gen_milestone_48800.py --band markers

  # Extension identity only (48701–48750):
  python3 user/libcgj/src/_gen_milestone_48800.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_48800.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_48700.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 48800 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 48800
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 48701
WAVE_HI = 48800  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 48701, 48750
IDENTITY_LO, IDENTITY_HI = 48751, 48775
FILL_LO, FILL_HI = 48776, 48790
MARKERS_LO, MARKERS_HI = 48791, 48800
...
# Stems become gj_<stem>_48800. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (48791, "shell_green", 1, "shell product green extract (wave 48800)"),
    (48792, "libcgj_green", 1, "libcgj product green extract (wave 48800)"),
    (48793, "bar3_ready", 0, "product bar3 readiness gate (wave 48800)"),
    (48794, "product_score", 0, "product readiness score tag (wave 48800)"),
    (48795, "continuum_ready", 1, "continuum readiness gate (wave 48800)"),
    (48796, "smoke_soft", 1, "smoke soft lamp (wave 48800)"),
    (48797, "dyn_soft", 1, "dyn soft lamp (wave 48800)"),
    (48798, "milestone_tag", MILESTONE, "milestone 48800 product tag"),
    (48799, "continuum_wave", MILESTONE, "milestone 48800 continuum wave identity"),
    # 48800 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        48776: "continuum wave fill soft unit + batch id",
        48780: "continuum bridge soft unit + batch id",
        48790: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (48791–48799), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 48793
    tag_batch = MARKERS_LO + 7  # 48798
    wave_batch = MARKERS_LO + 8  # 48799
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_48700 / gj_shell_green_48600 / gj_shell_green_48500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_48700 / gj_libcgj_green_48600 / gj_libcgj_green_48500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_48700 / gj_bar3_ready_48600 / gj_bar3_ready_48500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_48700 / gj_product_score_48600 / gj_product_score_48500\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_48700 / gj_continuum_ready_48600 /\n"
            f" * gj_continuum_ready_48500 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_48700 / gj_smoke_soft_48600 / gj_smoke_soft_48500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_48700 / gj_dyn_soft_48600 / gj_dyn_soft_48500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_48700 / gj_milestone_tag_48600 / gj_milestone_tag_48500,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_48700 / gj_continuum_wave_48600 /\n"
            f" * gj_continuum_wave_48500, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch48800.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 48799
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 48791–48799; 48800 is capstone
