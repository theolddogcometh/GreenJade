...
"""
Soft continuum generator — MILESTONE 48900 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 48801–48900 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=48900   when parent has wired through graph_batch48900.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 48801 48900
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 48801 48900
...
graph batches ending at M. For M = **48900** the exclusive range is
**48801–48900** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 48801–48850  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 48851–48875  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 48876–48890  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 48891–48900  │ decade soft gates (Wave 346 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 48900 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_48900        1          graph_batch48891.c
  gj_libcgj_green_48900       1          graph_batch48892.c
  gj_bar3_ready_48900         0          graph_batch48893.c  (bar3 open)
  gj_product_score_48900      0          graph_batch48894.c  (score open)
  gj_continuum_ready_48900    1          graph_batch48895.c
  gj_smoke_soft_48900         1          graph_batch48896.c
  gj_dyn_soft_48900           1          graph_batch48897.c
  gj_milestone_tag_48900      48900      graph_batch48898.c
  gj_continuum_wave_48900     48900      graph_batch48899.c
  gj_batch_id_48900           48900      graph_batch48900.c
  gj_graph_milestone_48900    48900      graph_batch48900.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=48900`` means parent wired CGJ_SRCS through
    ``graph_batch48900.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_48900`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_48900`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M48900_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_48800`` / ``gj_*_48700`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 346 exclusive CREATE-ONLY lands the **markers** band
  (48891–48900) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_48900.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_48900.py

  # Restrict to marker band only (soft-gate TUs; Wave 346 exclusive):
  python3 user/libcgj/src/_gen_milestone_48900.py --band markers

  # Extension identity only (48801–48850):
  python3 user/libcgj/src/_gen_milestone_48900.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_48900.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_48800.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 48900 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 48900
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 48801
WAVE_HI = 48900  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 48801, 48850
IDENTITY_LO, IDENTITY_HI = 48851, 48875
FILL_LO, FILL_HI = 48876, 48890
MARKERS_LO, MARKERS_HI = 48891, 48900
...
# Stems become gj_<stem>_48900. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (48891, "shell_green", 1, "shell product green extract (wave 48900)"),
    (48892, "libcgj_green", 1, "libcgj product green extract (wave 48900)"),
    (48893, "bar3_ready", 0, "product bar3 readiness gate (wave 48900)"),
    (48894, "product_score", 0, "product readiness score tag (wave 48900)"),
    (48895, "continuum_ready", 1, "continuum readiness gate (wave 48900)"),
    (48896, "smoke_soft", 1, "smoke soft lamp (wave 48900)"),
    (48897, "dyn_soft", 1, "dyn soft lamp (wave 48900)"),
    (48898, "milestone_tag", MILESTONE, "milestone 48900 product tag"),
    (48899, "continuum_wave", MILESTONE, "milestone 48900 continuum wave identity"),
    # 48900 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        48876: "continuum wave fill soft unit + batch id",
        48880: "continuum bridge soft unit + batch id",
        48890: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (48891–48899), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 48893
    tag_batch = MARKERS_LO + 7  # 48898
    wave_batch = MARKERS_LO + 8  # 48899
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_48800 / gj_shell_green_48700 / gj_shell_green_48600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_48800 / gj_libcgj_green_48700 / gj_libcgj_green_48600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_48800 / gj_bar3_ready_48700 / gj_bar3_ready_48600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_48800 / gj_product_score_48700 / gj_product_score_48600\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_48800 / gj_continuum_ready_48700 /\n"
            f" * gj_continuum_ready_48600 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_48800 / gj_smoke_soft_48700 / gj_smoke_soft_48600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_48800 / gj_dyn_soft_48700 / gj_dyn_soft_48600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_48800 / gj_milestone_tag_48700 / gj_milestone_tag_48600,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_48800 / gj_continuum_wave_48700 /\n"
            f" * gj_continuum_wave_48600, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch48900.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 48899
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 48891–48899; 48900 is capstone
