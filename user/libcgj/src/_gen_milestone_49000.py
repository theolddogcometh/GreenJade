...
"""
Soft continuum generator — MILESTONE 49000 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 48901–49000 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=49000   when parent has wired through graph_batch49000.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 48901 49000
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 48901 49000
...
graph batches ending at M. For M = **49000** the exclusive range is
**48901–49000** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 48901–48950  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 48951–48975  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 48976–48990  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 48991–49000  │ decade soft gates (Wave 346 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 49000 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_49000        1          graph_batch48991.c
  gj_libcgj_green_49000       1          graph_batch48992.c
  gj_bar3_ready_49000         0          graph_batch48993.c  (bar3 open)
  gj_product_score_49000      0          graph_batch48994.c  (score open)
  gj_continuum_ready_49000    1          graph_batch48995.c
  gj_smoke_soft_49000         1          graph_batch48996.c
  gj_dyn_soft_49000           1          graph_batch48997.c
  gj_milestone_tag_49000      49000      graph_batch48998.c
  gj_continuum_wave_49000     49000      graph_batch48999.c
  gj_batch_id_49000           49000      graph_batch49000.c
  gj_graph_milestone_49000    49000      graph_batch49000.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=49000`` means parent wired CGJ_SRCS through
    ``graph_batch49000.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_49000`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_49000`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M49000_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_48900`` / ``gj_*_48800`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  (48991–49000) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_49000.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_49000.py

  # Restrict to marker band only (soft-gate TUs; Wave 346 exclusive):
  python3 user/libcgj/src/_gen_milestone_49000.py --band markers

  # Extension identity only (48901–48950):
  python3 user/libcgj/src/_gen_milestone_49000.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_49000.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_48900.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 49000 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 49000
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 48901
WAVE_HI = 49000  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 48901, 48950
IDENTITY_LO, IDENTITY_HI = 48951, 48975
FILL_LO, FILL_HI = 48976, 48990
MARKERS_LO, MARKERS_HI = 48991, 49000
...
# Stems become gj_<stem>_49000. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (48991, "shell_green", 1, "shell product green extract (wave 49000)"),
    (48992, "libcgj_green", 1, "libcgj product green extract (wave 49000)"),
    (48993, "bar3_ready", 0, "product bar3 readiness gate (wave 49000)"),
    (48994, "product_score", 0, "product readiness score tag (wave 49000)"),
    (48995, "continuum_ready", 1, "continuum readiness gate (wave 49000)"),
    (48996, "smoke_soft", 1, "smoke soft lamp (wave 49000)"),
    (48997, "dyn_soft", 1, "dyn soft lamp (wave 49000)"),
    (48998, "milestone_tag", MILESTONE, "milestone 49000 product tag"),
    (48999, "continuum_wave", MILESTONE, "milestone 49000 continuum wave identity"),
    # 49000 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        48976: "continuum wave fill soft unit + batch id",
        48980: "continuum bridge soft unit + batch id",
        48990: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (48991–48999), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 48993
    tag_batch = MARKERS_LO + 7  # 48998
    wave_batch = MARKERS_LO + 8  # 48999
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_48900 / gj_shell_green_48800 / gj_shell_green_48700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_48900 / gj_libcgj_green_48800 / gj_libcgj_green_48700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_48900 / gj_bar3_ready_48800 / gj_bar3_ready_48700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_48900 / gj_product_score_48800 / gj_product_score_48700\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_48900 / gj_continuum_ready_48800 /\n"
            f" * gj_continuum_ready_48700 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_48900 / gj_smoke_soft_48800 / gj_smoke_soft_48700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_48900 / gj_dyn_soft_48800 / gj_dyn_soft_48700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_48900 / gj_milestone_tag_48800 / gj_milestone_tag_48700,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_48900 / gj_continuum_wave_48800 /\n"
            f" * gj_continuum_wave_48700, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch49000.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 48999
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 48991–48999; 49000 is capstone
