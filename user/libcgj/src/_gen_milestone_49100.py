...
"""
Soft continuum generator — MILESTONE 49100 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 49001–49100 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=49100   when parent has wired through graph_batch49100.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 49001 49100
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 49001 49100
...
graph batches ending at M. For M = **49100** the exclusive range is
**49001–49100** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 49001–49050  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 49051–49075  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 49076–49090  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 49091–49100  │ decade soft gates (Wave 346 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 49100 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_49100        1          graph_batch49091.c
  gj_libcgj_green_49100       1          graph_batch49092.c
  gj_bar3_ready_49100         0          graph_batch49093.c  (bar3 open)
  gj_product_score_49100      0          graph_batch49094.c  (score open)
  gj_continuum_ready_49100    1          graph_batch49095.c
  gj_smoke_soft_49100         1          graph_batch49096.c
  gj_dyn_soft_49100           1          graph_batch49097.c
  gj_milestone_tag_49100      49100      graph_batch49098.c
  gj_continuum_wave_49100     49100      graph_batch49099.c
  gj_batch_id_49100           49100      graph_batch49100.c
  gj_graph_milestone_49100    49100      graph_batch49100.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=49100`` means parent wired CGJ_SRCS through
    ``graph_batch49100.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_49100`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_49100`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M49100_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_49000`` / ``gj_*_48900`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  (49091–49100) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_49100.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_49100.py

  # Restrict to marker band only (soft-gate TUs; Wave 346 exclusive):
  python3 user/libcgj/src/_gen_milestone_49100.py --band markers

  # Extension identity only (49001–49050):
  python3 user/libcgj/src/_gen_milestone_49100.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_49100.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_49000.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 49100 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 49100
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 49001
WAVE_HI = 49100  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 49001, 49050
IDENTITY_LO, IDENTITY_HI = 49051, 49075
FILL_LO, FILL_HI = 49076, 49090
MARKERS_LO, MARKERS_HI = 49091, 49100
...
# Stems become gj_<stem>_49100. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (49091, "shell_green", 1, "shell product green extract (wave 49100)"),
    (49092, "libcgj_green", 1, "libcgj product green extract (wave 49100)"),
    (49093, "bar3_ready", 0, "product bar3 readiness gate (wave 49100)"),
    (49094, "product_score", 0, "product readiness score tag (wave 49100)"),
    (49095, "continuum_ready", 1, "continuum readiness gate (wave 49100)"),
    (49096, "smoke_soft", 1, "smoke soft lamp (wave 49100)"),
    (49097, "dyn_soft", 1, "dyn soft lamp (wave 49100)"),
    (49098, "milestone_tag", MILESTONE, "milestone 49100 product tag"),
    (49099, "continuum_wave", MILESTONE, "milestone 49100 continuum wave identity"),
    # 49100 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        49076: "continuum wave fill soft unit + batch id",
        49080: "continuum bridge soft unit + batch id",
        49090: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (49091–49099), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 49093
    tag_batch = MARKERS_LO + 7  # 49098
    wave_batch = MARKERS_LO + 8  # 49099
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_49000 / gj_shell_green_48900 / gj_shell_green_48800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_49000 / gj_libcgj_green_48900 / gj_libcgj_green_48800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_49000 / gj_bar3_ready_48900 / gj_bar3_ready_48800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_49000 / gj_product_score_48900 / gj_product_score_48800\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_49000 / gj_continuum_ready_48900 /\n"
            f" * gj_continuum_ready_48800 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_49000 / gj_smoke_soft_48900 / gj_smoke_soft_48800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_49000 / gj_dyn_soft_48900 / gj_dyn_soft_48800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_49000 / gj_milestone_tag_48900 / gj_milestone_tag_48800,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_49000 / gj_continuum_wave_48900 /\n"
            f" * gj_continuum_wave_48800, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch49100.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 49099
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 49091–49099; 49100 is capstone
