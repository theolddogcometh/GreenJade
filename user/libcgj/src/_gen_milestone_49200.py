...
"""
Soft continuum generator — MILESTONE 49200 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 49101–49200 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=49200   when parent has wired through graph_batch49200.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 49101 49200
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 49101 49200
...
graph batches ending at M. For M = **49200** the exclusive range is
**49101–49200** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 49101–49150  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 49151–49175  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 49176–49190  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 49191–49200  │ decade soft gates (Wave 350 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 49200 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_49200        1          graph_batch49191.c
  gj_libcgj_green_49200       1          graph_batch49192.c
  gj_bar3_ready_49200         0          graph_batch49193.c  (bar3 open)
  gj_product_score_49200      0          graph_batch49194.c  (score open)
  gj_continuum_ready_49200    1          graph_batch49195.c
  gj_smoke_soft_49200         1          graph_batch49196.c
  gj_dyn_soft_49200           1          graph_batch49197.c
  gj_milestone_tag_49200      49200      graph_batch49198.c
  gj_continuum_wave_49200     49200      graph_batch49199.c
  gj_batch_id_49200           49200      graph_batch49200.c
  gj_graph_milestone_49200    49200      graph_batch49200.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=49200`` means parent wired CGJ_SRCS through
    ``graph_batch49200.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_49200`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_49200`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M49200_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_49100`` / ``gj_*_49000`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  (49191–49200) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_49200.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_49200.py

  # Restrict to marker band only (soft-gate TUs; Wave 350 exclusive):
  python3 user/libcgj/src/_gen_milestone_49200.py --band markers

  # Extension identity only (49101–49150):
  python3 user/libcgj/src/_gen_milestone_49200.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_49200.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_49100.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 49200 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 49200
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 49101
WAVE_HI = 49200  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 49101, 49150
IDENTITY_LO, IDENTITY_HI = 49151, 49175
FILL_LO, FILL_HI = 49176, 49190
MARKERS_LO, MARKERS_HI = 49191, 49200
...
# Stems become gj_<stem>_49200. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (49191, "shell_green", 1, "shell product green extract (wave 49200)"),
    (49192, "libcgj_green", 1, "libcgj product green extract (wave 49200)"),
    (49193, "bar3_ready", 0, "product bar3 readiness gate (wave 49200)"),
    (49194, "product_score", 0, "product readiness score tag (wave 49200)"),
    (49195, "continuum_ready", 1, "continuum readiness gate (wave 49200)"),
    (49196, "smoke_soft", 1, "smoke soft lamp (wave 49200)"),
    (49197, "dyn_soft", 1, "dyn soft lamp (wave 49200)"),
    (49198, "milestone_tag", MILESTONE, "milestone 49200 product tag"),
    (49199, "continuum_wave", MILESTONE, "milestone 49200 continuum wave identity"),
    # 49200 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        49176: "continuum wave fill soft unit + batch id",
        49180: "continuum bridge soft unit + batch id",
        49190: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (49191–49199), rich Wave-16 style."""
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_49100 / gj_shell_green_49000 / gj_shell_green_48900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_49100 / gj_libcgj_green_49000 / gj_libcgj_green_48900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_49100 / gj_bar3_ready_49000 / gj_bar3_ready_48900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_49100 / gj_product_score_49000 / gj_product_score_48900\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_49100 / gj_continuum_ready_49000 /\n"
            f" * gj_continuum_ready_48900 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_49100 / gj_smoke_soft_49000 / gj_smoke_soft_48900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_49100 / gj_dyn_soft_49000 / gj_dyn_soft_48900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_49100 / gj_milestone_tag_49000 / gj_milestone_tag_48900,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_49100 / gj_continuum_wave_49000 /\n"
            f" * gj_continuum_wave_48900, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch49200.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 49191–49199; 49200 is capstone
