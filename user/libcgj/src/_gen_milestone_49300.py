...
"""
Soft continuum generator — MILESTONE 49300 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 49201–49300 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=49300   when parent has wired through graph_batch49300.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 49201 49300
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 49201 49300
...
graph batches ending at M. For M = **49300** the exclusive range is
**49201–49300** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 49201–49250  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 49251–49275  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 49276–49290  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 49291–49300  │ decade soft gates (Wave 350 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 49300 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_49300        1          graph_batch49291.c
  gj_libcgj_green_49300       1          graph_batch49292.c
  gj_bar3_ready_49300         0          graph_batch49293.c  (bar3 open)
  gj_product_score_49300      0          graph_batch49294.c  (score open)
  gj_continuum_ready_49300    1          graph_batch49295.c
  gj_smoke_soft_49300         1          graph_batch49296.c
  gj_dyn_soft_49300           1          graph_batch49297.c
  gj_milestone_tag_49300      49300      graph_batch49298.c
  gj_continuum_wave_49300     49300      graph_batch49299.c
  gj_batch_id_49300           49300      graph_batch49300.c
  gj_graph_milestone_49300    49300      graph_batch49300.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=49300`` means parent wired CGJ_SRCS through
    ``graph_batch49300.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_49300`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_49300`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M49300_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_49200`` / ``gj_*_49100`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 350 exclusive CREATE-ONLY lands the **markers** band
  (49291–49300) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_49300.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_49300.py

  # Restrict to marker band only (soft-gate TUs; Wave 350 exclusive):
  python3 user/libcgj/src/_gen_milestone_49300.py --band markers

  # Extension identity only (49201–49250):
  python3 user/libcgj/src/_gen_milestone_49300.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_49300.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_49300.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 49300 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 49300
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 49201
WAVE_HI = 49300  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 49201, 49250
IDENTITY_LO, IDENTITY_HI = 49251, 49275
FILL_LO, FILL_HI = 49276, 49290
MARKERS_LO, MARKERS_HI = 49291, 49300
...
# Stems become gj_<stem>_49300. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (49291, "shell_green", 1, "shell product green extract (wave 49300)"),
    (49292, "libcgj_green", 1, "libcgj product green extract (wave 49300)"),
    (49293, "bar3_ready", 0, "product bar3 readiness gate (wave 49300)"),
    (49294, "product_score", 0, "product readiness score tag (wave 49300)"),
    (49295, "continuum_ready", 1, "continuum readiness gate (wave 49300)"),
    (49296, "smoke_soft", 1, "smoke soft lamp (wave 49300)"),
    (49297, "dyn_soft", 1, "dyn soft lamp (wave 49300)"),
    (49298, "milestone_tag", MILESTONE, "milestone 49300 product tag"),
    (49299, "continuum_wave", MILESTONE, "milestone 49300 continuum wave identity"),
    # 49300 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        49276: "continuum wave fill soft unit + batch id",
        49280: "continuum bridge soft unit + batch id",
        49290: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (49291–49299), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 49193
    tag_batch = MARKERS_LO + 7  # 49198
    wave_batch = MARKERS_LO + 8  # 49199
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_49200 / gj_shell_green_49100 / gj_shell_green_49000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_49200 / gj_libcgj_green_49100 / gj_libcgj_green_49000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_49200 / gj_bar3_ready_49100 / gj_bar3_ready_49000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_49200 / gj_product_score_49100 / gj_product_score_49000\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_49200 / gj_continuum_ready_49100 /\n"
            f" * gj_continuum_ready_49000 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_49200 / gj_smoke_soft_49100 / gj_smoke_soft_49000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_49200 / gj_dyn_soft_49100 / gj_dyn_soft_49000 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_49200 / gj_milestone_tag_49100 / gj_milestone_tag_49000,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_49200 / gj_continuum_wave_49100 /\n"
            f" * gj_continuum_wave_49000, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch49300.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 49199
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 49291–49299; 49300 is capstone
