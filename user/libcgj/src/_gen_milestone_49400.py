...
"""
Soft continuum generator — MILESTONE 49400 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 49301–49400 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=49400   when parent has wired through graph_batch49400.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 49301 49400
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 49301 49400
...
graph batches ending at M. For M = **49400** the exclusive range is
**49301–49400** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 49301–49350  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 49351–49375  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 49376–49390  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 49391–49400  │ decade soft gates (Wave 352 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 49400 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_49400        1          graph_batch49391.c
  gj_libcgj_green_49400       1          graph_batch49392.c
  gj_bar3_ready_49400         0          graph_batch49393.c  (bar3 open)
  gj_product_score_49400      0          graph_batch49394.c  (score open)
  gj_continuum_ready_49400    1          graph_batch49395.c
  gj_smoke_soft_49400         1          graph_batch49396.c
  gj_dyn_soft_49400           1          graph_batch49397.c
  gj_milestone_tag_49400      49400      graph_batch49398.c
  gj_continuum_wave_49400     49400      graph_batch49399.c
  gj_batch_id_49400           49400      graph_batch49400.c
  gj_graph_milestone_49400    49400      graph_batch49400.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=49400`` means parent wired CGJ_SRCS through
    ``graph_batch49400.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_49400`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_49400`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M49400_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_49300`` / ``gj_*_49200`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 352 exclusive CREATE-ONLY lands the **markers** band
  (49391–49400) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_49400.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_49400.py

  # Restrict to marker band only (soft-gate TUs; Wave 352 exclusive):
  python3 user/libcgj/src/_gen_milestone_49400.py --band markers

  # Extension identity only (49301–49350):
  python3 user/libcgj/src/_gen_milestone_49400.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_49400.py --force
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 49400 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 49400
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 49301
WAVE_HI = 49400  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 49301, 49350
IDENTITY_LO, IDENTITY_HI = 49351, 49375
FILL_LO, FILL_HI = 49376, 49390
MARKERS_LO, MARKERS_HI = 49391, 49400
...
# Stems become gj_<stem>_49400. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (49391, "shell_green", 1, "shell product green extract (wave 49400)"),
    (49392, "libcgj_green", 1, "libcgj product green extract (wave 49400)"),
    (49393, "bar3_ready", 0, "product bar3 readiness gate (wave 49400)"),
    (49394, "product_score", 0, "product readiness score tag (wave 49400)"),
    (49395, "continuum_ready", 1, "continuum readiness gate (wave 49400)"),
    (49396, "smoke_soft", 1, "smoke soft lamp (wave 49400)"),
    (49397, "dyn_soft", 1, "dyn soft lamp (wave 49400)"),
    (49398, "milestone_tag", MILESTONE, "milestone 49400 product tag"),
    (49399, "continuum_wave", MILESTONE, "milestone 49400 continuum wave identity"),
    # 49400 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        49376: "continuum wave fill soft unit + batch id",
        49380: "continuum bridge soft unit + batch id",
        49390: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (49391–49399), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 49393
    tag_batch = MARKERS_LO + 7  # 49398
    wave_batch = MARKERS_LO + 8  # 49399
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_49300 / gj_shell_green_49200 / gj_shell_green_49100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_49300 / gj_libcgj_green_49200 / gj_libcgj_green_49100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_49300 / gj_bar3_ready_49200 / gj_bar3_ready_49100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_49300 / gj_product_score_49200 / gj_product_score_49100\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_49300 / gj_continuum_ready_49200 /\n"
            f" * gj_continuum_ready_49100 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_49300 / gj_smoke_soft_49200 / gj_smoke_soft_49100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_49300 / gj_dyn_soft_49200 / gj_dyn_soft_49100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_49300 / gj_milestone_tag_49200 / gj_milestone_tag_49100,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_49300 / gj_continuum_wave_49200 /\n"
            f" * gj_continuum_wave_49100, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch49400.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 49399
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 49391–49399; 49400 is capstone
