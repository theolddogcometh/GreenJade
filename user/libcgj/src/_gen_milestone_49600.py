...
"""
Soft continuum generator — MILESTONE 49600 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 49501–49600 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=49600   when parent has wired through graph_batch49600.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 49501 49600
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 49501 49600
...
graph batches ending at M. For M = **49600** the exclusive range is
**49501–49600** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 49501–49550  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 49551–49575  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 49576–49590  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 49591–49600  │ decade soft gates (Wave 354 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 49600 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_49600        1          graph_batch49591.c
  gj_libcgj_green_49600       1          graph_batch49592.c
  gj_bar3_ready_49600         0          graph_batch49593.c  (bar3 open)
  gj_product_score_49600      0          graph_batch49594.c  (score open)
  gj_continuum_ready_49600    1          graph_batch49595.c
  gj_smoke_soft_49600         1          graph_batch49596.c
  gj_dyn_soft_49600           1          graph_batch49597.c
  gj_milestone_tag_49600      49600      graph_batch49598.c
  gj_continuum_wave_49600     49600      graph_batch49599.c
  gj_batch_id_49600           49600      graph_batch49600.c
  gj_graph_milestone_49600    49600      graph_batch49600.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=49600`` means parent wired CGJ_SRCS through
    ``graph_batch49600.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_49600`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_49600`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M49600_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_49500`` / ``gj_*_49400`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 354 exclusive CREATE-ONLY lands the **markers** band
  (49591–49600) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_49600.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_49600.py

  # Restrict to marker band only (soft-gate TUs; Wave 354 exclusive):
  python3 user/libcgj/src/_gen_milestone_49600.py --band markers

  # Extension identity only (49501–49550):
  python3 user/libcgj/src/_gen_milestone_49600.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_49600.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_49500.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 49600 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 49600
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 49501
WAVE_HI = 49600  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 49501, 49550
IDENTITY_LO, IDENTITY_HI = 49551, 49575
FILL_LO, FILL_HI = 49576, 49590
MARKERS_LO, MARKERS_HI = 49591, 49600
...
# Stems become gj_<stem>_49600. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (49591, "shell_green", 1, "shell product green extract (wave 49600)"),
    (49592, "libcgj_green", 1, "libcgj product green extract (wave 49600)"),
    (49593, "bar3_ready", 0, "product bar3 readiness gate (wave 49600)"),
    (49594, "product_score", 0, "product readiness score tag (wave 49600)"),
    (49595, "continuum_ready", 1, "continuum readiness gate (wave 49600)"),
    (49596, "smoke_soft", 1, "smoke soft lamp (wave 49600)"),
    (49597, "dyn_soft", 1, "dyn soft lamp (wave 49600)"),
    (49598, "milestone_tag", MILESTONE, "milestone 49600 product tag"),
    (49599, "continuum_wave", MILESTONE, "milestone 49600 continuum wave identity"),
    # 49600 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        49576: "continuum wave fill soft unit + batch id",
        49580: "continuum bridge soft unit + batch id",
        49590: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (49591–49599), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 49593
    tag_batch = MARKERS_LO + 7  # 49598
    wave_batch = MARKERS_LO + 8  # 49599
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_49500 / gj_shell_green_49400 / gj_shell_green_49300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_49500 / gj_libcgj_green_49400 / gj_libcgj_green_49300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_49500 / gj_bar3_ready_49400 / gj_bar3_ready_49300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_49500 / gj_product_score_49400 / gj_product_score_49300\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_49500 / gj_continuum_ready_49400 /\n"
            f" * gj_continuum_ready_49300 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_49500 / gj_smoke_soft_49400 / gj_smoke_soft_49300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_49500 / gj_dyn_soft_49400 / gj_dyn_soft_49300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_49500 / gj_milestone_tag_49400 / gj_milestone_tag_49300,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_49500 / gj_continuum_wave_49400 /\n"
            f" * gj_continuum_wave_49300, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch49600.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 49599
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 49591–49599; 49600 is capstone
