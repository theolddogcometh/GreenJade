...
"""
Soft continuum generator — MILESTONE 49700 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 49601–49700 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=49700   when parent has wired through graph_batch49700.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 49601 49700
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 49601 49700
...
graph batches ending at M. For M = **49700** the exclusive range is
**49601–49700** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 49601–49650  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 49651–49675  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 49676–49690  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 49691–49700  │ decade soft gates (Wave 355 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 49700 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_49700        1          graph_batch49691.c
  gj_libcgj_green_49700       1          graph_batch49692.c
  gj_bar3_ready_49700         0          graph_batch49693.c  (bar3 open)
  gj_product_score_49700      0          graph_batch49694.c  (score open)
  gj_continuum_ready_49700    1          graph_batch49695.c
  gj_smoke_soft_49700         1          graph_batch49696.c
  gj_dyn_soft_49700           1          graph_batch49697.c
  gj_milestone_tag_49700      49700      graph_batch49698.c
  gj_continuum_wave_49700     49700      graph_batch49699.c
  gj_batch_id_49700           49700      graph_batch49700.c
  gj_graph_milestone_49700    49700      graph_batch49700.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=49700`` means parent wired CGJ_SRCS through
    ``graph_batch49700.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_49700`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_49700`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M49700_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_49600`` / ``gj_*_49500`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 355 exclusive CREATE-ONLY lands the **markers** band
  (49691–49700) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_49700.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_49700.py

  # Restrict to marker band only (soft-gate TUs; Wave 355 exclusive):
  python3 user/libcgj/src/_gen_milestone_49700.py --band markers

  # Extension identity only (49601–49650):
  python3 user/libcgj/src/_gen_milestone_49700.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_49700.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_49600.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 49700 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 49700
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 49601
WAVE_HI = 49700  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 49601, 49650
IDENTITY_LO, IDENTITY_HI = 49651, 49675
FILL_LO, FILL_HI = 49676, 49690
MARKERS_LO, MARKERS_HI = 49691, 49700
...
# Stems become gj_<stem>_49700. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (49691, "shell_green", 1, "shell product green extract (wave 49700)"),
    (49692, "libcgj_green", 1, "libcgj product green extract (wave 49700)"),
    (49693, "bar3_ready", 0, "product bar3 readiness gate (wave 49700)"),
    (49694, "product_score", 0, "product readiness score tag (wave 49700)"),
    (49695, "continuum_ready", 1, "continuum readiness gate (wave 49700)"),
    (49696, "smoke_soft", 1, "smoke soft lamp (wave 49700)"),
    (49697, "dyn_soft", 1, "dyn soft lamp (wave 49700)"),
    (49698, "milestone_tag", MILESTONE, "milestone 49700 product tag"),
    (49699, "continuum_wave", MILESTONE, "milestone 49700 continuum wave identity"),
    # 49700 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        49676: "continuum wave fill soft unit + batch id",
        49680: "continuum bridge soft unit + batch id",
        49690: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (49691–49699), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 49693
    tag_batch = MARKERS_LO + 7  # 49698
    wave_batch = MARKERS_LO + 8  # 49699
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_49600 / gj_shell_green_49500 / gj_shell_green_49400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_49600 / gj_libcgj_green_49500 / gj_libcgj_green_49400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_49600 / gj_bar3_ready_49500 / gj_bar3_ready_49400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_49600 / gj_product_score_49500 / gj_product_score_49400\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_49600 / gj_continuum_ready_49500 /\n"
            f" * gj_continuum_ready_49400 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_49600 / gj_smoke_soft_49500 / gj_smoke_soft_49400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_49600 / gj_dyn_soft_49500 / gj_dyn_soft_49400 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_49600 / gj_milestone_tag_49500 / gj_milestone_tag_49400,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_49600 / gj_continuum_wave_49500 /\n"
            f" * gj_continuum_wave_49400, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch49700.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 49699
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 49691–49699; 49700 is capstone
