...
"""
Soft continuum generator — MILESTONE 49900 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 49801–49900 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=49900   when parent has wired through graph_batch49900.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 49801 49900
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 49801 49900
...
graph batches ending at M. For M = **49900** the exclusive range is
**49801–49900** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 49801–49850  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 49851–49875  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 49876–49890  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 49891–49900  │ decade soft gates (Wave 357 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 49900 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_49900        1          graph_batch49891.c
  gj_libcgj_green_49900       1          graph_batch49892.c
  gj_bar3_ready_49900         0          graph_batch49893.c  (bar3 open)
  gj_product_score_49900      0          graph_batch49894.c  (score open)
  gj_continuum_ready_49900    1          graph_batch49895.c
  gj_smoke_soft_49900         1          graph_batch49896.c
  gj_dyn_soft_49900           1          graph_batch49897.c
  gj_milestone_tag_49900      49900      graph_batch49898.c
  gj_continuum_wave_49900     49900      graph_batch49899.c
  gj_batch_id_49900           49900      graph_batch49900.c
  gj_graph_milestone_49900    49900      graph_batch49900.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=49900`` means parent wired CGJ_SRCS through
    ``graph_batch49900.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_49900`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_49900`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M49900_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_49800`` / ``gj_*_49700`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 357 exclusive CREATE-ONLY lands the **markers** band
  (49891–49900) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_49900.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_49900.py

  # Restrict to marker band only (soft-gate TUs; Wave 357 exclusive):
  python3 user/libcgj/src/_gen_milestone_49900.py --band markers

  # Extension identity only (49801–49850):
  python3 user/libcgj/src/_gen_milestone_49900.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_49900.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_49800.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 49900 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 49900
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 49801
WAVE_HI = 49900  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 49801, 49850
IDENTITY_LO, IDENTITY_HI = 49851, 49875
FILL_LO, FILL_HI = 49876, 49890
MARKERS_LO, MARKERS_HI = 49891, 49900
...
# Stems become gj_<stem>_49900. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (49891, "shell_green", 1, "shell product green extract (wave 49900)"),
    (49892, "libcgj_green", 1, "libcgj product green extract (wave 49900)"),
    (49893, "bar3_ready", 0, "product bar3 readiness gate (wave 49900)"),
    (49894, "product_score", 0, "product readiness score tag (wave 49900)"),
    (49895, "continuum_ready", 1, "continuum readiness gate (wave 49900)"),
    (49896, "smoke_soft", 1, "smoke soft lamp (wave 49900)"),
    (49897, "dyn_soft", 1, "dyn soft lamp (wave 49900)"),
    (49898, "milestone_tag", MILESTONE, "milestone 49900 product tag"),
    (49899, "continuum_wave", MILESTONE, "milestone 49900 continuum wave identity"),
    # 49900 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        49876: "continuum wave fill soft unit + batch id",
        49880: "continuum bridge soft unit + batch id",
        49890: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (49891–49899), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 49893
    tag_batch = MARKERS_LO + 7  # 49898
    wave_batch = MARKERS_LO + 8  # 49899
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_49800 / gj_shell_green_49700 / gj_shell_green_49600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_49800 / gj_libcgj_green_49700 / gj_libcgj_green_49600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_49800 / gj_bar3_ready_49700 / gj_bar3_ready_49600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_49800 / gj_product_score_49700 / gj_product_score_49600\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_49800 / gj_continuum_ready_49700 /\n"
            f" * gj_continuum_ready_49600 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_49800 / gj_smoke_soft_49700 / gj_smoke_soft_49600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_49800 / gj_dyn_soft_49700 / gj_dyn_soft_49600 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_49800 / gj_milestone_tag_49700 / gj_milestone_tag_49600,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_49800 / gj_continuum_wave_49700 /\n"
            f" * gj_continuum_wave_49600, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch49900.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 49899
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 49891–49899; 49900 is capstone
