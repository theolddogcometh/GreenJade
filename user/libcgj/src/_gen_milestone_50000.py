...
"""
Soft continuum generator — MILESTONE 50000 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 49901–50000 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=50000   when parent has wired through graph_batch50000.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 49901 50000
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 49901 50000
...
graph batches ending at M. For M = **50000** the exclusive range is
**49901–50000** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 49901–49950  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 49951–49975  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 49976–49990  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 49991–50000  │ decade soft gates (Wave 358 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 50000 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_50000        1          graph_batch49991.c
  gj_libcgj_green_50000       1          graph_batch49992.c
  gj_bar3_ready_50000         0          graph_batch49993.c  (bar3 open)
  gj_product_score_50000      0          graph_batch49994.c  (score open)
  gj_continuum_ready_50000    1          graph_batch49995.c
  gj_smoke_soft_50000         1          graph_batch49996.c
  gj_dyn_soft_50000           1          graph_batch49997.c
  gj_milestone_tag_50000      50000      graph_batch49998.c
  gj_continuum_wave_50000     50000      graph_batch49999.c
  gj_batch_id_50000           50000      graph_batch50000.c
  gj_graph_milestone_50000    50000      graph_batch50000.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=50000`` means parent wired CGJ_SRCS through
    ``graph_batch50000.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_50000`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_50000`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M50000_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_49900`` / ``gj_*_49800`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  (49991–50000) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_50000.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_50000.py

  # Restrict to marker band only (soft-gate TUs; Wave 358 exclusive):
  python3 user/libcgj/src/_gen_milestone_50000.py --band markers

  # Extension identity only (49901–49950):
  python3 user/libcgj/src/_gen_milestone_50000.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_50000.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_49900.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 50000 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 50000
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 49901
WAVE_HI = 50000  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 49901, 49950
IDENTITY_LO, IDENTITY_HI = 49951, 49975
FILL_LO, FILL_HI = 49976, 49990
MARKERS_LO, MARKERS_HI = 49991, 50000
...
# Stems become gj_<stem>_50000. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (49991, "shell_green", 1, "shell product green extract (wave 50000)"),
    (49992, "libcgj_green", 1, "libcgj product green extract (wave 50000)"),
    (49993, "bar3_ready", 0, "product bar3 readiness gate (wave 50000)"),
    (49994, "product_score", 0, "product readiness score tag (wave 50000)"),
    (49995, "continuum_ready", 1, "continuum readiness gate (wave 50000)"),
    (49996, "smoke_soft", 1, "smoke soft lamp (wave 50000)"),
    (49997, "dyn_soft", 1, "dyn soft lamp (wave 50000)"),
    (49998, "milestone_tag", MILESTONE, "milestone 50000 product tag"),
    (49999, "continuum_wave", MILESTONE, "milestone 50000 continuum wave identity"),
    # 50000 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        49976: "continuum wave fill soft unit + batch id",
        49980: "continuum bridge soft unit + batch id",
        49990: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (49991–49999), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 49993
    tag_batch = MARKERS_LO + 7  # 49998
    wave_batch = MARKERS_LO + 8  # 49999
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_49900 / gj_shell_green_49800 / gj_shell_green_49700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_49900 / gj_libcgj_green_49800 / gj_libcgj_green_49700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_49900 / gj_bar3_ready_49800 / gj_bar3_ready_49700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_49900 / gj_product_score_49800 / gj_product_score_49700\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_49900 / gj_smoke_soft_49800 / gj_smoke_soft_49700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_49900 / gj_dyn_soft_49800 / gj_dyn_soft_49700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_49900 / gj_milestone_tag_49800 / gj_milestone_tag_49700,\n"
...
def render_milestone_capstone() -> str:
    """graph_batch50000.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 49999
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 49991–49999; 50000 is capstone
