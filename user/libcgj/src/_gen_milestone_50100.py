...
"""
Soft continuum generator — MILESTONE 50100 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 50001–50100 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=50100   when parent has wired through graph_batch50100.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 50001 50100
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 50001 50100
...
graph batches ending at M. For M = **50100** the exclusive range is
**50001–50100** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 50001–50050  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 50051–50075  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 50076–50090  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 50091–50100  │ decade soft gates (Wave 359 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 50100 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_50100        1          graph_batch50091.c
  gj_libcgj_green_50100       1          graph_batch50092.c
  gj_bar3_ready_50100         0          graph_batch50093.c  (bar3 open)
  gj_product_score_50100      0          graph_batch50094.c  (score open)
  gj_continuum_ready_50100    1          graph_batch50095.c
  gj_smoke_soft_50100         1          graph_batch50096.c
  gj_dyn_soft_50100           1          graph_batch50097.c
  gj_milestone_tag_50100      50100      graph_batch50098.c
  gj_continuum_wave_50100     50100      graph_batch50099.c
  gj_batch_id_50100           50100      graph_batch50100.c
  gj_graph_milestone_50100    50100      graph_batch50100.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=50100`` means parent wired CGJ_SRCS through
    ``graph_batch50100.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_50100`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_50100`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M50100_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_50000`` / ``gj_*_49900`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 359 exclusive CREATE-ONLY lands the **markers** band
  (50091–50100) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_50100.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_50100.py

  # Restrict to marker band only (soft-gate TUs; Wave 359 exclusive):
  python3 user/libcgj/src/_gen_milestone_50100.py --band markers

  # Extension identity only (50001–50050):
  python3 user/libcgj/src/_gen_milestone_50100.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_50100.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_50000.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 50100 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 50100
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 50001
WAVE_HI = 50100  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 50001, 50050
IDENTITY_LO, IDENTITY_HI = 50051, 50075
FILL_LO, FILL_HI = 50076, 50090
MARKERS_LO, MARKERS_HI = 50091, 50100
...
# Stems become gj_<stem>_50100. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (50091, "shell_green", 1, "shell product green extract (wave 50100)"),
    (50092, "libcgj_green", 1, "libcgj product green extract (wave 50100)"),
    (50093, "bar3_ready", 0, "product bar3 readiness gate (wave 50100)"),
    (50094, "product_score", 0, "product readiness score tag (wave 50100)"),
    (50095, "continuum_ready", 1, "continuum readiness gate (wave 50100)"),
    (50096, "smoke_soft", 1, "smoke soft lamp (wave 50100)"),
    (50097, "dyn_soft", 1, "dyn soft lamp (wave 50100)"),
    (50098, "milestone_tag", MILESTONE, "milestone 50100 product tag"),
    (50099, "continuum_wave", MILESTONE, "milestone 50100 continuum wave identity"),
    # 50100 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        50076: "continuum wave fill soft unit + batch id",
        50080: "continuum bridge soft unit + batch id",
        50090: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (50091–50099), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 50093
    tag_batch = MARKERS_LO + 7  # 50098
    wave_batch = MARKERS_LO + 8  # 50099
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_50000 / gj_shell_green_49900 / gj_shell_green_49800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_50000 / gj_libcgj_green_49900 / gj_libcgj_green_49800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_50000 / gj_bar3_ready_49900 / gj_bar3_ready_49800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_50000 / gj_product_score_49900 / gj_product_score_49800\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_50000 / gj_continuum_ready_49900 /\n"
            f" * gj_continuum_ready_49800 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_50000 / gj_smoke_soft_49900 / gj_smoke_soft_49800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_50000 / gj_dyn_soft_49900 / gj_dyn_soft_49800 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_50000 / gj_milestone_tag_49900 / gj_milestone_tag_49800,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_50000 / gj_continuum_wave_49900 /\n"
            f" * gj_continuum_wave_49800, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch50100.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 50099
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 50091–50099; 50100 is capstone
