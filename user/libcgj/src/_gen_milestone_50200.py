...
"""
Soft continuum generator — MILESTONE 50200 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 50101–50200 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=50200   when parent has wired through graph_batch50200.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 50101 50200
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 50101 50200
...
graph batches ending at M. For M = **50200** the exclusive range is
**50101–50200** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 50101–50150  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 50151–50175  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 50176–50190  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 50191–50200  │ decade soft gates (Wave 360 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 50200 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_50200        1          graph_batch50191.c
  gj_libcgj_green_50200       1          graph_batch50192.c
  gj_bar3_ready_50200         0          graph_batch50193.c  (bar3 open)
  gj_product_score_50200      0          graph_batch50194.c  (score open)
  gj_continuum_ready_50200    1          graph_batch50195.c
  gj_smoke_soft_50200         1          graph_batch50196.c
  gj_dyn_soft_50200           1          graph_batch50197.c
  gj_milestone_tag_50200      50200      graph_batch50198.c
  gj_continuum_wave_50200     50200      graph_batch50199.c
  gj_batch_id_50200           50200      graph_batch50200.c
  gj_graph_milestone_50200    50200      graph_batch50200.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=50200`` means parent wired CGJ_SRCS through
    ``graph_batch50200.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_50200`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_50200`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M50200_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_50100`` / ``gj_*_50200`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  (50191–50200) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_50200.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_50200.py

  # Restrict to marker band only (soft-gate TUs; Wave 360 exclusive):
  python3 user/libcgj/src/_gen_milestone_50200.py --band markers

  # Extension identity only (50101–50150):
  python3 user/libcgj/src/_gen_milestone_50200.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_50200.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_50100.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 50200 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 50200
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 50101
WAVE_HI = 50200  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 50101, 50150
IDENTITY_LO, IDENTITY_HI = 50151, 50175
FILL_LO, FILL_HI = 50176, 50190
MARKERS_LO, MARKERS_HI = 50191, 50200
...
# Stems become gj_<stem>_50200. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (50191, "shell_green", 1, "shell product green extract (wave 50200)"),
    (50192, "libcgj_green", 1, "libcgj product green extract (wave 50200)"),
    (50193, "bar3_ready", 0, "product bar3 readiness gate (wave 50200)"),
    (50194, "product_score", 0, "product readiness score tag (wave 50200)"),
    (50195, "continuum_ready", 1, "continuum readiness gate (wave 50200)"),
    (50196, "smoke_soft", 1, "smoke soft lamp (wave 50200)"),
    (50197, "dyn_soft", 1, "dyn soft lamp (wave 50200)"),
    (50198, "milestone_tag", MILESTONE, "milestone 50200 product tag"),
    (50199, "continuum_wave", MILESTONE, "milestone 50200 continuum wave identity"),
    # 50200 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        50176: "continuum wave fill soft unit + batch id",
        50180: "continuum bridge soft unit + batch id",
        50190: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (50191–50199), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 50193
    tag_batch = MARKERS_LO + 7  # 50198
    wave_batch = MARKERS_LO + 8  # 50199
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_50100 / gj_shell_green_50200 / gj_shell_green_49900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_50100 / gj_libcgj_green_50200 / gj_libcgj_green_49900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_50100 / gj_bar3_ready_50200 / gj_bar3_ready_49900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_50100 / gj_product_score_50200 / gj_product_score_49900\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_49900 / gj_continuum_ready_49800 /\n"
            f" * gj_continuum_ready_49700 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_50100 / gj_smoke_soft_50200 / gj_smoke_soft_49900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_50100 / gj_dyn_soft_50200 / gj_dyn_soft_49900 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_50100 / gj_milestone_tag_50200 / gj_milestone_tag_49900,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_49900 / gj_continuum_wave_49800 /\n"
            f" * gj_continuum_wave_49700, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch50200.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 50199
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 50191–50199; 50200 is capstone
