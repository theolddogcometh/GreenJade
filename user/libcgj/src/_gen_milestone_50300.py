...
"""
Soft continuum generator — MILESTONE 50300 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 50201–50300 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=50300   when parent has wired through graph_batch50300.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 50201 50300
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 50201 50300
...
graph batches ending at M. For M = **50300** the exclusive range is
**50201–50300** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 50201–50250  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 50251–50275  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 50276–50290  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 50291–50300  │ decade soft gates (Wave 361 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 50300 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_50300        1          graph_batch50291.c
  gj_libcgj_green_50300       1          graph_batch50292.c
  gj_bar3_ready_50300         0          graph_batch50293.c  (bar3 open)
  gj_product_score_50300      0          graph_batch50294.c  (score open)
  gj_continuum_ready_50300    1          graph_batch50295.c
  gj_smoke_soft_50300         1          graph_batch50296.c
  gj_dyn_soft_50300           1          graph_batch50297.c
  gj_milestone_tag_50300      50300      graph_batch50298.c
  gj_continuum_wave_50300     50300      graph_batch50299.c
  gj_batch_id_50300           50300      graph_batch50300.c
  gj_graph_milestone_50300    50300      graph_batch50300.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=50300`` means parent wired CGJ_SRCS through
    ``graph_batch50300.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_50300`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_50300`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M50300_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_50200`` / ``gj_*_50100`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 361 exclusive CREATE-ONLY lands the **markers** band
  (50291–50300) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_50300.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_50300.py

  # Restrict to marker band only (soft-gate TUs; Wave 361 exclusive):
  python3 user/libcgj/src/_gen_milestone_50300.py --band markers

  # Extension identity only (50201–50250):
  python3 user/libcgj/src/_gen_milestone_50300.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_50300.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_50200.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 50300 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 50300
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 50201
WAVE_HI = 50300  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 50201, 50250
IDENTITY_LO, IDENTITY_HI = 50251, 50275
FILL_LO, FILL_HI = 50276, 50290
MARKERS_LO, MARKERS_HI = 50291, 50300
...
# Stems become gj_<stem>_50300. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (50291, "shell_green", 1, "shell product green extract (wave 50300)"),
    (50292, "libcgj_green", 1, "libcgj product green extract (wave 50300)"),
    (50293, "bar3_ready", 0, "product bar3 readiness gate (wave 50300)"),
    (50294, "product_score", 0, "product readiness score tag (wave 50300)"),
    (50295, "continuum_ready", 1, "continuum readiness gate (wave 50300)"),
    (50296, "smoke_soft", 1, "smoke soft lamp (wave 50300)"),
    (50297, "dyn_soft", 1, "dyn soft lamp (wave 50300)"),
    (50298, "milestone_tag", MILESTONE, "milestone 50300 product tag"),
    (50299, "continuum_wave", MILESTONE, "milestone 50300 continuum wave identity"),
    # 50300 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        50276: "continuum wave fill soft unit + batch id",
        50280: "continuum bridge soft unit + batch id",
        50290: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (50291–50299), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 50293
    tag_batch = MARKERS_LO + 7  # 50298
    wave_batch = MARKERS_LO + 8  # 50299
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_50200 / gj_shell_green_50100 / gj_shell_green_50300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_50200 / gj_libcgj_green_50100 / gj_libcgj_green_50300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_50200 / gj_bar3_ready_50100 / gj_bar3_ready_50300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_50200 / gj_product_score_50100 / gj_product_score_50300\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_50200 / gj_continuum_ready_50100 /\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_50200 / gj_smoke_soft_50100 / gj_smoke_soft_50300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_50200 / gj_dyn_soft_50100 / gj_dyn_soft_50300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_50200 / gj_milestone_tag_50100 / gj_milestone_tag_50300,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_50200 / gj_continuum_wave_50100 /\n"
...
def render_milestone_capstone() -> str:
    """graph_batch50300.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 50299
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 50291–50299; 50300 is capstone
