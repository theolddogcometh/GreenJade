...
"""
Soft continuum generator — MILESTONE 50400 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 50301–50400 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=50400   when parent has wired through graph_batch50400.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 50301 50400
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 50301 50400
...
graph batches ending at M. For M = **50400** the exclusive range is
**50301–50400** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 50301–50350  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 50351–50375  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 50376–50390  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 50391–50400  │ decade soft gates (Wave 358 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 50400 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_50400        1          graph_batch50391.c
  gj_libcgj_green_50400       1          graph_batch50392.c
  gj_bar3_ready_50400         0          graph_batch50393.c  (bar3 open)
  gj_product_score_50400      0          graph_batch50394.c  (score open)
  gj_continuum_ready_50400    1          graph_batch50395.c
  gj_smoke_soft_50400         1          graph_batch50396.c
  gj_dyn_soft_50400           1          graph_batch50397.c
  gj_milestone_tag_50400      50400      graph_batch50398.c
  gj_continuum_wave_50400     50400      graph_batch50399.c
  gj_batch_id_50400           50400      graph_batch50400.c
  gj_graph_milestone_50400    50400      graph_batch50400.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=50400`` means parent wired CGJ_SRCS through
    ``graph_batch50400.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_50400`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_50400`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M50400_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_50300`` / ``gj_*_50200`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 358 exclusive CREATE-ONLY lands the **markers** band
  (50391–50400) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_50400.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_50400.py

  # Restrict to marker band only (soft-gate TUs; Wave 358 exclusive):
  python3 user/libcgj/src/_gen_milestone_50400.py --band markers

  # Extension identity only (50301–50350):
  python3 user/libcgj/src/_gen_milestone_50400.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_50400.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_50300.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 50400 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 50400
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 50301
WAVE_HI = 50400  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 50301, 50350
IDENTITY_LO, IDENTITY_HI = 50351, 50375
FILL_LO, FILL_HI = 50376, 50390
MARKERS_LO, MARKERS_HI = 50391, 50400
...
# Stems become gj_<stem>_50400. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (50391, "shell_green", 1, "shell product green extract (wave 50400)"),
    (50392, "libcgj_green", 1, "libcgj product green extract (wave 50400)"),
    (50393, "bar3_ready", 0, "product bar3 readiness gate (wave 50400)"),
    (50394, "product_score", 0, "product readiness score tag (wave 50400)"),
    (50395, "continuum_ready", 1, "continuum readiness gate (wave 50400)"),
    (50396, "smoke_soft", 1, "smoke soft lamp (wave 50400)"),
    (50397, "dyn_soft", 1, "dyn soft lamp (wave 50400)"),
    (50398, "milestone_tag", MILESTONE, "milestone 50400 product tag"),
    (50399, "continuum_wave", MILESTONE, "milestone 50400 continuum wave identity"),
    # 50400 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        50376: "continuum wave fill soft unit + batch id",
        50380: "continuum bridge soft unit + batch id",
        50390: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (50391–50399), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 50393
    tag_batch = MARKERS_LO + 7  # 50398
    wave_batch = MARKERS_LO + 8  # 50399
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_50300 / gj_shell_green_50200 / gj_shell_green_50100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_50300 / gj_libcgj_green_50200 / gj_libcgj_green_50100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_50300 / gj_bar3_ready_50200 / gj_bar3_ready_50100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_50300 / gj_product_score_50200 / gj_product_score_50100\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_50300 / gj_continuum_ready_50200 /\n"
            f" * gj_continuum_ready_50100 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_50300 / gj_smoke_soft_50200 / gj_smoke_soft_50100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_50300 / gj_dyn_soft_50200 / gj_dyn_soft_50100 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_50300 / gj_milestone_tag_50200 / gj_milestone_tag_50100,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_50300 / gj_continuum_wave_50200 /\n"
            f" * gj_continuum_wave_50100, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch50400.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 50399
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 50391–50399; 50400 is capstone
