...
"""
Soft continuum generator — MILESTONE 50500 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 50401–50500 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=50500   when parent has wired through graph_batch50500.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 50401 50500
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 50401 50500
...
graph batches ending at M. For M = **50500** the exclusive range is
**50401–50500** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 50401–50450  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 50451–50475  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 50476–50490  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 50491–50500  │ decade soft gates (Wave 362 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 50500 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_50500        1          graph_batch50491.c
  gj_libcgj_green_50500       1          graph_batch50492.c
  gj_bar3_ready_50500         0          graph_batch50493.c  (bar3 open)
  gj_product_score_50500      0          graph_batch50494.c  (score open)
  gj_continuum_ready_50500    1          graph_batch50495.c
  gj_smoke_soft_50500         1          graph_batch50496.c
  gj_dyn_soft_50500           1          graph_batch50497.c
  gj_milestone_tag_50500      50500      graph_batch50498.c
  gj_continuum_wave_50500     50500      graph_batch50499.c
  gj_batch_id_50500           50500      graph_batch50500.c
  gj_graph_milestone_50500    50500      graph_batch50500.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=50500`` means parent wired CGJ_SRCS through
    ``graph_batch50500.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_50500`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_50500`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M50500_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_50400`` / ``gj_*_50300`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 362 exclusive CREATE-ONLY lands the **markers** band
  (50491–50500) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_50500.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_50500.py

  # Restrict to marker band only (soft-gate TUs; Wave 362 exclusive):
  python3 user/libcgj/src/_gen_milestone_50500.py --band markers

  # Extension identity only (50401–50450):
  python3 user/libcgj/src/_gen_milestone_50500.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_50500.py --force
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 50500 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 50500
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 50401
WAVE_HI = 50500  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 50401, 50450
IDENTITY_LO, IDENTITY_HI = 50451, 50475
FILL_LO, FILL_HI = 50476, 50490
MARKERS_LO, MARKERS_HI = 50491, 50500
...
# Stems become gj_<stem>_50500. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (50491, "shell_green", 1, "shell product green extract (wave 50500)"),
    (50492, "libcgj_green", 1, "libcgj product green extract (wave 50500)"),
    (50493, "bar3_ready", 0, "product bar3 readiness gate (wave 50500)"),
    (50494, "product_score", 0, "product readiness score tag (wave 50500)"),
    (50495, "continuum_ready", 1, "continuum readiness gate (wave 50500)"),
    (50496, "smoke_soft", 1, "smoke soft lamp (wave 50500)"),
    (50497, "dyn_soft", 1, "dyn soft lamp (wave 50500)"),
    (50498, "milestone_tag", MILESTONE, "milestone 50500 product tag"),
    (50499, "continuum_wave", MILESTONE, "milestone 50500 continuum wave identity"),
    # 50500 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        50476: "continuum wave fill soft unit + batch id",
        50480: "continuum bridge soft unit + batch id",
        50490: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (50491–50499), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 50493
    tag_batch = MARKERS_LO + 7  # 50498
    wave_batch = MARKERS_LO + 8  # 50499
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_50400 / gj_shell_green_50300 / gj_shell_green_50200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_50400 / gj_libcgj_green_50300 / gj_libcgj_green_50200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_50400 / gj_bar3_ready_50300 / gj_bar3_ready_50200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_50400 / gj_product_score_50300 / gj_product_score_50200\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_50400 / gj_continuum_ready_50300 /\n"
            f" * gj_continuum_ready_50200 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_50400 / gj_smoke_soft_50300 / gj_smoke_soft_50200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_50400 / gj_dyn_soft_50300 / gj_dyn_soft_50200 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_50400 / gj_milestone_tag_50300 / gj_milestone_tag_50200,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_50400 / gj_continuum_wave_50300 /\n"
            f" * gj_continuum_wave_50200, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch50500.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 50499
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 50491–50499; 50500 is capstone
