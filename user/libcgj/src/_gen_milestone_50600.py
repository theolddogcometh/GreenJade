...
"""
Soft continuum generator — MILESTONE 50600 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 50501–50600 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=50600   when parent has wired through graph_batch50600.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 50501 50600
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 50501 50600
...
graph batches ending at M. For M = **50600** the exclusive range is
**50501–50600** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 50501–50550  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 50551–50575  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 50576–50590  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 50591–50600  │ decade soft gates (Wave 360 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 50600 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_50600        1          graph_batch50591.c
  gj_libcgj_green_50600       1          graph_batch50592.c
  gj_bar3_ready_50600         0          graph_batch50593.c  (bar3 open)
  gj_product_score_50600      0          graph_batch50594.c  (score open)
  gj_continuum_ready_50600    1          graph_batch50595.c
  gj_smoke_soft_50600         1          graph_batch50596.c
  gj_dyn_soft_50600           1          graph_batch50597.c
  gj_milestone_tag_50600      50600      graph_batch50598.c
  gj_continuum_wave_50600     50600      graph_batch50599.c
  gj_batch_id_50600           50600      graph_batch50600.c
  gj_graph_milestone_50600    50600      graph_batch50600.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=50600`` means parent wired CGJ_SRCS through
    ``graph_batch50600.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_50600`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_50600`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M50600_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_50500`` / ``gj_*_50400`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  Wave 360 exclusive CREATE-ONLY lands the **markers** band
  (50591–50600) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_50600.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_50600.py

  # Restrict to marker band only (soft-gate TUs; Wave 360 exclusive):
  python3 user/libcgj/src/_gen_milestone_50600.py --band markers

  # Extension identity only (50501–50550):
  python3 user/libcgj/src/_gen_milestone_50600.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_50600.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_50500.py  (prior decade generator; if present)
  user/libcgj/src/_gen_milestone_50400.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 50600 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 50600
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 50501
WAVE_HI = 50600  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 50501, 50550
IDENTITY_LO, IDENTITY_HI = 50551, 50575
FILL_LO, FILL_HI = 50576, 50590
MARKERS_LO, MARKERS_HI = 50591, 50600
...
# Stems become gj_<stem>_50600. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (50591, "shell_green", 1, "shell product green extract (wave 50600)"),
    (50592, "libcgj_green", 1, "libcgj product green extract (wave 50600)"),
    (50593, "bar3_ready", 0, "product bar3 readiness gate (wave 50600)"),
    (50594, "product_score", 0, "product readiness score tag (wave 50600)"),
    (50595, "continuum_ready", 1, "continuum readiness gate (wave 50600)"),
    (50596, "smoke_soft", 1, "smoke soft lamp (wave 50600)"),
    (50597, "dyn_soft", 1, "dyn soft lamp (wave 50600)"),
    (50598, "milestone_tag", MILESTONE, "milestone 50600 product tag"),
    (50599, "continuum_wave", MILESTONE, "milestone 50600 continuum wave identity"),
    # 50600 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        50576: "continuum wave fill soft unit + batch id",
        50580: "continuum bridge soft unit + batch id",
        50590: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (50591–50599), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 50593
    tag_batch = MARKERS_LO + 7  # 50598
    wave_batch = MARKERS_LO + 8  # 50599
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_50500 / gj_shell_green_50400 / gj_shell_green_50300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_50500 / gj_libcgj_green_50400 / gj_libcgj_green_50300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_50500 / gj_bar3_ready_50400 / gj_bar3_ready_50300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_50500 / gj_product_score_50400 / gj_product_score_50300\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_50500 / gj_continuum_ready_50400 /\n"
            f" * gj_continuum_ready_50300 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_50500 / gj_smoke_soft_50400 / gj_smoke_soft_50300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_50500 / gj_dyn_soft_50400 / gj_dyn_soft_50300 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_50500 / gj_milestone_tag_50400 / gj_milestone_tag_50300,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_50500 / gj_continuum_wave_50400 /\n"
            f" * gj_continuum_wave_50300, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch50600.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 50599
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 50591–50599; 50600 is capstone
