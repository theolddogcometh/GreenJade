...
"""
Soft continuum generator — MILESTONE 50700 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 50601–50700 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=50700   when parent has wired through graph_batch50700.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 50601 50700
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 50601 50700
...
graph batches ending at M. For M = **50700** the exclusive range is
**50601–50700** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 50601–50650  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 50651–50675  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 50676–50690  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 50691–50700  │ decade soft gates (Wave 361 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 50700 marker KATs (soft host probe expectations)
----------------------------------------------------------
  Symbol                      Expected   Source TU
  --------------------------  ---------  --------------------
  gj_shell_green_50700        1          graph_batch50691.c
  gj_libcgj_green_50700       1          graph_batch50692.c
  gj_bar3_ready_50700         0          graph_batch50693.c  (bar3 open)
  gj_product_score_50700      0          graph_batch50694.c  (score open)
  gj_continuum_ready_50700    1          graph_batch50695.c
  gj_smoke_soft_50700         1          graph_batch50696.c
  gj_dyn_soft_50700           1          graph_batch50697.c
  gj_milestone_tag_50700      50700      graph_batch50698.c
  gj_continuum_wave_50700     50700      graph_batch50699.c
  gj_batch_id_50700           50700      graph_batch50700.c
  gj_graph_milestone_50700    50700      graph_batch50700.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=50700`` means parent wired CGJ_SRCS through
    ``graph_batch50700.c``. It is **soft graph growth only**.
  * ``gj_bar3_ready_50700`` is intentionally **0** — Steam client / Deck
    Top-50 title matrix remain open (NOT-TRIED × 50).
  * ``gj_product_score_50700`` is intentionally **0**.
  * Soft rule for host probes: missing symbol → skip; present but wrong
    value → hard fail. Direct-link KATs use ``-DCGJ_SOFT_M50700_DIRECT``.
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_50600`` / ``gj_*_50500`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  (50691–50700) first; extension/identity/fill may be emitted later via
...
-----
  ...
  python3 user/libcgj/src/_gen_milestone_50700.py --dry-run

  # CREATE-ONLY emit (skip existing; typical after empty tree for this wave):
  python3 user/libcgj/src/_gen_milestone_50700.py

  # Restrict to marker band only (soft-gate TUs; Wave 361 exclusive):
  python3 user/libcgj/src/_gen_milestone_50700.py --band markers

  # Extension identity only (50601–50650):
  python3 user/libcgj/src/_gen_milestone_50700.py --band extension

  # Bands: extension | identity | fill | markers | all  (default: all)

  # Force rewrite (dangerous if hand-edited):
  python3 user/libcgj/src/_gen_milestone_50700.py --force
...
See also
--------
  user/libcgj/src/_gen_milestone_50600.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 50700 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 50700
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 50601
WAVE_HI = 50700  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 50601, 50650
IDENTITY_LO, IDENTITY_HI = 50651, 50675
FILL_LO, FILL_HI = 50676, 50690
MARKERS_LO, MARKERS_HI = 50691, 50700
...
# Stems become gj_<stem>_50700. bar3/product intentionally stay 0.
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (50691, "shell_green", 1, "shell product green extract (wave 50700)"),
    (50692, "libcgj_green", 1, "libcgj product green extract (wave 50700)"),
    (50693, "bar3_ready", 0, "product bar3 readiness gate (wave 50700)"),
    (50694, "product_score", 0, "product readiness score tag (wave 50700)"),
    (50695, "continuum_ready", 1, "continuum readiness gate (wave 50700)"),
    (50696, "smoke_soft", 1, "smoke soft lamp (wave 50700)"),
    (50697, "dyn_soft", 1, "dyn soft lamp (wave 50700)"),
    (50698, "milestone_tag", MILESTONE, "milestone 50700 product tag"),
    (50699, "continuum_wave", MILESTONE, "milestone 50700 continuum wave identity"),
    # 50700 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        50676: "continuum wave fill soft unit + batch id",
        50680: "continuum bridge soft unit + batch id",
        50690: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (50691-50699), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 50693
    tag_batch = MARKERS_LO + 7  # 50698
    wave_batch = MARKERS_LO + 8  # 50699
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_50600 / gj_shell_green_50500 / gj_shell_green_50700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_50600 / gj_libcgj_green_50500 / gj_libcgj_green_50700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_50600 / gj_bar3_ready_50500 / gj_bar3_ready_50700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_50600 / gj_product_score_50500 / gj_product_score_50700\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_50600 / gj_continuum_ready_50500 /\n"
            f" * gj_continuum_ready_50400 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_50600 / gj_smoke_soft_50500 / gj_smoke_soft_50700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_50600 / gj_dyn_soft_50500 / gj_dyn_soft_50700 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_50600 / gj_milestone_tag_50500 / gj_milestone_tag_50700,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_50600 / gj_continuum_wave_50500 /\n"
            f" * gj_continuum_wave_50400, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    """graph_batch50700.c — batch identity + graph milestone dual surface."""
    n = MILESTONE
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 50699
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 50691-50699; 50700 is capstone
