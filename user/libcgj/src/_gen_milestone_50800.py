...
"""
Soft continuum generator — MILESTONE 50800 (CREATE-ONLY).
...
``user/libcgj/src/``. It does **not**:
  ...
  - touch graph_batch*.c outside the 50701–50800 wave
...
------------------------------------------
  ...
  Honest high-water mark of wired batches::
    ...
    # → makefile_max=50800   when parent has wired through graph_batch50400.c

  Paste-ready CGJ_SRCS lines for this decade (after CREATE-ONLY lands)::

    ./scripts/gj-continuum-makefile-snippet.sh 50701 50800
    ./scripts/gj-continuum-makefile-snippet.sh --exist-only 50701 50800
...
graph batches ending at M. For M = **50800** the exclusive range is
**50701–50800** (100 TUs), layered as:

  ┌───────────────────┬──────────────┬──────────────────────────────────────┐
  │ Band              │ Batches      │ Surface role                         │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Extension identity│ 50701–50750  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (50 TUs)     │ (extension continuum identity lamps) │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Decade identity   │ 50751–50775  │ gj_batch_id_N + gj_graph_milestone_N │
  │                   │ (25 TUs)     │ (unique per-TU identity lamps)       │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Fill soft units   │ 50776–50790  │ gj_batch_id_N + one unique soft unit │
  │                   │ (15 TUs)     │ helper (wave_fill / slot_* / …)      │
  ├───────────────────┼──────────────┼──────────────────────────────────────┤
  │ Milestone markers │ 50791–50800  │ decade soft gates (Wave 362 exclusive)│
  │                   │ (10 TUs)     │ CREATE-ONLY marker/capstone band     │
  └───────────────────┴──────────────┴──────────────────────────────────────┘

Milestone 50800 marker KATs (soft host probe expectations)
----------------------------------------------------------
  ...
  gj_milestone_tag_50400      50800      graph_batch50398.c
  gj_continuum_wave_50400     50800      graph_batch50399.c
  gj_batch_id_50400           50800      graph_batch50400.c
  gj_graph_milestone_50400    50800      graph_batch50400.c
  __libcgj_batchNNNNN_marker  string     matching batch TU

Honesty / non-claims
--------------------
  * ``makefile_max=50800`` means parent wired CGJ_SRCS through
  ...
  * No multi-def: every public symbol is unique to its TU; never redefine
    prior ``gj_*_50700`` / ``gj_*_50600`` / bare ``gj_batch_id`` surfaces.
...
--------------------
  ...
  (50791–50800) first; extension/identity/fill may be emitted later via
...
-----
  ...
  # Extension identity only (50701–50750):
...
See also
--------
  user/libcgj/src/_gen_milestone_50700.py  (prior decade generator)
...
# ---------------------------------------------------------------------------
# Wave constants — MILESTONE 50800 soft continuum (honest ceiling)
# ---------------------------------------------------------------------------

MILESTONE = 50800
# Exclusive CREATE-ONLY range for this decade wave (100 TUs).
WAVE_LO = 50701
WAVE_HI = 50800  # inclusive; coincides with MILESTONE

# Band splits (inclusive ranges).
EXT_LO, EXT_HI = 50701, 50750
IDENTITY_LO, IDENTITY_HI = 50751, 50775
FILL_LO, FILL_HI = 50776, 50790
MARKERS_LO, MARKERS_HI = 50791, 50800
...
MARKER_SURFACES: Sequence[Tuple[int, str, int, str]] = (
    (50791, "shell_green", 1, "shell product green extract (wave 50800)"),
    (50792, "libcgj_green", 1, "libcgj product green extract (wave 50800)"),
    (50793, "bar3_ready", 0, "product bar3 readiness gate (wave 50800)"),
    (50794, "product_score", 0, "product readiness score tag (wave 50800)"),
    (50795, "continuum_ready", 1, "continuum readiness gate (wave 50800)"),
    (50796, "smoke_soft", 1, "smoke soft lamp (wave 50800)"),
    (50797, "dyn_soft", 1, "dyn soft lamp (wave 50800)"),
    (50798, "milestone_tag", MILESTONE, "milestone 50800 product tag"),
    (50799, "continuum_wave", MILESTONE, "milestone 50800 continuum wave identity"),
    # 50800 is dual-surface (batch_id + graph_milestone); handled specially.
)
...
def render_fill(n: int, unit: str) -> str:
    ...
    helper = f"b{n}_{unit}"
    title_map = {
        50776: "continuum wave fill soft unit + batch id",
        50780: "continuum bridge soft unit + batch id",
        50790: "continuum anchor soft unit + batch id",
    }
...
def render_marker_simple(n: int, stem: str, value: int, blurb: str) -> str:
    """Single-lamp milestone marker TU (50791–50799), rich Wave-16 style."""
    ...
    bar3_batch = MARKERS_LO + 2  # 50793
    tag_batch = MARKERS_LO + 7  # 50798
    wave_batch = MARKERS_LO + 8  # 50799
    ...
    if stem == "shell_green":
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_shell_green_50700 / gj_shell_green_50600 / gj_shell_green_50500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_libcgj_green_50700 / gj_libcgj_green_50600 / gj_libcgj_green_50500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_bar3_ready_50700 / gj_bar3_ready_50600 / gj_bar3_ready_50500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_product_score_50700 / gj_product_score_50600 / gj_product_score_50500\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_ready_50700 / gj_continuum_ready_50600 /\n"
            f" * gj_continuum_ready_50500 and sibling {MILESTONE} milestone symbols\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_smoke_soft_50700 / gj_smoke_soft_50600 / gj_smoke_soft_50500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_dyn_soft_50700 / gj_dyn_soft_50600 / gj_dyn_soft_50500 and\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_milestone_tag_50700 / gj_milestone_tag_50600 / gj_milestone_tag_50500,\n"
        ...
        exclusive_extra = (
            f" Distinct from\n"
            f" * gj_continuum_wave_50700 / gj_continuum_wave_50600 /\n"
            f" * gj_continuum_wave_50500, gj_milestone_tag_{MILESTONE} (batch{tag_batch}), and\n"
...
def render_milestone_capstone() -> str:
    ...
    mk = _marker_name(n)
    ms = _marker_str(n)
    markers_simple_hi = MARKERS_HI - 1  # 50799
    return f"""{HEADER_SPDX}\
...
def _validate_wave_constants() -> None:
    ...
    assert len(MARKER_SURFACES) == 9  # 50791–50799; 50800 is capstone
