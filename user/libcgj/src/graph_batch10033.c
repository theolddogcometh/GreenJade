 ...
 *
 * Desktop glibc graph batch10033: PulseAudio compat soft-ok unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_pulse_compat_ok_u_10033(void);
 *     - Always returns 0. Soft stub for pipewire/wireplumber soft-id
 *       continuum; PulseAudio compatibility is intentionally not
 *       reported ok.
 *   uint32_t __gj_pulse_compat_ok_u_10033  (alias)
 *   __libcgj_batch10033_marker = "libcgj-batch10033"
 *
...
const char __libcgj_batch10033_marker[] = "libcgj-batch10033";

/* Soft Pulse compat-ok lamp: always off for this continuum. */
#define B10033_PULSE_COMPAT_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b10033_pulse_compat_ok(void)
{
	return B10033_PULSE_COMPAT_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_pulse_compat_ok_u_10033 - PulseAudio compat soft-ok unit flag.
 *
 * Always returns 0. Soft pure-data stub; does not open PulseAudio or
 * call libc. No parent wires.
 */
uint32_t
gj_pulse_compat_ok_u_10033(void)
{
	(void)NULL;
	return b10033_pulse_compat_ok();
}
...
uint32_t __gj_pulse_compat_ok_u_10033(void)
    __attribute__((alias("gj_pulse_compat_ok_u_10033")));
