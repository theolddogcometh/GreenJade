 ...
 *
 * Desktop glibc graph batch10035: ALSA UCM soft-ok unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_alsa_ucm_ok_u_10035(void);
 *     - Always returns 0. Soft stub for pipewire/wireplumber soft-id
 *       continuum; ALSA Use Case Manager is intentionally not reported
 *       ok.
 *   uint32_t __gj_alsa_ucm_ok_u_10035  (alias)
 *   __libcgj_batch10035_marker = "libcgj-batch10035"
 *
...
const char __libcgj_batch10035_marker[] = "libcgj-batch10035";

/* Soft ALSA UCM-ok lamp: always off for this continuum. */
#define B10035_ALSA_UCM_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b10035_alsa_ucm_ok(void)
{
	return B10035_ALSA_UCM_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_alsa_ucm_ok_u_10035 - ALSA UCM soft-ok unit flag.
 *
 * Always returns 0. Soft pure-data stub; does not open ALSA UCM or call
 * libc. No parent wires.
 */
uint32_t
gj_alsa_ucm_ok_u_10035(void)
{
	(void)NULL;
	return b10035_alsa_ucm_ok();
}
...
uint32_t __gj_alsa_ucm_ok_u_10035(void)
    __attribute__((alias("gj_alsa_ucm_ok_u_10035")));
