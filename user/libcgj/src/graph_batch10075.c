 ...
 *
 * Desktop glibc graph batch10075: CPU boost ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_cpu_boost_ok_u_10075(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       CPU boost/frequency probe for the power/thermal continuum.
 *   uint32_t __gj_cpu_boost_ok_u_10075  (alias)
 *   __libcgj_batch10075_marker = "libcgj-batch10075"
 *
...
const char __libcgj_batch10075_marker[] = "libcgj-batch10075";

/* Soft CPU-boost-ok lamp: always off (not a real cpufreq probe). */
#define B10075_CPU_BOOST_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b10075_cpu_boost_ok(void)
{
	return B10075_CPU_BOOST_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_cpu_boost_ok_u_10075 - CPU boost ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe cpufreq/boost
 * or call libc. No parent wires.
 */
uint32_t
gj_cpu_boost_ok_u_10075(void)
{
	(void)NULL;
	return b10075_cpu_boost_ok();
}
...
uint32_t __gj_cpu_boost_ok_u_10075(void)
    __attribute__((alias("gj_cpu_boost_ok_u_10075")));
