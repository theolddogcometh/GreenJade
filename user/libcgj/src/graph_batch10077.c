 ...
 *
 * Desktop glibc graph batch10077: suspend ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_suspend_ok_u_10077(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       suspend/sleep state probe for the power/thermal continuum.
 *   uint32_t __gj_suspend_ok_u_10077  (alias)
 *   __libcgj_batch10077_marker = "libcgj-batch10077"
 *
...
const char __libcgj_batch10077_marker[] = "libcgj-batch10077";

/* Soft suspend-ok lamp: always off (not a real suspend probe). */
#define B10077_SUSPEND_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b10077_suspend_ok(void)
{
	return B10077_SUSPEND_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_suspend_ok_u_10077 - suspend ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe sleep states
 * or call libc. No parent wires.
 */
uint32_t
gj_suspend_ok_u_10077(void)
{
	(void)NULL;
	return b10077_suspend_ok();
}
...
uint32_t __gj_suspend_ok_u_10077(void)
    __attribute__((alias("gj_suspend_ok_u_10077")));
