 ...
 *
 * Desktop glibc graph batch10112: xdg portal ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_xdg_portal_ok_u_10112(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       XDG desktop portal probe for the flatpak/portal continuum.
 *   uint32_t __gj_xdg_portal_ok_u_10112  (alias)
 *   __libcgj_batch10112_marker = "libcgj-batch10112"
 *
...
const char __libcgj_batch10112_marker[] = "libcgj-batch10112";

/* Soft xdg-portal-ok lamp: always off (not a real portal probe). */
#define B10112_XDG_PORTAL_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b10112_xdg_portal_ok(void)
{
	return B10112_XDG_PORTAL_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_xdg_portal_ok_u_10112 - xdg portal ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe xdg-desktop-
 * portal or call libc. No parent wires.
 */
uint32_t
gj_xdg_portal_ok_u_10112(void)
{
	(void)NULL;
	return b10112_xdg_portal_ok();
}
...
uint32_t __gj_xdg_portal_ok_u_10112(void)
    __attribute__((alias("gj_xdg_portal_ok_u_10112")));
