 ...
 *
 * Desktop glibc graph batch10113: pipewire portal ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_pipewire_portal_ok_u_10113(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       PipeWire/screen-cast portal probe for the flatpak/portal
 *       continuum.
 *   uint32_t __gj_pipewire_portal_ok_u_10113  (alias)
 *   __libcgj_batch10113_marker = "libcgj-batch10113"
 *
...
const char __libcgj_batch10113_marker[] = "libcgj-batch10113";

/* Soft pipewire-portal-ok lamp: always off (not a real portal probe). */
#define B10113_PIPEWIRE_PORTAL_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b10113_pipewire_portal_ok(void)
{
	return B10113_PIPEWIRE_PORTAL_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_pipewire_portal_ok_u_10113 - pipewire portal ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe PipeWire or
 * ScreenCast portal and does not call libc. No parent wires.
 */
uint32_t
gj_pipewire_portal_ok_u_10113(void)
{
	(void)NULL;
	return b10113_pipewire_portal_ok();
}
...
uint32_t __gj_pipewire_portal_ok_u_10113(void)
    __attribute__((alias("gj_pipewire_portal_ok_u_10113")));
