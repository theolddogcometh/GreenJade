 ...
 *
 * Desktop glibc graph batch10166: hotplug soft-id stub.
 *
 * Surface (unique symbols):
 *   uint32_t gj_hotplug_ok_u_10166(void);
 ...
 *   uint32_t __gj_hotplug_ok_u_10166  (alias)
 *   __libcgj_batch10166_marker = "libcgj-batch10166"
 *
 ...
 * Unique gj_hotplug_ok_u_10166 surface only; no multi-def. Soft stub —
...
const char __libcgj_batch10166_marker[] = "libcgj-batch10166";

/* Soft unavailable / not-detected hotplug lamp. */
#define B10166_HOTPLUG_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b10166_hotplug_ok(void)
{
	return B10166_HOTPLUG_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_hotplug_ok_u_10166 - hotplug soft-id lamp.
 *
 * Always returns 0. Soft pure-data hotplug presence tag. Does not probe
 * netlink/uevent or call libc. No parent wires.
 */
uint32_t
gj_hotplug_ok_u_10166(void)
{
	(void)NULL;
	return b10166_hotplug_ok();
}
...
uint32_t __gj_hotplug_ok_u_10166(void)
    __attribute__((alias("gj_hotplug_ok_u_10166")));
