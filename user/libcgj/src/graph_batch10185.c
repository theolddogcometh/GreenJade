 ...
 *
 * Desktop glibc graph batch10185: PPD ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_ppd_ok_u_10185(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       PostScript Printer Description probe for the cups/print
 *       continuum.
 *   uint32_t __gj_ppd_ok_u_10185  (alias)
 *   __libcgj_batch10185_marker = "libcgj-batch10185"
 *
...
const char __libcgj_batch10185_marker[] = "libcgj-batch10185";

/* Soft PPD-ok lamp: always off (not a real PPD probe). */
#define B10185_PPD_OK  0u
...
b10185_ppd_ok(void)
{
	return B10185_PPD_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_ppd_ok_u_10185 - PPD ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not parse PPD files or
 * call libc. No parent wires.
 */
uint32_t
gj_ppd_ok_u_10185(void)
{
	(void)NULL;
	return b10185_ppd_ok();
}
...
uint32_t __gj_ppd_ok_u_10185(void)
    __attribute__((alias("gj_ppd_ok_u_10185")));
