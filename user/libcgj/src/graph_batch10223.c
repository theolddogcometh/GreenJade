 ...
 *
 * Desktop glibc graph batch10223: radeonsi ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_radeonsi_ok_u_10223(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       radeonsi/Gallium driver probe for the mesa/vulkan continuum.
 *   uint32_t __gj_radeonsi_ok_u_10223  (alias)
 *   __libcgj_batch10223_marker = "libcgj-batch10223"
 *
...
const char __libcgj_batch10223_marker[] = "libcgj-batch10223";

/* Soft radeonsi-ok lamp: always off (not a real radeonsi probe). */
#define B10223_RADEONSI_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b10223_radeonsi_ok(void)
{
	return B10223_RADEONSI_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_radeonsi_ok_u_10223 - radeonsi ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not inspect Gallium
 * radeonsi or call libc. No parent wires.
 */
uint32_t
gj_radeonsi_ok_u_10223(void)
{
	(void)NULL;
	return b10223_radeonsi_ok();
}
...
uint32_t __gj_radeonsi_ok_u_10223(void)
    __attribute__((alias("gj_radeonsi_ok_u_10223")));
