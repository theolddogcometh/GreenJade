 ...
 *
 * Desktop glibc graph batch10271: power ok soft unit (not a probe).
 *
 * Surface (unique symbols):
 *   uint32_t gj_power_ok_u_10271(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       power-rail / power-domain runtime probe for the power/thermal
 *       soft product continuum.
 *   uint32_t __gj_power_ok_u_10271  (alias)
 *   __libcgj_batch10271_marker = "libcgj-batch10271"
 *
...
const char __libcgj_batch10271_marker[] = "libcgj-batch10271";

/* Soft power-ok lamp: always off (not a real power-domain probe). */
#define B10271_POWER_OK  0u
...
b10271_ok(void)
{
	return B10271_POWER_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_power_ok_u_10271 - power ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe power rails
 * or call libc. No parent wires.
 */
uint32_t
gj_power_ok_u_10271(void)
{
	(void)NULL;
	return b10271_ok();
}
...
uint32_t __gj_power_ok_u_10271(void)
    __attribute__((alias("gj_power_ok_u_10271")));
