 ...
 *
 * Desktop glibc graph batch10272: tdp ok soft unit (not a probe).
 *
 * Surface (unique symbols):
 *   uint32_t gj_tdp_ok_u_10272(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       TDP / thermal design power configuration probe for the
 *       power/thermal soft product continuum.
 *   uint32_t __gj_tdp_ok_u_10272  (alias)
 *   __libcgj_batch10272_marker = "libcgj-batch10272"
 *
...
const char __libcgj_batch10272_marker[] = "libcgj-batch10272";

/* Soft tdp-ok lamp: always off (not a real TDP configuration probe). */
#define B10272_TDP_OK  0u
...
b10272_ok(void)
{
	return B10272_TDP_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_tdp_ok_u_10272 - tdp ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe TDP limits
 * or call libc. No parent wires.
 */
uint32_t
gj_tdp_ok_u_10272(void)
{
	(void)NULL;
	return b10272_ok();
}
...
uint32_t __gj_tdp_ok_u_10272(void)
    __attribute__((alias("gj_tdp_ok_u_10272")));
