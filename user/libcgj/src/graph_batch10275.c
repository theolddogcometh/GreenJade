 ...
 *
 * Desktop glibc graph batch10275: fan ok soft unit (not a probe).
 *
 * Surface (unique symbols):
 *   uint32_t gj_fan_ok_u_10275(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       fan / cooling-device runtime probe for the power/thermal soft
 *       product continuum.
 *   uint32_t __gj_fan_ok_u_10275  (alias)
 *   __libcgj_batch10275_marker = "libcgj-batch10275"
 *
...
const char __libcgj_batch10275_marker[] = "libcgj-batch10275";

/* Soft fan-ok lamp: always off (not a real cooling-device probe). */
#define B10275_FAN_OK  0u
...
b10275_ok(void)
{
	return B10275_FAN_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_fan_ok_u_10275 - fan ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe fan hardware
 * or call libc. No parent wires.
 */
uint32_t
gj_fan_ok_u_10275(void)
{
	(void)NULL;
	return b10275_ok();
}
...
uint32_t __gj_fan_ok_u_10275(void)
    __attribute__((alias("gj_fan_ok_u_10275")));
