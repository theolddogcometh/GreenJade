 ...
 *
 * Desktop glibc graph batch10414: camera portal ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_camera_portal_ok_u_10414(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       Camera portal probe for the flatpak/portal continuum.
 *   uint32_t __gj_camera_portal_ok_u_10414  (alias)
 *   __libcgj_batch10414_marker = "libcgj-batch10414"
 *
...
const char __libcgj_batch10414_marker[] = "libcgj-batch10414";

/* Soft camera-portal-ok lamp: always off (not a real portal probe). */
#define B10414_CAMERA_PORTAL_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b10414_camera_portal_ok(void)
{
	return B10414_CAMERA_PORTAL_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_camera_portal_ok_u_10414 - camera portal ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe Camera portal
 * or call libc. No parent wires.
 */
uint32_t
gj_camera_portal_ok_u_10414(void)
{
	(void)NULL;
	return b10414_camera_portal_ok();
}
...
uint32_t __gj_camera_portal_ok_u_10414(void)
    __attribute__((alias("gj_camera_portal_ok_u_10414")));
