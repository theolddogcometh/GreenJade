 ...
 *
 * Desktop glibc graph batch10415: file portal ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_file_portal_ok_u_10415(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       FileChooser portal probe for the flatpak/portal continuum.
 *   uint32_t __gj_file_portal_ok_u_10415  (alias)
 *   __libcgj_batch10415_marker = "libcgj-batch10415"
 *
...
const char __libcgj_batch10415_marker[] = "libcgj-batch10415";

/* Soft file-portal-ok lamp: always off (not a real portal probe). */
#define B10415_FILE_PORTAL_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b10415_file_portal_ok(void)
{
	return B10415_FILE_PORTAL_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_file_portal_ok_u_10415 - file portal ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe FileChooser
 * portal or call libc. No parent wires.
 */
uint32_t
gj_file_portal_ok_u_10415(void)
{
	(void)NULL;
	return b10415_file_portal_ok();
}
...
uint32_t __gj_file_portal_ok_u_10415(void)
    __attribute__((alias("gj_file_portal_ok_u_10415")));
