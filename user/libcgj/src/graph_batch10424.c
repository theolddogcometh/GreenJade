 ...
 *
 * Desktop glibc graph batch10424: cairo ok soft unit (not a probe).
 *
 * Surface (unique symbols):
 *   uint32_t gj_cairo_ok_u_10424(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       Cairo runtime probe for the font soft continuum.
 *   uint32_t __gj_cairo_ok_u_10424  (alias)
 *   __libcgj_batch10424_marker = "libcgj-batch10424"
 *
...
const char __libcgj_batch10424_marker[] = "libcgj-batch10424";

/* Soft cairo-ok lamp: always off (not a real runtime probe). */
#define B10424_CAIRO_OK  0u
...
b10424_ok(void)
{
	return B10424_CAIRO_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_cairo_ok_u_10424 - cairo ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe Cairo
 * presence or call libc. No parent wires.
 */
uint32_t
gj_cairo_ok_u_10424(void)
{
	(void)NULL;
	return b10424_ok();
}
...
uint32_t __gj_cairo_ok_u_10424(void)
    __attribute__((alias("gj_cairo_ok_u_10424")));
