 ...
 *
 * Desktop glibc graph batch10425: pango ok soft unit (not a probe).
 *
 * Surface (unique symbols):
 *   uint32_t gj_pango_ok_u_10425(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       Pango runtime probe for the font soft continuum.
 *   uint32_t __gj_pango_ok_u_10425  (alias)
 *   __libcgj_batch10425_marker = "libcgj-batch10425"
 *
...
const char __libcgj_batch10425_marker[] = "libcgj-batch10425";

/* Soft pango-ok lamp: always off (not a real runtime probe). */
#define B10425_PANGO_OK  0u
...
b10425_ok(void)
{
	return B10425_PANGO_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_pango_ok_u_10425 - pango ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe Pango
 * presence or call libc. No parent wires.
 */
uint32_t
gj_pango_ok_u_10425(void)
{
	(void)NULL;
	return b10425_ok();
}
...
uint32_t __gj_pango_ok_u_10425(void)
    __attribute__((alias("gj_pango_ok_u_10425")));
