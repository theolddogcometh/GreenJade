 ...
 *
 * Desktop glibc graph batch10444: journal ok soft unit (not a probe).
 *
 * Surface (unique symbols):
 *   uint32_t gj_journal_ok_u_10444(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       journald runtime probe for the dbus soft continuum.
 *   uint32_t __gj_journal_ok_u_10444  (alias)
 *   __libcgj_batch10444_marker = "libcgj-batch10444"
 *
...
const char __libcgj_batch10444_marker[] = "libcgj-batch10444";

/* Soft journal-ok lamp: always off (not a real runtime probe). */
#define B10444_JOURNAL_OK  0u
...
b10444_ok(void)
{
	return B10444_JOURNAL_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_journal_ok_u_10444 - journal ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe journald
 * presence or call libc. No parent wires.
 */
uint32_t
gj_journal_ok_u_10444(void)
{
	(void)NULL;
	return b10444_ok();
}
...
uint32_t __gj_journal_ok_u_10444(void)
    __attribute__((alias("gj_journal_ok_u_10444")));
