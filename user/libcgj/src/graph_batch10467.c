 ...
 *
 * Desktop glibc graph batch10467: input soft device-db-ok lamp (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_device_db_ok_u_10467(void);
 *     - Always returns 0. Soft stub only; intentionally not a live
 *       input device database presence probe for the input soft continuum.
 *   uint32_t __gj_device_db_ok_u_10467  (alias)
 *   __libcgj_batch10467_marker = "libcgj-batch10467"
 *
...
const char __libcgj_batch10467_marker[] = "libcgj-batch10467";

/* Soft device-db-ok lamp: always off (not a real device-db probe). */
#define B10467_DEVICE_DB_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b10467_device_db_ok(void)
{
	return B10467_DEVICE_DB_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_device_db_ok_u_10467 - input soft device-db-ok unit flag.
 *
 * Always returns 0. Soft continuum stub; does not open device
 * databases, parse ID files, or call libc. No parent wires.
 */
uint32_t
gj_device_db_ok_u_10467(void)
{
	(void)NULL;
	return b10467_device_db_ok();
}
...
uint32_t __gj_device_db_ok_u_10467(void)
    __attribute__((alias("gj_device_db_ok_u_10467")));
