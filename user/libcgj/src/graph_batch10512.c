 ...
 *
 * Desktop glibc graph batch10512: wine ok soft unit (not a probe).
 *
 * Surface (unique symbols):
 *   uint32_t gj_wine_ok_u_10512(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       Wine runtime probe for the proton soft continuum.
 *   uint32_t __gj_wine_ok_u_10512  (alias)
 *   __libcgj_batch10512_marker = "libcgj-batch10512"
 *
...
const char __libcgj_batch10512_marker[] = "libcgj-batch10512";

/* Soft wine-ok lamp: always off (not a real runtime probe). */
#define B10512_WINE_OK  0u
...
b10512_ok(void)
{
	return B10512_WINE_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_wine_ok_u_10512 - wine ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe Wine
 * presence or call libc. No parent wires.
 */
uint32_t
gj_wine_ok_u_10512(void)
{
	(void)NULL;
	return b10512_ok();
}
...
uint32_t __gj_wine_ok_u_10512(void)
    __attribute__((alias("gj_wine_ok_u_10512")));
