 ...
 *
 * Desktop glibc graph batch10516: compat-tool ok soft unit (not a probe).
 *
 * Surface (unique symbols):
 *   uint32_t gj_compat_tool_ok_u_10516(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       Steam compat-tool probe for the proton soft continuum.
 *   uint32_t __gj_compat_tool_ok_u_10516  (alias)
 *   __libcgj_batch10516_marker = "libcgj-batch10516"
 *
...
const char __libcgj_batch10516_marker[] = "libcgj-batch10516";

/* Soft compat-tool-ok lamp: always off (not a real runtime probe). */
#define B10516_COMPAT_TOOL_OK  0u
...
b10516_ok(void)
{
	return B10516_COMPAT_TOOL_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_compat_tool_ok_u_10516 - compat-tool ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe Steam
 * compatibility tool presence or call libc. No parent wires.
 */
uint32_t
gj_compat_tool_ok_u_10516(void)
{
	(void)NULL;
	return b10516_ok();
}
...
uint32_t __gj_compat_tool_ok_u_10516(void)
    __attribute__((alias("gj_compat_tool_ok_u_10516")));
