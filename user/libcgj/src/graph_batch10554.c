 ...
 *
 * Desktop glibc graph batch10554: hid ok soft unit (not a probe).
 *
 * Surface (unique symbols):
 *   uint32_t gj_hid_ok_u_10554(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       HID device runtime probe for the controller soft continuum.
 *   uint32_t __gj_hid_ok_u_10554  (alias)
 *   __libcgj_batch10554_marker = "libcgj-batch10554"
 *
...
const char __libcgj_batch10554_marker[] = "libcgj-batch10554";

/* Soft hid-ok lamp: always off (not a real HID probe). */
#define B10554_HID_OK  0u
...
b10554_ok(void)
{
	return B10554_HID_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_hid_ok_u_10554 - hid ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not open HID nodes or
 * call libc. No parent wires.
 */
uint32_t
gj_hid_ok_u_10554(void)
{
	(void)NULL;
	return b10554_ok();
}
...
uint32_t __gj_hid_ok_u_10554(void)
    __attribute__((alias("gj_hid_ok_u_10554")));
