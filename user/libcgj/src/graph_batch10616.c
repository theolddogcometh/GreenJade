 ...
 *
 * Desktop glibc graph batch10616: secret portal ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_secret_portal_ok_u_10616(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       Secret portal probe for the flatpak/portal continuum.
 *   uint32_t __gj_secret_portal_ok_u_10616  (alias)
 *   __libcgj_batch10616_marker = "libcgj-batch10616"
 *
...
const char __libcgj_batch10616_marker[] = "libcgj-batch10616";

/* Soft secret-portal-ok lamp: always off (not a real portal probe). */
#define B10616_SECRET_PORTAL_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b10616_secret_portal_ok(void)
{
	return B10616_SECRET_PORTAL_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_secret_portal_ok_u_10616 - secret portal ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe Secret portal
 * or call libc. No parent wires.
 */
uint32_t
gj_secret_portal_ok_u_10616(void)
{
	(void)NULL;
	return b10616_secret_portal_ok();
}
...
uint32_t __gj_secret_portal_ok_u_10616(void)
    __attribute__((alias("gj_secret_portal_ok_u_10616")));
