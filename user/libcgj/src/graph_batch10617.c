 ...
 *
 * Desktop glibc graph batch10617: notification portal ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_notification_portal_ok_u_10617(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       Notification portal probe for the flatpak/portal continuum.
 *   uint32_t __gj_notification_portal_ok_u_10617  (alias)
 *   __libcgj_batch10617_marker = "libcgj-batch10617"
 *
...
const char __libcgj_batch10617_marker[] = "libcgj-batch10617";

/* Soft notification-portal-ok lamp: always off (not a real portal probe). */
#define B10617_NOTIFICATION_PORTAL_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b10617_notification_portal_ok(void)
{
	return B10617_NOTIFICATION_PORTAL_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_notification_portal_ok_u_10617 - notification portal ok soft unit.
 *
 * Always returns 0. Soft continuum stub; does not probe Notification
 * portal or call libc. No parent wires.
 */
uint32_t
gj_notification_portal_ok_u_10617(void)
{
	(void)NULL;
	return b10617_notification_portal_ok();
}
...
uint32_t __gj_notification_portal_ok_u_10617(void)
    __attribute__((alias("gj_notification_portal_ok_u_10617")));
