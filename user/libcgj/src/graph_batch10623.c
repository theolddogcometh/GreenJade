 ...
 *
 * Desktop glibc graph batch10623: HarfBuzz soft-ok unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_harfbuzz_ok_u_10623(void);
 *     - Always returns 0. Soft stub for fontconfig/freetype soft-id
 *       continuum; HarfBuzz is intentionally not reported ok.
 *   uint32_t __gj_harfbuzz_ok_u_10623  (alias)
 *   __libcgj_batch10623_marker = "libcgj-batch10623"
 *
...
const char __libcgj_batch10623_marker[] = "libcgj-batch10623";

/* Soft HarfBuzz-ok lamp: always off for this continuum. */
#define B10623_HARFBUZZ_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b10623_harfbuzz_ok(void)
{
	return B10623_HARFBUZZ_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_harfbuzz_ok_u_10623 - HarfBuzz soft-ok unit flag.
 *
 * Always returns 0. Soft pure-data stub; does not shape text or call
 * HarfBuzz/libc. No parent wires.
 */
uint32_t
gj_harfbuzz_ok_u_10623(void)
{
	(void)NULL;
	return b10623_harfbuzz_ok();
}
...
uint32_t __gj_harfbuzz_ok_u_10623(void)
    __attribute__((alias("gj_harfbuzz_ok_u_10623")));
