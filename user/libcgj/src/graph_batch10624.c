 ...
 *
 * Desktop glibc graph batch10624: Cairo soft-ok unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_cairo_ok_u_10624(void);
 *     - Always returns 0. Soft stub for fontconfig/freetype soft-id
 *       continuum; Cairo is intentionally not reported ok.
 *   uint32_t __gj_cairo_ok_u_10624  (alias)
 *   __libcgj_batch10624_marker = "libcgj-batch10624"
 *
...
const char __libcgj_batch10624_marker[] = "libcgj-batch10624";

/* Soft Cairo-ok lamp: always off for this continuum. */
#define B10624_CAIRO_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b10624_cairo_ok(void)
{
	return B10624_CAIRO_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_cairo_ok_u_10624 - Cairo soft-ok unit flag.
 *
 * Always returns 0. Soft pure-data stub; does not create cairo_t or
 * call Cairo/libc. No parent wires.
 */
uint32_t
gj_cairo_ok_u_10624(void)
{
	(void)NULL;
	return b10624_cairo_ok();
}
...
uint32_t __gj_cairo_ok_u_10624(void)
    __attribute__((alias("gj_cairo_ok_u_10624")));
