 ...
 *
 * Desktop glibc graph batch10664: input soft hidraw-ok lamp (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_hidraw_ok_u_10664(void);
 *     - Always returns 0. Soft stub only; intentionally not a live
 *       hidraw device presence probe for the input soft continuum.
 *   uint32_t __gj_hidraw_ok_u_10664  (alias)
 *   __libcgj_batch10664_marker = "libcgj-batch10664"
 *
...
const char __libcgj_batch10664_marker[] = "libcgj-batch10664";

/* Soft hidraw-ok lamp: always off (not a real /dev/hidraw probe). */
#define B10664_HIDRAW_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b10664_hidraw_ok(void)
{
	return B10664_HIDRAW_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_hidraw_ok_u_10664 - input soft hidraw-ok unit flag.
 *
 * Always returns 0. Soft continuum stub; does not open /dev/hidraw*,
 * read HID reports, or call libc. No parent wires.
 */
uint32_t
gj_hidraw_ok_u_10664(void)
{
	(void)NULL;
	return b10664_hidraw_ok();
}
...
uint32_t __gj_hidraw_ok_u_10664(void)
    __attribute__((alias("gj_hidraw_ok_u_10664")));
