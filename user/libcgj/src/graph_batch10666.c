 ...
 *
 * Desktop glibc graph batch10666: input soft hotplug-ok lamp (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_hotplug_ok_u_10666(void);
 *     - Always returns 0. Soft stub only; intentionally not a live
 *       input hotplug monitor probe for the input soft continuum.
 *   uint32_t __gj_hotplug_ok_u_10666  (alias)
 *   __libcgj_batch10666_marker = "libcgj-batch10666"
 *
...
const char __libcgj_batch10666_marker[] = "libcgj-batch10666";

/* Soft hotplug-ok lamp: always off (not a real hotplug monitor). */
#define B10666_HOTPLUG_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b10666_hotplug_ok(void)
{
	return B10666_HOTPLUG_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_hotplug_ok_u_10666 - input soft hotplug-ok unit flag.
 *
 * Always returns 0. Soft continuum stub; does not watch udev netlink,
 * poll inotify on /dev/input, or call libc. No parent wires.
 */
uint32_t
gj_hotplug_ok_u_10666(void)
{
	(void)NULL;
	return b10666_hotplug_ok();
}
...
uint32_t __gj_hotplug_ok_u_10666(void)
    __attribute__((alias("gj_hotplug_ok_u_10666")));
