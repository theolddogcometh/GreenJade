 ...
 *
 * Desktop glibc graph batch10668: input soft hwdb-ok lamp (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_hwdb_ok_u_10668(void);
 *     - Always returns 0. Soft stub only; intentionally not a live
 *       udev hwdb presence probe for the input soft continuum.
 *   uint32_t __gj_hwdb_ok_u_10668  (alias)
 *   __libcgj_batch10668_marker = "libcgj-batch10668"
 *
...
const char __libcgj_batch10668_marker[] = "libcgj-batch10668";

/* Soft hwdb-ok lamp: always off (not a real hwdb probe). */
#define B10668_HWDB_OK  0u
...
uint32_t __gj_hwdb_ok_u_10668(void)
    __attribute__((alias("gj_hwdb_ok_u_10668")));
