 ...
 *
 * Desktop glibc graph batch10771: power ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_power_ok_u_10771(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       power/sysfs probe for the power soft continuum.
 *   uint32_t __gj_power_ok_u_10771  (alias)
 *   __libcgj_batch10771_marker = "libcgj-batch10771"
 *
...
const char __libcgj_batch10771_marker[] = "libcgj-batch10771";

/* Soft power-ok lamp: always off (not a real power probe). */
#define B10771_POWER_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b10771_power_ok(void)
{
	return B10771_POWER_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_power_ok_u_10771 - power ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe power/sysfs or
 * call libc. No parent wires.
 */
uint32_t
gj_power_ok_u_10771(void)
{
	(void)NULL;
	return b10771_power_ok();
}
...
uint32_t __gj_power_ok_u_10771(void)
    __attribute__((alias("gj_power_ok_u_10771")));
