 ...
 *
 * Desktop glibc graph batch10814: camera portal ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_camera_portal_ok_u_10814(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       Camera portal probe for the flatpak/portal continuum.
 *   uint32_t __gj_camera_portal_ok_u_10814  (alias)
 *   __libcgj_batch10814_marker = "libcgj-batch10814"
 *
...
const char __libcgj_batch10814_marker[] = "libcgj-batch10814";

/* Soft camera-portal-ok lamp: always off (not a real portal probe). */
#define B10814_CAMERA_PORTAL_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b10814_camera_portal_ok(void)
{
	return B10814_CAMERA_PORTAL_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_camera_portal_ok_u_10814 - camera portal ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe Camera portal
 * or call libc. No parent wires.
 */
uint32_t
gj_camera_portal_ok_u_10814(void)
{
	(void)NULL;
	return b10814_camera_portal_ok();
}
...
uint32_t __gj_camera_portal_ok_u_10814(void)
    __attribute__((alias("gj_camera_portal_ok_u_10814")));
