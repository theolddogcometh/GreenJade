 ...
 *
 * Desktop glibc graph batch10816: secret portal ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_secret_portal_ok_u_10816(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       Secret portal probe for the flatpak/portal continuum.
 *   uint32_t __gj_secret_portal_ok_u_10816  (alias)
 *   __libcgj_batch10816_marker = "libcgj-batch10816"
 *
...
const char __libcgj_batch10816_marker[] = "libcgj-batch10816";

/* Soft secret-portal-ok lamp: always off (not a real portal probe). */
#define B10816_SECRET_PORTAL_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b10816_secret_portal_ok(void)
{
	return B10816_SECRET_PORTAL_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_secret_portal_ok_u_10816 - secret portal ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe Secret portal
 * or call libc. No parent wires.
 */
uint32_t
gj_secret_portal_ok_u_10816(void)
{
	(void)NULL;
	return b10816_secret_portal_ok();
}
...
uint32_t __gj_secret_portal_ok_u_10816(void)
    __attribute__((alias("gj_secret_portal_ok_u_10816")));
