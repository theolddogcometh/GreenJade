 ...
 *
 * Desktop glibc graph batch10822: FreeType soft-ok unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_freetype_ok_u_10822(void);
 *     - Always returns 0. Soft stub for fontconfig/freetype soft-id
 *       continuum; FreeType is intentionally not reported ok.
 *   uint32_t __gj_freetype_ok_u_10822  (alias)
 *   __libcgj_batch10822_marker = "libcgj-batch10822"
 *
...
const char __libcgj_batch10822_marker[] = "libcgj-batch10822";

/* Soft FreeType-ok lamp: always off for this continuum. */
#define B10822_FREETYPE_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b10822_freetype_ok(void)
{
	return B10822_FREETYPE_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_freetype_ok_u_10822 - FreeType soft-ok unit flag.
 *
 * Always returns 0. Soft pure-data stub; does not open FT_Library or
 * call FreeType/libc. No parent wires.
 */
uint32_t
gj_freetype_ok_u_10822(void)
{
	(void)NULL;
	return b10822_freetype_ok();
}
...
uint32_t __gj_freetype_ok_u_10822(void)
    __attribute__((alias("gj_freetype_ok_u_10822")));
