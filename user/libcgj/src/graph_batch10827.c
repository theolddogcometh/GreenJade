 ...
 *
 * Desktop glibc graph batch10827: Graphite soft-ok unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_graphite_ok_u_10827(void);
 *     - Always returns 0. Soft stub for fontconfig/freetype soft-id
 *       continuum; Graphite is intentionally not reported ok.
 *   uint32_t __gj_graphite_ok_u_10827  (alias)
 *   __libcgj_batch10827_marker = "libcgj-batch10827"
 *
...
const char __libcgj_batch10827_marker[] = "libcgj-batch10827";

/* Soft Graphite-ok lamp: always off for this continuum. */
#define B10827_GRAPHITE_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b10827_graphite_ok(void)
{
	return B10827_GRAPHITE_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_graphite_ok_u_10827 - Graphite soft-ok unit flag.
 *
 * Always returns 0. Soft pure-data stub; does not load Graphite faces
 * or call Graphite/libc. No parent wires.
 */
uint32_t
gj_graphite_ok_u_10827(void)
{
	(void)NULL;
	return b10827_graphite_ok();
}
...
uint32_t __gj_graphite_ok_u_10827(void)
    __attribute__((alias("gj_graphite_ok_u_10827")));
