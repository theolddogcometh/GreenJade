 ...
 *
 * Desktop glibc graph batch10828: font match soft-ok unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_font_match_ok_u_10828(void);
 *     - Always returns 0. Soft stub for fontconfig/freetype soft-id
 *       continuum; font match is intentionally not reported ok.
 *   uint32_t __gj_font_match_ok_u_10828  (alias)
 *   __libcgj_batch10828_marker = "libcgj-batch10828"
 *
...
const char __libcgj_batch10828_marker[] = "libcgj-batch10828";

/* Soft font-match-ok lamp: always off for this continuum. */
#define B10828_FONT_MATCH_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b10828_font_match_ok(void)
{
	return B10828_FONT_MATCH_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_font_match_ok_u_10828 - font match soft-ok unit flag.
 *
 * Always returns 0. Soft pure-data stub; does not run FcFontMatch or
 * call fontconfig/libc. No parent wires.
 */
uint32_t
gj_font_match_ok_u_10828(void)
{
	(void)NULL;
	return b10828_font_match_ok();
}
...
uint32_t __gj_font_match_ok_u_10828(void)
    __attribute__((alias("gj_font_match_ok_u_10828")));
