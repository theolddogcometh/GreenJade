 ...
 *
 * Desktop glibc graph batch10842: systemd ok soft unit (not a probe).
 *
 * Surface (unique symbols):
 *   uint32_t gj_systemd_ok_u_10842(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       systemd runtime probe for the dbus soft continuum.
 *   uint32_t __gj_systemd_ok_u_10842  (alias)
 *   __libcgj_batch10842_marker = "libcgj-batch10842"
 *
...
const char __libcgj_batch10842_marker[] = "libcgj-batch10842";

/* Soft systemd-ok lamp: always off (not a real runtime probe). */
#define B10842_SYSTEMD_OK  0u
...
b10842_ok(void)
{
	return B10842_SYSTEMD_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_systemd_ok_u_10842 - systemd ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe systemd
 * presence or call libc. No parent wires.
 */
uint32_t
gj_systemd_ok_u_10842(void)
{
	(void)NULL;
	return b10842_ok();
}
...
uint32_t __gj_systemd_ok_u_10842(void)
    __attribute__((alias("gj_systemd_ok_u_10842")));
