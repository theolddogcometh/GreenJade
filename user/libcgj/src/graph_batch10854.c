 ...
 *
 * Desktop glibc graph batch10854: pam ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_pam_ok_u_10854(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       PAM probe for the auth soft continuum.
 *   uint32_t __gj_pam_ok_u_10854  (alias)
 *   __libcgj_batch10854_marker = "libcgj-batch10854"
 *
 ...
 * gj_pam_ok_u_10654 (batch10654) and gj_pam_ok_u_10454 (batch10454).
...
const char __libcgj_batch10854_marker[] = "libcgj-batch10854";

/* Soft pam-ok lamp: always off (not a real PAM probe). */
#define B10854_PAM_OK  0u
...
b10854_pam_ok(void)
{
	return B10854_PAM_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_pam_ok_u_10854 - pam ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not invoke PAM or call
 * libc. No parent wires.
 */
uint32_t
gj_pam_ok_u_10854(void)
{
	(void)NULL;
	return b10854_pam_ok();
}
...
uint32_t __gj_pam_ok_u_10854(void)
    __attribute__((alias("gj_pam_ok_u_10854")));
