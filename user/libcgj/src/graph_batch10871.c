 ...
 *
 * Desktop glibc graph batch10871: BlueZ soft-ok unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_bluez_ok_u_10871(void);
 *     - Always returns 0. Soft stub for bluez/networkmanager soft-id
 *       continuum; BlueZ path is intentionally not reported ok.
 *   uint32_t __gj_bluez_ok_u_10871  (alias)
 *   __libcgj_batch10871_marker = "libcgj-batch10871"
 *
...
const char __libcgj_batch10871_marker[] = "libcgj-batch10871";

/* Soft BlueZ-ok lamp: always off for this continuum. */
#define B10871_BLUEZ_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b10871_bluez_ok(void)
{
	return B10871_BLUEZ_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_bluez_ok_u_10871 - BlueZ soft-ok unit flag.
 *
 * Always returns 0. Soft pure-data stub; does not open BlueZ/
 * NetworkManager paths or call libc. No parent wires.
 */
uint32_t
gj_bluez_ok_u_10871(void)
{
	(void)NULL;
	return b10871_bluez_ok();
}
...
uint32_t __gj_bluez_ok_u_10871(void)
    __attribute__((alias("gj_bluez_ok_u_10871")));
