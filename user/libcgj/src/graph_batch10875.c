 ...
 *
 * Desktop glibc graph batch10875: ModemManager soft-ok unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_modemmanager_ok_u_10875(void);
 *     - Always returns 0. Soft stub for bluez/networkmanager soft-id
 *       continuum; ModemManager path is intentionally not reported ok.
 *   uint32_t __gj_modemmanager_ok_u_10875  (alias)
 *   __libcgj_batch10875_marker = "libcgj-batch10875"
 *
...
const char __libcgj_batch10875_marker[] = "libcgj-batch10875";

/* Soft ModemManager-ok lamp: always off for this continuum. */
#define B10875_MODEMMANAGER_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b10875_modemmanager_ok(void)
{
	return B10875_MODEMMANAGER_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_modemmanager_ok_u_10875 - ModemManager soft-ok unit flag.
 *
 * Always returns 0. Soft pure-data stub; does not open BlueZ/
 * NetworkManager paths or call libc. No parent wires.
 */
uint32_t
gj_modemmanager_ok_u_10875(void)
{
	(void)NULL;
	return b10875_modemmanager_ok();
}
...
uint32_t __gj_modemmanager_ok_u_10875(void)
    __attribute__((alias("gj_modemmanager_ok_u_10875")));
