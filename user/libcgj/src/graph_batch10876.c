 ...
 *
 * Desktop glibc graph batch10876: ConnMan soft-ok unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_connman_ok_u_10876(void);
 *     - Always returns 0. Soft stub for bluez/networkmanager soft-id
 *       continuum; ConnMan path is intentionally not reported ok.
 *   uint32_t __gj_connman_ok_u_10876  (alias)
 *   __libcgj_batch10876_marker = "libcgj-batch10876"
 *
...
const char __libcgj_batch10876_marker[] = "libcgj-batch10876";

/* Soft ConnMan-ok lamp: always off for this continuum. */
#define B10876_CONNMAN_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b10876_connman_ok(void)
{
	return B10876_CONNMAN_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_connman_ok_u_10876 - ConnMan soft-ok unit flag.
 *
 * Always returns 0. Soft pure-data stub; does not open BlueZ/
 * NetworkManager paths or call libc. No parent wires.
 */
uint32_t
gj_connman_ok_u_10876(void)
{
	(void)NULL;
	return b10876_connman_ok();
}
...
uint32_t __gj_connman_ok_u_10876(void)
    __attribute__((alias("gj_connman_ok_u_10876")));
