 ...
 *
 * Desktop glibc graph batch10878: resolvconf soft-ok unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_resolvconf_ok_u_10878(void);
 *     - Always returns 0. Soft stub for bluez/networkmanager soft-id
 *       continuum; resolvconf path is intentionally not reported ok.
 *   uint32_t __gj_resolvconf_ok_u_10878  (alias)
 *   __libcgj_batch10878_marker = "libcgj-batch10878"
 *
...
const char __libcgj_batch10878_marker[] = "libcgj-batch10878";

/* Soft resolvconf-ok lamp: always off for this continuum. */
#define B10878_RESOLVCONF_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b10878_resolvconf_ok(void)
{
	return B10878_RESOLVCONF_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_resolvconf_ok_u_10878 - resolvconf soft-ok unit flag.
 *
 * Always returns 0. Soft pure-data stub; does not open BlueZ/
 * NetworkManager paths or call libc. No parent wires.
 */
uint32_t
gj_resolvconf_ok_u_10878(void)
{
	(void)NULL;
	return b10878_resolvconf_ok();
}
...
uint32_t __gj_resolvconf_ok_u_10878(void)
    __attribute__((alias("gj_resolvconf_ok_u_10878")));
