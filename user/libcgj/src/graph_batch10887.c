 ...
 *
 * Desktop glibc graph batch10887: spool ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_spool_ok_u_10887(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       print spool / queue directory probe for the cups/print
 *       continuum.
 *   uint32_t __gj_spool_ok_u_10887  (alias)
 *   __libcgj_batch10887_marker = "libcgj-batch10887"
 *
...
const char __libcgj_batch10887_marker[] = "libcgj-batch10887";

/* Soft spool-ok lamp: always off (not a real spool probe). */
#define B10887_SPOOL_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b10887_spool_ok(void)
{
	return B10887_SPOOL_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_spool_ok_u_10887 - spool ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not inspect /var/spool
 * or call libc. No parent wires.
 */
uint32_t
gj_spool_ok_u_10887(void)
{
	(void)NULL;
	return b10887_spool_ok();
}
...
uint32_t __gj_spool_ok_u_10887(void)
    __attribute__((alias("gj_spool_ok_u_10887")));
