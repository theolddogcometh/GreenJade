 ...
 *
 * Desktop glibc graph batch10955: XInput ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_xinput_ok_u_10955(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       XInput probe for the controller soft continuum.
 *   uint32_t __gj_xinput_ok_u_10955  (alias)
 *   __libcgj_batch10955_marker = "libcgj-batch10955"
 *
...
const char __libcgj_batch10955_marker[] = "libcgj-batch10955";

/* Soft XInput-ok lamp: always off (not a real XInput probe). */
#define B10955_XINPUT_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b10955_xinput_ok(void)
{
	return B10955_XINPUT_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_xinput_ok_u_10955 - XInput ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not open XInput paths or
 * call libc. No parent wires.
 */
uint32_t
gj_xinput_ok_u_10955(void)
{
	(void)NULL;
	return b10955_xinput_ok();
}
...
uint32_t __gj_xinput_ok_u_10955(void)
    __attribute__((alias("gj_xinput_ok_u_10955")));
