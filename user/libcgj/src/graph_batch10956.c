 ...
 *
 * Desktop glibc graph batch10956: evdev ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_evdev_ok_u_10956(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       Linux evdev probe for the controller soft continuum.
 *   uint32_t __gj_evdev_ok_u_10956  (alias)
 *   __libcgj_batch10956_marker = "libcgj-batch10956"
 *
...
const char __libcgj_batch10956_marker[] = "libcgj-batch10956";

/* Soft evdev-ok lamp: always off (not a real /dev/input probe). */
#define B10956_EVDEV_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b10956_evdev_ok(void)
{
	return B10956_EVDEV_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_evdev_ok_u_10956 - evdev ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not open /dev/input or
 * call libc. No parent wires.
 */
uint32_t
gj_evdev_ok_u_10956(void)
{
	(void)NULL;
	return b10956_evdev_ok();
}
...
uint32_t __gj_evdev_ok_u_10956(void)
    __attribute__((alias("gj_evdev_ok_u_10956")));
