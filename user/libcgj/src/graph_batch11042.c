 ...
 *
 * Desktop glibc graph batch11042: systemd ok soft unit (not a probe).
 *
 * Surface (unique symbols):
 *   uint32_t gj_systemd_ok_u_11042(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       systemd runtime probe for the dbus soft continuum.
 *   uint32_t __gj_systemd_ok_u_11042  (alias)
 *   __libcgj_batch11042_marker = "libcgj-batch11042"
 *
 ...
 * gj_systemd_ok_u_10142. No parent wires.
 * No __int128.
 *
...
const char __libcgj_batch11042_marker[] = "libcgj-batch11042";

/* Soft systemd-ok lamp: always off (not a real runtime probe). */
#define B11042_SYSTEMD_OK  0u
...
b11042_ok(void)
{
	return B11042_SYSTEMD_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_systemd_ok_u_11042 - systemd ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe systemd
 * presence or call libc. No parent wires.
 */
uint32_t
gj_systemd_ok_u_11042(void)
{
	(void)NULL;
	return b11042_ok();
}
...
uint32_t __gj_systemd_ok_u_11042(void)
    __attribute__((alias("gj_systemd_ok_u_11042")));
