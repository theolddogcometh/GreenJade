 ...
 *
 * Desktop glibc graph batch11151: controller ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_controller_ok_u_11151(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       controller/input probe for the controller soft continuum.
 *   uint32_t __gj_controller_ok_u_11151  (alias)
 *   __libcgj_batch11151_marker = "libcgj-batch11151"
 *
...
const char __libcgj_batch11151_marker[] = "libcgj-batch11151";

/* Soft controller-ok lamp: always off (not a real controller probe). */
#define B11151_CONTROLLER_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b11151_controller_ok(void)
{
	return B11151_CONTROLLER_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_controller_ok_u_11151 - controller ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not open devices or
 * call libc. No parent wires.
 */
uint32_t
gj_controller_ok_u_11151(void)
{
	(void)NULL;
	return b11151_controller_ok();
}
...
uint32_t __gj_controller_ok_u_11151(void)
    __attribute__((alias("gj_controller_ok_u_11151")));
