 ...
 *
 * Desktop glibc graph batch11152: gamepad ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_gamepad_ok_u_11152(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       gamepad probe for the controller soft continuum.
 *   uint32_t __gj_gamepad_ok_u_11152  (alias)
 *   __libcgj_batch11152_marker = "libcgj-batch11152"
 *
...
const char __libcgj_batch11152_marker[] = "libcgj-batch11152";

/* Soft gamepad-ok lamp: always off (not a real gamepad probe). */
#define B11152_GAMEPAD_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b11152_gamepad_ok(void)
{
	return B11152_GAMEPAD_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_gamepad_ok_u_11152 - gamepad ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not open devices or
 * call libc. No parent wires.
 */
uint32_t
gj_gamepad_ok_u_11152(void)
{
	(void)NULL;
	return b11152_gamepad_ok();
}
...
uint32_t __gj_gamepad_ok_u_11152(void)
    __attribute__((alias("gj_gamepad_ok_u_11152")));
