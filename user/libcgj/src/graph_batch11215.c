 ...
 *
 * Desktop glibc graph batch11215: file portal ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_file_portal_ok_u_11215(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       FileChooser portal probe for the flatpak/portal continuum.
 *   uint32_t __gj_file_portal_ok_u_11215  (alias)
 *   __libcgj_batch11215_marker = "libcgj-batch11215"
 *
 ...
 * multi-def. Distinct from gj_file_portal_ok_u_11015 (batch11015). No
...
const char __libcgj_batch11215_marker[] = "libcgj-batch11215";

/* Soft file-portal-ok lamp: always off (not a real portal probe). */
#define B11215_FILE_PORTAL_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b11215_file_portal_ok(void)
{
	return B11215_FILE_PORTAL_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_file_portal_ok_u_11215 - file portal ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe FileChooser
 * portal or call libc. No parent wires.
 */
uint32_t
gj_file_portal_ok_u_11215(void)
{
	(void)NULL;
	return b11215_file_portal_ok();
}
...
uint32_t __gj_file_portal_ok_u_11215(void)
    __attribute__((alias("gj_file_portal_ok_u_11215")));
