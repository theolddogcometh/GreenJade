 ...
 *
 * Desktop glibc graph batch11242: systemd ok soft unit (not a probe).
 *
 * Surface (unique symbols):
 *   uint32_t gj_systemd_ok_u_11242(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       systemd runtime probe for the dbus soft continuum.
 *   uint32_t __gj_systemd_ok_u_11242  (alias)
 *   __libcgj_batch11242_marker = "libcgj-batch11242"
 *
 ...
 * no multi-def. Distinct from gj_systemd_ok_u_11042 /
...
const char __libcgj_batch11242_marker[] = "libcgj-batch11242";

/* Soft systemd-ok lamp: always off (not a real runtime probe). */
#define B11242_SYSTEMD_OK  0u
...
b11242_ok(void)
{
	return B11242_SYSTEMD_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_systemd_ok_u_11242 - systemd ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe systemd
 * presence or call libc. No parent wires.
 */
uint32_t
gj_systemd_ok_u_11242(void)
{
	(void)NULL;
	return b11242_ok();
}
...
uint32_t __gj_systemd_ok_u_11242(void)
    __attribute__((alias("gj_systemd_ok_u_11242")));
