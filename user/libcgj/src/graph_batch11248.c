 ...
 *
 * Desktop glibc graph batch11248: hostnamed ok soft unit (not a probe).
 *
 * Surface (unique symbols):
 *   uint32_t gj_hostnamed_ok_u_11248(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       systemd-hostnamed runtime probe for the dbus soft continuum.
 *   uint32_t __gj_hostnamed_ok_u_11248  (alias)
 *   __libcgj_batch11248_marker = "libcgj-batch11248"
 *
 ...
 * no multi-def. Distinct from gj_hostnamed_ok_u_11048 /
...
const char __libcgj_batch11248_marker[] = "libcgj-batch11248";

/* Soft hostnamed-ok lamp: always off (not a real runtime probe). */
#define B11248_HOSTNAMED_OK  0u
...
b11248_ok(void)
{
	return B11248_HOSTNAMED_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_hostnamed_ok_u_11248 - hostnamed ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe systemd-hostnamed
 * presence or call libc. No parent wires.
 */
uint32_t
gj_hostnamed_ok_u_11248(void)
{
	(void)NULL;
	return b11248_ok();
}
...
uint32_t __gj_hostnamed_ok_u_11248(void)
    __attribute__((alias("gj_hostnamed_ok_u_11248")));
