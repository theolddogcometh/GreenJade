 ...
 *
 * Desktop glibc graph batch11277: Avahi soft-ok unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_avahi_ok_u_11277(void);
 *     - Always returns 0. Soft stub for bluez/networkmanager soft-id
 *       continuum; Avahi path is intentionally not reported ok.
 *   uint32_t __gj_avahi_ok_u_11277  (alias)
 *   __libcgj_batch11277_marker = "libcgj-batch11277"
 *
 ...
 * multi-def. Distinct from gj_avahi_ok_u_11077 (batch11077) and prior
...
const char __libcgj_batch11277_marker[] = "libcgj-batch11277";

/* Soft Avahi-ok lamp: always off for this continuum. */
#define B11277_AVAHI_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b11277_avahi_ok(void)
{
	return B11277_AVAHI_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_avahi_ok_u_11277 - Avahi soft-ok unit flag.
 *
 * Always returns 0. Soft pure-data stub; does not open Avahi or call
 * libc. No parent wires.
 */
uint32_t
gj_avahi_ok_u_11277(void)
{
	(void)NULL;
	return b11277_avahi_ok();
}
...
uint32_t __gj_avahi_ok_u_11277(void)
    __attribute__((alias("gj_avahi_ok_u_11277")));
