 ...
 *
 * Desktop glibc graph batch11281: CUPS ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_cups_ok_u_11281(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       cups/cupsd probe for the cups/print continuum.
 *   uint32_t __gj_cups_ok_u_11281  (alias)
 *   __libcgj_batch11281_marker = "libcgj-batch11281"
 *
...
const char __libcgj_batch11281_marker[] = "libcgj-batch11281";

/* Soft CUPS-ok lamp: always off (not a real cups probe). */
#define B11281_CUPS_OK  0u
...
b11281_cups_ok(void)
{
	return B11281_CUPS_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_cups_ok_u_11281 - CUPS ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not inspect cupsd or
 * call libc. No parent wires.
 */
uint32_t
gj_cups_ok_u_11281(void)
{
	(void)NULL;
	return b11281_cups_ok();
}
...
uint32_t __gj_cups_ok_u_11281(void)
    __attribute__((alias("gj_cups_ok_u_11281")));
