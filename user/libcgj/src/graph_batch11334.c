 ...
 *
 * Desktop glibc graph batch11334: JACK compat soft-ok unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_jack_compat_ok_u_11334(void);
 *     - Always returns 0. Soft stub for pipewire/wireplumber soft-id
 *       continuum; JACK compatibility is intentionally not reported ok.
 *   uint32_t __gj_jack_compat_ok_u_11334  (alias)
 *   __libcgj_batch11334_marker = "libcgj-batch11334"
 *
...
const char __libcgj_batch11334_marker[] = "libcgj-batch11334";

/* Soft JACK compat-ok lamp: always off for this continuum. */
#define B11334_JACK_COMPAT_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b11334_jack_compat_ok(void)
{
	return B11334_JACK_COMPAT_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_jack_compat_ok_u_11334 - JACK compat soft-ok unit flag.
 *
 * Always returns 0. Soft pure-data stub; does not open JACK or call
 * libc. No parent wires.
 */
uint32_t
gj_jack_compat_ok_u_11334(void)
{
	(void)NULL;
	return b11334_jack_compat_ok();
}
...
uint32_t __gj_jack_compat_ok_u_11334(void)
    __attribute__((alias("gj_jack_compat_ok_u_11334")));
