 ...
 *
 * Desktop glibc graph batch11371: power ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_power_ok_u_11371(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       power/sysfs probe for the power soft continuum.
 *   uint32_t __gj_power_ok_u_11371  (alias)
 *   __libcgj_batch11371_marker = "libcgj-batch11371"
 *
...
const char __libcgj_batch11371_marker[] = "libcgj-batch11371";

/* Soft power-ok lamp: always off (not a real power probe). */
#define B11371_POWER_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b11371_power_ok(void)
{
	return B11371_POWER_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_power_ok_u_11371 - power ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe power/sysfs or
 * call libc. No parent wires.
 */
uint32_t
gj_power_ok_u_11371(void)
{
	(void)NULL;
	return b11371_power_ok();
}
...
uint32_t __gj_power_ok_u_11371(void)
    __attribute__((alias("gj_power_ok_u_11371")));
