 ...
 *
 * Desktop glibc graph batch11411: flatpak ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_flatpak_ok_u_11411(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       flatpak/portal probe for the flatpak portal soft continuum.
 *   uint32_t __gj_flatpak_ok_u_11411  (alias)
 *   __libcgj_batch11411_marker = "libcgj-batch11411"
 *
...
const char __libcgj_batch11411_marker[] = "libcgj-batch11411";

/* Soft flatpak-ok lamp: always off (not a real flatpak probe). */
#define B11411_FLATPAK_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b11411_flatpak_ok(void)
{
	return B11411_FLATPAK_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_flatpak_ok_u_11411 - flatpak ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe flatpak or
 * call libc. No parent wires.
 */
uint32_t
gj_flatpak_ok_u_11411(void)
{
	(void)NULL;
	return b11411_flatpak_ok();
}
...
uint32_t __gj_flatpak_ok_u_11411(void)
    __attribute__((alias("gj_flatpak_ok_u_11411")));
