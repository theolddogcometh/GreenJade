 ...
 *
 * Desktop glibc graph batch11481: CUPS ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_cups_ok_u_11481(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       cups/cupsd probe for the cups/print continuum.
 *   uint32_t __gj_cups_ok_u_11481  (alias)
 *   __libcgj_batch11481_marker = "libcgj-batch11481"
 *
 ...
 * gj_cups_ok_u_10181 (batch10181) and gj_cups_ready_u_9449
...
const char __libcgj_batch11481_marker[] = "libcgj-batch11481";

/* Soft CUPS-ok lamp: always off (not a real cups probe). */
#define B11481_CUPS_OK  0u
...
b11481_cups_ok(void)
{
	return B11481_CUPS_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_cups_ok_u_11481 - CUPS ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not inspect cupsd or
 * call libc. No parent wires.
 */
uint32_t
gj_cups_ok_u_11481(void)
{
	(void)NULL;
	return b11481_cups_ok();
}
...
uint32_t __gj_cups_ok_u_11481(void)
    __attribute__((alias("gj_cups_ok_u_11481")));
