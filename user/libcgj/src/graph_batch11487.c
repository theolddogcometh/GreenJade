 ...
 *
 * Desktop glibc graph batch11487: spool ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_spool_ok_u_11487(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       print spool / queue directory probe for the cups/print
 *       continuum.
 *   uint32_t __gj_spool_ok_u_11487  (alias)
 *   __libcgj_batch11487_marker = "libcgj-batch11487"
 *
 ...
 * gj_spool_ok_u_10187 (batch10187). No parent wires. No __int128.
...
const char __libcgj_batch11487_marker[] = "libcgj-batch11487";

/* Soft spool-ok lamp: always off (not a real spool probe). */
#define B11487_SPOOL_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b11487_spool_ok(void)
{
	return B11487_SPOOL_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_spool_ok_u_11487 - spool ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not inspect /var/spool
 * or call libc. No parent wires.
 */
uint32_t
gj_spool_ok_u_11487(void)
{
	(void)NULL;
	return b11487_spool_ok();
}
...
uint32_t __gj_spool_ok_u_11487(void)
    __attribute__((alias("gj_spool_ok_u_11487")));
