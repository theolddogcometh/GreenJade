 ...
 *
 * Desktop glibc graph batch11535: ALSA UCM soft-ok unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_alsa_ucm_ok_u_11535(void);
 *     - Always returns 0. Soft stub for pipewire/wireplumber soft-id
 *       continuum; ALSA Use Case Manager is intentionally not reported
 *       ok.
 *   uint32_t __gj_alsa_ucm_ok_u_11535  (alias)
 *   __libcgj_batch11535_marker = "libcgj-batch11535"
 *
...
const char __libcgj_batch11535_marker[] = "libcgj-batch11535";

/* Soft ALSA UCM-ok lamp: always off for this continuum. */
#define B11535_ALSA_UCM_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b11535_alsa_ucm_ok(void)
{
	return B11535_ALSA_UCM_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_alsa_ucm_ok_u_11535 - ALSA UCM soft-ok unit flag.
 *
 * Always returns 0. Soft pure-data stub; does not open UCM or call
 * libc. No parent wires.
 */
uint32_t
gj_alsa_ucm_ok_u_11535(void)
{
	(void)NULL;
	return b11535_alsa_ucm_ok();
}
...
uint32_t __gj_alsa_ucm_ok_u_11535(void)
    __attribute__((alias("gj_alsa_ucm_ok_u_11535")));
