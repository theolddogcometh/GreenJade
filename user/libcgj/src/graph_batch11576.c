 ...
 *
 * Desktop glibc graph batch11576: suspend ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_suspend_ok_u_11576(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       suspend/sleep state probe for the power soft continuum.
 *   uint32_t __gj_suspend_ok_u_11576  (alias)
 *   __libcgj_batch11576_marker = "libcgj-batch11576"
 *
...
const char __libcgj_batch11576_marker[] = "libcgj-batch11576";

/* Soft suspend-ok lamp: always off (not a real suspend probe). */
#define B11576_SUSPEND_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b11576_suspend_ok(void)
{
	return B11576_SUSPEND_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_suspend_ok_u_11576 - suspend ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe sleep states
 * or call libc. No parent wires.
 */
uint32_t
gj_suspend_ok_u_11576(void)
{
	(void)NULL;
	return b11576_suspend_ok();
}
...
uint32_t __gj_suspend_ok_u_11576(void)
    __attribute__((alias("gj_suspend_ok_u_11576")));
