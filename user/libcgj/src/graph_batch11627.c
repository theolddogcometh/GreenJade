 ...
 *
 * Desktop glibc graph batch11627: Graphite soft-ok unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_graphite_ok_u_11627(void);
 *     - Always returns 0. Soft stub for fontconfig/freetype soft-id
 *       continuum; Graphite is intentionally not reported ok.
 *   uint32_t __gj_graphite_ok_u_11627  (alias)
 *   __libcgj_batch11627_marker = "libcgj-batch11627"
 *
...
const char __libcgj_batch11627_marker[] = "libcgj-batch11627";

/* Soft Graphite-ok lamp: always off for this continuum. */
#define B11627_GRAPHITE_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b11627_graphite_ok(void)
{
	return B11627_GRAPHITE_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_graphite_ok_u_11627 - Graphite soft-ok unit flag.
 *
 * Always returns 0. Soft pure-data stub; does not load Graphite faces
 * or call Graphite/libc. No parent wires.
 */
uint32_t
gj_graphite_ok_u_11627(void)
{
	(void)NULL;
	return b11627_graphite_ok();
}
...
uint32_t __gj_graphite_ok_u_11627(void)
    __attribute__((alias("gj_graphite_ok_u_11627")));
