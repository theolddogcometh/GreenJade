 ...
 *
 * Desktop glibc graph batch11682: IPP ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_ipp_ok_u_11682(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       Internet Printing Protocol probe for the cups/print continuum.
 *   uint32_t __gj_ipp_ok_u_11682  (alias)
 *   __libcgj_batch11682_marker = "libcgj-batch11682"
 *
...
const char __libcgj_batch11682_marker[] = "libcgj-batch11682";

/* Soft IPP-ok lamp: always off (not a real IPP probe). */
#define B11682_IPP_OK  0u
...
b11682_ipp_ok(void)
{
	return B11682_IPP_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_ipp_ok_u_11682 - IPP ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not open IPP sockets or
 * call libc. No parent wires.
 */
uint32_t
gj_ipp_ok_u_11682(void)
{
	(void)NULL;
	return b11682_ipp_ok();
}
...
uint32_t __gj_ipp_ok_u_11682(void)
    __attribute__((alias("gj_ipp_ok_u_11682")));
