 ...
 *
 * Desktop glibc graph batch11683: print job ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_print_job_ok_u_11683(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       print-job / queue job probe for the cups/print continuum.
 *   uint32_t __gj_print_job_ok_u_11683  (alias)
 *   __libcgj_batch11683_marker = "libcgj-batch11683"
 *
...
const char __libcgj_batch11683_marker[] = "libcgj-batch11683";

/* Soft print-job-ok lamp: always off (not a real job probe). */
#define B11683_PRINT_JOB_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b11683_print_job_ok(void)
{
	return B11683_PRINT_JOB_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_print_job_ok_u_11683 - print job ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not inspect job queues or
 * call libc. No parent wires.
 */
uint32_t
gj_print_job_ok_u_11683(void)
{
	(void)NULL;
	return b11683_print_job_ok();
}
...
uint32_t __gj_print_job_ok_u_11683(void)
    __attribute__((alias("gj_print_job_ok_u_11683")));
