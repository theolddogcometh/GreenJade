 ...
 *
 * Desktop glibc graph batch11688: USB print ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_usb_print_ok_u_11688(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       USB printer / usblp probe for the cups/print continuum.
 *   uint32_t __gj_usb_print_ok_u_11688  (alias)
 *   __libcgj_batch11688_marker = "libcgj-batch11688"
 *
...
const char __libcgj_batch11688_marker[] = "libcgj-batch11688";

/* Soft USB-print-ok lamp: always off (not a real USB printer probe). */
#define B11688_USB_PRINT_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b11688_usb_print_ok(void)
{
	return B11688_USB_PRINT_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_usb_print_ok_u_11688 - USB print ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe usblp/sysfs or
 * call libc. No parent wires.
 */
uint32_t
gj_usb_print_ok_u_11688(void)
{
	(void)NULL;
	return b11688_usb_print_ok();
}
...
uint32_t __gj_usb_print_ok_u_11688(void)
    __attribute__((alias("gj_usb_print_ok_u_11688")));
