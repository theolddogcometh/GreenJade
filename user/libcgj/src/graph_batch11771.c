 ...
 *
 * Desktop glibc graph batch11771: power ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_power_ok_u_11771(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       power/sysfs probe for the power soft continuum.
 *   uint32_t __gj_power_ok_u_11771  (alias)
 *   __libcgj_batch11771_marker = "libcgj-batch11771"
 *
...
const char __libcgj_batch11771_marker[] = "libcgj-batch11771";

/* Soft power-ok lamp: always off (not a real power probe). */
#define B11771_POWER_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b11771_power_ok(void)
{
	return B11771_POWER_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_power_ok_u_11771 - power ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe power/sysfs or
 * call libc. No parent wires.
 */
uint32_t
gj_power_ok_u_11771(void)
{
	(void)NULL;
	return b11771_power_ok();
}
...
uint32_t __gj_power_ok_u_11771(void)
    __attribute__((alias("gj_power_ok_u_11771")));
