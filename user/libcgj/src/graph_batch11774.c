 ...
 *
 * Desktop glibc graph batch11774: thermal ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_thermal_ok_u_11774(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       thermal zone probe for the power soft continuum.
 *   uint32_t __gj_thermal_ok_u_11774  (alias)
 *   __libcgj_batch11774_marker = "libcgj-batch11774"
 *
...
const char __libcgj_batch11774_marker[] = "libcgj-batch11774";

/* Soft thermal-ok lamp: always off (not a real thermal probe). */
#define B11774_THERMAL_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b11774_thermal_ok(void)
{
	return B11774_THERMAL_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_thermal_ok_u_11774 - thermal ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe thermal_zone
 * or call libc. No parent wires.
 */
uint32_t
gj_thermal_ok_u_11774(void)
{
	(void)NULL;
	return b11774_thermal_ok();
}
...
uint32_t __gj_thermal_ok_u_11774(void)
    __attribute__((alias("gj_thermal_ok_u_11774")));
