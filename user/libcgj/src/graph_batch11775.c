 ...
 *
 * Desktop glibc graph batch11775: fan ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_fan_ok_u_11775(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       fan/tachometer probe for the power soft continuum.
 *   uint32_t __gj_fan_ok_u_11775  (alias)
 *   __libcgj_batch11775_marker = "libcgj-batch11775"
 *
...
const char __libcgj_batch11775_marker[] = "libcgj-batch11775";

/* Soft fan-ok lamp: always off (not a real fan probe). */
#define B11775_FAN_OK  0u
...
b11775_fan_ok(void)
{
	return B11775_FAN_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_fan_ok_u_11775 - fan ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe fan hwmon or
 * call libc. No parent wires.
 */
uint32_t
gj_fan_ok_u_11775(void)
{
	(void)NULL;
	return b11775_fan_ok();
}
...
uint32_t __gj_fan_ok_u_11775(void)
    __attribute__((alias("gj_fan_ok_u_11775")));
