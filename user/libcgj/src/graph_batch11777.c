 ...
 *
 * Desktop glibc graph batch11777: resume ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_resume_ok_u_11777(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       resume/wake probe for the power soft continuum.
 *   uint32_t __gj_resume_ok_u_11777  (alias)
 *   __libcgj_batch11777_marker = "libcgj-batch11777"
 *
...
const char __libcgj_batch11777_marker[] = "libcgj-batch11777";

/* Soft resume-ok lamp: always off (not a real resume probe). */
#define B11777_RESUME_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b11777_resume_ok(void)
{
	return B11777_RESUME_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_resume_ok_u_11777 - resume ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe wake sources
 * or call libc. No parent wires.
 */
uint32_t
gj_resume_ok_u_11777(void)
{
	(void)NULL;
	return b11777_resume_ok();
}
...
uint32_t __gj_resume_ok_u_11777(void)
    __attribute__((alias("gj_resume_ok_u_11777")));
