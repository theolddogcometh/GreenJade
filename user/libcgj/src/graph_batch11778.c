 ...
 *
 * Desktop glibc graph batch11778: performance ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_performance_ok_u_11778(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       performance/cpufreq probe for the power soft continuum.
 *   uint32_t __gj_performance_ok_u_11778  (alias)
 *   __libcgj_batch11778_marker = "libcgj-batch11778"
 *
...
const char __libcgj_batch11778_marker[] = "libcgj-batch11778";

/* Soft performance-ok lamp: always off (not a real perf probe). */
#define B11778_PERFORMANCE_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b11778_performance_ok(void)
{
	return B11778_PERFORMANCE_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_performance_ok_u_11778 - performance ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe performance
 * governors/cpufreq or call libc. No parent wires.
 */
uint32_t
gj_performance_ok_u_11778(void)
{
	(void)NULL;
	return b11778_performance_ok();
}
...
uint32_t __gj_performance_ok_u_11778(void)
    __attribute__((alias("gj_performance_ok_u_11778")));
