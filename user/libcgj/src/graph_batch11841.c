 ...
 *
 * Desktop glibc graph batch11841: dbus ok soft unit (not a probe).
 *
 * Surface (unique symbols):
 *   uint32_t gj_dbus_ok_u_11841(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       D-Bus runtime probe for the dbus soft continuum.
 *   uint32_t __gj_dbus_ok_u_11841  (alias)
 *   __libcgj_batch11841_marker = "libcgj-batch11841"
 *
...
const char __libcgj_batch11841_marker[] = "libcgj-batch11841";

/* Soft dbus-ok lamp: always off (not a real runtime probe). */
#define B11841_DBUS_OK  0u
...
b11841_ok(void)
{
	return B11841_DBUS_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_dbus_ok_u_11841 - dbus ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe D-Bus bus
 * presence or call libc. No parent wires.
 */
uint32_t
gj_dbus_ok_u_11841(void)
{
	(void)NULL;
	return b11841_ok();
}
...
uint32_t __gj_dbus_ok_u_11841(void)
    __attribute__((alias("gj_dbus_ok_u_11841")));
