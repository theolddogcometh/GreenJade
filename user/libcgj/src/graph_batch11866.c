 ...
 *
 * Desktop glibc graph batch11866: input soft hotplug-ok lamp (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_hotplug_ok_u_11866(void);
 *     - Always returns 0. Soft stub only; intentionally not a live
 *       input hotplug/monitor presence probe for the input soft
 *       continuum.
 *   uint32_t __gj_hotplug_ok_u_11866  (alias)
 *   __libcgj_batch11866_marker = "libcgj-batch11866"
 *
 ...
 * gj_hotplug_ok_u_11666 / gj_hotplug_ok_u_11466 prior waves. No parent
 * wires. No __int128.
 *
...
const char __libcgj_batch11866_marker[] = "libcgj-batch11866";

/* Soft hotplug-ok lamp: always off (not a real hotplug monitor). */
#define B11866_HOTPLUG_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b11866_hotplug_ok(void)
{
	return B11866_HOTPLUG_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_hotplug_ok_u_11866 - input soft hotplug-ok unit flag.
 *
 * Always returns 0. Soft continuum stub; does not watch netlink/uevent,
 * open monitors, or call libc. No parent wires.
 */
uint32_t
gj_hotplug_ok_u_11866(void)
{
	(void)NULL;
	return b11866_hotplug_ok();
}
...
uint32_t __gj_hotplug_ok_u_11866(void)
    __attribute__((alias("gj_hotplug_ok_u_11866")));
