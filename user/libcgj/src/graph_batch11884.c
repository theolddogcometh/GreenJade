 ...
 *
 * Desktop glibc graph batch11884: printer discover ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_printer_discover_ok_u_11884(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       printer discovery / browse probe for the cups/print continuum.
 *   uint32_t __gj_printer_discover_ok_u_11884  (alias)
 *   __libcgj_batch11884_marker = "libcgj-batch11884"
 *
...
const char __libcgj_batch11884_marker[] = "libcgj-batch11884";

/* Soft printer-discover-ok lamp: always off (not a real browse probe). */
#define B11884_PRINTER_DISCOVER_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b11884_printer_discover_ok(void)
{
	return B11884_PRINTER_DISCOVER_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_printer_discover_ok_u_11884 - printer discover ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not browse mDNS/IPP or
 * call libc. No parent wires.
 */
uint32_t
gj_printer_discover_ok_u_11884(void)
{
	(void)NULL;
	return b11884_printer_discover_ok();
}
...
uint32_t __gj_printer_discover_ok_u_11884(void)
    __attribute__((alias("gj_printer_discover_ok_u_11884")));
