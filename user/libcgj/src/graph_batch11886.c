 ...
 *
 * Desktop glibc graph batch11886: filter ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_filter_ok_u_11886(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       CUPS filter / raster filter probe for the cups/print continuum.
 *   uint32_t __gj_filter_ok_u_11886  (alias)
 *   __libcgj_batch11886_marker = "libcgj-batch11886"
 *
 ...
 * gj_filter_ok_u_11686 (batch11686), gj_filter_ok_u_10186
...
const char __libcgj_batch11886_marker[] = "libcgj-batch11886";

/* Soft filter-ok lamp: always off (not a real filter probe). */
#define B11886_FILTER_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b11886_filter_ok(void)
{
	return B11886_FILTER_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_filter_ok_u_11886 - filter ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not run filters or call
 * libc. No parent wires.
 */
uint32_t
gj_filter_ok_u_11886(void)
{
	(void)NULL;
	return b11886_filter_ok();
}
...
uint32_t __gj_filter_ok_u_11886(void)
    __attribute__((alias("gj_filter_ok_u_11886")));
