 ...
 *
 * Desktop glibc graph batch11935: ALSA UCM soft-ok unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_alsa_ucm_ok_u_11935(void);
 *     - Always returns 0. Soft stub for pipewire/wireplumber soft-id
 *       continuum; ALSA Use Case Manager is intentionally not reported
 *       ok.
 *   uint32_t __gj_alsa_ucm_ok_u_11935  (alias)
 *   __libcgj_batch11935_marker = "libcgj-batch11935"
 *
...
const char __libcgj_batch11935_marker[] = "libcgj-batch11935";

/* Soft ALSA UCM-ok lamp: always off for this continuum. */
#define B11935_ALSA_UCM_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b11935_alsa_ucm_ok(void)
{
	return B11935_ALSA_UCM_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_alsa_ucm_ok_u_11935 - ALSA UCM soft-ok unit flag.
 *
 * Always returns 0. Soft pure-data stub; does not open ALSA UCM or call
 * libc. No parent wires.
 */
uint32_t
gj_alsa_ucm_ok_u_11935(void)
{
	(void)NULL;
	return b11935_alsa_ucm_ok();
}
...
uint32_t __gj_alsa_ucm_ok_u_11935(void)
    __attribute__((alias("gj_alsa_ucm_ok_u_11935")));
