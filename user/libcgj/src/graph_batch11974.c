 ...
 *
 * Desktop glibc graph batch11974: thermal ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_thermal_ok_u_11974(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       thermal zone / temperature sensor probe for the power soft
 *       continuum.
 *   uint32_t __gj_thermal_ok_u_11974  (alias)
 *   __libcgj_batch11974_marker = "libcgj-batch11974"
 *
...
const char __libcgj_batch11974_marker[] = "libcgj-batch11974";

/* Soft thermal-ok lamp: always off (not a real thermal-zone probe). */
#define B11974_THERMAL_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b11974_thermal_ok(void)
{
return B11974_THERMAL_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_thermal_ok_u_11974 - thermal ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe thermal zones
 * or call libc. No parent wires.
 */
uint32_t
gj_thermal_ok_u_11974(void)
{
(void)NULL;
return b11974_thermal_ok();
}
...
uint32_t __gj_thermal_ok_u_11974(void)
    __attribute__((alias("gj_thermal_ok_u_11974")));
