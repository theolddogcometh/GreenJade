 ...
 *
 * Desktop glibc graph batch11978: performance ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_performance_ok_u_11978(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       performance / CPU-freq / power-profile probe for the power soft
 *       continuum.
 *   uint32_t __gj_performance_ok_u_11978  (alias)
 *   __libcgj_batch11978_marker = "libcgj-batch11978"
 *
...
const char __libcgj_batch11978_marker[] = "libcgj-batch11978";

/* Soft performance-ok lamp: always off (not a real power-profile probe). */
#define B11978_PERFORMANCE_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b11978_performance_ok(void)
{
return B11978_PERFORMANCE_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_performance_ok_u_11978 - performance ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe performance
 * governors or call libc. No parent wires.
 */
uint32_t
gj_performance_ok_u_11978(void)
{
(void)NULL;
return b11978_performance_ok();
}
...
uint32_t __gj_performance_ok_u_11978(void)
    __attribute__((alias("gj_performance_ok_u_11978")));
