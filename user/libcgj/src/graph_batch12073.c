 ...
 *
 * Desktop glibc graph batch12073: battery ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_battery_ok_u_12073(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       battery / fuel-gauge probe for the power soft continuum.
 *   uint32_t __gj_battery_ok_u_12073  (alias)
 *   __libcgj_batch12073_marker = "libcgj-batch12073"
 *
...
const char __libcgj_batch12073_marker[] = "libcgj-batch12073";

/* Soft battery-ok lamp: always off (not a real fuel-gauge probe). */
#define B12073_BATTERY_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b12073_battery_ok(void)
{
	return B12073_BATTERY_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_battery_ok_u_12073 - battery ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe battery state
 * or call libc. No parent wires.
 */
uint32_t
gj_battery_ok_u_12073(void)
{
	(void)NULL;
	return b12073_battery_ok();
}
...
uint32_t __gj_battery_ok_u_12073(void)
    __attribute__((alias("gj_battery_ok_u_12073")));
