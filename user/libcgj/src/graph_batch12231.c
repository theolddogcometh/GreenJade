 ...
 *
 * Desktop glibc graph batch12231: PipeWire soft-ok unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_pipewire_ok_u_12231(void);
 *     - Always returns 0. Soft stub for pipewire/wireplumber soft-id
 *       continuum; PipeWire is intentionally not reported ok.
 *   uint32_t __gj_pipewire_ok_u_12231  (alias)
 *   __libcgj_batch12231_marker = "libcgj-batch12231"
 *
 ...
 * gj_pipewire_ok_u_11931 (batch11931), gj_pipewire_ok_u_10931
 * (batch10931). No parent wires. No __int128.
 *
...
const char __libcgj_batch12231_marker[] = "libcgj-batch12231";

/* Soft PipeWire-ok lamp: always off for this continuum. */
#define B12231_PIPEWIRE_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b12231_pipewire_ok(void)
{
	return B12231_PIPEWIRE_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_pipewire_ok_u_12231 - PipeWire soft-ok unit flag.
 *
 * Always returns 0. Soft pure-data stub; does not open PipeWire or call
 * libc. No parent wires.
 */
uint32_t
gj_pipewire_ok_u_12231(void)
{
	(void)NULL;
	return b12231_pipewire_ok();
}
...
uint32_t __gj_pipewire_ok_u_12231(void)
    __attribute__((alias("gj_pipewire_ok_u_12231")));
