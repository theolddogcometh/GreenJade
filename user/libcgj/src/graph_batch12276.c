 ...
 *
 * Desktop glibc graph batch12276: suspend ok soft unit (not a probe).
 *
 * Surface (unique symbols):
 *   uint32_t gj_suspend_ok_u_12276(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       system suspend / S3-S0ix capability probe for the power/thermal
 *       soft product continuum.
 *   uint32_t __gj_suspend_ok_u_12276  (alias)
 *   __libcgj_batch12276_marker = "libcgj-batch12276"
 *
...
const char __libcgj_batch12276_marker[] = "libcgj-batch12276";

/* Soft suspend-ok lamp: always off (not a real suspend capability probe). */
#define B12276_SUSPEND_OK  0u
...
b12276_ok(void)
{
	return B12276_SUSPEND_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_suspend_ok_u_12276 - suspend ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe suspend state
 * or call libc. No parent wires.
 */
uint32_t
gj_suspend_ok_u_12276(void)
{
	(void)NULL;
	return b12276_ok();
}
...
uint32_t __gj_suspend_ok_u_12276(void)
    __attribute__((alias("gj_suspend_ok_u_12276")));
