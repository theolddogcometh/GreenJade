 ...
 *
 * Desktop glibc graph batch12277: resume ok soft unit (not a probe).
 *
 * Surface (unique symbols):
 *   uint32_t gj_resume_ok_u_12277(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       system resume / wake-path capability probe for the power/thermal
 *       soft product continuum.
 *   uint32_t __gj_resume_ok_u_12277  (alias)
 *   __libcgj_batch12277_marker = "libcgj-batch12277"
 *
...
const char __libcgj_batch12277_marker[] = "libcgj-batch12277";

/* Soft resume-ok lamp: always off (not a real wake-path probe). */
#define B12277_RESUME_OK  0u
...
b12277_ok(void)
{
	return B12277_RESUME_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_resume_ok_u_12277 - resume ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe resume path
 * or call libc. No parent wires.
 */
uint32_t
gj_resume_ok_u_12277(void)
{
	(void)NULL;
	return b12277_ok();
}
...
uint32_t __gj_resume_ok_u_12277(void)
    __attribute__((alias("gj_resume_ok_u_12277")));
