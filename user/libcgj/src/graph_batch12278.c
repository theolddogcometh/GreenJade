 ...
 *
 * Desktop glibc graph batch12278: performance ok soft unit (not a probe).
 *
 * Surface (unique symbols):
 *   uint32_t gj_performance_ok_u_12278(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       performance / CPU-freq / power-profile runtime probe for the
 *       power/thermal soft product continuum.
 *   uint32_t __gj_performance_ok_u_12278  (alias)
 *   __libcgj_batch12278_marker = "libcgj-batch12278"
 *
...
const char __libcgj_batch12278_marker[] = "libcgj-batch12278";

/* Soft performance-ok lamp: always off (not a real power-profile probe). */
#define B12278_PERFORMANCE_OK  0u
...
b12278_ok(void)
{
	return B12278_PERFORMANCE_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_performance_ok_u_12278 - performance ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe performance
 * governors or call libc. No parent wires.
 */
uint32_t
gj_performance_ok_u_12278(void)
{
	(void)NULL;
	return b12278_ok();
}
...
uint32_t __gj_performance_ok_u_12278(void)
    __attribute__((alias("gj_performance_ok_u_12278")));
