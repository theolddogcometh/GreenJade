 ...
 *
 * Desktop glibc graph batch12367: device_db soft-id stub.
 *
 * Surface (unique symbols):
 *   uint32_t gj_device_db_ok_u_12367(void);
 ...
 *   uint32_t __gj_device_db_ok_u_12367  (alias)
 *   __libcgj_batch12367_marker = "libcgj-batch12367"
 *
 ...
 * Unique gj_device_db_ok_u_12367 surface only; no multi-def. Soft stub —
...
const char __libcgj_batch12367_marker[] = "libcgj-batch12367";

/* Soft unavailable / not-detected device_db lamp. */
#define B12367_DEVICE_DB_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b12367_device_db_ok(void)
{
	return B12367_DEVICE_DB_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_device_db_ok_u_12367 - device_db soft-id lamp.
 *
 * Always returns 0. Soft pure-data device database presence tag. Does
 * not probe udev db or call libc. No parent wires.
 */
uint32_t
gj_device_db_ok_u_12367(void)
{
	(void)NULL;
	return b12367_device_db_ok();
}
...
uint32_t __gj_device_db_ok_u_12367(void)
    __attribute__((alias("gj_device_db_ok_u_12367")));
