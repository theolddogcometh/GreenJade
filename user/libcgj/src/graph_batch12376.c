 ...
 *
 * Desktop glibc graph batch12376: ConnMan soft-ok unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_connman_ok_u_12376(void);
 *     - Always returns 0. Soft stub for bluez/networkmanager soft-id
 *       continuum; ConnMan path is intentionally not reported ok.
 *   uint32_t __gj_connman_ok_u_12376  (alias)
 *   __libcgj_batch12376_marker = "libcgj-batch12376"
 *
...
const char __libcgj_batch12376_marker[] = "libcgj-batch12376";

/* Soft ConnMan-ok lamp: always off for this continuum. */
#define B12376_CONNMAN_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b12376_connman_ok(void)
{
	return B12376_CONNMAN_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_connman_ok_u_12376 - ConnMan soft-ok unit flag.
 *
 * Always returns 0. Soft pure-data stub; does not open ConnMan or call
 * libc. No parent wires.
 */
uint32_t
gj_connman_ok_u_12376(void)
{
	(void)NULL;
	return b12376_connman_ok();
}
...
uint32_t __gj_connman_ok_u_12376(void)
    __attribute__((alias("gj_connman_ok_u_12376")));
