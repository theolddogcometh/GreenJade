 ...
 *
 * Desktop glibc graph batch12381: CUPS ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_cups_ok_u_12381(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       cups/cupsd probe for the cups/print continuum.
 *   uint32_t __gj_cups_ok_u_12381  (alias)
 *   __libcgj_batch12381_marker = "libcgj-batch12381"
 *
...
const char __libcgj_batch12381_marker[] = "libcgj-batch12381";

/* Soft CUPS-ok lamp: always off (not a real cups probe). */
#define B12381_CUPS_OK  0u
...
b12381_cups_ok(void)
{
	return B12381_CUPS_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_cups_ok_u_12381 - CUPS ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not inspect cupsd or
 * call libc. No parent wires.
 */
uint32_t
gj_cups_ok_u_12381(void)
{
	(void)NULL;
	return b12381_cups_ok();
}
...
uint32_t __gj_cups_ok_u_12381(void)
    __attribute__((alias("gj_cups_ok_u_12381")));
