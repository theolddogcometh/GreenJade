 ...
 *
 * Desktop glibc graph batch12387: spool ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_spool_ok_u_12387(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       print spool / queue directory probe for the cups/print
 *       continuum.
 *   uint32_t __gj_spool_ok_u_12387  (alias)
 *   __libcgj_batch12387_marker = "libcgj-batch12387"
 *
...
const char __libcgj_batch12387_marker[] = "libcgj-batch12387";

/* Soft spool-ok lamp: always off (not a real spool probe). */
#define B12387_SPOOL_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b12387_spool_ok(void)
{
	return B12387_SPOOL_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_spool_ok_u_12387 - spool ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not inspect /var/spool
 * or call libc. No parent wires.
 */
uint32_t
gj_spool_ok_u_12387(void)
{
	(void)NULL;
	return b12387_spool_ok();
}
...
uint32_t __gj_spool_ok_u_12387(void)
    __attribute__((alias("gj_spool_ok_u_12387")));
