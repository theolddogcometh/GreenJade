 ...
 *
 * Desktop glibc graph batch12425: Lavapipe soft id ok stub.
 *
 * Surface (unique symbols):
 *   uint32_t gj_lavapipe_ok_u_12425(void);
 *     - Returns 0 (Lavapipe soft id stub not asserted ready). Soft
 *       compile-time product tag; not a live lavapipe software ICD
 *       probe.
 *   uint32_t __gj_lavapipe_ok_u_12425  (alias)
 *   __libcgj_batch12425_marker = "libcgj-batch12425"
 *
 * Exclusive continuum CREATE-ONLY (12421-12430: mesa soft stubs —
 * all soft lamps →0; batch_id→12430). Unique gj_lavapipe_ok_u_12425
 * surface only; no multi-def. Distinct from gj_lavapipe_ok_u_12225,
...
const char __libcgj_batch12425_marker[] = "libcgj-batch12425";

/* Soft lamp: Lavapipe ok stub (not asserted). */
#define B12425_LAVAPIPE_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b12425_lavapipe_ok(void)
{
	return B12425_LAVAPIPE_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_lavapipe_ok_u_12425 - Lavapipe soft id ok stub.
 *
...
uint32_t __gj_lavapipe_ok_u_12425(void)
    __attribute__((alias("gj_lavapipe_ok_u_12425")));
