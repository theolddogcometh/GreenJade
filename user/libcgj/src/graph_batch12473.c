 ...
 *
 * Desktop glibc graph batch12473: battery ok soft unit (not a probe).
 *
 * Surface (unique symbols):
 *   uint32_t gj_battery_ok_u_12473(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       battery / fuel-gauge runtime probe for the power/thermal soft
 *       product continuum.
 *   uint32_t __gj_battery_ok_u_12473  (alias)
 *   __libcgj_batch12473_marker = "libcgj-batch12473"
 *
...
const char __libcgj_batch12473_marker[] = "libcgj-batch12473";

/* Soft battery-ok lamp: always off (not a real fuel-gauge probe). */
#define B12473_BATTERY_OK  0u
...
b12473_ok(void)
{
	return B12473_BATTERY_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_battery_ok_u_12473 - battery ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe battery state
 * or call libc. No parent wires.
 */
uint32_t
gj_battery_ok_u_12473(void)
{
	(void)NULL;
	return b12473_ok();
}
...
uint32_t __gj_battery_ok_u_12473(void)
    __attribute__((alias("gj_battery_ok_u_12473")));
