 ...
 *
 * Desktop glibc graph batch12478: performance ok soft unit (not a probe).
 *
 * Surface (unique symbols):
 *   uint32_t gj_performance_ok_u_12478(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       performance / CPU-freq / power-profile runtime probe for the
 *       power/thermal soft product continuum.
 *   uint32_t __gj_performance_ok_u_12478  (alias)
 *   __libcgj_batch12478_marker = "libcgj-batch12478"
 *
...
const char __libcgj_batch12478_marker[] = "libcgj-batch12478";

/* Soft performance-ok lamp: always off (not a real power-profile probe). */
#define B12478_PERFORMANCE_OK  0u
...
b12478_ok(void)
{
	return B12478_PERFORMANCE_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_performance_ok_u_12478 - performance ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe performance
 * governors or call libc. No parent wires.
 */
uint32_t
gj_performance_ok_u_12478(void)
{
	(void)NULL;
	return b12478_ok();
}
...
uint32_t __gj_performance_ok_u_12478(void)
    __attribute__((alias("gj_performance_ok_u_12478")));
