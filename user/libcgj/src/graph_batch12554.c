 ...
 *
 * Desktop glibc graph batch12554: PAM soft id ok stub.
 *
 * Surface (unique symbols):
 *   uint32_t gj_pam_ok_u_12554(void);
 *     - Returns 0 (PAM soft id stub not asserted ready). Soft
 *       compile-time product tag; not a live libpam probe.
 *   uint32_t __gj_pam_ok_u_12554  (alias)
 *   __libcgj_batch12554_marker = "libcgj-batch12554"
 *
 ...
 * batch_id_12560). Unique gj_pam_ok_u_12554 surface only; no
...
const char __libcgj_batch12554_marker[] = "libcgj-batch12554";

/* Soft lamp: not asserted. */
#define B12554_PAM_OK  0u
...
b12554_pam_ok(void)
{
	return B12554_PAM_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_pam_ok_u_12554 - PAM soft id ok stub.
 *
 * Always returns 0. Soft pure-data lamp; does not open PAM stacks,
 * call pam_start, or probe authentication modules. Does not call
 * libc. No parent wires.
 */
uint32_t
gj_pam_ok_u_12554(void)
{
	(void)NULL;
	return b12554_pam_ok();
}
...
uint32_t __gj_pam_ok_u_12554(void)
    __attribute__((alias("gj_pam_ok_u_12554")));
