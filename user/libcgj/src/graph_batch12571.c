 ...
 *
 * Desktop glibc graph batch12571: BlueZ soft-ok unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_bluez_ok_u_12571(void);
 *     - Always returns 0. Soft stub for bluez/networkmanager soft-id
 *       continuum; BlueZ path is intentionally not reported ok.
 *   uint32_t __gj_bluez_ok_u_12571  (alias)
 *   __libcgj_batch12571_marker = "libcgj-batch12571"
 *
...
const char __libcgj_batch12571_marker[] = "libcgj-batch12571";

/* Soft BlueZ-ok lamp: always off for this continuum. */
#define B12571_BLUEZ_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b12571_bluez_ok(void)
{
	return B12571_BLUEZ_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_bluez_ok_u_12571 - BlueZ soft-ok unit flag.
 *
 * Always returns 0. Soft pure-data stub; does not open BlueZ or call
 * libc. No parent wires.
 */
uint32_t
gj_bluez_ok_u_12571(void)
{
	(void)NULL;
	return b12571_bluez_ok();
}
...
uint32_t __gj_bluez_ok_u_12571(void)
    __attribute__((alias("gj_bluez_ok_u_12571")));
