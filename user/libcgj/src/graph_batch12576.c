 ...
 *
 * Desktop glibc graph batch12576: ConnMan soft-ok unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_connman_ok_u_12576(void);
 *     - Always returns 0. Soft stub for bluez/networkmanager soft-id
 *       continuum; ConnMan path is intentionally not reported ok.
 *   uint32_t __gj_connman_ok_u_12576  (alias)
 *   __libcgj_batch12576_marker = "libcgj-batch12576"
 *
...
const char __libcgj_batch12576_marker[] = "libcgj-batch12576";

/* Soft ConnMan-ok lamp: always off for this continuum. */
#define B12576_CONNMAN_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b12576_connman_ok(void)
{
	return B12576_CONNMAN_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_connman_ok_u_12576 - ConnMan soft-ok unit flag.
 *
 * Always returns 0. Soft pure-data stub; does not open ConnMan or call
 * libc. No parent wires.
 */
uint32_t
gj_connman_ok_u_12576(void)
{
	(void)NULL;
	return b12576_connman_ok();
}
...
uint32_t __gj_connman_ok_u_12576(void)
    __attribute__((alias("gj_connman_ok_u_12576")));
