 ...
 *
 * Desktop glibc graph batch12584: printer discover ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_printer_discover_ok_u_12584(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       printer discovery / browse probe for the cups/print continuum.
 *   uint32_t __gj_printer_discover_ok_u_12584  (alias)
 *   __libcgj_batch12584_marker = "libcgj-batch12584"
 *
...
const char __libcgj_batch12584_marker[] = "libcgj-batch12584";

/* Soft printer-discover-ok lamp: always off (not a real browse probe). */
#define B12584_PRINTER_DISCOVER_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b12584_printer_discover_ok(void)
{
	return B12584_PRINTER_DISCOVER_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_printer_discover_ok_u_12584 - printer discover ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not browse mDNS/IPP or
 * call libc. No parent wires.
 */
uint32_t
gj_printer_discover_ok_u_12584(void)
{
	(void)NULL;
	return b12584_printer_discover_ok();
}
...
uint32_t __gj_printer_discover_ok_u_12584(void)
    __attribute__((alias("gj_printer_discover_ok_u_12584")));
