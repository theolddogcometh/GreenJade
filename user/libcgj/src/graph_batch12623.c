 ...
 *
 * Desktop glibc graph batch12623: AMDVLK soft id ok stub.
 *
 * Surface (unique symbols):
 *   uint32_t gj_amdvlk_ok_u_12623(void);
 *     - Returns 0 (AMDVLK soft id stub not asserted ready). Soft
 *       compile-time product tag; not a live amdvlk/Vulkan ICD probe.
 *   uint32_t __gj_amdvlk_ok_u_12623  (alias)
 *   __libcgj_batch12623_marker = "libcgj-batch12623"
 *
 * Exclusive continuum CREATE-ONLY (12621-12630: mesa soft stubs —
 * all soft lamps →0; batch_id→12630). Unique gj_amdvlk_ok_u_12623
 * surface only; no multi-def. Distinct from gj_amdvlk_ok_u_12423,
...
const char __libcgj_batch12623_marker[] = "libcgj-batch12623";

/* Soft lamp: AMDVLK ok stub (not asserted). */
#define B12623_AMDVLK_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b12623_amdvlk_ok(void)
{
	return B12623_AMDVLK_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_amdvlk_ok_u_12623 - AMDVLK soft id ok stub.
 *
...
uint32_t __gj_amdvlk_ok_u_12623(void)
    __attribute__((alias("gj_amdvlk_ok_u_12623")));
