 ...
 *
 * Desktop glibc graph batch12671: power ok soft unit (not a probe).
 *
 * Surface (unique symbols):
 *   uint32_t gj_power_ok_u_12671(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       power-rail / power-domain runtime probe for the power/thermal
 *       soft product continuum.
 *   uint32_t __gj_power_ok_u_12671  (alias)
 *   __libcgj_batch12671_marker = "libcgj-batch12671"
 *
...
const char __libcgj_batch12671_marker[] = "libcgj-batch12671";

/* Soft power-ok lamp: always off (not a real power-domain probe). */
#define B12671_POWER_OK  0u
...
b12671_ok(void)
{
	return B12671_POWER_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_power_ok_u_12671 - power ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe power rails
 * or call libc. No parent wires.
 */
uint32_t
gj_power_ok_u_12671(void)
{
	(void)NULL;
	return b12671_ok();
}
...
uint32_t __gj_power_ok_u_12671(void)
    __attribute__((alias("gj_power_ok_u_12671")));
