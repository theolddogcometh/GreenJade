 ...
 *
 * Desktop glibc graph batch12678: performance ok soft unit (not a probe).
 *
 * Surface (unique symbols):
 *   uint32_t gj_performance_ok_u_12678(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       performance / CPU-freq / power-profile runtime probe for the
 *       power/thermal soft product continuum.
 *   uint32_t __gj_performance_ok_u_12678  (alias)
 *   __libcgj_batch12678_marker = "libcgj-batch12678"
 *
...
const char __libcgj_batch12678_marker[] = "libcgj-batch12678";

/* Soft performance-ok lamp: always off (not a real power-profile probe). */
#define B12678_PERFORMANCE_OK  0u
...
b12678_ok(void)
{
	return B12678_PERFORMANCE_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_performance_ok_u_12678 - performance ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe performance
 * governors or call libc. No parent wires.
 */
uint32_t
gj_performance_ok_u_12678(void)
{
	(void)NULL;
	return b12678_ok();
}
...
uint32_t __gj_performance_ok_u_12678(void)
    __attribute__((alias("gj_performance_ok_u_12678")));
