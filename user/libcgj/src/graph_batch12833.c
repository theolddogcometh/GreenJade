 ...
 *
 * Desktop glibc graph batch12833: PulseAudio compat ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_pulse_compat_ok_u_12833(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       PulseAudio compatibility layer probe for the audio soft continuum.
 *   uint32_t __gj_pulse_compat_ok_u_12833  (alias)
 *   __libcgj_batch12833_marker = "libcgj-batch12833"
 *
 ...
 * (batch12433), gj_pulse_compat_ok_u_12233 (batch12233),
 * gj_pulse_compat_ok_u_12033 (batch12033), gj_pulse_compat_ok_u_10233
...
const char __libcgj_batch12833_marker[] = "libcgj-batch12833";

/* Soft PulseAudio-compat-ok lamp: always off (not a real pulse probe). */
#define B12833_PULSE_COMPAT_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b12833_pulse_compat_ok(void)
{
	return B12833_PULSE_COMPAT_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_pulse_compat_ok_u_12833 - PulseAudio compat ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not open PulseAudio or
 * call libc. No parent wires.
 */
uint32_t
gj_pulse_compat_ok_u_12833(void)
{
	(void)NULL;
	return b12833_pulse_compat_ok();
}
...
uint32_t __gj_pulse_compat_ok_u_12833(void)
    __attribute__((alias("gj_pulse_compat_ok_u_12833")));
