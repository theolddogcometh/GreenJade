 ...
 *
 * Desktop glibc graph batch12955: shadow ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_shadow_ok_u_12955(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       shadow/passwd probe for the auth soft continuum.
 *   uint32_t __gj_shadow_ok_u_12955  (alias)
 *   __libcgj_batch12955_marker = "libcgj-batch12955"
 *
...
const char __libcgj_batch12955_marker[] = "libcgj-batch12955";

/* Soft shadow-ok lamp: always off (not a real shadow probe). */
#define B12955_SHADOW_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b12955_shadow_ok(void)
{
	return B12955_SHADOW_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_shadow_ok_u_12955 - shadow ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not open /etc/shadow or
 * call libc. No parent wires.
 */
uint32_t
gj_shadow_ok_u_12955(void)
{
	(void)NULL;
	return b12955_shadow_ok();
}
...
uint32_t __gj_shadow_ok_u_12955(void)
    __attribute__((alias("gj_shadow_ok_u_12955")));
