 ...
 *
 * Desktop glibc graph batch12956: sudo ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_sudo_ok_u_12956(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       sudo probe for the auth soft continuum.
 *   uint32_t __gj_sudo_ok_u_12956  (alias)
 *   __libcgj_batch12956_marker = "libcgj-batch12956"
 *
...
const char __libcgj_batch12956_marker[] = "libcgj-batch12956";

/* Soft sudo-ok lamp: always off (not a real sudo probe). */
#define B12956_SUDO_OK  0u
...
b12956_sudo_ok(void)
{
	return B12956_SUDO_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_sudo_ok_u_12956 - sudo ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not invoke sudo or call
 * libc. No parent wires.
 */
uint32_t
gj_sudo_ok_u_12956(void)
{
	(void)NULL;
	return b12956_sudo_ok();
}
...
uint32_t __gj_sudo_ok_u_12956(void)
    __attribute__((alias("gj_sudo_ok_u_12956")));
