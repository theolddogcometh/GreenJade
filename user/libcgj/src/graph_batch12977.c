 ...
 *
 * Desktop glibc graph batch12977: Avahi soft-ok unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_avahi_ok_u_12977(void);
 *     - Always returns 0. Soft stub for bluez/networkmanager soft-id
 *       continuum; Avahi path is intentionally not reported ok.
 *   uint32_t __gj_avahi_ok_u_12977  (alias)
 *   __libcgj_batch12977_marker = "libcgj-batch12977"
 *
...
const char __libcgj_batch12977_marker[] = "libcgj-batch12977";

/* Soft Avahi-ok lamp: always off for this continuum. */
#define B12977_AVAHI_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b12977_avahi_ok(void)
{
	return B12977_AVAHI_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_avahi_ok_u_12977 - Avahi soft-ok unit flag.
 *
 * Always returns 0. Soft pure-data stub; does not open Avahi or call
 * libc. No parent wires.
 */
uint32_t
gj_avahi_ok_u_12977(void)
{
	(void)NULL;
	return b12977_avahi_ok();
}
...
uint32_t __gj_avahi_ok_u_12977(void)
    __attribute__((alias("gj_avahi_ok_u_12977")));
