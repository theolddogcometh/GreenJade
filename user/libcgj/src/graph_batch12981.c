 ...
 *
 * Desktop glibc graph batch12981: CUPS ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_cups_ok_u_12981(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       cups/cupsd probe for the cups/print continuum.
 *   uint32_t __gj_cups_ok_u_12981  (alias)
 *   __libcgj_batch12981_marker = "libcgj-batch12981"
 *
...
const char __libcgj_batch12981_marker[] = "libcgj-batch12981";

/* Soft CUPS-ok lamp: always off (not a real cups probe). */
#define B12981_CUPS_OK  0u
...
b12981_cups_ok(void)
{
	return B12981_CUPS_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_cups_ok_u_12981 - CUPS ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not inspect cupsd or
 * call libc. No parent wires.
 */
uint32_t
gj_cups_ok_u_12981(void)
{
	(void)NULL;
	return b12981_cups_ok();
}
...
uint32_t __gj_cups_ok_u_12981(void)
    __attribute__((alias("gj_cups_ok_u_12981")));
