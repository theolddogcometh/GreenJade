 ...
 *
 * Desktop glibc graph batch12989: print soft continuum lamp (on).
 *
 * Surface (unique symbols):
 *   uint32_t gj_print_soft_ready_u_12989(void);
 ...
 *       ok.
 *   uint32_t __gj_print_soft_ready_u_12989  (alias)
 *   __libcgj_batch12989_marker = "libcgj-batch12989"
 *
 ...
 * batch_id_12990). Unique surface only; no multi-def. cups/ipp/
 * print_job/printer_discover/ppd/filter/spool/usb_print ok units
 * remain 0. Distinct from gj_print_soft_ready_u_12789 (batch12789),
...
const char __libcgj_batch12989_marker[] = "libcgj-batch12989";

/* Soft continuum lamp: surfaces present (soft ready only). */
#define B12989_SOFT_READY  1u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b12989_soft_ready(void)
{
	return B12989_SOFT_READY;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_print_soft_ready_u_12989 - print soft continuum ready lamp.
 *
 * Always returns 1. Soft pure-data product tag that exclusive wave
 * 12981-12990 surfaces are present. Does not claim cups/ipp/print_job/
 * printer_discover/ppd/filter/spool/usb_print ok and does not call
 * libc. No parent wires.
 */
uint32_t
gj_print_soft_ready_u_12989(void)
{
	(void)NULL;
	return b12989_soft_ready();
}
...
uint32_t __gj_print_soft_ready_u_12989(void)
    __attribute__((alias("gj_print_soft_ready_u_12989")));
