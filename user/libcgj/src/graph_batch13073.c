 ...
 *
 * Desktop glibc graph batch13073: battery ok soft unit (not a probe).
 *
 * Surface (unique symbols):
 *   uint32_t gj_battery_ok_u_13073(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       battery / fuel-gauge runtime probe for the power/thermal soft
 *       product continuum.
 *   uint32_t __gj_battery_ok_u_13073  (alias)
 *   __libcgj_batch13073_marker = "libcgj-batch13073"
 *
...
const char __libcgj_batch13073_marker[] = "libcgj-batch13073";

/* Soft battery-ok lamp: always off (not a real fuel-gauge probe). */
#define B13073_BATTERY_OK  0u
...
b13073_ok(void)
{
	return B13073_BATTERY_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_battery_ok_u_13073 - battery ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe battery state
 * or call libc. No parent wires.
 */
uint32_t
gj_battery_ok_u_13073(void)
{
	(void)NULL;
	return b13073_ok();
}
...
uint32_t __gj_battery_ok_u_13073(void)
    __attribute__((alias("gj_battery_ok_u_13073")));
