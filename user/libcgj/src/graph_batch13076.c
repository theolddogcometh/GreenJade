 ...
 *
 * Desktop glibc graph batch13076: suspend ok soft unit (not a probe).
 *
 * Surface (unique symbols):
 *   uint32_t gj_suspend_ok_u_13076(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       system suspend / S3-S0ix capability probe for the power/thermal
 *       soft product continuum.
 *   uint32_t __gj_suspend_ok_u_13076  (alias)
 *   __libcgj_batch13076_marker = "libcgj-batch13076"
 *
...
const char __libcgj_batch13076_marker[] = "libcgj-batch13076";

/* Soft suspend-ok lamp: always off (not a real suspend capability probe). */
#define B13076_SUSPEND_OK  0u
...
b13076_ok(void)
{
	return B13076_SUSPEND_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_suspend_ok_u_13076 - suspend ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe suspend state
 * or call libc. No parent wires.
 */
uint32_t
gj_suspend_ok_u_13076(void)
{
	(void)NULL;
	return b13076_ok();
}
...
uint32_t __gj_suspend_ok_u_13076(void)
    __attribute__((alias("gj_suspend_ok_u_13076")));
