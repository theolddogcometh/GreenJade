 ...
 *
 * Desktop glibc graph batch13161: udev soft ok stub.
 *
 * Surface (unique symbols):
 *   uint32_t gj_udev_ok_u_13161(void);
 *     - Returns 0 (soft stub: udev/device enumeration stack slot is
 *       soft/unprobed; not a runtime udev or sysfs probe).
 *   uint32_t __gj_udev_ok_u_13161  (alias)
 *   __libcgj_batch13161_marker = "libcgj-batch13161"
 *
 ...
 * batch_id_13170). Unique gj_udev_ok_u_13161 surface only; no multi-def.
...
const char __libcgj_batch13161_marker[] = "libcgj-batch13161";

/* Soft-stub udev ok value (unprobed). */
#define B13161_UDEV_OK  0u
...
b13161_udev_ok(void)
{
	return B13161_UDEV_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_udev_ok_u_13161 - soft udev/device enumeration ok stub.
 *
 * Always returns 0 (soft/unprobed udev slot). Does not call libc or
 * probe sysfs/udev. No parent wires.
 */
uint32_t
gj_udev_ok_u_13161(void)
{
	(void)NULL;
	return b13161_udev_ok();
}
...
uint32_t __gj_udev_ok_u_13161(void)
    __attribute__((alias("gj_udev_ok_u_13161")));
