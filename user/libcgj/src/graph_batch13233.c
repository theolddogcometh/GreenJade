 ...
 *
 * Desktop glibc graph batch13233: PulseAudio compat ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_pulse_compat_ok_u_13233(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       PulseAudio compatibility layer probe for the audio soft continuum.
 *   uint32_t __gj_pulse_compat_ok_u_13233  (alias)
 *   __libcgj_batch13233_marker = "libcgj-batch13233"
 *
 ...
 * (batch12233), and gj_pulse_compat_ok_u_12033 (batch12033). No parent
...
const char __libcgj_batch13233_marker[] = "libcgj-batch13233";

/* Soft PulseAudio-compat-ok lamp: always off (not a real pulse probe). */
#define B13233_PULSE_COMPAT_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b13233_pulse_compat_ok(void)
{
	return B13233_PULSE_COMPAT_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_pulse_compat_ok_u_13233 - PulseAudio compat ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not open PulseAudio or
 * call libc. No parent wires.
 */
uint32_t
gj_pulse_compat_ok_u_13233(void)
{
	(void)NULL;
	return b13233_pulse_compat_ok();
}
...
uint32_t __gj_pulse_compat_ok_u_13233(void)
    __attribute__((alias("gj_pulse_compat_ok_u_13233")));
