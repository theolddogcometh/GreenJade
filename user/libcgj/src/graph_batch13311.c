 ...
 *
 * Desktop glibc graph batch13311: Flatpak ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_flatpak_ok_u_13311(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       flatpak / bubblewrap probe for the flatpak portal continuum.
 *   uint32_t __gj_flatpak_ok_u_13311  (alias)
 *   __libcgj_batch13311_marker = "libcgj-batch13311"
 *
...
const char __libcgj_batch13311_marker[] = "libcgj-batch13311";

/* Soft Flatpak-ok lamp: always off (not a real flatpak probe). */
#define B13311_FLATPAK_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b13311_flatpak_ok(void)
{
	return B13311_FLATPAK_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_flatpak_ok_u_13311 - Flatpak ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not inspect flatpakd or
 * call libc. No parent wires.
 */
uint32_t
gj_flatpak_ok_u_13311(void)
{
	(void)NULL;
	return b13311_flatpak_ok();
}
...
uint32_t __gj_flatpak_ok_u_13311(void)
    __attribute__((alias("gj_flatpak_ok_u_13311")));
