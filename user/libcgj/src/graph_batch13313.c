 ...
 *
 * Desktop glibc graph batch13313: Documents portal ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_documents_ok_u_13313(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       Documents portal / document-store probe for the flatpak portal
 *       continuum.
 *   uint32_t __gj_documents_ok_u_13313  (alias)
 *   __libcgj_batch13313_marker = "libcgj-batch13313"
 *
 ...
 * Documents portal implementation.
 *
...
const char __libcgj_batch13313_marker[] = "libcgj-batch13313";

/* Soft Documents-ok lamp: always off (not a real documents probe). */
#define B13313_DOCUMENTS_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b13313_documents_ok(void)
{
	return B13313_DOCUMENTS_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_documents_ok_u_13313 - Documents portal ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not inspect the
 * Documents portal or call libc. No parent wires.
 */
uint32_t
gj_documents_ok_u_13313(void)
{
	(void)NULL;
	return b13313_documents_ok();
}
...
uint32_t __gj_documents_ok_u_13313(void)
    __attribute__((alias("gj_documents_ok_u_13313")));
