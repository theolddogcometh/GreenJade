 ...
 *
 * Desktop glibc graph batch13315: OpenURI portal ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_open_uri_ok_u_13315(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       OpenURI portal probe for the flatpak portal continuum.
 *   uint32_t __gj_open_uri_ok_u_13315  (alias)
 *   __libcgj_batch13315_marker = "libcgj-batch13315"
 *
...
const char __libcgj_batch13315_marker[] = "libcgj-batch13315";

/* Soft OpenURI-ok lamp: always off (not a real OpenURI probe). */
#define B13315_OPEN_URI_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b13315_open_uri_ok(void)
{
	return B13315_OPEN_URI_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_open_uri_ok_u_13315 - OpenURI portal ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not inspect the OpenURI
 * portal or call libc. No parent wires.
 */
uint32_t
gj_open_uri_ok_u_13315(void)
{
	(void)NULL;
	return b13315_open_uri_ok();
}
...
uint32_t __gj_open_uri_ok_u_13315(void)
    __attribute__((alias("gj_open_uri_ok_u_13315")));
