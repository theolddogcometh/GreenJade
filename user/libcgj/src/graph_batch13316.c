 ...
 *
 * Desktop glibc graph batch13316: Screenshot portal ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_screenshot_ok_u_13316(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       Screenshot portal probe for the flatpak portal continuum.
 *   uint32_t __gj_screenshot_ok_u_13316  (alias)
 *   __libcgj_batch13316_marker = "libcgj-batch13316"
 *
...
const char __libcgj_batch13316_marker[] = "libcgj-batch13316";

/* Soft Screenshot-ok lamp: always off (not a real screenshot probe). */
#define B13316_SCREENSHOT_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b13316_screenshot_ok(void)
{
	return B13316_SCREENSHOT_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_screenshot_ok_u_13316 - Screenshot portal ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not inspect the
 * Screenshot portal or call libc. No parent wires.
 */
uint32_t
gj_screenshot_ok_u_13316(void)
{
	(void)NULL;
	return b13316_screenshot_ok();
}
...
uint32_t __gj_screenshot_ok_u_13316(void)
    __attribute__((alias("gj_screenshot_ok_u_13316")));
