 ...
 *
 * Desktop glibc graph batch13317: Notification portal ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_notification_ok_u_13317(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       Notification portal probe for the flatpak portal continuum.
 *   uint32_t __gj_notification_ok_u_13317  (alias)
 *   __libcgj_batch13317_marker = "libcgj-batch13317"
 *
...
const char __libcgj_batch13317_marker[] = "libcgj-batch13317";

/* Soft Notification-ok lamp: always off (not a real notify probe). */
#define B13317_NOTIFICATION_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b13317_notification_ok(void)
{
	return B13317_NOTIFICATION_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_notification_ok_u_13317 - Notification portal ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not inspect the
 * Notification portal or call libc. No parent wires.
 */
uint32_t
gj_notification_ok_u_13317(void)
{
	(void)NULL;
	return b13317_notification_ok();
}
...
uint32_t __gj_notification_ok_u_13317(void)
    __attribute__((alias("gj_notification_ok_u_13317")));
