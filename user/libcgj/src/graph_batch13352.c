 ...
 *
 * Desktop glibc graph batch13352: seatd soft id ok stub.
 *
 * Surface (unique symbols):
 *   uint32_t gj_seatd_ok_u_13352(void);
 *     - Returns 0 (seatd soft id stub not asserted ready). Soft
 *       compile-time product tag; not a live seatd/libseat probe.
 *   uint32_t __gj_seatd_ok_u_13352  (alias)
 *   __libcgj_batch13352_marker = "libcgj-batch13352"
 *
 ...
 * batch_id_13360). Unique gj_seatd_ok_u_13352 surface only; no
...
const char __libcgj_batch13352_marker[] = "libcgj-batch13352";

/* Soft lamp: not asserted. */
#define B13352_SEATD_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b13352_seatd_ok(void)
{
	return B13352_SEATD_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_seatd_ok_u_13352 - seatd soft id ok stub.
 *
...
uint32_t __gj_seatd_ok_u_13352(void)
    __attribute__((alias("gj_seatd_ok_u_13352")));
