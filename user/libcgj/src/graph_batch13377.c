 ...
 *
 * Desktop glibc graph batch13377: Avahi soft-ok unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_avahi_ok_u_13377(void);
 *     - Always returns 0. Soft stub for bluez/networkmanager soft-id
 *       continuum; Avahi path is intentionally not reported ok.
 *   uint32_t __gj_avahi_ok_u_13377  (alias)
 *   __libcgj_batch13377_marker = "libcgj-batch13377"
 *
...
const char __libcgj_batch13377_marker[] = "libcgj-batch13377";

/* Soft Avahi-ok lamp: always off for this continuum. */
#define B13377_AVAHI_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b13377_avahi_ok(void)
{
	return B13377_AVAHI_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_avahi_ok_u_13377 - Avahi soft-ok unit flag.
 *
 * Always returns 0. Soft pure-data stub; does not open Avahi or call
 * libc. No parent wires.
 */
uint32_t
gj_avahi_ok_u_13377(void)
{
	(void)NULL;
	return b13377_avahi_ok();
}
...
uint32_t __gj_avahi_ok_u_13377(void)
    __attribute__((alias("gj_avahi_ok_u_13377")));
