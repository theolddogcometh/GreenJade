 ...
 *
 * Desktop glibc graph batch13425: amdgpu ok soft unit (not a probe).
 *
 * Surface (unique symbols):
 *   uint32_t gj_amdgpu_ok_u_13425(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       amdgpu DRM/kernel driver probe for the mesa soft continuum.
 *   uint32_t __gj_amdgpu_ok_u_13425  (alias)
 *   __libcgj_batch13425_marker = "libcgj-batch13425"
 *
 ...
 * gj_amdgpu_ok_u_10525, gj_amdgpu_ok_u_10325, gj_amdgpu_ok_u_10225 and
...
const char __libcgj_batch13425_marker[] = "libcgj-batch13425";

/* Soft amdgpu-ok lamp: always off (not a real runtime probe). */
#define B13425_AMDGPU_OK  0u
...
b13425_ok(void)
{
	return B13425_AMDGPU_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_amdgpu_ok_u_13425 - amdgpu ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not open DRM nodes or
 * call libc. No parent wires.
 */
uint32_t
gj_amdgpu_ok_u_13425(void)
{
	(void)NULL;
	return b13425_ok();
}
...
uint32_t __gj_amdgpu_ok_u_13425(void)
    __attribute__((alias("gj_amdgpu_ok_u_13425")));
