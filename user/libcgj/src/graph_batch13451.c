 ...
 *
 * Desktop glibc graph batch13451: controller ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_controller_ok_u_13451(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       controller/input probe for the controller/input continuum.
 *   uint32_t __gj_controller_ok_u_13451  (alias)
 *   __libcgj_batch13451_marker = "libcgj-batch13451"
 *
...
const char __libcgj_batch13451_marker[] = "libcgj-batch13451";

/* Soft controller-ok lamp: always off (not a real controller probe). */
#define B13451_CONTROLLER_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b13451_controller_ok(void)
{
	return B13451_CONTROLLER_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_controller_ok_u_13451 - controller ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not open devices or
 * call libc. No parent wires.
 */
uint32_t
gj_controller_ok_u_13451(void)
{
	(void)NULL;
	return b13451_controller_ok();
}
...
uint32_t __gj_controller_ok_u_13451(void)
    __attribute__((alias("gj_controller_ok_u_13451")));
