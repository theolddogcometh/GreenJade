 ...
 *
 * Desktop glibc graph batch13455: XInput ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_xinput_ok_u_13455(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       XInput probe for the controller/input continuum.
 *   uint32_t __gj_xinput_ok_u_13455  (alias)
 *   __libcgj_batch13455_marker = "libcgj-batch13455"
 *
...
const char __libcgj_batch13455_marker[] = "libcgj-batch13455";

/* Soft XInput-ok lamp: always off (not a real XInput probe). */
#define B13455_XINPUT_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b13455_xinput_ok(void)
{
	return B13455_XINPUT_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_xinput_ok_u_13455 - XInput ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not open XInput paths or
 * call libc. No parent wires.
 */
uint32_t
gj_xinput_ok_u_13455(void)
{
	(void)NULL;
	return b13455_xinput_ok();
}
...
uint32_t __gj_xinput_ok_u_13455(void)
    __attribute__((alias("gj_xinput_ok_u_13455")));
