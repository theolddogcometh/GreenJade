 ...
 *
 * Desktop glibc graph batch13456: evdev ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_evdev_ok_u_13456(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       Linux evdev probe for the controller/input continuum.
 *   uint32_t __gj_evdev_ok_u_13456  (alias)
 *   __libcgj_batch13456_marker = "libcgj-batch13456"
 *
...
const char __libcgj_batch13456_marker[] = "libcgj-batch13456";

/* Soft evdev-ok lamp: always off (not a real /dev/input probe). */
#define B13456_EVDEV_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b13456_evdev_ok(void)
{
	return B13456_EVDEV_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_evdev_ok_u_13456 - evdev ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not open /dev/input or
 * call libc. No parent wires.
 */
uint32_t
gj_evdev_ok_u_13456(void)
{
	(void)NULL;
	return b13456_evdev_ok();
}
...
uint32_t __gj_evdev_ok_u_13456(void)
    __attribute__((alias("gj_evdev_ok_u_13456")));
