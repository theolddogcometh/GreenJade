 ...
 *
 * Desktop glibc graph batch13472: tdp ok soft unit (not a probe).
 *
 * Surface (unique symbols):
 *   uint32_t gj_tdp_ok_u_13472(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       TDP / thermal design power configuration probe for the
 *       power/thermal soft product continuum.
 *   uint32_t __gj_tdp_ok_u_13472  (alias)
 *   __libcgj_batch13472_marker = "libcgj-batch13472"
 *
...
const char __libcgj_batch13472_marker[] = "libcgj-batch13472";

/* Soft tdp-ok lamp: always off (not a real TDP configuration probe). */
#define B13472_TDP_OK  0u
...
b13472_ok(void)
{
	return B13472_TDP_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_tdp_ok_u_13472 - tdp ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe TDP limits
 * or call libc. No parent wires.
 */
uint32_t
gj_tdp_ok_u_13472(void)
{
	(void)NULL;
	return b13472_ok();
}
...
uint32_t __gj_tdp_ok_u_13472(void)
    __attribute__((alias("gj_tdp_ok_u_13472")));
