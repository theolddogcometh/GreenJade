 ...
 *
 * Desktop glibc graph batch13478: performance ok soft unit (not a probe).
 *
 * Surface (unique symbols):
 *   uint32_t gj_performance_ok_u_13478(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       performance / CPU-freq / power-profile runtime probe for the
 *       power/thermal soft product continuum.
 *   uint32_t __gj_performance_ok_u_13478  (alias)
 *   __libcgj_batch13478_marker = "libcgj-batch13478"
 *
...
const char __libcgj_batch13478_marker[] = "libcgj-batch13478";

/* Soft performance-ok lamp: always off (not a real power-profile probe). */
#define B13478_PERFORMANCE_OK  0u
...
b13478_ok(void)
{
	return B13478_PERFORMANCE_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_performance_ok_u_13478 - performance ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe performance
 * governors or call libc. No parent wires.
 */
uint32_t
gj_performance_ok_u_13478(void)
{
	(void)NULL;
	return b13478_ok();
}
...
uint32_t __gj_performance_ok_u_13478(void)
    __attribute__((alias("gj_performance_ok_u_13478")));
