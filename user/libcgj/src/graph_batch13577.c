 ...
 *
 * Desktop glibc graph batch13577: Avahi soft-ok unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_avahi_ok_u_13577(void);
 *     - Always returns 0. Soft stub for bluez/networkmanager soft-id
 *       continuum; Avahi path is intentionally not reported ok.
 *   uint32_t __gj_avahi_ok_u_13577  (alias)
 *   __libcgj_batch13577_marker = "libcgj-batch13577"
 *
...
const char __libcgj_batch13577_marker[] = "libcgj-batch13577";

/* Soft Avahi-ok lamp: always off for this continuum. */
#define B13577_AVAHI_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b13577_avahi_ok(void)
{
	return B13577_AVAHI_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_avahi_ok_u_13577 - Avahi soft-ok unit flag.
 *
 * Always returns 0. Soft pure-data stub; does not open Avahi or call
 * libc. No parent wires.
 */
uint32_t
gj_avahi_ok_u_13577(void)
{
	(void)NULL;
	return b13577_avahi_ok();
}
...
uint32_t __gj_avahi_ok_u_13577(void)
    __attribute__((alias("gj_avahi_ok_u_13577")));
