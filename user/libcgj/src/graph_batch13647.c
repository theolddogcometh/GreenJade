 ...
 *
 * Desktop glibc graph batch13647: fps_limit ok soft unit (not a probe).
 *
 * Surface (unique symbols):
 *   uint32_t gj_fps_limit_ok_u_13647(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       FPS limiter configuration probe for the bar3-related continuum.
 *   uint32_t __gj_fps_limit_ok_u_13647  (alias)
 *   __libcgj_batch13647_marker = "libcgj-batch13647"
 *
...
const char __libcgj_batch13647_marker[] = "libcgj-batch13647";

/* Soft fps-limit-ok lamp: always off (not a real limiter probe). */
#define B13647_FPS_LIMIT_OK  0u
...
b13647_ok(void)
{
	return B13647_FPS_LIMIT_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_fps_limit_ok_u_13647 - fps_limit ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe FPS limiter
 * configuration or call libc. No parent wires.
 */
uint32_t
gj_fps_limit_ok_u_13647(void)
{
	(void)NULL;
	return b13647_ok();
}
...
uint32_t __gj_fps_limit_ok_u_13647(void)
    __attribute__((alias("gj_fps_limit_ok_u_13647")));
