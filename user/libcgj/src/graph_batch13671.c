 ...
 *
 * Desktop glibc graph batch13671: power ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_power_ok_u_13671(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       power/sysfs probe for the power soft continuum.
 *   uint32_t __gj_power_ok_u_13671  (alias)
 *   __libcgj_batch13671_marker = "libcgj-batch13671"
 *
...
const char __libcgj_batch13671_marker[] = "libcgj-batch13671";

/* Soft power-ok lamp: always off (not a real power probe). */
#define B13671_POWER_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b13671_power_ok(void)
{
	return B13671_POWER_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_power_ok_u_13671 - power ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe power/sysfs or
 * call libc. No parent wires.
 */
uint32_t
gj_power_ok_u_13671(void)
{
	(void)NULL;
	return b13671_power_ok();
}
...
uint32_t __gj_power_ok_u_13671(void)
    __attribute__((alias("gj_power_ok_u_13671")));
