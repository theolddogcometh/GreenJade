 ...
 *
 * Desktop glibc graph batch13678: performance ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_performance_ok_u_13678(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       performance/cpufreq probe for the power soft continuum.
 *   uint32_t __gj_performance_ok_u_13678  (alias)
 *   __libcgj_batch13678_marker = "libcgj-batch13678"
 *
...
const char __libcgj_batch13678_marker[] = "libcgj-batch13678";

/* Soft performance-ok lamp: always off (not a real perf probe). */
#define B13678_PERFORMANCE_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b13678_performance_ok(void)
{
	return B13678_PERFORMANCE_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_performance_ok_u_13678 - performance ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe performance
 * governors/cpufreq or call libc. No parent wires.
 */
uint32_t
gj_performance_ok_u_13678(void)
{
	(void)NULL;
	return b13678_performance_ok();
}
...
uint32_t __gj_performance_ok_u_13678(void)
    __attribute__((alias("gj_performance_ok_u_13678")));
