 ...
 *
 * Desktop glibc graph batch13873: battery ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_battery_ok_u_13873(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       battery/sysfs probe for the power soft continuum.
 *   uint32_t __gj_battery_ok_u_13873  (alias)
 *   __libcgj_batch13873_marker = "libcgj-batch13873"
 *
...
const char __libcgj_batch13873_marker[] = "libcgj-batch13873";

/* Soft battery-ok lamp: always off (not a real battery probe). */
#define B13873_BATTERY_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b13873_battery_ok(void)
{
	return B13873_BATTERY_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_battery_ok_u_13873 - battery ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe power_supply
 * or call libc. No parent wires.
 */
uint32_t
gj_battery_ok_u_13873(void)
{
	(void)NULL;
	return b13873_battery_ok();
}
...
uint32_t __gj_battery_ok_u_13873(void)
    __attribute__((alias("gj_battery_ok_u_13873")));
