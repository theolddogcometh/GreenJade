 ...
 *
 * Desktop glibc graph batch13875: fan ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_fan_ok_u_13875(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       fan/tachometer probe for the power soft continuum.
 *   uint32_t __gj_fan_ok_u_13875  (alias)
 *   __libcgj_batch13875_marker = "libcgj-batch13875"
 *
...
const char __libcgj_batch13875_marker[] = "libcgj-batch13875";

/* Soft fan-ok lamp: always off (not a real fan probe). */
#define B13875_FAN_OK  0u
...
b13875_fan_ok(void)
{
	return B13875_FAN_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_fan_ok_u_13875 - fan ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe fan hwmon or
 * call libc. No parent wires.
 */
uint32_t
gj_fan_ok_u_13875(void)
{
	(void)NULL;
	return b13875_fan_ok();
}
...
uint32_t __gj_fan_ok_u_13875(void)
    __attribute__((alias("gj_fan_ok_u_13875")));
