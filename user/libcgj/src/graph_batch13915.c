 ...
 *
 * Desktop glibc graph batch13915: file portal ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_file_portal_ok_u_13915(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       FileChooser portal probe for the flatpak/portal continuum.
 *   uint32_t __gj_file_portal_ok_u_13915  (alias)
 *   __libcgj_batch13915_marker = "libcgj-batch13915"
 *
...
const char __libcgj_batch13915_marker[] = "libcgj-batch13915";

/* Soft file-portal-ok lamp: always off (not a real portal probe). */
#define B13915_FILE_PORTAL_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b13915_file_portal_ok(void)
{
	return B13915_FILE_PORTAL_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_file_portal_ok_u_13915 - file portal ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe FileChooser
 * portal or call libc. No parent wires.
 */
uint32_t
gj_file_portal_ok_u_13915(void)
{
	(void)NULL;
	return b13915_file_portal_ok();
}
...
uint32_t __gj_file_portal_ok_u_13915(void)
    __attribute__((alias("gj_file_portal_ok_u_13915")));
