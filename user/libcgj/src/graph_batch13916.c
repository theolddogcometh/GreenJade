 ...
 *
 * Desktop glibc graph batch13916: secret portal ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_secret_portal_ok_u_13916(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       Secret portal probe for the flatpak/portal continuum.
 *   uint32_t __gj_secret_portal_ok_u_13916  (alias)
 *   __libcgj_batch13916_marker = "libcgj-batch13916"
 *
...
const char __libcgj_batch13916_marker[] = "libcgj-batch13916";

/* Soft secret-portal-ok lamp: always off (not a real portal probe). */
#define B13916_SECRET_PORTAL_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b13916_secret_portal_ok(void)
{
	return B13916_SECRET_PORTAL_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_secret_portal_ok_u_13916 - secret portal ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe Secret portal
 * or call libc. No parent wires.
 */
uint32_t
gj_secret_portal_ok_u_13916(void)
{
	(void)NULL;
	return b13916_secret_portal_ok();
}
...
uint32_t __gj_secret_portal_ok_u_13916(void)
    __attribute__((alias("gj_secret_portal_ok_u_13916")));
