 ...
 *
 * Desktop glibc graph batch13956: sudo soft id ok stub.
 *
 * Surface (unique symbols):
 *   uint32_t gj_sudo_ok_u_13956(void);
 *     - Returns 0 (sudo soft id stub not asserted ready). Soft
 *       compile-time product tag; not a live sudo/sudoers probe.
 *   uint32_t __gj_sudo_ok_u_13956  (alias)
 *   __libcgj_batch13956_marker = "libcgj-batch13956"
 *
 ...
 * batch_id_13960). Unique gj_sudo_ok_u_13956 surface only; no
...
const char __libcgj_batch13956_marker[] = "libcgj-batch13956";

/* Soft lamp: not asserted. */
#define B13956_SUDO_OK  0u
...
b13956_sudo_ok(void)
{
	return B13956_SUDO_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_sudo_ok_u_13956 - sudo soft id ok stub.
 *
...
uint32_t __gj_sudo_ok_u_13956(void)
    __attribute__((alias("gj_sudo_ok_u_13956")));
