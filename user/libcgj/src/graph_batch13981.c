 ...
 *
 * Desktop glibc graph batch13981: CUPS ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_cups_ok_u_13981(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       CUPS / cupsd availability probe.
 *   uint32_t __gj_cups_ok_u_13981  (alias)
 *   __libcgj_batch13981_marker = "libcgj-batch13981"
 *
...
const char __libcgj_batch13981_marker[] = "libcgj-batch13981";

/* Soft CUPS ok lamp: always off (not a real cupsd probe). */
#define B13981_CUPS_OK  0u
...
b13981_cups_ok(void)
{
	return B13981_CUPS_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_cups_ok_u_13981 - CUPS ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not contact cupsd or call
 * libc. No parent wires.
 */
uint32_t
gj_cups_ok_u_13981(void)
{
	(void)NULL;
	return b13981_cups_ok();
}
...
uint32_t __gj_cups_ok_u_13981(void)
    __attribute__((alias("gj_cups_ok_u_13981")));
