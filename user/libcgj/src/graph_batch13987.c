 ...
 *
 * Desktop glibc graph batch13987: queue ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_queue_ok_u_13987(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       print-queue validity probe.
 *   uint32_t __gj_queue_ok_u_13987  (alias)
 *   __libcgj_batch13987_marker = "libcgj-batch13987"
 *
...
const char __libcgj_batch13987_marker[] = "libcgj-batch13987";

/* Soft queue-ok lamp: always off (not a real queue probe). */
#define B13987_QUEUE_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b13987_queue_ok(void)
{
	return B13987_QUEUE_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_queue_ok_u_13987 - print queue ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not inspect queues or
 * call cupsd/libc. No parent wires.
 */
uint32_t
gj_queue_ok_u_13987(void)
{
	(void)NULL;
	return b13987_queue_ok();
}
...
uint32_t __gj_queue_ok_u_13987(void)
    __attribute__((alias("gj_queue_ok_u_13987")));
