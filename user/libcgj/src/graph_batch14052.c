 ...
 *
 * Desktop glibc graph batch14052: soft udev/libinput action-remove id.
 *
 * Surface (unique symbols):
 *   uint32_t gj_udev_li_action_remove_id_14052(void);
 *     - Returns 2 (udev "remove" action soft catalog id). Pure-data
 *       action id stub; does not watch devices or call libudev/libinput.
 *   uint32_t __gj_udev_li_action_remove_id_14052  (alias)
 *   __libcgj_batch14052_marker = "libcgj-batch14052"
 *
...
const char __libcgj_batch14052_marker[] = "libcgj-batch14052";

/* Soft udev "remove" action catalog id. */
#define B14052_UDEV_LI_ACTION_REMOVE  2u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b14052_action_remove_id(void)
{
	return B14052_UDEV_LI_ACTION_REMOVE;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_udev_li_action_remove_id_14052 - soft udev/libinput "remove" id.
 *
 * Always returns 2. Soft continuum constant; does not watch devices or
 * call libudev/libinput/libc. No parent wires.
 */
uint32_t
gj_udev_li_action_remove_id_14052(void)
{
	(void)NULL;
	return b14052_action_remove_id();
}

/* ---- underscored alias ------------------------------------------------- */

uint32_t __gj_udev_li_action_remove_id_14052(void)
    __attribute__((alias("gj_udev_li_action_remove_id_14052")));
