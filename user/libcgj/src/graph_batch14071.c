 ...
 *
 * Desktop glibc graph batch14071: CUPS ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_cups_ok_u_14071(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       CUPS / cupsd availability probe.
 *   uint32_t __gj_cups_ok_u_14071  (alias)
 *   __libcgj_batch14071_marker = "libcgj-batch14071"
 *
...
const char __libcgj_batch14071_marker[] = "libcgj-batch14071";

/* Soft CUPS ok lamp: always off (not a real cupsd probe). */
#define B14071_CUPS_OK  0u
...
b14071_cups_ok(void)
{
	return B14071_CUPS_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_cups_ok_u_14071 - CUPS ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not contact cupsd or call
 * libc. No parent wires.
 */
uint32_t
gj_cups_ok_u_14071(void)
{
	(void)NULL;
	return b14071_cups_ok();
}
...
uint32_t __gj_cups_ok_u_14071(void)
    __attribute__((alias("gj_cups_ok_u_14071")));
