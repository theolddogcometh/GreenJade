 ...
 *
 * Desktop glibc graph batch14173: battery ok soft unit (off).
 *
 * Surface (unique symbols):
 *   uint32_t gj_battery_ok_u_14173(void);
 *     - Always returns 0. Soft stub only; intentionally not a real
 *       battery/sysfs probe for the power/thermal continuum.
 *   uint32_t __gj_battery_ok_u_14173  (alias)
 *   __libcgj_batch14173_marker = "libcgj-batch14173"
 *
...
const char __libcgj_batch14173_marker[] = "libcgj-batch14173";

/* Soft battery-ok lamp: always off (not a real battery probe). */
#define B14173_BATTERY_OK  0u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b14173_battery_ok(void)
{
	return B14173_BATTERY_OK;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_battery_ok_u_14173 - battery ok soft unit flag.
 *
 * Always returns 0. Soft continuum stub; does not probe power_supply
 * or call libc. No parent wires.
 */
uint32_t
gj_battery_ok_u_14173(void)
{
	(void)NULL;
	return b14173_battery_ok();
}
...
uint32_t __gj_battery_ok_u_14173(void)
    __attribute__((alias("gj_battery_ok_u_14173")));
