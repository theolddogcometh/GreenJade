 ...
 *
 * Desktop glibc graph batch234: binomial coefficient C(n, k) in uint64_t.
 *
 * Surface (unique symbols):
 *   uint64_t gj_binom(unsigned n, unsigned k);
 *     — C(n, k) = n! / (k! (n-k)!). Returns 0 if k > n, or if the
 *       value does not fit in uint64_t (overflow → 0). C(n, 0) = C(n, n)
 *       = 1. Uses the multiplicative product with gcd reduction so that
 *       intermediate multiplies stay exact when the final result fits.
 *   __gj_binom  (alias)
 *   __libcgj_batch234_marker = "libcgj-batch234"
 *
...
const char __libcgj_batch234_marker[] = "libcgj-batch234";

/* ---- freestanding helpers ---------------------------------------------- */

/* Euclidean GCD for uint64_t (a, b >= 0). */
static uint64_t
b234_gcd(uint64_t a, uint64_t b)
{
	while (b != 0u) {
		uint64_t t = a % b;
...
/*
 * gj_binom — binomial coefficient C(n, k) as uint64_t.
 *
 ...
 */
uint64_t
gj_binom(unsigned n, unsigned k)
{
	uint64_t r;
	unsigned i;
...
uint64_t __gj_binom(unsigned n, unsigned k)
    __attribute__((alias("gj_binom")));
