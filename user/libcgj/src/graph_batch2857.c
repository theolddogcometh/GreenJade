 ...
 *
 * Desktop glibc graph batch2857: first clear bit in a uint64_t word bitmap.
 *
 * Surface (unique symbols):
 *   size_t gj_bitmap_first_clr_u(const uint64_t *words, size_t nwords);
 ...
 *   size_t __gj_bitmap_first_clr_u  (alias)
 *   __libcgj_batch2857_marker = "libcgj-batch2857"
 *
 * Bitmap exclusive wave (2851-2860). Distinct from
 * gj_bitmap_find_first_zero_u (batch2447), gj_bitmap_first_zero
 * (batch535), and gj_bitmap_find_next_zero (batch1057). Unique
 * gj_bitmap_first_clr_u surface only.
 *
...
const char __libcgj_batch2857_marker[] = "libcgj-batch2857";
...
static size_t
b2857_first_clr(const uint64_t *pWords, size_t cWords)
{
	size_t iWord;
	uint64_t u64W;
	uint64_t u64Inv;
	...
	for (iWord = 0u; iWord < cWords; iWord++) {
		...
		u64Inv = ~u64W;
		return (iWord * 64u) + b2857_ctz64(u64Inv);
	}
	return cWords * 64u;
}
...
/*
 * gj_bitmap_first_clr_u — first clear bit index in a word bitmap.
 *
 ...
 */
size_t
gj_bitmap_first_clr_u(const uint64_t *pWords, size_t cWords)
{
	if (pWords == NULL || cWords == 0u) {
		return 0u;
	}
	return b2857_first_clr(pWords, cWords);
}

/* ---- underscored alias ------------------------------------------------- */

size_t __gj_bitmap_first_clr_u(const uint64_t *pWords, size_t cWords)
    __attribute__((alias("gj_bitmap_first_clr_u")));
