 ...
 *
 * Desktop glibc graph batch3327: freestanding uint32_t min-heap sift-up (_u).
 *
 * Surface (unique symbols):
 *   void gj_heap_u32_sift_up_u(uint32_t *a, size_t i);
 ...
 *   void __gj_heap_u32_sift_up_u  (alias)
 *   __libcgj_batch3327_marker = "libcgj-batch3327"
 *
 * Exclusive continuum CREATE-ONLY (3321-3330). Distinct from
 * gj_u32_heap_sift_up (batch671) — unique heap_u32_*_u surface only;
...
const char __libcgj_batch3327_marker[] = "libcgj-batch3327";

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_heap_u32_sift_up_u - restore min-heap order by bubbling a[i] upward.
 *
 ...
 */
void
gj_heap_u32_sift_up_u(uint32_t *pA, size_t i)
{
	size_t iParent;
	uint32_t uTmp;
...
void __gj_heap_u32_sift_up_u(uint32_t *pA, size_t i)
    __attribute__((alias("gj_heap_u32_sift_up_u")));
