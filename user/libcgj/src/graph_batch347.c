 ...
 *
 * Desktop glibc graph batch347: Internet checksum (RFC 1071).
 *
 * Surface (unique symbols):
 *   uint16_t gj_inet_cksum(const void *data, size_t n);
 ...
 *       the 16-bit ones-complement result). Empty / NULL+zero-length
 *       buffer yields 0xffff.
 *   uint16_t __gj_inet_cksum  (alias)
 *   __libcgj_batch347_marker = "libcgj-batch347"
 *
 * Distinct from batch124 gj_ip_checksum (separate symbol surface for
 * the graph expansion). Clean-room freestanding pure C (integer/pointer
 * only). Compiles with -ffreestanding -msse2 -Wall -Wextra -Werror.
 * No malloc, no errno, no libc helpers. No third-party source copied.
 */

#include <stddef.h>
#include <stdint.h>

const char __libcgj_batch347_marker[] = "libcgj-batch347";
...
/*
 * gj_inet_cksum — RFC 1071 Internet checksum over a byte buffer.
 *
 ...
 */
uint16_t
gj_inet_cksum(const void *data, size_t n)
{
	...
	return b347_fold(uSum);
}

/* ---- underscored alias ------------------------------------------------- */

uint16_t __gj_inet_cksum(const void *data, size_t n)
    __attribute__((alias("gj_inet_cksum")));
