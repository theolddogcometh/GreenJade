 ...
 *
 * Desktop glibc graph batch3497: product bar3 readiness gate (wave 3500).
 *
 * Surface (unique symbols):
 *   uint32_t gj_bar3_ready_3500(uint32_t mask);
 ...
 *       milestone 3500 continuum.
 *   uint32_t __gj_bar3_ready_3500  (alias)
 *   __libcgj_batch3497_marker = "libcgj-batch3497"
 *
 * Milestone 3500 exclusive continuum CREATE-ONLY (3491-3500). Unique
 * gj_bar3_ready_3500 surface only; no multi-def. Distinct from
...
const char __libcgj_batch3497_marker[] = "libcgj-batch3497";

/* Three product bar3 readiness bits (bits 0..2). */
#define B3497_BAR3_MASK  0x7u

/* ---- freestanding helpers ---------------------------------------------- */

/* 1u if all bar3 readiness bits are present in mask. */
static uint32_t
b3497_ready(uint32_t uMask)
{
	if ((uMask & B3497_BAR3_MASK) == B3497_BAR3_MASK) {
		return 1u;
	}
	return 0u;
}
...
/*
 * gj_bar3_ready_3500 - 1u iff bar3 readiness mask is complete.
 *
 ...
 */
uint32_t
gj_bar3_ready_3500(uint32_t uMask)
{
	(void)NULL;
	return b3497_ready(uMask);
}

/* ---- underscored alias ------------------------------------------------- */

uint32_t __gj_bar3_ready_3500(uint32_t uMask)
    __attribute__((alias("gj_bar3_ready_3500")));
