 ...
 *
 * Desktop glibc graph batch367: count leading zero bits in a uint64_t.
 *
 * Surface (unique symbols):
 *   unsigned gj_clz64(uint64_t x);
 *     — Number of consecutive 0-bits starting from bit 63 (MSB).
 *       Returns 64 when x == 0 (fully defined for the zero input).
 *   unsigned __gj_clz64  (alias)
 *   __libcgj_batch367_marker = "libcgj-batch367"
 *
...
const char __libcgj_batch367_marker[] = "libcgj-batch367";

/*
 * gj_clz64 — count leading zeros in a 64-bit word.
 *
 * x: value to inspect
 *
...
unsigned __gj_clz64(uint64_t u64X)
    __attribute__((alias("gj_clz64")));
