 ...
 *
 * Desktop glibc graph batch3719: full-string signed decimal → int64_t (_u).
 *
 * Surface (unique symbols):
 *   int gj_parse_i64_dec_u(const char *s, int64_t *out);
 ...
 *   int __gj_parse_i64_dec_u  (alias)
 *   __libcgj_batch3719_marker = "libcgj-batch3719"
 *
 * Exclusive continuum CREATE-ONLY (3711-3720). Distinct from
 * gj_parse_i64_dec (batch446), gj_parse_i64_dec_n (batch1532),
 * gj_parse_i64_dec_full (batch2482) — unique _u surface; no multi-def.
 * No parent wires.
 *
...
const char __libcgj_batch3719_marker[] = "libcgj-batch3719";

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_parse_i64_dec_u — full-string optional-signed decimal ASCII → int64_t.
 *
 ...
 */
int
gj_parse_i64_dec_u(const char *s, int64_t *out)
{
	const char *p;
	uint64_t uMag;
	int nDig;
...
int __gj_parse_i64_dec_u(const char *s, int64_t *out)
    __attribute__((alias("gj_parse_i64_dec_u")));
