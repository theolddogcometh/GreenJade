 ...
 *
 * Desktop glibc graph batch392: pure hex string → uint64_t.
 *
 * Surface (unique symbols):
 *   int gj_parse_u64_hex(const char *s, uint64_t *out);
 ...
 *   int __gj_parse_u64_hex  (alias)
 *   __libcgj_batch392_marker = "libcgj-batch392"
 *
...
const char __libcgj_batch392_marker[] = "libcgj-batch392";

/* ---- helpers ----------------------------------------------------------- */

static int
b392_hex_digit(char c)
{
	if (c >= '0' && c <= '9') {
		return (int)(c - '0');
	}
...
/*
 * gj_parse_u64_hex — full-string hexadecimal ASCII → uint64_t.
 *
 ...
 */
int
gj_parse_u64_hex(const char *s, uint64_t *out)
{
	const char *p;
	uint64_t u;
	int nDig;
...
int __gj_parse_u64_hex(const char *s, uint64_t *out)
    __attribute__((alias("gj_parse_u64_hex")));
