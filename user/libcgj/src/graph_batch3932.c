 ...
 *
 * Desktop glibc graph batch3932: freestanding uint64_t deque pop-back (_u).
 *
 * Surface (unique symbols):
 *   int gj_deque_u64_pop_back_u(uint64_t *buf, size_t cap, size_t *head,
 *                               size_t *tail, size_t *count, uint64_t *out);
 *     - Remove one uint64_t from the back (slot before head) into *out.
 *       Returns 0 on success, -1 if empty or bad args.
 *   int __gj_deque_u64_pop_back_u  (alias)
 *   __libcgj_batch3932_marker = "libcgj-batch3932"
 *
 * Exclusive continuum CREATE-ONLY (3931-3940). Distinct from
 * gj_deque_u32_pop_back_u (batch3325) and gj_u32_deque_pop_back (batch706)
 * — unique deque_u64_pop_back_u surface only; no multi-def.
 * No parent wires.
 *
...
const char __libcgj_batch3932_marker[] = "libcgj-batch3932";

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_deque_u64_pop_back_u - remove one uint64_t from the back.
 *
 ...
 */
int
gj_deque_u64_pop_back_u(uint64_t *pBuf, size_t cCap, size_t *pHead,
...
int __gj_deque_u64_pop_back_u(uint64_t *pBuf, size_t cCap, size_t *pHead,
                              size_t *pTail, size_t *pCount, uint64_t *pOut)
    __attribute__((alias("gj_deque_u64_pop_back_u")));
