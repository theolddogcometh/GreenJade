 ...
 *
 * Desktop glibc graph batch4061: map uint64_t across two ranges (remap_u).
 *
 * Surface (unique symbols):
 *   uint64_t gj_u64_remap_u(uint64_t x, uint64_t in_lo, uint64_t in_hi,
 *       uint64_t out_lo, uint64_t out_hi);
 *     - Linearly remap x from the closed interval between in_lo and in_hi
 ...
 *   uint64_t __gj_u64_remap_u  (alias)
 *   __libcgj_batch4061_marker = "libcgj-batch4061"
 *
 * Exclusive continuum CREATE-ONLY (4061-4070). Unique gj_u64_remap_u
 * surface only; no multi-def. Distinct from gj_u64_map_range_u
 * (batch3263) and gj_u32_map_range_u (batch3262). No parent wires.
 * No __int128.
 *
...
const char __libcgj_batch4061_marker[] = "libcgj-batch4061";
 ...
 */
static uint64_t
b4061_div128_64(uint64_t u64Hi, uint64_t u64Lo, uint64_t u64D)
{
	uint64_t u64Q;
	uint64_t u64R;
	uint32_t uBit;
 ...
 */
static uint64_t
b4061_mul_div(uint64_t u64A, uint64_t u64B, uint64_t u64C)
{
	uint64_t u64Hi;
	uint64_t u64Lo;
	...
	return b4061_div128_64(u64Hi, u64Lo, u64C);
}

/*
 * Linear remap of x from [in_lo, in_hi] (direction-aware) onto
 * [out_lo → out_hi]. No parent wires; mul_div inlined above.
 */
static uint64_t
b4061_remap(uint64_t u64X, uint64_t u64InLo, uint64_t u64InHi,
    uint64_t u64OutLo, uint64_t u64OutHi)
{
	...
	if (u64OutHi >= u64OutLo) {
		...
		u64Mix = b4061_mul_div(u64Off, u64OutSpan, u64InSpan);
...
/*
 * gj_u64_remap_u - remap x from one uint64 range onto another.
 *
 * x:               value to remap (clamped into ordered source span)
 ...
 */
uint64_t
gj_u64_remap_u(uint64_t u64X, uint64_t u64InLo, uint64_t u64InHi,
    uint64_t u64OutLo, uint64_t u64OutHi)
{
	(void)NULL;
	return b4061_remap(u64X, u64InLo, u64InHi, u64OutLo, u64OutHi);
}

/* ---- underscored alias ------------------------------------------------- */

uint64_t __gj_u64_remap_u(uint64_t u64X, uint64_t u64InLo, uint64_t u64InHi,
    uint64_t u64OutLo, uint64_t u64OutHi)
    __attribute__((alias("gj_u64_remap_u")));
