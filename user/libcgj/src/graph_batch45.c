 ...
 *
 * Desktop glibc graph batch45: AES (FIPS 197 table-based integer encrypt/
 * decrypt; AES-128/192/256), RC4 stream cipher, Blowfish (Schneier; OpenBSD
 * blf-shaped init/expand/encrypt). Integer/pointer only (no SSE).
 * Clean-room public ABI. batch50 provides AES_ecb_encrypt_block wrapper.
 */
#include <stddef.h>
#include <stdint.h>
#include <string.h>

const char __libcgj_batch45_marker[] = "libcgj-batch45";
...
static uint32_t
b45_load_be32(const unsigned char *p)
{
...
static uint32_t
b45_rol32(uint32_t x, unsigned n)
{
    return (x << n) | (x >> (32u - n));
}

/* ========================================================================
 * AES (FIPS 197) — table-based Te/Td, integer only
 * OpenSSL-shaped AES_KEY / set_encrypt / set_decrypt / encrypt / decrypt
 * ======================================================================== */
...
/* FIPS 197 S-box */
static const unsigned char s_b45_aes_s[256] = {
...
/* Inverse S-box */
static const unsigned char s_b45_aes_si[256] = {
...
/* Rcon[1..] for key expansion */
static const unsigned char s_b45_rcon[11] = {
    0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36};

static uint32_t
b45_mul(unsigned char a, unsigned char b)
{
    unsigned char p = 0;
    int i;
...
b45_xtime(unsigned char x)
{
    return (unsigned char)((x << 1) ^ (((x >> 7) & 1u) * 0x1bu));
}

/* Mix one state column (column-major layout). */
static void
b45_mix_column(unsigned char *a0, unsigned char *a1, unsigned char *a2,
               unsigned char *a3)
{
    unsigned char t0 = *a0, t1 = *a1, t2 = *a2, t3 = *a3;
    unsigned char u = (unsigned char)(t0 ^ t1 ^ t2 ^ t3);
    unsigned char v;

    v = b45_xtime((unsigned char)(t0 ^ t1));
    *a0 = (unsigned char)(t0 ^ v ^ u);
    v = b45_xtime((unsigned char)(t1 ^ t2));
    *a1 = (unsigned char)(t1 ^ v ^ u);
    v = b45_xtime((unsigned char)(t2 ^ t3));
    *a2 = (unsigned char)(t2 ^ v ^ u);
    v = b45_xtime((unsigned char)(t3 ^ t0));
    *a3 = (unsigned char)(t3 ^ v ^ u);
}

static void
b45_inv_mix_column(unsigned char *a0, unsigned char *a1, unsigned char *a2,
                   unsigned char *a3)
{
    unsigned char t0 = *a0, t1 = *a1, t2 = *a2, t3 = *a3;

    *a0 = (unsigned char)(b45_mul(t0, 0x0eu) ^ b45_mul(t1, 0x0bu) ^
                          b45_mul(t2, 0x0du) ^ b45_mul(t3, 0x09u));
    *a1 = (unsigned char)(b45_mul(t0, 0x09u) ^ b45_mul(t1, 0x0eu) ^
                          b45_mul(t2, 0x0bu) ^ b45_mul(t3, 0x0du));
    *a2 = (unsigned char)(b45_mul(t0, 0x0du) ^ b45_mul(t1, 0x09u) ^
                          b45_mul(t2, 0x0eu) ^ b45_mul(t3, 0x0bu));
    *a3 = (unsigned char)(b45_mul(t0, 0x0bu) ^ b45_mul(t1, 0x0du) ^
                          b45_mul(t2, 0x09u) ^ b45_mul(t3, 0x0eu));
}

static uint32_t
b45_sub_word(uint32_t x)
{
    return ((uint32_t)s_b45_aes_s[(x >> 24) & 0xffu] << 24) |
           ((uint32_t)s_b45_aes_s[(x >> 16) & 0xffu] << 16) |
           ((uint32_t)s_b45_aes_s[(x >> 8) & 0xffu] << 8) |
           (uint32_t)s_b45_aes_s[x & 0xffu];
}

int
AES_set_encrypt_key(const unsigned char *userKey, const int bits, AES_KEY *key)
{
    unsigned Nk, Nr, i, nWords;
    uint32_t *w;
    uint32_t temp;
    ...
    w = (uint32_t *)key->rd_key;
    nWords = 4u * (Nr + 1u);
    key->rounds = (int)Nr;
    for (i = 0; i < Nk; i++) {
        w[i] = b45_load_be32(userKey + i * 4u);
    }
    for (i = Nk; i < nWords; i++) {
        temp = w[i - 1u];
        if ((i % Nk) == 0u) {
            temp = b45_sub_word(b45_rol32(temp, 8)) ^
                   ((uint32_t)s_b45_rcon[i / Nk] << 24);
        } else if (Nk > 6u && (i % Nk) == 4u) {
            temp = b45_sub_word(temp);
        }
        w[i] = w[i - Nk] ^ temp;
    }
    return 0;
...
{
    /* Decrypt uses the encrypt schedule + InvCipher (no InvMix on keys). */
...
AES_encrypt(const unsigned char *in, unsigned char *out, const AES_KEY *key)
{
    ...
    Nr = (unsigned)key->rounds;
    rk = (const uint32_t *)key->rd_key;
    for (c = 0; c < 16u; c++) {
        s[c] = in[c];
    }
    /* AddRoundKey(0); state is column-major: s[r + 4*c] */
    for (c = 0; c < 4u; c++) {
        uint32_t kw = rk[c];
    ...
    for (r = 1u; r < Nr; r++) {
        ...
        for (c = 0; c < 4u; c++) {
            b45_mix_column(&s[4u * c + 0u], &s[4u * c + 1u], &s[4u * c + 2u],
                           &s[4u * c + 3u]);
        }
        rk = (const uint32_t *)key->rd_key + r * 4u;
        for (c = 0; c < 4u; c++) {
            uint32_t kw = rk[c];
            s[4u * c + 0u] ^= (unsigned char)(kw >> 24);
            s[4u * c + 1u] ^= (unsigned char)(kw >> 16);
    ...
    }
    {
        ...
        rk = (const uint32_t *)key->rd_key + Nr * 4u;
        for (c = 0; c < 4u; c++) {
            uint32_t kw = rk[c];
            s[4u * c + 0u] ^= (unsigned char)(kw >> 24);
            s[4u * c + 1u] ^= (unsigned char)(kw >> 16);
...
AES_decrypt(const unsigned char *in, unsigned char *out, const AES_KEY *key)
{
    ...
    for (r = Nr - 1u; r > 0u; r--) {
        ...
        for (c = 0; c < 4u; c++) {
            b45_inv_mix_column(&s[4u * c + 0u], &s[4u * c + 1u],
                               &s[4u * c + 2u], &s[4u * c + 3u]);
        }
    }
    {
...
int __AES_set_encrypt_key(const unsigned char *k, const int b, AES_KEY *key)
    __attribute__((alias("AES_set_encrypt_key")));
int __AES_set_decrypt_key(const unsigned char *k, const int b, AES_KEY *key)
    __attribute__((alias("AES_set_decrypt_key")));
void __AES_encrypt(const unsigned char *i, unsigned char *o, const AES_KEY *k)
    __attribute__((alias("AES_encrypt")));
void __AES_decrypt(const unsigned char *i, unsigned char *o, const AES_KEY *k)
    __attribute__((alias("AES_decrypt")));

/* ========================================================================
 * RC4 (Alleged RC4 / Arcfour) — OpenSSL-shaped RC4_KEY / RC4_set_key / RC4
 * ======================================================================== */
    ...
    unsigned char *outdata)
{
    ...
    if (key == NULL || outdata == NULL) {
        return;
    }
    if (indata == NULL && len != 0u) {
        return;
    }
    s = key->data;
    x = key->x;
    ...
    for (i = 0; i < len; i++) {
        ...
        s[x] = ty;
        s[y] = tx;
        outdata[i] = (unsigned char)(indata[i] ^ s[(tx + ty) & 0xffu]);
    }
    key->x = x;
    key->y = y;
}

void __RC4_set_key(RC4_KEY *k, int n, const unsigned char *d)
    __attribute__((alias("RC4_set_key")));
void __RC4(RC4_KEY *k, size_t n, const unsigned char *i, unsigned char *o)
    __attribute__((alias("RC4")));

/* ========================================================================
 * Blowfish (Schneier) — OpenBSD blf-shaped API
 * Blowfish_initstate / Blowfish_expand0state / Blowfish_encipher /
 * Blowfish_encrypt
 * ======================================================================== */
...
/* Origin P-array and S-boxes: hex digits of pi (Schneier / public domain) */
static const uint32_t s_b45_blf_p[18] = {
...
static const uint32_t s_b45_blf_s[4][256] = {
...
static uint32_t
b45_blf_f(const blf_ctx *c, uint32_t x)
{
    uint16_t a, b, cc, d;
    uint32_t y;
...
void
Blowfish_encrypt(blf_ctx *c, uint32_t *data, uint16_t blocks)
{
    uint32_t *d;
    uint16_t i;
...
void __Blowfish_initstate(blf_ctx *c) __attribute__((alias("Blowfish_initstate")));
void __Blowfish_expand0state(blf_ctx *c, const uint8_t *k, uint16_t n)
    __attribute__((alias("Blowfish_expand0state")));
void __Blowfish_encipher(blf_ctx *c, uint32_t *l, uint32_t *r)
