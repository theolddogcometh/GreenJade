/*
 * SPDX-License-Identifier: MIT OR Apache-2.0
 * Copyright (c) 2026 Project GreenJade contributors
 *
 * Desktop glibc graph batch49: MD2 (RFC 1319) digest family.
 * Integer/pointer only (no SSE). Clean-room public ABI from the RFC.
 */
#include <stddef.h>
#include <stdint.h>
#include <string.h>
#include <sys/types.h>

const char __libcgj_batch49_marker[] = "libcgj-batch49";

/* ---- MD2 (RFC 1319) ----------------------------------------------------- */

struct b49_md2 {
    unsigned char aX[48];
    unsigned char aC[16];
    unsigned char aBuf[16];
    size_t cbBuf;
    unsigned char uL; /* running checksum L = C[15] after last block */
};

/* RFC 1319 permutation of 0..255 constructed from digits of pi. */
static const unsigned char s_b49_md2_s[256] = {
    41,  46,  67,  201, 162, 216, 124, 1,   61,  54,  84,  161, 236, 240, 6,
    19,  98,  167, 5,   243, 192, 199, 115, 140, 152, 147, 43,  217, 188, 76,
    130, 202, 30,  155, 87,  60,  253, 212, 224, 22,  103, 66,  111, 24,  138,
    23,  229, 18,  190, 78,  196, 214, 218, 158, 222, 73,  160, 251, 245, 142,
    187, 47,  238, 122, 169, 104, 121, 145, 21,  178, 7,   63,  148, 194, 16,
    137, 11,  34,  95,  33,  128, 127, 93,  154, 90,  144, 50,  39,  53,  62,
    204, 231, 191, 247, 151, 3,   255, 25,  48,  179, 72,  165, 181, 209, 215,
    94,  146, 42,  172, 86,  170, 198, 79,  184, 56,  210, 150, 164, 125, 182,
    118, 252, 107, 226, 156, 116, 4,   241, 69,  157, 112, 89,  100, 113, 135,
    32,  134, 91,  207, 101, 230, 45,  168, 2,   27,  96,  37,  173, 174, 176,
    185, 246, 28,  70,  97,  105, 52,  64,  126, 15,  85,  71,  163, 35,  221,
    81,  175, 58,  195, 92,  249, 206, 186, 197, 234, 38,  44,  83,  13,  110,
    133, 40,  132, 9,   211, 223, 205, 244, 65,  129, 77,  82,  106, 220, 55,
    200, 108, 193, 171, 250, 36,  225, 123, 8,   12,  189, 177, 74,  120, 136,
    149, 139, 227, 99,  232, 109, 233, 203, 213, 254, 59,  0,   29,  57,  242,
    239, 183, 14,  102, 88,  208, 228, 166, 119, 114, 248, 235, 117, 75,  10,
    49,  68,  80,  180, 143, 237, 31,  26,  219, 153, 141, 51,  159, 17,  131,
    20};

static void
b49_md2_init(struct b49_md2 *p)
{
    memset(p, 0, sizeof(*p));
}

static void
b49_md2_block(struct b49_md2 *p, const unsigned char aBlk[16])
{
    unsigned char t;
    unsigned i;
    unsigned j;

    /* Update checksum C and L (RFC 1319 §3.2). */
    t = p->uL;
    for (i = 0; i < 16u; i++) {
        p->aC[i] ^= s_b49_md2_s[aBlk[i] ^ t];
        t = p->aC[i];
    }
    p->uL = t;

    /* Copy block into X[16..31] and X[32..47] = X[16..31] XOR X[0..15]. */
    for (i = 0; i < 16u; i++) {
        p->aX[16u + i] = aBlk[i];
        p->aX[32u + i] = (unsigned char)(aBlk[i] ^ p->aX[i]);
    }

    /* 18 rounds over the 48-byte state (RFC 1319 §3.2). */
    t = 0;
    for (j = 0; j < 18u; j++) {
        for (i = 0; i < 48u; i++) {
            t = (unsigned char)(p->aX[i] ^ s_b49_md2_s[t]);
            p->aX[i] = t;
        }
        t = (unsigned char)(t + (unsigned char)j);
    }
}

void
MD2Init(void *pCtx)
{
    if (pCtx != NULL) {
        b49_md2_init((struct b49_md2 *)pCtx);
    }
}

void
MD2Update(void *pCtx, const void *pData, size_t cb)
{
    struct b49_md2 *p = (struct b49_md2 *)pCtx;
    const unsigned char *q = (const unsigned char *)pData;
    size_t n;

    if (p == NULL || (pData == NULL && cb != 0)) {
        return;
    }
    while (cb > 0u) {
        n = 16u - p->cbBuf;
        if (n > cb) {
            n = cb;
        }
        memcpy(p->aBuf + p->cbBuf, q, n);
        p->cbBuf += n;
        q += n;
        cb -= n;
        if (p->cbBuf == 16u) {
            b49_md2_block(p, p->aBuf);
            p->cbBuf = 0;
        }
    }
}

void
MD2Final(unsigned char aDig[16], void *pCtx)
{
    struct b49_md2 *p = (struct b49_md2 *)pCtx;
    unsigned char aPad[16];
    unsigned char uPad;
    unsigned i;

    if (p == NULL || aDig == NULL) {
        return;
    }

    /* Pad with i bytes of value i (RFC 1319 §3.1); always at least one pad. */
    uPad = (unsigned char)(16u - p->cbBuf);
    for (i = 0; i < 16u; i++) {
        aPad[i] = uPad;
    }
    MD2Update(p, aPad, (size_t)uPad);

    /* Append the 16-byte checksum as the final block. */
    MD2Update(p, p->aC, 16u);

    memcpy(aDig, p->aX, 16u);
    memset(p, 0, sizeof(*p));
}

static char *
b49_hex_end(const unsigned char *pDig, size_t cb, char *szBuf)
{
    static char aLocal[40];
    static const char *szDigits = "0123456789abcdef";
    char *p;
    size_t i;

    p = (szBuf != NULL) ? szBuf : aLocal;
    for (i = 0; i < cb; i++) {
        p[i * 2u] = szDigits[(pDig[i] >> 4) & 0xfu];
        p[i * 2u + 1u] = szDigits[pDig[i] & 0xfu];
    }
    p[cb * 2u] = '\0';
    return p;
}

char *
MD2End(void *pCtx, char *szBuf)
{
    unsigned char aDig[16];

    MD2Final(aDig, pCtx);
    return b49_hex_end(aDig, 16u, szBuf);
}

char *
MD2Data(const void *pData, size_t cb, char *szBuf)
{
    unsigned char aCtx[128];

    memset(aCtx, 0, sizeof(aCtx));
    MD2Init(aCtx);
    MD2Update(aCtx, pData, cb);
    return MD2End(aCtx, szBuf);
}

void __MD2Init(void *pCtx) __attribute__((alias("MD2Init")));
void __MD2Update(void *pCtx, const void *pData, size_t cb)
    __attribute__((alias("MD2Update")));
void __MD2Final(unsigned char aDig[16], void *pCtx)
    __attribute__((alias("MD2Final")));
char *__MD2End(void *pCtx, char *szBuf) __attribute__((alias("MD2End")));
char *__MD2Data(const void *pData, size_t cb, char *szBuf)
    __attribute__((alias("MD2Data")));
