 ...
 *
 * Desktop glibc graph batch529: freestanding octet-wise bitwise AND.
 *
 * Surface (unique symbols):
 *   void gj_mem_and(void *dst, const void *a, const void *b, size_t n);
 *     — For each i in [0, n): dst[i] = a[i] & b[i] (unsigned octets).
 *       Any of dst/a/b NULL → no-op. n == 0 → no-op.
 *   void __gj_mem_and  (alias)
 *   __libcgj_batch529_marker = "libcgj-batch529"
 *
...
const char __libcgj_batch529_marker[] = "libcgj-batch529";

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_mem_and — store octet-wise bitwise AND of a and b into dst.
 *
 ...
 */
void
gj_mem_and(void *dst, const void *a, const void *b, size_t n)
{
	unsigned char *pd;
...
void __gj_mem_and(void *dst, const void *a, const void *b, size_t n)
    __attribute__((alias("gj_mem_and")));
