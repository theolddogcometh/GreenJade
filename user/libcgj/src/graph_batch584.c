 ...
 *
 * Desktop glibc graph batch584: format uint64_t as a decimal ASCII string.
 *
 * Surface (unique symbols):
 *   int gj_format_u64(uint64_t v, char *out, size_t cap);
 *     — Write the base-10 representation of v into out as a
 *       NUL-terminated string (no leading zeros except for v == 0).
 *       Examples:
 *         0          → "0"
 *         42         → "42"
 ...
 *   int __gj_format_u64  (alias)
 *   __libcgj_batch584_marker = "libcgj-batch584"
 *
...
const char __libcgj_batch584_marker[] = "libcgj-batch584";

/* UINT64_MAX needs 20 decimal digits; reverse buffer holds that. */
#define B584_MAX_DIGITS 20

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_format_u64 — write decimal digits of v into out.
 *
 ...
 */
int
gj_format_u64(uint64_t v, char *out, size_t cap)
{
	char aDig[B584_MAX_DIGITS];
	int nDig;
	int iDig;
	uint64_t uVal;
	size_t iOut;
	...
	if (uVal == 0ull) {
		...
		while (uVal > 0ull && nDig < B584_MAX_DIGITS) {
...
int __gj_format_u64(uint64_t v, char *out, size_t cap)
    __attribute__((alias("gj_format_u64")));
