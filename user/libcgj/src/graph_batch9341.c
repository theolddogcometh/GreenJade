 ...
 *
 * Desktop glibc graph batch9341: soft udev action-add id.
 *
 * Surface (unique symbols):
 *   uint32_t gj_udev_action_add_id_9341(void);
 *     - Returns 1 (udev "add" action soft catalog id). Pure-data
 *       action id stub; does not watch devices or call libudev.
 *   uint32_t __gj_udev_action_add_id_9341  (alias)
 *   __libcgj_batch9341_marker = "libcgj-batch9341"
 *
...
const char __libcgj_batch9341_marker[] = "libcgj-batch9341";

/* Soft udev "add" action catalog id. */
#define B9341_UDEV_ACTION_ADD  1u

/* ---- freestanding helpers ---------------------------------------------- */

static uint32_t
b9341_action_add_id(void)
{
	return B9341_UDEV_ACTION_ADD;
}

/* ---- public surface ---------------------------------------------------- */

/*
 * gj_udev_action_add_id_9341 - soft udev "add" action catalog id.
 *
 * Always returns 1. Soft continuum constant; does not watch devices or
 * call libudev. No parent wires.
 */
uint32_t
gj_udev_action_add_id_9341(void)
{
	(void)NULL;
	return b9341_action_add_id();
}

/* ---- underscored alias ------------------------------------------------- */

uint32_t __gj_udev_action_add_id_9341(void)
    __attribute__((alias("gj_udev_action_add_id_9341")));
