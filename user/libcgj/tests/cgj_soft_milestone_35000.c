 ...
 *
 * Soft host gates for continuum MILESTONE 35000 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_35000.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_35000.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_35000 \
 *     user/libcgj/tests/cgj_soft_milestone_35000.c -ldl
 *   ./build/cgj_soft_milestone_35000
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M35000_DIRECT \
 *     -o build/cgj_soft_milestone_35000_direct \
 *     user/libcgj/tests/cgj_soft_milestone_35000.c \
 *     user/libcgj/src/graph_batch34991.c \
 *     user/libcgj/src/graph_batch34992.c \
 *     user/libcgj/src/graph_batch34993.c \
 *     user/libcgj/src/graph_batch34994.c \
 *     user/libcgj/src/graph_batch34995.c \
 *     user/libcgj/src/graph_batch34996.c \
 *     user/libcgj/src/graph_batch34997.c \
 *     user/libcgj/src/graph_batch34998.c \
 *     user/libcgj/src/graph_batch34999.c \
 *     user/libcgj/src/graph_batch35000.c
 *   ./build/cgj_soft_milestone_35000_direct
 *
 * Expected soft KATs (wave 34991–35000 exclusive continuum markers):
 *   gj_shell_green_35000      → 1
 *   gj_libcgj_green_35000     → 1
 *   gj_bar3_ready_35000       → 0   (bar3 still open)
 *   gj_product_score_35000    → 0   (product score still open)
 *   gj_continuum_ready_35000  → 1
 *   gj_smoke_soft_35000       → 1
 *   gj_dyn_soft_35000         → 1
 *   gj_milestone_tag_35000    → 35000
 *   gj_continuum_wave_35000   → 35000
 *   gj_batch_id_35000         → 35000
 *   gj_graph_milestone_35000  → 35000
 *   __libcgj_batch34991_marker … __libcgj_batch35000_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch34991.c … graph_batch35000.c (and fill
 *     34976–34990 / identity 34951–34975) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 35000.
 *   - libc.map may lag milestone 35000 globals; add 35000 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_35000 == 0).
 ...
 *     continuum/smoke/dyn soft=1; milestone/wave/batch/graph = 35000.
 */
...
/* Grep: libcgj soft m35000 residual Soft!=product dual_dod_ab=OPEN G-AC-1 */
static const char g_szSoftHonesty[] =
	"libcgj: soft m35000 residual honesty Soft!=product dual_dod_a=OPEN "
...
#ifndef CGJ_SOFT_M35000_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M35000_DIRECT
/* Direct-link path: continuum soft pure markers (Soft!=product). */
extern uint32_t gj_shell_green_35000(void);
extern uint32_t gj_libcgj_green_35000(void);
extern uint32_t gj_bar3_ready_35000(void);
extern uint32_t gj_product_score_35000(void);
extern uint32_t gj_continuum_ready_35000(void);
extern uint32_t gj_smoke_soft_35000(void);
extern uint32_t gj_dyn_soft_35000(void);
extern uint32_t gj_milestone_tag_35000(void);
extern uint32_t gj_continuum_wave_35000(void);
extern uint32_t gj_batch_id_35000(void);
extern uint32_t gj_graph_milestone_35000(void);
extern const char __libcgj_batch34991_marker[];
extern const char __libcgj_batch34992_marker[];
extern const char __libcgj_batch34993_marker[];
extern const char __libcgj_batch34994_marker[];
extern const char __libcgj_batch34995_marker[];
extern const char __libcgj_batch34996_marker[];
extern const char __libcgj_batch34997_marker[];
extern const char __libcgj_batch34998_marker[];
extern const char __libcgj_batch34999_marker[];
extern const char __libcgj_batch35000_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_35000() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_35000 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_35000() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_35000 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_35000() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_35000 fail\n");
		nFail++;
	}
	if (gj_product_score_35000() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_35000 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_35000() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_35000 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_35000() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_35000 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_35000() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_35000 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_35000() != 35000u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_35000 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_35000() != 35000u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_35000 fail\n");
		nFail++;
	}
	if (gj_batch_id_35000() != 35000u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_35000 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_35000() != 35000u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_35000 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch34991_marker, "libcgj-batch34991") != 0 ||
	    strcmp(__libcgj_batch34992_marker, "libcgj-batch34992") != 0 ||
	    strcmp(__libcgj_batch34993_marker, "libcgj-batch34993") != 0 ||
	    strcmp(__libcgj_batch34994_marker, "libcgj-batch34994") != 0 ||
	    strcmp(__libcgj_batch34995_marker, "libcgj-batch34995") != 0 ||
	    strcmp(__libcgj_batch34996_marker, "libcgj-batch34996") != 0 ||
	    strcmp(__libcgj_batch34997_marker, "libcgj-batch34997") != 0 ||
	    strcmp(__libcgj_batch34998_marker, "libcgj-batch34998") != 0 ||
	    strcmp(__libcgj_batch34999_marker, "libcgj-batch34999") != 0 ||
	    strcmp(__libcgj_batch35000_marker, "libcgj-batch35000") != 0) {
		fprintf(stderr, "libcgj: direct m35000 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M35000_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m35000 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m35000 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m35000 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m35000 DIRECT PASS (all 11 lamps + markers; "
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m35000 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m35000 SOFT-SKIP (no SO; Soft!=product; "
	...
	nFail += soft_u32(h, "gj_shell_green_35000", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_35000", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_35000", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_35000", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_35000", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_35000", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_35000", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_35000", 35000u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_35000", 35000u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_35000", 35000u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_35000", 35000u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch34991_marker",
	                     "libcgj-batch34991", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch34992_marker",
	                     "libcgj-batch34992", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch34993_marker",
	                     "libcgj-batch34993", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch34994_marker",
	                     "libcgj-batch34994", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch34995_marker",
	                     "libcgj-batch34995", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch34996_marker",
	                     "libcgj-batch34996", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch34997_marker",
	                     "libcgj-batch34997", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch34998_marker",
	                     "libcgj-batch34998", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch34999_marker",
	                     "libcgj-batch34999", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch35000_marker",
	                     "libcgj-batch35000", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m35000 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m35000 PASS path=%s checked=%d "
