 ...
 *
 * Soft host gates for continuum MILESTONE 35200 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_35200.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_35200.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_35200 \
 *     user/libcgj/tests/cgj_soft_milestone_35200.c -ldl
 *   ./build/cgj_soft_milestone_35200
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M35200_DIRECT \
 *     -o build/cgj_soft_milestone_35200_direct \
 *     user/libcgj/tests/cgj_soft_milestone_35200.c \
 *     user/libcgj/src/graph_batch35191.c \
 *     user/libcgj/src/graph_batch35192.c \
 *     user/libcgj/src/graph_batch35193.c \
 *     user/libcgj/src/graph_batch35194.c \
 *     user/libcgj/src/graph_batch35195.c \
 *     user/libcgj/src/graph_batch35196.c \
 *     user/libcgj/src/graph_batch35197.c \
 *     user/libcgj/src/graph_batch35198.c \
 *     user/libcgj/src/graph_batch35199.c \
 *     user/libcgj/src/graph_batch35200.c
 *   ./build/cgj_soft_milestone_35200_direct
 *
 * Expected soft KATs (wave 35191–35200 exclusive continuum markers):
 *   gj_shell_green_35200      → 1
 *   gj_libcgj_green_35200     → 1
 *   gj_bar3_ready_35200       → 0   (bar3 still open)
 *   gj_product_score_35200    → 0   (product score still open)
 *   gj_continuum_ready_35200  → 1
 *   gj_smoke_soft_35200       → 1
 *   gj_dyn_soft_35200         → 1
 *   gj_milestone_tag_35200    → 35200
 *   gj_continuum_wave_35200   → 35200
 *   gj_batch_id_35200         → 35200
 *   gj_graph_milestone_35200  → 35200
 *   __libcgj_batch35191_marker … __libcgj_batch35200_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch35191.c … graph_batch35200.c (and fill
 *     35176–35190 / identity 35151–35175) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 35200.
 *   - libc.map may lag milestone 35200 globals; add 35200 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_35200 == 0).
 ...
 *     continuum/smoke/dyn soft=1; milestone/wave/batch/graph = 35200.
 */
...
/* Grep: libcgj soft m35200 residual Soft!=product dual_dod_ab=OPEN G-AC-1 */
static const char g_szSoftHonesty[] =
	"libcgj: soft m35200 residual honesty Soft!=product dual_dod_a=OPEN "
...
#ifndef CGJ_SOFT_M35200_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M35200_DIRECT
/* Direct-link path: continuum soft pure markers (Soft!=product). */
extern uint32_t gj_shell_green_35200(void);
extern uint32_t gj_libcgj_green_35200(void);
extern uint32_t gj_bar3_ready_35200(void);
extern uint32_t gj_product_score_35200(void);
extern uint32_t gj_continuum_ready_35200(void);
extern uint32_t gj_smoke_soft_35200(void);
extern uint32_t gj_dyn_soft_35200(void);
extern uint32_t gj_milestone_tag_35200(void);
extern uint32_t gj_continuum_wave_35200(void);
extern uint32_t gj_batch_id_35200(void);
extern uint32_t gj_graph_milestone_35200(void);
extern const char __libcgj_batch35191_marker[];
extern const char __libcgj_batch35192_marker[];
extern const char __libcgj_batch35193_marker[];
extern const char __libcgj_batch35194_marker[];
extern const char __libcgj_batch35195_marker[];
extern const char __libcgj_batch35196_marker[];
extern const char __libcgj_batch35197_marker[];
extern const char __libcgj_batch35198_marker[];
extern const char __libcgj_batch35199_marker[];
extern const char __libcgj_batch35200_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_35200() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_35200 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_35200() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_35200 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_35200() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_35200 fail\n");
		nFail++;
	}
	if (gj_product_score_35200() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_35200 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_35200() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_35200 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_35200() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_35200 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_35200() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_35200 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_35200() != 35200u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_35200 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_35200() != 35200u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_35200 fail\n");
		nFail++;
	}
	if (gj_batch_id_35200() != 35200u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_35200 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_35200() != 35200u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_35200 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch35191_marker, "libcgj-batch35191") != 0 ||
	    strcmp(__libcgj_batch35192_marker, "libcgj-batch35192") != 0 ||
	    strcmp(__libcgj_batch35193_marker, "libcgj-batch35193") != 0 ||
	    strcmp(__libcgj_batch35194_marker, "libcgj-batch35194") != 0 ||
	    strcmp(__libcgj_batch35195_marker, "libcgj-batch35195") != 0 ||
	    strcmp(__libcgj_batch35196_marker, "libcgj-batch35196") != 0 ||
	    strcmp(__libcgj_batch35197_marker, "libcgj-batch35197") != 0 ||
	    strcmp(__libcgj_batch35198_marker, "libcgj-batch35198") != 0 ||
	    strcmp(__libcgj_batch35199_marker, "libcgj-batch35199") != 0 ||
	    strcmp(__libcgj_batch35200_marker, "libcgj-batch35200") != 0) {
		fprintf(stderr, "libcgj: direct m35200 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M35200_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m35200 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m35200 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m35200 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m35200 DIRECT PASS (all 11 lamps + markers; "
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m35200 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m35200 SOFT-SKIP (no SO; Soft!=product; "
	...
	nFail += soft_u32(h, "gj_shell_green_35200", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_35200", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_35200", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_35200", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_35200", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_35200", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_35200", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_35200", 35200u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_35200", 35200u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_35200", 35200u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_35200", 35200u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch35191_marker",
	                     "libcgj-batch35191", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch35192_marker",
	                     "libcgj-batch35192", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch35193_marker",
	                     "libcgj-batch35193", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch35194_marker",
	                     "libcgj-batch35194", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch35195_marker",
	                     "libcgj-batch35195", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch35196_marker",
	                     "libcgj-batch35196", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch35197_marker",
	                     "libcgj-batch35197", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch35198_marker",
	                     "libcgj-batch35198", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch35199_marker",
	                     "libcgj-batch35199", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch35200_marker",
	                     "libcgj-batch35200", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m35200 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m35200 PASS path=%s checked=%d "
