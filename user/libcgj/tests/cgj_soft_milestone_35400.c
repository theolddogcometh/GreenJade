 ...
 *
 * Soft host gates for continuum MILESTONE 35400 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_35400.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_35400.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_35400 \
 *     user/libcgj/tests/cgj_soft_milestone_35400.c -ldl
 *   ./build/cgj_soft_milestone_35400
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M35400_DIRECT \
 *     -o build/cgj_soft_milestone_35400_direct \
 *     user/libcgj/tests/cgj_soft_milestone_35400.c \
 *     user/libcgj/src/graph_batch35391.c \
 *     user/libcgj/src/graph_batch35392.c \
 *     user/libcgj/src/graph_batch35393.c \
 *     user/libcgj/src/graph_batch35394.c \
 *     user/libcgj/src/graph_batch35395.c \
 *     user/libcgj/src/graph_batch35396.c \
 *     user/libcgj/src/graph_batch35397.c \
 *     user/libcgj/src/graph_batch35398.c \
 *     user/libcgj/src/graph_batch35399.c \
 *     user/libcgj/src/graph_batch35400.c
 *   ./build/cgj_soft_milestone_35400_direct
 *
 * Expected soft KATs (wave 35391–35400 exclusive continuum markers):
 *   gj_shell_green_35400      → 1
 *   gj_libcgj_green_35400     → 1
 *   gj_bar3_ready_35400       → 0   (bar3 still open)
 *   gj_product_score_35400    → 0   (product score still open)
 *   gj_continuum_ready_35400  → 1
 *   gj_smoke_soft_35400       → 1
 *   gj_dyn_soft_35400         → 1
 *   gj_milestone_tag_35400    → 35400
 *   gj_continuum_wave_35400   → 35400
 *   gj_batch_id_35400         → 35400
 *   gj_graph_milestone_35400  → 35400
 *   __libcgj_batch35391_marker … __libcgj_batch35400_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch35391.c … graph_batch35400.c (and fill
 *     35376–35390 / identity 35351–35375) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 35400.
 *   - libc.map may lag milestone 35400 globals; add 35400 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_35400 == 0).
 ...
 *     continuum/smoke/dyn soft=1; milestone/wave/batch/graph = 35400.
 */
...
/* Grep: libcgj soft m35400 residual Soft!=product dual_dod_ab=OPEN G-AC-1 */
static const char g_szSoftHonesty[] =
	"libcgj: soft m35400 residual honesty Soft!=product dual_dod_a=OPEN "
...
#ifndef CGJ_SOFT_M35400_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M35400_DIRECT
/* Direct-link path: continuum soft pure markers (Soft!=product). */
extern uint32_t gj_shell_green_35400(void);
extern uint32_t gj_libcgj_green_35400(void);
extern uint32_t gj_bar3_ready_35400(void);
extern uint32_t gj_product_score_35400(void);
extern uint32_t gj_continuum_ready_35400(void);
extern uint32_t gj_smoke_soft_35400(void);
extern uint32_t gj_dyn_soft_35400(void);
extern uint32_t gj_milestone_tag_35400(void);
extern uint32_t gj_continuum_wave_35400(void);
extern uint32_t gj_batch_id_35400(void);
extern uint32_t gj_graph_milestone_35400(void);
extern const char __libcgj_batch35391_marker[];
extern const char __libcgj_batch35392_marker[];
extern const char __libcgj_batch35393_marker[];
extern const char __libcgj_batch35394_marker[];
extern const char __libcgj_batch35395_marker[];
extern const char __libcgj_batch35396_marker[];
extern const char __libcgj_batch35397_marker[];
extern const char __libcgj_batch35398_marker[];
extern const char __libcgj_batch35399_marker[];
extern const char __libcgj_batch35400_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_35400() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_35400 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_35400() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_35400 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_35400() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_35400 fail\n");
		nFail++;
	}
	if (gj_product_score_35400() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_35400 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_35400() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_35400 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_35400() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_35400 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_35400() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_35400 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_35400() != 35400u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_35400 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_35400() != 35400u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_35400 fail\n");
		nFail++;
	}
	if (gj_batch_id_35400() != 35400u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_35400 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_35400() != 35400u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_35400 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch35391_marker, "libcgj-batch35391") != 0 ||
	    strcmp(__libcgj_batch35392_marker, "libcgj-batch35392") != 0 ||
	    strcmp(__libcgj_batch35393_marker, "libcgj-batch35393") != 0 ||
	    strcmp(__libcgj_batch35394_marker, "libcgj-batch35394") != 0 ||
	    strcmp(__libcgj_batch35395_marker, "libcgj-batch35395") != 0 ||
	    strcmp(__libcgj_batch35396_marker, "libcgj-batch35396") != 0 ||
	    strcmp(__libcgj_batch35397_marker, "libcgj-batch35397") != 0 ||
	    strcmp(__libcgj_batch35398_marker, "libcgj-batch35398") != 0 ||
	    strcmp(__libcgj_batch35399_marker, "libcgj-batch35399") != 0 ||
	    strcmp(__libcgj_batch35400_marker, "libcgj-batch35400") != 0) {
		fprintf(stderr, "libcgj: direct m35400 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M35400_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m35400 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m35400 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m35400 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m35400 DIRECT PASS (all 11 lamps + markers; "
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m35400 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m35400 SOFT-SKIP (no SO; Soft!=product; "
	...
	nFail += soft_u32(h, "gj_shell_green_35400", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_35400", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_35400", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_35400", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_35400", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_35400", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_35400", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_35400", 35400u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_35400", 35400u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_35400", 35400u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_35400", 35400u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch35391_marker",
	                     "libcgj-batch35391", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch35392_marker",
	                     "libcgj-batch35392", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch35393_marker",
	                     "libcgj-batch35393", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch35394_marker",
	                     "libcgj-batch35394", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch35395_marker",
	                     "libcgj-batch35395", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch35396_marker",
	                     "libcgj-batch35396", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch35397_marker",
	                     "libcgj-batch35397", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch35398_marker",
	                     "libcgj-batch35398", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch35399_marker",
	                     "libcgj-batch35399", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch35400_marker",
	                     "libcgj-batch35400", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m35400 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m35400 PASS path=%s checked=%d "
