 ...
 *
 * Soft host gates for continuum MILESTONE 35500 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_35500.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_35500.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_35500 \
 *     user/libcgj/tests/cgj_soft_milestone_35500.c -ldl
 *   ./build/cgj_soft_milestone_35500
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M35500_DIRECT \
 *     -o build/cgj_soft_milestone_35500_direct \
 *     user/libcgj/tests/cgj_soft_milestone_35500.c \
 *     user/libcgj/src/graph_batch35491.c \
 *     user/libcgj/src/graph_batch35492.c \
 *     user/libcgj/src/graph_batch35493.c \
 *     user/libcgj/src/graph_batch35494.c \
 *     user/libcgj/src/graph_batch35495.c \
 *     user/libcgj/src/graph_batch35496.c \
 *     user/libcgj/src/graph_batch35497.c \
 *     user/libcgj/src/graph_batch35498.c \
 *     user/libcgj/src/graph_batch35499.c \
 *     user/libcgj/src/graph_batch35500.c
 *   ./build/cgj_soft_milestone_35500_direct
 *
 * Expected soft KATs (wave 35491–35500 exclusive continuum markers):
 *   gj_shell_green_35500      → 1
 *   gj_libcgj_green_35500     → 1
 *   gj_bar3_ready_35500       → 0   (bar3 still open)
 *   gj_product_score_35500    → 0   (product score still open)
 *   gj_continuum_ready_35500  → 1
 *   gj_smoke_soft_35500       → 1
 *   gj_dyn_soft_35500         → 1
 *   gj_milestone_tag_35500    → 35500
 *   gj_continuum_wave_35500   → 35500
 *   gj_batch_id_35500         → 35500
 *   gj_graph_milestone_35500  → 35500
 *   __libcgj_batch35491_marker … __libcgj_batch35500_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch35491.c … graph_batch35500.c (and fill
 *     35476–35490 / identity 35451–35475) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 35500.
 *   - libc.map may lag milestone 35500 globals; add 35500 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_35500 == 0).
 ...
 *     continuum/smoke/dyn soft=1; milestone/wave/batch/graph = 35500.
 */
...
/* Grep: libcgj soft m35500 residual Soft!=product dual_dod_ab=OPEN G-AC-1 */
static const char g_szSoftHonesty[] =
	"libcgj: soft m35500 residual honesty Soft!=product dual_dod_a=OPEN "
...
#ifndef CGJ_SOFT_M35500_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M35500_DIRECT
/* Direct-link path: continuum soft pure markers (Soft!=product). */
extern uint32_t gj_shell_green_35500(void);
extern uint32_t gj_libcgj_green_35500(void);
extern uint32_t gj_bar3_ready_35500(void);
extern uint32_t gj_product_score_35500(void);
extern uint32_t gj_continuum_ready_35500(void);
extern uint32_t gj_smoke_soft_35500(void);
extern uint32_t gj_dyn_soft_35500(void);
extern uint32_t gj_milestone_tag_35500(void);
extern uint32_t gj_continuum_wave_35500(void);
extern uint32_t gj_batch_id_35500(void);
extern uint32_t gj_graph_milestone_35500(void);
extern const char __libcgj_batch35491_marker[];
extern const char __libcgj_batch35492_marker[];
extern const char __libcgj_batch35493_marker[];
extern const char __libcgj_batch35494_marker[];
extern const char __libcgj_batch35495_marker[];
extern const char __libcgj_batch35496_marker[];
extern const char __libcgj_batch35497_marker[];
extern const char __libcgj_batch35498_marker[];
extern const char __libcgj_batch35499_marker[];
extern const char __libcgj_batch35500_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_35500() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_35500 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_35500() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_35500 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_35500() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_35500 fail\n");
		nFail++;
	}
	if (gj_product_score_35500() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_35500 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_35500() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_35500 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_35500() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_35500 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_35500() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_35500 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_35500() != 35500u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_35500 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_35500() != 35500u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_35500 fail\n");
		nFail++;
	}
	if (gj_batch_id_35500() != 35500u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_35500 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_35500() != 35500u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_35500 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch35491_marker, "libcgj-batch35491") != 0 ||
	    strcmp(__libcgj_batch35492_marker, "libcgj-batch35492") != 0 ||
	    strcmp(__libcgj_batch35493_marker, "libcgj-batch35493") != 0 ||
	    strcmp(__libcgj_batch35494_marker, "libcgj-batch35494") != 0 ||
	    strcmp(__libcgj_batch35495_marker, "libcgj-batch35495") != 0 ||
	    strcmp(__libcgj_batch35496_marker, "libcgj-batch35496") != 0 ||
	    strcmp(__libcgj_batch35497_marker, "libcgj-batch35497") != 0 ||
	    strcmp(__libcgj_batch35498_marker, "libcgj-batch35498") != 0 ||
	    strcmp(__libcgj_batch35499_marker, "libcgj-batch35499") != 0 ||
	    strcmp(__libcgj_batch35500_marker, "libcgj-batch35500") != 0) {
		fprintf(stderr, "libcgj: direct m35500 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M35500_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m35500 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m35500 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m35500 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m35500 DIRECT PASS (all 11 lamps + markers; "
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m35500 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m35500 SOFT-SKIP (no SO; Soft!=product; "
	...
	nFail += soft_u32(h, "gj_shell_green_35500", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_35500", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_35500", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_35500", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_35500", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_35500", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_35500", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_35500", 35500u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_35500", 35500u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_35500", 35500u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_35500", 35500u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch35491_marker",
	                     "libcgj-batch35491", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch35492_marker",
	                     "libcgj-batch35492", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch35493_marker",
	                     "libcgj-batch35493", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch35494_marker",
	                     "libcgj-batch35494", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch35495_marker",
	                     "libcgj-batch35495", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch35496_marker",
	                     "libcgj-batch35496", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch35497_marker",
	                     "libcgj-batch35497", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch35498_marker",
	                     "libcgj-batch35498", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch35499_marker",
	                     "libcgj-batch35499", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch35500_marker",
	                     "libcgj-batch35500", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m35500 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m35500 PASS path=%s checked=%d "
