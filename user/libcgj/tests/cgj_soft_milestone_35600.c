 ...
 *
 * Soft host gates for continuum MILESTONE 35600 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_35600.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_35600.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_35600 \
 *     user/libcgj/tests/cgj_soft_milestone_35600.c -ldl
 *   ./build/cgj_soft_milestone_35600
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M35600_DIRECT \
 *     -o build/cgj_soft_milestone_35600_direct \
 *     user/libcgj/tests/cgj_soft_milestone_35600.c \
 *     user/libcgj/src/graph_batch35591.c \
 *     user/libcgj/src/graph_batch35592.c \
 *     user/libcgj/src/graph_batch35593.c \
 *     user/libcgj/src/graph_batch35594.c \
 *     user/libcgj/src/graph_batch35595.c \
 *     user/libcgj/src/graph_batch35596.c \
 *     user/libcgj/src/graph_batch35597.c \
 *     user/libcgj/src/graph_batch35598.c \
 *     user/libcgj/src/graph_batch35599.c \
 *     user/libcgj/src/graph_batch35600.c
 *   ./build/cgj_soft_milestone_35600_direct
 *
 * Expected soft KATs (wave 35591–35600 exclusive continuum markers):
 *   gj_shell_green_35600      → 1
 *   gj_libcgj_green_35600     → 1
 *   gj_bar3_ready_35600       → 0   (bar3 still open)
 *   gj_product_score_35600    → 0   (product score still open)
 *   gj_continuum_ready_35600  → 1
 *   gj_smoke_soft_35600       → 1
 *   gj_dyn_soft_35600         → 1
 *   gj_milestone_tag_35600    → 35600
 *   gj_continuum_wave_35600   → 35600
 *   gj_batch_id_35600         → 35600
 *   gj_graph_milestone_35600  → 35600
 *   __libcgj_batch35591_marker … __libcgj_batch35600_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch35591.c … graph_batch35600.c (and fill
 *     35576–35590 / identity 35551–35575) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 35600.
 *   - libc.map may lag milestone 35600 globals; add 35600 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_35600 == 0).
 ...
 *     continuum/smoke/dyn soft=1; milestone/wave/batch/graph = 35600.
 */
...
/* Grep: libcgj soft m35600 residual Soft!=product dual_dod_ab=OPEN G-AC-1 */
static const char g_szSoftHonesty[] =
	"libcgj: soft m35600 residual honesty Soft!=product dual_dod_a=OPEN "
...
#ifndef CGJ_SOFT_M35600_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M35600_DIRECT
/* Direct-link path: continuum soft pure markers (Soft!=product). */
extern uint32_t gj_shell_green_35600(void);
extern uint32_t gj_libcgj_green_35600(void);
extern uint32_t gj_bar3_ready_35600(void);
extern uint32_t gj_product_score_35600(void);
extern uint32_t gj_continuum_ready_35600(void);
extern uint32_t gj_smoke_soft_35600(void);
extern uint32_t gj_dyn_soft_35600(void);
extern uint32_t gj_milestone_tag_35600(void);
extern uint32_t gj_continuum_wave_35600(void);
extern uint32_t gj_batch_id_35600(void);
extern uint32_t gj_graph_milestone_35600(void);
extern const char __libcgj_batch35591_marker[];
extern const char __libcgj_batch35592_marker[];
extern const char __libcgj_batch35593_marker[];
extern const char __libcgj_batch35594_marker[];
extern const char __libcgj_batch35595_marker[];
extern const char __libcgj_batch35596_marker[];
extern const char __libcgj_batch35597_marker[];
extern const char __libcgj_batch35598_marker[];
extern const char __libcgj_batch35599_marker[];
extern const char __libcgj_batch35600_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_35600() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_35600 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_35600() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_35600 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_35600() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_35600 fail\n");
		nFail++;
	}
	if (gj_product_score_35600() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_35600 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_35600() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_35600 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_35600() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_35600 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_35600() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_35600 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_35600() != 35600u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_35600 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_35600() != 35600u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_35600 fail\n");
		nFail++;
	}
	if (gj_batch_id_35600() != 35600u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_35600 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_35600() != 35600u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_35600 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch35591_marker, "libcgj-batch35591") != 0 ||
	    strcmp(__libcgj_batch35592_marker, "libcgj-batch35592") != 0 ||
	    strcmp(__libcgj_batch35593_marker, "libcgj-batch35593") != 0 ||
	    strcmp(__libcgj_batch35594_marker, "libcgj-batch35594") != 0 ||
	    strcmp(__libcgj_batch35595_marker, "libcgj-batch35595") != 0 ||
	    strcmp(__libcgj_batch35596_marker, "libcgj-batch35596") != 0 ||
	    strcmp(__libcgj_batch35597_marker, "libcgj-batch35597") != 0 ||
	    strcmp(__libcgj_batch35598_marker, "libcgj-batch35598") != 0 ||
	    strcmp(__libcgj_batch35599_marker, "libcgj-batch35599") != 0 ||
	    strcmp(__libcgj_batch35600_marker, "libcgj-batch35600") != 0) {
		fprintf(stderr, "libcgj: direct m35600 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M35600_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m35600 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m35600 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m35600 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m35600 DIRECT PASS (all 11 lamps + markers; "
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m35600 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m35600 SOFT-SKIP (no SO; Soft!=product; "
	...
	nFail += soft_u32(h, "gj_shell_green_35600", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_35600", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_35600", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_35600", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_35600", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_35600", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_35600", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_35600", 35600u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_35600", 35600u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_35600", 35600u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_35600", 35600u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch35591_marker",
	                     "libcgj-batch35591", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch35592_marker",
	                     "libcgj-batch35592", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch35593_marker",
	                     "libcgj-batch35593", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch35594_marker",
	                     "libcgj-batch35594", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch35595_marker",
	                     "libcgj-batch35595", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch35596_marker",
	                     "libcgj-batch35596", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch35597_marker",
	                     "libcgj-batch35597", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch35598_marker",
	                     "libcgj-batch35598", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch35599_marker",
	                     "libcgj-batch35599", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch35600_marker",
	                     "libcgj-batch35600", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m35600 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m35600 PASS path=%s checked=%d "
