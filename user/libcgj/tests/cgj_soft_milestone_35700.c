 ...
 *
 * Soft host gates for continuum MILESTONE 35700 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_35700.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_35700.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_35700 \
 *     user/libcgj/tests/cgj_soft_milestone_35700.c -ldl
 *   ./build/cgj_soft_milestone_35700
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M35700_DIRECT \
 *     -o build/cgj_soft_milestone_35700_direct \
 *     user/libcgj/tests/cgj_soft_milestone_35700.c \
 *     user/libcgj/src/graph_batch35691.c \
 *     user/libcgj/src/graph_batch35692.c \
 *     user/libcgj/src/graph_batch35693.c \
 *     user/libcgj/src/graph_batch35694.c \
 *     user/libcgj/src/graph_batch35695.c \
 *     user/libcgj/src/graph_batch35696.c \
 *     user/libcgj/src/graph_batch35697.c \
 *     user/libcgj/src/graph_batch35698.c \
 *     user/libcgj/src/graph_batch35699.c \
 *     user/libcgj/src/graph_batch35700.c
 *   ./build/cgj_soft_milestone_35700_direct
 *
 * Expected soft KATs (wave 35691–35700 exclusive continuum markers):
 *   gj_shell_green_35700      → 1
 *   gj_libcgj_green_35700     → 1
 *   gj_bar3_ready_35700       → 0   (bar3 still open)
 *   gj_product_score_35700    → 0   (product score still open)
 *   gj_continuum_ready_35700  → 1
 *   gj_smoke_soft_35700       → 1
 *   gj_dyn_soft_35700         → 1
 *   gj_milestone_tag_35700    → 35700
 *   gj_continuum_wave_35700   → 35700
 *   gj_batch_id_35700         → 35700
 *   gj_graph_milestone_35700  → 35700
 *   __libcgj_batch35691_marker … __libcgj_batch35700_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch35691.c … graph_batch35700.c (and fill
 *     35676–35690 / identity 35651–35675) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 35700.
 *   - libc.map may lag milestone 35700 globals; add 35700 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_35700 == 0).
 ...
 *     continuum/smoke/dyn soft=1; milestone/wave/batch/graph = 35700.
 */
...
/* Grep: libcgj soft m35700 residual Soft!=product dual_dod_ab=OPEN G-AC-1 */
static const char g_szSoftHonesty[] =
	"libcgj: soft m35700 residual honesty Soft!=product dual_dod_a=OPEN "
...
#ifndef CGJ_SOFT_M35700_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M35700_DIRECT
/* Direct-link path: continuum soft pure markers (Soft!=product). */
extern uint32_t gj_shell_green_35700(void);
extern uint32_t gj_libcgj_green_35700(void);
extern uint32_t gj_bar3_ready_35700(void);
extern uint32_t gj_product_score_35700(void);
extern uint32_t gj_continuum_ready_35700(void);
extern uint32_t gj_smoke_soft_35700(void);
extern uint32_t gj_dyn_soft_35700(void);
extern uint32_t gj_milestone_tag_35700(void);
extern uint32_t gj_continuum_wave_35700(void);
extern uint32_t gj_batch_id_35700(void);
extern uint32_t gj_graph_milestone_35700(void);
extern const char __libcgj_batch35691_marker[];
extern const char __libcgj_batch35692_marker[];
extern const char __libcgj_batch35693_marker[];
extern const char __libcgj_batch35694_marker[];
extern const char __libcgj_batch35695_marker[];
extern const char __libcgj_batch35696_marker[];
extern const char __libcgj_batch35697_marker[];
extern const char __libcgj_batch35698_marker[];
extern const char __libcgj_batch35699_marker[];
extern const char __libcgj_batch35700_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_35700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_35700 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_35700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_35700 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_35700() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_35700 fail\n");
		nFail++;
	}
	if (gj_product_score_35700() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_35700 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_35700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_35700 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_35700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_35700 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_35700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_35700 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_35700() != 35700u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_35700 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_35700() != 35700u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_35700 fail\n");
		nFail++;
	}
	if (gj_batch_id_35700() != 35700u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_35700 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_35700() != 35700u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_35700 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch35691_marker, "libcgj-batch35691") != 0 ||
	    strcmp(__libcgj_batch35692_marker, "libcgj-batch35692") != 0 ||
	    strcmp(__libcgj_batch35693_marker, "libcgj-batch35693") != 0 ||
	    strcmp(__libcgj_batch35694_marker, "libcgj-batch35694") != 0 ||
	    strcmp(__libcgj_batch35695_marker, "libcgj-batch35695") != 0 ||
	    strcmp(__libcgj_batch35696_marker, "libcgj-batch35696") != 0 ||
	    strcmp(__libcgj_batch35697_marker, "libcgj-batch35697") != 0 ||
	    strcmp(__libcgj_batch35698_marker, "libcgj-batch35698") != 0 ||
	    strcmp(__libcgj_batch35699_marker, "libcgj-batch35699") != 0 ||
	    strcmp(__libcgj_batch35700_marker, "libcgj-batch35700") != 0) {
		fprintf(stderr, "libcgj: direct m35700 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M35700_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m35700 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m35700 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m35700 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m35700 DIRECT PASS (all 11 lamps + markers; "
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m35700 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m35700 SOFT-SKIP (no SO; Soft!=product; "
	...
	nFail += soft_u32(h, "gj_shell_green_35700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_35700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_35700", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_35700", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_35700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_35700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_35700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_35700", 35700u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_35700", 35700u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_35700", 35700u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_35700", 35700u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch35691_marker",
	                     "libcgj-batch35691", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch35692_marker",
	                     "libcgj-batch35692", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch35693_marker",
	                     "libcgj-batch35693", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch35694_marker",
	                     "libcgj-batch35694", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch35695_marker",
	                     "libcgj-batch35695", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch35696_marker",
	                     "libcgj-batch35696", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch35697_marker",
	                     "libcgj-batch35697", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch35698_marker",
	                     "libcgj-batch35698", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch35699_marker",
	                     "libcgj-batch35699", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch35700_marker",
	                     "libcgj-batch35700", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m35700 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m35700 PASS path=%s checked=%d "
