 ...
 *
 * Soft host gates for continuum MILESTONE 36300 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_36300.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_36300.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_36300 \
 *     user/libcgj/tests/cgj_soft_milestone_36300.c -ldl
 *   ./build/cgj_soft_milestone_36300
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M36300_DIRECT \
 *     -o build/cgj_soft_milestone_36300_direct \
 *     user/libcgj/tests/cgj_soft_milestone_36300.c \
 *     user/libcgj/src/graph_batch36291.c \
 *     user/libcgj/src/graph_batch36292.c \
 *     user/libcgj/src/graph_batch36293.c \
 *     user/libcgj/src/graph_batch36294.c \
 *     user/libcgj/src/graph_batch36295.c \
 *     user/libcgj/src/graph_batch36296.c \
 *     user/libcgj/src/graph_batch36297.c \
 *     user/libcgj/src/graph_batch36298.c \
 *     user/libcgj/src/graph_batch36299.c \
 *     user/libcgj/src/graph_batch36300.c
 *   ./build/cgj_soft_milestone_36300_direct
 *
 * Expected soft KATs (wave 36291–36300 exclusive continuum markers):
 *   gj_shell_green_36300      → 1
 *   gj_libcgj_green_36300     → 1
 *   gj_bar3_ready_36300       → 0   (bar3 still open)
 *   gj_product_score_36300    → 0   (product score still open)
 *   gj_continuum_ready_36300  → 1
 *   gj_smoke_soft_36300       → 1
 *   gj_dyn_soft_36300         → 1
 *   gj_milestone_tag_36300    → 36300
 *   gj_continuum_wave_36300   → 36300
 *   gj_batch_id_36300         → 36300
 *   gj_graph_milestone_36300  → 36300
 *   __libcgj_batch36291_marker … __libcgj_batch36300_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch36291.c … graph_batch36300.c (and fill
 *     36276–36290 / identity 36251–36275) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 36300.
 *   - libc.map may lag milestone 36300 globals; add 36300 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_36300 == 0).
 ...
 *     continuum/smoke/dyn soft=1; milestone/wave/batch/graph = 36300.
 */
...
/* Grep: libcgj soft m36300 residual Soft!=product dual_dod_ab=OPEN G-AC-1 */
static const char g_szSoftHonesty[] =
	"libcgj: soft m36300 residual honesty Soft!=product dual_dod_a=OPEN "
...
#ifndef CGJ_SOFT_M36300_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M36300_DIRECT
/* Direct-link path: continuum soft pure markers (Soft!=product). */
extern uint32_t gj_shell_green_36300(void);
extern uint32_t gj_libcgj_green_36300(void);
extern uint32_t gj_bar3_ready_36300(void);
extern uint32_t gj_product_score_36300(void);
extern uint32_t gj_continuum_ready_36300(void);
extern uint32_t gj_smoke_soft_36300(void);
extern uint32_t gj_dyn_soft_36300(void);
extern uint32_t gj_milestone_tag_36300(void);
extern uint32_t gj_continuum_wave_36300(void);
extern uint32_t gj_batch_id_36300(void);
extern uint32_t gj_graph_milestone_36300(void);
extern const char __libcgj_batch36291_marker[];
extern const char __libcgj_batch36292_marker[];
extern const char __libcgj_batch36293_marker[];
extern const char __libcgj_batch36294_marker[];
extern const char __libcgj_batch36295_marker[];
extern const char __libcgj_batch36296_marker[];
extern const char __libcgj_batch36297_marker[];
extern const char __libcgj_batch36298_marker[];
extern const char __libcgj_batch36299_marker[];
extern const char __libcgj_batch36300_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_36300() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_36300 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_36300() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_36300 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_36300() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_36300 fail\n");
		nFail++;
	}
	if (gj_product_score_36300() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_36300 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_36300() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_36300 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_36300() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_36300 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_36300() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_36300 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_36300() != 36300u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_36300 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_36300() != 36300u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_36300 fail\n");
		nFail++;
	}
	if (gj_batch_id_36300() != 36300u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_36300 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_36300() != 36300u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_36300 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch36291_marker, "libcgj-batch36291") != 0 ||
	    strcmp(__libcgj_batch36292_marker, "libcgj-batch36292") != 0 ||
	    strcmp(__libcgj_batch36293_marker, "libcgj-batch36293") != 0 ||
	    strcmp(__libcgj_batch36294_marker, "libcgj-batch36294") != 0 ||
	    strcmp(__libcgj_batch36295_marker, "libcgj-batch36295") != 0 ||
	    strcmp(__libcgj_batch36296_marker, "libcgj-batch36296") != 0 ||
	    strcmp(__libcgj_batch36297_marker, "libcgj-batch36297") != 0 ||
	    strcmp(__libcgj_batch36298_marker, "libcgj-batch36298") != 0 ||
	    strcmp(__libcgj_batch36299_marker, "libcgj-batch36299") != 0 ||
	    strcmp(__libcgj_batch36300_marker, "libcgj-batch36300") != 0) {
		fprintf(stderr, "libcgj: direct m36300 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M36300_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m36300 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m36300 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m36300 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m36300 DIRECT PASS (all 11 lamps + markers; "
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m36300 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m36300 SOFT-SKIP (no SO; Soft!=product; "
	...
	nFail += soft_u32(h, "gj_shell_green_36300", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_36300", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_36300", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_36300", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_36300", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_36300", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_36300", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_36300", 36300u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_36300", 36300u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_36300", 36300u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_36300", 36300u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch36291_marker",
	                     "libcgj-batch36291", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36292_marker",
	                     "libcgj-batch36292", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36293_marker",
	                     "libcgj-batch36293", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36294_marker",
	                     "libcgj-batch36294", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36295_marker",
	                     "libcgj-batch36295", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36296_marker",
	                     "libcgj-batch36296", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36297_marker",
	                     "libcgj-batch36297", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36298_marker",
	                     "libcgj-batch36298", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36299_marker",
	                     "libcgj-batch36299", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36300_marker",
	                     "libcgj-batch36300", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m36300 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m36300 PASS path=%s checked=%d "
