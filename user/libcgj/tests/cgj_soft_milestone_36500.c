 ...
 *
 * Soft host gates for continuum MILESTONE 36500 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_36500.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_36500.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_36500 \
 *     user/libcgj/tests/cgj_soft_milestone_36500.c -ldl
 *   ./build/cgj_soft_milestone_36500
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M36500_DIRECT \
 *     -o build/cgj_soft_milestone_36500_direct \
 *     user/libcgj/tests/cgj_soft_milestone_36500.c \
 *     user/libcgj/src/graph_batch36491.c \
 *     user/libcgj/src/graph_batch36492.c \
 *     user/libcgj/src/graph_batch36493.c \
 *     user/libcgj/src/graph_batch36494.c \
 *     user/libcgj/src/graph_batch36495.c \
 *     user/libcgj/src/graph_batch36496.c \
 *     user/libcgj/src/graph_batch36497.c \
 *     user/libcgj/src/graph_batch36498.c \
 *     user/libcgj/src/graph_batch36499.c \
 *     user/libcgj/src/graph_batch36500.c
 *   ./build/cgj_soft_milestone_36500_direct
 *
 * Expected soft KATs (wave 36491–36500 exclusive continuum markers):
 *   gj_shell_green_36500      → 1
 *   gj_libcgj_green_36500     → 1
 *   gj_bar3_ready_36500       → 0   (bar3 still open)
 *   gj_product_score_36500    → 0   (product score still open)
 *   gj_continuum_ready_36500  → 1
 *   gj_smoke_soft_36500       → 1
 *   gj_dyn_soft_36500         → 1
 *   gj_milestone_tag_36500    → 36500
 *   gj_continuum_wave_36500   → 36500
 *   gj_batch_id_36500         → 36500
 *   gj_graph_milestone_36500  → 36500
 *   __libcgj_batch36491_marker … __libcgj_batch36500_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch36491.c … graph_batch36500.c (and fill
 *     36476–36490 / identity 36451–36475) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 36500.
 *   - libc.map may lag milestone 36500 globals; add 36500 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_36500 == 0).
 ...
 *     continuum/smoke/dyn soft=1; milestone/wave/batch/graph = 36500.
 */
...
/* Grep: libcgj soft m36500 residual Soft!=product dual_dod_ab=OPEN G-AC-1 */
static const char g_szSoftHonesty[] =
	"libcgj: soft m36500 residual honesty Soft!=product dual_dod_a=OPEN "
...
#ifndef CGJ_SOFT_M36500_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M36500_DIRECT
/* Direct-link path: continuum soft pure markers (Soft!=product). */
extern uint32_t gj_shell_green_36500(void);
extern uint32_t gj_libcgj_green_36500(void);
extern uint32_t gj_bar3_ready_36500(void);
extern uint32_t gj_product_score_36500(void);
extern uint32_t gj_continuum_ready_36500(void);
extern uint32_t gj_smoke_soft_36500(void);
extern uint32_t gj_dyn_soft_36500(void);
extern uint32_t gj_milestone_tag_36500(void);
extern uint32_t gj_continuum_wave_36500(void);
extern uint32_t gj_batch_id_36500(void);
extern uint32_t gj_graph_milestone_36500(void);
extern const char __libcgj_batch36491_marker[];
extern const char __libcgj_batch36492_marker[];
extern const char __libcgj_batch36493_marker[];
extern const char __libcgj_batch36494_marker[];
extern const char __libcgj_batch36495_marker[];
extern const char __libcgj_batch36496_marker[];
extern const char __libcgj_batch36497_marker[];
extern const char __libcgj_batch36498_marker[];
extern const char __libcgj_batch36499_marker[];
extern const char __libcgj_batch36500_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_36500() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_36500 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_36500() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_36500 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_36500() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_36500 fail\n");
		nFail++;
	}
	if (gj_product_score_36500() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_36500 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_36500() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_36500 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_36500() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_36500 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_36500() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_36500 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_36500() != 36500u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_36500 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_36500() != 36500u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_36500 fail\n");
		nFail++;
	}
	if (gj_batch_id_36500() != 36500u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_36500 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_36500() != 36500u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_36500 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch36491_marker, "libcgj-batch36491") != 0 ||
	    strcmp(__libcgj_batch36492_marker, "libcgj-batch36492") != 0 ||
	    strcmp(__libcgj_batch36493_marker, "libcgj-batch36493") != 0 ||
	    strcmp(__libcgj_batch36494_marker, "libcgj-batch36494") != 0 ||
	    strcmp(__libcgj_batch36495_marker, "libcgj-batch36495") != 0 ||
	    strcmp(__libcgj_batch36496_marker, "libcgj-batch36496") != 0 ||
	    strcmp(__libcgj_batch36497_marker, "libcgj-batch36497") != 0 ||
	    strcmp(__libcgj_batch36498_marker, "libcgj-batch36498") != 0 ||
	    strcmp(__libcgj_batch36499_marker, "libcgj-batch36499") != 0 ||
	    strcmp(__libcgj_batch36500_marker, "libcgj-batch36500") != 0) {
		fprintf(stderr, "libcgj: direct m36500 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M36500_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m36500 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m36500 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m36500 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m36500 DIRECT PASS (all 11 lamps + markers; "
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m36500 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m36500 SOFT-SKIP (no SO; Soft!=product; "
	...
	nFail += soft_u32(h, "gj_shell_green_36500", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_36500", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_36500", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_36500", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_36500", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_36500", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_36500", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_36500", 36500u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_36500", 36500u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_36500", 36500u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_36500", 36500u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch36491_marker",
	                     "libcgj-batch36491", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36492_marker",
	                     "libcgj-batch36492", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36493_marker",
	                     "libcgj-batch36493", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36494_marker",
	                     "libcgj-batch36494", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36495_marker",
	                     "libcgj-batch36495", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36496_marker",
	                     "libcgj-batch36496", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36497_marker",
	                     "libcgj-batch36497", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36498_marker",
	                     "libcgj-batch36498", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36499_marker",
	                     "libcgj-batch36499", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36500_marker",
	                     "libcgj-batch36500", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m36500 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m36500 PASS path=%s checked=%d "
