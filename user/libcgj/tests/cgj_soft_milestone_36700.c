 ...
 *
 * Soft host gates for continuum MILESTONE 36700 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_36700.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_36700.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_36700 \
 *     user/libcgj/tests/cgj_soft_milestone_36700.c -ldl
 *   ./build/cgj_soft_milestone_36700
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M36700_DIRECT \
 *     -o build/cgj_soft_milestone_36700_direct \
 *     user/libcgj/tests/cgj_soft_milestone_36700.c \
 *     user/libcgj/src/graph_batch36691.c \
 *     user/libcgj/src/graph_batch36692.c \
 *     user/libcgj/src/graph_batch36693.c \
 *     user/libcgj/src/graph_batch36694.c \
 *     user/libcgj/src/graph_batch36695.c \
 *     user/libcgj/src/graph_batch36696.c \
 *     user/libcgj/src/graph_batch36697.c \
 *     user/libcgj/src/graph_batch36698.c \
 *     user/libcgj/src/graph_batch36699.c \
 *     user/libcgj/src/graph_batch36700.c
 *   ./build/cgj_soft_milestone_36700_direct
 *
 * Expected soft KATs (wave 36691–36700 exclusive continuum markers):
 *   gj_shell_green_36700      → 1
 *   gj_libcgj_green_36700     → 1
 *   gj_bar3_ready_36700       → 0   (bar3 still open)
 *   gj_product_score_36700    → 0   (product score still open)
 *   gj_continuum_ready_36700  → 1
 *   gj_smoke_soft_36700       → 1
 *   gj_dyn_soft_36700         → 1
 *   gj_milestone_tag_36700    → 36700
 *   gj_continuum_wave_36700   → 36700
 *   gj_batch_id_36700         → 36700
 *   gj_graph_milestone_36700  → 36700
 *   __libcgj_batch36691_marker … __libcgj_batch36700_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch36691.c … graph_batch36700.c (and fill
 *     36676–36690 / identity 36651–36675) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 36700.
 *   - libc.map may lag milestone 36700 globals; add 36700 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_36700 == 0).
 ...
 *     continuum/smoke/dyn soft=1; milestone/wave/batch/graph = 36700.
 */
...
/* Grep: libcgj soft m36700 residual Soft!=product dual_dod_ab=OPEN G-AC-1 */
static const char g_szSoftHonesty[] =
	"libcgj: soft m36700 residual honesty Soft!=product dual_dod_a=OPEN "
...
#ifndef CGJ_SOFT_M36700_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M36700_DIRECT
/* Direct-link path: continuum soft pure markers (Soft!=product). */
extern uint32_t gj_shell_green_36700(void);
extern uint32_t gj_libcgj_green_36700(void);
extern uint32_t gj_bar3_ready_36700(void);
extern uint32_t gj_product_score_36700(void);
extern uint32_t gj_continuum_ready_36700(void);
extern uint32_t gj_smoke_soft_36700(void);
extern uint32_t gj_dyn_soft_36700(void);
extern uint32_t gj_milestone_tag_36700(void);
extern uint32_t gj_continuum_wave_36700(void);
extern uint32_t gj_batch_id_36700(void);
extern uint32_t gj_graph_milestone_36700(void);
extern const char __libcgj_batch36691_marker[];
extern const char __libcgj_batch36692_marker[];
extern const char __libcgj_batch36693_marker[];
extern const char __libcgj_batch36694_marker[];
extern const char __libcgj_batch36695_marker[];
extern const char __libcgj_batch36696_marker[];
extern const char __libcgj_batch36697_marker[];
extern const char __libcgj_batch36698_marker[];
extern const char __libcgj_batch36699_marker[];
extern const char __libcgj_batch36700_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_36700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_36700 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_36700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_36700 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_36700() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_36700 fail\n");
		nFail++;
	}
	if (gj_product_score_36700() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_36700 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_36700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_36700 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_36700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_36700 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_36700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_36700 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_36700() != 36700u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_36700 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_36700() != 36700u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_36700 fail\n");
		nFail++;
	}
	if (gj_batch_id_36700() != 36700u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_36700 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_36700() != 36700u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_36700 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch36691_marker, "libcgj-batch36691") != 0 ||
	    strcmp(__libcgj_batch36692_marker, "libcgj-batch36692") != 0 ||
	    strcmp(__libcgj_batch36693_marker, "libcgj-batch36693") != 0 ||
	    strcmp(__libcgj_batch36694_marker, "libcgj-batch36694") != 0 ||
	    strcmp(__libcgj_batch36695_marker, "libcgj-batch36695") != 0 ||
	    strcmp(__libcgj_batch36696_marker, "libcgj-batch36696") != 0 ||
	    strcmp(__libcgj_batch36697_marker, "libcgj-batch36697") != 0 ||
	    strcmp(__libcgj_batch36698_marker, "libcgj-batch36698") != 0 ||
	    strcmp(__libcgj_batch36699_marker, "libcgj-batch36699") != 0 ||
	    strcmp(__libcgj_batch36700_marker, "libcgj-batch36700") != 0) {
		fprintf(stderr, "libcgj: direct m36700 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M36700_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m36700 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m36700 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m36700 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m36700 DIRECT PASS (all 11 lamps + markers; "
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m36700 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m36700 SOFT-SKIP (no SO; Soft!=product; "
	...
	nFail += soft_u32(h, "gj_shell_green_36700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_36700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_36700", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_36700", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_36700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_36700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_36700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_36700", 36700u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_36700", 36700u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_36700", 36700u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_36700", 36700u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch36691_marker",
	                     "libcgj-batch36691", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36692_marker",
	                     "libcgj-batch36692", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36693_marker",
	                     "libcgj-batch36693", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36694_marker",
	                     "libcgj-batch36694", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36695_marker",
	                     "libcgj-batch36695", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36696_marker",
	                     "libcgj-batch36696", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36697_marker",
	                     "libcgj-batch36697", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36698_marker",
	                     "libcgj-batch36698", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36699_marker",
	                     "libcgj-batch36699", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36700_marker",
	                     "libcgj-batch36700", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m36700 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m36700 PASS path=%s checked=%d "
