 ...
 *
 * Soft host gates for continuum MILESTONE 36800 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_36800.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_36800.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_36800 \
 *     user/libcgj/tests/cgj_soft_milestone_36800.c -ldl
 *   ./build/cgj_soft_milestone_36800
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M36800_DIRECT \
 *     -o build/cgj_soft_milestone_36800_direct \
 *     user/libcgj/tests/cgj_soft_milestone_36800.c \
 *     user/libcgj/src/graph_batch36791.c \
 *     user/libcgj/src/graph_batch36792.c \
 *     user/libcgj/src/graph_batch36793.c \
 *     user/libcgj/src/graph_batch36794.c \
 *     user/libcgj/src/graph_batch36795.c \
 *     user/libcgj/src/graph_batch36796.c \
 *     user/libcgj/src/graph_batch36797.c \
 *     user/libcgj/src/graph_batch36798.c \
 *     user/libcgj/src/graph_batch36799.c \
 *     user/libcgj/src/graph_batch36800.c
 *   ./build/cgj_soft_milestone_36800_direct
 *
 * Expected soft KATs (wave 36791–36800 exclusive continuum markers):
 *   gj_shell_green_36800      → 1
 *   gj_libcgj_green_36800     → 1
 *   gj_bar3_ready_36800       → 0   (bar3 still open)
 *   gj_product_score_36800    → 0   (product score still open)
 *   gj_continuum_ready_36800  → 1
 *   gj_smoke_soft_36800       → 1
 *   gj_dyn_soft_36800         → 1
 *   gj_milestone_tag_36800    → 36800
 *   gj_continuum_wave_36800   → 36800
 *   gj_batch_id_36800         → 36800
 *   gj_graph_milestone_36800  → 36800
 *   __libcgj_batch36791_marker … __libcgj_batch36800_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch36791.c … graph_batch36800.c (and fill
 *     36776–36790 / identity 36751–36775) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 36800.
 *   - libc.map may lag milestone 36800 globals; add 36800 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_36800 == 0).
 ...
 *     continuum/smoke/dyn soft=1; milestone/wave/batch/graph = 36800.
 */
...
/* Grep: libcgj soft m36800 residual Soft!=product dual_dod_ab=OPEN G-AC-1 */
static const char g_szSoftHonesty[] =
	"libcgj: soft m36800 residual honesty Soft!=product dual_dod_a=OPEN "
...
#ifndef CGJ_SOFT_M36800_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M36800_DIRECT
/* Direct-link path: continuum soft pure markers (Soft!=product). */
extern uint32_t gj_shell_green_36800(void);
extern uint32_t gj_libcgj_green_36800(void);
extern uint32_t gj_bar3_ready_36800(void);
extern uint32_t gj_product_score_36800(void);
extern uint32_t gj_continuum_ready_36800(void);
extern uint32_t gj_smoke_soft_36800(void);
extern uint32_t gj_dyn_soft_36800(void);
extern uint32_t gj_milestone_tag_36800(void);
extern uint32_t gj_continuum_wave_36800(void);
extern uint32_t gj_batch_id_36800(void);
extern uint32_t gj_graph_milestone_36800(void);
extern const char __libcgj_batch36791_marker[];
extern const char __libcgj_batch36792_marker[];
extern const char __libcgj_batch36793_marker[];
extern const char __libcgj_batch36794_marker[];
extern const char __libcgj_batch36795_marker[];
extern const char __libcgj_batch36796_marker[];
extern const char __libcgj_batch36797_marker[];
extern const char __libcgj_batch36798_marker[];
extern const char __libcgj_batch36799_marker[];
extern const char __libcgj_batch36800_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_36800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_36800 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_36800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_36800 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_36800() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_36800 fail\n");
		nFail++;
	}
	if (gj_product_score_36800() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_36800 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_36800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_36800 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_36800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_36800 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_36800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_36800 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_36800() != 36800u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_36800 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_36800() != 36800u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_36800 fail\n");
		nFail++;
	}
	if (gj_batch_id_36800() != 36800u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_36800 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_36800() != 36800u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_36800 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch36791_marker, "libcgj-batch36791") != 0 ||
	    strcmp(__libcgj_batch36792_marker, "libcgj-batch36792") != 0 ||
	    strcmp(__libcgj_batch36793_marker, "libcgj-batch36793") != 0 ||
	    strcmp(__libcgj_batch36794_marker, "libcgj-batch36794") != 0 ||
	    strcmp(__libcgj_batch36795_marker, "libcgj-batch36795") != 0 ||
	    strcmp(__libcgj_batch36796_marker, "libcgj-batch36796") != 0 ||
	    strcmp(__libcgj_batch36797_marker, "libcgj-batch36797") != 0 ||
	    strcmp(__libcgj_batch36798_marker, "libcgj-batch36798") != 0 ||
	    strcmp(__libcgj_batch36799_marker, "libcgj-batch36799") != 0 ||
	    strcmp(__libcgj_batch36800_marker, "libcgj-batch36800") != 0) {
		fprintf(stderr, "libcgj: direct m36800 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M36800_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m36800 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m36800 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m36800 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m36800 DIRECT PASS (all 11 lamps + markers; "
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m36800 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m36800 SOFT-SKIP (no SO; Soft!=product; "
	...
	nFail += soft_u32(h, "gj_shell_green_36800", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_36800", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_36800", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_36800", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_36800", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_36800", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_36800", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_36800", 36800u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_36800", 36800u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_36800", 36800u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_36800", 36800u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch36791_marker",
	                     "libcgj-batch36791", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36792_marker",
	                     "libcgj-batch36792", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36793_marker",
	                     "libcgj-batch36793", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36794_marker",
	                     "libcgj-batch36794", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36795_marker",
	                     "libcgj-batch36795", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36796_marker",
	                     "libcgj-batch36796", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36797_marker",
	                     "libcgj-batch36797", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36798_marker",
	                     "libcgj-batch36798", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36799_marker",
	                     "libcgj-batch36799", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36800_marker",
	                     "libcgj-batch36800", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m36800 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m36800 PASS path=%s checked=%d "
