 ...
 *
 * Soft host gates for continuum MILESTONE 36900 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_36900.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_36900.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_36900 \
 *     user/libcgj/tests/cgj_soft_milestone_36900.c -ldl
 *   ./build/cgj_soft_milestone_36900
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M36900_DIRECT \
 *     -o build/cgj_soft_milestone_36900_direct \
 *     user/libcgj/tests/cgj_soft_milestone_36900.c \
 *     user/libcgj/src/graph_batch36891.c \
 *     user/libcgj/src/graph_batch36892.c \
 *     user/libcgj/src/graph_batch36893.c \
 *     user/libcgj/src/graph_batch36894.c \
 *     user/libcgj/src/graph_batch36895.c \
 *     user/libcgj/src/graph_batch36896.c \
 *     user/libcgj/src/graph_batch36897.c \
 *     user/libcgj/src/graph_batch36898.c \
 *     user/libcgj/src/graph_batch36899.c \
 *     user/libcgj/src/graph_batch36900.c
 *   ./build/cgj_soft_milestone_36900_direct
 *
 * Expected soft KATs (wave 36891–36900 exclusive continuum markers):
 *   gj_shell_green_36900      → 1
 *   gj_libcgj_green_36900     → 1
 *   gj_bar3_ready_36900       → 0   (bar3 still open)
 *   gj_product_score_36900    → 0   (product score still open)
 *   gj_continuum_ready_36900  → 1
 *   gj_smoke_soft_36900       → 1
 *   gj_dyn_soft_36900         → 1
 *   gj_milestone_tag_36900    → 36900
 *   gj_continuum_wave_36900   → 36900
 *   gj_batch_id_36900         → 36900
 *   gj_graph_milestone_36900  → 36900
 *   __libcgj_batch36891_marker … __libcgj_batch36900_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch36891.c … graph_batch36900.c (and fill
 *     36876–36890 / identity 36851–36875) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 36900.
 *   - libc.map may lag milestone 36900 globals; add 36900 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_36900 == 0).
 ...
 *     continuum/smoke/dyn soft=1; milestone/wave/batch/graph = 36900.
 */
...
/* Grep: libcgj soft m36900 residual Soft!=product dual_dod_ab=OPEN G-AC-1 */
static const char g_szSoftHonesty[] =
	"libcgj: soft m36900 residual honesty Soft!=product dual_dod_a=OPEN "
...
#ifndef CGJ_SOFT_M36900_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M36900_DIRECT
/* Direct-link path: continuum soft pure markers (Soft!=product). */
extern uint32_t gj_shell_green_36900(void);
extern uint32_t gj_libcgj_green_36900(void);
extern uint32_t gj_bar3_ready_36900(void);
extern uint32_t gj_product_score_36900(void);
extern uint32_t gj_continuum_ready_36900(void);
extern uint32_t gj_smoke_soft_36900(void);
extern uint32_t gj_dyn_soft_36900(void);
extern uint32_t gj_milestone_tag_36900(void);
extern uint32_t gj_continuum_wave_36900(void);
extern uint32_t gj_batch_id_36900(void);
extern uint32_t gj_graph_milestone_36900(void);
extern const char __libcgj_batch36891_marker[];
extern const char __libcgj_batch36892_marker[];
extern const char __libcgj_batch36893_marker[];
extern const char __libcgj_batch36894_marker[];
extern const char __libcgj_batch36895_marker[];
extern const char __libcgj_batch36896_marker[];
extern const char __libcgj_batch36897_marker[];
extern const char __libcgj_batch36898_marker[];
extern const char __libcgj_batch36899_marker[];
extern const char __libcgj_batch36900_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_36900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_36900 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_36900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_36900 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_36900() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_36900 fail\n");
		nFail++;
	}
	if (gj_product_score_36900() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_36900 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_36900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_36900 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_36900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_36900 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_36900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_36900 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_36900() != 36900u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_36900 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_36900() != 36900u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_36900 fail\n");
		nFail++;
	}
	if (gj_batch_id_36900() != 36900u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_36900 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_36900() != 36900u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_36900 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch36891_marker, "libcgj-batch36891") != 0 ||
	    strcmp(__libcgj_batch36892_marker, "libcgj-batch36892") != 0 ||
	    strcmp(__libcgj_batch36893_marker, "libcgj-batch36893") != 0 ||
	    strcmp(__libcgj_batch36894_marker, "libcgj-batch36894") != 0 ||
	    strcmp(__libcgj_batch36895_marker, "libcgj-batch36895") != 0 ||
	    strcmp(__libcgj_batch36896_marker, "libcgj-batch36896") != 0 ||
	    strcmp(__libcgj_batch36897_marker, "libcgj-batch36897") != 0 ||
	    strcmp(__libcgj_batch36898_marker, "libcgj-batch36898") != 0 ||
	    strcmp(__libcgj_batch36899_marker, "libcgj-batch36899") != 0 ||
	    strcmp(__libcgj_batch36900_marker, "libcgj-batch36900") != 0) {
		fprintf(stderr, "libcgj: direct m36900 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M36900_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m36900 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m36900 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m36900 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m36900 DIRECT PASS (all 11 lamps + markers; "
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m36900 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m36900 SOFT-SKIP (no SO; Soft!=product; "
	...
	nFail += soft_u32(h, "gj_shell_green_36900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_36900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_36900", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_36900", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_36900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_36900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_36900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_36900", 36900u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_36900", 36900u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_36900", 36900u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_36900", 36900u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch36891_marker",
	                     "libcgj-batch36891", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36892_marker",
	                     "libcgj-batch36892", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36893_marker",
	                     "libcgj-batch36893", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36894_marker",
	                     "libcgj-batch36894", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36895_marker",
	                     "libcgj-batch36895", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36896_marker",
	                     "libcgj-batch36896", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36897_marker",
	                     "libcgj-batch36897", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36898_marker",
	                     "libcgj-batch36898", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36899_marker",
	                     "libcgj-batch36899", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36900_marker",
	                     "libcgj-batch36900", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m36900 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m36900 PASS path=%s checked=%d "
