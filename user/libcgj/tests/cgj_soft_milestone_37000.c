 ...
 *
 * Soft host gates for continuum MILESTONE 37000 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_37000.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_37000.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_37000 \
 *     user/libcgj/tests/cgj_soft_milestone_37000.c -ldl
 *   ./build/cgj_soft_milestone_37000
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M37000_DIRECT \
 *     -o build/cgj_soft_milestone_37000_direct \
 *     user/libcgj/tests/cgj_soft_milestone_37000.c \
 *     user/libcgj/src/graph_batch36991.c \
 *     user/libcgj/src/graph_batch36992.c \
 *     user/libcgj/src/graph_batch36993.c \
 *     user/libcgj/src/graph_batch36994.c \
 *     user/libcgj/src/graph_batch36995.c \
 *     user/libcgj/src/graph_batch36996.c \
 *     user/libcgj/src/graph_batch36997.c \
 *     user/libcgj/src/graph_batch36998.c \
 *     user/libcgj/src/graph_batch36999.c \
 *     user/libcgj/src/graph_batch37000.c
 *   ./build/cgj_soft_milestone_37000_direct
 *
 * Expected soft KATs (wave 36991–37000 exclusive continuum markers):
 *   gj_shell_green_37000      → 1
 *   gj_libcgj_green_37000     → 1
 *   gj_bar3_ready_37000       → 0   (bar3 still open)
 *   gj_product_score_37000    → 0   (product score still open)
 *   gj_continuum_ready_37000  → 1
 *   gj_smoke_soft_37000       → 1
 *   gj_dyn_soft_37000         → 1
 *   gj_milestone_tag_37000    → 37000
 *   gj_continuum_wave_37000   → 37000
 *   gj_batch_id_37000         → 37000
 *   gj_graph_milestone_37000  → 37000
 *   __libcgj_batch36991_marker … __libcgj_batch37000_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch36991.c … graph_batch37000.c (and fill
 *     36976–36990 / identity 36951–36975) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 37000.
 *   - libc.map may lag milestone 37000 globals; add 37000 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_37000 == 0).
 ...
 *     continuum/smoke/dyn soft=1; milestone/wave/batch/graph = 37000.
 */
...
/* Grep: libcgj soft m37000 residual Soft!=product dual_dod_ab=OPEN G-AC-1 */
static const char g_szSoftHonesty[] =
	"libcgj: soft m37000 residual honesty Soft!=product dual_dod_a=OPEN "
...
#ifndef CGJ_SOFT_M37000_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M37000_DIRECT
/* Direct-link path: continuum soft pure markers (Soft!=product). */
extern uint32_t gj_shell_green_37000(void);
extern uint32_t gj_libcgj_green_37000(void);
extern uint32_t gj_bar3_ready_37000(void);
extern uint32_t gj_product_score_37000(void);
extern uint32_t gj_continuum_ready_37000(void);
extern uint32_t gj_smoke_soft_37000(void);
extern uint32_t gj_dyn_soft_37000(void);
extern uint32_t gj_milestone_tag_37000(void);
extern uint32_t gj_continuum_wave_37000(void);
extern uint32_t gj_batch_id_37000(void);
extern uint32_t gj_graph_milestone_37000(void);
extern const char __libcgj_batch36991_marker[];
extern const char __libcgj_batch36992_marker[];
extern const char __libcgj_batch36993_marker[];
extern const char __libcgj_batch36994_marker[];
extern const char __libcgj_batch36995_marker[];
extern const char __libcgj_batch36996_marker[];
extern const char __libcgj_batch36997_marker[];
extern const char __libcgj_batch36998_marker[];
extern const char __libcgj_batch36999_marker[];
extern const char __libcgj_batch37000_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_37000() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_37000 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_37000() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_37000 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_37000() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_37000 fail\n");
		nFail++;
	}
	if (gj_product_score_37000() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_37000 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_37000() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_37000 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_37000() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_37000 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_37000() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_37000 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_37000() != 37000u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_37000 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_37000() != 37000u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_37000 fail\n");
		nFail++;
	}
	if (gj_batch_id_37000() != 37000u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_37000 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_37000() != 37000u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_37000 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch36991_marker, "libcgj-batch36991") != 0 ||
	    strcmp(__libcgj_batch36992_marker, "libcgj-batch36992") != 0 ||
	    strcmp(__libcgj_batch36993_marker, "libcgj-batch36993") != 0 ||
	    strcmp(__libcgj_batch36994_marker, "libcgj-batch36994") != 0 ||
	    strcmp(__libcgj_batch36995_marker, "libcgj-batch36995") != 0 ||
	    strcmp(__libcgj_batch36996_marker, "libcgj-batch36996") != 0 ||
	    strcmp(__libcgj_batch36997_marker, "libcgj-batch36997") != 0 ||
	    strcmp(__libcgj_batch36998_marker, "libcgj-batch36998") != 0 ||
	    strcmp(__libcgj_batch36999_marker, "libcgj-batch36999") != 0 ||
	    strcmp(__libcgj_batch37000_marker, "libcgj-batch37000") != 0) {
		fprintf(stderr, "libcgj: direct m37000 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M37000_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m37000 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m37000 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m37000 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m37000 DIRECT PASS (all 11 lamps + markers; "
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m37000 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m37000 SOFT-SKIP (no SO; Soft!=product; "
	...
	nFail += soft_u32(h, "gj_shell_green_37000", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_37000", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_37000", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_37000", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_37000", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_37000", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_37000", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_37000", 37000u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_37000", 37000u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_37000", 37000u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_37000", 37000u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch36991_marker",
	                     "libcgj-batch36991", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36992_marker",
	                     "libcgj-batch36992", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36993_marker",
	                     "libcgj-batch36993", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36994_marker",
	                     "libcgj-batch36994", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36995_marker",
	                     "libcgj-batch36995", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36996_marker",
	                     "libcgj-batch36996", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36997_marker",
	                     "libcgj-batch36997", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36998_marker",
	                     "libcgj-batch36998", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch36999_marker",
	                     "libcgj-batch36999", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch37000_marker",
	                     "libcgj-batch37000", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m37000 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m37000 PASS path=%s checked=%d "
