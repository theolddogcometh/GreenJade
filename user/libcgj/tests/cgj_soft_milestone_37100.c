 ...
 *
 * Soft host gates for continuum MILESTONE 37100 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_37100.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_37100.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_37100 \
 *     user/libcgj/tests/cgj_soft_milestone_37100.c -ldl
 *   ./build/cgj_soft_milestone_37100
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M37100_DIRECT \
 *     -o build/cgj_soft_milestone_37100_direct \
 *     user/libcgj/tests/cgj_soft_milestone_37100.c \
 *     user/libcgj/src/graph_batch37091.c \
 *     user/libcgj/src/graph_batch37092.c \
 *     user/libcgj/src/graph_batch37093.c \
 *     user/libcgj/src/graph_batch37094.c \
 *     user/libcgj/src/graph_batch37095.c \
 *     user/libcgj/src/graph_batch37096.c \
 *     user/libcgj/src/graph_batch37097.c \
 *     user/libcgj/src/graph_batch37098.c \
 *     user/libcgj/src/graph_batch37099.c \
 *     user/libcgj/src/graph_batch37100.c
 *   ./build/cgj_soft_milestone_37100_direct
 *
 * Expected soft KATs (wave 37091–37100 exclusive continuum markers):
 *   gj_shell_green_37100      → 1
 *   gj_libcgj_green_37100     → 1
 *   gj_bar3_ready_37100       → 0   (bar3 still open)
 *   gj_product_score_37100    → 0   (product score still open)
 *   gj_continuum_ready_37100  → 1
 *   gj_smoke_soft_37100       → 1
 *   gj_dyn_soft_37100         → 1
 *   gj_milestone_tag_37100    → 37100
 *   gj_continuum_wave_37100   → 37100
 *   gj_batch_id_37100         → 37100
 *   gj_graph_milestone_37100  → 37100
 *   __libcgj_batch37091_marker … __libcgj_batch37100_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch37091.c … graph_batch37100.c (and fill
 *     37076–37090 / identity 37051–37075) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 37100.
 *   - libc.map may lag milestone 37100 globals; add 37100 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_37100 == 0).
 ...
 *     continuum/smoke/dyn soft=1; milestone/wave/batch/graph = 37100.
 */
...
/* Grep: libcgj soft m37100 residual Soft!=product dual_dod_ab=OPEN G-AC-1 */
static const char g_szSoftHonesty[] =
	"libcgj: soft m37100 residual honesty Soft!=product dual_dod_a=OPEN "
...
#ifndef CGJ_SOFT_M37100_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M37100_DIRECT
/* Direct-link path: continuum soft pure markers (Soft!=product). */
extern uint32_t gj_shell_green_37100(void);
extern uint32_t gj_libcgj_green_37100(void);
extern uint32_t gj_bar3_ready_37100(void);
extern uint32_t gj_product_score_37100(void);
extern uint32_t gj_continuum_ready_37100(void);
extern uint32_t gj_smoke_soft_37100(void);
extern uint32_t gj_dyn_soft_37100(void);
extern uint32_t gj_milestone_tag_37100(void);
extern uint32_t gj_continuum_wave_37100(void);
extern uint32_t gj_batch_id_37100(void);
extern uint32_t gj_graph_milestone_37100(void);
extern const char __libcgj_batch37091_marker[];
extern const char __libcgj_batch37092_marker[];
extern const char __libcgj_batch37093_marker[];
extern const char __libcgj_batch37094_marker[];
extern const char __libcgj_batch37095_marker[];
extern const char __libcgj_batch37096_marker[];
extern const char __libcgj_batch37097_marker[];
extern const char __libcgj_batch37098_marker[];
extern const char __libcgj_batch37099_marker[];
extern const char __libcgj_batch37100_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_37100() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_37100 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_37100() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_37100 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_37100() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_37100 fail\n");
		nFail++;
	}
	if (gj_product_score_37100() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_37100 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_37100() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_37100 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_37100() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_37100 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_37100() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_37100 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_37100() != 37100u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_37100 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_37100() != 37100u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_37100 fail\n");
		nFail++;
	}
	if (gj_batch_id_37100() != 37100u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_37100 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_37100() != 37100u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_37100 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch37091_marker, "libcgj-batch37091") != 0 ||
	    strcmp(__libcgj_batch37092_marker, "libcgj-batch37092") != 0 ||
	    strcmp(__libcgj_batch37093_marker, "libcgj-batch37093") != 0 ||
	    strcmp(__libcgj_batch37094_marker, "libcgj-batch37094") != 0 ||
	    strcmp(__libcgj_batch37095_marker, "libcgj-batch37095") != 0 ||
	    strcmp(__libcgj_batch37096_marker, "libcgj-batch37096") != 0 ||
	    strcmp(__libcgj_batch37097_marker, "libcgj-batch37097") != 0 ||
	    strcmp(__libcgj_batch37098_marker, "libcgj-batch37098") != 0 ||
	    strcmp(__libcgj_batch37099_marker, "libcgj-batch37099") != 0 ||
	    strcmp(__libcgj_batch37100_marker, "libcgj-batch37100") != 0) {
		fprintf(stderr, "libcgj: direct m37100 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M37100_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m37100 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m37100 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m37100 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m37100 DIRECT PASS (all 11 lamps + markers; "
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m37100 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m37100 SOFT-SKIP (no SO; Soft!=product; "
	...
	nFail += soft_u32(h, "gj_shell_green_37100", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_37100", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_37100", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_37100", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_37100", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_37100", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_37100", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_37100", 37100u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_37100", 37100u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_37100", 37100u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_37100", 37100u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch37091_marker",
	                     "libcgj-batch37091", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch37092_marker",
	                     "libcgj-batch37092", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch37093_marker",
	                     "libcgj-batch37093", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch37094_marker",
	                     "libcgj-batch37094", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch37095_marker",
	                     "libcgj-batch37095", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch37096_marker",
	                     "libcgj-batch37096", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch37097_marker",
	                     "libcgj-batch37097", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch37098_marker",
	                     "libcgj-batch37098", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch37099_marker",
	                     "libcgj-batch37099", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch37100_marker",
	                     "libcgj-batch37100", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m37100 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m37100 PASS path=%s checked=%d "
