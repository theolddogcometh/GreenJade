 ...
 *
 * Soft host gates for continuum MILESTONE 37300 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_37300.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_37300.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_37300 \
 *     user/libcgj/tests/cgj_soft_milestone_37300.c -ldl
 *   ./build/cgj_soft_milestone_37300
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M37300_DIRECT \
 *     -o build/cgj_soft_milestone_37300_direct \
 *     user/libcgj/tests/cgj_soft_milestone_37300.c \
 *     user/libcgj/src/graph_batch37291.c \
 *     user/libcgj/src/graph_batch37292.c \
 *     user/libcgj/src/graph_batch37293.c \
 *     user/libcgj/src/graph_batch37294.c \
 *     user/libcgj/src/graph_batch37295.c \
 *     user/libcgj/src/graph_batch37296.c \
 *     user/libcgj/src/graph_batch37297.c \
 *     user/libcgj/src/graph_batch37298.c \
 *     user/libcgj/src/graph_batch37299.c \
 *     user/libcgj/src/graph_batch37300.c
 *   ./build/cgj_soft_milestone_37300_direct
 *
 * Expected soft KATs (wave 37291–37300 exclusive continuum markers):
 *   gj_shell_green_37300      → 1
 *   gj_libcgj_green_37300     → 1
 *   gj_bar3_ready_37300       → 0   (bar3 still open)
 *   gj_product_score_37300    → 0   (product score still open)
 *   gj_continuum_ready_37300  → 1
 *   gj_smoke_soft_37300       → 1
 *   gj_dyn_soft_37300         → 1
 *   gj_milestone_tag_37300    → 37300
 *   gj_continuum_wave_37300   → 37300
 *   gj_batch_id_37300         → 37300
 *   gj_graph_milestone_37300  → 37300
 *   __libcgj_batch37291_marker … __libcgj_batch37300_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch37291.c … graph_batch37300.c (and fill
 *     37276–37290 / identity 37251–37275) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 37300.
 *   - libc.map may lag milestone 37300 globals; add 37300 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_37300 == 0).
 ...
 *     continuum/smoke/dyn soft=1; milestone/wave/batch/graph = 37300.
 */
...
/* Grep: libcgj soft m37300 residual Soft!=product dual_dod_ab=OPEN G-AC-1 */
static const char g_szSoftHonesty[] =
	"libcgj: soft m37300 residual honesty Soft!=product dual_dod_a=OPEN "
...
#ifndef CGJ_SOFT_M37300_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M37300_DIRECT
/* Direct-link path: continuum soft pure markers (Soft!=product). */
extern uint32_t gj_shell_green_37300(void);
extern uint32_t gj_libcgj_green_37300(void);
extern uint32_t gj_bar3_ready_37300(void);
extern uint32_t gj_product_score_37300(void);
extern uint32_t gj_continuum_ready_37300(void);
extern uint32_t gj_smoke_soft_37300(void);
extern uint32_t gj_dyn_soft_37300(void);
extern uint32_t gj_milestone_tag_37300(void);
extern uint32_t gj_continuum_wave_37300(void);
extern uint32_t gj_batch_id_37300(void);
extern uint32_t gj_graph_milestone_37300(void);
extern const char __libcgj_batch37291_marker[];
extern const char __libcgj_batch37292_marker[];
extern const char __libcgj_batch37293_marker[];
extern const char __libcgj_batch37294_marker[];
extern const char __libcgj_batch37295_marker[];
extern const char __libcgj_batch37296_marker[];
extern const char __libcgj_batch37297_marker[];
extern const char __libcgj_batch37298_marker[];
extern const char __libcgj_batch37299_marker[];
extern const char __libcgj_batch37300_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_37300() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_37300 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_37300() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_37300 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_37300() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_37300 fail\n");
		nFail++;
	}
	if (gj_product_score_37300() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_37300 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_37300() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_37300 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_37300() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_37300 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_37300() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_37300 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_37300() != 37300u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_37300 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_37300() != 37300u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_37300 fail\n");
		nFail++;
	}
	if (gj_batch_id_37300() != 37300u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_37300 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_37300() != 37300u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_37300 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch37291_marker, "libcgj-batch37291") != 0 ||
	    strcmp(__libcgj_batch37292_marker, "libcgj-batch37292") != 0 ||
	    strcmp(__libcgj_batch37293_marker, "libcgj-batch37293") != 0 ||
	    strcmp(__libcgj_batch37294_marker, "libcgj-batch37294") != 0 ||
	    strcmp(__libcgj_batch37295_marker, "libcgj-batch37295") != 0 ||
	    strcmp(__libcgj_batch37296_marker, "libcgj-batch37296") != 0 ||
	    strcmp(__libcgj_batch37297_marker, "libcgj-batch37297") != 0 ||
	    strcmp(__libcgj_batch37298_marker, "libcgj-batch37298") != 0 ||
	    strcmp(__libcgj_batch37299_marker, "libcgj-batch37299") != 0 ||
	    strcmp(__libcgj_batch37300_marker, "libcgj-batch37300") != 0) {
		fprintf(stderr, "libcgj: direct m37300 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M37300_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m37300 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m37300 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m37300 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m37300 DIRECT PASS (all 11 lamps + markers; "
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m37300 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m37300 SOFT-SKIP (no SO; Soft!=product; "
	...
	nFail += soft_u32(h, "gj_shell_green_37300", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_37300", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_37300", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_37300", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_37300", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_37300", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_37300", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_37300", 37300u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_37300", 37300u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_37300", 37300u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_37300", 37300u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch37291_marker",
	                     "libcgj-batch37291", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch37292_marker",
	                     "libcgj-batch37292", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch37293_marker",
	                     "libcgj-batch37293", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch37294_marker",
	                     "libcgj-batch37294", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch37295_marker",
	                     "libcgj-batch37295", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch37296_marker",
	                     "libcgj-batch37296", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch37297_marker",
	                     "libcgj-batch37297", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch37298_marker",
	                     "libcgj-batch37298", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch37299_marker",
	                     "libcgj-batch37299", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch37300_marker",
	                     "libcgj-batch37300", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m37300 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m37300 PASS path=%s checked=%d "
