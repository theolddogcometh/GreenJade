 ...
 *
 * Soft host gates for continuum MILESTONE 37500 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_37500.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_37500.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_37500 \
 *     user/libcgj/tests/cgj_soft_milestone_37500.c -ldl
 *   ./build/cgj_soft_milestone_37500
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M37500_DIRECT \
 *     -o build/cgj_soft_milestone_37500_direct \
 *     user/libcgj/tests/cgj_soft_milestone_37500.c \
 *     user/libcgj/src/graph_batch37491.c \
 *     user/libcgj/src/graph_batch37492.c \
 *     user/libcgj/src/graph_batch37493.c \
 *     user/libcgj/src/graph_batch37494.c \
 *     user/libcgj/src/graph_batch37495.c \
 *     user/libcgj/src/graph_batch37496.c \
 *     user/libcgj/src/graph_batch37497.c \
 *     user/libcgj/src/graph_batch37498.c \
 *     user/libcgj/src/graph_batch37499.c \
 *     user/libcgj/src/graph_batch37500.c
 *   ./build/cgj_soft_milestone_37500_direct
 *
 * Expected soft KATs (wave 37491–37500 exclusive continuum markers):
 *   gj_shell_green_37500      → 1
 *   gj_libcgj_green_37500     → 1
 *   gj_bar3_ready_37500       → 0   (bar3 still open)
 *   gj_product_score_37500    → 0   (product score still open)
 *   gj_continuum_ready_37500  → 1
 *   gj_smoke_soft_37500       → 1
 *   gj_dyn_soft_37500         → 1
 *   gj_milestone_tag_37500    → 37500
 *   gj_continuum_wave_37500   → 37500
 *   gj_batch_id_37500         → 37500
 *   gj_graph_milestone_37500  → 37500
 *   __libcgj_batch37491_marker … __libcgj_batch37500_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch37491.c … graph_batch37500.c (and fill
 *     37476–37490 / identity 37451–37475) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 37500.
 *   - libc.map may lag milestone 37500 globals; add 37500 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_37500 == 0).
 ...
 *     continuum/smoke/dyn soft=1; milestone/wave/batch/graph = 37500.
 */
...
/* Grep: libcgj soft m37500 residual Soft!=product dual_dod_ab=OPEN G-AC-1 */
static const char g_szSoftHonesty[] =
	"libcgj: soft m37500 residual honesty Soft!=product dual_dod_a=OPEN "
...
#ifndef CGJ_SOFT_M37500_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M37500_DIRECT
/* Direct-link path: continuum soft pure markers (Soft!=product). */
extern uint32_t gj_shell_green_37500(void);
extern uint32_t gj_libcgj_green_37500(void);
extern uint32_t gj_bar3_ready_37500(void);
extern uint32_t gj_product_score_37500(void);
extern uint32_t gj_continuum_ready_37500(void);
extern uint32_t gj_smoke_soft_37500(void);
extern uint32_t gj_dyn_soft_37500(void);
extern uint32_t gj_milestone_tag_37500(void);
extern uint32_t gj_continuum_wave_37500(void);
extern uint32_t gj_batch_id_37500(void);
extern uint32_t gj_graph_milestone_37500(void);
extern const char __libcgj_batch37491_marker[];
extern const char __libcgj_batch37492_marker[];
extern const char __libcgj_batch37493_marker[];
extern const char __libcgj_batch37494_marker[];
extern const char __libcgj_batch37495_marker[];
extern const char __libcgj_batch37496_marker[];
extern const char __libcgj_batch37497_marker[];
extern const char __libcgj_batch37498_marker[];
extern const char __libcgj_batch37499_marker[];
extern const char __libcgj_batch37500_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_37500() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_37500 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_37500() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_37500 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_37500() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_37500 fail\n");
		nFail++;
	}
	if (gj_product_score_37500() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_37500 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_37500() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_37500 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_37500() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_37500 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_37500() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_37500 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_37500() != 37500u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_37500 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_37500() != 37500u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_37500 fail\n");
		nFail++;
	}
	if (gj_batch_id_37500() != 37500u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_37500 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_37500() != 37500u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_37500 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch37491_marker, "libcgj-batch37491") != 0 ||
	    strcmp(__libcgj_batch37492_marker, "libcgj-batch37492") != 0 ||
	    strcmp(__libcgj_batch37493_marker, "libcgj-batch37493") != 0 ||
	    strcmp(__libcgj_batch37494_marker, "libcgj-batch37494") != 0 ||
	    strcmp(__libcgj_batch37495_marker, "libcgj-batch37495") != 0 ||
	    strcmp(__libcgj_batch37496_marker, "libcgj-batch37496") != 0 ||
	    strcmp(__libcgj_batch37497_marker, "libcgj-batch37497") != 0 ||
	    strcmp(__libcgj_batch37498_marker, "libcgj-batch37498") != 0 ||
	    strcmp(__libcgj_batch37499_marker, "libcgj-batch37499") != 0 ||
	    strcmp(__libcgj_batch37500_marker, "libcgj-batch37500") != 0) {
		fprintf(stderr, "libcgj: direct m37500 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M37500_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m37500 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m37500 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m37500 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m37500 DIRECT PASS (all 11 lamps + markers; "
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m37500 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m37500 SOFT-SKIP (no SO; Soft!=product; "
	...
	nFail += soft_u32(h, "gj_shell_green_37500", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_37500", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_37500", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_37500", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_37500", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_37500", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_37500", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_37500", 37500u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_37500", 37500u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_37500", 37500u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_37500", 37500u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch37491_marker",
	                     "libcgj-batch37491", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch37492_marker",
	                     "libcgj-batch37492", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch37493_marker",
	                     "libcgj-batch37493", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch37494_marker",
	                     "libcgj-batch37494", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch37495_marker",
	                     "libcgj-batch37495", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch37496_marker",
	                     "libcgj-batch37496", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch37497_marker",
	                     "libcgj-batch37497", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch37498_marker",
	                     "libcgj-batch37498", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch37499_marker",
	                     "libcgj-batch37499", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch37500_marker",
	                     "libcgj-batch37500", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m37500 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m37500 PASS path=%s checked=%d "
