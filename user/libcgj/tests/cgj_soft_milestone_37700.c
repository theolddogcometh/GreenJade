 ...
 *
 * Soft host gates for continuum MILESTONE 37700 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_37700.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_37700.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_37700 \
 *     user/libcgj/tests/cgj_soft_milestone_37700.c -ldl
 *   ./build/cgj_soft_milestone_37700
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M37700_DIRECT \
 *     -o build/cgj_soft_milestone_37700_direct \
 *     user/libcgj/tests/cgj_soft_milestone_37700.c \
 *     user/libcgj/src/graph_batch37691.c \
 *     user/libcgj/src/graph_batch37692.c \
 *     user/libcgj/src/graph_batch37693.c \
 *     user/libcgj/src/graph_batch37694.c \
 *     user/libcgj/src/graph_batch37695.c \
 *     user/libcgj/src/graph_batch37696.c \
 *     user/libcgj/src/graph_batch37697.c \
 *     user/libcgj/src/graph_batch37698.c \
 *     user/libcgj/src/graph_batch37699.c \
 *     user/libcgj/src/graph_batch37700.c
 *   ./build/cgj_soft_milestone_37700_direct
 *
 * Expected soft KATs (wave 37691–37700 exclusive continuum markers):
 *   gj_shell_green_37700      → 1
 *   gj_libcgj_green_37700     → 1
 *   gj_bar3_ready_37700       → 0   (bar3 still open)
 *   gj_product_score_37700    → 0   (product score still open)
 *   gj_continuum_ready_37700  → 1
 *   gj_smoke_soft_37700       → 1
 *   gj_dyn_soft_37700         → 1
 *   gj_milestone_tag_37700    → 37700
 *   gj_continuum_wave_37700   → 37700
 *   gj_batch_id_37700         → 37700
 *   gj_graph_milestone_37700  → 37700
 *   __libcgj_batch37691_marker … __libcgj_batch37700_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch37691.c … graph_batch37700.c (and fill
 *     37676–37690 / identity 37651–37675) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 37700.
 *   - libc.map may lag milestone 37700 globals; add 37700 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_37700 == 0).
 ...
 *     continuum/smoke/dyn soft=1; milestone/wave/batch/graph = 37700.
 */
...
/* Grep: libcgj soft m37700 residual Soft!=product dual_dod_ab=OPEN G-AC-1 */
static const char g_szSoftHonesty[] =
	"libcgj: soft m37700 residual honesty Soft!=product dual_dod_a=OPEN "
...
#ifndef CGJ_SOFT_M37700_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M37700_DIRECT
/* Direct-link path: continuum soft pure markers (Soft!=product). */
extern uint32_t gj_shell_green_37700(void);
extern uint32_t gj_libcgj_green_37700(void);
extern uint32_t gj_bar3_ready_37700(void);
extern uint32_t gj_product_score_37700(void);
extern uint32_t gj_continuum_ready_37700(void);
extern uint32_t gj_smoke_soft_37700(void);
extern uint32_t gj_dyn_soft_37700(void);
extern uint32_t gj_milestone_tag_37700(void);
extern uint32_t gj_continuum_wave_37700(void);
extern uint32_t gj_batch_id_37700(void);
extern uint32_t gj_graph_milestone_37700(void);
extern const char __libcgj_batch37691_marker[];
extern const char __libcgj_batch37692_marker[];
extern const char __libcgj_batch37693_marker[];
extern const char __libcgj_batch37694_marker[];
extern const char __libcgj_batch37695_marker[];
extern const char __libcgj_batch37696_marker[];
extern const char __libcgj_batch37697_marker[];
extern const char __libcgj_batch37698_marker[];
extern const char __libcgj_batch37699_marker[];
extern const char __libcgj_batch37700_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_37700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_37700 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_37700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_37700 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_37700() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_37700 fail\n");
		nFail++;
	}
	if (gj_product_score_37700() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_37700 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_37700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_37700 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_37700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_37700 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_37700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_37700 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_37700() != 37700u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_37700 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_37700() != 37700u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_37700 fail\n");
		nFail++;
	}
	if (gj_batch_id_37700() != 37700u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_37700 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_37700() != 37700u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_37700 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch37691_marker, "libcgj-batch37691") != 0 ||
	    strcmp(__libcgj_batch37692_marker, "libcgj-batch37692") != 0 ||
	    strcmp(__libcgj_batch37693_marker, "libcgj-batch37693") != 0 ||
	    strcmp(__libcgj_batch37694_marker, "libcgj-batch37694") != 0 ||
	    strcmp(__libcgj_batch37695_marker, "libcgj-batch37695") != 0 ||
	    strcmp(__libcgj_batch37696_marker, "libcgj-batch37696") != 0 ||
	    strcmp(__libcgj_batch37697_marker, "libcgj-batch37697") != 0 ||
	    strcmp(__libcgj_batch37698_marker, "libcgj-batch37698") != 0 ||
	    strcmp(__libcgj_batch37699_marker, "libcgj-batch37699") != 0 ||
	    strcmp(__libcgj_batch37700_marker, "libcgj-batch37700") != 0) {
		fprintf(stderr, "libcgj: direct m37700 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M37700_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m37700 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m37700 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m37700 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m37700 DIRECT PASS (all 11 lamps + markers; "
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m37700 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m37700 SOFT-SKIP (no SO; Soft!=product; "
	...
	nFail += soft_u32(h, "gj_shell_green_37700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_37700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_37700", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_37700", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_37700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_37700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_37700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_37700", 37700u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_37700", 37700u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_37700", 37700u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_37700", 37700u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch37691_marker",
	                     "libcgj-batch37691", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch37692_marker",
	                     "libcgj-batch37692", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch37693_marker",
	                     "libcgj-batch37693", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch37694_marker",
	                     "libcgj-batch37694", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch37695_marker",
	                     "libcgj-batch37695", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch37696_marker",
	                     "libcgj-batch37696", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch37697_marker",
	                     "libcgj-batch37697", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch37698_marker",
	                     "libcgj-batch37698", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch37699_marker",
	                     "libcgj-batch37699", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch37700_marker",
	                     "libcgj-batch37700", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m37700 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m37700 PASS path=%s checked=%d "
