 ...
 *
 * Soft host gates for continuum MILESTONE 37900 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_37900.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_37900.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_37900 \
 *     user/libcgj/tests/cgj_soft_milestone_37900.c -ldl
 *   ./build/cgj_soft_milestone_37900
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M37900_DIRECT \
 *     -o build/cgj_soft_milestone_37900_direct \
 *     user/libcgj/tests/cgj_soft_milestone_37900.c \
 *     user/libcgj/src/graph_batch37891.c \
 *     user/libcgj/src/graph_batch37892.c \
 *     user/libcgj/src/graph_batch37893.c \
 *     user/libcgj/src/graph_batch37894.c \
 *     user/libcgj/src/graph_batch37895.c \
 *     user/libcgj/src/graph_batch37896.c \
 *     user/libcgj/src/graph_batch37897.c \
 *     user/libcgj/src/graph_batch37898.c \
 *     user/libcgj/src/graph_batch37899.c \
 *     user/libcgj/src/graph_batch37900.c
 *   ./build/cgj_soft_milestone_37900_direct
 *
 * Expected soft KATs (wave 37891–37900 exclusive continuum markers):
 *   gj_shell_green_37900      → 1
 *   gj_libcgj_green_37900     → 1
 *   gj_bar3_ready_37900       → 0   (bar3 still open)
 *   gj_product_score_37900    → 0   (product score still open)
 *   gj_continuum_ready_37900  → 1
 *   gj_smoke_soft_37900       → 1
 *   gj_dyn_soft_37900         → 1
 *   gj_milestone_tag_37900    → 37900
 *   gj_continuum_wave_37900   → 37900
 *   gj_batch_id_37900         → 37900
 *   gj_graph_milestone_37900  → 37900
 *   __libcgj_batch37891_marker … __libcgj_batch37900_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch37891.c … graph_batch37900.c (and fill
 *     37876–37890 / identity 37851–37875) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 37900.
 *   - libc.map may lag milestone 37900 globals; add 37900 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_37900 == 0).
 ...
 *     continuum/smoke/dyn soft=1; milestone/wave/batch/graph = 37900.
 */
...
/* Grep: libcgj soft m37900 residual Soft!=product dual_dod_ab=OPEN G-AC-1 */
static const char g_szSoftHonesty[] =
	"libcgj: soft m37900 residual honesty Soft!=product dual_dod_a=OPEN "
...
#ifndef CGJ_SOFT_M37900_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M37900_DIRECT
/* Direct-link path: continuum soft pure markers (Soft!=product). */
extern uint32_t gj_shell_green_37900(void);
extern uint32_t gj_libcgj_green_37900(void);
extern uint32_t gj_bar3_ready_37900(void);
extern uint32_t gj_product_score_37900(void);
extern uint32_t gj_continuum_ready_37900(void);
extern uint32_t gj_smoke_soft_37900(void);
extern uint32_t gj_dyn_soft_37900(void);
extern uint32_t gj_milestone_tag_37900(void);
extern uint32_t gj_continuum_wave_37900(void);
extern uint32_t gj_batch_id_37900(void);
extern uint32_t gj_graph_milestone_37900(void);
extern const char __libcgj_batch37891_marker[];
extern const char __libcgj_batch37892_marker[];
extern const char __libcgj_batch37893_marker[];
extern const char __libcgj_batch37894_marker[];
extern const char __libcgj_batch37895_marker[];
extern const char __libcgj_batch37896_marker[];
extern const char __libcgj_batch37897_marker[];
extern const char __libcgj_batch37898_marker[];
extern const char __libcgj_batch37899_marker[];
extern const char __libcgj_batch37900_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_37900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_37900 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_37900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_37900 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_37900() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_37900 fail\n");
		nFail++;
	}
	if (gj_product_score_37900() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_37900 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_37900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_37900 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_37900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_37900 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_37900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_37900 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_37900() != 37900u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_37900 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_37900() != 37900u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_37900 fail\n");
		nFail++;
	}
	if (gj_batch_id_37900() != 37900u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_37900 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_37900() != 37900u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_37900 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch37891_marker, "libcgj-batch37891") != 0 ||
	    strcmp(__libcgj_batch37892_marker, "libcgj-batch37892") != 0 ||
	    strcmp(__libcgj_batch37893_marker, "libcgj-batch37893") != 0 ||
	    strcmp(__libcgj_batch37894_marker, "libcgj-batch37894") != 0 ||
	    strcmp(__libcgj_batch37895_marker, "libcgj-batch37895") != 0 ||
	    strcmp(__libcgj_batch37896_marker, "libcgj-batch37896") != 0 ||
	    strcmp(__libcgj_batch37897_marker, "libcgj-batch37897") != 0 ||
	    strcmp(__libcgj_batch37898_marker, "libcgj-batch37898") != 0 ||
	    strcmp(__libcgj_batch37899_marker, "libcgj-batch37899") != 0 ||
	    strcmp(__libcgj_batch37900_marker, "libcgj-batch37900") != 0) {
		fprintf(stderr, "libcgj: direct m37900 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M37900_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m37900 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m37900 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m37900 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m37900 DIRECT PASS (all 11 lamps + markers; "
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m37900 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m37900 SOFT-SKIP (no SO; Soft!=product; "
	...
	nFail += soft_u32(h, "gj_shell_green_37900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_37900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_37900", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_37900", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_37900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_37900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_37900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_37900", 37900u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_37900", 37900u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_37900", 37900u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_37900", 37900u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch37891_marker",
	                     "libcgj-batch37891", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch37892_marker",
	                     "libcgj-batch37892", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch37893_marker",
	                     "libcgj-batch37893", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch37894_marker",
	                     "libcgj-batch37894", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch37895_marker",
	                     "libcgj-batch37895", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch37896_marker",
	                     "libcgj-batch37896", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch37897_marker",
	                     "libcgj-batch37897", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch37898_marker",
	                     "libcgj-batch37898", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch37899_marker",
	                     "libcgj-batch37899", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch37900_marker",
	                     "libcgj-batch37900", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m37900 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m37900 PASS path=%s checked=%d "
