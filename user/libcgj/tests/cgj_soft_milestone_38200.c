 ...
 *
 * Soft host gates for continuum MILESTONE 38200 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_38200.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_38200.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_38200 \
 *     user/libcgj/tests/cgj_soft_milestone_38200.c -ldl
 *   ./build/cgj_soft_milestone_38200
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M38200_DIRECT \
 *     -o build/cgj_soft_milestone_38200_direct \
 *     user/libcgj/tests/cgj_soft_milestone_38200.c \
 *     user/libcgj/src/graph_batch38191.c \
 *     user/libcgj/src/graph_batch38192.c \
 *     user/libcgj/src/graph_batch38193.c \
 *     user/libcgj/src/graph_batch38194.c \
 *     user/libcgj/src/graph_batch38195.c \
 *     user/libcgj/src/graph_batch38196.c \
 *     user/libcgj/src/graph_batch38197.c \
 *     user/libcgj/src/graph_batch38198.c \
 *     user/libcgj/src/graph_batch38199.c \
 *     user/libcgj/src/graph_batch38200.c
 *   ./build/cgj_soft_milestone_38200_direct
 *
 * Expected soft KATs (wave 38191–38200 exclusive continuum markers):
 *   gj_shell_green_38200      → 1
 *   gj_libcgj_green_38200     → 1
 *   gj_bar3_ready_38200       → 0   (bar3 still open)
 *   gj_product_score_38200    → 0   (product score still open)
 *   gj_continuum_ready_38200  → 1
 *   gj_smoke_soft_38200       → 1
 *   gj_dyn_soft_38200         → 1
 *   gj_milestone_tag_38200    → 38200
 *   gj_continuum_wave_38200   → 38200
 *   gj_batch_id_38200         → 38200
 *   gj_graph_milestone_38200  → 38200
 *   __libcgj_batch38191_marker … __libcgj_batch38200_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch38191.c … graph_batch38200.c (and fill
 *     38176–38190 / identity 38151–38175) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 38200.
 *   - libc.map may lag milestone 38200 globals; add 38200 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_38200 == 0).
 */
...
#ifndef CGJ_SOFT_M38200_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M38200_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_38200(void);
extern uint32_t gj_libcgj_green_38200(void);
extern uint32_t gj_bar3_ready_38200(void);
extern uint32_t gj_product_score_38200(void);
extern uint32_t gj_continuum_ready_38200(void);
extern uint32_t gj_smoke_soft_38200(void);
extern uint32_t gj_dyn_soft_38200(void);
extern uint32_t gj_milestone_tag_38200(void);
extern uint32_t gj_continuum_wave_38200(void);
extern uint32_t gj_batch_id_38200(void);
extern uint32_t gj_graph_milestone_38200(void);
extern const char __libcgj_batch38191_marker[];
extern const char __libcgj_batch38192_marker[];
extern const char __libcgj_batch38193_marker[];
extern const char __libcgj_batch38194_marker[];
extern const char __libcgj_batch38195_marker[];
extern const char __libcgj_batch38196_marker[];
extern const char __libcgj_batch38197_marker[];
extern const char __libcgj_batch38198_marker[];
extern const char __libcgj_batch38199_marker[];
extern const char __libcgj_batch38200_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_38200() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_38200 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_38200() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_38200 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_38200() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_38200 fail\n");
		nFail++;
	}
	if (gj_product_score_38200() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_38200 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_38200() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_38200 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_38200() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_38200 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_38200() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_38200 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_38200() != 38200u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_38200 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_38200() != 38200u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_38200 fail\n");
		nFail++;
	}
	if (gj_batch_id_38200() != 38200u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_38200 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_38200() != 38200u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_38200 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch38191_marker, "libcgj-batch38191") != 0 ||
	    strcmp(__libcgj_batch38192_marker, "libcgj-batch38192") != 0 ||
	    strcmp(__libcgj_batch38193_marker, "libcgj-batch38193") != 0 ||
	    strcmp(__libcgj_batch38194_marker, "libcgj-batch38194") != 0 ||
	    strcmp(__libcgj_batch38195_marker, "libcgj-batch38195") != 0 ||
	    strcmp(__libcgj_batch38196_marker, "libcgj-batch38196") != 0 ||
	    strcmp(__libcgj_batch38197_marker, "libcgj-batch38197") != 0 ||
	    strcmp(__libcgj_batch38198_marker, "libcgj-batch38198") != 0 ||
	    strcmp(__libcgj_batch38199_marker, "libcgj-batch38199") != 0 ||
	    strcmp(__libcgj_batch38200_marker, "libcgj-batch38200") != 0) {
		fprintf(stderr, "libcgj: direct m38200 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M38200_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m38200 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m38200 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m38200 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m38200 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m38200 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m38200 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_38200", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_38200", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_38200", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_38200", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_38200", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_38200", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_38200", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_38200", 38200u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_38200", 38200u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_38200", 38200u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_38200", 38200u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch38191_marker",
	                     "libcgj-batch38191", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch38192_marker",
	                     "libcgj-batch38192", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch38193_marker",
	                     "libcgj-batch38193", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch38194_marker",
	                     "libcgj-batch38194", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch38195_marker",
	                     "libcgj-batch38195", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch38196_marker",
	                     "libcgj-batch38196", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch38197_marker",
	                     "libcgj-batch38197", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch38198_marker",
	                     "libcgj-batch38198", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch38199_marker",
	                     "libcgj-batch38199", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch38200_marker",
	                     "libcgj-batch38200", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m38200 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m38200 PASS path=%s checked=%d "
