 ...
 *
 * Soft host gates for continuum MILESTONE 38700 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_38700.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_38700.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_38700 \
 *     user/libcgj/tests/cgj_soft_milestone_38700.c -ldl
 *   ./build/cgj_soft_milestone_38700
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M38700_DIRECT \
 *     -o build/cgj_soft_milestone_38700_direct \
 *     user/libcgj/tests/cgj_soft_milestone_38700.c \
 *     user/libcgj/src/graph_batch38691.c \
 *     user/libcgj/src/graph_batch38692.c \
 *     user/libcgj/src/graph_batch38693.c \
 *     user/libcgj/src/graph_batch38694.c \
 *     user/libcgj/src/graph_batch38695.c \
 *     user/libcgj/src/graph_batch38696.c \
 *     user/libcgj/src/graph_batch38697.c \
 *     user/libcgj/src/graph_batch38698.c \
 *     user/libcgj/src/graph_batch38699.c \
 *     user/libcgj/src/graph_batch38700.c
 *   ./build/cgj_soft_milestone_38700_direct
 *
 * Expected soft KATs (wave 38691–38700 exclusive continuum markers):
 *   gj_shell_green_38700      → 1
 *   gj_libcgj_green_38700     → 1
 *   gj_bar3_ready_38700       → 0   (bar3 still open)
 *   gj_product_score_38700    → 0   (product score still open)
 *   gj_continuum_ready_38700  → 1
 *   gj_smoke_soft_38700       → 1
 *   gj_dyn_soft_38700         → 1
 *   gj_milestone_tag_38700    → 38700
 *   gj_continuum_wave_38700   → 38700
 *   gj_batch_id_38700         → 38700
 *   gj_graph_milestone_38700  → 38700
 *   __libcgj_batch38691_marker … __libcgj_batch38700_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch38691.c … graph_batch38700.c (and fill
 *     38676–38690 / identity 38651–38675) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 38700.
 *   - libc.map may lag milestone 38700 globals; add 38700 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_38700 == 0).
 */
...
#ifndef CGJ_SOFT_M38700_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M38700_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_38700(void);
extern uint32_t gj_libcgj_green_38700(void);
extern uint32_t gj_bar3_ready_38700(void);
extern uint32_t gj_product_score_38700(void);
extern uint32_t gj_continuum_ready_38700(void);
extern uint32_t gj_smoke_soft_38700(void);
extern uint32_t gj_dyn_soft_38700(void);
extern uint32_t gj_milestone_tag_38700(void);
extern uint32_t gj_continuum_wave_38700(void);
extern uint32_t gj_batch_id_38700(void);
extern uint32_t gj_graph_milestone_38700(void);
extern const char __libcgj_batch38691_marker[];
extern const char __libcgj_batch38692_marker[];
extern const char __libcgj_batch38693_marker[];
extern const char __libcgj_batch38694_marker[];
extern const char __libcgj_batch38695_marker[];
extern const char __libcgj_batch38696_marker[];
extern const char __libcgj_batch38697_marker[];
extern const char __libcgj_batch38698_marker[];
extern const char __libcgj_batch38699_marker[];
extern const char __libcgj_batch38700_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_38700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_38700 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_38700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_38700 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_38700() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_38700 fail\n");
		nFail++;
	}
	if (gj_product_score_38700() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_38700 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_38700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_38700 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_38700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_38700 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_38700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_38700 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_38700() != 38700u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_38700 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_38700() != 38700u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_38700 fail\n");
		nFail++;
	}
	if (gj_batch_id_38700() != 38700u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_38700 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_38700() != 38700u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_38700 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch38691_marker, "libcgj-batch38691") != 0 ||
	    strcmp(__libcgj_batch38692_marker, "libcgj-batch38692") != 0 ||
	    strcmp(__libcgj_batch38693_marker, "libcgj-batch38693") != 0 ||
	    strcmp(__libcgj_batch38694_marker, "libcgj-batch38694") != 0 ||
	    strcmp(__libcgj_batch38695_marker, "libcgj-batch38695") != 0 ||
	    strcmp(__libcgj_batch38696_marker, "libcgj-batch38696") != 0 ||
	    strcmp(__libcgj_batch38697_marker, "libcgj-batch38697") != 0 ||
	    strcmp(__libcgj_batch38698_marker, "libcgj-batch38698") != 0 ||
	    strcmp(__libcgj_batch38699_marker, "libcgj-batch38699") != 0 ||
	    strcmp(__libcgj_batch38700_marker, "libcgj-batch38700") != 0) {
		fprintf(stderr, "libcgj: direct m38700 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M38700_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m38700 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m38700 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m38700 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m38700 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m38700 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m38700 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_38700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_38700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_38700", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_38700", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_38700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_38700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_38700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_38700", 38700u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_38700", 38700u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_38700", 38700u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_38700", 38700u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch38691_marker",
	                     "libcgj-batch38691", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch38692_marker",
	                     "libcgj-batch38692", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch38693_marker",
	                     "libcgj-batch38693", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch38694_marker",
	                     "libcgj-batch38694", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch38695_marker",
	                     "libcgj-batch38695", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch38696_marker",
	                     "libcgj-batch38696", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch38697_marker",
	                     "libcgj-batch38697", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch38698_marker",
	                     "libcgj-batch38698", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch38699_marker",
	                     "libcgj-batch38699", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch38700_marker",
	                     "libcgj-batch38700", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m38700 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m38700 PASS path=%s checked=%d "
