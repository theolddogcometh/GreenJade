 ...
 *
 * Soft host gates for continuum MILESTONE 38900 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_38900.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_38900.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_38900 \
 *     user/libcgj/tests/cgj_soft_milestone_38900.c -ldl
 *   ./build/cgj_soft_milestone_38900
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M38900_DIRECT \
 *     -o build/cgj_soft_milestone_38900_direct \
 *     user/libcgj/tests/cgj_soft_milestone_38900.c \
 *     user/libcgj/src/graph_batch38891.c \
 *     user/libcgj/src/graph_batch38892.c \
 *     user/libcgj/src/graph_batch38893.c \
 *     user/libcgj/src/graph_batch38894.c \
 *     user/libcgj/src/graph_batch38895.c \
 *     user/libcgj/src/graph_batch38896.c \
 *     user/libcgj/src/graph_batch38897.c \
 *     user/libcgj/src/graph_batch38898.c \
 *     user/libcgj/src/graph_batch38899.c \
 *     user/libcgj/src/graph_batch38900.c
 *   ./build/cgj_soft_milestone_38900_direct
 *
 * Expected soft KATs (wave 38891–38900 exclusive continuum markers):
 *   gj_shell_green_38900      → 1
 *   gj_libcgj_green_38900     → 1
 *   gj_bar3_ready_38900       → 0   (bar3 still open)
 *   gj_product_score_38900    → 0   (product score still open)
 *   gj_continuum_ready_38900  → 1
 *   gj_smoke_soft_38900       → 1
 *   gj_dyn_soft_38900         → 1
 *   gj_milestone_tag_38900    → 38900
 *   gj_continuum_wave_38900   → 38900
 *   gj_batch_id_38900         → 38900
 *   gj_graph_milestone_38900  → 38900
 *   __libcgj_batch38891_marker … __libcgj_batch38900_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch38891.c … graph_batch38900.c (and fill
 *     38876–38890 / identity 38851–38875) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 38900.
 *   - libc.map may lag milestone 38900 globals; add 38900 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_38900 == 0).
 */
...
#ifndef CGJ_SOFT_M38900_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M38900_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_38900(void);
extern uint32_t gj_libcgj_green_38900(void);
extern uint32_t gj_bar3_ready_38900(void);
extern uint32_t gj_product_score_38900(void);
extern uint32_t gj_continuum_ready_38900(void);
extern uint32_t gj_smoke_soft_38900(void);
extern uint32_t gj_dyn_soft_38900(void);
extern uint32_t gj_milestone_tag_38900(void);
extern uint32_t gj_continuum_wave_38900(void);
extern uint32_t gj_batch_id_38900(void);
extern uint32_t gj_graph_milestone_38900(void);
extern const char __libcgj_batch38891_marker[];
extern const char __libcgj_batch38892_marker[];
extern const char __libcgj_batch38893_marker[];
extern const char __libcgj_batch38894_marker[];
extern const char __libcgj_batch38895_marker[];
extern const char __libcgj_batch38896_marker[];
extern const char __libcgj_batch38897_marker[];
extern const char __libcgj_batch38898_marker[];
extern const char __libcgj_batch38899_marker[];
extern const char __libcgj_batch38900_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_38900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_38900 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_38900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_38900 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_38900() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_38900 fail\n");
		nFail++;
	}
	if (gj_product_score_38900() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_38900 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_38900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_38900 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_38900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_38900 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_38900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_38900 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_38900() != 38900u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_38900 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_38900() != 38900u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_38900 fail\n");
		nFail++;
	}
	if (gj_batch_id_38900() != 38900u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_38900 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_38900() != 38900u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_38900 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch38891_marker, "libcgj-batch38891") != 0 ||
	    strcmp(__libcgj_batch38892_marker, "libcgj-batch38892") != 0 ||
	    strcmp(__libcgj_batch38893_marker, "libcgj-batch38893") != 0 ||
	    strcmp(__libcgj_batch38894_marker, "libcgj-batch38894") != 0 ||
	    strcmp(__libcgj_batch38895_marker, "libcgj-batch38895") != 0 ||
	    strcmp(__libcgj_batch38896_marker, "libcgj-batch38896") != 0 ||
	    strcmp(__libcgj_batch38897_marker, "libcgj-batch38897") != 0 ||
	    strcmp(__libcgj_batch38898_marker, "libcgj-batch38898") != 0 ||
	    strcmp(__libcgj_batch38899_marker, "libcgj-batch38899") != 0 ||
	    strcmp(__libcgj_batch38900_marker, "libcgj-batch38900") != 0) {
		fprintf(stderr, "libcgj: direct m38900 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M38900_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m38900 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m38900 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m38900 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m38900 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m38900 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m38900 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_38900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_38900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_38900", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_38900", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_38900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_38900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_38900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_38900", 38900u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_38900", 38900u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_38900", 38900u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_38900", 38900u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch38891_marker",
	                     "libcgj-batch38891", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch38892_marker",
	                     "libcgj-batch38892", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch38893_marker",
	                     "libcgj-batch38893", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch38894_marker",
	                     "libcgj-batch38894", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch38895_marker",
	                     "libcgj-batch38895", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch38896_marker",
	                     "libcgj-batch38896", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch38897_marker",
	                     "libcgj-batch38897", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch38898_marker",
	                     "libcgj-batch38898", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch38899_marker",
	                     "libcgj-batch38899", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch38900_marker",
	                     "libcgj-batch38900", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m38900 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m38900 PASS path=%s checked=%d "
