 ...
 *
 * Soft host gates for continuum MILESTONE 39100 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_39100.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_39100.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_39100 \
 *     user/libcgj/tests/cgj_soft_milestone_39100.c -ldl
 *   ./build/cgj_soft_milestone_39100
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M39100_DIRECT \
 *     -o build/cgj_soft_milestone_39100_direct \
 *     user/libcgj/tests/cgj_soft_milestone_39100.c \
 *     user/libcgj/src/graph_batch39091.c \
 *     user/libcgj/src/graph_batch39092.c \
 *     user/libcgj/src/graph_batch39093.c \
 *     user/libcgj/src/graph_batch39094.c \
 *     user/libcgj/src/graph_batch39095.c \
 *     user/libcgj/src/graph_batch39096.c \
 *     user/libcgj/src/graph_batch39097.c \
 *     user/libcgj/src/graph_batch39098.c \
 *     user/libcgj/src/graph_batch39099.c \
 *     user/libcgj/src/graph_batch39100.c
 *   ./build/cgj_soft_milestone_39100_direct
 *
 * Expected soft KATs (wave 39091–39100 exclusive continuum markers):
 *   gj_shell_green_39100      → 1
 *   gj_libcgj_green_39100     → 1
 *   gj_bar3_ready_39100       → 0   (bar3 still open)
 *   gj_product_score_39100    → 0   (product score still open)
 *   gj_continuum_ready_39100  → 1
 *   gj_smoke_soft_39100       → 1
 *   gj_dyn_soft_39100         → 1
 *   gj_milestone_tag_39100    → 39100
 *   gj_continuum_wave_39100   → 39100
 *   gj_batch_id_39100         → 39100
 *   gj_graph_milestone_39100  → 39100
 *   __libcgj_batch39091_marker … __libcgj_batch39100_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch39091.c … graph_batch39100.c (and fill
 *     39076–39090 / identity 39051–39075) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 39100.
 *   - libc.map may lag milestone 39100 globals; add 39100 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_39100 == 0).
 */
...
#ifndef CGJ_SOFT_M39100_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M39100_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_39100(void);
extern uint32_t gj_libcgj_green_39100(void);
extern uint32_t gj_bar3_ready_39100(void);
extern uint32_t gj_product_score_39100(void);
extern uint32_t gj_continuum_ready_39100(void);
extern uint32_t gj_smoke_soft_39100(void);
extern uint32_t gj_dyn_soft_39100(void);
extern uint32_t gj_milestone_tag_39100(void);
extern uint32_t gj_continuum_wave_39100(void);
extern uint32_t gj_batch_id_39100(void);
extern uint32_t gj_graph_milestone_39100(void);
extern const char __libcgj_batch39091_marker[];
extern const char __libcgj_batch39092_marker[];
extern const char __libcgj_batch39093_marker[];
extern const char __libcgj_batch39094_marker[];
extern const char __libcgj_batch39095_marker[];
extern const char __libcgj_batch39096_marker[];
extern const char __libcgj_batch39097_marker[];
extern const char __libcgj_batch39098_marker[];
extern const char __libcgj_batch39099_marker[];
extern const char __libcgj_batch39100_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_39100() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_39100 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_39100() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_39100 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_39100() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_39100 fail\n");
		nFail++;
	}
	if (gj_product_score_39100() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_39100 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_39100() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_39100 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_39100() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_39100 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_39100() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_39100 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_39100() != 39100u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_39100 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_39100() != 39100u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_39100 fail\n");
		nFail++;
	}
	if (gj_batch_id_39100() != 39100u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_39100 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_39100() != 39100u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_39100 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch39091_marker, "libcgj-batch39091") != 0 ||
	    strcmp(__libcgj_batch39092_marker, "libcgj-batch39092") != 0 ||
	    strcmp(__libcgj_batch39093_marker, "libcgj-batch39093") != 0 ||
	    strcmp(__libcgj_batch39094_marker, "libcgj-batch39094") != 0 ||
	    strcmp(__libcgj_batch39095_marker, "libcgj-batch39095") != 0 ||
	    strcmp(__libcgj_batch39096_marker, "libcgj-batch39096") != 0 ||
	    strcmp(__libcgj_batch39097_marker, "libcgj-batch39097") != 0 ||
	    strcmp(__libcgj_batch39098_marker, "libcgj-batch39098") != 0 ||
	    strcmp(__libcgj_batch39099_marker, "libcgj-batch39099") != 0 ||
	    strcmp(__libcgj_batch39100_marker, "libcgj-batch39100") != 0) {
		fprintf(stderr, "libcgj: direct m39100 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M39100_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m39100 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m39100 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m39100 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m39100 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m39100 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m39100 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_39100", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_39100", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_39100", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_39100", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_39100", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_39100", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_39100", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_39100", 39100u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_39100", 39100u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_39100", 39100u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_39100", 39100u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch39091_marker",
	                     "libcgj-batch39091", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39092_marker",
	                     "libcgj-batch39092", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39093_marker",
	                     "libcgj-batch39093", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39094_marker",
	                     "libcgj-batch39094", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39095_marker",
	                     "libcgj-batch39095", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39096_marker",
	                     "libcgj-batch39096", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39097_marker",
	                     "libcgj-batch39097", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39098_marker",
	                     "libcgj-batch39098", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39099_marker",
	                     "libcgj-batch39099", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39100_marker",
	                     "libcgj-batch39100", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m39100 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m39100 PASS path=%s checked=%d "
