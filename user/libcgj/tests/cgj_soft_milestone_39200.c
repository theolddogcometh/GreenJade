 ...
 *
 * Soft host gates for continuum MILESTONE 39200 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_39200.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_39200.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_39200 \
 *     user/libcgj/tests/cgj_soft_milestone_39200.c -ldl
 *   ./build/cgj_soft_milestone_39200
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M39200_DIRECT \
 *     -o build/cgj_soft_milestone_39200_direct \
 *     user/libcgj/tests/cgj_soft_milestone_39200.c \
 *     user/libcgj/src/graph_batch39191.c \
 *     user/libcgj/src/graph_batch39192.c \
 *     user/libcgj/src/graph_batch39193.c \
 *     user/libcgj/src/graph_batch39194.c \
 *     user/libcgj/src/graph_batch39195.c \
 *     user/libcgj/src/graph_batch39196.c \
 *     user/libcgj/src/graph_batch39197.c \
 *     user/libcgj/src/graph_batch39198.c \
 *     user/libcgj/src/graph_batch39199.c \
 *     user/libcgj/src/graph_batch39200.c
 *   ./build/cgj_soft_milestone_39200_direct
 *
 * Expected soft KATs (wave 39191–39200 exclusive continuum markers):
 *   gj_shell_green_39200      → 1
 *   gj_libcgj_green_39200     → 1
 *   gj_bar3_ready_39200       → 0   (bar3 still open)
 *   gj_product_score_39200    → 0   (product score still open)
 *   gj_continuum_ready_39200  → 1
 *   gj_smoke_soft_39200       → 1
 *   gj_dyn_soft_39200         → 1
 *   gj_milestone_tag_39200    → 39200
 *   gj_continuum_wave_39200   → 39200
 *   gj_batch_id_39200         → 39200
 *   gj_graph_milestone_39200  → 39200
 *   __libcgj_batch39191_marker … __libcgj_batch39200_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch39191.c … graph_batch39200.c (and fill
 *     39176–39190 / identity 39151–39175) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 39200.
 *   - libc.map may lag milestone 39200 globals; add 39200 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_39200 == 0).
 */
...
#ifndef CGJ_SOFT_M39200_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M39200_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_39200(void);
extern uint32_t gj_libcgj_green_39200(void);
extern uint32_t gj_bar3_ready_39200(void);
extern uint32_t gj_product_score_39200(void);
extern uint32_t gj_continuum_ready_39200(void);
extern uint32_t gj_smoke_soft_39200(void);
extern uint32_t gj_dyn_soft_39200(void);
extern uint32_t gj_milestone_tag_39200(void);
extern uint32_t gj_continuum_wave_39200(void);
extern uint32_t gj_batch_id_39200(void);
extern uint32_t gj_graph_milestone_39200(void);
extern const char __libcgj_batch39191_marker[];
extern const char __libcgj_batch39192_marker[];
extern const char __libcgj_batch39193_marker[];
extern const char __libcgj_batch39194_marker[];
extern const char __libcgj_batch39195_marker[];
extern const char __libcgj_batch39196_marker[];
extern const char __libcgj_batch39197_marker[];
extern const char __libcgj_batch39198_marker[];
extern const char __libcgj_batch39199_marker[];
extern const char __libcgj_batch39200_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_39200() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_39200 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_39200() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_39200 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_39200() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_39200 fail\n");
		nFail++;
	}
	if (gj_product_score_39200() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_39200 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_39200() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_39200 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_39200() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_39200 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_39200() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_39200 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_39200() != 39200u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_39200 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_39200() != 39200u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_39200 fail\n");
		nFail++;
	}
	if (gj_batch_id_39200() != 39200u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_39200 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_39200() != 39200u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_39200 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch39191_marker, "libcgj-batch39191") != 0 ||
	    strcmp(__libcgj_batch39192_marker, "libcgj-batch39192") != 0 ||
	    strcmp(__libcgj_batch39193_marker, "libcgj-batch39193") != 0 ||
	    strcmp(__libcgj_batch39194_marker, "libcgj-batch39194") != 0 ||
	    strcmp(__libcgj_batch39195_marker, "libcgj-batch39195") != 0 ||
	    strcmp(__libcgj_batch39196_marker, "libcgj-batch39196") != 0 ||
	    strcmp(__libcgj_batch39197_marker, "libcgj-batch39197") != 0 ||
	    strcmp(__libcgj_batch39198_marker, "libcgj-batch39198") != 0 ||
	    strcmp(__libcgj_batch39199_marker, "libcgj-batch39199") != 0 ||
	    strcmp(__libcgj_batch39200_marker, "libcgj-batch39200") != 0) {
		fprintf(stderr, "libcgj: direct m39200 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M39200_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m39200 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m39200 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m39200 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m39200 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m39200 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m39200 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_39200", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_39200", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_39200", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_39200", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_39200", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_39200", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_39200", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_39200", 39200u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_39200", 39200u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_39200", 39200u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_39200", 39200u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch39191_marker",
	                     "libcgj-batch39191", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39192_marker",
	                     "libcgj-batch39192", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39193_marker",
	                     "libcgj-batch39193", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39194_marker",
	                     "libcgj-batch39194", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39195_marker",
	                     "libcgj-batch39195", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39196_marker",
	                     "libcgj-batch39196", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39197_marker",
	                     "libcgj-batch39197", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39198_marker",
	                     "libcgj-batch39198", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39199_marker",
	                     "libcgj-batch39199", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39200_marker",
	                     "libcgj-batch39200", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m39200 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m39200 PASS path=%s checked=%d "
