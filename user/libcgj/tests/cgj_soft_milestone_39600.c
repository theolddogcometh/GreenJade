 ...
 *
 * Soft host gates for continuum MILESTONE 39600 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_39600.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_39600.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_39600 \
 *     user/libcgj/tests/cgj_soft_milestone_39600.c -ldl
 *   ./build/cgj_soft_milestone_39600
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M39600_DIRECT \
 *     -o build/cgj_soft_milestone_39600_direct \
 *     user/libcgj/tests/cgj_soft_milestone_39600.c \
 *     user/libcgj/src/graph_batch39591.c \
 *     user/libcgj/src/graph_batch39592.c \
 *     user/libcgj/src/graph_batch39593.c \
 *     user/libcgj/src/graph_batch39594.c \
 *     user/libcgj/src/graph_batch39595.c \
 *     user/libcgj/src/graph_batch39596.c \
 *     user/libcgj/src/graph_batch39597.c \
 *     user/libcgj/src/graph_batch39598.c \
 *     user/libcgj/src/graph_batch39599.c \
 *     user/libcgj/src/graph_batch39600.c
 *   ./build/cgj_soft_milestone_39600_direct
 *
 * Expected soft KATs (wave 39591–39600 exclusive continuum markers):
 *   gj_shell_green_39600      → 1
 *   gj_libcgj_green_39600     → 1
 *   gj_bar3_ready_39600       → 0   (bar3 still open)
 *   gj_product_score_39600    → 0   (product score still open)
 *   gj_continuum_ready_39600  → 1
 *   gj_smoke_soft_39600       → 1
 *   gj_dyn_soft_39600         → 1
 *   gj_milestone_tag_39600    → 39600
 *   gj_continuum_wave_39600   → 39600
 *   gj_batch_id_39600         → 39600
 *   gj_graph_milestone_39600  → 39600
 *   __libcgj_batch39591_marker … __libcgj_batch39600_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch39591.c … graph_batch39600.c (and fill
 *     39576–39590 / identity 39551–39575) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 39600.
 *   - libc.map may lag milestone 39600 globals; add 39600 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_39600 == 0).
 */
...
#ifndef CGJ_SOFT_M39600_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M39600_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_39600(void);
extern uint32_t gj_libcgj_green_39600(void);
extern uint32_t gj_bar3_ready_39600(void);
extern uint32_t gj_product_score_39600(void);
extern uint32_t gj_continuum_ready_39600(void);
extern uint32_t gj_smoke_soft_39600(void);
extern uint32_t gj_dyn_soft_39600(void);
extern uint32_t gj_milestone_tag_39600(void);
extern uint32_t gj_continuum_wave_39600(void);
extern uint32_t gj_batch_id_39600(void);
extern uint32_t gj_graph_milestone_39600(void);
extern const char __libcgj_batch39591_marker[];
extern const char __libcgj_batch39592_marker[];
extern const char __libcgj_batch39593_marker[];
extern const char __libcgj_batch39594_marker[];
extern const char __libcgj_batch39595_marker[];
extern const char __libcgj_batch39596_marker[];
extern const char __libcgj_batch39597_marker[];
extern const char __libcgj_batch39598_marker[];
extern const char __libcgj_batch39599_marker[];
extern const char __libcgj_batch39600_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_39600() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_39600 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_39600() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_39600 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_39600() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_39600 fail\n");
		nFail++;
	}
	if (gj_product_score_39600() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_39600 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_39600() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_39600 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_39600() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_39600 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_39600() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_39600 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_39600() != 39600u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_39600 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_39600() != 39600u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_39600 fail\n");
		nFail++;
	}
	if (gj_batch_id_39600() != 39600u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_39600 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_39600() != 39600u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_39600 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch39591_marker, "libcgj-batch39591") != 0 ||
	    strcmp(__libcgj_batch39592_marker, "libcgj-batch39592") != 0 ||
	    strcmp(__libcgj_batch39593_marker, "libcgj-batch39593") != 0 ||
	    strcmp(__libcgj_batch39594_marker, "libcgj-batch39594") != 0 ||
	    strcmp(__libcgj_batch39595_marker, "libcgj-batch39595") != 0 ||
	    strcmp(__libcgj_batch39596_marker, "libcgj-batch39596") != 0 ||
	    strcmp(__libcgj_batch39597_marker, "libcgj-batch39597") != 0 ||
	    strcmp(__libcgj_batch39598_marker, "libcgj-batch39598") != 0 ||
	    strcmp(__libcgj_batch39599_marker, "libcgj-batch39599") != 0 ||
	    strcmp(__libcgj_batch39600_marker, "libcgj-batch39600") != 0) {
		fprintf(stderr, "libcgj: direct m39600 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M39600_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m39600 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m39600 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m39600 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m39600 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m39600 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m39600 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_39600", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_39600", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_39600", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_39600", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_39600", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_39600", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_39600", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_39600", 39600u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_39600", 39600u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_39600", 39600u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_39600", 39600u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch39591_marker",
	                     "libcgj-batch39591", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39592_marker",
	                     "libcgj-batch39592", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39593_marker",
	                     "libcgj-batch39593", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39594_marker",
	                     "libcgj-batch39594", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39595_marker",
	                     "libcgj-batch39595", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39596_marker",
	                     "libcgj-batch39596", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39597_marker",
	                     "libcgj-batch39597", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39598_marker",
	                     "libcgj-batch39598", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39599_marker",
	                     "libcgj-batch39599", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39600_marker",
	                     "libcgj-batch39600", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m39600 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m39600 PASS path=%s checked=%d "
