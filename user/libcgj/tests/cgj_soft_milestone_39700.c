 ...
 *
 * Soft host gates for continuum MILESTONE 39700 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_39700.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_39700.txt
 *
 * Test harness note:
 ...
 *     cgj_soft_milestone_39300.c / cgj_soft_milestone_39600.c /
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_39700 \
 *     user/libcgj/tests/cgj_soft_milestone_39700.c -ldl
 *   ./build/cgj_soft_milestone_39700
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M39700_DIRECT \
 *     -o build/cgj_soft_milestone_39700_direct \
 *     user/libcgj/tests/cgj_soft_milestone_39700.c \
 *     user/libcgj/src/graph_batch39691.c \
 *     user/libcgj/src/graph_batch39692.c \
 *     user/libcgj/src/graph_batch39693.c \
 *     user/libcgj/src/graph_batch39694.c \
 *     user/libcgj/src/graph_batch39695.c \
 *     user/libcgj/src/graph_batch39696.c \
 *     user/libcgj/src/graph_batch39697.c \
 *     user/libcgj/src/graph_batch39698.c \
 *     user/libcgj/src/graph_batch39699.c \
 *     user/libcgj/src/graph_batch39700.c
 *   ./build/cgj_soft_milestone_39700_direct
 *
 * Expected soft KATs (wave 39691–39700 exclusive continuum markers):
 *   gj_shell_green_39700      → 1
 *   gj_libcgj_green_39700     → 1
 *   gj_bar3_ready_39700       → 0   (bar3 still open)
 *   gj_product_score_39700    → 0   (product score still open)
 *   gj_continuum_ready_39700  → 1
 *   gj_smoke_soft_39700       → 1
 *   gj_dyn_soft_39700         → 1
 *   gj_milestone_tag_39700    → 39700
 *   gj_continuum_wave_39700   → 39700
 *   gj_batch_id_39700         → 39700
 *   gj_graph_milestone_39700  → 39700
 *   __libcgj_batch39691_marker … __libcgj_batch39700_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch39691.c … graph_batch39700.c (and fill
 *     39676–39690 / identity 39651–39675) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 39700.
 *   - libc.map may lag milestone 39700 globals; add 39700 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_39700 == 0).
 */
...
#ifndef CGJ_SOFT_M39700_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M39700_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_39700(void);
extern uint32_t gj_libcgj_green_39700(void);
extern uint32_t gj_bar3_ready_39700(void);
extern uint32_t gj_product_score_39700(void);
extern uint32_t gj_continuum_ready_39700(void);
extern uint32_t gj_smoke_soft_39700(void);
extern uint32_t gj_dyn_soft_39700(void);
extern uint32_t gj_milestone_tag_39700(void);
extern uint32_t gj_continuum_wave_39700(void);
extern uint32_t gj_batch_id_39700(void);
extern uint32_t gj_graph_milestone_39700(void);
extern const char __libcgj_batch39691_marker[];
extern const char __libcgj_batch39692_marker[];
extern const char __libcgj_batch39693_marker[];
extern const char __libcgj_batch39694_marker[];
extern const char __libcgj_batch39695_marker[];
extern const char __libcgj_batch39696_marker[];
extern const char __libcgj_batch39697_marker[];
extern const char __libcgj_batch39698_marker[];
extern const char __libcgj_batch39699_marker[];
extern const char __libcgj_batch39700_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_39700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_39700 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_39700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_39700 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_39700() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_39700 fail\n");
		nFail++;
	}
	if (gj_product_score_39700() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_39700 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_39700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_39700 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_39700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_39700 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_39700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_39700 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_39700() != 39700u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_39700 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_39700() != 39700u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_39700 fail\n");
		nFail++;
	}
	if (gj_batch_id_39700() != 39700u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_39700 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_39700() != 39700u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_39700 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch39691_marker, "libcgj-batch39691") != 0 ||
	    strcmp(__libcgj_batch39692_marker, "libcgj-batch39692") != 0 ||
	    strcmp(__libcgj_batch39693_marker, "libcgj-batch39693") != 0 ||
	    strcmp(__libcgj_batch39694_marker, "libcgj-batch39694") != 0 ||
	    strcmp(__libcgj_batch39695_marker, "libcgj-batch39695") != 0 ||
	    strcmp(__libcgj_batch39696_marker, "libcgj-batch39696") != 0 ||
	    strcmp(__libcgj_batch39697_marker, "libcgj-batch39697") != 0 ||
	    strcmp(__libcgj_batch39698_marker, "libcgj-batch39698") != 0 ||
	    strcmp(__libcgj_batch39699_marker, "libcgj-batch39699") != 0 ||
	    strcmp(__libcgj_batch39700_marker, "libcgj-batch39700") != 0) {
		fprintf(stderr, "libcgj: direct m39700 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M39700_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m39700 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m39700 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m39700 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m39700 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m39700 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m39700 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_39700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_39700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_39700", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_39700", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_39700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_39700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_39700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_39700", 39700u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_39700", 39700u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_39700", 39700u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_39700", 39700u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch39691_marker",
	                     "libcgj-batch39691", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39692_marker",
	                     "libcgj-batch39692", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39693_marker",
	                     "libcgj-batch39693", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39694_marker",
	                     "libcgj-batch39694", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39695_marker",
	                     "libcgj-batch39695", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39696_marker",
	                     "libcgj-batch39696", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39697_marker",
	                     "libcgj-batch39697", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39698_marker",
	                     "libcgj-batch39698", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39699_marker",
	                     "libcgj-batch39699", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39700_marker",
	                     "libcgj-batch39700", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m39700 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m39700 PASS path=%s checked=%d "
