 ...
 *
 * Soft host gates for continuum MILESTONE 39800 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_39800.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_39800.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_39800 \
 *     user/libcgj/tests/cgj_soft_milestone_39800.c -ldl
 *   ./build/cgj_soft_milestone_39800
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M39800_DIRECT \
 *     -o build/cgj_soft_milestone_39800_direct \
 *     user/libcgj/tests/cgj_soft_milestone_39800.c \
 *     user/libcgj/src/graph_batch39791.c \
 *     user/libcgj/src/graph_batch39792.c \
 *     user/libcgj/src/graph_batch39793.c \
 *     user/libcgj/src/graph_batch39794.c \
 *     user/libcgj/src/graph_batch39795.c \
 *     user/libcgj/src/graph_batch39796.c \
 *     user/libcgj/src/graph_batch39797.c \
 *     user/libcgj/src/graph_batch39798.c \
 *     user/libcgj/src/graph_batch39799.c \
 *     user/libcgj/src/graph_batch39800.c
 *   ./build/cgj_soft_milestone_39800_direct
 *
 * Expected soft KATs (wave 39791–39800 exclusive continuum markers):
 *   gj_shell_green_39800      → 1
 *   gj_libcgj_green_39800     → 1
 *   gj_bar3_ready_39800       → 0   (bar3 still open)
 *   gj_product_score_39800    → 0   (product score still open)
 *   gj_continuum_ready_39800  → 1
 *   gj_smoke_soft_39800       → 1
 *   gj_dyn_soft_39800         → 1
 *   gj_milestone_tag_39800    → 39800
 *   gj_continuum_wave_39800   → 39800
 *   gj_batch_id_39800         → 39800
 *   gj_graph_milestone_39800  → 39800
 *   __libcgj_batch39791_marker … __libcgj_batch39800_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch39791.c … graph_batch39800.c (and fill
 *     39776–39790 / identity 39751–39775) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 39800.
 *   - libc.map may lag milestone 39800 globals; add 39800 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_39800 == 0).
 */
...
#ifndef CGJ_SOFT_M39800_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M39800_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_39800(void);
extern uint32_t gj_libcgj_green_39800(void);
extern uint32_t gj_bar3_ready_39800(void);
extern uint32_t gj_product_score_39800(void);
extern uint32_t gj_continuum_ready_39800(void);
extern uint32_t gj_smoke_soft_39800(void);
extern uint32_t gj_dyn_soft_39800(void);
extern uint32_t gj_milestone_tag_39800(void);
extern uint32_t gj_continuum_wave_39800(void);
extern uint32_t gj_batch_id_39800(void);
extern uint32_t gj_graph_milestone_39800(void);
extern const char __libcgj_batch39791_marker[];
extern const char __libcgj_batch39792_marker[];
extern const char __libcgj_batch39793_marker[];
extern const char __libcgj_batch39794_marker[];
extern const char __libcgj_batch39795_marker[];
extern const char __libcgj_batch39796_marker[];
extern const char __libcgj_batch39797_marker[];
extern const char __libcgj_batch39798_marker[];
extern const char __libcgj_batch39799_marker[];
extern const char __libcgj_batch39800_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_39800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_39800 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_39800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_39800 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_39800() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_39800 fail\n");
		nFail++;
	}
	if (gj_product_score_39800() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_39800 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_39800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_39800 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_39800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_39800 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_39800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_39800 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_39800() != 39800u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_39800 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_39800() != 39800u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_39800 fail\n");
		nFail++;
	}
	if (gj_batch_id_39800() != 39800u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_39800 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_39800() != 39800u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_39800 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch39791_marker, "libcgj-batch39791") != 0 ||
	    strcmp(__libcgj_batch39792_marker, "libcgj-batch39792") != 0 ||
	    strcmp(__libcgj_batch39793_marker, "libcgj-batch39793") != 0 ||
	    strcmp(__libcgj_batch39794_marker, "libcgj-batch39794") != 0 ||
	    strcmp(__libcgj_batch39795_marker, "libcgj-batch39795") != 0 ||
	    strcmp(__libcgj_batch39796_marker, "libcgj-batch39796") != 0 ||
	    strcmp(__libcgj_batch39797_marker, "libcgj-batch39797") != 0 ||
	    strcmp(__libcgj_batch39798_marker, "libcgj-batch39798") != 0 ||
	    strcmp(__libcgj_batch39799_marker, "libcgj-batch39799") != 0 ||
	    strcmp(__libcgj_batch39800_marker, "libcgj-batch39800") != 0) {
		fprintf(stderr, "libcgj: direct m39800 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M39800_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m39800 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m39800 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m39800 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m39800 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m39800 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m39800 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_39800", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_39800", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_39800", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_39800", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_39800", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_39800", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_39800", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_39800", 39800u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_39800", 39800u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_39800", 39800u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_39800", 39800u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch39791_marker",
	                     "libcgj-batch39791", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39792_marker",
	                     "libcgj-batch39792", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39793_marker",
	                     "libcgj-batch39793", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39794_marker",
	                     "libcgj-batch39794", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39795_marker",
	                     "libcgj-batch39795", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39796_marker",
	                     "libcgj-batch39796", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39797_marker",
	                     "libcgj-batch39797", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39798_marker",
	                     "libcgj-batch39798", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39799_marker",
	                     "libcgj-batch39799", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39800_marker",
	                     "libcgj-batch39800", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m39800 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m39800 PASS path=%s checked=%d "
