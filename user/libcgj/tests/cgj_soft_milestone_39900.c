 ...
 *
 * Soft host gates for continuum MILESTONE 39900 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_39900.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_39900.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_39900 \
 *     user/libcgj/tests/cgj_soft_milestone_39900.c -ldl
 *   ./build/cgj_soft_milestone_39900
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M39900_DIRECT \
 *     -o build/cgj_soft_milestone_39900_direct \
 *     user/libcgj/tests/cgj_soft_milestone_39900.c \
 *     user/libcgj/src/graph_batch39891.c \
 *     user/libcgj/src/graph_batch39892.c \
 *     user/libcgj/src/graph_batch39893.c \
 *     user/libcgj/src/graph_batch39894.c \
 *     user/libcgj/src/graph_batch39895.c \
 *     user/libcgj/src/graph_batch39896.c \
 *     user/libcgj/src/graph_batch39897.c \
 *     user/libcgj/src/graph_batch39898.c \
 *     user/libcgj/src/graph_batch39899.c \
 *     user/libcgj/src/graph_batch39900.c
 *   ./build/cgj_soft_milestone_39900_direct
 *
 * Expected soft KATs (wave 39891–39900 exclusive continuum markers):
 *   gj_shell_green_39900      → 1
 *   gj_libcgj_green_39900     → 1
 *   gj_bar3_ready_39900       → 0   (bar3 still open)
 *   gj_product_score_39900    → 0   (product score still open)
 *   gj_continuum_ready_39900  → 1
 *   gj_smoke_soft_39900       → 1
 *   gj_dyn_soft_39900         → 1
 *   gj_milestone_tag_39900    → 39900
 *   gj_continuum_wave_39900   → 39900
 *   gj_batch_id_39900         → 39900
 *   gj_graph_milestone_39900  → 39900
 *   __libcgj_batch39891_marker … __libcgj_batch39900_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch39891.c … graph_batch39900.c (and fill
 *     39876–39890 / identity 39851–39875) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 39900.
 *   - libc.map may lag milestone 39900 globals; add 39900 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_39900 == 0).
 */
...
#ifndef CGJ_SOFT_M39900_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M39900_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_39900(void);
extern uint32_t gj_libcgj_green_39900(void);
extern uint32_t gj_bar3_ready_39900(void);
extern uint32_t gj_product_score_39900(void);
extern uint32_t gj_continuum_ready_39900(void);
extern uint32_t gj_smoke_soft_39900(void);
extern uint32_t gj_dyn_soft_39900(void);
extern uint32_t gj_milestone_tag_39900(void);
extern uint32_t gj_continuum_wave_39900(void);
extern uint32_t gj_batch_id_39900(void);
extern uint32_t gj_graph_milestone_39900(void);
extern const char __libcgj_batch39891_marker[];
extern const char __libcgj_batch39892_marker[];
extern const char __libcgj_batch39893_marker[];
extern const char __libcgj_batch39894_marker[];
extern const char __libcgj_batch39895_marker[];
extern const char __libcgj_batch39896_marker[];
extern const char __libcgj_batch39897_marker[];
extern const char __libcgj_batch39898_marker[];
extern const char __libcgj_batch39899_marker[];
extern const char __libcgj_batch39900_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_39900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_39900 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_39900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_39900 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_39900() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_39900 fail\n");
		nFail++;
	}
	if (gj_product_score_39900() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_39900 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_39900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_39900 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_39900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_39900 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_39900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_39900 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_39900() != 39900u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_39900 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_39900() != 39900u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_39900 fail\n");
		nFail++;
	}
	if (gj_batch_id_39900() != 39900u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_39900 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_39900() != 39900u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_39900 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch39891_marker, "libcgj-batch39891") != 0 ||
	    strcmp(__libcgj_batch39892_marker, "libcgj-batch39892") != 0 ||
	    strcmp(__libcgj_batch39893_marker, "libcgj-batch39893") != 0 ||
	    strcmp(__libcgj_batch39894_marker, "libcgj-batch39894") != 0 ||
	    strcmp(__libcgj_batch39895_marker, "libcgj-batch39895") != 0 ||
	    strcmp(__libcgj_batch39896_marker, "libcgj-batch39896") != 0 ||
	    strcmp(__libcgj_batch39897_marker, "libcgj-batch39897") != 0 ||
	    strcmp(__libcgj_batch39898_marker, "libcgj-batch39898") != 0 ||
	    strcmp(__libcgj_batch39899_marker, "libcgj-batch39899") != 0 ||
	    strcmp(__libcgj_batch39900_marker, "libcgj-batch39900") != 0) {
		fprintf(stderr, "libcgj: direct m39900 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M39900_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m39900 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m39900 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m39900 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m39900 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m39900 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m39900 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_39900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_39900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_39900", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_39900", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_39900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_39900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_39900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_39900", 39900u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_39900", 39900u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_39900", 39900u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_39900", 39900u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch39891_marker",
	                     "libcgj-batch39891", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39892_marker",
	                     "libcgj-batch39892", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39893_marker",
	                     "libcgj-batch39893", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39894_marker",
	                     "libcgj-batch39894", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39895_marker",
	                     "libcgj-batch39895", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39896_marker",
	                     "libcgj-batch39896", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39897_marker",
	                     "libcgj-batch39897", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39898_marker",
	                     "libcgj-batch39898", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39899_marker",
	                     "libcgj-batch39899", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39900_marker",
	                     "libcgj-batch39900", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m39900 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m39900 PASS path=%s checked=%d "
