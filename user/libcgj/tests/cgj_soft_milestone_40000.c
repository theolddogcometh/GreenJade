 ...
 *
 * Soft host gates for continuum MILESTONE 40000 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_40000.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_40000.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_40000 \
 *     user/libcgj/tests/cgj_soft_milestone_40000.c -ldl
 *   ./build/cgj_soft_milestone_40000
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M40000_DIRECT \
 *     -o build/cgj_soft_milestone_40000_direct \
 *     user/libcgj/tests/cgj_soft_milestone_40000.c \
 *     user/libcgj/src/graph_batch39991.c \
 *     user/libcgj/src/graph_batch39992.c \
 *     user/libcgj/src/graph_batch39993.c \
 *     user/libcgj/src/graph_batch39994.c \
 *     user/libcgj/src/graph_batch39995.c \
 *     user/libcgj/src/graph_batch39996.c \
 *     user/libcgj/src/graph_batch39997.c \
 *     user/libcgj/src/graph_batch39998.c \
 *     user/libcgj/src/graph_batch39999.c \
 *     user/libcgj/src/graph_batch40000.c
 *   ./build/cgj_soft_milestone_40000_direct
 *
 * Expected soft KATs (wave 39991–40000 exclusive continuum markers):
 *   gj_shell_green_40000      → 1
 *   gj_libcgj_green_40000     → 1
 *   gj_bar3_ready_40000       → 0   (bar3 still open)
 *   gj_product_score_40000    → 0   (product score still open)
 *   gj_continuum_ready_40000  → 1
 *   gj_smoke_soft_40000       → 1
 *   gj_dyn_soft_40000         → 1
 *   gj_milestone_tag_40000    → 40000
 *   gj_continuum_wave_40000   → 40000
 *   gj_batch_id_40000         → 40000
 *   gj_graph_milestone_40000  → 40000
 *   __libcgj_batch39991_marker … __libcgj_batch40000_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch39991.c … graph_batch40000.c (and fill
 *     39976–39990 / identity 39951–39975) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 40000.
 *   - libc.map may lag milestone 40000 globals; add 40000 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_40000 == 0).
 */
...
#ifndef CGJ_SOFT_M40000_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M40000_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_40000(void);
extern uint32_t gj_libcgj_green_40000(void);
extern uint32_t gj_bar3_ready_40000(void);
extern uint32_t gj_product_score_40000(void);
extern uint32_t gj_continuum_ready_40000(void);
extern uint32_t gj_smoke_soft_40000(void);
extern uint32_t gj_dyn_soft_40000(void);
extern uint32_t gj_milestone_tag_40000(void);
extern uint32_t gj_continuum_wave_40000(void);
extern uint32_t gj_batch_id_40000(void);
extern uint32_t gj_graph_milestone_40000(void);
extern const char __libcgj_batch39991_marker[];
extern const char __libcgj_batch39992_marker[];
extern const char __libcgj_batch39993_marker[];
extern const char __libcgj_batch39994_marker[];
extern const char __libcgj_batch39995_marker[];
extern const char __libcgj_batch39996_marker[];
extern const char __libcgj_batch39997_marker[];
extern const char __libcgj_batch39998_marker[];
extern const char __libcgj_batch39999_marker[];
extern const char __libcgj_batch40000_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_40000() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_40000 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_40000() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_40000 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_40000() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_40000 fail\n");
		nFail++;
	}
	if (gj_product_score_40000() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_40000 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_40000() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_40000 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_40000() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_40000 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_40000() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_40000 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_40000() != 40000u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_40000 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_40000() != 40000u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_40000 fail\n");
		nFail++;
	}
	if (gj_batch_id_40000() != 40000u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_40000 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_40000() != 40000u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_40000 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch39991_marker, "libcgj-batch39991") != 0 ||
	    strcmp(__libcgj_batch39992_marker, "libcgj-batch39992") != 0 ||
	    strcmp(__libcgj_batch39993_marker, "libcgj-batch39993") != 0 ||
	    strcmp(__libcgj_batch39994_marker, "libcgj-batch39994") != 0 ||
	    strcmp(__libcgj_batch39995_marker, "libcgj-batch39995") != 0 ||
	    strcmp(__libcgj_batch39996_marker, "libcgj-batch39996") != 0 ||
	    strcmp(__libcgj_batch39997_marker, "libcgj-batch39997") != 0 ||
	    strcmp(__libcgj_batch39998_marker, "libcgj-batch39998") != 0 ||
	    strcmp(__libcgj_batch39999_marker, "libcgj-batch39999") != 0 ||
	    strcmp(__libcgj_batch40000_marker, "libcgj-batch40000") != 0) {
		fprintf(stderr, "libcgj: direct m40000 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M40000_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m40000 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m40000 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m40000 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m40000 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m40000 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m40000 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_40000", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_40000", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_40000", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_40000", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_40000", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_40000", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_40000", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_40000", 40000u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_40000", 40000u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_40000", 40000u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_40000", 40000u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch39991_marker",
	                     "libcgj-batch39991", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39992_marker",
	                     "libcgj-batch39992", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39993_marker",
	                     "libcgj-batch39993", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39994_marker",
	                     "libcgj-batch39994", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39995_marker",
	                     "libcgj-batch39995", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39996_marker",
	                     "libcgj-batch39996", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39997_marker",
	                     "libcgj-batch39997", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39998_marker",
	                     "libcgj-batch39998", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch39999_marker",
	                     "libcgj-batch39999", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40000_marker",
	                     "libcgj-batch40000", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m40000 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m40000 PASS path=%s checked=%d "
