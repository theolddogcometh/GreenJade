 ...
 *
 * Soft host gates for continuum MILESTONE 40100 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_40100.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_40100.txt
 *
 * Test harness note:
 ...
 *     cgj_soft_milestone_39900.c /
 *     cgj_soft_milestone_40000.c /
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_40100 \
 *     user/libcgj/tests/cgj_soft_milestone_40100.c -ldl
 *   ./build/cgj_soft_milestone_40100
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M40100_DIRECT \
 *     -o build/cgj_soft_milestone_40100_direct \
 *     user/libcgj/tests/cgj_soft_milestone_40100.c \
 *     user/libcgj/src/graph_batch40091.c \
 *     user/libcgj/src/graph_batch40092.c \
 *     user/libcgj/src/graph_batch40093.c \
 *     user/libcgj/src/graph_batch40094.c \
 *     user/libcgj/src/graph_batch40095.c \
 *     user/libcgj/src/graph_batch40096.c \
 *     user/libcgj/src/graph_batch40097.c \
 *     user/libcgj/src/graph_batch40098.c \
 *     user/libcgj/src/graph_batch40099.c \
 *     user/libcgj/src/graph_batch40100.c
 *   ./build/cgj_soft_milestone_40100_direct
 *
 * Expected soft KATs (wave 40091–40100 exclusive continuum markers):
 *   gj_shell_green_40100      → 1
 *   gj_libcgj_green_40100     → 1
 *   gj_bar3_ready_40100       → 0   (bar3 still open)
 *   gj_product_score_40100    → 0   (product score still open)
 *   gj_continuum_ready_40100  → 1
 *   gj_smoke_soft_40100       → 1
 *   gj_dyn_soft_40100         → 1
 *   gj_milestone_tag_40100    → 40100
 *   gj_continuum_wave_40100   → 40100
 *   gj_batch_id_40100         → 40100
 *   gj_graph_milestone_40100  → 40100
 *   __libcgj_batch40091_marker … __libcgj_batch40100_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch40091.c … graph_batch40100.c (and fill
 *     40076–40090 / identity 40051–40075) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 40100.
 *   - libc.map may lag milestone 40100 globals; add 40100 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_40100 == 0).
 */
...
#ifndef CGJ_SOFT_M40100_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M40100_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_40100(void);
extern uint32_t gj_libcgj_green_40100(void);
extern uint32_t gj_bar3_ready_40100(void);
extern uint32_t gj_product_score_40100(void);
extern uint32_t gj_continuum_ready_40100(void);
extern uint32_t gj_smoke_soft_40100(void);
extern uint32_t gj_dyn_soft_40100(void);
extern uint32_t gj_milestone_tag_40100(void);
extern uint32_t gj_continuum_wave_40100(void);
extern uint32_t gj_batch_id_40100(void);
extern uint32_t gj_graph_milestone_40100(void);
extern const char __libcgj_batch40091_marker[];
extern const char __libcgj_batch40092_marker[];
extern const char __libcgj_batch40093_marker[];
extern const char __libcgj_batch40094_marker[];
extern const char __libcgj_batch40095_marker[];
extern const char __libcgj_batch40096_marker[];
extern const char __libcgj_batch40097_marker[];
extern const char __libcgj_batch40098_marker[];
extern const char __libcgj_batch40099_marker[];
extern const char __libcgj_batch40100_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_40100() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_40100 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_40100() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_40100 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_40100() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_40100 fail\n");
		nFail++;
	}
	if (gj_product_score_40100() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_40100 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_40100() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_40100 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_40100() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_40100 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_40100() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_40100 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_40100() != 40100u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_40100 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_40100() != 40100u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_40100 fail\n");
		nFail++;
	}
	if (gj_batch_id_40100() != 40100u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_40100 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_40100() != 40100u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_40100 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch40091_marker, "libcgj-batch40091") != 0 ||
	    strcmp(__libcgj_batch40092_marker, "libcgj-batch40092") != 0 ||
	    strcmp(__libcgj_batch40093_marker, "libcgj-batch40093") != 0 ||
	    strcmp(__libcgj_batch40094_marker, "libcgj-batch40094") != 0 ||
	    strcmp(__libcgj_batch40095_marker, "libcgj-batch40095") != 0 ||
	    strcmp(__libcgj_batch40096_marker, "libcgj-batch40096") != 0 ||
	    strcmp(__libcgj_batch40097_marker, "libcgj-batch40097") != 0 ||
	    strcmp(__libcgj_batch40098_marker, "libcgj-batch40098") != 0 ||
	    strcmp(__libcgj_batch40099_marker, "libcgj-batch40099") != 0 ||
	    strcmp(__libcgj_batch40100_marker, "libcgj-batch40100") != 0) {
		fprintf(stderr, "libcgj: direct m40100 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M40100_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m40100 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m40100 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m40100 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m40100 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m40100 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m40100 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_40100", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_40100", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_40100", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_40100", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_40100", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_40100", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_40100", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_40100", 40100u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_40100", 40100u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_40100", 40100u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_40100", 40100u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch40091_marker",
	                     "libcgj-batch40091", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40092_marker",
	                     "libcgj-batch40092", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40093_marker",
	                     "libcgj-batch40093", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40094_marker",
	                     "libcgj-batch40094", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40095_marker",
	                     "libcgj-batch40095", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40096_marker",
	                     "libcgj-batch40096", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40097_marker",
	                     "libcgj-batch40097", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40098_marker",
	                     "libcgj-batch40098", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40099_marker",
	                     "libcgj-batch40099", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40100_marker",
	                     "libcgj-batch40100", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m40100 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m40100 PASS path=%s checked=%d "
