 ...
 *
 * Soft host gates for continuum MILESTONE 40400 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_40400.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_40400.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_40400 \
 *     user/libcgj/tests/cgj_soft_milestone_40400.c -ldl
 *   ./build/cgj_soft_milestone_40400
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M40400_DIRECT \
 *     -o build/cgj_soft_milestone_40400_direct \
 *     user/libcgj/tests/cgj_soft_milestone_40400.c \
 *     user/libcgj/src/graph_batch40391.c \
 *     user/libcgj/src/graph_batch40392.c \
 *     user/libcgj/src/graph_batch40393.c \
 *     user/libcgj/src/graph_batch40394.c \
 *     user/libcgj/src/graph_batch40395.c \
 *     user/libcgj/src/graph_batch40396.c \
 *     user/libcgj/src/graph_batch40397.c \
 *     user/libcgj/src/graph_batch40398.c \
 *     user/libcgj/src/graph_batch40399.c \
 *     user/libcgj/src/graph_batch40400.c
 *   ./build/cgj_soft_milestone_40400_direct
 *
 * Expected soft KATs (wave 40391–40400 exclusive continuum markers):
 *   gj_shell_green_40400      → 1
 *   gj_libcgj_green_40400     → 1
 *   gj_bar3_ready_40400       → 0   (bar3 still open)
 *   gj_product_score_40400    → 0   (product score still open)
 *   gj_continuum_ready_40400  → 1
 *   gj_smoke_soft_40400       → 1
 *   gj_dyn_soft_40400         → 1
 *   gj_milestone_tag_40400    → 40400
 *   gj_continuum_wave_40400   → 40400
 *   gj_batch_id_40400         → 40400
 *   gj_graph_milestone_40400  → 40400
 *   __libcgj_batch40391_marker … __libcgj_batch40400_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch40391.c … graph_batch40400.c (and fill
 *     40376–40390 / identity 40351–40375) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 40400.
 *   - libc.map may lag milestone 40400 globals; add 40400 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_40400 == 0).
 */
...
#ifndef CGJ_SOFT_M40400_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M40400_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_40400(void);
extern uint32_t gj_libcgj_green_40400(void);
extern uint32_t gj_bar3_ready_40400(void);
extern uint32_t gj_product_score_40400(void);
extern uint32_t gj_continuum_ready_40400(void);
extern uint32_t gj_smoke_soft_40400(void);
extern uint32_t gj_dyn_soft_40400(void);
extern uint32_t gj_milestone_tag_40400(void);
extern uint32_t gj_continuum_wave_40400(void);
extern uint32_t gj_batch_id_40400(void);
extern uint32_t gj_graph_milestone_40400(void);
extern const char __libcgj_batch40391_marker[];
extern const char __libcgj_batch40392_marker[];
extern const char __libcgj_batch40393_marker[];
extern const char __libcgj_batch40394_marker[];
extern const char __libcgj_batch40395_marker[];
extern const char __libcgj_batch40396_marker[];
extern const char __libcgj_batch40397_marker[];
extern const char __libcgj_batch40398_marker[];
extern const char __libcgj_batch40399_marker[];
extern const char __libcgj_batch40400_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_40400() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_40400 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_40400() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_40400 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_40400() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_40400 fail\n");
		nFail++;
	}
	if (gj_product_score_40400() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_40400 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_40400() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_40400 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_40400() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_40400 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_40400() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_40400 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_40400() != 40400u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_40400 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_40400() != 40400u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_40400 fail\n");
		nFail++;
	}
	if (gj_batch_id_40400() != 40400u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_40400 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_40400() != 40400u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_40400 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch40391_marker, "libcgj-batch40391") != 0 ||
	    strcmp(__libcgj_batch40392_marker, "libcgj-batch40392") != 0 ||
	    strcmp(__libcgj_batch40393_marker, "libcgj-batch40393") != 0 ||
	    strcmp(__libcgj_batch40394_marker, "libcgj-batch40394") != 0 ||
	    strcmp(__libcgj_batch40395_marker, "libcgj-batch40395") != 0 ||
	    strcmp(__libcgj_batch40396_marker, "libcgj-batch40396") != 0 ||
	    strcmp(__libcgj_batch40397_marker, "libcgj-batch40397") != 0 ||
	    strcmp(__libcgj_batch40398_marker, "libcgj-batch40398") != 0 ||
	    strcmp(__libcgj_batch40399_marker, "libcgj-batch40399") != 0 ||
	    strcmp(__libcgj_batch40400_marker, "libcgj-batch40400") != 0) {
		fprintf(stderr, "libcgj: direct m40400 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M40400_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m40400 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m40400 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m40400 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m40400 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m40400 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m40400 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_40400", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_40400", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_40400", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_40400", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_40400", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_40400", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_40400", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_40400", 40400u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_40400", 40400u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_40400", 40400u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_40400", 40400u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch40391_marker",
	                     "libcgj-batch40391", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40392_marker",
	                     "libcgj-batch40392", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40393_marker",
	                     "libcgj-batch40393", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40394_marker",
	                     "libcgj-batch40394", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40395_marker",
	                     "libcgj-batch40395", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40396_marker",
	                     "libcgj-batch40396", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40397_marker",
	                     "libcgj-batch40397", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40398_marker",
	                     "libcgj-batch40398", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40399_marker",
	                     "libcgj-batch40399", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40400_marker",
	                     "libcgj-batch40400", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m40400 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m40400 PASS path=%s checked=%d "
