 ...
 *
 * Soft host gates for continuum MILESTONE 40500 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_40500.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_40500.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_40500 \
 *     user/libcgj/tests/cgj_soft_milestone_40500.c -ldl
 *   ./build/cgj_soft_milestone_40500
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M40500_DIRECT \
 *     -o build/cgj_soft_milestone_40500_direct \
 *     user/libcgj/tests/cgj_soft_milestone_40500.c \
 *     user/libcgj/src/graph_batch40491.c \
 *     user/libcgj/src/graph_batch40492.c \
 *     user/libcgj/src/graph_batch40493.c \
 *     user/libcgj/src/graph_batch40494.c \
 *     user/libcgj/src/graph_batch40495.c \
 *     user/libcgj/src/graph_batch40496.c \
 *     user/libcgj/src/graph_batch40497.c \
 *     user/libcgj/src/graph_batch40498.c \
 *     user/libcgj/src/graph_batch40499.c \
 *     user/libcgj/src/graph_batch40500.c
 *   ./build/cgj_soft_milestone_40500_direct
 *
 * Expected soft KATs (wave 40491–40500 exclusive continuum markers):
 *   gj_shell_green_40500      → 1
 *   gj_libcgj_green_40500     → 1
 *   gj_bar3_ready_40500       → 0   (bar3 still open)
 *   gj_product_score_40500    → 0   (product score still open)
 *   gj_continuum_ready_40500  → 1
 *   gj_smoke_soft_40500       → 1
 *   gj_dyn_soft_40500         → 1
 *   gj_milestone_tag_40500    → 40500
 *   gj_continuum_wave_40500   → 40500
 *   gj_batch_id_40500         → 40500
 *   gj_graph_milestone_40500  → 40500
 *   __libcgj_batch40491_marker … __libcgj_batch40500_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch40491.c … graph_batch40500.c (and fill
 *     40476–40490 / identity 40451–40475) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 40500.
 *   - libc.map may lag milestone 40500 globals; add 40500 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_40500 == 0).
 */
...
#ifndef CGJ_SOFT_M40500_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M40500_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_40500(void);
extern uint32_t gj_libcgj_green_40500(void);
extern uint32_t gj_bar3_ready_40500(void);
extern uint32_t gj_product_score_40500(void);
extern uint32_t gj_continuum_ready_40500(void);
extern uint32_t gj_smoke_soft_40500(void);
extern uint32_t gj_dyn_soft_40500(void);
extern uint32_t gj_milestone_tag_40500(void);
extern uint32_t gj_continuum_wave_40500(void);
extern uint32_t gj_batch_id_40500(void);
extern uint32_t gj_graph_milestone_40500(void);
extern const char __libcgj_batch40491_marker[];
extern const char __libcgj_batch40492_marker[];
extern const char __libcgj_batch40493_marker[];
extern const char __libcgj_batch40494_marker[];
extern const char __libcgj_batch40495_marker[];
extern const char __libcgj_batch40496_marker[];
extern const char __libcgj_batch40497_marker[];
extern const char __libcgj_batch40498_marker[];
extern const char __libcgj_batch40499_marker[];
extern const char __libcgj_batch40500_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_40500() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_40500 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_40500() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_40500 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_40500() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_40500 fail\n");
		nFail++;
	}
	if (gj_product_score_40500() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_40500 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_40500() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_40500 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_40500() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_40500 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_40500() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_40500 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_40500() != 40500u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_40500 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_40500() != 40500u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_40500 fail\n");
		nFail++;
	}
	if (gj_batch_id_40500() != 40500u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_40500 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_40500() != 40500u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_40500 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch40491_marker, "libcgj-batch40491") != 0 ||
	    strcmp(__libcgj_batch40492_marker, "libcgj-batch40492") != 0 ||
	    strcmp(__libcgj_batch40493_marker, "libcgj-batch40493") != 0 ||
	    strcmp(__libcgj_batch40494_marker, "libcgj-batch40494") != 0 ||
	    strcmp(__libcgj_batch40495_marker, "libcgj-batch40495") != 0 ||
	    strcmp(__libcgj_batch40496_marker, "libcgj-batch40496") != 0 ||
	    strcmp(__libcgj_batch40497_marker, "libcgj-batch40497") != 0 ||
	    strcmp(__libcgj_batch40498_marker, "libcgj-batch40498") != 0 ||
	    strcmp(__libcgj_batch40499_marker, "libcgj-batch40499") != 0 ||
	    strcmp(__libcgj_batch40500_marker, "libcgj-batch40500") != 0) {
		fprintf(stderr, "libcgj: direct m40500 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M40500_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m40500 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m40500 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m40500 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m40500 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m40500 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m40500 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_40500", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_40500", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_40500", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_40500", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_40500", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_40500", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_40500", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_40500", 40500u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_40500", 40500u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_40500", 40500u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_40500", 40500u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch40491_marker",
	                     "libcgj-batch40491", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40492_marker",
	                     "libcgj-batch40492", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40493_marker",
	                     "libcgj-batch40493", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40494_marker",
	                     "libcgj-batch40494", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40495_marker",
	                     "libcgj-batch40495", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40496_marker",
	                     "libcgj-batch40496", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40497_marker",
	                     "libcgj-batch40497", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40498_marker",
	                     "libcgj-batch40498", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40499_marker",
	                     "libcgj-batch40499", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40500_marker",
	                     "libcgj-batch40500", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m40500 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m40500 PASS path=%s checked=%d "
