 ...
 *
 * Soft host gates for continuum MILESTONE 40600 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_40600.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_40600.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_40600 \
 *     user/libcgj/tests/cgj_soft_milestone_40600.c -ldl
 *   ./build/cgj_soft_milestone_40600
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M40600_DIRECT \
 *     -o build/cgj_soft_milestone_40600_direct \
 *     user/libcgj/tests/cgj_soft_milestone_40600.c \
 *     user/libcgj/src/graph_batch40591.c \
 *     user/libcgj/src/graph_batch40592.c \
 *     user/libcgj/src/graph_batch40593.c \
 *     user/libcgj/src/graph_batch40594.c \
 *     user/libcgj/src/graph_batch40595.c \
 *     user/libcgj/src/graph_batch40596.c \
 *     user/libcgj/src/graph_batch40597.c \
 *     user/libcgj/src/graph_batch40598.c \
 *     user/libcgj/src/graph_batch40599.c \
 *     user/libcgj/src/graph_batch40600.c
 *   ./build/cgj_soft_milestone_40600_direct
 *
 * Expected soft KATs (wave 40591–40600 exclusive continuum markers):
 *   gj_shell_green_40600      → 1
 *   gj_libcgj_green_40600     → 1
 *   gj_bar3_ready_40600       → 0   (bar3 still open)
 *   gj_product_score_40600    → 0   (product score still open)
 *   gj_continuum_ready_40600  → 1
 *   gj_smoke_soft_40600       → 1
 *   gj_dyn_soft_40600         → 1
 *   gj_milestone_tag_40600    → 40600
 *   gj_continuum_wave_40600   → 40600
 *   gj_batch_id_40600         → 40600
 *   gj_graph_milestone_40600  → 40600
 *   __libcgj_batch40591_marker … __libcgj_batch40600_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch40591.c … graph_batch40600.c (and fill
 *     40576–40590 / identity 40551–40575) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 40600.
 *   - libc.map may lag milestone 40600 globals; add 40600 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_40600 == 0).
 */
...
#ifndef CGJ_SOFT_M40600_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M40600_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_40600(void);
extern uint32_t gj_libcgj_green_40600(void);
extern uint32_t gj_bar3_ready_40600(void);
extern uint32_t gj_product_score_40600(void);
extern uint32_t gj_continuum_ready_40600(void);
extern uint32_t gj_smoke_soft_40600(void);
extern uint32_t gj_dyn_soft_40600(void);
extern uint32_t gj_milestone_tag_40600(void);
extern uint32_t gj_continuum_wave_40600(void);
extern uint32_t gj_batch_id_40600(void);
extern uint32_t gj_graph_milestone_40600(void);
extern const char __libcgj_batch40591_marker[];
extern const char __libcgj_batch40592_marker[];
extern const char __libcgj_batch40593_marker[];
extern const char __libcgj_batch40594_marker[];
extern const char __libcgj_batch40595_marker[];
extern const char __libcgj_batch40596_marker[];
extern const char __libcgj_batch40597_marker[];
extern const char __libcgj_batch40598_marker[];
extern const char __libcgj_batch40599_marker[];
extern const char __libcgj_batch40600_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_40600() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_40600 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_40600() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_40600 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_40600() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_40600 fail\n");
		nFail++;
	}
	if (gj_product_score_40600() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_40600 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_40600() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_40600 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_40600() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_40600 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_40600() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_40600 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_40600() != 40600u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_40600 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_40600() != 40600u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_40600 fail\n");
		nFail++;
	}
	if (gj_batch_id_40600() != 40600u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_40600 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_40600() != 40600u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_40600 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch40591_marker, "libcgj-batch40591") != 0 ||
	    strcmp(__libcgj_batch40592_marker, "libcgj-batch40592") != 0 ||
	    strcmp(__libcgj_batch40593_marker, "libcgj-batch40593") != 0 ||
	    strcmp(__libcgj_batch40594_marker, "libcgj-batch40594") != 0 ||
	    strcmp(__libcgj_batch40595_marker, "libcgj-batch40595") != 0 ||
	    strcmp(__libcgj_batch40596_marker, "libcgj-batch40596") != 0 ||
	    strcmp(__libcgj_batch40597_marker, "libcgj-batch40597") != 0 ||
	    strcmp(__libcgj_batch40598_marker, "libcgj-batch40598") != 0 ||
	    strcmp(__libcgj_batch40599_marker, "libcgj-batch40599") != 0 ||
	    strcmp(__libcgj_batch40600_marker, "libcgj-batch40600") != 0) {
		fprintf(stderr, "libcgj: direct m40600 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M40600_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m40600 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m40600 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m40600 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m40600 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m40600 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m40600 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_40600", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_40600", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_40600", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_40600", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_40600", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_40600", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_40600", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_40600", 40600u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_40600", 40600u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_40600", 40600u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_40600", 40600u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch40591_marker",
	                     "libcgj-batch40591", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40592_marker",
	                     "libcgj-batch40592", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40593_marker",
	                     "libcgj-batch40593", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40594_marker",
	                     "libcgj-batch40594", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40595_marker",
	                     "libcgj-batch40595", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40596_marker",
	                     "libcgj-batch40596", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40597_marker",
	                     "libcgj-batch40597", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40598_marker",
	                     "libcgj-batch40598", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40599_marker",
	                     "libcgj-batch40599", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40600_marker",
	                     "libcgj-batch40600", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m40600 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m40600 PASS path=%s checked=%d "
