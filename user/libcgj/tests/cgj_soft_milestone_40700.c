 ...
 *
 * Soft host gates for continuum MILESTONE 40700 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_40700.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_40700.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_40700 \
 *     user/libcgj/tests/cgj_soft_milestone_40700.c -ldl
 *   ./build/cgj_soft_milestone_40700
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M40700_DIRECT \
 *     -o build/cgj_soft_milestone_40700_direct \
 *     user/libcgj/tests/cgj_soft_milestone_40700.c \
 *     user/libcgj/src/graph_batch40691.c \
 *     user/libcgj/src/graph_batch40692.c \
 *     user/libcgj/src/graph_batch40693.c \
 *     user/libcgj/src/graph_batch40694.c \
 *     user/libcgj/src/graph_batch40695.c \
 *     user/libcgj/src/graph_batch40696.c \
 *     user/libcgj/src/graph_batch40697.c \
 *     user/libcgj/src/graph_batch40698.c \
 *     user/libcgj/src/graph_batch40699.c \
 *     user/libcgj/src/graph_batch40700.c
 *   ./build/cgj_soft_milestone_40700_direct
 *
 * Expected soft KATs (wave 40691–40700 exclusive continuum markers):
 *   gj_shell_green_40700      → 1
 *   gj_libcgj_green_40700     → 1
 *   gj_bar3_ready_40700       → 0   (bar3 still open)
 *   gj_product_score_40700    → 0   (product score still open)
 *   gj_continuum_ready_40700  → 1
 *   gj_smoke_soft_40700       → 1
 *   gj_dyn_soft_40700         → 1
 *   gj_milestone_tag_40700    → 40700
 *   gj_continuum_wave_40700   → 40700
 *   gj_batch_id_40700         → 40700
 *   gj_graph_milestone_40700  → 40700
 *   __libcgj_batch40691_marker … __libcgj_batch40700_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch40691.c … graph_batch40700.c (and fill
 *     40676–40690 / identity 40651–40675) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 40700.
 *   - libc.map may lag milestone 40700 globals; add 40700 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_40700 == 0).
 */
...
#ifndef CGJ_SOFT_M40700_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M40700_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_40700(void);
extern uint32_t gj_libcgj_green_40700(void);
extern uint32_t gj_bar3_ready_40700(void);
extern uint32_t gj_product_score_40700(void);
extern uint32_t gj_continuum_ready_40700(void);
extern uint32_t gj_smoke_soft_40700(void);
extern uint32_t gj_dyn_soft_40700(void);
extern uint32_t gj_milestone_tag_40700(void);
extern uint32_t gj_continuum_wave_40700(void);
extern uint32_t gj_batch_id_40700(void);
extern uint32_t gj_graph_milestone_40700(void);
extern const char __libcgj_batch40691_marker[];
extern const char __libcgj_batch40692_marker[];
extern const char __libcgj_batch40693_marker[];
extern const char __libcgj_batch40694_marker[];
extern const char __libcgj_batch40695_marker[];
extern const char __libcgj_batch40696_marker[];
extern const char __libcgj_batch40697_marker[];
extern const char __libcgj_batch40698_marker[];
extern const char __libcgj_batch40699_marker[];
extern const char __libcgj_batch40700_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_40700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_40700 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_40700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_40700 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_40700() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_40700 fail\n");
		nFail++;
	}
	if (gj_product_score_40700() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_40700 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_40700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_40700 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_40700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_40700 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_40700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_40700 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_40700() != 40700u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_40700 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_40700() != 40700u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_40700 fail\n");
		nFail++;
	}
	if (gj_batch_id_40700() != 40700u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_40700 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_40700() != 40700u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_40700 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch40691_marker, "libcgj-batch40691") != 0 ||
	    strcmp(__libcgj_batch40692_marker, "libcgj-batch40692") != 0 ||
	    strcmp(__libcgj_batch40693_marker, "libcgj-batch40693") != 0 ||
	    strcmp(__libcgj_batch40694_marker, "libcgj-batch40694") != 0 ||
	    strcmp(__libcgj_batch40695_marker, "libcgj-batch40695") != 0 ||
	    strcmp(__libcgj_batch40696_marker, "libcgj-batch40696") != 0 ||
	    strcmp(__libcgj_batch40697_marker, "libcgj-batch40697") != 0 ||
	    strcmp(__libcgj_batch40698_marker, "libcgj-batch40698") != 0 ||
	    strcmp(__libcgj_batch40699_marker, "libcgj-batch40699") != 0 ||
	    strcmp(__libcgj_batch40700_marker, "libcgj-batch40700") != 0) {
		fprintf(stderr, "libcgj: direct m40700 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M40700_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m40700 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m40700 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m40700 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m40700 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m40700 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m40700 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_40700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_40700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_40700", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_40700", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_40700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_40700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_40700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_40700", 40700u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_40700", 40700u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_40700", 40700u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_40700", 40700u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch40691_marker",
	                     "libcgj-batch40691", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40692_marker",
	                     "libcgj-batch40692", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40693_marker",
	                     "libcgj-batch40693", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40694_marker",
	                     "libcgj-batch40694", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40695_marker",
	                     "libcgj-batch40695", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40696_marker",
	                     "libcgj-batch40696", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40697_marker",
	                     "libcgj-batch40697", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40698_marker",
	                     "libcgj-batch40698", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40699_marker",
	                     "libcgj-batch40699", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40700_marker",
	                     "libcgj-batch40700", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m40700 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m40700 PASS path=%s checked=%d "
