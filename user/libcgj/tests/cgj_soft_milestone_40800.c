 ...
 *
 * Soft host gates for continuum MILESTONE 40800 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_40800.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_40800.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_40800 \
 *     user/libcgj/tests/cgj_soft_milestone_40800.c -ldl
 *   ./build/cgj_soft_milestone_40800
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M40800_DIRECT \
 *     -o build/cgj_soft_milestone_40800_direct \
 *     user/libcgj/tests/cgj_soft_milestone_40800.c \
 *     user/libcgj/src/graph_batch40791.c \
 *     user/libcgj/src/graph_batch40792.c \
 *     user/libcgj/src/graph_batch40793.c \
 *     user/libcgj/src/graph_batch40794.c \
 *     user/libcgj/src/graph_batch40795.c \
 *     user/libcgj/src/graph_batch40796.c \
 *     user/libcgj/src/graph_batch40797.c \
 *     user/libcgj/src/graph_batch40798.c \
 *     user/libcgj/src/graph_batch40799.c \
 *     user/libcgj/src/graph_batch40800.c
 *   ./build/cgj_soft_milestone_40800_direct
 *
 * Expected soft KATs (wave 40791–40800 exclusive continuum markers):
 *   gj_shell_green_40800      → 1
 *   gj_libcgj_green_40800     → 1
 *   gj_bar3_ready_40800       → 0   (bar3 still open)
 *   gj_product_score_40800    → 0   (product score still open)
 *   gj_continuum_ready_40800  → 1
 *   gj_smoke_soft_40800       → 1
 *   gj_dyn_soft_40800         → 1
 *   gj_milestone_tag_40800    → 40800
 *   gj_continuum_wave_40800   → 40800
 *   gj_batch_id_40800         → 40800
 *   gj_graph_milestone_40800  → 40800
 *   __libcgj_batch40791_marker … __libcgj_batch40800_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch40791.c … graph_batch40800.c (and fill
 *     40776–40790 / identity 40751–40775) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 40800.
 *   - libc.map may lag milestone 40800 globals; add 40800 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_40800 == 0).
 */
...
#ifndef CGJ_SOFT_M40800_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M40800_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_40800(void);
extern uint32_t gj_libcgj_green_40800(void);
extern uint32_t gj_bar3_ready_40800(void);
extern uint32_t gj_product_score_40800(void);
extern uint32_t gj_continuum_ready_40800(void);
extern uint32_t gj_smoke_soft_40800(void);
extern uint32_t gj_dyn_soft_40800(void);
extern uint32_t gj_milestone_tag_40800(void);
extern uint32_t gj_continuum_wave_40800(void);
extern uint32_t gj_batch_id_40800(void);
extern uint32_t gj_graph_milestone_40800(void);
extern const char __libcgj_batch40791_marker[];
extern const char __libcgj_batch40792_marker[];
extern const char __libcgj_batch40793_marker[];
extern const char __libcgj_batch40794_marker[];
extern const char __libcgj_batch40795_marker[];
extern const char __libcgj_batch40796_marker[];
extern const char __libcgj_batch40797_marker[];
extern const char __libcgj_batch40798_marker[];
extern const char __libcgj_batch40799_marker[];
extern const char __libcgj_batch40800_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_40800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_40800 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_40800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_40800 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_40800() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_40800 fail\n");
		nFail++;
	}
	if (gj_product_score_40800() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_40800 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_40800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_40800 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_40800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_40800 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_40800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_40800 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_40800() != 40800u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_40800 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_40800() != 40800u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_40800 fail\n");
		nFail++;
	}
	if (gj_batch_id_40800() != 40800u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_40800 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_40800() != 40800u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_40800 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch40791_marker, "libcgj-batch40791") != 0 ||
	    strcmp(__libcgj_batch40792_marker, "libcgj-batch40792") != 0 ||
	    strcmp(__libcgj_batch40793_marker, "libcgj-batch40793") != 0 ||
	    strcmp(__libcgj_batch40794_marker, "libcgj-batch40794") != 0 ||
	    strcmp(__libcgj_batch40795_marker, "libcgj-batch40795") != 0 ||
	    strcmp(__libcgj_batch40796_marker, "libcgj-batch40796") != 0 ||
	    strcmp(__libcgj_batch40797_marker, "libcgj-batch40797") != 0 ||
	    strcmp(__libcgj_batch40798_marker, "libcgj-batch40798") != 0 ||
	    strcmp(__libcgj_batch40799_marker, "libcgj-batch40799") != 0 ||
	    strcmp(__libcgj_batch40800_marker, "libcgj-batch40800") != 0) {
		fprintf(stderr, "libcgj: direct m40800 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M40800_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m40800 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m40800 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m40800 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m40800 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m40800 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m40800 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_40800", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_40800", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_40800", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_40800", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_40800", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_40800", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_40800", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_40800", 40800u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_40800", 40800u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_40800", 40800u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_40800", 40800u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch40791_marker",
	                     "libcgj-batch40791", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40792_marker",
	                     "libcgj-batch40792", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40793_marker",
	                     "libcgj-batch40793", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40794_marker",
	                     "libcgj-batch40794", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40795_marker",
	                     "libcgj-batch40795", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40796_marker",
	                     "libcgj-batch40796", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40797_marker",
	                     "libcgj-batch40797", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40798_marker",
	                     "libcgj-batch40798", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40799_marker",
	                     "libcgj-batch40799", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40800_marker",
	                     "libcgj-batch40800", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m40800 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m40800 PASS path=%s checked=%d "
