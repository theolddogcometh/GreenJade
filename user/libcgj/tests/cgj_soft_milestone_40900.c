 ...
 *
 * Soft host gates for continuum MILESTONE 40900 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_40900.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_40900.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_40900 \
 *     user/libcgj/tests/cgj_soft_milestone_40900.c -ldl
 *   ./build/cgj_soft_milestone_40900
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M40900_DIRECT \
 *     -o build/cgj_soft_milestone_40900_direct \
 *     user/libcgj/tests/cgj_soft_milestone_40900.c \
 *     user/libcgj/src/graph_batch40891.c \
 *     user/libcgj/src/graph_batch40892.c \
 *     user/libcgj/src/graph_batch40893.c \
 *     user/libcgj/src/graph_batch40894.c \
 *     user/libcgj/src/graph_batch40895.c \
 *     user/libcgj/src/graph_batch40896.c \
 *     user/libcgj/src/graph_batch40897.c \
 *     user/libcgj/src/graph_batch40898.c \
 *     user/libcgj/src/graph_batch40899.c \
 *     user/libcgj/src/graph_batch40900.c
 *   ./build/cgj_soft_milestone_40900_direct
 *
 * Expected soft KATs (wave 40891–40900 exclusive continuum markers):
 *   gj_shell_green_40900      → 1
 *   gj_libcgj_green_40900     → 1
 *   gj_bar3_ready_40900       → 0   (bar3 still open)
 *   gj_product_score_40900    → 0   (product score still open)
 *   gj_continuum_ready_40900  → 1
 *   gj_smoke_soft_40900       → 1
 *   gj_dyn_soft_40900         → 1
 *   gj_milestone_tag_40900    → 40900
 *   gj_continuum_wave_40900   → 40900
 *   gj_batch_id_40900         → 40900
 *   gj_graph_milestone_40900  → 40900
 *   __libcgj_batch40891_marker … __libcgj_batch40900_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch40891.c … graph_batch40900.c (and fill
 *     40876–40890 / identity 40851–40875) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 40900.
 *   - libc.map may lag milestone 40900 globals; add 40900 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_40900 == 0).
 */
...
#ifndef CGJ_SOFT_M40900_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M40900_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_40900(void);
extern uint32_t gj_libcgj_green_40900(void);
extern uint32_t gj_bar3_ready_40900(void);
extern uint32_t gj_product_score_40900(void);
extern uint32_t gj_continuum_ready_40900(void);
extern uint32_t gj_smoke_soft_40900(void);
extern uint32_t gj_dyn_soft_40900(void);
extern uint32_t gj_milestone_tag_40900(void);
extern uint32_t gj_continuum_wave_40900(void);
extern uint32_t gj_batch_id_40900(void);
extern uint32_t gj_graph_milestone_40900(void);
extern const char __libcgj_batch40891_marker[];
extern const char __libcgj_batch40892_marker[];
extern const char __libcgj_batch40893_marker[];
extern const char __libcgj_batch40894_marker[];
extern const char __libcgj_batch40895_marker[];
extern const char __libcgj_batch40896_marker[];
extern const char __libcgj_batch40897_marker[];
extern const char __libcgj_batch40898_marker[];
extern const char __libcgj_batch40899_marker[];
extern const char __libcgj_batch40900_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_40900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_40900 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_40900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_40900 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_40900() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_40900 fail\n");
		nFail++;
	}
	if (gj_product_score_40900() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_40900 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_40900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_40900 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_40900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_40900 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_40900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_40900 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_40900() != 40900u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_40900 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_40900() != 40900u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_40900 fail\n");
		nFail++;
	}
	if (gj_batch_id_40900() != 40900u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_40900 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_40900() != 40900u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_40900 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch40891_marker, "libcgj-batch40891") != 0 ||
	    strcmp(__libcgj_batch40892_marker, "libcgj-batch40892") != 0 ||
	    strcmp(__libcgj_batch40893_marker, "libcgj-batch40893") != 0 ||
	    strcmp(__libcgj_batch40894_marker, "libcgj-batch40894") != 0 ||
	    strcmp(__libcgj_batch40895_marker, "libcgj-batch40895") != 0 ||
	    strcmp(__libcgj_batch40896_marker, "libcgj-batch40896") != 0 ||
	    strcmp(__libcgj_batch40897_marker, "libcgj-batch40897") != 0 ||
	    strcmp(__libcgj_batch40898_marker, "libcgj-batch40898") != 0 ||
	    strcmp(__libcgj_batch40899_marker, "libcgj-batch40899") != 0 ||
	    strcmp(__libcgj_batch40900_marker, "libcgj-batch40900") != 0) {
		fprintf(stderr, "libcgj: direct m40900 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M40900_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m40900 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m40900 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m40900 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m40900 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m40900 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m40900 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_40900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_40900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_40900", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_40900", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_40900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_40900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_40900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_40900", 40900u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_40900", 40900u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_40900", 40900u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_40900", 40900u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch40891_marker",
	                     "libcgj-batch40891", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40892_marker",
	                     "libcgj-batch40892", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40893_marker",
	                     "libcgj-batch40893", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40894_marker",
	                     "libcgj-batch40894", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40895_marker",
	                     "libcgj-batch40895", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40896_marker",
	                     "libcgj-batch40896", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40897_marker",
	                     "libcgj-batch40897", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40898_marker",
	                     "libcgj-batch40898", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40899_marker",
	                     "libcgj-batch40899", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40900_marker",
	                     "libcgj-batch40900", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m40900 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m40900 PASS path=%s checked=%d "
