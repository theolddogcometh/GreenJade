 ...
 *
 * Soft host gates for continuum MILESTONE 41000 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_41000.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_41000.txt
 *
 * Test harness note:
 ...
 *     cgj_soft_milestone_40700.c /
 *     cgj_soft_milestone_40800.c /
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_41000 \
 *     user/libcgj/tests/cgj_soft_milestone_41000.c -ldl
 *   ./build/cgj_soft_milestone_41000
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M41000_DIRECT \
 *     -o build/cgj_soft_milestone_41000_direct \
 *     user/libcgj/tests/cgj_soft_milestone_41000.c \
 *     user/libcgj/src/graph_batch40991.c \
 *     user/libcgj/src/graph_batch40992.c \
 *     user/libcgj/src/graph_batch40993.c \
 *     user/libcgj/src/graph_batch40994.c \
 *     user/libcgj/src/graph_batch40995.c \
 *     user/libcgj/src/graph_batch40996.c \
 *     user/libcgj/src/graph_batch40997.c \
 *     user/libcgj/src/graph_batch40998.c \
 *     user/libcgj/src/graph_batch40999.c \
 *     user/libcgj/src/graph_batch41000.c
 *   ./build/cgj_soft_milestone_41000_direct
 *
 * Expected soft KATs (wave 40991–41000 exclusive continuum markers):
 *   gj_shell_green_41000      → 1
 *   gj_libcgj_green_41000     → 1
 *   gj_bar3_ready_41000       → 0   (bar3 still open)
 *   gj_product_score_41000    → 0   (product score still open)
 *   gj_continuum_ready_41000  → 1
 *   gj_smoke_soft_41000       → 1
 *   gj_dyn_soft_41000         → 1
 *   gj_milestone_tag_41000    → 41000
 *   gj_continuum_wave_41000   → 41000
 *   gj_batch_id_41000         → 41000
 *   gj_graph_milestone_41000  → 41000
 *   __libcgj_batch40991_marker … __libcgj_batch41000_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch40991.c … graph_batch41000.c (and fill
 *     40976–40990 / identity 40951–40975) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 41000.
 *   - libc.map may lag milestone 41000 globals; add 41000 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_41000 == 0).
 */
...
#ifndef CGJ_SOFT_M41000_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M41000_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_41000(void);
extern uint32_t gj_libcgj_green_41000(void);
extern uint32_t gj_bar3_ready_41000(void);
extern uint32_t gj_product_score_41000(void);
extern uint32_t gj_continuum_ready_41000(void);
extern uint32_t gj_smoke_soft_41000(void);
extern uint32_t gj_dyn_soft_41000(void);
extern uint32_t gj_milestone_tag_41000(void);
extern uint32_t gj_continuum_wave_41000(void);
extern uint32_t gj_batch_id_41000(void);
extern uint32_t gj_graph_milestone_41000(void);
extern const char __libcgj_batch40991_marker[];
extern const char __libcgj_batch40992_marker[];
extern const char __libcgj_batch40993_marker[];
extern const char __libcgj_batch40994_marker[];
extern const char __libcgj_batch40995_marker[];
extern const char __libcgj_batch40996_marker[];
extern const char __libcgj_batch40997_marker[];
extern const char __libcgj_batch40998_marker[];
extern const char __libcgj_batch40999_marker[];
extern const char __libcgj_batch41000_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_41000() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_41000 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_41000() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_41000 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_41000() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_41000 fail\n");
		nFail++;
	}
	if (gj_product_score_41000() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_41000 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_41000() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_41000 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_41000() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_41000 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_41000() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_41000 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_41000() != 41000u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_41000 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_41000() != 41000u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_41000 fail\n");
		nFail++;
	}
	if (gj_batch_id_41000() != 41000u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_41000 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_41000() != 41000u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_41000 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch40991_marker, "libcgj-batch40991") != 0 ||
	    strcmp(__libcgj_batch40992_marker, "libcgj-batch40992") != 0 ||
	    strcmp(__libcgj_batch40993_marker, "libcgj-batch40993") != 0 ||
	    strcmp(__libcgj_batch40994_marker, "libcgj-batch40994") != 0 ||
	    strcmp(__libcgj_batch40995_marker, "libcgj-batch40995") != 0 ||
	    strcmp(__libcgj_batch40996_marker, "libcgj-batch40996") != 0 ||
	    strcmp(__libcgj_batch40997_marker, "libcgj-batch40997") != 0 ||
	    strcmp(__libcgj_batch40998_marker, "libcgj-batch40998") != 0 ||
	    strcmp(__libcgj_batch40999_marker, "libcgj-batch40999") != 0 ||
	    strcmp(__libcgj_batch41000_marker, "libcgj-batch41000") != 0) {
		fprintf(stderr, "libcgj: direct m41000 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M41000_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m41000 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m41000 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m41000 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m41000 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m41000 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m41000 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_41000", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_41000", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_41000", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_41000", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_41000", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_41000", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_41000", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_41000", 41000u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_41000", 41000u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_41000", 41000u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_41000", 41000u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch40991_marker",
	                     "libcgj-batch40991", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40992_marker",
	                     "libcgj-batch40992", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40993_marker",
	                     "libcgj-batch40993", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40994_marker",
	                     "libcgj-batch40994", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40995_marker",
	                     "libcgj-batch40995", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40996_marker",
	                     "libcgj-batch40996", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40997_marker",
	                     "libcgj-batch40997", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40998_marker",
	                     "libcgj-batch40998", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch40999_marker",
	                     "libcgj-batch40999", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch41000_marker",
	                     "libcgj-batch41000", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m41000 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m41000 PASS path=%s checked=%d "
