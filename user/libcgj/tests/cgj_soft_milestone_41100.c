 ...
 *
 * Soft host gates for continuum MILESTONE 41100 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_41100.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_41100.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_41100 \
 *     user/libcgj/tests/cgj_soft_milestone_41100.c -ldl
 *   ./build/cgj_soft_milestone_41100
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M41100_DIRECT \
 *     -o build/cgj_soft_milestone_41100_direct \
 *     user/libcgj/tests/cgj_soft_milestone_41100.c \
 *     user/libcgj/src/graph_batch41091.c \
 *     user/libcgj/src/graph_batch41092.c \
 *     user/libcgj/src/graph_batch41093.c \
 *     user/libcgj/src/graph_batch41094.c \
 *     user/libcgj/src/graph_batch41095.c \
 *     user/libcgj/src/graph_batch41096.c \
 *     user/libcgj/src/graph_batch41097.c \
 *     user/libcgj/src/graph_batch41098.c \
 *     user/libcgj/src/graph_batch41099.c \
 *     user/libcgj/src/graph_batch41100.c
 *   ./build/cgj_soft_milestone_41100_direct
 *
 * Expected soft KATs (wave 41091–41100 exclusive continuum markers):
 *   gj_shell_green_41100      → 1
 *   gj_libcgj_green_41100     → 1
 *   gj_bar3_ready_41100       → 0   (bar3 still open)
 *   gj_product_score_41100    → 0   (product score still open)
 *   gj_continuum_ready_41100  → 1
 *   gj_smoke_soft_41100       → 1
 *   gj_dyn_soft_41100         → 1
 *   gj_milestone_tag_41100    → 41100
 *   gj_continuum_wave_41100   → 41100
 *   gj_batch_id_41100         → 41100
 *   gj_graph_milestone_41100  → 41100
 *   __libcgj_batch41091_marker … __libcgj_batch41100_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch41091.c … graph_batch41100.c (and fill
 *     41076–41090 / identity 41051–41075) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 41100.
 *   - libc.map may lag milestone 41100 globals; add 41100 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_41100 == 0).
 */
...
#ifndef CGJ_SOFT_M41100_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M41100_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_41100(void);
extern uint32_t gj_libcgj_green_41100(void);
extern uint32_t gj_bar3_ready_41100(void);
extern uint32_t gj_product_score_41100(void);
extern uint32_t gj_continuum_ready_41100(void);
extern uint32_t gj_smoke_soft_41100(void);
extern uint32_t gj_dyn_soft_41100(void);
extern uint32_t gj_milestone_tag_41100(void);
extern uint32_t gj_continuum_wave_41100(void);
extern uint32_t gj_batch_id_41100(void);
extern uint32_t gj_graph_milestone_41100(void);
extern const char __libcgj_batch41091_marker[];
extern const char __libcgj_batch41092_marker[];
extern const char __libcgj_batch41093_marker[];
extern const char __libcgj_batch41094_marker[];
extern const char __libcgj_batch41095_marker[];
extern const char __libcgj_batch41096_marker[];
extern const char __libcgj_batch41097_marker[];
extern const char __libcgj_batch41098_marker[];
extern const char __libcgj_batch41099_marker[];
extern const char __libcgj_batch41100_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_41100() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_41100 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_41100() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_41100 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_41100() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_41100 fail\n");
		nFail++;
	}
	if (gj_product_score_41100() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_41100 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_41100() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_41100 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_41100() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_41100 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_41100() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_41100 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_41100() != 41100u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_41100 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_41100() != 41100u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_41100 fail\n");
		nFail++;
	}
	if (gj_batch_id_41100() != 41100u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_41100 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_41100() != 41100u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_41100 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch41091_marker, "libcgj-batch41091") != 0 ||
	    strcmp(__libcgj_batch41092_marker, "libcgj-batch41092") != 0 ||
	    strcmp(__libcgj_batch41093_marker, "libcgj-batch41093") != 0 ||
	    strcmp(__libcgj_batch41094_marker, "libcgj-batch41094") != 0 ||
	    strcmp(__libcgj_batch41095_marker, "libcgj-batch41095") != 0 ||
	    strcmp(__libcgj_batch41096_marker, "libcgj-batch41096") != 0 ||
	    strcmp(__libcgj_batch41097_marker, "libcgj-batch41097") != 0 ||
	    strcmp(__libcgj_batch41098_marker, "libcgj-batch41098") != 0 ||
	    strcmp(__libcgj_batch41099_marker, "libcgj-batch41099") != 0 ||
	    strcmp(__libcgj_batch41100_marker, "libcgj-batch41100") != 0) {
		fprintf(stderr, "libcgj: direct m41100 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M41100_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m41100 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m41100 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m41100 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m41100 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m41100 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m41100 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_41100", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_41100", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_41100", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_41100", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_41100", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_41100", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_41100", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_41100", 41100u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_41100", 41100u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_41100", 41100u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_41100", 41100u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch41091_marker",
	                     "libcgj-batch41091", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch41092_marker",
	                     "libcgj-batch41092", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch41093_marker",
	                     "libcgj-batch41093", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch41094_marker",
	                     "libcgj-batch41094", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch41095_marker",
	                     "libcgj-batch41095", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch41096_marker",
	                     "libcgj-batch41096", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch41097_marker",
	                     "libcgj-batch41097", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch41098_marker",
	                     "libcgj-batch41098", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch41099_marker",
	                     "libcgj-batch41099", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch41100_marker",
	                     "libcgj-batch41100", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m41100 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m41100 PASS path=%s checked=%d "
