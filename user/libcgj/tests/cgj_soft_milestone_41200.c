 ...
 *
 * Soft host gates for continuum MILESTONE 41200 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_41200.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_41200.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_41200 \
 *     user/libcgj/tests/cgj_soft_milestone_41200.c -ldl
 *   ./build/cgj_soft_milestone_41200
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M41200_DIRECT \
 *     -o build/cgj_soft_milestone_41200_direct \
 *     user/libcgj/tests/cgj_soft_milestone_41200.c \
 *     user/libcgj/src/graph_batch41191.c \
 *     user/libcgj/src/graph_batch41192.c \
 *     user/libcgj/src/graph_batch41193.c \
 *     user/libcgj/src/graph_batch41194.c \
 *     user/libcgj/src/graph_batch41195.c \
 *     user/libcgj/src/graph_batch41196.c \
 *     user/libcgj/src/graph_batch41197.c \
 *     user/libcgj/src/graph_batch41198.c \
 *     user/libcgj/src/graph_batch41199.c \
 *     user/libcgj/src/graph_batch41200.c
 *   ./build/cgj_soft_milestone_41200_direct
 *
 * Expected soft KATs (wave 41191–41200 exclusive continuum markers):
 *   gj_shell_green_41200      → 1
 *   gj_libcgj_green_41200     → 1
 *   gj_bar3_ready_41200       → 0   (bar3 still open)
 *   gj_product_score_41200    → 0   (product score still open)
 *   gj_continuum_ready_41200  → 1
 *   gj_smoke_soft_41200       → 1
 *   gj_dyn_soft_41200         → 1
 *   gj_milestone_tag_41200    → 41200
 *   gj_continuum_wave_41200   → 41200
 *   gj_batch_id_41200         → 41200
 *   gj_graph_milestone_41200  → 41200
 *   __libcgj_batch41191_marker … __libcgj_batch41200_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch41191.c … graph_batch41200.c (and fill
 *     41176–41190 / identity 41151–41175) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 41200.
 *   - libc.map may lag milestone 41200 globals; add 41200 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_41200 == 0).
 */
...
#ifndef CGJ_SOFT_M41200_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M41200_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_41200(void);
extern uint32_t gj_libcgj_green_41200(void);
extern uint32_t gj_bar3_ready_41200(void);
extern uint32_t gj_product_score_41200(void);
extern uint32_t gj_continuum_ready_41200(void);
extern uint32_t gj_smoke_soft_41200(void);
extern uint32_t gj_dyn_soft_41200(void);
extern uint32_t gj_milestone_tag_41200(void);
extern uint32_t gj_continuum_wave_41200(void);
extern uint32_t gj_batch_id_41200(void);
extern uint32_t gj_graph_milestone_41200(void);
extern const char __libcgj_batch41191_marker[];
extern const char __libcgj_batch41192_marker[];
extern const char __libcgj_batch41193_marker[];
extern const char __libcgj_batch41194_marker[];
extern const char __libcgj_batch41195_marker[];
extern const char __libcgj_batch41196_marker[];
extern const char __libcgj_batch41197_marker[];
extern const char __libcgj_batch41198_marker[];
extern const char __libcgj_batch41199_marker[];
extern const char __libcgj_batch41200_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_41200() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_41200 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_41200() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_41200 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_41200() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_41200 fail\n");
		nFail++;
	}
	if (gj_product_score_41200() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_41200 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_41200() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_41200 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_41200() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_41200 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_41200() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_41200 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_41200() != 41200u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_41200 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_41200() != 41200u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_41200 fail\n");
		nFail++;
	}
	if (gj_batch_id_41200() != 41200u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_41200 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_41200() != 41200u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_41200 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch41191_marker, "libcgj-batch41191") != 0 ||
	    strcmp(__libcgj_batch41192_marker, "libcgj-batch41192") != 0 ||
	    strcmp(__libcgj_batch41193_marker, "libcgj-batch41193") != 0 ||
	    strcmp(__libcgj_batch41194_marker, "libcgj-batch41194") != 0 ||
	    strcmp(__libcgj_batch41195_marker, "libcgj-batch41195") != 0 ||
	    strcmp(__libcgj_batch41196_marker, "libcgj-batch41196") != 0 ||
	    strcmp(__libcgj_batch41197_marker, "libcgj-batch41197") != 0 ||
	    strcmp(__libcgj_batch41198_marker, "libcgj-batch41198") != 0 ||
	    strcmp(__libcgj_batch41199_marker, "libcgj-batch41199") != 0 ||
	    strcmp(__libcgj_batch41200_marker, "libcgj-batch41200") != 0) {
		fprintf(stderr, "libcgj: direct m41200 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M41200_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m41200 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m41200 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m41200 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m41200 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m41200 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m41200 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_41200", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_41200", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_41200", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_41200", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_41200", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_41200", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_41200", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_41200", 41200u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_41200", 41200u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_41200", 41200u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_41200", 41200u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch41191_marker",
	                     "libcgj-batch41191", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch41192_marker",
	                     "libcgj-batch41192", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch41193_marker",
	                     "libcgj-batch41193", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch41194_marker",
	                     "libcgj-batch41194", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch41195_marker",
	                     "libcgj-batch41195", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch41196_marker",
	                     "libcgj-batch41196", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch41197_marker",
	                     "libcgj-batch41197", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch41198_marker",
	                     "libcgj-batch41198", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch41199_marker",
	                     "libcgj-batch41199", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch41200_marker",
	                     "libcgj-batch41200", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m41200 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m41200 PASS path=%s checked=%d "
