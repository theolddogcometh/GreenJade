 ...
 *
 * Soft host gates for continuum MILESTONE 41700 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_41700.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_41700.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_41700 \
 *     user/libcgj/tests/cgj_soft_milestone_41700.c -ldl
 *   ./build/cgj_soft_milestone_41700
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M41700_DIRECT \
 *     -o build/cgj_soft_milestone_41700_direct \
 *     user/libcgj/tests/cgj_soft_milestone_41700.c \
 *     user/libcgj/src/graph_batch41691.c \
 *     user/libcgj/src/graph_batch41692.c \
 *     user/libcgj/src/graph_batch41693.c \
 *     user/libcgj/src/graph_batch41694.c \
 *     user/libcgj/src/graph_batch41695.c \
 *     user/libcgj/src/graph_batch41696.c \
 *     user/libcgj/src/graph_batch41697.c \
 *     user/libcgj/src/graph_batch41698.c \
 *     user/libcgj/src/graph_batch41699.c \
 *     user/libcgj/src/graph_batch41700.c
 *   ./build/cgj_soft_milestone_41700_direct
 *
 * Expected soft KATs (wave 41691–41700 exclusive continuum markers):
 *   gj_shell_green_41700      → 1
 *   gj_libcgj_green_41700     → 1
 *   gj_bar3_ready_41700       → 0   (bar3 still open)
 *   gj_product_score_41700    → 0   (product score still open)
 *   gj_continuum_ready_41700  → 1
 *   gj_smoke_soft_41700       → 1
 *   gj_dyn_soft_41700         → 1
 *   gj_milestone_tag_41700    → 41700
 *   gj_continuum_wave_41700   → 41700
 *   gj_batch_id_41700         → 41700
 *   gj_graph_milestone_41700  → 41700
 *   __libcgj_batch41691_marker … __libcgj_batch41700_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch41691.c … graph_batch41700.c (and fill
 *     41676–41690 / identity 41651–41675) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 41700.
 *   - libc.map may lag milestone 41700 globals; add 41700 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_41700 == 0).
 */
...
#ifndef CGJ_SOFT_M41700_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M41700_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_41700(void);
extern uint32_t gj_libcgj_green_41700(void);
extern uint32_t gj_bar3_ready_41700(void);
extern uint32_t gj_product_score_41700(void);
extern uint32_t gj_continuum_ready_41700(void);
extern uint32_t gj_smoke_soft_41700(void);
extern uint32_t gj_dyn_soft_41700(void);
extern uint32_t gj_milestone_tag_41700(void);
extern uint32_t gj_continuum_wave_41700(void);
extern uint32_t gj_batch_id_41700(void);
extern uint32_t gj_graph_milestone_41700(void);
extern const char __libcgj_batch41691_marker[];
extern const char __libcgj_batch41692_marker[];
extern const char __libcgj_batch41693_marker[];
extern const char __libcgj_batch41694_marker[];
extern const char __libcgj_batch41695_marker[];
extern const char __libcgj_batch41696_marker[];
extern const char __libcgj_batch41697_marker[];
extern const char __libcgj_batch41698_marker[];
extern const char __libcgj_batch41699_marker[];
extern const char __libcgj_batch41700_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_41700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_41700 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_41700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_41700 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_41700() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_41700 fail\n");
		nFail++;
	}
	if (gj_product_score_41700() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_41700 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_41700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_41700 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_41700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_41700 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_41700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_41700 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_41700() != 41700u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_41700 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_41700() != 41700u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_41700 fail\n");
		nFail++;
	}
	if (gj_batch_id_41700() != 41700u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_41700 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_41700() != 41700u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_41700 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch41691_marker, "libcgj-batch41691") != 0 ||
	    strcmp(__libcgj_batch41692_marker, "libcgj-batch41692") != 0 ||
	    strcmp(__libcgj_batch41693_marker, "libcgj-batch41693") != 0 ||
	    strcmp(__libcgj_batch41694_marker, "libcgj-batch41694") != 0 ||
	    strcmp(__libcgj_batch41695_marker, "libcgj-batch41695") != 0 ||
	    strcmp(__libcgj_batch41696_marker, "libcgj-batch41696") != 0 ||
	    strcmp(__libcgj_batch41697_marker, "libcgj-batch41697") != 0 ||
	    strcmp(__libcgj_batch41698_marker, "libcgj-batch41698") != 0 ||
	    strcmp(__libcgj_batch41699_marker, "libcgj-batch41699") != 0 ||
	    strcmp(__libcgj_batch41700_marker, "libcgj-batch41700") != 0) {
		fprintf(stderr, "libcgj: direct m41700 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M41700_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m41700 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m41700 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m41700 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m41700 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m41700 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m41700 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_41700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_41700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_41700", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_41700", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_41700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_41700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_41700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_41700", 41700u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_41700", 41700u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_41700", 41700u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_41700", 41700u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch41691_marker",
	                     "libcgj-batch41691", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch41692_marker",
	                     "libcgj-batch41692", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch41693_marker",
	                     "libcgj-batch41693", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch41694_marker",
	                     "libcgj-batch41694", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch41695_marker",
	                     "libcgj-batch41695", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch41696_marker",
	                     "libcgj-batch41696", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch41697_marker",
	                     "libcgj-batch41697", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch41698_marker",
	                     "libcgj-batch41698", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch41699_marker",
	                     "libcgj-batch41699", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch41700_marker",
	                     "libcgj-batch41700", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m41700 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m41700 PASS path=%s checked=%d "
