 ...
 *
 * Soft host gates for continuum MILESTONE 41900 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_41900.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_41900.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_41900 \
 *     user/libcgj/tests/cgj_soft_milestone_41900.c -ldl
 *   ./build/cgj_soft_milestone_41900
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M41900_DIRECT \
 *     -o build/cgj_soft_milestone_41900_direct \
 *     user/libcgj/tests/cgj_soft_milestone_41900.c \
 *     user/libcgj/src/graph_batch41891.c \
 *     user/libcgj/src/graph_batch41892.c \
 *     user/libcgj/src/graph_batch41893.c \
 *     user/libcgj/src/graph_batch41894.c \
 *     user/libcgj/src/graph_batch41895.c \
 *     user/libcgj/src/graph_batch41896.c \
 *     user/libcgj/src/graph_batch41897.c \
 *     user/libcgj/src/graph_batch41898.c \
 *     user/libcgj/src/graph_batch41899.c \
 *     user/libcgj/src/graph_batch41900.c
 *   ./build/cgj_soft_milestone_41900_direct
 *
 * Expected soft KATs (wave 41891–41900 exclusive continuum markers):
 *   gj_shell_green_41900      → 1
 *   gj_libcgj_green_41900     → 1
 *   gj_bar3_ready_41900       → 0   (bar3 still open)
 *   gj_product_score_41900    → 0   (product score still open)
 *   gj_continuum_ready_41900  → 1
 *   gj_smoke_soft_41900       → 1
 *   gj_dyn_soft_41900         → 1
 *   gj_milestone_tag_41900    → 41900
 *   gj_continuum_wave_41900   → 41900
 *   gj_batch_id_41900         → 41900
 *   gj_graph_milestone_41900  → 41900
 *   __libcgj_batch41891_marker … __libcgj_batch41900_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch41891.c … graph_batch41900.c (and fill
 *     41876–41890 / identity 41851–41875) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 41900.
 *   - libc.map may lag milestone 41900 globals; add 41900 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_41900 == 0).
 */
...
#ifndef CGJ_SOFT_M41900_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M41900_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_41900(void);
extern uint32_t gj_libcgj_green_41900(void);
extern uint32_t gj_bar3_ready_41900(void);
extern uint32_t gj_product_score_41900(void);
extern uint32_t gj_continuum_ready_41900(void);
extern uint32_t gj_smoke_soft_41900(void);
extern uint32_t gj_dyn_soft_41900(void);
extern uint32_t gj_milestone_tag_41900(void);
extern uint32_t gj_continuum_wave_41900(void);
extern uint32_t gj_batch_id_41900(void);
extern uint32_t gj_graph_milestone_41900(void);
extern const char __libcgj_batch41891_marker[];
extern const char __libcgj_batch41892_marker[];
extern const char __libcgj_batch41893_marker[];
extern const char __libcgj_batch41894_marker[];
extern const char __libcgj_batch41895_marker[];
extern const char __libcgj_batch41896_marker[];
extern const char __libcgj_batch41897_marker[];
extern const char __libcgj_batch41898_marker[];
extern const char __libcgj_batch41899_marker[];
extern const char __libcgj_batch41900_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_41900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_41900 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_41900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_41900 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_41900() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_41900 fail\n");
		nFail++;
	}
	if (gj_product_score_41900() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_41900 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_41900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_41900 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_41900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_41900 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_41900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_41900 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_41900() != 41900u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_41900 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_41900() != 41900u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_41900 fail\n");
		nFail++;
	}
	if (gj_batch_id_41900() != 41900u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_41900 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_41900() != 41900u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_41900 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch41891_marker, "libcgj-batch41891") != 0 ||
	    strcmp(__libcgj_batch41892_marker, "libcgj-batch41892") != 0 ||
	    strcmp(__libcgj_batch41893_marker, "libcgj-batch41893") != 0 ||
	    strcmp(__libcgj_batch41894_marker, "libcgj-batch41894") != 0 ||
	    strcmp(__libcgj_batch41895_marker, "libcgj-batch41895") != 0 ||
	    strcmp(__libcgj_batch41896_marker, "libcgj-batch41896") != 0 ||
	    strcmp(__libcgj_batch41897_marker, "libcgj-batch41897") != 0 ||
	    strcmp(__libcgj_batch41898_marker, "libcgj-batch41898") != 0 ||
	    strcmp(__libcgj_batch41899_marker, "libcgj-batch41899") != 0 ||
	    strcmp(__libcgj_batch41900_marker, "libcgj-batch41900") != 0) {
		fprintf(stderr, "libcgj: direct m41900 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M41900_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m41900 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m41900 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m41900 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m41900 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m41900 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m41900 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_41900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_41900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_41900", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_41900", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_41900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_41900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_41900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_41900", 41900u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_41900", 41900u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_41900", 41900u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_41900", 41900u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch41891_marker",
	                     "libcgj-batch41891", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch41892_marker",
	                     "libcgj-batch41892", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch41893_marker",
	                     "libcgj-batch41893", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch41894_marker",
	                     "libcgj-batch41894", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch41895_marker",
	                     "libcgj-batch41895", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch41896_marker",
	                     "libcgj-batch41896", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch41897_marker",
	                     "libcgj-batch41897", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch41898_marker",
	                     "libcgj-batch41898", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch41899_marker",
	                     "libcgj-batch41899", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch41900_marker",
	                     "libcgj-batch41900", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m41900 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m41900 PASS path=%s checked=%d "
