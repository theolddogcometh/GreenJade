 ...
 *
 * Soft host gates for continuum MILESTONE 42800 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_42800.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_42800.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_42800 \
 *     user/libcgj/tests/cgj_soft_milestone_42800.c -ldl
 *   ./build/cgj_soft_milestone_42800
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M42800_DIRECT \
 *     -o build/cgj_soft_milestone_42800_direct \
 *     user/libcgj/tests/cgj_soft_milestone_42800.c \
 *     user/libcgj/src/graph_batch42791.c \
 *     user/libcgj/src/graph_batch42792.c \
 *     user/libcgj/src/graph_batch42793.c \
 *     user/libcgj/src/graph_batch42794.c \
 *     user/libcgj/src/graph_batch42795.c \
 *     user/libcgj/src/graph_batch42796.c \
 *     user/libcgj/src/graph_batch42797.c \
 *     user/libcgj/src/graph_batch42798.c \
 *     user/libcgj/src/graph_batch42799.c \
 *     user/libcgj/src/graph_batch42800.c
 *   ./build/cgj_soft_milestone_42800_direct
 *
 * Expected soft KATs (wave 42791–42800 exclusive continuum markers):
 *   gj_shell_green_42800      → 1
 *   gj_libcgj_green_42800     → 1
 *   gj_bar3_ready_42800       → 0   (bar3 still open)
 *   gj_product_score_42800    → 0   (product score still open)
 *   gj_continuum_ready_42800  → 1
 *   gj_smoke_soft_42800       → 1
 *   gj_dyn_soft_42800         → 1
 *   gj_milestone_tag_42800    → 42800
 *   gj_continuum_wave_42800   → 42800
 *   gj_batch_id_42800         → 42800
 *   gj_graph_milestone_42800  → 42800
 *   __libcgj_batch42791_marker … __libcgj_batch42800_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch42791.c … graph_batch42800.c (and fill
 *     42776–42790 / identity 42751–42775) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 42800.
 *   - libc.map may lag milestone 42800 globals; add 42800 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_42800 == 0).
 */
...
#ifndef CGJ_SOFT_M42800_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M42800_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_42800(void);
extern uint32_t gj_libcgj_green_42800(void);
extern uint32_t gj_bar3_ready_42800(void);
extern uint32_t gj_product_score_42800(void);
extern uint32_t gj_continuum_ready_42800(void);
extern uint32_t gj_smoke_soft_42800(void);
extern uint32_t gj_dyn_soft_42800(void);
extern uint32_t gj_milestone_tag_42800(void);
extern uint32_t gj_continuum_wave_42800(void);
extern uint32_t gj_batch_id_42800(void);
extern uint32_t gj_graph_milestone_42800(void);
extern const char __libcgj_batch42791_marker[];
extern const char __libcgj_batch42792_marker[];
extern const char __libcgj_batch42793_marker[];
extern const char __libcgj_batch42794_marker[];
extern const char __libcgj_batch42795_marker[];
extern const char __libcgj_batch42796_marker[];
extern const char __libcgj_batch42797_marker[];
extern const char __libcgj_batch42798_marker[];
extern const char __libcgj_batch42799_marker[];
extern const char __libcgj_batch42800_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_42800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_42800 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_42800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_42800 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_42800() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_42800 fail\n");
		nFail++;
	}
	if (gj_product_score_42800() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_42800 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_42800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_42800 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_42800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_42800 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_42800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_42800 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_42800() != 42800u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_42800 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_42800() != 42800u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_42800 fail\n");
		nFail++;
	}
	if (gj_batch_id_42800() != 42800u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_42800 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_42800() != 42800u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_42800 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch42791_marker, "libcgj-batch42791") != 0 ||
	    strcmp(__libcgj_batch42792_marker, "libcgj-batch42792") != 0 ||
	    strcmp(__libcgj_batch42793_marker, "libcgj-batch42793") != 0 ||
	    strcmp(__libcgj_batch42794_marker, "libcgj-batch42794") != 0 ||
	    strcmp(__libcgj_batch42795_marker, "libcgj-batch42795") != 0 ||
	    strcmp(__libcgj_batch42796_marker, "libcgj-batch42796") != 0 ||
	    strcmp(__libcgj_batch42797_marker, "libcgj-batch42797") != 0 ||
	    strcmp(__libcgj_batch42798_marker, "libcgj-batch42798") != 0 ||
	    strcmp(__libcgj_batch42799_marker, "libcgj-batch42799") != 0 ||
	    strcmp(__libcgj_batch42800_marker, "libcgj-batch42800") != 0) {
		fprintf(stderr, "libcgj: direct m42800 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M42800_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m42800 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m42800 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m42800 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m42800 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m42800 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m42800 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_42800", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_42800", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_42800", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_42800", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_42800", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_42800", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_42800", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_42800", 42800u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_42800", 42800u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_42800", 42800u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_42800", 42800u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch42791_marker",
	                     "libcgj-batch42791", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch42792_marker",
	                     "libcgj-batch42792", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch42793_marker",
	                     "libcgj-batch42793", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch42794_marker",
	                     "libcgj-batch42794", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch42795_marker",
	                     "libcgj-batch42795", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch42796_marker",
	                     "libcgj-batch42796", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch42797_marker",
	                     "libcgj-batch42797", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch42798_marker",
	                     "libcgj-batch42798", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch42799_marker",
	                     "libcgj-batch42799", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch42800_marker",
	                     "libcgj-batch42800", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m42800 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m42800 PASS path=%s checked=%d "
