 ...
 *
 * Soft host gates for continuum MILESTONE 43200 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_43200.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_43200.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_43200 \
 *     user/libcgj/tests/cgj_soft_milestone_43200.c -ldl
 *   ./build/cgj_soft_milestone_43200
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M43200_DIRECT \
 *     -o build/cgj_soft_milestone_43200_direct \
 *     user/libcgj/tests/cgj_soft_milestone_43200.c \
 *     user/libcgj/src/graph_batch43191.c \
 *     user/libcgj/src/graph_batch43192.c \
 *     user/libcgj/src/graph_batch43193.c \
 *     user/libcgj/src/graph_batch43194.c \
 *     user/libcgj/src/graph_batch43195.c \
 *     user/libcgj/src/graph_batch43196.c \
 *     user/libcgj/src/graph_batch43197.c \
 *     user/libcgj/src/graph_batch43198.c \
 *     user/libcgj/src/graph_batch43199.c \
 *     user/libcgj/src/graph_batch43200.c
 *   ./build/cgj_soft_milestone_43200_direct
 *
 * Expected soft KATs (wave 43191–43200 exclusive continuum markers):
 *   gj_shell_green_43200      → 1
 *   gj_libcgj_green_43200     → 1
 *   gj_bar3_ready_43200       → 0   (bar3 still open)
 *   gj_product_score_43200    → 0   (product score still open)
 *   gj_continuum_ready_43200  → 1
 *   gj_smoke_soft_43200       → 1
 *   gj_dyn_soft_43200         → 1
 *   gj_milestone_tag_43200    → 43200
 *   gj_continuum_wave_43200   → 43200
 *   gj_batch_id_43200         → 43200
 *   gj_graph_milestone_43200  → 43200
 *   __libcgj_batch43191_marker … __libcgj_batch43200_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch43191.c … graph_batch43200.c (and fill
 *     43176–43190 / identity 43151–43175) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 43200.
 *   - libc.map may lag milestone 43200 globals; add 43200 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_43200 == 0).
 */
...
#ifndef CGJ_SOFT_M43200_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M43200_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_43200(void);
extern uint32_t gj_libcgj_green_43200(void);
extern uint32_t gj_bar3_ready_43200(void);
extern uint32_t gj_product_score_43200(void);
extern uint32_t gj_continuum_ready_43200(void);
extern uint32_t gj_smoke_soft_43200(void);
extern uint32_t gj_dyn_soft_43200(void);
extern uint32_t gj_milestone_tag_43200(void);
extern uint32_t gj_continuum_wave_43200(void);
extern uint32_t gj_batch_id_43200(void);
extern uint32_t gj_graph_milestone_43200(void);
extern const char __libcgj_batch43191_marker[];
extern const char __libcgj_batch43192_marker[];
extern const char __libcgj_batch43193_marker[];
extern const char __libcgj_batch43194_marker[];
extern const char __libcgj_batch43195_marker[];
extern const char __libcgj_batch43196_marker[];
extern const char __libcgj_batch43197_marker[];
extern const char __libcgj_batch43198_marker[];
extern const char __libcgj_batch43199_marker[];
extern const char __libcgj_batch43200_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_43200() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_43200 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_43200() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_43200 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_43200() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_43200 fail\n");
		nFail++;
	}
	if (gj_product_score_43200() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_43200 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_43200() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_43200 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_43200() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_43200 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_43200() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_43200 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_43200() != 43200u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_43200 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_43200() != 43200u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_43200 fail\n");
		nFail++;
	}
	if (gj_batch_id_43200() != 43200u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_43200 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_43200() != 43200u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_43200 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch43191_marker, "libcgj-batch43191") != 0 ||
	    strcmp(__libcgj_batch43192_marker, "libcgj-batch43192") != 0 ||
	    strcmp(__libcgj_batch43193_marker, "libcgj-batch43193") != 0 ||
	    strcmp(__libcgj_batch43194_marker, "libcgj-batch43194") != 0 ||
	    strcmp(__libcgj_batch43195_marker, "libcgj-batch43195") != 0 ||
	    strcmp(__libcgj_batch43196_marker, "libcgj-batch43196") != 0 ||
	    strcmp(__libcgj_batch43197_marker, "libcgj-batch43197") != 0 ||
	    strcmp(__libcgj_batch43198_marker, "libcgj-batch43198") != 0 ||
	    strcmp(__libcgj_batch43199_marker, "libcgj-batch43199") != 0 ||
	    strcmp(__libcgj_batch43200_marker, "libcgj-batch43200") != 0) {
		fprintf(stderr, "libcgj: direct m43200 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M43200_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m43200 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m43200 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m43200 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m43200 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m43200 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m43200 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_43200", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_43200", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_43200", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_43200", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_43200", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_43200", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_43200", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_43200", 43200u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_43200", 43200u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_43200", 43200u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_43200", 43200u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch43191_marker",
	                     "libcgj-batch43191", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch43192_marker",
	                     "libcgj-batch43192", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch43193_marker",
	                     "libcgj-batch43193", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch43194_marker",
	                     "libcgj-batch43194", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch43195_marker",
	                     "libcgj-batch43195", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch43196_marker",
	                     "libcgj-batch43196", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch43197_marker",
	                     "libcgj-batch43197", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch43198_marker",
	                     "libcgj-batch43198", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch43199_marker",
	                     "libcgj-batch43199", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch43200_marker",
	                     "libcgj-batch43200", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m43200 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m43200 PASS path=%s checked=%d "
