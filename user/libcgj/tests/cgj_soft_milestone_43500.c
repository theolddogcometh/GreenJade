 ...
 *
 * Soft host gates for continuum MILESTONE 43500 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_43500.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_43500.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_43500 \
 *     user/libcgj/tests/cgj_soft_milestone_43500.c -ldl
 *   ./build/cgj_soft_milestone_43500
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M43500_DIRECT \
 *     -o build/cgj_soft_milestone_43500_direct \
 *     user/libcgj/tests/cgj_soft_milestone_43500.c \
 *     user/libcgj/src/graph_batch43491.c \
 *     user/libcgj/src/graph_batch43492.c \
 *     user/libcgj/src/graph_batch43493.c \
 *     user/libcgj/src/graph_batch43494.c \
 *     user/libcgj/src/graph_batch43495.c \
 *     user/libcgj/src/graph_batch43496.c \
 *     user/libcgj/src/graph_batch43497.c \
 *     user/libcgj/src/graph_batch43498.c \
 *     user/libcgj/src/graph_batch43499.c \
 *     user/libcgj/src/graph_batch43500.c
 *   ./build/cgj_soft_milestone_43500_direct
 *
 * Expected soft KATs (wave 43491–43500 exclusive continuum markers):
 *   gj_shell_green_43500      → 1
 *   gj_libcgj_green_43500     → 1
 *   gj_bar3_ready_43500       → 0   (bar3 still open)
 *   gj_product_score_43500    → 0   (product score still open)
 *   gj_continuum_ready_43500  → 1
 *   gj_smoke_soft_43500       → 1
 *   gj_dyn_soft_43500         → 1
 *   gj_milestone_tag_43500    → 43500
 *   gj_continuum_wave_43500   → 43500
 *   gj_batch_id_43500         → 43500
 *   gj_graph_milestone_43500  → 43500
 *   __libcgj_batch43491_marker … __libcgj_batch43500_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch43491.c … graph_batch43500.c (and fill
 *     43476–43490 / identity 43451–43475) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 43500.
 *   - libc.map may lag milestone 43500 globals; add 43500 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_43500 == 0).
 */
...
#ifndef CGJ_SOFT_M43500_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M43500_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_43500(void);
extern uint32_t gj_libcgj_green_43500(void);
extern uint32_t gj_bar3_ready_43500(void);
extern uint32_t gj_product_score_43500(void);
extern uint32_t gj_continuum_ready_43500(void);
extern uint32_t gj_smoke_soft_43500(void);
extern uint32_t gj_dyn_soft_43500(void);
extern uint32_t gj_milestone_tag_43500(void);
extern uint32_t gj_continuum_wave_43500(void);
extern uint32_t gj_batch_id_43500(void);
extern uint32_t gj_graph_milestone_43500(void);
extern const char __libcgj_batch43491_marker[];
extern const char __libcgj_batch43492_marker[];
extern const char __libcgj_batch43493_marker[];
extern const char __libcgj_batch43494_marker[];
extern const char __libcgj_batch43495_marker[];
extern const char __libcgj_batch43496_marker[];
extern const char __libcgj_batch43497_marker[];
extern const char __libcgj_batch43498_marker[];
extern const char __libcgj_batch43499_marker[];
extern const char __libcgj_batch43500_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_43500() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_43500 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_43500() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_43500 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_43500() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_43500 fail\n");
		nFail++;
	}
	if (gj_product_score_43500() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_43500 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_43500() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_43500 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_43500() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_43500 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_43500() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_43500 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_43500() != 43500u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_43500 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_43500() != 43500u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_43500 fail\n");
		nFail++;
	}
	if (gj_batch_id_43500() != 43500u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_43500 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_43500() != 43500u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_43500 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch43491_marker, "libcgj-batch43491") != 0 ||
	    strcmp(__libcgj_batch43492_marker, "libcgj-batch43492") != 0 ||
	    strcmp(__libcgj_batch43493_marker, "libcgj-batch43493") != 0 ||
	    strcmp(__libcgj_batch43494_marker, "libcgj-batch43494") != 0 ||
	    strcmp(__libcgj_batch43495_marker, "libcgj-batch43495") != 0 ||
	    strcmp(__libcgj_batch43496_marker, "libcgj-batch43496") != 0 ||
	    strcmp(__libcgj_batch43497_marker, "libcgj-batch43497") != 0 ||
	    strcmp(__libcgj_batch43498_marker, "libcgj-batch43498") != 0 ||
	    strcmp(__libcgj_batch43499_marker, "libcgj-batch43499") != 0 ||
	    strcmp(__libcgj_batch43500_marker, "libcgj-batch43500") != 0) {
		fprintf(stderr, "libcgj: direct m43500 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M43500_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m43500 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m43500 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m43500 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m43500 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m43500 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m43500 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_43500", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_43500", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_43500", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_43500", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_43500", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_43500", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_43500", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_43500", 43500u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_43500", 43500u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_43500", 43500u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_43500", 43500u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch43491_marker",
	                     "libcgj-batch43491", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch43492_marker",
	                     "libcgj-batch43492", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch43493_marker",
	                     "libcgj-batch43493", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch43494_marker",
	                     "libcgj-batch43494", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch43495_marker",
	                     "libcgj-batch43495", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch43496_marker",
	                     "libcgj-batch43496", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch43497_marker",
	                     "libcgj-batch43497", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch43498_marker",
	                     "libcgj-batch43498", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch43499_marker",
	                     "libcgj-batch43499", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch43500_marker",
	                     "libcgj-batch43500", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m43500 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m43500 PASS path=%s checked=%d "
