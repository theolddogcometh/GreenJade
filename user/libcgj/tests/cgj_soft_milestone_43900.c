 ...
 *
 * Soft host gates for continuum MILESTONE 43900 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_43900.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_43900.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_43900 \
 *     user/libcgj/tests/cgj_soft_milestone_43900.c -ldl
 *   ./build/cgj_soft_milestone_43900
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M43900_DIRECT \
 *     -o build/cgj_soft_milestone_43900_direct \
 *     user/libcgj/tests/cgj_soft_milestone_43900.c \
 *     user/libcgj/src/graph_batch43891.c \
 *     user/libcgj/src/graph_batch43892.c \
 *     user/libcgj/src/graph_batch43893.c \
 *     user/libcgj/src/graph_batch43894.c \
 *     user/libcgj/src/graph_batch43895.c \
 *     user/libcgj/src/graph_batch43896.c \
 *     user/libcgj/src/graph_batch43897.c \
 *     user/libcgj/src/graph_batch43898.c \
 *     user/libcgj/src/graph_batch43899.c \
 *     user/libcgj/src/graph_batch43900.c
 *   ./build/cgj_soft_milestone_43900_direct
 *
 * Expected soft KATs (wave 43891–43900 exclusive continuum markers):
 *   gj_shell_green_43900      → 1
 *   gj_libcgj_green_43900     → 1
 *   gj_bar3_ready_43900       → 0   (bar3 still open)
 *   gj_product_score_43900    → 0   (product score still open)
 *   gj_continuum_ready_43900  → 1
 *   gj_smoke_soft_43900       → 1
 *   gj_dyn_soft_43900         → 1
 *   gj_milestone_tag_43900    → 43900
 *   gj_continuum_wave_43900   → 43900
 *   gj_batch_id_43900         → 43900
 *   gj_graph_milestone_43900  → 43900
 *   __libcgj_batch43891_marker … __libcgj_batch43900_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch43891.c … graph_batch43900.c (and fill
 *     43876–43890 / identity 43851–43875) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 43900.
 *   - libc.map may lag milestone 43900 globals; add 43900 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_43900 == 0).
 */
...
#ifndef CGJ_SOFT_M43900_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M43900_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_43900(void);
extern uint32_t gj_libcgj_green_43900(void);
extern uint32_t gj_bar3_ready_43900(void);
extern uint32_t gj_product_score_43900(void);
extern uint32_t gj_continuum_ready_43900(void);
extern uint32_t gj_smoke_soft_43900(void);
extern uint32_t gj_dyn_soft_43900(void);
extern uint32_t gj_milestone_tag_43900(void);
extern uint32_t gj_continuum_wave_43900(void);
extern uint32_t gj_batch_id_43900(void);
extern uint32_t gj_graph_milestone_43900(void);
extern const char __libcgj_batch43891_marker[];
extern const char __libcgj_batch43892_marker[];
extern const char __libcgj_batch43893_marker[];
extern const char __libcgj_batch43894_marker[];
extern const char __libcgj_batch43895_marker[];
extern const char __libcgj_batch43896_marker[];
extern const char __libcgj_batch43897_marker[];
extern const char __libcgj_batch43898_marker[];
extern const char __libcgj_batch43899_marker[];
extern const char __libcgj_batch43900_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_43900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_43900 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_43900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_43900 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_43900() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_43900 fail\n");
		nFail++;
	}
	if (gj_product_score_43900() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_43900 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_43900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_43900 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_43900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_43900 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_43900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_43900 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_43900() != 43900u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_43900 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_43900() != 43900u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_43900 fail\n");
		nFail++;
	}
	if (gj_batch_id_43900() != 43900u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_43900 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_43900() != 43900u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_43900 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch43891_marker, "libcgj-batch43891") != 0 ||
	    strcmp(__libcgj_batch43892_marker, "libcgj-batch43892") != 0 ||
	    strcmp(__libcgj_batch43893_marker, "libcgj-batch43893") != 0 ||
	    strcmp(__libcgj_batch43894_marker, "libcgj-batch43894") != 0 ||
	    strcmp(__libcgj_batch43895_marker, "libcgj-batch43895") != 0 ||
	    strcmp(__libcgj_batch43896_marker, "libcgj-batch43896") != 0 ||
	    strcmp(__libcgj_batch43897_marker, "libcgj-batch43897") != 0 ||
	    strcmp(__libcgj_batch43898_marker, "libcgj-batch43898") != 0 ||
	    strcmp(__libcgj_batch43899_marker, "libcgj-batch43899") != 0 ||
	    strcmp(__libcgj_batch43900_marker, "libcgj-batch43900") != 0) {
		fprintf(stderr, "libcgj: direct m43900 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M43900_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m43900 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m43900 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m43900 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m43900 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m43900 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m43900 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_43900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_43900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_43900", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_43900", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_43900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_43900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_43900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_43900", 43900u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_43900", 43900u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_43900", 43900u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_43900", 43900u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch43891_marker",
	                     "libcgj-batch43891", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch43892_marker",
	                     "libcgj-batch43892", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch43893_marker",
	                     "libcgj-batch43893", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch43894_marker",
	                     "libcgj-batch43894", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch43895_marker",
	                     "libcgj-batch43895", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch43896_marker",
	                     "libcgj-batch43896", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch43897_marker",
	                     "libcgj-batch43897", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch43898_marker",
	                     "libcgj-batch43898", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch43899_marker",
	                     "libcgj-batch43899", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch43900_marker",
	                     "libcgj-batch43900", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m43900 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m43900 PASS path=%s checked=%d "
