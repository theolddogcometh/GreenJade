 ...
 *
 * Soft host gates for continuum MILESTONE 44300 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_44300.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_44300.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_44300 \
 *     user/libcgj/tests/cgj_soft_milestone_44300.c -ldl
 *   ./build/cgj_soft_milestone_44300
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M44300_DIRECT \
 *     -o build/cgj_soft_milestone_44300_direct \
 *     user/libcgj/tests/cgj_soft_milestone_44300.c \
 *     user/libcgj/src/graph_batch44291.c \
 *     user/libcgj/src/graph_batch44292.c \
 *     user/libcgj/src/graph_batch44293.c \
 *     user/libcgj/src/graph_batch44294.c \
 *     user/libcgj/src/graph_batch44295.c \
 *     user/libcgj/src/graph_batch44296.c \
 *     user/libcgj/src/graph_batch44297.c \
 *     user/libcgj/src/graph_batch44298.c \
 *     user/libcgj/src/graph_batch44299.c \
 *     user/libcgj/src/graph_batch44300.c
 *   ./build/cgj_soft_milestone_44300_direct
 *
 * Expected soft KATs (wave 44291–44300 exclusive continuum markers):
 *   gj_shell_green_44300      → 1
 *   gj_libcgj_green_44300     → 1
 *   gj_bar3_ready_44300       → 0   (bar3 still open)
 *   gj_product_score_44300    → 0   (product score still open)
 *   gj_continuum_ready_44300  → 1
 *   gj_smoke_soft_44300       → 1
 *   gj_dyn_soft_44300         → 1
 *   gj_milestone_tag_44300    → 44300
 *   gj_continuum_wave_44300   → 44300
 *   gj_batch_id_44300         → 44300
 *   gj_graph_milestone_44300  → 44300
 *   __libcgj_batch44291_marker … __libcgj_batch44300_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch44291.c … graph_batch44300.c (and fill
 *     44276–44290 / identity 44251–44275) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 44300.
 *   - libc.map may lag milestone 44300 globals; add 44300 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_44300 == 0).
 */
...
#ifndef CGJ_SOFT_M44300_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M44300_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_44300(void);
extern uint32_t gj_libcgj_green_44300(void);
extern uint32_t gj_bar3_ready_44300(void);
extern uint32_t gj_product_score_44300(void);
extern uint32_t gj_continuum_ready_44300(void);
extern uint32_t gj_smoke_soft_44300(void);
extern uint32_t gj_dyn_soft_44300(void);
extern uint32_t gj_milestone_tag_44300(void);
extern uint32_t gj_continuum_wave_44300(void);
extern uint32_t gj_batch_id_44300(void);
extern uint32_t gj_graph_milestone_44300(void);
extern const char __libcgj_batch44291_marker[];
extern const char __libcgj_batch44292_marker[];
extern const char __libcgj_batch44293_marker[];
extern const char __libcgj_batch44294_marker[];
extern const char __libcgj_batch44295_marker[];
extern const char __libcgj_batch44296_marker[];
extern const char __libcgj_batch44297_marker[];
extern const char __libcgj_batch44298_marker[];
extern const char __libcgj_batch44299_marker[];
extern const char __libcgj_batch44300_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_44300() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_44300 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_44300() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_44300 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_44300() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_44300 fail\n");
		nFail++;
	}
	if (gj_product_score_44300() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_44300 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_44300() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_44300 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_44300() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_44300 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_44300() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_44300 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_44300() != 44300u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_44300 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_44300() != 44300u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_44300 fail\n");
		nFail++;
	}
	if (gj_batch_id_44300() != 44300u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_44300 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_44300() != 44300u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_44300 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch44291_marker, "libcgj-batch44291") != 0 ||
	    strcmp(__libcgj_batch44292_marker, "libcgj-batch44292") != 0 ||
	    strcmp(__libcgj_batch44293_marker, "libcgj-batch44293") != 0 ||
	    strcmp(__libcgj_batch44294_marker, "libcgj-batch44294") != 0 ||
	    strcmp(__libcgj_batch44295_marker, "libcgj-batch44295") != 0 ||
	    strcmp(__libcgj_batch44296_marker, "libcgj-batch44296") != 0 ||
	    strcmp(__libcgj_batch44297_marker, "libcgj-batch44297") != 0 ||
	    strcmp(__libcgj_batch44298_marker, "libcgj-batch44298") != 0 ||
	    strcmp(__libcgj_batch44299_marker, "libcgj-batch44299") != 0 ||
	    strcmp(__libcgj_batch44300_marker, "libcgj-batch44300") != 0) {
		fprintf(stderr, "libcgj: direct m44300 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M44300_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m44300 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m44300 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m44300 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m44300 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m44300 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m44300 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_44300", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_44300", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_44300", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_44300", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_44300", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_44300", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_44300", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_44300", 44300u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_44300", 44300u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_44300", 44300u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_44300", 44300u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch44291_marker",
	                     "libcgj-batch44291", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch44292_marker",
	                     "libcgj-batch44292", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch44293_marker",
	                     "libcgj-batch44293", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch44294_marker",
	                     "libcgj-batch44294", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch44295_marker",
	                     "libcgj-batch44295", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch44296_marker",
	                     "libcgj-batch44296", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch44297_marker",
	                     "libcgj-batch44297", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch44298_marker",
	                     "libcgj-batch44298", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch44299_marker",
	                     "libcgj-batch44299", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch44300_marker",
	                     "libcgj-batch44300", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m44300 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m44300 PASS path=%s checked=%d "
