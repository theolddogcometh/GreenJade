 ...
 *
 * Soft host gates for continuum MILESTONE 44700 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_44700.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_44700.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_44700 \
 *     user/libcgj/tests/cgj_soft_milestone_44700.c -ldl
 *   ./build/cgj_soft_milestone_44700
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M44700_DIRECT \
 *     -o build/cgj_soft_milestone_44700_direct \
 *     user/libcgj/tests/cgj_soft_milestone_44700.c \
 *     user/libcgj/src/graph_batch44691.c \
 *     user/libcgj/src/graph_batch44692.c \
 *     user/libcgj/src/graph_batch44693.c \
 *     user/libcgj/src/graph_batch44694.c \
 *     user/libcgj/src/graph_batch44695.c \
 *     user/libcgj/src/graph_batch44696.c \
 *     user/libcgj/src/graph_batch44697.c \
 *     user/libcgj/src/graph_batch44698.c \
 *     user/libcgj/src/graph_batch44699.c \
 *     user/libcgj/src/graph_batch44700.c
 *   ./build/cgj_soft_milestone_44700_direct
 *
 * Expected soft KATs (wave 44691–44700 exclusive continuum markers):
 *   gj_shell_green_44700      → 1
 *   gj_libcgj_green_44700     → 1
 *   gj_bar3_ready_44700       → 0   (bar3 still open)
 *   gj_product_score_44700    → 0   (product score still open)
 *   gj_continuum_ready_44700  → 1
 *   gj_smoke_soft_44700       → 1
 *   gj_dyn_soft_44700         → 1
 *   gj_milestone_tag_44700    → 44700
 *   gj_continuum_wave_44700   → 44700
 *   gj_batch_id_44700         → 44700
 *   gj_graph_milestone_44700  → 44700
 *   __libcgj_batch44691_marker … __libcgj_batch44700_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch44691.c … graph_batch44700.c (and fill
 *     44676–44690 / identity 44651–44675) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 44700.
 *   - libc.map may lag milestone 44700 globals; add 44700 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_44700 == 0).
 */
...
#ifndef CGJ_SOFT_M44700_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M44700_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_44700(void);
extern uint32_t gj_libcgj_green_44700(void);
extern uint32_t gj_bar3_ready_44700(void);
extern uint32_t gj_product_score_44700(void);
extern uint32_t gj_continuum_ready_44700(void);
extern uint32_t gj_smoke_soft_44700(void);
extern uint32_t gj_dyn_soft_44700(void);
extern uint32_t gj_milestone_tag_44700(void);
extern uint32_t gj_continuum_wave_44700(void);
extern uint32_t gj_batch_id_44700(void);
extern uint32_t gj_graph_milestone_44700(void);
extern const char __libcgj_batch44691_marker[];
extern const char __libcgj_batch44692_marker[];
extern const char __libcgj_batch44693_marker[];
extern const char __libcgj_batch44694_marker[];
extern const char __libcgj_batch44695_marker[];
extern const char __libcgj_batch44696_marker[];
extern const char __libcgj_batch44697_marker[];
extern const char __libcgj_batch44698_marker[];
extern const char __libcgj_batch44699_marker[];
extern const char __libcgj_batch44700_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_44700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_44700 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_44700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_44700 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_44700() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_44700 fail\n");
		nFail++;
	}
	if (gj_product_score_44700() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_44700 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_44700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_44700 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_44700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_44700 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_44700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_44700 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_44700() != 44700u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_44700 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_44700() != 44700u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_44700 fail\n");
		nFail++;
	}
	if (gj_batch_id_44700() != 44700u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_44700 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_44700() != 44700u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_44700 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch44691_marker, "libcgj-batch44691") != 0 ||
	    strcmp(__libcgj_batch44692_marker, "libcgj-batch44692") != 0 ||
	    strcmp(__libcgj_batch44693_marker, "libcgj-batch44693") != 0 ||
	    strcmp(__libcgj_batch44694_marker, "libcgj-batch44694") != 0 ||
	    strcmp(__libcgj_batch44695_marker, "libcgj-batch44695") != 0 ||
	    strcmp(__libcgj_batch44696_marker, "libcgj-batch44696") != 0 ||
	    strcmp(__libcgj_batch44697_marker, "libcgj-batch44697") != 0 ||
	    strcmp(__libcgj_batch44698_marker, "libcgj-batch44698") != 0 ||
	    strcmp(__libcgj_batch44699_marker, "libcgj-batch44699") != 0 ||
	    strcmp(__libcgj_batch44700_marker, "libcgj-batch44700") != 0) {
		fprintf(stderr, "libcgj: direct m44700 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M44700_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m44700 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m44700 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m44700 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m44700 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m44700 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m44700 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_44700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_44700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_44700", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_44700", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_44700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_44700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_44700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_44700", 44700u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_44700", 44700u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_44700", 44700u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_44700", 44700u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch44691_marker",
	                     "libcgj-batch44691", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch44692_marker",
	                     "libcgj-batch44692", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch44693_marker",
	                     "libcgj-batch44693", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch44694_marker",
	                     "libcgj-batch44694", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch44695_marker",
	                     "libcgj-batch44695", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch44696_marker",
	                     "libcgj-batch44696", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch44697_marker",
	                     "libcgj-batch44697", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch44698_marker",
	                     "libcgj-batch44698", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch44699_marker",
	                     "libcgj-batch44699", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch44700_marker",
	                     "libcgj-batch44700", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m44700 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m44700 PASS path=%s checked=%d "
