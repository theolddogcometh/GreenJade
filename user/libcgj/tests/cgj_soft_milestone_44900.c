 ...
 *
 * Soft host gates for continuum MILESTONE 44900 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_44900.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_44900.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_44900 \
 *     user/libcgj/tests/cgj_soft_milestone_44900.c -ldl
 *   ./build/cgj_soft_milestone_44900
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M44900_DIRECT \
 *     -o build/cgj_soft_milestone_44900_direct \
 *     user/libcgj/tests/cgj_soft_milestone_44900.c \
 *     user/libcgj/src/graph_batch44891.c \
 *     user/libcgj/src/graph_batch44892.c \
 *     user/libcgj/src/graph_batch44893.c \
 *     user/libcgj/src/graph_batch44894.c \
 *     user/libcgj/src/graph_batch44895.c \
 *     user/libcgj/src/graph_batch44896.c \
 *     user/libcgj/src/graph_batch44897.c \
 *     user/libcgj/src/graph_batch44898.c \
 *     user/libcgj/src/graph_batch44899.c \
 *     user/libcgj/src/graph_batch44900.c
 *   ./build/cgj_soft_milestone_44900_direct
 *
 * Expected soft KATs (wave 44891–44900 exclusive continuum markers):
 *   gj_shell_green_44900      → 1
 *   gj_libcgj_green_44900     → 1
 *   gj_bar3_ready_44900       → 0   (bar3 still open)
 *   gj_product_score_44900    → 0   (product score still open)
 *   gj_continuum_ready_44900  → 1
 *   gj_smoke_soft_44900       → 1
 *   gj_dyn_soft_44900         → 1
 *   gj_milestone_tag_44900    → 44900
 *   gj_continuum_wave_44900   → 44900
 *   gj_batch_id_44900         → 44900
 *   gj_graph_milestone_44900  → 44900
 *   __libcgj_batch44891_marker … __libcgj_batch44900_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch44891.c … graph_batch44900.c (and fill
 *     44876–44890 / identity 44851–44875) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 44900.
 *   - libc.map may lag milestone 44900 globals; add 44900 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_44900 == 0).
 */
...
#ifndef CGJ_SOFT_M44900_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M44900_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_44900(void);
extern uint32_t gj_libcgj_green_44900(void);
extern uint32_t gj_bar3_ready_44900(void);
extern uint32_t gj_product_score_44900(void);
extern uint32_t gj_continuum_ready_44900(void);
extern uint32_t gj_smoke_soft_44900(void);
extern uint32_t gj_dyn_soft_44900(void);
extern uint32_t gj_milestone_tag_44900(void);
extern uint32_t gj_continuum_wave_44900(void);
extern uint32_t gj_batch_id_44900(void);
extern uint32_t gj_graph_milestone_44900(void);
extern const char __libcgj_batch44891_marker[];
extern const char __libcgj_batch44892_marker[];
extern const char __libcgj_batch44893_marker[];
extern const char __libcgj_batch44894_marker[];
extern const char __libcgj_batch44895_marker[];
extern const char __libcgj_batch44896_marker[];
extern const char __libcgj_batch44897_marker[];
extern const char __libcgj_batch44898_marker[];
extern const char __libcgj_batch44899_marker[];
extern const char __libcgj_batch44900_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_44900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_44900 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_44900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_44900 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_44900() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_44900 fail\n");
		nFail++;
	}
	if (gj_product_score_44900() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_44900 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_44900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_44900 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_44900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_44900 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_44900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_44900 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_44900() != 44900u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_44900 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_44900() != 44900u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_44900 fail\n");
		nFail++;
	}
	if (gj_batch_id_44900() != 44900u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_44900 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_44900() != 44900u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_44900 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch44891_marker, "libcgj-batch44891") != 0 ||
	    strcmp(__libcgj_batch44892_marker, "libcgj-batch44892") != 0 ||
	    strcmp(__libcgj_batch44893_marker, "libcgj-batch44893") != 0 ||
	    strcmp(__libcgj_batch44894_marker, "libcgj-batch44894") != 0 ||
	    strcmp(__libcgj_batch44895_marker, "libcgj-batch44895") != 0 ||
	    strcmp(__libcgj_batch44896_marker, "libcgj-batch44896") != 0 ||
	    strcmp(__libcgj_batch44897_marker, "libcgj-batch44897") != 0 ||
	    strcmp(__libcgj_batch44898_marker, "libcgj-batch44898") != 0 ||
	    strcmp(__libcgj_batch44899_marker, "libcgj-batch44899") != 0 ||
	    strcmp(__libcgj_batch44900_marker, "libcgj-batch44900") != 0) {
		fprintf(stderr, "libcgj: direct m44900 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M44900_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m44900 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m44900 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m44900 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m44900 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m44900 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m44900 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_44900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_44900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_44900", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_44900", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_44900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_44900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_44900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_44900", 44900u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_44900", 44900u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_44900", 44900u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_44900", 44900u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch44891_marker",
	                     "libcgj-batch44891", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch44892_marker",
	                     "libcgj-batch44892", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch44893_marker",
	                     "libcgj-batch44893", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch44894_marker",
	                     "libcgj-batch44894", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch44895_marker",
	                     "libcgj-batch44895", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch44896_marker",
	                     "libcgj-batch44896", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch44897_marker",
	                     "libcgj-batch44897", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch44898_marker",
	                     "libcgj-batch44898", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch44899_marker",
	                     "libcgj-batch44899", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch44900_marker",
	                     "libcgj-batch44900", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m44900 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m44900 PASS path=%s checked=%d "
