 ...
 *
 * Soft host gates for continuum MILESTONE 45800 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_45800.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_45800.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_45800 \
 *     user/libcgj/tests/cgj_soft_milestone_45800.c -ldl
 *   ./build/cgj_soft_milestone_45800
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M45800_DIRECT \
 *     -o build/cgj_soft_milestone_45800_direct \
 *     user/libcgj/tests/cgj_soft_milestone_45800.c \
 *     user/libcgj/src/graph_batch45791.c \
 *     user/libcgj/src/graph_batch45792.c \
 *     user/libcgj/src/graph_batch45793.c \
 *     user/libcgj/src/graph_batch45794.c \
 *     user/libcgj/src/graph_batch45795.c \
 *     user/libcgj/src/graph_batch45796.c \
 *     user/libcgj/src/graph_batch45797.c \
 *     user/libcgj/src/graph_batch45798.c \
 *     user/libcgj/src/graph_batch45799.c \
 *     user/libcgj/src/graph_batch45800.c
 *   ./build/cgj_soft_milestone_45800_direct
 *
 * Expected soft KATs (wave 45791–45800 exclusive continuum markers):
 *   gj_shell_green_45800      → 1
 *   gj_libcgj_green_45800     → 1
 *   gj_bar3_ready_45800       → 0   (bar3 still open)
 *   gj_product_score_45800    → 0   (product score still open)
 *   gj_continuum_ready_45800  → 1
 *   gj_smoke_soft_45800       → 1
 *   gj_dyn_soft_45800         → 1
 *   gj_milestone_tag_45800    → 45800
 *   gj_continuum_wave_45800   → 45800
 *   gj_batch_id_45800         → 45800
 *   gj_graph_milestone_45800  → 45800
 *   __libcgj_batch45791_marker … __libcgj_batch45800_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch45791.c … graph_batch45800.c (and fill
 *     45776–45790 / identity 45751–45775) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 45800.
 *   - libc.map may lag milestone 45800 globals; add 45800 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_45800 == 0).
 */
...
#ifndef CGJ_SOFT_M45800_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M45800_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_45800(void);
extern uint32_t gj_libcgj_green_45800(void);
extern uint32_t gj_bar3_ready_45800(void);
extern uint32_t gj_product_score_45800(void);
extern uint32_t gj_continuum_ready_45800(void);
extern uint32_t gj_smoke_soft_45800(void);
extern uint32_t gj_dyn_soft_45800(void);
extern uint32_t gj_milestone_tag_45800(void);
extern uint32_t gj_continuum_wave_45800(void);
extern uint32_t gj_batch_id_45800(void);
extern uint32_t gj_graph_milestone_45800(void);
extern const char __libcgj_batch45791_marker[];
extern const char __libcgj_batch45792_marker[];
extern const char __libcgj_batch45793_marker[];
extern const char __libcgj_batch45794_marker[];
extern const char __libcgj_batch45795_marker[];
extern const char __libcgj_batch45796_marker[];
extern const char __libcgj_batch45797_marker[];
extern const char __libcgj_batch45798_marker[];
extern const char __libcgj_batch45799_marker[];
extern const char __libcgj_batch45800_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_45800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_45800 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_45800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_45800 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_45800() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_45800 fail\n");
		nFail++;
	}
	if (gj_product_score_45800() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_45800 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_45800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_45800 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_45800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_45800 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_45800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_45800 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_45800() != 45800u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_45800 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_45800() != 45800u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_45800 fail\n");
		nFail++;
	}
	if (gj_batch_id_45800() != 45800u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_45800 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_45800() != 45800u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_45800 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch45791_marker, "libcgj-batch45791") != 0 ||
	    strcmp(__libcgj_batch45792_marker, "libcgj-batch45792") != 0 ||
	    strcmp(__libcgj_batch45793_marker, "libcgj-batch45793") != 0 ||
	    strcmp(__libcgj_batch45794_marker, "libcgj-batch45794") != 0 ||
	    strcmp(__libcgj_batch45795_marker, "libcgj-batch45795") != 0 ||
	    strcmp(__libcgj_batch45796_marker, "libcgj-batch45796") != 0 ||
	    strcmp(__libcgj_batch45797_marker, "libcgj-batch45797") != 0 ||
	    strcmp(__libcgj_batch45798_marker, "libcgj-batch45798") != 0 ||
	    strcmp(__libcgj_batch45799_marker, "libcgj-batch45799") != 0 ||
	    strcmp(__libcgj_batch45800_marker, "libcgj-batch45800") != 0) {
		fprintf(stderr, "libcgj: direct m45800 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M45800_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m45800 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m45800 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m45800 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m45800 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m45800 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m45800 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_45800", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_45800", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_45800", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_45800", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_45800", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_45800", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_45800", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_45800", 45800u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_45800", 45800u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_45800", 45800u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_45800", 45800u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch45791_marker",
	                     "libcgj-batch45791", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch45792_marker",
	                     "libcgj-batch45792", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch45793_marker",
	                     "libcgj-batch45793", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch45794_marker",
	                     "libcgj-batch45794", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch45795_marker",
	                     "libcgj-batch45795", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch45796_marker",
	                     "libcgj-batch45796", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch45797_marker",
	                     "libcgj-batch45797", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch45798_marker",
	                     "libcgj-batch45798", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch45799_marker",
	                     "libcgj-batch45799", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch45800_marker",
	                     "libcgj-batch45800", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m45800 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m45800 PASS path=%s checked=%d "
