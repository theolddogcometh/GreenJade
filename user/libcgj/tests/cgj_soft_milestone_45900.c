 ...
 *
 * Soft host gates for continuum MILESTONE 45900 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_45900.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_45900.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_45900 \
 *     user/libcgj/tests/cgj_soft_milestone_45900.c -ldl
 *   ./build/cgj_soft_milestone_45900
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M45900_DIRECT \
 *     -o build/cgj_soft_milestone_45900_direct \
 *     user/libcgj/tests/cgj_soft_milestone_45900.c \
 *     user/libcgj/src/graph_batch45891.c \
 *     user/libcgj/src/graph_batch45892.c \
 *     user/libcgj/src/graph_batch45893.c \
 *     user/libcgj/src/graph_batch45894.c \
 *     user/libcgj/src/graph_batch45895.c \
 *     user/libcgj/src/graph_batch45896.c \
 *     user/libcgj/src/graph_batch45897.c \
 *     user/libcgj/src/graph_batch45898.c \
 *     user/libcgj/src/graph_batch45899.c \
 *     user/libcgj/src/graph_batch45900.c
 *   ./build/cgj_soft_milestone_45900_direct
 *
 * Expected soft KATs (wave 45891–45900 exclusive continuum markers):
 *   gj_shell_green_45900      → 1
 *   gj_libcgj_green_45900     → 1
 *   gj_bar3_ready_45900       → 0   (bar3 still open)
 *   gj_product_score_45900    → 0   (product score still open)
 *   gj_continuum_ready_45900  → 1
 *   gj_smoke_soft_45900       → 1
 *   gj_dyn_soft_45900         → 1
 *   gj_milestone_tag_45900    → 45900
 *   gj_continuum_wave_45900   → 45900
 *   gj_batch_id_45900         → 45900
 *   gj_graph_milestone_45900  → 45900
 *   __libcgj_batch45891_marker … __libcgj_batch45900_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch45891.c … graph_batch45900.c (and fill
 *     45876–45890 / identity 45851–45875) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 45900.
 *   - libc.map may lag milestone 45900 globals; add 45900 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_45900 == 0).
 */
...
#ifndef CGJ_SOFT_M45900_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M45900_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_45900(void);
extern uint32_t gj_libcgj_green_45900(void);
extern uint32_t gj_bar3_ready_45900(void);
extern uint32_t gj_product_score_45900(void);
extern uint32_t gj_continuum_ready_45900(void);
extern uint32_t gj_smoke_soft_45900(void);
extern uint32_t gj_dyn_soft_45900(void);
extern uint32_t gj_milestone_tag_45900(void);
extern uint32_t gj_continuum_wave_45900(void);
extern uint32_t gj_batch_id_45900(void);
extern uint32_t gj_graph_milestone_45900(void);
extern const char __libcgj_batch45891_marker[];
extern const char __libcgj_batch45892_marker[];
extern const char __libcgj_batch45893_marker[];
extern const char __libcgj_batch45894_marker[];
extern const char __libcgj_batch45895_marker[];
extern const char __libcgj_batch45896_marker[];
extern const char __libcgj_batch45897_marker[];
extern const char __libcgj_batch45898_marker[];
extern const char __libcgj_batch45899_marker[];
extern const char __libcgj_batch45900_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_45900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_45900 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_45900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_45900 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_45900() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_45900 fail\n");
		nFail++;
	}
	if (gj_product_score_45900() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_45900 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_45900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_45900 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_45900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_45900 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_45900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_45900 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_45900() != 45900u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_45900 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_45900() != 45900u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_45900 fail\n");
		nFail++;
	}
	if (gj_batch_id_45900() != 45900u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_45900 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_45900() != 45900u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_45900 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch45891_marker, "libcgj-batch45891") != 0 ||
	    strcmp(__libcgj_batch45892_marker, "libcgj-batch45892") != 0 ||
	    strcmp(__libcgj_batch45893_marker, "libcgj-batch45893") != 0 ||
	    strcmp(__libcgj_batch45894_marker, "libcgj-batch45894") != 0 ||
	    strcmp(__libcgj_batch45895_marker, "libcgj-batch45895") != 0 ||
	    strcmp(__libcgj_batch45896_marker, "libcgj-batch45896") != 0 ||
	    strcmp(__libcgj_batch45897_marker, "libcgj-batch45897") != 0 ||
	    strcmp(__libcgj_batch45898_marker, "libcgj-batch45898") != 0 ||
	    strcmp(__libcgj_batch45899_marker, "libcgj-batch45899") != 0 ||
	    strcmp(__libcgj_batch45900_marker, "libcgj-batch45900") != 0) {
		fprintf(stderr, "libcgj: direct m45900 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M45900_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m45900 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m45900 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m45900 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m45900 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m45900 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m45900 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_45900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_45900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_45900", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_45900", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_45900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_45900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_45900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_45900", 45900u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_45900", 45900u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_45900", 45900u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_45900", 45900u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch45891_marker",
	                     "libcgj-batch45891", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch45892_marker",
	                     "libcgj-batch45892", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch45893_marker",
	                     "libcgj-batch45893", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch45894_marker",
	                     "libcgj-batch45894", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch45895_marker",
	                     "libcgj-batch45895", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch45896_marker",
	                     "libcgj-batch45896", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch45897_marker",
	                     "libcgj-batch45897", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch45898_marker",
	                     "libcgj-batch45898", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch45899_marker",
	                     "libcgj-batch45899", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch45900_marker",
	                     "libcgj-batch45900", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m45900 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m45900 PASS path=%s checked=%d "
