 ...
 *
 * Soft host gates for continuum MILESTONE 46100 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_46100.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_46100.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_46100 \
 *     user/libcgj/tests/cgj_soft_milestone_46100.c -ldl
 *   ./build/cgj_soft_milestone_46100
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M46100_DIRECT \
 *     -o build/cgj_soft_milestone_46100_direct \
 *     user/libcgj/tests/cgj_soft_milestone_46100.c \
 *     user/libcgj/src/graph_batch46091.c \
 *     user/libcgj/src/graph_batch46092.c \
 *     user/libcgj/src/graph_batch46093.c \
 *     user/libcgj/src/graph_batch46094.c \
 *     user/libcgj/src/graph_batch46095.c \
 *     user/libcgj/src/graph_batch46096.c \
 *     user/libcgj/src/graph_batch46097.c \
 *     user/libcgj/src/graph_batch46098.c \
 *     user/libcgj/src/graph_batch46099.c \
 *     user/libcgj/src/graph_batch46100.c
 *   ./build/cgj_soft_milestone_46100_direct
 *
 * Expected soft KATs (wave 46091–46100 exclusive continuum markers):
 *   gj_shell_green_46100      → 1
 *   gj_libcgj_green_46100     → 1
 *   gj_bar3_ready_46100       → 0   (bar3 still open)
 *   gj_product_score_46100    → 0   (product score still open)
 *   gj_continuum_ready_46100  → 1
 *   gj_smoke_soft_46100       → 1
 *   gj_dyn_soft_46100         → 1
 *   gj_milestone_tag_46100    → 46100
 *   gj_continuum_wave_46100   → 46100
 *   gj_batch_id_46100         → 46100
 *   gj_graph_milestone_46100  → 46100
 *   __libcgj_batch46091_marker … __libcgj_batch46100_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch46091.c … graph_batch46100.c (and fill
 *     46076–46090 / identity 46051–46075) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 46100.
 *   - libc.map may lag milestone 46100 globals; add 46100 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_46100 == 0).
 */
...
#ifndef CGJ_SOFT_M46100_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M46100_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_46100(void);
extern uint32_t gj_libcgj_green_46100(void);
extern uint32_t gj_bar3_ready_46100(void);
extern uint32_t gj_product_score_46100(void);
extern uint32_t gj_continuum_ready_46100(void);
extern uint32_t gj_smoke_soft_46100(void);
extern uint32_t gj_dyn_soft_46100(void);
extern uint32_t gj_milestone_tag_46100(void);
extern uint32_t gj_continuum_wave_46100(void);
extern uint32_t gj_batch_id_46100(void);
extern uint32_t gj_graph_milestone_46100(void);
extern const char __libcgj_batch46091_marker[];
extern const char __libcgj_batch46092_marker[];
extern const char __libcgj_batch46093_marker[];
extern const char __libcgj_batch46094_marker[];
extern const char __libcgj_batch46095_marker[];
extern const char __libcgj_batch46096_marker[];
extern const char __libcgj_batch46097_marker[];
extern const char __libcgj_batch46098_marker[];
extern const char __libcgj_batch46099_marker[];
extern const char __libcgj_batch46100_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_46100() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_46100 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_46100() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_46100 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_46100() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_46100 fail\n");
		nFail++;
	}
	if (gj_product_score_46100() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_46100 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_46100() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_46100 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_46100() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_46100 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_46100() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_46100 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_46100() != 46100u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_46100 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_46100() != 46100u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_46100 fail\n");
		nFail++;
	}
	if (gj_batch_id_46100() != 46100u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_46100 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_46100() != 46100u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_46100 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch46091_marker, "libcgj-batch46091") != 0 ||
	    strcmp(__libcgj_batch46092_marker, "libcgj-batch46092") != 0 ||
	    strcmp(__libcgj_batch46093_marker, "libcgj-batch46093") != 0 ||
	    strcmp(__libcgj_batch46094_marker, "libcgj-batch46094") != 0 ||
	    strcmp(__libcgj_batch46095_marker, "libcgj-batch46095") != 0 ||
	    strcmp(__libcgj_batch46096_marker, "libcgj-batch46096") != 0 ||
	    strcmp(__libcgj_batch46097_marker, "libcgj-batch46097") != 0 ||
	    strcmp(__libcgj_batch46098_marker, "libcgj-batch46098") != 0 ||
	    strcmp(__libcgj_batch46099_marker, "libcgj-batch46099") != 0 ||
	    strcmp(__libcgj_batch46100_marker, "libcgj-batch46100") != 0) {
		fprintf(stderr, "libcgj: direct m46100 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M46100_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m46100 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m46100 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m46100 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m46100 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m46100 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m46100 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_46100", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_46100", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_46100", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_46100", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_46100", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_46100", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_46100", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_46100", 46100u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_46100", 46100u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_46100", 46100u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_46100", 46100u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch46091_marker",
	                     "libcgj-batch46091", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch46092_marker",
	                     "libcgj-batch46092", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch46093_marker",
	                     "libcgj-batch46093", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch46094_marker",
	                     "libcgj-batch46094", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch46095_marker",
	                     "libcgj-batch46095", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch46096_marker",
	                     "libcgj-batch46096", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch46097_marker",
	                     "libcgj-batch46097", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch46098_marker",
	                     "libcgj-batch46098", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch46099_marker",
	                     "libcgj-batch46099", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch46100_marker",
	                     "libcgj-batch46100", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m46100 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m46100 PASS path=%s checked=%d "
