 ...
 *
 * Soft host gates for continuum MILESTONE 46200 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_46200.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_46200.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_46200 \
 *     user/libcgj/tests/cgj_soft_milestone_46200.c -ldl
 *   ./build/cgj_soft_milestone_46200
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M46200_DIRECT \
 *     -o build/cgj_soft_milestone_46200_direct \
 *     user/libcgj/tests/cgj_soft_milestone_46200.c \
 *     user/libcgj/src/graph_batch46191.c \
 *     user/libcgj/src/graph_batch46192.c \
 *     user/libcgj/src/graph_batch46193.c \
 *     user/libcgj/src/graph_batch46194.c \
 *     user/libcgj/src/graph_batch46195.c \
 *     user/libcgj/src/graph_batch46196.c \
 *     user/libcgj/src/graph_batch46197.c \
 *     user/libcgj/src/graph_batch46198.c \
 *     user/libcgj/src/graph_batch46199.c \
 *     user/libcgj/src/graph_batch46200.c
 *   ./build/cgj_soft_milestone_46200_direct
 *
 * Expected soft KATs (wave 46191–46200 exclusive continuum markers):
 *   gj_shell_green_46200      → 1
 *   gj_libcgj_green_46200     → 1
 *   gj_bar3_ready_46200       → 0   (bar3 still open)
 *   gj_product_score_46200    → 0   (product score still open)
 *   gj_continuum_ready_46200  → 1
 *   gj_smoke_soft_46200       → 1
 *   gj_dyn_soft_46200         → 1
 *   gj_milestone_tag_46200    → 46200
 *   gj_continuum_wave_46200   → 46200
 *   gj_batch_id_46200         → 46200
 *   gj_graph_milestone_46200  → 46200
 *   __libcgj_batch46191_marker … __libcgj_batch46200_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch46191.c … graph_batch46200.c (and fill
 *     46176–46190 / identity 46151–46175) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 46200.
 *   - libc.map may lag milestone 46200 globals; add 46200 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_46200 == 0).
 */
...
#ifndef CGJ_SOFT_M46200_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M46200_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_46200(void);
extern uint32_t gj_libcgj_green_46200(void);
extern uint32_t gj_bar3_ready_46200(void);
extern uint32_t gj_product_score_46200(void);
extern uint32_t gj_continuum_ready_46200(void);
extern uint32_t gj_smoke_soft_46200(void);
extern uint32_t gj_dyn_soft_46200(void);
extern uint32_t gj_milestone_tag_46200(void);
extern uint32_t gj_continuum_wave_46200(void);
extern uint32_t gj_batch_id_46200(void);
extern uint32_t gj_graph_milestone_46200(void);
extern const char __libcgj_batch46191_marker[];
extern const char __libcgj_batch46192_marker[];
extern const char __libcgj_batch46193_marker[];
extern const char __libcgj_batch46194_marker[];
extern const char __libcgj_batch46195_marker[];
extern const char __libcgj_batch46196_marker[];
extern const char __libcgj_batch46197_marker[];
extern const char __libcgj_batch46198_marker[];
extern const char __libcgj_batch46199_marker[];
extern const char __libcgj_batch46200_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_46200() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_46200 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_46200() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_46200 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_46200() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_46200 fail\n");
		nFail++;
	}
	if (gj_product_score_46200() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_46200 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_46200() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_46200 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_46200() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_46200 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_46200() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_46200 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_46200() != 46200u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_46200 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_46200() != 46200u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_46200 fail\n");
		nFail++;
	}
	if (gj_batch_id_46200() != 46200u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_46200 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_46200() != 46200u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_46200 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch46191_marker, "libcgj-batch46191") != 0 ||
	    strcmp(__libcgj_batch46192_marker, "libcgj-batch46192") != 0 ||
	    strcmp(__libcgj_batch46193_marker, "libcgj-batch46193") != 0 ||
	    strcmp(__libcgj_batch46194_marker, "libcgj-batch46194") != 0 ||
	    strcmp(__libcgj_batch46195_marker, "libcgj-batch46195") != 0 ||
	    strcmp(__libcgj_batch46196_marker, "libcgj-batch46196") != 0 ||
	    strcmp(__libcgj_batch46197_marker, "libcgj-batch46197") != 0 ||
	    strcmp(__libcgj_batch46198_marker, "libcgj-batch46198") != 0 ||
	    strcmp(__libcgj_batch46199_marker, "libcgj-batch46199") != 0 ||
	    strcmp(__libcgj_batch46200_marker, "libcgj-batch46200") != 0) {
		fprintf(stderr, "libcgj: direct m46200 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M46200_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m46200 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m46200 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m46200 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m46200 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m46200 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m46200 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_46200", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_46200", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_46200", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_46200", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_46200", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_46200", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_46200", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_46200", 46200u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_46200", 46200u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_46200", 46200u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_46200", 46200u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch46191_marker",
	                     "libcgj-batch46191", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch46192_marker",
	                     "libcgj-batch46192", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch46193_marker",
	                     "libcgj-batch46193", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch46194_marker",
	                     "libcgj-batch46194", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch46195_marker",
	                     "libcgj-batch46195", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch46196_marker",
	                     "libcgj-batch46196", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch46197_marker",
	                     "libcgj-batch46197", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch46198_marker",
	                     "libcgj-batch46198", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch46199_marker",
	                     "libcgj-batch46199", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch46200_marker",
	                     "libcgj-batch46200", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m46200 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m46200 PASS path=%s checked=%d "
