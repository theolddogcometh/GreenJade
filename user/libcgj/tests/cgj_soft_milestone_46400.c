 ...
 *
 * Soft host gates for continuum MILESTONE 46400 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_46400.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_46400.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_46400 \
 *     user/libcgj/tests/cgj_soft_milestone_46400.c -ldl
 *   ./build/cgj_soft_milestone_46400
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M46400_DIRECT \
 *     -o build/cgj_soft_milestone_46400_direct \
 *     user/libcgj/tests/cgj_soft_milestone_46400.c \
 *     user/libcgj/src/graph_batch46391.c \
 *     user/libcgj/src/graph_batch46392.c \
 *     user/libcgj/src/graph_batch46393.c \
 *     user/libcgj/src/graph_batch46394.c \
 *     user/libcgj/src/graph_batch46395.c \
 *     user/libcgj/src/graph_batch46396.c \
 *     user/libcgj/src/graph_batch46397.c \
 *     user/libcgj/src/graph_batch46398.c \
 *     user/libcgj/src/graph_batch46399.c \
 *     user/libcgj/src/graph_batch46400.c
 *   ./build/cgj_soft_milestone_46400_direct
 *
 * Expected soft KATs (wave 46391–46400 exclusive continuum markers):
 *   gj_shell_green_46400      → 1
 *   gj_libcgj_green_46400     → 1
 *   gj_bar3_ready_46400       → 0   (bar3 still open)
 *   gj_product_score_46400    → 0   (product score still open)
 *   gj_continuum_ready_46400  → 1
 *   gj_smoke_soft_46400       → 1
 *   gj_dyn_soft_46400         → 1
 *   gj_milestone_tag_46400    → 46400
 *   gj_continuum_wave_46400   → 46400
 *   gj_batch_id_46400         → 46400
 *   gj_graph_milestone_46400  → 46400
 *   __libcgj_batch46391_marker … __libcgj_batch46400_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch46391.c … graph_batch46400.c (and fill
 *     46376–46390 / identity 46351–46375) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 46400.
 *   - libc.map may lag milestone 46400 globals; add 46400 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_46400 == 0).
 */
...
#ifndef CGJ_SOFT_M46400_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M46400_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_46400(void);
extern uint32_t gj_libcgj_green_46400(void);
extern uint32_t gj_bar3_ready_46400(void);
extern uint32_t gj_product_score_46400(void);
extern uint32_t gj_continuum_ready_46400(void);
extern uint32_t gj_smoke_soft_46400(void);
extern uint32_t gj_dyn_soft_46400(void);
extern uint32_t gj_milestone_tag_46400(void);
extern uint32_t gj_continuum_wave_46400(void);
extern uint32_t gj_batch_id_46400(void);
extern uint32_t gj_graph_milestone_46400(void);
extern const char __libcgj_batch46391_marker[];
extern const char __libcgj_batch46392_marker[];
extern const char __libcgj_batch46393_marker[];
extern const char __libcgj_batch46394_marker[];
extern const char __libcgj_batch46395_marker[];
extern const char __libcgj_batch46396_marker[];
extern const char __libcgj_batch46397_marker[];
extern const char __libcgj_batch46398_marker[];
extern const char __libcgj_batch46399_marker[];
extern const char __libcgj_batch46400_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_46400() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_46400 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_46400() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_46400 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_46400() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_46400 fail\n");
		nFail++;
	}
	if (gj_product_score_46400() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_46400 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_46400() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_46400 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_46400() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_46400 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_46400() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_46400 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_46400() != 46400u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_46400 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_46400() != 46400u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_46400 fail\n");
		nFail++;
	}
	if (gj_batch_id_46400() != 46400u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_46400 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_46400() != 46400u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_46400 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch46391_marker, "libcgj-batch46391") != 0 ||
	    strcmp(__libcgj_batch46392_marker, "libcgj-batch46392") != 0 ||
	    strcmp(__libcgj_batch46393_marker, "libcgj-batch46393") != 0 ||
	    strcmp(__libcgj_batch46394_marker, "libcgj-batch46394") != 0 ||
	    strcmp(__libcgj_batch46395_marker, "libcgj-batch46395") != 0 ||
	    strcmp(__libcgj_batch46396_marker, "libcgj-batch46396") != 0 ||
	    strcmp(__libcgj_batch46397_marker, "libcgj-batch46397") != 0 ||
	    strcmp(__libcgj_batch46398_marker, "libcgj-batch46398") != 0 ||
	    strcmp(__libcgj_batch46399_marker, "libcgj-batch46399") != 0 ||
	    strcmp(__libcgj_batch46400_marker, "libcgj-batch46400") != 0) {
		fprintf(stderr, "libcgj: direct m46400 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M46400_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m46400 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m46400 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m46400 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m46400 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m46400 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m46400 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_46400", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_46400", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_46400", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_46400", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_46400", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_46400", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_46400", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_46400", 46400u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_46400", 46400u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_46400", 46400u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_46400", 46400u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch46391_marker",
	                     "libcgj-batch46391", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch46392_marker",
	                     "libcgj-batch46392", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch46393_marker",
	                     "libcgj-batch46393", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch46394_marker",
	                     "libcgj-batch46394", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch46395_marker",
	                     "libcgj-batch46395", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch46396_marker",
	                     "libcgj-batch46396", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch46397_marker",
	                     "libcgj-batch46397", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch46398_marker",
	                     "libcgj-batch46398", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch46399_marker",
	                     "libcgj-batch46399", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch46400_marker",
	                     "libcgj-batch46400", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m46400 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m46400 PASS path=%s checked=%d "
