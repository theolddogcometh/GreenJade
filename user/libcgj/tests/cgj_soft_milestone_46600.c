 ...
 *
 * Soft host gates for continuum MILESTONE 46600 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_46600.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_46600.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_46600 \
 *     user/libcgj/tests/cgj_soft_milestone_46600.c -ldl
 *   ./build/cgj_soft_milestone_46600
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M46600_DIRECT \
 *     -o build/cgj_soft_milestone_46600_direct \
 *     user/libcgj/tests/cgj_soft_milestone_46600.c \
 *     user/libcgj/src/graph_batch46591.c \
 *     user/libcgj/src/graph_batch46592.c \
 *     user/libcgj/src/graph_batch46593.c \
 *     user/libcgj/src/graph_batch46594.c \
 *     user/libcgj/src/graph_batch46595.c \
 *     user/libcgj/src/graph_batch46596.c \
 *     user/libcgj/src/graph_batch46597.c \
 *     user/libcgj/src/graph_batch46598.c \
 *     user/libcgj/src/graph_batch46599.c \
 *     user/libcgj/src/graph_batch46600.c
 *   ./build/cgj_soft_milestone_46600_direct
 *
 * Expected soft KATs (wave 46591–46600 exclusive continuum markers):
 *   gj_shell_green_46600      → 1
 *   gj_libcgj_green_46600     → 1
 *   gj_bar3_ready_46600       → 0   (bar3 still open)
 *   gj_product_score_46600    → 0   (product score still open)
 *   gj_continuum_ready_46600  → 1
 *   gj_smoke_soft_46600       → 1
 *   gj_dyn_soft_46600         → 1
 *   gj_milestone_tag_46600    → 46600
 *   gj_continuum_wave_46600   → 46600
 *   gj_batch_id_46600         → 46600
 *   gj_graph_milestone_46600  → 46600
 *   __libcgj_batch46591_marker … __libcgj_batch46600_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch46591.c … graph_batch46600.c (and fill
 *     46576–46590 / identity 46551–46575) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 46600.
 *   - libc.map may lag milestone 46600 globals; add 46600 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_46600 == 0).
 */
...
#ifndef CGJ_SOFT_M46600_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M46600_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_46600(void);
extern uint32_t gj_libcgj_green_46600(void);
extern uint32_t gj_bar3_ready_46600(void);
extern uint32_t gj_product_score_46600(void);
extern uint32_t gj_continuum_ready_46600(void);
extern uint32_t gj_smoke_soft_46600(void);
extern uint32_t gj_dyn_soft_46600(void);
extern uint32_t gj_milestone_tag_46600(void);
extern uint32_t gj_continuum_wave_46600(void);
extern uint32_t gj_batch_id_46600(void);
extern uint32_t gj_graph_milestone_46600(void);
extern const char __libcgj_batch46591_marker[];
extern const char __libcgj_batch46592_marker[];
extern const char __libcgj_batch46593_marker[];
extern const char __libcgj_batch46594_marker[];
extern const char __libcgj_batch46595_marker[];
extern const char __libcgj_batch46596_marker[];
extern const char __libcgj_batch46597_marker[];
extern const char __libcgj_batch46598_marker[];
extern const char __libcgj_batch46599_marker[];
extern const char __libcgj_batch46600_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_46600() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_46600 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_46600() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_46600 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_46600() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_46600 fail\n");
		nFail++;
	}
	if (gj_product_score_46600() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_46600 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_46600() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_46600 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_46600() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_46600 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_46600() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_46600 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_46600() != 46600u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_46600 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_46600() != 46600u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_46600 fail\n");
		nFail++;
	}
	if (gj_batch_id_46600() != 46600u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_46600 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_46600() != 46600u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_46600 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch46591_marker, "libcgj-batch46591") != 0 ||
	    strcmp(__libcgj_batch46592_marker, "libcgj-batch46592") != 0 ||
	    strcmp(__libcgj_batch46593_marker, "libcgj-batch46593") != 0 ||
	    strcmp(__libcgj_batch46594_marker, "libcgj-batch46594") != 0 ||
	    strcmp(__libcgj_batch46595_marker, "libcgj-batch46595") != 0 ||
	    strcmp(__libcgj_batch46596_marker, "libcgj-batch46596") != 0 ||
	    strcmp(__libcgj_batch46597_marker, "libcgj-batch46597") != 0 ||
	    strcmp(__libcgj_batch46598_marker, "libcgj-batch46598") != 0 ||
	    strcmp(__libcgj_batch46599_marker, "libcgj-batch46599") != 0 ||
	    strcmp(__libcgj_batch46600_marker, "libcgj-batch46600") != 0) {
		fprintf(stderr, "libcgj: direct m46600 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M46600_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m46600 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m46600 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m46600 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m46600 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m46600 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m46600 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_46600", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_46600", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_46600", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_46600", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_46600", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_46600", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_46600", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_46600", 46600u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_46600", 46600u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_46600", 46600u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_46600", 46600u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch46591_marker",
	                     "libcgj-batch46591", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch46592_marker",
	                     "libcgj-batch46592", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch46593_marker",
	                     "libcgj-batch46593", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch46594_marker",
	                     "libcgj-batch46594", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch46595_marker",
	                     "libcgj-batch46595", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch46596_marker",
	                     "libcgj-batch46596", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch46597_marker",
	                     "libcgj-batch46597", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch46598_marker",
	                     "libcgj-batch46598", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch46599_marker",
	                     "libcgj-batch46599", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch46600_marker",
	                     "libcgj-batch46600", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m46600 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m46600 PASS path=%s checked=%d "
