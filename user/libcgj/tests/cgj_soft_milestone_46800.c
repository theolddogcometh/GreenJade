 ...
 *
 * Soft host gates for continuum MILESTONE 46800 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_46800.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_46800.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_46800 \
 *     user/libcgj/tests/cgj_soft_milestone_46800.c -ldl
 *   ./build/cgj_soft_milestone_46800
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M46800_DIRECT \
 *     -o build/cgj_soft_milestone_46800_direct \
 *     user/libcgj/tests/cgj_soft_milestone_46800.c \
 *     user/libcgj/src/graph_batch46791.c \
 *     user/libcgj/src/graph_batch46792.c \
 *     user/libcgj/src/graph_batch46793.c \
 *     user/libcgj/src/graph_batch46794.c \
 *     user/libcgj/src/graph_batch46795.c \
 *     user/libcgj/src/graph_batch46796.c \
 *     user/libcgj/src/graph_batch46797.c \
 *     user/libcgj/src/graph_batch46798.c \
 *     user/libcgj/src/graph_batch46799.c \
 *     user/libcgj/src/graph_batch46800.c
 *   ./build/cgj_soft_milestone_46800_direct
 *
 * Expected soft KATs (wave 46791–46800 exclusive continuum markers):
 *   gj_shell_green_46800      → 1
 *   gj_libcgj_green_46800     → 1
 *   gj_bar3_ready_46800       → 0   (bar3 still open)
 *   gj_product_score_46800    → 0   (product score still open)
 *   gj_continuum_ready_46800  → 1
 *   gj_smoke_soft_46800       → 1
 *   gj_dyn_soft_46800         → 1
 *   gj_milestone_tag_46800    → 46800
 *   gj_continuum_wave_46800   → 46800
 *   gj_batch_id_46800         → 46800
 *   gj_graph_milestone_46800  → 46800
 *   __libcgj_batch46791_marker … __libcgj_batch46800_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch46791.c … graph_batch46800.c (and fill
 *     46776–46790 / identity 46751–46775) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 46800.
 *   - libc.map may lag milestone 46800 globals; add 46800 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_46800 == 0).
 */
...
#ifndef CGJ_SOFT_M46800_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M46800_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_46800(void);
extern uint32_t gj_libcgj_green_46800(void);
extern uint32_t gj_bar3_ready_46800(void);
extern uint32_t gj_product_score_46800(void);
extern uint32_t gj_continuum_ready_46800(void);
extern uint32_t gj_smoke_soft_46800(void);
extern uint32_t gj_dyn_soft_46800(void);
extern uint32_t gj_milestone_tag_46800(void);
extern uint32_t gj_continuum_wave_46800(void);
extern uint32_t gj_batch_id_46800(void);
extern uint32_t gj_graph_milestone_46800(void);
extern const char __libcgj_batch46791_marker[];
extern const char __libcgj_batch46792_marker[];
extern const char __libcgj_batch46793_marker[];
extern const char __libcgj_batch46794_marker[];
extern const char __libcgj_batch46795_marker[];
extern const char __libcgj_batch46796_marker[];
extern const char __libcgj_batch46797_marker[];
extern const char __libcgj_batch46798_marker[];
extern const char __libcgj_batch46799_marker[];
extern const char __libcgj_batch46800_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_46800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_46800 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_46800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_46800 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_46800() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_46800 fail\n");
		nFail++;
	}
	if (gj_product_score_46800() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_46800 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_46800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_46800 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_46800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_46800 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_46800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_46800 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_46800() != 46800u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_46800 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_46800() != 46800u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_46800 fail\n");
		nFail++;
	}
	if (gj_batch_id_46800() != 46800u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_46800 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_46800() != 46800u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_46800 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch46791_marker, "libcgj-batch46791") != 0 ||
	    strcmp(__libcgj_batch46792_marker, "libcgj-batch46792") != 0 ||
	    strcmp(__libcgj_batch46793_marker, "libcgj-batch46793") != 0 ||
	    strcmp(__libcgj_batch46794_marker, "libcgj-batch46794") != 0 ||
	    strcmp(__libcgj_batch46795_marker, "libcgj-batch46795") != 0 ||
	    strcmp(__libcgj_batch46796_marker, "libcgj-batch46796") != 0 ||
	    strcmp(__libcgj_batch46797_marker, "libcgj-batch46797") != 0 ||
	    strcmp(__libcgj_batch46798_marker, "libcgj-batch46798") != 0 ||
	    strcmp(__libcgj_batch46799_marker, "libcgj-batch46799") != 0 ||
	    strcmp(__libcgj_batch46800_marker, "libcgj-batch46800") != 0) {
		fprintf(stderr, "libcgj: direct m46800 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M46800_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m46800 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m46800 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m46800 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m46800 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m46800 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m46800 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_46800", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_46800", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_46800", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_46800", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_46800", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_46800", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_46800", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_46800", 46800u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_46800", 46800u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_46800", 46800u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_46800", 46800u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch46791_marker",
	                     "libcgj-batch46791", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch46792_marker",
	                     "libcgj-batch46792", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch46793_marker",
	                     "libcgj-batch46793", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch46794_marker",
	                     "libcgj-batch46794", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch46795_marker",
	                     "libcgj-batch46795", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch46796_marker",
	                     "libcgj-batch46796", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch46797_marker",
	                     "libcgj-batch46797", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch46798_marker",
	                     "libcgj-batch46798", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch46799_marker",
	                     "libcgj-batch46799", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch46800_marker",
	                     "libcgj-batch46800", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m46800 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m46800 PASS path=%s checked=%d "
