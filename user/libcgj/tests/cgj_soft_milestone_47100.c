 ...
 *
 * Soft host gates for continuum MILESTONE 47100 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_47100.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_47100.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_47100 \
 *     user/libcgj/tests/cgj_soft_milestone_47100.c -ldl
 *   ./build/cgj_soft_milestone_47100
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M47100_DIRECT \
 *     -o build/cgj_soft_milestone_47100_direct \
 *     user/libcgj/tests/cgj_soft_milestone_47100.c \
 *     user/libcgj/src/graph_batch47091.c \
 *     user/libcgj/src/graph_batch47092.c \
 *     user/libcgj/src/graph_batch47093.c \
 *     user/libcgj/src/graph_batch47094.c \
 *     user/libcgj/src/graph_batch47095.c \
 *     user/libcgj/src/graph_batch47096.c \
 *     user/libcgj/src/graph_batch47097.c \
 *     user/libcgj/src/graph_batch47098.c \
 *     user/libcgj/src/graph_batch47099.c \
 *     user/libcgj/src/graph_batch47100.c
 *   ./build/cgj_soft_milestone_47100_direct
 *
 * Expected soft KATs (wave 47091–47100 exclusive continuum markers):
 *   gj_shell_green_47100      → 1
 *   gj_libcgj_green_47100     → 1
 *   gj_bar3_ready_47100       → 0   (bar3 still open)
 *   gj_product_score_47100    → 0   (product score still open)
 *   gj_continuum_ready_47100  → 1
 *   gj_smoke_soft_47100       → 1
 *   gj_dyn_soft_47100         → 1
 *   gj_milestone_tag_47100    → 47100
 *   gj_continuum_wave_47100   → 47100
 *   gj_batch_id_47100         → 47100
 *   gj_graph_milestone_47100  → 47100
 *   __libcgj_batch47091_marker … __libcgj_batch47100_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch47091.c … graph_batch47100.c (and fill
 *     47076–47090 / identity 47051–47075) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 47100.
 *   - libc.map may lag milestone 47100 globals; add 47100 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_47100 == 0).
 */
...
#ifndef CGJ_SOFT_M47100_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M47100_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_47100(void);
extern uint32_t gj_libcgj_green_47100(void);
extern uint32_t gj_bar3_ready_47100(void);
extern uint32_t gj_product_score_47100(void);
extern uint32_t gj_continuum_ready_47100(void);
extern uint32_t gj_smoke_soft_47100(void);
extern uint32_t gj_dyn_soft_47100(void);
extern uint32_t gj_milestone_tag_47100(void);
extern uint32_t gj_continuum_wave_47100(void);
extern uint32_t gj_batch_id_47100(void);
extern uint32_t gj_graph_milestone_47100(void);
extern const char __libcgj_batch47091_marker[];
extern const char __libcgj_batch47092_marker[];
extern const char __libcgj_batch47093_marker[];
extern const char __libcgj_batch47094_marker[];
extern const char __libcgj_batch47095_marker[];
extern const char __libcgj_batch47096_marker[];
extern const char __libcgj_batch47097_marker[];
extern const char __libcgj_batch47098_marker[];
extern const char __libcgj_batch47099_marker[];
extern const char __libcgj_batch47100_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_47100() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_47100 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_47100() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_47100 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_47100() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_47100 fail\n");
		nFail++;
	}
	if (gj_product_score_47100() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_47100 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_47100() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_47100 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_47100() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_47100 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_47100() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_47100 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_47100() != 47100u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_47100 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_47100() != 47100u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_47100 fail\n");
		nFail++;
	}
	if (gj_batch_id_47100() != 47100u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_47100 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_47100() != 47100u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_47100 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch47091_marker, "libcgj-batch47091") != 0 ||
	    strcmp(__libcgj_batch47092_marker, "libcgj-batch47092") != 0 ||
	    strcmp(__libcgj_batch47093_marker, "libcgj-batch47093") != 0 ||
	    strcmp(__libcgj_batch47094_marker, "libcgj-batch47094") != 0 ||
	    strcmp(__libcgj_batch47095_marker, "libcgj-batch47095") != 0 ||
	    strcmp(__libcgj_batch47096_marker, "libcgj-batch47096") != 0 ||
	    strcmp(__libcgj_batch47097_marker, "libcgj-batch47097") != 0 ||
	    strcmp(__libcgj_batch47098_marker, "libcgj-batch47098") != 0 ||
	    strcmp(__libcgj_batch47099_marker, "libcgj-batch47099") != 0 ||
	    strcmp(__libcgj_batch47100_marker, "libcgj-batch47100") != 0) {
		fprintf(stderr, "libcgj: direct m47100 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M47100_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m47100 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m47100 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m47100 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m47100 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m47100 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m47100 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_47100", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_47100", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_47100", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_47100", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_47100", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_47100", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_47100", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_47100", 47100u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_47100", 47100u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_47100", 47100u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_47100", 47100u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch47091_marker",
	                     "libcgj-batch47091", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47092_marker",
	                     "libcgj-batch47092", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47093_marker",
	                     "libcgj-batch47093", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47094_marker",
	                     "libcgj-batch47094", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47095_marker",
	                     "libcgj-batch47095", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47096_marker",
	                     "libcgj-batch47096", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47097_marker",
	                     "libcgj-batch47097", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47098_marker",
	                     "libcgj-batch47098", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47099_marker",
	                     "libcgj-batch47099", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47100_marker",
	                     "libcgj-batch47100", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m47100 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m47100 PASS path=%s checked=%d "
