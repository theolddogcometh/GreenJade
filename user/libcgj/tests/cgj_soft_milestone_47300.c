 ...
 *
 * Soft host gates for continuum MILESTONE 47300 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_47300.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_47300.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_47300 \
 *     user/libcgj/tests/cgj_soft_milestone_47300.c -ldl
 *   ./build/cgj_soft_milestone_47300
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M47300_DIRECT \
 *     -o build/cgj_soft_milestone_47300_direct \
 *     user/libcgj/tests/cgj_soft_milestone_47300.c \
 *     user/libcgj/src/graph_batch47291.c \
 *     user/libcgj/src/graph_batch47292.c \
 *     user/libcgj/src/graph_batch47293.c \
 *     user/libcgj/src/graph_batch47294.c \
 *     user/libcgj/src/graph_batch47295.c \
 *     user/libcgj/src/graph_batch47296.c \
 *     user/libcgj/src/graph_batch47297.c \
 *     user/libcgj/src/graph_batch47298.c \
 *     user/libcgj/src/graph_batch47299.c \
 *     user/libcgj/src/graph_batch47300.c
 *   ./build/cgj_soft_milestone_47300_direct
 *
 * Expected soft KATs (wave 47291–47300 exclusive continuum markers):
 *   gj_shell_green_47300      → 1
 *   gj_libcgj_green_47300     → 1
 *   gj_bar3_ready_47300       → 0   (bar3 still open)
 *   gj_product_score_47300    → 0   (product score still open)
 *   gj_continuum_ready_47300  → 1
 *   gj_smoke_soft_47300       → 1
 *   gj_dyn_soft_47300         → 1
 *   gj_milestone_tag_47300    → 47300
 *   gj_continuum_wave_47300   → 47300
 *   gj_batch_id_47300         → 47300
 *   gj_graph_milestone_47300  → 47300
 *   __libcgj_batch47291_marker … __libcgj_batch47300_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch47291.c … graph_batch47300.c (and fill
 *     47276–47290 / identity 47251–47275) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 47300.
 *   - libc.map may lag milestone 47300 globals; add 47300 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_47300 == 0).
 */
...
#ifndef CGJ_SOFT_M47300_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M47300_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_47300(void);
extern uint32_t gj_libcgj_green_47300(void);
extern uint32_t gj_bar3_ready_47300(void);
extern uint32_t gj_product_score_47300(void);
extern uint32_t gj_continuum_ready_47300(void);
extern uint32_t gj_smoke_soft_47300(void);
extern uint32_t gj_dyn_soft_47300(void);
extern uint32_t gj_milestone_tag_47300(void);
extern uint32_t gj_continuum_wave_47300(void);
extern uint32_t gj_batch_id_47300(void);
extern uint32_t gj_graph_milestone_47300(void);
extern const char __libcgj_batch47291_marker[];
extern const char __libcgj_batch47292_marker[];
extern const char __libcgj_batch47293_marker[];
extern const char __libcgj_batch47294_marker[];
extern const char __libcgj_batch47295_marker[];
extern const char __libcgj_batch47296_marker[];
extern const char __libcgj_batch47297_marker[];
extern const char __libcgj_batch47298_marker[];
extern const char __libcgj_batch47299_marker[];
extern const char __libcgj_batch47300_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_47300() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_47300 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_47300() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_47300 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_47300() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_47300 fail\n");
		nFail++;
	}
	if (gj_product_score_47300() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_47300 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_47300() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_47300 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_47300() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_47300 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_47300() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_47300 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_47300() != 47300u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_47300 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_47300() != 47300u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_47300 fail\n");
		nFail++;
	}
	if (gj_batch_id_47300() != 47300u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_47300 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_47300() != 47300u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_47300 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch47291_marker, "libcgj-batch47291") != 0 ||
	    strcmp(__libcgj_batch47292_marker, "libcgj-batch47292") != 0 ||
	    strcmp(__libcgj_batch47293_marker, "libcgj-batch47293") != 0 ||
	    strcmp(__libcgj_batch47294_marker, "libcgj-batch47294") != 0 ||
	    strcmp(__libcgj_batch47295_marker, "libcgj-batch47295") != 0 ||
	    strcmp(__libcgj_batch47296_marker, "libcgj-batch47296") != 0 ||
	    strcmp(__libcgj_batch47297_marker, "libcgj-batch47297") != 0 ||
	    strcmp(__libcgj_batch47298_marker, "libcgj-batch47298") != 0 ||
	    strcmp(__libcgj_batch47299_marker, "libcgj-batch47299") != 0 ||
	    strcmp(__libcgj_batch47300_marker, "libcgj-batch47300") != 0) {
		fprintf(stderr, "libcgj: direct m47300 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M47300_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m47300 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m47300 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m47300 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m47300 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m47300 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m47300 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_47300", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_47300", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_47300", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_47300", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_47300", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_47300", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_47300", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_47300", 47300u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_47300", 47300u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_47300", 47300u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_47300", 47300u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch47291_marker",
	                     "libcgj-batch47291", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47292_marker",
	                     "libcgj-batch47292", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47293_marker",
	                     "libcgj-batch47293", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47294_marker",
	                     "libcgj-batch47294", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47295_marker",
	                     "libcgj-batch47295", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47296_marker",
	                     "libcgj-batch47296", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47297_marker",
	                     "libcgj-batch47297", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47298_marker",
	                     "libcgj-batch47298", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47299_marker",
	                     "libcgj-batch47299", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47300_marker",
	                     "libcgj-batch47300", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m47300 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m47300 PASS path=%s checked=%d "
