 ...
 *
 * Soft host gates for continuum MILESTONE 47600 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_47600.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_47600.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_47600 \
 *     user/libcgj/tests/cgj_soft_milestone_47600.c -ldl
 *   ./build/cgj_soft_milestone_47600
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M47600_DIRECT \
 *     -o build/cgj_soft_milestone_47600_direct \
 *     user/libcgj/tests/cgj_soft_milestone_47600.c \
 *     user/libcgj/src/graph_batch47591.c \
 *     user/libcgj/src/graph_batch47592.c \
 *     user/libcgj/src/graph_batch47593.c \
 *     user/libcgj/src/graph_batch47594.c \
 *     user/libcgj/src/graph_batch47595.c \
 *     user/libcgj/src/graph_batch47596.c \
 *     user/libcgj/src/graph_batch47597.c \
 *     user/libcgj/src/graph_batch47598.c \
 *     user/libcgj/src/graph_batch47599.c \
 *     user/libcgj/src/graph_batch47600.c
 *   ./build/cgj_soft_milestone_47600_direct
 *
 * Expected soft KATs (wave 47591–47600 exclusive continuum markers):
 *   gj_shell_green_47600      → 1
 *   gj_libcgj_green_47600     → 1
 *   gj_bar3_ready_47600       → 0   (bar3 still open)
 *   gj_product_score_47600    → 0   (product score still open)
 *   gj_continuum_ready_47600  → 1
 *   gj_smoke_soft_47600       → 1
 *   gj_dyn_soft_47600         → 1
 *   gj_milestone_tag_47600    → 47600
 *   gj_continuum_wave_47600   → 47600
 *   gj_batch_id_47600         → 47600
 *   gj_graph_milestone_47600  → 47600
 *   __libcgj_batch47591_marker … __libcgj_batch47600_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch47591.c … graph_batch47600.c (and fill
 *     47576–47590 / identity 47551–47575) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 47600.
 *   - libc.map may lag milestone 47600 globals; add 47600 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_47600 == 0).
 */
...
#ifndef CGJ_SOFT_M47600_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M47600_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_47600(void);
extern uint32_t gj_libcgj_green_47600(void);
extern uint32_t gj_bar3_ready_47600(void);
extern uint32_t gj_product_score_47600(void);
extern uint32_t gj_continuum_ready_47600(void);
extern uint32_t gj_smoke_soft_47600(void);
extern uint32_t gj_dyn_soft_47600(void);
extern uint32_t gj_milestone_tag_47600(void);
extern uint32_t gj_continuum_wave_47600(void);
extern uint32_t gj_batch_id_47600(void);
extern uint32_t gj_graph_milestone_47600(void);
extern const char __libcgj_batch47591_marker[];
extern const char __libcgj_batch47592_marker[];
extern const char __libcgj_batch47593_marker[];
extern const char __libcgj_batch47594_marker[];
extern const char __libcgj_batch47595_marker[];
extern const char __libcgj_batch47596_marker[];
extern const char __libcgj_batch47597_marker[];
extern const char __libcgj_batch47598_marker[];
extern const char __libcgj_batch47599_marker[];
extern const char __libcgj_batch47600_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_47600() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_47600 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_47600() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_47600 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_47600() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_47600 fail\n");
		nFail++;
	}
	if (gj_product_score_47600() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_47600 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_47600() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_47600 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_47600() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_47600 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_47600() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_47600 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_47600() != 47600u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_47600 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_47600() != 47600u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_47600 fail\n");
		nFail++;
	}
	if (gj_batch_id_47600() != 47600u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_47600 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_47600() != 47600u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_47600 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch47591_marker, "libcgj-batch47591") != 0 ||
	    strcmp(__libcgj_batch47592_marker, "libcgj-batch47592") != 0 ||
	    strcmp(__libcgj_batch47593_marker, "libcgj-batch47593") != 0 ||
	    strcmp(__libcgj_batch47594_marker, "libcgj-batch47594") != 0 ||
	    strcmp(__libcgj_batch47595_marker, "libcgj-batch47595") != 0 ||
	    strcmp(__libcgj_batch47596_marker, "libcgj-batch47596") != 0 ||
	    strcmp(__libcgj_batch47597_marker, "libcgj-batch47597") != 0 ||
	    strcmp(__libcgj_batch47598_marker, "libcgj-batch47598") != 0 ||
	    strcmp(__libcgj_batch47599_marker, "libcgj-batch47599") != 0 ||
	    strcmp(__libcgj_batch47600_marker, "libcgj-batch47600") != 0) {
		fprintf(stderr, "libcgj: direct m47600 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M47600_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m47600 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m47600 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m47600 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m47600 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m47600 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m47600 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_47600", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_47600", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_47600", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_47600", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_47600", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_47600", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_47600", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_47600", 47600u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_47600", 47600u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_47600", 47600u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_47600", 47600u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch47591_marker",
	                     "libcgj-batch47591", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47592_marker",
	                     "libcgj-batch47592", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47593_marker",
	                     "libcgj-batch47593", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47594_marker",
	                     "libcgj-batch47594", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47595_marker",
	                     "libcgj-batch47595", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47596_marker",
	                     "libcgj-batch47596", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47597_marker",
	                     "libcgj-batch47597", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47598_marker",
	                     "libcgj-batch47598", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47599_marker",
	                     "libcgj-batch47599", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47600_marker",
	                     "libcgj-batch47600", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m47600 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m47600 PASS path=%s checked=%d "
