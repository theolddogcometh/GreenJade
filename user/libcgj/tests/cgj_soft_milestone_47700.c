 ...
 *
 * Soft host gates for continuum MILESTONE 47700 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_47700.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_47700.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_47700 \
 *     user/libcgj/tests/cgj_soft_milestone_47700.c -ldl
 *   ./build/cgj_soft_milestone_47700
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M47700_DIRECT \
 *     -o build/cgj_soft_milestone_47700_direct \
 *     user/libcgj/tests/cgj_soft_milestone_47700.c \
 *     user/libcgj/src/graph_batch47691.c \
 *     user/libcgj/src/graph_batch47692.c \
 *     user/libcgj/src/graph_batch47693.c \
 *     user/libcgj/src/graph_batch47694.c \
 *     user/libcgj/src/graph_batch47695.c \
 *     user/libcgj/src/graph_batch47696.c \
 *     user/libcgj/src/graph_batch47697.c \
 *     user/libcgj/src/graph_batch47698.c \
 *     user/libcgj/src/graph_batch47699.c \
 *     user/libcgj/src/graph_batch47700.c
 *   ./build/cgj_soft_milestone_47700_direct
 *
 * Expected soft KATs (wave 47691–47700 exclusive continuum markers):
 *   gj_shell_green_47700      → 1
 *   gj_libcgj_green_47700     → 1
 *   gj_bar3_ready_47700       → 0   (bar3 still open)
 *   gj_product_score_47700    → 0   (product score still open)
 *   gj_continuum_ready_47700  → 1
 *   gj_smoke_soft_47700       → 1
 *   gj_dyn_soft_47700         → 1
 *   gj_milestone_tag_47700    → 47700
 *   gj_continuum_wave_47700   → 47700
 *   gj_batch_id_47700         → 47700
 *   gj_graph_milestone_47700  → 47700
 *   __libcgj_batch47691_marker … __libcgj_batch47700_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch47691.c … graph_batch47700.c (and fill
 *     47676–47690 / identity 47651–47675) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 47700.
 *   - libc.map may lag milestone 47700 globals; add 47700 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_47700 == 0).
 */
...
#ifndef CGJ_SOFT_M47700_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M47700_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_47700(void);
extern uint32_t gj_libcgj_green_47700(void);
extern uint32_t gj_bar3_ready_47700(void);
extern uint32_t gj_product_score_47700(void);
extern uint32_t gj_continuum_ready_47700(void);
extern uint32_t gj_smoke_soft_47700(void);
extern uint32_t gj_dyn_soft_47700(void);
extern uint32_t gj_milestone_tag_47700(void);
extern uint32_t gj_continuum_wave_47700(void);
extern uint32_t gj_batch_id_47700(void);
extern uint32_t gj_graph_milestone_47700(void);
extern const char __libcgj_batch47691_marker[];
extern const char __libcgj_batch47692_marker[];
extern const char __libcgj_batch47693_marker[];
extern const char __libcgj_batch47694_marker[];
extern const char __libcgj_batch47695_marker[];
extern const char __libcgj_batch47696_marker[];
extern const char __libcgj_batch47697_marker[];
extern const char __libcgj_batch47698_marker[];
extern const char __libcgj_batch47699_marker[];
extern const char __libcgj_batch47700_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_47700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_47700 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_47700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_47700 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_47700() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_47700 fail\n");
		nFail++;
	}
	if (gj_product_score_47700() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_47700 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_47700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_47700 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_47700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_47700 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_47700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_47700 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_47700() != 47700u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_47700 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_47700() != 47700u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_47700 fail\n");
		nFail++;
	}
	if (gj_batch_id_47700() != 47700u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_47700 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_47700() != 47700u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_47700 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch47691_marker, "libcgj-batch47691") != 0 ||
	    strcmp(__libcgj_batch47692_marker, "libcgj-batch47692") != 0 ||
	    strcmp(__libcgj_batch47693_marker, "libcgj-batch47693") != 0 ||
	    strcmp(__libcgj_batch47694_marker, "libcgj-batch47694") != 0 ||
	    strcmp(__libcgj_batch47695_marker, "libcgj-batch47695") != 0 ||
	    strcmp(__libcgj_batch47696_marker, "libcgj-batch47696") != 0 ||
	    strcmp(__libcgj_batch47697_marker, "libcgj-batch47697") != 0 ||
	    strcmp(__libcgj_batch47698_marker, "libcgj-batch47698") != 0 ||
	    strcmp(__libcgj_batch47699_marker, "libcgj-batch47699") != 0 ||
	    strcmp(__libcgj_batch47700_marker, "libcgj-batch47700") != 0) {
		fprintf(stderr, "libcgj: direct m47700 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M47700_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m47700 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m47700 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m47700 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m47700 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m47700 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m47700 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_47700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_47700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_47700", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_47700", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_47700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_47700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_47700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_47700", 47700u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_47700", 47700u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_47700", 47700u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_47700", 47700u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch47691_marker",
	                     "libcgj-batch47691", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47692_marker",
	                     "libcgj-batch47692", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47693_marker",
	                     "libcgj-batch47693", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47694_marker",
	                     "libcgj-batch47694", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47695_marker",
	                     "libcgj-batch47695", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47696_marker",
	                     "libcgj-batch47696", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47697_marker",
	                     "libcgj-batch47697", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47698_marker",
	                     "libcgj-batch47698", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47699_marker",
	                     "libcgj-batch47699", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47700_marker",
	                     "libcgj-batch47700", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m47700 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m47700 PASS path=%s checked=%d "
