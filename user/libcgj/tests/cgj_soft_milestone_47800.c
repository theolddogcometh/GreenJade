 ...
 *
 * Soft host gates for continuum MILESTONE 47800 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_47800.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_47800.txt
 *
 * Test harness note:
 ...
 *     cgj_soft_milestone_47000.c / cgj_soft_milestone_47100.c /
 *     cgj_soft_milestone_47200.c / cgj_soft_milestone_47300.c /
 *     cgj_soft_milestone_47400.c / cgj_soft_milestone_47500.c /
 *     cgj_soft_milestone_47600.c /
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_47800 \
 *     user/libcgj/tests/cgj_soft_milestone_47800.c -ldl
 *   ./build/cgj_soft_milestone_47800
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M47800_DIRECT \
 *     -o build/cgj_soft_milestone_47800_direct \
 *     user/libcgj/tests/cgj_soft_milestone_47800.c \
 *     user/libcgj/src/graph_batch47791.c \
 *     user/libcgj/src/graph_batch47792.c \
 *     user/libcgj/src/graph_batch47793.c \
 *     user/libcgj/src/graph_batch47794.c \
 *     user/libcgj/src/graph_batch47795.c \
 *     user/libcgj/src/graph_batch47796.c \
 *     user/libcgj/src/graph_batch47797.c \
 *     user/libcgj/src/graph_batch47798.c \
 *     user/libcgj/src/graph_batch47799.c \
 *     user/libcgj/src/graph_batch47800.c
 *   ./build/cgj_soft_milestone_47800_direct
 *
 * Expected soft KATs (wave 47791–47800 exclusive continuum markers):
 *   gj_shell_green_47800      → 1
 *   gj_libcgj_green_47800     → 1
 *   gj_bar3_ready_47800       → 0   (bar3 still open)
 *   gj_product_score_47800    → 0   (product score still open)
 *   gj_continuum_ready_47800  → 1
 *   gj_smoke_soft_47800       → 1
 *   gj_dyn_soft_47800         → 1
 *   gj_milestone_tag_47800    → 47800
 *   gj_continuum_wave_47800   → 47800
 *   gj_batch_id_47800         → 47800
 *   gj_graph_milestone_47800  → 47800
 *   __libcgj_batch47791_marker … __libcgj_batch47800_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch47791.c … graph_batch47800.c (and fill
 *     47776–47790 / identity 47751–47775) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 47800.
 *   - libc.map may lag milestone 47800 globals; add 47800 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_47800 == 0).
 */
...
#ifndef CGJ_SOFT_M47800_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M47800_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_47800(void);
extern uint32_t gj_libcgj_green_47800(void);
extern uint32_t gj_bar3_ready_47800(void);
extern uint32_t gj_product_score_47800(void);
extern uint32_t gj_continuum_ready_47800(void);
extern uint32_t gj_smoke_soft_47800(void);
extern uint32_t gj_dyn_soft_47800(void);
extern uint32_t gj_milestone_tag_47800(void);
extern uint32_t gj_continuum_wave_47800(void);
extern uint32_t gj_batch_id_47800(void);
extern uint32_t gj_graph_milestone_47800(void);
extern const char __libcgj_batch47791_marker[];
extern const char __libcgj_batch47792_marker[];
extern const char __libcgj_batch47793_marker[];
extern const char __libcgj_batch47794_marker[];
extern const char __libcgj_batch47795_marker[];
extern const char __libcgj_batch47796_marker[];
extern const char __libcgj_batch47797_marker[];
extern const char __libcgj_batch47798_marker[];
extern const char __libcgj_batch47799_marker[];
extern const char __libcgj_batch47800_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_47800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_47800 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_47800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_47800 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_47800() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_47800 fail\n");
		nFail++;
	}
	if (gj_product_score_47800() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_47800 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_47800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_47800 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_47800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_47800 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_47800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_47800 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_47800() != 47800u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_47800 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_47800() != 47800u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_47800 fail\n");
		nFail++;
	}
	if (gj_batch_id_47800() != 47800u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_47800 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_47800() != 47800u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_47800 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch47791_marker, "libcgj-batch47791") != 0 ||
	    strcmp(__libcgj_batch47792_marker, "libcgj-batch47792") != 0 ||
	    strcmp(__libcgj_batch47793_marker, "libcgj-batch47793") != 0 ||
	    strcmp(__libcgj_batch47794_marker, "libcgj-batch47794") != 0 ||
	    strcmp(__libcgj_batch47795_marker, "libcgj-batch47795") != 0 ||
	    strcmp(__libcgj_batch47796_marker, "libcgj-batch47796") != 0 ||
	    strcmp(__libcgj_batch47797_marker, "libcgj-batch47797") != 0 ||
	    strcmp(__libcgj_batch47798_marker, "libcgj-batch47798") != 0 ||
	    strcmp(__libcgj_batch47799_marker, "libcgj-batch47799") != 0 ||
	    strcmp(__libcgj_batch47800_marker, "libcgj-batch47800") != 0) {
		fprintf(stderr, "libcgj: direct m47800 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M47800_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m47800 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m47800 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m47800 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m47800 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m47800 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m47800 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_47800", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_47800", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_47800", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_47800", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_47800", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_47800", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_47800", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_47800", 47800u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_47800", 47800u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_47800", 47800u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_47800", 47800u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch47791_marker",
	                     "libcgj-batch47791", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47792_marker",
	                     "libcgj-batch47792", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47793_marker",
	                     "libcgj-batch47793", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47794_marker",
	                     "libcgj-batch47794", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47795_marker",
	                     "libcgj-batch47795", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47796_marker",
	                     "libcgj-batch47796", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47797_marker",
	                     "libcgj-batch47797", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47798_marker",
	                     "libcgj-batch47798", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47799_marker",
	                     "libcgj-batch47799", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47800_marker",
	                     "libcgj-batch47800", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m47800 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m47800 PASS path=%s checked=%d "
