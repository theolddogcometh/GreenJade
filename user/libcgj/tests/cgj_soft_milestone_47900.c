 ...
 *
 * Soft host gates for continuum MILESTONE 47900 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_47900.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_47900.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_47900 \
 *     user/libcgj/tests/cgj_soft_milestone_47900.c -ldl
 *   ./build/cgj_soft_milestone_47900
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M47900_DIRECT \
 *     -o build/cgj_soft_milestone_47900_direct \
 *     user/libcgj/tests/cgj_soft_milestone_47900.c \
 *     user/libcgj/src/graph_batch47891.c \
 *     user/libcgj/src/graph_batch47892.c \
 *     user/libcgj/src/graph_batch47893.c \
 *     user/libcgj/src/graph_batch47894.c \
 *     user/libcgj/src/graph_batch47895.c \
 *     user/libcgj/src/graph_batch47896.c \
 *     user/libcgj/src/graph_batch47897.c \
 *     user/libcgj/src/graph_batch47898.c \
 *     user/libcgj/src/graph_batch47899.c \
 *     user/libcgj/src/graph_batch47900.c
 *   ./build/cgj_soft_milestone_47900_direct
 *
 * Expected soft KATs (wave 47891–47900 exclusive continuum markers):
 *   gj_shell_green_47900      → 1
 *   gj_libcgj_green_47900     → 1
 *   gj_bar3_ready_47900       → 0   (bar3 still open)
 *   gj_product_score_47900    → 0   (product score still open)
 *   gj_continuum_ready_47900  → 1
 *   gj_smoke_soft_47900       → 1
 *   gj_dyn_soft_47900         → 1
 *   gj_milestone_tag_47900    → 47900
 *   gj_continuum_wave_47900   → 47900
 *   gj_batch_id_47900         → 47900
 *   gj_graph_milestone_47900  → 47900
 *   __libcgj_batch47891_marker … __libcgj_batch47900_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch47891.c … graph_batch47900.c (and fill
 *     47876–47890 / identity 47851–47875) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 47900.
 *   - libc.map may lag milestone 47900 globals; add 47900 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_47900 == 0).
 */
...
#ifndef CGJ_SOFT_M47900_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M47900_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_47900(void);
extern uint32_t gj_libcgj_green_47900(void);
extern uint32_t gj_bar3_ready_47900(void);
extern uint32_t gj_product_score_47900(void);
extern uint32_t gj_continuum_ready_47900(void);
extern uint32_t gj_smoke_soft_47900(void);
extern uint32_t gj_dyn_soft_47900(void);
extern uint32_t gj_milestone_tag_47900(void);
extern uint32_t gj_continuum_wave_47900(void);
extern uint32_t gj_batch_id_47900(void);
extern uint32_t gj_graph_milestone_47900(void);
extern const char __libcgj_batch47891_marker[];
extern const char __libcgj_batch47892_marker[];
extern const char __libcgj_batch47893_marker[];
extern const char __libcgj_batch47894_marker[];
extern const char __libcgj_batch47895_marker[];
extern const char __libcgj_batch47896_marker[];
extern const char __libcgj_batch47897_marker[];
extern const char __libcgj_batch47898_marker[];
extern const char __libcgj_batch47899_marker[];
extern const char __libcgj_batch47900_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_47900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_47900 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_47900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_47900 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_47900() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_47900 fail\n");
		nFail++;
	}
	if (gj_product_score_47900() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_47900 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_47900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_47900 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_47900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_47900 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_47900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_47900 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_47900() != 47900u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_47900 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_47900() != 47900u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_47900 fail\n");
		nFail++;
	}
	if (gj_batch_id_47900() != 47900u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_47900 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_47900() != 47900u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_47900 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch47891_marker, "libcgj-batch47891") != 0 ||
	    strcmp(__libcgj_batch47892_marker, "libcgj-batch47892") != 0 ||
	    strcmp(__libcgj_batch47893_marker, "libcgj-batch47893") != 0 ||
	    strcmp(__libcgj_batch47894_marker, "libcgj-batch47894") != 0 ||
	    strcmp(__libcgj_batch47895_marker, "libcgj-batch47895") != 0 ||
	    strcmp(__libcgj_batch47896_marker, "libcgj-batch47896") != 0 ||
	    strcmp(__libcgj_batch47897_marker, "libcgj-batch47897") != 0 ||
	    strcmp(__libcgj_batch47898_marker, "libcgj-batch47898") != 0 ||
	    strcmp(__libcgj_batch47899_marker, "libcgj-batch47899") != 0 ||
	    strcmp(__libcgj_batch47900_marker, "libcgj-batch47900") != 0) {
		fprintf(stderr, "libcgj: direct m47900 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M47900_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m47900 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m47900 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m47900 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m47900 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m47900 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m47900 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_47900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_47900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_47900", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_47900", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_47900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_47900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_47900", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_47900", 47900u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_47900", 47900u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_47900", 47900u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_47900", 47900u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch47891_marker",
	                     "libcgj-batch47891", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47892_marker",
	                     "libcgj-batch47892", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47893_marker",
	                     "libcgj-batch47893", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47894_marker",
	                     "libcgj-batch47894", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47895_marker",
	                     "libcgj-batch47895", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47896_marker",
	                     "libcgj-batch47896", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47897_marker",
	                     "libcgj-batch47897", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47898_marker",
	                     "libcgj-batch47898", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47899_marker",
	                     "libcgj-batch47899", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch47900_marker",
	                     "libcgj-batch47900", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m47900 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m47900 PASS path=%s checked=%d "
