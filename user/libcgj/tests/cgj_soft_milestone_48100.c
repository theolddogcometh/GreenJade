 ...
 *
 * Soft host gates for continuum MILESTONE 48100 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_48100.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_48100.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_48100 \
 *     user/libcgj/tests/cgj_soft_milestone_48100.c -ldl
 *   ./build/cgj_soft_milestone_48100
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M48100_DIRECT \
 *     -o build/cgj_soft_milestone_48100_direct \
 *     user/libcgj/tests/cgj_soft_milestone_48100.c \
 *     user/libcgj/src/graph_batch48091.c \
 *     user/libcgj/src/graph_batch48092.c \
 *     user/libcgj/src/graph_batch48093.c \
 *     user/libcgj/src/graph_batch48094.c \
 *     user/libcgj/src/graph_batch48095.c \
 *     user/libcgj/src/graph_batch48096.c \
 *     user/libcgj/src/graph_batch48097.c \
 *     user/libcgj/src/graph_batch48098.c \
 *     user/libcgj/src/graph_batch48099.c \
 *     user/libcgj/src/graph_batch48100.c
 *   ./build/cgj_soft_milestone_48100_direct
 *
 * Expected soft KATs (wave 48091–48100 exclusive continuum markers):
 *   gj_shell_green_48100      → 1
 *   gj_libcgj_green_48100     → 1
 *   gj_bar3_ready_48100       → 0   (bar3 still open)
 *   gj_product_score_48100    → 0   (product score still open)
 *   gj_continuum_ready_48100  → 1
 *   gj_smoke_soft_48100       → 1
 *   gj_dyn_soft_48100         → 1
 *   gj_milestone_tag_48100    → 48100
 *   gj_continuum_wave_48100   → 48100
 *   gj_batch_id_48100         → 48100
 *   gj_graph_milestone_48100  → 48100
 *   __libcgj_batch48091_marker … __libcgj_batch48100_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch48091.c … graph_batch48100.c (and fill
 *     48076–48090 / identity 48051–48075) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 48100.
 *   - libc.map may lag milestone 48100 globals; add 48100 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_48100 == 0).
 */
...
#ifndef CGJ_SOFT_M48100_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M48100_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_48100(void);
extern uint32_t gj_libcgj_green_48100(void);
extern uint32_t gj_bar3_ready_48100(void);
extern uint32_t gj_product_score_48100(void);
extern uint32_t gj_continuum_ready_48100(void);
extern uint32_t gj_smoke_soft_48100(void);
extern uint32_t gj_dyn_soft_48100(void);
extern uint32_t gj_milestone_tag_48100(void);
extern uint32_t gj_continuum_wave_48100(void);
extern uint32_t gj_batch_id_48100(void);
extern uint32_t gj_graph_milestone_48100(void);
extern const char __libcgj_batch48091_marker[];
extern const char __libcgj_batch48092_marker[];
extern const char __libcgj_batch48093_marker[];
extern const char __libcgj_batch48094_marker[];
extern const char __libcgj_batch48095_marker[];
extern const char __libcgj_batch48096_marker[];
extern const char __libcgj_batch48097_marker[];
extern const char __libcgj_batch48098_marker[];
extern const char __libcgj_batch48099_marker[];
extern const char __libcgj_batch48100_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_48100() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_48100 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_48100() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_48100 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_48100() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_48100 fail\n");
		nFail++;
	}
	if (gj_product_score_48100() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_48100 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_48100() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_48100 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_48100() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_48100 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_48100() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_48100 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_48100() != 48100u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_48100 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_48100() != 48100u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_48100 fail\n");
		nFail++;
	}
	if (gj_batch_id_48100() != 48100u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_48100 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_48100() != 48100u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_48100 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch48091_marker, "libcgj-batch48091") != 0 ||
	    strcmp(__libcgj_batch48092_marker, "libcgj-batch48092") != 0 ||
	    strcmp(__libcgj_batch48093_marker, "libcgj-batch48093") != 0 ||
	    strcmp(__libcgj_batch48094_marker, "libcgj-batch48094") != 0 ||
	    strcmp(__libcgj_batch48095_marker, "libcgj-batch48095") != 0 ||
	    strcmp(__libcgj_batch48096_marker, "libcgj-batch48096") != 0 ||
	    strcmp(__libcgj_batch48097_marker, "libcgj-batch48097") != 0 ||
	    strcmp(__libcgj_batch48098_marker, "libcgj-batch48098") != 0 ||
	    strcmp(__libcgj_batch48099_marker, "libcgj-batch48099") != 0 ||
	    strcmp(__libcgj_batch48100_marker, "libcgj-batch48100") != 0) {
		fprintf(stderr, "libcgj: direct m48100 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M48100_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m48100 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m48100 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m48100 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m48100 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m48100 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m48100 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_48100", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_48100", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_48100", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_48100", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_48100", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_48100", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_48100", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_48100", 48100u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_48100", 48100u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_48100", 48100u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_48100", 48100u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch48091_marker",
	                     "libcgj-batch48091", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch48092_marker",
	                     "libcgj-batch48092", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch48093_marker",
	                     "libcgj-batch48093", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch48094_marker",
	                     "libcgj-batch48094", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch48095_marker",
	                     "libcgj-batch48095", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch48096_marker",
	                     "libcgj-batch48096", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch48097_marker",
	                     "libcgj-batch48097", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch48098_marker",
	                     "libcgj-batch48098", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch48099_marker",
	                     "libcgj-batch48099", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch48100_marker",
	                     "libcgj-batch48100", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m48100 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m48100 PASS path=%s checked=%d "
