 ...
 *
 * Soft host gates for continuum MILESTONE 48600 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_48600.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_48600.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_48600 \
 *     user/libcgj/tests/cgj_soft_milestone_48600.c -ldl
 *   ./build/cgj_soft_milestone_48600
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M48600_DIRECT \
 *     -o build/cgj_soft_milestone_48600_direct \
 *     user/libcgj/tests/cgj_soft_milestone_48600.c \
 *     user/libcgj/src/graph_batch48591.c \
 *     user/libcgj/src/graph_batch48592.c \
 *     user/libcgj/src/graph_batch48593.c \
 *     user/libcgj/src/graph_batch48594.c \
 *     user/libcgj/src/graph_batch48595.c \
 *     user/libcgj/src/graph_batch48596.c \
 *     user/libcgj/src/graph_batch48597.c \
 *     user/libcgj/src/graph_batch48598.c \
 *     user/libcgj/src/graph_batch48599.c \
 *     user/libcgj/src/graph_batch48600.c
 *   ./build/cgj_soft_milestone_48600_direct
 *
 * Expected soft KATs (wave 48591–48600 exclusive continuum markers):
 *   gj_shell_green_48600      → 1
 *   gj_libcgj_green_48600     → 1
 *   gj_bar3_ready_48600       → 0   (bar3 still open)
 *   gj_product_score_48600    → 0   (product score still open)
 *   gj_continuum_ready_48600  → 1
 *   gj_smoke_soft_48600       → 1
 *   gj_dyn_soft_48600         → 1
 *   gj_milestone_tag_48600    → 48600
 *   gj_continuum_wave_48600   → 48600
 *   gj_batch_id_48600         → 48600
 *   gj_graph_milestone_48600  → 48600
 *   __libcgj_batch48591_marker … __libcgj_batch48600_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch48591.c … graph_batch48600.c (and fill
 *     48576–48590 / identity 48551–48575) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 48600.
 *   - libc.map may lag milestone 48600 globals; add 48600 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_48600 == 0).
 *
 ...
 *     continuum/smoke/dyn soft=1; tag/wave/batch/graph = 48600.
 * greppable: libcgj: soft m48600 residual Soft!=product
 * greppable: libcgj: soft m48600 Dual DoD A/B OPEN
 * greppable: libcgj: soft m48600 G-AC-1
 */
...
#ifndef CGJ_SOFT_M48600_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M48600_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_48600(void);
extern uint32_t gj_libcgj_green_48600(void);
extern uint32_t gj_bar3_ready_48600(void);
extern uint32_t gj_product_score_48600(void);
extern uint32_t gj_continuum_ready_48600(void);
extern uint32_t gj_smoke_soft_48600(void);
extern uint32_t gj_dyn_soft_48600(void);
extern uint32_t gj_milestone_tag_48600(void);
extern uint32_t gj_continuum_wave_48600(void);
extern uint32_t gj_batch_id_48600(void);
extern uint32_t gj_graph_milestone_48600(void);
extern const char __libcgj_batch48591_marker[];
extern const char __libcgj_batch48592_marker[];
extern const char __libcgj_batch48593_marker[];
extern const char __libcgj_batch48594_marker[];
extern const char __libcgj_batch48595_marker[];
extern const char __libcgj_batch48596_marker[];
extern const char __libcgj_batch48597_marker[];
extern const char __libcgj_batch48598_marker[];
extern const char __libcgj_batch48599_marker[];
extern const char __libcgj_batch48600_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_48600() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_48600 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_48600() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_48600 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_48600() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_48600 fail\n");
		nFail++;
	}
	if (gj_product_score_48600() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_48600 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_48600() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_48600 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_48600() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_48600 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_48600() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_48600 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_48600() != 48600u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_48600 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_48600() != 48600u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_48600 fail\n");
		nFail++;
	}
	if (gj_batch_id_48600() != 48600u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_48600 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_48600() != 48600u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_48600 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch48591_marker, "libcgj-batch48591") != 0 ||
	    strcmp(__libcgj_batch48592_marker, "libcgj-batch48592") != 0 ||
	    strcmp(__libcgj_batch48593_marker, "libcgj-batch48593") != 0 ||
	    strcmp(__libcgj_batch48594_marker, "libcgj-batch48594") != 0 ||
	    strcmp(__libcgj_batch48595_marker, "libcgj-batch48595") != 0 ||
	    strcmp(__libcgj_batch48596_marker, "libcgj-batch48596") != 0 ||
	    strcmp(__libcgj_batch48597_marker, "libcgj-batch48597") != 0 ||
	    strcmp(__libcgj_batch48598_marker, "libcgj-batch48598") != 0 ||
	    strcmp(__libcgj_batch48599_marker, "libcgj-batch48599") != 0 ||
	    strcmp(__libcgj_batch48600_marker, "libcgj-batch48600") != 0) {
		fprintf(stderr, "libcgj: direct m48600 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M48600_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m48600 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m48600 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m48600 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m48600 DIRECT PASS (all 11 lamps + markers; "
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m48600 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m48600 SOFT-SKIP (no product SO; "
		       "Soft!=product; Dual DoD A/B OPEN; G-AC-1)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_48600", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_48600", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_48600", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_48600", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_48600", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_48600", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_48600", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_48600", 48600u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_48600", 48600u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_48600", 48600u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_48600", 48600u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch48591_marker",
	                     "libcgj-batch48591", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch48592_marker",
	                     "libcgj-batch48592", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch48593_marker",
	                     "libcgj-batch48593", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch48594_marker",
	                     "libcgj-batch48594", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch48595_marker",
	                     "libcgj-batch48595", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch48596_marker",
	                     "libcgj-batch48596", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch48597_marker",
	                     "libcgj-batch48597", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch48598_marker",
	                     "libcgj-batch48598", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch48599_marker",
	                     "libcgj-batch48599", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch48600_marker",
	                     "libcgj-batch48600", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m48600 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m48600 PASS path=%s checked=%d "
