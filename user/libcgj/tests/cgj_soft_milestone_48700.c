 ...
 *
 * Soft host gates for continuum MILESTONE 48700 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_48700.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_48700.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_48700 \
 *     user/libcgj/tests/cgj_soft_milestone_48700.c -ldl
 *   ./build/cgj_soft_milestone_48700
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M48700_DIRECT \
 *     -o build/cgj_soft_milestone_48700_direct \
 *     user/libcgj/tests/cgj_soft_milestone_48700.c \
 *     user/libcgj/src/graph_batch48691.c \
 *     user/libcgj/src/graph_batch48692.c \
 *     user/libcgj/src/graph_batch48693.c \
 *     user/libcgj/src/graph_batch48694.c \
 *     user/libcgj/src/graph_batch48695.c \
 *     user/libcgj/src/graph_batch48696.c \
 *     user/libcgj/src/graph_batch48697.c \
 *     user/libcgj/src/graph_batch48698.c \
 *     user/libcgj/src/graph_batch48699.c \
 *     user/libcgj/src/graph_batch48700.c
 *   ./build/cgj_soft_milestone_48700_direct
 *
 * Expected soft KATs (wave 48691–48700 exclusive continuum markers):
 *   gj_shell_green_48700      → 1
 *   gj_libcgj_green_48700     → 1
 *   gj_bar3_ready_48700       → 0   (bar3 still open)
 *   gj_product_score_48700    → 0   (product score still open)
 *   gj_continuum_ready_48700  → 1
 *   gj_smoke_soft_48700       → 1
 *   gj_dyn_soft_48700         → 1
 *   gj_milestone_tag_48700    → 48700
 *   gj_continuum_wave_48700   → 48700
 *   gj_batch_id_48700         → 48700
 *   gj_graph_milestone_48700  → 48700
 *   __libcgj_batch48691_marker … __libcgj_batch48700_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch48691.c … graph_batch48700.c (and fill
 *     48676–48690 / identity 48651–48675) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 48700.
 *   - libc.map may lag milestone 48700 globals; add 48700 symbols to the
...
#ifndef CGJ_SOFT_M48700_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M48700_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_48700(void);
extern uint32_t gj_libcgj_green_48700(void);
extern uint32_t gj_bar3_ready_48700(void);
extern uint32_t gj_product_score_48700(void);
extern uint32_t gj_continuum_ready_48700(void);
extern uint32_t gj_smoke_soft_48700(void);
extern uint32_t gj_dyn_soft_48700(void);
extern uint32_t gj_milestone_tag_48700(void);
extern uint32_t gj_continuum_wave_48700(void);
extern uint32_t gj_batch_id_48700(void);
extern uint32_t gj_graph_milestone_48700(void);
extern const char __libcgj_batch48691_marker[];
extern const char __libcgj_batch48692_marker[];
extern const char __libcgj_batch48693_marker[];
extern const char __libcgj_batch48694_marker[];
extern const char __libcgj_batch48695_marker[];
extern const char __libcgj_batch48696_marker[];
extern const char __libcgj_batch48697_marker[];
extern const char __libcgj_batch48698_marker[];
extern const char __libcgj_batch48699_marker[];
extern const char __libcgj_batch48700_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_48700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_48700 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_48700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_48700 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_48700() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_48700 fail\n");
		nFail++;
	}
	if (gj_product_score_48700() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_48700 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_48700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_48700 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_48700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_48700 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_48700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_48700 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_48700() != 48700u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_48700 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_48700() != 48700u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_48700 fail\n");
		nFail++;
	}
	if (gj_batch_id_48700() != 48700u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_48700 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_48700() != 48700u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_48700 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch48691_marker, "libcgj-batch48691") != 0 ||
	    strcmp(__libcgj_batch48692_marker, "libcgj-batch48692") != 0 ||
	    strcmp(__libcgj_batch48693_marker, "libcgj-batch48693") != 0 ||
	    strcmp(__libcgj_batch48694_marker, "libcgj-batch48694") != 0 ||
	    strcmp(__libcgj_batch48695_marker, "libcgj-batch48695") != 0 ||
	    strcmp(__libcgj_batch48696_marker, "libcgj-batch48696") != 0 ||
	    strcmp(__libcgj_batch48697_marker, "libcgj-batch48697") != 0 ||
	    strcmp(__libcgj_batch48698_marker, "libcgj-batch48698") != 0 ||
	    strcmp(__libcgj_batch48699_marker, "libcgj-batch48699") != 0 ||
	    strcmp(__libcgj_batch48700_marker, "libcgj-batch48700") != 0) {
		fprintf(stderr, "libcgj: direct m48700 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M48700_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m48700 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m48700 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	nFail += soft_u32(h, "gj_shell_green_48700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_48700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_48700", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_48700", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_48700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_48700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_48700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_48700", 48700u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_48700", 48700u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_48700", 48700u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_48700", 48700u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch48691_marker",
	                     "libcgj-batch48691", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch48692_marker",
	                     "libcgj-batch48692", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch48693_marker",
	                     "libcgj-batch48693", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch48694_marker",
	                     "libcgj-batch48694", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch48695_marker",
	                     "libcgj-batch48695", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch48696_marker",
	                     "libcgj-batch48696", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch48697_marker",
	                     "libcgj-batch48697", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch48698_marker",
	                     "libcgj-batch48698", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch48699_marker",
	                     "libcgj-batch48699", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch48700_marker",
	                     "libcgj-batch48700", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m48700 FAIL path=%s fails=%d checked=%d\n",
