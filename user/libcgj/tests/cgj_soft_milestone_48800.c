 ...
 *
 * Soft host gates for continuum MILESTONE 48800 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_48800.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_48800.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_48800 \
 *     user/libcgj/tests/cgj_soft_milestone_48800.c -ldl
 *   ./build/cgj_soft_milestone_48800
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M48800_DIRECT \
 *     -o build/cgj_soft_milestone_48800_direct \
 *     user/libcgj/tests/cgj_soft_milestone_48800.c \
 *     user/libcgj/src/graph_batch48791.c \
 *     user/libcgj/src/graph_batch48792.c \
 *     user/libcgj/src/graph_batch48793.c \
 *     user/libcgj/src/graph_batch48794.c \
 *     user/libcgj/src/graph_batch48795.c \
 *     user/libcgj/src/graph_batch48796.c \
 *     user/libcgj/src/graph_batch48797.c \
 *     user/libcgj/src/graph_batch48798.c \
 *     user/libcgj/src/graph_batch48799.c \
 *     user/libcgj/src/graph_batch48800.c
 *   ./build/cgj_soft_milestone_48800_direct
 *
 * Expected soft KATs (wave 48791–48800 exclusive continuum markers):
 *   gj_shell_green_48800      → 1
 *   gj_libcgj_green_48800     → 1
 *   gj_bar3_ready_48800       → 0   (bar3 still open)
 *   gj_product_score_48800    → 0   (product score still open)
 *   gj_continuum_ready_48800  → 1
 *   gj_smoke_soft_48800       → 1
 *   gj_dyn_soft_48800         → 1
 *   gj_milestone_tag_48800    → 48800
 *   gj_continuum_wave_48800   → 48800
 *   gj_batch_id_48800         → 48800
 *   gj_graph_milestone_48800  → 48800
 *   __libcgj_batch48791_marker … __libcgj_batch48800_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch48791.c … graph_batch48800.c (and fill
 *     48776–48790 / identity 48751–48775) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 48800.
 *   - libc.map may lag milestone 48800 globals; add 48800 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_48800 == 0).
 *
 ...
 *     continuum/smoke/dyn soft=1; tag/wave/batch/graph = 48800.
 * greppable: libcgj: soft m48800 residual Soft!=product
 * greppable: libcgj: soft m48800 Dual DoD A/B OPEN
 * greppable: libcgj: soft m48800 G-AC-1
 */
...
#ifndef CGJ_SOFT_M48800_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M48800_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_48800(void);
extern uint32_t gj_libcgj_green_48800(void);
extern uint32_t gj_bar3_ready_48800(void);
extern uint32_t gj_product_score_48800(void);
extern uint32_t gj_continuum_ready_48800(void);
extern uint32_t gj_smoke_soft_48800(void);
extern uint32_t gj_dyn_soft_48800(void);
extern uint32_t gj_milestone_tag_48800(void);
extern uint32_t gj_continuum_wave_48800(void);
extern uint32_t gj_batch_id_48800(void);
extern uint32_t gj_graph_milestone_48800(void);
extern const char __libcgj_batch48791_marker[];
extern const char __libcgj_batch48792_marker[];
extern const char __libcgj_batch48793_marker[];
extern const char __libcgj_batch48794_marker[];
extern const char __libcgj_batch48795_marker[];
extern const char __libcgj_batch48796_marker[];
extern const char __libcgj_batch48797_marker[];
extern const char __libcgj_batch48798_marker[];
extern const char __libcgj_batch48799_marker[];
extern const char __libcgj_batch48800_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_48800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_48800 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_48800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_48800 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_48800() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_48800 fail\n");
		nFail++;
	}
	if (gj_product_score_48800() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_48800 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_48800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_48800 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_48800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_48800 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_48800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_48800 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_48800() != 48800u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_48800 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_48800() != 48800u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_48800 fail\n");
		nFail++;
	}
	if (gj_batch_id_48800() != 48800u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_48800 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_48800() != 48800u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_48800 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch48791_marker, "libcgj-batch48791") != 0 ||
	    strcmp(__libcgj_batch48792_marker, "libcgj-batch48792") != 0 ||
	    strcmp(__libcgj_batch48793_marker, "libcgj-batch48793") != 0 ||
	    strcmp(__libcgj_batch48794_marker, "libcgj-batch48794") != 0 ||
	    strcmp(__libcgj_batch48795_marker, "libcgj-batch48795") != 0 ||
	    strcmp(__libcgj_batch48796_marker, "libcgj-batch48796") != 0 ||
	    strcmp(__libcgj_batch48797_marker, "libcgj-batch48797") != 0 ||
	    strcmp(__libcgj_batch48798_marker, "libcgj-batch48798") != 0 ||
	    strcmp(__libcgj_batch48799_marker, "libcgj-batch48799") != 0 ||
	    strcmp(__libcgj_batch48800_marker, "libcgj-batch48800") != 0) {
		fprintf(stderr, "libcgj: direct m48800 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M48800_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m48800 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m48800 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m48800 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m48800 DIRECT PASS (all 11 lamps + markers; "
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m48800 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m48800 SOFT-SKIP (no product SO; "
		       "Soft!=product; Dual DoD A/B OPEN; G-AC-1)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_48800", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_48800", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_48800", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_48800", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_48800", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_48800", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_48800", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_48800", 48800u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_48800", 48800u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_48800", 48800u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_48800", 48800u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch48791_marker",
	                     "libcgj-batch48791", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch48792_marker",
	                     "libcgj-batch48792", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch48793_marker",
	                     "libcgj-batch48793", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch48794_marker",
	                     "libcgj-batch48794", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch48795_marker",
	                     "libcgj-batch48795", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch48796_marker",
	                     "libcgj-batch48796", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch48797_marker",
	                     "libcgj-batch48797", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch48798_marker",
	                     "libcgj-batch48798", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch48799_marker",
	                     "libcgj-batch48799", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch48800_marker",
	                     "libcgj-batch48800", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m48800 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m48800 PASS path=%s checked=%d "
