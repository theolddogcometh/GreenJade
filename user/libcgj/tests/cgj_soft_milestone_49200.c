 ...
 *
 * Soft host gates for continuum MILESTONE 49200 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_49200.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_49200.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_49200 \
 *     user/libcgj/tests/cgj_soft_milestone_49200.c -ldl
 *   ./build/cgj_soft_milestone_49200
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M49200_DIRECT \
 *     -o build/cgj_soft_milestone_49200_direct \
 *     user/libcgj/tests/cgj_soft_milestone_49200.c \
 *     user/libcgj/src/graph_batch49191.c \
 *     user/libcgj/src/graph_batch49192.c \
 *     user/libcgj/src/graph_batch49193.c \
 *     user/libcgj/src/graph_batch49194.c \
 *     user/libcgj/src/graph_batch49195.c \
 *     user/libcgj/src/graph_batch49196.c \
 *     user/libcgj/src/graph_batch49197.c \
 *     user/libcgj/src/graph_batch49198.c \
 *     user/libcgj/src/graph_batch49199.c \
 *     user/libcgj/src/graph_batch49200.c
 *   ./build/cgj_soft_milestone_49200_direct
 *
 * Expected soft KATs (wave 49191–49200 exclusive continuum markers):
 *   gj_shell_green_49200      → 1
 *   gj_libcgj_green_49200     → 1
 *   gj_bar3_ready_49200       → 0   (bar3 still open)
 *   gj_product_score_49200    → 0   (product score still open)
 *   gj_continuum_ready_49200  → 1
 *   gj_smoke_soft_49200       → 1
 *   gj_dyn_soft_49200         → 1
 *   gj_milestone_tag_49200    → 49200
 *   gj_continuum_wave_49200   → 49200
 *   gj_batch_id_49200         → 49200
 *   gj_graph_milestone_49200  → 49200
 *   __libcgj_batch49191_marker … __libcgj_batch49200_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch49191.c … graph_batch49200.c (and fill
 *     49176–49190 / identity 49151–49175) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 49200.
 *   - libc.map may lag milestone 49200 globals; add 49200 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_49200 == 0).
 */
...
#ifndef CGJ_SOFT_M49200_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M49200_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_49200(void);
extern uint32_t gj_libcgj_green_49200(void);
extern uint32_t gj_bar3_ready_49200(void);
extern uint32_t gj_product_score_49200(void);
extern uint32_t gj_continuum_ready_49200(void);
extern uint32_t gj_smoke_soft_49200(void);
extern uint32_t gj_dyn_soft_49200(void);
extern uint32_t gj_milestone_tag_49200(void);
extern uint32_t gj_continuum_wave_49200(void);
extern uint32_t gj_batch_id_49200(void);
extern uint32_t gj_graph_milestone_49200(void);
extern const char __libcgj_batch49191_marker[];
extern const char __libcgj_batch49192_marker[];
extern const char __libcgj_batch49193_marker[];
extern const char __libcgj_batch49194_marker[];
extern const char __libcgj_batch49195_marker[];
extern const char __libcgj_batch49196_marker[];
extern const char __libcgj_batch49197_marker[];
extern const char __libcgj_batch49198_marker[];
extern const char __libcgj_batch49199_marker[];
extern const char __libcgj_batch49200_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_49200() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_49200 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_49200() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_49200 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_49200() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_49200 fail\n");
		nFail++;
	}
	if (gj_product_score_49200() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_49200 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_49200() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_49200 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_49200() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_49200 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_49200() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_49200 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_49200() != 49200u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_49200 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_49200() != 49200u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_49200 fail\n");
		nFail++;
	}
	if (gj_batch_id_49200() != 49200u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_49200 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_49200() != 49200u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_49200 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch49191_marker, "libcgj-batch49191") != 0 ||
	    strcmp(__libcgj_batch49192_marker, "libcgj-batch49192") != 0 ||
	    strcmp(__libcgj_batch49193_marker, "libcgj-batch49193") != 0 ||
	    strcmp(__libcgj_batch49194_marker, "libcgj-batch49194") != 0 ||
	    strcmp(__libcgj_batch49195_marker, "libcgj-batch49195") != 0 ||
	    strcmp(__libcgj_batch49196_marker, "libcgj-batch49196") != 0 ||
	    strcmp(__libcgj_batch49197_marker, "libcgj-batch49197") != 0 ||
	    strcmp(__libcgj_batch49198_marker, "libcgj-batch49198") != 0 ||
	    strcmp(__libcgj_batch49199_marker, "libcgj-batch49199") != 0 ||
	    strcmp(__libcgj_batch49200_marker, "libcgj-batch49200") != 0) {
		fprintf(stderr, "libcgj: direct m49200 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M49200_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m49200 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m49200 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m49200 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m49200 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m49200 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m49200 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_49200", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_49200", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_49200", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_49200", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_49200", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_49200", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_49200", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_49200", 49200u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_49200", 49200u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_49200", 49200u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_49200", 49200u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch49191_marker",
	                     "libcgj-batch49191", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch49192_marker",
	                     "libcgj-batch49192", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch49193_marker",
	                     "libcgj-batch49193", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch49194_marker",
	                     "libcgj-batch49194", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch49195_marker",
	                     "libcgj-batch49195", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch49196_marker",
	                     "libcgj-batch49196", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch49197_marker",
	                     "libcgj-batch49197", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch49198_marker",
	                     "libcgj-batch49198", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch49199_marker",
	                     "libcgj-batch49199", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch49200_marker",
	                     "libcgj-batch49200", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m49200 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m49200 PASS path=%s checked=%d "
