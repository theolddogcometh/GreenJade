 ...
 *
 * Soft host gates for continuum MILESTONE 49300 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_49300.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_49300.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_49300 \
 *     user/libcgj/tests/cgj_soft_milestone_49300.c -ldl
 *   ./build/cgj_soft_milestone_49300
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M49300_DIRECT \
 *     -o build/cgj_soft_milestone_49300_direct \
 *     user/libcgj/tests/cgj_soft_milestone_49300.c \
 *     user/libcgj/src/graph_batch49291.c \
 *     user/libcgj/src/graph_batch49292.c \
 *     user/libcgj/src/graph_batch49293.c \
 *     user/libcgj/src/graph_batch49294.c \
 *     user/libcgj/src/graph_batch49295.c \
 *     user/libcgj/src/graph_batch49296.c \
 *     user/libcgj/src/graph_batch49297.c \
 *     user/libcgj/src/graph_batch49298.c \
 *     user/libcgj/src/graph_batch49299.c \
 *     user/libcgj/src/graph_batch49300.c
 *   ./build/cgj_soft_milestone_49300_direct
 *
 * Expected soft KATs (wave 49291–49300 exclusive continuum markers):
 *   gj_shell_green_49300      → 1
 *   gj_libcgj_green_49300     → 1
 *   gj_bar3_ready_49300       → 0   (bar3 still open)
 *   gj_product_score_49300    → 0   (product score still open)
 *   gj_continuum_ready_49300  → 1
 *   gj_smoke_soft_49300       → 1
 *   gj_dyn_soft_49300         → 1
 *   gj_milestone_tag_49300    → 49300
 *   gj_continuum_wave_49300   → 49300
 *   gj_batch_id_49300         → 49300
 *   gj_graph_milestone_49300  → 49300
 *   __libcgj_batch49291_marker … __libcgj_batch49300_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch49291.c … graph_batch49300.c (and fill
 *     49276–49290 / identity 49251–49275) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 49300.
 *   - libc.map may lag milestone 49300 globals; add 49300 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_49300 == 0).
 *
 ...
 *     continuum/smoke/dyn soft=1; tag/wave/batch/graph = 49300.
 * greppable: libcgj: soft m49300 residual Soft!=product
 * greppable: libcgj: soft m49300 Dual DoD A/B OPEN
 * greppable: libcgj: soft m49300 G-AC-1
 */
...
#ifndef CGJ_SOFT_M49300_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M49300_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_49300(void);
extern uint32_t gj_libcgj_green_49300(void);
extern uint32_t gj_bar3_ready_49300(void);
extern uint32_t gj_product_score_49300(void);
extern uint32_t gj_continuum_ready_49300(void);
extern uint32_t gj_smoke_soft_49300(void);
extern uint32_t gj_dyn_soft_49300(void);
extern uint32_t gj_milestone_tag_49300(void);
extern uint32_t gj_continuum_wave_49300(void);
extern uint32_t gj_batch_id_49300(void);
extern uint32_t gj_graph_milestone_49300(void);
extern const char __libcgj_batch49291_marker[];
extern const char __libcgj_batch49292_marker[];
extern const char __libcgj_batch49293_marker[];
extern const char __libcgj_batch49294_marker[];
extern const char __libcgj_batch49295_marker[];
extern const char __libcgj_batch49296_marker[];
extern const char __libcgj_batch49297_marker[];
extern const char __libcgj_batch49298_marker[];
extern const char __libcgj_batch49299_marker[];
extern const char __libcgj_batch49300_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_49300() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_49300 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_49300() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_49300 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_49300() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_49300 fail\n");
		nFail++;
	}
	if (gj_product_score_49300() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_49300 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_49300() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_49300 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_49300() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_49300 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_49300() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_49300 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_49300() != 49300u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_49300 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_49300() != 49300u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_49300 fail\n");
		nFail++;
	}
	if (gj_batch_id_49300() != 49300u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_49300 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_49300() != 49300u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_49300 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch49291_marker, "libcgj-batch49291") != 0 ||
	    strcmp(__libcgj_batch49292_marker, "libcgj-batch49292") != 0 ||
	    strcmp(__libcgj_batch49293_marker, "libcgj-batch49293") != 0 ||
	    strcmp(__libcgj_batch49294_marker, "libcgj-batch49294") != 0 ||
	    strcmp(__libcgj_batch49295_marker, "libcgj-batch49295") != 0 ||
	    strcmp(__libcgj_batch49296_marker, "libcgj-batch49296") != 0 ||
	    strcmp(__libcgj_batch49297_marker, "libcgj-batch49297") != 0 ||
	    strcmp(__libcgj_batch49298_marker, "libcgj-batch49298") != 0 ||
	    strcmp(__libcgj_batch49299_marker, "libcgj-batch49299") != 0 ||
	    strcmp(__libcgj_batch49300_marker, "libcgj-batch49300") != 0) {
		fprintf(stderr, "libcgj: direct m49300 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M49300_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m49300 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m49300 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m49300 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m49300 DIRECT PASS (all 11 lamps + markers; "
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m49300 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m49300 SOFT-SKIP (no product SO; "
		       "Soft!=product; Dual DoD A/B OPEN; G-AC-1)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_49300", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_49300", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_49300", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_49300", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_49300", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_49300", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_49300", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_49300", 49300u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_49300", 49300u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_49300", 49300u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_49300", 49300u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch49291_marker",
	                     "libcgj-batch49291", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch49292_marker",
	                     "libcgj-batch49292", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch49293_marker",
	                     "libcgj-batch49293", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch49294_marker",
	                     "libcgj-batch49294", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch49295_marker",
	                     "libcgj-batch49295", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch49296_marker",
	                     "libcgj-batch49296", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch49297_marker",
	                     "libcgj-batch49297", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch49298_marker",
	                     "libcgj-batch49298", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch49299_marker",
	                     "libcgj-batch49299", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch49300_marker",
	                     "libcgj-batch49300", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m49300 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m49300 PASS path=%s checked=%d "
