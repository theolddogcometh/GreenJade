 ...
 *
 * Soft host gates for continuum MILESTONE 49600 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_49600.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_49600.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_49600 \
 *     user/libcgj/tests/cgj_soft_milestone_49600.c -ldl
 *   ./build/cgj_soft_milestone_49600
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M49600_DIRECT \
 *     -o build/cgj_soft_milestone_49600_direct \
 *     user/libcgj/tests/cgj_soft_milestone_49600.c \
 *     user/libcgj/src/graph_batch49591.c \
 *     user/libcgj/src/graph_batch49592.c \
 *     user/libcgj/src/graph_batch49593.c \
 *     user/libcgj/src/graph_batch49594.c \
 *     user/libcgj/src/graph_batch49595.c \
 *     user/libcgj/src/graph_batch49596.c \
 *     user/libcgj/src/graph_batch49597.c \
 *     user/libcgj/src/graph_batch49598.c \
 *     user/libcgj/src/graph_batch49599.c \
 *     user/libcgj/src/graph_batch49600.c
 *   ./build/cgj_soft_milestone_49600_direct
 *
 * Expected soft KATs (wave 49591–49600 exclusive continuum markers):
 *   gj_shell_green_49600      → 1
 *   gj_libcgj_green_49600     → 1
 *   gj_bar3_ready_49600       → 0   (bar3 still open)
 *   gj_product_score_49600    → 0   (product score still open)
 *   gj_continuum_ready_49600  → 1
 *   gj_smoke_soft_49600       → 1
 *   gj_dyn_soft_49600         → 1
 *   gj_milestone_tag_49600    → 49600
 *   gj_continuum_wave_49600   → 49600
 *   gj_batch_id_49600         → 49600
 *   gj_graph_milestone_49600  → 49600
 *   __libcgj_batch49591_marker … __libcgj_batch49600_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch49591.c … graph_batch49600.c (and fill
 *     49576–49590 / identity 49551–49575) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 49600.
 *   - libc.map may lag milestone 49600 globals; add 49600 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_49600 == 0).
 *
 ...
 *     continuum/smoke/dyn soft=1; tag/wave/batch/graph = 49600.
 * greppable: libcgj: soft m49600 residual Soft!=product
 * greppable: libcgj: soft m49600 Dual DoD A/B OPEN
 * greppable: libcgj: soft m49600 G-AC-1
 */
...
#ifndef CGJ_SOFT_M49600_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M49600_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_49600(void);
extern uint32_t gj_libcgj_green_49600(void);
extern uint32_t gj_bar3_ready_49600(void);
extern uint32_t gj_product_score_49600(void);
extern uint32_t gj_continuum_ready_49600(void);
extern uint32_t gj_smoke_soft_49600(void);
extern uint32_t gj_dyn_soft_49600(void);
extern uint32_t gj_milestone_tag_49600(void);
extern uint32_t gj_continuum_wave_49600(void);
extern uint32_t gj_batch_id_49600(void);
extern uint32_t gj_graph_milestone_49600(void);
extern const char __libcgj_batch49591_marker[];
extern const char __libcgj_batch49592_marker[];
extern const char __libcgj_batch49593_marker[];
extern const char __libcgj_batch49594_marker[];
extern const char __libcgj_batch49595_marker[];
extern const char __libcgj_batch49596_marker[];
extern const char __libcgj_batch49597_marker[];
extern const char __libcgj_batch49598_marker[];
extern const char __libcgj_batch49599_marker[];
extern const char __libcgj_batch49600_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_49600() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_49600 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_49600() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_49600 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_49600() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_49600 fail\n");
		nFail++;
	}
	if (gj_product_score_49600() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_49600 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_49600() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_49600 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_49600() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_49600 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_49600() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_49600 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_49600() != 49600u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_49600 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_49600() != 49600u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_49600 fail\n");
		nFail++;
	}
	if (gj_batch_id_49600() != 49600u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_49600 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_49600() != 49600u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_49600 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch49591_marker, "libcgj-batch49591") != 0 ||
	    strcmp(__libcgj_batch49592_marker, "libcgj-batch49592") != 0 ||
	    strcmp(__libcgj_batch49593_marker, "libcgj-batch49593") != 0 ||
	    strcmp(__libcgj_batch49594_marker, "libcgj-batch49594") != 0 ||
	    strcmp(__libcgj_batch49595_marker, "libcgj-batch49595") != 0 ||
	    strcmp(__libcgj_batch49596_marker, "libcgj-batch49596") != 0 ||
	    strcmp(__libcgj_batch49597_marker, "libcgj-batch49597") != 0 ||
	    strcmp(__libcgj_batch49598_marker, "libcgj-batch49598") != 0 ||
	    strcmp(__libcgj_batch49599_marker, "libcgj-batch49599") != 0 ||
	    strcmp(__libcgj_batch49600_marker, "libcgj-batch49600") != 0) {
		fprintf(stderr, "libcgj: direct m49600 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M49600_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m49600 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m49600 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m49600 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m49600 DIRECT PASS (all 11 lamps + markers; "
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m49600 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m49600 SOFT-SKIP (no product SO; "
		       "Soft!=product; Dual DoD A/B OPEN; G-AC-1)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_49600", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_49600", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_49600", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_49600", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_49600", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_49600", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_49600", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_49600", 49600u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_49600", 49600u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_49600", 49600u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_49600", 49600u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch49591_marker",
	                     "libcgj-batch49591", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch49592_marker",
	                     "libcgj-batch49592", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch49593_marker",
	                     "libcgj-batch49593", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch49594_marker",
	                     "libcgj-batch49594", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch49595_marker",
	                     "libcgj-batch49595", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch49596_marker",
	                     "libcgj-batch49596", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch49597_marker",
	                     "libcgj-batch49597", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch49598_marker",
	                     "libcgj-batch49598", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch49599_marker",
	                     "libcgj-batch49599", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch49600_marker",
	                     "libcgj-batch49600", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m49600 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m49600 PASS path=%s checked=%d "
