 ...
 *
 * Soft host gates for continuum MILESTONE 50200 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_50200.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_50200.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_50200 \
 *     user/libcgj/tests/cgj_soft_milestone_50200.c -ldl
 *   ./build/cgj_soft_milestone_50200
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M50200_DIRECT \
 *     -o build/cgj_soft_milestone_50200_direct \
 *     user/libcgj/tests/cgj_soft_milestone_50200.c \
 *     user/libcgj/src/graph_batch50191.c \
 *     user/libcgj/src/graph_batch50192.c \
 *     user/libcgj/src/graph_batch50193.c \
 *     user/libcgj/src/graph_batch50194.c \
 *     user/libcgj/src/graph_batch50195.c \
 *     user/libcgj/src/graph_batch50196.c \
 *     user/libcgj/src/graph_batch50197.c \
 *     user/libcgj/src/graph_batch50198.c \
 *     user/libcgj/src/graph_batch50199.c \
 *     user/libcgj/src/graph_batch50200.c
 *   ./build/cgj_soft_milestone_50200_direct
 *
 * Expected soft KATs (wave 50191–50200 exclusive continuum markers):
 *   gj_shell_green_50200      → 1
 *   gj_libcgj_green_50200     → 1
 *   gj_bar3_ready_50200       → 0   (bar3 still open)
 *   gj_product_score_50200    → 0   (product score still open)
 *   gj_continuum_ready_50200  → 1
 *   gj_smoke_soft_50200       → 1
 *   gj_dyn_soft_50200         → 1
 *   gj_milestone_tag_50200    → 50200
 *   gj_continuum_wave_50200   → 50200
 *   gj_batch_id_50200         → 50200
 *   gj_graph_milestone_50200  → 50200
 *   __libcgj_batch50191_marker … __libcgj_batch50200_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch50191.c … graph_batch50200.c (and fill
 *     50176–50190 / identity 50151–50175) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 50200.
 *   - libc.map may lag milestone 50200 globals; add 50200 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_50200 == 0).
 */
...
#ifndef CGJ_SOFT_M50200_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M50200_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_50200(void);
extern uint32_t gj_libcgj_green_50200(void);
extern uint32_t gj_bar3_ready_50200(void);
extern uint32_t gj_product_score_50200(void);
extern uint32_t gj_continuum_ready_50200(void);
extern uint32_t gj_smoke_soft_50200(void);
extern uint32_t gj_dyn_soft_50200(void);
extern uint32_t gj_milestone_tag_50200(void);
extern uint32_t gj_continuum_wave_50200(void);
extern uint32_t gj_batch_id_50200(void);
extern uint32_t gj_graph_milestone_50200(void);
extern const char __libcgj_batch50191_marker[];
extern const char __libcgj_batch50192_marker[];
extern const char __libcgj_batch50193_marker[];
extern const char __libcgj_batch50194_marker[];
extern const char __libcgj_batch50195_marker[];
extern const char __libcgj_batch50196_marker[];
extern const char __libcgj_batch50197_marker[];
extern const char __libcgj_batch50198_marker[];
extern const char __libcgj_batch50199_marker[];
extern const char __libcgj_batch50200_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_50200() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_50200 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_50200() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_50200 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_50200() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_50200 fail\n");
		nFail++;
	}
	if (gj_product_score_50200() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_50200 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_50200() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_50200 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_50200() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_50200 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_50200() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_50200 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_50200() != 50200u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_50200 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_50200() != 50200u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_50200 fail\n");
		nFail++;
	}
	if (gj_batch_id_50200() != 50200u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_50200 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_50200() != 50200u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_50200 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch50191_marker, "libcgj-batch50191") != 0 ||
	    strcmp(__libcgj_batch50192_marker, "libcgj-batch50192") != 0 ||
	    strcmp(__libcgj_batch50193_marker, "libcgj-batch50193") != 0 ||
	    strcmp(__libcgj_batch50194_marker, "libcgj-batch50194") != 0 ||
	    strcmp(__libcgj_batch50195_marker, "libcgj-batch50195") != 0 ||
	    strcmp(__libcgj_batch50196_marker, "libcgj-batch50196") != 0 ||
	    strcmp(__libcgj_batch50197_marker, "libcgj-batch50197") != 0 ||
	    strcmp(__libcgj_batch50198_marker, "libcgj-batch50198") != 0 ||
	    strcmp(__libcgj_batch50199_marker, "libcgj-batch50199") != 0 ||
	    strcmp(__libcgj_batch50200_marker, "libcgj-batch50200") != 0) {
		fprintf(stderr, "libcgj: direct m50200 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M50200_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m50200 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m50200 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m50200 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m50200 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m50200 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m50200 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_50200", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_50200", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_50200", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_50200", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_50200", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_50200", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_50200", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_50200", 50200u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_50200", 50200u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_50200", 50200u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_50200", 50200u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch50191_marker",
	                     "libcgj-batch50191", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch50192_marker",
	                     "libcgj-batch50192", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch50193_marker",
	                     "libcgj-batch50193", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch50194_marker",
	                     "libcgj-batch50194", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch50195_marker",
	                     "libcgj-batch50195", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch50196_marker",
	                     "libcgj-batch50196", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch50197_marker",
	                     "libcgj-batch50197", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch50198_marker",
	                     "libcgj-batch50198", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch50199_marker",
	                     "libcgj-batch50199", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch50200_marker",
	                     "libcgj-batch50200", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m50200 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m50200 PASS path=%s checked=%d "
