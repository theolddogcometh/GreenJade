 ...
 *
 * Soft host gates for continuum MILESTONE 50700 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_50700.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_50700.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_50700 \
 *     user/libcgj/tests/cgj_soft_milestone_50700.c -ldl
 *   ./build/cgj_soft_milestone_50700
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M50700_DIRECT \
 *     -o build/cgj_soft_milestone_50700_direct \
 *     user/libcgj/tests/cgj_soft_milestone_50700.c \
 *     user/libcgj/src/graph_batch50691.c \
 *     user/libcgj/src/graph_batch50692.c \
 *     user/libcgj/src/graph_batch50693.c \
 *     user/libcgj/src/graph_batch50694.c \
 *     user/libcgj/src/graph_batch50695.c \
 *     user/libcgj/src/graph_batch50696.c \
 *     user/libcgj/src/graph_batch50697.c \
 *     user/libcgj/src/graph_batch50698.c \
 *     user/libcgj/src/graph_batch50699.c \
 *     user/libcgj/src/graph_batch50700.c
 *   ./build/cgj_soft_milestone_50700_direct
 *
 * Expected soft KATs (wave 50691–50700 exclusive continuum markers):
 *   gj_shell_green_50700      → 1
 *   gj_libcgj_green_50700     → 1
 *   gj_bar3_ready_50700       → 0   (bar3 still open)
 *   gj_product_score_50700    → 0   (product score still open)
 *   gj_continuum_ready_50700  → 1
 *   gj_smoke_soft_50700       → 1
 *   gj_dyn_soft_50700         → 1
 *   gj_milestone_tag_50700    → 50700
 *   gj_continuum_wave_50700   → 50700
 *   gj_batch_id_50700         → 50700
 *   gj_graph_milestone_50700  → 50700
 *   __libcgj_batch50691_marker … __libcgj_batch50700_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch50691.c … graph_batch50700.c (and fill
 *     50676–50690 / identity 50651–50675) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 50700.
 *   - libc.map may lag milestone 50700 globals; add 50700 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_50700 == 0).
 */
...
#ifndef CGJ_SOFT_M50700_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M50700_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_50700(void);
extern uint32_t gj_libcgj_green_50700(void);
extern uint32_t gj_bar3_ready_50700(void);
extern uint32_t gj_product_score_50700(void);
extern uint32_t gj_continuum_ready_50700(void);
extern uint32_t gj_smoke_soft_50700(void);
extern uint32_t gj_dyn_soft_50700(void);
extern uint32_t gj_milestone_tag_50700(void);
extern uint32_t gj_continuum_wave_50700(void);
extern uint32_t gj_batch_id_50700(void);
extern uint32_t gj_graph_milestone_50700(void);
extern const char __libcgj_batch50691_marker[];
extern const char __libcgj_batch50692_marker[];
extern const char __libcgj_batch50693_marker[];
extern const char __libcgj_batch50694_marker[];
extern const char __libcgj_batch50695_marker[];
extern const char __libcgj_batch50696_marker[];
extern const char __libcgj_batch50697_marker[];
extern const char __libcgj_batch50698_marker[];
extern const char __libcgj_batch50699_marker[];
extern const char __libcgj_batch50700_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_50700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_50700 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_50700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_50700 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_50700() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_50700 fail\n");
		nFail++;
	}
	if (gj_product_score_50700() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_50700 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_50700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_50700 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_50700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_50700 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_50700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_50700 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_50700() != 50700u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_50700 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_50700() != 50700u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_50700 fail\n");
		nFail++;
	}
	if (gj_batch_id_50700() != 50700u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_50700 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_50700() != 50700u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_50700 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch50691_marker, "libcgj-batch50691") != 0 ||
	    strcmp(__libcgj_batch50692_marker, "libcgj-batch50692") != 0 ||
	    strcmp(__libcgj_batch50693_marker, "libcgj-batch50693") != 0 ||
	    strcmp(__libcgj_batch50694_marker, "libcgj-batch50694") != 0 ||
	    strcmp(__libcgj_batch50695_marker, "libcgj-batch50695") != 0 ||
	    strcmp(__libcgj_batch50696_marker, "libcgj-batch50696") != 0 ||
	    strcmp(__libcgj_batch50697_marker, "libcgj-batch50697") != 0 ||
	    strcmp(__libcgj_batch50698_marker, "libcgj-batch50698") != 0 ||
	    strcmp(__libcgj_batch50699_marker, "libcgj-batch50699") != 0 ||
	    strcmp(__libcgj_batch50700_marker, "libcgj-batch50700") != 0) {
		fprintf(stderr, "libcgj: direct m50700 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M50700_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m50700 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m50700 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m50700 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m50700 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m50700 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m50700 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_50700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_50700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_50700", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_50700", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_50700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_50700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_50700", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_50700", 50700u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_50700", 50700u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_50700", 50700u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_50700", 50700u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch50691_marker",
	                     "libcgj-batch50691", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch50692_marker",
	                     "libcgj-batch50692", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch50693_marker",
	                     "libcgj-batch50693", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch50694_marker",
	                     "libcgj-batch50694", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch50695_marker",
	                     "libcgj-batch50695", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch50696_marker",
	                     "libcgj-batch50696", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch50697_marker",
	                     "libcgj-batch50697", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch50698_marker",
	                     "libcgj-batch50698", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch50699_marker",
	                     "libcgj-batch50699", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch50700_marker",
	                     "libcgj-batch50700", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m50700 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m50700 PASS path=%s checked=%d "
