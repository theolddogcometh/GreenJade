 ...
 *
 * Soft host gates for continuum MILESTONE 51000 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_51000.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_51000.txt
 *
 * Test harness note:
 ...
 *     cgj_soft_milestone_49600.c /
 *     cgj_soft_milestone_50000.c /
 *     cgj_soft_milestone_50100.c / cgj_soft_milestone_50200.c /
 *     cgj_soft_milestone_50300.c / cgj_soft_milestone_50400.c /
 *     cgj_soft_milestone_50800.c / cgj_soft_milestone_50900.c /
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_51000 \
 *     user/libcgj/tests/cgj_soft_milestone_51000.c -ldl
 *   ./build/cgj_soft_milestone_51000
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M51000_DIRECT \
 *     -o build/cgj_soft_milestone_51000_direct \
 *     user/libcgj/tests/cgj_soft_milestone_51000.c \
 *     user/libcgj/src/graph_batch50991.c \
 *     user/libcgj/src/graph_batch50992.c \
 *     user/libcgj/src/graph_batch50993.c \
 *     user/libcgj/src/graph_batch50994.c \
 *     user/libcgj/src/graph_batch50995.c \
 *     user/libcgj/src/graph_batch50996.c \
 *     user/libcgj/src/graph_batch50997.c \
 *     user/libcgj/src/graph_batch50998.c \
 *     user/libcgj/src/graph_batch50999.c \
 *     user/libcgj/src/graph_batch51000.c
 *   ./build/cgj_soft_milestone_51000_direct
 *
 * Expected soft KATs (wave 50991–51000 exclusive continuum markers):
 *   gj_shell_green_51000      → 1
 *   gj_libcgj_green_51000     → 1
 *   gj_bar3_ready_51000       → 0   (bar3 still open)
 *   gj_product_score_51000    → 0   (product score still open)
 *   gj_continuum_ready_51000  → 1
 *   gj_smoke_soft_51000       → 1
 *   gj_dyn_soft_51000         → 1
 *   gj_milestone_tag_51000    → 51000
 *   gj_continuum_wave_51000   → 51000
 *   gj_batch_id_51000         → 51000
 *   gj_graph_milestone_51000  → 51000
 *   __libcgj_batch50991_marker … __libcgj_batch51000_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch50991.c … graph_batch51000.c (and fill
 *     50976–50990 / identity 50951–50975) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 51000.
 *   - libc.map may lag milestone 51000 globals; add 51000 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_51000 == 0).
 *   - Pure soft ≠ product.
 */
...
#ifndef CGJ_SOFT_M51000_DIRECT
#include <dlfcn.h>
#endif

#ifdef CGJ_SOFT_M51000_DIRECT
/* Direct-link path: call product pure markers without dlopen. */
extern uint32_t gj_shell_green_51000(void);
extern uint32_t gj_libcgj_green_51000(void);
extern uint32_t gj_bar3_ready_51000(void);
extern uint32_t gj_product_score_51000(void);
extern uint32_t gj_continuum_ready_51000(void);
extern uint32_t gj_smoke_soft_51000(void);
extern uint32_t gj_dyn_soft_51000(void);
extern uint32_t gj_milestone_tag_51000(void);
extern uint32_t gj_continuum_wave_51000(void);
extern uint32_t gj_batch_id_51000(void);
extern uint32_t gj_graph_milestone_51000(void);
extern const char __libcgj_batch50991_marker[];
extern const char __libcgj_batch50992_marker[];
extern const char __libcgj_batch50993_marker[];
extern const char __libcgj_batch50994_marker[];
extern const char __libcgj_batch50995_marker[];
extern const char __libcgj_batch50996_marker[];
extern const char __libcgj_batch50997_marker[];
extern const char __libcgj_batch50998_marker[];
extern const char __libcgj_batch50999_marker[];
extern const char __libcgj_batch51000_marker[];
...
direct_check(void)
{
	...
	if (gj_shell_green_51000() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_51000 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_51000() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_51000 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_51000() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_51000 fail\n");
		nFail++;
	}
	if (gj_product_score_51000() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_51000 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_51000() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_51000 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_51000() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_51000 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_51000() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_51000 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_51000() != 51000u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_51000 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_51000() != 51000u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_51000 fail\n");
		nFail++;
	}
	if (gj_batch_id_51000() != 51000u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_51000 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_51000() != 51000u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_51000 fail\n");
		nFail++;
	}
	if (strcmp(__libcgj_batch50991_marker, "libcgj-batch50991") != 0 ||
	    strcmp(__libcgj_batch50992_marker, "libcgj-batch50992") != 0 ||
	    strcmp(__libcgj_batch50993_marker, "libcgj-batch50993") != 0 ||
	    strcmp(__libcgj_batch50994_marker, "libcgj-batch50994") != 0 ||
	    strcmp(__libcgj_batch50995_marker, "libcgj-batch50995") != 0 ||
	    strcmp(__libcgj_batch50996_marker, "libcgj-batch50996") != 0 ||
	    strcmp(__libcgj_batch50997_marker, "libcgj-batch50997") != 0 ||
	    strcmp(__libcgj_batch50998_marker, "libcgj-batch50998") != 0 ||
	    strcmp(__libcgj_batch50999_marker, "libcgj-batch50999") != 0 ||
	    strcmp(__libcgj_batch51000_marker, "libcgj-batch51000") != 0) {
		fprintf(stderr, "libcgj: direct m51000 marker string fail\n");
		nFail++;
	}
	return nFail;
}
#else /* !CGJ_SOFT_M51000_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m51000 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m51000 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m51000 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	printf("libcgj: soft m51000 DIRECT PASS (all 11 lamps + markers)\n");
	return 0;
#else
	void *h = NULL;
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m51000 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m51000 SOFT-SKIP (no product SO)\n");
		return 0;
	}

	/* Continuum soft gates: skip if missing, hard fail if wrong. */
	nFail += soft_u32(h, "gj_shell_green_51000", 1u, &nChecked);
	nFail += soft_u32(h, "gj_libcgj_green_51000", 1u, &nChecked);
	nFail += soft_u32(h, "gj_bar3_ready_51000", 0u, &nChecked);
	nFail += soft_u32(h, "gj_product_score_51000", 0u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_ready_51000", 1u, &nChecked);
	nFail += soft_u32(h, "gj_smoke_soft_51000", 1u, &nChecked);
	nFail += soft_u32(h, "gj_dyn_soft_51000", 1u, &nChecked);
	nFail += soft_u32(h, "gj_milestone_tag_51000", 51000u, &nChecked);
	nFail += soft_u32(h, "gj_continuum_wave_51000", 51000u, &nChecked);
	nFail += soft_u32(h, "gj_batch_id_51000", 51000u, &nChecked);
	nFail += soft_u32(h, "gj_graph_milestone_51000", 51000u, &nChecked);

	nFail += soft_marker(h, "__libcgj_batch50991_marker",
	                     "libcgj-batch50991", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch50992_marker",
	                     "libcgj-batch50992", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch50993_marker",
	                     "libcgj-batch50993", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch50994_marker",
	                     "libcgj-batch50994", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch50995_marker",
	                     "libcgj-batch50995", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch50996_marker",
	                     "libcgj-batch50996", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch50997_marker",
	                     "libcgj-batch50997", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch50998_marker",
	                     "libcgj-batch50998", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch50999_marker",
	                     "libcgj-batch50999", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch51000_marker",
	                     "libcgj-batch51000", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m51000 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m51000 PASS path=%s checked=%d "
