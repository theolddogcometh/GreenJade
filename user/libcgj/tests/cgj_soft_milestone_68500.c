 ...
 *
 * Soft host gates for continuum MILESTONE 68500 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_68500.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_68500.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_68500 \
 *     user/libcgj/tests/cgj_soft_milestone_68500.c -ldl
 *   ./build/cgj_soft_milestone_68500
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M68500_DIRECT \
 *     -o build/cgj_soft_milestone_68500_direct \
 *     user/libcgj/tests/cgj_soft_milestone_68500.c \
 *     user/libcgj/src/graph_batch68491.c \
 *     user/libcgj/src/graph_batch68492.c \
 *     user/libcgj/src/graph_batch68493.c \
 *     user/libcgj/src/graph_batch68494.c \
 *     user/libcgj/src/graph_batch68495.c \
 *     user/libcgj/src/graph_batch68496.c \
 *     user/libcgj/src/graph_batch68497.c \
 *     user/libcgj/src/graph_batch68498.c \
 *     user/libcgj/src/graph_batch68499.c \
 *     user/libcgj/src/graph_batch68500.c
 *   ./build/cgj_soft_milestone_68500_direct
 *
 * Expected soft KATs (wave 68491–68500 exclusive continuum markers):
 *   gj_shell_green_68500      → 1
 *   gj_libcgj_green_68500     → 1
 *   gj_bar3_ready_68500       → 0   (bar3 still open)
 *   gj_product_score_68500    → 0   (product score still open)
 *   gj_continuum_ready_68500  → 1
 *   gj_smoke_soft_68500       → 1
 *   gj_dyn_soft_68500         → 1
 *   gj_milestone_tag_68500    → 68500
 *   gj_continuum_wave_68500   → 68500
 *   gj_batch_id_68500         → 68500
 *   gj_graph_milestone_68500  → 68500
 *   __libcgj_batch68491_marker … __libcgj_batch68500_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch68491.c … graph_batch68500.c (and fill
 *     25776–25790 / identity 25751–25775) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 68500.
 *   - libc.map may lag milestone 68500 globals; add 68500 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_68500 == 0).
 */
...
direct_check(void)
{
	...
	if (gj_shell_green_68500() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_68500 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_68500() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_68500 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_68500() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_68500 fail\n");
		nFail++;
	}
	if (gj_product_score_68500() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_68500 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_68500() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_68500 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_68500() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_68500 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_68500() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_68500 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_68500() != 68500u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_68500 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_68500() != 68500u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_68500 fail\n");
		nFail++;
	}
	if (gj_batch_id_68500() != 68500u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_68500 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_68500() != 68500u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_68500 fail\n");
		nFail++;
	}
...
#else /* !CGJ_SOFT_M68500_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m68500 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m68500 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m68500 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m68500 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m68500 SOFT-SKIP (no product SO)\n");
		return 0;
	}
	...
	nFail += soft_marker(h, "__libcgj_batch68491_marker",
	                     "libcgj-batch68491", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68492_marker",
	                     "libcgj-batch68492", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68493_marker",
	                     "libcgj-batch68493", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68494_marker",
	                     "libcgj-batch68494", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68495_marker",
	                     "libcgj-batch68495", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68496_marker",
	                     "libcgj-batch68496", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68497_marker",
	                     "libcgj-batch68497", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68498_marker",
	                     "libcgj-batch68498", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68499_marker",
	                     "libcgj-batch68499", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68500_marker",
	                     "libcgj-batch68500", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m68500 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m68500 PASS path=%s checked=%d "
