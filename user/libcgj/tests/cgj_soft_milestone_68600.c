 ...
 *
 * Soft host gates for continuum MILESTONE 68600 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_68600.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_68600.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_68600 \
 *     user/libcgj/tests/cgj_soft_milestone_68600.c -ldl
 *   ./build/cgj_soft_milestone_68600
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M68600_DIRECT \
 *     -o build/cgj_soft_milestone_68600_direct \
 *     user/libcgj/tests/cgj_soft_milestone_68600.c \
 *     user/libcgj/src/graph_batch68591.c \
 *     user/libcgj/src/graph_batch68592.c \
 *     user/libcgj/src/graph_batch68593.c \
 *     user/libcgj/src/graph_batch68594.c \
 *     user/libcgj/src/graph_batch68595.c \
 *     user/libcgj/src/graph_batch68596.c \
 *     user/libcgj/src/graph_batch68597.c \
 *     user/libcgj/src/graph_batch68598.c \
 *     user/libcgj/src/graph_batch68599.c \
 *     user/libcgj/src/graph_batch68600.c
 *   ./build/cgj_soft_milestone_68600_direct
 *
 * Expected soft KATs (wave 68591–68600 exclusive continuum markers):
 *   gj_shell_green_68600      → 1
 *   gj_libcgj_green_68600     → 1
 *   gj_bar3_ready_68600       → 0   (bar3 still open)
 *   gj_product_score_68600    → 0   (product score still open)
 *   gj_continuum_ready_68600  → 1
 *   gj_smoke_soft_68600       → 1
 *   gj_dyn_soft_68600         → 1
 *   gj_milestone_tag_68600    → 68600
 *   gj_continuum_wave_68600   → 68600
 *   gj_batch_id_68600         → 68600
 *   gj_graph_milestone_68600  → 68600
 *   __libcgj_batch68591_marker … __libcgj_batch68600_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch68591.c … graph_batch68600.c (and fill
 *     25776–25790 / identity 25751–25775) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 68600.
 *   - libc.map may lag milestone 68600 globals; add 68600 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_68600 == 0).
 */
...
direct_check(void)
{
	...
	if (gj_shell_green_68600() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_68600 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_68600() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_68600 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_68600() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_68600 fail\n");
		nFail++;
	}
	if (gj_product_score_68600() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_68600 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_68600() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_68600 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_68600() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_68600 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_68600() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_68600 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_68600() != 68600u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_68600 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_68600() != 68600u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_68600 fail\n");
		nFail++;
	}
	if (gj_batch_id_68600() != 68600u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_68600 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_68600() != 68600u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_68600 fail\n");
		nFail++;
	}
...
#else /* !CGJ_SOFT_M68600_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m68600 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m68600 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m68600 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m68600 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m68600 SOFT-SKIP (no product SO)\n");
		return 0;
	}
	...
	nFail += soft_marker(h, "__libcgj_batch68591_marker",
	                     "libcgj-batch68591", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68592_marker",
	                     "libcgj-batch68592", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68593_marker",
	                     "libcgj-batch68593", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68594_marker",
	                     "libcgj-batch68594", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68595_marker",
	                     "libcgj-batch68595", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68596_marker",
	                     "libcgj-batch68596", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68597_marker",
	                     "libcgj-batch68597", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68598_marker",
	                     "libcgj-batch68598", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68599_marker",
	                     "libcgj-batch68599", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68600_marker",
	                     "libcgj-batch68600", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m68600 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m68600 PASS path=%s checked=%d "
