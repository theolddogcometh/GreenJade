 ...
 *
 * Soft host gates for continuum MILESTONE 68700 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_68700.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_68700.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_68700 \
 *     user/libcgj/tests/cgj_soft_milestone_68700.c -ldl
 *   ./build/cgj_soft_milestone_68700
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M68700_DIRECT \
 *     -o build/cgj_soft_milestone_68700_direct \
 *     user/libcgj/tests/cgj_soft_milestone_68700.c \
 *     user/libcgj/src/graph_batch68691.c \
 *     user/libcgj/src/graph_batch68692.c \
 *     user/libcgj/src/graph_batch68693.c \
 *     user/libcgj/src/graph_batch68694.c \
 *     user/libcgj/src/graph_batch68695.c \
 *     user/libcgj/src/graph_batch68696.c \
 *     user/libcgj/src/graph_batch68697.c \
 *     user/libcgj/src/graph_batch68698.c \
 *     user/libcgj/src/graph_batch68699.c \
 *     user/libcgj/src/graph_batch68700.c
 *   ./build/cgj_soft_milestone_68700_direct
 *
 * Expected soft KATs (wave 68691–68700 exclusive continuum markers):
 *   gj_shell_green_68700      → 1
 *   gj_libcgj_green_68700     → 1
 *   gj_bar3_ready_68700       → 0   (bar3 still open)
 *   gj_product_score_68700    → 0   (product score still open)
 *   gj_continuum_ready_68700  → 1
 *   gj_smoke_soft_68700       → 1
 *   gj_dyn_soft_68700         → 1
 *   gj_milestone_tag_68700    → 68700
 *   gj_continuum_wave_68700   → 68700
 *   gj_batch_id_68700         → 68700
 *   gj_graph_milestone_68700  → 68700
 *   __libcgj_batch68691_marker … __libcgj_batch68700_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch68691.c … graph_batch68700.c (and fill
 *     25776–25790 / identity 25751–25775) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 68700.
 *   - libc.map may lag milestone 68700 globals; add 68700 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_68700 == 0).
 */
...
direct_check(void)
{
	...
	if (gj_shell_green_68700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_68700 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_68700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_68700 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_68700() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_68700 fail\n");
		nFail++;
	}
	if (gj_product_score_68700() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_68700 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_68700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_68700 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_68700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_68700 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_68700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_68700 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_68700() != 68700u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_68700 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_68700() != 68700u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_68700 fail\n");
		nFail++;
	}
	if (gj_batch_id_68700() != 68700u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_68700 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_68700() != 68700u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_68700 fail\n");
		nFail++;
	}
...
#else /* !CGJ_SOFT_M68700_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m68700 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m68700 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m68700 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m68700 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m68700 SOFT-SKIP (no product SO)\n");
		return 0;
	}
	...
	nFail += soft_marker(h, "__libcgj_batch68691_marker",
	                     "libcgj-batch68691", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68692_marker",
	                     "libcgj-batch68692", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68693_marker",
	                     "libcgj-batch68693", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68694_marker",
	                     "libcgj-batch68694", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68695_marker",
	                     "libcgj-batch68695", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68696_marker",
	                     "libcgj-batch68696", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68697_marker",
	                     "libcgj-batch68697", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68698_marker",
	                     "libcgj-batch68698", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68699_marker",
	                     "libcgj-batch68699", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68700_marker",
	                     "libcgj-batch68700", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m68700 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m68700 PASS path=%s checked=%d "
