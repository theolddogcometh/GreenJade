 ...
 *
 * Soft host gates for continuum MILESTONE 68800 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_68800.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_68800.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_68800 \
 *     user/libcgj/tests/cgj_soft_milestone_68800.c -ldl
 *   ./build/cgj_soft_milestone_68800
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M68800_DIRECT \
 *     -o build/cgj_soft_milestone_68800_direct \
 *     user/libcgj/tests/cgj_soft_milestone_68800.c \
 *     user/libcgj/src/graph_batch68791.c \
 *     user/libcgj/src/graph_batch68792.c \
 *     user/libcgj/src/graph_batch68793.c \
 *     user/libcgj/src/graph_batch68794.c \
 *     user/libcgj/src/graph_batch68795.c \
 *     user/libcgj/src/graph_batch68796.c \
 *     user/libcgj/src/graph_batch68797.c \
 *     user/libcgj/src/graph_batch68798.c \
 *     user/libcgj/src/graph_batch68799.c \
 *     user/libcgj/src/graph_batch68800.c
 *   ./build/cgj_soft_milestone_68800_direct
 *
 * Expected soft KATs (wave 68791–68800 exclusive continuum markers):
 *   gj_shell_green_68800      → 1
 *   gj_libcgj_green_68800     → 1
 *   gj_bar3_ready_68800       → 0   (bar3 still open)
 *   gj_product_score_68800    → 0   (product score still open)
 *   gj_continuum_ready_68800  → 1
 *   gj_smoke_soft_68800       → 1
 *   gj_dyn_soft_68800         → 1
 *   gj_milestone_tag_68800    → 68800
 *   gj_continuum_wave_68800   → 68800
 *   gj_batch_id_68800         → 68800
 *   gj_graph_milestone_68800  → 68800
 *   __libcgj_batch68791_marker … __libcgj_batch68800_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch68791.c … graph_batch68800.c (and fill
 *     25776–25790 / identity 25751–25775) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 68800.
 *   - libc.map may lag milestone 68800 globals; add 68800 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_68800 == 0).
 */
...
direct_check(void)
{
	...
	if (gj_shell_green_68800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_68800 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_68800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_68800 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_68800() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_68800 fail\n");
		nFail++;
	}
	if (gj_product_score_68800() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_68800 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_68800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_68800 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_68800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_68800 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_68800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_68800 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_68800() != 68800u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_68800 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_68800() != 68800u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_68800 fail\n");
		nFail++;
	}
	if (gj_batch_id_68800() != 68800u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_68800 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_68800() != 68800u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_68800 fail\n");
		nFail++;
	}
...
#else /* !CGJ_SOFT_M68800_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m68800 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m68800 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m68800 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m68800 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m68800 SOFT-SKIP (no product SO)\n");
		return 0;
	}
	...
	nFail += soft_marker(h, "__libcgj_batch68791_marker",
	                     "libcgj-batch68791", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68792_marker",
	                     "libcgj-batch68792", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68793_marker",
	                     "libcgj-batch68793", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68794_marker",
	                     "libcgj-batch68794", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68795_marker",
	                     "libcgj-batch68795", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68796_marker",
	                     "libcgj-batch68796", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68797_marker",
	                     "libcgj-batch68797", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68798_marker",
	                     "libcgj-batch68798", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68799_marker",
	                     "libcgj-batch68799", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68800_marker",
	                     "libcgj-batch68800", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m68800 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m68800 PASS path=%s checked=%d "
