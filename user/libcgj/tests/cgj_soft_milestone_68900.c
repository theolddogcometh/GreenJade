 ...
 *
 * Soft host gates for continuum MILESTONE 68900 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_68900.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_68900.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_68900 \
 *     user/libcgj/tests/cgj_soft_milestone_68900.c -ldl
 *   ./build/cgj_soft_milestone_68900
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M68900_DIRECT \
 *     -o build/cgj_soft_milestone_68900_direct \
 *     user/libcgj/tests/cgj_soft_milestone_68900.c \
 *     user/libcgj/src/graph_batch68891.c \
 *     user/libcgj/src/graph_batch68892.c \
 *     user/libcgj/src/graph_batch68893.c \
 *     user/libcgj/src/graph_batch68894.c \
 *     user/libcgj/src/graph_batch68895.c \
 *     user/libcgj/src/graph_batch68896.c \
 *     user/libcgj/src/graph_batch68897.c \
 *     user/libcgj/src/graph_batch68898.c \
 *     user/libcgj/src/graph_batch68899.c \
 *     user/libcgj/src/graph_batch68900.c
 *   ./build/cgj_soft_milestone_68900_direct
 *
 * Expected soft KATs (wave 68891–68900 exclusive continuum markers):
 *   gj_shell_green_68900      → 1
 *   gj_libcgj_green_68900     → 1
 *   gj_bar3_ready_68900       → 0   (bar3 still open)
 *   gj_product_score_68900    → 0   (product score still open)
 *   gj_continuum_ready_68900  → 1
 *   gj_smoke_soft_68900       → 1
 *   gj_dyn_soft_68900         → 1
 *   gj_milestone_tag_68900    → 68900
 *   gj_continuum_wave_68900   → 68900
 *   gj_batch_id_68900         → 68900
 *   gj_graph_milestone_68900  → 68900
 *   __libcgj_batch68891_marker … __libcgj_batch68900_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch68891.c … graph_batch68900.c (and fill
 *     25776–25790 / identity 25751–25775) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 68900.
 *   - libc.map may lag milestone 68900 globals; add 68900 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_68900 == 0).
 */
...
direct_check(void)
{
	...
	if (gj_shell_green_68900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_68900 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_68900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_68900 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_68900() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_68900 fail\n");
		nFail++;
	}
	if (gj_product_score_68900() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_68900 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_68900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_68900 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_68900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_68900 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_68900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_68900 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_68900() != 68900u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_68900 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_68900() != 68900u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_68900 fail\n");
		nFail++;
	}
	if (gj_batch_id_68900() != 68900u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_68900 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_68900() != 68900u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_68900 fail\n");
		nFail++;
	}
...
#else /* !CGJ_SOFT_M68900_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m68900 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m68900 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m68900 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m68900 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m68900 SOFT-SKIP (no product SO)\n");
		return 0;
	}
	...
	nFail += soft_marker(h, "__libcgj_batch68891_marker",
	                     "libcgj-batch68891", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68892_marker",
	                     "libcgj-batch68892", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68893_marker",
	                     "libcgj-batch68893", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68894_marker",
	                     "libcgj-batch68894", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68895_marker",
	                     "libcgj-batch68895", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68896_marker",
	                     "libcgj-batch68896", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68897_marker",
	                     "libcgj-batch68897", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68898_marker",
	                     "libcgj-batch68898", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68899_marker",
	                     "libcgj-batch68899", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68900_marker",
	                     "libcgj-batch68900", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m68900 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m68900 PASS path=%s checked=%d "
