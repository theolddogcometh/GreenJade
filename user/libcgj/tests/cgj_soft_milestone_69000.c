 ...
 *
 * Soft host gates for continuum MILESTONE 69000 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_69000.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_69000.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_69000 \
 *     user/libcgj/tests/cgj_soft_milestone_69000.c -ldl
 *   ./build/cgj_soft_milestone_69000
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M69000_DIRECT \
 *     -o build/cgj_soft_milestone_69000_direct \
 *     user/libcgj/tests/cgj_soft_milestone_69000.c \
 *     user/libcgj/src/graph_batch68991.c \
 *     user/libcgj/src/graph_batch68992.c \
 *     user/libcgj/src/graph_batch68993.c \
 *     user/libcgj/src/graph_batch68994.c \
 *     user/libcgj/src/graph_batch68995.c \
 *     user/libcgj/src/graph_batch68996.c \
 *     user/libcgj/src/graph_batch68997.c \
 *     user/libcgj/src/graph_batch68998.c \
 *     user/libcgj/src/graph_batch68999.c \
 *     user/libcgj/src/graph_batch69000.c
 *   ./build/cgj_soft_milestone_69000_direct
 *
 * Expected soft KATs (wave 68991–69000 exclusive continuum markers):
 *   gj_shell_green_69000      → 1
 *   gj_libcgj_green_69000     → 1
 *   gj_bar3_ready_69000       → 0   (bar3 still open)
 *   gj_product_score_69000    → 0   (product score still open)
 *   gj_continuum_ready_69000  → 1
 *   gj_smoke_soft_69000       → 1
 *   gj_dyn_soft_69000         → 1
 *   gj_milestone_tag_69000    → 69000
 *   gj_continuum_wave_69000   → 69000
 *   gj_batch_id_69000         → 69000
 *   gj_graph_milestone_69000  → 69000
 *   __libcgj_batch68991_marker … __libcgj_batch69000_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch68991.c … graph_batch69000.c (and fill
 *     25776–25790 / identity 25751–25775) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 69000.
 *   - libc.map may lag milestone 69000 globals; add 69000 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_69000 == 0).
 */
...
direct_check(void)
{
	...
	if (gj_shell_green_69000() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_69000 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_69000() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_69000 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_69000() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_69000 fail\n");
		nFail++;
	}
	if (gj_product_score_69000() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_69000 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_69000() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_69000 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_69000() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_69000 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_69000() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_69000 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_69000() != 69000u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_69000 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_69000() != 69000u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_69000 fail\n");
		nFail++;
	}
	if (gj_batch_id_69000() != 69000u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_69000 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_69000() != 69000u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_69000 fail\n");
		nFail++;
	}
...
#else /* !CGJ_SOFT_M69000_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m69000 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m69000 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m69000 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m69000 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m69000 SOFT-SKIP (no product SO)\n");
		return 0;
	}
	...
	nFail += soft_marker(h, "__libcgj_batch68991_marker",
	                     "libcgj-batch68991", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68992_marker",
	                     "libcgj-batch68992", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68993_marker",
	                     "libcgj-batch68993", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68994_marker",
	                     "libcgj-batch68994", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68995_marker",
	                     "libcgj-batch68995", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68996_marker",
	                     "libcgj-batch68996", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68997_marker",
	                     "libcgj-batch68997", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68998_marker",
	                     "libcgj-batch68998", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch68999_marker",
	                     "libcgj-batch68999", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69000_marker",
	                     "libcgj-batch69000", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m69000 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m69000 PASS path=%s checked=%d "
