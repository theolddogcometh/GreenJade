 ...
 *
 * Soft host gates for continuum MILESTONE 69100 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_69100.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_69100.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_69100 \
 *     user/libcgj/tests/cgj_soft_milestone_69100.c -ldl
 *   ./build/cgj_soft_milestone_69100
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M69100_DIRECT \
 *     -o build/cgj_soft_milestone_69100_direct \
 *     user/libcgj/tests/cgj_soft_milestone_69100.c \
 *     user/libcgj/src/graph_batch69091.c \
 *     user/libcgj/src/graph_batch69092.c \
 *     user/libcgj/src/graph_batch69093.c \
 *     user/libcgj/src/graph_batch69094.c \
 *     user/libcgj/src/graph_batch69095.c \
 *     user/libcgj/src/graph_batch69096.c \
 *     user/libcgj/src/graph_batch69097.c \
 *     user/libcgj/src/graph_batch69098.c \
 *     user/libcgj/src/graph_batch69099.c \
 *     user/libcgj/src/graph_batch69100.c
 *   ./build/cgj_soft_milestone_69100_direct
 *
 * Expected soft KATs (wave 69091–69100 exclusive continuum markers):
 *   gj_shell_green_69100      → 1
 *   gj_libcgj_green_69100     → 1
 *   gj_bar3_ready_69100       → 0   (bar3 still open)
 *   gj_product_score_69100    → 0   (product score still open)
 *   gj_continuum_ready_69100  → 1
 *   gj_smoke_soft_69100       → 1
 *   gj_dyn_soft_69100         → 1
 *   gj_milestone_tag_69100    → 69100
 *   gj_continuum_wave_69100   → 69100
 *   gj_batch_id_69100         → 69100
 *   gj_graph_milestone_69100  → 69100
 *   __libcgj_batch69091_marker … __libcgj_batch69100_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch69091.c … graph_batch69100.c (and fill
 *     25776–25790 / identity 25751–25775) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 69100.
 *   - libc.map may lag milestone 69100 globals; add 69100 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_69100 == 0).
 */
...
direct_check(void)
{
	...
	if (gj_shell_green_69100() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_69100 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_69100() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_69100 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_69100() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_69100 fail\n");
		nFail++;
	}
	if (gj_product_score_69100() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_69100 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_69100() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_69100 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_69100() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_69100 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_69100() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_69100 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_69100() != 69100u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_69100 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_69100() != 69100u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_69100 fail\n");
		nFail++;
	}
	if (gj_batch_id_69100() != 69100u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_69100 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_69100() != 69100u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_69100 fail\n");
		nFail++;
	}
...
#else /* !CGJ_SOFT_M69100_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m69100 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m69100 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m69100 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m69100 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m69100 SOFT-SKIP (no product SO)\n");
		return 0;
	}
	...
	nFail += soft_marker(h, "__libcgj_batch69091_marker",
	                     "libcgj-batch69091", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69092_marker",
	                     "libcgj-batch69092", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69093_marker",
	                     "libcgj-batch69093", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69094_marker",
	                     "libcgj-batch69094", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69095_marker",
	                     "libcgj-batch69095", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69096_marker",
	                     "libcgj-batch69096", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69097_marker",
	                     "libcgj-batch69097", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69098_marker",
	                     "libcgj-batch69098", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69099_marker",
	                     "libcgj-batch69099", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69100_marker",
	                     "libcgj-batch69100", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m69100 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m69100 PASS path=%s checked=%d "
