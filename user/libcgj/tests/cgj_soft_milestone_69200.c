 ...
 *
 * Soft host gates for continuum MILESTONE 69200 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_69200.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_69200.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_69200 \
 *     user/libcgj/tests/cgj_soft_milestone_69200.c -ldl
 *   ./build/cgj_soft_milestone_69200
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M69200_DIRECT \
 *     -o build/cgj_soft_milestone_69200_direct \
 *     user/libcgj/tests/cgj_soft_milestone_69200.c \
 *     user/libcgj/src/graph_batch69191.c \
 *     user/libcgj/src/graph_batch69192.c \
 *     user/libcgj/src/graph_batch69193.c \
 *     user/libcgj/src/graph_batch69194.c \
 *     user/libcgj/src/graph_batch69195.c \
 *     user/libcgj/src/graph_batch69196.c \
 *     user/libcgj/src/graph_batch69197.c \
 *     user/libcgj/src/graph_batch69198.c \
 *     user/libcgj/src/graph_batch69199.c \
 *     user/libcgj/src/graph_batch69200.c
 *   ./build/cgj_soft_milestone_69200_direct
 *
 * Expected soft KATs (wave 69191–69200 exclusive continuum markers):
 *   gj_shell_green_69200      → 1
 *   gj_libcgj_green_69200     → 1
 *   gj_bar3_ready_69200       → 0   (bar3 still open)
 *   gj_product_score_69200    → 0   (product score still open)
 *   gj_continuum_ready_69200  → 1
 *   gj_smoke_soft_69200       → 1
 *   gj_dyn_soft_69200         → 1
 *   gj_milestone_tag_69200    → 69200
 *   gj_continuum_wave_69200   → 69200
 *   gj_batch_id_69200         → 69200
 *   gj_graph_milestone_69200  → 69200
 *   __libcgj_batch69191_marker … __libcgj_batch69200_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch69191.c … graph_batch69200.c (and fill
 *     25776–25790 / identity 25751–25775) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 69200.
 *   - libc.map may lag milestone 69200 globals; add 69200 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_69200 == 0).
 */
...
direct_check(void)
{
	...
	if (gj_shell_green_69200() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_69200 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_69200() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_69200 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_69200() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_69200 fail\n");
		nFail++;
	}
	if (gj_product_score_69200() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_69200 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_69200() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_69200 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_69200() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_69200 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_69200() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_69200 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_69200() != 69200u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_69200 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_69200() != 69200u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_69200 fail\n");
		nFail++;
	}
	if (gj_batch_id_69200() != 69200u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_69200 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_69200() != 69200u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_69200 fail\n");
		nFail++;
	}
...
#else /* !CGJ_SOFT_M69200_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m69200 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m69200 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m69200 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m69200 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m69200 SOFT-SKIP (no product SO)\n");
		return 0;
	}
	...
	nFail += soft_marker(h, "__libcgj_batch69191_marker",
	                     "libcgj-batch69191", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69192_marker",
	                     "libcgj-batch69192", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69193_marker",
	                     "libcgj-batch69193", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69194_marker",
	                     "libcgj-batch69194", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69195_marker",
	                     "libcgj-batch69195", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69196_marker",
	                     "libcgj-batch69196", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69197_marker",
	                     "libcgj-batch69197", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69198_marker",
	                     "libcgj-batch69198", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69199_marker",
	                     "libcgj-batch69199", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69200_marker",
	                     "libcgj-batch69200", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m69200 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m69200 PASS path=%s checked=%d "
