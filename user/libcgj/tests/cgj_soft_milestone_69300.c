 ...
 *
 * Soft host gates for continuum MILESTONE 69300 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_69300.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_69300.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_69300 \
 *     user/libcgj/tests/cgj_soft_milestone_69300.c -ldl
 *   ./build/cgj_soft_milestone_69300
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M69300_DIRECT \
 *     -o build/cgj_soft_milestone_69300_direct \
 *     user/libcgj/tests/cgj_soft_milestone_69300.c \
 *     user/libcgj/src/graph_batch69291.c \
 *     user/libcgj/src/graph_batch69292.c \
 *     user/libcgj/src/graph_batch69293.c \
 *     user/libcgj/src/graph_batch69294.c \
 *     user/libcgj/src/graph_batch69295.c \
 *     user/libcgj/src/graph_batch69296.c \
 *     user/libcgj/src/graph_batch69297.c \
 *     user/libcgj/src/graph_batch69298.c \
 *     user/libcgj/src/graph_batch69299.c \
 *     user/libcgj/src/graph_batch69300.c
 *   ./build/cgj_soft_milestone_69300_direct
 *
 * Expected soft KATs (wave 69291–69300 exclusive continuum markers):
 *   gj_shell_green_69300      → 1
 *   gj_libcgj_green_69300     → 1
 *   gj_bar3_ready_69300       → 0   (bar3 still open)
 *   gj_product_score_69300    → 0   (product score still open)
 *   gj_continuum_ready_69300  → 1
 *   gj_smoke_soft_69300       → 1
 *   gj_dyn_soft_69300         → 1
 *   gj_milestone_tag_69300    → 69300
 *   gj_continuum_wave_69300   → 69300
 *   gj_batch_id_69300         → 69300
 *   gj_graph_milestone_69300  → 69300
 *   __libcgj_batch69291_marker … __libcgj_batch69300_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch69291.c … graph_batch69300.c (and fill
 *     25776–25790 / identity 25751–25775) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 69300.
 *   - libc.map may lag milestone 69300 globals; add 69300 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_69300 == 0).
 */
...
direct_check(void)
{
	...
	if (gj_shell_green_69300() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_69300 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_69300() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_69300 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_69300() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_69300 fail\n");
		nFail++;
	}
	if (gj_product_score_69300() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_69300 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_69300() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_69300 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_69300() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_69300 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_69300() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_69300 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_69300() != 69300u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_69300 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_69300() != 69300u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_69300 fail\n");
		nFail++;
	}
	if (gj_batch_id_69300() != 69300u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_69300 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_69300() != 69300u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_69300 fail\n");
		nFail++;
	}
...
#else /* !CGJ_SOFT_M69300_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m69300 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m69300 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m69300 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m69300 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m69300 SOFT-SKIP (no product SO)\n");
		return 0;
	}
	...
	nFail += soft_marker(h, "__libcgj_batch69291_marker",
	                     "libcgj-batch69291", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69292_marker",
	                     "libcgj-batch69292", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69293_marker",
	                     "libcgj-batch69293", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69294_marker",
	                     "libcgj-batch69294", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69295_marker",
	                     "libcgj-batch69295", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69296_marker",
	                     "libcgj-batch69296", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69297_marker",
	                     "libcgj-batch69297", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69298_marker",
	                     "libcgj-batch69298", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69299_marker",
	                     "libcgj-batch69299", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69300_marker",
	                     "libcgj-batch69300", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m69300 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m69300 PASS path=%s checked=%d "
