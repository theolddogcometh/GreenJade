 ...
 *
 * Soft host gates for continuum MILESTONE 69400 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_69400.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_69400.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_69400 \
 *     user/libcgj/tests/cgj_soft_milestone_69400.c -ldl
 *   ./build/cgj_soft_milestone_69400
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M69400_DIRECT \
 *     -o build/cgj_soft_milestone_69400_direct \
 *     user/libcgj/tests/cgj_soft_milestone_69400.c \
 *     user/libcgj/src/graph_batch69391.c \
 *     user/libcgj/src/graph_batch69392.c \
 *     user/libcgj/src/graph_batch69393.c \
 *     user/libcgj/src/graph_batch69394.c \
 *     user/libcgj/src/graph_batch69395.c \
 *     user/libcgj/src/graph_batch69396.c \
 *     user/libcgj/src/graph_batch69397.c \
 *     user/libcgj/src/graph_batch69398.c \
 *     user/libcgj/src/graph_batch69399.c \
 *     user/libcgj/src/graph_batch69400.c
 *   ./build/cgj_soft_milestone_69400_direct
 *
 * Expected soft KATs (wave 69391–69400 exclusive continuum markers):
 *   gj_shell_green_69400      → 1
 *   gj_libcgj_green_69400     → 1
 *   gj_bar3_ready_69400       → 0   (bar3 still open)
 *   gj_product_score_69400    → 0   (product score still open)
 *   gj_continuum_ready_69400  → 1
 *   gj_smoke_soft_69400       → 1
 *   gj_dyn_soft_69400         → 1
 *   gj_milestone_tag_69400    → 69400
 *   gj_continuum_wave_69400   → 69400
 *   gj_batch_id_69400         → 69400
 *   gj_graph_milestone_69400  → 69400
 *   __libcgj_batch69391_marker … __libcgj_batch69400_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch69391.c … graph_batch69400.c (and fill
 *     25776–25790 / identity 25751–25775) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 69400.
 *   - libc.map may lag milestone 69400 globals; add 69400 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_69400 == 0).
 */
...
direct_check(void)
{
	...
	if (gj_shell_green_69400() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_69400 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_69400() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_69400 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_69400() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_69400 fail\n");
		nFail++;
	}
	if (gj_product_score_69400() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_69400 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_69400() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_69400 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_69400() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_69400 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_69400() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_69400 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_69400() != 69400u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_69400 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_69400() != 69400u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_69400 fail\n");
		nFail++;
	}
	if (gj_batch_id_69400() != 69400u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_69400 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_69400() != 69400u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_69400 fail\n");
		nFail++;
	}
...
#else /* !CGJ_SOFT_M69400_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m69400 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m69400 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m69400 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m69400 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m69400 SOFT-SKIP (no product SO)\n");
		return 0;
	}
	...
	nFail += soft_marker(h, "__libcgj_batch69391_marker",
	                     "libcgj-batch69391", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69392_marker",
	                     "libcgj-batch69392", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69393_marker",
	                     "libcgj-batch69393", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69394_marker",
	                     "libcgj-batch69394", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69395_marker",
	                     "libcgj-batch69395", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69396_marker",
	                     "libcgj-batch69396", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69397_marker",
	                     "libcgj-batch69397", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69398_marker",
	                     "libcgj-batch69398", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69399_marker",
	                     "libcgj-batch69399", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69400_marker",
	                     "libcgj-batch69400", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m69400 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m69400 PASS path=%s checked=%d "
