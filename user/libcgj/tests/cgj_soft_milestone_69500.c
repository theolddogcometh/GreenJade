 ...
 *
 * Soft host gates for continuum MILESTONE 69500 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_69500.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_69500.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_69500 \
 *     user/libcgj/tests/cgj_soft_milestone_69500.c -ldl
 *   ./build/cgj_soft_milestone_69500
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M69500_DIRECT \
 *     -o build/cgj_soft_milestone_69500_direct \
 *     user/libcgj/tests/cgj_soft_milestone_69500.c \
 *     user/libcgj/src/graph_batch69491.c \
 *     user/libcgj/src/graph_batch69492.c \
 *     user/libcgj/src/graph_batch69493.c \
 *     user/libcgj/src/graph_batch69494.c \
 *     user/libcgj/src/graph_batch69495.c \
 *     user/libcgj/src/graph_batch69496.c \
 *     user/libcgj/src/graph_batch69497.c \
 *     user/libcgj/src/graph_batch69498.c \
 *     user/libcgj/src/graph_batch69499.c \
 *     user/libcgj/src/graph_batch69500.c
 *   ./build/cgj_soft_milestone_69500_direct
 *
 * Expected soft KATs (wave 69491–69500 exclusive continuum markers):
 *   gj_shell_green_69500      → 1
 *   gj_libcgj_green_69500     → 1
 *   gj_bar3_ready_69500       → 0   (bar3 still open)
 *   gj_product_score_69500    → 0   (product score still open)
 *   gj_continuum_ready_69500  → 1
 *   gj_smoke_soft_69500       → 1
 *   gj_dyn_soft_69500         → 1
 *   gj_milestone_tag_69500    → 69500
 *   gj_continuum_wave_69500   → 69500
 *   gj_batch_id_69500         → 69500
 *   gj_graph_milestone_69500  → 69500
 *   __libcgj_batch69491_marker … __libcgj_batch69500_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch69491.c … graph_batch69500.c (and fill
 *     25776–25790 / identity 25751–25775) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 69500.
 *   - libc.map may lag milestone 69500 globals; add 69500 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_69500 == 0).
 */
...
direct_check(void)
{
	...
	if (gj_shell_green_69500() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_69500 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_69500() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_69500 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_69500() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_69500 fail\n");
		nFail++;
	}
	if (gj_product_score_69500() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_69500 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_69500() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_69500 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_69500() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_69500 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_69500() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_69500 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_69500() != 69500u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_69500 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_69500() != 69500u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_69500 fail\n");
		nFail++;
	}
	if (gj_batch_id_69500() != 69500u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_69500 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_69500() != 69500u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_69500 fail\n");
		nFail++;
	}
...
#else /* !CGJ_SOFT_M69500_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m69500 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m69500 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m69500 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m69500 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m69500 SOFT-SKIP (no product SO)\n");
		return 0;
	}
	...
	nFail += soft_marker(h, "__libcgj_batch69491_marker",
	                     "libcgj-batch69491", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69492_marker",
	                     "libcgj-batch69492", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69493_marker",
	                     "libcgj-batch69493", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69494_marker",
	                     "libcgj-batch69494", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69495_marker",
	                     "libcgj-batch69495", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69496_marker",
	                     "libcgj-batch69496", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69497_marker",
	                     "libcgj-batch69497", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69498_marker",
	                     "libcgj-batch69498", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69499_marker",
	                     "libcgj-batch69499", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69500_marker",
	                     "libcgj-batch69500", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m69500 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m69500 PASS path=%s checked=%d "
