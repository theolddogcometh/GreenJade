 ...
 *
 * Soft host gates for continuum MILESTONE 69600 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_69600.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_69600.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_69600 \
 *     user/libcgj/tests/cgj_soft_milestone_69600.c -ldl
 *   ./build/cgj_soft_milestone_69600
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M69600_DIRECT \
 *     -o build/cgj_soft_milestone_69600_direct \
 *     user/libcgj/tests/cgj_soft_milestone_69600.c \
 *     user/libcgj/src/graph_batch69591.c \
 *     user/libcgj/src/graph_batch69592.c \
 *     user/libcgj/src/graph_batch69593.c \
 *     user/libcgj/src/graph_batch69594.c \
 *     user/libcgj/src/graph_batch69595.c \
 *     user/libcgj/src/graph_batch69596.c \
 *     user/libcgj/src/graph_batch69597.c \
 *     user/libcgj/src/graph_batch69598.c \
 *     user/libcgj/src/graph_batch69599.c \
 *     user/libcgj/src/graph_batch69600.c
 *   ./build/cgj_soft_milestone_69600_direct
 *
 * Expected soft KATs (wave 69591–69600 exclusive continuum markers):
 *   gj_shell_green_69600      → 1
 *   gj_libcgj_green_69600     → 1
 *   gj_bar3_ready_69600       → 0   (bar3 still open)
 *   gj_product_score_69600    → 0   (product score still open)
 *   gj_continuum_ready_69600  → 1
 *   gj_smoke_soft_69600       → 1
 *   gj_dyn_soft_69600         → 1
 *   gj_milestone_tag_69600    → 69600
 *   gj_continuum_wave_69600   → 69600
 *   gj_batch_id_69600         → 69600
 *   gj_graph_milestone_69600  → 69600
 *   __libcgj_batch69591_marker … __libcgj_batch69600_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch69591.c … graph_batch69600.c (and fill
 *     25776–25790 / identity 25751–25775) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 69600.
 *   - libc.map may lag milestone 69600 globals; add 69600 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_69600 == 0).
 */
...
direct_check(void)
{
	...
	if (gj_shell_green_69600() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_69600 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_69600() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_69600 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_69600() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_69600 fail\n");
		nFail++;
	}
	if (gj_product_score_69600() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_69600 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_69600() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_69600 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_69600() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_69600 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_69600() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_69600 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_69600() != 69600u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_69600 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_69600() != 69600u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_69600 fail\n");
		nFail++;
	}
	if (gj_batch_id_69600() != 69600u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_69600 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_69600() != 69600u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_69600 fail\n");
		nFail++;
	}
...
#else /* !CGJ_SOFT_M69600_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m69600 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m69600 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m69600 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m69600 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m69600 SOFT-SKIP (no product SO)\n");
		return 0;
	}
	...
	nFail += soft_marker(h, "__libcgj_batch69591_marker",
	                     "libcgj-batch69591", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69592_marker",
	                     "libcgj-batch69592", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69593_marker",
	                     "libcgj-batch69593", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69594_marker",
	                     "libcgj-batch69594", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69595_marker",
	                     "libcgj-batch69595", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69596_marker",
	                     "libcgj-batch69596", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69597_marker",
	                     "libcgj-batch69597", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69598_marker",
	                     "libcgj-batch69598", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69599_marker",
	                     "libcgj-batch69599", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69600_marker",
	                     "libcgj-batch69600", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m69600 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m69600 PASS path=%s checked=%d "
