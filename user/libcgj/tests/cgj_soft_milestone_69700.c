 ...
 *
 * Soft host gates for continuum MILESTONE 69700 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_69700.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_69700.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_69700 \
 *     user/libcgj/tests/cgj_soft_milestone_69700.c -ldl
 *   ./build/cgj_soft_milestone_69700
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M69700_DIRECT \
 *     -o build/cgj_soft_milestone_69700_direct \
 *     user/libcgj/tests/cgj_soft_milestone_69700.c \
 *     user/libcgj/src/graph_batch69691.c \
 *     user/libcgj/src/graph_batch69692.c \
 *     user/libcgj/src/graph_batch69693.c \
 *     user/libcgj/src/graph_batch69694.c \
 *     user/libcgj/src/graph_batch69695.c \
 *     user/libcgj/src/graph_batch69696.c \
 *     user/libcgj/src/graph_batch69697.c \
 *     user/libcgj/src/graph_batch69698.c \
 *     user/libcgj/src/graph_batch69699.c \
 *     user/libcgj/src/graph_batch69700.c
 *   ./build/cgj_soft_milestone_69700_direct
 *
 * Expected soft KATs (wave 69691–69700 exclusive continuum markers):
 *   gj_shell_green_69700      → 1
 *   gj_libcgj_green_69700     → 1
 *   gj_bar3_ready_69700       → 0   (bar3 still open)
 *   gj_product_score_69700    → 0   (product score still open)
 *   gj_continuum_ready_69700  → 1
 *   gj_smoke_soft_69700       → 1
 *   gj_dyn_soft_69700         → 1
 *   gj_milestone_tag_69700    → 69700
 *   gj_continuum_wave_69700   → 69700
 *   gj_batch_id_69700         → 69700
 *   gj_graph_milestone_69700  → 69700
 *   __libcgj_batch69691_marker … __libcgj_batch69700_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch69691.c … graph_batch69700.c (and fill
 *     25776–25790 / identity 25751–25775) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 69700.
 *   - libc.map may lag milestone 69700 globals; add 69700 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_69700 == 0).
 */
...
direct_check(void)
{
	...
	if (gj_shell_green_69700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_69700 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_69700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_69700 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_69700() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_69700 fail\n");
		nFail++;
	}
	if (gj_product_score_69700() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_69700 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_69700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_69700 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_69700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_69700 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_69700() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_69700 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_69700() != 69700u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_69700 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_69700() != 69700u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_69700 fail\n");
		nFail++;
	}
	if (gj_batch_id_69700() != 69700u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_69700 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_69700() != 69700u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_69700 fail\n");
		nFail++;
	}
...
#else /* !CGJ_SOFT_M69700_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m69700 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m69700 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m69700 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m69700 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m69700 SOFT-SKIP (no product SO)\n");
		return 0;
	}
	...
	nFail += soft_marker(h, "__libcgj_batch69691_marker",
	                     "libcgj-batch69691", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69692_marker",
	                     "libcgj-batch69692", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69693_marker",
	                     "libcgj-batch69693", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69694_marker",
	                     "libcgj-batch69694", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69695_marker",
	                     "libcgj-batch69695", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69696_marker",
	                     "libcgj-batch69696", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69697_marker",
	                     "libcgj-batch69697", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69698_marker",
	                     "libcgj-batch69698", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69699_marker",
	                     "libcgj-batch69699", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69700_marker",
	                     "libcgj-batch69700", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m69700 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m69700 PASS path=%s checked=%d "
