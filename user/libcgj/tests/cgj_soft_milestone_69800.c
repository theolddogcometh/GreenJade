 ...
 *
 * Soft host gates for continuum MILESTONE 69800 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_69800.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_69800.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_69800 \
 *     user/libcgj/tests/cgj_soft_milestone_69800.c -ldl
 *   ./build/cgj_soft_milestone_69800
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M69800_DIRECT \
 *     -o build/cgj_soft_milestone_69800_direct \
 *     user/libcgj/tests/cgj_soft_milestone_69800.c \
 *     user/libcgj/src/graph_batch69791.c \
 *     user/libcgj/src/graph_batch69792.c \
 *     user/libcgj/src/graph_batch69793.c \
 *     user/libcgj/src/graph_batch69794.c \
 *     user/libcgj/src/graph_batch69795.c \
 *     user/libcgj/src/graph_batch69796.c \
 *     user/libcgj/src/graph_batch69797.c \
 *     user/libcgj/src/graph_batch69798.c \
 *     user/libcgj/src/graph_batch69799.c \
 *     user/libcgj/src/graph_batch69800.c
 *   ./build/cgj_soft_milestone_69800_direct
 *
 * Expected soft KATs (wave 69791–69800 exclusive continuum markers):
 *   gj_shell_green_69800      → 1
 *   gj_libcgj_green_69800     → 1
 *   gj_bar3_ready_69800       → 0   (bar3 still open)
 *   gj_product_score_69800    → 0   (product score still open)
 *   gj_continuum_ready_69800  → 1
 *   gj_smoke_soft_69800       → 1
 *   gj_dyn_soft_69800         → 1
 *   gj_milestone_tag_69800    → 69800
 *   gj_continuum_wave_69800   → 69800
 *   gj_batch_id_69800         → 69800
 *   gj_graph_milestone_69800  → 69800
 *   __libcgj_batch69791_marker … __libcgj_batch69800_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch69791.c … graph_batch69800.c (and fill
 *     25776–25790 / identity 25751–25775) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 69800.
 *   - libc.map may lag milestone 69800 globals; add 69800 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_69800 == 0).
 */
...
direct_check(void)
{
	...
	if (gj_shell_green_69800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_69800 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_69800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_69800 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_69800() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_69800 fail\n");
		nFail++;
	}
	if (gj_product_score_69800() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_69800 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_69800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_69800 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_69800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_69800 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_69800() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_69800 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_69800() != 69800u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_69800 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_69800() != 69800u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_69800 fail\n");
		nFail++;
	}
	if (gj_batch_id_69800() != 69800u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_69800 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_69800() != 69800u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_69800 fail\n");
		nFail++;
	}
...
#else /* !CGJ_SOFT_M69800_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m69800 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m69800 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m69800 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m69800 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m69800 SOFT-SKIP (no product SO)\n");
		return 0;
	}
	...
	nFail += soft_marker(h, "__libcgj_batch69791_marker",
	                     "libcgj-batch69791", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69792_marker",
	                     "libcgj-batch69792", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69793_marker",
	                     "libcgj-batch69793", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69794_marker",
	                     "libcgj-batch69794", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69795_marker",
	                     "libcgj-batch69795", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69796_marker",
	                     "libcgj-batch69796", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69797_marker",
	                     "libcgj-batch69797", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69798_marker",
	                     "libcgj-batch69798", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69799_marker",
	                     "libcgj-batch69799", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69800_marker",
	                     "libcgj-batch69800", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m69800 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m69800 PASS path=%s checked=%d "
