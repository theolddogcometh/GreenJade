 ...
 *
 * Soft host gates for continuum MILESTONE 69900 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_69900.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_69900.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_69900 \
 *     user/libcgj/tests/cgj_soft_milestone_69900.c -ldl
 *   ./build/cgj_soft_milestone_69900
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M69900_DIRECT \
 *     -o build/cgj_soft_milestone_69900_direct \
 *     user/libcgj/tests/cgj_soft_milestone_69900.c \
 *     user/libcgj/src/graph_batch69891.c \
 *     user/libcgj/src/graph_batch69892.c \
 *     user/libcgj/src/graph_batch69893.c \
 *     user/libcgj/src/graph_batch69894.c \
 *     user/libcgj/src/graph_batch69895.c \
 *     user/libcgj/src/graph_batch69896.c \
 *     user/libcgj/src/graph_batch69897.c \
 *     user/libcgj/src/graph_batch69898.c \
 *     user/libcgj/src/graph_batch69899.c \
 *     user/libcgj/src/graph_batch69900.c
 *   ./build/cgj_soft_milestone_69900_direct
 *
 * Expected soft KATs (wave 69891–69900 exclusive continuum markers):
 *   gj_shell_green_69900      → 1
 *   gj_libcgj_green_69900     → 1
 *   gj_bar3_ready_69900       → 0   (bar3 still open)
 *   gj_product_score_69900    → 0   (product score still open)
 *   gj_continuum_ready_69900  → 1
 *   gj_smoke_soft_69900       → 1
 *   gj_dyn_soft_69900         → 1
 *   gj_milestone_tag_69900    → 69900
 *   gj_continuum_wave_69900   → 69900
 *   gj_batch_id_69900         → 69900
 *   gj_graph_milestone_69900  → 69900
 *   __libcgj_batch69891_marker … __libcgj_batch69900_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch69891.c … graph_batch69900.c (and fill
 *     25776–25790 / identity 25751–25775) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 69900.
 *   - libc.map may lag milestone 69900 globals; add 69900 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_69900 == 0).
 */
...
direct_check(void)
{
	...
	if (gj_shell_green_69900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_69900 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_69900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_69900 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_69900() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_69900 fail\n");
		nFail++;
	}
	if (gj_product_score_69900() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_69900 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_69900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_69900 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_69900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_69900 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_69900() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_69900 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_69900() != 69900u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_69900 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_69900() != 69900u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_69900 fail\n");
		nFail++;
	}
	if (gj_batch_id_69900() != 69900u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_69900 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_69900() != 69900u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_69900 fail\n");
		nFail++;
	}
...
#else /* !CGJ_SOFT_M69900_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m69900 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m69900 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m69900 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m69900 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m69900 SOFT-SKIP (no product SO)\n");
		return 0;
	}
	...
	nFail += soft_marker(h, "__libcgj_batch69891_marker",
	                     "libcgj-batch69891", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69892_marker",
	                     "libcgj-batch69892", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69893_marker",
	                     "libcgj-batch69893", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69894_marker",
	                     "libcgj-batch69894", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69895_marker",
	                     "libcgj-batch69895", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69896_marker",
	                     "libcgj-batch69896", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69897_marker",
	                     "libcgj-batch69897", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69898_marker",
	                     "libcgj-batch69898", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69899_marker",
	                     "libcgj-batch69899", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69900_marker",
	                     "libcgj-batch69900", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m69900 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m69900 PASS path=%s checked=%d "
