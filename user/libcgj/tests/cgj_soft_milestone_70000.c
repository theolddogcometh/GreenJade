 ...
 *
 * Soft host gates for continuum MILESTONE 70000 markers (CREATE-ONLY).
 *
 * Coherent soft package (see NOTES_milestone_70000.txt):
 *   - this TU (libcgj continuum dyn/direct soft KATs)
 *   - NOTES_milestone_70000.txt
 *
 * Test harness note:
 ...
 *   cc -std=c11 -Wall -Wextra -O2 -o build/cgj_soft_milestone_70000 \
 *     user/libcgj/tests/cgj_soft_milestone_70000.c -ldl
 *   ./build/cgj_soft_milestone_70000
 *
 * Optional hard unit check (link pure graph batches directly; no SO map):
 *   cc -std=c11 -Wall -Wextra -O2 -DCGJ_SOFT_M70000_DIRECT \
 *     -o build/cgj_soft_milestone_70000_direct \
 *     user/libcgj/tests/cgj_soft_milestone_70000.c \
 *     user/libcgj/src/graph_batch69991.c \
 *     user/libcgj/src/graph_batch69992.c \
 *     user/libcgj/src/graph_batch69993.c \
 *     user/libcgj/src/graph_batch69994.c \
 *     user/libcgj/src/graph_batch69995.c \
 *     user/libcgj/src/graph_batch69996.c \
 *     user/libcgj/src/graph_batch69997.c \
 *     user/libcgj/src/graph_batch69998.c \
 *     user/libcgj/src/graph_batch69999.c \
 *     user/libcgj/src/graph_batch70000.c
 *   ./build/cgj_soft_milestone_70000_direct
 *
 * Expected soft KATs (wave 69991–70000 exclusive continuum markers):
 *   gj_shell_green_70000      → 1
 *   gj_libcgj_green_70000     → 1
 *   gj_bar3_ready_70000       → 0   (bar3 still open)
 *   gj_product_score_70000    → 0   (product score still open)
 *   gj_continuum_ready_70000  → 1
 *   gj_smoke_soft_70000       → 1
 *   gj_dyn_soft_70000         → 1
 *   gj_milestone_tag_70000    → 70000
 *   gj_continuum_wave_70000   → 70000
 *   gj_batch_id_70000         → 70000
 *   gj_graph_milestone_70000  → 70000
 *   __libcgj_batch69991_marker … __libcgj_batch70000_marker
 *     → "libcgj-batchNNNNN" string match when present
 *
 * Notes for parent wiring (outside this soft exclusive scope):
 *   - Sources graph_batch69991.c … graph_batch70000.c (and fill
 *     25776–25790 / identity 25751–25775) are CREATE-ONLY; wire into
 *     CGJ_SRCS when parent advances makefile_max to 70000.
 *   - libc.map may lag milestone 70000 globals; add 70000 symbols to the
 *     GLIBC_2.2.5 global list so dyn dlsym can see them.
 *   - Optional: fold these soft gates into cgj_dyn_smoke.c /
 *     cgj_smoke.c continuum soft blocks (same skip-if-missing rule).
 *   - Soft gates; bar3 still open (gj_bar3_ready_70000 == 0).
 */
...
direct_check(void)
{
	...
	if (gj_shell_green_70000() != 1u) {
		fprintf(stderr, "libcgj: direct gj_shell_green_70000 fail\n");
		nFail++;
	}
	if (gj_libcgj_green_70000() != 1u) {
		fprintf(stderr, "libcgj: direct gj_libcgj_green_70000 fail\n");
		nFail++;
	}
	if (gj_bar3_ready_70000() != 0u) {
		fprintf(stderr, "libcgj: direct gj_bar3_ready_70000 fail\n");
		nFail++;
	}
	if (gj_product_score_70000() != 0u) {
		fprintf(stderr, "libcgj: direct gj_product_score_70000 fail\n");
		nFail++;
	}
	if (gj_continuum_ready_70000() != 1u) {
		fprintf(stderr, "libcgj: direct gj_continuum_ready_70000 fail\n");
		nFail++;
	}
	if (gj_smoke_soft_70000() != 1u) {
		fprintf(stderr, "libcgj: direct gj_smoke_soft_70000 fail\n");
		nFail++;
	}
	if (gj_dyn_soft_70000() != 1u) {
		fprintf(stderr, "libcgj: direct gj_dyn_soft_70000 fail\n");
		nFail++;
	}
	if (gj_milestone_tag_70000() != 70000u) {
		fprintf(stderr, "libcgj: direct gj_milestone_tag_70000 fail\n");
		nFail++;
	}
	if (gj_continuum_wave_70000() != 70000u) {
		fprintf(stderr, "libcgj: direct gj_continuum_wave_70000 fail\n");
		nFail++;
	}
	if (gj_batch_id_70000() != 70000u) {
		fprintf(stderr, "libcgj: direct gj_batch_id_70000 fail\n");
		nFail++;
	}
	if (gj_graph_milestone_70000() != 70000u) {
		fprintf(stderr,
		        "libcgj: direct gj_graph_milestone_70000 fail\n");
		nFail++;
	}
...
#else /* !CGJ_SOFT_M70000_DIRECT — dyn soft path */
...
soft_u32(void *h, const char *szName, uint32_t u32Expect, int *pChecked)
{
	...
	if (u32Got != u32Expect) {
		fprintf(stderr,
		        "libcgj: soft m70000 %s KAT fail got=%u expect=%u\n",
...
soft_marker(void *h, const char *szSym, const char *szExpect, int *pChecked)
{
	...
	if (strcmp(szGot, szExpect) != 0) {
		fprintf(stderr,
		        "libcgj: soft m70000 %s marker fail got=\"%s\" "
		        "expect=\"%s\"\n",
...
main(void)
{
	...
	if (nFail != 0) {
		fprintf(stderr,
		        "libcgj: soft m70000 DIRECT FAIL fails=%d\n", nFail);
		return 1;
	}
	...
	if (h == NULL) {
		 ...
		 */
		fprintf(stderr,
		        "libcgj: soft m70000 skip (dlopen fail: %s)\n",
		        dlerror());
		printf("libcgj: soft m70000 SOFT-SKIP (no product SO)\n");
		return 0;
	}
	...
	nFail += soft_marker(h, "__libcgj_batch69991_marker",
	                     "libcgj-batch69991", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69992_marker",
	                     "libcgj-batch69992", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69993_marker",
	                     "libcgj-batch69993", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69994_marker",
	                     "libcgj-batch69994", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69995_marker",
	                     "libcgj-batch69995", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69996_marker",
	                     "libcgj-batch69996", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69997_marker",
	                     "libcgj-batch69997", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69998_marker",
	                     "libcgj-batch69998", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch69999_marker",
	                     "libcgj-batch69999", &nChecked);
	nFail += soft_marker(h, "__libcgj_batch70000_marker",
	                     "libcgj-batch70000", &nChecked);

	if (nFail != 0) {
		(void)dlclose(h);
		fprintf(stderr,
		        "libcgj: soft m70000 FAIL path=%s fails=%d checked=%d\n",
		        szPath, nFail, nChecked);
		return 2;
	}

	printf("libcgj: soft m70000 PASS path=%s checked=%d "
