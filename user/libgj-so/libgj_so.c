 ...
 *
 * Soft deepen surface (Wave 85 exclusive; extra dynsyms enrich DT_HASH
...
/* Wave 85 soft inventory stamp (source greppable; not a smoke contract). */
...
static const char g_szSoSoftDeepen[] =
    ...
    "retcrownangle=1 retshardangle=1 continuum_toward=22700";
...
/*
 * Soft honesty line (Wave 85 exclusive).
 * Grep: libgj-so: soft honesty
 * Grep: libgj-so: soft retcrownangle / retshardangle (CREATE-ONLY soft ≠ product)
 */
static const char g_szSoSoftHonesty[] =
    ...
    "retcrownangle=1 retshardangle=1 continuum_toward=22700 soft_ne_product=1";

/* Soft area name catalog (Wave 85; cold only). */
...
/*
 * Cold soft deepen stamp (Wave 85). Grep: libgj-so: soft deepen
 */
const char *
...
/*
 * Cold soft honesty line (Wave 85 exclusive). Grep: libgj-so: soft honesty
 */
const char *
...
/* Cold soft inventory: Wave 85 stamp. Grep: libgj-so: soft wave= */
unsigned
