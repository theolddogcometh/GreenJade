 ...
 *
 * Soft deepen surface (Wave 55 exclusive; extra dynsyms enrich DT_HASH
 ...
 *   greppable: libgj-so: soft deepen wave=55
 * Soft batches live under src/ (unwired). Diagnostics only — not bar3.
 *
 * Soft inventory (Wave 55 exclusive deepen):
 *   libgj-so: soft inventory wave=55 surfaces=12 caps=0x1f stamp=SYSV
 *   libgj-so: soft deepen wave=55 areas=export,init,get,hash,stamp,caps,
...
/* Wave 55 soft inventory stamp (source greppable; not a smoke contract). */
...
static const char g_szSoSoftInventory[] =
    ...
    "stamp=SYSV caps=0x1f areas=12 wave=55 surfaces=12 "
...
/*
 * Wave 55 soft deepen stamp.
 * Grep: libgj-so: soft deepen
 * greppable: GJ_SO_SOFT_DEEPEN / libgj-so: soft deepen wave=55
 */
static const char g_szSoSoftDeepen[] =
    "libgj-so: soft deepen wave=55 areas=12 "
    ...
    "retflank=1 retface=1 continuum_toward=19700";
...
/*
 * Soft honesty line (Wave 55 exclusive).
 * Grep: libgj-so: soft honesty
 * Grep: libgj-so: soft retflank / retface (CREATE-ONLY soft ≠ product)
 */
static const char g_szSoSoftHonesty[] =
    "libgj-so: soft honesty multi_server=0 confine=0 bar3=0 exclusive=1 "
    "soft=1 product_kernel=OPEN wave=55 "
    "retflank=1 retface=1 continuum_toward=19700 soft_ne_product=1";

/* Soft area name catalog (Wave 55; cold only). */
...
/*
 * Cold soft deepen stamp (Wave 55). Grep: libgj-so: soft deepen
 */
const char *
...
/*
 * Cold soft honesty line (Wave 55 exclusive). Grep: libgj-so: soft honesty
 */
const char *
...
/* Cold soft inventory: Wave 55 stamp. Grep: libgj-so: soft wave= */
unsigned
