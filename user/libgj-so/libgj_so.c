 ...
 *
 * Soft deepen surface (Wave 32 exclusive; extra dynsyms enrich DT_HASH
 ...
 *   greppable: libgj-so: soft deepen wave=32
 * Soft batches live under src/ (unwired). Diagnostics only — not bar3.
 *
 * Soft inventory (Wave 32 exclusive deepen):
 *   libgj-so: soft inventory wave=32 surfaces=12 caps=0x1f stamp=SYSV
 *   libgj-so: soft deepen wave=32 areas=export,init,get,hash,stamp,caps,
...
/* Wave 32 soft inventory stamp (source greppable; not a smoke contract). */
#define GJ_SO_SOFT_WAVE         32u
...
static const char g_szSoSoftInventory[] =
    ...
    "stamp=SYSV caps=0x1f areas=12 wave=32 surfaces=12 "
 ...
 * greppable: GJ_SO_SOFT_DEEPEN / libgj-so: soft deepen wave=32
 */
static const char g_szSoSoftDeepen[] =
    "libgj-so: soft deepen wave=32 areas=12 "
...
/*
 * Cold soft deepen stamp (Wave 32). Grep: libgj-so: soft deepen
 */
const char *
...
/*
 * Cold soft honesty line (Wave 32 exclusive). Grep: libgj-so: soft honesty
 */
const char *
...
/* Cold soft inventory: Wave 32 stamp. Grep: libgj-so: soft wave= */
unsigned
