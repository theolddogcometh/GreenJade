 ...
 *
 * Soft deepen surface (Wave 73 exclusive; extra dynsyms enrich DT_HASH
...
/* Wave 73 soft inventory stamp (source greppable; not a smoke contract). */
...
static const char g_szSoSoftDeepen[] =
    ...
    "retcurtainface=1 retbastionangle=1 continuum_toward=21500";
...
/*
 * Soft honesty line (Wave 73 exclusive).
 * Grep: libgj-so: soft honesty
 * Grep: libgj-so: soft retcurtainface / retbastionangle (CREATE-ONLY soft ≠ product)
 */
static const char g_szSoSoftHonesty[] =
    ...
    "retcurtainface=1 retbastionangle=1 continuum_toward=21500 soft_ne_product=1";

/* Soft area name catalog (Wave 73; cold only). */
...
/*
 * Cold soft deepen stamp (Wave 73). Grep: libgj-so: soft deepen
 */
const char *
...
/*
 * Cold soft honesty line (Wave 73 exclusive). Grep: libgj-so: soft honesty
 */
const char *
...
/* Cold soft inventory: Wave 73 stamp. Grep: libgj-so: soft wave= */
unsigned
