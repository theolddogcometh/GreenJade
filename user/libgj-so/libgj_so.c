 ...
 *
 * Soft deepen surface (Wave 31 exclusive; extra dynsyms enrich DT_HASH
 ...
 *   greppable: libgj-so: soft deepen wave=31
 * Soft batches live under src/ (unwired). Diagnostics only — not bar3.
 *
 * Soft inventory (Wave 31 exclusive deepen):
 *   libgj-so: soft inventory wave=31 surfaces=12 caps=0x1f stamp=SYSV
 *   libgj-so: soft deepen wave=31 areas=export,init,get,hash,stamp,caps,
...
/* Wave 31 soft inventory stamp (source greppable; not a smoke contract). */
#define GJ_SO_SOFT_WAVE         31u
...
static const char g_szSoSoftInventory[] =
    ...
    "stamp=SYSV caps=0x1f areas=12 wave=31 surfaces=12 "
...
/*
 * Wave 31 soft deepen stamp.
 * Grep: libgj-so: soft deepen
 * greppable: GJ_SO_SOFT_DEEPEN / libgj-so: soft deepen wave=31
 */
static const char g_szSoSoftDeepen[] =
    "libgj-so: soft deepen wave=31 areas=12 "
...
/*
 * Soft honesty line (Wave 31 exclusive).
...
static const char g_szSoSoftHonesty[] =
    ...
    "soft=1 product_kernel=OPEN wave=31";

/* Soft area name catalog (Wave 31; cold only). */
...
/*
 * Cold soft deepen stamp (Wave 31). Grep: libgj-so: soft deepen
 */
const char *
...
/*
 * Cold soft honesty line (Wave 31 exclusive). Grep: libgj-so: soft honesty
 */
const char *
...
/* Cold soft inventory: Wave 31 stamp. Grep: libgj-so: soft wave= */
unsigned
