 ...
 *
 * Soft deepen surface (Wave 28 exclusive; extra dynsyms enrich DT_HASH
 ...
 *   greppable: libgj-so: soft deepen wave=28
 * Soft batches live under src/ (unwired). Diagnostics only — not bar3.
 *
 * Soft inventory (Wave 28 exclusive deepen):
 *   libgj-so: soft inventory wave=28 surfaces=12 caps=0x1f stamp=SYSV
 *   libgj-so: soft deepen wave=28 areas=export,init,get,hash,stamp,caps,
...
/* Wave 28 soft inventory stamp (source greppable; not a smoke contract). */
...
static const char g_szSoSoftInventory[] =
    ...
    "stamp=SYSV caps=0x1f areas=12 wave=28 surfaces=12 "
...
/*
 * Wave 28 soft deepen stamp.
 * Grep: libgj-so: soft deepen
 * greppable: GJ_SO_SOFT_DEEPEN / libgj-so: soft deepen wave=28
 */
static const char g_szSoSoftDeepen[] =
    "libgj-so: soft deepen wave=28 areas=12 "
...
/*
 * Soft honesty line (Wave 28 exclusive).
...
static const char g_szSoSoftHonesty[] =
    ...
    "soft=1 product_kernel=OPEN wave=28";

/* Soft area name catalog (Wave 28; cold only). */
...
/*
 * Cold soft deepen stamp (Wave 28). Grep: libgj-so: soft deepen
 */
const char *
...
/*
 * Cold soft honesty line (Wave 28 exclusive). Grep: libgj-so: soft honesty
 */
const char *
...
/* Cold soft inventory: Wave 28 stamp. Grep: libgj-so: soft wave= */
unsigned
