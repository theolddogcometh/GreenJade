 ...
 *
 * Soft deepen surface (Wave 67 exclusive; extra dynsyms enrich DT_HASH
 ...
 *   greppable: libgj-so: soft deepen wave=67
 * Soft batches live under src/ (unwired). Diagnostics only — not bar3.
 *
 * Soft inventory (Wave 67 exclusive deepen):
 *   libgj-so: soft inventory wave=67 surfaces=12 caps=0x1f stamp=SYSV
 *   libgj-so: soft deepen wave=67 areas=export,init,get,hash,stamp,caps,
...
/* Wave 67 soft inventory stamp (source greppable; not a smoke contract). */
#define GJ_SO_SOFT_WAVE 67u
...
static const char g_szSoSoftInventory[] =
    ...
    "stamp=SYSV caps=0x1f areas=12 wave=67 surfaces=12 "
...
/*
 * Wave 67 soft deepen stamp.
 * Grep: libgj-so: soft deepen
 * greppable: GJ_SO_SOFT_DEEPEN / libgj-so: soft deepen wave=67
 */
static const char g_szSoSoftDeepen[] =
    "libgj-so: soft deepen wave=67 areas=12 "
    ...
    "retcaponierangle=1 retredanangle=1 continuum_toward=20900";
...
/*
 * Soft honesty line (Wave 67 exclusive).
 * Grep: libgj-so: soft honesty
 * Grep: libgj-so: soft retcaponierangle / retredanangle (CREATE-ONLY soft ≠ product)
 */
static const char g_szSoSoftHonesty[] =
    "libgj-so: soft honesty multi_server=0 confine=0 bar3=0 exclusive=1 "
    "soft=1 product_kernel=OPEN wave=67 "
    "retcaponierangle=1 retredanangle=1 continuum_toward=20900 soft_ne_product=1";

/* Soft area name catalog (Wave 67; cold only). */
...
/*
 * Cold soft deepen stamp (Wave 67). Grep: libgj-so: soft deepen
 */
const char *
...
/*
 * Cold soft honesty line (Wave 67 exclusive). Grep: libgj-so: soft honesty
 */
const char *
...
/* Cold soft inventory: Wave 67 stamp. Grep: libgj-so: soft wave= */
unsigned
