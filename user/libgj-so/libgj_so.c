 ...
 *
 * Soft deepen surface (Wave 89 exclusive; extra dynsyms enrich DT_HASH
...
/* Wave 89 soft inventory stamp (source greppable; not a smoke contract). */
...
static const char g_szSoSoftDeepen[] =
    ...
    "retbannerangle=1 retvaultangle=1 continuum_toward=23100";
...
/*
 * Soft honesty line (Wave 89 exclusive).
 * Grep: libgj-so: soft honesty
 * Grep: libgj-so: soft retbannerangle / retvaultangle (CREATE-ONLY soft ≠ product)
 */
static const char g_szSoSoftHonesty[] =
    ...
    "retbannerangle=1 retvaultangle=1 continuum_toward=23100 soft_ne_product=1";

/* Soft area name catalog (Wave 89; cold only). */
...
/*
 * Cold soft deepen stamp (Wave 89). Grep: libgj-so: soft deepen
 */
const char *
...
/*
 * Cold soft honesty line (Wave 89 exclusive). Grep: libgj-so: soft honesty
 */
const char *
...
/* Cold soft inventory: Wave 89 stamp. Grep: libgj-so: soft wave= */
unsigned
