 ...
 *
 * Soft deepen surface (Wave 64 exclusive; extra dynsyms enrich DT_HASH
 ...
 *   greppable: libgj-so: soft deepen wave=64
 * Soft batches live under src/ (unwired). Diagnostics only — not bar3.
 *
 * Soft inventory (Wave 64 exclusive deepen):
 *   libgj-so: soft inventory wave=64 surfaces=12 caps=0x1f stamp=SYSV
 *   libgj-so: soft deepen wave=64 areas=export,init,get,hash,stamp,caps,
...
/* Wave 64 soft inventory stamp (source greppable; not a smoke contract). */
#define GJ_SO_SOFT_WAVE 64u
...
static const char g_szSoSoftInventory[] =
    ...
    "stamp=SYSV caps=0x1f areas=12 wave=64 surfaces=12 "
...
/*
 * Wave 64 soft deepen stamp.
 * Grep: libgj-so: soft deepen
 * greppable: GJ_SO_SOFT_DEEPEN / libgj-so: soft deepen wave=64
 */
static const char g_szSoSoftDeepen[] =
    "libgj-so: soft deepen wave=64 areas=12 "
    ...
    "retreentrant=1 retsallyport=1 continuum_toward=20600";
...
/*
 * Soft honesty line (Wave 64 exclusive).
 * Grep: libgj-so: soft honesty
 * Grep: libgj-so: soft retreentrant / retsallyport (CREATE-ONLY soft ≠ product)
 */
static const char g_szSoSoftHonesty[] =
    "libgj-so: soft honesty multi_server=0 confine=0 bar3=0 exclusive=1 "
    "soft=1 product_kernel=OPEN wave=64 "
    "retreentrant=1 retsallyport=1 continuum_toward=20600 soft_ne_product=1";

/* Soft area name catalog (Wave 64; cold only). */
...
/*
 * Cold soft deepen stamp (Wave 64). Grep: libgj-so: soft deepen
 */
const char *
...
/*
 * Cold soft honesty line (Wave 64 exclusive). Grep: libgj-so: soft honesty
 */
const char *
...
/* Cold soft inventory: Wave 64 stamp. Grep: libgj-so: soft wave= */
unsigned
