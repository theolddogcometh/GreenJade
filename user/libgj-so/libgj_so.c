 ...
 *
 * Soft deepen surface (Wave 70 exclusive; extra dynsyms enrich DT_HASH
...
/* Wave 70 soft inventory stamp (source greppable; not a smoke contract). */
...
static const char g_szSoSoftDeepen[] =
    ...
    "retfosseangle=1 retcounterscarple=1 continuum_toward=21200";
...
/*
 * Soft honesty line (Wave 70 exclusive).
 * Grep: libgj-so: soft honesty
 * Grep: libgj-so: soft retfosseangle / retcounterscarple (CREATE-ONLY soft ≠ product)
 */
static const char g_szSoSoftHonesty[] =
    ...
    "retfosseangle=1 retcounterscarple=1 continuum_toward=21200 soft_ne_product=1";

/* Soft area name catalog (Wave 70; cold only). */
...
/*
 * Cold soft deepen stamp (Wave 70). Grep: libgj-so: soft deepen
 */
const char *
...
/*
 * Cold soft honesty line (Wave 70 exclusive). Grep: libgj-so: soft honesty
 */
const char *
...
/* Cold soft inventory: Wave 70 stamp. Grep: libgj-so: soft wave= */
unsigned
