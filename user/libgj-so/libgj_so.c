 ...
 *
 * Soft deepen surface (Wave 21 exclusive; extra dynsyms enrich DT_HASH
 ...
 *   greppable: libgj-so: soft deepen wave=21
 * Soft batches live under src/ (unwired). Diagnostics only — not bar3.
 *
 * Soft inventory (Wave 21 exclusive deepen):
 *   libgj-so: soft inventory wave=21 surfaces=12 caps=0x1f stamp=SYSV
 *   libgj-so: soft deepen wave=21 areas=export,init,get,hash,stamp,caps,
...
/* Wave 21 soft inventory stamp (source greppable; not a smoke contract). */
#define GJ_SO_SOFT_WAVE         21u
...
static const char g_szSoSoftInventory[] =
    ...
    "stamp=SYSV caps=0x1f areas=12 wave=21 surfaces=12 "
...
/*
 * Wave 21 soft deepen stamp.
 * Grep: libgj-so: soft deepen
 * greppable: GJ_SO_SOFT_DEEPEN / libgj-so: soft deepen wave=21
 */
static const char g_szSoSoftDeepen[] =
    "libgj-so: soft deepen wave=21 areas=12 "
...
/*
 * Soft honesty line (Wave 21 exclusive).
...
static const char g_szSoSoftHonesty[] =
    ...
    "soft=1 product_kernel=OPEN wave=21";

/* Soft area name catalog (Wave 21; cold only). */
...
/*
 * Cold soft deepen stamp (Wave 21). Grep: libgj-so: soft deepen
 */
const char *
...
/*
 * Cold soft honesty line (Wave 21 exclusive). Grep: libgj-so: soft honesty
 */
const char *
...
/* Cold soft inventory: Wave 21 stamp. Grep: libgj-so: soft wave= */
unsigned
