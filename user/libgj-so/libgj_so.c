 ...
 *
 * Soft deepen surface (Wave 42 exclusive; extra dynsyms enrich DT_HASH
 ...
 *   greppable: libgj-so: soft deepen wave=42
 * Soft batches live under src/ (unwired). Diagnostics only — not bar3.
 *
 * Soft inventory (Wave 42 exclusive deepen):
 *   libgj-so: soft inventory wave=42 surfaces=12 caps=0x1f stamp=SYSV
 *   libgj-so: soft deepen wave=42 areas=export,init,get,hash,stamp,caps,
...
/* Wave 42 soft inventory stamp (source greppable; not a smoke contract). */
...
static const char g_szSoSoftInventory[] =
    ...
    "stamp=SYSV caps=0x1f areas=12 wave=42 surfaces=12 "
...
/*
 * Wave 42 soft deepen stamp.
 * Grep: libgj-so: soft deepen
 * greppable: GJ_SO_SOFT_DEEPEN / libgj-so: soft deepen wave=42
 */
static const char g_szSoSoftDeepen[] =
    "libgj-so: soft deepen wave=42 areas=12 "
    ...
    "retportcullis=1 retbattlement=1 continuum_toward=18400";
...
/*
 * Soft honesty line (Wave 42 exclusive).
 * Grep: libgj-so: soft honesty
 * Grep: libgj-so: soft retportcullis / retbattlement (CREATE-ONLY soft ≠ product)
 */
static const char g_szSoSoftHonesty[] =
    ...
    "retportcullis=1 retbattlement=1 continuum_toward=18400 soft_ne_product=1";
...
/*
 * Cold soft deepen stamp (Wave 42). Grep: libgj-so: soft deepen
 */
const char *
...
/*
 * Cold soft honesty line (Wave 42 exclusive). Grep: libgj-so: soft honesty
 */
const char *
...
/* Cold soft inventory: Wave 42 stamp. Grep: libgj-so: soft wave= */
unsigned
