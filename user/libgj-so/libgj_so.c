 ...
 *
 * Soft deepen surface (Wave 52 exclusive; extra dynsyms enrich DT_HASH
 ...
 *   greppable: libgj-so: soft deepen wave=52
 * Soft batches live under src/ (unwired). Diagnostics only — not bar3.
 *
 * Soft inventory (Wave 52 exclusive deepen):
 *   libgj-so: soft inventory wave=52 surfaces=12 caps=0x1f stamp=SYSV
 *   libgj-so: soft deepen wave=52 areas=export,init,get,hash,stamp,caps,
...
/* Wave 52 soft inventory stamp (source greppable; not a smoke contract). */
#define GJ_SO_SOFT_WAVE 52u
...
static const char g_szSoSoftInventory[] =
    ...
    "stamp=SYSV caps=0x1f areas=12 wave=52 surfaces=12 "
...
/*
 * Wave 52 soft deepen stamp.
 * Grep: libgj-so: soft deepen
 * greppable: GJ_SO_SOFT_DEEPEN / libgj-so: soft deepen wave=52
 */
static const char g_szSoSoftDeepen[] =
    "libgj-so: soft deepen wave=52 areas=12 "
    ...
    "rettenaille=1 retdemilune=1 continuum_toward=19400";
...
/*
 * Soft honesty line (Wave 52 exclusive).
 * Grep: libgj-so: soft honesty
 * Grep: libgj-so: soft rettenaille / retdemilune (CREATE-ONLY soft ≠ product)
 */
static const char g_szSoSoftHonesty[] =
    "libgj-so: soft honesty multi_server=0 confine=0 bar3=0 exclusive=1 "
    "soft=1 product_kernel=OPEN wave=52 "
    "rettenaille=1 retdemilune=1 continuum_toward=19400 soft_ne_product=1";

/* Soft area name catalog (Wave 52; cold only). */
...
/*
 * Cold soft deepen stamp (Wave 52). Grep: libgj-so: soft deepen
 */
const char *
...
/*
 * Cold soft honesty line (Wave 52 exclusive). Grep: libgj-so: soft honesty
 */
const char *
...
/* Cold soft inventory: Wave 52 stamp. Grep: libgj-so: soft wave= */
unsigned
