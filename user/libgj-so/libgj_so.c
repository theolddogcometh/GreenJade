 ...
 *
 * Soft deepen surface (Wave 98 exclusive; extra dynsyms enrich DT_HASH
...
/* Wave 98 soft inventory stamp (source greppable; not a smoke contract). */
...
static const char g_szSoSoftDeepen[] =
    ...
    "retcircumangle=1 retellipseangle=1 continuum_toward=24000 soft_ne_product=1 wave=98";
...
/*
 * Soft honesty line (Wave 98 exclusive).
 * Grep: libgj-so: soft honesty
 * Grep: libgj-so: soft retcircumangle / retellipseangle (CREATE-ONLY soft ≠ product)
 */
...
/* Soft area name catalog (Wave 98; cold only). */
...
/*
 * Cold soft deepen stamp (Wave 98). Grep: libgj-so: soft deepen
 */
const char *
...
/*
 * Cold soft honesty line (Wave 98 exclusive). Grep: libgj-so: soft honesty
 */
const char *
...
/* Cold soft inventory: Wave 98 stamp. Grep: libgj-so: soft wave= */
unsigned
