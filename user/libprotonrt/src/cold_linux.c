...
/*
 * Soft product inventory (Wave 22 exclusive deepen). Enter-only tallies.
 ...
 * Twin prefixes so either agent grep works (Wave 22 exclusive deepen):
 *   protonrt: soft inventory|fd|io|fd_alloc|stat|namei|id|time|poll|sock|
 *             proc|uring|enosys|query|path|groups|last|open|deepen|wave …
 *   cold_linux: soft inventory|fd|io|fd_alloc|stat|namei|id|time|poll|sock|
 *               proc|uring|enosys|query|path|groups|last|open|deepen|wave …
...
soft_inventory_log(void)
{
    ...
    /* Grep: protonrt: soft groups (Wave 22 deepen — all enters one line) */
    ...
    /* Grep: protonrt: soft last (Wave 22 deepen) */
    ...
    /*
     * Grep: protonrt: soft honesty (Wave 22 exclusive deepen).
    ...
    /* Grep: cold_linux: soft groups (Wave 22 deepen) */
    ...
    /* Grep: cold_linux: soft last (Wave 22 deepen) */
    ...
    /* Grep: cold_linux: soft open (Wave 22 honesty) */
    kprintf("cold_linux: soft open kernel_notify=OPEN multi_process_spsc=OPEN "
            "driver_host=OPEN bar3=OPEN product=0 soft=1 wave=%u\n",
            (unsigned)COLD_SOFT_WAVE);

    /* Grep: cold_linux: soft deepen wave (Wave 22 stamp) */
    ...
    /* Grep: cold_linux: soft honesty (Wave 22 exclusive deepen) */
...
proton_rt_query(struct proton_rt_info *pOut)
{
    ...
    /* Wave 22: one-shot greppable soft inventory after first query. */
    soft_inventory_maybe_once();
    return 0;
}
                    ...
                    uint64_t a3, uint64_t a4, uint64_t a5)
{
    ...
    /* Wave 22 soft enter — never rewrites ret. greppable: protonrt: soft */
