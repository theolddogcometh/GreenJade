...
| Binary | Build | Role |
|--------|-------|------|
| `build/user/netstackd.elf` | `make netstackd-gj` | Freestanding product daemon (net door, TCP multi-seg, virtio ring) |
...
- Freestanding `netstackd-gj`: door CLAIM, dgram echo, **TCP listen/accept/send/recv**, **multi-segment TX/RX**, virtio ring path.
...
| `README.md` | This file — markers, TCP multi-seg, door bounce |
...
Typical freestanding success sequence (order fixed where noted):
...
| `avail push PASS` | `AVAIL_PUSH` + ring state soft green |
| `user ring PASS` | UDX user desc/avail programming path |
