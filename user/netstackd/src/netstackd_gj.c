 ...
 * Soft inventory (Wave 52 exclusive deepen — greppable "netstackd-gj: soft …"):
 ...
 *   netstackd-gj: soft deepen wave=52 areas=… ok=… skip=…
...
/* Soft free-path bit (Wave 52 inventory; post-RELEASE no-op). */
...
/* Wave stamp for greppable soft inventory lines (Wave 52 exclusive). */
#define GJ_SOFT_WAVE 52u
 ...
 * Wave 52 inventory tallies: ok/skip + door lamps + free bit + snapshots.
 */
struct soft_ctx {
...
/* Wave 52: 0/1 lamp from soft door bit mask. */
static unsigned
...
soft_inventory_log(const struct soft_ctx *pSoft)
{
    ...
    /* Grep: netstackd-gj: soft deepen wave (Wave 52 stamp) */
    o = 0;
    ...
    /*
     * Grep: netstackd-gj: soft honesty (Wave 52 exclusive deepen).
     * Soft inventory ≠ product multi-server confine.
     */
               ...
               int fTcpStats)
{
     ...
     * free_rel may still be 0 here; final Wave 52 rollup re-emits after free.
     */
...
_start(void)
{
     ...
     * Wave 52: also stamp uFreeBits and re-emit soft inventory (free_rel).
     */
    ...
    /* Final Wave 52 soft inventory rollup (includes free_rel / free bits). */
