 ...
 * Soft inventory (Wave 66 exclusive deepen — greppable "netstackd-gj: soft …"):
 ...
 *   netstackd-gj: soft deepen wave=66 areas=… ok=… skip=…
...
/* Soft free-path bit (Wave 66 inventory; post-RELEASE no-op). */
...
/* Wave stamp for greppable soft inventory lines (Wave 66 exclusive). */
#define GJ_SOFT_WAVE 66u
 ...
 * Wave 66 inventory tallies: ok/skip + door lamps + free bit + snapshots.
 */
struct soft_ctx {
...
/* Wave 66: 0/1 lamp from soft door bit mask. */
static unsigned
...
soft_inventory_log(const struct soft_ctx *pSoft)
{
    ...
    /* Grep: netstackd-gj: soft deepen wave (Wave 66 stamp) */
    o = 0;
    ...
    /*
     * Grep: netstackd-gj: soft honesty (Wave 66 exclusive deepen).
     * Soft inventory ≠ product multi-server confine.
     */
               ...
               int fTcpStats)
{
     ...
     * free_rel may still be 0 here; final Wave 66 rollup re-emits after free.
     */
...
_start(void)
{
     ...
     * Wave 66: also stamp uFreeBits and re-emit soft inventory (free_rel).
     */
    ...
    /* Final Wave 66 soft inventory rollup (includes free_rel / free bits). */
