 ...
 * Soft inventory (Wave 76 exclusive deepen — greppable "netstackd-gj: soft …"):
...
/* Soft free-path bit (Wave 76 inventory; post-RELEASE no-op). */
...
/* Wave stamp for greppable soft inventory lines (Wave 76 exclusive). */
 ...
 * Wave 76 inventory tallies: ok/skip + door lamps + free bit + snapshots.
 */
struct soft_ctx {
...
/* Wave 76: 0/1 lamp from soft door bit mask. */
static unsigned
...
soft_inventory_log(const struct soft_ctx *pSoft)
{
    ...
    /* Grep: netstackd-gj: soft deepen wave (Wave 76 stamp) */
    o = 0;
    ...
    /*
     * Grep: netstackd-gj: soft honesty (Wave 76 exclusive deepen).
     * Soft inventory ≠ product multi-server confine.
     */
               ...
               int fTcpStats)
{
     ...
     * free_rel may still be 0 here; final Wave 76 rollup re-emits after free.
     */
...
_start(void)
{
     ...
     * Wave 76: also stamp uFreeBits and re-emit soft inventory (free_rel).
     */
    ...
    /* Final Wave 76 soft inventory rollup (includes free_rel / free bits). */
