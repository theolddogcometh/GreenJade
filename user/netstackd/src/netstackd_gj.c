 ...
 * Soft inventory (Wave 30 exclusive deepen — greppable "netstackd-gj: soft …"):
 ...
 *   netstackd-gj: soft deepen wave=30 areas=… ok=… skip=…
...
/* Soft free-path bit (Wave 30 inventory; post-RELEASE no-op). */
...
/* Wave stamp for greppable soft inventory lines (Wave 30 exclusive). */
#define GJ_SOFT_WAVE         30u
 ...
 * Wave 30 inventory tallies: ok/skip + door lamps + free bit + snapshots.
 */
struct soft_ctx {
...
/* Wave 30: 0/1 lamp from soft door bit mask. */
static unsigned
...
soft_inventory_log(const struct soft_ctx *pSoft)
{
    ...
    /* Grep: netstackd-gj: soft deepen wave (Wave 30 stamp) */
    o = 0;
    ...
    /*
     * Grep: netstackd-gj: soft honesty (Wave 30 exclusive deepen).
     * Soft inventory ≠ product multi-server confine.
     */
               ...
               int fTcpStats)
{
     ...
     * free_rel may still be 0 here; final Wave 30 rollup re-emits after free.
     */
...
_start(void)
{
     ...
     * Wave 30: also stamp uFreeBits and re-emit soft inventory (free_rel).
     */
    ...
    /* Final Wave 30 soft inventory rollup (includes free_rel / free bits). */
