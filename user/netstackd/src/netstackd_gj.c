 ...
 * Soft inventory (Wave 104 exclusive deepen — greppable "netstackd-gj: soft …"):
...
/* Soft free-path bit (Wave 104 inventory; post-RELEASE no-op). */
...
/* Wave stamp for greppable soft inventory lines (Wave 104 exclusive). */
 ...
 * Wave 104 inventory tallies: ok/skip + door lamps + free bit + snapshots.
 */
struct soft_ctx {
...
/* Wave 104: 0/1 lamp from soft door bit mask. */
static unsigned
...
soft_inventory_log(const struct soft_ctx *pSoft)
{
    ...
    /* Grep: netstackd-gj: soft deepen wave (Wave 104 stamp) */
    o = 0;
    ...
    /*
     * Grep: netstackd-gj: soft honesty (Wave 104 exclusive deepen).
     * Soft inventory ≠ product multi-server confine.
     */
               ...
               int fTcpStats)
{
     ...
     * free_rel may still be 0 here; final Wave 104 rollup re-emits after free.
     */
...
_start(void)
{
     ...
     * Wave 104: also stamp uFreeBits and re-emit soft inventory (free_rel).
     */
    ...
    /* Final Wave 104 soft inventory rollup (includes free_rel / free bits). */
