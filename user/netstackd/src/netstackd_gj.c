 ...
 * Soft inventory (Wave 18 exclusive deepen — greppable "netstackd-gj: soft …"):
 ...
 *   netstackd-gj: soft deepen wave=18 areas=… ok=… skip=…
...
/* Soft free-path bit (Wave 18 inventory; post-RELEASE no-op). */
...
/* Wave stamp for greppable soft inventory lines (Wave 18 exclusive). */
#define GJ_SOFT_WAVE         18u
 ...
 * Wave 18 inventory tallies: ok/skip + door lamps + free bit + snapshots.
 */
struct soft_ctx {
...
/* Wave 18: 0/1 lamp from soft door bit mask. */
static unsigned
...
soft_inventory_log(const struct soft_ctx *pSoft)
{
    ...
    /* Grep: netstackd-gj: soft deepen wave (Wave 18 stamp) */
    o = 0;
    ...
    /*
     * Grep: netstackd-gj: soft honesty (Wave 18 exclusive deepen).
     * Soft inventory ≠ product multi-server confine.
     */
               ...
               int fTcpStats)
{
     ...
     * free_rel may still be 0 here; final Wave 18 rollup re-emits after free.
     */
...
_start(void)
{
     ...
     * Wave 18: also stamp uFreeBits and re-emit soft inventory (free_rel).
     */
    ...
    /* Final Wave 18 soft inventory rollup (includes free_rel / free bits). */
