 ...
 * Soft inventory (Wave 105 exclusive deepen — greppable "netstackd-gj: soft …"):
...
/* Soft free-path bit (Wave 105 inventory; post-RELEASE no-op). */
...
/* Wave stamp for greppable soft inventory lines (Wave 105 exclusive). */
 ...
 * Wave 105 inventory tallies: ok/skip + door lamps + free bit + snapshots.
 */
struct soft_ctx {
...
/* Wave 105: 0/1 lamp from soft door bit mask. */
static unsigned
...
soft_inventory_log(const struct soft_ctx *pSoft)
{
    ...
    /* Grep: netstackd-gj: soft deepen wave (Wave 105 stamp) */
    o = 0;
    ...
    /*
     * Grep: netstackd-gj: soft honesty (Wave 105 exclusive deepen).
     * Soft inventory ≠ product multi-server confine.
     */
               ...
               int fTcpStats)
{
     ...
     * free_rel may still be 0 here; final Wave 105 rollup re-emits after free.
     */
...
_start(void)
{
     ...
     * Wave 105: also stamp uFreeBits and re-emit soft inventory (free_rel).
     */
    ...
    /* Final Wave 105 soft inventory rollup (includes free_rel / free bits). */
