 ...
 * Soft inventory (Wave 43 exclusive deepen — greppable "netstackd-gj: soft …"):
 ...
 *   netstackd-gj: soft deepen wave=43 areas=… ok=… skip=…
...
/* Soft free-path bit (Wave 43 inventory; post-RELEASE no-op). */
...
/* Wave stamp for greppable soft inventory lines (Wave 43 exclusive). */
 ...
 * Wave 43 inventory tallies: ok/skip + door lamps + free bit + snapshots.
 */
struct soft_ctx {
...
/* Wave 43: 0/1 lamp from soft door bit mask. */
static unsigned
...
soft_inventory_log(const struct soft_ctx *pSoft)
{
    ...
    /* Grep: netstackd-gj: soft deepen wave (Wave 43 stamp) */
    o = 0;
    ...
    /*
     * Grep: netstackd-gj: soft honesty (Wave 43 exclusive deepen).
     * Soft inventory ≠ product multi-server confine.
     */
               ...
               int fTcpStats)
{
     ...
     * free_rel may still be 0 here; final Wave 43 rollup re-emits after free.
     */
...
_start(void)
{
     ...
     * Wave 43: also stamp uFreeBits and re-emit soft inventory (free_rel).
     */
    ...
    /* Final Wave 43 soft inventory rollup (includes free_rel / free bits). */
