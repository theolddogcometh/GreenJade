 ...
 * Soft inventory (Wave 41 exclusive deepen — greppable "netstackd-gj: soft …"):
 ...
 *   netstackd-gj: soft deepen wave=41 areas=… ok=… skip=…
...
/* Soft free-path bit (Wave 41 inventory; post-RELEASE no-op). */
...
/* Wave stamp for greppable soft inventory lines (Wave 41 exclusive). */
 ...
 * Wave 41 inventory tallies: ok/skip + door lamps + free bit + snapshots.
 */
struct soft_ctx {
...
/* Wave 41: 0/1 lamp from soft door bit mask. */
static unsigned
...
soft_inventory_log(const struct soft_ctx *pSoft)
{
    ...
    /* Grep: netstackd-gj: soft deepen wave (Wave 41 stamp) */
    o = 0;
    ...
    /*
     * Grep: netstackd-gj: soft honesty (Wave 41 exclusive deepen).
     * Soft inventory ≠ product multi-server confine.
     */
               ...
               int fTcpStats)
{
     ...
     * free_rel may still be 0 here; final Wave 41 rollup re-emits after free.
     */
...
_start(void)
{
     ...
     * Wave 41: also stamp uFreeBits and re-emit soft inventory (free_rel).
     */
    ...
    /* Final Wave 41 soft inventory rollup (includes free_rel / free bits). */
