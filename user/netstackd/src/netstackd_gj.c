 ...
 * Soft inventory (Wave 49 exclusive deepen — greppable "netstackd-gj: soft …"):
 ...
 *   netstackd-gj: soft deepen wave=49 areas=… ok=… skip=…
...
/* Soft free-path bit (Wave 49 inventory; post-RELEASE no-op). */
...
/* Wave stamp for greppable soft inventory lines (Wave 49 exclusive). */
 ...
 * Wave 49 inventory tallies: ok/skip + door lamps + free bit + snapshots.
 */
struct soft_ctx {
...
/* Wave 49: 0/1 lamp from soft door bit mask. */
static unsigned
...
soft_inventory_log(const struct soft_ctx *pSoft)
{
    ...
    /* Grep: netstackd-gj: soft deepen wave (Wave 49 stamp) */
    o = 0;
    ...
    /*
     * Grep: netstackd-gj: soft honesty (Wave 49 exclusive deepen).
     * Soft inventory ≠ product multi-server confine.
     */
               ...
               int fTcpStats)
{
     ...
     * free_rel may still be 0 here; final Wave 49 rollup re-emits after free.
     */
...
_start(void)
{
     ...
     * Wave 49: also stamp uFreeBits and re-emit soft inventory (free_rel).
     */
    ...
    /* Final Wave 49 soft inventory rollup (includes free_rel / free bits). */
