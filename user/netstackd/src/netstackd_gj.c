 ...
 * Soft inventory (Wave 29 exclusive deepen — greppable "netstackd-gj: soft …"):
 ...
 *   netstackd-gj: soft deepen wave=29 areas=… ok=… skip=…
...
/* Soft free-path bit (Wave 29 inventory; post-RELEASE no-op). */
...
/* Wave stamp for greppable soft inventory lines (Wave 29 exclusive). */
 ...
 * Wave 29 inventory tallies: ok/skip + door lamps + free bit + snapshots.
 */
struct soft_ctx {
...
/* Wave 29: 0/1 lamp from soft door bit mask. */
static unsigned
...
soft_inventory_log(const struct soft_ctx *pSoft)
{
    ...
    /* Grep: netstackd-gj: soft deepen wave (Wave 29 stamp) */
    o = 0;
    ...
    /*
     * Grep: netstackd-gj: soft honesty (Wave 29 exclusive deepen).
     * Soft inventory ≠ product multi-server confine.
     */
               ...
               int fTcpStats)
{
     ...
     * free_rel may still be 0 here; final Wave 29 rollup re-emits after free.
     */
...
_start(void)
{
     ...
     * Wave 29: also stamp uFreeBits and re-emit soft inventory (free_rel).
     */
    ...
    /* Final Wave 29 soft inventory rollup (includes free_rel / free bits). */
