 ...
 * Soft inventory (Wave 57 exclusive deepen — greppable "netstackd-gj: soft …"):
 ...
 *   netstackd-gj: soft deepen wave=57 areas=… ok=… skip=…
...
/* Soft free-path bit (Wave 57 inventory; post-RELEASE no-op). */
...
/* Wave stamp for greppable soft inventory lines (Wave 57 exclusive). */
#define GJ_SOFT_WAVE 55u
 ...
 * Wave 57 inventory tallies: ok/skip + door lamps + free bit + snapshots.
 */
struct soft_ctx {
...
/* Wave 57: 0/1 lamp from soft door bit mask. */
static unsigned
...
soft_inventory_log(const struct soft_ctx *pSoft)
{
    ...
    /* Grep: netstackd-gj: soft deepen wave (Wave 57 stamp) */
    o = 0;
    ...
    /*
     * Grep: netstackd-gj: soft honesty (Wave 57 exclusive deepen).
     * Soft inventory ≠ product multi-server confine.
     */
               ...
               int fTcpStats)
{
     ...
     * free_rel may still be 0 here; final Wave 57 rollup re-emits after free.
     */
...
_start(void)
{
     ...
     * Wave 57: also stamp uFreeBits and re-emit soft inventory (free_rel).
     */
    ...
    /* Final Wave 57 soft inventory rollup (includes free_rel / free bits). */
