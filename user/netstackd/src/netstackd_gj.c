 ...
 * Soft inventory (Wave 53 exclusive deepen — greppable "netstackd-gj: soft …"):
 ...
 *   netstackd-gj: soft deepen wave=53 areas=… ok=… skip=…
...
/* Soft free-path bit (Wave 53 inventory; post-RELEASE no-op). */
...
/* Wave stamp for greppable soft inventory lines (Wave 53 exclusive). */
#define GJ_SOFT_WAVE 53u
 ...
 * Wave 53 inventory tallies: ok/skip + door lamps + free bit + snapshots.
 */
struct soft_ctx {
...
/* Wave 53: 0/1 lamp from soft door bit mask. */
static unsigned
...
soft_inventory_log(const struct soft_ctx *pSoft)
{
    ...
    /* Grep: netstackd-gj: soft deepen wave (Wave 53 stamp) */
    o = 0;
    ...
    /*
     * Grep: netstackd-gj: soft honesty (Wave 53 exclusive deepen).
     * Soft inventory ≠ product multi-server confine.
     */
               ...
               int fTcpStats)
{
     ...
     * free_rel may still be 0 here; final Wave 53 rollup re-emits after free.
     */
...
_start(void)
{
     ...
     * Wave 53: also stamp uFreeBits and re-emit soft inventory (free_rel).
     */
    ...
    /* Final Wave 53 soft inventory rollup (includes free_rel / free bits). */
