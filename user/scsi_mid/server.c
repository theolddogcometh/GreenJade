 ...
 *   soft inventory (Wave 22) → stats
 *
 * Soft inventory (Wave 22 exclusive deepen — greppable):
 *   scsi_mid-server: soft inventory …
 *   scsi_mid-server: soft deepen wave=22 …
...
/* Wave 22 soft inventory surface from cdb.c (no public header change). */
void scsi_mid_soft_inventory_log(void);

#define SOFT_HOST_WAVE 17u
...
main(void)
{
    ...
    /*
     * Wave 22 deepen probes (always soft for inventory; hard fail host smoke
    ...
    /* Wave 22 soft inventory — library + host skeleton surfaces. */
    ...
    /* Grep: scsi_mid-server: soft honesty (Wave 22 exclusive deepen) */
