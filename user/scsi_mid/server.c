 ...
 *   soft inventory (Wave 62) → stats
 *
 * Soft inventory (Wave 62 exclusive deepen — greppable):
 *   scsi_mid-server: soft inventory …
 *   scsi_mid-server: soft deepen wave=62 …
...
/* Wave 62 soft inventory surface from cdb.c (no public header change). */
...
main(void)
{
    ...
    /*
     * Wave 62 deepen probes (always soft for inventory; hard fail host smoke
    ...
    /* Wave 62 soft inventory — library + host skeleton surfaces. */
    ...
    /* Grep: scsi_mid-server: soft honesty (Wave 62 exclusive deepen) */
