 ...
 *   soft inventory (Wave 30) → stats
 *
 * Soft inventory (Wave 30 exclusive deepen — greppable):
 *   scsi_mid-server: soft inventory …
 *   scsi_mid-server: soft deepen wave=30 …
...
/* Wave 30 soft inventory surface from cdb.c (no public header change). */
...
main(void)
{
    ...
    /*
     * Wave 30 deepen probes (always soft for inventory; hard fail host smoke
    ...
    /* Wave 30 soft inventory — library + host skeleton surfaces. */
    ...
    /* Grep: scsi_mid-server: soft honesty (Wave 30 exclusive deepen) */
