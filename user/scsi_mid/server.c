 ...
 *   soft inventory (Wave 48) → stats
 *
 * Soft inventory (Wave 48 exclusive deepen — greppable):
 *   scsi_mid-server: soft inventory …
 *   scsi_mid-server: soft deepen wave=48 …
...
/* Wave 48 soft inventory surface from cdb.c (no public header change). */
...
main(void)
{
    ...
    /*
     * Wave 48 deepen probes (always soft for inventory; hard fail host smoke
    ...
    /* Wave 48 soft inventory — library + host skeleton surfaces. */
    ...
    /* Grep: scsi_mid-server: soft honesty (Wave 48 exclusive deepen) */
