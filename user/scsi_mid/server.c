 ...
 *   soft inventory (Wave 42) → stats
 *
 * Soft inventory (Wave 42 exclusive deepen — greppable):
 *   scsi_mid-server: soft inventory …
 *   scsi_mid-server: soft deepen wave=42 …
...
/* Wave 42 soft inventory surface from cdb.c (no public header change). */
...
main(void)
{
    ...
    /*
     * Wave 42 deepen probes (always soft for inventory; hard fail host smoke
    ...
    /* Wave 42 soft inventory — library + host skeleton surfaces. */
    ...
    /* Grep: scsi_mid-server: soft honesty (Wave 42 exclusive deepen) */
