 ...
 *   soft inventory (Wave 23) → stats
 *
 * Soft inventory (Wave 23 exclusive deepen — greppable):
 *   scsi_mid-server: soft inventory …
 *   scsi_mid-server: soft deepen wave=23 …
...
/* Wave 23 soft inventory surface from cdb.c (no public header change). */
void scsi_mid_soft_inventory_log(void);

#define SOFT_HOST_WAVE 23u
...
main(void)
{
    ...
    /*
     * Wave 23 deepen probes (always soft for inventory; hard fail host smoke
    ...
    /* Wave 23 soft inventory — library + host skeleton surfaces. */
    ...
    /* Grep: scsi_mid-server: soft honesty (Wave 23 exclusive deepen) */
