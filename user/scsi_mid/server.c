 ...
 *   soft inventory (Wave 24) → stats
 *
 * Soft inventory (Wave 24 exclusive deepen — greppable):
 *   scsi_mid-server: soft inventory …
 *   scsi_mid-server: soft deepen wave=24 …
...
/* Wave 24 soft inventory surface from cdb.c (no public header change). */
...
main(void)
{
    ...
    /*
     * Wave 24 deepen probes (always soft for inventory; hard fail host smoke
    ...
    /* Wave 24 soft inventory — library + host skeleton surfaces. */
    ...
    /* Grep: scsi_mid-server: soft honesty (Wave 24 exclusive deepen) */
