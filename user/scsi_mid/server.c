 ...
 *   soft inventory (Wave 54) → stats
 *
 * Soft inventory (Wave 54 exclusive deepen — greppable):
 *   scsi_mid-server: soft inventory …
 *   scsi_mid-server: soft deepen wave=54 …
...
/* Wave 54 soft inventory surface from cdb.c (no public header change). */
...
main(void)
{
    ...
    /*
     * Wave 54 deepen probes (always soft for inventory; hard fail host smoke
    ...
    /* Wave 54 soft inventory — library + host skeleton surfaces. */
    ...
    /* Grep: scsi_mid-server: soft honesty (Wave 54 exclusive deepen) */
