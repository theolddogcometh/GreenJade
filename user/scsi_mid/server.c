 ...
 *   soft inventory (Wave 64) → stats
 *
 * Soft inventory (Wave 64 exclusive deepen — greppable):
 *   scsi_mid-server: soft inventory …
 *   scsi_mid-server: soft deepen wave=64 …
...
/* Wave 64 soft inventory surface from cdb.c (no public header change). */
...
main(void)
{
    ...
    /*
     * Wave 64 deepen probes (always soft for inventory; hard fail host smoke
    ...
    /* Wave 64 soft inventory — library + host skeleton surfaces. */
    ...
    /* Grep: scsi_mid-server: soft honesty (Wave 64 exclusive deepen) */
