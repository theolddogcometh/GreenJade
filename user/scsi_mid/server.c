 ...
 *   soft inventory (Wave 28) → stats
 *
 * Soft inventory (Wave 28 exclusive deepen — greppable):
 *   scsi_mid-server: soft inventory …
 *   scsi_mid-server: soft deepen wave=28 …
...
/* Wave 28 soft inventory surface from cdb.c (no public header change). */
...
main(void)
{
    ...
    /*
     * Wave 28 deepen probes (always soft for inventory; hard fail host smoke
    ...
    /* Wave 28 soft inventory — library + host skeleton surfaces. */
    ...
    /* Grep: scsi_mid-server: soft honesty (Wave 28 exclusive deepen) */
