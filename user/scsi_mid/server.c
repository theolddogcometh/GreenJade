 ...
 *   soft inventory (Wave 29) → stats
 *
 * Soft inventory (Wave 29 exclusive deepen — greppable):
 *   scsi_mid-server: soft inventory …
 *   scsi_mid-server: soft deepen wave=29 …
...
/* Wave 29 soft inventory surface from cdb.c (no public header change). */
void scsi_mid_soft_inventory_log(void);

#define SOFT_HOST_WAVE 25u
...
main(void)
{
    ...
    /*
     * Wave 29 deepen probes (always soft for inventory; hard fail host smoke
    ...
    /* Wave 29 soft inventory — library + host skeleton surfaces. */
    ...
    /* Grep: scsi_mid-server: soft honesty (Wave 29 exclusive deepen) */
