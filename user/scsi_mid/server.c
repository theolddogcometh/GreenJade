 ...
 *   soft inventory (Wave 46) → stats
 *
 * Soft inventory (Wave 46 exclusive deepen — greppable):
 *   scsi_mid-server: soft inventory …
 *   scsi_mid-server: soft deepen wave=46 …
...
/* Wave 46 soft inventory surface from cdb.c (no public header change). */
...
main(void)
{
    ...
    /*
     * Wave 46 deepen probes (always soft for inventory; hard fail host smoke
    ...
    /* Wave 46 soft inventory — library + host skeleton surfaces. */
    ...
    /* Grep: scsi_mid-server: soft honesty (Wave 46 exclusive deepen) */
