 ...
 *   soft inventory (Wave 33) → stats
 *
 * Soft inventory (Wave 33 exclusive deepen — greppable):
 *   scsi_mid-server: soft inventory …
 *   scsi_mid-server: soft deepen wave=33 …
...
/* Wave 33 soft inventory surface from cdb.c (no public header change). */
...
main(void)
{
    ...
    /*
     * Wave 33 deepen probes (always soft for inventory; hard fail host smoke
    ...
    /* Wave 33 soft inventory — library + host skeleton surfaces. */
    ...
    /* Grep: scsi_mid-server: soft honesty (Wave 33 exclusive deepen) */
