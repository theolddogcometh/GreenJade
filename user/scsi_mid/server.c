 ...
 *   soft inventory (Wave 40) → stats
 *
 * Soft inventory (Wave 40 exclusive deepen — greppable):
 *   scsi_mid-server: soft inventory …
 *   scsi_mid-server: soft deepen wave=40 …
...
/* Wave 40 soft inventory surface from cdb.c (no public header change). */
...
main(void)
{
    ...
    /*
     * Wave 40 deepen probes (always soft for inventory; hard fail host smoke
    ...
    /* Wave 40 soft inventory — library + host skeleton surfaces. */
    ...
    /* Grep: scsi_mid-server: soft honesty (Wave 40 exclusive deepen) */
