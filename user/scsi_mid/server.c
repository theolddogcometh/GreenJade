 ...
 *   soft inventory (Wave 63) → stats
 *
 * Soft inventory (Wave 63 exclusive deepen — greppable):
 *   scsi_mid-server: soft inventory …
 *   scsi_mid-server: soft deepen wave=63 …
...
/* Wave 63 soft inventory surface from cdb.c (no public header change). */
...
main(void)
{
    ...
    /*
     * Wave 63 deepen probes (always soft for inventory; hard fail host smoke
    ...
    /* Wave 63 soft inventory — library + host skeleton surfaces. */
    ...
    /* Grep: scsi_mid-server: soft honesty (Wave 63 exclusive deepen) */
