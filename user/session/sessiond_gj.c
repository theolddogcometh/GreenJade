 ...
 *   soft free → soft inventory (Wave 15) → ownership path PASS
 ...
 *
 * Soft inventory (Wave 15 exclusive deepen — greppable "sessiond-gj: soft …"):
 *   sessiond-gj: soft inventory health_ok=… health_skip=… free_ok=… free_skip=…
 *                flags_ok=… stats_snap=… areas=… wave=15
 ...
 *   sessiond-gj: soft deepen wave=15 areas=… ok=… skip=… token=0x…
 ...
 *   sessiond-gj: soft honesty multi_server=0 confine=0 bar3=0 exclusive=1 soft=1 wave=15
 ...
 * Wave 15 free path also soft-probes STATS / MAP_SCANOUT / DISPLAY_INFO
...
/* Soft health sub-step bits (Wave 10 inventory; Wave 15 still tallies). */
...
/* Soft free-path bits (Wave 15 deepen: stats/map/display while unowned). */
...
/* Wave 15 soft inventory area count (fixed greppable categories). */
#define SOFT_INV_AREAS  12u
/* Soft inventory wave stamp (Wave 15 exclusive deepen). */
#define SOFT_INV_WAVE   15u
...
/*
 * Soft inventory tallies (Wave 15 exclusive deepen).
...
/* Wave 15 deepen snapshots / tallies. */
...
/* Note one soft health sub-step into Wave 15 inventory counters. */
static void
...
/* Note one soft free sub-step into Wave 15 inventory counters. */
static void
...
/* Soft: remember a successful STATS snapshot for Wave 15 inventory. */
static void
...
/* Soft: refresh display-vs-map geometry match lamp (Wave 15). */
static void
...
/*
 * Soft inventory dump (Wave 15 exclusive deepen).
...
soft_inventory_log(void)
{
    ...
    /* Grep: sessiond-gj: soft geom (Wave 15) */
    o = 0u;
    ...
    /* Grep: sessiond-gj: soft present (Wave 15) */
    o = 0u;
    ...
    /* Grep: sessiond-gj: soft input (Wave 15) */
    o = 0u;
    ...
    /* Grep: sessiond-gj: soft reclaim (Wave 15) */
    o = 0u;
    ...
    /* Grep: sessiond-gj: soft deepen wave (Wave 15 stamp) */
    o = 0u;
    ...
    /*
     * Grep: sessiond-gj: soft path (Wave 15 honesty; not bar3).
    ...
    /*
     * Grep: sessiond-gj: soft honesty (Wave 15 exclusive deepen).
     * Soft inventory ≠ product multi-server confine.
     */
 ...
 * Tallies Wave 15 soft flag inventory (sessiond-gj: soft flags …).
 ...
 * Tallies Wave 15 soft inventory (sessiond-gj: soft …).
 */
static void
...
_start(void)
{
     ...
     * Wave 15 deepen: also soft-probe MAP_SCANOUT + DISPLAY_INFO unowned.
     * Tallies Wave 15 soft free inventory (sessiond-gj: soft free …).
     */
    {
        unsigned long u64Va = 0;
        unsigned aInfo[3];
        ...
        /* Wave 15: MAP_SCANOUT while free (soft — geometry optional) */
        u64Va = 0;
        aInfo[0] = aInfo[1] = aInfo[2] = 0;
        n = gj_session(GJ_SESS_OP_MAP_SCANOUT, (long)(uintptr_t)&u64Va,
                       (long)(uintptr_t)aInfo, 0);
        ...
        /* Wave 15: DISPLAY_INFO while free (soft — dimensions optional) */
        aWh[0] = aWh[1] = 0;
        n = gj_session(GJ_SESS_OP_DISPLAY_INFO, (long)(uintptr_t)aWh, 0, 0);
        if (n == 0 && aWh[0] != 0u && aWh[1] != 0u) {
            g_uSoftDispW = aWh[0];
    ...
    /*
     * Wave 15 exclusive soft inventory rollup (greppable "sessiond-gj: soft …").
