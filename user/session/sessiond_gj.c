 ...
 *   soft free → soft inventory (Wave 105) → ownership path PASS
 ...
 *
 * Soft inventory (Wave 105 exclusive deepen — greppable "sessiond-gj: soft …"):
 ...
 * Wave 105 free path also soft-probes STATS / MAP_SCANOUT / DISPLAY_INFO
...
/* Soft health sub-step bits (Wave 10 inventory; Wave 105 still tallies). */
...
/* Soft free-path bits (Wave 105 deepen: stats/map/display while unowned). */
...
/* Wave 105 soft inventory area count (fixed greppable categories). */
#define SOFT_INV_AREAS  12u
/* Soft inventory wave stamp (Wave 105 exclusive deepen). */
...
/*
 * Soft inventory tallies (Wave 105 exclusive deepen).
...
/* Wave 105 deepen snapshots / tallies. */
...
/* Note one soft health sub-step into Wave 105 inventory counters. */
static void
...
/* Note one soft free sub-step into Wave 105 inventory counters. */
static void
...
/* Soft: remember a successful STATS snapshot for Wave 105 inventory. */
static void
...
/* Soft: refresh display-vs-map geometry match lamp (Wave 105). */
static void
...
/*
 * Soft inventory dump (Wave 105 exclusive deepen).
...
soft_inventory_log(void)
{
    ...
    /* Grep: sessiond-gj: soft geom (Wave 105) */
    o = 0u;
    ...
    /* Grep: sessiond-gj: soft present (Wave 105) */
    o = 0u;
    ...
    /* Grep: sessiond-gj: soft input (Wave 105) */
    o = 0u;
    ...
    /* Grep: sessiond-gj: soft reclaim (Wave 105) */
    o = 0u;
    ...
    /* Grep: sessiond-gj: soft deepen wave (Wave 105 stamp) */
    o = 0u;
    ...
    /*
     * Grep: sessiond-gj: soft path (Wave 105 honesty; not bar3).
    ...
    /*
     * Grep: sessiond-gj: soft honesty (Wave 105 exclusive deepen).
     * Soft inventory ≠ product multi-server confine.
     */
 ...
 * Tallies Wave 105 soft flag inventory (sessiond-gj: soft flags …).
 ...
 * Tallies Wave 105 soft inventory (sessiond-gj: soft …).
 */
static void
...
_start(void)
{
     ...
     * Wave 105 deepen: also soft-probe MAP_SCANOUT + DISPLAY_INFO unowned.
     * Tallies Wave 105 soft free inventory (sessiond-gj: soft free …).
     */
    {
        unsigned long u64Va = 0;
        unsigned aInfo[3];
        ...
        /* Wave 105: MAP_SCANOUT while free (soft — geometry optional) */
        u64Va = 0;
        aInfo[0] = aInfo[1] = aInfo[2] = 0;
        n = gj_session(GJ_SESS_OP_MAP_SCANOUT, (long)(uintptr_t)&u64Va,
                       (long)(uintptr_t)aInfo, 0);
        ...
        /* Wave 105: DISPLAY_INFO while free (soft — dimensions optional) */
        aWh[0] = aWh[1] = 0;
        n = gj_session(GJ_SESS_OP_DISPLAY_INFO, (long)(uintptr_t)aWh, 0, 0);
        if (n == 0 && aWh[0] != 0u && aWh[1] != 0u) {
            g_uSoftDispW = aWh[0];
    ...
    /*
     * Wave 105 exclusive soft inventory rollup (greppable "sessiond-gj: soft …").
