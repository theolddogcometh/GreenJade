 ...
 *   soft free → soft inventory (Wave 92) → ownership path PASS
 ...
 *
 * Soft inventory (Wave 92 exclusive deepen — greppable "sessiond-gj: soft …"):
 ...
 * Wave 92 free path also soft-probes STATS / MAP_SCANOUT / DISPLAY_INFO
...
/* Soft health sub-step bits (Wave 10 inventory; Wave 92 still tallies). */
/* Wave 92 soft deepen surfaces (CREATE-ONLY soft ≠ product):
 *   greppable: soft retmarkangle continuum_toward=23400 soft_ne_product=1
 *   greppable: soft retpulseangle exclusive=1 continuum_toward=23400
...
/* Soft free-path bits (Wave 92 deepen: stats/map/display while unowned). */
...
/* Wave 92 soft inventory area count (fixed greppable categories). */
#define SOFT_INV_AREAS  12u
/* Soft inventory wave stamp (Wave 92 exclusive deepen). */
...
/*
 * Soft inventory tallies (Wave 92 exclusive deepen).
...
/* Wave 92 deepen snapshots / tallies. */
...
/* Note one soft health sub-step into Wave 92 inventory counters. */
static void
...
/* Note one soft free sub-step into Wave 92 inventory counters. */
static void
...
/* Soft: remember a successful STATS snapshot for Wave 92 inventory. */
static void
...
/* Soft: refresh display-vs-map geometry match lamp (Wave 92). */
static void
...
/*
 * Soft inventory dump (Wave 92 exclusive deepen).
...
soft_inventory_log(void)
{
    ...
    /* Grep: sessiond-gj: soft geom (Wave 92) */
    o = 0u;
    ...
    /* Grep: sessiond-gj: soft present (Wave 92) */
    o = 0u;
    ...
    /* Grep: sessiond-gj: soft input (Wave 92) */
    o = 0u;
    ...
    /* Grep: sessiond-gj: soft reclaim (Wave 92) */
    o = 0u;
    ...
    /* Grep: sessiond-gj: soft deepen wave (Wave 92 stamp) */
    o = 0u;
    ...
    /*
     * Grep: sessiond-gj: soft path (Wave 92 honesty; not bar3).
    ...
    /*
     * Grep: sessiond-gj: soft honesty (Wave 92 exclusive deepen).
     * Soft inventory ≠ product multi-server confine.
     */
 ...
 * Tallies Wave 92 soft flag inventory (sessiond-gj: soft flags …).
 ...
 * Tallies Wave 92 soft inventory (sessiond-gj: soft …).
 */
static void
...
_start(void)
{
     ...
     * Wave 92 deepen: also soft-probe MAP_SCANOUT + DISPLAY_INFO unowned.
     * Tallies Wave 92 soft free inventory (sessiond-gj: soft free …).
     */
    {
        unsigned long u64Va = 0;
        unsigned aInfo[3];
        ...
        /* Wave 92: MAP_SCANOUT while free (soft — geometry optional) */
        u64Va = 0;
        aInfo[0] = aInfo[1] = aInfo[2] = 0;
        n = gj_session(GJ_SESS_OP_MAP_SCANOUT, (long)(uintptr_t)&u64Va,
                       (long)(uintptr_t)aInfo, 0);
        ...
        /* Wave 92: DISPLAY_INFO while free (soft — dimensions optional) */
        aWh[0] = aWh[1] = 0;
        n = gj_session(GJ_SESS_OP_DISPLAY_INFO, (long)(uintptr_t)aWh, 0, 0);
        if (n == 0 && aWh[0] != 0u && aWh[1] != 0u) {
            g_uSoftDispW = aWh[0];
    ...
    /*
     * Wave 92 exclusive soft inventory rollup (greppable "sessiond-gj: soft …").
