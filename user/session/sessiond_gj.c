 ...
 *   soft free → soft inventory (Wave 57) → ownership path PASS
 ...
 *
 * Soft inventory (Wave 57 exclusive deepen — greppable "sessiond-gj: soft …"):
 *   sessiond-gj: soft inventory health_ok=… health_skip=… free_ok=… free_skip=…
 *                flags_ok=… stats_snap=… areas=… wave=57
 ...
 *   sessiond-gj: soft deepen wave=57 areas=… ok=… skip=… token=0x…
 ...
 *   sessiond-gj: soft honesty multi_server=0 confine=0 bar3=0 exclusive=1 soft=1 product_kernel=OPEN wave=57
 ...
 * Wave 57 free path also soft-probes STATS / MAP_SCANOUT / DISPLAY_INFO
...
/* Soft health sub-step bits (Wave 10 inventory; Wave 57 still tallies). */
/* Wave 57 soft deepen surfaces (CREATE-ONLY soft ≠ product):
 *   greppable: soft retraverse continuum_toward=19900 soft_ne_product=1
 *   greppable: soft retcasemate exclusive=1 continuum_toward=19900
...
/* Soft free-path bits (Wave 57 deepen: stats/map/display while unowned). */
...
/* Wave 57 soft inventory area count (fixed greppable categories). */
#define SOFT_INV_AREAS  12u
/* Soft inventory wave stamp (Wave 57 exclusive deepen). */
#define SOFT_INV_WAVE   50u
...
/*
 * Soft inventory tallies (Wave 57 exclusive deepen).
...
/* Wave 57 deepen snapshots / tallies. */
...
/* Note one soft health sub-step into Wave 57 inventory counters. */
static void
...
/* Note one soft free sub-step into Wave 57 inventory counters. */
static void
...
/* Soft: remember a successful STATS snapshot for Wave 57 inventory. */
static void
...
/* Soft: refresh display-vs-map geometry match lamp (Wave 57). */
static void
...
/*
 * Soft inventory dump (Wave 57 exclusive deepen).
...
soft_inventory_log(void)
{
    ...
    /* Grep: sessiond-gj: soft geom (Wave 57) */
    o = 0u;
    ...
    /* Grep: sessiond-gj: soft present (Wave 57) */
    o = 0u;
    ...
    /* Grep: sessiond-gj: soft input (Wave 57) */
    o = 0u;
    ...
    /* Grep: sessiond-gj: soft reclaim (Wave 57) */
    o = 0u;
    ...
    /* Grep: sessiond-gj: soft deepen wave (Wave 57 stamp) */
    o = 0u;
    ...
    /*
     * Grep: sessiond-gj: soft path (Wave 57 honesty; not bar3).
    ...
    /*
     * Grep: sessiond-gj: soft honesty (Wave 57 exclusive deepen).
     * Soft inventory ≠ product multi-server confine.
     */
 ...
 * Tallies Wave 57 soft flag inventory (sessiond-gj: soft flags …).
 ...
 * Tallies Wave 57 soft inventory (sessiond-gj: soft …).
 */
static void
...
_start(void)
{
     ...
     * Wave 57 deepen: also soft-probe MAP_SCANOUT + DISPLAY_INFO unowned.
     * Tallies Wave 57 soft free inventory (sessiond-gj: soft free …).
     */
    {
        unsigned long u64Va = 0;
        unsigned aInfo[3];
        ...
        /* Wave 57: MAP_SCANOUT while free (soft — geometry optional) */
        u64Va = 0;
        aInfo[0] = aInfo[1] = aInfo[2] = 0;
        n = gj_session(GJ_SESS_OP_MAP_SCANOUT, (long)(uintptr_t)&u64Va,
                       (long)(uintptr_t)aInfo, 0);
        ...
        /* Wave 57: DISPLAY_INFO while free (soft — dimensions optional) */
        aWh[0] = aWh[1] = 0;
        n = gj_session(GJ_SESS_OP_DISPLAY_INFO, (long)(uintptr_t)aWh, 0, 0);
        if (n == 0 && aWh[0] != 0u && aWh[1] != 0u) {
            g_uSoftDispW = aWh[0];
    ...
    /*
     * Wave 57 exclusive soft inventory rollup (greppable "sessiond-gj: soft …").
