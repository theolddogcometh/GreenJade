 ...
 *   soft free → soft inventory (Wave 14) → ownership path PASS
 ...
 *
 * Soft inventory (Wave 14 exclusive deepen — greppable "sessiond-gj: soft …"):
 *   sessiond-gj: soft inventory health_ok=… health_skip=… free_ok=… free_skip=…
 *                flags_ok=… stats_snap=… areas=… wave=14
 ...
 *   sessiond-gj: soft deepen wave=14 areas=… ok=… skip=… token=0x…
 ...
 * Wave 14 free path also soft-probes STATS / MAP_SCANOUT / DISPLAY_INFO
...
/* Soft health sub-step bits (Wave 10 inventory; Wave 14 still tallies). */
...
/* Soft free-path bits (Wave 14 deepen: stats/map/display while unowned). */
...
/* Wave 14 soft inventory area count (fixed greppable categories). */
#define SOFT_INV_AREAS  11u
/* Soft inventory wave stamp (Wave 14 exclusive deepen). */
#define SOFT_INV_WAVE   14u
...
/*
 * Soft inventory tallies (Wave 14 exclusive deepen).
...
/* Wave 14 deepen snapshots / tallies. */
...
/* Note one soft health sub-step into Wave 14 inventory counters. */
static void
...
/* Note one soft free sub-step into Wave 14 inventory counters. */
static void
...
/* Soft: remember a successful STATS snapshot for Wave 14 inventory. */
static void
...
/* Soft: refresh display-vs-map geometry match lamp (Wave 14). */
static void
...
/*
 * Soft inventory dump (Wave 14 exclusive deepen).
...
soft_inventory_log(void)
{
    ...
    /* Grep: sessiond-gj: soft geom (Wave 14) */
    o = 0u;
    ...
    /* Grep: sessiond-gj: soft present (Wave 14) */
    o = 0u;
    ...
    /* Grep: sessiond-gj: soft input (Wave 14) */
    o = 0u;
    ...
    /* Grep: sessiond-gj: soft reclaim (Wave 14) */
    o = 0u;
    ...
    /* Grep: sessiond-gj: soft deepen wave (Wave 14 stamp) */
    o = 0u;
    ...
    /*
     * Grep: sessiond-gj: soft path (Wave 14 honesty; not bar3).
 ...
 * Tallies Wave 14 soft flag inventory (sessiond-gj: soft flags …).
 ...
 * Tallies Wave 14 soft inventory (sessiond-gj: soft …).
 */
static void
...
_start(void)
{
     ...
     * Wave 14 deepen: also soft-probe MAP_SCANOUT + DISPLAY_INFO unowned.
     * Tallies Wave 14 soft free inventory (sessiond-gj: soft free …).
     */
    {
        unsigned long u64Va = 0;
        unsigned aInfo[3];
        ...
        /* Wave 14: MAP_SCANOUT while free (soft — geometry optional) */
        u64Va = 0;
        aInfo[0] = aInfo[1] = aInfo[2] = 0;
        n = gj_session(GJ_SESS_OP_MAP_SCANOUT, (long)(uintptr_t)&u64Va,
                       (long)(uintptr_t)aInfo, 0);
        ...
        /* Wave 14: DISPLAY_INFO while free (soft — dimensions optional) */
        aWh[0] = aWh[1] = 0;
        n = gj_session(GJ_SESS_OP_DISPLAY_INFO, (long)(uintptr_t)aWh, 0, 0);
        if (n == 0 && aWh[0] != 0u && aWh[1] != 0u) {
            g_uSoftDispW = aWh[0];
    ...
    /*
     * Wave 14 exclusive soft inventory rollup (greppable "sessiond-gj: soft …").
