 ...
 *   soft free → soft inventory (Wave 53) → ownership path PASS
 ...
 *
 * Soft inventory (Wave 53 exclusive deepen — greppable "sessiond-gj: soft …"):
 *   sessiond-gj: soft inventory health_ok=… health_skip=… free_ok=… free_skip=…
 *                flags_ok=… stats_snap=… areas=… wave=53
 ...
 *   sessiond-gj: soft deepen wave=53 areas=… ok=… skip=… token=0x…
 ...
 *   sessiond-gj: soft honesty multi_server=0 confine=0 bar3=0 exclusive=1 soft=1 product_kernel=OPEN wave=53
 ...
 * Wave 53 free path also soft-probes STATS / MAP_SCANOUT / DISPLAY_INFO
...
/* Soft health sub-step bits (Wave 10 inventory; Wave 53 still tallies). */
...
/* Soft free-path bits (Wave 53 deepen: stats/map/display while unowned). */
...
/* Wave 53 soft inventory area count (fixed greppable categories). */
#define SOFT_INV_AREAS  12u
/* Soft inventory wave stamp (Wave 53 exclusive deepen). */
...
/*
 * Soft inventory tallies (Wave 53 exclusive deepen).
...
/* Wave 53 deepen snapshots / tallies. */
...
/* Note one soft health sub-step into Wave 53 inventory counters. */
static void
...
/* Note one soft free sub-step into Wave 53 inventory counters. */
static void
...
/* Soft: remember a successful STATS snapshot for Wave 53 inventory. */
static void
...
/* Soft: refresh display-vs-map geometry match lamp (Wave 53). */
static void
...
/*
 * Soft inventory dump (Wave 53 exclusive deepen).
...
soft_inventory_log(void)
{
    ...
    /* Grep: sessiond-gj: soft geom (Wave 53) */
    o = 0u;
    ...
    /* Grep: sessiond-gj: soft present (Wave 53) */
    o = 0u;
    ...
    /* Grep: sessiond-gj: soft input (Wave 53) */
    o = 0u;
    ...
    /* Grep: sessiond-gj: soft reclaim (Wave 53) */
    o = 0u;
    ...
    /* Grep: sessiond-gj: soft deepen wave (Wave 53 stamp) */
    o = 0u;
    ...
    /*
     * Grep: sessiond-gj: soft path (Wave 53 honesty; not bar3).
    ...
    /*
     * Grep: sessiond-gj: soft honesty (Wave 53 exclusive deepen).
     * Soft inventory ≠ product multi-server confine.
     */
 ...
 * Tallies Wave 53 soft flag inventory (sessiond-gj: soft flags …).
 ...
 * Tallies Wave 53 soft inventory (sessiond-gj: soft …).
 */
static void
...
_start(void)
{
     ...
     * Wave 53 deepen: also soft-probe MAP_SCANOUT + DISPLAY_INFO unowned.
     * Tallies Wave 53 soft free inventory (sessiond-gj: soft free …).
     */
    {
        unsigned long u64Va = 0;
        unsigned aInfo[3];
        ...
        /* Wave 53: MAP_SCANOUT while free (soft — geometry optional) */
        u64Va = 0;
        aInfo[0] = aInfo[1] = aInfo[2] = 0;
        n = gj_session(GJ_SESS_OP_MAP_SCANOUT, (long)(uintptr_t)&u64Va,
                       (long)(uintptr_t)aInfo, 0);
        ...
        /* Wave 53: DISPLAY_INFO while free (soft — dimensions optional) */
        aWh[0] = aWh[1] = 0;
        n = gj_session(GJ_SESS_OP_DISPLAY_INFO, (long)(uintptr_t)aWh, 0, 0);
        if (n == 0 && aWh[0] != 0u && aWh[1] != 0u) {
            g_uSoftDispW = aWh[0];
    ...
    /*
     * Wave 53 exclusive soft inventory rollup (greppable "sessiond-gj: soft …").
