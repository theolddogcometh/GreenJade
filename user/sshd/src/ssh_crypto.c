 ...
 *
 * Soft inventory (Wave 40 exclusive deepen — greppable when hostkey init runs
...
/* Wave 40 exclusive soft inventory stamp (observability; never hard-gates). */
...
/* Grep surface: ssh_crypto: soft deepen product_kernel=OPEN wave=40 multi_server=0 confine=0 */
static const char g_szSshCryptoSoftDeepen[] =
	"ssh_crypto: soft deepen product_kernel=OPEN wave=40 multi_server=0 confine=0 "
...
gj_ssh_poly1305_selfcheck(void)
{
	...
	/* Soft inventory touch (Wave 40): keep greppable strings live. */
