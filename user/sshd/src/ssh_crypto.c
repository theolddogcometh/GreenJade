 ...
 *
 * Soft inventory (Wave 58 exclusive deepen — greppable when hostkey init runs
...
/* Wave 58 exclusive soft inventory stamp (observability; never hard-gates). */
...
#define SSH_CRYPTO_SOFT_WAVE 58u

/* Grep surface: ssh_crypto: soft deepen product_kernel=OPEN wave=58 multi_server=0 confine=0 */
static const char g_szSshCryptoSoftDeepen[] =
	"ssh_crypto: soft deepen product_kernel=OPEN wave=58 multi_server=0 confine=0 "
...
gj_ssh_poly1305_selfcheck(void)
{
	...
	/* Soft inventory touch (Wave 58): keep greppable strings live. */
