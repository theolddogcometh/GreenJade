 ...
 *
 * Soft inventory (Wave 100 exclusive deepen) — honesty, not product SSH.
 ...
 * Soft inventory / path / stats (Wave 100; greppable "sshd-gj: soft …"):
...
/*
 * Soft product inventory (Wave 100). Cumulative milestone lamps + suite tallies.
...
/* Offline soft-suite bit lamps (Wave 100; never hard-gate). */
...
/*
 * Greppable soft inventory + honesty (Wave 100 exclusive deepen).
 *   sshd-gj: soft honesty not-product-ssh …
 *   sshd-gj: soft inventory …
 *   sshd-gj: soft seq …
 *   sshd-gj: soft crypto …
...
soft_inventory_log(void)
{
	...
	/* Grep: sshd-gj: soft crypto (Wave 100 offline suite lamps) */
	o = 0;
	...
	/* Grep: sshd-gj: soft crypto (Wave 100 deepen legs continued) */
	o = 0;
	...
	/* Grep: sshd-gj: soft kex (live-path KEX lamps, Wave 100) */
	o = 0;
	...
	/* Grep: sshd-gj: soft channel (post-NEWKEYS session lamps, Wave 100) */
	o = 0;
	...
	/* Grep: sshd-gj: soft deepen wave (Wave 100 stamp) */
	o = 0;
	...
	/*
	 * Grep: sshd-gj: soft exclusive (Wave 100 exclusive deepen).
	 * Soft inventory ≠ product multi-server confine.
	 */
 ...
 * Wave 100 deepen: freestanding crypto/shape surface beyond the TCP smoke alone.
 * greppable: sshd-gj: soft …
 */
static void
soft_suite(void)
...
_start(void)
{
	...
	/*
	 * Wave 100 soft inventory: offline suite + honesty / path / stats.
