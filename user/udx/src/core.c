 ...
 *
 * Soft inventory (Wave 59 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 59): UDX kernel notify multi-process SPSC product remains
 ...
 *   udx: soft deepen     — wave=59 stamp + area count
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 59 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 59 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 59 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 59 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 59 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retcrownwork — Wave 59 return-crownwork honesty
     * Grep: udx: soft rethornwork — Wave 59 exclusive hornwork stamp
     * CREATE-ONLY soft names; continuum toward 20100; Soft≠product.
     */
    soft_emit("udx: soft retcrownwork soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=20100 wave=%u "
              "(retcrownwork honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft rethornwork exclusive=1 soft_ne_product=1 "
              "continuum_toward=20100 wave=%u "
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 59 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 59 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
