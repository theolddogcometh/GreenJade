 ...
 *
 * Soft inventory (Wave 88 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 88): UDX kernel notify multi-process SPSC product remains
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 88 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 88 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 88 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 88 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 88 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retbeaconangle — Wave 88 return-beaconangle honesty
     * Grep: udx: soft retledgerangle — Wave 88 exclusive ledgerangle stamp
     * CREATE-ONLY soft names; continuum toward 23000; Soft≠product.
     */
    soft_emit("udx: soft retbeaconangle soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=23000 wave=%u "
              "(retbeaconangle honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retledgerangle exclusive=1 soft_ne_product=1 "
              "continuum_toward=23000 wave=%u "
              "(retledgerangle stamp; Soft!=product)\n",
              UDX_SOFT_WAVE);
}
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 88 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 88 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
