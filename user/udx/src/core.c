 ...
 *
 * Soft inventory (Wave 55 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 55): UDX kernel notify multi-process SPSC product remains
 ...
 *   udx: soft deepen     — wave=55 stamp + area count
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 55 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 55 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 55 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 55 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 55 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retflank — Wave 55 return-flank honesty
     * Grep: udx: soft retface — Wave 55 exclusive face stamp
     * CREATE-ONLY soft names; continuum toward 19700; Soft≠product.
     */
    soft_emit("udx: soft retflank soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=19700 wave=%u "
              "(retflank honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retface exclusive=1 soft_ne_product=1 "
              "continuum_toward=19700 wave=%u "
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 55 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 55 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
