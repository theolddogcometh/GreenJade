 ...
 *
 * Soft inventory (Wave 87 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 87): UDX kernel notify multi-process SPSC product remains
...
/*
 * Soft product inventory (Wave 87 exclusive deepen). Cumulative for this
...
/**
 * Greppable soft inventory (Wave 87 exclusive deepen).
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 87 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 87 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 87 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 87 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 87 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retflameangle — Wave 87 return-flameangle honesty
     * Grep: udx: soft retcipherangle — Wave 87 exclusive cipherangle stamp
     * CREATE-ONLY soft names; continuum toward 22900; Soft≠product.
     */
    soft_emit("udx: soft retflameangle soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=22900 wave=%u "
              "(retflameangle honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retcipherangle exclusive=1 soft_ne_product=1 "
              "continuum_toward=22900 wave=%u "
              "(retcipherangle stamp; Soft!=product)\n",
              UDX_SOFT_WAVE);
}
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 87 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 87 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
