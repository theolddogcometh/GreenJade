 ...
 *
 * Honesty (Wave 114): UDX kernel notify multi-process SPSC product remains
...
/*
 * Soft product inventory (Wave 114 exclusive deepen). Cumulative for this
...
/**
 * Greppable soft inventory (Wave 114 exclusive deepen).
...
soft_inventory_log(void)
{
    ...
    /*
     * Grep: udx: soft honesty (Wave 114 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retframebufferangle — Wave 114 return-framebufferangle honesty
     * Grep: udx: soft retswapchainangle — Wave 114 exclusive swapchainangle stamp
     * CREATE-ONLY soft names; continuum toward 25600; Soft≠product.
     */
    soft_emit("udx: soft retframebufferangle soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=25600 wave=%u "
              "(retframebufferangle honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retswapchainangle exclusive=1 soft_ne_product=1 "
              "continuum_toward=25600 wave=%u "
              "(retswapchainangle stamp; Soft!=product)\n",
              UDX_SOFT_WAVE);
}
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 114 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 114 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
