 ...
 *
 * Soft inventory (Wave 89 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 89): UDX kernel notify multi-process SPSC product remains
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 89 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 89 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 89 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 89 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 89 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retbannerangle — Wave 89 return-bannerangle honesty
     * Grep: udx: soft retvaultangle — Wave 89 exclusive vaultangle stamp
     * CREATE-ONLY soft names; continuum toward 23100; Soft≠product.
     */
    soft_emit("udx: soft retbannerangle soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=23100 wave=%u "
              "(retbannerangle honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retvaultangle exclusive=1 soft_ne_product=1 "
              "continuum_toward=23100 wave=%u "
              "(retvaultangle stamp; Soft!=product)\n",
              UDX_SOFT_WAVE);
}
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 89 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 89 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
