 ...
 *
 * Soft inventory (Wave 80 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 80): UDX kernel notify multi-process SPSC product remains
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 80 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 80 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 80 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 80 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 80 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retcitadelangle — Wave 80 return-keepangle honesty
     * Grep: udx: soft retbastionkeep — Wave 80 exclusive redoubtangle stamp
     * CREATE-ONLY soft names; continuum toward 22200; Soft≠product.
     */
    soft_emit("udx: soft retcitadelangle soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=22200 wave=%u "
              "(retcitadelangle honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retbastionkeep exclusive=1 soft_ne_product=1 "
              "continuum_toward=22200 wave=%u "
              "(retbastionkeep stamp; Soft!=product)\n",
              UDX_SOFT_WAVE);
}
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 80 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 80 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
