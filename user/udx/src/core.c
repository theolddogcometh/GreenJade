 ...
 *
 * Soft inventory (Wave 95 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 95): UDX kernel notify multi-process SPSC product remains
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 95 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 95 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 95 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 95 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 95 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retchordangle — Wave 95 return-chordangle honesty
     * Grep: udx: soft retarcangle — Wave 95 exclusive arcangle stamp
     * CREATE-ONLY soft names; continuum toward 23700; Soft≠product.
     */
    soft_emit("udx: soft retchordangle soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=23700 wave=%u "
              "(retchordangle honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retarcangle exclusive=1 soft_ne_product=1 "
              "continuum_toward=23700 wave=%u "
              "(retarcangle stamp; Soft!=product)\n",
              UDX_SOFT_WAVE);
}
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 95 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 95 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
