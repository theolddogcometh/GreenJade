 ...
 *
 * Soft inventory (Wave 92 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 92): UDX kernel notify multi-process SPSC product remains
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 92 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 92 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 92 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 92 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 92 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retmarkangle — Wave 92 return-markangle honesty
     * Grep: udx: soft retpulseangle — Wave 92 exclusive pulseangle stamp
     * CREATE-ONLY soft names; continuum toward 23400; Soft≠product.
     */
    soft_emit("udx: soft retmarkangle soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=23400 wave=%u "
              "(retmarkangle honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retpulseangle exclusive=1 soft_ne_product=1 "
              "continuum_toward=23400 wave=%u "
              "(retpulseangle stamp; Soft!=product)\n",
              UDX_SOFT_WAVE);
}
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 92 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 92 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
