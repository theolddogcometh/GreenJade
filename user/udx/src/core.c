 ...
 *
 * Honesty (Wave 82): UDX kernel notify multi-process SPSC product remains
...
/*
 * Soft product inventory (Wave 82 exclusive deepen). Cumulative for this
...
/**
 * Greppable soft inventory (Wave 82 exclusive deepen).
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 82 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 82 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 82 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 82 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 82 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retmantleangle — Wave 82 return-keepangle honesty
     * Grep: udx: soft retaegisangle — Wave 82 exclusive redoubtangle stamp
     * CREATE-ONLY soft names; continuum toward 22400; Soft≠product.
     */
    soft_emit("udx: soft retmantleangle soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=22400 wave=%u "
              "(retmantleangle honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retaegisangle exclusive=1 soft_ne_product=1 "
              "continuum_toward=22400 wave=%u "
              "(retaegisangle stamp; Soft!=product)\n",
              UDX_SOFT_WAVE);
}
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 82 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 82 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
