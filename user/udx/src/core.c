 ...
 *
 * Soft inventory (Wave 110 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 110): UDX kernel notify multi-process SPSC product remains
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 110 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 110 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 110 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 110 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 110 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retmeshangle — Wave 110 return-nurbsangle honesty
     * Grep: udx: soft retgridangle — Wave 110 exclusive bsplineangle stamp
     * CREATE-ONLY soft names; continuum toward 25200; Soft≠product.
     */
    soft_emit("udx: soft retmeshangle soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=25200 wave=%u "
              "(retmeshangle honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retgridangle exclusive=1 soft_ne_product=1 "
              "continuum_toward=25200 wave=%u "
              "(retgridangle stamp; Soft!=product)\n",
              UDX_SOFT_WAVE);
}
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 110 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 110 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
