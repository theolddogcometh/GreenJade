 ...
 *
 * Soft inventory (Wave 50 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 50): UDX kernel notify multi-process SPSC product remains
 ...
 *   udx: soft deepen     — wave=50 stamp + area count
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 50 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 50 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 50 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 50 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 50 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retsally — Wave 50 return-glacisgate honesty
     * Grep: udx: soft retcounterscarp — Wave 50 exclusive outwork stamp
     * CREATE-ONLY soft names; continuum toward 19200; Soft≠product.
     */
    soft_emit("udx: soft retsally soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=19200 wave=%u "
              "(retsally honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retcounterscarp exclusive=1 soft_ne_product=1 "
              "continuum_toward=19200 wave=%u "
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 50 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 50 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
