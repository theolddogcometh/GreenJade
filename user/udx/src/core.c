 ...
 *
 * Honesty (Wave 103): UDX kernel notify multi-process SPSC product remains
...
/*
 * Soft product inventory (Wave 103 exclusive deepen). Cumulative for this
...
/**
 * Greppable soft inventory (Wave 103 exclusive deepen).
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 103 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 103 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 103 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 103 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 103 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retprojectangle — Wave 103 return-projectangle honesty
     * Grep: udx: soft retaffineangle — Wave 103 exclusive affineangle stamp
     * CREATE-ONLY soft names; continuum toward 24500; Soft≠product.
     */
    soft_emit("udx: soft retprojectangle soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=24500 wave=%u "
              "(retprojectangle honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retaffineangle exclusive=1 soft_ne_product=1 "
              "continuum_toward=24500 wave=%u "
              "(retaffineangle stamp; Soft!=product)\n",
              UDX_SOFT_WAVE);
}
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 103 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 103 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
