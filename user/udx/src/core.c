 ...
 *
 * Soft inventory (Wave 73 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 73): UDX kernel notify multi-process SPSC product remains
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 73 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 73 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 73 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 73 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 73 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retcurtainface — Wave 73 return-placeofarmsangle honesty
     * Grep: udx: soft retbastionangle — Wave 73 exclusive doubletenailleangle stamp
     * CREATE-ONLY soft names; continuum toward 21500; Soft≠product.
     */
    soft_emit("udx: soft retcurtainface soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=21500 wave=%u "
              "(retcurtainface honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retbastionangle exclusive=1 soft_ne_product=1 "
              "continuum_toward=21500 wave=%u "
              "(retbastionangle stamp; Soft!=product)\n",
              UDX_SOFT_WAVE);
}
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 73 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 73 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
