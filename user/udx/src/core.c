 ...
 *
 * Soft inventory (Wave 54 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 54): UDX kernel notify multi-process SPSC product remains
 ...
 *   udx: soft deepen     — wave=54 stamp + area count
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 54 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 54 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 54 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 54 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 54 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retcaponier — Wave 54 return-glacisgate honesty
     * Grep: udx: soft retredan — Wave 54 exclusive outwork stamp
     * CREATE-ONLY soft names; continuum toward 19600; Soft≠product.
     */
    soft_emit("udx: soft retcaponier soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=19600 wave=%u "
              "(retcaponier honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retredan exclusive=1 soft_ne_product=1 "
              "continuum_toward=19600 wave=%u "
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 54 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 54 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
