 ...
 *
 * Soft inventory (Wave 45 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 45): UDX kernel notify multi-process SPSC product remains
 ...
 *   udx: soft deepen     — wave=45 stamp + area count
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 45 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 45 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 45 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 45 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 45 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retkeepgate — Wave 45 return-keepgate honesty
     * Grep: udx: soft retouterward — Wave 45 exclusive outerward stamp
     * CREATE-ONLY soft names; continuum toward 18700; Soft≠product.
     */
    soft_emit("udx: soft retkeepgate soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=18700 wave=%u "
              "(retkeepgate honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retouterward exclusive=1 soft_ne_product=1 "
              "continuum_toward=18700 wave=%u "
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 45 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 45 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
