 ...
 *
 * Soft inventory (Wave 64 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 64): UDX kernel notify multi-process SPSC product remains
 ...
 *   udx: soft deepen     — wave=64 stamp + area count
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 64 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 64 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 64 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 64 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 64 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retreentrant — Wave 64 return-reentrant honesty
     * Grep: udx: soft retsallyport — Wave 64 exclusive sallyport stamp
     * CREATE-ONLY soft names; continuum toward 20600; Soft≠product.
     */
    soft_emit("udx: soft retreentrant soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=20600 wave=%u "
              "(retreentrant honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retsallyport exclusive=1 soft_ne_product=1 "
              "continuum_toward=20600 wave=%u "
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 64 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 64 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
