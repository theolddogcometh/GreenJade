 ...
 *
 * Soft inventory (Wave 104 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 104): UDX kernel notify multi-process SPSC product remains
...
/*
 * Soft product inventory (Wave 104 exclusive deepen). Cumulative for this
...
/**
 * Greppable soft inventory (Wave 104 exclusive deepen).
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 104 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 104 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 104 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 104 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 104 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retlinearangle — Wave 104 return-linearangle honesty
     * Grep: udx: soft retbilinearangle — Wave 104 exclusive bilinearangle stamp
     * CREATE-ONLY soft names; continuum toward 24600; Soft≠product.
     */
    soft_emit("udx: soft retlinearangle soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=24600 wave=%u "
              "(retlinearangle honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retbilinearangle exclusive=1 soft_ne_product=1 "
              "continuum_toward=24600 wave=%u "
              "(retbilinearangle stamp; Soft!=product)\n",
              UDX_SOFT_WAVE);
}
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 104 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 104 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
