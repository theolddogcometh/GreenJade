 ...
 *
 * Honesty (Wave 53): UDX kernel notify multi-process SPSC product remains
 ...
 *   udx: soft deepen     — wave=53 stamp + area count
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 53 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 53 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 53 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 53 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 53 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retravelin — Wave 53 return-glacisgate honesty
     * Grep: udx: soft retlunette — Wave 53 exclusive outwork stamp
     * CREATE-ONLY soft names; continuum toward 19500; Soft≠product.
     */
    soft_emit("udx: soft retravelin soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=19500 wave=%u "
              "(retravelin honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retlunette exclusive=1 soft_ne_product=1 "
              "continuum_toward=19500 wave=%u "
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 53 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 53 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
