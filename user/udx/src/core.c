 ...
 *
 * Soft inventory (Wave 83 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 83): UDX kernel notify multi-process SPSC product remains
...
/*
 * Soft product inventory (Wave 83 exclusive deepen). Cumulative for this
...
/**
 * Greppable soft inventory (Wave 83 exclusive deepen).
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 83 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 83 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 83 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 83 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 83 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retemblemangle — Wave 83 return-emblemangle honesty
     * Grep: udx: soft retsigilangle — Wave 83 exclusive sigilangle stamp
     * CREATE-ONLY soft names; continuum toward 22500; Soft≠product.
     */
    soft_emit("udx: soft retemblemangle soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=22500 wave=%u "
              "(retemblemangle honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retsigilangle exclusive=1 soft_ne_product=1 "
              "continuum_toward=22500 wave=%u "
              "(retsigilangle stamp; Soft!=product)\n",
              UDX_SOFT_WAVE);
}
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 83 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 83 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
