 ...
 *
 * Soft inventory (Wave 115 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 115): UDX kernel notify multi-process SPSC product remains
...
/*
 * Soft product inventory (Wave 115 exclusive deepen). Cumulative for this
...
/**
 * Greppable soft inventory (Wave 115 exclusive deepen).
...
soft_inventory_log(void)
{
    ...
    /*
     * Grep: udx: soft honesty (Wave 115 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retpresentangle — Wave 115 return-presentangle honesty
     * Grep: udx: soft retvsyncangle — Wave 115 exclusive vsyncangle stamp
     * CREATE-ONLY soft names; continuum toward 25700; Soft≠product.
     */
    soft_emit("udx: soft retpresentangle soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=25700 wave=%u "
              "(retpresentangle honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retvsyncangle exclusive=1 soft_ne_product=1 "
              "continuum_toward=25700 wave=%u "
              "(retvsyncangle stamp; Soft!=product)\n",
              UDX_SOFT_WAVE);
}
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 115 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 115 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
