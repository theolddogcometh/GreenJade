 ...
 *
 * Soft inventory (Wave 84 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 84): UDX kernel notify multi-process SPSC product remains
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 84 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 84 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 84 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 84 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 84 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retscepterangle — Wave 84 return-scepterangle honesty
     * Grep: udx: soft retglyphangle — Wave 84 exclusive glyphangle stamp
     * CREATE-ONLY soft names; continuum toward 22600; Soft≠product.
     */
    soft_emit("udx: soft retscepterangle soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=22600 wave=%u "
              "(retscepterangle honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retglyphangle exclusive=1 soft_ne_product=1 "
              "continuum_toward=22600 wave=%u "
              "(retglyphangle stamp; Soft!=product)\n",
              UDX_SOFT_WAVE);
}
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 84 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 84 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
