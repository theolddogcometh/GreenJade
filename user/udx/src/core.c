 ...
 *
 * Soft inventory (Wave 69 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 69): UDX kernel notify multi-process SPSC product remains
...
/*
 * Soft product inventory (Wave 69 exclusive deepen). Cumulative for this
...
/**
 * Greppable soft inventory (Wave 69 exclusive deepen).
 ...
 *   udx: soft deepen     — wave=69 stamp + area count
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 69 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 69 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 69 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 69 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 69 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retdemiluneangle — Wave 69 return-demiluneangle honesty
     * Grep: udx: soft retcoveredwayangle — Wave 69 exclusive coveredwayangle stamp
     * CREATE-ONLY soft names; continuum toward 21100; Soft≠product.
     */
    soft_emit("udx: soft retdemiluneangle soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=21100 wave=%u "
              "(retdemiluneangle honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retcoveredwayangle exclusive=1 soft_ne_product=1 "
              "continuum_toward=21100 wave=%u "
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 69 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 69 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
