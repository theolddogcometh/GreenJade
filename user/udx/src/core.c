 ...
 *
 * Soft inventory (Wave 68 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 68): UDX kernel notify multi-process SPSC product remains
 ...
 *   udx: soft deepen     — wave=68 stamp + area count
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 68 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 68 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 68 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 68 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 68 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retlunetteangle — Wave 68 return-gorgeangle honesty
     * Grep: udx: soft rettenailleangle — Wave 68 exclusive shoulderangle stamp
     * CREATE-ONLY soft names; continuum toward 21000; Soft≠product.
     */
    soft_emit("udx: soft retlunetteangle soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=21000 wave=%u "
              "(retlunetteangle honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft rettenailleangle exclusive=1 soft_ne_product=1 "
              "continuum_toward=21000 wave=%u "
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 68 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 68 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
