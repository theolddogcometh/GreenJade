 ...
 *
 * Soft inventory (Wave 60 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 60): UDX kernel notify multi-process SPSC product remains
 ...
 *   udx: soft deepen     — wave=60 stamp + area count
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 60 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 60 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 60 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 60 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 60 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retplace — Wave 60 return-place honesty
     * Grep: udx: soft retenvelope — Wave 60 exclusive envelope stamp
     * CREATE-ONLY soft names; continuum toward 20200; Soft≠product.
     */
    soft_emit("udx: soft retplace soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=20200 wave=%u "
              "(retplace honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retenvelope exclusive=1 soft_ne_product=1 "
              "continuum_toward=20200 wave=%u "
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 60 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 60 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
