 ...
 *
 * Soft inventory (Wave 44 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 44): UDX kernel notify multi-process SPSC product remains
 ...
 *   udx: soft deepen     — wave=44 stamp + area count
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 44 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 44 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 44 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 44 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 44 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retmerlon — Wave 44 return-merlon honesty
     * Grep: udx: soft retembrasure — Wave 44 exclusive embrasure stamp
     * CREATE-ONLY soft names; continuum toward 18600; Soft≠product.
     */
    soft_emit("udx: soft retmerlon soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=18600 wave=%u "
              "(retmerlon honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retembrasure exclusive=1 soft_ne_product=1 "
              "continuum_toward=18600 wave=%u "
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 44 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 44 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
