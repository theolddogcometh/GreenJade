 ...
 *
 * Honesty (Wave 76): UDX kernel notify multi-process SPSC product remains
...
/*
 * Soft product inventory (Wave 76 exclusive deepen). Cumulative for this
...
/**
 * Greppable soft inventory (Wave 76 exclusive deepen).
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 76 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 76 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 76 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 76 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 76 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retgateangle — Wave 76 return-gateangle honesty
     * Grep: udx: soft retwallangle — Wave 76 exclusive wallangle stamp
     * CREATE-ONLY soft names; continuum toward 21800; Soft≠product.
     */
    soft_emit("udx: soft retgateangle soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=21800 wave=%u "
              "(retgateangle honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retwallangle exclusive=1 soft_ne_product=1 "
              "continuum_toward=21800 wave=%u "
              "(retwallangle stamp; Soft!=product)\n",
              UDX_SOFT_WAVE);
}
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 76 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 76 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
