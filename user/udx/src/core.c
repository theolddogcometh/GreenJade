 ...
 *
 * Soft inventory (Wave 100 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 100): UDX kernel notify multi-process SPSC product remains
...
/*
 * Soft product inventory (Wave 100 exclusive deepen). Cumulative for this
...
/**
 * Greppable soft inventory (Wave 100 exclusive deepen).
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 100 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 100 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 100 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 100 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 100 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retspiralangle — Wave 100 return-spiralangle honesty
     * Grep: udx: soft rethelixangle — Wave 100 exclusive helixangle stamp
     * CREATE-ONLY soft names; continuum toward 24200; Soft≠product.
     */
    soft_emit("udx: soft retspiralangle soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=24200 wave=%u "
              "(retspiralangle honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft rethelixangle exclusive=1 soft_ne_product=1 "
              "continuum_toward=24200 wave=%u "
              "(rethelixangle stamp; Soft!=product)\n",
              UDX_SOFT_WAVE);
}
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 100 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 100 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
