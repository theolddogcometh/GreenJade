 ...
 *
 * Soft inventory (Wave 67 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 67): UDX kernel notify multi-process SPSC product remains
 ...
 *   udx: soft deepen     — wave=67 stamp + area count
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 67 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 67 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 67 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 67 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 67 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retcaponierangle — Wave 67 return-gorgeangle honesty
     * Grep: udx: soft retredanangle — Wave 67 exclusive shoulderangle stamp
     * CREATE-ONLY soft names; continuum toward 20900; Soft≠product.
     */
    soft_emit("udx: soft retcaponierangle soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=20900 wave=%u "
              "(retcaponierangle honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retredanangle exclusive=1 soft_ne_product=1 "
              "continuum_toward=20900 wave=%u "
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 67 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 67 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
