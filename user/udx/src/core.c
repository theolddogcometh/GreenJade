 ...
 *
 * Soft inventory (Wave 85 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 85): UDX kernel notify multi-process SPSC product remains
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 85 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 85 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 85 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 85 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 85 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retcrownangle — Wave 85 return-scepterangle honesty
     * Grep: udx: soft retshardangle — Wave 85 exclusive glyphangle stamp
     * CREATE-ONLY soft names; continuum toward 22700; Soft≠product.
     */
    soft_emit("udx: soft retcrownangle soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=22700 wave=%u "
              "(retcrownangle honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retshardangle exclusive=1 soft_ne_product=1 "
              "continuum_toward=22700 wave=%u "
              "(retshardangle stamp; Soft!=product)\n",
              UDX_SOFT_WAVE);
}
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 85 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 85 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
