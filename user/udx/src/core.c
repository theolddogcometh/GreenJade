 ...
 *
 * Honesty (Wave 81): UDX kernel notify multi-process SPSC product remains
...
/*
 * Soft product inventory (Wave 81 exclusive deepen). Cumulative for this
...
/**
 * Greppable soft inventory (Wave 81 exclusive deepen).
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 81 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 81 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 81 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 81 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 81 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retpanoplyangle — Wave 81 return-keepangle honesty
     * Grep: udx: soft retbulwarkangle — Wave 81 exclusive redoubtangle stamp
     * CREATE-ONLY soft names; continuum toward 22300; Soft≠product.
     */
    soft_emit("udx: soft retpanoplyangle soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=22300 wave=%u "
              "(retpanoplyangle honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retbulwarkangle exclusive=1 soft_ne_product=1 "
              "continuum_toward=22300 wave=%u "
              "(retbulwarkangle stamp; Soft!=product)\n",
              UDX_SOFT_WAVE);
}
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 81 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 81 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
