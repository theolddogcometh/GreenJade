 ...
 *
 * Soft inventory (Wave 77 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 77): UDX kernel notify multi-process SPSC product remains
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 77 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 77 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 77 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 77 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 77 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retspireangle — Wave 77 return-gateangle honesty
     * Grep: udx: soft retholdangle — Wave 77 exclusive wallangle stamp
     * CREATE-ONLY soft names; continuum toward 21900; Soft≠product.
     */
    soft_emit("udx: soft retspireangle soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=21900 wave=%u "
              "(retspireangle honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retholdangle exclusive=1 soft_ne_product=1 "
              "continuum_toward=21900 wave=%u "
              "(retholdangle stamp; Soft!=product)\n",
              UDX_SOFT_WAVE);
}
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 77 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 77 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
