 ...
 *
 * Honesty (Wave 26): UDX kernel notify multi-process SPSC product remains
 ...
 *   udx: soft deepen     — wave=26 stamp + area count
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 26 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 26 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 26 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 26 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 26 exclusive deepen).
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 26 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 26 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
