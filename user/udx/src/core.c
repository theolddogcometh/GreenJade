 ...
 *
 * Soft inventory (Wave 101 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 101): UDX kernel notify multi-process SPSC product remains
...
/*
 * Soft product inventory (Wave 101 exclusive deepen). Cumulative for this
...
/**
 * Greppable soft inventory (Wave 101 exclusive deepen).
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 101 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 101 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 101 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 101 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 101 exclusive deepen).
    ...
    /*
     * Grep: udx: soft rettorusangle — Wave 101 return-torusangle honesty
     * Grep: udx: soft retknotangle — Wave 101 exclusive knotangle stamp
     * CREATE-ONLY soft names; continuum toward 24300; Soft≠product.
     */
    soft_emit("udx: soft rettorusangle soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=24300 wave=%u "
              "(rettorusangle honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retknotangle exclusive=1 soft_ne_product=1 "
              "continuum_toward=24300 wave=%u "
              "(retknotangle stamp; Soft!=product)\n",
              UDX_SOFT_WAVE);
}
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 101 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 101 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
