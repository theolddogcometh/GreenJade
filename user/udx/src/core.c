 ...
 *
 * Soft inventory (Wave 78 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 78): UDX kernel notify multi-process SPSC product remains
...
/*
 * Soft product inventory (Wave 78 exclusive deepen). Cumulative for this
...
/**
 * Greppable soft inventory (Wave 78 exclusive deepen).
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 78 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 78 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 78 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 78 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 78 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retpalaceangle — Wave 78 return-palaceangle honesty
     * Grep: udx: soft retfortressangle — Wave 78 exclusive fortressangle stamp
     * CREATE-ONLY soft names; continuum toward 22000; Soft≠product.
     */
    soft_emit("udx: soft retpalaceangle soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=22000 wave=%u "
              "(retpalaceangle honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retfortressangle exclusive=1 soft_ne_product=1 "
              "continuum_toward=22000 wave=%u "
              "(retfortressangle stamp; Soft!=product)\n",
              UDX_SOFT_WAVE);
}
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 78 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 78 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
