 ...
 *
 * Soft inventory (Wave 71 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 71): UDX kernel notify multi-process SPSC product remains
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 71 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 71 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 71 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 71 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 71 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retsallyportangle — Wave 71 return-sallyportangle honesty
     * Grep: udx: soft retreentrantangle — Wave 71 exclusive reentrantangle stamp
     * CREATE-ONLY soft names; continuum toward 21300; Soft≠product.
     */
    soft_emit("udx: soft retsallyportangle soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=21300 wave=%u "
              "(retsallyportangle honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retreentrantangle exclusive=1 soft_ne_product=1 "
              "continuum_toward=21300 wave=%u "
              "(retreentrantangle stamp; Soft!=product)\n",
              UDX_SOFT_WAVE);
}
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 71 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 71 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
