 ...
 *
 * Soft inventory (Wave 79 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 79): UDX kernel notify multi-process SPSC product remains
...
/*
 * Soft product inventory (Wave 79 exclusive deepen). Cumulative for this
...
/**
 * Greppable soft inventory (Wave 79 exclusive deepen).
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 79 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 79 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 79 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 79 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 79 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retkeepangle — Wave 79 return-keepangle honesty
     * Grep: udx: soft retredoubtangle — Wave 79 exclusive redoubtangle stamp
     * CREATE-ONLY soft names; continuum toward 22100; Soft≠product.
     */
    soft_emit("udx: soft retkeepangle soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=22100 wave=%u "
              "(retkeepangle honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retredoubtangle exclusive=1 soft_ne_product=1 "
              "continuum_toward=22100 wave=%u "
              "(retredoubtangle stamp; Soft!=product)\n",
              UDX_SOFT_WAVE);
}
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 79 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 79 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
