 ...
 *
 * Soft inventory (Wave 43 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 43): UDX kernel notify multi-process SPSC product remains
 ...
 *   udx: soft deepen     — wave=43 stamp + area count
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 43 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 43 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 43 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 43 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 43 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retmachicolation — Wave 43 return-curtain honesty
     * Grep: udx: soft retarrowslit — Wave 43 exclusive parapet stamp
     * CREATE-ONLY soft names; continuum toward 18500; Soft≠product.
     */
    soft_emit("udx: soft retmachicolation soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=18500 wave=%u "
              "(retmachicolation honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retarrowslit exclusive=1 soft_ne_product=1 "
              "continuum_toward=18500 wave=%u "
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 43 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 43 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
