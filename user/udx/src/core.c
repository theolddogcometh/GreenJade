 ...
 *
 * Soft inventory (Wave 46 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 46): UDX kernel notify multi-process SPSC product remains
 ...
 *   udx: soft deepen     — wave=46 stamp + area count
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 46 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 46 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 46 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 46 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 46 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retbailey — Wave 46 return-bailey honesty
     * Grep: udx: soft retpostern — Wave 46 exclusive postern stamp
     * CREATE-ONLY soft names; continuum toward 18800; Soft≠product.
     */
    soft_emit("udx: soft retbailey soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=18800 wave=%u "
              "(retbailey honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retpostern exclusive=1 soft_ne_product=1 "
              "continuum_toward=18800 wave=%u "
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 46 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 46 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
