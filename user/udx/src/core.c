 ...
 *
 * Soft inventory (Wave 116 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 116): UDX kernel notify multi-process SPSC product remains
...
/*
 * Soft product inventory (Wave 116 exclusive deepen). Cumulative for this
...
/**
 * Greppable soft inventory (Wave 116 exclusive deepen).
...
soft_inventory_log(void)
{
    ...
    /*
     * Grep: udx: soft honesty (Wave 116 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retfenceangle — Wave 116 return-fenceangle honesty
     * Grep: udx: soft retsemaphoreangle — Wave 116 exclusive semaphoreangle stamp
     * CREATE-ONLY soft names; continuum toward 25800; Soft≠product.
     */
    soft_emit("udx: soft retfenceangle soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=25800 wave=%u "
              "(retfenceangle honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retsemaphoreangle exclusive=1 soft_ne_product=1 "
              "continuum_toward=25800 wave=%u "
              "(retsemaphoreangle stamp; Soft!=product)\n",
              UDX_SOFT_WAVE);
}
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 116 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 116 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
