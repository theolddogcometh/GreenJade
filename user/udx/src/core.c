 ...
 *
 * Soft inventory (Wave 86 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 86): UDX kernel notify multi-process SPSC product remains
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 86 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 86 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 86 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 86 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 86 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retforgeangle — Wave 86 return-forgeangle honesty
     * Grep: udx: soft retprismangle — Wave 86 exclusive prismangle stamp
     * CREATE-ONLY soft names; continuum toward 22800; Soft≠product.
     */
    soft_emit("udx: soft retforgeangle soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=22800 wave=%u "
              "(retforgeangle honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retprismangle exclusive=1 soft_ne_product=1 "
              "continuum_toward=22800 wave=%u "
              "(retprismangle stamp; Soft!=product)\n",
              UDX_SOFT_WAVE);
}
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 86 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 86 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
