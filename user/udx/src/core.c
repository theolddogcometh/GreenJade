 ...
 *
 * Honesty (Wave 52): UDX kernel notify multi-process SPSC product remains
 ...
 *   udx: soft deepen     — wave=52 stamp + area count
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 52 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 52 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 52 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 52 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 52 exclusive deepen).
    ...
    /*
     * Grep: udx: soft rettenaille — Wave 52 return-glacisgate honesty
     * Grep: udx: soft retdemilune — Wave 52 exclusive outwork stamp
     * CREATE-ONLY soft names; continuum toward 19400; Soft≠product.
     */
    soft_emit("udx: soft rettenaille soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=19400 wave=%u "
              "(rettenaille honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retdemilune exclusive=1 soft_ne_product=1 "
              "continuum_toward=19400 wave=%u "
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 52 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 52 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
