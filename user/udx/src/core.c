 ...
 *
 * Soft inventory (Wave 74 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 74): UDX kernel notify multi-process SPSC product remains
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 74 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 74 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 74 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 74 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 74 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retglacisangle — Wave 74 return-glacisangle honesty
     * Grep: udx: soft retparapetangle — Wave 74 exclusive parapetangle stamp
     * CREATE-ONLY soft names; continuum toward 21600; Soft≠product.
     */
    soft_emit("udx: soft retglacisangle soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=21600 wave=%u "
              "(retglacisangle honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retparapetangle exclusive=1 soft_ne_product=1 "
              "continuum_toward=21600 wave=%u "
              "(retparapetangle stamp; Soft!=product)\n",
              UDX_SOFT_WAVE);
}
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 74 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 74 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
