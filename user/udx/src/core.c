 ...
 *
 * Soft inventory (Wave 118 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 118): UDX kernel notify multi-process SPSC product remains
...
/*
 * Soft product inventory (Wave 118 exclusive deepen). Cumulative for this
...
/**
 * Greppable soft inventory (Wave 118 exclusive deepen).
...
soft_inventory_log(void)
{
    ...
    /*
     * Grep: udx: soft honesty (Wave 118 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retbarrierangle — Wave 118 return-barrierangle honesty
     * Grep: udx: soft retatomicangle — Wave 118 exclusive atomicangle stamp
     * CREATE-ONLY soft names; continuum toward 26000; Soft≠product.
     */
    soft_emit("udx: soft retbarrierangle soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=26000 wave=%u "
              "(retbarrierangle honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retatomicangle exclusive=1 soft_ne_product=1 "
              "continuum_toward=26000 wave=%u "
              "(retatomicangle stamp; Soft!=product)\n",
              UDX_SOFT_WAVE);
}
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 118 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 118 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
