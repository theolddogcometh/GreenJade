 ...
 *
 * Soft inventory (Wave 98 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 98): UDX kernel notify multi-process SPSC product remains
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 98 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 98 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 98 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 98 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 98 exclusive deepen).
    ...
    soft_emit("udx: soft retcircumangle soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=24000 wave=%u "
              "(retcircumangle honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retellipseangle exclusive=1 soft_ne_product=1 "
              "continuum_toward=24000 wave=%u "
              "(retellipseangle stamp; Soft!=product)\n",
              UDX_SOFT_WAVE);
}
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 98 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 98 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
