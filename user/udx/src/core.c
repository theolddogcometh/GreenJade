 ...
 *
 * Honesty (Wave 111): UDX kernel notify multi-process SPSC product remains
...
soft_inventory_log(void)
{
    ...
    /*
     * Grep: udx: soft honesty (Wave 111 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retvoxelangle — Wave 111 return-voxelangle honesty
     * Grep: udx: soft rettexelangle — Wave 111 exclusive texelangle stamp
     * CREATE-ONLY soft names; continuum toward 25300; Soft≠product.
     */
    soft_emit("udx: soft retvoxelangle soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=25300 wave=%u "
              "(retvoxelangle honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft rettexelangle exclusive=1 soft_ne_product=1 "
              "continuum_toward=25300 wave=%u "
              "(rettexelangle stamp; Soft!=product)\n",
              UDX_SOFT_WAVE);
}
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 111 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 111 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
