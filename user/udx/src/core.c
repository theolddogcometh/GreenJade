 ...
 *
 * Honesty (Wave 39): UDX kernel notify multi-process SPSC product remains
 ...
 *   udx: soft deepen     — wave=39 stamp + area count
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 39 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 39 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 39 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 39 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 39 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retbarbican — Wave 39 return-barbican honesty
     * Grep: udx: soft retglacis — Wave 39 exclusive glacis stamp
     * CREATE-ONLY soft names; continuum toward 18100; Soft≠product.
     */
    soft_emit("udx: soft retbarbican soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=18100 wave=%u "
              "(retbarbican honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retglacis exclusive=1 soft_ne_product=1 "
              "continuum_toward=18100 wave=%u "
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 39 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 39 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
