 ...
 *
 * Soft inventory (Wave 96 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 96): UDX kernel notify multi-process SPSC product remains
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 96 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 96 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 96 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 96 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 96 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retsectorangle — Wave 96 return-chordangle honesty
     * Grep: udx: soft retwedgeangle — Wave 96 exclusive arcangle stamp
     * CREATE-ONLY soft names; continuum toward 23800; Soft≠product.
     */
    soft_emit("udx: soft retsectorangle soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=23800 wave=%u "
              "(retsectorangle honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retwedgeangle exclusive=1 soft_ne_product=1 "
              "continuum_toward=23800 wave=%u "
              "(retwedgeangle stamp; Soft!=product)\n",
              UDX_SOFT_WAVE);
}
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 96 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 96 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
