 ...
 *
 * Soft inventory (Wave 47 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 47): UDX kernel notify multi-process SPSC product remains
 ...
 *   udx: soft deepen     — wave=47 stamp + area count
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 47 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 47 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 47 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 47 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 47 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retinnerward — Wave 47 return-innerward honesty
     * Grep: udx: soft retdonjon — Wave 47 exclusive donjon stamp
     * CREATE-ONLY soft names; continuum toward 18900; Soft≠product.
     */
    soft_emit("udx: soft retinnerward soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=18900 wave=%u "
              "(retinnerward honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retdonjon exclusive=1 soft_ne_product=1 "
              "continuum_toward=18900 wave=%u "
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 47 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 47 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
