 ...
 *
 * Soft inventory (Wave 42 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 42): UDX kernel notify multi-process SPSC product remains
...
/*
 * Soft product inventory (Wave 42 exclusive deepen). Cumulative for this
...
/**
 * Greppable soft inventory (Wave 42 exclusive deepen).
 ...
 *   udx: soft deepen     — wave=42 stamp + area count
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 42 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 42 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 42 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 42 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 42 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retportcullis — Wave 42 return-curtain honesty
     * Grep: udx: soft retbattlement — Wave 42 exclusive parapet stamp
     * CREATE-ONLY soft names; continuum toward 18400; Soft≠product.
     */
    soft_emit("udx: soft retportcullis soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=18400 wave=%u "
              "(retportcullis honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retbattlement exclusive=1 soft_ne_product=1 "
              "continuum_toward=18400 wave=%u "
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 42 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 42 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
