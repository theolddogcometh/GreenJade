 ...
 *
 * Soft inventory (Wave 49 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 49): UDX kernel notify multi-process SPSC product remains
 ...
 *   udx: soft deepen     — wave=49 stamp + area count
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 49 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 49 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 49 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 49 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 49 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retglacisgate — Wave 49 return-glacisgate honesty
     * Grep: udx: soft retoutwork — Wave 49 exclusive outwork stamp
     * CREATE-ONLY soft names; continuum toward 19100; Soft≠product.
     */
    soft_emit("udx: soft retglacisgate soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=19100 wave=%u "
              "(retglacisgate honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retoutwork exclusive=1 soft_ne_product=1 "
              "continuum_toward=19100 wave=%u "
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 49 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 49 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
