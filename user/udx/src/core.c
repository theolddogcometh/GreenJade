 ...
 *
 * Soft inventory (Wave 57 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 57): UDX kernel notify multi-process SPSC product remains
 ...
 *   udx: soft deepen     — wave=57 stamp + area count
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 57 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 57 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 57 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 57 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 57 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retraverse — Wave 57 return-traverse honesty
     * Grep: udx: soft retcasemate — Wave 57 exclusive casemate stamp
     * CREATE-ONLY soft names; continuum toward 19900; Soft≠product.
     */
    soft_emit("udx: soft retraverse soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=19900 wave=%u "
              "(retraverse honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retcasemate exclusive=1 soft_ne_product=1 "
              "continuum_toward=19900 wave=%u "
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 57 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 57 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
