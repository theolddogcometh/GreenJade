 ...
 *
 * Soft inventory (Wave 97 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 97): UDX kernel notify multi-process SPSC product remains
...
/*
 * Soft product inventory (Wave 97 exclusive deepen). Cumulative for this
...
/**
 * Greppable soft inventory (Wave 97 exclusive deepen).
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 97 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 97 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 97 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 97 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 97 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retradiusangle — Wave 97 return-radiusangle honesty
     * Grep: udx: soft retdiameterangle — Wave 97 exclusive arcangle stamp
     * CREATE-ONLY soft names; continuum toward 23900; Soft≠product.
     */
    soft_emit("udx: soft retradiusangle soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=23900 wave=%u "
              "(retradiusangle honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retdiameterangle exclusive=1 soft_ne_product=1 "
              "continuum_toward=23900 wave=%u "
              "(retdiameterangle stamp; Soft!=product)\n",
              UDX_SOFT_WAVE);
}
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 97 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 97 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
