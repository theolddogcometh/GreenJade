 ...
 *
 * Soft inventory (Wave 107 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 107): UDX kernel notify multi-process SPSC product remains
...
/*
 * Soft product inventory (Wave 107 exclusive deepen). Cumulative for this
...
/**
 * Greppable soft inventory (Wave 107 exclusive deepen).
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 107 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 107 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 107 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 107 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 107 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retsplineangle — Wave 107 return-splineangle honesty
     * Grep: udx: soft retbezierangle — Wave 107 exclusive bezierangle stamp
     * CREATE-ONLY soft names; continuum toward 24900; Soft≠product.
     */
    soft_emit("udx: soft retsplineangle soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=24900 wave=%u "
              "(retsplineangle honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retbezierangle exclusive=1 soft_ne_product=1 "
              "continuum_toward=24900 wave=%u "
              "(retbezierangle stamp; Soft!=product)\n",
              UDX_SOFT_WAVE);
}
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 107 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 107 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
