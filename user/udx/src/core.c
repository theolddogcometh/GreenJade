 ...
 *
 * Soft inventory (Wave 48 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 48): UDX kernel notify multi-process SPSC product remains
 ...
 *   udx: soft deepen     — wave=48 stamp + area count
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 48 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 48 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 48 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 48 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 48 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retchevaux — Wave 48 return-chevaux honesty
     * Grep: udx: soft retpalisade — Wave 48 exclusive palisade stamp
     * CREATE-ONLY soft names; continuum toward 19000; Soft≠product.
     */
    soft_emit("udx: soft retchevaux soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=19000 wave=%u "
              "(retchevaux honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retpalisade exclusive=1 soft_ne_product=1 "
              "continuum_toward=19000 wave=%u "
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 48 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 48 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
