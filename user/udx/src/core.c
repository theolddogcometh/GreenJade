 ...
 *
 * Honesty (Wave 70): UDX kernel notify multi-process SPSC product remains
...
/*
 * Soft product inventory (Wave 70 exclusive deepen). Cumulative for this
...
/**
 * Greppable soft inventory (Wave 70 exclusive deepen).
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 70 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 70 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 70 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 70 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 70 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retfosseangle — Wave 70 return-fosseangle honesty
     * Grep: udx: soft retcounterscarple — Wave 70 exclusive counterscarple stamp
     * CREATE-ONLY soft names; continuum toward 21200; Soft≠product.
     */
    soft_emit("udx: soft retfosseangle soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=21200 wave=%u "
              "(retfosseangle honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retcounterscarple exclusive=1 soft_ne_product=1 "
              "continuum_toward=21200 wave=%u "
              "(retcounterscarple stamp; Soft!=product)\n",
              UDX_SOFT_WAVE);
}
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 70 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 70 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
