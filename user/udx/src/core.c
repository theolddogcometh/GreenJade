 ...
 *
 * Soft inventory (Wave 51 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 51): UDX kernel notify multi-process SPSC product remains
 ...
 *   udx: soft deepen     — wave=51 stamp + area count
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 51 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 51 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 51 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 51 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 51 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retfosse — Wave 51 return-glacisgate honesty
     * Grep: udx: soft retcoveredway — Wave 51 exclusive outwork stamp
     * CREATE-ONLY soft names; continuum toward 19300; Soft≠product.
     */
    soft_emit("udx: soft retfosse soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=19300 wave=%u "
              "(retfosse honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retcoveredway exclusive=1 soft_ne_product=1 "
              "continuum_toward=19300 wave=%u "
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 51 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 51 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
