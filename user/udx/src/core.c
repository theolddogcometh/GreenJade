 ...
 *
 * Soft inventory (Wave 61 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 61): UDX kernel notify multi-process SPSC product remains
 ...
 *   udx: soft deepen     — wave=61 stamp + area count
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 61 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 61 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 61 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 61 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 61 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retcounterguard — Wave 61 return-counterguard honesty
     * Grep: udx: soft retcoveredface — Wave 61 exclusive covered-face stamp
     * CREATE-ONLY soft names; continuum toward 20300; Soft≠product.
     */
    soft_emit("udx: soft retcounterguard soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=20300 wave=%u "
              "(retcounterguard honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retcoveredface exclusive=1 soft_ne_product=1 "
              "continuum_toward=20300 wave=%u "
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 61 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 61 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
