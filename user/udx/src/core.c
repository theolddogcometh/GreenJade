 ...
 *
 * Soft inventory (Wave 56 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 56): UDX kernel notify multi-process SPSC product remains
 ...
 *   udx: soft deepen     — wave=56 stamp + area count
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 56 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 56 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 56 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 56 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 56 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retgorge — Wave 56 return-gorge honesty
     * Grep: udx: soft retshoulder — Wave 56 exclusive shoulder stamp
     * CREATE-ONLY soft names; continuum toward 19800; Soft≠product.
     */
    soft_emit("udx: soft retgorge soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=19800 wave=%u "
              "(retgorge honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retshoulder exclusive=1 soft_ne_product=1 "
              "continuum_toward=19800 wave=%u "
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 56 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 56 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
