 ...
 *
 * Soft inventory (Wave 58 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 58): UDX kernel notify multi-process SPSC product remains
 ...
 *   udx: soft deepen     — wave=58 stamp + area count
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 58 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 58 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 58 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 58 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 58 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retorillon — Wave 58 return-orillon honesty
     * Grep: udx: soft retbonnette — Wave 58 exclusive bonnette stamp
     * CREATE-ONLY soft names; continuum toward 20000; Soft≠product.
     */
    soft_emit("udx: soft retorillon soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=20000 wave=%u "
              "(retorillon honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retbonnette exclusive=1 soft_ne_product=1 "
              "continuum_toward=20000 wave=%u "
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 58 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 58 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
