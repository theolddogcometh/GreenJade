 ...
 *
 * Soft inventory (Wave 123 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 123): UDX kernel notify multi-process SPSC product remains
...
soft_inventory_log(void)
{
    ...
    /*
     * Grep: udx: soft honesty (Wave 123 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retlayerangle — Wave 123 return-layerangle honesty
     * Grep: udx: soft retcanvasangle — Wave 123 exclusive canvasangle stamp
     * CREATE-ONLY soft names; continuum toward 26500; Soft≠product.
     */
    soft_emit("udx: soft retlayerangle soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=26500 wave=%u "
              "(retlayerangle honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retcanvasangle exclusive=1 soft_ne_product=1 "
              "continuum_toward=26500 wave=%u "
              "(retcanvasangle stamp; Soft!=product)\n",
              UDX_SOFT_WAVE);
}
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 123 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 123 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
