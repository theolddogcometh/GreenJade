 ...
 *
 * Soft inventory (Wave 72 exclusive deepen; this unit only) —
 * greppable "udx: soft …":
 *   udx: soft inventory …
 *   udx: soft lifecycle …
 *   udx: soft run …
 ...
 *
 * Honesty (Wave 72): UDX kernel notify multi-process SPSC product remains
...
soft_inventory_log(void)
{
    ...
    /* Grep: udx: soft should (Wave 72 deepen) */
    ...
    /* Grep: udx: soft idle (Wave 72 deepen) */
    ...
    /*
     * Gap catalog — what core cannot close alone (Wave 72 honesty).
    ...
    /* Grep: udx: soft deepen wave (Wave 72 stamp; areas = prior soft lines). */
    ...
    /*
     * Grep: udx: soft honesty (Wave 72 exclusive deepen).
    ...
    /*
     * Grep: udx: soft retplaceofarmsangle — Wave 72 return-placeofarmsangle honesty
     * Grep: udx: soft retdoubletenailleangle — Wave 72 exclusive doubletenailleangle stamp
     * CREATE-ONLY soft names; continuum toward 21400; Soft≠product.
     */
    soft_emit("udx: soft retplaceofarmsangle soft_only=1 product_gate=0 "
              "soft_ne_product=1 continuum_toward=21400 wave=%u "
              "(retplaceofarmsangle honesty; Soft!=product; not bar3)\n",
              UDX_SOFT_WAVE);
    soft_emit("udx: soft retdoubletenailleangle exclusive=1 soft_ne_product=1 "
              "continuum_toward=21400 wave=%u "
              "(retdoubletenailleangle stamp; Soft!=product)\n",
              UDX_SOFT_WAVE);
}
...
udx_init(void)
{
    ...
    udx_printk("udx: init %s %s v%s (Linux-porter surface)\n",
               UDX_PRODUCT_NAME, UDX_PRODUCT_TAG, UDX_PRODUCT_VERSION);
    /* Wave 72 soft inventory baseline (greppable udx: soft …). */
    soft_inventory_log();
    return UDX_OK;
}
...
udx_run(void)
{
    ...
    udx_work_flush();
    /* Wave 72 soft run snapshot (greppable udx: soft …). */
    soft_inventory_log();
}
