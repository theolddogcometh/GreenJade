 ...
 *
 * Soft SPSC ownership protocol (Wave exclusive deepen; this unit only):
 ...
 *
 * Honesty: soft observation only — not multi-process driver-host product.
 */
...
/**
 * Greppable soft SPSC ownership + ring handoff inventory.
...
spsc_soft_inventory_log(void)
{
    ...
    spsc_soft_emit(
        ...
        "product=0 soft=1 sec_core_s6=shape\n");
    ...
    spsc_soft_emit(
        ...
        "peak_pend=%u live_pend=%u log_n=%u\n",
    ...
    spsc_soft_emit(
        ...
        "indexes=pending_bit+fifo kernel_interprets=0\n");
