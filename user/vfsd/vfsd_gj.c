 ...
 *
 * Soft inventory (Wave 45 exclusive deepen — greppable "vfsd-gj: soft …"):
 ...
 *   vfsd-gj: soft deepen wave=45 areas=… ok=… skip=…
 *   vfsd-gj: soft path multi_server=0 confine=0 (soft; not bar3)
 *   vfsd-gj: soft honesty multi_server=0 confine=0 bar3=0 exclusive=1 soft=1 product_kernel=OPEN wave=45
...
/* Soft store path bits (Wave 45 inventory; LBA0 mirror + read). */
...
/* Soft free path bits (Wave 45 inventory; post-RELEASE no-op). */
...
/*
 * Wave 45 soft inventory tallies (file-local; wrap OK).
...
/* Note one soft door sub-step into Wave 45 inventory counters. */
static void
...
/* Note one soft store sub-step into Wave 45 inventory counters. */
static void
...
soft_inventory_log(void)
{
    ...
    /* Grep: vfsd-gj: soft deepen wave (Wave 45 stamp) */
    o = 0u;
    ...
    /*
     * Grep: vfsd-gj: soft path (Wave 45 honesty).
    ...
    /*
     * Grep: vfsd-gj: soft honesty (Wave 45 exclusive deepen).
     * Soft inventory ≠ product multi-server confine.
     */
    ...
    /*
     * Grep: vfsd-gj: soft exclusive (Wave 45 exclusive deepen).
     * Soft inventory ≠ product multi-server / continuum.
     */
    msg("vfsd-gj: soft exclusive product_kernel=OPEN wave=45 multi_server=0 confine=0 "
 ...
 * Tallies Wave 45 soft inventory (vfsd-gj: soft …).
...
soft_door_path(volatile unsigned *pCache)
{
     ...
     *    Wave 45: greppable soft SEEK_END + soft SEEKFD dual markers.
     */
    if ((uBits & VFSD_SOFT_BIT_WRITEFD) != 0u) {
        hFd = gj_vfs(GJ_VFS_OP_OPEN, (long)(uintptr_t)szSoft,
    ...
    /*
     * Door-local soft stats rollup (Wave 45 deepened fields).
 ...
 * Tallies Wave 45 soft store inventory (vfsd-gj: soft store …).
 */
static void
...
_start(void)
{
     ...
     * Tallies Wave 45 soft free inventory.
     */
    ...
    /*
     * Wave 45 soft inventory dump — greppable "vfsd-gj: soft …".
