 ...
 *
 * Soft inventory (Wave 68 exclusive deepen — greppable "vfsd-gj: soft …"):
 ...
 *   vfsd-gj: soft deepen wave=68 areas=… ok=… skip=…
 *   vfsd-gj: soft path multi_server=0 confine=0 (soft; not bar3)
 *   vfsd-gj: soft honesty multi_server=0 confine=0 bar3=0 exclusive=1 soft=1 product_kernel=OPEN wave=68
...
/* Soft store path bits (Wave 68 inventory; LBA0 mirror + read). */
...
/* Note one soft door sub-step into Wave 68 inventory counters. */
static void
...
/* Note one soft store sub-step into Wave 68 inventory counters. */
static void
...
soft_inventory_log(void)
{
    ...
    /* Grep: vfsd-gj: soft deepen wave (Wave 68 stamp) */
    o = 0u;
    ...
    /*
     * Grep: vfsd-gj: soft path (Wave 68 honesty).
    ...
    /*
     * Grep: vfsd-gj: soft honesty (Wave 68 exclusive deepen).
     * Soft inventory ≠ product multi-server confine.
     */
    ...
    /*
     * Grep: vfsd-gj: soft exclusive (Wave 68 exclusive deepen).
     * Soft inventory ≠ product multi-server / continuum.
     */
    msg("vfsd-gj: soft exclusive product_kernel=OPEN wave=68 multi_server=0 confine=0 "
 ...
 * Tallies Wave 68 soft inventory (vfsd-gj: soft …).
...
soft_door_path(volatile unsigned *pCache)
{
     ...
     *    Wave 68: greppable soft SEEK_END + soft SEEKFD dual markers.
     */
    if ((uBits & VFSD_SOFT_BIT_WRITEFD) != 0u) {
        hFd = gj_vfs(GJ_VFS_OP_OPEN, (long)(uintptr_t)szSoft,
    ...
    /*
     * Door-local soft stats rollup (Wave 68 deepened fields).
 ...
 * Tallies Wave 68 soft store inventory (vfsd-gj: soft store …).
 */
static void
...
_start(void)
{
     ...
     * Tallies Wave 68 soft free inventory.
     */
    ...
    /*
     * Wave 68 soft inventory dump — greppable "vfsd-gj: soft …".
