 ...
 *
 * Soft inventory (Wave 101 exclusive deepen — greppable "vfsd-gj: soft …"):
...
/* Soft store path bits (Wave 101 inventory; LBA0 mirror + read). */
...
/* Soft free path bits (Wave 101 inventory; post-RELEASE no-op). */
...
/*
 * Wave 101 soft inventory tallies (file-local; wrap OK).
...
/* Note one soft door sub-step into Wave 101 inventory counters. */
static void
...
/* Note one soft store sub-step into Wave 101 inventory counters. */
static void
...
soft_inventory_log(void)
{
    ...
    /* Grep: vfsd-gj: soft deepen wave (Wave 101 stamp) */
    o = 0u;
    ...
    /*
     * Grep: vfsd-gj: soft path (Wave 101 honesty).
    ...
    /*
     * Grep: vfsd-gj: soft honesty (Wave 101 exclusive deepen).
     * Soft inventory ≠ product multi-server confine.
     */
    ...
    /*
     * Grep: vfsd-gj: soft exclusive (Wave 101 exclusive deepen).
 ...
 * Tallies Wave 101 soft inventory (vfsd-gj: soft …).
...
soft_door_path(volatile unsigned *pCache)
{
     ...
     *    Wave 101: greppable soft SEEK_END + soft SEEKFD dual markers.
     */
    if ((uBits & VFSD_SOFT_BIT_WRITEFD) != 0u) {
        hFd = gj_vfs(GJ_VFS_OP_OPEN, (long)(uintptr_t)szSoft,
    ...
    /*
     * Door-local soft stats rollup (Wave 101 deepened fields).
 ...
 * Tallies Wave 101 soft store inventory (vfsd-gj: soft store …).
 */
static void
...
_start(void)
{
     ...
     * Tallies Wave 101 soft free inventory.
     */
    ...
    /*
     * Wave 101 soft inventory dump — greppable "vfsd-gj: soft …".
