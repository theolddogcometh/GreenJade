 ...
 *
 * Soft inventory (Wave 28 exclusive deepen — greppable "vfsd-gj: soft …"):
 ...
 *   vfsd-gj: soft deepen wave=28 areas=… ok=… skip=…
 *   vfsd-gj: soft path multi_server=0 confine=0 (soft; not bar3)
 *   vfsd-gj: soft honesty multi_server=0 confine=0 bar3=0 exclusive=1 soft=1 product_kernel=OPEN wave=28
...
/* Soft store path bits (Wave 28 inventory; LBA0 mirror + read). */
#define VFSD_SOFT_STORE_MIRROR 1u
#define VFSD_SOFT_STORE_READ   2u

/* Soft free path bits (Wave 28 inventory; post-RELEASE no-op). */
...
/*
 * Wave 28 soft inventory tallies (file-local; wrap OK).
...
/* Note one soft door sub-step into Wave 28 inventory counters. */
static void
...
/* Note one soft store sub-step into Wave 28 inventory counters. */
static void
...
soft_inventory_log(void)
{
    ...
    /* Grep: vfsd-gj: soft deepen wave (Wave 28 stamp) */
    o = 0u;
    ...
    /*
     * Grep: vfsd-gj: soft path (Wave 28 honesty).
    ...
    /*
     * Grep: vfsd-gj: soft honesty (Wave 28 exclusive deepen).
     * Soft inventory ≠ product multi-server confine.
     */
    ...
    /*
     * Grep: vfsd-gj: soft exclusive (Wave 28 exclusive deepen).
     * Soft inventory ≠ product multi-server / continuum.
     */
    msg("vfsd-gj: soft exclusive product_kernel=OPEN wave=28 multi_server=0 confine=0 "
 ...
 * Tallies Wave 28 soft inventory (vfsd-gj: soft …).
...
soft_door_path(volatile unsigned *pCache)
{
     ...
     *    Wave 28: greppable soft SEEK_END + soft SEEKFD dual markers.
     */
    if ((uBits & VFSD_SOFT_BIT_WRITEFD) != 0u) {
        hFd = gj_vfs(GJ_VFS_OP_OPEN, (long)(uintptr_t)szSoft,
    ...
    /*
     * Door-local soft stats rollup (Wave 28 deepened fields).
 ...
 * Tallies Wave 28 soft store inventory (vfsd-gj: soft store …).
 */
static void
...
_start(void)
{
     ...
     * Tallies Wave 28 soft free inventory.
     */
    ...
    /*
     * Wave 28 soft inventory dump — greppable "vfsd-gj: soft …".
