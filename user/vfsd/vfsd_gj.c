 ...
 *
 * Soft inventory (Wave 61 exclusive deepen — greppable "vfsd-gj: soft …"):
 ...
 *   vfsd-gj: soft deepen wave=61 areas=… ok=… skip=…
 *   vfsd-gj: soft path multi_server=0 confine=0 (soft; not bar3)
 *   vfsd-gj: soft honesty multi_server=0 confine=0 bar3=0 exclusive=1 soft=1 product_kernel=OPEN wave=61
...
/* Soft store path bits (Wave 61 inventory; LBA0 mirror + read). */
...
/* Note one soft door sub-step into Wave 61 inventory counters. */
static void
...
/* Note one soft store sub-step into Wave 61 inventory counters. */
static void
...
soft_inventory_log(void)
{
    ...
    /* Grep: vfsd-gj: soft deepen wave (Wave 61 stamp) */
    o = 0u;
    ...
    /*
     * Grep: vfsd-gj: soft path (Wave 61 honesty).
    ...
    /*
     * Grep: vfsd-gj: soft honesty (Wave 61 exclusive deepen).
     * Soft inventory ≠ product multi-server confine.
     */
    ...
    /*
     * Grep: vfsd-gj: soft exclusive (Wave 61 exclusive deepen).
     * Soft inventory ≠ product multi-server / continuum.
     */
    msg("vfsd-gj: soft exclusive product_kernel=OPEN wave=61 multi_server=0 confine=0 "
 ...
 * Tallies Wave 61 soft inventory (vfsd-gj: soft …).
...
soft_door_path(volatile unsigned *pCache)
{
     ...
     *    Wave 61: greppable soft SEEK_END + soft SEEKFD dual markers.
     */
    if ((uBits & VFSD_SOFT_BIT_WRITEFD) != 0u) {
        hFd = gj_vfs(GJ_VFS_OP_OPEN, (long)(uintptr_t)szSoft,
    ...
    /*
     * Door-local soft stats rollup (Wave 61 deepened fields).
 ...
 * Tallies Wave 61 soft store inventory (vfsd-gj: soft store …).
 */
static void
...
_start(void)
{
     ...
     * Tallies Wave 61 soft free inventory.
     */
    ...
    /*
     * Wave 61 soft inventory dump — greppable "vfsd-gj: soft …".
