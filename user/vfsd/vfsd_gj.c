 ...
 *
 * Soft inventory (Wave 40 exclusive deepen — greppable "vfsd-gj: soft …"):
 ...
 *   vfsd-gj: soft deepen wave=40 areas=… ok=… skip=…
 *   vfsd-gj: soft path multi_server=0 confine=0 (soft; not bar3)
 *   vfsd-gj: soft honesty multi_server=0 confine=0 bar3=0 exclusive=1 soft=1 product_kernel=OPEN wave=40
...
/* Soft store path bits (Wave 40 inventory; LBA0 mirror + read). */
...
/* Soft free path bits (Wave 40 inventory; post-RELEASE no-op). */
...
/*
 * Wave 40 soft inventory tallies (file-local; wrap OK).
...
/* Note one soft door sub-step into Wave 40 inventory counters. */
static void
...
/* Note one soft store sub-step into Wave 40 inventory counters. */
static void
...
soft_inventory_log(void)
{
    ...
    /* Grep: vfsd-gj: soft deepen wave (Wave 40 stamp) */
    o = 0u;
    ...
    /*
     * Grep: vfsd-gj: soft path (Wave 40 honesty).
    ...
    /*
     * Grep: vfsd-gj: soft honesty (Wave 40 exclusive deepen).
     * Soft inventory ≠ product multi-server confine.
     */
    ...
    /*
     * Grep: vfsd-gj: soft exclusive (Wave 40 exclusive deepen).
     * Soft inventory ≠ product multi-server / continuum.
     */
    msg("vfsd-gj: soft exclusive product_kernel=OPEN wave=40 multi_server=0 confine=0 "
 ...
 * Tallies Wave 40 soft inventory (vfsd-gj: soft …).
...
soft_door_path(volatile unsigned *pCache)
{
     ...
     *    Wave 40: greppable soft SEEK_END + soft SEEKFD dual markers.
     */
    if ((uBits & VFSD_SOFT_BIT_WRITEFD) != 0u) {
        hFd = gj_vfs(GJ_VFS_OP_OPEN, (long)(uintptr_t)szSoft,
    ...
    /*
     * Door-local soft stats rollup (Wave 40 deepened fields).
 ...
 * Tallies Wave 40 soft store inventory (vfsd-gj: soft store …).
 */
static void
...
_start(void)
{
     ...
     * Tallies Wave 40 soft free inventory.
     */
    ...
    /*
     * Wave 40 soft inventory dump — greppable "vfsd-gj: soft …".
