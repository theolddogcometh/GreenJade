 ...
 *
 * Soft inventory (Wave 109 exclusive deepen — greppable "vfsd-gj: soft …"):
...
/* Soft store path bits (Wave 109 inventory; LBA0 mirror + read). */
...
/* Soft free path bits (Wave 109 inventory; post-RELEASE no-op). */
...
/*
 * Wave 109 soft inventory tallies (file-local; wrap OK).
...
/* Note one soft door sub-step into Wave 109 inventory counters. */
static void
...
/* Note one soft store sub-step into Wave 109 inventory counters. */
static void
...
soft_inventory_log(void)
{
    ...
    /* Grep: vfsd-gj: soft deepen wave (Wave 109 stamp) */
    o = 0u;
    ...
    /*
     * Grep: vfsd-gj: soft path (Wave 109 honesty).
    ...
    /*
     * Grep: vfsd-gj: soft honesty (Wave 109 exclusive deepen).
     * Soft inventory ≠ product multi-server confine.
     */
    ...
    /*
     * Grep: vfsd-gj: soft exclusive (Wave 109 exclusive deepen).
 ...
 * Tallies Wave 109 soft inventory (vfsd-gj: soft …).
...
soft_door_path(volatile unsigned *pCache)
{
     ...
     *    Wave 109: greppable soft SEEK_END + soft SEEKFD dual markers.
     */
    if ((uBits & VFSD_SOFT_BIT_WRITEFD) != 0u) {
        hFd = gj_vfs(GJ_VFS_OP_OPEN, (long)(uintptr_t)szSoft,
    ...
    /*
     * Door-local soft stats rollup (Wave 109 deepened fields).
 ...
 * Tallies Wave 109 soft store inventory (vfsd-gj: soft store …).
 */
static void
...
_start(void)
{
     ...
     * Tallies Wave 109 soft free inventory.
     */
    ...
    /*
     * Wave 109 soft inventory dump — greppable "vfsd-gj: soft …".
