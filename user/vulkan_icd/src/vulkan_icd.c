 ...
 *
 * Soft inventory (Wave 30 exclusive deepen; greppable; not bar3 GPU):
 *   vk_icd: soft inventory wave=30 negotiate=… present=… submit=… acquire=…
 *   vk_icd: soft deepen wave=30 areas=negotiate,instance,device,swapchain,
...
/*
 * Wave 30 soft inventory counters (file-local; wrap OK; never hard-gate).
 * Grep: vk_icd: soft
 */
#define VK_ICD_SOFT_WAVE     30u
...
/*
 * Soft inventory blob (Wave 30). Grep: vk_icd: soft inventory
 */
static const char g_szVkIcdSoftInventory[] =
    "vk_icd: soft inventory wave=30 surfaces=10 areas=14 "
...
static const char g_szVkIcdSoftDeepen[] =
    "vk_icd: soft deepen wave=30 areas=14 "
...
/*
 * Cold soft honesty line (Wave 30 exclusive). Grep: vk_icd: soft honesty
 */
const char *
...
/*
 * Soft host present CRC note (Wave 30 deepen). Grep: vk_icd: soft crc
 */
uint32_t
