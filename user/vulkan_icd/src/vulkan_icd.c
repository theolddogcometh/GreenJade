 ...
 *
 * Soft inventory (Wave 60 exclusive deepen; greppable; not bar3 GPU):
 *   vk_icd: soft inventory wave=60 negotiate=… present=… submit=… acquire=…
 *   vk_icd: soft deepen wave=60 areas=negotiate,instance,device,swapchain,
...
/*
 * Wave 60 soft inventory counters (file-local; wrap OK; never hard-gate).
 * Grep: vk_icd: soft
 */
#define VK_ICD_SOFT_WAVE 60u
...
/*
 * Soft inventory blob (Wave 60). Grep: vk_icd: soft inventory
 */
static const char g_szVkIcdSoftInventory[] =
    "vk_icd: soft inventory wave=60 surfaces=10 areas=14 "
...
static const char g_szVkIcdSoftDeepen[] =
    "vk_icd: soft deepen wave=60 areas=14 "
...
/*
 * Cold soft honesty line (Wave 60 exclusive). Grep: vk_icd: soft honesty
 */
const char *
...
/*
 * Soft host present CRC note (Wave 60 deepen). Grep: vk_icd: soft crc
 */
uint32_t
